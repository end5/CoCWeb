...
class Seal extends CombatAction {
    ...
    public useAction(char: Character, enemy: Character) {
        ...
        if (select === 0) {
            ...
            enemy.combat.effects.add(CombatEffectType.Sealed, char, { expireCountdown: 4, abilityFlags: CombatActionFlags.Attack });
        }
        else if (select === 1) {
            // Tease:
            ...
            enemy.combat.effects.add(CombatEffectType.Sealed, char, { expireCountdown: 4, abilityFlags: CombatActionFlags.Tease });
        }
        // Spells:
        else if (select === 2) {
            ...
            enemy.combat.effects.add(CombatEffectType.Sealed, char, { expireCountdown: 4, abilityFlags: CombatActionFlags.Spells });
        }
        // Items:
        else if (select === 3) {
            ...
            enemy.combat.effects.add(CombatEffectType.Sealed, char, { expireCountdown: 4, abilityFlags: CombatActionFlags.Items });
        }
        // Run:
        else if (select === 4) {
            ...
            enemy.combat.effects.add(CombatEffectType.Sealed, char, { expireCountdown: 4, abilityFlags: CombatActionFlags.MoveAway });
        }
        // P.Special:
        else if (select === 5) {
            ...
            enemy.combat.effects.add(CombatEffectType.Sealed, char, { expireCountdown: 4, abilityFlags: CombatActionFlags.PhysSpec });
        }
        // M.Special:
        else {
            ...
            enemy.combat.effects.add(CombatEffectType.Sealed, char, { expireCountdown: 4, abilityFlags: CombatActionFlags.MagicSpec });
        }
...
class KitsuneAction extends CombatAction {
    public name: string = "Action";
    public flag: CombatActionFlags = CombatActionFlags.All;
    public reasonCannotUse: string = "";
    public subActions: CombatAction[] = [new FoxFire(), new Tease(), new Seal(), new Entwine(), new Illusion()];
    public isPossible(character: Character): boolean {
        return true;
    }
    public canUse(character: Character, target: Character): boolean {
...
const Responses = new Dictionary<string, IReaction>();
Responses.set("Wait", {
...
export class Kitsune extends Character {
    ...
    public constructor(hairColor: "red" | "blonde" | "black") {
        ...
        this.combatContainer = new CombatContainer(this, new KitsuneAction(), Responses, new KitsuneEndScenes(this), {
            gems: randInt(10) + 10,
