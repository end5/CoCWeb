define(["require", "exports", "./internals/Utils", "./CockTypesEnum", "../console"], function (require, exports, Utils_1, CockTypesEnum_1, console_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class Cock {
        /**
         * @return string description of errors
         */
        validate() {
            var error = "";
            error += Utils_1.Utils.validateNonNegativeNumberFields(this, "Cock.validate", ["cockLength", "cockThickness", "knotMultiplier", "pierced"]);
            if (!this._isPierced) {
                if (this._pShortDesc.length > 0)
                    error += "Not pierced but _pShortDesc = " + this._pShortDesc + ". ";
                if (this._pLongDesc.length > 0)
                    error += "Not pierced but pLong = " + this._pLongDesc + ". ";
            }
            else {
                if (this._pShortDesc.length == 0)
                    error += "Pierced but no _pShortDesc. ";
                if (this._pLongDesc.length == 0)
                    error += "Pierced but no pLong. ";
            }
            return error;
        }
        //constructor. Default type is HUMAN
        constructor(i_cockLength = 5.5, i_cockThickness = 1, i_cockType) {
            if (i_cockType == undefined)
                i_cockType = CockTypesEnum_1.CockTypesEnum.HUMAN;
            this._cockLength = i_cockLength;
            this._cockThickness = i_cockThickness;
            this._cockType = i_cockType;
            this._pierced = 0;
            this._knotMultiplier = 1;
            this._isPierced = false;
            //_pierceType = PiercingTypesEnum.NONE;
            this._pShortDesc = "";
            this._pLongDesc = "";
            this._sock = "";
        }
        //MEMBER FUNCTIONS
        cArea() {
            return this.cockThickness * this.cockLength;
        }
        growCock(lengthDelta, bigCock) {
            if (lengthDelta == 0) {
                console_1.trace("Whoops! growCock called with 0, aborting...");
                return lengthDelta;
            }
            var threshhold = 0;
            console_1.trace("growcock starting at:" + lengthDelta);
            if (lengthDelta > 0) { // growing
                console_1.trace("and growing...");
                threshhold = 24;
                // BigCock Perk increases incoming change by 50% and adds 12 to the length before diminishing returns set in
                if (bigCock) {
                    console_1.trace("growCock found BigCock Perk");
                    lengthDelta *= 1.5;
                    threshhold += 12;
                }
                // Not a human cock? Multiple the length before dimishing returns set in by 3
                if (this.cockType != CockTypesEnum_1.CockTypesEnum.HUMAN)
                    threshhold *= 2;
                // Modify growth for cock socks
                if (this.sock == "scarlet") {
                    console_1.trace("growCock found Scarlet sock");
                    lengthDelta *= 1.5;
                }
                else if (this.sock == "cobalt") {
                    console_1.trace("growCock found Cobalt sock");
                    lengthDelta *= .5;
                }
                // Do diminishing returns
                if (this.cockLength > threshhold)
                    lengthDelta /= 4;
                else if (this.cockLength > threshhold / 2)
                    lengthDelta /= 2;
            }
            else {
                console_1.trace("and shrinking...");
                threshhold = 0;
                // BigCock Perk doubles the incoming change value and adds 12 to the length before diminishing returns set in
                if (bigCock) {
                    console_1.trace("growCock found BigCock Perk");
                    lengthDelta *= 0.5;
                    threshhold += 12;
                }
                // Not a human cock? Add 12 to the length before dimishing returns set in
                if (this.cockType != CockTypesEnum_1.CockTypesEnum.HUMAN)
                    threshhold += 12;
                // Modify growth for cock socks
                if (this.sock == "scarlet") {
                    console_1.trace("growCock found Scarlet sock");
                    lengthDelta *= 0.5;
                }
                else if (this.sock == "cobalt") {
                    console_1.trace("growCock found Cobalt sock");
                    lengthDelta *= 1.5;
                }
                // Do diminishing returns
                if (this.cockLength > threshhold)
                    lengthDelta /= 3;
                else if (this.cockLength > threshhold / 2)
                    lengthDelta /= 2;
            }
            console_1.trace("then changing by: " + lengthDelta);
            this.cockLength += lengthDelta;
            if (this.cockLength < 1)
                this.cockLength = 1;
            if (this.cockThickness > this.cockLength * .33)
                this.cockThickness = this.cockLength * .33;
            return lengthDelta;
        }
        thickenCock(increase) {
            var amountGrown = 0;
            var temp = 0;
            if (increase > 0) {
                while (increase > 0) {
                    if (increase < 1)
                        temp = increase;
                    else
                        temp = 1;
                    //Cut thickness growth for huge dicked
                    if (this.cockThickness > 1 && this.cockLength < 12) {
                        temp /= 4;
                    }
                    if (this.cockThickness > 1.5 && this.cockLength < 18)
                        temp /= 5;
                    if (this.cockThickness > 2 && this.cockLength < 24)
                        temp /= 5;
                    if (this.cockThickness > 3 && this.cockLength < 30)
                        temp /= 5;
                    //proportional thickness diminishing returns.
                    if (this.cockThickness > this.cockLength * .15)
                        temp /= 3;
                    if (this.cockThickness > this.cockLength * .20)
                        temp /= 3;
                    if (this.cockThickness > this.cockLength * .30)
                        temp /= 5;
                    //massive thickness limiters
                    if (this.cockThickness > 4)
                        temp /= 2;
                    if (this.cockThickness > 5)
                        temp /= 2;
                    if (this.cockThickness > 6)
                        temp /= 2;
                    if (this.cockThickness > 7)
                        temp /= 2;
                    //Start adding up bonus length
                    amountGrown += temp;
                    this.cockThickness += temp;
                    temp = 0;
                    increase--;
                }
                increase = 0;
            }
            else if (increase < 0) {
                while (increase < 0) {
                    temp = -1;
                    //Cut length growth for huge dicked
                    if (this.cockThickness <= 1)
                        temp /= 2;
                    if (this.cockThickness < 2 && this.cockLength < 10)
                        temp /= 2;
                    //Cut again for massively dicked
                    if (this.cockThickness < 3 && this.cockLength < 18)
                        temp /= 2;
                    if (this.cockThickness < 4 && this.cockLength < 24)
                        temp /= 2;
                    //MINIMUM Thickness of OF .5!
                    if (this.cockThickness <= .5)
                        temp = 0;
                    //Start adding up bonus length
                    amountGrown += temp;
                    this.cockThickness += temp;
                    temp = 0;
                    increase++;
                }
            }
            console_1.trace("thickenCock called and thickened by: " + amountGrown);
            return amountGrown;
        }
        get cockLength() {
            return this._cockLength;
        }
        set cockLength(value) {
            this._cockLength = value;
        }
        get cockThickness() {
            return this._cockThickness;
        }
        set cockThickness(value) {
            this._cockThickness = value;
        }
        get cockType() {
            return this._cockType;
        }
        set cockType(value) {
            this._cockType = value;
        }
        get knotMultiplier() {
            return this._knotMultiplier;
        }
        set knotMultiplier(value) {
            this._knotMultiplier = value;
        }
        get isPierced() {
            return this._isPierced;
        }
        set isPierced(value) {
            this._isPierced = value;
        }
        /*
        public  get pierceType():PiercingTypesEnum
        {
            return _pierceType;
        }
        
        public  set pierceType(value:PiercingTypesEnum): void
        {
            _pierceType = value;
        }
        */
        //{ region Getter/Setters
        get pShortDesc() {
            return this._pShortDesc;
        }
        set pShortDesc(value) {
            this._pShortDesc = value;
        }
        get pLongDesc() {
            return this._pLongDesc;
        }
        set pLongDesc(value) {
            this._pLongDesc = value;
        }
        get sock() {
            return this._sock;
        }
        set sock(value) {
            this._sock = value;
        }
        get pierced() {
            return this._pierced;
        }
        set pierced(value) {
            this._pierced = value;
        }
    }
    exports.Cock = Cock;
});
//# sourceMappingURL=Cock.js.map