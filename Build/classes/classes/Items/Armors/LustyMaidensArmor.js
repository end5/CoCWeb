define(["require", "exports", "../Armor", "../../GlobalFlags/kFLAGS", "../../PerkLib", "../../CoC", "../../Scenes/Areas/Mountain/Minotaur", "../../Scenes/Areas/HighMountains/MinotaurMob"], function (require, exports, Armor_1, kFLAGS_1, PerkLib_1, CoC_1, Minotaur_1, MinotaurMob_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    /**
     * Created by aimozg on 11.01.14.
     */
    class LustyMaidensArmor extends Armor_1.Armor {
        constructor() {
        ...
        get def() {
            if (this.game.player.hasVirginVagina())
                return 9 + this.game.flags[kFLAGS_1.kFLAGS.BIKINI_ARMOR_BONUS];
            return 6 + this.game.flags[kFLAGS_1.kFLAGS.BIKINI_ARMOR_BONUS];
        }
        canUse() {
            if (this.game.player.biggestTitSize() < CoC_1.CoC.BREAST_CUP_A) { //{No titties}
            ...
            if (this.game.player.biggestTitSize() < CoC_1.CoC.BREAST_CUP_D) { //{Too small titties}
            ...
            if (this.game.player.vaginas[0].virgin) {
                ...
                if (this.game.player.cor < 33)
                    this.outputText("  Wait, that isn't right...");
            }
            else {
            ...
            if (this.game.player.wetness() >= 3)
                this.outputText("  The moisture you normally drip seems to soak right into the gusset instead of running down your [legs] like normal, giving you a much more chaste appearance in spite of the lewd garments that even now seem to shape your femininity and [butt] into perfectly arousing shapes.");
        ...
        playerEquip() {
            while (this.game.player.findPerk(PerkLib_1.PerkLib.SluttySeduction) >= 0)
                this.game.player.removePerk(PerkLib_1.PerkLib.SluttySeduction);
            if (this.game.player.hasVirginVagina()) {
                this.game.player.createPerk(PerkLib_1.PerkLib.SluttySeduction, 10 + this.game.flags[kFLAGS_1.kFLAGS.BIKINI_ARMOR_BONUS], 0, 0, 0);
            }
            else {
                this.game.player.createPerk(PerkLib_1.PerkLib.SluttySeduction, 6 + this.game.flags[kFLAGS_1.kFLAGS.BIKINI_ARMOR_BONUS], 0, 0, 0);
            }
        ...
        lustyMaidenPaizuri(player, monster) {
            if (player == undefined)
            ...
            if (!player.hasVirginVagina())
                ...
                this.outputText("sacred and to be shared only with a cherished loved one");
            ...
            if (monster.balls > 0)
                this.outputText(" and fondle " + monster.pronoun3 + " balls one-handed, squeezing the virile orbs to try and coax more of " + monster.pronoun3 + " dirty, perverted thoughts to distill into salty seed");
            else if (monster.short == "anemone")
                this.outputText(" and stroke " + monster.pronoun3 + " taint, even brushing over the featureless spot where an asshole would be, if she had one, to try and coax more of " + monster.pronoun3 + " dirty, perverted thoughts to distill into salty seed");
            else
                this.outputText(" and stroke " + monster.pronoun3 + " taint, even brushing close to " + monster.pronoun3 + " asshole to try and coax more of " + monster.pronoun3 + " dirty, perverted thoughts to distill into salty seed");
            ...
            if (monster.cockArea(0) >= 80)
                this.outputText("  It's so big and strong that it pushes right into your throat, stretching out your neck in the shape of the intruding cock.");
            ...
            if (player.hasVirginVagina())
                this.outputText("maidenhead");
            else
                this.outputText("sloppy gash");
            ...
            if (monster.cockArea(0) >= 80)
                this.outputText(" with a messy cough to clear your throat");
            ...
            if (monster.balls > 0)
                this.outputText(", " + monster.pronoun3 + " balls pumping and bouncing in " + monster.pronoun3 + " sack");
            ...
            player.slimeFeed();
            this.game.flags[kFLAGS_1.kFLAGS.BIKINI_ARMOR_BONUS] += 2;
            if (this.game.flags[kFLAGS_1.kFLAGS.BIKINI_ARMOR_BONUS] > 8)
                this.game.flags[kFLAGS_1.kFLAGS.BIKINI_ARMOR_BONUS] = 8;
            player.orgasm();
            this.game.dynStats("sen", 2);
            if (player.hasVirginVagina())
                this.game.dynStats("cor", -1);
            //If minotaur, increase addiction slightly.
            if (monster instanceof Minotaur_1.Minotaur || monster instanceof MinotaurMob_1.MinotaurMob)
                player.minoCumAddiction(3);
            if (monster.short == "Ceraph")
                this.game.flags[kFLAGS_1.kFLAGS.UNKNOWN_FLAG_NUMBER_00291]++;
            ...
            if (this.game.inCombat)
                ...
                this.game.doNext(this.game.camp.returnToCampUseOneHour);
        }
    }
    exports.LustyMaidensArmor = LustyMaidensArmor;
});
//# sourceMappingURL=LustyMaidensArmor.js.map