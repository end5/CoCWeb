define(["require", "exports", "../Consumable", "../../StatusAffects", "../../internals/Utils", "../../PerkLib", "../../PregnancyStore", "../../GlobalFlags/kFLAGS", "../../CoC", "../../CockTypesEnum"], function (require, exports, Consumable_1, StatusAffects_1, Utils_1, PerkLib_1, PregnancyStore_1, kFLAGS_1, CoC_1, CockTypesEnum_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    /**
     * Created by K.Quesom 11.06.14
     */
    class BeeHoney extends Consumable_1.Consumable {
        constructor(pure, special) {
            var honeyName;
            var honeyLong;
            var honeyDesc;
            var honeyValue;
        ...
        canUse() {
            if (this.value == BeeHoney.SPECIAL_HONEY_VALUE && this.getGame().player.statusAffectv1(StatusAffects_1.StatusAffects.Exgartuan) == 1) { //Exgartuan doesn't like the special honey
        ...
        useItem() {
            var player = this.getGame().player;
            var pure = (this.value == BeeHoney.PURE_HONEY_VALUE);
            var special = (this.value == BeeHoney.SPECIAL_HONEY_VALUE);
            var changes = 0;
            var changeLimit = 1;
            this.clearOutput();
            player.slimeFeed();
            ...
            if ((pure || special) && player.pregnancyType == PregnancyStore_1.PregnancyStore.PREGNANCY_FAERIE) { //Pure or special honey can reduce the corruption of a phouka baby
                if (this.getGame().flags[kFLAGS_1.kFLAGS.PREGNANCY_CORRUPTION] > 1) { //Child is phouka, hates pure honey
                ...
                else if (this.getGame().flags[kFLAGS_1.kFLAGS.PREGNANCY_CORRUPTION] < 1) { //Child is faerie, loves pure honey
                ...
                this.getGame().flags[kFLAGS_1.kFLAGS.PREGNANCY_CORRUPTION]--;
                if (pure)
            ...
            if (changes < changeLimit && pure) { //Special honey will also reduce corruption, but uses different text and is handled separately
                ...
                changes++;
                if (player.cor > 80)
                    this.outputText("Your head aches, as if thunder was echoing around your skull.  ");
                else if (player.cor > 60)
                    this.outputText("You feel a headache forming just behind your eyes.  In no time flat it reaches full strength.  ");
                else if (player.cor > 40)
                    this.outputText("A wave of stinging pain slices through your skull.  ");
                else if (player.cor > 20)
                    this.outputText("A prickling pain spreads throughout your skull.  ");
                else
                    this.outputText("You feel a mildly unpleasant tingling inside your skull.  ");
                if (player.cor > 0)
                    this.outputText("It quickly passes, leaving you more clearheaded");
                this.getGame().dynStats("cor", -(1 + (player.cor / 20)));
                //Libido Reduction
                if (player.cor > 0 && changes < changeLimit && Utils_1.Utils.rand(1.5) == 0 && player.lib > 40) {
                    this.outputText(" and settling your overcharged sex-drive a bit.");
                    this.getGame().dynStats("lib", -3, "lus", -20);
                    changes++;
                }
                else if (player.cor > 0)
                    this.outputText(".");
            }
            //bee item corollary:
            if (changes < changeLimit && player.hairType == 4 && Utils_1.Utils.rand(2) == 0) {
            ...
            //Intelligence Boost
            if (changes < changeLimit && Utils_1.Utils.rand(2) == 0 && player.inte < 80) {
            ...
            //Appearance Stuff
            //Hair Color
            if (changes < changeLimit && (player.hairColor != "shiny black" && player.hairColor != "black and yellow") && player.hairLength > 10 && Utils_1.Utils.rand(5) == 0) {
                this.outputText("\n\nYou feel your scalp tingling, and you grab your hair in a panic, pulling a strand forward.  ");
                if (Utils_1.Utils.rand(9) == 0)
            ...
            //Hair Length
            if (changes < changeLimit && player.hairLength < 25 && Utils_1.Utils.rand(3) == 0) {
                this.outputText("\n\nFeeling a bit off-balance, you discover your hair has lengthened, ");
                player.hairLength += Utils_1.Utils.rand(4) + 1;
                this.outputText("becoming " + this.getGame().hairDescript() + ".");
                changes++;
            }
            //-Remove extra breast rows
            if (changes < changeLimit && player.bRows() > 2 && Utils_1.Utils.rand(3) == 0 && !this.getGame().flags[kFLAGS_1.kFLAGS.HYPER_HAPPY]) {
                changes++;
                this.outputText("\n\nYou stumble back when your center of balance shifts, and though you adjust before you can fall over, you're left to watch in awe as your bottom-most " + this.getGame().breastDescript(player.breastRows.length - 1) + " shrink down, disappearing completely into your ");
                if (player.bRows() >= 3)
                    this.outputText("abdomen");
                else
                    this.outputText("chest");
                this.outputText(". The " + this.getGame().nippleDescript(player.breastRows.length - 1) + "s even fade until nothing but ");
                if (player.skinType == CoC_1.CoC.SKIN_TYPE_FUR)
                    this.outputText(player.hairColor + " " + player.skinDesc);
                else
                    this.outputText(player.skinTone + " " + player.skinDesc);
                this.outputText(" remains. <b>You've lost a row of breasts!</b>");
                this.getGame().dynStats("sen", -5);
                player.removeBreastRow(player.breastRows.length - 1, 1);
            }
            //Antennae
            if (changes < changeLimit && player.antennae == CoC_1.CoC.ANTENNAE_NONE && player.horns == 0 && Utils_1.Utils.rand(3) == 0) {
                this.outputText("\n\nYour head itches momentarily as two floppy antennae sprout from your " + this.getGame().hairDescript() + ".");
                player.antennae = CoC_1.CoC.ANTENNAE_BEE;
                changes++;
            }
            //Horns
            if (changes < changeLimit && player.horns > 0 && Utils_1.Utils.rand(3) == 0) {
                player.horns = 0;
                player.hornType = CoC_1.CoC.HORNS_NONE;
                this.outputText("\n\nYour horns crumble, falling apart in large chunks until they flake away to nothing.");
                changes++;
            }
            //Bee Legs
            if (changes < changeLimit && player.lowerBody != CoC_1.CoC.LOWER_BODY_TYPE_BEE && player.lowerBody != CoC_1.CoC.LOWER_BODY_TYPE_CENTAUR && Utils_1.Utils.rand(4) == 0) {
                ...
                player.lowerBody = CoC_1.CoC.LOWER_BODY_TYPE_BEE;
                changes++;
            }
            //-Nipples reduction to 1 per tit.
            if (player.averageNipplesPerBreast() > 1 && changes < changeLimit && Utils_1.Utils.rand(4) == 0) {
                this.outputText("\n\nA chill runs over your " + this.getGame().allBreastsDescript() + " and vanishes.  You stick a hand under your " + player.armorName + " and discover that your extra nipples are missing!  You're down to just one per ");
                if (player.biggestTitSize() < 1)
                    this.outputText("'breast'.");
                else
                    this.outputText("breast.");
                changes++;
            ...
            //Gain oviposition!
            if (changes < changeLimit && player.findPerk(PerkLib_1.PerkLib.BeeOvipositor) < 0 && player.tailType == CoC_1.CoC.TAIL_TYPE_BEE_ABDOMEN && Utils_1.Utils.rand(2) == 0) {
                ...
                player.createPerk(PerkLib_1.PerkLib.BeeOvipositor, 0, 0, 0, 0);
                changes++;
            }
            //Bee butt - 66% lower chance if already has a tail
            if (changes < changeLimit && player.tailType != CoC_1.CoC.TAIL_TYPE_BEE_ABDOMEN && (player.tailType == CoC_1.CoC.TAIL_TYPE_NONE || Utils_1.Utils.rand(1.5) == 0) && Utils_1.Utils.rand(4) == 0) {
                if (player.tailType > CoC_1.CoC.TAIL_TYPE_NONE)
                ...
                player.tailType = CoC_1.CoC.TAIL_TYPE_BEE_ABDOMEN;
                player.tailVenom = 10;
                player.tailRecharge = 2;
                changes++;
            }
            //Venom Increase
            if (changes < changeLimit && player.tailType == CoC_1.CoC.TAIL_TYPE_BEE_ABDOMEN && player.tailRecharge < 15 && Utils_1.Utils.rand(2)) {
                if (player.tailRecharge < 5)
                    player.tailRecharge += 1;
            ...
            if (changes < changeLimit && player.wingType == CoC_1.CoC.WING_TYPE_BEE_LIKE_SMALL && Utils_1.Utils.rand(4)) {
                changes++;
                player.wingType = CoC_1.CoC.WING_TYPE_BEE_LIKE_LARGE;
            ...
            if (changes < changeLimit && (player.wingType == CoC_1.CoC.WING_TYPE_NONE || player.wingType == CoC_1.CoC.WING_TYPE_SHARK_FIN) && Utils_1.Utils.rand(4)) {
                changes++;
                if (player.wingType == CoC_1.CoC.WING_TYPE_SHARK_FIN)
                ...
                player.wingType = CoC_1.CoC.WING_TYPE_BEE_LIKE_SMALL;
                player.wingDesc = "small bee-like";
            }
            //Melt demon wings!
            if (changes < changeLimit && (player.wingType == CoC_1.CoC.WING_TYPE_BAT_LIKE_TINY || player.wingType == CoC_1.CoC.WING_TYPE_BAT_LIKE_LARGE)) {
                changes++;
                ...
                player.wingType = CoC_1.CoC.WING_TYPE_NONE;
                player.wingDesc = "";
            }
            if (Utils_1.Utils.rand(4) == 0 && player.gills && changes < changeLimit) {
            ...
            if (special) { //All the speical honey effects occur after any normal bee transformations (if the player wasn't a full bee morph)
                if (!player.hasCock()) {
                    ...
                    player.createCock();
                    player.cocks[0].cockLength = Utils_1.Utils.rand(3) + 8;
                    ...
                    if (player.cocks[0].cArea() <= 20)
                        ...
                        this.outputText("Your member is now simply huge, you wonder what in the world could actually take your massive size now?");
                    this.outputText("</b>");
                    player.cocks[0].cockLength += Utils_1.Utils.rand(3) + 4; //4 to 6 inches in length
                    player.cocks[0].cockThickness += 0.1 * Utils_1.Utils.rand(5) + 0.5; //0.5 to 1 inches in thickness
                    this.getGame().dynStats("sen", 5);
                }
                else if (player.cocks[0].cockType != CockTypesEnum_1.CockTypesEnum.BEE && player.race() == "bee-morph") {
                    ...
                    player.cocks[0].cockType = CockTypesEnum_1.CockTypesEnum.BEE;
                    ...
                    player.cocks[0].cockLength += 0.1 * Utils_1.Utils.rand(10) + 1;
                    player.cocks[0].cockThickness += 0.1 * Utils_1.Utils.rand(2) + 0.1;
    ...
    BeeHoney.PURE_HONEY_VALUE = 40;
    BeeHoney.SPECIAL_HONEY_VALUE = 20;
    exports.BeeHoney = BeeHoney;
});
