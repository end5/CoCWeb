define(["require", "exports", "../BaseContent", "../GlobalFlags/kFLAGS", "../StatusAffects", "../PerkLib", "../CockTypesEnum", "../../console", "../../../includes/appearanceDefs", "../Appearance", "../CoC_Settings", "../PregnancyStore", "../GlobalFlags/kGAMECLASS"], function (require, exports, BaseContent_1, kFLAGS_1, StatusAffects_1, PerkLib_1, CockTypesEnum_1, console_1, appearanceDefs_1, Appearance_1, CoC_Settings_1, PregnancyStore_1, kGAMECLASS_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class Mutations extends BaseContent_1.BaseContent {
        ...
        //Cerulean P.
        ceruleanPotion(player) {
            player.slimeFeed();
            //Repeat genderless encounters
            if (player.gender == 0 && this.flags[kFLAGS_1.kFLAGS.CERULEAN_POTION_NEUTER_ATTEMPTED] > 0) {
                this.outputText("You take another sip of the Cerulean Potion.  You find it soothing and become very excited about the possibility of another visit from the succubus.", true);
            }
            else if (player.gender == 3 && this.flags[kFLAGS_1.kFLAGS.UNKNOWN_FLAG_NUMBER_00111] > 0) {
                this.outputText("With anticipation, you chug down another bottle of the Cerulean Potion. A warm sensation radiates out from your stomach as you feel the potion course through your body.", true);
            }
            //All else
            else {
                this.outputText("The liquid tastes rather bland and goes down easily. ", true);
                //Special repeat texts
                if (player.findStatusAffect(StatusAffects_1.StatusAffects.RepeatSuccubi) >= 0)
                    this.outputText("You look forwards to tonight's encounter.", false);
                //First timer huh?
                else
                    this.outputText("You do not notice any real effects.  Did the merchant con you?", false);
            }
            if (player.findStatusAffect(StatusAffects_1.StatusAffects.SuccubiNight) >= 0) {
                if (player.statusAffectv1(StatusAffects_1.StatusAffects.SuccubiNight) < 3)
                    player.addStatusValue(StatusAffects_1.StatusAffects.SuccubiNight, 1, 1);
            }
            else
                player.createStatusAffect(StatusAffects_1.StatusAffects.SuccubiNight, 1, 0, 0, 0);
        }
        //Vitality Tincture
        vitalityTincture(player) {
            player.slimeFeed();
            this.outputText("You down the contents of the bottle. The liquid is thick and tastes remarkably like cherries. Within moments, you feel much more fit and healthy.", true);
            //str change
            ...
            if (player.HP < player.maxHP())
                this.outputText("  Any aches, pains and bruises you have suffered no longer hurt and you feel much better.", false);
            this.HPChange(50, false);
            // if (this.HPChange(50, false)) this.outputText("  Any aches, pains and bruises you have suffered no longer hurt and you feel much better.", false);
            if (Mutations.rand(3) == 0)
                this.outputText(player.modTone(95, 3), false);
        }
        //Scholar's Tea
        scholarsTea(player) {
            player.slimeFeed();
            ...
            if (Mutations.rand(3) == 0)
                this.outputText(player.modTone(15, 1), false);
            this.dynStats("int", (2.5 + Mutations.rand(5)));
        }
        /* ITEMZZZZZ FUNCTIONS GO HERE */
        incubiDraft(tainted, player) {
            player.slimeFeed();
            var temp2 = 0;
            var temp3 = 0;
            var rando = Mutations.rand(100);
            if (player.findPerk(PerkLib_1.PerkLib.HistoryAlchemist) >= 0)
                rando += 10;
            this.outputText("The draft is slick and sticky, ", true);
            if (player.cor <= 33)
                this.outputText("just swallowing it makes you feel unclean.", false);
            if (player.cor > 33 && player.cor <= 66)
                this.outputText("reminding you of something you just can't place.", false);
            if (player.cor > 66)
                this.outputText("deliciously sinful in all the right ways.", false);
            if (player.cor >= 90)
                this.outputText("  You're sure it must be distilled from the cum of an incubus.", false);
            //Lowlevel changes
            if (rando < 50) {
                if (player.cocks.length == 1) {
                    if (player.cocks[0].cockType != CockTypesEnum_1.CockTypesEnum.DEMON)
                        this.outputText("\n\nYour " + this.cockDescript(0) + " becomes shockingly hard.  It turns a shiny inhuman purple and spasms, dribbling hot demon-like cum as it begins to grow.", false);
                    else
                        this.outputText("\n\nYour " + this.cockDescript(0) + " becomes shockingly hard.  It dribbles hot demon-like cum as it begins to grow.", false);
                    if (Mutations.rand(4) == 0)
                        this.temp = player.increaseCock(0, 3);
                    else
                        this.temp = player.increaseCock(0, 1);
                    this.dynStats("int", 1, "lib", 2, "sen", 1, "lust", 5 + this.temp * 3, "cor", tainted ? 1 : 0);
                    if (this.temp < .5)
                        this.outputText("  It stops almost as soon as it starts, growing only a tiny bit longer.", false);
                    if (this.temp >= .5 && this.temp < 1)
                        this.outputText("  It grows slowly, stopping after roughly half an inch of growth.", false);
                    if (this.temp >= 1 && this.temp <= 2)
                        this.outputText("  The sensation is incredible as more than an inch of lengthened dick-flesh grows in.", false);
                    if (this.temp > 2)
                        this.outputText("  You smile and idly stroke your lengthening " + this.cockDescript(0) + " as a few more inches sprout.", false);
                    if (tainted)
                        this.dynStats("int", 1, "lib", 2, "sen", 1, "lus", 5 + this.temp * 3, "cor", 1);
                    else
                        this.dynStats("int", 1, "lib", 2, "sen", 1, "lus", 5 + this.temp * 3);
                    if (player.cocks[0].cockType != CockTypesEnum_1.CockTypesEnum.DEMON)
                        this.outputText("  With the transformation complete, your " + this.cockDescript(0) + " returns to its normal coloration.", false);
                    else
                        this.outputText("  With the transformation complete, your " + this.cockDescript(0) + " throbs in an almost happy way as it goes flaccid once more.", false);
                }
                if (player.cocks.length > 1) {
                    ...
                    if (player.cockTotal() == 2)
                        this.outputText("\n\nBoth of your " + this.multiCockDescriptLight() + " become shockingly hard, swollen and twitching as they turn a shiny inhuman purple in color.  They spasm, dripping thick ropes of hot demon-like pre-cum along their lengths as your shortest " + this.cockDescript(temp2) + " begins to grow.", false);
                    //For more than 2
                    else
                        this.outputText("\n\nAll of your " + this.multiCockDescriptLight() + " become shockingly hard, swollen and twitching as they turn a shiny inhuman purple in color.  They spasm, dripping thick ropes of hot demon-like pre-cum along their lengths as your shortest " + this.cockDescript(temp2) + " begins to grow.", false);
                    if (temp3 < .5)
                        this.outputText("  It stops almost as soon as it starts, growing only a tiny bit longer.", false);
                    if (temp3 >= .5 && temp3 < 1)
                        this.outputText("  It grows slowly, stopping after roughly half an inch of growth.", false);
                    if (temp3 >= 1 && temp3 <= 2)
                        this.outputText("  The sensation is incredible as more than an inch of lengthened dick-flesh grows in.", false);
                    if (temp3 > 2)
                        this.outputText("  You smile and idly stroke your lengthening " + this.cockDescript(temp2) + " as a few more inches sprout.", false);
                    this.outputText("  With the transformation complete, your " + this.multiCockDescriptLight() + " return to their normal coloration.", false);
                }
                //NO CAWKS?
                if (player.cocks.length == 0) {
                    ...
                    this.outputText("The skin seems to fold back as a fully formed demon-cock bursts forth from your loins, drizzling hot cum everywhere as it orgasms.  Eventually the orgasm ends as your " + this.cockDescript(0) + " fades to a more normal " + player.skinTone + " tone.", false);
                    if (tainted)
                        this.dynStats("lib", 3, "sen", 5, "lus", 10, "cor", 5);
                    else
                        this.dynStats("lib", 3, "sen", 5, "lus", 10);
                }
            ...
            if (rando >= 50 && rando < 93) {
                if (player.cocks.length > 1) {
                    ...
                    while (this.temp > 0) {
                        ...
                        if (temp3 < .1)
                            player.cocks[this.temp].cockThickness += .05;
                    }
                    ...
                    if (tainted)
                        this.dynStats("lib", 3, "sen", 5, "lus", 10, "cor", 3);
                    else
                        this.dynStats("lib", 3, "sen", 5, "lus", 10);
                }
                if (player.cocks.length == 1) {
                    ...
                    if (temp3 >= 1) {
                        if (player.cocks.length == 1)
                            this.outputText("  Your cock spreads rapidly, swelling an inch or more in girth, making it feel fat and floppy.", false);
                        else
                            this.outputText("  Your cocks spread rapidly, swelling as they grow an inch or more in girth, making them feel fat and floppy.", false);
                    }
                    if (temp3 <= .5) {
                        if (player.cocks.length > 1)
                            this.outputText("  Your cocks feel swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. They are definitely thicker.", false);
                        else
                            this.outputText("  Your cock feels swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. It is definitely thicker.", false);
                    }
                    if (temp3 > .5 && temp2 < 1) {
                        if (player.cocks.length == 1)
                            this.outputText("  Your cock seems to swell up, feeling heavier. You look down and watch it growing fatter as it thickens.", false);
                        if (player.cocks.length > 1)
                            this.outputText("  Your cocks seem to swell up, feeling heavier. You look down and watch them growing fatter as they thicken.", false);
                    }
                    if (tainted)
                        this.dynStats("lib", 3, "sen", 5, "lus", 10, "cor", 3);
                    else
            ...
            if (Mutations.rand(40) + player.cor / 3 > 35 && tainted)
                this.demonChanges(player);
            player.genderCheck();
            if (Mutations.rand(4) == 0 && tainted)
                this.outputText(player.modFem(5, 2), false);
            if (Mutations.rand(4) == 0 && tainted)
                this.outputText(player.modThickness(30, 2), false);
        }
        growDemonCock(growCocks) {
            this.temp = 0;
            while (growCocks > 0) {
                this.player.createCock();
                console_1.trace("COCK LENGTH: " + this.player.cocks[length - 1].cockLength);
            ...
            this.outputText("\n\nYou shudder as a pressure builds in your crotch, peaking painfully as a large bulge begins to push out from your body.  ", false);
            if (this.temp == 1) {
                this.outputText("The skin seems to fold back as a fully formed demon-cock bursts forth from your loins, drizzling hot cum everywhere as it orgasms.  In time it fades to a more normal coloration and human-like texture.  ", false);
            }
            else {
                this.outputText("The skin bulges obscenely, darkening and splitting around " + Mutations.num2Text(this.temp) + " of your new dicks.  For an instant they turn a demonic purple and dribble in thick spasms of scalding demon-cum.  After, they return to a more humanoid coloration.  ", false);
            }
            if (this.temp > 4)
                this.outputText("Your tender bundle of new cocks feels deliciously sensitive, and you cannot stop yourself from wrapping your hands around the slick demonic bundle and pleasuring them.\n\nNearly an hour later, you finally pull your slick body away from the puddle you left on the ground.  When you look back, you notice it has already been devoured by the hungry earth.", false);
            this.player.orgasm();
        }
        tatteredScroll(player) {
            this.outputText("Your wobbly " + player.legs() + " give out underneath you as your body's willpower seems to evaporate, your mouth reading the words on the scroll with a backwards sounding sing-song voice.\n\n", true);
            if (player.hairColor == "sandy blonde") {
                ...
                if (player.biggestTitSize() > 0 && player.biggestTitSize() < 3) {
                    this.outputText("Your breasts suddenly balloon outwards, stopping as they reach a perfectly rounded C-cup.  ", false);
                    player.breastRows[0].breastRating = 3;
                    this.dynStats("sen", 1, "lus", 1);
                }
                if (player.averageNipplesPerBreast() < 1) {
                    this.outputText("A dark spot appears on each breast, rapidly forming into a sensitive nipple.  ", false);
                ...
                if (player.biggestLactation() > 0) {
                    this.outputText("A strong pressure builds in your chest, painful in its intensity.  You yank down your top as ", false);
                    if (player.biggestLactation() < 2)
                        this.outputText("powerful jets of milk spray from your nipples, spraying thick streams over the ground.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ", false);
                    if (player.biggestLactation() >= 2 && player.biggestLactation() <= 2.6)
                        this.outputText("eruptions of milk squirt from your nipples, hosing thick streams everywhere.  The feeling of the constant gush of fluids is very erotic, and you feel yourself getting more and more turned on.  You start squeezing your breasts as the flow diminishes, anxious to continue the pleasure, but eventually all good things come to an end.  ", false);
                    if (player.biggestLactation() > 2.6 && player.biggestLactation() < 3)
                        this.outputText("thick hoses of milk erupt from your aching nipples, forming puddles on the ground.  You smile at how well you're feeding the earth, your milk coating the ground faster than it can be absorbed.  The constant lactation is pleasurable... in a highly erotic way, and you find yourself moaning and pulling on your nipples, your hands completely out of control.  In time you realize the milk has stopped, and even had time to soak into the dirt.  You wonder at your strange thoughts and pull your hands from your sensitive nipples.  ", false);
                    if (player.biggestLactation() >= 3)
                        this.outputText("you drop to your knees and grab your nipples.  With a very sexual moan you begin milking yourself, hosing out huge quantities of milk.  You pant and grunt, offering as much of your milk as you can.  It cascades down a hill in a small stream, and you can't help but blush with pride... and lust.  The erotic pleasures build as you do your best to feed the ground all of your milk.  You ride the edge of orgasm for an eternity, milk everywhere.  When you come to, you realize you're kneeling there, tugging your dry nipples.  Embarrassed, you stop, but your arousal remains.  ", false);
                    if (player.biggestLactation() < 3) {
                        player.boostLactation(.7);
                        this.outputText("Your breasts feel fuller... riper... like your next milking could be even bigger.  ", false);
                    }
                    this.dynStats("lib", 1, "sen", 4, "lus", 15);
                }
                if (player.biggestLactation() == 0) {
                    this.outputText("A pleasurable release suddenly erupts from your nipples!  Twin streams of milk are spraying from your breasts, soaking into the ground immediately.  It stops all too soon, though a voice in your head assures you that you can lactate quite often now.  ", false);
            ...
            if (!kGAMECLASS_1.kGAMECLASS.inCombat) {
                //RAEP
        ...
        minotaurCum(player) {
            ...
            this.outputText("As soon as you crack the seal on the bottled white fluid, a ", false);
            if (this.flags[kFLAGS_1.kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] == 0)
                this.outputText("potent musk washes over you.", false);
            else
                this.outputText("heavenly scent fills your nostrils.", false);
            if (this.flags[kFLAGS_1.kFLAGS.MINOTAUR_CUM_ADDICTION_TRACKER] < 50)
                this.outputText("  It makes you feel dizzy, ditzy, and placid.", false);
            else
                this.outputText("  It makes you feel euphoric, happy, and willing to do ANYTHING to keep feeling this way.", false);
            this.outputText("  Unbidden, your hand brings the bottle to your lips, and the heady taste fills your mouth as you convulsively swallow the entire bottle.", false);
            //-Raises lust by 10.
            //-Raises sensitivity
            ...
            if (player.cocks.length > 0) {
                ...
                this.outputText("our " + this.multiCockDescriptLight() + " aches, flooding with blood until it's bloating and trembling.", false);
            }
            if (player.hasVagina()) {
                this.outputText("  Your " + this.clitDescript() + " engorges, ", false);
                if (player.clitLength < 3)
                    this.outputText("parting your lips.", false);
                else
                    this.outputText("bursting free of your lips and bobbing under its own weight.", false);
                if (player.vaginas[0].vaginalWetness <= appearanceDefs_1.VAGINA_WETNESS_NORMAL)
                    this.outputText("  Wetness builds inside you as your " + this.vaginaDescript(0) + " tingles and aches to be filled.", false);
                else if (player.vaginas[0].vaginalWetness <= appearanceDefs_1.VAGINA_WETNESS_SLICK)
                    this.outputText("  A trickle of wetness escapes your " + this.vaginaDescript(0) + " as your body reacts to the desire burning inside you.", false);
                else if (player.vaginas[0].vaginalWetness <= appearanceDefs_1.VAGINA_WETNESS_DROOLING)
                    this.outputText("  Wet fluids leak down your thighs as your body reacts to this new stimulus.", false);
                else
                    this.outputText("  Slick fluids soak your thighs as your body reacts to this new stimulus.", false);
            }
            //(Minotaur fantasy)
            if (!kGAMECLASS_1.kGAMECLASS.inCombat && Mutations.rand(10) == 1) {
                ...
                this.dynStats("lib", 1, "lus", Mutations.rand(5) + player.cor / 20 + this.flags[kFLAGS_1.kFLAGS.MINOTAUR_CUM_ADDICTION_TRACKER] / 5);
            }
            //(Healing – if hurt and uber-addicted (hasperk))
            if (player.HP < player.maxHP() && player.findPerk(PerkLib_1.PerkLib.MinotaurCumAddict) >= 0) {
            ...
            if (player.findPerk(PerkLib_1.PerkLib.MinotaurCumAddict) >= 0 && this.flags[kFLAGS_1.kFLAGS.MINOTAUR_CUM_REALLY_ADDICTED_STATE] <= 0) {
                this.flags[kFLAGS_1.kFLAGS.MINOTAUR_CUM_REALLY_ADDICTED_STATE] = 3 + Mutations.rand(2);
        ...
        minotaurBlood(player) {
            ...
            var temp3 = 0;
            //Set up output
            this.outputText("You drink the bubbling red fluid, tasting the tangy iron after-taste.", true);
            //STATS
            //Strength h
            ...
            if (changes < changeLimit && Mutations.rand(2) == 0 && player.ballSize <= 5 && player.horseCocks() > 0) {
                ...
                if (player.balls == 0) {
                    ...
                    if (player.ballSize <= 2)
                        this.outputText("\n\nA flash of warmth passes through you and a sudden weight develops in your groin.  You pause to examine the changes and your roving fingers discover your " + this.simpleBallsDescript() + " have grown larger than a human's.", false);
                    if (player.ballSize > 2)
                        this.outputText("\n\nA sudden onset of heat envelops your groin, focusing on your " + this.sackDescript() + ".  Walking becomes difficult as you discover your " + this.simpleBallsDescript() + " have enlarged again.", false);
                    this.dynStats("lib", 1, "lus", 3);
                }
                changes++;
            }
            //-Remove feather-arms (copy this for goblin ale, mino blood, equinum, canine pepps, demon items)
            if (changes < changeLimit && player.armType == appearanceDefs_1.ARM_TYPE_HARPY && Mutations.rand(4) == 0) {
                ...
                player.armType = appearanceDefs_1.ARM_TYPE_HUMAN;
                changes++;
            }
            //-Remove chitin-arms (copy this for goblin ale, mino blood, equinum, canine pepps, demon items)
            if (changes < changeLimit && player.armType == appearanceDefs_1.ARM_TYPE_SPIDER && Mutations.rand(4) == 0) {
            ...
            //+hooves
            if (player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_HOOFED && player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_CENTAUR) {
                if (changes < changeLimit && Mutations.rand(3) == 0) {
                    changes++;
                    if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HUMAN)
                    ...
                    if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DOG)
                    ...
                    if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_NAGA)
                    ...
                    if (player.skinType != appearanceDefs_1.SKIN_TYPE_FUR)
                        this.outputText("  A fine coat of fur grows out below your waist, itching briefly as it fills in.");
                    this.outputText("<b>  You now have hooves in place of your feet!</b>", false);
                    player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_HOOFED;
                    this.dynStats("spe", 1);
                    changes++;
                }
            }
            if (!this.flags[kFLAGS_1.kFLAGS.HYPER_HAPPY]) {
                ...
                if (player.vaginas.length > 0) {
                    if (player.vaginas[0].vaginalLooseness > appearanceDefs_1.VAGINA_LOOSENESS_TIGHT) {
                        //tighten that bitch up!
                        this.outputText("\n\nYour " + this.vaginaDescript(0) + " clenches up painfully as it tightens up, becoming smaller and tighter.", false);
                        player.vaginas[0].vaginalLooseness--;
                    }
                    else {
                        this.outputText("\n\nA tightness in your groin is the only warning you get before your <b>" + this.vaginaDescript(0) + " disappears forever</b>!", false);
                        //Goodbye womanhood!
                        player.removeVagina(0, 1);
                        if (player.cocks.length == 0) {
                            this.outputText("  Strangely, your clit seems to have resisted the change, and is growing larger by the moment... shifting into the shape of a small ribbed minotaur-like penis!  <b>You now have a horse-cock!</b>", false);
                            player.createCock();
                            player.cocks[0].cockLength = player.clitLength + 2;
                            player.cocks[0].cockThickness = 1;
                            player.cocks[0].cockType = CockTypesEnum_1.CockTypesEnum.HORSE;
                ...
                if (changes < changeLimit && player.bRows() > 1 && Mutations.rand(3) == 0) {
                    changes++;
                    this.outputText("\n\nYou stumble back when your center of balance shifts, and though you adjust before you can fall over, you're left to watch in awe as your bottom-most " + this.breastDescript(player.breastRows.length - 1) + " shrink down, disappearing completely into your ", false);
                    if (player.bRows() >= 3)
                        this.outputText("abdomen", false);
                    else
                        this.outputText("chest", false);
                    this.outputText(". The " + this.nippleDescript(player.breastRows.length - 1) + "s even fade until nothing but ", false);
                    if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR)
                        this.outputText(player.hairColor + " " + player.skinDesc, false);
                    else
                        this.outputText(player.skinTone + " " + player.skinDesc, false);
                    this.outputText(" remains. <b>You've lost a row of breasts!</b>", false);
                    this.dynStats("sen", -5);
                    player.removeBreastRow(player.breastRows.length - 1, 1);
                }
                //Shrink boobages till they are normal
                else if (Mutations.rand(2) == 0 && changes < changeLimit && player.breastRows.length > 0) {
                    //Single row
                    if (player.breastRows.length == 1) {
                        ...
                        if (player.breastRows[0].breastRating >= 1) {
                            ...
                            if (temp == 1)
                                this.outputText("\n\nYou feel a weight lifted from you, and realize your " + this.breastDescript(0) + " have shrunk to " + player.breastCup(0) + "s.", false);
                            if (temp == 2)
                        ...
                        while (temp3 < player.breastRows.length) {
                            if (player.breastRows[temp3].breastRating >= 1) {
                                ...
                                if (temp2 > 1)
                                    ...
                                    this.outputText("Y", false);
                                this.outputText("our " + this.breastDescript(temp3) + " shrink, dropping to " + player.breastCup(temp3) + "s.", false);
                            }
                            temp3++;
                        }
                        if (temp2 == 2)
            ...
            if (changes < changeLimit && Mutations.rand(2) == 0 && player.cocks.length > 0) {
                ...
                for (var i = 0; i < player.cocks.length; i++) {
                    if (player.cocks[i].cockType == CockTypesEnum_1.CockTypesEnum.HORSE && (player.cocks[i].cockLength < 36 || player.cocks[i].cockThickness < 5)) {
                        selectedCock = i;
                        break;
                    }
                }
                ...
                if (selectedCock != -1) {
                    ...
                    if (player.cocks[selectedCock].cockThickness < 5) {
                        ...
                        if (temp > 6)
                            this.outputText("\n\nGasping in sudden pleasure, your " + this.cockDescript(selectedCock) + " surges free of its sheath, emerging with over half a foot of new dick-flesh.", false);
                        if (temp <= 6 && temp >= 3)
                            this.outputText("\n\nYou pant in delight as a few inches of " + this.cockDescript(selectedCock) + " pop free from your sheath, the thick new horse-flesh still slick and sensitive.", false);
                        if (temp < 3)
                        ...
                        this.outputText("  To your delight and surprise, you discover it has grown slightly thicker as well!", false);
                    }
                    //Just length...
                    else {
                        //Increase by 2 + rand(8), and store the actual amount in temp
            ...
            if (player.cocks.length > 0 && Mutations.rand(2) == 0 && changes < changeLimit) {
                ...
                for (var indexI = 0; indexI < player.cocks.length; indexI++) {
                    if (player.cocks[indexI].cockType != CockTypesEnum_1.CockTypesEnum.HORSE) {
                ...
                if (selectedCockValue != -1) {
                    ...
                    if (player.cocks[selectedCockValue].cockType == CockTypesEnum_1.CockTypesEnum.HUMAN || player.cocks[selectedCockValue].cockType.Index > 2)
                    ...
                    //Text for dogdicks
                    if (player.cocks[selectedCockValue].cockType == CockTypesEnum_1.CockTypesEnum.DOG)
                        this.outputText("\n\nYour " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.DOG) + " begins to feel odd...  You pull down your clothes to take a look and see it darkening.  You feel a growing tightness in the tip of your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.DOG) + " as it flattens, flaring outwards.  Your cock pushes out of your sheath, inch after inch of animal-flesh growing beyond its traditional size.  You notice your knot vanishing, the extra flesh pushing more fresh horsecock out from your sheath.  <b>Your hands are drawn to the strange new " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.HORSE) + "</b>, and you jerk yourself off, splattering thick ropes of cum with intense force.", false);
                    player.cocks[selectedCockValue].cockType = CockTypesEnum_1.CockTypesEnum.HORSE;
                    ...
                    if (player.horseCocks() > 1)
                        this.outputText("nother");
                    this.outputText(" horse-penis.</b>", false);
                    changes++;
                }
            }
            //Males go into rut
            ...
            if (Mutations.rand(4) == 0 && changes < changeLimit && player.findStatusAffect(StatusAffects_1.StatusAffects.Dysfunction) < 0) {
                if (player.cocks.length > 0)
                    this.outputText("\n\nYour " + this.cockDescript(0) + " tingles abruptly, then stops.  Worried, you reach down to check it, only to discover that it feels... numb.  It will be very hard to masturbate like this.", false);
                else if (player.hasVagina())
                    this.outputText("\n\nYour " + this.vaginaDescript(0) + " tingles abruptly, then stops.  Worried, you reach down to check it, only to discover that it feels... numb.  It will be very hard to masturbate like this.", false);
                if (player.cocks.length > 0 || player.hasVagina()) {
                    player.createStatusAffect(StatusAffects_1.StatusAffects.Dysfunction, 96, 0, 0, 0);
                    changes++;
                }
            }
            //Appearance shit:
            ...
            if (player.earType == appearanceDefs_1.EARS_COW && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HOOFED && player.tallness >= 90
                && changes < changeLimit && Mutations.rand(3) == 0) {
                if (player.faceType != appearanceDefs_1.FACE_COW_MINOTAUR) {
                    ...
                    changes++;
                    player.faceType = appearanceDefs_1.FACE_COW_MINOTAUR;
                }
            }
            //+mino horns require ears/tail
            if (changes < changeLimit && Mutations.rand(3) == 0 && player.earType == appearanceDefs_1.EARS_COW && player.tailType == appearanceDefs_1.TAIL_TYPE_COW) {
                temp = 1;
                //New horns or expanding mino horns
                if (player.hornType == appearanceDefs_1.HORNS_COW_MINOTAUR || player.hornType == appearanceDefs_1.HORNS_NONE) {
                    //Get bigger if player has horns
                    if (player.hornType == appearanceDefs_1.HORNS_COW_MINOTAUR) {
                        //Fems horns don't get bigger.
                        if (player.vaginas.length > 0) {
                            if (player.horns > 4) {
                                this.outputText("\n\nYou feel a pressure in your head around your horns, but they don't grow any larger.  ", false);
                                this.outputText("Your headache clears as lust washes through you unnaturally.  You feel as if you haven't cum in months.", false);
                        ...
                        //Males horns get 'uge.
                            ...
                            if (player.horns < 3)
                                this.outputText("They are the size of tiny nubs.", false);
                            if (player.horns >= 3 && player.horns < 6)
                                this.outputText("They are similar to what you would see on a young bull.", false);
                            if (player.horns >= 6 && player.horns < 12)
                                this.outputText("They look like the horns on a grown bull, big enough and dangerous enough to do some damage.", false);
                            if (player.horns >= 12 && player.horns < 20)
                                this.outputText("They are large and wicked looking.", false);
                            if (player.horns >= 20)
                                this.outputText("They are huge, heavy, and tipped with dangerous points.", false);
                    ...
                    //If no horns yet..
                        ...
                        player.hornType = appearanceDefs_1.HORNS_COW_MINOTAUR;
                        player.horns = 2;
                        changes++;
                    }
                }
                //Not mino horns, change to cow-horns
                if (player.hornType == appearanceDefs_1.HORNS_DEMON || player.hornType > appearanceDefs_1.HORNS_COW_MINOTAUR) {
            ...
            if (player.earType != appearanceDefs_1.EARS_COW && changes < changeLimit && player.tailType == appearanceDefs_1.TAIL_TYPE_COW && Mutations.rand(2) == 0) {
                this.outputText("\n\nYou feel your ears tug on your scalp as they twist shape, becoming oblong and cow-like.  <b>You now have cow ears.</b>", false);
                player.earType = appearanceDefs_1.EARS_COW;
                changes++;
            }
            //+cow tail
            if (changes < changeLimit && Mutations.rand(2) == 0 && player.tailType != appearanceDefs_1.TAIL_TYPE_COW) {
                if (player.tailType == appearanceDefs_1.TAIL_TYPE_NONE)
                    this.outputText("\n\nYou feel the flesh above your " + this.buttDescript() + " knotting and growing.  It twists and writhes around itself before flopping straight down, now shaped into a distinctly bovine form.  You have a <b>cow tail</b>.", false);
                else {
                    if (player.tailType < appearanceDefs_1.TAIL_TYPE_SPIDER_ADBOMEN || player.tailType > appearanceDefs_1.TAIL_TYPE_BEE_ABDOMEN) {
                ...
                player.tailType = appearanceDefs_1.TAIL_TYPE_COW;
                changes++;
            }
            ...
            if (changes < changeLimit && Mutations.rand(4) == 0 && ((player.ass.analWetness > 0 && player.findPerk(PerkLib_1.PerkLib.MaraesGiftButtslut) < 0) || player.ass.analWetness > 1)) {
            ...
            if (Mutations.rand(4) == 0)
                this.outputText(player.modFem(5, 10), false);
            if (Mutations.rand(4) == 0)
                this.outputText(player.modTone(85, 3), false);
            if (Mutations.rand(4) == 0)
                this.outputText(player.modThickness(70, 4), false);
            //Default
            if (changes == 0) {
        ...
        equinum(player) {
            ...
            //Set up output
            this.outputText("You down the potion, grimacing at the strong taste.", true);
            //CHANCE OF BAD END - 20% if face/tail/skin/cock are appropriate.
            //If hooved bad end doesn't appear till centaured
            if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && player.faceType == appearanceDefs_1.FACE_HORSE && player.tailType == appearanceDefs_1.TAIL_TYPE_HORSE && (player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_HOOFED)) {
                //WARNINGS
                //Repeat warnings
                if (player.findStatusAffect(StatusAffects_1.StatusAffects.HorseWarning) >= 0 && Mutations.rand(3) == 0) {
                    if (player.statusAffectv1(StatusAffects_1.StatusAffects.HorseWarning) == 0)
                    ...
                    player.addStatusValue(StatusAffects_1.StatusAffects.HorseWarning, 1, 1);
                }
                //First warning
                if (player.findStatusAffect(StatusAffects_1.StatusAffects.HorseWarning) < 0) {
                    ...
                    player.createStatusAffect(StatusAffects_1.StatusAffects.HorseWarning, 0, 0, 0, 0);
                }
                //Bad End
                if (Mutations.rand(4) == 0 && player.findStatusAffect(StatusAffects_1.StatusAffects.HorseWarning) >= 0) {
                        ...
                        if (player.cockTotal() > 0) {
                            ...
                            if (player.horseCocks() > 0) {
                                ...
                                if (player.gender == 0 || player.gender == 1)
                                    this.outputText("He is clearly a male, but you are somewhat confused as you can see not one but three bulges where his manhood would be.\n\n", false);
                                if (player.gender == 2)
                                    this.outputText("She is clearly a female, as you can see her six breasts jiggle as she walks towards you, small stains appearing on her shirt where her nipples are.\n\n", false);
                                if (player.gender == 3)
                                    this.outputText("You are somewhat confused as you can see a bulge near her thighs but also huge boobs jiggling as she walks, and you can't say if she's a male or female.\n\n", false);
            ...
            if (Mutations.rand(2) == 0) {
                ...
                if (player.tou >= 75) {
                    ...
                    if (player.gender == 1 || player.gender == 3)
                        this.outputText("stallion's.", false);
                    else
                        this.outputText("mare's.", false);
                }
                //NOT MAXXED
                else {
            ...
            //
            //MALENESS.
            if ((player.gender == 1 || player.gender == 3) && Mutations.rand(1.5) == 0 && changes < changeLimit) {
                ...
                if ((player.horseCocks() + player.demonCocks()) < player.cocks.length) {
                    ...
                    if (player.cocks.length == 1) {
                        ...
                        temp3 = 0;
                        if (player.cocks[0].cockType == CockTypesEnum_1.CockTypesEnum.HUMAN) {
                            this.outputText("\n\nYour " + this.cockDescript(0) + " begins to feel strange... you pull down your pants to take a look and see it darkening as you feel a tightness near the base where your skin seems to be bunching up.  A sheath begins forming around your cock's base, tightening and pulling your cock inside its depths.  A hot feeling envelops your member as it suddenly grows into a horse penis, dwarfing its old size.  The skin is mottled brown and black and feels more sensitive than normal.  Your hands are irresistibly drawn to it, and you jerk yourself off, splattering cum with intense force.", false);
                            temp = player.addHorseCock();
                            temp2 = player.increaseCock(temp, Mutations.rand(4) + 4);
                            temp3 = 1;
                            this.dynStats("lib", 5, "sen", 4, "lus", 35);
                        }
                        if (player.cocks[0].cockType == CockTypesEnum_1.CockTypesEnum.DOG) {
                            temp = player.addHorseCock();
                            this.outputText("\n\nYour " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.DOG) + " begins to feel odd... you pull down your clothes to take a look and see it darkening.  You feel a growing tightness in the tip of your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.DOG) + " as it flattens, flaring outwards.  Your cock pushes out of your sheath, inch after inch of animal-flesh growing beyond it's traditional size.  You notice your knot vanishing, the extra flesh pushing more horsecock out from your sheath.  Your hands are drawn to the strange new " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.HORSE) + ", and you jerk yourself off, splattering thick ropes of cum with intense force.", false);
                            temp2 = player.increaseCock(temp, Mutations.rand(4) + 4);
                            temp3 = 1;
                            this.dynStats("lib", 5, "sen", 4, "lus", 35);
                        }
                        if (player.cocks[0].cockType == CockTypesEnum_1.CockTypesEnum.TENTACLE) {
                            temp = player.addHorseCock();
                            this.outputText("\n\nYour " + this.cockDescript(0) + " begins to feel odd... you pull down your clothes to take a look and see it darkening.  You feel a growing tightness in the tip of your " + this.cockDescript(0) + " as it flattens, flaring outwards.  Your skin folds and bunches around the base, forming an animalistic sheath.  The slick inhuman texture you recently had fades, taking on a more leathery texture.  Your hands are drawn to the strange new " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.HORSE) + ", and you jerk yourself off, splattering thick ropes of cum with intense force.", false);
                            temp2 = player.increaseCock(temp, Mutations.rand(4) + 4);
                            temp3 = 1;
                            this.dynStats("lib", 5, "sen", 4, "lus", 35);
                        }
                        ...
                        if (temp3 == 1)
                            this.outputText("  <b>Your penis has transformed into a horse's!</b>", false);
                    }
                    //MULTICOCK
                    else {
                        this.dynStats("lib", 5, "sen", 4, "lus", 35);
                        temp = player.addHorseCock();
                        this.outputText("\n\nOne of your penises begins to feel strange.  You pull down your clothes to take a look and see the skin of your " + this.cockDescript(temp) + " darkening to a mottled brown and black pattern.", false);
                        if (temp == -1) {
                        ...
                        if (player.horseCocks() > 1 || player.dogCocks() > 0)
                            this.outputText("  Your sheath tingles and begins growing larger as the cock's base shifts to lie inside it.", false);
                        else
                            this.outputText("  You feel a tightness near the base where your skin seems to be bunching up.  A sheath begins forming around your " + this.cockDescript(temp) + "'s root, tightening and pulling your " + this.cockDescript(temp) + " inside its depths.", false);
                        temp2 = player.increaseCock(temp, Mutations.rand(4) + 4);
                        this.outputText("  The shaft suddenly explodes with movement, growing longer and developing a thick flared head leaking steady stream of animal-cum.", false);
                        this.outputText("  <b>You now have a horse-cock.</b>", false);
                    }
                ...
                //Players cocks are all horse-type - increase size!
                    ...
                    this.outputText("\n\n", false);
                    if (temp2 > 2)
                        this.outputText("Your " + this.cockDescript(temp) + " tightens painfully, inches of taut horse-flesh pouring out from your sheath as it grows longer.  Thick animal-pre forms at the flared tip, drawn out from the pleasure of the change.", false);
                    if (temp2 > 1 && temp2 <= 2)
                        this.outputText("Aching pressure builds within your sheath, suddenly releasing as an inch or more of extra dick flesh spills out.  A dollop of pre beads on the head of your enlarged " + this.cockDescript(temp) + " from the pleasure of the growth.", false);
                    if (temp2 <= 1)
                        this.outputText("A slight pressure builds and releases as your " + this.cockDescript(temp) + " pushes a bit further out of your sheath.", false);
                    changes++;
                }
                ...
                if (Mutations.rand(2) == 0 && changes < changeLimit && player.horseCocks() > 0) {
                    ...
                    this.outputText("\n\nYour " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.HORSE) + " thickens inside its sheath, growing larger and fatter as your veins thicken, becoming more noticeable.  It feels right", false);
                    if (player.cor + player.lib < 50)
                        this.outputText(" to have such a splendid tool.  You idly daydream about cunts and pussies, your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.HORSE) + " plowing them relentlessly, stuffing them pregnant with cum", false);
                    if (player.cor + player.lib >= 50 && player.cor + player.lib < 80)
                        this.outputText(" to be this way... You breath the powerful animalistic scent and fantasize about fucking centaurs night and day until their bellies slosh with your cum", false);
                    if (player.cor + player.lib >= 75 && player.cor + player.lib <= 125)
                        this.outputText(" to be a rutting stud.  You ache to find a mare or centaur to breed with.  Longing to spend your evenings plunging a " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.HORSE) + " deep into their musky passages, dumping load after load of your thick animal-cum into them.  You'd be happy just fucking horsecunts morning, noon, and night.  Maybe somewhere there is a farm needing a breeder..", false);
                    if (player.cor + player.lib > 125)
                        this.outputText(" to whinny loudly like a rutting stallion.  Your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.HORSE) + " is perfect for fucking centaurs and mares.  You imagine the feel of plowing an equine pussy deeply, bottoming out and unloading sticky jets of horse-jizz into its fertile womb.  Your hand strokes your horsecock of its own accord, musky pre dripping from the flared tip with each stroke.  Your mind wanders to the thought of you with a harem of pregnant centaurs.", false);
                    this.outputText(".", false);
                    if (player.cor < 30)
                        this.outputText("  You shudder in revulsion at the strange thoughts and vow to control yourself better.", false);
                    if (player.cor >= 30 && player.cor < 60)
                        this.outputText("  You wonder why you thought such odd things, but they have a certain appeal.", false);
                    if (player.cor >= 60 && player.cor < 90)
                        this.outputText("  You relish your twisted fantasies, hoping to dream of them again.", false);
                    if (player.cor >= 90)
                        this.outputText("  You flush hotly and give a twisted smile, resolving to find a fitting subject to rape and relive your fantasies.", false);
                    this.dynStats("lib", .5, "lus", 10);
                }
                //Chance of ball growth if not 3" yet
                if (Mutations.rand(2) == 0 && changes < changeLimit && player.ballSize <= 3 && player.horseCocks() > 0) {
                    if (player.balls == 0) {
                        player.balls = 2;
                        player.ballSize = 1;
                        this.outputText("\n\nA nauseating pressure forms just under the base of your maleness.  With agonizing pain the flesh bulges and distends, pushing out a rounded lump of flesh that you recognize as a testicle!  A moment later relief overwhelms you as the second drops into your newly formed sack.", false);
            ...
            if (player.gender == 2 || player.gender == 3) {
                ...
                if (player.vaginas.length == 1) {
                    if (player.vaginas[0].vaginalLooseness <= appearanceDefs_1.VAGINA_LOOSENESS_GAPING && changes < changeLimit && Mutations.rand(2) == 0) {
                        this.outputText("\n\nYou grip your gut in pain as you feel your organs shift slightly.  When the pressure passes, you realize your " + this.vaginaDescript(0) + " has grown larger, in depth AND size.", false);
                        player.vaginas[0].vaginalLooseness++;
                        changes++;
                    }
                    if (player.vaginas[0].vaginalWetness <= appearanceDefs_1.VAGINA_WETNESS_NORMAL && changes < changeLimit && Mutations.rand(2) == 0) {
                        this.outputText("\n\nYour " + this.vaginaDescript(0) + " moistens perceptably, giving off an animalistic scent.", false);
                ...
                //Multicooch
                    ...
                    if (player.vaginas[temp].vaginalWetness <= appearanceDefs_1.VAGINA_WETNESS_NORMAL && changes < changeLimit && Mutations.rand(2) == 0) {
                        this.outputText("\n\nOne of your " + this.vaginaDescript(temp) + " moistens perceptably, giving off an animalistic scent.", false);
                    ...
                    if (player.vaginas[0].vaginalLooseness <= appearanceDefs_1.VAGINA_LOOSENESS_GAPING && changes < changeLimit && Mutations.rand(2) == 0) {
                        this.outputText("\n\nYou grip your gut in pain as you feel your organs shift slightly.  When the pressure passes, you realize one of your " + this.vaginaDescript(temp) + " has grown larger, in depth AND size.", false);
                        player.vaginas[temp].vaginalLooseness++;
                        changes++;
                    }
                }
                if (player.statusAffectv2(StatusAffects_1.StatusAffects.Heat) < 30 && Mutations.rand(2) == 0 && changes < changeLimit) {
                    ...
                    if (Mutations.rand(2) == 0 && changes < changeLimit) {
                        ...
                        if (player.breastRows.length == 1) {
                            ...
                            if (player.breastRows[0].breastRating > 3) {
                                ...
                                //Talk about shrinkage
                                if (temp == 1)
                                    this.outputText("\n\nYou feel a weight lifted from you, and realize your " + this.breastDescript(0) + " have shrunk to a " + player.breastCup(0) + ".", false);
                                if (temp == 2)
                                    this.outputText("\n\nYou feel significantly lighter.  Looking down, you realize your breasts are MUCH smaller, down to " + player.breastCup(0) + "s.", false);
                                changes++;
                            }
            ...
            //Tail -> Ears -> Fur -> Face
            //Centaur if hooved
            if (changes < changeLimit && Mutations.rand(6) == 0 && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HOOFED) {
                changes++;
                ...
                if (player.gender > 0) {
                    this.outputText("  After taking a moment to get used to your new body, you notice that your genitals now reside between the back legs on your centaur body.", false);
                }
                this.dynStats("spe", 3);
                player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_CENTAUR;
            }
            //Remove odd eyes
            if (changes < changeLimit && Mutations.rand(5) == 0 && player.eyeType > appearanceDefs_1.EYES_HUMAN) {
                if (player.eyeType == appearanceDefs_1.EYES_BLACK_EYES_SAND_TRAP) {
                    ...
                    if (player.eyeType == appearanceDefs_1.EYES_FOUR_SPIDER_EYES)
                        this.outputText("  Your multiple, arachnid eyes are gone!</b>", false);
                    this.outputText("  <b>You have normal, humanoid eyes again.</b>", false);
                }
                player.eyeType = appearanceDefs_1.EYES_HUMAN;
                changes++;
            }
            //HorseFace - Req's Fur && Ears
            if (player.faceType != appearanceDefs_1.FACE_HORSE && player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && changes < changeLimit &&
                Mutations.rand(5) == 0 && player.earType == appearanceDefs_1.EARS_HORSE) {
                if (player.faceType == appearanceDefs_1.FACE_DOG)
                ...
                changes++;
                player.faceType = appearanceDefs_1.FACE_HORSE;
            }
            //Fur - if has horsetail && ears and not at changelimit
            if (player.skinType != appearanceDefs_1.SKIN_TYPE_FUR && changes < changeLimit &&
                Mutations.rand(4) == 0 && player.tailType == appearanceDefs_1.TAIL_TYPE_HORSE) {
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_PLAIN)
                ...
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES) {
                    player.skinDesc = "fur";
                ...
                changes++;
                player.skinType = appearanceDefs_1.SKIN_TYPE_FUR;
                player.skinDesc = "fur";
            }
            //Ears - requires tail
            if (player.earType != appearanceDefs_1.EARS_HORSE && player.tailType == appearanceDefs_1.TAIL_TYPE_HORSE && changes < changeLimit &&
                Mutations.rand(3) == 0) {
                if (player.earType == -1)
                    this.outputText("\n\nTwo painful lumps sprout on the top of your head, forming into tear-drop shaped ears, covered with short fur.  ", false);
                if (player.earType == appearanceDefs_1.EARS_HUMAN)
                ...
                if (player.earType == appearanceDefs_1.EARS_DOG)
                ...
                player.earType = appearanceDefs_1.EARS_HORSE;
                player.earValue = 0;
                this.outputText("<b>You now have horse ears.</b>", false);
                changes++;
            }
            //Tail - no-prereq
            if (player.tailType != appearanceDefs_1.TAIL_TYPE_HORSE && Mutations.rand(2) == 0 && changes < changeLimit) {
                //no tail
                ...
                if (player.tailType > appearanceDefs_1.TAIL_TYPE_HORSE && player.tailType <= appearanceDefs_1.TAIL_TYPE_COW) {
                    this.outputText("\n\nPain lances up your " + this.assholeDescript() + " as your tail shifts and morphs disgustingly.  With one last wave of pain, it splits into hundreds of tiny filaments, transforming into a horsetail.", false);
                }
                //if bee/spider-butt.
                if ((player.tailType > appearanceDefs_1.TAIL_TYPE_COW && player.tailType < appearanceDefs_1.TAIL_TYPE_SHARK)) {
                ...
                if (player.tailType >= appearanceDefs_1.TAIL_TYPE_SHARK) {
                    this.outputText("\n\nPain lances up your " + this.assholeDescript() + " as your tail shifts and morphs disgustingly.  With one last wave of pain, it splits into hundreds of tiny filaments, transforming into a horsetail.", false);
                }
                this.outputText("  <b>You now have a horse-tail.</b>", false);
                player.tailType = appearanceDefs_1.TAIL_TYPE_HORSE;
                player.tailVenom = 0;
            ...
            if (Mutations.rand(3) == 0)
                this.outputText(player.modTone(60, 1), false);
            //FAILSAFE CHANGE
            if (changes == 0) {
        ...
        succubiMilk(tainted, player) {
            ...
            if (rando >= 90 && !tainted)
                rando -= 10;
            if (player.cor < 35)
                this.outputText("You wonder why in the gods' names you would drink such a thing, but you have to admit, it is the best thing you have ever tasted.", true);
            if (player.cor >= 35 && player.cor < 70) {
                this.outputText("You savor the incredible flavor as you greedily gulp it down.", true);
                if (player.gender == 2 || player.gender == 3) {
                    this.outputText("  The taste alone makes your " + this.vaginaDescript(0) + " feel ", false);
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_DRY)
                        this.outputText("tingly.", false);
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_NORMAL)
                        this.outputText("wet.", false);
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_WET)
                        this.outputText("sloppy and wet.", false);
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_SLICK)
                        this.outputText("sopping and juicy.", false);
                    if (player.vaginas[0].vaginalWetness >= appearanceDefs_1.VAGINA_WETNESS_DROOLING)
                        this.outputText("dripping wet.", false);
                }
                else if (player.hasCock())
                    this.outputText("  You feel a building arousal, but it doesn't affect your cock.", false);
            }
            if (player.cor >= 70) {
                this.outputText("You pour the milk down your throat, chugging the stuff as fast as you can.  You want more.", true);
                if (player.gender == 2 || player.gender == 3) {
                    this.outputText("  Your " + this.vaginaDescript(0), false);
                    if (player.vaginas.length > 1)
                        this.outputText(" quiver in orgasm, ", false);
                    if (player.vaginas.length == 1)
                        this.outputText(" quivers in orgasm, ", false);
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_DRY)
                        this.outputText("becoming slightly sticky.", false);
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_NORMAL)
                        this.outputText("leaving your undergarments sticky.", false);
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_WET)
                        this.outputText("wet with girlcum.", false);
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_SLICK)
                        this.outputText("staining your undergarments with cum.", false);
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_DROOLING)
                        this.outputText("leaving cunt-juice trickling down your leg.", false);
                    if (player.vaginas[0].vaginalWetness >= appearanceDefs_1.VAGINA_WETNESS_SLAVERING)
                        this.outputText("spraying your undergarments liberally with slick girl-cum.", false);
                    player.orgasm();
                }
                else if (player.gender != 0) {
                    if (player.cocks.length == 1)
                        this.outputText("  You feel a strange sexual pleasure, but your " + this.multiCockDescript() + " remains unaffected.", false);
                    else
                        this.outputText("  You feel a strange sexual pleasure, but your " + this.multiCockDescript() + " remain unaffected.", false);
                }
            }
            if (tainted)
                this.dynStats("spe", 1, "lus", 3, "cor", 1);
            else
                this.dynStats("spe", 1, "lus", 3);
            //Breast growth (maybe cock reduction!)
            if (rando <= 75) {
                //Temp stores the level of growth...
                this.temp = 1 + Mutations.rand(3);
                if (player.breastRows.length > 0) {
                    if (player.breastRows[0].breastRating < 2 && Mutations.rand(3) == 0)
                        this.temp++;
                    if (player.breastRows[0].breastRating < 5 && Mutations.rand(4) == 0)
                        this.temp++;
                    if (player.breastRows[0].breastRating < 6 && Mutations.rand(5) == 0)
                        this.temp++;
                }
                this.outputText("\n\n", false);
                player.growTits(this.temp, player.breastRows.length, true, 3);
                if (player.breastRows.length == 0) {
                    this.outputText("A perfect pair of B cup breasts, complete with tiny nipples, form on your chest.", false);
                    player.createBreastRow();
                    player.breastRows[0].breasts = 2;
                ...
                if (!this.flags[kFLAGS_1.kFLAGS.HYPER_HAPPY]) {
                    ...
                    if (player.cocks.length > 0) {
                        ...
                        //Shrink said cock
                        if (player.cocks[this.temp].cockLength < 6 && player.cocks[this.temp].cockLength >= 2.9) {
                            player.cocks[this.temp].cockLength -= .5;
                            temp3 -= .5;
                            if (player.cocks[this.temp].cockThickness * 6 > player.cocks[this.temp].cockLength)
                                player.cocks[this.temp].cockThickness -= .2;
                            if (player.cocks[this.temp].cockThickness * 8 > player.cocks[this.temp].cockLength)
                                player.cocks[this.temp].cockThickness -= .2;
                            if (player.cocks[this.temp].cockThickness < .5)
                                player.cocks[this.temp].cockThickness = .5;
                        }
            ...
            if (player.vaginas.length == 0 && (Mutations.rand(3) == 0 || (rando > 75 && rando < 90))) {
                player.createVagina();
                player.vaginas[0].vaginalLooseness = appearanceDefs_1.VAGINA_LOOSENESS_TIGHT;
                player.vaginas[0].vaginalWetness = appearanceDefs_1.VAGINA_WETNESS_NORMAL;
                player.vaginas[0].virgin = true;
                player.clitLength = .25;
                if (player.fertility <= 5)
                    player.fertility = 6;
                this.outputText("\n\nAn itching starts in your crotch and spreads vertically.  You reach down and discover an opening.  You have grown a <b>new " + this.vaginaDescript(0) + "</b>!", false);
            }
            //Increase pussy wetness or grow one!!
            else if (rando > 75 && rando < 90) {
                //Shrink cawk
                if (player.cocks.length > 0 && !this.flags[kFLAGS_1.kFLAGS.HYPER_HAPPY]) {
                    this.outputText("\n\n", false);
                    this.temp = 0;
                    temp2 = player.cocks.length;
                    //Find biggest cock
                ...
                if (player.vaginas.length > 0) {
                    ...
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_SLAVERING) {
                        if (player.vaginas.length == 1)
                            this.outputText("Your " + this.vaginaDescript(0) + " gushes fluids down your leg as you spontaneously orgasm.", false);
                        else
                            this.outputText("Your " + this.vaginaDescript(0) + "s gush fluids down your legs as you spontaneously orgasm, leaving a thick puddle of pussy-juice on the ground.  It is rapidly absorbed by the earth.", false);
                        player.orgasm();
                        if (tainted)
                            this.dynStats("cor", 1);
                    }
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_DROOLING) {
                        if (player.vaginas.length == 1)
                            this.outputText("Your pussy feels hot and juicy, aroused and tender.  You cannot resist as your hands dive into your " + this.vaginaDescript(0) + ".  You quickly orgasm, squirting fluids everywhere.  <b>You are now a squirter</b>.", false);
                        if (player.vaginas.length > 1)
                            this.outputText("Your pussies feel hot and juicy, aroused and tender.  You cannot resist plunging your hands inside your " + this.vaginaDescript(0) + "s.  You quiver around your fingers, squirting copious fluids over yourself and the ground.  The fluids quickly disappear into the dirt.", false);
                        player.orgasm();
                        if (tainted)
                            this.dynStats("cor", 1);
                    }
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_SLICK) {
                        if (player.vaginas.length == 1)
                            this.outputText("You feel a sudden trickle of fluid down your leg.  You smell it and realize it's your pussy-juice.  Your " + this.vaginaDescript(0) + " now drools lubricant constantly down your leg.", false);
                        if (player.vaginas.length > 1)
                            this.outputText("You feel sudden trickles of fluids down your leg.  You smell the stuff and realize it's your pussies-juices.  They seem to drool lubricant constantly down your legs.", false);
                    }
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_WET) {
                        this.outputText("You flush in sexual arousal as you realize how moist your cunt-lips have become.  Once you've calmed down a bit you realize they're still slick and ready to fuck, and always will be.", false);
                    }
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_NORMAL) {
                        if (player.vaginas.length == 1)
                            this.outputText("A feeling of intense arousal passes through you, causing you to masturbate furiously.  You realize afterwards that your " + this.vaginaDescript(0) + " felt much wetter than normal.", false);
                        else
                            this.outputText("A feeling of intense arousal passes through you, causing you to masturbate furiously.  You realize afterwards that your " + this.vaginaDescript(0) + " were much wetter than normal.", false);
                    }
                    if (player.vaginas[0].vaginalWetness == appearanceDefs_1.VAGINA_WETNESS_DRY) {
                        this.outputText("You feel a tingling in your crotch, but cannot identify it.", false);
                    }
            ...
            if (rando >= 90) {
                if (player.skinTone == "blue" || player.skinTone == "purple" || player.skinTone == "indigo" || player.skinTone == "shiny black") {
                    if (player.vaginas.length > 0) {
                        ...
                        if (player.clitLength > 3 && player.findPerk(PerkLib_1.PerkLib.BigClit) < 0)
                            this.outputText("  After some time it shrinks, returning to its normal aroused size.  You guess it can't get any bigger.", false);
                        if (player.clitLength > 5 && player.findPerk(PerkLib_1.PerkLib.BigClit) >= 0)
                            this.outputText("  Eventually it shrinks back down to its normal (but still HUGE) size.  You guess it can't get any bigger.", false);
                        if (((player.findPerk(PerkLib_1.PerkLib.BigClit) >= 0) && player.clitLength < 6)
                            || player.clitLength < 3) {
                            this.temp += 2;
                    ...
                    if (this.temp == 3 || this.temp == 4 || this.temp == 5)
            ...
            if (Mutations.rand(40) + player.cor / 3 > 35 && tainted)
                this.demonChanges(player);
            if (tainted) {
                this.outputText(player.modFem(100, 2), false);
                if (Mutations.rand(3) == 0)
                    this.outputText(player.modTone(15, 2), false);
            }
            else {
                this.outputText(player.modFem(90, 1), false);
                if (Mutations.rand(3) == 0)
                    this.outputText(player.modTone(20, 2), false);
            }
            player.genderCheck();
        }
        ...
        caninePepper(type, player) {
            ...
            if (type == 0) {
                if (Mutations.rand(100) < 15) {
                    ...
                    this.outputText("The pepper tastes particularly potent, searingly hot and spicy.", false);
                }
                else
                    this.outputText("The pepper is strangely spicy but very tasty.", false);
            }
            //Oversized pepper
            if (type == 1) {
                crit = Math.floor(Math.random() * 20) / 10 + 2;
                this.outputText("The pepper is so large and thick that you have to eat it in several large bites.  It is not as spicy as the normal ones, but is delicious and flavorful.", false);
            }
            //Double Pepper
            if (type == 2) {
                crit = Math.floor(Math.random() * 20) / 10 + 2;
                this.outputText("The double-pepper is strange, looking like it was formed when two peppers grew together near their bases.", false);
            }
            //Black Pepper
            if (type == 3) {
                crit = Math.floor(Math.random() * 20) / 10 + 2;
                this.outputText("This black pepper tastes sweet, but has a bit of a tangy aftertaste.", false);
            }
            //Knotty Pepper
            if (type == 4) {
                crit = Math.floor(Math.random() * 20) / 10 + 2;
                this.outputText("The pepper is a bit tough to eat due to the swollen bulge near the base, but you manage to cram it down and munch on it.  It's extra spicy!", false);
            }
            //Bulbous Pepper
            if (type == 5) {
                crit = Math.floor(Math.random() * 20) / 10 + 2;
                this.outputText("You eat the pepper, even the two orb-like growths that have grown out from the base.  It's delicious!", false);
            }
            //OVERDOSE Bad End!
            if (type <= 0 && crit > 1 && player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && player.faceType == appearanceDefs_1.FACE_DOG && player.earType == appearanceDefs_1.EARS_DOG && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DOG && player.tailType == appearanceDefs_1.TAIL_TYPE_DOG && Mutations.rand(2) == 0 && player.findStatusAffect(StatusAffects_1.StatusAffects.DogWarning) >= 0) {
                ...
                if (this.temp == 0) {
                    ...
                    if (player.findPerk(PerkLib_1.PerkLib.MarblesMilk) >= 0)
                        this.outputText("All you know is that there is a scent on the wind, it is time to hunt, and at the end of the day you need to come home for your milk.", false);
                    else
                        this.outputText("All you know is that there is a scent on the wind, and it is time to hunt.", false);
                }
                if (this.temp == 1)
            ...
            if (type <= 0 && player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && player.faceType == appearanceDefs_1.FACE_DOG && player.tailType == appearanceDefs_1.TAIL_TYPE_DOG && player.earType == appearanceDefs_1.EARS_DOG && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DOG && player.findStatusAffect(StatusAffects_1.StatusAffects.DogWarning) >= 0 && Mutations.rand(3) == 0) {
            ...
            if (type <= 0 && player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && player.faceType == appearanceDefs_1.FACE_DOG && player.tailType == appearanceDefs_1.TAIL_TYPE_DOG && player.earType == appearanceDefs_1.EARS_DOG && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DOG && player.findStatusAffect(StatusAffects_1.StatusAffects.DogWarning) < 0) {
                player.createStatusAffect(StatusAffects_1.StatusAffects.DogWarning, 0, 0, 0, 0);
                this.outputText("<b>\n\nEating the pepper, you realize how dog-like you've become, and you wonder what else the peppers could change...</b>", false);
            }
            if (type == 3) {
                this.dynStats("lib", 2 + Mutations.rand(4), "lus", 5 + Mutations.rand(5), "cor", 2 + Mutations.rand(4));
                ...
                if (player.cor < 50)
                    this.outputText("  You shake your head, blushing hotly.  Where did that thought come from?", false);
            }
            ...
            if (player.inte > 30 && Mutations.rand(3) == 0 && changes < changeLimit && type != 3) {
                ...
                this.outputText("\n\nYou feel ", false);
                if (crit > 1)
                    this.outputText("MUCH ", false);
                this.outputText("dumber.", false);
                changes++;
            }
            //-Remove feather-arms (copy this for goblin ale, mino blood, equinum, canine pepps, demon items)
            if (changes < changeLimit && player.armType == appearanceDefs_1.ARM_TYPE_HARPY && Mutations.rand(4) == 0) {
                ...
                //If player doesnt have 2 dogdicks
                    ...
                    if (player.dogCocks() == 0) {
                        ...
                        if (player.cockTotal() == 0) {
                            ...
                            player.cocks[0].cockType = CockTypesEnum_1.CockTypesEnum.DOG;
                            player.cocks[1].knotMultiplier = 1.7;
                            player.cocks[1].cockType = CockTypesEnum_1.CockTypesEnum.DOG;
                            this.dynStats("lus", 50);
                        }
                        //1 dick - grow 1 and convert 1
                        else if (player.cockTotal() == 1) {
                            this.outputText("\n\nYour " + this.cockDescript(0) + " vibrates, the veins clearly visible as it reddens and distorts.  The head narrows into a pointed tip while a gradually widening bulge forms around the base.  Where it meets your crotch, the skin bunches up around it, forming a canine-like sheath.  ", false);
                            player.cocks[0].cockType = CockTypesEnum_1.CockTypesEnum.DOG;
                    ...
                    //If player has 1 dogdicks
                        ...
                        if (player.cockTotal() >= 1) {
                            ...
                            if (player.cocks[0].cockType == CockTypesEnum_1.CockTypesEnum.DOG) {
                                this.outputText("\n\nYour crotch twitches, and you pull open your " + player.armorName + " to get a better look.  You watch in horror and arousal as your " + this.cockDescript(1) + " warps and twists, becoming red and pointed, just like other dog-dick, growing thick bulges near the base.  When it stops you have two dog-cocks and an animal-like sheath.  The whole episode turns you on far more than it should, leaving you dripping animal pre and ready to breed.", false);
                                player.cocks[1].cockType = CockTypesEnum_1.CockTypesEnum.DOG;
                                player.cocks[1].knotMultiplier = 1.4;
                            }
                            //first dick is not dog
                            else {
                                this.outputText("\n\nYour crotch twitches, and you pull open your " + player.armorName + " to get a better look.  You watch in horror and arousal as your " + this.cockDescript(0) + " warps and twists, becoming red and pointed, just like other dog-dick, growing thick bulges near the base.  When it stops you have two dog-cocks and an animal-like sheath.  The whole episode turns you on far more than it should, leaving you dripping animal pre and ready to breed.", false);
                                player.cocks[0].cockType = CockTypesEnum_1.CockTypesEnum.DOG;
                                player.cocks[0].knotMultiplier = 1.4;
                            }
                            this.dynStats("lib", 2, "lus", 50);
                ...
                if (player.cockTotal() > 0) {
                    ...
                    if (player.dogCocks() > 0) {
                        ...
                        while (this.temp < player.cocks.length) {
                            if (player.cocks[this.temp].cockType == CockTypesEnum_1.CockTypesEnum.DOG) {
                                temp2 = this.temp;
                                break;
                            }
                            else
                        ...
                        while (this.temp > 0) {
                            this.temp--;
                            if (player.cocks[this.temp].cockType == CockTypesEnum_1.CockTypesEnum.DOG && player.cocks[this.temp].knotMultiplier < player.cocks[temp2].knotMultiplier)
                                temp2 = this.temp;
                        }
                        ...
                        this.outputText("\n\n", false);
                        if (temp3 < .06)
                            this.outputText("Your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.DOG) + " feels unusually tight in your sheath as your knot grows.", false);
                        if (temp3 >= .06 && temp3 <= .12)
                            this.outputText("Your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.DOG) + " pops free of your sheath, thickening nicely into a bigger knot.", false);
                        if (temp3 > .12)
                            this.outputText("Your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.DOG) + " surges free of your sheath, swelling thicker with each passing second.  Your knot bulges out at the base, growing far beyond normal.", false);
                        this.dynStats("sen", .5, "lus", 5 * crit);
                    }
                    //Grow dogdick with big knot
                    else {
                        this.outputText("\n\nYour " + this.cockDescript(0) + " twitches, reshaping itself.  The crown tapers down to a point while the base begins swelling.  It isn't painful in the slightest, actually kind of pleasant.  Your dog-like knot slowly fills up like a balloon, eventually stopping when it's nearly twice as thick as the rest.  You touch and shiver with pleasure, oozing pre-cum.", false);
                        player.cocks[0].cockType = CockTypesEnum_1.CockTypesEnum.DOG;
            ...
            if (player.cocks.length > 0) {
                ...
                if (type != 4 && player.dogCocks() > 0 && ((changes < changeLimit && Mutations.rand(1.4) == 0) || type == 1)) {
                    this.temp = 0;
                    //set temp2 to first dogdick for initialization
                    while (this.temp < player.cocks.length) {
                        if (player.cocks[this.temp].cockType == CockTypesEnum_1.CockTypesEnum.DOG) {
                    ...
                    player.cocks[temp2].knotMultiplier += (temp3);
                    if (temp3 < .06)
                        this.outputText("\n\nYour " + this.cockDescript(temp2) + " feels unusually tight in your sheath as your knot grows.", false);
                    if (temp3 >= .06 && temp3 <= .12)
                        this.outputText("\n\nYour " + this.cockDescript(temp2) + " pops free of your sheath, thickening nicely into a bigger knot.", false);
                    if (temp3 > .12)
                        this.outputText("\n\nYour " + this.cockDescript(temp2) + " surges free of your sheath, swelling thicker with each passing second.  Your knot bulges out at the base, growing far beyond normal.", false);
                    this.dynStats("sen", .5, "lus", 5 * crit);
                    changes++;
                }
                //Cock Xform if player has free cocks.
                if (player.dogCocks() < player.cocks.length && ((changes < changeLimit && Mutations.rand(1.6)) || type == 1) == 0) {
                    ...
                    //Talk about it
                    //Hooooman
                    if (player.cocks[temp3].cockType == CockTypesEnum_1.CockTypesEnum.HUMAN) {
                        this.outputText("\n\nYour " + this.cockDescript(temp3) + " clenches painfully, becoming achingly, throbbingly erect.  A tightness seems to squeeze around the base, and you wince as you see your skin and flesh shifting forwards into a canine-looking sheath.  You shudder as the crown of your " + this.cockDescript(temp3) + " reshapes into a point, the sensations nearly too much for you.  You throw back your head as the transformation completes, your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.DOG) + " much thicker than it ever was before.  <b>You now have a dog-cock.</b>", false);
                        this.dynStats("sen", 10, "lus", 5 * crit);
                    }
                    //Horse
                    if (player.cocks[temp3].cockType == CockTypesEnum_1.CockTypesEnum.HORSE) {
                        this.outputText("\n\nYour " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.HORSE) + " shrinks, the extra equine length seeming to shift into girth.  The flared tip vanishes into a more pointed form, a thick knotted bulge forming just above your sheath.  <b>You now have a dog-cock.</b>", false);
                    ...
                    //Tentacular Tuesday!
                    if (player.cocks[temp3].cockType == CockTypesEnum_1.CockTypesEnum.TENTACLE) {
                        this.outputText("\n\nYour " + this.cockDescript(temp3) + " coils in on itself, reshaping and losing its plant-like coloration as it thickens near the base, bulging out in a very canine-looking knot.  Your skin bunches painfully around the base, forming into a sheath.  <b>You now have a dog-cock.</b>", false);
                        this.dynStats("sen", 4, "lus", 5 * crit);
                    }
                    //Misc
                    ...
                    this.temp = 0;
                    //Demon
                    if (player.cocks[temp3].cockType == CockTypesEnum_1.CockTypesEnum.DEMON) {
                        this.outputText("\n\nYour " + this.cockDescript(temp3) + " color shifts red for a moment and begins to swell at the base, but within moments it smooths out, retaining its distinctive demonic shape, only perhaps a bit thicker.", false);
                        this.dynStats("sen", 1, "lus", 2 * crit);
                        this.temp = 1;
                    }
                    //Xform it!
                    player.cocks[temp3].cockType = CockTypesEnum_1.CockTypesEnum.DOG;
                    player.cocks[temp3].knotMultiplier = 1.1;
                    player.cocks[temp3].thickenCock(2);
                    if (this.temp == 1) {
                        player.cocks[temp3].cockType = CockTypesEnum_1.CockTypesEnum.DEMON;
                    }
                    changes++;
                }
                ...
                if (player.cumMultiplier < 2 && Mutations.rand(2) == 0 && changes < changeLimit) {
                    ...
                    if (player.findPerk(PerkLib_1.PerkLib.MessyOrgasms) >= 0)
                        this.temp = 2;
                    if (this.temp < player.cumMultiplier + .05 * crit) {
                        changes--;
                    }
                    else {
                        ...
                        if (player.balls > 0)
                            this.outputText("\n\nYou feel a churning in your " + this.ballsDescriptLight() + ".  It quickly settles, leaving them feeling somewhat more dense.", false);
                        if (crit > 1)
                            this.outputText("  A bit of milky pre dribbles from your " + this.multiCockDescriptLight() + ", pushed out by the change.", false);
                    }
                    changes++;
                }
                //Oversized pepper
                if (type == 1) {
                    ...
                    if (temp2 > 2)
                        this.outputText("\n\nYour " + this.cockDescript(this.temp) + " tightens painfully, inches of bulging dick-flesh pouring out from your crotch as it grows longer.  Thick pre forms at the pointed tip, drawn out from the pleasure of the change.", false);
                    if (temp2 > 1 && temp2 <= 2)
                        this.outputText("\n\nAching pressure builds within your crotch, suddenly releasing as an inch or more of extra dick-flesh spills out.  A dollop of pre beads on the head of your enlarged " + this.cockDescript(this.temp) + " from the pleasure of the growth.", false);
                    if (temp2 <= 1)
                        this.outputText("\n\nA slight pressure builds and releases as your " + this.cockDescript(this.temp) + " pushes a bit further out of your crotch.", false);
                }
            }
            //Female Stuff
            //Multiboobages
            if (player.breastRows.length > 0) {
                ...
                if (player.breastRows[0].breastRating > 0 && player.vaginas.length > 0) {
                    ...
                    if (player.breastRows.length < 3 && Mutations.rand(2) == 0 && changes < changeLimit) {
                        player.createBreastRow();
                        ...
                        if (player.vaginas.length > 0 && player.breastRows[0].breastRating <= player.breastRows.length) {
                            this.outputText("\n\nYour " + this.breastDescript(0) + " feel constrained and painful against your top as they grow larger by the moment, finally stopping as they reach ", false);
                            player.breastRows[0].breastRating += 2;
                            this.outputText(player.breastCup(0) + " size.  But it doesn't stop there, you feel a tightness beginning lower on your torso...", false);
                            changes++;
                        }
                        //Had 1 row to start
                        if (player.breastRows.length == 2) {
                            //1 size below primary breast row!
                            player.breastRows[this.temp].breastRating = player.breastRows[0].breastRating - 1;
                            if (player.breastRows[0].breastRating - 1 == 0)
                                ...
                                this.outputText("\n\nA second set of breasts bulges forth under your current pair, stopping as they reach " + player.breastCup(this.temp) + "s.", false);
                            this.outputText("  A sensitive nub grows on the summit of each new tit, becoming a new nipple.", false);
                        ...
                        if (player.breastRows.length > 2 && player.breastRows[0].breastRating > player.breastRows.length) {
                            ...
                            player.breastRows[this.temp].breastRating = player.breastRows[this.temp - 1].breastRating - 1;
                            ...
                            if (player.breastRows[this.temp].breastRating == 0)
                                ...
                                this.outputText("\n\nYour abdomen tingles and twitches as a new row of " + player.breastCup(this.temp) + " " + this.breastDescript(this.temp) + " sprouts below your others.", false);
                            this.outputText("  A sensitive nub grows on the summit of each new tit, becoming a new nipple.", false);
                            changes++;
                        }
                        ...
                        if (crit > 1) {
                            if (crit > 2) {
                                this.outputText("  You heft your new chest experimentally, exploring the new flesh with tender touches.  Your eyes nearly roll back in your head from the intense feelings.", false);
                                this.dynStats("sen", 6, "lus", 15, "cor", 0);
                            }
                            else {
                                this.outputText("  You touch your new nipples with a mixture of awe and desire, the experience arousing beyond measure.  You squeal in delight, nearly orgasming, but in time finding the willpower to stop yourself.", false);
                    ...
                    //If already has max doggie breasts!
                        ...
                        while (this.temp > 1 && temp2 == 0) {
                            this.temp--;
                            //Gimme a sec
                            if (player.breastRows[this.temp].breastRating + 1 < player.breastRows[this.temp - 1].breastRating) {
                                if (!evened) {
                                    evened = true;
                                    this.outputText("\n", false);
                                }
                                ...
                                if (this.temp == 2)
                                    this.outputText("third ", false);
                                if (this.temp == 3)
                                    this.outputText("fourth ", false);
                                if (this.temp == 4)
                                    this.outputText("fifth ", false);
                                if (this.temp > 4)
                                    this.outputText("", false);
                                this.outputText("row of " + this.breastDescript(this.temp) + " grows larger, as if jealous of the jiggling flesh above.", false);
                                temp2 = (player.breastRows[this.temp - 1].breastRating) - player.breastRows[this.temp].breastRating - 1;
                                if (temp2 > 5)
                                    temp2 = 5;
                                if (temp2 < 1)
                                    temp2 = 1;
            ...
            //Grow tits if have NO breasts/nipples AT ALL
                ...
                this.outputText("  A sensitive nub grows on the summit of each tit, becoming a new nipple.", false);
                player.createBreastRow();
                player.breastRows[0].breastRating = 2;
                player.breastRows[0].breasts = 2;
            ...
            if (changes < changeLimit && player.dogScore() >= 3 && Mutations.rand(4) == 0) {
                changes++;
                this.outputText("\n\n", false);
                this.outputText("Images and thoughts come unbidden to your mind, overwhelming your control as you rapidly lose yourself in them, daydreaming of... ", false);
                //cawk fantasies
                if (player.gender <= 1 || (player.gender == 3 && Mutations.rand(2) == 0)) {
                    this.outputText("bounding through the woods, hunting with your master.  Feeling the wind in your fur and the thrill of the hunt coursing through your veins intoxicates you.  You have your nose to the ground, tracking your quarry as you run, until a heavenly scent stops you in your tracks.", false);
                    ...
                    if (player.cor < 33 || !player.hasCock())
                        ...
                        this.outputText("  Heart pounding, your shaft pops free of its sheath on instinct, as you take off after the new scent.  Caught firmly in the grip of a female's heat, you ignore your master's cry as you disappear into the wild, " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.DOG) + " growing harder as you near your quarry.  You burst through a bush, spotting a white-furred female.  She drops, exposing her dripping fem-sex to you, the musky scent of her sex channeling straight through your nose and sliding into your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.DOG) + ".", false);
                        this.dynStats("lus", 5 + player.lib / 20);
                        //Break 2
                        if (player.cor < 66)
                            this.outputText("\nYou blink a few times, the fantasy fading as you master yourself.  That daydream was so strange, yet so hot.", false);
                        else {
                            this.outputText("  Unable to wait any longer, you mount her, pressing your bulging knot against her vulva as she yips in pleasure. The heat of her sex is unreal, the tight passage gripping you like a vice as you jackhammer against her, biting her neck gently in spite of the violent pounding.", false);
                            this.dynStats("lus", 5 + player.lib / 20);
                            //break3
                            if (player.cor < 80) {
                                if (player.vaginas.length > 0)
                                    this.outputText("\nYou reluctantly pry your hand from your aching " + this.vaginaDescript(0) + " as you drag yourself out of your fantasy.", false);
                                else
                                    this.outputText("\nYou reluctantly pry your hand from your aching " + this.cockDescript(0) + " as you drag yourself out of your fantasy.", false);
                            }
                            else {
                                this.outputText("  At last your knot pops into her juicy snatch, splattering her groin with a smattering of her arousal.  The scents of your mating reach a peak as the velvet vice around your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.DOG) + " quivers in the most indescribably pleasant way.  You clamp down on her hide as your whole body tenses, unleashing a torrent of cum into her sex.  Each blast is accompanied by a squeeze of her hot passage, milking you of the last of your spooge.  Your " + player.legs() + " give out as your fantasy nearly brings you to orgasm, the sudden impact with the ground jarring you from your daydream.", false);
                                this.dynStats("lus", 5 + player.lib / 20);
                            }
                        }
                    }
                ...
                //Pure female fantasies
                    ...
                    this.outputText("wagging your dripping " + this.vaginaDescript(0) + " before a pack of horny wolves, watching their shiny red doggie-pricks practically jump out of their sheaths at your fertile scent.", false);
                    this.dynStats("lus", 5 + player.lib / 20);
                    //BREAK 1
                    if (player.cor < 33) {
                        this.outputText("\nYou shake your head to clear the unwanted fantasy from your mind, repulsed by it.", false);
                        ...
                        this.outputText("  In moments they begin their advance, plunging their pointed beast-dicks into you, one after another.  You yip and howl with pleasure as each one takes his turn knotting you.", false);
                        this.dynStats("lus", 5 + player.lib / 20);
                        //BREAK 2
                        if (player.cor <= 66) {
                            this.outputText("\nYou blink a few times, the fantasy fading as you master yourself.  That daydream was so strange, yet so hot.", false);
                            ...
                            this.outputText("  The feeling of all that hot wolf-spooge spilling from your overfilled snatch and running down your thighs is heavenly, nearly making you orgasm on the spot.  You see the alpha of the pack is hard again, and his impressive member is throbbing with the need to breed you.", false);
                            this.dynStats("lus", 5 + player.lib / 20);
                            //break3
                            if (player.cor < 80) {
                                this.outputText("\nYou reluctantly pry your hand from your aching " + this.vaginaDescript(0) + " as you drag yourself out of your fantasy.", false);
                    ...
                    if (player.cor < 33) {
                        ...
                        if (player.cor <= 66) {
                            ...
                            this.outputText("  The feeling of all that hot wolf-spooge spilling from your overfilled ass and running down your thighs is heavenly, nearly making you orgasm on the spot.  You see the alpha of the pack is hard again, and his impressive member is throbbing with the need to spend his lust on you.", false);
                            this.dynStats("lus", 5 + player.lib / 20);
                            //break3
                            if (player.cor < 80) {
            ...
            if (Mutations.rand(5) == 0 && changes < changeLimit &&
                player.faceType != appearanceDefs_1.FACE_DOG && player.skinType == appearanceDefs_1.SKIN_TYPE_FUR &&
                player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DOG) {
                if (player.faceType == appearanceDefs_1.FACE_HORSE)
                ...
                player.faceType = appearanceDefs_1.FACE_DOG;
                changes++;
            }
            ...
            if (Mutations.rand(4) == 0 && changes < changeLimit &&
                player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DOG && player.tailType == appearanceDefs_1.TAIL_TYPE_DOG &&
                player.skinType != appearanceDefs_1.SKIN_TYPE_FUR) {
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_PLAIN)
            ...
            if (Mutations.rand(3) == 0 && player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_DOG && player.tailType == appearanceDefs_1.TAIL_TYPE_DOG && player.earType == appearanceDefs_1.EARS_DOG && changes < changeLimit) {
                //Feet -> paws
                if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HUMAN)
                    this.outputText("\n\nYou scream in agony as you feel the bones in your feet break and begin to rearrange. <b>You now have paws</b>.", false);
                //Hooves -> Paws
                else if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HOOFED)
            ...
            if (Mutations.rand(2) == 0 && player.earType != appearanceDefs_1.EARS_DOG && player.tailType == appearanceDefs_1.TAIL_TYPE_DOG && changes < changeLimit) {
                if (player.earType == -1)
                ...
                if (player.earType == appearanceDefs_1.EARS_HORSE)
                ...
                player.earType = appearanceDefs_1.EARS_DOG;
                player.earValue = 2;
                this.outputText("<b>You now have dog ears.</b>", false);
                changes++;
            }
            //Grow tail if not dog-tailed
            if (Mutations.rand(3) == 0 && changes < changeLimit && player.tailType != appearanceDefs_1.TAIL_TYPE_DOG) {
                if (player.tailType == appearanceDefs_1.TAIL_TYPE_NONE)
                ...
                if (player.tailType == appearanceDefs_1.TAIL_TYPE_HORSE)
                ...
                if (player.tailType == appearanceDefs_1.TAIL_TYPE_DEMONIC)
                ...
                if (player.tailType >= appearanceDefs_1.TAIL_TYPE_COW)
                    this.outputText("\n\nYou feel your backside shift and change, flesh molding and displacing into a long puffy tail!  ", false);
                changes++;
                player.tailType = appearanceDefs_1.TAIL_TYPE_DOG;
                this.outputText("<b>You now have a dog-tail.</b>", false);
            }
            if (Mutations.rand(4) == 0 && player.gills && changes < changeLimit) {
                this.outputText("\n\nYour chest itches, and as you reach up to scratch it, you realize your gills have withdrawn into your skin.", false);
                player.gills = false;
            ...
            if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && changes < changeLimit && Mutations.rand(3) == 0) {
        ...
        impFood(player) {
            this.outputText("", true);
            if (player.cocks.length > 0) {
                this.outputText("The food tastes strange and corrupt - you can't really think of a better word for it, but it's unclean.", false);
                ...
                if (Mutations.rand(30) == 0 && player.skinTone != "red") {
                    ...
                    this.outputText("begins to lose its color, fading until you're as white as an albino.  Then, starting at the crown of your head, a reddish hue rolls down your body in a wave, turning you completely " + player.skinTone + ".", false);
                }
                return;
            }
            else {
        ...
        succubisDelight(tainted, player) {
            player.slimeFeed();
            var changes = 0;
            var crit = 1;
            ...
            this.outputText("You uncork the bottle and drink down the strange substance, struggling to down the thick liquid.", true);
            //low corruption thoughts
            if (player.cor < 33)
                this.outputText("  This stuff is gross, why are you drinking it?", false);
            //high corruption
            if (player.cor >= 66)
                this.outputText("  You lick your lips, marvelling at how thick and sticky it is.", false);
            //Corruption increase
            ...
            if (changes < changeLimit && Mutations.rand(2) == 0 && player.cocks.length > 0) {
                if (player.cumMultiplier < 6 && Mutations.rand(2) == 0 && changes < changeLimit) {
                    ...
                    if (player.findPerk(PerkLib_1.PerkLib.MessyOrgasms) >= 0)
                        this.temp = 6;
                    if (this.temp < player.cumMultiplier + .4 * crit) {
                        changes--;
                    }
                    else {
            ...
            if (player.balls > 0 && Mutations.rand(3) == 0) {
                this.outputText(player.modFem(12, 3), false);
            }
        }
        succubisDream(player) {
            player.slimeFeed();
            var changes = 0;
            var crit = 1;
            //Determine crit multiplier (x2 or x3)
            ...
            //Makes your balls biggah! (Or cummultiplier higher if futa!)
            if (Mutations.rand(1.5) == 0 && changes < changeLimit && player.balls > 0 && player.cocks.length > 0) {
                player.ballSize++;
                //They grow slower as they get bigger...
                if (player.ballSize > 10)
                    player.ballSize -= .5;
            ...
            if (player.balls > 0 && Mutations.rand(3) == 0) {
                this.outputText(player.modFem(12, 5), false);
            }
        }
        //Oviposition Elixer!
        /*
        ...
        */
        //butt expansion
        brownEgg(large, player) {
            this.outputText("You devour the egg, momentarily sating your hunger.\n\n", true);
            if (!large) {
                this.outputText("You feel a bit of additional weight on your backside as your " + this.buttDescript() + " gains a bit more padding.", true);
                player.buttRating++;
            }
            else {
                this.outputText("Your " + this.buttDescript() + " wobbles, nearly throwing you off balance as it grows much bigger!", true);
                player.buttRating += 2 + Mutations.rand(3);
            }
            if (Mutations.rand(3) == 0) {
                if (large)
                    this.outputText(player.modThickness(100, 8), false);
                else
                    this.outputText(player.modThickness(95, 3), false);
            }
        }
        //hip expansion
        purpleEgg(large, player) {
            this.outputText("You devour the egg, momentarily sating your hunger.\n\n", true);
            if (!large || player.hipRating > 20) {
                this.outputText("You stumble as you feel your " + this.hipDescript() + " widen, altering your gait slightly.", false);
                player.hipRating++;
            }
            else {
                this.outputText("You stagger wildly as your hips spread apart, widening by inches.  When the transformation finishes you feel as if you have to learn to walk all over again.", false);
                player.hipRating += 2 + Mutations.rand(2);
            }
            if (Mutations.rand(3) == 0) {
                if (large)
                    this.outputText(player.modThickness(80, 8), false);
                else
                    this.outputText(player.modThickness(80, 3), false);
            }
        }
        //Femminess
        pinkEgg(large, player) {
            this.outputText("You devour the egg, momentarily sating your hunger.\n\n", true);
            if (!large) {
                //Remove a dick
                if (player.cocks.length > 0) {
                ...
                if (player.balls > 0) {
                    if (player.ballSize > 15) {
                        player.ballSize -= 8;
                        this.outputText("Your scrotum slowly shrinks, settling down at a MUCH smaller size.  <b>Your " + this.ballsDescriptLight() + " are much smaller.</b>\n\n", false);
                    }
                    else {
                        player.balls = 0;
                        player.ballSize = 1;
                ...
                if (player.vaginas.length > 0 && player.fertility < 40) {
                    this.outputText("You feel a tingle deep inside your body, just above your " + this.vaginaDescript(0) + ", as if you were becoming more fertile.\n\n", false);
                    player.fertility += 5;
                }
            }
            //LARGE
                ...
                if (player.vaginas.length > 0 && player.fertility < 70) {
                    this.outputText("You feel a powerful tingle deep inside your body, just above your " + this.vaginaDescript(0) + ". Instinctively you know you have become more fertile.\n\n", false);
                    player.fertility += 10;
                }
            }
            if (Mutations.rand(3) == 0) {
                if (large)
                    this.outputText(player.modFem(100, 8), false);
                else
                    this.outputText(player.modFem(95, 3), false);
            }
        }
        //Maleness
        blueEgg(large, player) {
            var temp2 = 0;
            var temp3 = 0;
            ...
            if (!large) {
                ...
                if (player.cocks.length > 0) {
                    //Multiz
                    if (player.cocks.length > 1) {
                        this.outputText("\n\nYour " + this.multiCockDescript() + " fill to full-size... and begin growing obscenely.", false);
                        this.temp = player.cocks.length;
                        while (this.temp > 0) {
                            this.temp--;
                            temp2 = player.increaseCock(this.temp, Mutations.rand(3) + 2);
                        ...
                        if (temp3 >= 1) {
                            if (player.cocks.length == 1)
                                this.outputText("\n\nYour " + this.multiCockDescriptLight() + " spreads rapidly, swelling an inch or more in girth, making it feel fat and floppy.", false);
                            else
                                this.outputText("\n\nYour " + this.multiCockDescriptLight() + " spread rapidly, swelling as they grow an inch or more in girth, making them feel fat and floppy.", false);
                        }
                        if (temp3 <= .5) {
                            if (player.cocks.length > 1)
                                this.outputText("\n\nYour " + this.multiCockDescriptLight() + " feel swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. They are definitely thicker.", false);
                            else
                                this.outputText("\n\nYour " + this.multiCockDescriptLight() + " feels swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. It is definitely thicker.", false);
                        }
                        if (temp3 > .5 && temp2 < 1) {
                            if (player.cocks.length == 1)
                                this.outputText("\n\nYour " + this.multiCockDescriptLight() + " seems to swell up, feeling heavier. You look down and watch it growing fatter as it thickens.", false);
                            if (player.cocks.length > 1)
                                this.outputText("\n\nYour " + this.multiCockDescriptLight() + " seem to swell up, feeling heavier. You look down and watch them growing fatter as they thicken.", false);
                        }
                        this.dynStats("lib", 1, "sen", 1, "lus", 20);
                    }
                    //SINGLEZ
                    if (player.cocks.length == 1) {
                        this.outputText("\n\nYour " + this.multiCockDescriptLight() + " fills to its normal size... and begins growing... ", false);
                        temp3 = player.cocks[0].thickenCock(1);
                        temp2 = player.increaseCock(0, Mutations.rand(3) + 2);
                        player.lengthChange(temp2, 1);
                        //Display the degree of thickness change.
                        if (temp3 >= 1) {
                            if (player.cocks.length == 1)
                                this.outputText("  Your " + this.multiCockDescriptLight() + " spreads rapidly, swelling an inch or more in girth, making it feel fat and floppy.", false);
                            else
                                this.outputText("  Your " + this.multiCockDescriptLight() + " spread rapidly, swelling as they grow an inch or more in girth, making them feel fat and floppy.", false);
                        }
                        if (temp3 <= .5) {
                            if (player.cocks.length > 1)
                                this.outputText("  Your " + this.multiCockDescriptLight() + " feel swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. They are definitely thicker.", false);
                            else
                                this.outputText("  Your " + this.multiCockDescriptLight() + " feels swollen and heavy. With a firm, but gentle, squeeze, you confirm your suspicions. It is definitely thicker.", false);
                        }
                        if (temp3 > .5 && temp2 < 1) {
                            if (player.cocks.length == 1)
                                this.outputText("  Your " + this.multiCockDescriptLight() + " seems to swell up, feeling heavier. You look down and watch it growing fatter as it thickens.", false);
                            if (player.cocks.length > 1)
                                this.outputText("  Your " + this.multiCockDescriptLight() + " seem to swell up, feeling heavier. You look down and watch them growing fatter as they thicken.", false);
                        }
                        this.dynStats("lib", 1, "sen", 1, "lus", 20);
                    }
                }
            ...
            //LARGE
                ...
                //Kill extra boobages
                if (player.bRows() > 1) {
                    this.outputText("Your back relaxes as extra weight vanishes from your chest.  <b>Your lowest " + this.breastDescript(player.bRows() - 1) + " have vanished.</b>", false);
                ...
                if (player.buttRating > 5) {
                    this.outputText("Muscles firm and tone as you feel your " + this.buttDescript() + " become smaller and tighter.", false);
                    if (player.hipRating > 5)
                        this.outputText("  ", false);
                    player.buttRating -= 2;
                }
                if (player.hipRating > 5) {
                    this.outputText("Feeling the sudden burning of lactic acid in your " + this.hipDescript() + ", you realize they have slimmed down and firmed up some.", false);
                    player.hipRating -= 2;
                }
                //Shrink tits!
            ...
            if (Mutations.rand(3) == 0) {
                if (large)
                    this.outputText(player.modFem(0, 8), false);
                else
                    this.outputText(player.modFem(5, 3), false);
            }
        }
        //Nipplezzzzz
        whiteEgg(large, player) {
            var temp2 = 0;
            this.outputText("You devour the egg, momentarily sating your hunger.", true);
            if (!large) {
                //Grow nipples
            ...
            //LARGE
                ...
                if (temp2 > 0)
                    this.outputText("\n\nYour " + this.allBreastsDescript() + " tingle with warmth that slowly migrates to your nipples, filling them with warmth.  You pant and moan, rubbing them with your fingers.  A trickle of wetness suddenly coats your finger as it slips inside the nipple.  Shocked, you pull the finger free.  <b>You now have fuckable nipples!</b>", false);
            }
        }
        blackRubberEgg(large, player) {
            this.outputText("You devour the egg, momentarily sating your hunger.", true);
            //Small
            if (!large) {
                ...
                if ((player.skinAdj != "smooth" && player.skinAdj != "latex" && player.skinAdj != "rubber") || player.skinDesc != "skin") {
                    ...
                    if (player.skinType == appearanceDefs_1.SKIN_TYPE_PLAIN)
                        this.outputText(" loses its blemishes, becoming flawless smooth skin.", false);
                    if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR)
                        this.outputText(" falls out in clumps, revealing smooth skin underneath.", false);
                    if (player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES)
                        this.outputText(" begins dropping to the ground in a pile around you, revealing smooth skin underneath.", false);
                    if (player.skinType > appearanceDefs_1.SKIN_TYPE_SCALES)
                        this.outputText(" shifts and changes into flawless smooth skin.", false);
                    player.skinDesc = "skin";
                    player.skinAdj = "smooth";
                    if (player.skinTone == "rough gray")
                        player.skinTone = "gray";
                    player.skinType = appearanceDefs_1.SKIN_TYPE_PLAIN;
                }
                //chance of hair change
                    ...
                    if (player.hairColor.indexOf("rubbery") == -1 && player.hairColor.indexOf("latex-textured") && player.hairLength != 0) {
                        ...
                        if (player.skinDesc == "skin" && player.skinAdj == "rubber") {
                            this.outputText("\n\nYour scalp tingles and your " + this.hairDescript() + " thickens, the strands merging into ", false);
                            this.outputText(" thick rubbery hair.", false);
                            player.hairColor = "rubbery " + player.hairColor;
                            this.dynStats("cor", 2);
                        }
                        if (player.skinDesc == "skin" && player.skinAdj == "latex") {
                            this.outputText("\n\nYour scalp tingles and your " + this.hairDescript() + " thickens, the strands merging into ", false);
                            this.outputText(" shiny latex hair.", false);
            ...
            if (large) {
                ...
                if (player.skinDesc == "skin" && player.skinAdj == "smooth") {
                    ...
                    if (Mutations.rand(2) == 0) {
                        ...
                        this.outputText("a layer of pure latex.  ", false);
                    }
                    else {
                        player.skinDesc = "skin";
                        player.skinAdj = "rubber";
                        this.outputText("a layer of sensitive rubber.  ", false);
                    }
                    this.flags[kFLAGS_1.kFLAGS.PC_KNOWS_ABOUT_BLACK_EGGS] = 1;
                    if (player.cor < 66)
                        this.outputText("You feel like some kind of freak.", false);
                    else
                        this.outputText("You feel like some kind of sexy " + player.skinDesc + " love-doll.", false);
        ...
        hairDye(color, player) {
            if (player.hairColor.indexOf("rubbery") != -1 || player.hairColor.indexOf("latex-textured") != -1) {
                this.outputText("You massage the dye into your " + this.hairDescript() + " but the dye cannot penetrate the impermeable material your hair is composed of.", true);
                return;
            }
            if (player.hairLength == 0) {
                this.outputText("You rub the dye into your bald head, but it has no effect.", true);
                return;
            }
            this.outputText("You rub the dye into your " + this.hairDescript() + ", then use a bucket of cool lakewater to rinse clean a few minutes later.  ", true);
            player.hairColor = color;
            this.outputText("You now have " + this.hairDescript() + ".", false);
            if (player.lust > 50) {
        ...
        purePearl(player) {
            this.outputText("You cram the pearl in your mouth and swallow it like a giant pill with some difficulty.  Surprisingly there is no discomfort, only a cool calming sensation that springs up from your core.", true);
            this.dynStats("lib", -5, "lus", -25, "cor", -10);
            if (player.findPerk(PerkLib_1.PerkLib.PurityBlessing) < 0)
                player.createPerk(PerkLib_1.PerkLib.PurityBlessing, 0, 0, 0, 0);
        }
        //TODO FIX
        /* Moved to new GroPlus item
                public  growPlus(player:Player): void {
                var  gpBalls = (player.balls > 0 ? growPlusBalls : undefined);
                var  gpBreasts = (player.breastRows.length > 0 ? growPlusBreasts : undefined);
                var  gpClit = (player.vaginas.length > 0 ? growPlusClit : undefined);
                var  gpCock = (player.cockTotal() > 0 ? growPlusCock : undefined);
                var  gpNipples = (player.totalNipples() > 0 ? growPlusNipples : undefined);
                    ...
                    choices("Balls", gpBalls, "Breasts", gpBreasts, "Clit", gpClit, "Cock", gpCock, "Nipples", gpNipples, "", undefined, "", undefined, "", undefined, "", undefined, "Nevermind", growPlusCancel);
        */ /*
        ...
        /*		}
                ...
                //Grow nipples
                    ...
                    var  nowFuckable: boolean = false;
                        for (var x: number = 0; x < player.breastRows.length; x++) {
        ...
        */
        /* Moved to new Reducto item
                public  reducto(player:Player): void
                {
                var  rdtBalls = (player.balls > 0 && player.ballSize > 1 ? reductoBalls : undefined);
                var  rdtBreasts = (player.breastRows.length > 0 && player.biggestTitSize() > 0 ? reductoBreasts : undefined);
                var  rdtButt = (player.buttRating > 1 ? reductoButt : undefined);
                var  rdtClit = (player.vaginas.length > 0 && player.clitLength > 0.25 ? reductoClit : undefined);
                var  rdtCock = (player.cockTotal() > 0 && player.biggestCockArea() > 6 ? reductoCock : undefined);
                var  rdtHips = (player.hipRating > 2 ? reductoHips : undefined);
                var  rdtNipples = (player.nippleLength > 0.25 ? reductoNipples : undefined);
        */ /*
                var  ballsEvent: number = 0;
        ...
        */
        lactaid(player) {
            player.slimeFeed();
            var i = 0;
            this.outputText("You gulp down the bottle of lactaid, easily swallowing the creamy liquid.", true);
            //Bump up size!
            if (player.averageBreastSize() < 8) {
                this.outputText("\n\n", false);
                if (player.breastRows.length == 1)
                    player.growTits((1 + Mutations.rand(5)), 1, true, 1);
                else
                    player.growTits(1 + Mutations.rand(2), player.breastRows.length, true, 1);
            }
            //Player doesn't lactate
            if (player.biggestLactation() < 1) {
                this.outputText("\n\n", false);
                this.outputText("You feel your " + this.nippleDescript(0) + "s become tight and engorged.  A single droplet of milk escapes each, rolling down the curves of your breasts.  <b>You are now lactating!</b>", false);
            ...
            //Boost lactation
                ...
                this.outputText("\n\n", false);
                this.outputText("Milk leaks from your " + this.nippleDescript(0) + "s in thick streams.  You're lactating even more!", false);
                for (i = 0; i < player.breastRows.length; i++) {
            ...
            if (Mutations.rand(3) == 0) {
                this.outputText(player.modFem(95, 1), false);
            }
        }
        useMarbleMilk(player) {
            player.slimeFeed();
            ...
            if (player.statusAffectv2(StatusAffects_1.StatusAffects.Marble) < 30 && player.statusAffectv3(StatusAffects_1.StatusAffects.Marble) == 0)
                this.outputText("You gulp down the bottle's contents; Marble makes some good tasting milk.\n\n", false);
            //[before the player is addicted, Addiction < 50]
            else if (player.statusAffectv3(StatusAffects_1.StatusAffects.Marble) <= 0)
                this.outputText("You gulp down the bottle's contents; Marble makes some really good tasting milk.\n\n", false);
            else if (player.statusAffectv3(StatusAffects_1.StatusAffects.Marble) > 0) {
                //[player is completely addicted]
                if (player.findPerk(PerkLib_1.PerkLib.MarblesMilk) >= 0)
                    this.outputText("You gulp down the bottle's contents; it's no substitute for the real thing, but it's a nice pick me up.\n\n", false);
                else {
                    //[player is no longer addicted]
                    if (player.findPerk(PerkLib_1.PerkLib.MarbleResistant) >= 0)
                        this.outputText("You gulp down the bottle's contents; you're careful not to get too attached to the taste.\n\n", false);
                    //[player is addicted]
                    else
                        this.outputText("You gulp down the bottle's contents; you really needed that.\n\n", false);
                }
            }
            //Increases addiction by 5, up to a max of 50 before the player becomes addicted, no max after the player is addicted.
            kGAMECLASS_1.kGAMECLASS.marbleScene.marbleStatusChange(0, 5);
            //Does not apply the 'Marble's Milk' effect
            //Purge withdrawl
            if (player.findStatusAffect(StatusAffects_1.StatusAffects.MarbleWithdrawl) >= 0) {
                player.removeStatusAffect(StatusAffects_1.StatusAffects.MarbleWithdrawl);
            ...
            kGAMECLASS_1.kGAMECLASS.changeFatigue(-25);
            ...
            if (player.findStatusAffect(StatusAffects_1.StatusAffects.BottledMilk) >= 0) {
                player.addStatusValue(StatusAffects_1.StatusAffects.BottledMilk, 1, (6 + Mutations.rand(6)));
            }
            else
                player.createStatusAffect(StatusAffects_1.StatusAffects.BottledMilk, 12, 0, 0, 0);
        }
        /*Purified LaBova:
         ...
         */
        laBova(tainted, enhanced, player) {
            player.slimeFeed();
            //Changes done
            var changes = 0;
            //Change limit
            ...
            //ItemUseText:
            this.outputText("You drink the ", true);
            if (enhanced)
                this.outputText("Pro Bova", false);
            else
                this.outputText("La Bova", false);
            this.outputText(".  The drink has an odd texture, but is very sweet.  It has a slight aftertaste of milk.", false);
            ...
            //Sex bits - Duderiffic
            if (player.cocks.length > 0 && Mutations.rand(2) == 0 && !this.flags[kFLAGS_1.kFLAGS.HYPER_HAPPY]) {
            ...
            if (((tainted && player.biggestTitSize() <= 11) || (!tainted && player.biggestTitSize() <= 5)) && changes < changeLimit && (Mutations.rand(3) == 0 || enhanced)) {
                if (Mutations.rand(2) == 0)
                    this.outputText("\n\nYour " + this.breastDescript(0) + " tingle for a moment before becoming larger.", false);
                else
                    this.outputText("\n\nYou feel a little weight added to your chest as your " + this.breastDescript(0) + " seem to inflate and settle in a larger size.", false);
            ...
            if (player.biggestTitSize() >= 4 && player.breastRows[0].lactationMultiplier < 1 && changes < changeLimit && (Mutations.rand(3) == 0 || boobsGrew || enhanced)) {
                this.outputText("\n\nYou gasp as your " + this.breastDescript(0) + " feel like they are filling up with something.  Within moments, a drop of milk leaks from your " + this.breastDescript(0) + "; <b> you are now lactating</b>.", false);
            ...
            if (enhanced) {
                ...
                if (player.breastRows[0].nipplesPerBreast == 1) {
                    ...
                    this.outputText("\n\nYour " + this.nippleDescript(0) + "s tingle and itch.  You pull back your " + player.armorName + " and watch in shock as they split into four distinct nipples!  <b>You now have four nipples on each side of your chest!</b>", false);
                    if (player.breastRows.length >= 2 && player.breastRows[1].nipplesPerBreast == 1) {
                        this.outputText("A moment later your second row of " + this.breastDescript(1) + " does the same.  <b>You have sixteen nipples now!</b>", false);
                    ...
                    if (player.breastRows.length >= 3 && player.breastRows[2].nipplesPerBreast == 1) {
                        ...
                        if (player.bRows() == 3)
                            this.outputText("third row of " + this.breastDescript(2) + " mutates along with its sisters, sprouting into a wonderland of nipples.", false);
                        else if (player.bRows() >= 4) {
                            this.outputText("everything from the third row down mutates, sprouting into a wonderland of nipples.", false);
                        ...
                        this.outputText("  <b>You have a total of " + Mutations.num2Text(player.totalNipples()) + " nipples.</b>");
                    }
                }
                //QUAD DAMAGE IF WEIRD SHIT BROKE BEFORE
                    ...
                    if (player.breastRows[1].nipplesPerBreast == 1) {
                        this.outputText("\n\nYour second row of " + this.breastDescript(1) + " tingle and itch.  You pull back your " + player.armorName + " and watch in shock as your " + this.nippleDescript(1) + " split into four distinct nipples!  <b>You now have four nipples on each breast in your second row of breasts</b>.", false);
                        player.breastRows[1].nipplesPerBreast = 4;
                    }
                }
                else if (player.breastRows.length > 2 && player.breastRows[2].nipplesPerBreast == 1) {
                    if (player.breastRows[2].nipplesPerBreast == 1) {
                        this.outputText("\n\nYour third row of " + this.breastDescript(2) + " tingle and itch.  You pull back your " + player.armorName + " and watch in shock as your " + this.nippleDescript(2) + " split into four distinct nipples!  <b>You now have four nipples on each breast in your third row of breasts</b>.", false);
                        player.breastRows[2].nipplesPerBreast = 4;
                    }
                }
                else if (player.breastRows.length > 3 && player.breastRows[3].nipplesPerBreast == 1) {
                    if (player.breastRows[3].nipplesPerBreast == 1) {
                        this.outputText("\n\nYour fourth row of " + this.breastDescript(3) + " tingle and itch.  You pull back your " + player.armorName + " and watch in shock as your " + this.nippleDescript(3) + " split into four distinct nipples!  <b>You now have four nipples on each breast in your fourth row of breasts</b>.", false);
                        player.breastRows[3].nipplesPerBreast = 4;
                    }
                }
                else if (player.biggestLactation() > 1) {
                    if (Mutations.rand(2) == 0)
                        this.outputText("\n\nA wave of pleasure passes through your chest as your " + this.breastDescript(0) + " start leaking milk from a massive jump in production.", false);
                    else
                        this.outputText("\n\nSomething shifts inside your " + this.breastDescript(0) + " and they feel MUCH fuller and riper.  You know that you've started producing much more milk.", false);
                    player.boostLactation(2.5);
                    if ((player.nippleLength < 1.5 && tainted) || (!tainted && player.nippleLength < 1)) {
                        this.outputText("  Your " + this.nippleDescript(0) + "s swell up, growing larger to accommodate your increased milk flow.", false);
            ...
            //If breasts are already lactating and the player is not lactating beyond a reasonable level, they start lactating more:
                ...
                if (tainted && player.breastRows[0].lactationMultiplier > 1 && player.breastRows[0].lactationMultiplier < 5 && changes < changeLimit && (Mutations.rand(3) == 0 || enhanced)) {
                    if (Mutations.rand(2) == 0)
                        this.outputText("\n\nA wave of pleasure passes through your chest as your " + this.breastDescript(0) + " start producing more milk.", false);
                    else
                        this.outputText("\n\nSomething shifts inside your " + this.breastDescript(0) + " and they feel fuller and riper.  You know that you've started producing more milk.", false);
                ...
                if (!tainted) {
                    if (player.breastRows[0].lactationMultiplier > 1 && player.breastRows[0].lactationMultiplier < 3.2 && changes < changeLimit && Mutations.rand(3) == 0) {
                        if (Mutations.rand(2) == 0)
                            this.outputText("\n\nA wave of pleasure passes through your chest as your " + this.breastDescript(0) + " start producing more milk.", false);
                        else
                            this.outputText("\n\nSomething shifts inside your " + this.breastDescript(0) + " and they feel fuller and riper.  You know that you've started producing more milk.", false);
                    ...
                    if ((player.breastRows[0].lactationMultiplier > 2 && player.findStatusAffect(StatusAffects_1.StatusAffects.Feeder) >= 0) || player.breastRows[0].lactationMultiplier > 5) {
                        if (Mutations.rand(2) == 0)
            ...
            if (player.findStatusAffect(StatusAffects_1.StatusAffects.Feeder) < 0 && player.biggestLactation() >= 3 && Mutations.rand(2) == 0 && player.biggestTitSize() >= 5 && player.cor >= 35) {
                ...
                player.createStatusAffect(StatusAffects_1.StatusAffects.Feeder, 0, 0, 0, 0);
                player.createPerk(PerkLib_1.PerkLib.Feeder, 0, 0, 0, 0);
                changes++;
            }
            //UNFINISHED
            ...
            if (player.hasVagina()) {
                if (player.vaginas[0].vaginalLooseness < appearanceDefs_1.VAGINA_LOOSENESS_LOOSE && changes < changeLimit && Mutations.rand(2) == 0) {
                    this.outputText("\n\nYou feel a relaxing sensation in your groin.  On further inspection you discover your " + this.vaginaDescript(0) + " has somehow relaxed, permanently loosening.", false);
                    player.vaginas[0].vaginalLooseness++;
                    //Cunt Stretched used to determine how long since last enlargement
                    if (player.findStatusAffect(StatusAffects_1.StatusAffects.CuntStretched) < 0)
                        player.createStatusAffect(StatusAffects_1.StatusAffects.CuntStretched, 0, 0, 0, 0);
                    //Reset the timer on it to 0 when restretched.
                    else
                        player.changeStatusValue(StatusAffects_1.StatusAffects.CuntStretched, 1, 0);
                    player.vaginas[0].vaginalLooseness++;
                    changes++;
                    this.dynStats("lus", 10);
                }
            ...
            if (tainted && player.tailType != appearanceDefs_1.TAIL_TYPE_COW && changes < changeLimit && Mutations.rand(3) == 0) {
                if (player.tailType == appearanceDefs_1.TAIL_TYPE_NONE)
                    this.outputText("\n\nYou feel the flesh above your " + this.buttDescript() + " knotting and growing.  It twists and writhes around itself before flopping straight down, now shaped into a distinctly bovine form.  You have a <b>cow tail</b>.", false);
                else {
                    if (player.tailType < appearanceDefs_1.TAIL_TYPE_SPIDER_ADBOMEN || player.tailType > appearanceDefs_1.TAIL_TYPE_BEE_ABDOMEN) {
            ...
            if (tainted && player.earType != appearanceDefs_1.EARS_COW && changes < changeLimit && Mutations.rand(4) == 0 && player.tailType == appearanceDefs_1.TAIL_TYPE_COW) {
                this.outputText("\n\nYou feel your ears tug on your scalp as they twist shape, becoming oblong and cow-like.  <b>You now have cow ears.</b>", false);
                player.earType = appearanceDefs_1.EARS_COW;
                changes++;
            }
            //If the player is under 7 feet in height, increase their height, similar to the minotaur
            if (((enhanced && player.tallness < 96) || player.tallness < 84) && changes < changeLimit && Mutations.rand(2) == 0) {
                temp = Mutations.rand(5) + 3;
                //Slow rate of growth near ceiling
            ...
            if (tainted && player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_HOOFED && player.earType == appearanceDefs_1.EARS_COW) {
                if (changes < changeLimit && Mutations.rand(3) == 0) {
                    changes++;
                    if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HUMAN)
                        this.outputText("\n\nYou stagger as your feet change, curling up into painful angry lumps of flesh.  They get tighter and tighter, harder and harder, until at last they solidify into hooves!", false);
                    ...
                    if (player.lowerBody > appearanceDefs_1.LOWER_BODY_TYPE_NAGA)
                        this.outputText("\n\nYou stagger as your " + player.feet() + " change, curling up into painful angry lumps of flesh.  They get tighter and tighter, harder and harder, until at last they solidify into hooves!", false);
                    this.outputText("  A coat of beastial fur springs up below your waist, itching as it fills in.<b>  You now have hooves in place of your feet!</b>", false);
                    player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_HOOFED;
                    this.dynStats("cor", 0);
                    changes++;
                }
            ...
            if (!enhanced && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HOOFED && player.faceType != appearanceDefs_1.FACE_HUMAN && changes < changeLimit && Mutations.rand(4) == 0) {
                //Remove face before fur!
                this.outputText("\n\nYour visage twists painfully, returning to a normal human shape.  <b>Your face is human again!</b>", false);
                player.faceType = appearanceDefs_1.FACE_HUMAN;
                changes++;
            }
            ...
            else if (enhanced && player.faceType != appearanceDefs_1.FACE_COW_MINOTAUR) {
            ...
            //New horns or expanding mino horns
            if (tainted && changes < changeLimit && Mutations.rand(3) == 0 && player.faceType == appearanceDefs_1.FACE_HUMAN) {
                ...
                if (player.hornType == appearanceDefs_1.HORNS_COW_MINOTAUR || player.hornType == appearanceDefs_1.HORNS_NONE) {
                    ...
                    //If no horns yet..
                    if (player.hornType == appearanceDefs_1.HORNS_NONE || player.horns == 0) {
                        this.outputText("\n\nWith painful pressure, the skin on your forehead splits around two tiny nub-like horns, similar to those you would see on the cattle back in your homeland.", false);
                        player.hornType = appearanceDefs_1.HORNS_COW_MINOTAUR;
                        player.horns = 1;
                        changes++;
                    ...
                    //TF other horns
                    if (player.hornType != appearanceDefs_1.HORNS_NONE && player.hornType != appearanceDefs_1.HORNS_COW_MINOTAUR && player.horns > 0) {
            ...
            //Nipples Turn Back:
            if (player.findStatusAffect(StatusAffects_1.StatusAffects.BlackNipples) >= 0 && changes < changeLimit && Mutations.rand(3) == 0) {
                this.outputText("\n\nSomething invisible brushes against your " + this.nippleDescript(0) + ", making you twitch.  Undoing your clothes, you take a look at your chest and find that your nipples have turned back to their natural flesh colour.");
                changes++;
                player.removeStatusAffect(StatusAffects_1.StatusAffects.BlackNipples);
            }
            //Debugcunt
            if (changes < changeLimit && Mutations.rand(3) == 0 && player.vaginaType() == 5 && player.hasVagina()) {
                this.outputText("\n\nSomething invisible brushes against your sex, making you twinge.  Undoing your clothes, you take a look at your vagina and find that it has turned back to its natural flesh colour.");
                player.vaginaType(0);
                changes++;
            }
            if (Mutations.rand(3) == 0)
                this.outputText(player.modFem(79, 3), false);
            if (Mutations.rand(3) == 0)
                this.outputText(player.modThickness(70, 4), false);
            if (Mutations.rand(5) == 0)
                this.outputText(player.modTone(10, 5), false);
        }
        blackSpellbook(player) {
            ...
            if (player.inte >= 25 && player.findStatusAffect(StatusAffects_1.StatusAffects.KnowsArouse) < 0) {
                this.outputText("\n\nYou blink in surprise, assaulted by the knowledge of a <b>new spell: Arouse.</b>", false);
                player.createStatusAffect(StatusAffects_1.StatusAffects.KnowsArouse, 0, 0, 0, 0);
                return;
            }
            //Smart enough for arouse and doesnt have it
            if (player.inte >= 30 && player.findStatusAffect(StatusAffects_1.StatusAffects.KnowsHeal) < 0) {
                this.outputText("\n\nYou blink in surprise, assaulted by the knowledge of a <b>new spell: Heal.</b>", false);
                player.createStatusAffect(StatusAffects_1.StatusAffects.KnowsHeal, 0, 0, 0, 0);
                return;
            }
            //Smart enough for arouse and doesnt have it
            if (player.inte >= 40 && player.findStatusAffect(StatusAffects_1.StatusAffects.KnowsMight) < 0) {
                this.outputText("\n\nYou blink in surprise, assaulted by the knowledge of a <b>new spell: Might.</b>", false);
                player.createStatusAffect(StatusAffects_1.StatusAffects.KnowsMight, 0, 0, 0, 0);
            }
        }
        whiteSpellbook(player) {
            this.outputText("You open the white tome, and discover it to be an instructional book on the use of white magic.  Most of it is filled with generic information about white magic - how it is drawn for mental focus, is difficult to use when tired or aroused, and can be used to create and control energy.  In no time at all you've read the whole thing, but it disappears into thin air before you can put it away.", true);
            if (player.inte < 30) {
                this.outputText("\n\nYou feel greatly enlightened by your time spent reading.", false);
                this.dynStats("int", 4);
            }
            ...
            //Smart enough for arouse and doesnt have it
            if (player.inte >= 25 && player.findStatusAffect(StatusAffects_1.StatusAffects.KnowsCharge) < 0) {
                this.outputText("\n\nYou blink in surprise, assaulted by the knowledge of a <b>new spell: Charge Weapon.</b>", false);
                player.createStatusAffect(StatusAffects_1.StatusAffects.KnowsCharge, 0, 0, 0, 0);
                return;
            }
            //Smart enough for arouse and doesnt have it
            if (player.inte >= 30 && player.findStatusAffect(StatusAffects_1.StatusAffects.KnowsBlind) < 0) {
                this.outputText("\n\nYou blink in surprise, assaulted by the knowledge of a <b>new spell: Blind.</b>", false);
                player.createStatusAffect(StatusAffects_1.StatusAffects.KnowsBlind, 0, 0, 0, 0);
                return;
            }
            //Smart enough for arouse and doesnt have it
            if (player.inte >= 40 && player.findStatusAffect(StatusAffects_1.StatusAffects.KnowsWhitefire) < 0) {
                this.outputText("\n\nYou blink in surprise, assaulted by the knowledge of a <b>new spell: Whitefire.</b>", false);
                player.createStatusAffect(StatusAffects_1.StatusAffects.KnowsWhitefire, 0, 0, 0, 0);
            }
        }
        lustDraft(fuck, player) {
            player.slimeFeed();
            this.outputText("You drink the ", true);
            if (fuck)
                this.outputText("red", false);
            else
                this.outputText("pink", false);
            this.outputText(" potion, and its unnatural warmth immediately flows to your groin.", false);
            ...
            //ORGAZMO
            if (player.lust >= 100 && !kGAMECLASS_1.kGAMECLASS.inCombat) {
                ...
                if (player.cocks.length > 0) {
                    this.outputText(this.multiCockDescriptLight() + " erupts in front of you, liberally spraying the ground around you.  ", false);
                }
                if (player.cocks.length > 0 && player.vaginas.length > 0) {
                    this.outputText("At the same time your ", false);
                }
                if (player.vaginas.length > 0) {
                    this.outputText(this.vaginaDescript(0) + " soaks your thighs.  ", false);
                }
                if (player.gender == 0)
                    this.outputText("body begins to quiver with orgasmic bliss.  ", false);
                this.outputText("Once you've had a chance to calm down, you notice that the explosion of pleasure you just experienced has rocked you to your core.  You are a little hornier than you were before.", false);
        ...
        goblinAle(player) {
            ...
            if (player.findPerk(PerkLib_1.PerkLib.HistoryAlchemist) >= 0)
                changeLimit++;
            this.outputText("You drink the ale, finding it to have a remarkably smooth yet potent taste.  You lick your lips and sneeze, feeling slightly tipsy.", true);
            this.dynStats("lus", 15);
            //Stronger
            if (player.str > 50) {
                this.dynStats("str", -1);
            ...
            if (changes < changeLimit && Mutations.rand(3) == 0 && player.hasVagina() && player.statusAffectv1(StatusAffects_1.StatusAffects.BonusVCapacity) < 40) {
                if (player.findStatusAffect(StatusAffects_1.StatusAffects.BonusVCapacity) < 0)
                    player.createStatusAffect(StatusAffects_1.StatusAffects.BonusVCapacity, 0, 0, 0, 0);
                player.addStatusValue(StatusAffects_1.StatusAffects.BonusVCapacity, 1, 5);
                this.outputText("\n\nThere is a sudden... emptiness within your " + this.vaginaDescript(0) + ".  Somehow you know you could accommodate even larger... insertions.", false);
                changes++;
            }
            //Boost fertility
            if (changes < changeLimit && Mutations.rand(4) == 0 && player.fertility < 40 && player.hasVagina()) {
            ...
            else if (player.cocks.length == 1 && Mutations.rand(2) == 0 && changes < changeLimit && !this.flags[kFLAGS_1.kFLAGS.HYPER_HAPPY]) {
                if (player.cocks[0].cockLength > 12) {
                    changes++;
                    var temp3 = 0;
                    this.outputText("\n\n", false);
            ...
            if ((player.wingType == appearanceDefs_1.WING_TYPE_BEE_LIKE_SMALL || player.wingType == appearanceDefs_1.WING_TYPE_BEE_LIKE_LARGE || player.wingType >= appearanceDefs_1.WING_TYPE_HARPY) && changes < changeLimit && Mutations.rand(4) == 0) {
                if (player.wingType == appearanceDefs_1.WING_TYPE_SHARK_FIN)
                ...
                player.wingType = appearanceDefs_1.WING_TYPE_NONE;
                changes++;
            }
            //Removes wings and antennaes!
            if (player.antennae > appearanceDefs_1.ANTENNAE_NONE && changes < changeLimit && Mutations.rand(3) == 0) {
                this.outputText("\n\nYour " + this.hairDescript() + " itches so you give it a scratch, only to have your antennae fall to the ground.  What a relief.  <b>You've lost your antennae!</b>", false);
                changes++;
                player.antennae = appearanceDefs_1.ANTENNAE_NONE;
            }
            //Remove odd eyes
            if (changes < changeLimit && Mutations.rand(5) == 0 && player.eyeType > appearanceDefs_1.EYES_HUMAN) {
                if (player.eyeType == appearanceDefs_1.EYES_BLACK_EYES_SAND_TRAP) {
            ...
            //Skin/fur
            if (player.skinType != appearanceDefs_1.SKIN_TYPE_PLAIN && changes < changeLimit && Mutations.rand(4) == 0 && player.faceType == appearanceDefs_1.FACE_HUMAN) {
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR)
            ...
            //skinTone
            if (player.skinTone != "green" && player.skinTone != "grayish-blue" && player.skinTone != "dark green" && player.skinTone != "pale yellow" && changes < changeLimit && Mutations.rand(2) == 0) {
                ...
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR)
                    this.outputText("skin", false);
                else
                    this.outputText(player.skinDesc, false);
                this.outputText(" turned " + player.skinTone + ".  No way!  It's staying, it really changed color!", false);
            }
            //Face!
            if (player.faceType != appearanceDefs_1.FACE_HUMAN && changes < changeLimit && Mutations.rand(4) == 0 && player.earType == appearanceDefs_1.EARS_ELFIN) {
                changes++;
                player.faceType = appearanceDefs_1.FACE_HUMAN;
            ...
            //Ears!
            if (player.earType != appearanceDefs_1.EARS_ELFIN && changes < changeLimit && Mutations.rand(3) == 0) {
                this.outputText("\n\nA weird tingling runs through your scalp as your " + this.hairDescript() + " shifts slightly.  You reach up to touch and bump <b>your new pointed elfin ears</b>.  You bet they look cute!", false);
                changes++;
                player.earType = appearanceDefs_1.EARS_ELFIN;
            }
            if (Mutations.rand(4) == 0 && player.gills && changes < changeLimit) {
                this.outputText("\n\nYour chest itches, and as you reach up to scratch it, you realize your gills have withdrawn into your skin.", false);
                player.gills = false;
        ...
        gooGasmic(player) {
            this.outputText("You take the wet cloth in hand and rub it over your body, smearing the strange slime over your " + player.skinDesc + " slowly.", true);
            //Stat changes
            //libido up to 80
            if (player.lib < 80) {
                this.dynStats("lib", (.5 + (90 - player.lib) / 10), "lus", player.lib / 2);
            ...
            //Remove wings
            if (player.wingType > appearanceDefs_1.WING_TYPE_NONE) {
                if (player.wingType == appearanceDefs_1.WING_TYPE_SHARK_FIN)
            ...
            if (player.hairType != 3) {
                ...
                //if bald
                if (player.hairLength <= 0) {
                    this.outputText("\n\nYour head buzzes pleasantly, feeling suddenly hot and wet.  You instinctively reach up to feel the source of your wetness, and discover you've grown some kind of gooey hair.  From time to time it drips, running down your back to the crack of your " + this.buttDescript() + ".", false);
                    player.hairLength = 5;
                }
                else {
                    //if hair isnt rubbery or latexy
                    if (player.hairColor.indexOf("rubbery") == -1 && player.hairColor.indexOf("latex-textured") == -1) {
                        this.outputText("\n\nYour head buzzes pleasantly, feeling suddenly hot and wet.  You instinctively reach up to feel the source of your wetness, and discover your hair has become a slippery, gooey mess.  From time to time it drips, running down your back to the crack of your " + this.buttDescript() + ".", false);
                    }
                    //Latexy stuff
                    else {
                ...
                if (player.hairColor != "green" && player.hairColor != "purple" && player.hairColor != "blue" && player.hairColor != "cerulean" && player.hairColor != "emerald") {
                    ...
                    this.outputText(player.hairColor + ".");
                }
                this.dynStats("lus", 10);
                return;
            }
            ...
            if (player.hairType == 3 && (player.skinDesc != "skin" || player.skinAdj != "slimy")) {
                ...
                else if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR)
                ...
                else if (player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES)
                ...
                else if (player.skinType > appearanceDefs_1.SKIN_TYPE_GOO)
                ...
                player.skinType = appearanceDefs_1.SKIN_TYPE_GOO;
                player.skinDesc = "skin";
                ...
                if (player.skinTone != "green" && player.skinTone != "purple" && player.skinTone != "blue" && player.skinTone != "cerulean" && player.skinTone != "emerald") {
                    ...
                    this.outputText(player.skinTone + "!");
                }
                return;
            }
            ...
            //2.Goo legs
            if (player.skinAdj == "slimy" && player.skinDesc == "skin" && player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_GOO) {
                this.outputText("\n\nYour viewpoint rapidly drops as everything below your " + this.buttDescript() + " and groin melts together into an amorphous blob.  Thankfully, you discover you can still roll about on your new slimey undercarriage, but it's still a whole new level of strange.", false);
                player.tallness -= 3 + Mutations.rand(2);
                if (player.tallness < 36) {
                    player.tallness = 36;
                    this.outputText("  The goo firms up and you return to your previous height.  It would truly be hard to get any shorter than you already are!", false);
                }
                player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_GOO;
                return;
            }
            ...
            if (!player.hasVagina()) {
                ...
                player.createVagina();
                player.vaginas[0].vaginalWetness = appearanceDefs_1.VAGINA_WETNESS_DROOLING;
                player.vaginas[0].vaginalLooseness = appearanceDefs_1.VAGINA_LOOSENESS_GAPING;
                player.clitLength = .4;
                player.genderCheck();
                return;
            }
            ...
            if (player.vaginalCapacity() < 9000) {
                if (player.findStatusAffect(StatusAffects_1.StatusAffects.BonusVCapacity) < 0)
                    player.createStatusAffect(StatusAffects_1.StatusAffects.BonusVCapacity, 9000, 0, 0, 0);
                else
                    player.addStatusValue(StatusAffects_1.StatusAffects.BonusVCapacity, 1, 9000);
                this.outputText("\n\nYour " + this.vaginaDescript(0) + "'s internal walls feel a tingly wave of strange tightness.  Experimentally, you slip a few fingers, then your hand, then most of your forearm inside yourself.  <b>It seems you're now able to accommodate just about ANYTHING inside your sex.</b>", false);
                return;
            }
            ...
            //Big slime girl
                ...
                if (player.findStatusAffect(StatusAffects_1.StatusAffects.SlimeCraving) < 0) {
                    ...
                    player.createStatusAffect(StatusAffects_1.StatusAffects.SlimeCraving, 0, 0, 0, 1); //Value four indicates this tracks strength and speed separately
                }
                else {
                    ...
                    player.changeStatusValue(StatusAffects_1.StatusAffects.SlimeCraving, 1, 0);
                }
            }
            if (Mutations.rand(2) == 0)
                this.outputText(player.modFem(85, 3), false);
            if (Mutations.rand(2) == 0)
                this.outputText(player.modThickness(20, 3), false);
            if (Mutations.rand(2) == 0)
                this.outputText(player.modTone(15, 5), false);
        }
        sharkTooth(type, player) {
            var changes = 0;
            var changeLimit = 2;
            if (Mutations.rand(2) == 0)
                changeLimit++;
            ...
            if (player.findPerk(PerkLib_1.PerkLib.HistoryAlchemist) >= 0)
                changeLimit++;
            if (type == 0)
                this.outputText("You have no idea why, but you decide to eat the pointed tooth. To your surprise, it's actually quite brittle, turning into a fishy-tasting dust. You figure it must just be a tablet made to look like a shark's tooth.", true);
            else if (type == 1)
                this.outputText("You have no idea why, but you decide to eat the pointed, glowing tooth. To your surprise, it's actually quite brittle, crumbling into a fishy-tasting dust. Maybe it's just a tablet made to look like a shark's tooth.", true);
            //STATS
            ...
            if (((player.lib < 100 && type == 1) || player.lib < 75) && Mutations.rand(3) == 0 && changes < changeLimit) {
            ...
            if (type == 1 && (player.gender == 0 || (!player.hasVagina() && changes < changeLimit && Mutations.rand(3) == 0))) {
                changes++;
                //(balls)
                if (player.balls > 0)
                    this.outputText("\n\nAn itch starts behind your " + this.ballsDescriptLight() + ", but before you can reach under to scratch it, the discomfort fades. A moment later a warm, wet feeling brushes your " + this.sackDescript() + ", and curious about the sensation, <b>you lift up your balls to reveal your new vagina.</b>", false);
                //(dick)
                else if (player.hasCock())
                    this.outputText("\n\nAn itch starts on your groin, just below your " + this.multiCockDescriptLight() + ". You pull the manhood aside to give you a better view, and you're able to watch as <b>your skin splits to give you a new vagina, complete with a tiny clit.</b>", false);
                //(neither)
                else
            ...
            if (type == 1 && (!player.hasCock()) && changes < changeLimit && Mutations.rand(3) == 0) {
                ...
                //Female:
                    ...
                    this.outputText("\n\nYou feel a sudden stabbing pain just above your " + this.vaginaDescript() + " and bend over, moaning in agony. Your hands clasp protectively over the surface - which is swelling in an alarming fashion under your fingers! Stripping off your clothes, you are presented with the shocking site of once-smooth flesh swelling and flowing like self-animate clay, resculpting itself into the form of male genitalia! When the pain dies down, you are the proud owner of not only a " + this.vaginaDescript() + ", but a new human-shaped penis", false);
                if (player.balls == 0) {
                    this.outputText(" and a pair of balls", false);
                    player.balls = 2;
                    player.ballSize = 2;
                }
                this.outputText("!", false);
            ...
            if (type == 1 && (player.balls == 0 || player.balls == 2) && player.hasCock() && changes < changeLimit && Mutations.rand(3) == 0) {
                if (player.balls == 2) {
                    this.outputText("\n\nYou gasp in shock as a sudden pain racks your abdomen. Within seconds, two more testes drop down into your " + this.sackDescript() + ", your skin stretching out to accommodate them. Once the pain clears, you examine <b>your new quartet of testes.</b>", false);
                    player.balls = 4;
                }
            ...
            //Transformations:
            //Mouth TF
            if (player.faceType != appearanceDefs_1.FACE_SHARK_TEETH && Mutations.rand(3) == 0 && changes < changeLimit) {
                this.outputText("\n\n", false);
                if (player.faceType > appearanceDefs_1.FACE_HUMAN && player.faceType < appearanceDefs_1.FACE_SHARK_TEETH)
                    this.outputText("Your " + player.face() + " explodes with agony, reshaping into a more human-like visage.  ", false);
                player.faceType = appearanceDefs_1.FACE_SHARK_TEETH;
            ...
            //Tail TF
            if (player.tailType != appearanceDefs_1.TAIL_TYPE_SHARK && Mutations.rand(3) == 0 && changes < changeLimit) {
                changes++;
                if (player.tailType == appearanceDefs_1.TAIL_TYPE_NONE)
                ...
                player.tailType = appearanceDefs_1.TAIL_TYPE_SHARK;
            }
            //Hair
            ...
            //Skin
            if (((player.skinTone != "rough gray" && player.skinTone != "orange and black striped") || player.skinType != appearanceDefs_1.SKIN_TYPE_PLAIN) && Mutations.rand(7) == 0 && changes < changeLimit) {
                this.outputText("\n\n", false);
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR || player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES)
                    this.outputText("Your " + player.skinDesc + " falls out, collecting on the floor and exposing your supple skin underneath.  ", false);
                else if (player.skinType == appearanceDefs_1.SKIN_TYPE_GOO)
                    this.outputText("Your gooey skin solidifies, thickening up as your body starts to solidy into a more normal form. ", false);
                else if (type == 0)
                    this.outputText("Your skin itches and tingles becoming slightly rougher and turning gray.  ", false);
                if (type == 0) {
                    this.outputText("You abruptly stop moving and gasp sharply as a shudder goes up your entire frame. Your skin begins to shift and morph, growing slightly thicker and changing into a shiny grey color. Your skin now feels oddly rough too, comparable to that of a marine mammal. You smile and run your hands across your new shark skin.", false);
                    player.skinType = appearanceDefs_1.SKIN_TYPE_PLAIN;
                    player.skinDesc = "skin";
                    ...
                    this.outputText("Your skin begins to tingle and itch, before rapidly shifting to a shiny orange color, marked by random black stripes. You take a quick look in a nearby pool of water, to see your skin has morphed in appearance and texture to become more like a tigershark!", false);
                    player.skinType = appearanceDefs_1.SKIN_TYPE_PLAIN;
                    player.skinDesc = "skin";
            ...
            //FINZ R WINGS
            if (player.wingType != appearanceDefs_1.WING_TYPE_SHARK_FIN && changes < changeLimit && Mutations.rand(3) == 0) {
                this.outputText("\n\n", false);
                if (player.wingType > appearanceDefs_1.WING_TYPE_NONE)
                    this.outputText("Your wings fold into themselves, merging together with your back.  ", false);
                ...
                player.wingType = appearanceDefs_1.WING_TYPE_SHARK_FIN;
                player.wingDesc = "";
                changes++;
            }
            if (changes == 0) {
        ...
        snakeOil(player) {
            ...
            this.outputText("Pinching your nose, you quickly uncork the vial and bring it to your mouth, determined to see what effects it might have on your body. Pouring in as much as you can take, you painfully swallow before going for another shot, emptying the bottle.", false);
            //(if outside combat)
            if (!kGAMECLASS_1.kGAMECLASS.inCombat)
                this.outputText("  Minutes pass as you start wishing you had water with you, to get rid of the aftertaste.", false);
            //+ speed to 70!
            if (player.spe < 70 && Mutations.rand(2) == 0) {
                ...
                if (player.spe < 40)
                    this.outputText("  Of course, you're nowhere near as fast as that.", false);
                changes++;
            }
            //Removes wings
            if (player.wingType > appearanceDefs_1.WING_TYPE_NONE && Mutations.rand(3) == 0 && changes < changeLimit) {
                if (player.wingType == appearanceDefs_1.WING_TYPE_SHARK_FIN)
            ...
            //Removes antennae
            if (player.antennae > appearanceDefs_1.ANTENNAE_NONE && Mutations.rand(3) == 0 && changes < changeLimit) {
            ...
            if (changes == 0 && player.tongueType != appearanceDefs_1.TONUGE_SNAKE && Mutations.rand(3) == 0 && changes < changeLimit) {
                if (player.tongueType == appearanceDefs_1.TONUGE_HUMAN)
                ...
                player.tongueType = appearanceDefs_1.TONUGE_SNAKE;
                this.dynStats("sen", 5);
                changes++;
            }
            //9c) III The fangs
            if (changes == 0 && player.tongueType == appearanceDefs_1.TONUGE_SNAKE && player.faceType != appearanceDefs_1.FACE_SNAKE_FANGS && Mutations.rand(3) == 0 && changes < changeLimit) {
                ...
                if (player.faceType != appearanceDefs_1.FACE_HUMAN && player.faceType != appearanceDefs_1.FACE_SHARK_TEETH && player.faceType != appearanceDefs_1.FACE_BUNNY && player.faceType != appearanceDefs_1.FACE_SPIDER_FANGS) {
                    this.outputText("  As the change progresses, your " + player.face() + " reshapes.  The sensation is far more pleasant than teeth cutting into gums, and as the tingling transformation completes, <b>you've gained with a normal-looking, human visage.</b>");
                }
                player.faceType = appearanceDefs_1.FACE_SNAKE_FANGS;
                changes++;
            }
            ...
            if (changes == 0 && player.faceType == appearanceDefs_1.FACE_SNAKE_FANGS && player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_NAGA && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                if (player.balls > 0 && player.ballSize > 10)
                    this.outputText("  You're happy not to have to drag those testicles around with you anymore.", false);
                ...
                player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_NAGA;
                changes++;
            }
            if (Mutations.rand(4) == 0 && player.gills && changes < changeLimit) {
                this.outputText("\n\nYour chest itches, and as you reach up to scratch it, you realize your gills have withdrawn into your skin.", false);
        ...
        */
        Hummus(player) {
            this.outputText("", true);
            if (this.debug) {
                this.outputText("You're about to eat the humus when you see it has bugs in it. Not wanting to eat bugged humus or try to debug it you throw it into the portal and find something else to eat.", false);
                player.destroyItems(this.consumables.HUMMUS_, 1);
                return;
            }
            this.outputText("You shovel the stuff into your face, not sure WHY you're eating it, but once you start, you just can't stop.  It tastes incredibly bland, and with a slight hint of cheese.", false);
            player.str = 30;
            player.spe = 30;
            player.tou = 30;
            player.inte = 30;
            ...
            player.antennae = appearanceDefs_1.ANTENNAE_NONE;
            player.faceType = appearanceDefs_1.FACE_HUMAN;
            player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_HUMAN;
            player.wingType = appearanceDefs_1.WING_TYPE_NONE;
            player.wingDesc = "non-existant";
            player.tailType = appearanceDefs_1.TAIL_TYPE_NONE;
            player.tongueType = appearanceDefs_1.TONUGE_HUMAN;
            player.tailRecharge = 0;
            player.horns = 0;
            player.hornType = appearanceDefs_1.HORNS_NONE;
            player.earType = appearanceDefs_1.EARS_HUMAN;
            player.skinType = appearanceDefs_1.SKIN_TYPE_PLAIN;
            player.skinDesc = "skin";
            player.skinAdj = "";
            player.armType = appearanceDefs_1.ARM_TYPE_HUMAN;
            ...
            while (player.vaginas.length > 0) {
                ...
                player.removeVagina(0, 1);
                console_1.trace("1 vagina purged.");
            }
            ...
            player.gills = false;
            player.removeStatusAffect(StatusAffects_1.StatusAffects.Uniball);
            player.removeStatusAffect(StatusAffects_1.StatusAffects.BlackNipples);
            player.vaginaType(0);
        }
        coal(player) {
            // var changes: number = 0;
            this.outputText("", true);
            this.outputText("You handle the coal rocks experimentally and they crumble to dust in your hands!  You cough as you breathe in the cloud, sputtering and wheezing.  After a minute of terrible coughing, you recover and realize there's no remaining trace of the rocks, not even a sooty stain on your hands!", false);
            //Try to go into intense heat
            ...
            //Males go into rut
                ...
                if (player.statusAffectv1(StatusAffects_1.StatusAffects.BonusACapacity) < 80) {
                    if (player.findStatusAffect(StatusAffects_1.StatusAffects.BonusACapacity) < 0)
                        player.createStatusAffect(StatusAffects_1.StatusAffects.BonusACapacity, 0, 0, 0, 0);
                    player.addStatusValue(StatusAffects_1.StatusAffects.BonusACapacity, 1, 5);
                    this.outputText("\n\nYou feel... more accommodating somehow.  Your " + this.assholeDescript() + " is tingling a bit, and though it doesn't seem to have loosened, it has grown more elastic.", false);
                    // changes++;
                }
                else {
        ...
        catTransformation(player) {
            ...
            //Text go!
            this.outputText("", true);
            this.outputText("You take a bite of the fruit and gulp it down. It's thick and juicy and has an almost overpowering sweetness. Nevertheless, it is delicious and you certainly could use a meal.  You devour the fruit, stopping only when the hard, nubby pit is left; which you toss aside.", false);
            ...
            if (Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                //medium intelligence
                    ...
                    if (Mutations.rand(2) == 0) {
                        this.outputText("somewhere and ", false);
                        this.temp = Mutations.rand(3);
                        if (this.temp == 0)
                            this.outputText("toss a ball around or something", false);
                        else if (this.temp == 1)
                            this.outputText("play with some yarn", false);
                        else if (this.temp == 2)
                            this.outputText("take a nap and stop worrying", false);
                    }
                    else
                        this.outputText("in the sun and let your troubles slip away", false);
                    this.outputText(".", false);
                }
                //High intelligence
                else
            ...
            if (player.lib < 80 && changes < changeLimit && Mutations.rand(4) == 0) {
                ...
                if (player.catCocks() > 0) {
                    this.temp = player.findFirstCockType(CockTypesEnum_1.CockTypesEnum.CAT);
                    this.outputText("\n\nYou feel your " + this.cockDescript(this.temp) + " growing hard, the barbs becoming more sensitive. You gently run your hands down them and imagine the feeling of raking the insides of a cunt as you pull.  The fantasy continues, and after ejaculating and hearing the female yowl with pleasure, you shake your head and try to drive off the image.  ", false);
                    if (player.cor < 33)
                        this.outputText("You need to control yourself better.", false);
                    else if (player.cor < 66)
                        this.outputText("You're not sure how you feel about the fantasy.", false);
                    else
                        this.outputText("You hope to find a willing partner to make this a reality.", false);
                }
                //Else –
                else {
                    ...
                    if (player.lust > 60)
                        this.outputText("even more ", false);
                    this.outputText("turned on.", false);
                }
                    ...
                    for (var k = 0; k < player.breastRows.length; k++) {
                        ...
                        if (player.breastRows[k].breastRating > temp2) {
                            ...
                            if (player.breastRows[k].breastRating > 10) {
                                ...
                                if (temp3 == 0)
                                    this.outputText("\n\nThe " + this.breastDescript(0) + " on your chest wobble for a second, then tighten up, losing several cup-sizes in the process!", false);
                                else
                                    this.outputText("  The change moves down to your " + Mutations.num2Text2(k + 1) + " row of " + this.breastDescript(0) + ". They shrink greatly, losing a couple cup-sizes.", false);
                            }
                            //Small change
                            else {
                                player.breastRows[k].breastRating -= 1;
                                if (temp3 == 0)
                                    this.outputText("\n\nAll at once, your sense of gravity shifts.  Your back feels a sense of relief, and it takes you a moment to realize your " + this.breastDescript(k) + " have shrunk!", false);
                                else
                                    this.outputText("  Your " + Mutations.num2Text2(k + 1) + " row of " + this.breastDescript(k) + " gives a tiny jiggle as it shrinks, losing some off its mass.", false);
                            }
            ...
            if (player.cockTotal() > 0 && player.catCocks() < player.cockTotal() &&
                changes < changeLimit && Mutations.rand(4) == 0) {
                //loop through and find a non-cat wang.
                for (var i = 0; i < (player.cockTotal()) && player.cocks[i].cockType == CockTypesEnum_1.CockTypesEnum.CAT; i++) { }
                this.outputText("\n\nYour " + this.cockDescript(i) + " swells up with near-painful arousal and begins to transform.  It turns pink and begins to narrow until the tip is barely wide enough to accommodate your urethra.  Barbs begin to sprout from its flesh, if you can call the small, fleshy nubs barbs. They start out thick around the base of your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.HUMAN) + " and shrink towards the tip. The smallest are barely visible. <b>Your new feline dong throbs powerfully</b> and spurts a few droplets of cum.  ", false);
                if (!player.hasSheath()) {
                    this.outputText("Then, it begins to shrink and sucks itself inside your body.  Within a few moments, a fleshy sheath is formed.", false);
                    if (player.balls > 0)
                        this.outputText("  Thankfully, your balls appear untouched.", false);
                }
                else
                    this.outputText("Then, it disappears back into your sheath.", false);
                player.cocks[i].cockType = CockTypesEnum_1.CockTypesEnum.CAT;
                player.cocks[i].knotMultiplier = 1;
                changes++;
            }
            //Cat penorz shrink
            if (player.catCocks() > 0 && Mutations.rand(3) == 0 && changes < changeLimit && !this.flags[kFLAGS_1.kFLAGS.HYPER_HAPPY]) {
                //loop through and find a cat wang.
                this.temp = 0;
                for (var j = 0; j < (player.cockTotal()); j++) {
                    if (player.cocks[j].cockType == CockTypesEnum_1.CockTypesEnum.CAT && player.cocks[j].cockLength > 6) {
                        this.temp = 1;
                        break;
                    }
                }
                if (this.temp == 1) {
                    ...
                    if (player.cocks[j].cockLength > 16) {
                        this.outputText("\n\nYour " + this.cockDescript(j) + " tingles, making your sheath feel a little less tight.  It dwindles in size, losing a full third of its length and a bit of girth before the change finally stops.", false);
                        player.cocks[j].cockLength *= .66;
                    }
                    else if (player.cocks[j].cockLength > 6) {
                        this.outputText("\n\nYour " + this.cockDescript(j) + " tingles and withdraws further into your sheath.  If you had to guess, you'd say you've lost about two inches of total length and perhaps some girth.", false);
                    ...
                    for (j; j < player.cocks.length; j++) {
                        ...
                        if (player.cocks[j].cockType == CockTypesEnum_1.CockTypesEnum.CAT) {
                    ...
                    this.outputText("  Although the package is smaller, it feels even more sensitive – as if it retained all sensation of its larger size in its smaller form.", false);
                    this.dynStats("sen", 5);
                    //Make note of other dicks changing
                    if (temp2 == 1)
                        this.outputText("  Upon further inspection, all your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.CAT) + "s have shrunk!", false);
                    changes++;
                }
            }
            //Body type changes.  Teh rarest of the rare.
            //DA EARZ
            if (player.earType != appearanceDefs_1.EARS_CAT && Mutations.rand(5) == 0 && changes < changeLimit) {
                //human to cat:
                if (player.earType == appearanceDefs_1.EARS_HUMAN) {
                    if (Mutations.rand(2) == 0)
                ...
                player.earType = appearanceDefs_1.EARS_CAT;
                changes++;
            }
            //DA TAIL (IF ALREADY HAZ URZ)
            if (player.tailType != appearanceDefs_1.TAIL_TYPE_CAT && player.earType == appearanceDefs_1.EARS_CAT && Mutations.rand(5) == 0 && changes < changeLimit) {
                if (player.tailType == appearanceDefs_1.TAIL_TYPE_NONE) {
                    this.temp = Mutations.rand(3);
                    if (this.temp == 0)
                ...
                player.tailType = appearanceDefs_1.TAIL_TYPE_CAT;
                changes++;
            }
            //Da paws (if already haz ears & tail)
            if (player.tailType == appearanceDefs_1.TAIL_TYPE_CAT && player.earType == appearanceDefs_1.EARS_CAT &&
                Mutations.rand(5) == 0 && changes < changeLimit &&
                player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_CAT) {
                //hoof to cat:
                if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HOOFED || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_CENTAUR) {
                    ...
                    if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_CENTAUR)
                        this.outputText("  You feel woozy and collapse on your side.  When you wake, you're no longer a centaur and your body has returned to a humanoid shape.", false);
                }
                //Goo to cat
                else if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_GOO) {
            ...
            if (player.tailType == appearanceDefs_1.TAIL_TYPE_CAT && player.earType == appearanceDefs_1.EARS_CAT &&
                Mutations.rand(5) == 0 && changes < changeLimit &&
                player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_CAT && player.skinType != appearanceDefs_1.SKIN_TYPE_FUR) {
            ...
            if (player.tailType == appearanceDefs_1.TAIL_TYPE_CAT && player.earType == appearanceDefs_1.EARS_CAT &&
                Mutations.rand(5) == 0 && changes < changeLimit &&
                player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_CAT && player.skinType == appearanceDefs_1.SKIN_TYPE_FUR &&
                player.faceType != appearanceDefs_1.FACE_CAT) {
            ...
            if (changes < changeLimit) {
                if (Mutations.rand(2) == 0)
                    this.outputText(player.modThickness(5, 2), false);
                if (Mutations.rand(2) == 0)
                    this.outputText(player.modTone(76, 2), false);
                if (player.gender < 2)
                    if (Mutations.rand(2) == 0)
                        this.outputText(player.modFem(65, 1), false);
                    else
                        this.outputText(player.modFem(85, 2), false);
            }
        }
        reptilum(player) {
            player.slimeFeed();
            ...
            //clear screen
            this.outputText("", true);
            this.outputText("You uncork the vial of fluid and drink it down.  The taste is sour, like a dry wine with an aftertaste not entirely dissimilar to alcohol.  Instead of the warmth you'd expect, it leaves your throat feeling cold and a little numb.", false);
            //Statistical changes:
            ...
            if (player.lib < 100 && changes < changeLimit && Mutations.rand(3) == 0) {
                ...
                if (player.cocks.length > 0 && (player.gender != 3 || Mutations.rand(2) == 0)) {
                    this.outputText("filling ", false);
                    if (player.cocks.length > 1)
                        this.outputText("each of ", false);
                    this.outputText("your " + this.multiCockDescriptLight() + " with the desire to breed.  You get a bit hornier when you realize your sex-drive has gotten a boost.", false);
                }
                //(COOCH)
                else if (player.hasVagina())
                    this.outputText("puddling in your " + this.vaginaDescript(0) + ".  An instinctive desire to mate and lay eggs spreads through you, increasing your lust and boosting your sex-drive.", false);
                //(TARDS)
                else
                    this.outputText("puddling in your featureless crotch for a split-second before it slides into your " + this.assDescript() + ".  You want to be fucked, filled, and perhaps even gain a proper gender again.  Through the lust you realize your sex-drive has been permanently increased.", false);
            ...
            if (player.lizardCocks() == 0 && player.cockTotal() > 0 && changes < changeLimit && Mutations.rand(4) == 0) {
                //Find the first non-lizzy dick
                for (temp2 = 0; temp2 < player.cocks.length; temp2++) {
                    //Stop loopahn when dick be found
                    if (player.cocks[temp2].cockType != CockTypesEnum_1.CockTypesEnum.LIZARD)
                        break;
                }
                this.outputText("\n\nA slow tingle warms your groin.  Before it can progress any further, you yank back your " + player.armorName + " to investigate.  Your " + this.cockDescript(temp2) + " is changing!  It ripples loosely from ", false);
                if (player.hasSheath())
                    this.outputText("sheath ", false);
                else
                    this.outputText("base ", false);
                this.outputText("to tip, undulating and convulsing as its color lightens, darkens, and finally settles on a purplish hue.  Your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.HUMAN) + " resolves itself into a bulbous form, with a slightly pointed tip.  The 'bulbs' throughout its shape look like they would provide an interesting ride for your sexual partners, but the perverse, alien pecker ", false);
                if (player.cor < 33)
                    this.outputText("horrifies you.", false);
                else if (player.cor < 66)
                    this.outputText("is a little strange for your tastes.", false);
                else {
                    this.outputText("looks like it might be more fun to receive than use on others.  ", false);
                    if (player.hasVagina())
                        this.outputText("Maybe you could find someone else with one to ride?", false);
                    else
                        this.outputText("Maybe you should test it out on someone and ask them exactly how it feels?", false);
                }
                this.outputText("  <b>You now have a bulbous, lizard-like cock.</b>", false);
                //Actually xform it nau
                if (player.hasSheath()) {
                    player.cocks[temp2].cockType = CockTypesEnum_1.CockTypesEnum.LIZARD;
                    if (!player.hasSheath())
                        this.outputText("\n\nYour sheath tightens and starts to smooth out, revealing ever greater amounts of your " + this.cockDescript(temp2) + "'s lower portions.  After a few moments <b>your groin is no longer so animalistic – the sheath is gone.</b>", false);
                }
                else
                    player.cocks[temp2].cockType = CockTypesEnum_1.CockTypesEnum.LIZARD;
                changes++;
            ...
            if (player.cockTotal() > 1 && player.lizardCocks() > 0 && player.cockTotal() > player.lizardCocks() && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                if (player.cockTotal() == 2)
                    this.outputText("your other dick", false);
                else
                    this.outputText("another one of your dicks", false);
                this.outputText(" starts to change into the strange reptilian shape you've grown familiar with.  It warps visibly, trembling and radiating pleasurable feelings back to you as the transformation progresses.  ", false);
                if (player.cumQ() < 50)
                    this.outputText("pre-cum oozes from the tip", false);
                else if (player.cumQ() < 700)
                    this.outputText("Thick pre-cum rains from the tip", false);
                else
                    this.outputText("A wave of pre-cum splatters on the ground", false);
                this.outputText(" from the pleasure of the change.  In moments <b>you have a bulbous, lizard-like cock.</b>", false);
            ...
            if (player.lizardCocks() == 1 && player.cocks.length == 1 && Mutations.rand(4) == 0 && changes < changeLimit) {
                this.outputText("\n\nA knot of pressure forms in your groin, forcing you off your " + player.feet() + " as you try to endure it.  You examine the affected area and see a lump starting to bulge under your " + player.skinDesc + ", adjacent to your " + this.cockDescript(0) + ".  The flesh darkens, turning purple", false);
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR || player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES)
                    this.outputText(" and shedding " + player.skinDesc, false);
                ...
                player.createCock();
                player.cocks[1].cockType = CockTypesEnum_1.CockTypesEnum.LIZARD;
            ...
            //Require mammals?
            if (player.lizardCocks() == player.cockTotal() && changes < changeLimit && player.findStatusAffect(StatusAffects_1.StatusAffects.Infested) >= 0) {
                ...
                if (player.balls > 1)
                    this.outputText("  The remaining " + Mutations.num2Text(player.balls - 1) + " slither out the pre-stretched holes with ease, though the last one hangs from your tip for a moment before dropping to the ground.", false);
                this.outputText("  The white creature joins its kin on the ground and slowly slithers away.  Perhaps they prefer mammals? In any event, <b>you are no longer infected with worms</b>.", false);
                player.removeStatusAffect(StatusAffects_1.StatusAffects.Infested);
                changes++;
            }
            //-Breasts vanish to 0 rating if male
            if (player.biggestTitSize() >= 1 && player.gender == 1 && changes < changeLimit && Mutations.rand(3) == 0) {
                //(HUEG)
                ...
                //(BOTH – no new PG)
                this.outputText("  With the change in weight and gravity, you find it's gotten much easier to move about.", false);
            ...
            if (player.biggestLactation() >= 1 && changes < changeLimit && Mutations.rand(4) == 0) {
                ...
                this.outputText(" nipples relax.  It's a strange feeling, and you pull back your top to touch one.  It feels fine, though there doesn't seem to be any milk leaking out.  You give it a squeeze and marvel when nothing ", false);
                if (player.hasFuckableNipples())
                    this.outputText("but sexual fluid ", false);
                this.outputText("escapes it.  <b>You are no longer lactating.</b>  That makes sense, only mammals lactate!  Smiling, you muse at how much time this will save you when cleaning your gear.", false);
                if (player.findPerk(PerkLib_1.PerkLib.Feeder) >= 0 || player.findStatusAffect(StatusAffects_1.StatusAffects.Feeder) >= 0) {
                    this.outputText("\n\n(<b>Feeder perk lost!</b>)", false);
                    player.removePerk(PerkLib_1.PerkLib.Feeder);
                    player.removeStatusAffect(StatusAffects_1.StatusAffects.Feeder);
                }
                changes++;
            ...
            if (player.averageNipplesPerBreast() > 1 && changes < changeLimit && Mutations.rand(4) == 0) {
                this.outputText("\n\nA chill runs over your " + this.allBreastsDescript() + " and vanishes.  You stick a hand under your " + player.armorName + " and discover that your extra nipples are missing!  You're down to just one per ", false);
                if (player.biggestTitSize() < 1)
                    this.outputText("'breast'.", false);
                else
                    this.outputText("breast.", false);
                changes++;
            ...
            //-VAGs
            if (player.hasVagina() && player.findPerk(PerkLib_1.PerkLib.Oviposition) < 0 && changes < changeLimit && Mutations.rand(5) == 0 && player.lizardScore() > 3) {
                ...
                this.outputText("(<b>Perk Gained: Oviposition</b>)", false);
                player.createPerk(PerkLib_1.PerkLib.Oviposition, 0, 0, 0, 0);
                changes++;
            }
            //Physical changes:
            //-Existing horns become draconic, max of 4, max length of 1'
            if (player.hornType != appearanceDefs_1.HORNS_DRACONIC_X4_12_INCH_LONG && changes < changeLimit && Mutations.rand(5) == 0) {
                //No dragon horns yet.
                if (player.hornType != appearanceDefs_1.HORNS_DRACONIC_X2 && player.hornType != appearanceDefs_1.HORNS_DRACONIC_X4_12_INCH_LONG) {
                    //Already have horns
                    if (player.horns > 0) {
                        //High quantity demon horns
                        if (player.hornType == appearanceDefs_1.HORNS_DEMON && player.horns > 4) {
                            ...
                            player.horns = 12;
                            player.hornType = appearanceDefs_1.HORNS_DRACONIC_X4_12_INCH_LONG;
                        }
                        else {
                            ...
                            player.hornType = appearanceDefs_1.HORNS_DRACONIC_X2;
                            if (player.horns > 13) {
                                this.outputText("  The change seems to have shrunken the horns, they're about a foot long now.", false);
                                player.horns = 12;
                            }
                        }
                        changes++;
                ...
                //ALREADY DRAGON
                    ...
                    if (player.hornType == appearanceDefs_1.HORNS_DRACONIC_X2) {
                        if (player.horns < 12) {
                            if (Mutations.rand(2) == 0) {
                            ...
                            if (player.horns >= 12)
                                this.outputText("  <b>Your horns settle down quickly, as if they're reached their full size.</b>", false);
                            changes++;
                        }
                        //maxxed out, new row
                        else {
            ...
            //-Hair stops growing!
            if (this.flags[kFLAGS_1.kFLAGS.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD] == 0 && changes < changeLimit && Mutations.rand(4) == 0) {
                this.outputText("\n\nYour scalp tingles oddly.  In a panic, you reach up to your " + this.hairDescript() + ", but thankfully it appears unchanged.\n\n", false);
                this.outputText("(<b>Your hair has stopped growing.</b>)", false);
                changes++;
                this.flags[kFLAGS_1.kFLAGS.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD]++;
            }
            //Big physical changes:
            //-Legs – Draconic, clawed feet
            if (player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_LIZARD && changes < changeLimit && Mutations.rand(5) == 0) {
                //Hooves -
                if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HOOFED)
                ...
                //TAURS -
                else if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_CENTAUR)
                ...
                //feet types -
                else if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HUMAN || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DOG || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DEMONIC_HIGH_HEELS || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DEMONIC_CLAWS || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_BEE || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_CAT || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_LIZARD)
                ...
                this.outputText("  <b>You have reptilian legs and claws!</b>", false);
                player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_LIZARD;
                changes++;
            }
            //-Tail – sinuous lizard tail
            if (player.tailType != appearanceDefs_1.TAIL_TYPE_LIZARD && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_LIZARD && changes < changeLimit && Mutations.rand(5) == 0) {
                //No tail
                if (player.tailType == appearanceDefs_1.TAIL_TYPE_NONE)
                    this.outputText("\n\nYou drop onto the ground as your spine twists and grows, forcing the flesh above your " + this.assDescript() + " to bulge out.  New bones form, one after another, building a tapered, prehensile tail onto the back of your body.  <b>You now have a reptilian tail!</b>", false);
                //Yes tail
                else
                ...
                player.tailType = appearanceDefs_1.TAIL_TYPE_LIZARD;
                changes++;
            }
            //Remove odd eyes
            if (changes < changeLimit && Mutations.rand(5) == 0 && player.eyeType > appearanceDefs_1.EYES_HUMAN) {
            ...
            if (player.earType != appearanceDefs_1.EARS_LIZARD && player.tailType == appearanceDefs_1.TAIL_TYPE_LIZARD && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_LIZARD && changes < changeLimit && Mutations.rand(5) == 0) {
                ...
                player.earType = appearanceDefs_1.EARS_LIZARD;
                changes++;
            }
            //-Scales – color changes to red, green, white, blue, or black.  Rarely: purple or silver.
            if (player.skinType != appearanceDefs_1.SKIN_TYPE_SCALES && player.earType == appearanceDefs_1.EARS_LIZARD && player.tailType == appearanceDefs_1.TAIL_TYPE_LIZARD && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_LIZARD && changes < changeLimit && Mutations.rand(5) == 0) {
                //(fur)
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR) {
                    //set new skinTone
                    if (Mutations.rand(10) == 0) {
                ...
                //(no fur)
                    ...
                    this.outputText(player.skinTone + " scales.</b>", false);
                }
                player.skinType = appearanceDefs_1.SKIN_TYPE_SCALES;
                player.skinDesc = "scales";
                changes++;
            }
            //-Lizard-like face.
            if (player.faceType != appearanceDefs_1.FACE_LIZARD && player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES && player.earType == appearanceDefs_1.EARS_LIZARD && player.tailType == appearanceDefs_1.TAIL_TYPE_LIZARD && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_LIZARD && changes < changeLimit && Mutations.rand(5) == 0) {
                ...
                player.faceType = appearanceDefs_1.FACE_LIZARD;
            }
            if (Mutations.rand(4) == 0 && player.gills && changes < changeLimit) {
                this.outputText("\n\nYour chest itches, and as you reach up to scratch it, you realize your gills have withdrawn into your skin.", false);
                player.gills = false;
        ...
        */
        neonPinkEgg(pregnantChange, player) {
            var changes = 0;
            var changeLimit = 1;
            if (Mutations.rand(2) == 0)
                changeLimit++;
            ...
            if (!pregnantChange)
                this.outputText("You eat the neon pink egg, and to your delight it tastes sweet, like candy.  In seconds you've gobbled down the entire thing, and you lick your fingers clean before you realize you ate the shell – and it still tasted like candy.", false);
            //If pregnancy, warning!
            if (pregnantChange) {
                this.outputText("\n<b>Your egg-stuffed ", false);
                if (player.pregnancyType == PregnancyStore_1.PregnancyStore.PREGNANCY_BUNNY) {
                    this.outputText("womb ", false);
                    if (player.buttPregnancyType == PregnancyStore_1.PregnancyStore.PREGNANCY_BUNNY)
                        this.outputText("and ", false);
                }
                if (player.buttPregnancyType == PregnancyStore_1.PregnancyStore.PREGNANCY_BUNNY)
                    this.outputText("backdoor ", false);
                if (player.buttPregnancyType == PregnancyStore_1.PregnancyStore.PREGNANCY_BUNNY && player.pregnancyType == PregnancyStore_1.PregnancyStore.PREGNANCY_BUNNY)
                    this.outputText("rumble", false);
                else
                    this.outputText("rumbles", false);
                this.outputText(" oddly, and you have a hunch that something's about to change</b>.", false);
            }
            //STATS CHANGURYUUUUU
            //Boost speed (max 80!)
            ...
            if (changes < changeLimit && Mutations.rand(5) == 0) {
                ...
                if (player.lib < 60) {
                    ...
                    if (!player.hasCock() || (player.gender == 3 && Mutations.rand(2) == 0)) {
                        if (player.lib < 30) {
                            ...
                            if (player.cor < 25)
                                this.outputText("You're repulsed by such shameful thoughts.", false);
                            else if (player.cor < 60)
                                this.outputText("You worry that this place is really getting to you.", false);
                            else if (player.cor < 90)
                                this.outputText("You pant a little and wonder where the nearest fertile male is.", false);
                            else
                                this.outputText("You grunt and groan with desire and disappointment.  You should get bred soon!", false);
                        }
                        else
                    ...
                    if (player.hasCock()) {
                        if (player.lib < 30) {
                            ...
                            if (Mutations.rand(2) == 0)
                                this.outputText("female hare until she's immobilized by all her eggs", false);
                            else
                                this.outputText("herm rabbit until her sack is so swollen that she's forced to masturbate over and over again just to regain mobility", false);
                            this.outputText(". ", false);
                            if (player.cor < 25)
                                this.outputText("You're repulsed by such shameful thoughts.", false);
                                ...
                                this.outputText("You pant a little and wonder where the nearest fertile female is.", false);
                            else
                                this.outputText("You grunt and groan with desire and disappointment.  Gods you need to fuck!", false);
                        }
                        else
                            this.outputText("\n\nYour mouth rolls open as you start to pant with desire.  Did it get hotter?  Your hand reaches down to " + this.sMultiCockDesc() + ", and you groan from how tight and hard it feels.  The desire to squeeze it, not with your hand but with a tight pussy or puckered asshole, runs through you like a wave, steadily increasing your desire for sex.", false);
                    }
                }
                //Libido over 60? FUCK YEAH!
                    ...
                    if (player.skinType > appearanceDefs_1.SKIN_TYPE_PLAIN)
                        this.outputText(" through your " + player.skinDesc, false);
                    this.outputText(".  ", false);
                    if (player.gender == 1)
                        this.outputText("Compression tightens down on " + this.sMultiCockDesc() + " as it strains against your " + player.armorName + ".  You struggle to fight down your heightened libido, but it's hard – so very hard.", false);
                    else if (player.gender == 0)
                        this.outputText("Sexual hunger seems to gnaw at your " + this.assholeDescript() + ", demanding it be filled, but you try to resist your heightened libido.  It's so very, very hard.", false);
                    else if (player.gender == 2)
                        this.outputText("Moisture grows between your rapidly-engorging vulva, making you squish and squirm as you try to fight down your heightened libido, but it's hard – so very hard.", false);
                    else
                        this.outputText("Steamy moisture and tight compression war for your awareness in your groin as " + this.sMultiCockDesc() + " starts to strain against your " + player.armorName + ".  Your vulva engorges with blood, growing slicker and wetter.  You try so hard to fight down your heightened libido, but it's so very, very hard.  The urge to breed lingers in your mind, threatening to rear its ugly head.", false);
                }
                //MEGALIBIDO
                else {
                    ...
                    if (player.cor < 33)
                        this.outputText("You sigh, trying not to give in completely.", false);
                    else if (player.cor < 66)
                        this.outputText("You pant and groan, not sure how long you'll even want to resist.", false);
                    else {
                        this.outputText("You smile and wonder if you can ", false);
                        if (player.lib < 100)
                            this.outputText("get your libido even higher.", false);
                        else
                            this.outputText("find someone to fuck right now.", false);
                    }
                }
            }
            ...
            if (player.sens < 60 && changes < changeLimit && Mutations.rand(3) == 0) {
                ...
                if (Mutations.rand(3) != 2) {
                    this.outputText("The feeling of small breezes blowing over your " + player.skinDesc + " gets a little bit stronger.  How strange.  You pinch yourself and nearly jump when it hurts a tad more than you'd think. You've gotten more sensitive!", false);
                    this.dynStats("sen", 5);
                }
                //(BIG boost 1/3 chance)
                else {
                    this.dynStats("sen", 15);
                    this.outputText("Every movement of your body seems to bring heightened waves of sensation that make you woozy.  Your " + player.armorName + " rubs your " + this.nippleDescript(0) + "s deliciously", false);
                    if (player.hasFuckableNipples()) {
                        this.outputText(", sticking to the ", false);
                        if (player.biggestLactation() > 2)
                            this.outputText("milk-leaking nipple-twats", false);
                        else
                            this.outputText("slippery nipple-twats", false);
                    }
                    else if (player.biggestLactation() > 2)
                        this.outputText(", sliding over the milk-leaking teats with ease", false);
                    else
                        this.outputText(" catching on each of the hard nubs repeatedly", false);
                    this.outputText(".  Meanwhile, your crotch... your crotch is filled with such heavenly sensations from ", false);
                    if (player.gender == 1) {
                        this.outputText(this.sMultiCockDesc() + " and your ", false);
                        if (player.balls > 0)
                            this.outputText(this.ballsDescriptLight(), false);
                        else
                            this.outputText(this.assholeDescript(), false);
                    }
                    else if (player.gender == 2)
                        this.outputText("your " + this.vaginaDescript(0) + " and " + this.clitDescript(), false);
                    else if (player.gender == 3) {
                        this.outputText(this.sMultiCockDesc() + ", ", false);
                        if (player.balls > 0)
                            this.outputText(this.ballsDescriptLight() + ", ", false);
                        this.outputText(this.vaginaDescript(0) + ", and " + this.clitDescript(), false);
                    }
                    //oh god genderless
                    else
                        this.outputText("you " + this.assholeDescript(), false);
                    this.outputText(" that you have to stay stock-still to keep yourself from falling down and masturbating on the spot.  Thankfully the orgy of tactile bliss fades after a minute, but you still feel way more sensitive than your previous norm.  This will take some getting used to!", false);
                }
            }
            ...
            if (player.wetness() > 3 && changes < changeLimit && Mutations.rand(3) == 0) {
                //Just to be safe
                if (player.hasVagina()) {
                    this.outputText("\n\nThe constant flow of fluids that sluice from your " + this.vaginaDescript(0) + " slow down, leaving you feeling a bit less like a sexual slip-'n-slide.", false);
                    player.vaginas[0].vaginalWetness--;
                    changes++;
                }
            }
            //Fertility boost!
            if (changes < changeLimit && Mutations.rand(4) == 0 && player.fertility < 50 && player.hasVagina()) {
                player.fertility += 2 + Mutations.rand(5);
                changes++;
            ...
            //-VAGs
            if (player.hasVagina() && player.findPerk(PerkLib_1.PerkLib.BunnyEggs) < 0 && changes < changeLimit && Mutations.rand(4) == 0 && player.bunnyScore() > 3) {
                this.outputText("\n\nDeep inside yourself there is a change.  It makes you feel a little woozy, but passes quickly.  Beyond that, you aren't sure exactly what just happened, but you are sure it originated from your womb.\n\n", false);
                this.outputText("(<b>Perk Gained: Bunny Eggs</b>)", false);
                player.createPerk(PerkLib_1.PerkLib.BunnyEggs, 0, 0, 0, 0);
                changes++;
            }
            //Shrink Balls!
            if (player.balls > 0 && player.ballSize > 5 && Mutations.rand(3) == 0 && changes < changeLimit) {
                if (player.ballSize < 10) {
                    this.outputText("\n\nRelief washes through your groin as your " + this.ballsDescript() + " lose about an inch of their diameter.", false);
                    player.ballSize--;
                }
                else if (player.ballSize < 25) {
                    this.outputText("\n\nRelief washes through your groin as your " + this.ballsDescript() + " lose a few inches of their diameter.  Wow, it feels so much easier to move!", false);
                    player.ballSize -= (2 + Mutations.rand(3));
                }
                else {
                    this.outputText("\n\nRelief washes through your groin as your " + this.ballsDescript() + " lose at least six inches of diameter.  Wow, it feels SOOOO much easier to move!", false);
            ...
            if (player.balls > 2 && changes < changeLimit && Mutations.rand(3) == 0) {
                changes++;
                this.outputText("\n\nThere's a tightening in your " + this.sackDescript() + " that only gets higher and higher until you're doubled over and wheezing.  When it passes, you reach down and discover that <b>two of your testicles are gone.</b>", false);
                player.balls -= 2;
            }
            //Boost cum production
            if ((player.balls > 0 || player.hasCock()) && player.cumQ() < 3000 && Mutations.rand(3) == 0 && changeLimit > 1) {
                changes++;
                ...
                if (player.balls > 0) {
                    ...
                    if (player.cumQ() < 50)
                        this.outputText("\n\nA twinge of discomfort runs through your " + this.ballsDescriptLight() + ", but quickly vanishes.  You heft your orbs but they haven't changed in size – they just feel a little bit denser.", false);
                    //(medium cum quantity) < 250
                    else if (player.cumQ() < 250) {
                        this.outputText("\n\nA ripple of discomfort runs through your " + this.ballsDescriptLight() + ", but it fades into a pleasant tingling.  You reach down to heft the orbs experimentally but they don't seem any larger.", false);
                        if (player.hasCock())
                            this.outputText("  In the process, you brush " + this.sMultiCockDesc() + " and discover a bead of pre leaking at the tip.", false);
                    }
                    //(large cum quantity) < 750
                    else if (player.cumQ() < 750) {
                        this.outputText("\n\nA strong contraction passes through your " + this.sackDescript() + ", almost painful in its intensity.  ", false);
                        if (player.hasCock())
                            this.outputText(this.SMultiCockDesc() + " leaks and dribbles pre-cum down your " + player.legs() + " as your body's cum production kicks up even higher.", false);
                        else
                            this.outputText("You wince, feeling pent up and yet unable to release.  You really wish you had a cock right about now.", false);
                    }
                    //(XL cum quantity) < 2000
                    else if (player.cumQ() < 2000) {
                        this.outputText("\n\nAn orgasmic contraction wracks your " + this.ballsDescriptLight() + ", shivering through the potent orbs and passing as quickly as it came.  ", false);
                        if (player.hasCock())
                            this.outputText("A thick trail of slime leaks from " + this.sMultiCockDesc() + " down your " + player.leg() + ", pooling below you.", false);
                        else
                            this.outputText("You grunt, feeling terribly pent-up and needing to release.  Maybe you should get a penis to go with these balls...", false);
                        this.outputText("  It's quite obvious that your cum production has gone up again.", false);
                    }
                    //(XXL cum quantity)
                    else {
                        this.outputText("\n\nA body-wrenching contraction thrums through your " + this.ballsDescriptLight() + ", bringing with it the orgasmic feeling of your body kicking into cum-production overdrive.  ", false);
                        if (player.hasCock())
                            this.outputText("pre-cum explodes from " + this.sMultiCockDesc() + ", running down your " + player.leg() + " and splattering into puddles that would shame the orgasms of lesser " + player.mf("males", "persons") + ".  You rub yourself a few times, nearly starting to masturbate on the spot, but you control yourself and refrain for now.", false);
                        else
                            this.outputText("You pant and groan but the pleasure just turns to pain.  You're so backed up – if only you had some way to vent all your seed!", false);
                    }
                }
                //NO BALLZ (guaranteed cock tho)
                    ...
                    //(Medium cum quantity) < 250)
                        ...
                        this.outputText("\n\nA ripple of discomfort runs through your body, but it fades into a pleasant tingling that rushes down to " + this.sMultiCockDesc() + ".  You reach down to heft yourself experimentally and smile when you see pre-beading from your maleness.  Your cum production has increased!", false);
                    //(large cum quantity) < 750
                    else if (player.cumQ() < 750)
                        this.outputText("\n\nA strong contraction passes through your body, almost painful in its intensity.  " + this.SMultiCockDesc() + " leaks and dribbles pre-cum down your " + player.legs() + " as your body's cum production kicks up even higher!  Wow, it feels kind of... good.", false);
                    //(XL cum quantity) < 2000
                    else if (player.cumQ() < 2000)
                        this.outputText("\n\nAn orgasmic contraction wracks your abdomen, shivering through your midsection and down towards your groin.  A thick trail of slime leaks from " + this.sMultiCockDesc() + "  and trails down your " + player.leg() + ", pooling below you.  It's quite obvious that your body is producing even more cum now.", false);
                    //(XXL cum quantity)
                    else
                        this.outputText("\n\nA body-wrenching contraction thrums through your gut, bringing with it the orgasmic feeling of your body kicking into cum-production overdrive.  pre-cum explodes from " + this.sMultiCockDesc() + ", running down your " + player.legs() + " and splattering into puddles that would shame the orgasms of lesser " + player.mf("males", "persons") + ".  You rub yourself a few times, nearly starting to masturbate on the spot, but you control yourself and refrain for now.", false);
                }
            }
            //Bunny feet! - requirez earz
            if (player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_BUNNY && changes < changeLimit && Mutations.rand(5) == 0 && player.earType == appearanceDefs_1.EARS_BUNNY) {
                //Taurs
                if (player.isTaur())
                ...
                //Non-taurs
                    ...
                    this.outputText("\n\nNumbness envelops your " + player.legs() + " as they pull tighter and tighter.  You overbalance and drop on your " + this.assDescript(), false);
                    if (player.tailType > appearanceDefs_1.TAIL_TYPE_NONE)
                        this.outputText(", nearly smashing your tail flat", false);
                    else
                        this.outputText(" hard enough to sting", false);
                    this.outputText(" while the change works its way through you.  Once it finishes, <b>you discover that you now have fuzzy bunny feet and legs</b>!", false);
                }
                changes++;
                player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_BUNNY;
            }
            //BUN FACE!  REQUIREZ EARZ
            if (player.earType == appearanceDefs_1.EARS_BUNNY && player.faceType != appearanceDefs_1.FACE_BUNNY && Mutations.rand(3) == 0 && changes < changeLimit) {
                this.outputText("\n\n", false);
                changes++;
                //Human(ish) face
                if (player.faceType == appearanceDefs_1.FACE_HUMAN || player.faceType == appearanceDefs_1.FACE_SHARK_TEETH)
                    this.outputText("You catch your nose twitching on its own at the bottom of your vision, but as soon as you focus on it, it stops.  A moment later, some of your teeth tingle and brush past your lips, exposing a white pair of buckteeth!  <b>Your face has taken on some rabbit-like characteristics!</b>", false);
                //Crazy furry TF shit
                else
                    this.outputText("You grunt as your " + player.face() + " twists and reforms.  Even your teeth ache as their positions are rearranged to match some new, undetermined order.  When the process finishes, <b>you're left with a perfectly human looking face, save for your constantly twitching nose and prominent buck-teeth.</b>", false);
                player.faceType = appearanceDefs_1.FACE_BUNNY;
            }
            //DAH BUNBUN EARZ - requires poofbutt!
            if (player.earType != appearanceDefs_1.EARS_BUNNY && changes < changeLimit && Mutations.rand(3) == 0 && player.tailType == appearanceDefs_1.TAIL_TYPE_RABBIT) {
                ...
                player.earType = appearanceDefs_1.EARS_BUNNY;
                changes++;
            }
            //DAH BUNBUNTAILZ
            if (player.tailType != appearanceDefs_1.TAIL_TYPE_RABBIT && Mutations.rand(2) == 0 && changes < changeLimit) {
                if (player.tailType > appearanceDefs_1.TAIL_TYPE_NONE)
                ...
                player.tailType = appearanceDefs_1.TAIL_TYPE_RABBIT;
                changes++;
            }
            if (Mutations.rand(4) == 0 && player.gills && changes < changeLimit) {
                this.outputText("\n\nYour chest itches, and as you reach up to scratch it, you realize your gills have withdrawn into your skin.", false);
        ...
        goldenSeed(type, player) {
            ...
            //Generic eating text:
            this.outputText("", true);
            this.outputText("You pop the nut into your mouth, chewing the delicious treat and swallowing it quickly.  No wonder harpies love these things so much!", false);
            //****************
            //Stats:
            //****************
            ...
            if ((player.lib < 90 || Mutations.rand(3) == 0) && Mutations.rand(3) == 0 && changes < changeLimit) {
                changes++;
                if (player.lib < 90)
                    this.dynStats("lib", 1);
                //(sub 40 lib)
                if (player.lib < 40) {
                    ...
                    if (player.hasVagina())
                        this.outputText(" The moistness of your " + this.vaginaDescript() + " seems to agree.", false);
                    else if (player.hasCock())
                        this.outputText(" The hardness of " + this.sMultiCockDesc() + " seems to agree.", false);
                    this.dynStats("lus", 5);
                }
                //(sub 75 lib)
            ...
            if (player.gender == 0 || (!player.hasVagina() && changes < changeLimit && Mutations.rand(3) == 0)) {
                changes++;
                //(balls)
                if (player.balls > 0)
                    this.outputText("\n\nAn itch starts behind your " + this.ballsDescriptLight() + ", but before you can reach under to scratch it, the discomfort fades. A moment later a warm, wet feeling brushes your " + this.sackDescript() + ", and curious about the sensation, <b>you lift up your balls to reveal your new vagina.</b>", false);
                //(dick)
                    ...
                    this.outputText("\n\nAn itch starts on your groin, just below your " + this.multiCockDescriptLight() + ". You pull your manhood aside to give you a better view, and you're able to watch as <b>your skin splits to give you a new vagina, complete with a tiny clit.</b>", false);
                //(neither)
                else
                    this.outputText("\n\nAn itch starts on your groin and fades before you can take action. Curious about the intermittent sensation, <b>you peek under your " + player.armorName + " to discover your brand new vagina, complete with pussy lips and a tiny clit.</b>", false);
                player.createVagina();
            ...
            //-Remove extra breast rows
            if (changes < changeLimit && player.breastRows.length > 1 && Mutations.rand(3) == 0 && !this.flags[kFLAGS_1.kFLAGS.HYPER_HAPPY]) {
                changes++;
                this.outputText("\n\nYou stumble back when your center of balance shifts, and though you adjust before you can fall over, you're left to watch in awe as your bottom-most " + this.breastDescript(player.breastRows.length - 1) + " shrink down, disappearing completely into your ", false);
            ...
            else if (changes < changeLimit && player.breastRows.length == 1 && Mutations.rand(3) == 0 && player.breastRows[0].breastRating >= 7 && !this.flags[kFLAGS_1.kFLAGS.HYPER_HAPPY]) {
                changes++;
            ...
            if (changes < changeLimit && player.breastRows[0].breastRating < 2 && Mutations.rand(3) == 0) {
                changes++;
                ...
                if (player.breastRows[0].breastRating < 1)
                    this.outputText("  <b>You have breasts now!</b>", false);
                player.breastRows[0].breastRating = 2;
            }
            //****************
            //General Appearance:
            ...
            if (player.femininity < 85 && changes < changeLimit && Mutations.rand(3) == 0) {
                changes++;
                this.outputText(player.modFem(85, 3 + Mutations.rand(5)), false);
            }
            //-Skin color change – tan, olive, dark, light
            if ((player.skinTone != "tan" && player.skinTone != "olive" && player.skinTone != "dark" && player.skinTone != "light") && changes < changeLimit && Mutations.rand(5) == 0) {
                changes++;
                this.outputText("\n\nIt takes a while for you to notice, but <b>", false);
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR)
                    this.outputText("the skin under your " + player.hairColor + " " + player.skinDesc, false);
                else
                    this.outputText("your " + player.skinDesc, false);
                this.outputText(" has changed to become ", false);
                ...
                this.outputText(player.skinTone + " colored.</b>", false);
            }
            //-Grow hips out if narrow.
            if (player.hipRating < 10 && changes < changeLimit && Mutations.rand(3) == 0) {
                this.outputText("\n\nYour gait shifts slightly to accommodate your widening " + this.hipDescript() + ". The change is subtle, but they're definitely broader.", false);
                player.hipRating++;
                changes++;
            }
            //-Narrow hips if crazy wide
            if (player.hipRating >= 15 && changes < changeLimit && Mutations.rand(3) == 0) {
                this.outputText("\n\nYour gait shifts inward, your " + this.hipDescript() + " narrowing significantly. They remain quite thick, but they're not as absurdly wide as before.", false);
                player.hipRating--;
                changes++;
            }
            //-Big booty
            if (player.buttRating < 8 && changes < changeLimit && Mutations.rand(3) == 0) {
                player.buttRating++;
                changes++;
                this.outputText("\n\nA slight jiggle works through your rear, but instead of stopping it starts again. You can actually feel your " + player.armorName + " being filled out by the growing cheeks. When it stops, you find yourself the proud owner of a " + this.buttDescript() + ".", false);
            }
            //-Narrow booty if crazy huge.
            if (player.buttRating >= 14 && changes < changeLimit && Mutations.rand(4) == 0) {
                changes++;
                player.buttRating--;
                this.outputText("\n\nA feeling of tightness starts in your " + this.buttDescript() + ", increasing gradually. The sensation grows and grows, but as it does your center of balance shifts. You reach back to feel yourself, and sure enough your massive booty is shrinking into a more manageable size.", false);
            }
            //-Body thickness to 25ish
            if (player.thickness > 25 && changes < changeLimit && Mutations.rand(3) == 0) {
                this.outputText(player.modThickness(25, 3 + Mutations.rand(4)), false);
                changes++;
            }
            //Remove odd eyes
            if (changes < changeLimit && Mutations.rand(5) == 0 && player.eyeType > appearanceDefs_1.EYES_HUMAN) {
            ...
            //****************
            //-Harpy legs
            if (player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_HARPY && changes < changeLimit && (type == 1 || player.tailType == appearanceDefs_1.TAIL_TYPE_HARPY) && Mutations.rand(4) == 0) {
                //(biped/taur)
                if (!player.isGoo())
                ...
                player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_HARPY;
                changes++;
                //(cont)
                this.outputText("While humanoid in shape, they have two large, taloned toes on the front and a single claw protruding from the heel. The entire ensemble is coated in " + player.hairColor + " feathers from ankle to hip, reminding you of the bird-women of the mountains. <b>You now have harpy legs!</b>", false);
            }
            //-Feathery Tail
            if (player.tailType != appearanceDefs_1.TAIL_TYPE_HARPY && changes < changeLimit && (type == 1 || player.wingType == appearanceDefs_1.WING_TYPE_FEATHERED_LARGE) && Mutations.rand(4) == 0) {
                //(tail)
                if (player.tailType > appearanceDefs_1.TAIL_TYPE_NONE)
                    this.outputText("\n\nYour tail shortens, folding into the crack of your " + this.buttDescript() + " before it disappears. A moment later, a fan of feathers erupts in its place, fluffing up and down instinctively every time the breeze shifts. <b>You have a feathery harpy tail!</b>", false);
                //(no tail)
                else
                ...
                player.tailType = appearanceDefs_1.TAIL_TYPE_HARPY;
                changes++;
            }
            //-Propah Wings
            if (player.wingType == appearanceDefs_1.WING_TYPE_NONE && changes < changeLimit && (type == 1 || player.armType == appearanceDefs_1.ARM_TYPE_HARPY) && Mutations.rand(4) == 0) {
                ...
                player.wingType = appearanceDefs_1.WING_TYPE_FEATHERED_LARGE;
                player.wingDesc = "large, feathered";
                changes++;
            }
            //-Remove old wings
            if (player.wingType != appearanceDefs_1.WING_TYPE_FEATHERED_LARGE && player.wingType > appearanceDefs_1.WING_TYPE_NONE && changes < changeLimit && Mutations.rand(4) == 0) {
                if (player.wingType != appearanceDefs_1.WING_TYPE_SHARK_FIN)
            ...
            //-Feathery Arms
            if (player.armType != appearanceDefs_1.ARM_TYPE_HARPY && changes < changeLimit && (type == 1 || player.hairType == 1) && Mutations.rand(4) == 0) {
                ...
                changes++;
                player.armType = appearanceDefs_1.ARM_TYPE_HARPY;
            }
            //-Feathery Hair
            if (player.hairType != 1 && changes < changeLimit && (type == 1 || player.faceType == appearanceDefs_1.FACE_HUMAN) && Mutations.rand(4) == 0) {
            ...
            //-Human face
            if (player.faceType != appearanceDefs_1.FACE_HUMAN && changes < changeLimit && (type == 1 || (player.earType == appearanceDefs_1.EARS_HUMAN || player.earType == appearanceDefs_1.EARS_ELFIN)) && Mutations.rand(4) == 0) {
            ...
            if ((player.earType != appearanceDefs_1.EARS_HUMAN && player.earType != appearanceDefs_1.EARS_ELFIN) && changes < changeLimit && Mutations.rand(4) == 0) {
            ...
            if (player.findPerk(PerkLib_1.PerkLib.HarpyWomb) < 0 && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HARPY && player.tailType == appearanceDefs_1.TAIL_TYPE_HARPY && Mutations.rand(4) == 0 && changes < changeLimit) {
                player.createPerk(PerkLib_1.PerkLib.HarpyWomb, 0, 0, 0, 0);
        ...
        /*
         ...
         -Fur
         -Roo face*/
        kangaFruit(type, player) {
            this.outputText("", true);
            this.outputText("You squeeze the pod around the middle, forcing the end open.  Scooping out a handful of the yeasty-smelling seeds, you shovel them in your mouth.  Blech!  Tastes like soggy burnt bread... and yet, you find yourself going for another handful...", false);
            ...
            if (player.ballSize >= 4 && changes < changeLimit && Mutations.rand(2) == 0) {
                ...
                this.outputText("\n\nYour " + this.sackDescript() + " pulls tight against your groin, vibrating slightly as it changes.  Once it finishes, you give your " + this.ballsDescriptLight() + " a gentle squeeze and discover they've shrunk.  Even with the reduced volume, they feel just as heavy.", false);
                changes++;
            }
            //-Shorten clits to reasonable size
            if (player.clitLength >= 4 && changes < changeLimit && Mutations.rand(5) == 0) {
                this.outputText("\n\nPainful pricks work through your " + this.clitDescript() + ", all the way into its swollen clitoral sheath.  Gods, it feels afire with pain!  Agony runs up and down its length, and by the time the pain finally fades, the feminine organ has lost half its size.", false);
                player.clitLength /= 2;
                changes++;
            }
            //Find biggest dick!
            ...
            //-Shrink dicks down to 8\" max.
            if (player.hasCock()) {
                if (player.cocks[biggestCock].cockLength >= 16 && changes < changeLimit && Mutations.rand(5) == 0) {
                    this.outputText("\n\nA roiling inferno of heat blazes in your " + this.cockDescript(biggestCock) + ", doubling you over in the dirt.  You rock back and forth while tears run unchecked down your cheeks.  Once the pain subsides and you're able to move again, you find the poor member has lost nearly half its size.", false);
                ...
                //COCK TF!
                if (player.kangaCocks() < player.cockTotal() && (type == 1 && Mutations.rand(2) == 0) && changes < changeLimit) {
                    ...
                    if (player.hasSheath())
                        this.outputText("your sheath", false);
                    else
                        this.outputText("a sheath that forms at the base of it", false);
                    this.outputText(".  <b>You now have a kangaroo-penis!</b>", false);
                    var cockIdx = 0;
                    //Find first non-roocock!
                    while (cockIdx < player.cockTotal()) {
                        if (player.cocks[cockIdx].cockType != CockTypesEnum_1.CockTypesEnum.KANGAROO) {
                            player.cocks[cockIdx].cockType = CockTypesEnum_1.CockTypesEnum.KANGAROO;
            ...
            //****************
            //-Face (Req: Fur + Feet)
            if (player.faceType != appearanceDefs_1.FACE_KANGAROO && ((player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_KANGAROO) || type == 1) && changes < changeLimit && Mutations.rand(4) == 0) {
                //gain roo face from human/naga/shark/bun:
                if (player.faceType == appearanceDefs_1.FACE_HUMAN || player.faceType == appearanceDefs_1.FACE_SNAKE_FANGS || player.faceType == appearanceDefs_1.FACE_SHARK_TEETH || player.faceType == appearanceDefs_1.FACE_BUNNY)
                ...
                changes++;
                player.faceType = appearanceDefs_1.FACE_KANGAROO;
            }
            //-Fur (Req: Footsies)
            if (player.skinType != appearanceDefs_1.SKIN_TYPE_FUR && (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_KANGAROO || type == 1) && changes < changeLimit && Mutations.rand(4) == 0) {
            ...
            if (player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_KANGAROO && (type == 1 || player.tailType == appearanceDefs_1.TAIL_TYPE_KANGAROO) && changes < changeLimit && Mutations.rand(4) == 0) {
                ...
                else if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_NAGA)
                ...
                player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_KANGAROO;
                changes++;
            }
            //-Roo tail (Req: Ears)
            if (player.tailType != appearanceDefs_1.TAIL_TYPE_KANGAROO && changes < changeLimit && Mutations.rand(4) == 0 && (type != 1 || player.earType == appearanceDefs_1.EARS_KANGAROO)) {
                //gain roo tail:
                if (player.tailType == appearanceDefs_1.TAIL_TYPE_NONE)
                ...
                //gain roo tail from bee tail:
                else if (player.tailType == appearanceDefs_1.TAIL_TYPE_SPIDER_ADBOMEN || player.tailType == appearanceDefs_1.TAIL_TYPE_BEE_ABDOMEN) {
                    ...
                    if (player.tailType == appearanceDefs_1.TAIL_TYPE_BEE_ABDOMEN)
                        this.outputText("hollow stinger drops out of the end, taking the venom organ with it.", false);
                    else
                        this.outputText("spinnerets drop out of the end, taking the last of your webbing with it.", false);
                    this.outputText("  By the time you're back to yourself, the insectile carapace has fallen off completely, leaving you with a long, thick, fleshy tail in place of your proud, insectile abdomen.  <b>You now have a kangaroo tail!</b>  You wipe the errant spittle from your mouth as you idly bob your new tail about.", false);
                }
                ...
                player.tailType = appearanceDefs_1.TAIL_TYPE_KANGAROO;
                changes++;
            }
            //-Roo ears
            if (player.earType != appearanceDefs_1.EARS_KANGAROO && changes < changeLimit && Mutations.rand(4) == 0) {
                //Bunbun ears get special texts!
                if (player.earType == appearanceDefs_1.EARS_BUNNY)
                ...
                changes++;
                player.earType = appearanceDefs_1.EARS_KANGAROO;
            }
            //UBEROOOO
            //kangaroo perk: - any liquid or food intake will accelerate a pregnancy, but it will not progress otherwise
            if (player.findPerk(PerkLib_1.PerkLib.Diapause) < 0 && player.kangaScore() > 4 && Mutations.rand(4) == 0 && changes < changeLimit && player.hasVagina()) {
                //Perk name and description:
                player.createPerk(PerkLib_1.PerkLib.Diapause, 0, 0, 0, 0);
        ...
        giantChocolateCupcake(player) {
            ...
            this.outputText(player.modTone(0, 100), false);
            this.outputText(player.modThickness(100, 100), false);
        }
        sweetGossamer(type, player) {
            this.outputText("", true);
            var changes = 0;
            var changeLimit = 1;
            if (Mutations.rand(2) == 0)
            ...
            //Consuming Text
            if (type == 0)
                this.outputText("You wad up the sweet, pink gossamer and eat it, finding it to be delicious and chewy, almost like gum.  Munching away, your mouth generates an enormous amount of spit until you're drooling all over yourself while you devour the sweet treat.", false);
            else if (type == 1)
                this.outputText("You wad up the sweet, black gossamer and eat it, finding it to be delicious and chewy, almost like licorice.  Munching away, your mouth generates an enormous amount of spit until you're drooling all over yourself while you devour the sweet treat.", false);
            //*************
            //Stat Changes
            //*************
            ...
            if (player.str > 70 && Mutations.rand(3) == 0) {
                ...
                if (player.spiderScore() < 4)
                    this.outputText("Wait, you're not a spider, that doesn't make any sense!", false);
                else
                    this.outputText("Well, maybe you should put your nice, heavy abdomen to work.", false);
                this.dynStats("str", -1);
                changes++;
            }
            //****************
            ...
            if (player.tailType == appearanceDefs_1.TAIL_TYPE_SPIDER_ADBOMEN && player.tailRecharge < 25 && changes < changeLimit) {
                changes++;
            ...
            if (player.hasVagina()) {
                if (player.looseness() > 1 && changes < changeLimit && Mutations.rand(3) == 0) {
                    this.outputText("\n\nWith a gasp, you feel your " + this.vaginaDescript(0) + " tightening, making you leak sticky girl-juice. After a few seconds, it stops, and you rub on your " + this.vaginaDescript(0) + " excitedly. You can't wait to try this out!", false);
            ...
            if (player.ass.analLooseness > 1 && changes < changeLimit && Mutations.rand(3) == 0) {
                this.outputText("\n\nYou let out a small cry as your " + this.assholeDescript() + " shrinks, becoming smaller and tighter. When it's done, you feel much hornier and eager to stretch it out again.", false);
                this.dynStats("lib", 2, "lus", 25);
                changes++;
                player.ass.analLooseness--;
            }
            ...
            if (player.hasCock() && changes < changeLimit && Mutations.rand(4) == 0) {
                ...
                if (this.temp == 1) {
                    this.outputText("\n\nYou can feel your " + this.multiCockDescriptLight() + " filling out in your " + player.armorName + ". Pulling ", false);
                    if (player.cockTotal() == 1)
                        this.outputText("it", false);
                    else
                        this.outputText("them", false);
                    this.outputText(" out, you look closely.  ", false);
                    if (player.cockTotal() == 1)
                        this.outputText("It's", false);
                    else
                        this.outputText("They're", false);
            ...
            if (player.smallestTitSize() < 6 && changes < changeLimit && Mutations.rand(4) == 0) {
                this.outputText("\n\nAfter eating it, your chest aches and tingles, and your hands reach up to scratch at it unthinkingly.  Silently, you hope that you aren't allergic to it.  Just as you start to scratch at your " + this.breastDescript(player.smallestTitRow()) + ", your chest pushes out in slight but sudden growth.", false);
            ...
            if (player.buttRating < 11 && changes < changeLimit && Mutations.rand(4) == 0) {
                this.outputText("\n\nYou look over your shoulder at your " + this.buttDescript() + " only to see it expand just slightly. You gape in confusion before looking back at the remaining silk in your hands. You finish it anyway. Dammit!", false);
                player.buttRating++;
                changes++;
            }
            //***************
            ...
            if (player.earType != appearanceDefs_1.EARS_HUMAN && player.earType != appearanceDefs_1.EARS_ELFIN && Mutations.rand(4) == 0 && changes < changeLimit) {
            ...
            //(Fur/Scales fall out)
            if (player.skinType != appearanceDefs_1.SKIN_TYPE_PLAIN && (player.earType == appearanceDefs_1.EARS_HUMAN || player.earType == appearanceDefs_1.EARS_ELFIN) && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES)
                    this.outputText("are", false);
                else
                    this.outputText("is", false);
                this.outputText(" falling to the ground, revealing flawless, almost pearly-white skin underneath.  <b>You now have pale white skin.</b>", false);
            ...
            //(Gain human face)
            if (player.skinType == appearanceDefs_1.SKIN_TYPE_PLAIN && (player.faceType != appearanceDefs_1.FACE_SPIDER_FANGS && player.faceType != appearanceDefs_1.FACE_HUMAN) && changes < changeLimit && Mutations.rand(4) == 0) {
            ...
            if (changes < changeLimit && player.bRows() > 2 && Mutations.rand(3) == 0 && !this.flags[kFLAGS_1.kFLAGS.HYPER_HAPPY]) {
                changes++;
                this.outputText("\n\nYou stumble back when your center of balance shifts, and though you adjust before you can fall over, you're left to watch in awe as your bottom-most " + this.breastDescript(player.breastRows.length - 1) + " shrink down, disappearing completely into your ", false);
                if (player.bRows() >= 3)
                    this.outputText("abdomen", false);
            ...
            //Nipples Turn Black:
            if (player.findStatusAffect(StatusAffects_1.StatusAffects.BlackNipples) < 0 && Mutations.rand(6) == 0 && changes < changeLimit) {
                ...
                player.createStatusAffect(StatusAffects_1.StatusAffects.BlackNipples, 0, 0, 0, 0);
                changes++;
            }
            //eyes!
            if (player.skinType == appearanceDefs_1.SKIN_TYPE_PLAIN && (player.faceType != appearanceDefs_1.FACE_SPIDER_FANGS || player.faceType != appearanceDefs_1.FACE_HUMAN) && player.eyeType == appearanceDefs_1.EYES_HUMAN && Mutations.rand(4) == 0 && changes < changeLimit) {
                player.eyeType = appearanceDefs_1.EYES_FOUR_SPIDER_EYES;
                changes++;
            ...
            //(Gain spider fangs)
            if (player.faceType == appearanceDefs_1.FACE_HUMAN && player.skinType == appearanceDefs_1.SKIN_TYPE_PLAIN && changes < changeLimit && Mutations.rand(4) == 0) {
                ...
                player.faceType = appearanceDefs_1.FACE_SPIDER_FANGS;
                changes++;
            }
            //(Arms to carapace-covered arms)
            if (player.armType != appearanceDefs_1.ARM_TYPE_SPIDER && changes < changeLimit && Mutations.rand(4) == 0) {
                this.outputText("\n\n", false);
                //(Bird pretext)
                if (player.armType == appearanceDefs_1.ARM_TYPE_HARPY)
                    this.outputText("The feathers covering your arms fall away, leaving them to return to a far more human appearance.  ", false);
                ...
                player.armType = appearanceDefs_1.ARM_TYPE_SPIDER;
                changes++;
            }
            ...
            //Drider butt
            if (type == 1 && player.findPerk(PerkLib_1.PerkLib.SpiderOvipositor) < 0 && player.isDrider() && player.tailType == appearanceDefs_1.TAIL_TYPE_SPIDER_ADBOMEN && changes < changeLimit && Mutations.rand(3) == 0 && (player.hasVagina || Mutations.rand(2) == 0)) {
                ...
                //V1 - Egg Count
                //V2 - Fertilized Count
                player.createPerk(PerkLib_1.PerkLib.SpiderOvipositor, 0, 0, 0, 0);
            ...
            if (((type == 1 && player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_DRIDER_LOWER_BODY && player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_CHITINOUS_SPIDER_LEGS) || (type != 1 && player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_CHITINOUS_SPIDER_LEGS)) && (!player.isGoo() && !player.isNaga() && !player.isTaur()) && changes < changeLimit && Mutations.rand(4) == 0) {
            ...
            if (type == 1 && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_CHITINOUS_SPIDER_LEGS && Mutations.rand(4) == 0 && player.tailType == appearanceDefs_1.TAIL_TYPE_SPIDER_ADBOMEN) {
                ...
                player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_DRIDER_LOWER_BODY;
                changes++;
            }
            if (Mutations.rand(4) == 0 && player.gills && changes < changeLimit) {
                this.outputText("\n\nYour chest itches, and as you reach up to scratch it, you realize your gills have withdrawn into your skin.", false);
        ...
        */
        broBrew(player) {
            this.outputText("", true);
            //no drink for bimbos!
            if (player.findPerk(PerkLib_1.PerkLib.BimboBody) >= 0) {
                this.outputText("The stuff hits you like a giant cube, nearly staggering you as it begins to settle.", false);
                if (player.tallness < 77) {
                    player.tallness = 77;
                    this.outputText(".. Did the ground just get farther away?  You glance down and realize, you're growing!  Like a sped-up flower sprout, you keep on getting taller until finally stopping around... six and a half feet, you assume.  Huh.  You didn't expect that to happen!", false);
                }
                if (player.tone < 100) {
                ...
                if (!player.hasCock()) {
                    ...
                    if (player.balls == 0) {
                        this.outputText("  Right on cue, two cum-laden testicles drop in behind it, their contents swirling and churning.", false);
                        player.balls = 2;
                        player.ballSize = 3;
                    }
                    this.outputText("\n\n", false);
                ...
                this.outputText("Finally, you feel the transformation skittering to a halt, leaving you to openly roam your new chiseled and sex-ready body.  So what if you can barely form coherent sentences anymore?  A body like this does all the talking you need, you figure!", false);
                if (player.inte > 35) {
                    player.inte = 35;
                ...
                this.outputText("\n\n", false);
                if (player.findPerk(PerkLib_1.PerkLib.BimboBrains) >= 0)
                    this.outputText("<b>(Lost Perks - Bimbo Brains, Bimbo Body)\n", false);
                else
                    this.outputText("<b>(Lost Perk - Bimbo Body)\n", false);
                player.removePerk(PerkLib_1.PerkLib.BimboBrains);
                player.removePerk(PerkLib_1.PerkLib.BimboBody);
                player.createPerk(PerkLib_1.PerkLib.FutaForm, 0, 0, 0, 0);
                player.createPerk(PerkLib_1.PerkLib.FutaFaculties, 0, 0, 0, 0);
                this.outputText("(Gained Perks - Futa Form, Futa Faculties)</b>", false);
                player.genderCheck();
                return;
            }
            //HP restore for bros!
            if (player.findPerk(PerkLib_1.PerkLib.BroBody) >= 0 || player.findPerk(PerkLib_1.PerkLib.FutaForm) >= 0) {
            ...
            this.outputText("A churning, full sensation wells up in your gut, and without thinking, you open wide to release a massive burp. It rumbles through your chest, startling birds into flight in the distance.  Awesome!  You slam the can into your forehead hard enough to smash the fragile aluminum into a flat, crushed disc.  Damn, you feel stronger already", false);
            if (player.inte > 50)
                this.outputText(", though you're a bit worried by how much you enjoyed the simple, brutish act", false);
            this.outputText(".\n\n", false);
            //(Tits b' gone)
            if (player.biggestTitSize() >= 1) {
                this.outputText("A tingle starts in your " + this.nippleDescript(0) + "s before the tight buds grow warm, hot even.  ", false);
                if (player.biggestLactation() >= 1)
                    this.outputText("Somehow, you know that the milk you had been producing is gone, reabsorbed by your body.  ", false);
                this.outputText("They pinch in towards your core, shrinking along with your flattening " + this.allChestDesc() + ".  You shudder and flex in response.  Your chest isn't just shrinking, it's reforming, sculping itself into a massive pair of chiseled pecs.  ", false);
                if (player.breastRows.length > 1) {
                    this.outputText("The breasts below vanish entirely.  ", false);
            ...
            if (player.tone >= 70)
                this.outputText("further define themselves", false);
            else
                this.outputText("become extraordinarily visible", false);
            this.outputText(".  The strange, muscle-building changes flow down your " + player.legs() + ", making them just as fit and strong as the rest of you.  You curl your arm and kiss your massive, flexing bicep.  You're awesome!\n\n", false);
            ...
            if (player.hasCock()) {
                if (player.cocks[0].cockLength < 10) {
                    this.outputText("As if on cue, the familiar tingling gathers in your groin, and you dimly remember you have one muscle left to enlarge.  If only you had the intelligence left to realize that your penis is not a muscle.  In any event, your " + this.cockDescript(0) + " swells in size, ", false);
                    if (player.cocks[0].cockThickness < 2.75) {
                        this.outputText("thickening and ", false);
                        player.cocks[0].cockThickness = 2.75;
                    }
                    this.outputText("lengthening until it's ten inches long and almost three inches wide.  Fuck, you're hung!  ", false);
                    player.cocks[0].cockLength = 10;
                }
                //Dick already big enough! BALL CHECK!
                if (player.balls > 0) {
                    this.outputText("Churning audibly, your " + this.sackDescript() + " sways, but doesn't show any outward sign of change.  Oh well, it's probably just like, getting more endurance or something.", false);
                }
                else {
                    this.outputText("Two rounded orbs drop down below, filling out a new, fleshy sac above your " + player.legs() + ".  Sweet!  You can probably cum buckets with balls like these.", false);
                    player.balls = 2;
                    player.ballSize = 3;
                }
                this.outputText("\n\n", false);
            ...
            //(Pussy b gone)
            if (player.hasVagina()) {
                this.outputText("At the same time, your " + this.vaginaDescript(0) + " burns hot, nearly feeling on fire.  You cuss in a decidedly masculine way for a moment before the pain fades to a dull itch.  Scratching it, you discover your lady-parts are gone.  Only a sensitive patch of skin remains.\n\n", false);
                player.removeVagina(0, 1);
            }
            player.genderCheck();
            ...
            this.outputText("You finish admiring yourself and adjust your " + player.armorName + " to better fit your new physique.  Maybe there's some bitches around you can fuck.  Hell, as good as you look, you might have other dudes wanting you to fuck them too, no homo.\n\n", false);
            //max tone.  Thickness + 50
            player.modTone(100, 100);
            player.modThickness(100, 50);
            //Bonus cum production!
            player.createPerk(PerkLib_1.PerkLib.BroBrains, 0, 0, 0, 0);
            player.createPerk(PerkLib_1.PerkLib.BroBody, 0, 0, 0, 0);
            this.outputText("<b>(Bro Body - Perk Gained!)\n", false);
            this.outputText("(Bro Brains - Perk Gained!)</b>\n", false); //int to 20.  max int 50)
            if (player.findPerk(PerkLib_1.PerkLib.Feeder) >= 0) {
        ...
        ectoplasm(player) {
            ...
            if (player.lib < 100 && Mutations.rand(3) == 0 && changes < changeLimit) {
                ...
                this.outputText(" a trace amount of the ghost girl's lust is transferred into you.  How horny IS she, you have to wonder...", false);
                this.dynStats("lib", 1);
                if (player.lib < 50)
                    this.dynStats("lib", 1);
                changes++;
            ...
            if (player.hasCock() && changes < changeLimit) {
                if (Mutations.rand(3) == 0 && player.cocks[0].cockType != CockTypesEnum_1.CockTypesEnum.HUMAN) {
                    this.outputText("\n\nA strange tingling begins behind your " + this.cockDescript(0) + ", slowly crawling up across its entire length.  While neither particularly arousing nor uncomfortable, you do shift nervously as the feeling intensifies.  You resist the urge to undo your " + player.armorName + " to check, but by the feel of it, your penis is shifting form.  Eventually the transformative sensation fades, <b>leaving you with a completely human penis.</b>", false);
                    player.cocks[0].cockType = CockTypesEnum_1.CockTypesEnum.HUMAN;
                    changes++;
                }
            }
            //Appearnace Change
            ...
            //Skin
            if (Mutations.rand(4) == 0 && changes < changeLimit && (player.skinTone != "sable" && player.skinTone != "white")) {
                if (Mutations.rand(2) == 0) {
            ...
            //Legs
            if (changes < changeLimit && player.findPerk(PerkLib_1.PerkLib.Incorporeality) < 0 && (player.skinTone == "white" || player.skinTone == "sable") && player.hairType == 2) {
                ...
                this.outputText("<b>(Gained Perk:  Incorporeality</b>)", false);
                player.createPerk(PerkLib_1.PerkLib.Incorporeality, 0, 0, 0, 0);
            }
            //Effect Script 8: 100% chance of healing
            if (changes == 0) {
                this.outputText("You feel strangely refreshed, as if you just gobbled down a bottle of sunshine.  A smile graces your lips as vitality fills you.  ", false);
        ...
        isabellaMilk(player) {
            this.outputText("", true);
            this.outputText("You swallow down the bottle of Isabella's milk.", false);
            if (player.fatigue > 0)
                this.outputText("  You feel much less tired! (-33 fatigue)", false);
            this.fatigue(-33);
        }
        //TF item - Shriveled Tentacle
        //tooltip:
        shriveledTentacle(player) {
            this.outputText("", true);
            this.outputText("You chew on the rubbery tentacle; its texture and taste are somewhat comparable to squid, but the half-dormant nematocysts cause your mouth to tingle sensitively.", false);
            var changes = 0;
            var changeLimit = 1;
            if (Mutations.rand(2) == 0)
                changeLimit++;
            ...
            if (Mutations.rand(4) == 0 && changes < changeLimit && player.tailType == appearanceDefs_1.TAIL_TYPE_BEE_ABDOMEN) {
            ...
            if (Mutations.rand(4) == 0 && (player.wingType == appearanceDefs_1.WING_TYPE_BEE_LIKE_SMALL || player.wingType == appearanceDefs_1.WING_TYPE_BEE_LIKE_LARGE) && changes < changeLimit) {
            ...
            if (player.gills && player.hairType != 4 && changes < changeLimit && Mutations.rand(5) == 0) {
                ...
                player.hairType = 4;
                player.hairLength = 5;
                if (this.flags[kFLAGS_1.kFLAGS.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD] == 0) {
                    this.outputText("  <b>(Your hair has stopped growing.)</b>", false);
                    this.flags[kFLAGS_1.kFLAGS.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD] = 1;
                }
                changes++;
                changes++;
                changes++;
            ...
            if (Mutations.rand(5) == 0 && !player.gills && player.skinTone == "aphotic blue-black" && changes < changeLimit) {
                this.outputText("\n\nYou feel a pressure in your lower esophageal region and pull your garments down to check the area.  <b>Before your eyes a pair of feathery gills start to push out of the center of your chest, just below your neckline, parting sideways and draping over your " + this.nippleDescript(0) + "s.</b>  They feel a bit uncomfortable in the open air at first, but soon a thin film of mucus covers them and you hardly notice anything at all.  You redress carefully.", false);
                player.gills = true;
                changes++;
            }
            //-[aphotic] skin tone (blue-black)
            if (Mutations.rand(5) == 0 && changes < changeLimit && player.skinTone != "aphotic blue-black") {
            ...
            if (player.hairType == 4 && player.hairLength < 36 &&
                Mutations.rand(2) == 0 && changes < changeLimit) {
                this.temp = 5 + Mutations.rand(3);
                player.hairLength += this.temp;
                this.outputText("\n\nAs you laboriously chew the rubbery dried anemone, your head begins to feel heavier.  Using your newfound control, you snake one of your own tentacles forward; holding it out where you can see it, the first thing you notice is that it appears quite a bit longer.  <b>Your head-tentacles are now " + Mutations.num2Text(this.temp) + " inches longer!</b>", false);
        ...
        numbRocks(player) {
            ...
            this.outputText("You pop open the package of numb rocks and dump it into your waiting mouth.  The strange candy fizzes and pops, leaving the nerves on your tongue feeling a bit deadened as you swallow the sweet mess.", false);
            if (player.lust >= 33) {
            ...
            if (Mutations.rand(5) == 0) {
                if (player.findStatusAffect(StatusAffects_1.StatusAffects.Dysfunction) < 0) {
                    ...
                    if (!(player.hasCock() || player.hasVagina()))
                        this.outputText(this.assholeDescript() + " ", false);
                    this.outputText(" numbs up too.  You give yourself a gentle touch, but are quite disturbed when you realize you can barely feel it.  You can probably still fuck something to get off, but regular masturbation is out of the question...", false);
                    player.createStatusAffect(StatusAffects_1.StatusAffects.Dysfunction, 50 + Mutations.rand(100), 0, 0, 0);
                }
                else {
                    ...
                    player.addStatusValue(StatusAffects_1.StatusAffects.Dysfunction, 1, 50 + Mutations.rand(100));
                }
            }
            ...
            if (player.findPerk(PerkLib_1.PerkLib.ThickSkin) < 0 && player.sens < 30 && Mutations.rand(4) == 0) {
                this.outputText("Slowly, ", false);
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_PLAIN)
                    this.outputText("your skin", false);
                else
                    this.outputText("the skin under your " + player.skinDesc, false);
                ...
                player.createPerk(PerkLib_1.PerkLib.ThickSkin, 0, 0, 0, 0);
            }
        ...
        //2. Sensitivity Draft
        sensitivityDraft(player) {
            this.outputText("", true);
            this.outputText("You pop the cork on this small vial and drink down the clear liquid.  It makes your lips and tongue tingle strangely, letting you feel each globule of spit in your mouth and each breath of air as it slides past your lips.", false);
            if (player.findStatusAffect(StatusAffects_1.StatusAffects.Dysfunction) >= 0) {
                ...
                player.removeStatusAffect(StatusAffects_1.StatusAffects.Dysfunction);
            }
            if (Mutations.rand(4) == 0 && player.findStatusAffect(StatusAffects_1.StatusAffects.LustyTongue) < 0) {
                this.outputText("The constant tingling in your mouth grows and grows, particularly around your lips, until they feel as sensitive as ", false);
                if (player.hasVagina())
                    this.outputText("your", false);
                else
                    this.outputText("a woman's", false);
                this.outputText(" lower lips.  You'll have to be careful not to lick them!", false);
                //(Lustytongue status)
                player.createStatusAffect(StatusAffects_1.StatusAffects.LustyTongue, 25, 0, 0, 0);
            }
            ...
            if (player.lust > 70 || player.lib > 70) {
                this.outputText("You shiver and think of how much better it'll make sex and masturbation.", false);
            }
            else
                this.outputText("You worry it'll make it harder to resist the attentions of a demon.", false);
            this.dynStats("sen", 10, "lus", 5);
        }
        foxTF(enhanced, player) {
            this.clearOutput();
            if (!enhanced)
                this.outputText("You examine the berry a bit, rolling the orangish-red fruit in your hand for a moment before you decide to take the plunge and chow down.  It's tart and sweet at the same time, and the flavors seem to burst across your tongue with potent strength.  Juice runs from the corners of your lips as you finish the tasty snack.");
            else
                this.outputText("You pop the cap on the enhanced \"Vixen's Vigor\" and decide to take a swig of it.  Perhaps it will make you as cunning as the crude fox Lumi drew on the front?");
            var changes = 0;
            var changeLimit = 1;
            if (enhanced)
                changeLimit += 2;
            ...
            var counter = 0;
            if (player.faceType == appearanceDefs_1.FACE_FOX && player.tailType == appearanceDefs_1.TAIL_TYPE_FOX && player.earType == appearanceDefs_1.EARS_FOX && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_FOX && player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && Mutations.rand(3) == 0) {
                if (this.flags[kFLAGS_1.kFLAGS.FOX_BAD_END_WARNING] == 0) {
                    ...
                    this.flags[kFLAGS_1.kFLAGS.FOX_BAD_END_WARNING] = 1;
                }
                else {
                    this.outputText("\n\nYou scarf down the ");
                    if (enhanced)
                        this.outputText("fluid ");
                    else
                        this.outputText("berries ");
                    ...
                    if (player.tailVenom > 1)
                        this.outputText("  Your tails thrash around violently as they begin to fuse painfully back into one, the fur bristling back out with a flourish.");
            ...
            if (changes < changeLimit && Mutations.rand(3) == 0 && (player.lib < 80 || player.inte < 80 || player.sens < 80)) {
            ...
            if (player.hairColor != "golden-blonde" && player.hairColor != "reddish-orange" && player.hairColor != "silver" && player.hairColor != "white" && player.hairColor != "red" && player.hairColor != "black" && changes < changeLimit && Mutations.rand(4) == 0) {
                var hairTemp = Mutations.rand(10);
                if (hairTemp < 5)
            ...
            if (player.hairType != 4 && (player.hairLength > 26 || player.hairLength < 16) && changes < changeLimit && Mutations.rand(4) == 0) {
                if (player.hairLength < 16) {
                    player.hairLength += 1 + Mutations.rand(4);
                    this.outputText("\n\nYou experience a tingling sensation in your scalp.  Feeling a bit off-balance, you discover your hair has lengthened, becoming " + Mutations.num2Text(Math.round(player.hairLength)) + " inches long.");
                }
                else {
                    player.hairLength -= 1 + Mutations.rand(4);
                    this.outputText("\n\nYou experience a tingling sensation in your scalp.  Feeling a bit off-balance, you discover your hair has shed a bit of its length, becoming " + Mutations.num2Text(Math.round(player.hairLength)) + " inches long.");
                }
                changes++;
            }
            ...
            //dog cocks!
            if (changes < changeLimit && Mutations.rand(3) == 0 && player.dogCocks() < player.cocks.length) {
                var choices = [];
                counter = player.cockTotal();
                while (counter > 0) {
                    counter--;
                    //Add non-dog locations to the array
                    if (player.cocks[counter].cockType != CockTypesEnum_1.CockTypesEnum.DOG)
                        choices[choices.length] = counter;
                }
                if (choices.length != 0) {
                    var select = choices[Mutations.rand(choices.length)];
                    if (player.cocks[select].cockType == CockTypesEnum_1.CockTypesEnum.HUMAN) {
                        this.outputText("\n\nYour " + this.cockDescript(select) + " clenches painfully, becoming achingly, throbbingly erect.  A tightness seems to squeeze around the base, and you wince as you see your skin and flesh shifting forwards into a canine-looking sheath.  You shudder as the crown of your " + this.cockDescript(select) + " reshapes into a point, the sensations nearly too much for you.  You throw back your head as the transformation completes, your " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.DOG) + " much thicker than it ever was before.  <b>You now have a dog-cock.</b>", false);
                        player.cocks[select].cockThickness += .3;
                        this.dynStats("sen", 10, "lus", 5);
                    }
                    //Horse
                    else if (player.cocks[select].cockType == CockTypesEnum_1.CockTypesEnum.HORSE) {
                        this.outputText("\n\nYour " + Appearance_1.Appearance.cockNoun(CockTypesEnum_1.CockTypesEnum.HORSE) + " shrinks, the extra equine length seeming to shift into girth.  The flared tip vanishes into a more pointed form, a thick knotted bulge forming just above your sheath.  <b>You now have a dog-cock.</b>", false);
                        //Tweak length/thickness.
                    ...
                    //Tentacular Tuesday!
                    else if (player.cocks[select].cockType == CockTypesEnum_1.CockTypesEnum.TENTACLE) {
                        this.outputText("\n\nYour " + this.cockDescript(select) + " coils in on itself, reshaping and losing its plant-like coloration as thickens near the base, bulging out in a very canine-looking knot.  Your skin bunches painfully around the base, forming into a sheath.  <b>You now have a dog-cock.</b>", false);
                        this.dynStats("sen", 4, "lus", 10);
                    }
                    //Misc
                    else {
                        this.outputText("\n\nYour " + this.cockDescript(select) + " trembles, reshaping itself into a shiny red doggie-dick with a fat knot at the base.  <b>You now have a dog-cock.</b>", false);
                        this.dynStats("sen", 4, "lus", 10);
                    }
                    player.cocks[select].cockType = CockTypesEnum_1.CockTypesEnum.DOG;
                    player.cocks[select].knotMultiplier = 1.25;
                    changes++;
                }
            }
            //Cum Multiplier Xform
            if (player.cumQ() < 5000 && Mutations.rand(3) == 0 && changes < changeLimit && player.hasCock()) {
            ...
            if (changes < changeLimit && player.balls > 0 && player.ballSize > 4 && Mutations.rand(3) == 0) {
            ...
            if (changes < changeLimit && enhanced && player.bRows() < 4 && player.breastRows[player.bRows() - 1].breastRating > 1) {
                ...
                if (!enhanced)
                    this.outputText("berry ");
                else
                    this.outputText("drink ");
                this.outputText("settles deeper inside you.  A second later, the unpleasant gut-gurgle passes, and you let out a tiny burp of relief.  Before you finish taking a few breaths, there's an itching below your " + this.allChestDesc() + ".  You idly scratch at it, but gods be damned, it hurts!  You peel off part of your " + player.armorName + " to inspect the unwholesome itch, ");
                if (player.biggestTitSize() >= 8)
                    this.outputText("it's difficult to see past the wall of tits obscuring your view.");
                else
                    this.outputText("it's hard to get a good look at.");
            ...
            while (counter > 1) {
                ...
                if (player.breastRows[counter].breastRating <= (player.breastRows[counter - 1].breastRating - 1) && changes < changeLimit && Mutations.rand(2) == 0) {
                    if (tits)
                        ...
                        var select2 = Mutations.rand(3);
                        if (select2 == 1)
                            this.outputText("\n\nA faint warmth buzzes to the surface of your " + this.breastDescript(counter) + ", the fluttering tingles seeming to vibrate faster and faster just underneath your " + player.skin() + ".  Soon, the heat becomes uncomfortable, and that row of chest-flesh begins to feel tight, almost thrumming like a newly-stretched drum.  You " + this.nippleDescript(counter) + "s go rock hard, and though the discomforting feeling of being stretched fades, the pleasant, warm buzz remains.  It isn't until you cup your tingly tits that you realize they've grown larger, almost in envy of the pair above.");
                        else if (select2 == 2)
                            this.outputText("\n\nA faintly muffled gurgle emanates from your " + this.breastDescript(counter) + " for a split-second, just before your flesh shudders and shakes, stretching your " + player.skinFurScales() + " outward with newly grown breast.  Idly, you cup your hands to your swelling bosom, and though it stops soon, you realize that your breasts have grown closer in size to the pair above.");
                        else {
                            this.outputText("\n\nAn uncomfortable stretching sensation spreads its way across the curves of your " + this.breastDescript(counter) + ", threads of heat tingling through your flesh.  It feels as though your heartbeat has been magnified tenfold within the expanding mounds, your " + player.skin() + " growing flushed with arousal and your " + this.nippleDescript(counter) + " filling with warmth.  As the tingling heat gradually fades, a few more inches worth of jiggling breast spill forth.  Cupping them experimentally, you confirm that they have indeed grown to be a bit more in line with the size of the pair above.");
                        }
                    }
                    //Bigger change!
                    ...
                    this.outputText("  You do a quick measurement and determine that your " + Mutations.num2Text2(counter + 1) + " row of breasts are now " + player.breastCup(counter) + "s.");
                    if (!tits) {
                        tits = true;
                        changes++;
                    }
            ...
            //HEAT!
            if (player.statusAffectv2(StatusAffects_1.StatusAffects.Heat) < 30 && Mutations.rand(6) == 0 && changes < changeLimit) {
                if (player.goIntoHeat(true)) {
                    changes++;
                }
            }
            //[Grow Fur]
            //FOURTH
            if ((enhanced || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_FOX) && player.skinType != appearanceDefs_1.SKIN_TYPE_FUR && changes < changeLimit && Mutations.rand(4) == 0) {
                //from scales
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES)
            ...
            //[Grow Fox Legs]
            //THIRD
            if ((enhanced || player.earType == appearanceDefs_1.EARS_FOX) && player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_FOX && changes < changeLimit && Mutations.rand(5) == 0) {
                ...
                //other digitigrade
                else if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HOOFED || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DOG || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_CAT || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_BUNNY || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_KANGAROO)
                ...
                //red drider bb gone
                else if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DRIDER_LOWER_BODY)
                ...
                //reg legs, not digitigrade
                    ...
                    if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_BEE)
                        this.outputText(", while a hideous cracking fills the air");
                ...
                player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_FOX;
                changes++;
            }
            //Grow Fox Ears]
            //SECOND
            if ((enhanced || player.tailType == appearanceDefs_1.TAIL_TYPE_FOX) && player.earType != appearanceDefs_1.EARS_FOX && changes < changeLimit && Mutations.rand(4) == 0) {
                //from human/gob/liz ears
                if (player.earType == appearanceDefs_1.EARS_HUMAN || player.earType == appearanceDefs_1.EARS_ELFIN || player.earType == appearanceDefs_1.EARS_LIZARD) {
                ...
                player.earType = appearanceDefs_1.EARS_FOX;
                changes++;
            }
            //[Grow Fox Tail](fairly common)
            //FIRST
            if (player.tailType != appearanceDefs_1.TAIL_TYPE_FOX && changes < changeLimit && Mutations.rand(4) == 0) {
                //from no tail
                if (player.tailType == appearanceDefs_1.TAIL_TYPE_NONE)
                ...
                player.tailType = appearanceDefs_1.TAIL_TYPE_FOX;
                player.tailVenom = 1;
                changes++;
            }
            //[Grow Fox Face]
            ...
            if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && player.faceType != appearanceDefs_1.FACE_FOX && changes < changeLimit && Mutations.rand(5) == 0) {
                ...
                if (this.silly())
                    this.outputText("  And they called you crazy...");
                changes++;
                player.faceType = appearanceDefs_1.FACE_FOX;
            }
        ...
        godMead(player) {
            ...
            this.HPChange(Math.round(player.maxHP() * .33), false);
            if (Mutations.rand(3) == 0) {
        ...
        sheepMilk(player) {
            this.outputText("You gulp the bottle's contents, and its sweet taste immediately invigorates you, making you feel calm and concentrated", true);
        ...
        eatEmberEgg(player) {
            ...
            if (player.findPerk(PerkLib_1.PerkLib.Dragonfire) >= 0) {
                if (player.findStatusAffect(StatusAffects_1.StatusAffects.DragonBreathCooldown) >= 0)
                    player.removeStatusAffect(StatusAffects_1.StatusAffects.DragonBreathCooldown);
                else {
                    if (player.findStatusAffect(StatusAffects_1.StatusAffects.DragonBreathBoost) < 0)
                        player.createStatusAffect(StatusAffects_1.StatusAffects.DragonBreathBoost, 0, 0, 0, 0);
                }
        ...
        foxJewel(mystic, player) {
            ...
            //**********************
            //[increase Intelligence, Libido and Sensitivity]
            if (player.inte < 100 && changes < changeLimit && ((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(4) == 0))) {
            ...
            if (player.str > 15 && changes < changeLimit && ((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(3) == 0))) {
                this.outputText("\n\nYou can feel your muscles softening as they slowly relax, becoming a tad weaker than before.  Who needs physical strength when you can outwit your foes with trickery and mischief?  You tilt your head a bit, wondering where that thought came from.");
                this.dynStats("str", -1);
                if (player.str > 70)
                    this.dynStats("str", -1);
            ...
            if (player.tou > 20 && changes < changeLimit && ((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(3) == 0))) {
            ...
            if (((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(4) == 0)) && changes < changeLimit && player.femininity != 50) {
                this.outputText(player.modFem(50, 2), false);
                changes++;
            }
            //[decrease muscle tone toward 40]
            if (player.tone >= 40 && changes < changeLimit && ((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(4) == 0))) {
                this.outputText("\n\nMoving brings with it a little more jiggle than you're used to.  You don't seem to have gained weight, but your muscles seem less visible, and various parts of you are pleasantly softer.");
            ...
            //[Adjust hips toward 10 – wide/curvy/flared]
            //from narrow to wide
            if (player.hipRating < 10 && ((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(3) == 0)) && changes < changeLimit) {
                player.hipRating++;
                if (player.hipRating < 7)
                    player.hipRating++;
            ...
            //[Adjust hair length toward range of 16-26 – very long to ass-length]
            if ((player.hairLength < 16 || player.hairLength > 26) && ((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(3) == 0)) && changes < changeLimit) {
                //from short to long
                if (player.hairLength < 16) {
            ...
            if (player.hasVagina() && ((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(3) == 0)) && player.statusAffectv1(StatusAffects_1.StatusAffects.BonusVCapacity) < 200 && changes < changeLimit) {
                this.outputText("\n\nA gurgling sound issues from your abdomen, and you double over as a trembling ripple passes through your womb.  The flesh of your stomach roils as your internal organs begin to shift, and when the sensation finally passes, you are instinctively aware that your " + this.vaginaDescript(0) + " is a bit deeper than it was before.");
                if (player.findStatusAffect(StatusAffects_1.StatusAffects.BonusVCapacity) < 0) {
                    player.createStatusAffect(StatusAffects_1.StatusAffects.BonusVCapacity, 0, 0, 0, 0);
                }
                player.addStatusValue(StatusAffects_1.StatusAffects.BonusVCapacity, 1, 5 + Mutations.rand(10));
                changes++;
            }
            //[Vag of Holding] - rare effect, only if PC has high vaginal looseness
            else if (player.hasVagina() && ((mystic) || (!mystic && Mutations.rand(5) == 0)) && player.statusAffectv1(StatusAffects_1.StatusAffects.BonusVCapacity) >= 200 && player.statusAffectv1(StatusAffects_1.StatusAffects.BonusVCapacity) < 8000 && changes < changeLimit) {
                ...
                if (this.silly())
                    this.outputText("  <b>Your vagina has become a universe unto itself, capable of accepting colossal insertions beyond the scope of human comprehension!</b>");
                else
                    this.outputText("  <b>Your vagina is now capable of accepting even the most ludicrously sized insertions with no ill effects.</b>");
                player.changeStatusValue(StatusAffects_1.StatusAffects.BonusVCapacity, 1, 8000);
                changes++;
            }
            //**********************
            ...
            //**********************
            //[Grow Fox Tail]
            if (player.tailType != appearanceDefs_1.TAIL_TYPE_FOX && changes < changeLimit && ((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(4) == 0))) {
                //if PC has no tail
                if (player.tailType == appearanceDefs_1.TAIL_TYPE_NONE) {
                ...
                else if (player.tailType != appearanceDefs_1.TAIL_TYPE_FOX) {
            ...
            if (!mystic && player.earType == appearanceDefs_1.EARS_FOX && player.tailType == appearanceDefs_1.TAIL_TYPE_FOX && player.tailVenom == 8 && Mutations.rand(3) == 0) {
            ...
            //(rare effect, up to max of 8 tails, requires PC level and int*10 = number of tail to be added)
            else if (player.tailType == appearanceDefs_1.TAIL_TYPE_FOX && player.tailVenom < 8 && player.tailVenom + 1 <= player.level && player.tailVenom + 1 <= player.inte / 10 && changes < changeLimit && ((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(3) == 0))) {
                ...
                //else if PC has 2 or more fox tails
                    ...
                    this.outputText("\n\nA tingling pressure builds on your backside, and your bushy tails begin to glow with an eerie, ghostly light.  With a crackle of electrical energy, one of your tails splits in two, giving you " + Mutations.num2Text(player.tailVenom + 1) + "!  <b>You now have a cluster of " + Mutations.num2Text(player.tailVenom + 1) + " fox-tails.</b>");
                    //increment tail by 1
                }
                player.tailVenom++;
                changes++;
            ...
            //[Grow 9th tail and gain Corrupted Nine-tails perk]
            else if (mystic && Mutations.rand(4) == 0 && changes < changeLimit && player.tailType == appearanceDefs_1.TAIL_TYPE_FOX && player.tailVenom == 8 && player.level >= 9 && player.earType == appearanceDefs_1.EARS_FOX && player.inte >= 90 && player.findPerk(PerkLib_1.PerkLib.CorruptedNinetails) < 0 && player.findPerk(PerkLib_1.PerkLib.EnlightenedNinetails) < 0) {
                this.outputText("Your bushy tails begin to glow with an eerie, ghostly light, and with a crackle of electrical energy, split into nine tails.  <b>You are now a nine-tails!  But something is wrong...  The cosmic power radiating from your body feels...  tainted somehow.  The corruption pouring off your body feels...  good.</b>");
                ...
                player.createPerk(PerkLib_1.PerkLib.CorruptedNinetails, 0, 0, 0, 0);
                this.dynStats("lib", 2, "lus", 10, "cor", 10);
                player.tailVenom = 9;
                changes++;
            }
            //[Grow Fox Ears]
            if (player.tailType == appearanceDefs_1.TAIL_TYPE_FOX && ((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(4) == 0)) && player.earType != appearanceDefs_1.EARS_FOX && changes < changeLimit) {
            ...
            if (((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(4) == 0)) && changes < changeLimit && player.hairColor != "golden blonde" && player.hairColor != "silver blonde" && player.hairColor != "white" && player.hairColor != "black" && player.hairColor != "red") {
                var hairTemp = Mutations.rand(10);
                if (hairTemp == 0)
                    player.hairColor = "golden blonde";
                else if (hairTemp == 1)
            ...
            if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR || player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES && ((mystic) || (!mystic && Mutations.rand(2) == 0))) {
                ...
                if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR)
                    this.outputText("  You stare in horror as you pull your fingers away holding a handful of " + player.hairColor + " fur!  Your fur sloughs off your body in thick clumps, falling away to reveal patches of bare, " + player.skinTone + " skin.");
                else if (player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES)
                    this.outputText("  You stare in horror as you pull your fingers away holding a handful of dried up scales!  Your scales continue to flake and peel off your skin in thick patches, revealing the tender " + player.skinTone + " skin underneath.");
                ...
                this.outputText(player.skinTone + " complexion.");
                this.outputText("  <b>You now have " + player.skin() + "!</b>");
                changes++;
            }
            //Change skin tone if not changed you!
            else if (mystic && player.skinTone != "dark" && player.skinTone != "ebony" && player.skinTone != "ashen" && player.skinTone != "sable" && player.skinTone != "milky white" && changes < changeLimit && ((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(3) == 0))) {
                ...
                var mtoneTemp = Mutations.rand(5);
                if (mtoneTemp == 0)
                    player.skinTone = "dark";
                else if (mtoneTemp == 1)
                    player.skinTone = "ebony";
                    ...
                    player.skinTone = "milky white";
                this.outputText(player.skin() + "!</b>");
                changes++;
            }
            //Change skin tone if not changed you!
            else if (!mystic && player.skinTone != "tan" && player.skinTone != "olive" && player.skinTone != "light" && changes < changeLimit && ((mystic && Mutations.rand(2) == 0) || (!mystic && Mutations.rand(3) == 0))) {
                this.outputText("\n\nYou feel a crawling sensation on the surface of your skin, starting at the small of your back and spreading to your extremities, ultimately reaching your face.  Holding an arm up to your face, you discover that <b>you now have ");
                var toneTemp = Mutations.rand(3);
                if (toneTemp == 0)
                    player.skinTone = "tan";
                else if (toneTemp == 1)
                    player.skinTone = "olive";
            ...
            //From Tan, Olive, or Light skin tones
            else if ((mystic && false && (player.skinTone == "dark" || player.skinTone == "ebony" || player.skinTone == "ashen" || player.skinTone == "sable" || player.skinTone == "milky white")) || (!mystic && false && (player.skinTone == "tan" || player.skinTone == "olive" || player.skinTone || "light")) && changes < changeLimit) {
                ...
                if (mystic)
                    this.outputText("angular");
                else
                    this.outputText("curved");
        ...
        */
        ...
        fishFillet(player) {
            ...
            if (!this.getGame().inCombat)
                this.outputText("You sit down and unwrap your fish fillet. It's perfectly flaky, allowing you to break it off in bite-sized chunks.  The salty meal disappears quickly, and your stomach gives an appreciative gurgle.");
            //(In combat?)
            else
                this.outputText("You produce the fish fillet from your bag.  Rather than unwrap it and savor the taste as you normally would, you take a large bite out of it, leaf wrapping and all.  In no time your salty meal is gone, your stomach giving an appreciative gurgle.");
            //Increase HP by quite a bit!)
            //(Slight chance at increasing Toughness?)
            //(If lake has been tainted, +1 Corruption?)
            if (player.findStatusAffect(StatusAffects_1.StatusAffects.FactoryOverload) >= 0)
                this.dynStats("cor", 0.5);
            this.dynStats("cor", 0.1);
            this.HPChange(Math.round(player.maxHP() * .25), false);
        }
        //Trap Oil
        ...
        trapOil(player) {
            ...
            if (player.cor < 30)
                this.outputText("hesitantly ");
            else if (player.cor > 70)
                this.outputText("eagerly ");
            ...
            if (player.sens < 70 && player.hasCock() && Mutations.rand(3) == 0 && changes < changeLimit) {
                this.outputText("\n\nA light breeze brushes over you and your skin tingles.  You have become more sensitive to physical sensation.");
                this.dynStats("sen", 5);
                changes++;
            }
            //Libido Increase:
            if (player.lib < 70 && player.hasVagina() && Mutations.rand(3) == 0 && changes < changeLimit) {
            ...
            if (player.hipRating >= 10 && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                player.hipRating--;
                if (player.hipRating > 15)
                    player.hipRating -= 2 + Mutations.rand(3);
                changes++;
            }
            ...
            if (player.biggestTitSize() > 1 && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                while (this.temp < player.bRows()) {
                    if (player.breastRows[this.temp].breastRating > 70)
                        ...
                        player.breastRows[this.temp].breastRating -= 2 + Mutations.rand(2);
            ...
            if (player.biggestTitSize() < 1 || player.breastRows[0].breastRating < 1 && Mutations.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.longestCockLength() >= 3.5 && player.hasCock() && Mutations.rand(2) == 0 && changes < changeLimit) {
                ...
                if (player.cockTotal() == 1)
                    this.outputText("it seems");
                else
                    this.outputText("they seem");
                ...
                while (this.temp < player.cockTotal()) {
                    if (player.cocks[this.temp].cockLength >= 3.5) {
                        ...
                        player.increaseCock(this.temp, Math.round(player.cocks[this.temp].cockLength * 0.33) * -1);
                    }
                    this.temp++;
                }
                changes++;
            ...
            //Testicle Reduction:
            if (player.balls > 0 && player.hasCock() && (player.ballSize > 1 || player.findStatusAffect(StatusAffects_1.StatusAffects.Uniball) < 0) && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                if (player.ballSize < 1 && player.findStatusAffect(StatusAffects_1.StatusAffects.Uniball) < 0) {
                    this.outputText("  You whimper as once again, your balls tighten and shrink.  Your eyes widen when you feel the gentle weight of your testicles pushing against the top of your [hips], and a few hesitant swings of your rear confirm what you can feel - you've tightened your balls up so much they no longer hang beneath your " + this.multiCockDescriptLight() + ", but press perkily upwards.  Heat ringing your ears, you explore your new sack with a careful hand.  You are deeply grateful you apparently haven't reversed puberty, but you discover that though you still have " + Mutations.num2Text(player.balls) + ", your balls now look and feel like one: one cute, tight little sissy parcel, its warm, insistent pressure upwards upon the joining of your thighs a never-ending reminder of it.");
                    ...
                    player.ballSize = 1;
                    player.createStatusAffect(StatusAffects_1.StatusAffects.Uniball, 0, 0, 0, 0);
                }
            ...
            if (player.hasVagina() && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                if (player.fertility >= 30)
                    this.outputText("It feels like your overcharged reproductive organs have simmered down a bit.");
                //Average fertility:
                else if (player.fertility >= 5)
                    this.outputText("You feel like you have dried up a bit inside; you are left feeling oddly tranquil.");
                //[Low/No fertility:
                else {
                    this.outputText("Although the numbness makes you feel serene, the trap oil has no effect upon your ");
                    if (player.fertility > 0)
                        this.outputText("mostly ");
                    this.outputText("sterile system.");
                    //[Low/No fertility + Trap/Corruption  >70:
                    if (player.cor > 70)
                        this.outputText("  For some reason the fact that you cannot function as nature intended makes you feel helpless and submissive.  Perhaps the only way to be a useful creature now is to find a dominant, fertile being willing to plow you full of eggs? You shake the alien, yet oddly alluring thought away.");
                }
            ...
            if (player.gender == 1) {
                ...
                if (Mutations.rand(4) == 0 && changes < changeLimit) {
                    if (player.femininity < 70 && player.femininity >= 60) {
                        ...
                        if (player.findPerk(PerkLib_1.PerkLib.Androgyny) < 0) {
                            player.createPerk(PerkLib_1.PerkLib.Androgyny, 0, 0, 0, 0);
            ...
            //Remove odd eyes
            if (player.eyeType == appearanceDefs_1.EYES_FOUR_SPIDER_EYES && Mutations.rand(2) == 0 && changes < changeLimit) {
                this.outputText("\n\nYou blink and stumble, a wave of vertigo threatening to pull your " + player.feet() + " from under you.  As you steady and open your eyes, you realize something seems different.  Your vision is changed somehow.", false);
                if (player.eyeType == appearanceDefs_1.EYES_FOUR_SPIDER_EYES)
                    this.outputText("  Your multiple, arachnid eyes are gone!</b>", false);
                this.outputText("  <b>You have normal, humanoid eyes again.</b>", false);
            ...
            //PC Trap Effects
            if (player.eyeType != appearanceDefs_1.EYES_BLACK_EYES_SAND_TRAP && Mutations.rand(4) == 0 && changes < changeLimit) {
                player.eyeType = appearanceDefs_1.EYES_BLACK_EYES_SAND_TRAP;
                //Eyes Turn Black:
            ...
            //Vagina Turns Black:
            if (player.hasVagina() && player.vaginaType() != 5 && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                //(Wet:
                if (player.wetness() >= 3)
                    this.outputText("  Your natural lubrication makes it gleam invitingly.");
                //(Corruption <50:
                if (player.cor < 50)
                    this.outputText("  After a few cautious touches you decide it doesn't feel any different- it does certainly look odd, though.");
                else
                    this.outputText("  After a few cautious touches you decide it doesn't feel any different - the sheer bizarreness of it is a big turn on though, and you feel it beginning to shine with anticipation at the thought of using it.");
            ...
            //Dragonfly Wings:
            if (player.wingType != appearanceDefs_1.WING_TYPE_GIANT_DRAGONFLY && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                changes++;
                player.wingType = appearanceDefs_1.WING_TYPE_GIANT_DRAGONFLY;
            }
            if (changes == 0) {
        ...
        ringtailFig(player) {
            ...
            if (player.hasCock()) {
                //gain ball size
                if (player.balls > 0 && player.ballSize < 15 && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                if (player.balls == 0 && player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && 9999 == 9999 && Mutations.rand(3) == 0 && changes < changeLimit) {
            ...
            if (Mutations.rand(4) == 0 && player.thickness < 80 && changes < changeLimit) {
                this.outputText(player.modThickness(80, 2), false);
                changes++;
            }
            //bodypart changes:
            if (player.tailType != appearanceDefs_1.TAIL_TYPE_RACCOON && Mutations.rand(4) == 0 && changes < changeLimit) {
                //grow da tail
                //from no tail:
                if (player.tailType == appearanceDefs_1.TAIL_TYPE_NONE) {
                ...
                player.tailType = appearanceDefs_1.TAIL_TYPE_RACCOON;
                changes++;
            }
            //gain fur
            if ((player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_RACCOON && player.earType == appearanceDefs_1.EARS_RACCOON) && player.skinType != appearanceDefs_1.SKIN_TYPE_FUR && changes < changeLimit && Mutations.rand(4) == 0) {
            ...
            //gain coon ears
            if (player.tailType == appearanceDefs_1.TAIL_TYPE_RACCOON && player.earType != appearanceDefs_1.EARS_RACCOON && Mutations.rand(4) == 0 && changes < changeLimit) {
                //from dog, kangaroo, bunny, other long ears
                if (player.earType == appearanceDefs_1.EARS_DOG || player.earType == appearanceDefs_1.EARS_BUNNY || player.earType == appearanceDefs_1.EARS_KANGAROO)
                ...
                else if (player.earType == appearanceDefs_1.EARS_HORSE || player.earType == appearanceDefs_1.EARS_COW || player.earType == appearanceDefs_1.EARS_CAT)
                ...
                player.earType = appearanceDefs_1.EARS_RACCOON;
                changes++;
            }
            //gain feet-coon
            if (player.earType == appearanceDefs_1.EARS_RACCOON && player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_RACCOON && changes < changeLimit && Mutations.rand(4) == 0) {
                ...
                //from hooves or hard feet, including centaurs and bees
                else if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HOOFED || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_CENTAUR || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_BEE || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_PONY || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_CHITINOUS_SPIDER_LEGS) {
                    ...
                    if (player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HOOFED || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_CENTAUR || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_PONY)
                        this.outputText("pops right out of its hoof just in time");
                    else
                        this.outputText("comes loose inside its long boot, and you pull it free with irritation only");
                ...
                //from human, demon, paw feet
                    ...
                    if ((player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HUMAN || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DEMONIC_HIGH_HEELS || player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DEMONIC_CLAWS) && player.skinType != appearanceDefs_1.SKIN_TYPE_FUR)
                        this.outputText("The sensation of walking around on what feels like a second pair of hands is so weird that you miss noticing the itchy fur growing in over your legs...  ");
                    this.outputText("<b>You now have raccoon paws!</b>");
                }
                player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_RACCOON;
                changes++;
            }
            //gain half-coon face (prevented if already full-coon)
            if (player.faceType != appearanceDefs_1.FACE_RACCOON_MASK && player.faceType != appearanceDefs_1.FACE_RACCOON && Mutations.rand(4) == 0 && changes < changeLimit) {
                //from human/naga/shark/bun face
                if (player.faceType == appearanceDefs_1.FACE_HUMAN || player.faceType == appearanceDefs_1.FACE_SHARK_TEETH || player.faceType == appearanceDefs_1.FACE_SNAKE_FANGS || player.faceType == appearanceDefs_1.FACE_BUNNY) {
                    this.outputText("\n\nA sudden wave of exhaustion passes over you, and your face goes partially numb around your eyes.  ");
                    //(nagasharkbunnies)
                    if (player.faceType == appearanceDefs_1.FACE_SHARK_TEETH || player.faceType == appearanceDefs_1.FACE_SNAKE_FANGS || player.faceType == appearanceDefs_1.FACE_BUNNY) {
                    ...
                    if (((player.skinTone == "ebony" || player.skinTone == "black") && (player.skinType == appearanceDefs_1.SKIN_TYPE_PLAIN || player.skinType == appearanceDefs_1.SKIN_TYPE_GOO)) || ((player.hairColor == "black" || player.hairColor == "midnight") && (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR || player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES))) {
                        ...
                        this.outputText("A dark, almost black mask shades the " + player.skinFurScales() + " around your eyes and over the topmost portion of your nose, lending you a criminal air!  <b>You now have a raccoon mask!</b>");
                }
                //from snout (will not overwrite full-coon snout but will overwrite others)
                    ...
                    if (((player.hairColor == "black" || player.hairColor == "midnight") && (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR || player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES)))
                        this.outputText("your face apparently returned to normal shape, albeit still covered in " + player.skinFurScales() + ".  You look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask on your otherwise normal human face.</b>");
                    else if ((player.skinTone == "ebony" || player.skinTone == "black") && (player.skinType == appearanceDefs_1.SKIN_TYPE_PLAIN || player.skinType == appearanceDefs_1.SKIN_TYPE_GOO))
                        this.outputText("your face apparently returned to normal shape.  You look closer and discover a darker, mask-line outline on your already inky visage.  <b>You now have a barely-visible raccoon mask on your normal human face.</b>");
                    else
                        this.outputText("your face returned to human dimensions, but shaded by a black mask around the eyes and over the nose!  <b>You now have a humanoid face with a raccoon mask!</b>");
                }
                player.faceType = appearanceDefs_1.FACE_RACCOON_MASK;
                changes++;
            }
            //gain full-coon face (requires half-coon and fur)
            //from humanoid - should be the only one possible
            else if (player.faceType == appearanceDefs_1.FACE_RACCOON_MASK && player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_RACCOON && player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                changes++;
                player.faceType = appearanceDefs_1.FACE_RACCOON;
            }
            //fatigue damage (only if face change was not triggered)
            else if (Mutations.rand(2) == 0 && changes < changeLimit && (player.faceType != appearanceDefs_1.FACE_RACCOON_MASK && player.faceType != appearanceDefs_1.FACE_RACCOON)) {
        ...
        mouseCocoa(player) {
            ...
            if (player.tallness < 60 && (player.analCapacity() < 100 || (player.vaginalCapacity() < 100 && player.hasVagina())) && changes < changeLimit && Mutations.rand(3) == 0) {
                this.outputText("\n\nYour ");
                ...
                if (player.isTaur())
                    this.outputText("Backing up to a likely rock, you rub your hindquarters against it, only to be surprised when you feel your hole part smoothly against the surface, wider than you're used to!");
                else
                    this.outputText("Slipping a hand in your [armor], you rub vigorously; your hole opens more easily and your fingers poke in farther than you're used to!");
                ...
                if (player.isTaur())
                    this.outputText("back away from your erstwhile sedimentary lover");
                else
                    this.outputText("pull your hand back out");
                this.outputText(".");
                //adds some lust
                ...
                if (player.goIntoHeat(false)) {
                    if (intensified) {
                        ...
                        //(mino cum in inventory and non-horse, 100 lust)
                            ...
                            player.knockUp(PregnancyStore_1.PregnancyStore.PREGNANCY_MINOTAUR, PregnancyStore_1.PregnancyStore.INCUBATION_MINOTAUR, 175);
            ...
            //bodypart changes:
            //gain ears
            if (player.earType != appearanceDefs_1.EARS_MOUSE && changes < changeLimit && Mutations.rand(4) == 0) {
                this.outputText("\n\nYour ears ");
                if (player.earType == appearanceDefs_1.EARS_HORSE || player.earType == appearanceDefs_1.EARS_COW || player.earType == appearanceDefs_1.EARS_DOG || player.earType == appearanceDefs_1.EARS_BUNNY || player.earType == appearanceDefs_1.EARS_KANGAROO)
                    this.outputText("shrink suddenly");
                else
                    this.outputText("pull away from your head");
                this.outputText(", like they're being pinched, and you can distinctly feel the auricles taking a rounded shape through the pain.  Reaching up to try and massage away their stings, <b>you're not terribly surprised when you find a pair of fuzzy mouse's ears poking through your " + this.hairDescript() + ".</b>");
                player.earType = appearanceDefs_1.EARS_MOUSE;
                changes++;
            }
            //gain tail
            //from no tail
            if (player.earType == appearanceDefs_1.EARS_MOUSE && player.tailType != appearanceDefs_1.TAIL_TYPE_MOUSE && changes < changeLimit && Mutations.rand(4) == 0) {
                //from other tail
                if (player.tailType > appearanceDefs_1.TAIL_TYPE_NONE) {
                    ...
                    if (player.tailType == appearanceDefs_1.TAIL_TYPE_HORSE)
                        this.outputText("elongating");
                    else
                        this.outputText("compressing");
                ...
                player.tailType = appearanceDefs_1.TAIL_TYPE_MOUSE;
                changes++;
            }
            //get teeth - from human, bunny, coonmask, or other humanoid teeth faces
            if (player.earType == appearanceDefs_1.EARS_MOUSE && (player.faceType == appearanceDefs_1.FACE_HUMAN || player.faceType == appearanceDefs_1.FACE_SHARK_TEETH || player.faceType == appearanceDefs_1.FACE_BUNNY || player.faceType == appearanceDefs_1.FACE_SPIDER_FANGS || player.faceType == appearanceDefs_1.FACE_RACCOON_MASK) && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                if (player.faceType != appearanceDefs_1.FACE_HUMAN)
                    this.outputText("the sharp teeth receding and ");
                ...
                player.faceType = appearanceDefs_1.FACE_BUCKTEETH;
                changes++;
            }
            //get mouse muzzle from mouse teeth or other muzzle
            if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && player.faceType != appearanceDefs_1.FACE_MOUSE && (player.faceType != appearanceDefs_1.FACE_HUMAN || player.faceType != appearanceDefs_1.FACE_SHARK_TEETH || player.faceType != appearanceDefs_1.FACE_BUNNY || player.faceType != appearanceDefs_1.FACE_SPIDER_FANGS || player.faceType != appearanceDefs_1.FACE_RACCOON_MASK) && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                player.faceType = appearanceDefs_1.FACE_MOUSE;
                changes++;
            }
            //get fur
            if ((player.skinType != appearanceDefs_1.SKIN_TYPE_FUR || (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && (player.hairColor != "brown" && player.hairColor != "white"))) && changes < changeLimit && Mutations.rand(4) == 0) {
                //from skinscales
                if (player.skinType != appearanceDefs_1.SKIN_TYPE_FUR) {
                    ...
                    if (player.tailType > appearanceDefs_1.TAIL_TYPE_NONE)
                        this.outputText(", except on your tail");
                    ...
                    if (player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES)
                        this.outputText(", pushing your scales out with little pinches");
        ...
        demonChanges(player) {
            ...
            if (player.tailType != appearanceDefs_1.TAIL_TYPE_DEMONIC && player.horns > 0) {
                if (player.tailType != appearanceDefs_1.TAIL_TYPE_NONE) {
                    this.outputText("\n\n", false);
                    if (player.tailType == appearanceDefs_1.TAIL_TYPE_SPIDER_ADBOMEN || player.tailType == appearanceDefs_1.TAIL_TYPE_BEE_ABDOMEN)
                        this.outputText("You feel a tingling in your insectile abdomen as it stretches, narrowing, the exoskeleton flaking off as it transforms into a flexible demon-tail, complete with a round spaded tip.  ", false);
                    else
                        this.outputText("You feel a tingling in your tail.  You are amazed to discover it has shifted into a flexible demon-tail, complete with a round spaded tip.  ", false);
                    this.outputText("<b>Your tail is now demonic in appearance.</b>", false);
                }
                else
                ...
                this.dynStats("cor", 4);
                player.tailType = appearanceDefs_1.TAIL_TYPE_DEMONIC;
            }
            //grow horns!
            if (player.horns == 0 || (Mutations.rand(player.horns + 3) == 0)) {
                if (player.horns < 12 && (player.hornType == appearanceDefs_1.HORNS_NONE || player.hornType == appearanceDefs_1.HORNS_DEMON)) {
                    this.outputText("\n\n", false);
                    if (player.horns == 0) {
                        this.outputText("A small pair of demon horns erupts from your forehead.  They actually look kind of cute.  <b>You have horns!</b>", false);
                    }
                    else
                        this.outputText("Another pair of demon horns, larger than the last, forms behind the first row.", false);
                    if (player.hornType == appearanceDefs_1.HORNS_NONE)
                        player.hornType = appearanceDefs_1.HORNS_DEMON;
                    player.horns++;
                    player.horns++;
                    this.dynStats("cor", 3);
                }
                //Text for shifting horns
                else if (player.hornType > appearanceDefs_1.HORNS_DEMON) {
                    this.outputText("\n\n", false);
                    this.outputText("Your horns shift, shrinking into two small demonic-looking horns.", false);
                    player.horns = 2;
                    player.hornType = appearanceDefs_1.HORNS_DEMON;
                    this.dynStats("cor", 3);
                }
            ...
            //Nipples Turn Back:
            if (player.findStatusAffect(StatusAffects_1.StatusAffects.BlackNipples) >= 0 && Mutations.rand(3) == 0) {
                this.outputText("\n\nSomething invisible brushes against your " + this.nippleDescript(0) + ", making you twitch.  Undoing your clothes, you take a look at your chest and find that your nipples have turned back to their natural flesh colour.");
                player.removeStatusAffect(StatusAffects_1.StatusAffects.BlackNipples);
            }
            //remove fur
            if ((player.faceType != appearanceDefs_1.FACE_HUMAN || player.skinType != appearanceDefs_1.SKIN_TYPE_PLAIN) && Mutations.rand(3) == 0) {
                //Remove face before fur!
                if (player.faceType != appearanceDefs_1.FACE_HUMAN) {
                    this.outputText("\n\n", false);
                    this.outputText("Your visage twists painfully, returning to a more normal human shape, albeit with flawless skin.  <b>Your face is human again!</b>", false);
                    player.faceType = appearanceDefs_1.FACE_HUMAN;
                }
                //De-fur
                else if (player.skinType != appearanceDefs_1.SKIN_TYPE_PLAIN) {
                    this.outputText("\n\n", false);
                    if (player.skinType == appearanceDefs_1.SKIN_TYPE_FUR)
                        this.outputText("Your skin suddenly feels itchy as your fur begins falling out in clumps, <b>revealing inhumanly smooth skin</b> underneath.", false);
                    if (player.skinType == appearanceDefs_1.SKIN_TYPE_SCALES)
                        this.outputText("Your scales begin to itch as they begin falling out in droves, <b>revealing your inhumanly smooth " + player.skinTone + " skin</b> underneath.", false);
                    player.skinType = appearanceDefs_1.SKIN_TYPE_PLAIN;
                    player.skinDesc = "skin";
                }
            }
            //Demon tongue
            if (player.tongueType == appearanceDefs_1.TONUGE_SNAKE && Mutations.rand(3) == 0) {
                ...
                player.tongueType = appearanceDefs_1.TONUGE_DEMONIC;
            }
            //foot changes - requires furless
            if (player.skinType == appearanceDefs_1.SKIN_TYPE_PLAIN && Mutations.rand(4) == 0) {
                //Males/genderless get clawed feet
                if (player.gender <= 1) {
                    if (player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_DEMONIC_CLAWS) {
                        this.outputText("\n\n", false);
                        ...
                        player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_DEMONIC_CLAWS;
                    }
                }
                //Females/futa get high heels
                else if (player.lowerBody != appearanceDefs_1.LOWER_BODY_TYPE_DEMONIC_HIGH_HEELS) {
                    this.outputText("\n\n", false);
                    ...
                    player.lowerBody = appearanceDefs_1.LOWER_BODY_TYPE_DEMONIC_HIGH_HEELS;
                }
            }
            //Grow demon wings
            if (player.wingType != appearanceDefs_1.WING_TYPE_BAT_LIKE_LARGE && Mutations.rand(8) == 0 && player.cor >= 50) {
                //grow smalls to large
                if (player.wingType == appearanceDefs_1.WING_TYPE_BAT_LIKE_TINY && player.cor >= 75) {
                    this.outputText("\n\n", false);
                    this.outputText("Your small demonic wings stretch and grow, tingling with the pleasure of being attached to such a tainted body.  You stretch over your shoulder to stroke them as they unfurl, turning into full-sized demon-wings.  <b>Your demonic wings have grown!</b>", false);
                    player.wingType = appearanceDefs_1.WING_TYPE_BAT_LIKE_LARGE;
                    player.wingDesc = "large, bat-like";
                }
                else if (player.wingType == appearanceDefs_1.WING_TYPE_SHARK_FIN) {
                    this.outputText("\n\n", false);
                    this.outputText("The muscles around your shoulders bunch up uncomfortably, changing to support the new bat-like wings growing from your back.  You twist your head as far as you can for a look and realize your fin has changed into ", false);
                    this.outputText("small ", false);
                    player.wingType = appearanceDefs_1.WING_TYPE_BAT_LIKE_TINY;
                    player.wingDesc = "tiny, bat-like";
                    this.outputText("bat-like demon-wings!", false);
                }
                else if (player.wingType == appearanceDefs_1.WING_TYPE_BEE_LIKE_SMALL || player.wingType == appearanceDefs_1.WING_TYPE_BEE_LIKE_LARGE) {
                    this.outputText("\n\n", false);
                    this.outputText("The muscles around your shoulders bunch up uncomfortably, changing to support your wings as you feel their weight increasing.  You twist your head as far as you can for a look and realize they've changed into ", false);
                    if (player.wingType == appearanceDefs_1.WING_TYPE_BEE_LIKE_SMALL) {
                        this.outputText("small ", false);
                        player.wingType = appearanceDefs_1.WING_TYPE_BAT_LIKE_TINY;
                        player.wingDesc = "tiny, bat-like";
                    }
                    ...
                    this.outputText("<b>bat-like demon-wings!</b>", false);
                }
                //No wings
                else if (player.wingType == appearanceDefs_1.WING_TYPE_NONE) {
                    this.outputText("\n\n", false);
                    this.outputText("A knot of pain forms in your shoulders as they tense up.  With a surprising force, a pair of small demonic wings sprout from your back, ripping a pair of holes in the back of your " + player.armorName + ".  <b>You now have tiny demonic wings</b>.", false);
                    player.wingType = appearanceDefs_1.WING_TYPE_BAT_LIKE_TINY;
                    player.wingDesc = "tiny, bat-like";
                }
            }
        }
        herbalContraceptive(player) {
            this.clearOutput();
            // Placeholder, sue me
            this.outputText("You chew on the frankly awfully bitter leaves as quickly as possible before swallowing them down.");
            player.createStatusAffect(StatusAffects_1.StatusAffects.Contraceptives, 1, 48, 0, 0);
        }
        princessPucker(player) {
            this.clearOutput();
            ...
            if (player.hairColor != "pink") {
                if (Mutations.rand(5) == 0) {
                    this.outputText("A slight tingle across your scalp draws your attention to your hair.  It seems your " + player.hairColor + " is rapidly gaining a distinctly pink hue, growing in from the roots!\n\n");
        ...
        ferretTF(player) {
            ...
            if (player.ferretScore() >= 6) {
                //Get warned!
                if (this.flags[kFLAGS_1.kFLAGS.FERRET_BAD_END_WARNING] == 0) {
                    ...
                    if (player.inte < 5)
                        player.inte = 5;
                    this.flags[kFLAGS_1.kFLAGS.FERRET_BAD_END_WARNING] = 1;
                }
            ...
            //Reset the warning if ferret score drops.
                ...
                this.flags[kFLAGS_1.kFLAGS.FERRET_BAD_END_WARNING] = 0;
            }
            var changes = 0;
            var changeLimit = 1;
            var temp = 0;
            ...
            if (((!player.hasCock() && player.hipRating > 6) || (player.hasCock() && player.hipRating > 4)) && Mutations.rand(3) == 0 && changes < changeLimit) {
            ...
            if (!this.flags[kFLAGS_1.kFLAGS.HYPER_HAPPY] && (player.biggestTitSize() > 2 || (player.hasCock() && player.biggestTitSize() >= 1)) && Mutations.rand(2) == 0 && changes < changeLimit) {
            ...
            if (player.hasCock()) {
                ...
                if (temp >= 0 && Mutations.rand(2) == 0 && changes < changeLimit) {
                    if (player.cocks[temp].cockLength > 6 && !this.flags[kFLAGS_1.kFLAGS.HYPER_HAPPY]) {
                        this.outputText("\n\nA pinching sensation racks the entire length of your " + this.cockDescript(temp) + ".  Within moments, the sensation is gone, but it appears to have become smaller.");
                        ...
                        if (player.cocks[temp].cockLength / 6 >= player.cocks[temp].cockThickness) {
                            this.outputText("  Luckily, it doen’t seem to have lost its previous thickness.");
                        }
                        else {
            ...
            if (player.averageNipplesPerBreast() > 1 && Mutations.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.hairType == appearanceDefs_1.HAIR_ANEMONE && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                //Turn hair growth on.
                this.flags[kFLAGS_1.kFLAGS.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD] = 0;
                player.hairType = 0;
                changes++;
            }
            //If the PC has goo hair:
            if (player.hairType == appearanceDefs_1.HAIR_GOO && Mutations.rand(3) == 0 && changes < changeLimit) {
            ...
            if (player.eyeType == appearanceDefs_1.EYES_FOUR_SPIDER_EYES && Mutations.rand(3) == 0 && changes < changeLimit) {
            ...
            if (player.faceType == appearanceDefs_1.FACE_FERRET_MASK && player.skinType == appearanceDefs_1.SKIN_TYPE_FUR && player.earType == appearanceDefs_1.EARS_FERRET && player.tailType == appearanceDefs_1.TAIL_TYPE_FERRET && player.lowerBody == appearanceDefs_1.LOWER_BODY_FERRET && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                player.faceType = appearanceDefs_1.FACE_FERRET;
                changes++;
            }
            //If face is human:
            if (player.faceType == 0 && Mutations.rand(3) == 0 && changes < changeLimit) {
                ...
                player.faceType = appearanceDefs_1.FACE_FERRET_MASK;
                changes++;
            }
            //If face is not ferret, has ferret ears, tail, and legs:
            if (player.faceType != appearanceDefs_1.FACE_HUMAN && player.faceType != appearanceDefs_1.FACE_FERRET_MASK && player.faceType != appearanceDefs_1.FACE_FERRET && Mutations.rand(3) == 0 && changes < changeLimit) {
            ...
            if (player.skinType != appearanceDefs_1.SKIN_TYPE_FUR && player.earType == appearanceDefs_1.EARS_FERRET && player.tailType == appearanceDefs_1.TAIL_TYPE_FERRET && player.lowerBody == appearanceDefs_1.LOWER_BODY_FERRET && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                this.outputText("  <b>You now have " + player.hairColor + " fur!</b>");
                player.skinType = appearanceDefs_1.SKIN_TYPE_FUR;
                changes++;
            }
            //Tail TFs!
            if (player.tailType != appearanceDefs_1.TAIL_TYPE_FERRET && player.earType == appearanceDefs_1.EARS_FERRET && Mutations.rand(3) == 0 && changes < changeLimit) {
                ...
                else if (player.tailType == appearanceDefs_1.TAIL_TYPE_HORSE && player.isTaur())
                ...
                else if (player.tailType == appearanceDefs_1.TAIL_TYPE_HARPY)
                ...
                else if (player.tailType == appearanceDefs_1.TAIL_TYPE_RABBIT)
                ...
                else if (player.tailType == appearanceDefs_1.TAIL_TYPE_DRACONIC || player.tailType == appearanceDefs_1.TAIL_TYPE_LIZARD)
                ...
                else if (player.tailType == appearanceDefs_1.TAIL_TYPE_COW)
                ...
                else if (player.tailType == appearanceDefs_1.TAIL_TYPE_CAT)
                ...
                else if (player.tailType == appearanceDefs_1.TAIL_TYPE_DOG)
                ...
                else if (player.tailType == appearanceDefs_1.TAIL_TYPE_KANGAROO)
                ...
                else if (player.tailType == appearanceDefs_1.TAIL_TYPE_RACCOON)
                ...
                else if (player.tailType == appearanceDefs_1.TAIL_TYPE_HORSE)
                ...
                else if (player.tailType == appearanceDefs_1.TAIL_TYPE_MOUSE)
                ...
                player.tailType = appearanceDefs_1.TAIL_TYPE_FERRET;
                changes++;
            }
            ...
            //(NOTE: this is the only exception to the legs coming after the tail, as the ferret tail will only go away right after it appears because of your snake lower half)
            else if (player.isNaga() && player.earType == appearanceDefs_1.EARS_FERRET && Mutations.rand(4) == 0 && changes < changeLimit) {
                ...
                changes++;
                player.lowerBody = appearanceDefs_1.LOWER_BODY_FERRET;
            }
            //If legs are not ferret, has ferret ears and tail
            if (player.lowerBody != appearanceDefs_1.LOWER_BODY_FERRET && player.earType == appearanceDefs_1.EARS_FERRET && player.tailType == appearanceDefs_1.TAIL_TYPE_FERRET && Mutations.rand(4) == 0 && changes < changeLimit) {
            ...
            if (player.earType != appearanceDefs_1.EARS_FERRET && Mutations.rand(4) == 0 && changes < changeLimit && Mutations.rand(2.5) == 0 && changes < changeLimit) {
                ...
                player.earType = appearanceDefs_1.EARS_FERRET;
                changes++;
            }
