define(["require", "exports", "./TelAdreAbstractContent", "../../../GlobalFlags/kFLAGS", "../../../StatusAffects", "../../../PerkLib", "../../../Items/WeaponLib"], function (require, exports, TelAdreAbstractContent_1, kFLAGS_1, StatusAffects_1, PerkLib_1, WeaponLib_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class Library extends TelAdreAbstractContent_1.TelAdreAbstractContent {
        //const TIMES_VISITED_MALI: number = 860
        //const TIMES_BEEN_TO_LIBRARY: number = 861;
        //const MALI_TAKEN_BLADE: number = 862;
        //[Mage's Tower]
        visitZeMagesTower() {
            if (this.flags[kFLAGS_1.kFLAGS.TIMES_BEEN_TO_LIBRARY] == 0)
                this.firstTowerVisit();
            else
                this.towerFollowUpVisits();
            this.menu();
            if (this.flags[kFLAGS_1.kFLAGS.TIMES_BEEN_TO_LIBRARY] == 0 || this.model.time.hours <= 17) {
                this.addButton(1, "You Okay?", this.youOkayBuddy);
                if (this.flags[kFLAGS_1.kFLAGS.UNKNOWN_FLAG_NUMBER_00175] > 0)
                    this.addButton(2, "Mali", this.talkToMali);
            }
            if (this.flags[kFLAGS_1.kFLAGS.TIMES_VISITED_MALI] > 0)
                this.addButton(2, "Mali", this.talkToMali);
            this.addButton(0, "Study", this.studyInTA);
            this.flags[kFLAGS_1.kFLAGS.TIMES_BEEN_TO_LIBRARY]++;
            this.addButton(4, "Back", this.telAdre.telAdreMenu);
        }
        //(first visit)
        firstTowerVisit() {
            this.clearOutput();
            this.outputText("You make your way to the largest fixture of the city, the impressive tower in the center.  The large spire could easily hold everyone you know ten times over and still have room to spare.  It is far too large for a city with Tel'adre's population – but then, you reflect, so is Tel'adre itself.");
            ...
            if (this.model.time.hours <= 17) { //Don't want to meet Quinn if he's not supposed to be there
                ...
                this.flags[kFLAGS_1.kFLAGS.TIMES_BEEN_TO_LIBRARY] = -2; //This will be incremented to -1 by the visitZeMagesTower function after we return
            }
        }
        towerFollowUpVisits() {
            this.clearOutput();
            if (this.flags[kFLAGS_1.kFLAGS.TIMES_BEEN_TO_LIBRARY] == -1) { //Return visits before you meet Quinn. Either you meet him or you continue to go to the library at night like some bibliophile vampire
                if (this.model.time.hours <= 17) {
                    this.outputText("You return to the mage's tower.  Entering the main room, you're surprised to see a man carefully turning the pages of one of the tomes");
                    this.commonQuinnTroduction();
                }
                else {
                    this.outputText("As before, there's no one here.  At least there's no lack of reading material.  Looking up and down the staircases reveals two locked doors, so it's unlikely you could do anything but study here.");
                    this.flags[kFLAGS_1.kFLAGS.TIMES_BEEN_TO_LIBRARY] = -2; //This will be incremented to -1 by the visitZeMagesTower function after we return
                }
                return;
            }
            ...
            if (this.model.time.hours <= 17) {
                this.outputText("You return to the mage's tower.  Entering the main room, Quinn is carefully inspecting the pages of a book.  The room looks slightly more organized from when you last saw it, but it looks as though Quinn will be working on it for some time.");
            ...
            //(follow-up visits, 18:00-20:00)
                ...
                this.outputText("You return to the mage's tower.  Entering the main room, Quinn is nowhere to be seen.  The room looks slightly cleaner from when you saw it last, but at the rate he's going it seems like it will be some time before it's finished.");
        ...
        commonQuinnTroduction() {
            this.outputText(", though he does not seem to be reading it.  Stacks of books sit next to him.  As you close the door, he glances up at you.");
        ...
        studyInTA() {
            ...
            //[Study, 18:00-20:00]
                ...
                if (this.flags[kFLAGS_1.kFLAGS.TIMES_BEEN_TO_LIBRARY] == -1) {
                    this.outputText("Looking around you decide to spend some time reading");
                }
                else {
                    this.outputText("Without Quinn to hassle you and request your absence from the presences, you have some time to read");
                }
                ...
                if (Library.rand(3) == 0) {
                    ...
                    if (this.player.inte >= 25 && this.player.findStatusAffect(StatusAffects_1.StatusAffects.KnowsArouse) < 0) {
                    ...
                    //Smart enough for arouse and doesnt have it
                    else if (this.player.inte >= 30 && this.player.findStatusAffect(StatusAffects_1.StatusAffects.KnowsHeal) < 0) {
                    ...
                    //Smart enough for arouse and doesnt have it
                    else if (this.player.inte >= 40 && this.player.findStatusAffect(StatusAffects_1.StatusAffects.KnowsMight) < 0) {
                    ...
                    //Smart enough for arouse and doesnt have it
                    else if (this.player.inte >= 25 && this.player.findStatusAffect(StatusAffects_1.StatusAffects.KnowsCharge) < 0) {
                    ...
                    //Smart enough for arouse and doesnt have it
                    else if (this.player.inte >= 30 && this.player.findStatusAffect(StatusAffects_1.StatusAffects.KnowsBlind) < 0) {
                    ...
                    //Smart enough for arouse and doesnt have it
                    else if (this.player.inte >= 40 && this.player.findStatusAffect(StatusAffects_1.StatusAffects.KnowsWhitefire) < 0) {
                ...
                else if ((this.player.lib > 75 || this.player.cor > 75 || this.player.findPerk(PerkLib_1.PerkLib.BimboBrains) >= 0 || this.player.findPerk(PerkLib_1.PerkLib.FutaFaculties) >= 0 || this.player.findPerk(PerkLib_1.PerkLib.BroBrains) >= 0) && Library.rand(2) == 0)
        ...
        youOkayBuddy() {
            this.clearOutput();
            this.outputText("A bit perturbed by Quinn's countenance and apparent exhaustion you can't help but inquire as to his well-being.");
        ...
        talkToMali() {
            ...
            //[[Mali], player has spellblade]
            else if ((this.player.weaponName == "inscribed spellblade" || this.player.hasItem(this.weapons.S_BLADE)) && this.flags[kFLAGS_1.kFLAGS.MALI_TAKEN_BLADE] == 0) {
                this.outputText("You tell Quinn you're here to see Mali.  He seems intrigued by the wrapped blade you're carrying, but doesn't ask any questions.  Unlocking the second floor as usual, he escorts you to Mali's quarters.");
                ...
                this.flags[kFLAGS_1.kFLAGS.MALI_TAKEN_BLADE] = 1;
                this.doNext(this.camp.returnToCampUseOneHour);
            }
            //[[Mali], player does not have spellblade]
            else {
                this.outputText("You tell Quinn you're here to see Mali.  He rolls his eyes but doesn't say anything, unlocking the second floor and leading you up once more.");
            ...
            this.flags[kFLAGS_1.kFLAGS.TIMES_VISITED_MALI]++;
        }
    }
