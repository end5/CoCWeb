define(["require", "exports", "../../../Monster", "../../../StatusAffects", "../../../GlobalFlags/kGAMECLASS", "../../../Items/WeaponLib", "../../../CockTypesEnum", "../../../../../includes/appearanceDefs"], function (require, exports, Monster_1, StatusAffects_1, kGAMECLASS_1, WeaponLib_1, CockTypesEnum_1, appearanceDefs_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class MinotaurLord extends Monster_1.Monster {
        performCombatAction() {
            if (this.HP < 300 && this.statusAffectv1(StatusAffects_1.StatusAffects.MinoMilk) < 4)
                this.minotaurDrankMalk();
            else if (MinotaurLord.rand(4) == 0 && this.player.weaponName != "fists")
                this.minotaurDisarm();
            else if (this.findStatusAffect(StatusAffects_1.StatusAffects.Timer) < 0)
                this.minotaurLordEntangle();
            else if (this.findStatusAffect(StatusAffects_1.StatusAffects.MinotaurEntangled) >= 0)
        ...
        minotaurDrankMalk() {
            ...
            //Success: any
            if (this.statusAffectv1(StatusAffects_1.StatusAffects.MinoMilk) < 3) {
                ...
                this.addHP(300);
                this.lust += 10;
                if (this.findStatusAffect(StatusAffects_1.StatusAffects.MinoMilk) < 0)
                    this.createStatusAffect(StatusAffects_1.StatusAffects.MinoMilk, 1, 0, 0, 0);
                else
                    this.addStatusValue(StatusAffects_1.StatusAffects.MinoMilk, 1, 1);
            }
            //Failure: any
            else {
        ...
        minotaurLordEntangle() {
            this.outputText("The minotaur lord lashes out with his chain, swinging in a wide arc!\n");
            this.createStatusAffect(StatusAffects_1.StatusAffects.Timer, 2 + MinotaurLord.rand(4), 0, 0, 0);
            //{dodge/whatever}
            if (this.combatMiss() || this.combatEvade() || this.combatFlexibility() || this.combatMisdirect()) {
                ...
                this.createStatusAffect(StatusAffects_1.StatusAffects.MinotaurEntangled, 0, 0, 0, 0);
            }
        ...
        minotaurCumPress() {
            ...
            kGAMECLASS_1.kGAMECLASS.dynStats("lus", 22 + this.player.lib / 8 + this.player.sens / 8);
            this.outputText("You tumble to your knees a few feet away, compulsively licking it up.  Once it's gone, ");
            if (this.player.lust > 99)
                this.outputText("you rise up, horny and hungry for more.");
            else {
                this.outputText("you realize what you've been doing.  Your embarrassment gives you the strength to re-adopt your fighting pose, but it's hard with how rigid");
                if (this.player.lust >= 80)
                    this.outputText(" and drippy");
                this.outputText(" your cock has become.  You want another taste...");
            }
            this.removeStatusAffect(StatusAffects_1.StatusAffects.MinotaurEntangled);
            this.combatRoundOver();
        }
        minotaurPrecumTease() {
            ...
            if (MinotaurLord.rand(2) == 0) {
                ...
                if (this.player.lust >= 70)
                    this.outputText("swallowing it into your mouth without thinking.  You greedily guzzle the potent, narcotic aphrodisiac down, even going so far as to lick it from each of your fingers in turn, sucking every drop into your waiting gullet.");
                else
                    this.outputText("feeling your heart hammer lustily.");
                kGAMECLASS_1.kGAMECLASS.dynStats("lus", 15 + this.player.lib / 8 + this.player.sens / 8);
            }
            else {
                this.outputText(" right past your head, but the smell alone is enough to make you weak at the knees.");
                this.outputText("  The animalistic scent of it seems to get inside you, the musky aroma burning a path of liquid heat to your groin, stiffening your horse-cock to absurd degrees.");
                kGAMECLASS_1.kGAMECLASS.dynStats("lus", 11 + this.player.lib / 10);
            }
            //(1)
            if (this.player.lust <= 75)
                this.outputText("  You shiver with need, wanting nothing more than to bury your face under that loincloth and slurp out every drop of goopey goodness.");
            else
                this.outputText("  <b>You groan and lick your lips over and over, craving the taste of him in your mouth.</b>");
            kGAMECLASS_1.kGAMECLASS.combatRoundOver();
        }
        defeated(hpVictory) {
        ...
        won(hpVictory, pcCameWorms) {
            if (hpVictory)
                this.game.urtaQuest.urtaLosesToMinotaurRoughVersion();
            else
                this.game.urtaQuest.urtaSubmitsToMinotaurBadEnd();
        }
        constructor() {
            super();
            this.a = "the ";
            ...
            // this.plural = false;
            this.createCock(MinotaurLord.rand(13 + 24), 2 + MinotaurLord.rand(3), CockTypesEnum_1.CockTypesEnum.HORSE);
            this.balls = 2;
    ...
    exports.MinotaurLord = MinotaurLord;
});
//# sourceMappingURL=MinotaurLord.js.map