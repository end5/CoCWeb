define(["require", "exports", "../BaseContent", "./Explore/ExploreDebug", "./Explore/Giacomo", "../GlobalFlags/kFLAGS", "../GlobalFlags/kGAMECLASS", "../StatusAffects", "../../../includes/appearanceDefs", "./Monsters/Imp", "./Monsters/Goblin", "../Appearance"], function (require, exports, BaseContent_1, ExploreDebug_1, Giacomo_1, kFLAGS_1, kGAMECLASS_1, StatusAffects_1, appearanceDefs_1, Imp_1, Goblin_1, Appearance_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    /**
     * Created by aimozg on 05.01.14.
     */
    class Exploration extends BaseContent_1.BaseContent {
        constructor() {
            super(...arguments);
            this.exploreDebug = new ExploreDebug_1.ExploreDebug();
            this.giacomo = new Giacomo_1.Giacomo();
        }
        ...
        doExplore() {
            if (this.player.explored == 0) {
                ...
                this.outputText("You can continue to search for new locations, or explore your previously discovered locations.", true);
            if (this.flags[kFLAGS_1.kFLAGS.EXPLORATION_PAGE] == 2) {
                this.explorePageII();
                return;
            }
            this.menu();
            ...
            if (this.player.exploredDesert > 0)
                this.addButton(1, "Desert", kGAMECLASS_1.kGAMECLASS.desert.exploreDesert);
            if (this.player.exploredForest > 0)
                this.addButton(2, "Forest", kGAMECLASS_1.kGAMECLASS.forest.exploreForest);
            if (this.player.exploredLake > 0)
                this.addButton(3, "Lake", kGAMECLASS_1.kGAMECLASS.lake.exploreLake);
            this.addButton(4, "Next", this.explorePageII);
            if (this.flags[kFLAGS_1.kFLAGS.TIMES_EXPLORED_PLAINS] > 0)
                this.addButton(5, "Plains", kGAMECLASS_1.kGAMECLASS.plains.explorePlains);
            if (this.flags[kFLAGS_1.kFLAGS.UNKNOWN_FLAG_NUMBER_00272] > 0)
                this.addButton(6, "Swamp", kGAMECLASS_1.kGAMECLASS.swamp.exploreSwamp);
            if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.ExploredDeepwoods) >= 0)
                this.addButton(7, "Deepwoods", kGAMECLASS_1.kGAMECLASS.forest.exploreDeepwoods);
            if (this.player.exploredMountain > 0)
                this.addButton(8, "Mountain", kGAMECLASS_1.kGAMECLASS.mountain.exploreMountain);
            this.addButton(9, "Back", this.playerMenu);
        }
        explorePageII() {
            this.flags[kFLAGS_1.kFLAGS.EXPLORATION_PAGE] = 2;
            this.menu();
            if (this.flags[kFLAGS_1.kFLAGS.DISCOVERED_HIGH_MOUNTAIN] > 0)
                this.addButton(0, "High Mountain", kGAMECLASS_1.kGAMECLASS.highMountains.exploreHighMountain);
            if (this.flags[kFLAGS_1.kFLAGS.BOG_EXPLORED] > 0)
                this.addButton(1, "Bog", kGAMECLASS_1.kGAMECLASS.bog.exploreBog);
            this.addButton(4, "Previous", this.goBackToPageI);
            if (this.debug)
                this.addButton(8, "Debug", this.exploreDebug.doExploreDebug);
            this.addButton(9, "Back", this.playerMenu);
        }
        goBackToPageI() {
            this.flags[kFLAGS_1.kFLAGS.EXPLORATION_PAGE] = 1;
            this.doExplore();
        }
        ...
        tryDiscover() {
            ...
            // return;
            if (this.flags[kFLAGS_1.kFLAGS.PC_PROMISED_HEL_MONOGAMY_FUCKS] == 1 && this.flags[kFLAGS_1.kFLAGS.HEL_RAPED_TODAY] == 0 && Exploration.rand(10) == 0 && this.player.gender > 0 && !kGAMECLASS_1.kGAMECLASS.helFollower.followerHel()) {
            ...
            if (this.player.explored > 1) {
                ...
                if (this.player.exploredLake >= 1 && Exploration.rand(3) == 0 && this.player.exploredDesert == 0) {
                    this.outputText("You stumble as the ground shifts a bit underneath you.  Groaning in frustration, you straighten up and discover the rough feeling of sand ", true);
                    if (this.player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_HUMAN)
                ...
                if (this.player.exploredMountain >= 1 && Exploration.rand(3) == 0 && this.flags[kFLAGS_1.kFLAGS.TIMES_EXPLORED_PLAINS] == 0) {
                    this.flags[kFLAGS_1.kFLAGS.TIMES_EXPLORED_PLAINS] = 1;
                    this.player.explored++;
                ...
                //EXPLOOOOOOORE
                if (this.flags[kFLAGS_1.kFLAGS.UNKNOWN_FLAG_NUMBER_00272] == 0 && this.flags[kFLAGS_1.kFLAGS.TIMES_EXPLORED_PLAINS] > 0 && Exploration.rand(3) == 0) {
                    this.flags[kFLAGS_1.kFLAGS.UNKNOWN_FLAG_NUMBER_00272] = 1;
                    this.player.explored++;
                ...
                var choosey = Exploration.rand(6);
                //2 (gargoyle) is never chosen once cathedral is discovered.
                if (choosey == 2 && this.flags[kFLAGS_1.kFLAGS.FOUND_CATHEDRAL] == 1) {
                ...
                if (choosey == 0) {
                    ...
                    this.player.explored++;
                    kGAMECLASS_1.kGAMECLASS.lumi.lumiEncounter();
                    return;
                }
                else if (choosey == 2) {
                    this.player.explored++;
                    if (this.flags[kFLAGS_1.kFLAGS.GAR_NAME] == '')
                        kGAMECLASS_1.kGAMECLASS.gargoyle.gargoylesTheShowNowOnWBNetwork();
                    else
                        kGAMECLASS_1.kGAMECLASS.gargoyle.returnToCathedral();
                    return;
                }
        ...
        bigJunkDesertScene() {
            ...
            //PARAGRAPH 1
            this.outputText("Walking along the sandy dunes of the desert you find yourself increasingly impeded by the bulk of your " + this.cockDescript(x) + " dragging along the sandscape behind you.  The incredibly hot surface of the desert causes your loins to sweat heavily and fills them with relentless heat.", false);
            if (this.player.cocks.length == 1)
                this.outputText("  As it drags along the dunes, the sensation forces you to imagine the rough textured tongue of a monstrous animal sliding along the head of your " + Appearance_1.Appearance.cockNoun(this.player.cocks[x].cockType) + ".", false);
            else if (this.player.cocks.length >= 2)
                this.outputText("  With all of your " + this.multiCockDescriptLight() + " dragging through the sands they begin feeling as if the rough textured tongues of " + Exploration.num2Text(this.player.cockTotal()) + " different monstrous animals were slobbering over each one.", false);
            this.outputText("\n\n", false);
            //PARAGRAPH 2
            //FOR NON-CENTAURS]
            if (!this.player.isTaur()) {
                this.outputText("The impending erection can't seem to be stopped.  Your sexual frustration forces stiffness into your " + this.multiCockDescriptLight() + ", which forces your torso to the ground.  Normally your erection would merely raise itself skyward but your genitals have grown too large and heavy for your " + this.hipDescript() + " to hold them aloft.  Instead you feel your body forcibly pivoting at the hips until your torso is compelled to rest face down on top of your obscene " + this.multiCockDescriptLight() + ".", false);
                //IF CHARACTER HAS GIANT BREASTS ADD SENTENCE
                if (this.player.biggestTitSize() >= 35)
                    this.outputText("  Your " + kGAMECLASS_1.kGAMECLASS.allBreastsDescript() + " hang lewdly off your torso to rest on the desert sands, seeming to bury the dunes on either side of you.  Their immense weight anchors your body, further preventing your torso from lifting itself up.  The burning heat of the desert teases your " + this.nippleDescript(0) + "s mercilessly as they grind in the sand.", false);
                //IF CHARACTER HAS A BALLS ADD SENTENCE
                if (this.player.balls > 0)
                    this.outputText("  Your " + this.player.skinTone + this.sackDescript() + " rests beneath your raised " + this.buttDescript() + ".  The fiery warmth of the desert caresses it, causing your " + this.ballsDescriptLight() + " to pulse with the need to release their sperm through your " + this.multiCockDescriptLight() + ".", false);
                ...
                if (this.player.vaginas.length >= 1) {
                    ...
                    if (this.player.vaginas[0].vaginalWetness >= appearanceDefs_1.VAGINA_WETNESS_DROOLING)
                        this.outputText("  Juices stream from your womanhood and begin pooling on the hot sand beneath you.  Wisps of steam rise up into the air only to tease your genitals further.  ", false);
                }
            }
            //FOR CENTAURS
            else {
                this.outputText("The impending erection can't seem to be stopped.  Your sexual frustration forces stiffness into your " + this.multiCockDescriptLight() + ", which forces the barrel of your horse-like torso to the ground.  Normally your erection would merely hover above the ground in between your centaurian legs, but your genitals have grown too large and heavy for your " + this.hipDescript() + " to hold them aloft.  Instead, you feel your body being forcibly pulled down at your hindquarters until you rest atop your " + this.multiCockDescriptLight() + ".", false);
                //IF CHARACTER HAS GIANT BREASTS ADD SENTENCE
                if (this.player.biggestTitSize() >= 35)
                    this.outputText("  Your " + kGAMECLASS_1.kGAMECLASS.allBreastsDescript() + " pull your human torso forward until it also is forced to rest facedown, just like your horse half.  Your tits rest, pinned on the desert sand to either side of you.  Their immense weight anchors you, further preventing any part of your equine body from lifting itself up.  The burning heat of the desert teases your " + this.nippleDescript(0) + "s incessantly.", false);
                //IF CHARACTER HAS A BALLS ADD SENTENCE
                if (this.player.balls > 0)
                    this.outputText("  Your " + this.player.skinTone + this.sackDescript() + " rests beneath your raised " + this.buttDescript() + ".  The airy warmth of the desert teases it, causing your " + this.ballsDescriptLight() + " pulse with the need to release their sperm through your " + this.multiCockDescriptLight() + ".", false);
                //IF CHARACTER HAS A VAGINA ADD SENTENCE
                if (this.player.vaginas.length >= 1) {
                    this.outputText("  Your " + this.vaginaDescript() + " and " + this.clitDescript() + " are thoroughly squashed between the bulky flesh where your male genitals protrude from between your hips and the " + this.buttDescript() + " above.", false);
                    //IF CHARACTER HAS A DROOLING PUSSY ADD SENTENCE
                    if (this.player.vaginas[0].vaginalWetness >= appearanceDefs_1.VAGINA_WETNESS_DROOLING)
                        this.outputText("  The desert sun beats down on your body, its fiery heat inflaming the senses of your vaginal lips.  Juices stream from your womanhood and begin pooling on the hot sand beneath you.", false);
                }
            }
            this.outputText("\n\n", false);
            //PARAGRAPH 3
            ...
            if (this.player.canFly())
                this.outputText("  You extend your wings and flap as hard as you can, until at last you manage to lighten the bulk of your body somewhat - enough to allow yourself to drag your genitals across the hot sands and back to camp.  The ordeal takes nearly an hour.", false);
