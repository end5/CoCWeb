define(["require", "exports", "../../../BaseContent", "../../../StatusAffects", "../../../GlobalFlags/kFLAGS", "../../../../../includes/appearanceDefs", "./Izumi", "../../../CockTypesEnum"], function (require, exports, BaseContent_1, StatusAffects_1, kFLAGS_1, appearanceDefs_1, Izumi_1, CockTypesEnum_1) {
    "use strict";
     ...
     * @programmer Gedan
     */
    class IzumiScene extends BaseContent_1.BaseContent {
        constructor() {
            /**
             * Support Functions
             */
            ...
            this.SMOKE_SPEED_REDUCE = 1.1;
            this.SMOKE_SENS_BOOST = 1.1;
            this.SMOKE_LIBIDO_BOOST = 1.1;
            this.SMOKE_DURATION = 24;
        }
        // Return a height-based nickname for the player
        heightDesc(doName = true) {
            if (IzumiScene.rand(4) != 0)
                return "kid";
            else {
        ...
        actsLikeACow() {
            ...
            if (this.player.inRut)
                return true;
            if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.Dysfunction))
                return true;
            return false;
        }
        /**
        ...
        encounter() {
            this.clearOutput();
            if (this.flags[kFLAGS_1.kFLAGS.IZUMI_MET] == 0) {
        ...
        enterAfterMet() {
            this.clearOutput();
            if (this.flags[kFLAGS_1.kFLAGS.IZUMI_LAST_ENCOUNTER] == 0)
                this.flags[kFLAGS_1.kFLAGS.IZUMI_LAST_ENCOUNTER] = 2;
            if (this.flags[kFLAGS_1.kFLAGS.IZUMI_LAST_ENCOUNTER] == 1) {
            ...
            else if (this.flags[kFLAGS_1.kFLAGS.IZUMI_LAST_ENCOUNTER] == 2) {
            ...
            else if (this.flags[kFLAGS_1.kFLAGS.IZUMI_LAST_ENCOUNTER] == 3) {
        ...
        enterFirstTime() {
            this.clearOutput();
            this.flags[kFLAGS_1.kFLAGS.IZUMI_MET] = 1;
            ...
            if (this.player.tallness >= 108) {
                ...
                if (this.player.str >= 75)
                    this.outputText(" forcing");
                else
                    this.outputText(" trying to force");
                ...
                if (this.player.hornType != appearanceDefs_1.HORNS_COW_MINOTAUR) {
            ...
            if (this.player.tou <= 75)
                this.outputText(" and of course, that obnoxiously endurance-sapping heat");
            this.outputText(".\n\n");
            if (this.player.tallness < 108) {
                ...
                if (this.player.minoScore() > 3)
                    this.outputText("plus it looks like you're thinking with the head on top of your neck, rather than the head between your legs, unlike the stupid brutes who usually wander around these parts.  ");
                else if (this.player.dogScore() > 3)
                    this.outputText("plus it looks like you'd actually be able to resist chasing down a stick, if I threw one outta' the cave.  ");
                else if (this.player.goblinScore() > 3)
                    this.outputText("plus you don't seem to be as insufferable as the rest of those little green whores who trollop around these parts, hunting their next lay.  ");
                else if (this.player.demonScore() > 3)
                    this.outputText("plus you're not forever going on about corruption this, enslavement that, demonic taint the other, unlike the insufferable clods who usually wander around these parts.  ");
                else if (this.player.harpyScore() > 3)
                    this.outputText("plus you're not screeching incessantly, like the feathery sluts who usually flap their way around these parts.  They're lucky they </i>can<i> fly, 'cause I'd give them something to really squawk about if I could get my hands on them.  ");
                else if (this.player.lizardScore() > 3)
                    this.outputText("plus, you don't have the same creepy eyes that the scaley idiots from around these parts keep trying to use on me.  ");
                else
                    this.outputText("plus you don’t look like the troglodytes who usually wander around these parts.  ");
            ...
            if (this.player.tou < 60)
                this.outputText(" causes you to cough and splutter.\n\n");
            else
                this.outputText(" momentarily steals your voice, having been ill-prepared for the sudden burn.\n\n");
        ...
        enterFirstTimePartII() {
            this.clearOutput();
            this.outputText("Still");
            if (this.player.tou < 60)
                this.outputText(" struggling with");
            else
                this.outputText(" distracted by");
            this.outputText(" the smoke, you turn to see her, only barely through the gloom, at the boulder blocking entry to the cave.");
            if (this.player.str <= 75)
                this.outputText("  To your amazement, the boulder begins to move, and as more light - along with a much-needed rush of fresh air - floods into the smoky cavern, you see she’s actually rolling it aside with her <i> bare hands </i>.  Even given her great size, this woman’s strength must be immense!\n\n");
            else
                this.outputText("  The boulder begins to move, and as more light - along with a welcome rush of fresh air - floods into the smoky cavern, you see she’s rolling it aside with nothing more than her bare hands with graceful, if unnerving, ease.\n\n");
            ...
            if (this.player.cor <= 33)
                this.outputText(" exposing an uncomfortable amount of creamy-white thigh flesh.");
            else if (this.player.cor > 33 && this.player.cor < 66)
                this.outputText(" exposing a tantalising view of creamy-white thigh flesh.");
            else
                this.outputText(" exposing a delicious view of her thigh, the creamy-white flesh almost screaming out for your caresses.");
            ...
            if (this.player.tallness <= 96)
                this.outputText(" - if a little... imposing -");
            else
                this.outputText(",");
            ...
            if (this.player.str <= 75 || this.player.tallness < 96)
                this.outputText(" You can’t help but feel like a sheep yourself - one who’s just noticed the wolf is staring it down.\n\n");
        ...
        smokeEffect(smokedPipe) {
            var deltaSpd = this.player.spe - (this.player.spe * this.SMOKE_SPEED_REDUCE);
            var deltaSns = (this.player.sens * this.SMOKE_SENS_BOOST) - this.player.sens;
            var deltaLib = (this.player.lib * this.SMOKE_LIBIDO_BOOST) - this.player.lib;
            var lustMod = 7;
            ...
            this.player.createStatusAffect(StatusAffects_1.StatusAffects.IzumisPipeSmoke, this.SMOKE_DURATION, deltaSpd, deltaSns, deltaLib);
            ...
            if (this.player.spe <= 0)
                this.player.spe = 1;
            if (this.player.sens >= 100)
                this.player.sens = 100;
            if (this.player.lib >= 100)
                this.player.lib = 100;
        ...
        updateSmokeDuration(hours) {
            var affectIndex = this.player.findStatusAffect(StatusAffects_1.StatusAffects.IzumisPipeSmoke);
            if (affectIndex >= 0) {
        ...
        smokeEffectWearsOff() {
            ...
            if (affectIndex >= 0) {
                ...
                this.player.removeStatusAffect(StatusAffects_1.StatusAffects.IzumisPipeSmoke);
            }
        }
        ...
        smokeThePipe() {
            ...
            if (this.player.tou <= 60)
                this.outputText(" you cough and splutter");
            else if (this.player.tou < 75)
                this.outputText(" you flinch, holding back a cough");
            else
                this.outputText(" your eyes water");
            ...
            if (this.player.tou <= 80)
                this.outputText("  After a while, you find it hard to even raise your arms to reach for the pipe again.\n\n");
            else
                this.outputText("\n\n");
        ...
        smokeJoins() {
            ...
            if (this.player.str <= 75 || this.player.tou <= 75 || this.player.tallness <= 96)
                this.outputText("  You swallow, hard, suddenly feeling oddly nervous.");
        ...
        fightTheFuckhugeOni() {
            ...
            if (this.flags[kFLAGS_1.kFLAGS.IZUMI_LAST_ENCOUNTER] == 0) {
                ...
                if (this.player.tone <= 75)
                    this.outputText("The idea that she thinks she can beat you with one hand behind her back is insulting to say the least, but on the other hand, her overconfidence could be her downfall.\n\n");
            ...
            // PC lost last fight
            else if (this.flags[kFLAGS_1.kFLAGS.IZUMI_LAST_ENCOUNTER] == 1) {
            ...
            this.startCombat(new Izumi_1.Izumi());
        }
        /**
         * SURRENDER SCENES
         */
        fuckhugeOniGetsWhatSheWants() {
            this.hideUpDown();
            this.clearOutput();
            // Male/Herms
            if (this.player.hasCock()) {
                ...
                if (this.player.isTaur())
                    this.outputText(" flank and forcing you sideways onto the ground.");
                else
                    this.outputText(" chest and forcing you backwards onto the ground.");
                ...
                else if (this.player.biggestCockLength() <= 14)
            ...
            this.flags[kFLAGS_1.kFLAGS.IZUMI_TIMES_SUBMITTED]++;
            this.flags[kFLAGS_1.kFLAGS.IZUMI_LAST_ENCOUNTER] = 1;
            this.flags[kFLAGS_1.kFLAGS.IZUMI_SEEN_PC_GENDER] = this.player.gender;
        }
        ...
        surrenderSmallCock() {
            ...
            if (this.player.isTaur())
                this.outputText(" perches her muscular ass on your flank, your body making for a suitable seat.");
            else
                this.outputText(" drops to the floor, laying next to you on her side.");
            ...
            if (this.player.biggestTitSize() >= 1 && this.player.biggestTitSize() < 3)
                this.outputText(", your [chest] dwarfed by the larger womans own pair.");
            else if (this.player.biggestTitSize() >= 3 && this.player.biggestTitSize() < 5)
                this.outputText(", your [chest] moulding themselves against the underside of the Oni’s larger pair.");
            else if (this.player.biggestTitSize() >= 5 && this.player.biggestTitSize() <= 8)
                this.outputText(", your [chest] compressing against the Oni’s larger pair, forming a delicious valley of flesh between your bodies.");
            else
                this.outputText(", your [chest] dwarfing the larger womans own pair.");
            ...
            if (this.player.str <= 75)
                this.outputText(" and you would be powerless to resist...");
            else
                this.outputText(" and you would struggle to deny her will...");
            ...
            if (this.player.hasVagina())
                this.outputText(", sticky moisture dribbling across your thigh, your [vagina] clenching in lust - fueled harmony with your cock");
            ...
            if (this.player.gender == 3)
                this.outputText(" just a regular");
            else
                this.outputText(" a");
            this.outputText(" girl?</i>”  Suddenly, her hand bottoms out around your [cock biggest]");
            if (this.player.balls > 0)
                this.outputText(" and her free fingers reach out to gently stroke against your [balls].");
            else if (this.player.hasVagina())
                this.outputText(" and her free fingers reach out to gently stroke against your [vag].");
            else
                this.outputText(".");
            ...
            if (this.flags[kFLAGS_1.kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] >= 1) {
                ...
                if (!this.player.isTaur())
                    this.outputText("As you squirm in her arms, she releases your head from her grip, instead pushing you up into a seated position.");
                ...
                if (!this.player.isTaur())
                    this.outputText(" as one arm reaches around to continue the handjob from the other side");
                this.outputText(".  She stares intently at your cock, an expectant smirk plastered over her face, her free hand working");
                if (this.player.isTaur())
                    this.outputText(" over your flank");
                else
                    this.outputText(" you over with increased vigour");
                ...
                if (this.player.cumQ() <= 100)
                    this.outputText(" dribbles a hot, sticky mess onto the cold cavern floor.");
                else if (this.player.cumQ() > 100 && this.player.cumQ() <= 250)
                    this.outputText(" spurts and splutters a hot, sticky mess onto the cold cavern floor.");
                    ...
                    if (this.player.cumQ() > 500) {
                        ...
                        if (this.player.cumQ() > 1000)
                            this.outputText(", slowly expanding into a small lake....\n\n");
                    }
                    else {
                        this.outputText(".\n\n");
                    }
        ...
        surrenderMediumCock() {
            ...
            if (this.player.isTaur())
                this.outputText(" bigger");
            else
                this.outputText(" smaller");
            ...
            if (this.player.hasVagina()) {
                ...
                if (this.player.wetness() == 0)
                    this.outputText(" moistening");
                else if (this.player.wetness() == 1)
                    this.outputText(" dribbling runnels of sticky fem-juice");
                else if (this.player.wetness() == 2)
                    this.outputText(" constantly oozing feminine arousal");
                else
                    this.outputText(" a soppy mess, gushing a veritable lake of fem-slime");
            ...
            if (this.player.isTaur())
                this.outputText(" perching her muscular ass on your flank, your lower body offering a comfortable seat for Izumi.");
            else
                this.outputText(" laying on her side next to you.");
        ...
        surrenderMediumCockObey() {
            ...
            if (this.player.hasVagina())
                this.outputText(" mixed puddle of your own fem-slime and");
            else
                this.outputText(" puddle of your own");
        ...
        surrenderMediumCockRefuse() {
            ...
            if (this.player.hasVagina())
                this.outputText(", [vagina] spasming in sympathy");
            ...
            if (this.player.cumQ() <= 250)
                this.outputText(" spurting free, spattering down randomly over you as Izumi watches, happily observing the effects of your ruined orgasm.");
            else if (this.player.cumQ() <= 500)
                this.outputText(" sputtering freely from your [cock], glob after glob of thick jizz showering down randomly over you as Izumi watches, mildly amused with the results of your ruined orgasm.");
            else if (this.player.cumQ() <= 1000)
                this.outputText(" a torrent of fertile spunk rushes forth from your [cock], glob after glob of thick jizz showering down randomly over you, covering you from head to toe.  Izumi simply watches, amused with the results of your ruined orgasm.");
            else
                this.outputText("a torrent of fertile spunk rushes forth from your [cock], rope after rope of the thick cum launching into the air and splattering down over you.  Your orgasm continues unabated as you slowly paint yourself in a creamy white layer, Izumi giggling quietly, evidently quite proud of herself.");
        ...
        surrenderLargeCock() {
            ...
            if (this.player.isTaur())
                this.outputText(" back on your haunches, front end raised");
            ...
            if (this.player.balls > 0 && !this.player.hasVagina())
                this.outputText(", occasionally sliding down to massage your [balls] before returning to your cock.");
            else if (this.player.balls == 0 && this.player.hasVagina())
                this.outputText(", occasionally sliding down to tease at the lips of your [vagina].");
            else if (this.player.balls > 0 && this.player.hasVagina())
                this.outputText(", occasionally sliding down, alternating between a gentle massage of your [balls] and teasing caresses of your [vagina].");
            this.outputText("\n\n");
            ...
            if (this.player.balls > 0)
                this.outputText("  You groan, your aching balls coming to the same conclusion as you have, twitching under a passing stroke from one of Izumi’s soles and tensing up in readiness.");
            ...
            if (this.player.cumQ() > 500)
                this.outputText("You remain sat there for a crippling eternity, unable to do anything but moan as you inexplicably continue to cum, your aching erection firing out shot after shot of hot, dripping goo as Izumi watches you from the safety of the shadows, grinning happily like the cat that got the cream.  ");
            this.outputText("Eventually, your orgasm subsides, and you collapse");
            if (this.player.isTaur())
                this.outputText(" sideways");
            else
                this.outputText(" backwards");
        ...
        surrenderOhGodTheFemaleSurrenderContentIsFuckingHugeSendHelp() {
            ...
            if (this.flags[kFLAGS_1.kFLAGS.IZUMI_TIMES_SUBMITTED] == 0) {
                ...
                if (this.player.cor <= 33)
                    this.outputText("  You can’t help but feel a crippling sense of embarrassment as the strange woman undresses you, and you look away, embarrassed, while Izumi’s hands explore your body.");
                else if (this.player.cor >= 66)
                    this.outputText("  As Izumi slowly reveals your naked body to the world, you begin to feel an oh-so-familiar heat building up inside of you.  You look down at Izumi with pride as she inspects your body, running a skillful hand over your chest with a thoughtful look on her face.");
                this.outputText("\n\n");
                // Big knockers!
            ...
            if (this.flags[kFLAGS_1.kFLAGS.PC_FETISH] >= 1) {
                ...
                if (this.player.hasCock())
                    this.outputText(", your [cock] slowly thickening under her scrutiny");
                this.outputText(".  You shift in place,");
                if (this.player.isNaga())
                    this.outputText(" wriggling atop your [legs]");
                else
                    this.outputText(" subtly parting your [legs] and even ");
                if (this.player.isTaur())
                    this.outputText(" rolling your butt upward");
                else
                    (" tilting your hips forwards");
                ...
                if (!this.player.isNaga())
                    this.outputText("You rub your thighs together in delight");
                else
                    this.outputText("You wriggle and writhe in delight");
                ...
                if (this.player.isNaga())
                    this.outputText(" deep within your [vagina].  You slither around in place, your [leg] shifting and weaving around, moving and undulating... b");
                else
                    this.outputText(" between your [legs] as well.  As you move your weight repeatedly from one [foot] to the other, you shiver in enjoyment.  B");
            ...
            if (!this.player.isTaur()) {
                ...
                if (!this.player.isNaga())
                    this.outputText("  Before you know it, she has you down on your knees, the cold stone of the floor digging into your [skinfurscales] a little as she circles behind you, smirking cruelly.");
                else
                    this.outputText("  She takes hold of your shoulders and forces you lower to the ground, forced to slither more and more of your tail out from under yourself, until Izumi towers over you.  She circles behind you, a cruel smirk the only hint of her intentions.");
                this.outputText("\n\n");
        ...
        surrenderFemaleNonExhibitionVariant() {
            this.clearOutput();
            ...
            if (this.player.isTaur())
                this.outputText(" sitting upright on your back and");
            ...
            if (this.player.biggestTitSize() <= 2) {
                ...
                if (!this.player.isTaur())
                    this.outputText(", guiding your hand down to your own nethers, which you’re shocked - in a foggy, distant kind of way - to discover are sopping wet");
                this.outputText(".\n\n");
            ...
            // C-Cup+
                ...
                if (this.player.isNaga())
                    this.outputText(" over your [hip].");
                else if (this.player.isDrider())
                    this.outputText(" between your spindly legs, slapping them lightly to force you to spread them wider.");
                else
                    this.outputText(" between your thighs, slapping them lightly to force you to open them wider.");
                ...
                if (this.player.looseness() <= appearanceDefs_1.VAGINA_LOOSENESS_NORMAL)
                    this.outputText("  She takes it slowly, pushing into you with agonizing care so as not to cause you any distress.  Inch by inch, centimeter by centimeter, she slides her finger into your pussy, finally bottoming out down to the knuckle.  You let out a little gasp of surprise and alarm as she gives her finger a sudden quick, tickling twitch, as though tickling a pet under its chin, then grins savagely at your reaction.\n\n");
                else if (this.player.looseness() >= appearanceDefs_1.VAGINA_LOOSENESS_GAPING)
                    this.outputText("  Izumi’s finger slips between your cavernous walls with ease, quickly bottoming out inside you.  She lets out an amused grunt of surprise at the ease with which she’s able to fit her finger inside you.\n\n");
                ...
                if (this.player.lactationQ() > 0)
                    this.outputText("You let out a moan of pleasure as a sudden jet of milk spurts from your heavy breasts, splattering across the rocky ground.  Izumi doesn’t let up though, instead working your nipple even harder, almost feverishly pinching and squeezing away, forcing more and more of the warm liquid to be coaxed free of your bust.\n\n");
                if ((this.player.findStatusAffect(StatusAffects_1.StatusAffects.BreastsMilked) > 0) && (this.player.lactationQ() > 750)) {
        ...
        surrenderFemaleExhibitionVariant() {
            this.clearOutput();
            ...
            if (this.player.isBiped())
                this.outputText(", you spread your legs a little wider,");
            this.outputText(" you");
        ...
        surrenderFemaleLookitYouTheCenterOfAttentionYouSlut() {
            ...
            if (this.player.isBiped())
                this.outputText(" and on your knees");
         ...
         * LOSS SCENES
         */
        fuckedUpByAFuckhugeOni(titLoss = false) {
            this.flags[kFLAGS_1.kFLAGS.IZUMI_LAST_ENCOUNTER] = 1;
            this.flags[kFLAGS_1.kFLAGS.IZUMI_SEEN_PC_GENDER] = this.player.gender;
            this.clearOutput();
            if (this.player.HP <= 0) {
            ...
            if (this.player.isNaga())
                ...
                this.outputText(" scuttle");
            else
                this.outputText(" scurry");
            ...
            if (this.player.lust >= 100)
                this.outputText("  Despite how much the fight with the enormous Oni has turned you on, you’re still hesitant to capitulate to her will entirely.");
            ...
            if (this.player.lust >= 75)
                this.outputText(" the idea of sex with Izumi <i>is</i> quite a tantalizing prospect.  F");
            else
                this.outputText(" f");
            ...
            if (this.player.lust >= 75)
                this.outputText("  Despite yourself, you have to wonder exactly what it would feel like to slide between those giant sweat-slicked thighs, bury your face into Izumi’s mountainous bust, and just let your libido run wild.");
            this.outputText("\n\n");
            this.menu();
            if (this.player.hasCock()) {
        ...
        lossSceneStay() {
            ...
            if (this.player.hasCock())
                this.outputText("“<i>Or maybe,</i>”  She whispers seductively into your ear, “<i>You wanna bury that little Oni of yours in </i>me<i>.  Hmmmm?</i>”  You shudder as you feel her hand snake into your underwear and run sinuously across your crotch, readying you for the ordeal ahead...\n\n");
        }
        // Special loss scene if the player loses whilst being titsmothered by izumi
        deathBySnuSnuIMeanGiantOniTits() {
            this.flags[kFLAGS_1.kFLAGS.IZUMI_LAST_ENCOUNTER] = 1;
            ...
            if (this.player.totalCocks() > 1)
                this.outputText("s relent");
            else
                this.outputText(" relents");
        ...
        cowedIntoPuttingItInIntoHerGiantOniButt() {
            this.lossSceneStay();
        ...
        fuckhugeOniWantsYourBabiesOrSomeShit() {
            this.lossSceneStay();
        ...
        noDickNoVagNoService() {
            ...
            if (!this.player.isNaga())
                this.outputText("before grabbing onto your other ankle, forcibly spreading your legs apart and leaving you creating a very undignified Y-shape");
            this.outputText(".\n\n");
        ...
        littleChampLittleChampFuckhugeOniIsCominTaEatcha() {
            this.lossSceneStay();
            if (this.flags[kFLAGS_1.kFLAGS.IZUMI_SEEN_PC_GENDER] != 2 && !this.player.hasCock()) {
            ...
            if (!this.player.isNaga())
                this.outputText(" grabbing onto your other ankle, forcibly spreading your legs apart and leaving you creating a very undignified Y-shape");
            this.outputText(".\n\n");
        ...
        touchThatFluffyHorn() {
            this.flags[kFLAGS_1.kFLAGS.IZUMI_LAST_ENCOUNTER] = 3;
            this.flags[kFLAGS_1.kFLAGS.IZUMI_TIMES_GRABBED_THE_HORN]++;
            this.clearOutput();
            this.outputText("Izumi slips backwards onto one knee and holds up a hand for pause,");
            if ((this.player.HP / this.player.maxHP()) <= 0.1)
                this.outputText(" the brief reprieve giving both of you a moment to recover.\n\n");
            else if (this.monster.lust >= 100)
                this.outputText(" the Oni’s cheeks tinged a rosy peach in contrast to her alabaster skin.\n\n");
            else
                this.outputText(" momentarily staggered by your furious onslaught.\n\n");
            ...
            if (this.flags[kFLAGS_1.kFLAGS.SILLY_MODE_ENABLE_FLAG] == true) {
            ...
            // Super awesome Gedan edit mode!
                ...
                if (this.player.spe >= 90) {
                    ...
                    if (this.player.biggestTitSize() > 2)
                        this.outputText("maybe giving her face a little personal time nestled between your [chest] will finally distract the Oni enough.  You reach out for the bright red curve of Izumi’s horn, ");
                    else
                        this.outputText("thinking quickly, you slip open your [armor] with one hand, bearing your [cock] to the world, your other hand reaching for the bright red curve of Izumi’s horn, ");
                ...
                if (this.player.spe < 90 || this.player.str < 90) {
                    ...
                    this.outputText("“<i>C-Could you please let go of that?</i>”");
            ...
            if (this.player.hasCock()) {
                ...
                if (this.player.tentacleCocks() >= 8)
                    this.addButton(5, "Tentacruel", this.tentacruelRevengeFux);
            }
        }
        // Pfft, nofux
        letGoAndLeaveYouSillyFuck() {
            this.clearOutput();
        ...
        letGoAndFuck() {
            ...
            if (this.player.tallness < 80)
                this.outputText(" from above");
            else if (this.player.tallness > 108)
                this.outputText(" from below");
            this.outputText(".\n\n");
            ...
            if (this.player.biggestTitSize() >= 1 && this.player.biggestTitSize() < 3)
                this.outputText(" smothering your [chest] with her own, much larger, pair.\n\n");
            else if (this.player.biggestTitSize() >= 3 && this.player.biggestTitSize() < 5)
                this.outputText(" your [chest] moulding themselves against the Oni’s larger pair.\n\n");
            else if (this.player.biggestTitSize() >= 5 && this.player.biggestTitSize() <= 8)
                this.outputText(" your [chest] compressing against the Oni’s larger pair, forming a delicious valley of flesh.\n\n");
            else if (this.player.biggestTitSize() > 8)
                this.outputText(" your [chest] dominating the Oni’s own in the battle for space between the two of you.\n\n");
            else
                this.outputText(" pressing you into her chest.\n\n");
            ...
            if (this.player.biggestTitSize() > 0)
                this.outputText(" against your own");
        ...
        gonnaGetMeSomeRevengeButtsexin() {
            this.clearOutput();
            ...
            if (this.player.biggestCockLength() <= 4) {
                ...
                if (this.player.cor >= 50)
                    this.outputText("  You hate to admit it, but in a messed up way, her apathy is beginning to turn you on a bit by itself.  You knew you weren’t the most well-endowed adventurer in the first place, and to someone physically huge like Izumi the problem must be magnified.  For some reason, you find that idea hot as hell....\n\n");
        ...
        tentacruelRevengeFux() {
            ...
            if (this.player.tentacleCocks() >= 10)
                this.outputText("  You decide to test the theory, snaking another prehensile appendage into each of her open, outstretched palms.  Sure enough, after a few fumbling attempts, Izumi manages to grasp the concept of a handjob and awkwardly starts to jerk them off.");
        ...
        // Facefuck
        sayThatsAPrettyFaceBeAShameIfSomebodyJizzedAllOverIt() {
            this.clearOutput();
            ...
            var cockIndex = this.player.biggestCockIndex();
            if (this.player.cocks[cockIndex].cockType == CockTypesEnum_1.CockTypesEnum.HORSE)
                this.outputText("  Visibly unsure of where to put herself, Izumi finally settles for brushing her lips against the exact center of the flared mass that makes up the head of your stallion-prick.");
            else if (this.player.cocks[cockIndex].cockType == CockTypesEnum_1.CockTypesEnum.DOG)
                this.outputText("  Her eyes darting nervously from side to side, Izumi leans forwards and takes the very tip of your dogcock into her mouth, pressing her lips against the main bulk of what passes for your head.");
            else
                this.outputText("  You feel the soft, warm sensation of Izumi’s lips brushing against the sensitive head of your cock for a moment, then the process repeats itself as Izumi actually darts in again, unbidden, for a second smooch.");
            this.outputText("\n\n");
            ...
            if (this.player.cumQ() > 500)
                this.outputText("  Her surprise only deepens when you simply keep cumming, your prolonged orgasm flooding her mouth faster than she can swallow, forcing her cheeks to bulge out and tiny rivulets of cum to occasionally slip free of the tight seal her lips have formed around your cock.\n\n");
            if (this.player.cumQ() > 100) {
        ...
        tribThatRockyHornGirl() {
            ...
            if (this.player.cor > 80)
                this.outputText("“<i>Whatever.  Just try not to cum too hard while I’m making you my bitch, you little slut.</i>” You shoot back, returning");
            else
                this.outputText("Ignoring her protests entirely, you return");
    ...
    exports.IzumiScene = IzumiScene;
});
//# sourceMappingURL=IzumiScene.js.map