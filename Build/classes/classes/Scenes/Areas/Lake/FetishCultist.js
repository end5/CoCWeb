define(["require", "exports", "../../../Monster", "../../../StatusAffects", "../../../../console", "../../../../../includes/appearanceDefs", "../../../Appearance", "../../../internals/WeightedDrop"], function (require, exports, Monster_1, StatusAffects_1, console_1, appearanceDefs_1, Appearance_1, WeightedDrop_1) {
    ...
    class FetishCultist extends Monster_1.Monster {
        constructor() {
            super();
            console_1.trace("FetishCultist Constructor!");
            this.a = "the ";
            ...
            // this.plural = false;
            this.createVagina(false, appearanceDefs_1.VAGINA_LOOSENESS_GAPING, appearanceDefs_1.VAGINA_WETNESS_WET);
            ...
            this.drop = new WeightedDrop_1.WeightedDrop().add(this.consumables.LABOVA_, 1)
        ...
        combatRoundUpdate() {
            ...
            //Talk abouts it mang!
            if (changed)
                this.outputText("The fetish cultist's clothing shifts and twists, taking on the appearance of a " + this.armorName + ".\n\n", false);
        ...
        cultistRaisePlayerLust() {
            ...
            if (FetishCultist.rand(2) == 0) {
                if (this.armorName == FetishCultist.PERVY_NUNS_CLOTHING)
                    this.outputText("She suddenly stops chanting and spreads her legs, opening her loose pussy wide with one hand while moaning like a whore.  She toys with her breasts and fondles one of her nipples with her other hand.\n\nDespite yourself,  you can't help but be aroused by the scene.", false);
                if (this.armorName == FetishCultist.NOBLES_CLOTHING)
                    this.outputText("She suddenly blushes and start giggling, saying: 'Really my lord!' in a suave, submitting voice while pulling down her skirt.  The movement lets you get an even better look down her cleavage, and her breasts appear even fuller than before.\n\nDespite yourself, you can't help but be aroused by the scene.", false);
                if (this.armorName == FetishCultist.SWIMSUIT)
                    this.outputText("She does a series of lewd stretches, showing off her tightly-clad, sexy body in every possible detail.  In particular, her excited, blood-gorged pussy lips, clearly outlined, seem to be begging for you to come and grope them... and that's just for a start.  Despite yourself, you can't help but be aroused by the scene.", false);
                if (this.armorName == FetishCultist.TEACHERS_OUTFIT)
                ...
                if (this.armorName == FetishCultist.NAUGHTY_NURSES_UNIFORM)
                    this.outputText("Still displaying her figure in her tight suit, she asks with a lewd smile, \"<i>Is there one of your needs I could satisfy, my dear?</i>\"  She grabs one of her firm, full breasts, \"<i>Are you thirsty, maybe?</i>\"  Despite yourself, you can't help but be aroused by the scene.", false);
                if (this.armorName == FetishCultist.FETISHY_OUTFIT)
                    this.outputText("She suddenly starts posing in sexy outfits.  Despite yourself, you can't help but be aroused by it.", false);
            }
            else {
                this.outputText("She suddenly starts mauling her shapely breasts, her fingers nearly disappearing briefly in the soft, full flesh, while fingering herself eagerly, emitting a variety of lewd noises.  You are entranced by the scene, the sexual excitement she's experiencing penetrating your body in warm waves coming from your groin.", false);
            }
            this.game.dynStats("lus", (this.player.lib / 10 + this.player.cor / 20) + 4);
            if (this.player.lust >= 100)
                this.doNext(this.game.endLustLoss);
            else
                this.doNext(this.game.combatMenu);
        }
        cultistLustTransfer() {
            if (this.lust <= 30 || FetishCultist.rand(2) == 0) {
                this.outputText("Her eyes glaze over, ", false);
                if (this.player.cor < 40)
                    this.outputText("and you're almost afraid to know ", false);
                else
                    this.outputText("and you wish you knew ", false);
                this.outputText("what she is thinking of since you can almost feel her own lust building.", false);
                this.lust += 10;
            }
            else {
                this.outputText("Her eyes glaze over and you feel your mind suddenly becoming filled with a blur of every sexual perversion you could possibly think of, and then some.", false);
                if (this.player.vaginas.length > 0) {
                    this.outputText("  You feel your " + this.vaginaDescript(0) + " soaking itself in a sudden burst", false);
                    if (this.player.cockTotal() > 0)
                        this.outputText(", while a sudden influx of pre-cum blurts out and streams down your " + this.player.multiCockDescriptLight() + ", painfully hardened by a vast amount of blood rushing to your groin", false);
                    this.outputText(".", false);
                }
                else if (this.player.cockTotal() > 0)
                    this.outputText("  A sudden influx of pre-cum blurts out and streams down your " + this.player.multiCockDescriptLight() + ", painfully hardened by a vast amount of blood rushing to your groin.", false);
                if (this.player.gender == 0)
                    this.outputText("  Your genderless body is suddenly filled by a perverted warmth.", false);
        ...
        defeated(hpVictory) {
            ...
            if (hpVictory) {
                this.outputText("Hurt too much to continue controlling her powers, the cultist collapses helplessly.", true);
            }
            else {
                this.outputText("Overwhelmed by her lusts, the cultist loses the ability to control herself and collapses.", true);
            }
    ...
    FetishCultist.NAUGHTY_NURSES_UNIFORM = "naughty nurse's uniform";
    FetishCultist.TEACHERS_OUTFIT = "teacher's outfit";
    FetishCultist.SWIMSUIT = "swimsuit";
    FetishCultist.NOBLES_CLOTHING = "noble's clothing";
    FetishCultist.PERVY_NUNS_CLOTHING = "pervy nun's clothing";
    FetishCultist.FETISHY_OUTFIT = "fetishy outfit";
    exports.FetishCultist = FetishCultist;
});
//# sourceMappingURL=FetishCultist.js.map