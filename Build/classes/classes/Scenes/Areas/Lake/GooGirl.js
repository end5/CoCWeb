define(["require", "exports", "../../../Monster", "../../../PerkLib", "../../../StatusAffects", "../../../../../includes/appearanceDefs", "../../../internals/ChainedDrop"], function (require, exports, Monster_1, PerkLib_1, StatusAffects_1, appearanceDefs_1, ChainedDrop_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class GooGirl extends Monster_1.Monster {
        /*Fight-
        ...
        gooGalAttack() {
            ...
            if (this.findPerk(PerkLib_1.PerkLib.Acid) >= 0)
                this.outputText("Her body quivering from your flames, the goo-girl ", false);
            else
                this.outputText("The slime holds its hands up and they morph into a replica of your " + this.player.weaponName + ".  Happily, she swings at you", false);
            //Determine if dodged!
            if (this.player.spe - this.spe > 0 && Math.floor(Math.random() * (((this.player.spe - this.spe) / 4) + 80)) > 80) {
                if (this.findPerk(PerkLib_1.PerkLib.Acid) >= 0)
                    ...
                    this.outputText(", missing as you dodge aside.", false);
                return;
            }
            //Determine if evaded
            if (this.short != "Kiha" && this.player.findPerk(PerkLib_1.PerkLib.Evade) >= 0 && GooGirl.rand(100) < 10) {
                if (this.findPerk(PerkLib_1.PerkLib.Acid) >= 0)
                    this.outputText("tries to slap you, but you evade her attack.", false);
                else
                    this.outputText(", but you evade the clumsy attack.", false);
                return;
            }
            //("Misdirection"
            if (this.player.findPerk(PerkLib_1.PerkLib.Misdirection) >= 0 && GooGirl.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
                if (this.findPerk(PerkLib_1.PerkLib.Acid) >= 0)
                    this.outputText("tries to slap you.  You misdirect her, avoiding the hit.", false);
                else
                    this.outputText(", missing as you misdirect her attentions.", false);
                return;
            }
            //Determine if cat'ed
            if (this.player.findPerk(PerkLib_1.PerkLib.Flexibility) >= 0 && GooGirl.rand(100) < 6) {
                if (this.findPerk(PerkLib_1.PerkLib.Acid) >= 0)
                    this.outputText("tries to slap you, but misses due to your cat-like evasion.", false);
                else
                    this.outputText(", missing due to your cat-like evasion.", false);
                return;
            }
            //Determine damage - str modified by enemy toughness!
            if (this.findPerk(PerkLib_1.PerkLib.Acid) >= 0)
                damage = Math.floor((this.str + 10 + this.weaponAttack) - GooGirl.rand(this.player.tou) - this.player.armorDef);
            else
                damage = Math.floor((this.str + this.weaponAttack) - GooGirl.rand(this.player.tou) - this.player.armorDef);
            if (damage > 0)
            ...
            if (damage <= 0) {
                ...
                if (this.findPerk(PerkLib_1.PerkLib.Acid) >= 0) {
                    if (GooGirl.rand(this.player.armorDef + this.player.tou) < this.player.armorDef)
                        this.outputText("tries to slap you, but the acid-bearing slap spatters weakly off your " + this.player.armorName + ".", false);
                    else
                        this.outputText("tries to slap you with an acid-loaded hand, but it splatters off you ineffectually.", false);
                }
                else {
                    //Due to toughness or amor...
                    if (GooGirl.rand(this.player.armorDef + this.player.tou) < this.player.armorDef)
                        this.outputText(", her attack slapping fruitlessly against your " + this.player.armorName + ".", false);
                    else
                        this.outputText(", her attack splattering ineffectually against you.", false);
                }
            }
            //everyone else
            else {
                if (this.findPerk(PerkLib_1.PerkLib.Acid) >= 0) {
                    this.outputText("delivers a painful slap across your cheek.  You gasp when the light stinging becomes a searing burn that seems to get worse as time goes on!", false);
                    if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.AcidSlap) < 0)
                        this.player.createStatusAffect(StatusAffects_1.StatusAffects.AcidSlap, 0, 0, 0, 0);
                }
                else
                    this.outputText(", painfully smacking her gooey limbs against your head.  You shake your " + this.player.hairDescript() + ", clearing your head of the dazing slap.", false);
        ...
        gooEngulph() {
            ...
            if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.GooBind) < 0)
                this.player.createStatusAffect(StatusAffects_1.StatusAffects.GooBind, 0, 0, 0, 0);
            this.combatRoundOver();
        }
        performCombatAction() {
            //1/3 chance of base attack + bonus if in acid mode
            if ((this.findPerk(PerkLib_1.PerkLib.Acid) >= 0 && GooGirl.rand(3) == 0) || GooGirl.rand(3) == 0)
                this.gooGalAttack();
            else if (GooGirl.rand(5) == 0)
                this.gooEngulph();
            else if (GooGirl.rand(3) == 0)
                this.gooPlay();
            else
                this.gooThrow();
        }
        ...
        constructor(noInit = false) {
            ...
            this.createStatusAffect(StatusAffects_1.StatusAffects.BonusACapacity, 9001, 0, 0, 0);
            ...
            var tone = GooGirl.randomChoice("blue", "purple", "crystal");
            this.skinTone = tone;
    ...
    exports.GooGirl = GooGirl;
});
//# sourceMappingURL=GooGirl.js.map