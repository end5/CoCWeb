define(["require", "exports", "../../BaseContent", "./Mountain/HellHoundScene", "./Mountain/InfestedHellhoundScene", "./Mountain/MinotaurScene", "./Mountain/Salon", "../../GlobalFlags/kFLAGS", "../../../console", "../../../../includes/appearanceDefs", "../../PregnancyStore", "../../StatusAffects", "../../PerkLib", "../Monsters/Imp", "../Monsters/Goblin", "../../GlobalFlags/kGAMECLASS"], function (require, exports, BaseContent_1, HellHoundScene_1, InfestedHellhoundScene_1, MinotaurScene_1, Salon_1, kFLAGS_1, console_1, appearanceDefs_1, PregnancyStore_1, StatusAffects_1, PerkLib_1, Imp_1, Goblin_1, kGAMECLASS_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    /**
     * Created by aimozg on 06.01.14.
     */
    class Mountain extends BaseContent_1.BaseContent {
        constructor() {
            super(...arguments);
            this.hellHoundScene = new HellHoundScene_1.HellHoundScene();
            this.infestedHellhoundScene = new InfestedHellhoundScene_1.InfestedHellhoundScene();
            this.minotaurScene = new MinotaurScene_1.MinotaurScene();
            this.salon = new Salon_1.Salon();
        }
        //Explore Mountain
        exploreMountain() {
            this.player.exploredMountain++;
            var chooser = Mountain.rand(4);
            //Helia monogamy fucks
            if (this.flags[kFLAGS_1.kFLAGS.PC_PROMISED_HEL_MONOGAMY_FUCKS] == 1 && this.flags[kFLAGS_1.kFLAGS.HEL_RAPED_TODAY] == 0 && Mountain.rand(10) == 0 && this.player.gender > 0 && !kGAMECLASS_1.kGAMECLASS.helScene.followerHel()) {
                kGAMECLASS_1.kGAMECLASS.helScene.helSexualAmbush();
                return;
            }
            //Discover 'high mountain' at level 5 or 40 explores of mountain
            if ((this.player.level >= 5 || this.player.exploredMountain >= 40) && this.flags[kFLAGS_1.kFLAGS.DISCOVERED_HIGH_MOUNTAIN] == 0) {
            ...
            if (this.isHolidays()) {
                //Gats xmas adventure!
                if (Mountain.rand(5) == 0 && this.player.gender > 0 && this.isHolidays() && this.flags[kFLAGS_1.kFLAGS.GATS_ANGEL_DISABLED] == 0 && this.flags[kFLAGS_1.kFLAGS.GATS_ANGEL_GOOD_ENDED] == 0 && (this.flags[kFLAGS_1.kFLAGS.GATS_ANGEL_QUEST_BEGAN] > 0 && this.player.hasKeyItem("North Star Key") < 0)) {
                    kGAMECLASS_1.kGAMECLASS.gatsSpectacularRouter();
                    return;
                }
                if (Mountain.rand(6) == 0 && this.flags[kFLAGS_1.kFLAGS.JACK_FROST_YEAR] < this.date.fullYear && this.silly()) {
                    kGAMECLASS_1.kGAMECLASS.meetJackFrostInTheMountains();
                    return;
                }
            }
            //8% chance of hellhoundsplosions if appropriate
            if (Mountain.rand(100) <= 77) {
                if (this.flags[kFLAGS_1.kFLAGS.UNKNOWN_FLAG_NUMBER_00141] < 3) {
                    console_1.trace("CHANCE AT HELLHOUND GAO");
                    ...
                    if (this.player.faceType == appearanceDefs_1.FACE_DOG && (this.player.dogCocks() >= 2 || (this.player.hasVagina() && this.player.pregnancyType == PregnancyStore_1.PregnancyStore.PREGNANCY_HELL_HOUND)) && this.player.cor >= 60 && this.player.tailType == appearanceDefs_1.TAIL_TYPE_DOG && (this.player.lowerBody == appearanceDefs_1.LOWER_BODY_TYPE_DOG || this.player.hairColor == "midnight black")) {
                        console_1.trace("PASS BODYCHECK");
                        ...
                        else if (this.flags[kFLAGS_1.kFLAGS.UNKNOWN_FLAG_NUMBER_00141] == 1 && this.player.hasKeyItem("Marae's Lethicite") >= 0 && this.player.keyItemv2("Marae's Lethicite") < 3) {
                            this.hellHoundScene.HellHoundMasterEncounter();
                            return;
                        }
                    }
            ...
            if (!kGAMECLASS_1.kGAMECLASS.ceraphFollowerScene.ceraphIsFollower() && this.player.level > 2 && (this.player.exploredMountain % 30 == 0) && this.flags[kFLAGS_1.kFLAGS.PC_FETISH] > 0) {
                kGAMECLASS_1.kGAMECLASS.ceraphScene.friendlyNeighborhoodSpiderManCeraph();
                return;
            }
            //15% chance of Ceraph
            if (!kGAMECLASS_1.kGAMECLASS.ceraphFollowerScene.ceraphIsFollower() && this.player.level > 2 && (this.player.exploredMountain % 15 == 0) && this.flags[kFLAGS_1.kFLAGS.PC_FETISH] != 1) {
                kGAMECLASS_1.kGAMECLASS.ceraphScene.encounterCeraph();
                return;
            }
            //10% chance of hairdresser encounter if not found yet
            if (Mountain.rand(10) == 0 && this.player.findStatusAffect(StatusAffects_1.StatusAffects.HairdresserMeeting) < 0)
                chooser = 4;
            if ((Mountain.rand(8) == 0 && this.player.findStatusAffect(StatusAffects_1.StatusAffects.MetMarae) >= 0)
                && this.player.findStatusAffect(StatusAffects_1.StatusAffects.FoundFactory) < 0) {
                kGAMECLASS_1.kGAMECLASS.enterFactory();
                return;
            }
            //Boosts mino and hellhound rates!
            if (this.player.findPerk(PerkLib_1.PerkLib.PiercedFurrite) >= 0 && Mountain.rand(3) == 0) {
                if (Mountain.rand(2) == 0)
                    chooser = 1;
                else
                    chooser = 3;
            }
            //10% chance to mino encounter rate if addicted
            if (this.flags[kFLAGS_1.kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] > 0 && Mountain.rand(10) == 0) {
                chooser = 1;
            }
            //10% MORE chance for minos if uber-addicted
            if (this.player.findPerk(PerkLib_1.PerkLib.MinotaurCumAddict) >= 0 && Mountain.rand(10) == 0) {
                chooser = 1;
            }
            //Every 15 explorations chance at mino bad-end!
            if (this.player.exploredMountain % 16 == 0 && this.player.findPerk(PerkLib_1.PerkLib.MinotaurCumAddict) >= 0) {
            ...
            if (chooser == 0) {
                ...
                if (Mountain.rand(10) < impGob) {
                    if (this.player.level >= 8 && Mountain.rand(2) == 0) {
                        ...
                        this.outputText("An imp leaps out from behind a rock and attacks!", true);
            ...
            if (chooser == 1) {
                this.spriteSelect(44);
                if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.TF2) < 0 && this.player.level <= 1 && this.player.str <= 40) {
                    if (this.silly()) {
                        ...
                        this.outputText("Crossing over the treacherous mountain paths, you walk past an ominous cave.  The bones and the smell of death convince you to hasten your pace.  However, as you walk by, you hear a deep bellow and a snort as a monstrous man with a bull's head steps out.  With hell in his eyes and a giant ax in his hand, he begins to approach you in clear rage.  As he comes out into the light, you see that he is completely naked and sports a monstrous erection as angry as the minotaur himself, freely leaking a steady stream of pre-cum as he stalks you.\n\n", true);
                        this.outputText("You stumble in your attempt to escape and realize that you are completely helpless.  The minotaur towers over you and heaves his ax for a <i>coup de grace</i>.  As he readies the blow, a monstrous explosion rocks the entire mountainside, causing the bull-man to stumble before he can finish you off. You look around, bewildered, trying to understand this strange new turn of events, and notice a group of maybe half a dozen people approaching from further up the path.  They appear to be a motley crew clad in blue and carrying monstrous weapons.  The tallest man holds a weapon made of multiple rotating tubes, and begins spinning the barrels.  A second later, while screaming in a language you do not understand, a rain of lead begins shredding the minotaur into a cloud of blood and flesh.\n\n", false);
                        this.outputText("An equally imposing black man with a patch over one eye begins firing canisters at the beast, which explode violently.  \"<i>Ya ragged-arsed beast man!</i>\" he taunts.  \"<i>Ye should pick on someone yer own size, BOY-O! HEHEHE!</i>\"\n\n", false);
                        this.outputText("Coming up the path next is a freak of a person clad in a contained shiny suit with a weapon that burns with flame.  He freely walks into the explosions and gunfire and begins igniting the beast.\n\n", false);
                        this.outputText("\"<i>MRPHHUMFHRUFH!!!!! HUMFHUFMMRUF!</i>\" the freak mumbles through his mask.\n\n", false);
                        this.outputText("\"<i>I like me steak well done, ye crazy bugger!</i>\" yells the black man.\n\n", false);
                        this.outputText("The beast collapses in a charred and bloody heap.   As you stand back up, you hear a strange noise behind you.  You turn around to find a well-dressed man wearing a ski mask and smoking a cigarette.  \"<i>Don't you know ze mountains are dangereuse,</i>\" the man says with a thick accent.  \"<i>You will get FUCKED up here if you are not careful.</i>\"\n\n", false);
                        this.outputText("You thank the man and his team, but they brush off your gratitude.  \"<i>Non, non!</i>\" the man with the accent says.  \"<i>As zey say, everyone gets ONE.</i>\" With that, he touches the watch on his wrist and disappears.  The rest of the group continues on their way.\n\n", false);
                        this.outputText("As they leave, the giant with the chain gun yells in a horribly accented manner, \"<i>YOU LEAVE SANDVICH ALONE! SANDVICH IS MINE!</i>\"\n\n", false);
                        this.outputText("With that, another hail of bullets break the scene as they walk away, leaving you safe from the minotaur, but utterly baffled as to what in hell just happened.", false);
                    }
                    else {
                        this.outputText("Crossing over the treacherous mountain paths, you walk past an ominous cave.  The bones and the smell of death convince you to hasten your pace.  However, as you walk by, you hear a deep bellow and a snort as a monstrous man with a bull's head steps out.  With hell in his eyes and a giant ax in his hand, he begins to approach you in clear rage.  As he comes out into the light, you see that he is completely naked and sports a monstrous erection as angry as the minotaur himself, freely leaking a steady stream of pre-cum as he stalks you.\n\n", true);
                    ...
                    this.player.createStatusAffect(StatusAffects_1.StatusAffects.TF2, 0, 0, 0, 0);
                    this.doNext(this.camp.returnToCampUseOneHour);
                    return;
                }
                //Mino gangbang
                if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.MinoPlusCowgirl) < 0 || Mountain.rand(10) == 0) {
                    if (this.flags[kFLAGS_1.kFLAGS.HAS_SEEN_MINO_AND_COWGIRL] == 1 && this.player.horns > 0 && this.player.hornType == appearanceDefs_1.HORNS_COW_MINOTAUR && this.player.earType == appearanceDefs_1.EARS_COW && this.player.tailType == appearanceDefs_1.TAIL_TYPE_COW && this.player.lactationQ() >= 200 && this.player.biggestTitSize() >= 3 && this.player.minotaurAddicted()) {
                        //PC must be a cowmorph (horns, legs, ears, tail, lactating, breasts at least C-cup)
                        //Must be addicted to minocum
                        this.outputText("As you pass a shadowy cleft in the mountainside, you hear the now-familiar call of a cowgirl echoing from within.  Knowing what's in store, you carefully inch closer and peek around the corner.");
                        ...
                        if (this.player.hasVagina())
                            ...
                            this.outputText("twinges");
                        this.outputText(", aching to be filled");
                        if (this.player.hasCock())
                            this.outputText(", while [eachCock] rises to attention, straining at your [armor]");
                        this.outputText(".");
                    ...
                    this.flags[kFLAGS_1.kFLAGS.HAS_SEEN_MINO_AND_COWGIRL] = 1;
                    if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.MinoPlusCowgirl) < 0)
                        this.player.createStatusAffect(StatusAffects_1.StatusAffects.MinoPlusCowgirl, 0, 0, 0, 0);
                    else
                        this.player.addStatusValue(StatusAffects_1.StatusAffects.MinoPlusCowgirl, 1, 1);
                    this.outputText("As you pass a shadowy cleft in the mountainside, you hear the sounds of a cow coming out from it. Wondering how a cow got up here, but mindful of this land's dangers, you cautiously sneak closer and peek around the corner.\n\n", true);
                    ...
                    this.outputText("A part of your mind registers bits of clothing tossed aside and the heady scent of impending sex in the air, but your attention is riveted on the actions of the pair. The cow-girl turns and places her hands on a low ledge, causing her to bend over, her ample ass facing the minotaur. The minotaur closes the distance between them in a single step.\n\n", false);
            ...
            if (chooser == 2) {
                ...
                if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.WormsOn) >= 0 && this.player.findStatusAffect(StatusAffects_1.StatusAffects.Infested) < 0) {
                    if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.WormsHalf) >= 0 && Mountain.rand(2) == 0) {
                        if (this.player.cor < 90) {
                            this.outputText("Your hike in the mountains, while fruitless, reveals pleasant vistas and provides you with good exercise and relaxation.", true);
                            this.dynStats("tou", .25, "spe", .5, "lus", this.player.lib / 10 - 15);
                        }
                        else {
                    ...
                    kGAMECLASS_1.kGAMECLASS.wormEncounter();
                }
                else {
                    //If worms are off or the PC is infested, no worms.
                    if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.WormsOff) >= 0 || this.player.findStatusAffect(StatusAffects_1.StatusAffects.Infested) >= 0 || (Mountain.rand(2) == 0 && this.player.findStatusAffect(StatusAffects_1.StatusAffects.WormsHalf) >= 0)) {
                        if (this.player.cor < 90) {
                            this.outputText("Your hike in the mountains, while fruitless, reveals pleasant vistas and provides you with good exercise and relaxation.", true);
                            this.dynStats("tou", .25, "spe", .5, "lus", this.player.lib / 10 - 15);
                        }
                        ...
                        kGAMECLASS_1.kGAMECLASS.wormToggle();
                    }
                }
            }
            //Hellhound
            if (chooser == 3) {
                ...
                if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.WormsOn) >= 0 && Mountain.rand(2) == 0) {
        ...
        joinBeingAMinoCumSlut() {
            ...
            if (this.player.tallness <= 96)
                this.outputText(" despite your immense height");
            this.outputText(" as they look you up and down.  The entire area goes silent, even the goblins and the imps that are no doubt watching seem to be holding their breath, wondering what will happen to you.");
            ...
            if (this.player.hasCock()) {
                this.outputText("ignoring your cock");
                if (this.player.cockTotal() > 1)
                    this.outputText("s");
                this.outputText(" entirely, ");
            }
            this.outputText("slipping a finger into your moistening ");
            if (this.player.hasVagina())
                this.outputText("pussy");
            else
                this.outputText("asshole");
            this.outputText(".  A low moan escapes your lips as a second finger slips in while the busty bovine woman's thumb ");
            if (this.player.hasVagina())
                this.outputText("swirls around your clitoris");
            else
                this.outputText("presses against your perineum");
            this.outputText(".");
            ...
            if (this.player.hasCock())
                this.outputText(", once again ignoring your manhood completely");
            this.outputText(", dives tongue first into your wet ");
            if (this.player.hasVagina())
                this.outputText("quim");
            else
                this.outputText("back door");
            this.outputText(".  The movement is so quick that you can't even suppress the sudden, perverted moan that leaves your lips... a moan that sounds shockingly like a cow's moo.  The surprise at your sudden bovine outburst quickly dissipates as the cow-girl's large tongue dips in and out of your ");
            if (this.player.hasVagina())
                this.outputText("sodden box");
            else
                this.outputText("moist butthole");
            this.outputText(".  Any remaining fears of joining this very public sex show are gone, and you wonder why you didn't join in sooner.");
            ...
            if (this.player.hasVagina())
                this.outputText("cow-pussy");
            else
                this.outputText("ass");
            this.outputText(" right there,</i>\" he groans, more of his bombastic cock slipping deep inside you.  The minotaur hooks an arm under each of your knees, lifting up your lower body, pressing even deeper.  Powerful sensations drift up from your ");
            if (this.player.hasVagina())
                this.outputText("g-spot");
            else
                this.outputText("prostate");
            this.outputText(" as the minotaur's wide flare strokes it through your ");
            if (this.player.hasVagina())
                this.outputText("vaginal");
            else
                this.outputText("anal");
            this.outputText(" walls.  Biting your lip with barely contained pleasure, you bring your hands to your breasts, playing with your milk-sodden nipples in between each orgasmic thrust of the bull's hips.");
            ...
            this.outputText("tongue inside her, eagerly licking out and guzzling down the remnants of the minotaur's present.");
            ...
            if (this.player.hasVagina())
                ...
                this.outputText("back door");
            this.outputText(" increases slowly, and you can feel your limbs tingling at the prospect of your mino-cum-induced orgasm.");
            ...
            if (this.player.hasVagina())
                ...
                this.outputText("[asshole]");
            this.outputText(" like a Christmas tree.  You're acutely aware of every single movement, every pulse, every little bit of contact between you and the huge cock living inside you.");
            ...
            this.outputText(".  The moment that long-awaited jism hits your walls, it's like another lightning bolt hits.  It travels up your spine and sets your entire brain aglow.  Ecstasy wrapped in bliss with a side of euphoric rapture consumes your thoughts.  Your vision goes white, pearly white like the seed filling your body, and your lips part as a primal \"<i>moo</i>\" slips out.");
            ...
            if (!this.player.hasVagina())
                this.outputText("new ");
            this.outputText("womb with his divine dick.");
            ...
            this.outputText("breasts around his monolithic shaft.  Your faces meet and her soft lips press against yours, each of you earnestly pressing your tongues into the other's mouths, swapping the juices you've collected over the past hour or so.  The bull beneath you groans, awakening to the feeling of four breasts surrounding his love muscle.");
            ...
            this.player.orgasm();
            this.dynStats("lib", .5, "sen", -3, "cor", 1);
            if (this.flags[kFLAGS_1.kFLAGS.PC_FETISH] > 0) {
                this.outputText("  A thrill runs through you.  Even though you were brought to such a satisfying climax, the whole thought that goblins and imps were watching you and getting off on it... it just makes you hornier than you were before.");
        ...
        watchAMinoCumSlut() {
            this.clearOutput();
            this.outputText("Deciding not to risk it, you settle back into your nook in the rocks and watch on eagerly.  The cow-girl turns and places her hands on a low ledge, causing her to bend over, her ample ass facing the minotaur.  The minotaur closes the distance between them in a single step.");
        ...
        continueMinoVoyeurism() {
            this.outputText("They go at it for nearly an hour, oblivious to you watching them, before their intensity heightens as they near orgasm. The results are almost explosive, both of them crying out as they begin twitching uncontrollably. Clinging desperately to the cow-girl's ass, the minotaur pumps so much cum into her depths that it begins spurting out. This accidental lubrication releases his grip and the pair collapse to the ground. Yet the minotaur isn't finished, his man-milk spraying into the air almost like his still-erect dick is a hose and splattering down onto both of them.\n\n", true);
            this.outputText("As you look at the two cum-covered creatures laying their in their exhausted sex-induced stupors, the minotaur's thick horse-cock now slowly deflating, you realize that you've been touching yourself.  You make yourself stop ", false);
            //[low corruption]
            if (this.player.cor < 33)
                this.outputText("in disgust.", false);
            else if (this.player.cor < 66)
                this.outputText("in confusion.", false);
            else
                this.outputText("reluctantly.", false);
            ...
            if (this.player.statusAffectv1(StatusAffects_1.StatusAffects.MinoPlusCowgirl) == 0)
