define(["require", "exports", "../../../BaseContent", "../../../PerkLib", "../../../../../includes/appearanceDefs", "../../../CockTypesEnum", "../../../StatusAffects"], function (require, exports, BaseContent_1, PerkLib_1, appearanceDefs_1, CockTypesEnum_1, StatusAffects_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class CorruptedGlade extends BaseContent_1.BaseContent {
        intro() {
        ...
        flowerFun() {
            ...
            if (this.player.hasCock()) { //Sex scenes for those with cawks
                ...
                this.outputText("You lay there for some time until your muscle control returns, your cock" + (this.player.cocks.length > 1 ? "s" : "") + " still slightly over-large " + (this.player.balls >= 2 ? "and your " + this.ballsDescriptLight() : "and") + " feeling sore from the exertion.  At least you hope it's just from the exertion and not from whatever the plant did.\n\n");
                ...
                if (this.player.balls == 0)
                    ...
                    booster += 2;
                if (this.player.findPerk(PerkLib_1.PerkLib.MessyOrgasms) >= 0 && this.player.cumMultiplier < 3)
                    booster += 1;
        ...
        tentacleFun() {
            this.clearOutput();
            this.spriteSelect(92);
            if (this.player.vaginas.length > 0 && CorruptedGlade.rand(2) == 0) { //Vaginal Variant 50% of the time
                ...
                var tentacle = CorruptedGlade.rand(3); //Big, medium or small size tentacle
                var tentacleSize = 0;
                if (tentacle == 0) { //Small
                    this.outputText("a little small for your ");
                    switch (this.player.vaginas[0].vaginalLooseness) {
                        case appearanceDefs_1.VAGINA_LOOSENESS_TIGHT:
                            tentacleSize = 0;
                            break;
                        case appearanceDefs_1.VAGINA_LOOSENESS_NORMAL:
                            tentacleSize = 4;
                            break;
                        case appearanceDefs_1.VAGINA_LOOSENESS_LOOSE:
                            tentacleSize = 16;
                            break;
                        case appearanceDefs_1.VAGINA_LOOSENESS_GAPING:
                            tentacleSize = 40;
                            break;
                        case appearanceDefs_1.VAGINA_LOOSENESS_GAPING_WIDE:
                            tentacleSize = 65;
                            break;
                ...
                //resume secksings
                this.outputText(this.vaginaDescript(0) + ". Pulling gently, you manage to yank a bit more vine free, allowing it to brush against the damp forest loam. That same soft earth makes the perfect cushion for you as you lay down, spreading your legs. With both hands you grasp the vine, guiding it towards the entrance of your " + this.vaginaDescript(0) + ". The beaded moisture that covers the vine tingles tantalizingly at the first contact with your lips.\n\n");
                if (tentacle == 0) //Small
                    this.outputText("With a sexy little sigh, you slip the mushroom-like tip between your nether-lips, feeling it bulge a little as it penetrates you. The vine's lubricants combine with your own, turning your horny cunt into a sloppy little slip-and-slide. You take it all the way to your cervix, easily handling its smaller size as you begin to use it like a favorite dildo. Deep inside your " + this.vaginaDescript(0) + ", the vine's lubricants begin to make your passage tingle, intensifying until your entire channel is overloaded with clit-like levels of sensation.\n\n");
                if (tentacle == 1) //Medium
                    this.outputText("With a soft grunt, you manage to wrangle the fat tip of the vine between your nether-lips, feeling the swollen bulge pulse inside you penetrate yourself with it. The vine's lubricants combine with your own, turning your horny cunt into a sloppy slip-and-slide. You force in the rest of the vine's length, taking it all the way to your cervix, enjoying the feeling of fullness it gives you as you begin pumping it in and out like an obscene green dildo. Deep inside your " + this.vaginaDescript(0) + ", the vine's lubricants begin to make your passage tingle, intensifying until your entire channel is overloaded with clit-like levels of sensation.\n\n");
                if (tentacle == 2) //Large
                    this.outputText("With a desperate grunt, you barely manage to force the obscene cock-head of the vine between your nether-lips. The swollen bulge pulses inside you, stretching you uncomfortably as it reacts to the warmth and tightness of your " + this.vaginaDescript(0) + ". The vine's lubricants begin to combine with your own, rapidly transforming your horny cunt into a sloppy slip-and-slide. You manage to cram the vine the rest of the way inside, bottoming it out against your cervix, reveling in the feeling of being stretched so wide, as you begin pumping it in and out of your " + this.vaginaDescript(0) + " like an over-sized sex-toy. Deep inside your " + this.vaginaDescript(0) + ", the vine's lubricants begin to make your passage tingle, intensifying until your entire channel is overloaded with clit-like levels of sensation.\n\n");
                if (this.player.cuntChange(tentacleSize, true))
                    this.outputText("\n\n"); //Stretch cuuuuunt and newline if it gets stretched
                this.outputText("The rest of the world disappears as your mind tries to cope with the sensation overload coming from your groin. You're dimly aware of your hands pumping the slippery vine in and out, in and out, over and over.  Hips bucking, " + this.vaginaDescript(0) + " squeezing, thighs trembling, you achieve the first of many orgasms.  Incredibly, the sensitivity of your groin redoubles, nearly blacking you out from the pleasure.  Cumming over and over, you writhe in the dirt, pumping the corrupted prick-vine in and out of your spasming cunt.  Your eyes roll back in your head when the vine begins pumping you full of its strange fluid, and you finally lose your battle to remain conscious.\n\n");
                ...
                //Xforms
                if (CorruptedGlade.rand(3) == 0 && this.player.hairColor != "green") { //Change hair to green sometimes
                ...
                if (CorruptedGlade.rand(4) == 0 && this.player.hipRating <= 10) { //+hip up to 10
                    this.outputText("A strange shifting occurs below your waist, making your " + this.player.armorName + " feel tight.  <b>Your hips have grown larger</b>, becoming " + this.hipDescript() + ".  ");
                ...
                if (this.player.cocks.length > 0) { //Cocks (and maybe vagina)
                    this.outputText(this.player.cocks.length == 1 ? this.cockDescript(0) : this.multiCockDescriptLight());
                    ...
                    this.outputText("nipples"); //Nipples
                //Finish sentance
                ...
                if (CorruptedGlade.rand(3) == 0 && this.player.cocks.length > 0 && this.player.hairColor == "green") {
                    if (this.player.tentacleCocks() < this.player.cockTotal()) {
                        if (this.player.cocks.length == 1) { //Single cawks
                            this.outputText("Your feel your " + this.cockDescript(0) + " bending and flexing of its own volition... looking down, you see it morph into a green vine-like shape.  <b>You now have a tentacle cock!</b>  ");
                            //Set primary cock flag
                            this.player.cocks[0].cockType = CockTypesEnum_1.CockTypesEnum.TENTACLE;
                        }
                        if (this.player.cockTotal() > 1) { //multi
                            this.outputText("Your feel your " + this.multiCockDescriptLight() + " bending and flexing of their own volition... looking down, you watch them morph into flexible vine-like shapes.  <b>You now have green tentacle cocks!</b>  ");
                            for (var x = 0; x < this.player.cocks.length; x++)
                                this.player.cocks[x].cockType = CockTypesEnum_1.CockTypesEnum.TENTACLE;
                        }
                    }
                }
            }
        ...
        treeBoobFun() {
            ...
            this.player.slimeFeed();
            if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.LustyTongue) < 0) {
                if (CorruptedGlade.rand(4) == 0) { //25% Chance of sensitive mouth status – increased lust gain/hour due to licking your lips :3
                    ...
                    this.player.createStatusAffect(StatusAffects_1.StatusAffects.LustyTongue, 24, 0, 0, 0);
                }
                else
                    this.outputText("  Thankfully, the feeling goes away after a few minutes, leaving you feeling just a bit more sensitive.");
            }
            if (CorruptedGlade.rand(2) == 0) { //50% Chance of breast-growth
    ...
    exports.CorruptedGlade = CorruptedGlade;
});
