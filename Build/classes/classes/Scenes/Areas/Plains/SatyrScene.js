define(["require", "exports", "../../../BaseContent", "./Satyr", "../../../GlobalFlags/kFLAGS", "../../../PregnancyStore"], function (require, exports, BaseContent_1, Satyr_1, kFLAGS_1, PregnancyStore_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class SatyrScene extends BaseContent_1.BaseContent {
        ...
        */
        satyrEncounter(location = 0) {
            this.clearOutput();
            this.spriteSelect(98);
            if (SatyrScene.rand(2) == 0 || this.player.pregnancyIncubation > 0 || this.player.buttPregnancyIncubation > 0 || this.player.gender == 0) {
                this.outputText("As you cross the ");
                if (location == 0)
                    this.outputText("grassy plains");
                else
                    this.outputText("sodden expanse of the swamp");
                this.outputText(", you hear lewd bellowings and drunken curses.  From out of the expanse of green comes a humanoid figure with a set of goat-like horns curling from his head.  Seeing you, he lets out a randy bleat and charges, naked erection jabbing before him!");
                this.startCombat(new Satyr_1.Satyr());
            }
        ...
        keepDrinking() {
            this.clearOutput();
            this.spriteSelect(98);
            this.outputText("You grin at the satyr's encouragement and continue drinking, setting on a slower pace so you won't spill any more; shortly you pass him the empty skin and ask for more.");
        ...
        leavePartySatyr() {
            this.clearOutput();
            this.spriteSelect(98);
            this.outputText("You thank the satyr for his generosity, but you don't think you can handle this kind of booze, so you get up and start on your way back to your camp.");
        ...
        trickZeSatyr() {
            this.clearOutput();
            this.spriteSelect(98);
            this.outputText("You come up with a plan and pretend to start drinking again; once you notice the satyr is distracted, you quickly spill most of your drink on the floor and return an empty skin to him.");
        ...
        loseToSatyr() {
            ...
            if (this.player.lust > 99)
                this.outputText("You pant as you begin masturbating furiously, too horny to care about anything the grinning satyr before you has in mind.");
            else
                this.outputText("You try to steady yourself, clutching your body in pain as the satyr grins at you.");
            ...
            if (this.player.isNaga())
                this.outputText("pins down your serpentine body");
            else if (this.player.isBiped())
                this.outputText("spreads your [legs]");
            else
                this.outputText("pins you down");
            this.outputText(" and reaches forward, probing for a vagina.  ");
            if (this.player.hasVagina())
                this.outputText("Upon finding one, he grins lustfully, his already impressively erect cock rising to full flag.");
            else
                this.outputText("Finding you lacking that trait, he grunts disdainfully and probes roughly for an asshole, which he locates right between your buttcheeks where it belongs, then jerks himself with several strokes to help coax his masculinity to full erection.");
            this.outputText("He grabs your [ass] and roughly squeezes your cheeks, kneeling down while lifting you up so he can impale your [vagOrAss].");
            ...
            if ((this.player.hasVagina() && this.player.vaginalCapacity() < this.monster.cockArea(0)) || (!this.player.hasVagina() && this.player.analCapacity() < this.monster.cockArea(0)))
                this.outputText(", not even bothered by the fact that his cock doesn't even fit inside you");
            this.outputText(".");
            //[cunt/buttchange]
            if (this.player.hasVagina())
            ...
            if (this.player.hasCock())
                this.outputText("  [EachCock] throbs as it send strings of cum flying straight into the satyr's chest, painting it white.");
            if (this.player.hasVagina())
                this.outputText("  A veritable flood of girlcum escapes the small space between your vaginal walls and the satyr's massive shaft, covering his balls and legs with your female fluids.");
            ...
            this.outputText("; your belly begins to swell and bulge as he pumps liters of semen into you.");
            ...
            if (this.player.cor < 50)
                ...
                this.outputText("shamelessly ");
            this.outputText(" gather and re-dress in your garments and head back to camp, cum still dribbling from you as you go.");
        ...
        defeatASatyr() {
            ...
            if (this.monster.lust > 99)
                this.outputText("The satyr collapses to its caprine knees, bleating in dismay as it paws frantically at its huge cock, oblivious to everything in its need to get off.  Already, pre-cum is fountaining from the goat-man's shaft, his jerking motions smearing the pungent sexual fluid across the crown.");
            //HP Victory
            else
                this.outputText("Beaten and dazed, the satyr collapses to its caprine knees, shaking his head in a futile attempt to recover himself from the brutal trouncing you've just given him.  The combination of the blows and his previous drunken state mean he's quite incapable of getting back, however.");
            var butt = undefined;
            var faces = undefined;
            ...
            if (this.player.hasVagina() && this.player.biggestTitSize() >= 4 && this.player.armor == this.armors.LMARMOR)
        ...
        femaleTakesAdvantageOfSatyr() {
            ...
            if (this.player.hasCock())
                this.outputText(", your own cock belching its usual load across his stomach and the surroundings");
            this.outputText(".");
        ...
        malesTakeAdvantageOfSatyrs() {
            ...
            this.outputText("\n\nThe moment your fingers slide inside the helpless satyr he bleats, not in horror, but in delight.  His insistent struggles to get away from you make a complete U-turn, and instead he struggles to get more of your hand inside his gripping hole; his cock throbs and grows even harder, spurting pre on the ground below, as his balls churn and slosh eager for the prostate massage you'll be giving him with your own " + this.cockDescript(x) + ".  You can't help but laugh at the satyr's change of heart; what a slut.");
            ...
            if (this.player.cumQ() < 250)
                this.outputText("  He tries to milk you for far more than you can give him, and at one point you feel like his anal muscles are going to tear your dick off, until finally they go lax and lets you slip out.");
            //(Medium Cum Amount)
            else if (this.player.cumQ() < 1000)
                this.outputText("  You dump far more than you usually do; the satyr's clenching asshole makes sure to drain you of every little drop of cum you have until finally it goes lax and lets you slip out, leaving the goat-man with a pudgy belly.");
            //(High Cum Amount)
            else
                this.outputText("  You shoot jet after jet of cum, far faster than the satyr's ass can milk you for it, and still he doesn't seem to stop trying.  His rippling muscles draw your cum inside his bowels in big bubbles.  When his ass finally goes lax, you're propelled halfway out of him by the force of your own backflowing seed; the satyr's ass looks like a wreck, and his belly is overflowing with so much spunk you're surprised it didn't come out of his mouth.");
        ...
        willinglyBoneSatyr() {
            this.clearOutput();
            this.spriteSelect(98);
            this.outputText("The satyr eyes you up and down hungrily; his hands move to grope your [chest], gently tweaking your [nipples], then he moves down towards your ");
            if (this.player.hasCock())
                this.outputText("hardening maleness, giving it a few strokes to bring it to full mast, then he moves on to your ");
            if (this.player.balls > 0)
                this.outputText("[balls], rolling each of your orbs around with reverence at the life-giving load they produce; finally he addresses your ");
            //((if PC has a pussy)
            if (this.player.hasVagina())
                this.outputText("[vagina], spreading your labia wide to see the interior of your rapidly moistening walls.");
            else
                this.outputText("[ass], spreading your cheeks apart to gaze at your winking rosebud.");
            this.outputText("\"<i>Wonderful!  With a [vagOrAss] like this I'm sure you'll make a great " + this.player.mf("father", "mother") + " for our children,</i>\" he says, leaning closer to sniff at your ");
            if (this.player.hasVagina())
                this.outputText("[vagina], licking all over your netherlips and tasting your juices.");
            else
                this.outputText("[butt], licking all around your hole to prepare it for what's coming.");
            ...
            if (!this.player.hasVagina())
                this.outputText("clenching butt");
            else
                this.outputText("spasming vagina");
            this.outputText(".  It's like someone shoved a hose into you and opened the nozzle, filling you with an endless stream of hot spooge; you can feel it pooling inside you, inflating your belly with his gigantic load.");
            ...
            if (this.player.hasVagina())
                this.outputText("pussy gushes fluids down his massive manhood");
            if (this.player.gender == 3)
                this.outputText(" and ");
            if (this.player.hasCock())
                this.outputText("cock spurts cream across your intertwined forms");
            this.outputText(".  Moaning softly and heaving in great lungfuls of breath, you sink back down onto him, feeling well and truly sated.");
            this.outputText("\n\nThe satyr's load has reduced to a trickle, but he still tries to pump more of it inside your ");
            if (!this.player.hasVagina())
                this.outputText("bowels");
            else
                this.outputText("womb");
            this.outputText("; making slow, short thrusts to ensure you've got all the seed that you need.  Panting, he grins at you from his prone position.  \"<i>Tell me then... did you like it?  Think that was enough baby batter to put a little satyr inside you?  Because if not I can go again.</i>\"  He grins confidently.");
            //(if male/genderless and 0 satyr children)
            if (!this.player.hasVagina() && this.flags[kFLAGS_1.kFLAGS.SATYR_KIDS] == 0) {
                ...
                if (this.player.hasCock())
                    this.outputText("males");
                else
                    this.outputText("neuters");
                this.outputText(" like you, lover.</i>\"");
        ...
        satyrPreggo() {
            if (this.player.hasVagina()) {
                this.player.knockUp(PregnancyStore_1.PregnancyStore.PREGNANCY_SATYR, PregnancyStore_1.PregnancyStore.INCUBATION_SATYR); //Satyrs can now fertilize eggs for ovipositing players
            }
            else {
                this.player.buttKnockUp(PregnancyStore_1.PregnancyStore.PREGNANCY_SATYR, PregnancyStore_1.PregnancyStore.INCUBATION_SATYR, 1, 1);
            }
        }
        //Birthing (Z)
        //Baby is always male.
        satyrBirth(vag) {
            ...
            if (vag && !this.player.hasVagina()) {
                this.outputText("newly grown vagina");
            ...
            this.outputText(", a gripping, writhing sensation as if something is thrashing around inside you and wants to get out.  Looks like it's time for whatever is inside you to emerge.");
            ...
            this.outputText("; it's adorned with a pair of miniscule horns.  You groan in pain as its shoulders stretch you even wider, but the worst part of it is over and, within moments, you are delivered of your child.  ");
            if (vag)
                this.player.cuntChange(40, true, false, true);
            else
                this.player.buttChange(40, true, false, true);
            this.outputText("Long minutes pass as your body recovers from the strain, bouncing back with unnatural vitality from the exhaustion and pain of giving birth, but finally you are ready to look at it.");
            ...
            if (this.player.cor < 33)
                this.outputText(" and more than a bit disgusted");
            this.outputText(", you slip into a short, fitful sleep.");
            //badabingbadaboom
            this.flags[kFLAGS_1.kFLAGS.SATYR_KIDS]++;
        }
    }
    exports.SatyrScene = SatyrScene;
});
//# sourceMappingURL=SatyrScene.js.map