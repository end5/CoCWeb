define(["require", "exports", "../../../Monster", "../../../StatusAffects", "../../../PerkLib", "../../../../../includes/appearanceDefs", "../../../Appearance", "../../../internals/ChainedDrop"], function (require, exports, Monster_1, StatusAffects_1, PerkLib_1, appearanceDefs_1, Appearance_1, ChainedDrop_1) {
    "use strict";
     ...
     * @author ...
     */
    class GnollSpearThrower extends Monster_1.Monster {
        hyenaPhysicalAttack() {
            var damage = 0;
            //return to combat menu when finished
            this.doNext(this.game.playerMenu);
            //Blind dodge change
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.Blind) >= 0 && GnollSpearThrower.rand(3) < 2) {
                ...
                if (this.statusAffectv1(StatusAffects_1.StatusAffects.Attacks) - 1 >= 0) {
            ...
            //("Misdirection"
            if (this.player.findPerk(PerkLib_1.PerkLib.Misdirection) >= 0 && GnollSpearThrower.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
            ...
            //Determine if cat'ed
            if (this.player.findPerk(PerkLib_1.PerkLib.Flexibility) >= 0 && GnollSpearThrower.rand(100) < 6) {
            ...
            if (damage <= 0) {
                ...
                if (GnollSpearThrower.rand(this.player.armorDef + this.player.tou) < this.player.armorDef)
                    this.outputText("The gnoll before you suddenly charges, almost too fast to see.  Twin fists slam into your " + this.player.armorName + " with enough force to stagger you, but the force is absorbed without doing any real damage.  As jaws powerful enough to crush bone flash at your neck, you are able to twist to the side, letting the furious hyena slip by you.", false);
                else
                    this.outputText("You deflect and block every " + this.weaponVerb + " " + this.a + this.short + " throws at you.", false);
            }
            else {
                if (damage < 10)
                    this.outputText("The gnoll runs forward, fury in her dark eyes as twin fists glance off your chest.  The glancing blow sends her off balance and the flashing ivory jaws barely miss your throat.  You push back, stumbling away from the furious hyena. (" + damage + ")", false);
                else
                    this.outputText("The gnoll rushes forward, almost too fast to detect before twin fists slam into your torso.  Before you can recover, ivory jaws flash before your eyes and you feel the sharp teeth start to clamp onto the " + this.player.skinDesc + " of your neck.  Blinding pain causes you to fling yourself backwards, away from the teeth and drawing angry scrapes as you escape the jaws.  You roll away before picking yourself up, the hyena moving confidently towards you as you try to shake off the pain from the blow. (" + damage + ")", false);
            }
            if (damage > 0) {
        ...
        hyenaJavelinAttack() {
            ...
            //Blind dodge change
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.Blind) >= 0 && GnollSpearThrower.rand(3) < 2) {
                this.outputText("The gnoll pulls a javelin from behind her and throws it at you, but blind as she is, it goes wide.", false);
            }
            //Determine if dodged!
            else if (this.player.spe - this.spe > 0 && Math.floor(Math.random() * (((this.player.spe - this.spe) / 4) + 80)) > 80) {
                this.outputText("The gnoll pulls a long, dark wooden javelin from over her shoulder.  Her spotted arm strikes forward, launching the missile through the air.  The spear flashes through the distance towards your vulnerable form.  Even as you see doom sailing towards you, a primal instinct to duck pulls you down, and you feel the wind from the massive missile as it passes close to your ear.", false);
            }
            //Determine if evaded
            else if (this.player.findPerk(PerkLib_1.PerkLib.Evade) >= 0 && GnollSpearThrower.rand(100) < 10) {
                this.outputText("Using your skills at evading attacks, you anticipate and sidestep " + this.a + this.short + "'s thrown spear.\n", false);
            }
            //("Misdirection"
            else if (this.player.findPerk(PerkLib_1.PerkLib.Misdirection) >= 0 && GnollSpearThrower.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
                this.outputText("Using Raphael's teachings, you anticipate and sidestep " + this.a + this.short + "' thrown spear.\n", false);
            }
            //Determine if cat'ed
            else if (this.player.findPerk(PerkLib_1.PerkLib.Flexibility) >= 0 && GnollSpearThrower.rand(100) < 6) {
                this.outputText("With your incredible flexibility, you squeeze out of the way of " + this.a + this.short + "'s thrown spear.", false);
            }
            //<Hyena Attack 2 – Javelin – Unsuccessful – Absorbed>
            else if (this.player.armorDef > 10 && GnollSpearThrower.rand(2) == 0) {
                this.outputText("The gnoll pulls a long, dark wooden javelin from over her shoulder.  Her spotted arm strikes forward, launching the missile through the air.  The spear flashes through the air but hits at an angle, sliding off your " + this.player.armorName + " without doing any damage.  It disappears into the grass.", false);
            }
            else if (this.player.findPerk(PerkLib_1.PerkLib.Resolute) >= 0 && this.player.tou >= 75) {
                this.outputText("You resolutely ignore the spear, brushing the blunted tip away when it hits you.\n");
            }
            //<Hyena Attack 2 – Javelin – Successful – Player Entangled>
                ...
                this.outputText("The gnoll pulls a long, black javelin from over her shoulder.  Her spotted arm strikes forward, launching the missile through the air.  You attempt to dive to the side, but are too late.  The powerful shaft slams, hard, into your back.  Pain radiates from the powerful impact.  Instead of piercing you, however, the tip seems to explode into a sticky goo that instantly bonds with your " + this.player.armorName + ".  The four foot, heavy shaft pulls down on you awkwardly, catching at things and throwing your balance off.  You try to tug the javelin off of you but find that it has glued itself to you.  It will take time and effort to remove; making it impossible to do while a dominant hyena stalks you. (" + damage + ")", false);
                if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.GnollSpear) < 0)
                    this.player.createStatusAffect(StatusAffects_1.StatusAffects.GnollSpear, 0, 0, 0, 0);
                slow = 15;
                while (slow > 0 && this.player.spe > 2) {
                    slow--;
                    this.player.addStatusValue(StatusAffects_1.StatusAffects.GnollSpear, 1, 1);
                    this.player.spe--;
        ...
        hyenaSnapKicku() {
            ...
            //Blind dodge change
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.Blind) >= 0 && GnollSpearThrower.rand(3) < 2) {
                this.outputText("The gnoll tries to catch you with a brutal snap-kick, but blind as she is, she completely misses.", false);
            }
            //Determine if dodged!
            else if (this.player.spe - this.spe > 0 && Math.floor(Math.random() * (((this.player.spe - this.spe) / 4) + 80)) > 80) {
                this.outputText("The gnoll grins at you before striding forward and pivoting.  A spotted leg snaps up and out, flashing through the air towards your " + this.chestDesc() + ".  You step back just in time, robbing the blow of force.  The paw lightly strikes your torso before the female hyena springs back, glaring at you.", false);
            }
            //Determine if evaded
            else if (this.player.findPerk(PerkLib_1.PerkLib.Evade) >= 0 && GnollSpearThrower.rand(100) < 10) {
                this.outputText("Using your skills at evading attacks, you anticipate and sidestep " + this.a + this.short + "'s snap-kick.\n", false);
            }
            //("Misdirection"
            else if (this.player.findPerk(PerkLib_1.PerkLib.Misdirection) >= 0 && GnollSpearThrower.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
                this.outputText("Using Raphael's teachings, you anticipate and sidestep " + this.a + this.short + "' snap-kick.\n", false);
            }
            //Determine if cat'ed
            else if (this.player.findPerk(PerkLib_1.PerkLib.Flexibility) >= 0 && GnollSpearThrower.rand(100) < 6) {
                this.outputText("With your incredible flexibility, you squeeze out of the way of " + this.a + this.short + "'s snap-kick.", false);
            }
            //Determine damage - str modified by enemy toughness!
            else {
                ...
                //No damage
                if (damage <= 0) {
                    this.outputText("The gnoll tries to catch your " + this.chestDesc() + " with a snap-kick, but you manage to block the vicious blow.", false);
                }
                //<Hyena Attack 3 – Snap Kick – Successful>
                else {
                    this.outputText("A glint enters the dark eyes of the gnoll before she strides forward and pivots.  A long, spotted leg snaps up and out to slam against your " + this.chestDesc(), false);
                    if (this.player.biggestTitSize() >= 1)
                        this.outputText(", sending a wave of pain through the sensitive flesh", false);
                    this.outputText(".  A small, traitorous part of you can't help but notice a flash of long, dark flesh beneath her loincloth even as you stagger back from the impact. (" + damage + ")", false);
        ...
        hyenaArousalAttack() {
            ...
            //<Hyena Attack 4 – Arousal Attack – Mildly Successful>
                ...
                this.outputText("A lazy grin spreads across the gnoll's face before she sprints towards you.  Too fast to follow, she flies forward, and you desperately brace for an impact that doesn't come.  Instead of striking you, two spotted paws clamp behind your neck and pull your head down, planting your face against her leather loincloth.  A powerful, musky smell burns in your nose and the feel of firm flesh behind the flimsy leather leaves a tingling sensation along your face.  Instinctively, you tear away from the hold, stumbling away from the sensations filling your mind, though some desire remains kindled within you.", false);
                this.game.dynStats("lus", (15 + this.player.lib / 20 + this.player.sens / 5));
            }
            //<Hyena Attack 4 – Arousal Attack – Unsuccessful>
            else {
                this.outputText("A knowing glint fills the dark eyes of the gnoll before she sprints forward.  Your muscles tense as she reaches you and starts to lock two spotted paws behind your neck.  She pulls you down towards her musky crotch, but just as you brush her loincloth, you twist away.  The hyena snarls in frustration, and you're left wondering if that was her idea of foreplay.", false);
            }
            this.combatRoundOver();
        }
        eAttack() {
        ...
        constructor() {
            ...
            this.drop = new ChainedDrop_1.ChainedDrop().add(this.consumables.GROPLUS, 1 / 5).add(this.consumables.INCUBID, 1 / 2).elseDrop(this.consumables.BROWN_D);
    ...
    exports.GnollSpearThrower = GnollSpearThrower;
});
//# sourceMappingURL=GnollSpearThrower.js.map