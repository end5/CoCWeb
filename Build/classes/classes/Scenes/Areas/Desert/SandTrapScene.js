define(["require", "exports", "../../../BaseContent", "./SandTrap", "../../../GlobalFlags/kFLAGS", "../../../../../includes/appearanceDefs", "../../../StatusAffects", "../../../PregnancyStore"], function (require, exports, BaseContent_1, SandTrap_1, kFLAGS_1, appearanceDefs_1, StatusAffects_1, PregnancyStore_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class SandTrapScene extends BaseContent_1.BaseContent {
        get sandTrap() {
            return this.monster;
        }
        ...
        encounterASandTarp() {
            ...
            if (this.flags[kFLAGS_1.kFLAGS.SANDTRAP_LOSS_REPEATS] >= 2 && this.player.eyeType == appearanceDefs_1.EYES_BLACK_EYES_SAND_TRAP && this.player.wingType == appearanceDefs_1.WING_TYPE_GIANT_DRAGONFLY) {
                this.sandTrapBadEnd();
                return;
            }
            this.flags[kFLAGS_1.kFLAGS.TIMES_ENCOUNTERED_SAND_TRAPS]++;
            if (this.flags[kFLAGS_1.kFLAGS.TIMES_ENCOUNTERED_SAND_TRAPS] == 1) {
                //femininity <= 65: 
                ...
                //Speed check fail: 
                    ...
                    this.startCombat(new SandTrap_1.SandTrap());
        ...
        saveTheSandTarps() {
            ...
            this.startCombat(new SandTrap_1.SandTrap());
            this.monster.changeStatusValue(StatusAffects_1.StatusAffects.Level, 1, 2);
            this.doNext(this.playerMenu);
        }
        //[Don't Save]: 
        dontSaveTheTarps() {
        ...
        sandtrapmentLoss(clear = false) {
            if (this.flags[kFLAGS_1.kFLAGS.SANDTRAP_LOSS_REPEATS] >= 2 && this.player.eyeType == appearanceDefs_1.EYES_BLACK_EYES_SAND_TRAP && this.player.wingType == appearanceDefs_1.WING_TYPE_GIANT_DRAGONFLY) {
                this.loseLastFightWithSandTrap();
                return;
            }
            this.flags[kFLAGS_1.kFLAGS.SANDTRAP_LOSS_REPEATS]++;
            if (clear)
                this.clearOutput();
            else
                this.outputText("\n\n");
            this.spriteSelect(97);
            if (this.sandTrap.trapLevel() == 1)
                this.outputText("You are sunk to your belly in the depthless sand at the bottom of the pit, and are still falling fast.  The sun above you is blotted out by a shape which leans downwards towards you, smiling triumphantly.  Desperately, you try to keep your arms free so you can swing a blow at it, but with consummate ease, the sandtrap grabs your wrists with one set of hands while pushing you downwards with the others.  Within moments you are up to your armpits in the stuff, staring helplessly up at the strange androgynous creature which has you entirely at its mercy.");
            //PC lust loss: 
            else
                this.outputText("You feel as radiant and molten as the sun above you... you just want to sink into the warm sand surrounding you forever.  Why are you struggling against it again?  You can't remember; with a sigh, you fall backwards onto the soft powder and allow yourself to be carried right down to the bottom.  The sandtrap chuckles softly as it envelopes you in its waiting arms.  \"<i>Good " + this.player.mf("boy", "girl") + "...</i>\"");
            if (this.player.hasCock())
                this.doNext(this.dickwieldersLoseToSandTarps);
            else if (this.player.hasVagina())
                this.doNext(this.chicksLoseToSandTarp);
            else
                this.doNext(this.genderlessLoss);
        }
        pcBeatsATrap() {
            this.clearOutput();
            this.spriteSelect(97);
            this.flags[kFLAGS_1.kFLAGS.SANDTRAP_LOSS_REPEATS] = 0;
            //PC HP victory: 
            if (this.monster.HP < 1)
                this.outputText("The sandtrap groans and collapses forwards into the dirt.  With the creature no longer controlling it, the sand underneath you feels a great deal more stable... climbing out won't be too difficult.");
            //PC lust victory: 
            else
                this.outputText("The sand around you stops sinking.  Overpowered with lust, the sandtrap moans and gives up control over itself and its pit to desperately run one set of hands over its flat chest whilst the other pushes into the sand to masturbate... whatever it has down there.  With your foe in this state, climbing out won't be so difficult now.");
            ...
            if (this.player.isNaga() && this.player.findStatusAffect(StatusAffects_1.StatusAffects.Naga) >= 0 && this.player.gender > 0 && this.player.faceType == appearanceDefs_1.FACE_SNAKE_FANGS)
                nagaThreeSome = this.nagaThreesomeWithSandTrap;
            //Requires: Penis and str requirement
            if (this.player.hasCock() && this.player.str >= 60)
                putYourDickInIt = this.stickWangInSandgina;
            //\"<i>Ride</i>\" (Z)
            if (this.player.hasVagina())
                rideDatSantTrap = this.rideDatSandTarpLikeIts1999;
            //\"<i>Hands</i>\" (Z)
            if (this.player.gender > 0)
                useSandTarpsHand = this.useSandTarpsHands;
            ...
            if (this.player.lust >= 33) {
                ...
                this.choices("Naga3Some", nagaThreeSome, "UseYourCock", putYourDickInIt, "RideVaginal", rideDatSantTrap, "Handjob", useSandTarpsHand, "", undefined, "", undefined, "", undefined, "", undefined, "B.Titfuck", bikiniTits, "Leave", this.cleanupAfterCombat);
            }
            else
        ...
        dickwieldersLoseToSandTarps() {
            ...
            //6 inches to 15: 
            else if (this.player.cocks[0].cockLength < 15)
            ...
            if (this.player.cumQ() > 1000)
                this.outputText("  The sandtrap gasps and coos as you ejaculate over and over again, and beneath you feel your cum wet the sands as it beads around the clenching grip it has you in.");
            if (this.player.hasVagina())
                this.outputText("  Ignored entirely, your " + this.vaginaDescript(0) + " quivers and orgasms in tandem, flexing and wetting fruitlessly against the dry water pressing against it.");
            ...
            if (this.player.analCapacity() < 40)
                this.outputText("  Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight hole wide.");
            else
                this.outputText("  Your accommodating ass feels like it was made for this purpose, and with the creature's oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your quim.  You moan as something round travels down the creature's ovipositor and into your wet ass, the sensation making [eachCock] hard again.");
            ...
            this.monster.createStatusAffect(StatusAffects_1.StatusAffects.Fertilized, 0, 0, 0, 0);
        ...
        chicksLoseToSandTarp() {
            ...
            if (this.monster.findStatusAffect(StatusAffects_1.StatusAffects.Fertilized) >= 0)
                this.outputText("  \"<i>A cute little wwworker hasz fallen into my home.  Wwwell, that isz ok.  Fortunately for hwyou, I already have some eggsz ready.  Hhyou will get to be a queen! Iszn't that exciting?</i>\"");
            else
                this.outputText("  \"<i>A cute little wwworker hasz fallen into my home, but I have no fertile eggsz to givve it.  Wwwell, that isz ok.  Hwwe can still have fun, can't we?</i>\"");
            ...
            if (this.player.analCapacity() < 30)
                this.outputText("Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight ass wide.");
            else
        ...
        genderlessLoss() {
            ...
            if (this.monster.findStatusAffect(StatusAffects_1.StatusAffects.Fertilized) >= 0)
                this.outputText("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Even ants who hhhave nothing can be someone for somebody.  And hhyou can be mommy for my childrrhen!</i>\"");
            else
                this.outputText("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Hwwe can still have fun, can't we?</i>\"");
        ...
        sandTrapPregChance() {
            ...
            if (this.monster.findStatusAffect(StatusAffects_1.StatusAffects.Fertilized) >= 0)
                this.player.buttKnockUp(PregnancyStore_1.PregnancyStore.PREGNANCY_SANDTRAP_FERTILE, PregnancyStore_1.PregnancyStore.INCUBATION_SANDTRAP, 1, 1);
            else
                this.player.buttKnockUp(PregnancyStore_1.PregnancyStore.PREGNANCY_SANDTRAP, PregnancyStore_1.PregnancyStore.INCUBATION_SANDTRAP, 1, 1);
        }
        birfSandTarps() {
            this.spriteSelect(97);
            ...
            if (this.player.gender == 1) {
                ...
                if (!this.player.isTaur() || (this.player.tallness * (5 / 6) < this.player.cocks[this.player.longestCock()].cockLength))
                    this.outputText("  Your oily hands descend upon your cock, and you massage your shaft as you feel the pressure in your bowels intensify again.");
            }
            //Female: 
                ...
                if (!this.player.isTaur())
                    this.outputText("  Your oily hands push softly into your cleft, fingering your needy " + this.clitDescript() + " as you feel the pressure in your bowels intensify again.");
            }
            //Herm: 
                ...
                //[(no horses)]
                if (!this.player.isTaur())
                    this.outputText("  Your oily hands descend upon your genitals and you begin to slowly pump your shaft and finger your needy " + this.clitDescript() + " as you feel the pressure in your bowels intensify again.");
            }
        ...
        useSandTarpsHands() {
            ...
            if (this.player.gender == 1) {
                ...
                //Three cocks: 
                    ...
                    if (this.player.balls > 0)
                        this.outputText("  It grins up at you shyly and, with its final hand, cups your [balls].  ]You laugh with an almost hysterical edge to the sky at how mindboggling getting a triple handjob feels, and the trap giggles slightly as well despite itself.");
            ...
            //Female: 
                ...
                if (this.player.tone < 60)
                    this.outputText("  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands sinking deep into your soft, pillowy behind as it clutches onto you.");
                else
                    this.outputText("  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands clutching onto your tight, muscled behind so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
        ...
        rideDatSandTarpLikeIts1999() {
            ...
            if (this.player.hasCock())
                this.outputText("  [EachCock] flexes in delicious concert with your quim, spurting jizz onto the trap's chest and face.");
        ...
        stickWangInSandgina() {
            ...
            if (this.player.balls > 0)
                this.outputText(", makes your " + this.ballsDescriptLight() + " swell in readiness,");
            ...
            //8 inches to 35: 
            else if (this.player.cocks[x].cockLength < 35)
            ...
            //Megacock: 
                ...
                if (this.player.balls > 0)
                    this.outputText("  Your " + this.ballsDescriptLight() + " slap against the creature's wet opening.  ");
            ...
            if (this.silly())
                this.outputText(", though with fewer pouting blue lips and doe eyes");
            ...
            if (this.player.balls > 0)
                this.outputText("balls ache and your ");
        ...
        nagaThreesomeWithSandTrap() {
            ...
            //First encounter: 
            if (this.flags[kFLAGS_1.kFLAGS.SANDTRAP_NAGA_3SOME] == 0) {
            ...
            if (this.player.hasCock()) {
                ...
                if (this.player.cocks[0].cockLength < 14) {
                    ...
                    if (this.player.balls > 0)
                        this.outputText("slapping your " + this.ballsDescriptLight() + " into its nose, forcing your cock all the way to the top of its tight throat.");
                }
                //14 inches or more: 
                else {
                    ...
                    if (this.player.skinType == appearanceDefs_1.SKIN_TYPE_PLAIN)
                        this.outputText("beading sweat, ");
                    this.outputText("pistoning into its submissive opening with fervor, ");
                    if (this.player.balls > 0)
                        this.outputText("your " + this.ballsDescriptLight() + " swinging heavily in front of the sandtrap's nose, ");
                ...
                if (this.player.cocks[0].cockLength >= 14)
                    this.outputText("  You're not sure which is the more pleasing sight; her, or the rhythmic bulging in the sandtrap's neck.");
            ...
            //Female: 
                ...
                if (this.player.wetness() < 4)
                    this.outputText("  Its gentle worship of your " + this.vaginaDescript(0) + " soon has you streaming rivulets of girlcum from your oversexed hole, dribbling down the sandtrap's face as you eagerly press yourself down for more.");
                ...
                if (this.player.cumQ() > 1000)
                    this.outputText("  The long, sensuous tongue bath has pushed your body into overdrive and the waterfall you unleash is even more uncontrollable than the last; you wonder dizzily if you haven't just lost control of your bladder.  The hypnotised sandtrap gladly drinks what you give it, but it's far too much and soon it is coughing your girlcum back up, through its nose and back onto your still contracting " + this.vaginaDescript(0) + ".");
            }
        ...
        sandTrapBadEnd() {
            ...
            if (this.player.hasCock())
                this.outputText("[EachCock] hardens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ");
            //Female: 
            else if (this.player.hasVagina())
                this.outputText("Your [vagina] moistens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ");
        ...
        desztiny() {
            ...
            //Male: 
            if (this.player.hasCock())
                this.outputText("Your cock strains upwards and you moan; feeling as sensitive as you do, you are both desperate to be touched and desperate not to be.  ");
            //Female:
            else if (this.player.hasVagina())
                this.outputText("You feel but barely hear the drip of moisture on the sand, both your [asshole] and [vagina] beading with oily need.  ");
        ...
        sandTrapBadEndFinale() {
            ...
            if (this.player.gender > 0) {
                ...
                if (this.player.hasCock())
                    this.outputText("cock harden");
                else
                    this.outputText("vagina dribble");
            }
            ...
            if (this.player.hasCock())
                this.outputText(" and masturbating [oneCock] desperately");
            else if (this.player.hasVagina())
                this.outputText(" and fingering your [vagina] desperately");
    ...
    exports.SandTrapScene = SandTrapScene;
});
//# sourceMappingURL=SandTrapScene.js.map