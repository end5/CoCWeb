define(["require", "exports", "../../../BaseContent", "../../../CoC", "../../../StatusAffects", "../../../GlobalFlags/kFLAGS", "./Phouka", "../../../PregnancyStore", "../../../GlobalFlags/kGAMECLASS"], function (require, exports, BaseContent_1, CoC_1, StatusAffects_1, kFLAGS_1, Phouka_1, PregnancyStore_1, kGAMECLASS_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    /**
     * Created by K.Quesom 11.06.14
     */
    class PhoukaScene extends BaseContent_1.BaseContent {
        constructor() {
            super();
        ...
        timeChange() {
            if (this.player.statusAffectv1(StatusAffects_1.StatusAffects.PhoukaWhiskeyAffect) > 0) {
                this.player.addStatusValue(StatusAffects_1.StatusAffects.PhoukaWhiskeyAffect, 1, -1); //Count down hours until player is not drunk
                if (this.player.statusAffectv1(StatusAffects_1.StatusAffects.PhoukaWhiskeyAffect) <= 0) {
                    kGAMECLASS_1.kGAMECLASS.consumables.P_WHSKY.phoukaWhiskeyExpires(this.player);
                    return true;
                }
            }
            return false;
        ...
        phoukaNameText(known, unknown) {
            return (this.flags[kFLAGS_1.kFLAGS.PHOUKA_LORE] == 0 ? unknown : known);
        }
        phoukaName() {
        ...
        phoukaEncounter() {
            ...
            var choiceChance = 0;
            if (this.flags[kFLAGS_1.kFLAGS.PHOUKA_ENCOUNTER_STATUS] == 0) { //Guarantee first Phouka encounter is with faerie fire
                this.flags[kFLAGS_1.kFLAGS.TREACLE_MINE_YEAR_DONE] = this.date.fullYear - 1; //If you've never encountered phoukas before then we can safely set all this stuff here
                this.flags[kFLAGS_1.kFLAGS.BIRTHS_PHOUKA] = 0;
                this.flags[kFLAGS_1.kFLAGS.BIRTHS_FAERIE] = 0;
            }
            else if ((this.date.month > 4) && (this.date.month < 11))
                choiceChance = PhoukaScene.rand(7); //All through the late summer and autumn they might be masturbating on berries
            else
                choiceChance = PhoukaScene.rand(6);
            if (choiceChance < 2)
                this.phoukaFaerieFire();
            else if (choiceChance < 5)
                this.phoukaStuck();
            else
                this.phoukaMasturbating();
        }
        phoukaFaerieFire() {
            this.clearOutput();
            this.outputText("As you’re wading through the bog a flickering light off in the distance catches your eye.  ");
            if (this.flags[kFLAGS_1.kFLAGS.PHOUKA_ENCOUNTER_STATUS] != 0) {
                this.outputText("You recall your last encounter with one of these 'lanterns' out here in the bog.\n\nPerhaps it would be best to leave this faerie fire alone.");
            }
            else { //Special text for the first encounter
                this.outputText("It seems to be moving around in one of the dense parts of the bog, where the canopy of dead trees and creeping vines create shadows as deep as twilight even at noon.\n\nYou can't make out a figure holding the light, but you decide to follow.  Spurred on by the thought that it may be someone else like yourself, or perhaps some agent of the demons, you keep trying to catch up with the light up ahead.  This trip into the bog is about to pay off.");
            ...
            this.menu();
            if (this.flags[kFLAGS_1.kFLAGS.PHOUKA_ENCOUNTER_STATUS] != 0 || this.player.hasKeyItem("Traveler's Guide") >= 0)
                this.addButton(4, "Leave", this.phoukaFaerieFireLeave);
            this.addButton(0, "Follow", this.phoukaStuck);
        }
        phoukaFaerieFireLeave() {
            if (this.flags[kFLAGS_1.kFLAGS.PHOUKA_ENCOUNTER_STATUS] == 0)
        ...
        phoukaStuck() {
            this.clearOutput();
            var hasAlcohol = this.player.hasItem(this.consumables.P_WHSKY, 1);
            this.outputText("You follow some firmer ground into one of the many copses of dead trees.  The bog is quiet and still here.  Up ahead you see something glint in the hollow of a dead tree, but before you can investigate the firm ground beneath your feet liquefies and you begin to sink.\n\nYou try to leap to another muddy lump of exposed ground, but even before you land it has gone soft as well.  You are soon encased in mud almost to your waist.\n\nThat's when you feel something slick and warm dart past you in the muck.  Wherever it goes it drags a thick net behind it.  Your ");
            if (this.player.isNaga())
                this.outputText("tail is");
            else
                this.outputText("legs are");
            this.outputText(" soon tangled in a solid web of intertwined roots.  Once you are bound in place, a large black eel launches itself free of the muck and lands in a nearby puddle.\n\nYour attacker seems to melt, then reforms, becoming a five inch tall " + this.phoukaName() + ".  His skin and wings are coal black, and his eyes are green and shiny like those of a cat.  He isn't wearing any clothes, and a fully erect cock almost an inch long juts out of his groin.  Apart from that and a lack of breasts, his body looks feminine, much like the faeries of the woods.\n\nThe " + this.phoukaName() + " leers at you, clearly deciding what he wants to do next.");
            if (hasAlcohol)
                this.outputText("\n\nYou suspect that if you offer up that whiskey in your pack the " + this.phoukaName() + " probably won’t try to rape you.");
            if (this.flags[kFLAGS_1.kFLAGS.PHOUKA_ENCOUNTER_STATUS] == 0)
                this.flags[kFLAGS_1.kFLAGS.PHOUKA_ENCOUNTER_STATUS] = 1;
            this.menu();
            if (hasAlcohol)
                this.addButton(0, "Offer Drink", this.phoukaStuckOfferWhiskey);
            this.addButton(1, "Talk", this.phoukaTalk);
            this.addButton(2, "Fight", this.phoukaStartFight);
        }
        phoukaStuckOfferWhiskey() {
            this.clearOutput();
            this.outputText("You pull out the bottle and ask the " + this.phoukaName() + " if he might be interested in a drink.  He looks at the bottle and says <i>“Well we both know where ya picked that up, hmmm?”</i>  Then he smiles and adds <i>“Course that’s some other phouka’s problem if he let ye beat him.”</i>");
            if (this.flags[kFLAGS_1.kFLAGS.PHOUKA_LORE] == 0)
                this.flags[kFLAGS_1.kFLAGS.PHOUKA_LORE] = 1; //Now you know what to call them
            this.player.consumeItem(this.consumables.P_WHSKY, 1);
            ...
            if ((this.player.pregnancyIncubation == 0) && (this.player.buttPregnancyIncubation == 0)) {
                this.menu();
                this.addButton(1, "Drink", this.phoukaDrinkAccept);
            }
            else
        ...
        phoukaStartFight() {
            ...
            this.startCombat(new Phouka_1.Phouka(this.phoukaName()));
        }
        phoukaStartFightSilenced() {
            this.startCombat(new Phouka_1.Phouka(this.phoukaName()));
            this.player.createStatusAffect(StatusAffects_1.StatusAffects.WebSilence, 0, 0, 0, 0);
        }
        phoukaTalk() {
            this.clearOutput();
            this.outputText("You ask the " + this.phoukaName());
            if (this.player.level < 10)
                this.outputText(" if perhaps there's some way he could release you.");
            else
                this.outputText(" if he really wants a taste of what you're going to give him.");
            if (this.player.cor + PhoukaScene.rand(125) <= 110) { //Players with 0% corruption have a 12% chance that the Phouka is willing to talk, players with 100% corruption have a 92% chance
                ...
                if (this.player.cor < 34)
                    this.outputText("vile little monster");
                else if (this.player.cor < 67)
                    this.outputText("perverted " + this.phoukaName());
                else
                    this.outputText("fuckable little " + this.phoukaName());
                this.outputText(" but he must not feel like listening.  He launches a ball of black mud at your face, sealing your mouth with sticky and [if (corruption <= 50)ewww -][if (corruption > 50)mmmm -] salty muck.");
                this.startCombat(new Phouka_1.Phouka(this.phoukaName()));
                this.player.createStatusAffect(StatusAffects_1.StatusAffects.WebSilence, 0, 0, 0, 0);
            }
            else { //The phouka would rather talk
                this.outputText("\n\nThe " + this.phoukaName());
                switch (this.player.level < 10 ? PhoukaScene.rand(8) : PhoukaScene.rand(12)) { //Low level characters don't get the chance to be offered whiskey
                    case 0:
                    case 1:
                    case 2:
                    case 3:
                    ...
                    case 6: //Offers you a chance to submit, which leads to the various defeat sex scenes
                        this.outputText(" considers for a moment and then says, <i>“You can't get away from me right now, so why don't you enjoy it?  Otherwise, maybe I bring in some of my friends an we all have at ya.”</i> The confidence in the " + this.phoukaName() + "'s gaze suggests he isn't bluffing.  He slowly circles you, hovering a few feet out of reach.  His eyes seem to be drinking in every detail of your body in a way that [if (corruption < 50)makes shivers run down your spine][if (corruption >= 50)makes your heart beat faster].\n\nThe " + this.phoukaName() + " continues, ");
                        if (this.player.hasVagina())
                            this.outputText("<i>“Maybe yer inta bunnies, hrm?  Big bunny with a nice hard cock and strong legs, I can do that fer ya.  Fill that tight little muff o' yers up with a big load o' spunk!”\n\n“Or maybe ");
                        else
                            this.outputText("<i>“");
                        this.outputText("I could change inta a goat for ya.  That suit yer fancy?  Is it a fond dream o' yers to let a goat blow its load in yer cornhole?");
                        if (this.player.hasVagina())
        ...
        phoukaDrinkWhilePregnant(playerOfferedTheBooze) {
            if ((this.player.pregnancyIncubation <= 100) || (this.player.buttPregnancyIncubation <= 100)) { //Pregnancy is obvious to the phouka
            ...
            var acceptable = this.consumables.P_WHSKY.phoukaWhiskeyAcceptable(this.player);
            if (acceptable < 0) {
                this.outputText("\n\nYou think about the unborn child" + (acceptable <= -3 ? "ren" : "") + " you're carrying.");
                if (acceptable == -4)
                    this.outputText("  One of them seems to crave the whiskey, but the other is a more normal child.");
                this.outputText("  Sure, some of the elders around the village said a beer or some light cider was all right for a woman heavy with child, but you doubt that whiskey would be acceptable.  You politely refuse the drink, hoping the phouka won't be offended.");
            }
            else {
                this.outputText("\n\nYou put one hand on your belly, about to refuse the drink.  Then you realize that yes, this is exactly what your ");
                if (acceptable <= 2)
                    this.outputText("baby needs.  Some light kicks inside your " + (acceptable == 2 ? "distended colon" : "womb") + " suggest that your child knows");
                else
                    this.outputText("babies need.  Some light kicks inside your womb and your distended colon suggest that your children know");
                this.outputText(" what's up for grabs.  <i>“Go on and have some already. I try not to drink alone.  Always end up fucked by an imp or a drider or something that way.”</i>");
            }
            this.menu();
            this.addButton(0, "Refuse", (playerOfferedTheBooze ? this.phoukaDrinkRefuseAfterOffer : this.phoukaDrinkRefuse));
            if (acceptable)
                this.addButton(1, "Drink", this.phoukaDrinkAccept);
        }
        phoukaDrinkAccept() {
            this.clearOutput();
            this.outputText("You accept the cup and give it a careful sniff.  Who knows what the phouka put in this stuff?  The smell reminds you of some scents from the old general store back home.  You try some and find it remarkably smooth for something supposedly cooked up from bog water and peat.  The black bunny gives you a crooked smile and settles down next to you, pouring out some more of the powerful spirits.\n\nBy the time you and the phouka have drunk half the bottle, both of you are feeling a lot better.");
            this.player.slimeFeed();
            switch (this.consumables.P_WHSKY.phoukaWhiskeyDrink(this.player)) {
                case 0: //Player isn't pregnant
                    this.dynStats("lus", PhoukaScene.rand(5) + 1, "cor", PhoukaScene.rand(2) + 1); //Minor lust and corruption gains (since you aren’t drinking it all yourself). These gains are permanent
                    break;
                case 1: //Child is a phouka or satyr, loves alcohol
                    this.outputText("A warm feeling in your belly lets you know your passenger enjoyed the meal as well.");
                    break;
                case 2: //Child is a faerie but will become a phouka with this drink
                    this.outputText("At first, you feel your baby struggle against the whiskey, then it seems to grow content and enjoy it.");
                    break;
                case 3: //Child is a faerie, hates phouka whiskey
                    this.outputText("You feel queasy and want to throw up.  There's a pain in your belly and you realize the baby you're carrying didn't like that at all.");
            }
            this.flags[kFLAGS_1.kFLAGS.PREGNANCY_CORRUPTION]++; //Faerie or phouka babies become more corrupted, no effect if the player is not pregnant or on other types of babies
            this.consumables.P_WHSKY.phoukaWhiskeyAddStatus(this.player);
            ...
            if (this.player.tou < 70) {
                this.outputText("  It looks like the phouka is holding his liquor about as well as you are.");
            }
            else {
                this.outputText("  It seems you can handle your liquor better than the phouka.");
            }
            this.outputText("  Soon both of you start to share stories about your adventures in Mareth.");
            this.phoukaDrinkTalk(false);
        }
        phoukaDrinkRefuse() {
            this.clearOutput();
            if ((this.player.pregnancyIncubation == 0) && (this.player.buttPregnancyIncubation == 0) && (this.player.pregnancyIncubation <= 100)) { //Pregnancy is obvious to the phouka
                this.outputText("The rabbit morph sits down heavily on the wet ground and sips his booze.  <i>“Don't want to fuck, don't want to drink.  What gives?  Live a little and let me have fun with your pregnant pussy.  I’ll get ya off, and it’s not like I can knock you up again.  Course, that'd be fun too.”</i>");
                this.menu();
                this.phoukaSexAddStandardMenuChoices();
                this.addButton(0, "Leave", this.phoukaDrinkRefusePregnantLeave);
            }
            else { //Less obvious pregnancy or player not pregnant
                this.outputText("<i>“You damn prude.  That does it! I don't have to sit around listening to this.  I try to be friendly like some people say ya should instead of just raping ya.  What's it get me, huh?”</i>");
                if ((this.player.lust > 66) || ((this.player.cor > 50) && (this.player.lust > 33))) {
                    ...
                    this.menu();
                this.addButton(0, "Leave", this.phoukaDrinkRefuseLeave);
            }
        }
        phoukaDrinkRefuseLeave() {
            this.clearOutput();
            this.outputText("You extract yourself from the mud and get out of the bog before the phouka changes its mind.");
            this.doNext(this.camp.returnToCampUseOneHour); //Return to camp, 1 hour used
        }
        phoukaDrinkRefusePregnantLeave() {
            this.clearOutput();
            this.outputText("<i>“Ah fuck.  You're a killjoy.  Don't you know the best time to get porked is when you've already got a bun in the oven?”</i>\n\nThe black bunny waves you away dismissively. <i>“Fine, yeah, go home.  I hope the little bastard hurts when it pops out.  I would have stretched you out but good.”</i>");
            this.doNext(this.camp.returnToCampUseOneHour); //Return to camp, 1 hour used
        }
        phoukaDrinkRefuseAfterOffer() {
            this.clearOutput();
            this.outputText("The phouka looks disappointed but downs one of the cups and says <i>“How come chicks that’re knocked up are always such stick in the muds?”</i> He starts to sip the second cup, looks over at you and chuckles. <i>“I guess in yer case you’ve got a good excuse - ye are stuck in the mud!”</i> The phouka keeps drinking and it isn’t long before he starts to reminisce.");
            this.phoukaDrinkTalk(true);
        }
        phoukaDrinkTalk(playerNotDrinking) {
            if (this.flags[kFLAGS_1.kFLAGS.PHOUKA_LORE] == 1) {
                this.flags[kFLAGS_1.kFLAGS.PHOUKA_LORE]++;
            ...
            else if (this.flags[kFLAGS_1.kFLAGS.PHOUKA_LORE] == 2) {
                this.flags[kFLAGS_1.kFLAGS.PHOUKA_LORE]++;
            ...
            else if (this.flags[kFLAGS_1.kFLAGS.PHOUKA_LORE] == 3) {
                this.flags[kFLAGS_1.kFLAGS.PHOUKA_LORE] += 1;
            ...
            else if (this.flags[kFLAGS_1.kFLAGS.PHOUKA_LORE] == 4) {
            ...
            var playerIsInebriated = (playerNotDrinking == false) && (this.player.tou < 70);
            if (playerIsInebriated) {
                ...
                this.menu();
                this.addButton(0, "Leave", this.phoukaDrinkLeave);
            }
            else {
                ...
                this.outputText("  He lets out a little giggle and begins to shrink. Tendrils of black smoke rise from the bunny morph’s black fur as he shrinks back to the size and shape of a faerie.");
                PhoukaScene.phoukaForm = PhoukaScene.PHOUKA_FORM_FAERIE;
                if (this.player.lust < 33)
                    this.outputText("  It looks like he needs a nap and you take that as your cue to get out of here.");
                else {
                    this.outputText("\n\nYou look at the sleeping phouka");
                    if (this.player.hasCock())
                        this.outputText(", [eachCock] beginning to swell" + (this.player.hasVagina() ? " and your [vagina] starting to moisten" : "") + ".");
                    else if (this.player.hasVagina())
                        this.outputText(", your [vagina] starting to moisten.");
                    else
                        this.outputText("and you feel your sphincter twitch as your brain thinks about what you could do with this phouka now."); //Genderless
                }
                this.menu();
                this.addButton(0, "Leave", this.phoukaDrinkLeave);
                if (this.player.lust >= 33) {
                    this.addButton(1, "Ride Faerie", this.phoukaSexFaerieNonCombat); //Works for all
                    if (this.player.hasCock())
                        this.addButton(2, "Fuck Faerie", this.phoukaSexFaerieFemaleNonCombat); //Male or Herm Only
                }
            }
        }
        phoukaDrinkLeave() {
        ...
        phoukaHalloween() {
            ...
            if (this.player.str < 40) {
                this.outputText("You find it very difficult to operate and have to squeeze it with your thighs, but you manage to pump a respectable amount before passing it on.");
            }
            else if (this.player.str < 80) {
                this.outputText("You find that treacle pumping is as hard as the phouka said, but you keep at it.  With what you pumped you must be as strong as they are.");
            }
            else {
                this.outputText("You don't know why they said this was difficult.  You have no problem pumping the treacle; in fact they have to stop you partway through to change barrels.");
            }
        ...
        phoukaPlayerWins(hpVictory) {
            ...
            if (hpVictory) { //You win by physical damage, the phouka cheats and runs
                ...
                this.outputText("The " + this.phoukaName() + " collapses to the ground and begins to jab his cock into the peat.");
                if (PhoukaScene.phoukaForm != PhoukaScene.PHOUKA_FORM_FAERIE) {
                    this.outputText("  As you watch, wisps of black smoke start to rise from him and his body starts to shrink.  In moments the ");
                    if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_BUNNY)
                        this.outputText("bunny morph");
                    else if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_GOAT)
                        this.outputText("goat morph");
                    else if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_HORSE)
                        this.outputText("stallion");
                    this.outputText(" has shrunk back down to the size and shape of a " + this.phoukaNameText("phouka", "faerie") + ".");
                    PhoukaScene.phoukaForm = PhoukaScene.PHOUKA_FORM_FAERIE;
                }
                if (this.player.lust < 33) {
                    this.outputText("  The threat dealt with you scoop, dig and finally extract yourself from the mire.  Your lower half is soaked and you long to sit in front of the fire at your camp.  Hefting your supplies you turn back the way you came.");
                    this.phoukaLeaveOnLustWin(false);
                }
                else {
                    this.outputText("  While he's busy, you extract yourself from the muck, and once you're free, you look over at the lusty " + this.phoukaName());
                    if (this.player.cor < 34)
                        this.outputText(".  You could sate yourself just to be sure you're not distracted on the way home.");
                    else if (this.player.cor < 67)
                        this.outputText(".  You could take advantage of the situation to slake your lust.");
                    else
                        this.outputText(".  It would be so easy to rape this perverted faerie, how can you resist?");
                    this.menu();
                    this.addButton(0, "Leave", this.phoukaLeaveOnLustWin);
                    this.addButton(1, "Anal Ride", this.phoukaSexFaeriePostCombat); //Works for all
                    if (this.player.hasCock())
                        this.addButton(2, "Fuck Faerie", this.phoukaSexFaerieFemalePostCombat); //Male or Herm Only
                }
            }
        }
        phoukaPregBirth() {
        ...
        phoukaPregUpdate() {
            ...
            if (this.player.pregnancyIncubation == 140) { //Stage 2:
                if (this.flags[kFLAGS_1.kFLAGS.PREGNANCY_CORRUPTION] > 0)
            ...
            if (this.player.pregnancyIncubation == 100) { //Stage 3:
                ...
                this.outputText(" You've noticed that this pregnancy doesn't seem to be affecting your breasts. It's as if the child inside you has no use for your milk.\n");
                return true;
            }
            if (this.player.pregnancyIncubation == 60) { //Stage 4:
                if (this.flags[kFLAGS_1.kFLAGS.PREGNANCY_CORRUPTION] > 6)
            ...
            if (this.player.pregnancyIncubation == 36) { //Stage 5:
                this.outputText("\nEven though your belly remains the same size you somehow feel that your pregnancy is drawing to a close. ");
                if (this.flags[kFLAGS_1.kFLAGS.PREGNANCY_CORRUPTION] > 6)
                    this.outputText("Despite the small size of your belly you spend most of your time feeling deeply ill and you can't wait to force this thing out.  You can feel a constant dull pain from your womb and ovaries, probably the result of the tainted fluid inside you.  Only drinking alcohol settles your stomach.\n");
                else if (this.flags[kFLAGS_1.kFLAGS.PREGNANCY_CORRUPTION] > 0)
                    this.outputText("Despite the relatively small size of your belly, you spend most of your time feeling mildly ill and you can't wait to force this thing out.\n");
                else
                    this.outputText("Despite being pregnant you still don't feel restricted or slowed in any way.  It's as if the tiny life inside is living in harmony with you.\n");
                return true;
            }
            return false;
        }
        ...
        phoukaSexBunny(postCombat, lustLoss) {
            this.clearOutput();
            if (postCombat) {
                this.outputText("As you collapse the " + this.phoukaName());
                if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_BUNNY)
                    this.outputText(" hops over and stands in front of you");
                else
                    this.outputText(" begins to shapeshift in front of you.  He changes into various forms, looks you over and finally changes into a black furred bunny-morph");
                this.outputText(".  You watch as the bunny's erection stiffens, your ");
                if (lustLoss)
                    this.outputText("mind so overcome with lust you can't");
                else
                    this.outputText("body too bruised to");
                this.outputText(" resist any further.");
            }
            else {
                if (this.player.cor <= 50) {
                    this.outputText("It might not be the wisest move, but you decide to let the " + this.phoukaName() + " have his way with you.  You tell him you would prefer a little bunny loving and the " + this.phoukaName() + " grins from ear to ear. <i>“Comin' right up,”</i> he says");
                    if (PhoukaScene.phoukaForm != PhoukaScene.PHOUKA_FORM_BUNNY)
                        this.outputText(", and morphs into a rabbit");
                    this.outputText(".  Hopefully he'll be gentle with a willing partner.");
                }
                else {
                    this.outputText("You can't think of any good reason to refuse a roll in the hay (or the mire) with this " + this.phoukaName() + ".  You tell him that you want to feel his big bunny cock deep inside you.  The " + this.phoukaName() + " grins as you begin seductively stripping off you armor.");
                    if (PhoukaScene.phoukaForm != PhoukaScene.PHOUKA_FORM_BUNNY)
                        this.outputText("  He goes through a short series of warm up exercises and then transforms himself into the rabbit you desire.");
                }
            }
            PhoukaScene.phoukaForm = PhoukaScene.PHOUKA_FORM_BUNNY;
            ...
            if ((this.player.lust > 80) || (this.player.cor > 50))
                this.outputText(" You can only moan in response.  The thought of this rabbit stuffing you has your [vagina] leaking already.");
            else
                this.outputText(" You are powerless to stop the bunny-morph as he undoes the last few straps holding your [armor] in place.");
            this.outputText("\n\nThe bunny's large feet take up position on either side of your [if (isNaga = true)tail][if (isNaga = false)rump] and you feel the tip of his cock brush against your cunt.  The bunny whispers <i>“[if (isPregnant = true)Time te feed the baby][if (isPregnant = false)Maybe today I'll leave ye a present].”</i>\n\nThe " + this.phoukaName() + " [if (isNaga = true)bends you over backwards, exposing your slit.  He][if (isTaur = true)strokes your flanks with his paws.  Next he] begins to grind his cock [if (isNaga = true)against your exposed underbelly][if (isNaga = false)between your legs], sliding it against your [clit] each time.  Shivers run up your spine as his fur tickles your [if (isNaga = true)scales from the top to the tip of your tail][if (isNaga = false)legs, back, butt and thighs].  When the bunny-morph is good and ready, he lines up and jams his pole into your moist pussy.  ");
            if (this.player.hasVirginVagina())
                this.outputText("You scream as the " + this.phoukaName() + "'s cock tears your hymen apart.  He stops thrusting and says <i>“A virgin? Here? Good goddess, missy, ya should have told me.”</i> In a lower tone he adds <i>“so I could have enjoyed it more.”</i> Your whole vagina burns as every fold is stretched and widened by the passage of the rabbit's cock.");
            else if (postCombat && (this.player.vaginas[0].vaginalLooseness < 2))
                this.outputText("You nearly pass out from the pain as the " + this.phoukaName() + "'s cock lengthens and thickens inside you.");
            else if (!postCombat || (this.player.cor > 50))
                this.outputText("Even as the invader lengthens and thickens you push back against it. <i>“Good girl. Good little fuck slut,”</i> whispers the " + this.phoukaName());
            this.player.cuntChange(50, true);
            ...
            this.phoukaSexPregnate(postCombat);
        }
        phoukaSexBunnyChoice() {
            this.phoukaSexBunny(false, false);
        }
        phoukaSexGoat(postCombat, lustLoss) {
            this.clearOutput();
            if (postCombat) {
                if (lustLoss)
                    this.outputText("Panting with need, you begin to strip off your [armor].  The " + this.phoukaName() + " watches the show with an evil smile.");
                else
                    this.outputText("Your body falls to the ground, too weak to fight back against the " + this.phoukaName() + " any longer.");
                if (PhoukaScene.phoukaForm != PhoukaScene.PHOUKA_FORM_GOAT)
                    this.outputText("  As you lay there, the " + this.phoukaName() + "'s body stretches and morphs until it takes the form of a large, black furred goat-morph.");
            }
            else {
                this.outputText("You tell the " + this.phoukaName() + " you're willing to let him take your ass in his goat form.  He smiles and lands near you.  His body warps into the form of a black furred goat morph.  ");
                if (this.player.cor <= 50)
                    this.outputText("You steel yourself for an asspounding while you remove your [armor].");
                else
                    this.outputText("Then you put on a show, taking your clothes off piece by piece.  You strip off your final undergarment, bend over and wink your asshole at the goat.");
            }
            ...
            if (postCombat)
                this.outputText("He mounts you immediately, giving you no chance to adjust to his invading cock.  The goat-morph's first powerful thrust drives your face into the muck as his tries to force his narrow cockhead into your ass.");
            else
                this.outputText("The goat rubs his cockhead against your pucker, spreading a thick layer of pre over your hole before slowly pressing home his cock.  His first few thrusts are gentle enough that you have the chance to spread your ass and get used to this invader.");
            this.outputText("  Disturbingly you feel his front 'hooves' split into hard little fingers that he wraps around your [if (isTaur = true)belly][if (isTaur = false)[chest]] to hold onto you even tighter.\n\n");
            if (postCombat)
                this.outputText("The " + this.phoukaName() + " shows no interest in you except as a receptacle for his cum.  After stabbing his hips at you several times you feel the full length of his cock penetrate your pucker.");
            else
                this.outputText("Once your [butt] has adjusted, the goat morph begins to thrust more quickly.  You are surprised when you feel his balls slapping against your [if (isNaga = true)tail][if (isNaga = false)inner thighs] - he can't already be in that deep, can he?");
            this.outputText("  Once the goat's cock is inside, he stops moving and tenses up.  You feel his cock growing larger and larger until you fear your abused anus is going to split apart.");
            this.player.buttChange(45, true);
            this.outputText("\n\nWith his cock expanded to full size and buried inside you, the goat begins rocking back and forth. <i>“How do ye like it?”</i> the " + this.phoukaName() + " bleats in your ear as he tries to force yet more of his cock inside you.  [if (corruption <= 50)You can't answer the goat, the pain of being split open is too great][if (corruption > 50)You pant with excitement from being dominated by this filthy shapeshifting goat].  The " + this.phoukaName() + " tenses once more, and this time you feel a thick deposit of semen being squirted into your violated ass.  ");
            if (postCombat)
                this.outputText("The goat-morph waits only a few seconds before unceremoniously yanking his deflating cock from your ass.");
            else
                this.outputText("Realizing that you haven't yet cum, the goat morph continues to rock in and out of your ass.  The air fills with the sucking and slurping noises of the goat's seed alternately being sucked deeper or forced out of your ass.  Finally you reach your limit and cum[if (hasCock = true), your own cock turning the mud white beneath you].");
            this.dynStats("cor", PhoukaScene.rand(1) + (postCombat ? 1 : 3)); //Extra two corruption for being enough of a pervert to want to fuck the phouka
            this.player.orgasm();
            if (postCombat)
                this.outputText("  While you're recovering the goat-morph reaches into your gem pouch and takes a handful.");
        ...
        phoukaSexHorse(postCombat, lustLoss) {
            this.clearOutput();
            if (postCombat) {
                this.outputText("As you collapse the " + this.phoukaName());
                if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_HORSE)
                    this.outputText(" circles you in a slow trot, his top half shaping into a centaur form before finally stopping directly in front of you.");
                else
                    this.outputText(" begins to shapeshift in front of you.  It grows and grows, finally taking the form of a massive, black, centaur stallion with an equally massive prick.");
            }
            else { //Phouka can’t be a horse if the player is choosing this. Must be either a faerie or a bunny.
                if (this.player.cor <= 50)
                    this.outputText("Perhaps this land is finally starting to get to you.  You tell the " + this.phoukaName() + " you want him to fuck you with a great big horse cock.  The " + this.phoukaName() + "'s eyes light up. <i>“Sure thing missy”</i> he says, and his form stretches into that of a huge, black furred, centaur stallion.  You hope he'll be gentle with a willing partner.");
                else
                    this.outputText("The more you think about it the more you decide you could use a real fucking.  You smile at the " + this.phoukaName() + ", telling him you want the biggest cock he can muster.  The " + this.phoukaName() + "morphs into the form you expected, that of a black, centaur stallion [if (tallness <= 96)taller than you are][if (tallness > 96)that's about your height].  Maybe that tree trunk between his legs can scratch your itch.");
            }
            PhoukaScene.phoukaForm = PhoukaScene.PHOUKA_FORM_HORSE;
            this.outputText("  On closer inspection the centaur is a bit unusual.  For a start, he has shiny green eyes.  Parts of his flanks look like thin wings pressed against its sides.  His nose is a little shorter and his face a lot more human than you would have expected.  He sees you looking him over and says <i>[if (isTaur = true)“I've been waitin for this.  A nice big mare to fuck the daylights out of][if (isTaur = false)“Yer gonna be sorry ye weren't born with four legs and a cunt you could lose an arm in, princess].”</i>\n\n");
            if (!postCombat || lustLoss)
                this.outputText("Considering the fire in your vagina that's crying out to be quenched, you don't care how big this horse's cock is.  You just need something to fill you NOW.");
            else
                this.outputText("You're hardly in any condition to refuse.  This stallion is going to pound you with that meatstick no matter what you do[if (corruption <= 50), so you steel yourself for what is to come][if (corruption > 50).  The thought of being wrecked by such an enormous staff has your cunt dripping girl-cum already.]");
            ...
            if (this.player.isTaur())
                this.outputText("He walks up behind you and mounts you.  You feel his front legs rubbing against your flanks and have to admit to yourself that this feels right.  The " + this.phoukaName() + "'s equine cock was built to force its way inside a mare[if (vagCapacity < 40), but it has difficulty with your tight cuntlips.  The stallion has to try twice more before his cock slides into place between your labia][if (vagCapacity >= 40) and it does just that].");
            else {
                if (this.player.isNaga())
                    this.outputText("He walks in front of you, turns your way and then simply walks over you, forcing your upper body to bend backward over your tail.  Thankfully your snake-like spine means bending into a 'U' shape isn't that uncomfortable.");
                else
                    this.outputText("He walks up behind you and then simply walks right over top of you, his chest knocking you to the ground.  You find yourself on all fours with a massive cockhead grinding against your ass.");
                this.outputText("  You hear the " + this.phoukaName() + " say <i>“Now line me up with yer cunt lass, or I'll lay down on top of ya and have a nap.”</i> Not wanting to be crushed under that kind of bulk, you reach [if (isNaga = true)up][if (isNaga = false)behind you] and grab his member.  You place his cock at the entrance to your [vagina] and the stallion steps forward.  [if (vagCapacity < 50)His cock bends and then snaps free of your cunt, slapping against his belly. <i>“Again! And do it right this time!”</i> he shouts.  You line it up once more and press against it with your hands so it has nowhere to go but in.]");
            }
            this.outputText("  You're convinced you're going to be split in half by this monster horse cock.\n\n");
            if (this.player.hasVirginVagina())
                this.outputText("You scream as the " + this.phoukaName() + "'s cock tears your hymen apart.  He stops mid-thrust and asks <i>“A virgin? Here? Good goddess missy ya should have told me.”</i> In a lower tone he adds <i>“so I could have enjoyed it more.”</i> Your whole vagina burns as every fold is stretched and widened by the passage of the " + this.phoukaName() + "'s huge horse cock.  You think he's delighted to have taken a woman from virgin to gaping in just one thrust.  ");
            else if ((postCombat) && (this.player.vaginas[0].vaginalLooseness < 3))
                this.outputText("You nearly pass out from the pain as the " + this.phoukaName() + "'s cock drives deeper and deeper inside you.  With such a wide head forcing its way in, you're sure the stretching will be permanent.  ");
            else if (!postCombat || (this.player.cor > 50))
                this.outputText("Even as the invader thrusts deeper you push back against it. <i>“Good girl, ya love horse cock, don't ya?”</i> whispers the " + this.phoukaName() + ".  ");
            this.player.cuntChange(80, true);
        ...
        phoukaSexFaerie(postCombat) {
            ...
            if (this.player.hasVagina()) {
                ...
                this.dynStats("cor", PhoukaScene.rand(1) + 3); //Extra two corruption for being enough of a pervert to want to fuck the phouka
                if (postCombat) {
                    kGAMECLASS_1.kGAMECLASS.clearStatuses(false);
                    kGAMECLASS_1.kGAMECLASS.awardPlayer(); //This will provide loot and return to camp, 1 hour used
                }
        ...
        phoukaSexFaerieNonCombat() {
            this.phoukaSexFaerie(false);
        }
        phoukaSexFaeriePostCombat() {
            this.phoukaSexFaerie(true);
        }
        phoukaSexFaerieFemale(postCombat) {
            ...
            if (this.player.biggestCockLength() < 8)
                this.outputText("You push the last inch of your " + Math.floor(this.player.biggestCockLength()) + " prick into the moaning slut.");
            else if (this.player.biggestCockLength() < 15)
                this.outputText("You feed the last inch of your " + Math.floor(this.player.biggestCockLength()) + " cock into her.  Your cockhead bumps against her cervix and the slut beneath you begs for more.");
            else {
                this.outputText("You drive as much of your cock as you can into the little faerie slut, but there's just too much meat for her to take in.");
                if (this.player.cor <= 50)
                    this.outputText("  It still feels amazing and you give her even more pleasure by rubbing your cockhead against her cervix.");
                else
            ...
            if (this.player.cor <= 33)
                this.outputText("You certainly don't want a phouka to learn where your campsite is.  You help her into the bushes and tell her she just needs to hide out for a few days.");
            else if (this.player.cor <= 66)
                this.outputText("You laugh and remind her that's what she's wanted to do to you.");
            else
                this.outputText("Why shouldn't the other phouka get the chance to enjoy her?  Before she can fully recover you grab some tree limbs and vines.  You pin her to the ground and use them to tie her up tight.  With her arms and legs lashed together and her ass in the air she's an inviting target.");
            this.outputText("  Satisfied, you march back to camp.");
            this.dynStats("cor", PhoukaScene.rand(1) + 3); //Extra two corruption for being enough of a pervert to want to fuck the phouka
            if (postCombat) {
                kGAMECLASS_1.kGAMECLASS.clearStatuses(false);
                kGAMECLASS_1.kGAMECLASS.awardPlayer(); //This will provide loot and return to camp, 1 hour used
        ...
        phoukaSexFaerieFemaleNonCombat() {
            this.phoukaSexFaerieFemale(false);
        }
        phoukaSexFaerieFemalePostCombat() {
            this.phoukaSexFaerieFemale(true);
        }
        phoukaSexPregnate(postCombat) {
            if (this.player.isPregnant()) {
                ...
                if (this.player.cor > 50)
                    this.outputText("  The feeling of all that seed flowing into you and then out of you again finally brings you to your own orgasm.[if (hasCock = true)  [EachCock] fires long strands of cum into the bog water beneath you.]");
                else
                    this.outputText("  As the thick grey cum leaks from your cunt you finally give in and get yourself off.  There's no way you could have made it back to camp feeling like that.");
                if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_FAERIE)
            ...
            if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_FAERIE)
                ...
                this.outputText("\n\nHe");
            if (postCombat)
                this.outputText(" rummages around in your gem pouch and then");
            this.outputText(" takes to the air and hovers nearby as if waiting for something.\n\n");
            if (this.player.isPregnant()) {
                var isFirstTime = (this.flags[kFLAGS_1.kFLAGS.BIRTHS_PHOUKA] == 0) && (this.flags[kFLAGS_1.kFLAGS.BIRTHS_FAERIE] == 0);
                this.flags[kFLAGS_1.kFLAGS.PREGNANCY_CORRUPTION] = 2; //Start off with a corrupt little phouka. It will take three drinks of pure honey to turn it into a faerie
                this.outputText("You're about to lift yourself up when you feel " + (isFirstTime ? "an unusual" : "a familiar") + " sensation inside your belly.  All that cum is hardening!  You can feel it set, almost like mortar left out in the sun!  You stand and some clear water drains out, your belly shrinking in size to that of a watermelon.  Whatever's left inside your belly is solid, like you're carrying a smooth ball of wood inside you.\n\nYou run a hand over your belly.  Thankfully it isn't any heavier than a normal pregnancy, nor does it hurt at all.\n\n<i>“Yes!”</i> shouts the " + this.phoukaName() + ". <i>“I'm gonna be a daddy!  Thanks fer letting me fill ya up girl, there’s plenty o’ space in there fer the little guy.  I gotta tell all the other phouka about ya");
                if (this.flags[kFLAGS_1.kFLAGS.PHOUKA_LORE] == 0)
                    this.flags[kFLAGS_1.kFLAGS.PHOUKA_LORE] = 1; //Now you know what to call them
                if (!postCombat || (this.player.cor > 50))
                    this.outputText(", yer a real prize slut");
                this.outputText(".”</i> The phouka gains more altitude and flies off deeper into the bog, whistling happily to himself.  ");
                if (isFirstTime)
                    this.outputText("You have to wonder how long this pregnancy will last. If this were a human child people would judge you to be at least four months along.");
                else
                    this.outputText("At least you know what to expect from this form of pregnancy now.");
            }
            else
                this.outputText("When you pull yourself up your belly begins to deflate and leak its gooey grey payload into the mud.  The " + this.phoukaName() + " says <i>“Ah well, there's always next time,”</i> and buzzes away to some other part of the bog.");
            this.phoukaSexPregnateEnd(postCombat);
        }
        phoukaSexPregnateEnd(postCombat) {
            this.player.orgasm();
            this.dynStats("cor", PhoukaScene.rand(1) + (postCombat && (PhoukaScene.phoukaForm != PhoukaScene.PHOUKA_FORM_FAERIE) ? 1 : 3)); //Extra two corruption for being enough of a pervert to want to fuck the phouka
            if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_FAERIE) { //In this case postCombat means you need an award because you must have won to get faerie sex
                ...
                if (this.player.cor > 50)
                    this.outputText("  But either way you plan to return and give all of them that lesson.");
                if (postCombat) {
                    kGAMECLASS_1.kGAMECLASS.clearStatuses(false);
                    kGAMECLASS_1.kGAMECLASS.awardPlayer(); //This will provide loot and return to camp, 1 hour used
                }
                ...
                if (postCombat && this.player.gems > 0)
                    this.outputText("  While you're recovering the " + this.phoukaName() + " reaches into your gem pouch and takes a handful.");
                ...
                this.outputText("\n\nYou ");
                if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_HORSE && !this.player.isTaur())
                    this.outputText("wait for the throbbing pain in your pelvis to subside.  Then you ");
                if (postCombat) {
                    this.outputText("pull your [if (isNaga = true)tail][if (isNaga = false)legs] free from the cool muck before you get chilled to the bone.");
                    this.cleanupAfterCombat();
                }
                else {
                    this.outputText("collect your clothes and begin the long march out of the bog.  ");
                    this.doNext(this.camp.returnToCampUseOneHour); //Return to camp, 1 hour used
                }
                if (this.player.cor <= 50)
                    this.outputText("As you trudge back to camp you have to wonder - why did you decide to visit the bog again?");
                else if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_HORSE && this.player.isTaur())
                    this.outputText("  It was quite the ride and you find yourself looking forward to your next trip into the bog.");
                else
                    this.outputText("Not what you expected, but at least you got off.");
            }
        }
    }
    ...
    exports.PhoukaScene = PhoukaScene;
});
//# sourceMappingURL=PhoukaScene.js.map