define(["require", "exports", "../../Monster", "../../StatusAffects", "../../PerkLib", "../../../console", "../../GlobalFlags/kFLAGS", "../../../../includes/appearanceDefs", "../../Appearance", "../../internals/ChainedDrop"], function (require, exports, Monster_1, StatusAffects_1, PerkLib_1, console_1, kFLAGS_1, appearanceDefs_1, Appearance_1, ChainedDrop_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class Hel extends Monster_1.Monster {
        helAttack() {
            var damage;
            //return to combat menu when finished
            this.doNext(this.game.playerMenu);
            //Blind dodge change
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.Blind) >= 0 && Hel.rand(3) < 1) {
            ...
            //Determine if evaded
            else if (this.player.findPerk(PerkLib_1.PerkLib.Evade) >= 0 && Hel.rand(100) < 10) {
                this.outputText("Using your skills at evading attacks, you anticipate and sidestep " + this.a + this.short + "'s attack.\n", false);
            }
            //("Misdirection"
            else if (this.player.findPerk(PerkLib_1.PerkLib.Misdirection) >= 0 && Hel.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
                this.outputText("Using Raphael's teachings, you anticipate and sidestep " + this.a + this.short + "' attacks.\n", false);
            }
            //Determine if cat'ed
            else if (this.player.findPerk(PerkLib_1.PerkLib.Flexibility) >= 0 && Hel.rand(100) < 6) {
                this.outputText("With your incredible flexibility, you squeeze out of the way of " + this.a + this.short + "", false);
            }
            //Determine damage - str modified by enemy toughness!
            else {
                damage = Math.floor((this.str + this.weaponAttack) - Hel.rand(this.player.tou / 2) - this.player.armorDef / 2);
                if (damage > 0)
                ...
                //Take Damage
                    ...
                    this.outputText("The salamander lunges at you, sword swinging in a high, savage arc.  You attempt to duck her attack, but she suddenly spins about mid-swing, bringing the sword around on a completely different path.  It bites deep into your flesh, sending you stumbling back. (" + damage + ")", false);
                if (damage > 0) {
        ...
        helAttack2() {
            ...
            //Determine if dodged!
            if (this.player.spe - this.spe > 0 && Math.floor(Math.random() * (((this.player.spe - this.spe) / 4) + 80)) > 83) {
                this.outputText("The salamander rushes at you, knocking aside your defensive feint and trying to close the distance between you.  She lashes out at your feet with her tail, and you're only just able to dodge the surprise attack.", false);
                return;
            }
            //Determine if evaded
            if (this.player.findPerk(PerkLib_1.PerkLib.Evade) >= 0 && Hel.rand(100) < 5) {
                this.outputText("Using your skills at evading attacks, you anticipate and sidestep " + this.a + this.short + "'s tail-swipe.\n", false);
                return;
            }
            //("Misdirection"
            if (this.player.findPerk(PerkLib_1.PerkLib.Misdirection) >= 0 && Hel.rand(100) < 5 && this.player.armorName == "red, high-society bodysuit") {
                this.outputText("Using Raphael's teachings, you anticipate and sidestep " + this.a + this.short + "' tail-swipe.\n", false);
                return;
            }
            //Determine if cat'ed
            if (this.player.findPerk(PerkLib_1.PerkLib.Flexibility) >= 0 && Hel.rand(100) < 3) {
                this.outputText("With your incredible flexibility, you squeeze out of the way of a tail-swipe!", false);
                return;
            }
            //Determine damage - str modified by enemy toughness!
            ...
            if (damage <= 0) {
                ...
                //Due to toughness or amor...
                if (Hel.rand(this.player.armorDef + this.player.tou) < this.player.armorDef)
                    this.outputText("The salamander's tail-swipe harmlessly deflects off your armor!", false);
                else
                    this.outputText("The salamander's tail-swipe hits you but fails to move or damage you.", false);
            }
            //Take Damage
            else
                this.outputText("The salamander rushes at you, knocking aside your defensive feint and sliding in past your guard.  She lashes out at your feet with her tail, and you can feel the heated wake of the fiery appendage on your ensuing fall toward the now-smouldering grass. (" + damage + ")", false);
            if (damage > 0) {
                if (this.lustVuln > 0 && this.player.armorName == "barely-decent bondage straps") {
                    this.outputText("\n" + this.capitalA + this.short + " brushes against your exposed skin and jerks back in surprise, coloring slightly from seeing so much of you revealed.", false);
                    this.lust += 5 * this.lustVuln;
        ...
        performCombatAction() {
            console_1.trace("Hel Perform Combat Action Called");
            var select = Hel.rand(3);
            console_1.trace("Selected: " + select);
            switch (select) {
                case 0:
                    this.helAttack();
                    break;
        ...
        defeated(hpVictory) {
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.Sparring) >= 0)
                this.game.helFollower.PCBeatsUpSalamanderSparring();
            else
                this.game.helScene.beatUpHel();
        }
        won(hpVictory, pcCameWorms) {
            if (pcCameWorms) {
                ...
                if (this.findStatusAffect(StatusAffects_1.StatusAffects.Sparring) >= 0)
                    ...
                    this.game.helScene.loseToSalamander();
            }
        }
        constructor() {
            super();
            if (this.game.flags[kFLAGS_1.kFLAGS.HEL_TALKED_ABOUT_HER] == 1) {
                this.a = "";
                this.short = "Hel";
            }
            else {
            ...
            this.createVagina(true, appearanceDefs_1.VAGINA_WETNESS_NORMAL, appearanceDefs_1.VAGINA_LOOSENESS_NORMAL);
