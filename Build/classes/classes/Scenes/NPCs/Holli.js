define(["require", "exports", "../../Monster", "../../StatusAffects", "../../../../includes/appearanceDefs", "../../CockTypesEnum", "../../Appearance"], function (require, exports, Monster_1, StatusAffects_1, appearanceDefs_1, CockTypesEnum_1, Appearance_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class Holli extends Monster_1.Monster {
        /*Fight -Z
        ...
        escapeFailWithHolli() {
            ...
            if (this.player.canFly())
                this.outputText("beat your wings and ");
            this.outputText("try to escape, but " + this.short + " wraps one of her writhing roots around your [leg], slamming you to the ground and tying you up with several more!  \"<i>And just where do you think you're going, my little meat?</i>\" she hisses.  Her bark splits open, exposing her body, and a green shaft snakes out of her crotch, sprouting thorns and blooming into a rose at the tip.  She holds the drooling blossom over your [face] as she forces your mouth open with her roots!");
            //hp loss, begin lust constrict next round
            var damage = 15;
            damage = this.player.takeDamage(damage);
            this.outputText(" (" + damage + ")\n\n");
            this.player.createStatusAffect(StatusAffects_1.StatusAffects.HolliConstrict, 0, 0, 0, 0);
            this.combatRoundOver();
        }
        //End of Round, if no Jojo Fire -Z
        holliBonusHealing() {
            //(monster hp < 100%)
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.HolliBurning) < 0) {
            ...
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.JojoIsAssisting) >= 0) {
                if (this.findStatusAffect(StatusAffects_1.StatusAffects.HolliBurning) >= 0) {
                ...
                else if (this.findStatusAffect(StatusAffects_1.StatusAffects.JojoPyre) < 0) {
                    ...
                    this.createStatusAffect(StatusAffects_1.StatusAffects.JojoPyre, 0, 0, 0, 0);
                }
                //End of Round, Rounds 2 and 3 with Jojo Helping - light a spark
                else if (this.statusAffectv1(StatusAffects_1.StatusAffects.JojoPyre) <= 1) {
                    //display applicable EOR outputs for fire not lit, then these
                    //Round 2:
                    if (this.statusAffectv1(StatusAffects_1.StatusAffects.JojoPyre) == 0) {
                        ...
                        this.addStatusValue(StatusAffects_1.StatusAffects.JojoPyre, 1, 1);
                    }
                    //Round 3:
                    else {
                        ...
                        this.createStatusAffect(StatusAffects_1.StatusAffects.HolliBurning, 0, 0, 0, 0);
                    }
                }
            }
            this.combatRoundOver();
        ...
        lightHolliOnFireMagically() {
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.JojoIsAssisting) >= 0) {
                if (this.findStatusAffect(StatusAffects_1.StatusAffects.HolliBurning) < 0) {
                    this.outputText("The magical fire effectively ignites a wide swath of Jojo's tinder, and the demon howls in rage.  Seeing this, Jojo drops the burning torch he carries and turns back toward the forest to fetch more tinder.\n\n");
                    this.createStatusAffect(StatusAffects_1.StatusAffects.HolliBurning, 0, 0, 0, 0);
                }
            }
        }
        ...
        fuckinJamanjiFlowerDarts() {
            this.outputText("A blossom opens up on a high branch of the tree, revealing an evil-looking eye surrounded by vicious spines.  With a gesture, " + this.short + " fires several at you!");
            //Blinded - no hit penalty
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.Blind) >= 0)
                this.outputText("  Though the demon herself is blinded, the fresh eye on the flower seems more than capable of aiming for her!");
            if (this.combatMiss() || this.combatEvade() || this.combatFlexibility() || this.combatMisdirect()) {
                this.outputText("  Nimbly, you step aside and let the darts whistle by.");
            }
            //Hit
            else {
                this.outputText("  The darts find flesh, and you feel yourself slowing down drastically; all you want to do as the plant woman's poison takes you is fuck and sleep.  \"<i>Just give up,</i>\" Holli coos.  \"<i>Think how good it would be to fall into my arms and ");
                if (this.player.hasCock())
                    this.outputText("come inside me");
                else
                    this.outputText("have me inside you");
                this.outputText(", forever...</i>\"");
                //lust damage, fatigue damage, light HP damage
                this.game.fatigue(10);
                this.game.dynStats("lus", 25);
                var damage = 20 + Holli.rand(10);
                damage = this.player.takeDamage(damage);
                this.outputText(" (" + damage + ")");
            }
            this.combatRoundOver();
        ...
        //constrict -Z
        holliConstrictAttack() {
            this.outputText("A forest of thick roots bursts from the ground and several lash toward your [legs], trying to ensnare you!");
            //Blinded - hit penalty, but not 100%
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.Blind) >= 0 && Holli.rand(6) == 0) {
                this.outputText("  Luckily, the demon's blindness makes it fairly easy to dodge the grasping roots, though there are a few close scrapes.");
            }
            //Miss
            if (this.combatMiss() || this.combatEvade() || this.combatFlexibility() || this.combatMisdirect()) {
                this.outputText("  It's a narrow thing, but you manage to avoid the roots - one of them almost grabs you, but you duck aside and let it find only its neighbor.");
            }
            //Hit
            else {
                this.outputText("  She latches onto you with a painful smack and several more root tentacles join the first; as she pulls you close, her bark opens and a long, phallic stalk extends from her crotch, wrapped in thorns and flowering with a rose!  It caresses your face, then dangles the blossom above your mouth, dripping her sap.  Several of the roots pry your jaws apart, forcing you to drink the tainted fluids from her pseudo-cock!  \"<i>What do you think of my little sap rose?</i>\"");
                //plus med HP damage on turn one, plus med-heavy lust damage every turn while constricted
                //sap rose shitposting
                var damage = 10 + Holli.rand(5);
                damage = this.player.takeDamage(damage);
        ...
        struggleOutOfHolli() {
            ...
            this.player.addStatusValue(StatusAffects_1.StatusAffects.HolliConstrict, 1, 9);
            //Struggle Succeed
            //if demon/dragon tongue, automatic success
            if (this.player.tongueType > appearanceDefs_1.TONUGE_HUMAN) {
                this.outputText("You can't move an arm nor a [leg] to bat the flower away... but she's literally holding your mouth open.  Your long tongue rolls out, gripping and ripping out several of the petals on the end of her stalk!  Holli screams and her roots slacken, allowing you to batter your way out of them.");
                this.player.removeStatusAffect(StatusAffects_1.StatusAffects.HolliConstrict);
            }
            //else if normal str-based success
            else if (this.player.str / 10 + Holli.rand(20) + 1 + this.player.statusAffectv1(StatusAffects_1.StatusAffects.HolliConstrict) > 30) {
                this.outputText("You manage to force the roots open when the distracted Holli begins to stroke her plant-shaft, pulling out of the bindings just as a drop of sap oozes out and falls where you were standing.  You're free!");
                //sap rose pls go
                this.player.removeStatusAffect(StatusAffects_1.StatusAffects.HolliConstrict);
            }
            //Struggle Fail/Wait
            else {
                this.outputText("You try to escape the entangling roots, but cannot break their grip!  ");
                this.waitForHolliConstrict(false);
                return;
            }
            this.combatRoundOver();
        }
        waitForHolliConstrict(newScreen = true) {
            if (newScreen)
                this.clearOutput();
            this.outputText("The ominous roseate shaft hovers over you, and its owner strokes the base lewdly, moaning.  \"<i>Oooh, gonna... cum!</i>\" she shrieks.  As a low moan escapes her, the stalk bloats and begins to spill milky-white sap into your mouth!  Held rigid, you're eventually forced to swallow just to breathe; the sap slides into your stomach and warmth radiates to your midsection and groin, making you feel flushed and hot.  Holli sighs in satisfaction, evidently more relaxed after her climax.");
        ...
        teased(lustDelta) {
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.HolliBurning) >= 0) {
                this.outputText("Holli doesn't even seem to notice, so concerned is she with defeating you before the mounting bonfire causes her any more pain.");
                lustDelta = 0;
            }
