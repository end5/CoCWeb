define(["require", "exports", "../../Monster", "../../PerkLib", "../../StatusAffects", "../../../../includes/appearanceDefs", "../../Appearance"], function (require, exports, Monster_1, PerkLib_1, StatusAffects_1, appearanceDefs_1, Appearance_1) {
    ...
    class Kiha extends Monster_1.Monster {
        kihaTimeWaster() {
            this.game.spriteSelect(72);
            this.outputText("She supports the axe on a shoulder, cracking her neck and arching her back to stretch herself, giving you an unintended show.  ", false);
        ...
        sillyModeKihaAttack() {
            this.game.spriteSelect(72);
            this.outputText("Before you can stop to think, the dragon-woman steps back - throwing her axe into the air before she starts sprinting towards you. In seconds she's reached a hair's distance between her lithe form and your own, her fist recoiling and time seemingly stopping to allow you to note the powerful energy seeping from her arms.  ", false);
            //Miss:
            if (this.player.spe - this.spe > 0 && Math.floor(Math.random() * (((this.player.spe - this.spe) / 4) + 80)) > 80) {
                this.outputText("You take the opportunity to walk away, watching the slow-motion attack unravel before you; the fire bursts from her knuckle in the shape of a bird in flight, wings unfurled.  ", false);
                if (Kiha.rand(2) == 0)
                    this.outputText("You only owned an XJasun back home, so you don't really understand the reference.", false);
                else
                    this.outputText("You stifle a laugh as your memories turn to many an evening spent with your friends in front of your SharkCube console, contesting each other in games of ridiculous, stylized combat.", false);
            }
            else {
                //Determine damage - str modified by enemy toughness!
                var damage = Math.floor((this.str + this.weaponAttack) - Kiha.rand(this.player.tou) - this.player.armorDef);
                damage += 5;
                damage = this.player.takeDamage(damage);
                this.outputText("A torrent of heat bursts from between her fingertips as she thrusts her clenched fist forward, the ball of intense flame writhing and burning with a fury unknown to mankind. With one fell swoop, the combined power of her love, anger, and sorrow pushes you backward, launching you out of the swamp and into Marble's pillowy chest. \"<i>Ara ara,</i>\" she begins, but you've already pushed yourself away from the milky hell-prison as you run back towards ");
                if (!this.game.kihaFollower.followerKiha())
                    this.outputText("the swamp");
                else
                    this.outputText("the fight");
                this.outputText(". (" + damage + ")\n", false);
                if (this.player.HP >= 1)
                    this.outputText("You follow the shrill cry of \"<i>B-BAKA!</i>\" in the distance until you reach the exact location you were in a few seconds earlier, prepared to fight again.", false);
            }
            this.combatRoundOver();
        }
        kihaFirePunch() {
            this.game.spriteSelect(72);
            this.outputText("The draconic girl throws her trusty weapon into the sodden ground, using the distraction to build up balls of flame around her fists.  She runs towards you, launching herself in your direction with a flurry of punches.\n", false);
            //Dodged
            if (this.player.spe - this.spe > 0 && Math.floor(Math.random() * (((this.player.spe - this.spe) / 4) + 80)) > 80) {
                this.outputText("You manage to jump to the side, intense heat rushing past you as you narrowly avoid her advance.  You twist around, finding that she's reunited with her axe and angrier than before.", false);
            }
            //Determine if evaded
            else if (this.player.findPerk(PerkLib_1.PerkLib.Evade) >= 0 && Kiha.rand(100) < 10) {
                this.outputText("Using your skills at evasion, you manage to jump to the side, intense heat rushing past you as you narrowly avoid her advance.  You twist around, finding that she's reunited with her axe and angrier than before.", false);
            }
            //("Misdirection"
            else if (this.player.findPerk(PerkLib_1.PerkLib.Misdirection) >= 0 && Kiha.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
                this.outputText("Using your skills at misdirection, you manage to make Kiha think you're going to dodge one way before stepping in the other direction.  You turn back, finding she has her axe in hand and looks rather steamed.", false);
            }
            //Determine if cat'ed
            else if (this.player.findPerk(PerkLib_1.PerkLib.Flexibility) >= 0 && Kiha.rand(100) < 6) {
                this.outputText("Using your cat-like reflexes, you manage to jump to the side, intense heat rushing past you as you narrowly avoid her advance.  You twist around, finding that she's reunited with her axe and angrier than before.", false);
            }
            //HIT!
            else {
                var damage = Math.floor((this.str) - (this.player.armorDef));
                damage = this.player.takeDamage(damage);
                this.outputText("Before you can react, you're struck by the power of her blows, feeling an intense pain in your chest as each fist makes contact.  With a final thrust, you're pushed backwards onto the ground; the dragoness smiles as she pulls her axe out of the ground, her hands still steaming from the fingertips. (" + damage + ")\n", false);
            }
            this.combatRoundOver();
        }
        //Fire breath
        kihaFireBreath() {
            this.game.spriteSelect(72);
            this.outputText("Kiha throws her arms back and roars, exhaling a swirling tornado of fire directly at you!\n", false);
            //Miss:
            //Determine if evaded
            if (this.player.findPerk(PerkLib_1.PerkLib.Evade) >= 0 && Kiha.rand(100) < 10) {
                this.outputText("Using your talent for evasion, you manage to sidestep the flames in the nick of time; much to the dragoness' displeasure.", false);
            }
            //("Misdirection"
            else if (this.player.findPerk(PerkLib_1.PerkLib.Misdirection) >= 0 && Kiha.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
                this.outputText("Using your talent for misdirection, you manage to sidestep the flames in the nick of time; much to the dragoness' displeasure.", false);
            }
            //Determine if cat'ed
            else if (this.player.findPerk(PerkLib_1.PerkLib.Flexibility) >= 0 && Kiha.rand(100) < 6) {
                this.outputText("Using your cat-like flexibility, you manage to sidestep the flames in the nick of time; much to the dragoness' displeasure.", false);
            }
            else {
                var damage = Math.round(90 + Kiha.rand(10));
                damage = this.player.takeDamage(damage);
                this.outputText("You try to avoid the flames, but you're too slow!  The inferno slams into you, setting you alight!  You drop and roll on the ground, putting out the fires as fast as you can.  As soon as the flames are out, you climb back up, smelling of smoke and soot. (" + damage + ")\n", false);
            }
            this.combatRoundOver();
        }
        /*
        ...
        */
        handleFear() {
            ...
            this.outputText("Kiha shudders for a moment, then looks your way with a clear head.  \"<i>Fear was the first thing the demons taught us to overcome.  Do you think it would stay my blade?</i>\"\n", false);
            return true;
        }
        handleBlind() {
            return true;
        ...
        defeated(hpVictory) {
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.spiderfight) >= 0)
                this.game.kihaFollower.playerBeatsUpKihaPreSpiderFight();
            else if (this.findStatusAffect(StatusAffects_1.StatusAffects.DomFight) >= 0)
                this.game.kihaFollower.pcWinsDomFight();
            else if (this.findStatusAffect(StatusAffects_1.StatusAffects.Spar) >= 0)
                this.game.kihaFollower.winSparWithKiha();
            else
                this.game.kihaScene.kihaVictoryIntroduction();
        }
        ...
        constructor() {
            ...
            this.ass.analLooseness = appearanceDefs_1.ANAL_LOOSENESS_LOOSE;
