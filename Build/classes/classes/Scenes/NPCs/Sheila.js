define(["require", "exports", "../../Monster", "../../StatusAffects", "../../PerkLib", "../../GlobalFlags/kFLAGS", "../../../../includes/appearanceDefs", "../../internals/WeightedDrop", "../../internals/ChainedDrop", "../../GlobalFlags/kGAMECLASS"], function (require, exports, Monster_1, StatusAffects_1, PerkLib_1, kFLAGS_1, appearanceDefs_1, WeightedDrop_1, ChainedDrop_1, kGAMECLASS_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class Sheila extends Monster_1.Monster {
        /*
        ...
        sheilaFrogPunch() {
            ...
            if (this.player.tallness < 42 && Sheila.rand(2) == 0) {
                this.outputText("Sheila bounces up to you and crouches low, curling her body like a watchspring.  She uncoils with her fist aimed at your jaw, but you easily perform a crouch of your own and duck under her lanky form, unbending yourself to push her legs up as she flies harmlessly overhead.  You can hear a partial shriek before she crashes face-first into the dirt behind you.");
                damage = 3 + Sheila.rand(10);
                damage = kGAMECLASS_1.kGAMECLASS.doDamage(damage);
                this.outputText(" (" + damage + ")");
            }
            //Miss:
            else if (this.combatMiss() || this.combatFlexibility() || this.combatEvade() || this.combatMisdirect() || this.findStatusAffect(StatusAffects_1.StatusAffects.Blind) >= 0) {
                this.outputText("Sheila bounces up to you and crouches low, curling up her body like a watchspring.  The girl uncoils with fist raised, but you lean away from the uppercut, catching a faceful of her breasts instead!  Sheila squeals and pushes away from you");
                //[(libido>40)
                if (this.player.lib > 40) {
                    this.outputText(", though not before you have a chance to stick your tongue in her cleavage!");
                }
                else
                    this.outputText(".");
                this.outputText("  Blushing pinkly, she crosses her arms over her chest as she resumes her distance.");
                //(+med-small lib-based lust damage to PC)
                kGAMECLASS_1.kGAMECLASS.dynStats("lus", 10 + Sheila.rand(this.player.sens / 10));
            }
            //Hit:
            else {
                this.outputText("Sheila bounces up to you and crouches low, curling up her body like a watchspring.  The girl uncoils just as quickly, launching herself at your face with a fist raised in front of her.  She lands a staggering crack on your jaw which knocks your head back and blurs your vision!");
                ...
                kGAMECLASS_1.kGAMECLASS.fatigue(5 + Sheila.rand(5));
                if (Sheila.rand(2) == 0 && this.player.findPerk(PerkLib_1.PerkLib.Resolute) < 0) {
                    this.player.createStatusAffect(StatusAffects_1.StatusAffects.Stunned, 1, 0, 0, 0);
                    this.outputText("  <b>You are stunned!</b>");
                }
                damage = Math.floor((this.str + this.weaponAttack) - Sheila.rand(this.player.tou) - this.player.armorDef);
                if (damage < 1)
                    damage = 2;
        ...
        sheilaFlyingKick() {
            ...
            //Hit:
                ...
                damage = Math.floor((this.str + 50 + this.weaponAttack) - Sheila.rand(this.player.tou) - this.player.armorDef);
                if (damage < 1)
                    damage = 2;
                damage = this.player.takeDamage(damage);
                this.outputText(" (" + damage + ")");
                kGAMECLASS_1.kGAMECLASS.fatigue(10 + Sheila.rand(6));
            }
            this.spe += 60;
            this.combatRoundOver();
        }
        ...
        suspiciousGlint() {
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.Blind) >= 0 && Sheila.rand(2) == 0) {
                this.outputText("Sheila's blind eyes glint suspiciously as she focuses her power, trying to send her fantasy to anything caught in their stare.  It seems to work - the rock next to you vibrates a little.");
            }
            //Miss:
            else if (this.player.inte / 15 + Sheila.rand(20) + 1 > 16) {
                this.outputText("Sheila's eyes glint suspiciously as she proclaims her affection for you and begs you to look into them, but you keep your head down and focus on her feet.  You can feel her stare boring holes into you, but eventually she abandons the attempt.");
            }
            //Hit:
            else {
                this.outputText("Sheila's eyes glint suspiciously, and you feel your mind slowing down and your body heating up as you meet her lascivious gaze.  Too late you look away, but the damage is done; her fantasies of ");
                if (!this.player.hasCock())
                    this.outputText("burying her drooling tail inside you until it squirts");
                else
                    this.outputText("riding your dick to the hilt");
                this.outputText(" run rampant inside your head and crowd out everything else.  \"<i>Did you see it, [name]?  My love for you?</i>\" Sheila asks, smiling.  God, did you ever!  You can hardly focus on anything!");
                ...
                if (this.findStatusAffect(StatusAffects_1.StatusAffects.TwuWuv) < 0) {
                    this.createStatusAffect(StatusAffects_1.StatusAffects.TwuWuv, 0, 0, 0, 0);
                    var counter = 40 + Sheila.rand(5);
                    ...
                    while (counter > 0) {
                        if (this.player.inte >= 2) {
                            this.player.inte--;
                            this.addStatusValue(StatusAffects_1.StatusAffects.TwuWuv, 1, 1);
                        }
                        counter--;
                    }
                }
        ...
        //2: Tittymonster
        tittyMonsterAttack() {
            this.outputText("Sheila giggles and strokes her " + this.game.sheilaScene.sheilaCup() + " breasts, trying to entice you.");
            //results, no new pg
            //[(sheila corruption < 20; 'miss')
            if (this.game.sheilaScene.sheilaCorruption() < 20)
                this.outputText("  But with nothing there for her to work with, it's a lot like being teased by a dressmaker's mannequin.");
            ...
            //(else; 'miss')
                ...
                this.outputText("  She has trouble even budging tits so comically mismatched to her slender frame; her hands just sink into the voluminous flesh when she tries to squeeze them together, but the demon doesn't manage to move mountains.  It's like watching someone try to push half-inflated swimming equipment around.  You actually laugh a little as she gives up, rubbing her lower back with a gripe.");
            }
            this.combatRoundOver();
        }
        //3: Splash (spd-based hit chance)
        splashAttackLookOutShellEvolveIntoGyrados() {
            this.outputText("Sheila waits patiently, staring at you and stroking her dark, spaded tail with its opposite.  A line of the always-oozing oil falls from the slit, pooling in the smooth brown coil; she unwinds it rapidly, flinging the liquid at your face playfully.  ");
            //results, no new PG
            //Hit:
            if (!this.combatMiss() && !this.combatEvade() && !this.combatMisdirect() && !this.combatFlexibility()) {
                this.outputText("It lands on target, and you're forced to close your eyes lest it get in them!");
                this.player.createStatusAffect(StatusAffects_1.StatusAffects.Blind, 1, 0, 0, 0);
                this.player.createStatusAffect(StatusAffects_1.StatusAffects.SheilaOil, 0, 0, 0, 0);
            }
            else {
                this.outputText("You easily lean away from the path of her tainted fluids, and she sighs.  \"<i>You're no fun, mate.</i>\"");
            }
            this.combatRoundOver();
        }
        //4: Sit 'n Pout
        ...
        lickEmAndStickEm() {
            this.outputText("Sheila's voice gets closer, becoming disarmingly apologetic as you scrub furiously at your face in darkness.  \"<i>Oh, my.  I didn't mean to get that in your eyes... let me help clean you up, love.</i>\"  Your face is gently gripped between her hands and pulled down, then the demon begins passing her tongue over you affectionately, wiping the fluid away with long, ticklish licks as you wait for the other shoe to fall.");
            this.outputText("\"<i>All better,</i>\" Sheila announces.  With her thumb, she gingerly pushes one eyelid up before you can pull away, proving her claim - and causing you to look right into her own glowing, purple iris.  A fantasy invades your mind, one where ");
            if (this.player.hasCock())
                this.outputText("[oneCock] fucks Sheila to the base while her tail snakes around and penetrates your [vagOrAss]");
            else
                this.outputText("you take Sheila from behind by plunging her spaded tail into your [vagina] as she lies face-down on the ground with her ass pushed in the air");
            this.outputText(".");
            //results, no new pg
            //[(int check passed)
            //Miss:
        ...
        demonSheilaAI() {
            ...
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.Counter) < 0)
                this.createStatusAffect(StatusAffects_1.StatusAffects.Counter, 0, 0, 0, 0);
            this.addStatusValue(StatusAffects_1.StatusAffects.Counter, 1, 1);
            if (this.statusAffectv1(StatusAffects_1.StatusAffects.Counter) >= 5) {
                this.sitAndPout();
                return;
            }
            var choices = [];
            if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.SheilaOil) < 0) {
        ...
        performCombatAction() {
            if (this.game.flags[kFLAGS_1.kFLAGS.SHEILA_DEMON] == 1) {
                this.demonSheilaAI();
                return;
            }
            if (Sheila.rand(3) == 0)
                this.eAttack();
            else if (Sheila.rand(2) == 0)
        ...
        defeated(hpVictory) {
            if (this.game.flags[kFLAGS_1.kFLAGS.SHEILA_DEMON] == 1)
                this.game.sheilaScene.beatUpDemonSheila();
            else
                this.game.sheilaScene.sheilaGotWhomped();
        }
        won(hpVictory, pcCameWorms) {
            if (this.game.flags[kFLAGS_1.kFLAGS.SHEILA_DEMON] == 1)
                this.game.sheilaScene.loseToSheila();
            else
                this.game.sheilaScene.getBeatUpBySheila();
        }
        constructor() {
            super();
            var sheilaDemon = this.game.flags[kFLAGS_1.kFLAGS.SHEILA_DEMON] == 1;
            this.a = "";
            this.short = "Sheila";
            this.imageName = "sheila";
            if (sheilaDemon) {
                this.long = "Sheila is a slim, somewhat athletic woman, over six feet in height.  Her smooth, dark skin is exposed from her head to her clawed feet, and she makes no effort to conceal anything your eyes might linger on.  The " + this.game.sheilaScene.sheilaCup() + " breasts on her chest" + (this.game.sheilaScene.sheilaCorruption() <= 40 ? " are firm, squeezable teardrops; she runs a hand absently over one from time to time." : " jiggle as she moves, and she shoves them out to make sure you see just how lewd her body has become since your first meeting.") + "  Straight, jaw-length auburn hair frames her face along with two long, smooth ears that stick out sideways.  Her only nods to civilization are a dangling purple earring and the finger rings that she wears on her hands, and the wild woman stares openly at you, touching herself.";
            }
            else {
                this.long = "Sheila is a slim, somewhat athletic woman, over six feet in height.  Most of her lightly-tanned skin is hidden, either by her vest and shorts or by the fuzzy fur that covers her legs from the thighs down to her prominent nails.  Her " + this.game.sheilaScene.sheilaCup() + " breasts are briefly defined against the white of her shirt as she sways on her feet, " + (this.game.sheilaScene.sheilaCorruption() <= 40 ? "small, round things that match her slender frame." : "swollen, jiggling globes that stand in contrast to her slender body and tell a tale of all the corruption that has been pumped into her.") + "  Her straight, jaw-length auburn hair hangs unrestrained, falling around the fuzzy ears that stick out sideways from her head.  The hat she usually wears is hanging on her back by a string, pushed off to prevent its being lost in the chaos.  Something about slipping a rope around her own neck just to keep a hat tells you that Sheila's mind isn't really staying in the fight - though it could also be the desperate, faraway look in her eyes.";
            }
            // this.plural = false;
            this.createVagina(this.game.flags[kFLAGS_1.kFLAGS.SHEILA_XP] <= 3 && !sheilaDemon, appearanceDefs_1.VAGINA_WETNESS_SLICK, appearanceDefs_1.VAGINA_LOOSENESS_NORMAL);
            this.createStatusAffect(StatusAffects_1.StatusAffects.BonusVCapacity, 30, 0, 0, 0);
            this.createBreastRow(this.game.flags[kFLAGS_1.kFLAGS.SHEILA_CORRUPTION] / 10);
            ...
            if (this.game.flags[kFLAGS_1.kFLAGS.SHEILA_DEMON] == 0) {
                this.drop = new WeightedDrop_1.WeightedDrop(this.consumables.KANGAFT, 1);
            }
            else {
                this.drop = new ChainedDrop_1.ChainedDrop(this.consumables.KANGAFT).
                    add(this.consumables.SUCMILK, 1 / 3).
                    add(this.consumables.INCUBID, 1 / 2);
            }
            this.tailType = appearanceDefs_1.TAIL_TYPE_KANGAROO;
            this.checkMonster();
        }
    }
    exports.Sheila = Sheila;
});
//# sourceMappingURL=Sheila.js.map