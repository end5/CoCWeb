define(["require", "exports", "../../../Monster", "../../../../../includes/appearanceDefs", "../../../StatusAffects", "../../../GlobalFlags/kFLAGS", "../../../PerkLib"], function (require, exports, Monster_1, appearanceDefs_1, StatusAffects_1, kFLAGS_1, PerkLib_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class Lethice extends Monster_1.Monster {
        constructor() {
            super();
            this._roundCount = 0;
            this._fightPhase = 1;
            this._wingsDestroyed = false;
            this._defMode = 0; // 0- none, 1- hp, 2- lust
            this.a = "";
            this.short = "Lethice";
            // this.long = "";
            this.tallness = 12 * 9;
        ...
        get long() {
            ...
            if (this._fightPhase == 1) {
                ...
                else if (this.findStatusAffect(StatusAffects_1.StatusAffects.OnFire) >= 0) {
            ...
            if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.LethicesRapeTentacles) >= 0) {
                ...
                if (this.player.statusAffectv3(StatusAffects_1.StatusAffects.LethicesRapeTentacles) != 0)
                    this.outputText(" Unfortunately, they’ve grabbed you. You need to break free to do anything!");
            ...
            if (this.game.monster.findStatusAffect(StatusAffects_1.StatusAffects.Shell) >= 0) {
                str += "\n\n<b>Lethice is surrounded by a shimmering dome of magical energy. Spells and ranged attacks will be ineffective!</b>";
            }
            if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.PigbysHands) >= 0) {
        ...
        get fightPhase() { return this._fightPhase; }
        performCombatAction() {
            this._roundCount++;
        ...
        phase1() {
            var atks = [this.demonicArouse, this.demonfire];
            if (this._roundCount % 10 == 3)
                atks.push(this.rapetacles);
            if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.Blind) < 0)
                atks.push(this.wingbuffet);
            atks[Lethice.rand(atks.length)]();
        }
        demonicArouse() {
            this.outputText("Lethice’s hands blur in a familiar set of arcane motions, similar to the magical gestures you’ve seen from the imps. Hers are a thousand times more intricate. Her slender fingers move with all the precision of a master artist’s brush, wreathed in sparks of black energy.");
            var l = this.player.lib / 10 + this.player.cor / 10 + 25;
            if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.MinotaurKingsTouch) >= 0)
                l *= 1.25;
        ...
        rapetacles() {
            ...
            if (this.player.canFly()) {
                ...
                this.player.createStatusAffect(StatusAffects_1.StatusAffects.LethicesRapeTentacles, 4 + Lethice.rand(2), 0, 1, 0);
            }
        }
        dispellRapetacles() {
            this.clearOutput();
            if (this.player.statusAffectv3(StatusAffects_1.StatusAffects.LethicesRapeTentacles) == 0) {
                this.outputText("You raise your arm and");
            }
            else {
                this.outputText("You struggle and manage to raise your arm against the tight grasp of the tentacles, managing to");
            }
            this.outputText(" spray forth a torrent of white flame, burning the shadowy constructs away in the light of your pure, focused fire. In the span of seconds, Lethice’s spell is gone.");
            this.game.doNext(this.game.combatMenu);
            this.game.fatigue(30, 1);
            this.outputText("\n\n", false);
            this.flags[kFLAGS_1.kFLAGS.SPELLS_CAST]++;
        ...
        demonfire() {
            ...
            this.outputText("Lethice narrows her eyes, focusing her mind with deadly intent. She snaps her fingers and a gout of black, twisting flames engulfs you!");
            var damage = 100 + Lethice.rand(25);
            damage = this.player.takeDamage(damage);
            this.outputText(" (" + damage + ")");
        }
        wingbuffet() {
            this.outputText("Lethice flutters toward a burning brazier and flaps her wings, causing the flames to flare and thick gusts of smoke to flow past the assembled demons, straight at you!");
            if (this.combatMiss() || this.combatFlexibility()) {
                this.outputText(" You manage to slide under the waves of smoke.");
            }
            else {
                this.outputText(" The cloying smoke gets in your eyes and your mouth, making you cough and sputter. Worst of all, you can’t see anything!");
        ...
        claw() {
            this.outputText("Swooping low, the Demonic Queen takes a swipe at you with claws that are suddenly six inches long and as sharp as razors!");
            var damage = this.str + this.weaponAttack - Lethice.rand(this.player.tou);
            if (this.combatMiss()) {
                this.outputText("You manage to dodge her slash!");
            }
            else if (this.combatMisdirect()) {
                this.outputText(" Misdirecting her with your movements, you avoid the swipe.");
            }
            else if (this.combatFlexibility()) {
                this.outputText(" With your feline flexibility, you bend double to avoid the swipe.");
            }
            else if (this.combatEvade()) {
                this.outputText(" You evade her slash with a quick roll.");
            }
            else {
                damage = this.player.takeDamage(damage);
                if (damage <= 5) {
                    this.outputText(" She barely scratches you. She’ll need stronger weapons than that to take you down. (" + damage + ")");
                }
                else {
        ...
        grappleStruggle() {
            this.clearOutput();
            this.outputText("You pull with all your might against the grasping tentacles to no avail; their grip is simply too strong!");
            this.game.enemyAI();
        }
        grappleWait() {
            this.clearOutput();
            this.outputText("You can't bring yourself to fight back against Lethice's tentaclespawn. The sensuous, coiling grasp around your limbs, their questing, pliant tips digging around inside your [armor]... you relax in their grip for a little while longer, too enticed by their movement to struggle right now.");
            this.game.enemyAI();
        }
        phase1Ends(hpVictory) {
            ...
            if (hpVictory) {
                if (this.player.canFly()) {
                    this.outputText("Even held aloft by her sprawling dragon wings, Lethice can’t hide from your righteous wrath.");
                    this.outputText(" Taking wing yourself, you slam into the demoness, striking a final blow that sends her toppling to the ground. She shrieks and spirals, crashing into the hard stone floor just before her throne.");
                }
                else if (this.flags[kFLAGS_1.kFLAGS.LAST_ATTACK_TYPE] == 1) {
                    this.outputText("Even held aloft by her sprawling dragon wings, Lethice can’t hide from your righteous wrath.");
                    this.outputText(" You draw your bowstring and let loose one last arrow, sending the missile hurtling through the air - and right into Lethice’s wing! The Demon Queen lets out an ear-piercing shriek of pain and, with her wing flopping weakly beside her, goes tumbling to the earth! She’s down!");
                }
                else if (this.flags[kFLAGS_1.kFLAGS.LAST_ATTACK_TYPE] == 2) {
                    this.outputText("Unable to resist your arcane assault, Lethice lets loose a howl of frustration and swoops back to the earth, mounting her throne once again.");
                }
            }
            else {
                this.outputText("Unable to resist your sensual assault, Lethice lets loose a howl of frustration and swoops back to the earth, mounting her throne once again.");
            }
            ...
            this.pronoun2 = "them";
            this.pronoun3 = "their";
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.PhysicalDisabled) >= 0)
                this.removeStatusAffect(StatusAffects_1.StatusAffects.PhysicalDisabled);
            if (this.findStatusAffect(StatusAffects_1.StatusAffects.AttackDisabled) >= 0)
                this.removeStatusAffect(StatusAffects_1.StatusAffects.AttackDisabled);
            this.combatRoundOver();
        }
        phase2() {
            var atks = [this.demonLustMagic, this.dirtyDancing, this.hornyPoke, this.crushingBodies];
            if (Lethice.rand(10) == 0 && this.player.findStatusAffect(StatusAffects_1.StatusAffects.Blind) < 0)
        ...
        dirtyDancing() {
            ...
            if (Lethice.rand(100) >= this.player.lib / 2) {
                ...
                var l = this.player.lib / 10 + this.player.cor / 10 + 10;
                if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.MinotaurKingsTouch) >= 0)
                    l *= 1.25;
                this.game.dynStats("lus", l);
            }
        ...
        hornyPoke() {
            this.outputText("Several of the demons nearest you have grown immense sets of curling, sharp horns. When they can’t get at you to sexually provoke or hurl magic at you, they’re more than content to just give you an old-fashioned ram!");
            if (this.combatMiss()) {
                this.outputText(" You deftly dodge out of the way!");
            }
            else if (this.combatMisdirect()) {
                this.outputText(" At least, they try to! Too bad for them you’re already elsewhere in the crowd, well away from harm!");
            }
            else if (this.combatFlexibility()) {
                this.outputText(" You contort and bend in ways a human never could, easily twisting between all the pairs of horns the demons can thrust at you. By the time they’re done, you’re sitting on top of a demon’s head, balanced on his antlers until with a furious howl he throws you back into the sea of maledicts.");
            }
            else if (this.combatEvade()) {
                this.outputText(" You manage to duck down enough to avoid the worst of the horn-spikes, and your [armor] deflects the rest!");
            }
            else {
                var damage = 70 - Lethice.rand(this.player.tou);
                damage = this.player.takeDamage(damage);
                this.outputText(" Several of the big, stout horns slam into you, given all the more force thanks to the crushing waves of demons pushing them forward. You yelp in pain as they score your flesh! (" + damage + ")");
            }
        }
        crushingBodies() {
            this.outputText("The sheer weight of a hundred demonic bodies crushing down on you is enough to make you cry out in discomfort, then pain. Are they just trying to crush you to death!?");
            if (this.combatEvade()) {
                ...
                var damage = 70 - Lethice.rand(this.player.tou);
                damage = this.player.takeDamage(damage);
                this.outputText(" You squirm out of their crushing embrace, trying to crawl away on the ground, but if they can't squeeze you down the demons seem happy to trample you! You scream in alarm and pain as dozens of feet, hooves, claws, and other appendages come crushing down on you! (" + damage + ")");
            }
        }
        bukkakeTime() {
            this.outputText("Considering how half of the demon host is just getting fucked by whoever’s behind them, it’s just a question of how long they last before the cum starts flowing. The answer just happens to be now! You gasp and gag as the air is suddenly flooded by the reek of potent, virile jizz, and ropes of thick white spunk start flying through the air. This is less of a gank mob and more of an orgy now!");
            if (this.combatMiss()) {
                this.outputText(" You grab the biggest incubus you can find and shove him in the way of the airborne bukkake, letting his burly back take the brunt of the demons’ load. He grunts in displeasure, giving you a rough shove back into the demonic gang-bang. <i>“No escape!”</i>");
            }
            else if (this.combatMisdirect()) {
                this.outputText(" You duck under the nearest group of succubi, happily letting the demonic hussies get plastering with the wave of flying spooge. They seem to enjoy it, too, and quickly you’re surrounded by less demonic fighting and much, much more infernal cock-sucking. Seems they’re hungry!");
            }
            else if (this.combatFlexibility()) {
                this.outputText(" You do a graceful backflip out of the way, making sure it’s a group of eager succubi who get painted white with cum rather than you!");
            }
            else if (this.combatEvade()) {
                this.outputText(" You at least manage to close your eyes before the wave of spooge hits you, splattering all over your [armor].");
                this.game.dynStats("lus", 5);
            }
            else {
                this.outputText(" You take a huge, fat, musky glob of spunk right to the eyes! You yelp in alarm, trying to wipe the salty, burning demonic cock-cream out, but it's simply too thick! Yuck!");
                this.game.dynStats("lus", 5);
                this.player.createStatusAffect(StatusAffects_1.StatusAffects.Blind, 2 + Lethice.rand(2), 0, 0, 0);
            }
        }
        phase2Ends(hpVictory) {
            this.clearOutput();
            if (hpVictory) {
                this.outputText("The last of the demons falls to your [weapon], crumpling into an unconscious lump before you. A few are crawling haplessly away, retreating to the stands with the rest of their fellows - those too timid to fight at all.");
            }
            else {
                this.outputText("Lethice’s minions have all but turned into an orgy, completely forgetting their original intent, no matter how much their draconic queen screeches for them to attack.");
            }
        ...
        fuckDemon() {
            this.clearOutput();
            this.outputText("The demons’ urging tears down your self-control, and with a grunt of effort and desire, you shove the omnibus down on her back. She yelps with surprise, but her fall is cushioned by the massive, milky tits of a cow-girl-turned-demon behind her who’s busily sucking a pair of demonic dongs. Your omnibus recovers from her surprise in a heartbeat, turning it into a luxurious stretch that spreads her pussy-lips open under the hefty, churning weight of her equine-like balls.");
            ...
            if (this.player.hasFuckableNipples())
                this.outputText(" until you give her a wicked grin and guide the tip inside your [nipple]");
            else
                this.outputText(" and smearing your [skinFurScales] with pre");
            this.outputText(". You grab her melon-sized tits and squeeze them as hard as you can while you hammer your [hips] forward, taking advantage of the silky-wet vice of her twat to milk yourself to orgasm. She’s practically an ona-hole for you, only able to squirm around and moan while you pound away.");
            ...
            if (this.player.balls > 0)
                this.outputText(" and around your balls");
            else
                this.outputText(" down to the base of your cock");
            this.outputText(". Another demoness snuggles up behind you, pressing a hefty pair of jugs into your back and murmuring about how she wants to see you breed that hermaphroditic cum-slut under you.");
            ...
            if (!this.player.hasFuckableNipples())
                this.outputText(" that splatters all over your chest");
            else
                this.outputText(" right into your tit");
            this.outputText("! You give her a reproachful look, still grinding your hips as the aftershocks of orgasm pass.");
        ...
        rideCock() {
            this.clearOutput();
            this.outputText("Considering the demons seem intent on having you get down and dirty with their defeated compatriot, you figure you might as well get the most out of the moment. Lethice doesn’t seem to be in any hurry to resume your battle, after all!");
            ...
            if (this.player.hasVagina())
                this.outputText(" licking at your [cunt]");
            else
                this.outputText(" reaching around to give your unused [cocks] a tug}. Another demoness snuggles up behind you, pressing a hefty pair of jugs into your back and murmuring about how she wants to see you get bred by that hermaphroditic stud under you.");
            ...
            this.outputText(". You only barely keep your wits about you enough to push her back down before she can force her knot inside you, though you’re treated to a veritable waterfall of white spunk pouring out of your well-bred hole instead.");
        ...
        p2Heal() {
            this.clearOutput();
            this.outputText("Drawing on your magic, you use the opportunity to mend your wounds. No foe dares challenge you during the brief lull in battle, enabling you to maintain perfect concentration. With your flesh freshly knit and ready for battle, you look to Lethice.");
            var temp = Math.floor((this.player.inte / (2 + Lethice.rand(3)) * this.game.spellMod()) * (this.player.maxHP() / 150));
            if (this.player.armorName == "skimpy nurse's outfit")
                temp *= 1.2;
        ...
        beginPhase3(doLethNext) {
            ...
            this.game.menu();
            if (doLethNext)
                this.game.addButton(0, "Next", this.p2Next);
            else
                this.combatRoundOver();
        }
        phase3() {
            ...
            if (this.flags[kFLAGS_1.kFLAGS.LAST_ATTACK_TYPE] == 1 || this.flags[kFLAGS_1.kFLAGS.LAST_ATTACK_TYPE] == 2 || this.flags[kFLAGS_1.kFLAGS.LAST_ATTACK_TYPE] == 4)
                this._defMode = 1;
            else
                this._defMode = 2;
            if (this._defMode == 1) {
                this.outputText("Lethice dabs at her injury, then licks her finger, smiling sensually. She seems excited and invigorated. Scales appear over her skin, and her spaded tail swishes back and forth in a blur, too fast to follow.\n\n");
                this.spe = 400;
                this.armorDef = 400;
                this.lustVuln = 1.125;
            }
            else {
                this.outputText("Shuddering, the Demon Queen steels herself. Her curves soften visibly as she prepares herself for additional arousing efforts, forgoing physical defense.\n\n");
                this.spe = 80;
                this.armorDef = 14;
                this.lustVuln = 0.0;
            }
            if (this._roundCount == 5)
                ...
                var atks = [this.parasiteThrowingStars, this.whiptrip, this.sonicwhip];
                if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.WhipSilence) < 0)
                    atks.push(this.whipchoke);
                atks[Lethice.rand(atks.length)]();
            }
        }
        parasiteThrowingStars() {
            this.outputText("Lethice retrieves three squirming, star-shaped creatures from beneath her clothes and flings them at you. A split second after they leave her hand, needles burst from their edges!");
            if (this.combatMiss()) {
                this.outputText(" The living throwing stars whistle by you, barely missing you.");
            }
            else if (this.combatEvade()) {
                this.outputText(" You barely avoid the living throwing stars.");
            }
            else if (this.combatMisdirect()) {
                this.outputText(" Your misdirecting movements allow you to avoid the living throwing stars.");
            }
            else if (this.combatFlexibility()) {
                ...
                var damage = this.str + this.weaponAttack - Lethice.rand(this.player.tou);
                damage = this.player.takeDamage(damage);
                this.outputText(" You can’t avoid them all! One clips you on its way past, ripping into your [skin] and leaving you feeling... flushed and hot in its wake.");
                if (this.player.hasCock()) {
                    this.outputText(" There’s suddenly a dearth of room in the crotch of your [armor], rubbing distractingly and slowing you with added weight. <b>[EachCock] has grown bigger!</b>");
                    this.player.increaseEachCock(1);
                }
                else if (this.player.biggestTitSize() <= 0) {
                    this.outputText(" The inside of your [armor] is suddenly pressing back on your [nipples]. <b>You’ve grown breasts!</b> That bitch!");
                    this.player.growTits(3, this.player.breastRows.length, false, 2);
                }
                else {
                    this.outputText(" There’s suddenly a dearth of room in the chest of your [armor]. Your [chest] are expanding, bouncing distractingly and slowing you with the extra weight!");
        ...
        whiptrip() {
            ...
            if (this.combatMiss()) {
                ...
                this.outputText(" You evade her trip, but the heat pouring off the whip singes your [feet].");
                minDamage = true;
            }
            else {
                this.outputText("You try to avoid it, but the burning weapon catches your [leg], simultaneously scorching your flesh and attempting to pull you off balance.");
                if (this.player.findPerk(PerkLib_1.PerkLib.Resolute) < 0 && Lethice.rand(this.player.tou) <= 25) {
                    this.outputText(" The ground rushes up at you awful fast. Lethice has tripped you, <b>stunning you!</b>");
                    this.player.createStatusAffect(StatusAffects_1.StatusAffects.Stunned, 1, 0, 0, 0);
                }
                else {
                    this.outputText(" Lethice is going to need to pull a lot harder if she wants to trip you.");
                }
            }
            var damage;
            if (minDamage) {
        ...
        sonicwhip() {
            this.outputText("Lethice raises her sizzling, flame-spitting whip high up overhead, then snaps her arm out and back in an instant, cracking the whip so hard that it gives birth to a shockwave of flame and cacophonous thunder. There’s no avoiding the all-encompassing wave of energy. There’s not even time to brace yourself. It slams into you, rattling bones and scorching flesh.");
            var damage = 75 + this.weaponAttack + this.str;
            damage = this.player.takeDamage(damage);
            this.outputText(" (" + damage + ")");
        }
        whipchoke() {
            this.outputText("<i>“Silence your prattling, curr.”</i> Lethice strikes out with her whip, aimed at your neck!");
            if (this.combatMiss()) {
                this.outputText(" You barely avoid it.");
            }
            else if (this.combatEvade()) {
                this.outputText(" You evade the targeted strike.");
            }
            else if (this.combatFlexibility()) {
                this.outputText(" You twist aside at the last moment.");
            }
            else if (this.combatMisdirect()) {
                this.outputText(" Raphael taught you well. Lethice failed to account for your misleading movements and swung wide.");
            }
            else {
                ...
                var damage = this.weaponAttack + 25 - Lethice.rand(this.player.tou);
                damage = this.player.takeDamage(damage);
                this.player.createStatusAffect(StatusAffects_1.StatusAffects.WhipSilence, 3, 0, 0, 0);
                this.outputText(" (" + damage + ")");
            }
        }
        triplestroke() {
            //Three normal whip attacks
            this.outputText("Lethice’s arm blurs in figure eights, snapping the whip at you from every sides. You’ll have a tough time avoiding so many strikes!");
            for (var i = 0; i < 3; i++) {
        ...
        gropehands() {
            ...
            this.game.dynStats("lus", 5);
            this.player.createStatusAffect(StatusAffects_1.StatusAffects.PigbysHands, 0, 0, 0, 0);
        }
    }
    exports.Lethice = Lethice;
});
//# sourceMappingURL=Lethice.js.map