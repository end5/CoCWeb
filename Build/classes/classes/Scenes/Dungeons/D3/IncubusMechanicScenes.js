define(["require", "exports", "../../../BaseContent", "../../../StatusAffects", "../../../GlobalFlags/kFLAGS", "../Factory/IncubusMechanic", "../../../PerkLib", "../../../CockTypesEnum", "../../../PregnancyStore"], function (require, exports, BaseContent_1, StatusAffects_1, kFLAGS_1, IncubusMechanic_1, PerkLib_1, CockTypesEnum_1, PregnancyStore_1) {
    "use strict";
     ...
     * @author Gedan
     */
    class IncubusMechanicScenes extends BaseContent_1.BaseContent {
        constructor() {
            super(...arguments);
            this.MECHNIC_NO_SELECTION = 0;
            this.MECHANIC_PAID = 1;
            this.MECHANIC_SUCKED = 2;
            this.MECHANIC_FOUGHT = 3;
            this.MECHANIC_KILLED = 1;
            this.MECHANIC_RELEASED = 2;
            this.MECHANIC_FUCKED = 3;
            this.MECHANIC_FUCKED_YOU = 4;
            this.MECHANIC_DEMON_COCK = 0;
            this.MECHANIC_HORZ_GOG = 1;
            this.MECHANIC_DOG_COCK = 2;
        }
        ...
        meetAtElevator() {
            if (this.flags[kFLAGS_1.kFLAGS.D3_MECHANIC_LAST_GREET] == this.MECHNIC_NO_SELECTION) {
                ...
                this.addButton(4, "Fight", this.startCombatImmediate, new IncubusMechanic_1.IncubusMechanic());
            }
            else if (this.flags[kFLAGS_1.kFLAGS.D3_MECHANIC_LAST_GREET] == this.MECHANIC_SUCKED) {
            ...
            else if (this.flags[kFLAGS_1.kFLAGS.D3_MECHANIC_LAST_GREET] == this.MECHANIC_PAID) {
            ...
            else if (this.flags[kFLAGS_1.kFLAGS.D3_MECHANIC_LAST_GREET] == this.MECHANIC_FOUGHT) {
                this.addButton(2, "Lift", this.useLiftPostDefeat);
            }
        }
        payDaToll() {
            this.flags[kFLAGS_1.kFLAGS.D3_MECHANIC_LAST_GREET] = this.MECHANIC_PAID;
            this.clearOutput();
        ...
        useLiftPostDefeat() {
            this.clearOutput();
            this.outputText("You spend a minute or two fiddling with the lift controls, trying to understand how, exactly, to control the contraption. When you think you've got a handle on the system, you tug on a lever with authority and hastily step onto the platform.");
            this.outputText("\n\nThere's a railing to hold onto as you're lowered down to the mountains. While the platform does rock from side to side along the slow trip down, its motions are not erratic enough to put you in any significant danger. Even better, you aren't bothered by a single harpy along the way. This section of the high mountains seems to have been cleansed of them.");
            this.outputText("\n\nThe platform touches down roughly thirty minutes after your departure, and you head back to camp with all due haste.");
            this.addButton(0, "Next", this.getGame().d3.exitD3);
        }
        suckIncubusDick() {
            this.flags[kFLAGS_1.kFLAGS.D3_MECHANIC_LAST_GREET] = this.MECHANIC_SUCKED;
            this.clearOutput();
            if (this.player.gems < 500)
                this.outputText("You spare a glance to your depleted pouches and admit, \"<i>I guess I'll suck your dick then.</i>\"");
            else
                this.outputText("You shrug and smile. \"<i>I guess now is as good a time as any to suck some dick, huh?</i>\"");
            ...
            if (this.player.hasVagina()) {
                this.outputText(" You find your crotch becoming");
                if (this.player.wetness() <= 2)
                    this.outputText(" even more soaked than normal");
                else if (this.player.wetness() <= 4)
                    this.outputText(" wet with fresh lust");
                else
                    this.outputText(" sticky with lust");
                this.outputText(" at his masculine scent and appearance.");
            }
            else if (this.player.hasCock()) {
                this.outputText(" You feel a heady, turgid warmth growing in your own crotch, mirroring the ardor displayed by the cock you're about to service.");
            }
            this.outputText(" A patch of neatly trimmed pubic hairs lead up his abdomen to a set well-defined muscles, half-hidden under his loose not-so-coveralls.");
        ...
        beatDaMechanic(hpVictory) {
            this.flags[kFLAGS_1.kFLAGS.D3_MECHANIC_LAST_GREET] = this.MECHANIC_FOUGHT;
            this.clearOutput();
            this.outputText("The mechanic collapses, his overalls");
            if (hpVictory)
                this.outputText(" falling off his prone from, shredded.");
            else
                this.outputText(" hanging open as he reaches down for his exposed, dangling cock to wildly masturbate.");
            ...
            if (this.player.hasCock())
                this.addButton(2, "Buttfuck", this.buttfuckTheMechanic, hpVictory);
            if (this.player.hasVagina())
                this.addButton(3, "Ride Cock", this.rideMechanicsCock);
        }
        killMechanic() {
            this.flags[kFLAGS_1.kFLAGS.D3_MECHANIC_FIGHT_RESULT] = this.MECHANIC_KILLED;
            this.clearOutput();
        ...
        letMechanicGo(hpVictory) {
            this.flags[kFLAGS_1.kFLAGS.D3_MECHANIC_FIGHT_RESULT] = this.MECHANIC_RELEASED;
            this.clearOutput();
            this.outputText("Feeling merciful, you point off the side of the cliff. <i>\"Leave this place and never return. If you do, I might have to kill you. I'd rather not shed any more blood than necessary, but I will do what I have to in order to cleanse this land. Leave and never return. Maybe you can find your own peace without driving everything around you to corruption.\"</i>");
            this.outputText("\n\nThe incubus");
            if (hpVictory)
                this.outputText(" slowly struggles to his feet");
            else
                this.outputText(" reluctantly stops masturbating");
            this.outputText(", nodding. His skin cracks as two leathery wings unfurl. <i>\"If that's how it has to be.... For what it's worth, I hope you win. Lethice is a bitch.\"</i> Slowly walking towards the cliff, he beats his wings and lifts off the ground. \"<i>Come visit me sometime. Maybe I can show you a good time as thanks.</i>\"");
            this.outputText("\n\nThe incubus mechanic flies away for good, though you can’t can say whether he will wreak havoc or live quietly.");
            this.cleanupAfterCombat(this.getGame().d3.resumeFromFight);
        }
        buttfuckTheMechanic(hpVictory) {
            this.flags[kFLAGS_1.kFLAGS.D3_MECHANIC_FIGHT_RESULT] = this.MECHANIC_FUCKED;
            var x = this.player.cockThatFits(200);
            if (x == -1)
                x = 0;
            this.clearOutput();
            this.outputText("\n\nThe incubus may have done his best to seduce you with his");
            if (!hpVictory)
                this.outputText(" throbbing hard");
            else
                this.outputText(" gently bobbing");
            this.outputText(" dick, but you've got a better idea. You got more than one look at the curvature of his butt while you were fighting, tempting your burgeoning bulge to the point of distraction. You hastily remove your [armor] and smile. [EachCock] springs up, full and hard. You take");
            if (this.player.cocks.length > 1)
                this.outputText(" one");
            else
                this.outputText(" it");
            this.outputText(" in hand, pumping slowly as you eye your tainted prey. He whimpers");
            if (hpVictory)
                this.outputText(" in need, and");
            else
                this.outputText(", but");
            this.outputText(" his cock seems to get even bigger and harder. The poor guy doesn't realize what a reaming he's in for.");
            ...
            //Medium cum exit
                ...
                if (this.player.hasKnot(x))
                    this.outputText(", popping your knot");
                this.outputText(" out of the demon's creampied fuck-hole with a smile. His stretched-out ass looks virtually glazed with cum, and you can see small rivers of it trickling down over his balls and slowly shrinking cock.");
            }
            //High cum exit
                ...
                if (this.player.hasKnot(x))
                    this.outputText(", popping your knot");
                this.outputText(" out of the demon's well-fucked asshole with a smile. As soon as the bulk of your dick unplugs him, a river of white spills out, washing over his balls and cock to drip down into his puddle. You smirk at the white-glazed demon. His butt is still full of your spunk.");
            }
            //Huge cum
            else {
                this.outputText("\n\nYou pull");
                if (this.player.hasKnot(x))
                    this.outputText(", popping your knot");
                this.outputText(" out after depositing a massive, belly-bulging load directly into the demon's guts. A tidal wave of goo bursts out of his over-pressurized asshole to fall on the ground, and as it dies off, small rivers of ivory spunk escape the lake in his ass to run down his balls and cock. His belly is");
                if (this.player.cumQ() <= 5000)
                    this.outputText(" pudgy");
                else if (this.player.cumQ() <= 10000)
                    this.outputText(" swollen");
                else
                    this.outputText(" absolutely gravid");
                this.outputText(" with the distended weight of the cum packing his body. From time to time, fresh streamers of sperm squirt out of his thoroughly cream-pied backdoor.");
            }
            //No new paragraph
            this.outputText("The incubus slumps down into the puddled semen, gasping in exhaustion.");
            this.outputText("\n\nYou wipe your " + this.cockDescript(x) + " off on his practically glowing bottom before turning to re-dress. When you turn back, he's gone, though a trail of white drops indicates the direction of his flight. It looks like he fled towards the cliff, perhaps growing wings. Oh well, that's one less obstacle between you and Lethice.");
            this.player.orgasm();
            this.dynStats("cor+", 5);
            this.cleanupAfterCombat(this.getGame().d3.resumeFromFight);
        }
        rideMechanicsCock() {
            this.flags[kFLAGS_1.kFLAGS.D3_MECHANIC_FIGHT_RESULT] = this.MECHANIC_FUCKED_YOU;
            this.clearOutput();
            this.outputText("Well, he does have quite the thick, throbbing piece of male genitalia on display for you.... There's no sense in letting a good cock go to waste, right? You stalk over to him, looming over the defeated demon with a predatory grin. Your tongue flicks out to lick your lips. Meanwhile, your lower ones go suddenly and irrevocably slick, excited and instantly ready for a helping of thick demon-cock. Your [vagina] is getting warmer and warmer as you tease the poor demon, leaving him to wonder if you're going to kill him or fuck him. The worry is as obvious on his face as the upraised tumescence that jabs spear-like out from his crotch, aimed straight at you.");
            this.outputText("\n\nAbruptly, you drop your [butt] down on top of him, sitting square on top of his chest");
            if (this.player.tailType != 0)
                this.outputText(", tail draped across his face");
            this.outputText(", and casually begin to undress. Looking over your shoulders, you see hope in his soulless eyes. You tease, \"<i>I wouldn't want to stain my clothes when I finish you off, would I?</i>\" His dick wilts a little at that, and he looks about to faint. Giggling, you toss aside your [armor] and grab hold of his mostly-hard cock, feeling the corrupted, textured nodules that encircle his shaft flaring up at your gentle squeeze. He firms up nicely in your grip, filling back to a pulsating, rigid hardness in seconds.");
        ...
        rideMechanicsCockII(cType) {
            ...
            //Demon first:
                ...
                this.monster.cocks[0].cockType = CockTypesEnum_1.CockTypesEnum.HORSE;
            }
            //Dogdick
            else if (cType == this.MECHANIC_DOG_COCK) {
                this.outputText("Giving the incubus's sweaty tool an affectionate squeeze, you decide, \"<i>Grow a dog-cock. Let's see if I can take the knot, shall we, pet?</i>\"");
                ...
                this.monster.cocks[0].cockType = CockTypesEnum_1.CockTypesEnum.DOG;
            }
            ...
            if (this.player.wetness() <= 2)
                this.outputText(", letting your free-flowing pussyjuice wash over it, lubricating it with your glorious girlcum");
            else if (this.player.wetness() <= 3)
                this.outputText(", letting your oh-so-juicy vagina drip and dribble, soaking it with your copious girlcum");
            else
                this.outputText(", letting your lubricant drip over it");
            this.outputText(". You stuff two fingers inside and stroke deep before pulling them out and using them to spread you wide open. The " + this.monster.cockDescriptShort() + " pulsates with every beat of its demonic owner's heart, needing you every bit as much as you now need it.");
            ...
            if (tightFit)
                this.outputText(" well-stretched");
            else
                this.outputText(" stuffed");
            this.outputText(" twat to silence him.");
            ...
            if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.IncubusBribed) >= 0)
                this.outputText(" Nothing but hentai mags for you to drain your balls with....");
            this.outputText("</i>\"");
            ...
            if (this.player.clitLength >= 5)
                this.outputText(" You grab hold of it in your hand and start to stroke it to the tempo your rising and falling body is setting, pumping the oversensitive organ like a dick. Every stroke sends your walls into fits of fluttering ecstasy. It feels so good that you don't think you'll be able to stop until you're cumming and gushing all over your demonic lover.");
            else
                this.outputText(" You thumb at it, rubbing in slow circles around it. Sometimes you push on its hood. Other times you oh-so-gently squeeze at it. It feels so good that the touches drive you crazy with ecstasy, and your walls flutter excitedly around your demonic lover.");
            this.outputText(" His only reaction is to gasp and groan, balls lurching in his sack.");
            ...
            if (cType == this.MECHANIC_HORZ_GOG)
                this.outputText(", the head flaring to seal itself against your cervix");
            else if (cType == this.MECHANIC_DOG_COCK)
                this.outputText(", the knot popping in just in time to start bloating, locking you in place");
            this.outputText(", and jism begins to shoot out in waves.");
            if (!this.player.isPregnant()) {
                this.outputText(" Your empty womb drinks up the virile, seed-filled sludge with abandon.");
                if (cType == this.MECHANIC_HORZ_GOG)
                    this.outputText(" After all, that bestial horse-cock doesn't really leave it anywhere else to go. The demonic animal-cunt just continues spurting straight on in to have its way with your eggs, and you're sure there isn't a contraceptive this side of Mareth that could stop you from having his imps.");
                else if (cType == this.MECHANIC_DOG_COCK)
                    this.outputText(" After all, that immense knot doesn't really leave it anywhere else to go. Once every fold is bathed in alabaster doggie-cream, the continued ejaculations start pouring in after your eggs. His knot is so big and stretching you so wide; you don't think he would pop out even if he filled your belly to look nine months pregnant. Worse, or perhaps better, you're sure with how thick and virile his seed feels that you'll be getting pregnant with his imps from this.");
                this.player.knockUp(PregnancyStore_1.PregnancyStore.PREGNANCY_IMP, PregnancyStore_1.PregnancyStore.INCUBATION_IMP);
            }
            else {
                this.outputText(" Your blocked womb deflects the seed, forcing it all to stay in your channel");
                if (cType != this.MECHANIC_DOG_COCK)
                    this.outputText(" and drip out over his balls, much to his chagrin.");
                else
                    this.outputText(" and stretch you further.");
            }
            ...
            if (this.player.wetness() < 4)
                this.outputText(", even though you aren't normally a squirter");
            this.outputText(". The pleasure is so great that you can feel electric impulses arcing between neurons, short-circuiting your throughs into involuntary ecstasy. Your eyes roll back, and your [hips] twitch and shake, vibrating around the object that brough you to this perfect");
            if (!this.player.isPregnant())
                this.outputText(" insemination");
            else
                this.outputText(" tryst");
            this.outputText(".");
        ...
        mechanicFuckedYouUp(hpVictory, pcCameWorms) {
        ...
        maleLossToMechanic(hpVictory) {
            ...
            if (!hpVictory) {
                ...
                //Medium cocks
                    ...
                    this.outputText("\"<i>You're so... average. I suppose you'll do, if the succubi feel like having a drab, everyday orgasm.</i>\" The incubus shrugs. \"<i>A dick will do...</i>\"");
                }
                //Big cocks
                ...
                //Huge cocks
                    ...
                    this.outputText("\"<i>Seriously?</i>\" the incubus asks. \"<i>How am I supposed to work with something so gargantuan?</i>\" He disdainfully pulls a vial from his pocket and upends it over your crotch. The effect is immediate. [Eachcock] shrinks from ridiculously large down to horse-sized. <i>\"There, that's better.\"</i>");
                ...
                if (this.player.cocks.length == 1)
                    ...
                    this.outputText(" a");
                this.outputText(" length inside any hole and thrust until you're quivering in orgasmic release. That gorgeous demon-cock taunts you, swaying just out of reach.");
            }
            //HP Loss
            else {
                this.outputText("Dropping prone, you gasp for breath, struggling against your injured body to rise. Your muscles burn with the effort, exhausted from the fight, and ultimately, you fail to stand. Slumping down on the ground, you're forced to come to terms with your defeat. The incubus has won. You came all this way, gave your best, and got smacked down by a lower demon for all your troubles. Some champion you are.");
            ...
            if (this.player.cocks.length > 1) {
                ...
                var allDemon = false;
                if (this.player.demonCocks() == this.player.cocks.length)
                    allDemon = true;
                if (!allDemon)
                    this.outputText(" The skin of your transforming length is darkening to a purplish hue, and in the wake of the magical cock-ring, nodules are slowly emerging, growing into an imitation of the cock you're still suckling.");
                this.outputText(" The incubus chuckles, easily pushing the ring the rest of the way down, smiling at the way your demonic tip pulsates and leaks pre-cum. Once it's nestled firmly against your groin, your new master withdraws his hands (and his cock). <b>Your singular, demonic cock is impossibly hard.</b>");
            }
            else {
                //Single
        ...
        maleLossToMechanicII() {
            this.clearOutput();
            this.outputText("An agonizingly perfect pussy is suddenly pressing down on your cock, subsuming it in slick, hot depths that cling and squeeze with undulations so skillful you'd be cumming already if it weren't for the magic ring.");
        ...
        maleLossToMechanicIII() {
            this.clearOutput();
            this.outputText("The champion finally got to cum once Lethice had her turn, but by that point, pussy was all " + this.player.mf("he", "she") + " could think about. Release came more frequently with the demon queen's permission. It didn't really matter, though. The champion was addicted to riding on the edge of climax, broken into nothing more than a demonic vibrator.");
        ...
        errybodyelseLossToMechanic(hpVictory) {
            ...
            //Genderless HP Loss
                ...
                this.outputText("Collapsing onto the ground, you look up at the demonic aggressor with hate in your eyes, unwilling to admit defeat even when staring full-on into its face. The incubus, for his part, simply smiles down at you as he steps closer, claws clicking noisily on the smooth, tiled pavement.");
                ...
                if (this.player.hasFuckableNipples())
                    this.outputText(" slipping inside a [nipple] to thrust just like your other hand. You work your upper and lower pussies like a woman possessed.");
                else {
                    this.outputText(" pinching and tugging on");
                    if (this.player.totalNipples() > this.player.totalBreasts())
                        this.outputText(" nipple after nipple, your myriad breastly endowments providing you with oh so many buttons to tend to");
                    else
                        this.outputText("your [nipples] one after another");
                }
                this.outputText(" Alas, all good things must come to an end, and your lovely, lovely little lollipop is rudely pulled from your questing lips with a noisy pop.");
            ...
            if (this.player.isNaga())
                this.outputText("r coils don't get sore");
            else if (!this.player.isBiped())
                this.outputText(" don't even need to support yourself anymore");
            else
                this.outputText(" knees don't get sore");
            this.outputText(". How wonderful! If you move just right, you can make the whole contraption sway slightly to get a proper dick-sucking rhythm going. His pearly pre-cream oozes onto your tongue, and you swallow as much of it as you can.");
            ...
            if (this.player.clitLength > 5)
                this.outputText(", even if that makes it truly monstrous indeed");
            this.outputText(". It drips long, sticky strands of girlish lust as you swallow, only becoming more soaked as time passes as your body becomes even more productive. A plaintive whimper escapes around the edges of your phallic gag, but your cry falls on deaf ears. The incubus is too busy fitting pipes together beneath you to care.");
    ...
    exports.IncubusMechanicScenes = IncubusMechanicScenes;
});
//# sourceMappingURL=IncubusMechanicScenes.js.map