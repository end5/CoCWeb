define(["require", "exports", "../../../BaseContent", "../../../GlobalFlags/kFLAGS", "../../../StatusAffects", "./MinotaurKing", "../../../PerkLib", "../../../Items/WeaponLib"], function (require, exports, BaseContent_1, kFLAGS_1, StatusAffects_1, MinotaurKing_1, PerkLib_1, WeaponLib_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    class MinotaurKingScenes extends BaseContent_1.BaseContent {
        encounterMinotaurKing() {
            this.clearOutput();
            this.outputText("The moment you step away from the defeated demon, Lethice’s assembled host parts like some kind of tentacled sea, revealing a path that would lead directly to the Queen of Corruption were it not barred by a form out of nightmares. A minotaur like none you’ve ever seen bars your path - the last obstacle before a face-to-face encounter with the cause of so much discontent.");
            ...
            if (this.flags[kFLAGS_1.kFLAGS.ISABELLA_FOLLOWER_ACCEPTED] != 0)
                this.outputText(" Even Isabella would struggle to wield such a weapon.");
            else if (this.player.findStatusAffect(StatusAffects_1.StatusAffects.CampMarble) >= 0)
                this.outputText(" Even Marble would struggle to wield such a weapon");
            else if (this.getGame().helFollower.followerHel())
                this.outputText(" Even Helia would struggle to wield such a weapon.");
            this.outputText(" This poor cow-slut, this ‘Excellia’ can barely to bring it to her lord without tipping forward onto her too-large tits.");
            ...
            this.player.createStatusAffect(StatusAffects_1.StatusAffects.MinotaurKingMusk, 0, 0, 0, 0);
            this.startCombat(new MinotaurKing_1.MinotaurKing());
        }
        theKingIsDeadLongLiveTheKing(hpVictory) {
            this.flags[kFLAGS_1.kFLAGS.MINOTAURKING_DEFEATED] = 1;
            //Cannot be defeated by HP loss, so only writing lust win texts.
            this.clearOutput();
            this.outputText("The King’s axe clatters on noisily on the ground. A gasp runs through the corrupt host as one of their mightiest champions drops to his knees, staring at his dick and furiously pumping it with both hands. Nearly insensate with need, he topples over, humping his own hands with so much enthusiasm that he’s lost interest in anything but immediate sexual gratification.");
            ...
            if (this.player.cor <= 33)
                this.outputText(" The poor things are so corrupted.");
            this.menu();
            this.addButton(0, "Kill Them", this.murderhobo);
            var smallCockIdx = -1;
            for (var i = 0; i < this.player.cocks.length; i++) {
                if (this.player.cocks[i].cockLength <= 12) {
                    if (smallCockIdx != -1 && this.player.cocks[smallCockIdx].cockLength < this.player.cocks[i].cockLength) {
                        smallCockIdx = i;
                    }
                }
            }
            if (smallCockIdx != -1)
                this.addButton(1, "Docking", this.dockucocku, smallCockIdx);
            ...
            if (this.player.hasVagina())
                this.addButton(5, "Ride Him", this.mechanicalbullhue);
        }
        murderhobo() {
            this.flags[kFLAGS_1.kFLAGS.MINOTAURKING_KILLED] = 1;
            this.clearOutput();
            this.outputText("You make it quick, then straighten to stare Lethice in the eye. You’ll purge every single ounce of corruption from this world by any means necessary.");
            this.cleanupAfterCombat(this.getGame().d3.resumeFromFight);
        }
        dockucocku(cockIdx) {
            this.flags[kFLAGS_1.kFLAGS.MINOTAURKING_FUCKED] = 1;
            this.clearOutput();
            this.outputText("With a dick that big and that hard at your disposal... you could do something that some might consider crazy, something with enough erotic potential that it makes your [cocks] even harder than the fuck-scent in the air and the nude, well-endowed figures before you.");
            ...
            if (this.player.horseCocks() > 0)
                this.outputText(" a horse-dick like yours");
            else
                this.outputText(" the average equine prick");
            this.outputText(", but you discard the thought as irrelevant.");
            ...
            this.outputText(" in the steady flow of pre that leaks around your dong, savoring this moment for what it is. You might not get another chance to do something like this once you take down Lethice, and you want it to be memorable. Grinning up at her, you mouth, <i>“You’re next,”</i> and resume feeding the rest of yourself into your cylindrical fucktube.");
        ...
        dockucockuknotu(cockIdx) {
            this.clearOutput();
            this.outputText("When you try to pull out, there’s a problem: your knot. It inflated while you were cumming, and now that you’ve finished, it has expanded to lock you inside your unusual mate. No matter how you tug or pull, you can’t seem to extricate yourself from the cream-filled cow-man, and worse still, he’s starting to moan and squirm. Getting filled full of your spunk must have been more enjoyable for him than you thought.");
            ...
            if (this.player.balls == 0)
                this.outputText(", fat with bull-cream.");
            else
                this.outputText(". Your [sack] swells larger and larger, complete with the realization that your [balls] are being transformed into spooge-filled balloons.");
        ...
        buttufucku() {
            this.flags[kFLAGS_1.kFLAGS.MINOTAURKING_FUCKED] = 1;
            this.clearOutput();
            this.outputText("Why fuck the spoiled tart when you’ve got such a virile, muscular specimen at your beck and call? He’s pumping his cock with wild, frenetic strokes, spurting bullet-sized bursts of pre-cum over his musky balls and abdominal fur, but no matter how he tugs or squeezes, he doesn’t seem to be getting any closer to release.");
            ...
            if (this.player.cocks.length == 1)
                this.outputText("s");
            this.outputText(" at the thought, growing hard and firm, ready to batter through the bull’s clenching ring. This is going to be fun.");
            ...
            if (this.player.biggestCockLength() >= 24)
                this.outputText(" until his belly bulges with the clear outline of your [cockBiggest], displaying just how fully he has been taken to the entire assemblage. You’ve claimed this once noble beast, turned his body into a fuzzy, pleasant-smelling condom for your excessively-large cock.");
            else if (this.player.biggestCockLength() >= 12) {
                this.outputText(" until you’ve fed his ass the whole of your girthy rod");
                if (this.player.hasKnot())
                    this.outputText(", knot and all");
                this.outputText(". A visible bulge presents itself on the once-noble beast’s belly, declaring your dominion over him for all to see. Whatever he was before, he’s your cock-sleeve now.");
            }
            else
                this.outputText(" until you’ve buried the entirety of your [cockBiggest] inside his snugly squeezing intestines, repurposing his digestive system into your own personal, phallus playground.");
            ...
            if (this.player.cocks.length > 1) {
                this.outputText(", and the feeling of your other member");
                if (this.player.cocks.length > 2)
                    this.outputText("s");
                this.outputText(" slipping and sliding past his quivering balls certainly doesn’t hurt");
            }
            this.outputText(". If you didn’t have better things to do, or his scent toying with your mind, driving your ardor higher and higher the longer you breath it in, you could savor this for hours. But you’re dizzy with need and in a hurry.");
            ...
            if (this.player.hasKnot())
                this.outputText(" Your knot balloons.");
            else if (this.player.horseCocks() > 0)
                this.outputText(" Your flare expands right on cue, matching the one pressing back against your tongue.");
            else
                this.outputText(" His flare blooms with all the force of a mushroom cloud, nearly pushing your tongue back into your mouth.");
            this.outputText(" You erupt a split-second before he does, squirting thick ropes of jism into his sucking asshole. Then he goes off, shooting a torrent of spunk twenty feet into the air. Horned goblins dance in the deluge, tongues extended and hands cupped to catch as much as possible.");
            ...
            if (this.player.cumQ() > 2500) {
                this.outputText(" The King’s belly swells");
                if (this.player.biggestCockLength() >= 18)
                    this.outputText(" even bigger");
                if (this.player.cumQ() > 5000) {
                    this.outputText(", but you’re far from done. You pressurize his entire digestive system. The last few ejaculations backblast out over");
                    if (this.player.balls > 0)
                        this.outputText(" your balls");
                    else
                        this.outputText(" your crotch");
                    this.outputText(", creating a sticky, salty puddle on the floor");
                }
                this.outputText(".");
            }
        ...
        mechanicalbullhue() {
            this.flags[kFLAGS_1.kFLAGS.MINOTAURKING_FUCKED] = 1;
            this.clearOutput();
            this.outputText("Well, you’ve been handed a prize bull on a silver platter, why not help yourself to his delectable meat? It’s right there, trembling with need and polished to perfection by his erstwhile lover’s affections. It’d slip right in");
            if (this.player.averageVaginalLooseness() <= 3)
                this.outputText(", probably");
            this.outputText(".");
            if (this.player.averageVaginalLooseness() == 4)
                this.outputText(" One of the perks of having experience with well-endowed lovers is not having to second-guess yourself when you come upon a real treat like this one.");
            else if (this.player.averageVaginalLooseness() < 4)
                this.outputText(" You’re not the most experienced at handling such well-endowed studs, but even if you’re wrong, nothing beats trying.");
            else
                this.outputText(" A big thing like that would stretch you so wonderfully. You barely manage to swallow before you start to drool.");
            this.outputText(" All you have to do is climb on. Excellia doesn’t look like she’d mind.");
            ...
            if (this.player.wetness() >= 4)
                this.outputText(" gushes, drenching your thighs");
            else if (this.player.wetness() >= 3)
                this.outputText(" drools rivulets of desire down your thighs");
            else
                this.outputText(" drenches itself anew");
            this.outputText(". Part of you is tempted to drop to");
            if (!this.player.isNaga() && !this.player.isGoo())
                this.outputText(" your knees");
            else
                this.outputText(" the ground");
            this.outputText(" and take him into your mouth, to see if he tastes as good as he smells. You could take him deep into your throat until your nose nestles deep into the curly brush of his pubic hairs, fully immersed in the taste and scent of this bastion of masculinity.");
            ...
            if (this.player.hasCock()) {
                ...
                if (this.player.cocks.length == 1)
                    this.outputText("es");
                this.outputText(" along with the eager beats of your heart, and in a moment of weakness, you lean down to drag your [cockHeadBiggest] along the Minotaur King’s length. It is but a brief dalliance - foreplay before you put him where he belongs. With a regretful sigh, you stop frotting and line him back up.");
            }
            ...
            if (this.player.hasCock())
                this.outputText(", stimulating the base of your jutting she-dick");
            this.outputText(". You reverse direction and slide up, filling the air with the sloppy sound of withdrawing phallus, then drop back down, using the momentum to spread your buttery lips over the first obstruction. Your eyes actually roll back from the raw sensation of it.");
            ...
            if (this.player.lust >= 80)
                this.outputText(" - not that you needed much help -");
            this.outputText(" or his unbelievably potent fluids addling your mind, you find it hard to care. The heat burning");
            if (this.player.isBiped() || this.player.isTaur())
                this.outputText(" between your legs");
            else
                this.outputText(" in your loins");
            this.outputText(" is too demanding, too all consuming to mind one way or the other. Why should it matter anyway? You’re going to bounce on this lordly stud’s prick until your pussy is raw and flooded, bathed in spunk.");
            ...
            if (!this.player.isTaur())
                this.outputText(" in its entirety");
            this.outputText(" while his hips lift to meet your downward strokes. A cylindrical bulge distends your belly. The assembled audience can actually watch the progress of his member into your canal. They can see the head flare wider when your cunt pleases him");
            if (this.player.tallness <= 50 && this.player.thickness <= 4)
                this.outputText(", stretching your skin until it shines");
            this.outputText(".");
            ...
            if (this.player.isLactating())
                this.outputText(", tricking your full nipples into spraying their milk everywhere");
            else
                this.outputText(", denying you even the chance to climb off");
            this.outputText(". Your body has given itself over to this hedonistic relief. So long as the minotaur’s hips saw in and out of your soaked delta, you’re going to keep cumming.");
            ...
            if (this.player.pregnancyIncubation != 0)
                this.outputText(" force its way past your cervix");
            else
                this.outputText(" tear open your womb");
            this.outputText(". Your insides stretch like they’re made of rubber");
            if (this.player.averageVaginalLooseness() >= 5)
                this.outputText(", which isn’t far from the truth,");
            this.outputText(" in order to accommodate the swelling intrusion. The beast below roars. You can feel the bass rumble in your bones - and your [clit].");
            if (this.player.hasCock()) {
                this.outputText(" Hell, you can feel it surging through your [cocks], vibrationally expelling the last few unspent drops from your urethra");
                if (this.player.cocks.length > 1)
                    this.outputText("s");
                this.outputText(".");
            }
            ...
            if (this.player.isBiped())
                this.outputText(", [legs] akimbo");
            this.outputText(". The artificial euphoria this has brought you makes it difficult not to crawl over to the nearest incubus and offer your body up, but you still have to deal with Lethice. Maybe you can make her lick the drippings from your slit after you take her down.");
            ...
            this.flags[kFLAGS_1.kFLAGS.MINOTAURKINGS_TOUCH] = 1;
            this.cleanupAfterCombat(this.getGame().d3.resumeFromFight);
        }
        titfuckCowslut() {
            this.flags[kFLAGS_1.kFLAGS.MINOTAURKINGSSLUT_FUCKED] = 1;
            this.clearOutput();
            this.outputText("Forget that beefcake; there’s a primo slut close at hand with a set of tits that’d be worth damnation to fuck. It’s a good thing Lethice is giving you this chance to sate yourself. You might have had to risk giving them a good fucking even if she didn’t. Pulling [oneCock] out of your [armor], you advance on the fallen slut, feeling momentarily sorry for her.");
            ...
            if (this.player.cumQ() <= 500)
                this.outputText(" Most fall short, but one daring squirt manages to coat Excellia’s tongue.");
            else if (this.player.cumQ() <= 1500)
                this.outputText(" Most manage to fall on Excellia’s tongue, painting it a nearly white save for a few off-color splotches. The rest puddles in the nape of her neck, giving you time to enjoy the cow-print tongue you’ve created.");
            else {
                this.outputText(" Excellia’s tongue never stood a chance. You cover most of it in the first shot alone, pouring much of the rest down her throat. Bits of sperm fleck her cheeks, and a deep puddle forms at the nape of her neck, complete with dribbling tributaries along her collarbones.");
                if (this.player.cumQ() >= 5000)
                    this.outputText(" And her breasts? They’re basically white save for their peaks.");
            }
        ...
        sloppySeconds() {
            this.flags[kFLAGS_1.kFLAGS.MINOTAURKINGSSLUT_FUCKED] = 1;
            this.clearOutput();
            var x = this.player.biggestCockIndex() + 1;
            this.outputText("Why resort to making use of the brutal minotaur when there’s a puddle of wet trollop so close at hand? It’s a good thing that all the spunk leaking from between her legs actually smells");
            if (this.player.findPerk(PerkLib_1.PerkLib.MinotaurCumAddict) >= 0)
                this.outputText(" wonderful");
            else
                this.outputText(" sort of... nice... sexy even");
            this.outputText(", or you might have had second thoughts. Her creampied honeypot ought to feel like teflon-treated silk against your [cock " + x + "]. It glitters in the flickering, bedroom light of Lethice’s throne room, beckoning you to plunge inside and see just how good it will feel.");
            ...
            if (this.player.balls > 0) {
                this.outputText(" and " + MinotaurKingScenes.num2Text(this.player.balls) + " balls full of cum");
            }
            else
                this.outputText(" and a load of backed-up cum");
            this.outputText(". You don’t see much point in correcting her, not when she’s presenting so enthusiastically. Grabbing hold of her tail, you yank her into position, positioning your [cockHead " + x + "] just outside the cream-drizzling gates of her cunt.");
            ...
            if (this.player.balls > 0)
                this.outputText(" ball");
            else
                this.outputText(" ass");
            this.outputText("-slapping impact ripples through the plush cow-slave’s derriere like waves through a pond. She exhales blissful little moos, pushing back against you to the same tempo, ever the faithful cock-milker.");
            if (this.player.biggestCockLength() < 18)
                this.outputText(" You don’t think she could possibly confuse your less-impressive erection for her minotaur master’s, but she doesn’t give any sign of caring. Perhaps all the minotaur jism stuffed in her twat has left her beyond caring, or maybe she’s just happy to serve a dick - any dick.");
            else if (this.player.biggestCockLength() <= 30)
                this.outputText(" You didn’t think she’d confuse your prick for her master’s, but she’s happily fucking it all the same. It probably stretches her cunt just about as much.");
            else
            ...
            this.outputText(" You can see why the overgrown beast-man keeps her around.");
            ...
            if (this.player.cocks.length == 2)
                this.outputText(" Your other penis is enjoying sliding between her perspiring thighs almost as much.");
            else if (this.player.cocks.length > 2)
                this.outputText(" Your other penises are enjoying sliding between her perspiring thighs nearly as much.");
            ...
            if (this.player.hasKnot())
                this.outputText(" There’s not much point in it; your knot does that much and more, trapping your load inside as effectively as the remainder of the Minotaur King’s.");
            this.outputText(" Your");
            if (this.player.isBiped() || this.player.isTaur())
                this.outputText(" [legs] wobble");
            else
                this.outputText(" [leg] wobbles");
            this.outputText(" weakly beneath you, dumping you and your jiggly concubine to the floor mid-orgasm, but it doesn’t slow your ejaculation in the slightest. You feel like you could keep cuming for ever, clenching and releasing, feeding wave after wave into the happily mooing bovine temptress.");
            this.outputText("\n\n");
            if (this.player.hasKnot() && this.player.cumQ() >= 5000)
                this.outputText(" Excellia’s belly bulges into an exceedingly pregnant state, stuffed with so much of your cum that she looks nine months pregnant and then some. Her belly button pops out just before you finish squirting into her, diluting the bestial brute’s druggy load with the seed of a Champion.");
            else if (this.player.cumQ() >= 5000 && !this.player.hasKnot())
                this.outputText(" Excellia’s muscular control is no match for the sheer virility of your load. The gates of her cunt give out in short order, releasing a flood of Champion-diluted minotaur spunk onto the ground. She whimpers and cries out in ecstasy, driven to the peak again and again by the continuous flooding of her womb with your seed.");
        ...
        hailToTheKingBaby(hpVictory, pcCameWorms) {
            this.clearOutput();
            var mk = this.monster;
            if (!hpVictory) {
                this.outputText("Your determination burns away in the fires of your desire, nothing against molten need burning through your brain. How could you ever resist this magnificent beast? His magnificent cock is so big... so hard");
                if (mk.orgasms > 0)
                    this.outputText("... so wet with the juices of his slut");
                this.outputText(". It fills your vision and your thoughts. It would be so easy to bend over for him, to lift your [butt] up in the air and wiggle it in his direction until the big brute forced himself on you. Gods, he could stuff you so well. It’s be so perfect, him inside you, the head of his gigantic monster-cock clearly outlined on your belly.");
                ...
                this.outputText("You’ve been beaten. Staying upright has become an almost impossible task, and the idea of fighting little more than a pipe dream. Still wobbling in place, you look up in time to see the brute approach, laughing jovially.");
            ...
            this.outputText(" He drags you up against his crotch, smearing his sweat and sex-soaked balls against your face. It feels thick and oily on your cheek but smells like getting fucked bareback until your eyes roll back and your tongue lolls out.");
            ...
            this.outputText("\n\n<i>“Atta " + this.player.mf("boy", "girl") + ",”</i> the triumphant beast-man croons. <i>“This is where you belong.”</i>");
            ...
            if (this.player.findPerk(PerkLib_1.PerkLib.BimboBrains) >= 0 || this.player.cor >= 66)
                this.outputText(" You’re long past the point of having a gag reflex and glad for it.");
            else
                this.outputText(" Miraculously, your gag reflex is completely absent. Maybe it’s too drenched in his heavenly pre to protest?");
            ...
            if (this.player.biggestTitSize() >= 5)
                this.outputText(", jiggling your tits wildly");
            else if (this.player.hasCock()) {
                this.outputText(", making your oozing, half-hard prick");
                if (this.player.cocks.length > 1)
                    this.outputText("s");
                this.outputText(" flail around comically, sliming the floor");
            }
            else
                this.outputText(", shaking your whole body with the force of his passion");
            this.outputText(". His flare somehow grows even fatter, wide enough to be visible through the [skinFurScales] of your belly and hard enough for you to stroke through the intervening flesh.");
        ...
        gurglegurgle() {
            this.clearOutput();
            this.outputText("You wind up being grateful for that half-forgotten assfucking later, when the minotaur King finally lays claim to your other end. That little bit of extra gape and lubrication is enough to help him lodge his humongous pecker deep into your asshole. Just like with your throat, you find him easier to handle than his size alone would suggest, but that does nothing to diminish the obscene fullness occupying your mind.");
        ...
        leavethem() {
            this.flags[kFLAGS_1.kFLAGS.MINOTAURKING_ALIVE] = 1;
            this.clearOutput();
            this.outputText("\n\nYou square yourself off at the demon queen");
            if (this.player.weapon != WeaponLib_1.WeaponLib.FISTS)
                this.outputText(", [weapon] in hand");
            else
                this.outputText(", balled fists raised");
            this.outputText(" and reject her offer out of hand. <i>“");
            if (this.player.findPerk(PerkLib_1.PerkLib.BimboBrains) >= 0)
                this.outputText("Ummm, like, I’m not </i>that<i> horny. Besides, I can fuck all I want once I take you out, cutie!");
            else if (this.player.cor >= 66)
                this.outputText("Oh come on. If you’re going to give me something to fuck, at least make it something worth my time. How about your ass, after I beat you?");
            else if (this.player.cor <= 20)
                this.outputText("Why would I debase myself in front of you like that? The pure need no such indulgence.");
            else
                this.outputText("Not all of us are as enslaved by our desires as you.");
            this.outputText("”</i>");
            this.cleanupAfterCombat(this.getGame().d3.resumeFromFight);
        }
    }
    exports.MinotaurKingScenes = MinotaurKingScenes;
});
//# sourceMappingURL=MinotaurKingScenes.js.map