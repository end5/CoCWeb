define(["require", "exports", "../../../BaseContent", "../../../GlobalFlags/kFLAGS", "../../../StatusAffects", "../../../CockTypesEnum", "../../../../../includes/appearanceDefs"], function (require, exports, BaseContent_1, kFLAGS_1, StatusAffects_1, CockTypesEnum_1, appearanceDefs_1) {
    "use strict";
     ...
     * @author Gedan
     */
    class SuccubusGardenerScenes extends BaseContent_1.BaseContent {
        constructor() {
            super(...arguments);
            this.GARDENER_LEFT = 1;
            // private GARDENER_FUCKED: number = 2;
            this.GARDENER_KILLED = 3;
        }
        gardenerDefeated() {
        ...
        gardenerFucked() {
            if (this.flags[kFLAGS_1.kFLAGS.D3_GARDENER_DEFEATED] == 2)
                return true;
            return false;
        }
        gardenerLeft() {
            if (this.flags[kFLAGS_1.kFLAGS.D3_GARDENER_DEFEATED] == 1)
                return true;
            return false;
        }
        fuckUpTheGardener(hpVictory) {
            this.clearOutput();
        ...
        tentaFail() {
            ...
            if (this.player.hasVagina())
                this.outputText(" cunt-kissers");
            else
                this.outputText(" cock-suckers");
            this.outputText(" like those are one in a million.");
            if (this.flags[kFLAGS_1.kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA] > 0)
                this.outputText(" Perhaps Scylla's ruby mouth could give her a run for her money, but there's only one way to be sure.");
            else if (this.getGame().telAdre.dominika.fellatrixSucked())
                this.outputText(" Perhaps Dominika's all-consuming mouth could give her a run for her money, but there's only one way to be sure.");
            ...
            if (this.player.hasCock())
                ...
                this.outputText(" the sensitive skin of your groin");
            ...
            if (this.player.hasCock())
                this.outputText(" blow a wad down this slut's throat");
            else if (this.player.hasVagina())
                this.outputText(" cream yourself on this slut's face");
            else
                this.outputText(" teach this bitch how to pleasure an asshole");
            ...
            if (!this.player.hasCock())
                this.outputText(" Its outline is clearly phallic, but how could such a huge thing ever fit inside you, let alone anyone?");
            else {
                ...
                if (this.player.biggestCockLength() < 36)
                    this.outputText(" How could you ever hope to fill it?");
                else
                    this.outputText(" How long were they preparing this for you?");
            }
            var createdVag = false;
            ...
            //Dicks
                ...
                if (this.player.biggestCockLength() <= 12)
                    this.outputText(" The ease with which you slide inside is no surprise, given the disparity in size.");
                else if (this.player.biggestCockLength() <= 30)
                    this.outputText(" The ease with which you enter is no real surprise, given that even your bloated length" + ((this.player.cocks.length > 1) ? "s are" : " is") + " small by comparison.");
                else if (this.player.biggestCockLength() <= 48)
                    this.outputText(" You slide in with ease, perfectly matched to the hungry fuck-tunnel in size.");
                else
                    this.outputText(" Sliding in must take quite the effort. The orange-hued twat is drooling around you, slowly working down an inch at a time. It takes all of you, even if it has to stretch beyond all reason to do so.");
                this.outputText(" It feels good, better");
                if (this.player.biggestCockLength() <= 12)
                    this.outputText(" than a giant that looks that loose should. You'd swear it was tight little twat from how firmly it's squeezing you!");
                else if (this.player.biggestCockLength() <= 30)
                    this.outputText(" than you had thought at a glance. Not only is it the perfect size, but it seems to hug and touch every part of your maleness" + ((this.player.cocks.length > 1) ? "es" : "") + " just right.");
                else if (this.player.biggestCockLength() > 48)
                    this.outputText(" than you would expect given how tightly-stretched it looks. You'd think it'd be pressing down painfully, but it feels tailor made to take you instead.");
                //Dicks need to grow
        ...
        endHer() {
            ...
            this.flags[kFLAGS_1.kFLAGS.D3_GARDENER_DEFEATED] = this.GARDENER_KILLED;
            this.menu();
        ...
        leaveHer() {
            ...
            this.flags[kFLAGS_1.kFLAGS.D3_GARDENER_DEFEATED] = this.GARDENER_LEFT;
            this.menu();
            this.cleanupAfterCombat(this.getGame().d3.resumeFromFight);
        }
        surrenderToTheGardener(hpVictory = false) {
            // Male
            if (this.player.hasCock() && !this.player.hasVagina()) {
            ...
            if (this.player.hasVagina() && this.player.hasCock()) {
                if (SuccubusGardenerScenes.rand(2) == 0)
                    this.femGenderlessLoss(hpVictory);
                else
                    this.maleLoss(hpVictory);
            }
        ...
        femGenderlessLoss(hpVictory) {
            ...
            if (hpVictory)
                this.outputText(", not even able to push your bruised body up onto its [feet].");
            else
                this.outputText(", too turned on to try and stop your hands from diving into your needy crotch.");
            ...
            if (!hpVictory) {
                ...
                if (this.player.hasVagina())
                    this.outputText(" Your underthings were already swampy with need, but the sight of watching those eager vines squirm elicits fresh trickles of lube to course down your [legs].");
                else
                    this.outputText(" You were already horny, but you find yourself wishing you had a pussy, just so you could take even more of them inside you.");
            }
            else
                this.outputText(" You cast a worried look towards the writhing vines. They seem so eager, yet they hold back for now, perhaps waiting for their mistress to finish. No... you can't go down like this!");
            ...
            if (this.player.hasVagina())
                this.outputText(" Against your better judgment, you're starting to get wet from the promises of her words. You know you're being given to a neverending hell of sexual stimulation, but you can't stop your body's libido from responding to the idea.");
            ...
            if (this.player.balls > 0)
                this.outputText(" across your [sack],");
            else if (this.player.hasCock())
                this.outputText(" across the underside of your [cock],");
            else
        ...
        femGenderlessLossII() {
            ...
            if (this.player.isBiped())
                this.outputText(" Your [legs] are eased open, but who are you to stop them? You're as wet as anything, and if spreading wide is the next step towards scratching your itch, so be it.");
            this.outputText(" Your [clit] must look like a");
            if (this.player.clitLength > 6)
                this.outputText(" glistening, cum-soaked shaft");
            else if (this.player.clitLength > 3)
                this.outputText(" tiny, cum-soaked cock");
            else if (this.player.clitLength > 1)
                this.outputText(" swollen, lube-lacquered gumball");
            else
                this.outputText(" glistening, pink target");
            ...
            if (this.player.clitLength > 6)
                this.outputText(" It stretches some around the cock-sized button");
            else if (this.player.clitLength > 3)
                this.outputText(" It has no trouble handling your plus-sized button");
            else
                this.outputText(" Your tiny button seems dwarfed by the tube enveloping it");
            ...
            if (this.player.biggestTitSize() > 2)
                this.outputText(" and jiggle your tits");
            ...
            if (this.player.isPregnant())
                this.outputText(" to splatter against your occupied womb.");
            else
                this.outputText(" to fill your womb, to make it round with life so that others can feel what you're feeling.");
            this.outputText("\n\nYour girlhoney squirts, splattering over the ground.");
            if (this.player.wetness() < 4)
                this.outputText(" You may not normally be capable of such soaking orgasms, but that seems of little concern to you.");
        ...
        maleLoss(hpVictory) {
            ...
            if (hpVictory)
                ...
                this.outputText(", too turned on to try and stop your hands from rushing to your crotch. Why bother?");
        ...
        maleLossPet() {
            ...
            if (this.player.biggestCockLength() >= 18)
                this.outputText(" <i>\"Oh, it's quite the big bone, isn't it? You're going to have an awful hard time dragging that around on all fours, [puppy]. Why, I bet the moment you see a bitch, you're going to be humping the ground, practically immobilized by it.");
                ...
                if (this.player.cocks[this.player.biggestCockIndex()].cockType != CockTypesEnum_1.CockTypesEnum.DOG)
                    this.outputText(", once you have an appropriately-shaped cock");
                this.outputText(".");
            }
            else {
            ...
            if (this.player.balls > 0)
                this.outputText(", just above your [balls],");
            ...
            if (!this.player.hasMuzzle())
                this.outputText(", heedless of the fact that you seem to be able to reach a little farther with every one you consume");
            ...
            if (this.player.isNaga())
                this.outputText(" threes");
            else if (this.player.isTaur())
                this.outputText(" your hooves");
            else
                this.outputText(" fours");
            this.outputText(".");
            if (!this.player.isTaur())
                this.outputText(" You scrabble over, unusued to having to move in such a way,");
            else
                this.outputText(" You gallop over and drop down on your forelegs");
            ...
            if (this.player.tailType != 0) {
                ...
                if (this.player.tailType == 13 && this.player.tailVenom > 1)
                    this.outputText("s");
            }
            else
                this.outputText(" your stubby, new tail");
            this.outputText(" wagging.");
            if (this.player.tailType != 2)
                this.outputText(" Never mind that it gets longer and fuzzier with every gulp or that your");
            else
                this.outputText(" Never mind that your");
            ...
            if (this.player.hasVagina())
                this.outputText(", lubricated with a sheen vaginal juices from your forgotten sex organ");
            ...
            if (this.player.cocks.length > 1)
                this.outputText("s thrum");
            else
                this.outputText(" thrums");
            ...
            if (this.player.isTaur() || this.player.isDrider()) {
                ...
                if (this.player.isDrider())
                    this.outputText(" clip-clop of your hooves");
                else
                    this.outputText(" sharp clacks of your spidery claws");
                ...
                if (this.player.isGoo())
                    this.outputText(" Your gooey undercarriage easily divides into two hind legs, slapping wetly with regularity as you plod along.");
                else if (this.player.isNaga())
                    this.outputText(" Your slithering tail twitches, stopping you for a second, but then it splits into two limbs, each firmer and more canine than the snake-like appendage you once had. They wobble unsteadily for a second before jumping into action.");
                else
                    this.outputText(" Your [legs] shorten as you plod along, slowing you momentarily as they change. You lift your knees up so that you can stand on your [feet] once more, only they're soft, padded paws now, tipped with short nails.");
        ...
        maleLossPetII() {
            ...
            if (this.player.skinType != appearanceDefs_1.SKIN_TYPE_FUR)
                this.outputText(" Was that there before?");
            ...
            if (this.player.cumQ() >= 2500)
                this.outputText(" A few thick ropes shoot past your head to cream the wall, eliciting a pleased giggle from your owner as she squeezes yet more from your length.");
        ...
        maleLossFeeder() {
            ...
            if (this.player.biggestTitSize() >= 3)
                this.outputText(" the underside of your breasts");
            else
                this.outputText(" your chin");
            ...
            //Ball filling
                ...
                if (this.player.ballSize > 15)
                    this.outputText(" Thankfully, a few tentacles loop under the gigantic orbs to help support them.");
            }
            this.outputText("\n\nYou happily gurgle into the tentacle in your mouth and arch your back, blowing the first huge rope of your enhanced load");
            // 9999 - Check muh
            if (this.player.biggestTitSize() >= 3)
                this.outputText(" all over your body");
            else
                this.outputText(" directly onto the soil below");
        ...
        maleLossFeederII() {
            ...
            if (this.player.tailType != 0)
                this.outputText(" past your tail to slither");
            this.outputText(" between your cheeks.");
            if (this.player.ass.analLooseness == 0)
                this.outputText(" The virginal tightness of your pucker holds the tentacle at bay at first. Still, it's no match for the combination of slippery plant fluid and constant pressure.");
            else if (this.player.ass.analLooseness <= 3)
                this.outputText(" The tentacle has little problem slipping through your tight backdoor, slick as it is with fluid and backed by constant pressure.");
            else
                this.outputText(" The tentacle has little difficulty squeezing into your well-loosened ass. Even without its constant, firm pressure and slippery fluid, it could enter with ease.");
            ...
            if (this.player.hasVagina())
                this.outputText(", and once a thick, cock-shaped plant slides inside your [vagina], you truly are");
            this.outputText(".");
            //Fuckable nipples
    ...
    exports.SuccubusGardenerScenes = SuccubusGardenerScenes;
});
//# sourceMappingURL=SuccubusGardenerScenes.js.map