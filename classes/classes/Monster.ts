...
export class Monster extends Creature {
    ...
    */
    ...
    public get drop(): RandomDrop { return this._drop; }
    ...
    public get checkCalled(): boolean { return this._checkCalled; }
    ...
    public checkMonster(): boolean {
        ...
        if (this.checkError.length > 0) CoC_Settings.error("Monster not initialized:" + this.checkError);
    ...
    public outputAttack(damage: number): void {
        if (damage <= 0) {
            ...
            if (Monster.rand(this.player.armorDef + this.player.tou) < this.player.armorDef) this.outputText("You absorb and deflect every " + this.weaponVerb + " with your " + this.player.armorName + ".", false);
            else {
                if (this.plural) this.outputText("You deflect and block every " + this.weaponVerb + " " + this.a + this.short + " throw at you.", false);
                else this.outputText("You deflect and block every " + this.weaponVerb + " " + this.a + this.short + " throws at you.", false);
            }
        }
        else if (damage < 6) this.outputText("You are struck a glancing blow by " + this.a + this.short + "! (" + damage + ")", false);
        else if (damage < 11) {
    ...
    protected handleBlind(): boolean {
        if (Monster.rand(3) < 2) {
            if (this.weaponVerb == "tongue-slap") this.outputText(this.capitalA + this.short + " completely misses you with a thrust from " + this.pronoun3 + " tongue!\n", false);
            else this.outputText(this.capitalA + this.short + " completely misses you with a blind attack!\n", false);
            return false;
        }
        return true;
    }
    ...
    protected outputPlayerDodged(dodge: number): void {
        if (dodge == 1) this.outputText("You narrowly avoid " + this.a + this.short + "'s " + this.weaponVerb + "!\n", false);
        else if (dodge == 2) this.outputText("You dodge " + this.a + this.short + "'s " + this.weaponVerb + " with superior quickness!\n", false);
        else {
    ...
    private playerDodged(): boolean {
        ...
        //("Misdirection"
        if (this.player.findPerk(PerkLib.Misdirection) >= 0 && Monster.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
    ...
    public doAI(): void {
        ...
        if (this.game.player.findStatusAffect(StatusAffects.Exgartuan) >= 0 && this.game.player.statusAffectv2(StatusAffects.Exgartuan) == 0 && Monster.rand(3) == 0) {
            if (this.game.exgartuan.exgartuanCombatUpdate()) this.game.outputText("\n\n", false);
        }
    ...
    protected handleFear(): boolean {
        if (this.statusAffectv1(StatusAffects.Fear) == 0) {
            ...
            if (this.plural) this.game.outputText(this.capitalA + this.short + " are too busy shivering with fear to fight.", false);
            else this.game.outputText(this.capitalA + this.short + " is too busy shivering with fear to fight.", false);
        }
        this.game.combatRoundOver();
        return false;
    }
    ...
    protected handleStun(): boolean {
        if (this.plural) this.game.outputText("Your foes are too dazed from your last hit to strike back!", false);
        else this.game.outputText("Your foe is too dazed from your last hit to strike back!", false);
        if (this.statusAffectv1(StatusAffects.Stunned) <= 0) this.removeStatusAffect(StatusAffects.Stunned);
    ...
    public generateDebugDescription(): string {
        ...
        // APPEARANCE
        result += Heis + Appearance.inchesAndFeetsAndInches(this.tallness) + " tall with " +
            Appearance.describeByScale(this.hipRating, Appearance.DEFAULT_HIP_RATING_SCALES, "thinner than", "wider than") + " hips and " +
            Appearance.describeByScale(this.buttRating, Appearance.DEFAULT_BUTT_RATING_SCALES, "thinner than", "wider than") + " butt.\n";
        result += this.Pronoun3 + " lower body is " + (Appearance.DEFAULT_LOWER_BODY_NAMES[this.lowerBody] || ("lowerBody#" + this.lowerBody));
        result += ", " + this.pronoun3 + " arms are " + (Appearance.DEFAULT_ARM_NAMES[this.armType] || ("armType#" + this.armType));
        result += ", " + this.pronoun1 + " " + have + " " + this.skinTone + " " + this.skinAdj + " " + this.skinDesc + " (type " + (Appearance.DEFAULT_SKIN_NAMES[this.skinType] || ("skinType#" + this.skinType)) + ").\n";
        result += Hehas;
        if (this.hairLength > 0) {
            result += this.hairColor + " " + Appearance.inchesAndFeetsAndInches(this.hairLength) + " long " + (Appearance.DEFAULT_HAIR_NAMES[this.hairType] || ("hairType#" + this.hairType)) + " hair.\n";
        } else {
            result += "no hair.\n";
        }
        result += Hehas
            + (Appearance.DEFAULT_FACE_NAMES[this.faceType] || ("faceType#" + this.faceType)) + " face, "
            + (Appearance.DEFAULT_EARS_NAMES[this.earType] || ("earType#" + this.earType)) + " ears, "
            + (Appearance.DEFAULT_TONGUE_NAMES[this.tongueType] || ("tongueType#" + this.tongueType)) + " tongue and "
            + (Appearance.DEFAULT_EYES_NAMES[this.eyeType] || ("eyeType#" + this.eyeType)) + " eyes.\n";
        result += Hehas;
        ...
        else result += (Appearance.DEFAULT_ANTENNAE_NAMES[this.antennae] || ("antennaeType#" + this.antennae)) + " antennae.\n\n";
        ...
        for (i = 0; i < this.vaginas.length; i++) {
            ...
            result += this.Pronoun3 + (i > 0 ? (" #" + (i + 1)) : "") + " " + (Appearance.DEFAULT_VAGINA_TYPE_NAMES[vagina.type] || ("vaginaType#" + vagina.type)) + (vagina.virgin ? " " : " non-") + "virgin vagina is ";
            ...
            if (this.statusAffectv1(StatusAffects.BonusVCapacity) > 0) {
                result += "; vaginal capacity is increased by " + this.statusAffectv1(StatusAffects.BonusVCapacity);
            }
            result += ".\n";
        }
    ...
    public combatRoundUpdate(): void {
        ...
        if (this.findStatusAffect(StatusAffects.CoonWhip) >= 0) {
            if (this.statusAffectv2(StatusAffects.CoonWhip) <= 0) {
                ...
                this.outputText(" armor by " + this.statusAffectv1(StatusAffects.CoonWhip) + ".</b>\n\n")
            }
        }
        if (this.findStatusAffect(StatusAffects.Blind) >= 0) {
            ...
            else this.outputText("<b>" + this.capitalA + this.short + (this.plural ? " are" : " is") + " currently blind!</b>\n\n", false);
        }
        ...
        if (this.findStatusAffect(StatusAffects.IzmaBleed) >= 0) {
            ...
            //Deal damage if still wounded.
                ...
                if (this.plural) this.outputText(this.capitalA + this.short + " bleed profusely from the jagged wounds your weapon left behind. (" + store + ")\n\n", false);
                else this.outputText(this.capitalA + this.short + " bleeds profusely from the jagged wounds your weapon left behind. (" + store + ")\n\n", false);
            }
        }
        ...
        if (this.findStatusAffect(StatusAffects.LustStick) >= 0) {
            ...
            switch (this.statusAffectv1(StatusAffects.LustStick)) {
                //First:
                case 1:
                    if (this.plural) this.outputText("One of " + this.a + this.short + " pants and crosses " + this.mf("his", "her") + " eyes for a moment.  " + this.mf("His", "Her") + " dick flexes and bulges, twitching as " + this.mf("he", "she") + " loses himself in a lipstick-fueled fantasy.  When " + this.mf("he", "she") + " recovers, you lick your lips and watch " + this.mf("his", "her") + " blush spread.\n\n", false);
                    else this.outputText(this.capitalA + this.short + " pants and crosses " + this.pronoun3 + " eyes for a moment.  " + this.mf("His", "Her") + " dick flexes and bulges, twitching as " + this.pronoun1 + " loses " + this.mf("himself", "herself") + " in a lipstick-fueled fantasy.  When " + this.pronoun1 + " recovers, you lick your lips and watch " + this.mf("his", "her") + " blush spread.\n\n", false);
                    break;
                //Second:
                case 2:
                    if (this.plural) this.outputText(this.capitalA + this.short + " moan out loud, " + this.pronoun3 + " dicks leaking and dribbling while " + this.pronoun1 + " struggle not to touch " + this.pronoun2 + ".\n\n", false);
                    else this.outputText(this.capitalA + this.short + " moans out loud, " + this.pronoun3 + " dick leaking and dribbling while " + this.pronoun1 + " struggles not to touch it.\n\n", false);
                    break;
                //Third:
                case 3:
                    if (this.plural) this.outputText(this.capitalA + this.short + " pump " + this.pronoun3 + " hips futilely, air-humping non-existent partners.  Clearly your lipstick is getting to " + this.pronoun2 + ".\n\n", false);
                    else this.outputText(this.capitalA + this.short + " pumps " + this.pronoun3 + " hips futilely, air-humping a non-existent partner.  Clearly your lipstick is getting to " + this.pronoun2 + ".\n\n", false);
                    break;
                //Fourth:
                case 4:
                    if (this.plural) this.outputText(this.capitalA + this.short + " close " + this.pronoun3 + " eyes and grunt, " + this.pronoun3 + " cocks twitching, bouncing, and leaking pre-cum.\n\n", false);
                    else this.outputText(this.capitalA + this.short + " closes " + this.pronoun2 + " eyes and grunts, " + this.pronoun3 + " cock twitching, bouncing, and leaking pre-cum.\n\n", false);
                    break;
                //Fifth and repeat:
                default:
                    if (this.plural) this.outputText("Drops of pre-cum roll steadily out of their dicks.  It's a marvel " + this.pronoun1 + " haven't given in to " + this.pronoun3 + " lusts yet.\n\n", false);
                    else this.outputText("Drops of pre-cum roll steadily out of " + this.a + this.short + "'s dick.  It's a marvel " + this.pronoun1 + " hasn't given in to " + this.pronoun3 + " lust yet.\n\n", false);
                    break;
            }
        ...
        if (this.short === "secretarial succubus" || this.short === "milky succubus") {
            ...
            if (this.player.lust >= 90) this.outputText("You have trouble keeping your greedy hands away from your groin.  It would be so easy to just lay down and masturbate to the sight of your curvy enemy.  The succubus looks at you with a sexy, knowing expression.\n\n", false);
            this.game.dynStats("lus", 1 + Monster.rand(8));
        }
        //[LUST GAINED PER ROUND] - Omnibus
        if (this.findStatusAffect(StatusAffects.LustAura) >= 0) {
            if (this.player.lust < 33) this.outputText("Your groin tingles warmly.  The demon's aura is starting to get to you.\n\n", false);
            if (this.player.lust >= 33 && this.player.lust < 66) this.outputText("You blush as the demon's aura seeps into you, arousing you more and more.\n\n", false);
            if (this.player.lust >= 66) {
                ...
                temp = Monster.rand(4);
                if (temp == 0) this.outputText("You have a hard time not dropping to your knees to service her right now.\n\n", false);
                if (temp == 2) this.outputText("The urge to bury your face in her breasts and suckle her pink nipples nearly overwhelms you.\n\n", false);
                if (temp == 1) this.outputText("You swoon and lick your lips, tasting the scent of the demon's pussy in the air.\n\n", false);
                if (temp == 3) this.outputText("She winks at you and licks her lips, and you can't help but imagine her tongue sliding all over your body.  You regain composure moments before throwing yourself at her.  That was close.\n\n", false);
            }
