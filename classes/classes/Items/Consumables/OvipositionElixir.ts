...
export class OvipositionElixir extends Consumable {
    ...
    public useItem(): boolean {
        ...
        if (this.game.player.pregnancyIncubation == 0) { //If the player is not pregnant, get preggers with eggs!
            ...
            this.game.player.createStatusAffect(StatusAffects.Eggs, Utils.rand(6), 0, Utils.rand(3) + 5, 0);
            return (false);
        }
        ...
        if (!changeOccurred && this.game.player.pregnancyIncubation > 20 && this.game.player.pregnancyType != PregnancyStore.PREGNANCY_BUNNY) { //If no changes, speed up pregnancy.
            ...
            var newIncubation: number = this.game.player.pregnancyIncubation - Math.floor(this.game.player.pregnancyIncubation * 0.3 + 10);
