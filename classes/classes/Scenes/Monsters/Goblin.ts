...
export class Goblin extends Monster {
    protected goblinDrugAttack(): void {
        ...
        if (color != "blue") {
            if (this.short == "Tamani's daughters") this.outputText("Tamani uncorks a glass bottle full of " + color + " fluid and swings her arm, flinging a wave of fluid at you.", false);
            else this.outputText(this.capitalA + this.short + " uncorks a glass bottle full of " + color + " fluid and swings her arm, flinging a wave of fluid at you.", false);
        }
        //Drink blue pots
        else {
        ...
        //Dodge chance!
        if ((this.player.findPerk(PerkLib.Evade) >= 0 && Goblin.rand(10) <= 3) || (Goblin.rand(100) < this.player.spe / 5)) {
            ...
            if (color == "red") {
                ...
                if (this.player.findStatusAffect(StatusAffects.TemporaryHeat) < 0) this.player.createStatusAffect(StatusAffects.TemporaryHeat, 0, 0, 0, 0);
            }
            else if (color == "green") {
                //Green poison
                this.outputText("\nThe greenish fluids splash over you, making you feel slimy and gross.  Nausea plagues you immediately - you have been poisoned!\n", false);
                if (this.player.findStatusAffect(StatusAffects.Poison) < 0) this.player.createStatusAffect(StatusAffects.Poison, 0, 0, 0, 0);
            }
            else if (color == "white") {
                //sticky flee prevention
                this.outputText("\nYou try to avoid it, but it splatters the ground around you with very sticky white fluid, making it difficult to run.  You'll have a hard time escaping now!\n", false);
                if (this.player.findStatusAffect(StatusAffects.NoFlee) < 0) this.player.createStatusAffect(StatusAffects.NoFlee, 0, 0, 0, 0);
            }
            else if (color == "black") {
                //Increase fatigue
    ...
    protected goblinTeaseAttack(): void {
        ...
        if (det == 0) this.outputText(this.capitalA + this.short + " runs her hands along her leather-clad body and blows you a kiss. \"<i>Why not walk on the wild side?</i>\" she asks.", false);
        if (det == 1) this.outputText(this.capitalA + this.short + " grabs her heel and lifts it to her head in an amazing display of flexibility.  She caresses her snatch and gives you a come hither look.", false);
        if (det == 2) this.outputText(this.capitalA + this.short + " bends over, putting on a show and jiggling her heart-shaped ass at you.  She looks over her shoulder and sucks on her finger, batting her eyelashes.", false);
