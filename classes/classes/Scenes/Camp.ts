...
export class Camp extends NPCAwareContent {
    ...
    */
        ...
        */
        ...
        if (this.marbleScene.marbleFollower()) {
            ...
            if (this.player.cor < 50 && !this.campCorruptJojo() && !this.amilyScene.amilyCorrupt() && !this.vapulaSlave()
                && this.flags[kFLAGS.MARBLE_PURIFICATION_STAGE] == 0 && this.flags[kFLAGS.MARBLE_COUNTUP_TO_PURIFYING] >= 200
            ...
            if (this.flags[kFLAGS.MARBLE_PURIFICATION_STAGE] >= 5) {
                if (this.flags[kFLAGS.MARBLE_WARNED_ABOUT_CORRUPTION] == 0 && this.player.cor >= 50) {
                    this.hideMenus();
                    this.marblePurification.marbleWarnsPCAboutCorruption();
                    return;
                }
                if (this.flags[kFLAGS.MARBLE_WARNED_ABOUT_CORRUPTION] == 1 && this.flags[kFLAGS.MARBLE_LEFT_OVER_CORRUPTION] == 0 && this.player.cor >= 60) {
                    this.hideMenus();
        ...
        if (this.player.hasKeyItem("Super Reducto") < 0 && this.milkSlave() && this.player.findStatusAffect(StatusAffects.CampRathazul) >= 0 && this.player.statusAffectv2(StatusAffects.MetRathazul) >= 4) {
            this.hideMenus();
            ...
            if (kGAMECLASS.helScene.pregnancy.isPregnant) {
                switch (kGAMECLASS.helScene.pregnancy.eventTriggered()) {
                    ...
                    case 4: this.hideMenus();
                        ...
                        return;
                    default:
                        if (kGAMECLASS.helScene.pregnancy.incubation == 0 && (this.model.time.hours == 6 || this.model.time.hours == 7)) {
                            this.hideMenus();
        ...
        if (this.model.time.hours >= 10 && this.model.time.hours <= 18 && (this.model.time.days % 20 == 0 || this.model.time.hours == 12) && this.flags[kFLAGS.HELSPAWN_DADDY] == 2 && this.helSpawnScene.helspawnFollower()) {
            this.hideMenus();
            this.helSpawnScene.maiVisitsHerKids();
            return;
        }
        if (this.model.time.hours == 6 && this.flags[kFLAGS.HELSPAWN_DADDY] == 1 && this.model.time.days % 30 == 0 && this.flags[kFLAGS.SPIDER_BRO_GIFT] == 0 && this.helSpawnScene.helspawnFollower()) {
            this.hideMenus();
            this.helSpawnScene.spiderBrosGift();
            return;
        }
        if (this.model.time.hours >= 10 && this.model.time.hours <= 18 && (this.model.time.days % 15 == 0 || this.model.time.hours == 12) && this.helSpawnScene.helspawnFollower() && this.flags[kFLAGS.HAKON_AND_KIRI_VISIT] == 0) {
            this.hideMenus();
            this.helSpawnScene.hakonAndKiriComeVisit();
            return;
        }
        if (this.flags[kFLAGS.HELSPAWN_AGE] == 2 && this.flags[kFLAGS.HELSPAWN_DISCOVER_BOOZE] == 0 && (Camp.rand(10) == 0 || this.flags[kFLAGS.HELSPAWN_GROWUP_COUNTER] == 6)) {
            this.hideMenus();
        ...
        if (this.flags[kFLAGS.HELSPAWN_AGE] == 2 && (this.model.time.hours == 6 || this.model.time.hours == 7) && this.flags[kFLAGS.HELSPAWN_GROWUP_COUNTER] == 7 && this.flags[kFLAGS.HELSPAWN_FUCK_INTERRUPTUS] == 1) {
        ...
        if (!kGAMECLASS.nieveHoliday() && this.model.time.hours == 6 && this.flags[kFLAGS.NIEVE_STAGE] > 0) {
        ...
        if (this.flags[kFLAGS.FUCK_FLOWER_KILLED] == 0 && this.flags[kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] > 0) {
        ...
        //Jojo treeflips!
        if (this.flags[kFLAGS.FUCK_FLOWER_LEVEL] >= 4 && this.flags[kFLAGS.FUCK_FLOWER_KILLED] == 0 && this.player.findStatusAffect(StatusAffects.PureCampJojo) >= 0) {
        ...
        //Exgartuan clearing
        if (this.player.statusAffectv1(StatusAffects.Exgartuan) == 1 && (this.player.cockArea(0) < 100 || this.player.cocks.length == 0)) {
        ...
        if (this.flags[kFLAGS.ISABELLA_MURBLE_BLEH] == 1 && this.isabellaFollower() && this.player.findStatusAffect(StatusAffects.CampMarble) >= 0) {
            this.isabellaFollowerScene.angryMurble();
            this.hideMenus();
            return;
        }
        //Cotton preg freakout
        if (this.player.pregnancyIncubation <= 280 && this.player.pregnancyType == PregnancyStore.PREGNANCY_COTTON &&
            this.flags[kFLAGS.COTTON_KNOCKED_UP_PC_AND_TALK_HAPPENED] == 0 && (this.model.time.hours == 6 || this.model.time.hours == 7)) {
            kGAMECLASS.telAdre.cotton.goTellCottonShesAMomDad();
            this.hideMenus();
            return;
        }
        //Bimbo Sophie finds ovi elixer in chest!
        if (this.bimboSophie() && this.hasItemInStorage(this.consumables.OVIELIX) && Camp.rand(5) == 0 && this.flags[kFLAGS.TIMES_SOPHIE_HAS_DRUNK_OVI_ELIXIR] == 0 && this.player.gender > 0) {
            this.sophieBimbo.sophieEggApocalypse();
            this.hideMenus();
            return;
        }
        //Amily + Urta freakout!
        if (!kGAMECLASS.urtaQuest.urtaBusy() && this.flags[kFLAGS.AMILY_VISITING_URTA] == 0 && Camp.rand(10) == 0 && this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00146] >= 0 && this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00147] == 0 && this.flags[kFLAGS.AMILY_NEED_TO_FREAK_ABOUT_URTA] == 1 && this.amilyScene.amilyFollower() && this.flags[kFLAGS.AMILY_FOLLOWER] == 1 && !this.amilyScene.pregnancy.isPregnant) {
        ...
        if (this.flags[kFLAGS.RATHAZUL_CORRUPT_JOJO_FREAKOUT] == 0 && Camp.rand(5) == 0 && this.player.findStatusAffect(StatusAffects.CampRathazul) >= 0 && this.campCorruptJojo()) {
        ...
        if (this.flags[kFLAGS.AMILY_NOT_FREAKED_OUT] == 0 && this.player.findStatusAffect(StatusAffects.CampMarble) >= 0 && this.flags[kFLAGS.AMILY_FOLLOWER] == 1 && this.amilyScene.amilyFollower() && this.marbleScene.marbleAtCamp()) {
            this.finter.marbleVsAmilyFreakout();
            this.hideMenus();
            return;
        }
        //Amily and/or Jojo freakout about Vapula!!
        if (this.vapulaSlave() && (this.player.findStatusAffect(StatusAffects.PureCampJojo) >= 0 || (this.amilyScene.amilyFollower() && !this.amilyScene.amilyCorrupt()))) {
            //Jojo but not Amily
            if (this.player.findStatusAffect(StatusAffects.PureCampJojo) >= 0 && !(this.amilyScene.amilyFollower() && !this.amilyScene.amilyCorrupt()))
                this.vapula.mouseWaifuFreakout(false, true);
            //Amily but not Jojo
            else if ((this.amilyScene.amilyFollower() && !this.amilyScene.amilyCorrupt())) this.vapula.mouseWaifuFreakout(true, false);
            //Both
        ...
        //Clear stuff
        if (this.player.findStatusAffect(StatusAffects.SlimeCravingOutput) >= 0) this.player.removeStatusAffect(StatusAffects.SlimeCravingOutput);
        ...
        if (this.isabellaFollower()) {
            ...
            if (this.model.time.days >= 20) this.outputText("  You've even managed to carve some artwork into the rocks around the camp's perimeter.", false);
        }
        //Live in-ness
        else {
            if (this.model.time.days < 10) this.outputText("Your campsite is fairly simple at the moment.  Your tent and bedroll are set in front of the rocks that lead to the portal.  You have a small fire pit as well.", false);
            else if (this.model.time.days < 20) this.outputText("Your campsite is starting to get a very 'lived-in' look.  The fire-pit is well defined with some rocks you've arranged around it, and your bedroll and tent have been set up in the area most sheltered by rocks.", false);
            else this.outputText("Your new home is as comfy as a camp site can be.  The fire-pit and tent are both set up perfectly, and in good repair, and you've even managed to carve some artwork into the rocks around the camp's perimeter.", false);
        }
        if (this.flags[kFLAGS.CLARA_IMPRISONED] > 0) {
            this.marblePurification.claraCampAddition();
        }
        //Nursery
        if (this.flags[kFLAGS.MARBLE_NURSERY_CONSTRUCTION] == 100 && this.player.findStatusAffect(StatusAffects.CampMarble) >= 0) {
        ...
        else this.outputText("  You have a number of traps surrounding your makeshift home, but they are fairly simple and may not do much to deter a demon.", false);
        ...
        //BIMBO SOPHAH
        if (this.bimboSophie() && this.flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 0) this.sophieBimbo.sophieCampLines();
        if (this.player.findStatusAffect(StatusAffects.CampMarble) >= 0) {
            ...
            //Marble is out!
            if (this.flags[kFLAGS.MARBLE_PURIFICATION_STAGE] == 4) this.outputText("Marble isn’t here right now; she’s still off to see her family.");
            //requires at least 1 kid, time is just before sunset, this scene always happens at this time if the PC has at least one kid.
            else if (this.flags[kFLAGS.MARBLE_KIDS] >= 1 && (this.model.time.hours == 19 || this.model.time.hours == 20)) {
            ...
            else if (this.model.time.hours == 6 || this.model.time.hours == 7) this.outputText("Marble is off in an open area to the side of your camp right now.  She is practicing with her large hammer, going through her daily training.");
            //after nightfall, scene always displays at this time unless PC is wormed
            else if (this.model.time.hours >= 21 && this.player.findStatusAffect(StatusAffects.Infested) < 0) {
            ...
            else if (this.flags[kFLAGS.MARBLE_KIDS] > 0 && this.model.time.hours < 19 && this.model.time.hours > 7) {
                //requires at least 6 kids, and no other parental characters in camp
                if (Camp.rand(2) == 0 && this.flags[kFLAGS.MARBLE_KIDS] > 5) this.outputText("Marble is currently tending to your kids, but she looks a bit stressed out right now.  It looks like " + Camp.num2Text(this.flags[kFLAGS.MARBLE_KIDS]) + " might just be too many for her to handle on her own...");
                //requires at least 4 kids
                else if (Camp.rand(3) == 0 && this.flags[kFLAGS.MARBLE_KIDS] > 3) this.outputText("Marble herself is in the camp right now, telling a story about her travels around the world to her kids as they gather around her.  The children are completely enthralled by her words.  You can't help but smile.");
                //Requires 2 boys
                ...
                else if (Camp.rand(3) == 0 && this.flags[kFLAGS.MARBLE_KIDS] - this.flags[kFLAGS.MARBLE_BOYS] > 1) this.outputText("Marble herself is involved in a play fight with two of your kids brandishing small sticks.  It seems that the <i>mommy monster</i> is terrorising the camp and needs to be stopped by the <i>Mighty Moo and her sidekick Bovine Lass</i>.");
                else if (Camp.rand(3) == 0 && this.flags[kFLAGS.MARBLE_KIDS] > 1) this.outputText("Marble herself is out right now; she's taken her kids to go visit Whitney.  You're sure though that she'll be back within the hour, so you could just wait if you needed her.");
                else {
            ...
            else if (this.temp == 0) this.outputText("Marble herself has gone off to Whitney's farm to get milked right now.  You're sure she'd be back in moments if you needed her.", false);
            else if (this.temp == 1) this.outputText("Marble herself has gone off to Whitney's farm to do some chores right now.  You're sure she'd be back in moments if you needed her.", false);
            else if (this.temp == 2) this.outputText("Marble herself isn't at the camp right now; she is probably off getting supplies, though she'll be back soon enough.  You're sure she'd be back in moments if you needed her.", false);
            else if (this.temp == 3) {
        ...
        if (this.player.findStatusAffect(StatusAffects.CampRathazul) >= 0) {
            if (this.flags[kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN] <= 1) {
                ...
                if (this.flags[kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN] == 1) this.outputText("  Some kind of spider-silk-based equipment is hanging from a nearby rack.  <b>He's finished with the task you gave him!</b>", false);
                this.outputText("\n\n", false);
            }
        ...
        if (this.amilyScene.amilyFollower() && this.flags[kFLAGS.AMILY_FOLLOWER] == 1) {
            if (this.flags[kFLAGS.FUCK_FLOWER_LEVEL] >= 4) this.outputText("Amily has relocated her grass bedding to the opposite side of the camp from the strange tree; every now and then, she gives it a suspicious glance, as if deciding whether to move even further.");
            else this.outputText("A surprisingly tidy nest of soft grasses and sweet-smelling herbs has been built close to your bedroll. A much-patched blanket draped neatly over the top is further proof that Amily sleeps here. She changes the bedding every few days, to ensure it stays as nice as possible.\n\n", false);
        }
        //Corrupt mousebitch!
        else if (this.amilyScene.amilyFollower() && this.flags[kFLAGS.AMILY_FOLLOWER] == 2 && this.flags[kFLAGS.FOLLOWER_AT_FARM_AMILY] == 0) {
        ...
        else if (this.flags[kFLAGS.AMILY_VISITING_URTA] == 1 || this.flags[kFLAGS.AMILY_VISITING_URTA] == 2) {
        ...
        if (this.campCorruptJojo() && this.flags[kFLAGS.FOLLOWER_AT_FARM_JOJO] == 0) this.outputText("From time to time you can hear movement from around your camp, and you routinely find thick puddles of mouse semen.  You are sure Jojo is here if you ever need to sate yourself.\n\n", false);
        //Pure Jojo
        if (this.player.findStatusAffect(StatusAffects.PureCampJojo) >= 0) this.outputText("There is a small bedroll for Jojo near your own, though the mouse is probably hanging around the camp's perimeter.\n\n", false);
        //Izma
        ...
        if (kGAMECLASS.desert.antsScene.phyllaWaifu()) {
            ...
            if (this.flags[kFLAGS.ANT_KIDS] > 0) this.outputText(" one of your many children exit the anthill to unload some dirt before continuing back down into the colony.  It makes you feel good knowing your offspring are so productive.");
            else this.outputText(" Phylla appear out of the anthill to unload some dirt.  She looks over to your campsite and gives you an excited wave before heading back into the colony.  It makes you feel good to know she's so close.");
            this.outputText("\n\n");
        }
        //Clear bee-status
        if (this.player.findStatusAffect(StatusAffects.ParalyzeVenom) >= 0) {
            this.dynStats("str", this.player.statusAffectv1(StatusAffects.ParalyzeVenom), "spe", this.player.statusAffectv2(StatusAffects.ParalyzeVenom));
        ...
        if (this.player.lust >= 100) {
            ...
            else if (this.flags[kFLAGS.UNABLE_TO_MASTURBATE_BECAUSE_CENTAUR] > 0 && this.player.isTaur()) {
        ...
        if (((this.player.findPerk(PerkLib.HistoryReligious) >= 0 && this.player.cor <= 66) || (this.player.findPerk(PerkLib.Enlightened) >= 0 && this.player.cor < 10)) && !(this.player.findStatusAffect(StatusAffects.Exgartuan) >= 0 && this.player.statusAffectv2(StatusAffects.Exgartuan) == 0)) baitText = "Meditate";
    ...
    public loversCount(): number {
        ...
        //Izma!
        if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00238] == 1 && this.flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 0) counter++;
        if (this.isabellaFollower() && this.flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 0) counter++;
        if (this.player.findStatusAffect(StatusAffects.CampMarble) >= 0 && this.flags[kFLAGS.FOLLOWER_AT_FARM_MARBLE] == 0) counter++;
    ...
    public campLoversMenu(): void {
        ...
        if (this.isabellaFollower() && this.flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 0) {
            ...
            if (this.model.time.hours >= 21 || this.model.time.hours <= 5) this.outputText("Isabella is sound asleep in her bunk and quietly snoring.", false);
            else if (this.model.time.hours == 6) this.outputText("Isabella is busy eating some kind of grain-based snack for breakfast.  The curly-haired cow-girl gives you a smile when she sees you look her way.", false);
            else if (this.model.time.hours == 7) this.outputText("Isabella, the red-headed cow-girl, is busy with a needle and thread, fixing up some of her clothes.", false);
            else if (this.model.time.hours == 8) this.outputText("Isabella is busy cleaning up the camp, but when she notices you looking her way, she stretches up and arches her back, pressing eight bullet-hard nipples into the sheer silk top she prefers to wear.", false);
            else if (this.model.time.hours == 9) this.outputText("Isabella is out near the fringes of your campsite.  She has her massive shield in one hand and appears to be keeping a sharp eye out for intruders or demons.  When she sees you looking her way, she gives you a wave.", false);
            else if (this.model.time.hours == 10) this.outputText("The cow-girl warrioress, Isabella, is sitting down on a chair and counting out gems from a strange pouch.  She must have defeated someone or something recently.", false);
            else if (this.model.time.hours == 11) this.outputText("Isabella is sipping from a bottle labelled 'Lactaid' in a shaded corner.  When she sees you looking she blushes, though dark spots appear on her top and in her skirt's middle.", false);
            else if (this.model.time.hours == 12) this.outputText("Isabella is cooking a slab of meat over the fire.  From the smell that's wafting this way, you think it's beef.  Idly, you wonder if she realizes just how much like her chosen food animal she has become.", false);
                ...
                if (this.amilyScene.amilyFollower() && this.flags[kFLAGS.AMILY_FOLLOWER] == 1 && this.flags[kFLAGS.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO] == 0)
                ...
                if (izzyCreeps[choice] == 0) this.outputText("is sitting down with Rathazul, chatting amiably about the weather.", false);
                else if (izzyCreeps[choice] == 1) this.outputText("is sitting down with Jojo, smiling knowingly as the mouse struggles to keep his eyes on her face.", false);
                else if (izzyCreeps[choice] == 2) this.outputText("is talking with Amily, sharing stories of the fights she's been in and the enemies she's faced down.  Amily seems interested but unimpressed.", false);
                else if (izzyCreeps[choice] == 3) this.outputText("is sitting down chatting with Amily, but the corrupt mousette is just staring at Isabella's boobs and masturbating.  The cow-girl is pretending not to notice.", false);
                else if (izzyCreeps[choice] == 4) this.outputText("is sitting down with Izma and recounting some stories, somewhat nervously.  Izma keeps flashing her teeth in a predatory smile.", false);
                else this.outputText("is sitting down and thumbing through a book.", false);
            }
            else if (this.model.time.hours == 14) this.outputText("Isabella is working a grindstone and sharpening her tools.  She even hones the bottom edge of her shield into a razor-sharp cutting edge.  The cow-girl is sweating heavily, but it only makes the diaphanous silk of her top cling more alluringly to her weighty chest.", false);
            else if (this.model.time.hours == 15) this.outputText("The warrior-woman, Isabella is busy constructing dummies of wood and straw, then destroying them with vicious blows from her shield.  Most of the time she finishes by decapitating them with the sharp, bottom edge of her weapon.  She flashes a smile your way when she sees you.", false);
            else if (this.model.time.hours == 16) this.outputText("Isabella is sitting down with a knife, the blade flashing in the sun as wood shavings fall to the ground.  Her hands move with mechanical, practiced rhythm as she carves a few hunks of shapeless old wood into tools or art.", false);
            else if (this.model.time.hours == 17) this.outputText("Isabella is sitting against one of the large rocks near the outskirts of your camp, staring across the wasteland while idly munching on what you assume to be a leg of lamb.  She seems lost in thought, though that doesn't stop her from throwing a wink and a goofy food-filled grin toward you.", false);
            else if (this.model.time.hours == 18) this.outputText("The dark-skinned cow-girl, Isabella, is sprawled out on a carpet and stretching.  She seems surprisingly flexible for someone with hooves and oddly-jointed lower legs.", false);
            else if (this.model.time.hours == 19) {
                //[(Izzy Milked Yet flag = -1)
                if (this.flags[kFLAGS.ISABELLA_MILKED_YET] == -1) this.outputText("Isabella has just returned from a late visit to Whitney's farm, bearing a few filled bottles and a small pouch of gems.", false);
            ...
            else if (this.model.time.hours == 20) this.outputText("Your favorite chocolate-colored cowgirl, Isabella, is moving about, gathering all of her scattered belongings and replacing them in her personal chest.  She yawns more than once, indicating her readiness to hit the hay, but her occasional glance your way lets you know she wouldn't mind some company before bed.", false);
        ...
        //MARBLE
        if (this.player.findStatusAffect(StatusAffects.CampMarble) >= 0 && this.flags[kFLAGS.FOLLOWER_AT_FARM_MARBLE] == 0) {
            this.temp = Camp.rand(5);
            this.outputText("A second bedroll rests next to yours; a large two handed hammer sometimes rests against it, depending on whether or not its owner needs it at the time.  ", false);
            //Normal Murbles
            if (this.flags[kFLAGS.MARBLE_PURIFICATION_STAGE] != 4) {
                //(Choose one of these at random to display each hour)
                if (this.temp == 0) this.outputText("Marble herself has gone off to Whitney's farm to get milked right now.", false);
                if (this.temp == 1) this.outputText("Marble herself has gone off to Whitney's farm to do some chores right now.", false);
                if (this.temp == 2) this.outputText("Marble herself isn't at the camp right now; she is probably off getting supplies, though she'll be back soon enough.", false);
                if (this.temp == 3) {
                    this.outputText("Marble herself is resting on her bedroll right now.", false);
                }
                if (this.temp == 4) {
                ...
                if (this.temp < 3) this.outputText("  You're sure she'd be back in moments if you needed her.", false);
                marbleEvent = this.marbleScene.interactWithMarbleAtCamp;
            }
            //Out getting family
            else this.outputText("Marble is out in the wilderness right now, searching for a relative.");
            this.outputText("\n\n", false);
        }
        //AMILY
        if (this.amilyScene.amilyFollower() && this.flags[kFLAGS.AMILY_FOLLOWER] == 1 && this.flags[kFLAGS.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO] == 0) {
            ...
            if (this.temp == 0) {
                ...
                if (this.player.findStatusAffect(StatusAffects.CampRathazul) >= 0) this.outputText(".  Rathazul glances over and immediately gets a nosebleed", false);
            }
            else if (this.temp == 1) this.outputText("slouching in the shade of some particularly prominent rocks, whittling twigs to create darts for her blowpipe", false);
            else if (this.temp == 2) this.outputText("dipping freshly-made darts into a jar of something that looks poisonous", false);
            else if (this.temp == 3) this.outputText("eating some of your supplies", false);
            else if (this.temp == 4) this.outputText("and she flops down on her nest to have a rest", false);
            else this.outputText("peeling the last strips of flesh off of an imp's skull and putting it on a particularly flat, sun-lit rock to bleach as a trophy", false);
            this.outputText(".\n\n", false);
    ...
    public campFollowers(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.CampRathazul) >= 0) {
            ...
            if (this.flags[kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN] <= 1) {
                this.outputText("Tucked into a shaded corner of the rocks is a bevy of alchemical devices and equipment.  The alchemist Rathazul looks to be hard at work with his chemicals, working on who knows what.", false);
                if (this.flags[kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN] == 1) this.outputText("  Some kind of spider-silk-based equipment is hanging from a nearby rack.  He's finished with the task you gave him!", false);
                this.outputText("\n\n", false);
            }
            else this.outputText("Tucked into a shaded corner of the rocks is a bevy of alchemical devices and equipment.  The alchemist Rathazul looks to be hard at work on the silken equipment you've commissioned him to craft.\n\n", false);
        }
        if (this.sophieFollower() && this.flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 0) {
            if (Camp.rand(5) == 0) this.outputText("Sophie is sitting by herself, applying yet another layer of glittering lip gloss to her full lips.\n\n");
            else if (Camp.rand(4) == 0) this.outputText("Sophie is sitting in her nest, idly brushing out her feathers.  Occasionally, she looks up from her work to give you a sultry wink and a come-hither gaze.\n\n");
            else if (Camp.rand(3) == 0) this.outputText("Sophie is fussing around in her nest, straightening bits of straw and grass, trying to make it more comfortable.  After a few minutes, she flops down in the middle and reclines, apparently satisfied for the moment.\n\n");
            else if (Camp.rand(2) == 0 || this.flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] == 0) {
                if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00282] > 0) this.outputText("Your platinum-blonde harpy, Sophie, is currently reading a book - a marked change from her bimbo-era behavior.  Occasionally, though, she glances up from the page and gives you a lusty look.  Some things never change....\n\n");
    ...
    private rest(): void {
        ...
        if (this.timeQ == 0) {
            ...
            if (this.player.findStatusAffect(StatusAffects.MarbleWithdrawl) >= 0) {
                ...
                if (this.player.findPerk(PerkLib.SpeedyRecovery) >= 0) this.fatigue(-1 * this.timeQ);
            }
            //REGULAR HP/FATIGUE RECOVERY
                ...
                if (this.player.findPerk(PerkLib.SpeedyRecovery) >= 0) this.fatigue(-2 * this.timeQ);
            }
        }
        else {
    ...
    private doWait(): void {
        ...
        if (this.timeQ == 0) {
            ...
            if (this.player.findStatusAffect(StatusAffects.MarbleWithdrawl) >= 0) {
                ...
                if (this.player.findPerk(PerkLib.SpeedyRecovery) >= 0) this.fatigue(-0.5 * this.timeQ);
            }
            //REGULAR HP/FATIGUE RECOVERY
            else {
                //fatigue
    ...
    public doSleep(clrScreen: boolean = true): void {
        if (kGAMECLASS.urta.pregnancy.incubation == 0 && kGAMECLASS.urta.pregnancy.type == PregnancyStore.PREGNANCY_PLAYER && this.model.time.hours >= 20 && this.model.time.hours < 2) {
        ...
        if (this.timeQ == 0) {
            ...
            if (this.player.hasCock() && this.followerShouldra() && this.player.statusAffectv1(StatusAffects.Exgartuan) == 1) {
            ...
            /******************************************************************/
            //Marble Sleepies
            if (this.marbleScene.marbleAtCamp() && this.player.findStatusAffect(StatusAffects.CampMarble) >= 0 && this.flags[kFLAGS.SLEEP_WITH] == "Marble" && this.flags[kFLAGS.FOLLOWER_AT_FARM_MARBLE] == 0) {
                ...
                else this.outputText("You curl up, planning to sleep for " + Camp.num2Text(this.timeQ) + " ", false);
    ...
    private farmFound(): boolean { //Returns true as soon as any known dungeon is found
        if (this.player.findStatusAffect(StatusAffects.MetWhitney) >= 0 && this.player.statusAffectv1(StatusAffects.MetWhitney) > 1) {
            if (this.flags[kFLAGS.FARM_DISABLED] == 0) return true;
            if (this.player.cor >= 70 && this.player.level >= 12 && this.getGame().farm.farmCorruption.corruptFollowers() >= 2 && this.flags[kFLAGS.FARM_CORRUPTION_DISABLED] == 0) return true;
        }
