...
export class Vapula extends NPCAwareContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        if (this.model.time.hours > 23) {
            if (this.vapulaSlave() && this.flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 0) {
                if (this.flags[kFLAGS.VAPULA_HAREM_FUCK] == 0) this.flags[kFLAGS.VAPULA_DAYS_SINCE_FED]++;
                else this.flags[kFLAGS.VAPULA_DAYS_SINCE_FED] = 0;
            }
            if (this.flags[kFLAGS.VAPULA_FOLLOWER] == .5 || this.flags[kFLAGS.VAPULA_FOLLOWER] == 1.5) this.flags[kFLAGS.VAPULA_FOLLOWER]++;
            this.flags[kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS]++;
        }
        if (this.vapulaSlave() && this.player.hasKeyItem("Demonic Strap-On") < 0 && this.player.gender == 2 && this.flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 0) {
    ...
    public timeChangeLarge(): boolean {
        if (this.flags[kFLAGS.VAPULA_FOLLOWER] >= 2.5 && this.model.time.hours == 6 && this.flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 0) {
            this.femaleVapulaRecruitmentPartII();
            return true;
        }
        if (this.model.time.hours == 2 && this.vapulaSlave() && this.flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 0 && this.flags[kFLAGS.VAPULA_DAYS_SINCE_FED] >= 5 && (this.player.hasCock() || (this.player.hasKeyItem("Demonic Strap-On") >= 0 && this.player.hasVagina()))) {
    ...
    public vapulaSlaveFlavorText(): void {
        ...
        if (choice == 0) this.outputText("Vapula is currently resting on a very rough bed of leaves she's gathered herself.  She sleeps quietly, her hand absent-mindedly laid on her vagina and the other one resting on her breasts.  A thin trickle of her juice is leaking out of her crotch.");
        if (choice == 1) this.outputText("Your succubus slut, Vaplua, is furiously stroking herself on her bed of leaves, her purple body shining in sweat.  She moans raggedly, often looking up to you with lust-consumed eyes.");
        if (choice == 2) {
            if (this.flags[kFLAGS.VAPULA_HAREM_FUCK] == 0) choice = 3;
            else this.outputText("Vapula is currently absent, probably getting screwed by some of the imps surrounding your camp in order to get her semen rations.  You're sure she would come back instantly if you ever needed to give her YOUR cum instead.");
        }
        if (choice == 3) this.outputText("Vapula is busy drinking a vial of milk that looks very much like the kind you'd usually find on imps.  Her purple cheeks seem to blush darker when she sees you; she awkwardly gulps the rest of the bottle, hiccups, and giggles softly, eyeing you with a coy smile and a lust-filled gaze.  The potion seems to have made her even more aroused than usual.");
        if (choice == 4) {
            this.outputText("Vapula is currently toying with ");
            if (this.player.hasKeyItem("Deluxe Dildo") >= 0) this.outputText("your deluxe dildo");
            else this.outputText("a dildo");
            this.outputText(", pumping as fast as she can.  In her feverish display of lust she doesn't even notice you.");
        }
        if (choice == 5) {
            if (this.flags[kFLAGS.VAPULA_HAREM_FUCK] == 0 || !(this.sophieBimbo.bimboSophie())) choice = 6;
            //if harem fucking is on)
            else this.outputText("Vapula is fingering herself while eating out Sophie, your harpy slut.  The dumb avian matron keeps moaning like she's in heat while the succubus furiously assaults her lush fuck-hole; both of their pair of wings flap in rhythm as they feel their pleasure rising in waves.");
        }
        if (choice == 6) {
            if (this.flags[kFLAGS.VAPULA_HAREM_FUCK] == 0 || this.flags[kFLAGS.IZMA_NO_COCK] > 0 || !this.izmaFollower()) choice = 7;
            //if Izma has cock and harem fucking is on)
            else this.outputText("Your succubus is currently very busy with Izma, your tigershark lover.  Izma is standing and hiding Vapula from your sight and you don't notice what Vapula is doing to her until you see her frantic bobbing movements, and hear Izma's hisses and growls of pleasure.  Obviously your succubus must get the cum she needs from somewhere.");
        }
        if (choice == 7) {
            //if C. Jojo and harem fucking is on)
            if (this.flags[kFLAGS.VAPULA_HAREM_FUCK] == 0 || !this.jojoScene.campCorruptJojo()) choice = 8;
            else {
                ...
                if (this.jojoScene.tentacleJojo()) this.outputText("tentacled ");
                this.outputText("mouse pet.  Vapula seems to be having a lot of fun with your fuck-toy!");
            }
        }
        if (choice == 8) {
            if (this.flags[kFLAGS.VAPULA_HAREM_FUCK] == 0 || !(this.amilyScene.amilyFollower() && this.amilyScene.amilyCorrupt())) choice = 9;
            //if C. Amily and harem fucking on)
            else this.outputText("Vapula is busy caressing your corrupted mousette, Amily while softly kissing her breasts.  Her fingers are buried in your fuck-toy's pussy, and Amily is moaning whorishly and doesn't even bother to stop when she looks at you, her helpless eyes betraying her lack of control over her own lust.");
        }
        if (choice == 9) {
            if (this.player.findStatusAffect(StatusAffects.CampRathazul) < 0) choice = 10;
            else this.outputText("Vapula is having a very active conversation with Rathazul, the alchemist.  They seem to be discussing the chemical properties of demonic mixtures and the various taints that could occur from their ingestion.  The succubus is holding a bottle of a purplish white fluid that appears to be her own milk, probably waiting for an analysis.");
        }
        if (choice == 10) this.outputText("Vapula is resting on her stomach, a buttplug shoved into her ass.  She doesn't seem to notice you watching her butt jiggle under the effect of her unconscious anal contractions.");
    }
    ...
    public mouseWaifuFreakout(amily: boolean = false, jojo: boolean = false): void {
        ...
        if (amily) {
            ...
            if (jojo) this.outputText("and ");
        }
        if (jojo) this.outputText("Jojo ");
        this.outputText("walk");
        if (!(amily && jojo)) this.outputText("s");
        ...
        //[if libido >=50]
        if (this.player.lib >= 50) this.outputText("\"<i>Well, as long as she swallows, I don't care,</i>\" you quip.");
        this.outputText("\n\n\"<i>That's it!</i>\"  The ");
        if (jojo && amily) this.outputText("mice are");
        else this.outputText("mouse is");
    ...
    public callSlaveVapula(output: boolean = true): void {
        ...
        if (this.player.hasVagina() && this.player.hasKeyItem("Demonic Strap-On") >= 0) {
            ...
            else if (output && !this.player.hasCock()) this.outputText("\n\nYou aren't suitably aroused to feed Vapula right now.");
        }
    ...
    private harvestMilk(): void {
        ...
        if (this.flags[kFLAGS.FARM_UPGRADES_REFINERY] == 0) this.outputText("“<i>It’ll sure as hell beat whatever disgusting thing the bitch would have me doing instead. Why don’t you make her build a concentration machine? Lethice has loads; makes one dose of cream go so much further.</i>”");
        else this.outputText("“<i>As you insist, [master]. It’ll sure as hell beat working.</i>”");
    ...
    private feedVapulaACupOfJizz(): void {
        ...
        if (this.player.balls > 0) this.outputText("Your " + this.ballsDescriptLight() + " churn");
        else this.outputText("Your " + this.cockDescript(0) + " twitches");
        ...
        if (this.player.cumQ() >= 1500) this.outputText(" until she looks six months pregnant");
    ...
    private vapulaThreesomeMenu(): void {
        ...
        if (this.player.hasCock() || (this.player.hasVagina() && this.player.hasKeyItem("Demonic Strap-On") >= 0)) {
            ...
            if (this.amilyScene.amilyFollower() && this.amilyScene.amilyCorrupt() && this.player.hasCock()) amily = this.vapulaAndAmilyThreesome;
            if (this.ceraphFollowerScene.ceraphIsFollower() && this.player.hasCock()) ceraph = this.vapulaCeraphThreesome;
    ...
    private vapulaCeraphThreesomePleaseBoth(): void {
        ...
        if (this.player.cumQ() >= 1500) this.outputText(" and swelling her belly until she looks ready to give birth to a full horde of imps");
        this.outputText(".");
        this.outputText("\n\nYou feel your ");
        if (this.player.balls > 0) this.outputText(this.ballsDescriptLight() + " twitch and your " + this.cockDescript(x) + " pulsate as they unload their");
        else this.outputText("your " + this.cockDescript(x) + " twitch and pulsate as it unloads its");
        ...
        if (this.player.balls > 0) this.outputText(this.ballsDescriptLight() + " hitting her butt-hole with a faint thud");
        else this.outputText("thudding up against her cushy butt-cheeks");
        ...
        //[if multicocks]
        if (this.player.cockTotal() > 1) this.outputText("  She expertly jerks each of your rods in turn, gently polishing them as they grow back into an erect state.");
        ...
        if (this.player.balls > 0) this.outputText(" as the girls keep fondling your " + this.ballsDescriptLight());
    ...
    private vapulaAndCeraphButtfuckTrainYeehaw(): void {
        ...
        if (this.silly()) this.outputText("screaming at the top of your lungs: \"<i>BUTT-FUCK! BUTT-FUCK! BUTT-FUCK!</i>\", soon joined by your horny anal partners");
        else this.outputText("groaning and panting as you ravage Ceraph's pucker with your junk");
        this.outputText(".\n\n");
        ...
        //[if balls]
        if (this.player.balls > 0) this.outputText("  As you relentlessly smash your groin against Ceraph's plump rump, your " + this.ballsDescriptLight() + " repeatedly hit her quivering vagina; they are soon utterly coated in fem-spunk.");
        ...
        if (this.player.cumQ() >= 1500) this.outputText("  She automatically clenches her anus in order to keep as much of your baby-batter inside as possible, but to no avail; torrents of white goo start spurting out of her rectum, splattering her legs and your groin.");
    ...
    private vapulaSophieThreeSome(): void {
        ...
        if (this.player.cockArea(x) > 75) this.outputText("you've filled the entirety of her colon");
        else this.outputText("your cock is already buried to the hilt, its mammoth girth bulging between her broad hips");
        ...
        if (this.player.balls > 0) this.outputText("balls and massages your ");
        this.outputText(this.buttDescript() + ".");
        //[if third cock]
        if (this.player.totalCocks() > 2) this.outputText("  Using her extra flexibility, she bends over and start jerking your third pole, the sweet handjob adding to your pure cocktail of pleasure.");
        ...
        if (this.player.cockTotal() < 3) this.outputText("hastily pull out and ");
        this.outputText("wave ");
        if (this.player.cockTotal() >= 3) this.outputText("one of ");
    ...
    private vapulaAndAmilyThreesome(): void {
        ...
        if (this.player.balls > 0) this.outputText(": her drooling pussy needs to be prepared because you're going to take her next");
        ...
        //[if not defurred]
        if (this.flags[kFLAGS.AMILY_NOT_FURRY] == 0) this.outputText(" and her fur starts shrinking, letting her rosy skin shine below");
        ...
        if (this.player.balls > 0) {
            ...
            if (this.player.cumQ() >= 1500) this.outputText("  Sticky globs of white goo are shot from the tip of your rod, effectively stuffing Vapula so full of your spooge that some of it drips out and rains down on Amily's face, sopping her with fluids. The succubus quivers in your arms, giving in to her lust before squirting a powerful spray of fem-spooge, splattering Amily even more.");
    ...
    public vapulaAssistsCeruleanSuccubus(): void {
        ...
        //[if multicock]
        if (this.player.cockTotal() > 1) this.outputText("  She suddenly seems to have an idea: leaning down to reach your second cock, she starts licking it, accompanying the savage thrusting of the blue demoness as she slurps your extra junk, giving you even more pleasure.");
        else this.outputText("  The blue demoness leans down and starts licking Vapula's moist pussy, intertwining her flexible tongue with yours and pleasuring the purple slut even more.");
        ...
        if (this.player.cockTotal() > 1) {
            ...
            //[if third cock]
            if (this.player.cockTotal() > 2) this.outputText("and your blue lover's mouth ");
        ...
        if (this.player.tallness < 60) this.outputText(", enclosing your head in her cleavage");
    ...
    private vapulaJojoThreesomes(): void {
        ...
        var tease = undefined;
        var train = undefined;
        if (this.player.hasVagina() || this.player.hasKeyItem("Demonic Strap-On") >= 0) tease = this.NTRSomeJojos;
        if (!this.player.hasCock()) { } else {
    ...
    private NTRSomeJojos(): void {
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.cockTotal() > 1) {
                ...
                //[if third cock]
                if (this.player.cockTotal() > 2) this.outputText("  Since the horny slut has a demonic vaginal capacity, you might as well fit more dick in it: in no time your third dick joins its colleague in this infernal vaginal assault.");
                this.outputText("  ");
            }
            ...
            if (this.jojoScene.tentacleJojo()) this.outputText("his tentacles start flying toward his ass, ");
            this.outputText("his-");
            this.outputText("\n\n\"<i>No.</i>\"");
            ...
            //[if second cock]
            if (this.player.cockTotal() > 1) this.outputText("  Your extra junk shoved down her love-tunnel also explodes in a milky torrent, blasting her fuck-hole full of spooge too.");
    ...
    private jojoButtFuckTrain(): void {
        ...
        if (this.player.cockArea(x) > 100) this.outputText("  You can actually feel his internal organs being bumped against your meat as your " + this.cockDescript(x) + " stretches his colon beyond its normal size and thrusts in and out like a charging mammoth.");
        ...
        if (this.jojoScene.tentacleJojo()) {
            ...
            //[if vagina]
            if (this.player.hasVagina()) this.outputText("  Last but not least, a soft cock-knock at the entrance of your " + this.vaginaDescript() + " signals an incoming intrusion.  If your mouth weren't so full of cock you would sigh in relief as your aching pussy is thoroughly penetrated.");
        }
        ...
        //[if second multicock]
        if (n1 >= 0) this.outputText(", as well as Vapula's love-tunnel");
        if (n2 >= 0) this.outputText(" and throat, ");
        if (n3 >= 0) this.outputText("and Jojo's too");
        ...
        //[if tentacle Jojo]
        if (this.jojoScene.tentacleJojo()) this.outputText("; his tentacles absolutely drench your every hole with greenish mousey-cum");
    ...
    private teaseVapula(): void {
        ...
        if (this.flags[kFLAGS.VAPULA_HAREM_FUCK] == 0 && (this.flags[kFLAGS.VAPULA_DAYS_SINCE_FED] == 0)) {
        ...
        //Requires: Vapula not fucking harem, teased the day before
        else if (this.flags[kFLAGS.VAPULA_HAREM_FUCK] == 0 && this.flags[kFLAGS.VAPULA_DAYS_SINCE_FED] == 1) {
            ...
            //[Hooves:
            if (this.player.lowerBody == LOWER_BODY_TYPE_HOOFED || this.player.isTaur()) this.outputText("\"<i>I would worship your feet, mistress.  I would clean your hooves with my tongue to a shine, tease out every scrap of dirt from every crevice.  And I would thank you for the privilege.</i>\"");
            //[Naga:
        ...
        //Third Tease
            ...
            //Naga:
            if (this.player.isNaga()) this.outputText("you slide your tail under and over her trunk, wrapping your coils around her and drawing her face into your stuffed crotch.");
            //Other:
            else if (this.player.isTaur() || this.player.isGoo() || this.player.isDrider()) this.outputText("you sit the monstrous half of your body down comfortably and draw her face into your stuffed cotch.");
            else this.outputText("you open your [legs] and wrap them around her head, drawing her face into your stuffed crotch.");
    ...
    public vapulaForceFeeds(): void {
        ...
        //[if no fuck harem is on and PC hasn't fed/fucked Vapula for 5 days that scene will trigger at night]
            ...
            if (this.player.balls > 0) this.outputText("  Your " + this.ballsDescriptLight() + " churn");
            else this.outputText("  Your " + this.cockDescript(x) + " twitches");
            ...
            if (this.player.cumQ() >= 1500) this.outputText(" until she looks 6 months pregnant.  At last, the cum-flow spilling through your urethra starts to ebb and your " + this.cockDescript(x) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your tool appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
            this.player.orgasm();
            this.dynStats("cor", 1);
            this.flags[kFLAGS.VAPULA_DAYS_SINCE_FED] = 0;
            this.flags[kFLAGS.VAPULA_TEASE_COUNT] = 0;
    ...
    private spankVapulaLikeABoss(): void {
        ...
        if (this.player.hasCock()) this.outputText(" and a healthy surge of blood to [eachCock]");
        else if (this.player.hasVagina()) this.outputText(" and a teltale moistness to your nethers");
        this.outputText(".");
        ...
        if (this.camp.companionsCount() > 2) this.outputText(", drawing the eyes of your other companions.");
        else if (this.camp.companionsCount() == 2) this.outputText(", drawing the attention of your other companion.");
        else this.outputText(", echoing off the sentinel-like rocks that seem to ring your encampment.");
        ...
        if (this.player.findPerk(PerkLib.Sadist) >= 0) this.outputText("  Dishing out all this suffering is getting you a little hot under the collar, and you squirm a bit in place in anticipation of the pain to come.");
        ...
        else this.outputText("tingling anus");
