...
export class UrtaPregs extends NPCAwareContent {
    ...
    */
    ...
    public urtaKids(): number {
        return this.flags[kFLAGS.URTA_KIDS_MALES] + this.flags[kFLAGS.URTA_KIDS_FEMALES] + this.flags[kFLAGS.URTA_KIDS_HERMS];
    }

    private urtaSexMenu(): void {
        var spank = undefined;
        if ((this.player.hasCock() && this.player.cockThatFits(this.urta.urtaCapacity()) >= 0) || this.player.hasKeyItem("Deluxe Dildo") >= 0) spank = this.urta.spankTheShitOutOfUrtaAndMakeHerCreamHerselfFromProstateStimulationAloneLikeTheHornyDrunkenSlutSheReallyIs;
        //NOT Drunk
    ...
    public urtaPregAppearance(): void {
        switch (this.pregnancy.event) {
            case 1:
            case 2: this.outputText("Urta is sitting at her usual table, looking more clear-eyed and sober than usual.  She looks extremely happy, tail wagging gently behind her, and she gives you a thrilled look when she sees you, smiling blissfully and patting her belly in a meaningful gesture.");
                break;
            case 3:
            case 4:
            case 5:
            case 6:
            case 7: this.outputText("Urta has moved from her usual table to a more private stall, apparently for the comfort of its padded seats.  Her bulging belly makes it obvious she's pregnant - and the wide grin that seems fixed to her face makes it obvious she's ecstatic about it.");
                break;
            case 8:
            case 9: this.outputText("Urta's pregnancy has caused her to outgrow the stall; she's moved herself to an entirely new position in a quiet, out of the way part of the bar.  Looking at the sheer size of her, she's doubtlessly going to pop any day now.");
            default:
        }
    }
    ...
    private IAintGotTimeForUrtaBirfs(): void {
        this.clearOutput();
        this.outputText("The centaur looks perplexed at your refusal.  \"<i>I - what?  Really?</i>\"");
        ...
        this.flags[kFLAGS.URTA_TIMES_BIRTHED]++;
        this.flags[kFLAGS.URTA_LATESTBORN_GENDER] = UrtaPregs.rand(3) + 1;
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] == 1) this.flags[kFLAGS.URTA_KIDS_MALES]++;
        else if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] == 2) this.flags[kFLAGS.URTA_KIDS_FEMALES]++;
        else this.flags[kFLAGS.URTA_KIDS_HERMS]++;
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] == 3 || this.flags[kFLAGS.URTA_LATESTBORN_GENDER] == 1) {
            if (UrtaPregs.rand(2) == 0) this.flags[kFLAGS.URTA_LATESTBORN_COCKTYPE] = 2;
            else this.flags[kFLAGS.URTA_LATESTBORN_COCKTYPE] = 1;
        }
        //If firstborn, set as current.
        if (this.urtaKids() == 1) {
            this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] = this.flags[kFLAGS.URTA_LATESTBORN_GENDER];
    ...
    public getUrtaLetter(): void {
        ...
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] == 1) this.outputText("son");
        else if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] == 2) this.outputText("daughter");
        else this.outputText("hermaphrodite daughter");
        this.outputText(", though.\n");
        this.flags[kFLAGS.NEED_URTA_LETTER] = 0;
    }
    ...
    private goVisitUrtaBirfs(withUrta: boolean = false): void {
        ...
        this.flags[kFLAGS.URTA_PREGNANT_DELIVERY_SCENE] = 0;
        this.flags[kFLAGS.NEW_BABY_ANNOUNCED] = 0;
        if (!withUrta) this.outputText("The trip to the clinic where Urta is giving birth to your child is a blur; all that matters is getting there as fast as possible.  The centaur abandons you at the hospital doors, and you make your own way inside, approaching the counter inside.");

        //[1st time:
        if (this.flags[kFLAGS.URTA_TIMES_BIRTHED] == 0) {
            this.outputText("\n\nA perky-looking young female mouse-morph, her nurse's outfit stretched a little tighter over her breasts than is considered professional, a small name tag with \"Splinter\" clipped onto one hem, looks up at you with a welcoming grin.  \"<i>Hello, " + this.player.mf("mister", "miss") + "; please state the nature of the problem.</i>\"  You explain your reason and her eyes widen with understanding.  \"<i>Ah, I see.  Please, follow me, [name]; I will take you to Urta's room.</i>\"  She promptly gets up out of her seat - allowing you to see a rather unladylike bulge at her crotch, though you have no idea if she's a herm or merely an effeminate male - and starts walking, with you following close behind.");
        }
        //Else:
        else {
        ...
        if (gender == 2) this.outputText("girl!");
        else if (gender == 1) this.outputText("boy!");
        else this.outputText("boy...?  Well... isn't that unusual, looks like you actually gave birth to a healthy herm!");
        this.outputText("</i>\"");


        //If Boy or Herm: FoxlingCockCheck
        if (gender == 1 || gender == 3) {
            if (UrtaPregs.rand(2) == 0) {
                this.outputText("\n\n\"<i>Well, now... ");
                if (gender == 3) this.outputText("s");
                this.outputText("he's a perfectly normal little fox,</i>\" the doctor notes, and you can catch a quick glimpse of a somewhat bigger than normal vulpine sheathe on your ");
                if (gender == 1) this.outputText("son");
                else this.outputText("daughter");
                this.outputText("'s groin from where you are.");
                this.flags[kFLAGS.URTA_LATESTBORN_COCKTYPE] = 2;
            }
            //[Horse Dick:
            else {
                this.outputText("\n\n\"<i>...My, my; looks like ");
                if (gender == 3) this.outputText("s");
                this.outputText("he takes after ");
                if (gender == 3) this.outputText("her");
                else this.outputText("his");
                this.outputText(" mother,</i>\" the doctor comments.  Snatching a quick glance reveals the leathery sheath of an equine phallus swaying between the newborn's legs.  Although infantile, it already promises that ");
                if (gender == 1) this.outputText("he");
                else this.outputText("she");
                this.outputText(" is going to be quite a big ");
                if (gender == 1) this.outputText("boy when he grows up.");
                else this.outputText("girl when she grows up.");
                this.flags[kFLAGS.URTA_LATESTBORN_COCKTYPE] = 1;
            }
        }
        ...
        this.outputText(" to the breast, watching with joyous awe as the little fox-morph roots for her black nipple and then latches on.  Tears begin to roll down her cheeks at the sight.  \"<i>Our ");
        if (gender >= 2) this.outputText("daughter");
        else this.outputText("son");
        this.outputText("... look, [name], ");
        //[she's]
        if (gender >= 2) this.outputText("she's");
        else this.outputText("he's");
        this.outputText(" really here; our beautiful, beautiful little baby...</i>\" She sobs, ");
        if (this.urtaKids() == 0) this.outputText("overwhelmed by the fact she's a mother at long last");
        else this.outputText("still as delighted as ever to have a new addition to your growing brood");
        this.outputText(".");
        ...
        //set all appropriate birth variables here.
        this.flags[kFLAGS.URTA_TIMES_BIRTHED]++;
        this.flags[kFLAGS.URTA_LATESTBORN_GENDER] = gender;
        if (gender == 1) this.flags[kFLAGS.URTA_KIDS_MALES]++;
        else if (gender == 2) this.flags[kFLAGS.URTA_KIDS_FEMALES]++;
        else this.flags[kFLAGS.URTA_KIDS_HERMS]++;
        //If firstborn, set as current.
        if (this.urtaKids() == 1) {
            this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] = this.flags[kFLAGS.URTA_LATESTBORN_GENDER];
    ...
    private stayAfterUrtaBirf(): void {
        ...
        this.outputText("Of course, if that's what she wants, you'd be happy to oblige.  The vixen smiles at you thankfully.  \"<i>You won't regret it, [name]; I have an eventful day planned for us,</i>\" she finishes with a mischievous grin and a giggle of delight.");
    ...
    private wakeUpWithUrtaAfterStaying(): void {
        ...
        //PC Wakes with Urta
        this.outputText("As the morning sun shines on the blinds, you open your eyes.  Then you remember the events of the last day.  You spent the whole day with Urta, not having sex, just walking together and buying stuff for your newborn ");
        if (this.flags[kFLAGS.URTA_KIDS_MALES] == 1) this.outputText("son");
        else this.outputText("daughter");
        this.outputText(", then you went home and spent the better part of the evening making out with Urta which, of course, led to hot vixen sex in her bedroom.  After that... you probably fell asleep on her bed, and considering the current state of affairs, she didn't mind it one bit.");
        ...
        this.outputText("\n\nYou hold the shaft ");
        if (this.player.cor < 50) this.outputText("gently,");
        else this.outputText("tightly,");
        this.outputText(" earning a ");
        if (this.player.cor < 50) this.outputText("whimpering moan");
        else this.outputText("gasping shudder");
        ...
        if (this.urtaKids() == 1) this.outputText("a child");
        else this.outputText("children");
        this.outputText(" to look after now; I can't be leaving broken glass around, now can I?</i>\"  She puts her hands behind her head and stretches, perhaps not so coincidentally thrusting out her naked breasts.  \"<i>Mmm... now I feel awake.  Come on, lover; you're clearly in the mood, and last night was wonderful... ready for round two, hmm?</i>\"  She croons, wriggling herself over to sit right next to you and leaning onto your shoulder.");
        ...
        if (this.urtaKids() == 1) this.outputText("new ");
        else this.outputText("newest ");
        this.outputText("little monster,</i>\" she notes, already walking off to the nursery, indifferent to any need for clothes.  After all, it's not like the baby will mind that she's not wearing pants while feeding ");
        if (this.urtaKids() == 1) this.outputText("him");
        else this.outputText("her");
        this.outputText(".");
        if (this.urtaKids() == 2) this.outputText("  The other kid is staying at Edryn's, so there's no risk of you two needing to answer... complicated questions.");
        if (this.urtaKids() > 2) this.outputText("  The other kids are staying at Edryn's, so there's no risk of you two needing to answer... complicated questions.");
    ...
    private drinkSomeUrtaPostBirthTitMilk(): void {
        this.clearOutput();
        this.outputText("You decide to ask if Urta wouldn't mind you getting breakfast before going.  Your hand wanders to caress her unoccupied breast.  She moans appreciatively and then giggles.  \"<i>Naughty [name]... but, I'm okay with that.  Just let me finish up with this little one first; I can't juggle the two of you at the same time.</i>\"");
    ...
    public urtaPregooUpdates(): boolean {
        ...
        //3:
        else if (this.player.pregnancyIncubation == 360) {
        ...
        //4: 
            ...
            if (this.pregnancy.type != PregnancyStore.PREGNANCY_PLAYER) this.outputText("sounding somewhat jealous that it's you carrying and not her.");
            else this.outputText("sounding a little smug as she caresses her own gravid tummy.");
            this.outputText("  You shake your head, but considering how far you are into the pregnancy, it shouldn't take long... if Urta keeps visiting, you're pretty sure it'll be kicking the next time she comes around.");
        ...
        //5
        else if (this.player.pregnancyIncubation == 216) {
            ...
            if (this.urtaKids() == 0) this.outputText(" Oh, [name], we really are going to be parents...");
            this.outputText("</i>\" She babbles happily, tail wagging so enthusiastically you wonder if it's going to fall off.");
        ...
        else if (this.player.pregnancyIncubation == 72) {
    ...
    private getUrtaHelpPreggoPC(): void {
        this.clearOutput();
        this.outputText("Good, you hope this means she'll be having plenty of time, because you sure could use a fair share of vixen right now.  \"<i>As long as you want me, I'll be here.</i>\"  Urta laughs already starting to fiddle with the straps of her armor.  \"<i>Do you have any preferences for what we do?</i>\"");
    ...
    private getUrtaSexWhenPreggoz(): void {
        this.clearOutput();
        this.outputText("Well... you could use a break from minding your passenger, so how does she plan to help you take your mind off your current predicament?  You ask with a knowing smile.");
    ...
    private getAnUrtaMassageWhenPreggo(): void {
        this.clearOutput();
        this.outputText("You tell her that a backrub sounds fine, but you'd rather not lie down.  It could hurt the baby.  \"<i>Right, sorry, what was I thinking?  Well, I think I can be flexible...</i>\"  Urta grins, cracking her fingers. \"<i>Now, let's just find somewhere comfortable to put you first...</i>\"");
    ...
    private declineUrtaLevelSixPreggoHelps(): void {
        this.clearOutput();
        this.outputText("You thank Urta for the offer, but you'll have to decline.  You still have business to take care of, so you'll just rest for a while longer then be on your way.  Besides, it wouldn't look good for her to be absent for too long, she has a city to take care of.");
    ...
    private acceptUrtaLevelSixPreggoHelps(): void {
        this.clearOutput();
        this.outputText("You're feeling really tired right now, and besides, spending a whole day with Urta would be nice, so you gladly accept her offer.");
    ...
    public PCGivesBirf(): void {
        ...
        if (gender == 1) {
            ...
            this.outputText(" sheath between his legs that looks like it's going to be bigger than normal when he grows up.");
            this.flags[kFLAGS.URTA_KIDS_MALES]++;
            this.flags[kFLAGS.URTA_LATESTBORN_GENDER] = 1;
        }
        //Girl:
        else if (gender == 2) {
            ...
            this.flags[kFLAGS.URTA_LATESTBORN_GENDER] = 2;
        }
        //Herm:
        else {
            ...
            this.outputText(" sheath between her legs that looks like it's going to be bigger than normal when she grows up.");
            this.flags[kFLAGS.URTA_KIDS_HERMS]++;
            this.flags[kFLAGS.URTA_LATESTBORN_GENDER] = 3;
        ...
        this.outputText(" out here.  I know you wouldn't let anything bad happen to ");
        if (gender >= 2) this.outputText("her");
        else this.outputText("him");
        this.outputText(", but, well, you can't really take care of ");
        if (gender >= 2) this.outputText("her");
        else this.outputText("him");
        this.outputText(" properly in this wilderness, now can you?</i>\"");

        this.outputText("\n\nYou are eventually forced to admit that it would probably be better for the baby if ");
        if (gender >= 2) this.outputText("she");
        else this.outputText("he");
        this.outputText(" was brought up in Tel'adre.  Urta smiles thankfully and gives both of you one particularly eager cuddle.  \"<i>Thank you for understanding, [name].  ");
        //[She]
        if (gender >= 2) this.outputText("She");
        else this.outputText("He");
        this.outputText(" will be brought up safely, I promise.</i>\"  With that, she carefully bundles up the sleeping child and quietly vanishes into the wilderness, leaving you to take yourself off to bed for a proper rest.\n");

        //SET ALL PREGGOGOGOGOGOGO
        this.flags[kFLAGS.URTA_TIMES_PC_BIRTHED]++;
        //If firstborn, set as current.
        if (this.urtaKids() == 1) {
            this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] = this.flags[kFLAGS.URTA_LATESTBORN_GENDER];
            this.flags[kFLAGS.URTA_FIRSTBORN_COCKTYPE] = this.flags[kFLAGS.URTA_LATESTBORN_COCKTYPE];
    ...
    public urtaIsHappyAboutPregnancyAtTheBar(): void {
        this.clearOutput();
        this.flags[kFLAGS.URTA_INCUBATION_CELEBRATION] = 1;
    ...
    public urtaIsAPregnantCopScene(): void {
        ...
        if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) this.outputText("  Edryn's pregnant belly isn't helping her, either.");
    ...
    public urtaSpecialDeliveries(): void {
        ...
        this.outputText("As you enter through the gates, Edryn clops up to you, a small basket swinging from the crook of one arm");
        if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) this.outputText(" and her pregnant belly jiggling from the motions");
        this.outputText(", a faint smile on her face.  \"<i>Hey there, [name]; can you do me a favor, please?  Urta's been getting deliveries to her place, but I got an unexpected call, so I'm too busy to take her groceries to her today - you're her " + this.player.mf("boyfriend", "girlfriend") + "; can you please take this for me?</i>\"  The centauress asks.");
        ...
        if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) this.outputText("as quickly as a pregnant mare can, anyway,");
        this.outputText(" leaving you to head on to Urta's house.");
        ...
        if (this.pregnancy.event >= 7) this.outputText("heavily ");
        this.outputText("pregnant vixen is snoring softly, laying on her side under the covers, forming a sizable bump in the soft, fluffy quilt that practically envelops her, almost invisible amidst the big plump cushions that serve to keep her comfortable as she sleeps. ");
    ...
    public talkToUrtaAboutHerHouse(): void {
        this.clearOutput();
        this.outputText("You've been wondering why Urta keeps her old apartment if she has a nicer and bigger house now.  It would be much better for her to put the lease money towards something more productive than keeping that apartment...");
        if (this.player.cor < 50) this.outputText(" despite it having some emotional value to you.");
        ...
        if (this.urtaKids() > 1) this.outputText("or two ");
        this.outputText("pipes up asking what we're doing.</i>\"  She shakes her head with amused smirk.  \"<i>No, I'd rather keep the old place for our little love nest; we can have our fun there, then come home and be all respectable around the kids.</i>\"");
    ...
    private yesUrtaHouseSex(): void {
        this.clearOutput();
        this.outputText("You smile seductively at her and raise a brow.");
        ...
        if (this.player.lib > 50) this.outputText("  Sometimes you almost give up on waiting to get to her apartment and instead strip and do her right then and there... but it wouldn't bode well for the Watch captain to get a fine for public indecency.");
        ...
        if (!this.urtaLove()) this.urta.goBackToUrtasForLuvinzII();
        else {
            this.outputText("\n\n");
            this.urta.urtaHomeLuvLuvinsMenu();
        }
    }
    //[=No=]
    private noUrtaHouseSex(): void {
        this.clearOutput();
        this.outputText("You smile at her and apologize, you're just not in the mood right now.");
    ...
    private talkAboutLiana(): void {
        this.clearOutput();
        this.outputText("You tell Urta that you need to talk with her about something.");
        this.outputText("\n\n\"<i>Yes, [name]?</i>\"  The politely puzzled prick-vixen responds.");

        //{Had sex with Lianna:
        if (this.flags[kFLAGS.LIANNA_HAVESTED_LADIES] > 0 || this.flags[kFLAGS.LIANNA_HAVESTED_MALES] > 0) {
        ...
        if (this.urtaKids() > 1) this.outputText("ren have");
        else this.outputText(" has");
        this.outputText(" a special place in her heart.</i>\"");
    ...
    private preggoUrtaSmexOrSomething(): void {
        this.clearOutput();
        this.outputText("You let Urta know that you're for helping her vent some steam, if that's what she wants.");

        this.outputText("\n\nUrta grins lecherously, precum ");
        switch (this.pregnancy.event) {
            case 7: this.outputText("dripping wetly onto the floor");
                break;
            case 8:
            case 9: this.outputText("slopping into the filling bucket");
                break;
            default: this.outputText("staining her dress");
        }
        this.outputText(".  \"<i>Really?  Good, because I could use a little release... let's get back to my place, shall we?</i>\"  She starts to pull herself upright, prompting you to step in and lend her a hand.  She smiles and kisses you thankfully, letting you feel the swell of her belly and dick pressed against you as she does.  \"<i>So...lead on, lover</i>,\" she croons.");
    ...
    private massagePregnantUrtasBelly(): void {
        ...
        if (this.flags[kFLAGS.TIMES_MASSAGED_URTA_BELLY] == 0) this.outputText("  She looks at you and her newly cream-adorned belly contemplatively.  \"<i>I never knew my sweet tooth would bring me to this...</i>\"");
        ...
        if (this.player.tongueType == TONUGE_SNAKE) this.outputText("forked ");
        ...
        this.outputText("tongue hang and begin licking the cream off her breasts");
        if (this.player.tongueType > TONUGE_HUMAN) this.outputText(", a task that is made far easier due to your tongue's ability to stretch to a surprising length");
        this.outputText(".  You make sure to pay extra attention to her cream covered nubs.");
    ...
    private rubUrtaDick(): void {
        ...
        this.urtaMassageClimax();
    }

    //[=RubPussy=]
    private rubUrtaPussy(): void {
        this.clearOutput();
        this.outputText("You decide that by \"her,\" she could only mean her pussy.  So you stretch your arm, as if reaching for her shaft, then find your way under her furry balls to her waiting slit, gently touching and massaging her labia, teasing her clit with little pinches, forcing her wetness out of her slavering honeypot.  She lets out a surprised yelp, then laughs, \"<i>Not actually what I wanted... ooh!  But I'm not complaining...</i>\"");
    ...
    private urtaMassageClimax(): void {
        ...
        //Low Cor:
        if (this.player.cor < 33) this.outputText("  You walk up to her and carefully remove her condom, tying it up with a knot to prevent any of her seed from escaping, then dispose of it.");
        //Medium Cor:
        else if (this.player.cor < 66) this.outputText("Well, whatever, she's probably too sleepy to do anything about it either way.");
        //High Cor:
    ...
    private urtaBoobWorshipScene(): void {
        this.clearOutput();
        this.outputText("You gaze at Urta's breasts, heavy with their milky load, a few droplets already escaping her black nipples.  \"<i>Oh, you like these, do you?  You like a nice, big pair of foxy-boobs, huh?</i>\"  She jokes, jiggling them for your amusement, before wincing at their sensitivity.");

        this.outputText("\n\nYou can't help but note that Urta's breasts are pretty swollen.  Has she been milking herself lately?  Doesn't look like it.");

        //1st Time:
        if (this.flags[kFLAGS.TIMES_URTA_BOOB_WORSHIPPED] == 0) {
        ...
        this.flags[kFLAGS.TIMES_URTA_BOOB_WORSHIPPED]++;
        ...
        if (this.player.faceType == FACE_SHARK_TEETH || this.player.faceType == FACE_SNAKE_FANGS || this.player.faceType == FACE_SPIDER_FANGS) this.outputText(", always minding your sharp teeth");
        this.outputText(".  She yelps loudly, but makes no move to push you off, so she's obviously enjoying it.  You alternate between biting and pulling, never repeating the same motion twice.  \"<i>I- I'm gonna...!</i>\"  She gasps out, bucking and wriggling under you.  You smile wickedly and bite her nipple one last time, pulling and sucking on it.");
    ...
    private nurseFromMommaUrtasBooBees(): void {
        this.clearOutput();
        this.outputText("You look at Urta's breasts, full of milk and dripping, and absentmindedly lick your lips. Urta follows your gaze and gives you a knowing smile.  \"<i>You know, no matter how much I milk myself, I always seem to fill up again right away... I wonder if that's part of Taoth's so-called parting gift,</i>\" she muses to herself.");

        //1st Time:
        if (this.flags[kFLAGS.TIMES_NURSED_FROM_URTA] == 0) {
        ...
        this.outputText("tongue.  Urta lets out a sigh of pleasure, yipping softly as your tongue tantalizes her over-sensitive black nipples.  Noticing that she seemed to enjoy the licking you keep doing so, slathering her soft mounds in a thin layer of saliva, alternating between breasts.");
        if (this.player.tongueType > TONUGE_HUMAN) this.outputText("  Every few licks you coil your tongue around one of her nipples and gently pull at it, milking a thin strand of white goodness straight on your tongue, much to the fox-herm's delight.");
    ...
    private titWorshipAndStroke(): void {
        this.clearOutput();
        this.outputText("You smile mischievously around Urta's breast and eagerly grab her shaft with a hand, careful not to hurt her, but roughly enough for her to feel what you just did.  Urta lets out a surprised groan.  \"<i>Lover?  What are you...?</i>\"  She moans again as your fingers shift on her distended dick.");
        ...
        this.finishTitWorshipWivUrta();
    }
    //Else:
    private ignoreUrtaBonerWhenWorship(): void {
        this.clearOutput();
        this.outputText("You have other things to worry about right now, but you're glad she seems to be enjoying your ministrations.");
        this.finishTitWorshipWivUrta();
    }
    ...
    private milkPregnantUrta(): void {
        ...
        if (this.player.hasCock() && !this.player.isTaur() && this.player.cockThatFits(this.urta.urtaCapacity())) {
    ...
    private stayForHorseDickings(): void {
        this.clearOutput();
        this.outputText("You yawn yourself, and suggest that maybe Urta would like some company.  You're kinda sleepy yourself.  She smirks and reclines on the bed, \"<i>Help yourself, lover - there's plenty of me to cuddle,</i>\" she giggles softly as she pats her swollen belly.");
    ...
    private urtaPregCravings(): void {
        this.clearOutput();
        this.outputText("You decide to ask Urta if she has any particular cravings right now.  The pregnant fox visibly thinks about, then surreptitiously eyes your crotch.");
        //noDick:
        if (!this.player.hasCock()) {
        ...
        if (this.player.balls > 0) this.outputText(" with your [balls]");
        this.outputText(".  You smile apologetically down at her and ask her if she's found out what she's been craving yet.  \"<i>What do you think?</i>\"  She mumbles sarcastically, a playful twinkle in her eye, then starts noisily slurping and sucking, tongue frenziedly lapping at your intruding dick and her eyes rolling shut in obvious enjoyment as she gulps down the precum she's eagerly coaxing from you.");

        this.outputText("\n\nYou moan and try your best not to just grab onto her head and face-fuck her.  ");
        if (this.player.cockTotal() > 2) this.outputText("The rest of your " + this.multiCockDescriptLight() + " are as erect as your [cock biggest] dribbling pre along their lengths as they slap Urta during her frenzied bobs.  ");
        this.outputText("You tell Urta that you're getting close; if she keeps up with this you won't last much longer.");
        ...
        if (this.player.balls > 0 || this.player.hasVagina()) {
            this.outputText(", a hand playing expertly with ");
            if (this.player.balls > 0) {
                this.outputText("your balls");
                if (this.player.hasVagina()) this.outputText(" and another with ");
            }
            if (this.player.hasVagina()) this.outputText("your pussy");
        }
    ...
    private getBlownByPregnantUrta(): void {
        this.clearOutput();
        this.outputText("If she wants it so bad, she can have it, you think to yourself, bucking against her hand as you finally feel ");
        if (this.player.balls > 0) this.outputText("your balls churn and ");
        this.outputText("cum travelling down your urethra, expanding your shaft with the force of their passage, and finally jetting out of your [cockHead biggest] to arch gracefully through the air and splatter against Urta's face.");
        ...
        if (this.player.cockTotal() > 1) {
            this.outputText("  Even if it did, you wouldn't be able to do anything about your other cock");
            if (this.player.cockTotal() > 2) this.outputText("s as they join");
            else this.outputText(" as it joins");
            this.outputText(" in on the onslaught.");
        }
        ...
        //(Low Cum Amount)
        if (this.player.cumQ() < 250) {
            this.outputText("  Soon, all too soon, you're spent.  Urta is left with strands of cum covering her naked body, only a few having made it inside her open maw.  You pant as you slump down, satisfied with the outcome.");
    ...
    private holdOnYouAintGettingYerCumYetYouNaughtyFox(): void {
        this.clearOutput();
        this.outputText("You're not going to give in so easily.  If she wants it she'd best work hard for it.  You smile at her mischievously, even as you hump against her grip.");
        ...
        if (this.player.cockTotal() > 1) {
            ...
            if (this.player.cockTotal() > 2) this.outputText("s bulging ominously as they too prepare to deposit their loads on the willing fox-herm.");
            else this.outputText(" bulging ominously as it too prepares to deposit its load on the willing fox-herm.");
        }
        this.outputText("  Urta eagerly gulps and slurps, doing her best to swallow every drop.");

        //(Low Cum Amount)
        if (this.player.cumQ() < 250) {
            this.outputText("\n\nYou feed her all you can, ");
            if (this.player.cockTotal() > 1) {
                this.outputText("even splashing some cum over her face as your other cock");
                if (this.player.cockTotal() == 2) this.outputText(" convulses");
                else this.outputText("s convulse");
                this.outputText(" in orgasm, ");
            }
            this.outputText("but ultimately you reach your limit and can do nothing but slump as she continues sucking on your [cock biggest] like a straw, hungry for your male milk.");
        ...
        //(Medium Cum Amount)
            ...
            if (this.player.cockTotal() > 1) {
                this.outputText("Your other cock");
                if (this.player.cockTotal() == 2) this.outputText(" contributes");
                else this.outputText("s contribute");
                this.outputText(" by making a complete mess of her as ");
                if (this.player.cockTotal() > 2) this.outputText("they randomly spatter");
                else this.outputText("it randomly spatters");
                this.outputText(" her with cum, not that she seems to mind.");
            }
            this.outputText("  You lay down and relax, letting the cock-hungry fox-herm suck you dry as you continue pumping your male milk.  By the time she's done her belly looks a little larger, and you know that some of that bloating is not entirely due to the baby.  You smile happily at the vixen hungrily slurping on your [cock biggest] despite there being nothing more for her to drink.");
        ...
        //(High Cum Amount)
            ...
            if (this.player.cockTotal() > 1) {
                this.outputText("  Your other cock");
                if (this.player.cockTotal() > 2) this.outputText("s shoot their loads");
                else this.outputText(" shoots its load");
                this.outputText(" erratically, some of it splattering against Urta's face, but most of it winds up making a complete mess of the room.");
            }
            ...
            if (this.player.hasKnot(this.player.biggestCockIndex())) this.outputText(", just above your knot,");
            this.outputText(" with both hands, trying to pinch it shut long enough to inhale a couple of deep breaths, then goes back to trying to drink every last drop; she must really have a hankering for what you're packing.");
        ...
        this.outputText(" drained of your cum.");
        ...
        if (this.player.cumQ() >= 500) this.outputText(", audibly gurgling");
        this.outputText(" belly, clearly feeling tired even though she wasn't the one being pleasured this time.");
    ...
    private weirdUrtaCravingsConclusion(): void {
        this.clearOutput();
        this.outputText("You have the distinct feeling that someone is groping you and something is wiggling inside your mouth... actually it feels like someone is lapping you inside your mouth.  You open your eyes to gaze at Urta's green eyes, as she continues kissing you.  She holds your [face] with both hands, closing her eyes as she deepens the kiss, then slowly lets go, opening her eyes and smiling.  \"<i>Morning, lover; sleep well?</i>\"  She laughs.");

        this.outputText("\n\nYou nod, then notice that she's ");
        if (this.player.cumQ() < 1000) this.outputText("completely clean.");
        else this.outputText("lacking the extra bit of paunch you gave her earlier.");
        this.outputText("  You gaze at her questioningly.  \"<i>I woke up before you and went and... shall we say cleaned myself up a little,</i>\" she replies, flicking her ears in the direction of the shower.");
    ...
    private urtaRaepsJoo(): void {
        ...
        if (this.player.hasCock()) this.outputText("Every time her swollen tip rubs against your prostate you feel droplets of pre escape your dilating cum-hole.  ");
        if (this.player.hasVagina()) this.outputText("Each slap of her balls against your [vagina] sends electric waves of pleasure that expand throughout your body.  ");
        this.outputText("You moan audibly, making it pretty clear you're getting off from this kind of treatment.  \"<i>See?  I knew you'd enjoy this too, now that I'm not so tense...</i>\"  She draws out the last word as she makes a particularly deep thrust inside of you.  \"<i>Mmm...  I can feel your sexy ass squeezing my dick; you're enjoying this, really, aren't you?</i>\"");
        ...
        if (this.player.cockTotal() > 0) {
            if (this.player.cockTotal() > 1) this.outputText("  [EachCock] tenses in unison");
            else this.outputText("  [EachCock] throbs, tensing in preparation");
            //(Low Cum Amount)
            if (this.player.cumQ() < 250) this.outputText(" and you unload on the floor, spilling rope after rope of cum, forming a small puddle where you stand.  An undeniable testament to your enjoyment of the kinkiness of the act, however small.");
            //(Medium Cum Amount)
            else if (this.player.cumQ() < 1000) this.outputText(" and you deliver your payload right there on the floor.  Jet after jet splash against it, and you have no choice but to continue moaning and spilling your cum as Urta remains firmly lodged in your depths, no longer able to move now that you're squeezing her shaft so deliciously tight.  By the time you're done, the puddle you've formed is big enough that you can see your own face on the reflection, enraptured in pleasure and enjoyment of Urta's equine meatstick.");
            //(High Cum Amount)
            else {
                this.outputText(" and you let your prodigious load escape the confines of your ");
                if (this.player.balls > 0) this.outputText("churning balls");
                else this.outputText("being");
                this.outputText(", splashing against the floor with such force that you form veritable streams of enjoyment in the rapidly expanding lake of perversion.  The pleasure of the kinky act, aided by your earlier drink, ensures you feel nothing but ultimate happiness for this brief moment.  By the time you've spilled the last drop, the lake has reached Urta's knees, and you pity the poor soul in charge of cleaning the back rooms.");
            }
        }
        if (this.player.hasVagina()) {
            ...
            this.outputText(" juices that paint Urta's balls, which are resting flush against your [vagina].  The spilled juice slides down Urta's balls");
            if (this.player.wetness() >= 4) this.outputText(" to join your previous discharge on");
            else this.outputText(" and down towards");
            this.outputText(" the floor, where it gathers in a ");
            this.outputText("puddle of its own.");
        }
        //Genderless:
        if (this.player.gender == 0) {
            ...
            this.outputText(" anal muscles milk Urta's shaft much like a pussy would.");
            if (this.player.ass.analWetness > 0) this.outputText(" It even secretes its own wetness to slide lovingly down the fox-herm's balls and drip on the floor, an undeniable proof of your enjoyment.");
        }
        ...
        if (this.player.hasCock()) {
            if (this.player.cumQ() < 250) puddle++;
            else if (this.player.cumQ() < 1000) puddle += 2;
            else puddle += 3;
        }
        if (puddle > 0) {
            if (puddle == 1) this.outputText("small");
            else if (puddle == 2) this.outputText("reasonable");
            else this.outputText("enormous");
            this.outputText(" puddle of cum you created");
        }
        else this.outputText("floor");
        this.outputText(" and close your eyes, letting sleep take you over even as the fox behind you drools on your back and continues to pound your ass raw.");
    ...
    private declineARimJob(): void {
        this.clearOutput();
        this.outputText("You tell her that you appreciate the offer, but you're going to sit this one out.  Though she tries to hide it, she looks relieved at your refusal; she probably isn't really comfortable with that level of kinkiness.  Instead, she eagerly captures your lips with hers, murmuring into your mouth as she devours your kiss, tongue sliding between your lips to caress your own tongue.");
        //Both choices converge here.
    ...
    private acceptUrtaRimJobbies(): void {
        this.clearOutput();
        this.outputText("Well... she is responsible for your state, and her broad vulpine tongue must feel amazing on your sensitive ass... so you nod in acceptance of her offer.  \"<i>All right then... you do know you're the only person I'd ever do this for, right?</i>\" Urta comments.  \"<i>Now, roll around and present yourself to me; you can't expect a pregnant lady to do a lot of bending, now can you?</i>\"  She smirks.");
        ...
        if (this.player.cockTotal() > 1) this.outputText("  Especially not if your " + this.multiCockDescriptLight() + " have anything to say about it, throbbing at the mere feeling of Urta's wet tongue.");
        else if (this.player.cockTotal() == 1) this.outputText("  Especially not if your [cock] has anything to say about it, throbbing at the mere feeling of Urta's wet tongue.");
        else if (this.player.hasVagina()) this.outputText("  Especially not if your [pussy] has any say in the matter, winking at Urta, hoping for a licking of its own.");
        this.outputText("  She slowly begins deepening the pseudo-kiss, pressing more and more of her wet tongue against you until she is taking slow, languid licks up and down the lengths of the chasm, slathering your burning skin with her cooling goo.");
    ...
    public visitKidsFirstTime(): void {
        this.clearOutput();
        this.outputText("You ask Urta if she can take you to see your child");
        if (this.urtaKids() > 1) this.outputText("ren");
        this.outputText(".");

        this.outputText("\n\n\"<i>You want to see ");
        if (this.urtaKids() == 1) this.outputText("our child");
        else this.outputText("our babies");
        this.outputText("?</i>\"  Urta asks, tail wagging excitedly at the thought.  \"<i>Why, of course, [name] - " + this.urtaKidsText("he'll", "she'll", "they'll") + " be so happy to see " + this.urtaKidsText("his", "her", "their") + " " + this.player.mf("father", "other-mother") + "!  Come on, let's get going!</i>\"  She insists, immediately getting up from her seat");
        if (this.flags[kFLAGS.URTA_TIME_SINCE_LAST_CAME] == 0) this.outputText(", erection already diminishing as she forgets about her horniness in her parental pride");
        this.outputText(".  You waste no time in following her; indeed, she seems liable to leave you behind in her excitement to go and see her offspring.");
        ...
        if (this.urtaKids() == 1) this.outputText("child");
        else this.outputText("children");
        this.outputText(".  One door leads out to the back courtyard, while another looks like it closes off stairs leading down to a basement.");
    ...
    private pickKidSceneHere(): void {
        ...
        //ANNOUNCE BEHBIES
        //Once per pregnancy?
        if ((this.pregnancy.type == PregnancyStore.PREGNANCY_PLAYER || (this.player.pregnancyIncubation < 300 && this.player.pregnancyType == PregnancyStore.PREGNANCY_URTA)) && this.flags[kFLAGS.NEW_BABY_ANNOUNCED] == 0) {
            this.newBabyComing();
        }
        ...
        else if (this.model.time.hours >= 12 && this.model.time.hours <= 14) {
            this.hugAttackScene();
        }
        //Take the kids for a walk - Special thanks to Belin
        //Randomly picked when selecting Kids option in Urta's house.
        else if (UrtaPregs.rand(2) == 0) this.takeTheKidsForWalk();
        //Bathtime
    ...
    private newBabyComing(): void {
        this.clearOutput();
        this.outputText("You announce to your ");
        if (this.urtaKids() == 1) this.outputText("child");
        else this.outputText(UrtaPregs.num2Text(this.urtaKids()) + " kids");
        this.outputText(" that you and Urta have something important to tell them.");

        //If UrtaKids <8:
        if (this.urtaKids() < 8) {
            this.outputText("\n\nAs ");
            if (this.urtaKids() == 1) {
                if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] >= 2) this.outputText("she");
                else this.outputText("he");
                this.outputText(" approaches");
            }
            else this.outputText("they gather");
            this.outputText(" and curiously look");
            if (this.urtaKids() == 1) this.outputText("s");
            this.outputText(" at you, Urta gives ");
            if (this.urtaKids() > 1) this.outputText("them");
            else if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] >= 2) this.outputText("her");
            else this.outputText("him");
            this.outputText(" a loving smile.");

            //Both Preg:
            if (this.pregnancy.type == PregnancyStore.PREGNANCY_PLAYER && this.player.pregnancyType == PregnancyStore.PREGNANCY_URTA) {
            ...
            //One person preg:
            else if (this.pregnancy.type == PregnancyStore.PREGNANCY_PLAYER || (this.player.pregnancyType == PregnancyStore.PREGNANCY_URTA)) {
                this.outputText("\n\n\"<i>You're going to have a little brother or sister soon!</i>\" she says, beaming with delight as she pats ");
                if (this.pregnancy.type == PregnancyStore.PREGNANCY_PLAYER) this.outputText("her");
                else this.outputText("your");
                this.outputText(" belly.");
            }
            this.outputText("\n\nYour child");
            if (this.urtaKids() > 1) this.outputText("ren are");
            else this.outputText(" is");
            this.outputText(" silent for a moment, digesting this information.  Then ");
            if (this.urtaKids() == 1) this.outputText("a small fluffy bundle is");
            else this.outputText("several small fluffy bundles are");
            this.outputText(" doing ");
            if (this.urtaKids() == 1) this.outputText("its");
            else this.outputText("their");
            this.outputText(" best to hug you both; ");
            if (this.urtaKids() > 1) this.outputText("they");
            else if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] >= 2) this.outputText("she");
            else this.outputText("he");
            if (this.urtaKids() == 1) this.outputText(" is");
            else this.outputText(" are");
            this.outputText(" clearly delighted at the prospect.  You and Urta smile at each other over ");
            if (this.urtaKids() > 1) this.outputText("their");
            else if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] >= 2) this.outputText("her");
            else this.outputText("his");
            this.outputText(" head");
            if (this.urtaKids() > 1) this.outputText("s");
            this.outputText(", then return the embrace, pleased that ");
            if (this.urtaKids() > 1) this.outputText("they");
            else if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] >= 2) this.outputText("she");
            else this.outputText("he");
            if (this.urtaKids() == 1) this.outputText(" is");
            else this.outputText(" are");
            this.outputText(" excited at the prospect.");
        }
        //If UrtaKids =>8:
        else {
    ...
    private hugAttackScene(): void {
        this.clearOutput();
        this.outputText("Before you can exchange any further pleasantries the sound of running padded feet fills the house.  Lianna simply looks at the approaching foxling");
        if (this.urtaKids() > 1) this.outputText("s");
        this.outputText(" from the safety of the kitchen, leaving you to handle the incoming charge.  It takes a bit of effort to keep from losing your balance as your brood tackles you, hugging you in affection.  \"<i>All right, all right, let your " + this.player.mf("father", "other-mother") + " go, or do you not want to play?</i>\" Urta says, smirking with amusement as she closes the front door.  With a groan and a bit of prying the little fox");
        if (this.urtaKids() > 1) this.outputText("es do");
        else this.outputText(" does");
        this.outputText(" eventually clamor" + this.urtaKidsText("s", "s", "") + " off of you, but not before taking your hands in " + this.urtaKidsText("his", "her", "their") + " own.  ");
        if (this.urtaKids() > 1) this.outputText("They");
        else if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] >= 2) this.outputText("She");
        else this.outputText("He");
        this.outputText(" insistently pull");
        if (this.urtaKids() == 1) this.outputText("s");
        this.outputText(" you towards the backyard, while your lover chuckles at your predicament.");
    ...
    private stayToPlay(): void {
        this.clearOutput();
        this.outputText("You nod your head in acceptance.  Urta smiles in delight.  \"<i>Take a seat, please, make yourself comfortable - I'll go and get it,</i>\" she says happily, tail wagging as she saunters off to the kitchen.");

        //1st time:
        if (this.flags[kFLAGS.URTA_LUNCH_PLAY] == 0) {
            this.flags[kFLAGS.URTA_LUNCH_PLAY]++;
    ...
    private bathtime(): void {
        this.clearOutput();
        this.outputText("You tell Lianna you just wanted to check in on your ");
        if (this.urtaKids() > 1) this.outputText("kids");
        else if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] >= 2) this.outputText("daughter");
        else this.outputText("son");
    ...
    private giveTheKidsABath(): void {
        ...
        if (this.urtaKids() == 1) {
            if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] == 1) this.outputText("your son");
            else this.outputText("your daughter");
        }
        else this.outputText("the kids");
        this.outputText(".</i>\"  You contemplate what to do, then remove your outer layer of clothes, carefully placing them on a shelf in case they get splashed.  Now you just need to wait for Lianna to return.");

        this.outputText("\n\nLianna enters the bathroom followed by your ");
        if (this.urtaKids() == 1) {
            if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] == 1) this.outputText("son");
            else this.outputText("daughter");
        }
        else if (this.urtaKids() < 8) this.outputText("kids");
        else this.outputText("army of kids");
        this.outputText(".  \"<i>All right, ");
        if (this.urtaKids() > 1) this.outputText("everyone ");
        this.outputText("strip up and hop in.</i>\"  ");
        if (this.urtaKids() == 1) {
            if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] == 1) this.outputText("He");
            else this.outputText("She");
        }
        else this.outputText("They");
        this.outputText(" mutter" + this.urtaKidsText("s", "s", "") + " rebelliously, but then perk" + this.urtaKidsText("s", "s", "") + " up at the sight of you.  Immediately " + this.urtaKidsText("he", "she", "they") + " start" + this.urtaKidsText("s", "s", "") + " pulling off " + this.urtaKidsText("his", "her", "their") + " clothes and then, naked as the day " + this.urtaKidsText("he was", "she was", "they were") + " born, swarm" + this.urtaKidsText("s", "s", "") + " you, hugging you tightly.  You dole out comforting pats and playful strokes, then instruct " + this.urtaKidsText("him", "her", "them") + " to get into the tub.  Mournful eyes look at you, but, seeing you won't be swayed, " + this.urtaKidsText("he leaps", "she leaps", "they leap") + " with a splash into the bathtub.  Once in the water, " + this.urtaKidsText("he", "she", "they") + " start" + this.urtaKidsText("s", "s", "") + " to enjoy " + this.urtaKidsText("himself", "herself", "themselves") + ", merrily kicking and splashing about in what is, for " + this.urtaKidsText("him", "her", "them") + ", a decent-sized pool.  This gives you an opportunity to note the sex of your little rugrat" + this.urtaKidsText("", "", "s") + ".");
        ...
        if (this.flags[kFLAGS.URTA_KIDS_MALES] > 0) {
            ...
            if (this.flags[kFLAGS.URTA_KIDS_MALES] > 1) this.outputText("s");
            this.outputText(".");
            if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] == 1) {
                this.outputText("  Your firstborn is ");
                if (this.flags[kFLAGS.URTA_KIDS_MALES] == 1) this.outputText("the only one,");
                else this.outputText("among them,");
                this.outputText(" sporting a");
                if (this.flags[kFLAGS.URTA_FIRSTBORN_COCKTYPE] == 2) this.outputText(" vulpine penis");
                else this.outputText("n equine phallus, similar to what Urta has");
                this.outputText(".");
            }
            if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] == 1 && this.urtaKids() > 1) {
                this.outputText("  Your newest is ");
                if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] == 1) this.outputText("also ");
                this.outputText("a boy, with a");
                if (this.flags[kFLAGS.URTA_LATESTBORN_COCKTYPE] == 2) this.outputText(" vulpine penis");
                else this.outputText("n equine phallus, just like his mom's");
                this.outputText(".");
            }
            this.outputText("  You have a feeling ");
            if (this.flags[kFLAGS.URTA_KIDS_MALES] > 1) this.outputText("they're");
            else this.outputText("he's");
            this.outputText(" going to be quite developed when ");
            if (this.flags[kFLAGS.URTA_KIDS_MALES] > 1) this.outputText("they hit");
            else this.outputText("he hits");
            this.outputText(" puberty, if not as big as Urta.  Urta is quite proud of ");
            if (this.flags[kFLAGS.URTA_KIDS_MALES] > 1) this.outputText("them");
            else this.outputText("him");
            this.outputText(", and often boasts that ");
            if (this.flags[kFLAGS.URTA_KIDS_MALES] > 1) this.outputText("they");
            else this.outputText("he");
            this.outputText(" will grow up to be ");
            if (this.flags[kFLAGS.URTA_KIDS_MALES] > 1) this.outputText("strong fighters like she is.");
            else this.outputText("a strong fighter like she is.");
        }
        //If any UrtaKids=Herm:
        if (this.flags[kFLAGS.URTA_KIDS_HERMS] > 0) {
            this.outputText("\n\nYou have " + UrtaPregs.num2Text(this.flags[kFLAGS.URTA_KIDS_HERMS]) + " vulpine, hermaphroditic daughter");
            if (this.flags[kFLAGS.URTA_KIDS_HERMS] > 1) this.outputText("s");
            this.outputText(".");
            if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] == 3) {
                this.outputText("  Your firstborn is a hermaphrodite, sporting a");
                if (this.flags[kFLAGS.URTA_FIRSTBORN_COCKTYPE] == 2) this.outputText(" vulpine penis");
                else this.outputText("n equine phallus, a peculiarity she inherited from her mother");
                this.outputText(".");
            }
            if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] == 3 && this.urtaKids() > 1) {
                this.outputText("  Your latest addition is ");
                if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] == 3) this.outputText("also ");
                this.outputText("a herm, with a");
                if (this.flags[kFLAGS.URTA_LATESTBORN_COCKTYPE] == 2) this.outputText(" vulpine penis");
                else this.outputText("n equine phallus.  No doubt such a characteristic was passed from her mother");
                this.outputText(".");
            }
            this.outputText("  You have a feeling ");
            if (this.flags[kFLAGS.URTA_KIDS_HERMS] > 1) this.outputText("they're");
            else this.outputText("she's");
            this.outputText(" going to be quite developed when ");
            if (this.flags[kFLAGS.URTA_KIDS_HERMS] > 1) this.outputText("they hit");
            else this.outputText("she hits");
            this.outputText(" puberty, if maybe not quite as big as Urta.  Urta particularly dotes on ");
            if (this.flags[kFLAGS.URTA_KIDS_HERMS] > 1) this.outputText("them");
            else this.outputText("her");
            this.outputText(", maybe because she remembers how much her parents hated her for being a herm.");
        }
        //If any UrtaKids=Female:
        if (this.flags[kFLAGS.URTA_KIDS_FEMALES] > 0) {
            this.outputText("\n\nYou have " + UrtaPregs.num2Text(this.flags[kFLAGS.URTA_KIDS_FEMALES]) + " vulpine daughter");
            if (this.flags[kFLAGS.URTA_KIDS_FEMALES] > 1) this.outputText("s");
            this.outputText(".");
            if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] == 2) {
                this.outputText("  Your firstborn is a girl.");
            }
            if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] == 2 && this.urtaKids() > 1) {
                this.outputText("  Your latest addition is ");
                if (this.flags[kFLAGS.URTA_FIRSTBORN_GENDER] == 2) this.outputText("also ");
                this.outputText("a girl.");
            }
            this.outputText("  Urta loves to play with ");
            if (this.flags[kFLAGS.URTA_KIDS_FEMALES] > 1) this.outputText("them");
            else this.outputText("her");
            this.outputText(", and swears ");
            if (this.flags[kFLAGS.URTA_KIDS_FEMALES] > 1) this.outputText("they will");
            else this.outputText("she will");
            this.outputText(" be quite the heartbreaker");
            if (this.flags[kFLAGS.URTA_KIDS_FEMALES] > 1) this.outputText("s");
            this.outputText(" when ");
            if (this.flags[kFLAGS.URTA_KIDS_FEMALES] > 1) this.outputText("they grow");
            else this.outputText("she grows");
            this.outputText(" up.");
        }
        ...
        if (this.urtaKids() > 1) this.outputText("nearest ");
        this.outputText("furry little form of your child and gently tug them over to you.  ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("She");
        else this.outputText("He");
        this.outputText(" kicks and splashes with ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
        else this.outputText("his");
        this.outputText(" little legs, but smiles up at you, clearly willing to cooperate, giving you no problem as you spurt a generous dollop of the soapy goo into ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
        else this.outputText("his");
        this.outputText(" hair and start to massage it... now, just how do you shampoo somebody covered from head to toe in fur...?");

        this.outputText("\n\nLuckily Lianna arrives, carrying the folded clothes and naked... very naked.  She sits beside you and gently asks, \"<i>How's it going?  " + this.urtaKidsText("He", "She", "They") + " didn't try to splash you, did " + this.urtaKidsText("he", "she", "they") + "?</i>\"  Just as she finishes posing the question, your ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("daughter");
        else this.outputText("son");
        this.outputText(" flicks ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
        else this.outputText("his");
        this.outputText(" tail, splashing Lianna with water.  The skunkette, however, fails to display any reaction at this.");

        this.outputText("\n\nYou apologize for that, noting that " + this.urtaKidsText("he", "she", "they") + " got you the same way earlier.  You then flick your ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("daughter");
        else this.outputText("son");
        this.outputText("'s ear and tell ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
        else this.outputText("him");
        this.outputText(" to apologize to Lianna for that.  \"<i>It's all right, [name].  " + this.urtaKidsText("He's just a kid", "She's just a kid", "They're just kids") + ", you can't expect to bathe kids and not get wet.</i>\"  You think you have a pretty clear idea of why Urta picked Lianna to care for the children.  Casually, you ask if Lianna's got any younger siblings; she seems like she's got a lot of practice at this.  She shakes her head, \"<i>This is the first time I got a job where kids are involved.  Either way, it's a lot easier than taking care of minotaurs.</i>\"  You can't help but wonder if she really did somehow take care of minotaurs at one point, or if this is just some colloquial expression.");

        this.outputText("\n\n\"<i>Here, let me help you.</i>\"  She squeezes a handful of shampoo on her hands and begins scrubbing your ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("daughter");
        else this.outputText("son");
        this.outputText("'s chest, tickling ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
        else this.outputText("him");
        this.outputText(" sides as she does so.  The little fox giggles and squirms, clearly finding the actions quite ticklish, and you have to hold on a little harder to keep ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
        else this.outputText("him");
        this.outputText(" from escaping.  \"<i>Now all you have to do is rinse off.</i>\"  She grabs a small bucket nearby, scooping up some water.  \"<i>Close your eyes, dear.</i>\"  Your child does as Lianna instructs; this is evidently a familiar enough routine that ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("she");
        else this.outputText("he");
        this.outputText(" has no intention of winding up with soap in ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
        else this.outputText("his");
        this.outputText(" eyes.  Lianna turns the bucket, letting the water cascade over ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
        else this.outputText("his");
        this.outputText(" head.  Your little fox squeals and then shakes wildly to fling the water off of ");
        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
        else this.outputText("his");
        this.outputText(" head, kicking up a cascade of water as ");

        if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("she");
        else this.outputText("he");
        this.outputText(" does so");
        if (this.urtaKids() > 2) {
            this.outputText(", to the delight of ");
            if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
            else this.outputText("his");
            this.outputText(" siblings, who are safely watching from the other side of the bath");
        }
        this.outputText(".");
        ...
        this.outputText(" hair is done, shaking some of the water out of your own face.");
        ...
        if (this.urtaKids() == 1) {
            ...
            this.outputText(" tail.  Get out of the tub.</i>\"  The skunkette instructs.  The young fox nods and scrabbles at the side of the bath, hauling ");
            if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
            else this.outputText("him");
            this.outputText("self up and out so Lianna can start tending to ");
            if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
            else this.outputText("his");
            this.outputText(" tail.  You promptly make yourself useful and start scrubbing as well...");
        }
        else {
            this.outputText("\n\n\"<i>Good, now who's next?</i>\"  \"<i>Me!</i>\"  ");
            if (this.urtaKids() > 2) this.outputText("One of your other children");
            else this.outputText("Your other child");
            this.outputText(" pipes up.  You let go of the ");
            if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("girl");
            else this.outputText("boy");
            this.outputText(" you're currently holding so ");
            if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("she");
            else this.outputText("he");
            this.outputText(" can swim away, allowing the new volunteer to take ");
            if (this.flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2) this.outputText("her");
            else this.outputText("his");
            this.outputText(" place.  You promptly make yourself useful and start scrubbing as well...");
        }
    ...
    private noBathPleaseUrtaLian(): void {
        this.clearOutput();
        this.outputText("You politely refuse her invitation.  \"<i>Okay, but can you wash mine then?</i>\" she asks.  You give the idea a moment's thought and then agree; it's a harmless offer, right?  \"<i>Thanks,</i>\" she replies, getting out of the tub and sitting on a small wooden bench nearby.  \"<i>Use the flower extract shampoo, please.</i>\"  You nod your head in understanding as you climb out after her, taking up the indicated shampoo and approaching her before asking how much you should use.  \"<i>Plenty,</i>\" is her reply.");
    ...
    private getABackWashFromLianna(): void {
        this.clearOutput();
        this.outputText("You tell Lianna that if she is willing, you wouldn't object to her washing your back.");
    ...
    private takeTheKidsForWalk(): void {
        this.clearOutput();
        this.outputText("\"<i>You'd like to see " + this.urtaKidsText("him", "her", "them") + "?  Sure, but first I need to get " + this.urtaKidsText("him", "her", "them") + " ready.</i>\"   You ask her where she plans on taking " + this.urtaKidsText("him", "her", "them") + ".");

        this.outputText("\n\n\"<i>I'm just taking " + this.urtaKidsText("him", "her", "them") + " for a walk.  Kids need fresh air sometimes, too,</i>\"  Lianna states matter-of-factly, exiting the kitchen and heading off to the backyard.");

        this.outputText("\n\nYou suggest that you could take " + this.urtaKidsText("him", "her", "them") + " for a walk instead.  \"<i>You sure?  " + this.urtaKidsText("A kid", "A kid", "Kids") + " can be a handful sometimes... though yours usually behave" + this.urtaKidsText("s", "s", "") + ".</i>\"  You insist you'd like to do so; " + this.urtaKidsText("he", "she", "they") + " might enjoy having you along anyway.  \"<i>Fine, I'll just get " + this.urtaKidsText("him", "her", "them") + " ready and you can be on your way.</i>\"");

        this.outputText("\n\nMaking sure your child");
        if (this.urtaKids() > 1) this.outputText("ren");
        this.outputText(" are suitably dressed for a traipse around a desert city, you usher your kit");
        if (this.urtaKids() > 1) this.outputText("s");
        this.outputText(" out the door and into the street.  \"<i>Can " + this.urtaKidsText("I", "I", "we") + " get some ice cream?!</i>\"");
        if (this.urtaKids() > 1) this.outputText("  \"<i>Yeah, can we " + this.player.mf("daddy", "mommy") + "?!</i>\"");
        this.outputText("  Your child");
        if (this.urtaKids() > 1) this.outputText("ren plead");
        else this.outputText(" pleads");
        this.outputText(".  Well, the vendors down at the market would make for a suitably far destination, which should give Lianna plenty of time for herself, and you yourself wouldn't mind some ice cream either at that, so it's decided.  \"<i>Yay! You're the best " + this.player.mf("dad", "mom") + " ever!</i>\" ");
        if (this.urtaKids() > 1) this.outputText("a chorus of excited cheers sounds out.");
        else this.outputText("an excited cheer sounds out.");
        this.outputText("  It's hard not to want to live up to that title with such ");
        if (this.urtaKids() == 1) this.outputText("a happy face");
        else this.outputText("happy faces");
        this.outputText(" staring up at you.");

        this.outputText("\n\nYou take a meandering path to the markets, making sure you keep a vigilant eye on your curious offspring lest " + this.urtaKidsText("he", "she", "they") + " run off and get " + this.urtaKidsText("him", "her", "them") + "sel" + this.urtaKidsText("f", "f", "ves") + " into too much trouble.  Some of the denizens who recognize you, if only as Urta's mate, give you their regards.");
        ...
        if (this.urtaKids() > 1) this.outputText("they fan out and curiously inspect");
        else this.outputText(this.urtaKidsText("he", "she", "") + " runs off to curiously inspect");
        this.outputText(" various produce, clothing and trinket stalls.  " + this.urtaKidsText("He knows", "She knows", "They know") + " what " + this.urtaKidsText("he is", "she is", "they are") + " here for, however and " + this.urtaKidsText("doesn't", "doesn't", "don't") + " give you much of a chance to browse before a furred paw takes your hand and drags you off in the direction of the ice cream shop tucked away in a sheltered and cool part of the bazaar.  Once you're inside, you take a look around as your child");
        if (this.urtaKids() > 1) this.outputText("ren");
        this.outputText(" run");
        if (this.urtaKids() == 1) this.outputText("s");
        this.outputText(" up and press");
        if (this.urtaKids() == 1) this.outputText("es");
        this.outputText(" " + this.urtaKidsText("his face", "her face", "their faces") + " against the frosty glass of the counter, almost drooling in anticipation.  This shop must have some magical element to it to keep their frozen treats cold in the middle of the desert.  There's some credence to this as the shops owner leans over the counter to greet you.");
        ...
        if (this.urtaKids() > 1) this.outputText("ren");
        this.outputText(" pick out flavours while the shopkeeper scoops them up onto wafer cones.  Once he's done, you thank the wolf and leave the shop followed by your ");
        if (this.urtaKids() > 10) this.outputText("army of ");
        this.outputText("child");
        if (this.urtaKids() > 1) this.outputText("ren");
        this.outputText(".");

        this.outputText("\n\nYour child");
        if (this.urtaKids() > 1) this.outputText("ren");
        this.outputText(" don't bother waiting or savouring their ice cream and neither do you, it's damned hot walking around this desert.  There's less cause for mischief too, " + this.urtaKidsText("his", "her", "their") + " face");
        if (this.urtaKids() == 1) this.outputText("buried in cone");
        else this.outputText("s buried in their cones");
        this.outputText(" along with your own, greedily licking up melting droplets.  There's just a bit of cone left as you return to Urta's house, and you quickly polish that off, upon entering.  \"<i>Oh, hey.  You're back.</i>\"  Lianna greets you in her usual inexpressive fashion.");

        this.outputText("\n\n\"<i>How did it go?</i>\"");

        this.outputText("\n\nWithout a hitch, you reply.  The " + UrtaPregs.num2Text(this.urtaKids() + 1) + " of you stopped for some ice cream along the way.  \"<i>Good, now off you go.  Bathtime.</i>\"  She states, ushering your little fox");
        if (this.urtaKids() > 1) this.outputText("es");
        this.outputText(" towards the bathroom.  There is a " + this.urtaKidsText("shrill protest", "shrill protest", "chorus of shrill protests") + ", but " + this.urtaKidsText("he", "she", "they") + " clearly understand");
        if (this.urtaKids() == 1) this.outputText("s");
        this.outputText(" that Lianna is not one to be trifled with and so " + this.urtaKidsText("he", "she", "they") + " march");
        if (this.urtaKids() == 1) this.outputText("s");
        this.outputText(" off to the bathroom.  You can't help but note how good Lianna is at making kids do what she says.");
    ...
    public visitTheHouse(): void {
        ...
        //1st time only:
        if (this.flags[kFLAGS.FIRST_TIME_AT_URTA_HOUSE] == 0) {
            this.flags[kFLAGS.FIRST_TIME_AT_URTA_HOUSE]++;
    ...
    private talkToLianaAboutHerJob(): void {
        this.clearOutput();
        this.outputText("You ask her about her jobs, both for Urta and what she intends to do afterwards.");
        ...
        this.outputText("\n\n\"<i>Something else?</i>\"");
        //(Display Options)
        this.talkToLiana(false);
    }

    //Her Research
    private talkToLianaAboutHerResearch(): void {
        this.clearOutput();
        this.outputText("You ask her why she's so eager to get samples from you, a human.  What sort of research is she doing that she needs human samples?");
        ...
        if (this.player.cor >= 50) this.outputText("however half-heartedly it may be, ");
        this.outputText("and ask if that's what pushed her to studying humans and Marethians.");

        this.outputText("\n\nShe shrugs.  \"<i>I guess, though what I have is genuine curiosity.  Maybe someday one of your kids will ask why they don't look like " + this.player.mf("daddy", "mommy") + ".  Wouldn't you want to have an answer to that question?</i>\"  She smiles.");
    ...
    private talkAboutLiannasLubeSpray(): void {
        this.clearOutput();
        this.flags[kFLAGS.DISCUSSED_LUBE_SPRAY] = 1;
        this.outputText("You ask why it is that she produces a lubricating liquid rather than a noxious one, like you would expect of a skunk.");
    ...
    private lubeSpraySideEffects(): void {
        this.clearOutput();
        this.outputText("So, if she's experimented on herself in the past, that must have gone wrong for her at least once or twice in the past, you comment.");
        ...
        if (this.player.cor < 33) this.outputText("mild");
        else if (this.player.cor < 66) this.outputText("some");
        else this.outputText("great");
        this.outputText(" reluctance.");
    ...
    private sexWithLianna(): void {
        this.clearOutput();
        this.outputText("\"<i>So you want to help me with my experiments?  Great!</i>\"  She takes your hand and basically drags you into her room.  You allow her to lead you, wondering what she has in mind.");
        ...
        if (this.player.cockTotal() > 1) this.outputText("  Your lower garments suddenly feel much tighter.");
    ...
    private acceptARandomGenderFromASkunk(): void {
        ...
        if (UrtaPregs.rand(10) <= 3) {
            ...
            this.player.cocks[0].cockLength = 4;
            this.player.cocks[0].cockThickness = 0.75;
        ...
        //if PC becomes Female:
            ...
            if (this.player.biggestTitSize() < 1) {
                ...
                if (this.player.nippleLength < .5) {
                    this.outputText("  Your nipples erect into small half-inch nubs, complementing the transformation.");
        ...
        //if PC becomes Herm:
            ...
            if (this.player.biggestTitSize() < 3) {
                ...
                if (this.player.biggestTitSize() < 1) this.outputText("s");
                this.outputText(" swelling into a soft C-Cup.");
                if (this.player.nippleLength < 0.5) {
                    this.outputText("  Your nipples erect into small, half-inch nubs, complementing the transformation.");
                    this.player.nippleLength = 0.5;
                }
                this.outputText("  You watch the soft mounds in your chest, unable to do anything but pant.");
    ...
    private getEggsHarvested(): void {
        this.clearOutput();
        this.outputText("\"<i>All right, let's get this show on the road.</i>\"  She ducks out into her closet once more, and comes out carrying what looks like a dildo and a lollipop.  \"<i>So, let me explain.  It's pretty simple really.</i>\"  She takes the dildo in her hand.  \"<i>This is a special dildo I had custom-made for this experiment, It will pleasure you at the same time it lets me harvest your pussy juice and hopefully one of your ovum.  All you have to do is sit back and relax.  Any questions?</i>\"");
        ...
        //1st time:
        if (this.flags[kFLAGS.LIANNA_HAVESTED_LADIES] == 0) {
            this.outputText("confusion, wondering what she's playing at");
            this.flags[kFLAGS.LIANNA_HAVESTED_LADIES]++;
        }
        else this.outputText("remembrance, knowing what's coming next");
        this.outputText(".  She unceremoniously begins shoving it inside without even a hint of pleasure or discomfort; the dildo slides in without resistance, spreading her buttcheeks apart and penetrating her with a wet squelch.  You watch the display, not sure whether or not you should find this kinky.");
        ...
        if (this.player.vaginalCapacity() >= 50) this.outputText("  \"<i>Hmm, I might not have had to lube it up after all...</i>\"  The skunkette remarks.");
        this.outputText("  She aligns the tip of the dildo with you and without saying a word she begins pushing in.  Instinctively you moan and try to spread yourself as wide as possible.");
        ...
        if (this.player.looseness() >= 4) this.outputText("; your loose pussy doesn't hurt, either");
        this.outputText(".  Finally, you can feel it stop inside of you, the tip pushing against your cervix, which is what has brought it to a halt.");
        ...
        if (this.player.hasCock()) {
            this.outputText(" except for the ");
            if (this.player.cumQ() < 50) this.outputText("small");
            else if (this.player.cumQ() < 250) this.outputText("normal");
            else if (this.player.cumQ() < 1000) this.outputText("sizable");
            else this.outputText("huge");
            this.outputText(" splatters of cum from your neglected penis");
        }
        this.outputText("; the dildo has absorbed it all.  You can feel it clenching tighter, shrinking down so that it no longer fills you up so perfectly, leaving you panting in release as Lianna gently frees it from inside you.");
    ...
    private liannaHandjobbies(): void {
        this.clearOutput();
        this.flags[kFLAGS.LIANNA_HAVESTED_MALES]++;
        this.outputText("\"<i>All right, let's get this show on the road.</i>\"  She ducks out into her closet once more, and comes out carrying what looks like a condom and a lollipop.  \"<i>Here's how this works.  You put on this condom and I milk your seed out of you.  Simple, yes?</i>\"");
        ...
        if (this.player.cockTotal() > 1) this.outputText("  \"<i>However I think milking just one is good enough.</i>\"");
        this.outputText("  She motions to the bed.");
        ...
        if (this.player.cockTotal() > 1) {
            ...
            this.outputText(", effectively binding them at their base.  You groan hollowly at this; though this certainly will mean less mess, you're not so sure you're going to enjoy the sensation of trying to cum through the " + UrtaPregs.num2Text(this.player.cockTotal() - 1) + " cocks that have been blocked up.  Satisfied with her work, she looks at your [cock biggest].");
        }
        ...
        if (this.player.balls > 0) this.outputText("occasionally stopping to caress your [balls], ");
        this.outputText("while the other massages your [cockHead biggest], rolling the steadily growing bubble of pre around as she kneads it.");
