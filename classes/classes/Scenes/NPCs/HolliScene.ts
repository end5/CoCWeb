...
export class HolliScene extends NPCAwareContent {
    ...
    private fightHolli(): void {
        this.flags[kFLAGS.FOUGHT_HOLLI] = 1;
    ...
    public treeMenu(output: boolean = true): void {
        ...
        if (this.flags[kFLAGS.FOUGHT_HOLLI] > 0) {
            ...
            if (output) this.outputText("Before you is a large, waist-high plant of questionable origin.  A huge, violet bloom sits atop a strong, central stalk, and the obscene flower looks positively female in every sense, right down to a clit-shaped bump and moist, gently parted lips.  Inside that central passage, you can see a bevy of wiggling staman surrounding a larger, flared stalk.  The scent that wafts out of that opening is sweet and floral, but undoubtedly sexual.  It looks like an organic sex toy and is clearly corrupt.  It wouldn't be hard to get rid of right now, assuming you don't let it grow larger.");
            ...
            if (this.player.findPerk(PerkLib.Dragonfire) >= 0 || this.player.findPerk(PerkLib.FireLord) >= 0 || this.player.findPerk(PerkLib.Hellfire) >= 0 || this.player.findStatusAffect(StatusAffects.KnowsWhitefire) >= 0) {
            ...
            if (output) {
                ...
                if (this.flags[kFLAGS.HOLLI_SUBMISSIVE] == 1) {
                    this.outputText("\n\n\"<i>Welcome back, [name].  ");
                    if (this.flags[kFLAGS.TIMES_FUCKED_FLOWER] > 0 && HolliScene.rand(3) == 0) this.outputText("Did you want to fertilize me again?  My flower is ready and waiting...");
                    else if (this.flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0 && HolliScene.rand(3) == 0) this.outputText("Did you want to use my tentacles again?  They've been restless without you...");
                    else if (HolliScene.rand(2) == 0) this.outputText("Did you come for a drink?  I've built up lots of nectar for you.  Surely, a sip would be refreshing for you...");
                    else this.outputText("How may I serve you?  Come, give me a command so that I can fulfill it for you.");
                    this.outputText("</i>\"");

                }
                else {
                    this.outputText("\n\n\"<i>Welcome back, [name].  ");
                    if (this.flags[kFLAGS.TIMES_FUCKED_FLOWER] > 0 && HolliScene.rand(3) == 0) this.outputText("Did you want to fertilize me again?  Gardening is such exhausting work, but so rewarding...");
                    else if (this.flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0 && HolliScene.rand(2) == 0) this.outputText("Did you come back to play again?  My tentacles are so restless.");
                    else if (HolliScene.rand(2) == 0) this.outputText("Did you come for a drink?  I've built up lots of nectar for you.  Surely a hungry adventurer like yourself needs a sip, right?");
            ...
            if (this.flags[kFLAGS.HOLLI_SUBMISSIVE] == 0) {
                ...
                if (this.player.hasCock() && this.player.lust >= 33) fuck = this.fuckHolliInZeFlowerPuss;
                if (this.player.hasVagina() && this.player.lust >= 33) ride = this.level4RideHollisTentacruels;
    ...
    private helpWithFarm(): void {
        ...
        if (this.flags[kFLAGS.HOLLI_FUCKED_TODAY] == 0) {
    ...
    private fertilizeHolli(cock: boolean = true): void {
        ...
        if (cock && this.player.hasCock()) {
            ...
            if (odds > 40) odds = 40;
            if (this.player.findPerk(PerkLib.MaraesGiftStud) >= 0) odds += 10;
            if (this.player.findPerk(PerkLib.FerasBoonAlpha) >= 0) odds += 10;
            if (this.player.findPerk(PerkLib.ElvenBounty) >= 0) odds += 2;
        }
        if (!cock && this.player.hasVagina()) {
            odds += this.player.totalFertility() / 5;
            if (odds > 40) odds = 40;
            if (this.player.findPerk(PerkLib.MaraesGiftFertility) >= 0) odds += 10;
            if (this.player.findPerk(PerkLib.FerasBoonBreedingBitch) >= 0) odds += 10;
            if (this.player.findPerk(PerkLib.ElvenBounty) >= 0) odds += 2;
        }
        if (odds >= HolliScene.rand(101)) {
            this.flags[kFLAGS.HOLLI_FRUIT] += 1 + HolliScene.rand(Math.floor(odds / 10));
            trace("Holli pregged!");
        }
    }
    ...
    public getASprout(): void {
        ...
        if (this.flags[kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] == 2) this.outputText("  She even promised to plant a tree in your camp...");
    ...
    public fuckPlantGrowsToLevel2(): void {
        ...
        if (this.player.gender == 0) this.outputText("  Sucks to be it!  You don't waste time with stupid shit, like genitals.");
        this.outputText("  Is this Marae's idea of a gift?");
        if (this.player.gender > 0) this.outputText("  Well, you could give it a whirl... just being around it seems to slowly turn you on.");
    ...
    private fuckFuckingFuckFlowerP2(): void {
        ...
        if (this.player.cor < 33) this.outputText(" and you timidly look around, hoping you haven't attracted an audience.");
        else if (this.player.cor < 66) this.outputText(" and you worriedly glance around, hoping yet terrified that someone will walk in on it.");
        else this.outputText(" and you glance around, hoping someone will walk in on the sexy show you're making.");
        ...
        if (this.player.hasSheath()) this.outputText(" inside your violated sheath");
        ...
        if (this.player.balls > 0) this.outputText("Your [balls] gurgle, a surprisingly happy sound, and cling tight to your loins.  ");
        ...
        if (this.player.cumQ() > 500) this.outputText("  You pour forth your prodigious essence without pause or fail, and the plant immediately begins to drink it down, the stem bulging with rounded balls of sperm that it carries underground.");
        if (this.player.cumQ() > 1500) this.outputText("  Soon, the meager stem cannot swallow it all.  Spunk froths and squirts out around the base of your tool for a moment, and then a veritable tide of the stuff gushes out to soak  you.");
    ...
    private rideDatFuckingFukkFlowerP2(): void {
        ...
        if (this.player.clitLength > 4) this.outputText("  Your heavy buzzer is so big that the flexible plant-cock has plenty of room to curl around it, and if you could see it, your clit would look almost like a candy-cane (if one set of stripes was jerking off the cane beneath).");
        if (this.player.cockTotal() > 0) this.outputText("  [EachCock] soon has its own partner twisting around it, a pussy-slicked vine to jerk and twine about its girth.");
        this.outputText("\n\nWith nothing to occupy your roving hands, you let them slide up your " + this.player.skinFurScales() + " to your ");
        if (this.player.bRows() > 1) this.outputText("lowest row of tits, cupping them before groping to your top row of breasts");
        else this.outputText("[chest]");
        this.outputText(", caressing the");
        if (this.player.biggestTitSize() > 1) this.outputText("ir");
        ...
        if (this.player.lactationQ() > 50) {
            ...
            if (this.player.lactationQ() < 150) this.outputText("trickle");
            else if (this.player.lactationQ() < 500) this.outputText("leak");
            else this.outputText("spray torrents of");
        ...
        if (this.flags[kFLAGS.PC_FETISH] == 0 && this.player.cor + this.player.lib < 150) this.outputText("embarrassment");
        ...
        if (this.player.hasCock()) this.outputText("  [EachCock] explodes in release, pumping heavy flows of seed into the petals, even as y");
        ...
        if (this.player.cumQ() < 50) this.outputText("drizzle");
        else if (this.player.cumQ() < 250) this.outputText("stream");
        else if (this.player.cumQ() < 750) this.outputText("spray");
    ...
    public flowerGrowsToP3(): void {
        ...
        if (this.flags[kFLAGS.TIMES_FUCKED_FLOWER] + this.flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0) this.outputText("  The fucking probably only helped it to grow faster.");
    ...
    private fuckTheFlower(): void {
        ...
        if (this.player.hasVagina()) this.outputText("  Wetness builds in your [vagina] as well, but you're focusing on your maleness just now.  Your neglected twat will have to wait its turn.");
        ...
        if (this.player.cockArea(x) >= 150) this.outputText(", sliding down the trunk as easily if the entire inside was a giant pussy just waiting to get fucked");
        this.outputText(".");
        ...
        if (this.player.balls > 0) this.outputText(" with your gurgling balls slapping against the flower's edge");
        ...
        if (this.player.cockTotal() > 1) this.outputText(", as your multiple endowments allow you to drizzle the purple bloom's exterior with alabaster cream");
        this.outputText(".");
        if (this.player.cumQ() > 500) this.outputText("  Pearly ropes big enough to fill a flask lurch out from your cum-slit, propelled by your quivering orgasmic muscles deep into the trunk, and you swear you can hear the pitch of your sexual pumping rising in tone, changed by the flooded vessel.");
        if (this.player.cumQ() > 1500) this.outputText("  A wave of your thick gunk rolls out from under your pumping cock - you've filled the tree beyond even its capacity, and the thick puddle forming beneath your [feet] is all the proof you need of your virility.");
        if (this.player.hasVagina()) {
            this.outputText("  Your hungry cunt, long denied in this exchange, trembles as it ");
            if (this.player.wetness() < 4) this.outputText("leaks");
    ...
    private torchP3Tree(): void {
        ...
        //[(nerdfire)
        if (this.player.findStatusAffect(StatusAffects.KnowsWhitefire) >= 0) this.outputText("Closing your eyes to focus, you gather your energies, and unleash your white, supernatural flames upon the thing.");
        //(bromancefire)
        else if (this.player.findPerk(PerkLib.FireLord) >= 0) this.outputText("Sucking in your breath, you inflate your chest and stir the embers of the jaguar demon's gift.  Dredging up an enormous pine-green fireball, you exhale and launch it at the tree.");
        else this.outputText("Sucking in your breath, you inflate your chest and stir the embers of your fiery gift.  Dredging up an enormous fireball, you exhale and launch it at the tree.");
        ...
        if (this.player.findStatusAffect(StatusAffects.KnowsWhitefire) >= 0) this.outputText("magical will");
        else this.outputText("breath");
        this.outputText(".");
    ...
    public treePhaseFourGo(): void {
        ...
        if (this.flags[kFLAGS.TIMES_FUCKED_FLOWER] + this.flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0) this.outputText("Did you enjoy feeding me?  You should have seen the expression on your face when you were cumming in me.  Your eyes were rolling back and your mouth was just hanging open.  Do you know what it made me want to do?");
        else this.outputText("Did you enjoy watching me grow?  You should have fed me.  It was so much harder to grow without proper nutrients.  Do you know what I thought about during all the time I spent waiting?  Do you know what it made me want to do?");
        this.outputText("</i>\"");
    ...
    public JojoTransformAndRollOut(): void {
        ...
        if (this.player.cor < 25) this.outputText("Not willing to lie to your friend, you also admit that she seems to have changed after your actions in the factory - that, though you were trying to render the tanks permanently inoperable, the spillage you created pushed the cthonic spirit over the edge.");
        else this.outputText("Though, you do leave out the precise, needless details of what caused the change.");
        this.outputText("  The monk's eyes widen in surprise and surmise.");

        this.outputText("\n\n\"<i>Unbelievable...</i>\" he blurts.  \"<i>");
        if (this.flags[kFLAGS.TIMES_FUCKED_FLOWER] + this.flags[kFLAGS.TIMES_RIDDEN_FLOWER] == 0) this.outputText("To think that I must now consider Marae herself my enemy.  Forgive me, but this place is not safe for us anymore if it has attracted the attention of such a creature.  For now, I will return to the forest.  I know that you believe your mission keeps you here, but I urge you to stay away from that thing and watch the portal from an adequate distance.");
        //(totally deflowered that flower)
        else this.outputText("And even knowing that this could be the case, you still chose to fornicate with it?  [name]... I cannot say that I am not disappointed.  This place is not safe for me anymore.  For now, I will return to the forest.");
        this.outputText("</i>\"");
        ...
        if (this.player.level < 20) this.outputText("  Nonetheless, if you find yourself in a position where you must confront this demon or perish, call upon me.  I will render what aid I can, though I prefer it not be in vain.");
        else this.outputText("  Though, a mortal of your stature and resources may be able to destroy her and escape the reprisal.  I would counsel against direct confrontation until you are sure of victory, but if you must fight, then call upon me and you will have what aid I can give.");
    ...
    private slapAmilysWhoreFace(): void {
        ...
        if (this.flags[kFLAGS.AMILY_NOT_FURRY] == 0) this.outputText("nose");
        ...
        if (this.flags[kFLAGS.AMILY_NOT_FURRY] == 0) this.outputText("mouse-");
    ...
    private fuckHolliInZeFlowerPuss(): void {
        ...
        if (this.player.tallness < 84) this.outputText("little");
        ...
        if (this.player.cockTotal() > 1) {
            if (this.player.cockTotal() == 2) this.outputText("  Your other penis splays");
            else this.outputText("The rest of your penises splay");
        ...
        if (this.player.balls > 0) this.outputText("cradle your [balls], gently massaging your sack");
        else {
        ...
        if (this.player.cockArea(x) < 100) this.outputText("leans down and kisses the tip, opening her glossy green puckers to slurp it down, sucking just hard enough to make your " + this.player.cockHead(x) + " tingle with erotic warmth.");
        else this.outputText("kisses up and down your huge, towering shaft, slobbering her glossy green puckers all over your urethral bulge, making it even more of a wet mess.");
        ...
        if (this.player.tallness < 60) this.outputText("tits");
        else this.outputText("arms");
        ...
        if (this.player.cockTotal() > 1) this.outputText("s, massaging the bundle of cocks with uncommon ferver");
        else this.outputText(", massing your tool with uncommon fervor");
        ...
        if (this.player.biggestTitSize() >= 2) this.outputText("your own");
    ...
    private haveAMapleSyrupSnack(): void {
        ...
        if (this.flags[kFLAGS.HOLLI_SUBMISSIVE] == 0) this.outputText("knowingly, Holli asks, \"<i>Come for a little pick-me-up?  Well, go on; drink deeply of my nectar, </i>champion.\"  The last word is inundated with malicious, mocking tones.");
        else this.outputText("eagerly, Holli asks, \"<i>Care for a little pick-me-up?  They're getting very engorged, and I'm sure it would be a refreshing snack for you.</i>\"");
    ...
    private level4RideHollisTentacruels(): void {
        ...
        if (this.silly()) this.outputText("  She proceeds to sing about Jack Sparrow's awesome adventures for a solid ten minutes, then goes on as if nothing had happened.");
        ...
        if (this.player.lactationQ() == 0) this.outputText("droplets of milk dripping from your nipples.  When did you start lactating?");
        else this.outputText("steady streams of milk erupting from your nipples.  When did you start lactating this hard?");
        ...
        if (!this.player.isBiped()) this.outputText("you'd be walking bowlegged for a week, if you had legs.");
        else this.outputText("you'll be walking bowlegged for a week!");
    ...
    private eatHolliFruit(): void {
        ...
        if (this.flags[kFLAGS.HOLLI_FRUIT] == 1) this.outputText("the fruit");
        else this.outputText("one of the fruits");
        this.outputText(".  ");
        if (this.flags[kFLAGS.HOLLI_FRUIT_EXPLAINED] == 0) {
            ...
            this.flags[kFLAGS.HOLLI_FRUIT_EXPLAINED]++;
        }
        ...
        if (HolliScene.rand(2) == 0 && this.player.cockTotal() > this.player.tentacleCocks()) {
            ...
            while (this.temp < this.player.cockTotal()) {
                if (this.player.cocks[this.temp].cockType != CockTypesEnum.TENTACLE) choices[choices.length] = this.temp;
                this.temp++;
            }
            ...
            if (this.player.tentacleCocks() > 0) this.outputText("nother");
    ...
    private askHolliToWatch(): void {
        this.clearOutput();
        if (this.flags[kFLAGS.HOLLI_DEFENSE_ON] == 0) {
            ...
            if (this.flags[kFLAGS.THREATENED_HOLLI] > 0) this.outputText("<b>  You could always just slap the cunt.</b>");
    ...
    private domUpSomeHolli(): void {
        ...
        //[Next] - Holli's Menu
        this.flags[kFLAGS.HOLLI_SUBMISSIVE] = 1;
        this.treeMenu(false);
    }
    ...
    private askBrokenHolliToGuard(): void {
        ...
        if (this.flags[kFLAGS.HOLLI_DEFENSE_ON] == 1) this.outputText("keeping watch");
        else this.outputText("sleeping, instead");
        this.outputText(".</i>\"");
    ...
    private holliGetsDickDommed(): void {
        ...
        if (this.player.biggestCockArea() > 80) this.outputText(", jaw stretching obscenely");
        this.outputText(".");
        ...
        else this.outputText(", swallowed as deep as Holli's distended neck can handle.");
        ...
        if (this.player.balls > 0) this.outputText("[sack] feel taut and ready to burst");
        else this.outputText("body feel strung tight like an over-tuned guitar");
        ...
        if (this.player.biggestCockArea() > 80) this.outputText("  It bends slightly as the immense girth is forced inside her, aiming down, deep inside the trunk.");
        ...
        if (this.player.cockTotal() > 1) {
            ...
            if (this.player.cumQ() >= 500) this.outputText("  The constant sprays turn them white in little time, dripping from the peaks of her nipples along with her sticky sap.");
        }
        if (this.player.cumQ() > 1000) this.outputText("  A trickle of sperm washes out of the demoness's entrance, somehow escaping the heavy suction.");
        if (this.player.cumQ() > 2000) this.outputText("  A moment later, that trickle turns into a river.");
        if (this.player.cumQ() > 3000) this.outputText("  You swear that her trunk must be absolutely stuffed with spunk.");
        if (this.player.cumQ() > 5000) this.outputText("  A spooge-spout sprays out as the pressure goes through the roof, confirming your suspicions in the messiest way possible.");
    ...
    private fullOnTentacleTasticGangBangForHolli(): void {
        ...
        if (this.silly()) this.outputText(", inflatable-arm, flailing tube-cocks");
        ...
        if (this.player.balls > 0) this.outputText("  Your [balls] draw up tight to your groin and squeeze, rhythmically discharging their heavy loads.");
        ...
        if (this.player.cumQ() > 500) this.outputText(", and the next");
        if (this.player.cumQ() > 1400) this.outputText("... and the next");
    ...
    private vaginalDomHollisTentacruels(): void {
        ...
        if (!this.player.hasFuckableNipples() && this.player.lactationQ() < 50) this.outputText(" and pay special attention to your [nipples], tugging them in time with every sluggish thrust in your slit.");
        else if (this.player.lactationQ() >= 50) {
            this.outputText(" and sigh at the ");
            if (this.player.lactationQ() < 150) this.outputText("squirts");
            else if (this.player.lactationQ() < 800) this.outputText("sprays");
            else if (this.player.lactationQ() < 2000) this.outputText("outpouring");
            else this.outputText("thick fountains");
            this.outputText(" of milk that escape with every tug, timed to match the sluggish thrusts at your slit.");
        }
        else this.outputText(" and sink your fingers into your [nipples], fucking them to the sluggish tempo of the thrusting tentacle.");
        ...
        if (this.player.clitLength >= 3.5) this.outputText(" for a moment before wrapping around and jerking on it");
        this.outputText(".");
        ...
        if (this.player.wetness() < 4) this.outputText("onto the ground");
        else this.outputText("into the leavings of your squirting orgasm");
    ...
    private threatenHolli(): void {
        ...
        if (this.player.level < 30) this.outputText("Mighty as you are, Marae is likely still beyond your power.  Perhaps you should just ignore this demon for the time being.");
        else this.outputText("Marae is likely beyond your power yet, but... you feel like she might not pressure you too hard if you can make a suitably credible threat.");
        ...
        if ((this.player.gender > 0 && this.player.findStatusAffect(StatusAffects.JojoNightWatch) >= 0 && this.player.findStatusAffect(StatusAffects.PureCampJojo) >= 0) || this.flags[kFLAGS.ANEMONE_WATCH] > 0) {
    ...
    private callDatJojo(): void {
        ...
        this.flags[kFLAGS.FOUGHT_HOLLI] = 1;
        this.startCombat(new Holli());
        this.monster.createStatusAffect(StatusAffects.JojoIsAssisting, 0, 0, 0, 0);
    }
    ...
    public defeatHolli(): void {
        ...
        this.flags[kFLAGS.FUCK_FLOWER_KILLED] = 1;
        //Win without Jojo -Z
        if (this.monster.findStatusAffect(StatusAffects.JojoIsAssisting) < 0) {
            ...
            if (this.player.weaponName != "large axe") this.outputText("fetch an axe and ");
    ...
    public enjoyYourBadEndBIYAAAATCH(): void {
        ...
        if (this.player.gender == 1 || (HolliScene.rand(2) == 0 && this.player.hasCock())) {
            ...
            if (this.player.cocks[this.player.biggestCockIndex()].cockThickness > 6) this.outputText(", stretching her jaw wide open in a way that makes you wince,");
            ...
            if (this.player.biggestCockLength() > 24) this.outputText("her neck ripples and extends grotesquely, and her head begins to migrate down all " + HolliScene.num2Text(Math.round(this.player.biggestCockLength())) + " of your prick with alarming speed, pushing the stem inside along with it.  ");
            else this.outputText("she sinks her head down to the base of your prick in one quick thrust, bringing the stem with it.  ");
            ...
            if (this.player.balls > 0) this.outputText(", and your [balls] draw close to your body to unload");
            ...
            if (this.player.balls > 0) this.outputText("migrating the other direction into your [balls], entangling and infiltrating them, filling them with the same warmth, then pressing out against your sack.  ");
            else this.outputText("exploring the depths of your body and curling back against the surface of your " + this.player.skin() + ".  ");
        ...
        //branch of vagina-having
            ...
            if (this.player.isTaur()) this.outputText(" and turning you around");
            ...
            this.outputText("\n\nSomething ");
            if (!this.player.isGoo() && !this.player.isNaga()) this.outputText("slips between your thighs and ");
            ...
            if (this.player.hasCock()) this.outputText(" and a glob of precum from [eachCock]");
            this.outputText(".");
            ...
            if (this.player.hasCock()) this.outputText("  [OneCock] drools precum down its length and onto the flower, triggering another spasm and another gush from your prick.  Holli laughs as you continue to self-stimulate with your uncontrollable fluids.");
            ...
            if (this.player.biggestTitSize() > 1) this.outputText("wrapping around and under your breasts to keep them permanently jutting out in a lurid display");
            else this.outputText("tracing a lattice of slender, criss-crossing ivy over your chest that hangs just far enough over your [nipples] that a stiff breeze will drag it into them");
            ...
            if (this.player.biggestTitSize() > 1) this.outputText("your breasts cupped and pressed together by vines, ");
            if (!this.player.isNaga() && !this.player.isGoo()) this.outputText("your thighs spread wide open, ");
            this.outputText("and your nectar-drooling flower-sex exposed with your [clit]");
            if (this.player.hasCock()) this.outputText(" and [eachCock]");
            ...
            if (this.player.ass.analWetness > 0) this.outputText("Betraying your depravity, the slick crack opens wide and your greedy, wet asshole practically welcomes the demon's entire hand, sucking it up and making you shiver with repressed delight.  \"<i>Oh, mother!  It seems as though this is not a new lesson to you, is it?  My little whore, will you beg and moan as I fill you?</i>\"");
            else if (this.player.analCapacity() < 30) this.outputText("With some difficulty, she forces it into your pucker.  \"<i>I think we'll just use this as our inroads, instead.  It may be slow going, but the journey is half the fun.</i>\"");
            else this.outputText("Despite your clenching, your loose hole proves no impediment to the demon's exploration.  \"<i>Ah, how roomy you are!  I think I'll move right in.</i>\"");
            ...
            //[(iff buttslut)
            if (this.player.ass.analWetness > 0) this.outputText("Try as you might, you can't see far enough down your back to determine what the demon has done.  The question answers itself, though, when she presses a shaft almost as thick as her leg into your [butt], nosing it into your wet, receptive asshole.  Despite yourself, you shiver in anticipation at being filled so completely.  ");
            else if (this.player.analCapacity() < 30) this.outputText("Your jaw hits your collarbone as Holli angles the florid head of a long, narrow penis into your field of view.  Judging by how far it comes up her chest, it's almost a foot and half long, and it's drooling sticky sap all over your back.  With a twisted smile, the demon pushes it along your spine until it reaches your [butt], then shoves it toward the hole.  ");
            else this.outputText("The demon presses up to your back, brushing her nipples against your skin and shoving her new growth between your thighs.  Looking down, you can see a fat, green shaft sticking from your crotch, glistening and dripping with sticky slime.  With another moan, Holli pulls it back from between your legs, then pushes apart your asscheeks and plunges it into your [asshole].  ");
            this.outputText("The sap oozing from her member coats your passage, ");
            if (this.player.ass.analWetness > 0) this.outputText("mingling with your own lubrication and ]easing her thrusts.  Holli's tempo picks up quickly, and soon you can feel her twitch inside you as she nears her climax.");
            ...
            //[(buttslut or loose)
            if (this.player.ass.analWetness > 0 || this.player.analCapacity() > 30) this.outputText("spilling a wave of her deposited fluids down your [legs].  ");
            else this.outputText("and your once-tight pucker reflexively squeezes shut as best it can, trapping all but a few trickles of Holli's deposit inside you.  ");
            ...
            if (this.player.skinType == SKIN_TYPE_FUR || this.player.skinType == SKIN_TYPE_SCALES) this.outputText(" under your " + this.player.skinFurScales());
            ...
            if (this.player.biggestTitSize() < 1) this.outputText("folded atop your chest");
            else this.outputText("crooked at your sides with your hands resting atop your [chest]");
            ...
            if (this.player.biggestTitSize() >= 1) this.outputText(", your breasts exposed from the sides,");
    ...
    private holliPlaysWithPenisesBadEnd(): void {
        ...
        if (this.player.biggestTitSize() < 1) this.outputText("masculine shape, it bears");
        else this.outputText("feminine shape, it nonetheless bears");
        ...
        if (this.player.smallestCockArea() > 36) this.outputText("  As the dryad speaks, the silent figure's [cock smallest] shudders and begins to shrink, dwindling to a size that would fit " + this.player.mf("him", "her") + " prospective lover.");
        ...
        if (this.player.cocks.length > 1) this.outputText("spraying the girl's stomach and tits with waste seed from " + this.player.mf("his", "her") + " unused manhood and ");
    ...
    private girlsGetANiceManToBadEnd(): void {
        ...
        //[(if tits)
        if (this.player.biggestTitSize() > 1) this.outputText("[chest] cupped by vines and held out toward the young man, ");
        else this.outputText("chest crisscrossed with a mesh of fine vines, ");
        this.outputText("its legs elevated and spread apart to display a ");
        if (this.player.balls > 0 && this.player.ballSize > 10 && this.player.gender == 3) this.outputText(this.multiCockDescriptLight() + " with [balls] swinging below.");
        //(if H and ballsize < 10)
        else if (this.player.balls > 0 && this.player.ballSize <= 10 && this.player.gender == 3) this.outputText(this.multiCockDescriptLight() + " and [balls] that hang slightly over a [vagina].");
        //(if H and noballs)
        else if (this.player.gender == 3) this.outputText(this.multiCockDescriptLight() + " and a [vagina].");
        else this.outputText("a wet, drooling cunt that practically begs to be filled.");
        ...
        if (this.player.ballSize > 10 && this.player.balls > 0) this.outputText("lifting the hefty testicles to reveal a wet, eager pussy.");
        else this.outputText("spreading the labia and exposing the eager, clenching hole inside.");
        ...
        if (this.player.biggestTitSize() >= 1) {
            ...
            if (this.player.gender == 2) this.outputText("woman");
            else this.outputText("hermaphrodite");
            this.outputText("'s mouth open and close in bliss.  ");
        }
        else this.outputText("leans down and kisses the [woman/hermaphrodite]'s smooth chest, causing her to arch her back in pleasure.  ");
        if (this.player.hasCock()) this.outputText("With a nervous glance downward, he slides one hand to her [cock smallest].  \"<i>It's not gay if it's a chick's,</i>\" he mumbles to himself, and then begins to stroke haltingly, playing his finger along the urethra and drawing a squirt of precum from his shivering partner.  ");
        else {
            ...
            if (this.player.clitLength < 6) this.outputText("he smears it over the little button and then begins to trace up and down the underside with a finger.  ");
            else this.outputText("he coats the swollen organ with it and begins to jerk it off valiantly, like a cock.  ");
        ...
        if (this.player.hasCock()) {
            this.outputText("; ");
            if (this.player.balls > 0) this.outputText("her balls draw closer to her body and ");
            this.outputText("her trembling dick");
            if (this.player.cockTotal() == 1) this.outputText(" throbs");
            else this.outputText("s throb");
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.biggestCockArea() < 20) {
                ...
                if (this.player.cumQ() >= 500) this.outputText("So much so that by the time she finishes her climax, both her partner's legs and her trunk are covered in a sheen of milky white seed.  ");
            }
                ...
                //[(big skeet)]
                if (this.player.cumQ() >= 500) this.outputText("The waves of seed don't stop coming, pouring into her and swelling her belly out until its round bulge pushes the cock away from her mouth and the remaining squirts fall on her [chest].  ");
            }
            //else cock biggest > 80)
                ...
                if (this.player.cumQ() >= 1000) this.outputText("The overproductive hermaphrodite's body pushes out so much seed that the entire canopy soon turns white and drips constantly as though it were in a tropical rainforest.  ");
            }
        }
        //(vagina and no cock)
        else {
            this.outputText("\n\nA ");
            if (this.player.wetness() < 3) this.outputText("gush");
            else if (this.player.wetness() < 4) this.outputText("rush");
            else this.outputText("torrent");
        ...
        if (this.player.cockTotal() == 1) this.outputText("cock");
        else if (this.player.cockTotal() > 1) this.outputText("cocks");
        else this.outputText("chest");
    ...
    private holliAndGenderlessSittingInATree(): void {
        ...
        if (this.player.skinType != SKIN_TYPE_FUR && this.player.skinType != SKIN_TYPE_SCALES) this.outputText(this.player.skinTone + "-colored");
        else this.outputText("covered in " + this.player.skinFurScales());
        ...
        if (this.player.skinType == SKIN_TYPE_SCALES) this.outputText("a quiver of arousal shakes its scales");
        else if (this.player.skinType == SKIN_TYPE_FUR) this.outputText("its fur begins to prick up with arousal");
        else this.outputText("a blush of lust colors its " + this.player.skinFurScales());
        this.outputText("...");
