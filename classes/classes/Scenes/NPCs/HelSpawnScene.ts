...
export class HelSpawnScene extends NPCAwareContent {
    ...
    private championRef(): string {
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) return this.player.mf("Daddy", "Mom");
        else return this.player.short;
    }

    public helspawnFollower(): boolean {
        return this.flags[kFLAGS.HELSPAWN_AGE] == 3;
    }
        ...
        if (this.flags[kFLAGS.HELSPAWN_NAME] != 0) {
            ...
            switch (kGAMECLASS.helScene.pregnancy.event) {
                case 1: //She's pregnant, but no special text yet
                    break;
                case 2: this.outputText("  Hel's just starting to show a little bulge of pregnancy.");
                    break;
                case 3: this.outputText("  Hel’s belly is starting to look fairly bloated, swelling with her child.");
                    break;
                case 4: this.outputText("  Hel’s belly is positively gravid, full of a little salamander child.");
                    break;
                default: //She's not pregnant, check if she's ready for it
                    if (this.flags[kFLAGS.HEL_BONUS_POINTS] >= 150) this.outputText("  Hel’s long, reptilian tail is currently burning white-hot, signaling her body’s ready for motherhood.");
            }
        }
    ...
    private helSpawnsAppearanceScreen(): void {
        this.clearOutput();
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) this.outputText("Your ");
        else this.outputText("Hel’s ");
        this.outputText("salamander daughter, " + this.flags[kFLAGS.HELSPAWN_NAME] + ", stands about six-foot-six tall, with pale skin and thick, ");
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 1) this.outputText("chitin-black");
        else this.outputText("bright-red");
        this.outputText(" scales covering her arms and legs, though she has a normal human face and torso.  A long leathery tail swishes happily behind her, surrounded by a dim fiery aura that emits a pleasant heat, though thankfully it isn’t hot to the touch, seemingly incapable of harming you.  " + this.flags[kFLAGS.HELSPAWN_NAME] + " is wearing ");
        if (this.flags[kFLAGS.HELSPAWN_PERSONALITY] >= 50) this.outputText("a scale bikini, just like her mother’s");
        else this.outputText("a short skirt, thigh-high boots, and a sky-blue blouse, in stark contrast to her mother’s sluttier attire");
        if (this.flags[kFLAGS.HEL_ISABELLA_THREESOME_ENABLED] >= 1) this.outputText(", with a brightly-colored scarf wrapped around her neck");
        this.outputText(". She has a human face, with ");
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) this.outputText("golden");
        else if (this.flags[kFLAGS.HELSPAWN_DADDY] == 2) this.outputText("bright green");
        else this.outputText("crimson");
        ...
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 2) this.outputText("puppy-dog");
        else this.outputText("reptilian");
        ...
        this.outputText("\n\n" + this.flags[kFLAGS.HELSPAWN_NAME] + " has a pair of perky, palmable C-cup breasts, each with a sensitive half-inch nipple at its tip.");
    ...
    public heliaBonusPointsAward(): void {
        this.flags[kFLAGS.HELIA_KIDS_CHAT] = 1;
        this.spriteSelect(68);
        this.outputText("\n<b>That night...</b>\n");
        ...
        if (this.player.hasVagina() && this.player.hasCock()) this.outputText(" and your [chest] and your [vagina]");
        if (!this.player.hasCock() && this.player.hasVagina()) this.outputText(" [vagina] and [chest]");
    ...
    private heliaLoveFollowup(): void {
        ...
        if (this.player.hasVagina()) this.outputText("even another woman, it seems, ");
        ...
        //If HelLove = No:
        if (this.flags[kFLAGS.HEL_LOVE] == -1) this.outputText("Even if you don't love me, you'd love our child, right?  You can fuck me full of kids, or even... even get someone else to, I don't care.  I just </i>need<i> it, [name].  More than anything else in the whole wide world.");
        else {
            this.outputText("I love you, [name].  I love you so much.  I want to share this with you.  ");
            if (this.player.hasCock()) this.outputText("You'll fuck me full of kids, right?  Please?  Give me your seed, [name].  Give it all to me until I'm fucking gravid with your salamander babies.");
            else this.outputText("Even if you can't fuck me full of babies... you could grow a dick!  There's plenty of things in this god-forsaken world that do that. O-or if you don't want to, I bet I can get one of the fox-girls, or a minotaur, or something.  You know I can find a dick somewhere, just say the word.");
        }
    ...
    public haveAKid(): void {
        ...
        else this.outputText("the base of your prick");
        ...
        if (this.player.biggestTitSize() <= 2) this.outputText("supports her, pressing down on your [chest]");
        else this.outputText("gropes one of your tits, kneading your tender breasts under you squirm underneath her, unable to hold in your pleasured little gasps");
        ...
        if (this.player.pregnancyIncubation > 0) this.outputText("We're going to be mothers!");
        else this.outputText("You're going to be a father... and me a mother!");
    ...
    private HaveAHellKidPartII(): void {
        ...
        if (this.flags[kFLAGS.HEL_LOVE] == 1) this.outputText(", and I'm so very, very glad you love me back.");
        else this.outputText(", even if you can't say the same.  And that's alright; it doesn't change my heart, and it never will. I love you");
        this.outputText(".</i>\"");
        ...
        if (this.player.tallness >= 90) this.outputText("up ");
        else if (this.player.tallness <= 72) this.outputText("down ");
    ...
    private getAnotherDad(): void {
        ...
        if (!this.player.hasCock()) {
            ...
            if (this.player.statusAffectv1(StatusAffects.TelAdre) >= 1) this.outputText("There's Miko and Mai from the bar.  Mai's said she wanted a kid, but can't take care of one... she'd probably be willing to fuck one into me!  If that's not alright, then... lemme think.  ");
    ...
    private noKidsHel(): void {
        ...
        if (this.player.hasCock() && this.player.cockThatFits(this.helFollower.heliaCapacity()) >= 0) this.outputText("You couldn't if you wanted to anyway - you're too big to fit.  ");
    ...
    public helGotKnockedUp(): void {
        this.spriteSelect(68);
        this.flags[kFLAGS.HEL_NTR_TRACKER] = 2;
        ...
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 2) this.outputText("warm ");
        else this.outputText("wan ");
    ...
    private sureHelGimmeSpidahBoyDetails(): void {
        ...
        if (this.followerKiha()) this.outputText(", toward hotwings' old place");
        ...
        if (this.player.biggestCockArea() >= 20) this.outputText(", though still nothing compared to you, lover mine");
    ...
    private youWantABoy(): void {
        ...
        if (this.player.hasCock() && !this.player.hasVagina()) this.outputText("If I were a guy, I'd want a big strong son to hang out with, too.  Take him fishing, teach him how to fight the way you do... you'll make a great dad, my love.  I'm sure you will.");
    ...
    private youWantAGirl(): void {
        ...
        else this.outputText("We're going to be a gaggle of tittering girls before you know it, though.  Salamanders grow up so fast, [name]...  I just hope she doesn't turn out like me, you know?  I don't know if I could stomach seeing my little girl becoming a wanton slut like her mom.  I might get jealous, after all...");
    ...
    public heliaTalkSeven(): void {
        this.clearOutput();
        this.spriteSelect(68);
        this.flags[kFLAGS.HELIA_TALK_SEVEN] = 1;
    ...
    private encouragePregalia(): void {
        ...
        if (this.flags[kFLAGS.HEL_LOVE] == 1) this.outputText("You love Hel");
    ...
    private helsLifestyle(): void {
        ...
        if (this.flags[kFLAGS.HEL_LOVE] == 1) this.outputText("and I hope that's part of why you love me");
        else this.outputText("even though I know that's probably part of why you can't love me back");
    ...
    public heliaBirthtime(): void {
        ...
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 1) this.outputText("your ");
    ...
    private applyHelspawnName(): void {
        ...
        //Easter Egg Names:
        this.clearOutput();
        if (this.flags[kFLAGS.HELSPAWN_NAME] == "Helspawn") this.outputText("\"<i>That's what I was gonna call her!  Except not at all.  God dammit, seriously, [name]?</i>\"\n\n");
        if (this.flags[kFLAGS.HELSPAWN_NAME] == "Jayne") this.outputText("\"<i>That sounds like a hero's name... I like it.  A name that demands respect!</i>\"\n\n");
        if (this.flags[kFLAGS.HELSPAWN_NAME] == "Hesper") this.outputText("\"<i>Ah, the Evening Star.  She'll be the star of my life, alright...</i>\"\n\n");
        ...
        this.outputText("\"<i>So... " + this.flags[kFLAGS.HELSPAWN_NAME] + " it is.  That's a good name, my love.  A strong name.  ");
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) this.outputText("Our");
        else this.outputText("my");
    ...
    private helSpawnsSetup(): void {
        ...
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 2) this.flags[kFLAGS.HELSPAWN_PERSONALITY] += 10;
        //>If Spiderbro is daddy: +10 HelspawnChaste
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 1) this.flags[kFLAGS.HELSPAWN_PERSONALITY] -= 10;
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) {
            //>If Corrupt (66+ Corr) PC is daddy: +10 Helspawn Slutty
            if (this.player.cor >= 66) this.flags[kFLAGS.HELSPAWN_PERSONALITY] += 10;
            //>If Pure (33> Corr) PC is daddy: +10 Helspawn Chaste
            if (this.player.cor <= 33) this.flags[kFLAGS.HELSPAWN_PERSONALITY] -= 10;
        }
    ...
    public heliaTalkEight(): void {
        this.clearOutput();
        this.flags[kFLAGS.HEL_TALK_EIGHT] = 1;
        this.spriteSelect(68);
        ...
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) this.outputText("parental ");
        else this.outputText("care giver's ");
    ...
    public playWithYourKid(): void {
        this.clearOutput();
        this.spriteSelect(68);
        this.outputText("Picking " + this.flags[kFLAGS.HELSPAWN_NAME] + " up off the ground, you spin the giggling little girl around, laughing as your ");
        if (this.flags[kFLAGS.HELSPAWN_DADDY] != 0) this.outputText("ward ");
        else this.outputText("daughter ");
        this.outputText("cries out in surprise and glee.  Soon, you're bouncing her, then down on the ground crawling around with her, playing peek-a-boo and a dozen other games as you and " + this.flags[kFLAGS.HELSPAWN_NAME] + " laugh and hug.  Helia herself slips down with you before you know it, holding her daughter close as she tries to communicate, teaching the ever-growing girl a few words at a time.");

        this.outputText("\n\n\"<i>Mommy!</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " eventually cries, bopping her mother on the nose. Hel laughs, but it turns into a sigh as her daughter reaches down and fiddles with her top, obviously hungry.");
    ...
    public helSpawnGraduation(): void {
        ...
        this.outputText("You wake up to a scream, high-pitched and terrified.  You bolt up out of bed, tangling up in your bedroll for a minute before stumbling over to Hel and... " + this.flags[kFLAGS.HELSPAWN_NAME] + "?");
        this.outputText("\n\nYou do a double take as you lay eyes on the ");
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 1) this.outputText("black");
        ...
        this.outputText("\n\n\"<i>Ugh, what's wrong?</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " groans, looking up at Hel, then to you.  \"<i>" + this.championRef() + ", why's mom screaming?</i>\"");
        ...
        this.outputText("\n\nYou have to admit, the change is sudden and drastic... but the girl's scales, her eyes, they're certainly " + this.flags[kFLAGS.HELSPAWN_NAME] + "'s.  ");
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 2) this.outputText("And the puppy ears, now more wolf-like, are a dead giveaway.  Utterly unique to her.  ");
        ...
        this.outputText("\n\n\"<i>It's just me, mom,</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " says, sliding over to sit beside her mother, who's still wide-eyed with shock.  Your girl tries for a few moments to talk to Hel, but seeing that venture prove fruitless - Hel seems utterly insensate, paralyzed by shock - she instead curls back up beside her mother, head rested on Helia's shoulder.  Eventually, warily, Hel reaches out a trembling hand to brush her daughter's cheek.");
    ...
    public helspawnDiscoversBooze(): void {
        this.clearOutput();
        this.spriteSelect(68);
        this.flags[kFLAGS.HELSPAWN_DISCOVER_BOOZE] = 1;
        ...
        if (this.player.race() != "cat-boy" && this.player.race() != "cat-girl") this.outputText("a cat");
        else this.outputText("the cat you are");
    ...
    private encourageHelspawn(): void {
        ...
        this.outputText("\n\n\"<i>Wha...?</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " starts to say, before catching on as you drink, inviting her to join you.");
        ...
        this.outputText("\n\n\"<i>Uh...</i>\" you groan, slumping over onto your [ass].  " + this.flags[kFLAGS.HELSPAWN_NAME] + "'s right behind you, collapsing on top of you in a pile of ");
        if (this.flags[kFLAGS.HELSPAWN_DADDY] != 1) this.outputText("red ");
        ...
        this.outputText("\n\n\"<i>I saved some for ya, mom,</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " groans, retching.");
    ...
    public helSpawnChoosesAFightingStyle(): void {
        this.clearOutput();
        this.spriteSelect(68);
        this.outputText("As you make your way back to camp, you begin to hear the sounds of fighting from ahead.  Readying your [weapon], you pick up the pace until your campsite comes into view.  You relax, seeing that it's only " + this.flags[kFLAGS.HELSPAWN_NAME] + " and her mother.  Helia's standing near the center of camp, ");
        if (this.camp.companionsCount() > 3) this.outputText("apparently having cleared out most of your other followers for the moment, ");
        this.outputText("her sword held at a guard pose between herself and a training dummy. " + this.flags[kFLAGS.HELSPAWN_NAME] + " stands a few feet behind her mother, watching intently as Hel paces around the dummy, breathing hard.");
        ...
        this.outputText("\n\n\"<i>Hi, " + this.championRef() + ",</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " says cheerily as you wander over, inspecting the carnage.  \"<i>Mom's teaching me how to be a berzerker.</i>\"");
    ...
    private snipermanders(): void {
        this.clearOutput();
        this.spriteSelect(68);
        this.flags[kFLAGS.HELSPAWN_WEAPON] = "bow";
        ...
        this.outputText("\n\nYou slip behind " + this.flags[kFLAGS.HELSPAWN_NAME] + ", putting your arms on hers as you guide her through nocking an arrow, head resting on her fist, a lone finger outstretched toward the dessicated dummy.");
        ...
        this.outputText("\n\n\"<i>I-I dunno, " + this.championRef() + ",</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " whines, biting her lip as she traces the arrow's path with her eyes.  \"<i>Maybe mom was right. I should just stick to a sword. That's easy....</i>\"");

        this.outputText("\n\nYou sigh and tell her to try again.  She can't expect to be perfect on the first try, after all.  It takes a little convincing, but soon you've got " + this.flags[kFLAGS.HELSPAWN_NAME] + " back into form and nocking an arrow.  You put your hands on hers, helping the young salamander take aim, drawing down on her straw-stuffed target.  Barely audible over " + this.flags[kFLAGS.HELSPAWN_NAME] + "'s nervous breathing, you whisper, \"<i>Take it easy.  Relax.... Good.  Take aim... now breathe out.  Exhale and loose.</i>\"");

        this.outputText("\n\nAgain, " + this.flags[kFLAGS.HELSPAWN_NAME] + " lets an arrow fly - and this one strikes true.  You watch the missile streak across camp, slamming into the training dummy's crotch with deadly force.  It sways, buckles, and falls to pieces.");

        this.outputText("\n\n\"<i>Ouch,</i>\" Hel laughs, sipping on her flask behind you.  \"<i>What'd that poor dummy ever do to you?</i>\"");

        this.outputText("\n\n\"<i>I did it!</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " shouts, pumping her fist in the air. Her mother reaches over to ruffle her fiery hair, and you both shower her with congratulations.");

        this.outputText("\n\nBefore she can get cocky, though, you push the bow back into her hands.  \"<i>Lesson 2, kiddo: one shot is never enough.</i>\"");

        this.outputText("\n\nBy the time " + this.flags[kFLAGS.HELSPAWN_NAME] + " has her shooting stance back, her mother's already set the dummy back up, ready for round two.  It's several hours long before the three of you quit: time spent drilling " + this.flags[kFLAGS.HELSPAWN_NAME] + " on stance and aim, even going a round with her and a pair of daggers to remind her that enemies will eventually close in if she's not careful.  She learns quickly, though, and soon she hardly needs your help to plant an arrow in the target's chest.");
    ...
    private swordAndBoardmander(): void {
        this.clearOutput();
        this.spriteSelect(68);
        this.flags[kFLAGS.HELSPAWN_WEAPON] = "scimitar and shield";
        ...
        this.outputText("\n\n" + this.flags[kFLAGS.HELSPAWN_NAME] + " gives the blade a few practice swings before grabbing the wooden blade in both hands and charging at you.  You put your guard up, catching her first overhand blow and turning it aside, nearly toppling the girl over.  She rolls with it, spinning out of your guard and trying a strike from the side.  You parry it, catch an opening, and give her a nice wallop on the shoulder.  A real blade would have nearly cut her arm off, but " + this.flags[kFLAGS.HELSPAWN_NAME] + " merely stumbles back, yelping in pain.");
    ...
    private dasBarbarimander(): void {
        ...
        if (this.player.gems >= 200) {
            this.outputText("You sigh and dig out a handful of gems.  Helia beams at you, planting a quick kiss on your cheek before grabbing " + this.flags[kFLAGS.HELSPAWN_NAME] + " and the loot.  \"<i>Thanks for the loan, lover mine.  C'mon, sweetie, we're going for a trip to town.</i>\"");
    ...
    public helspawnIsASlut(): void {
        ...
        if (this.player.weaponName != "fists") this.outputText("clutch your [weapon]");
        else this.outputText("ball your fists");
        this.outputText(" as the intruder passes by, ready to defend yourself and your loved ones.  The moment they're out of reach, you roll onto your [feet], looking around for signs of a massed demon attack.  Instead, you see the outline of " + this.flags[kFLAGS.HELSPAWN_NAME] + " in the dying embers of the fire light, leading what looks like a spider girl by the hand toward her bed.");

        this.outputText("\n\nYou creep closer, trying to get a closer look at what your ");
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) this.outputText("daughter ");
        else this.outputText("ward ");
        this.outputText("is doing.  Well, it doesn't take a genius to figure that out, really, but still, you have to be sure.  You slide right up to them, just out of their lines of sight, as " + this.flags[kFLAGS.HELSPAWN_NAME] + " turns around, letting the spider-kin unclasp her bra and grab her tits, squeezing the small mounds as " + this.flags[kFLAGS.HELSPAWN_NAME] + " suppresses a moan, reaching back to slip a hand into the spider's crotch, grabbing her - his dick.");
    ...
    private helSpawnStopFucking(): void {
        this.clearOutput();
        this.spriteSelect(68);
        this.outputText("You sigh and step out of the shadows, grabbing the feminine little spider boy by the shoulder and yanking him away from " + this.flags[kFLAGS.HELSPAWN_NAME] + ".  \"<i>And just what the hell do you two think you're doing?</i>\" you yell, scowling ");
        if (this.player.tallness >= 90) this.outputText("up ");
        else if (this.player.tallness <= 72) this.outputText("down ");
        this.outputText("at your ");
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) this.outputText("daughter ");
        else this.outputText("ward ");
        this.outputText(". " + this.flags[kFLAGS.HELSPAWN_NAME] + " gives a shrill cry of surprise and tumbles away from you, trying to cover herself as best she can; the spider trap squirms in your unrelenting grasp, one set of arms covering his rapidly-wilting erection as the other flails around, trying to get you off of him.  Not happening.");

        this.outputText("\n\n\"<i>You!</i>\" you say, pointing an accusing finger at " + this.flags[kFLAGS.HELSPAWN_NAME] + ".  \"<i>What the hell, kid?  Who the fuck is this?</i>\"");

        this.outputText("\n\n\"<i>That's, uh.... Alex, meet ");
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) this.outputText("my dad");
        else this.outputText("[name]");
        ...
        this.outputText("\n\n\"<i>" + this.championRef() + "...</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " groans as you grab the spider boy and toss him out of camp, watching with hawk-like eyes as he slinks back to his jungle.  When he's gone, you sigh and walk over to " + this.flags[kFLAGS.HELSPAWN_NAME] + ", wiping a tear from her cheek.  Putting an arm around her shoulder, you give her a long, long talk about not being a whore.");

        this.outputText("\n\n\"<i>B-but mom...</i>\"");

        this.outputText("\n\n\"<i>That's why you </i>shouldn't<i> be bringing men home.  Or women.  Or anything else, kiddo.");
        //If No Hel Love:
        if (this.flags[kFLAGS.HEL_LOVE] != 0) this.outputText("  You don't want to be a whore like your mother, do you?  The girl that everyone's had a ride with.");
        //if HelLove:
        if (this.flags[kFLAGS.HEL_LOVE] == 1) this.outputText("  I'm not going to a raise a whore, " + this.flags[kFLAGS.HELSPAWN_NAME] + ".  Your mother's made mistakes, sure, but...</i>\" you sigh.  \"<i>");
        this.outputText("  You don't have to be that person, " + this.flags[kFLAGS.HELSPAWN_NAME] + ".  Don't just give yourself out to the first person that asks, you know?  Save it for someone you really care about.  Trust me, it'll be worth it.</i>\"");
    ...
    public helspawnsMainMenu(): void {
        this.clearOutput();
        this.outputText("You call " + this.flags[kFLAGS.HELSPAWN_NAME] + " over to you and she comes running, ");
        if (this.player.tallness <= 72) this.outputText("grabbing you off your [feet] and clutching you in a tight hug");
        else this.outputText("throwing her arms around you and hugging you tightly");
    ...
    private hugHelspawn(): void {
        ...
        if (this.player.tallness <= 72) {
            this.outputText("You reach up and pull " + this.flags[kFLAGS.HELSPAWN_NAME] + " down to your level, wrapping your arms tight around her slender shoulders.  Grinning from ear to ear, the towering girl squeezes you right back, her powerful muscles just about crushing your smaller body.  Her tail slips around your [hips], the warm leather binding you to your beloved ");
            if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) this.outputText("daughter ");
            else this.outputText("ward ");
        ...
        //{If PC is 6' or taller:}
            ...
            this.outputText("You grab " + this.flags[kFLAGS.HELSPAWN_NAME] + " and pull her into a tight hug.  She lets out a girlish giggle as she falls into your arms and squeezes you right back.  Her powerful arms threaten to crush your bigger body, smothering you in affection.  Her tail slips around your [leg], the warm leather binding you to your beloved ");
            if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) this.outputText("daughter ");
            else this.outputText("ward ");
    ...
    private talkToHelspawn(): void {
        ...
        if (this.flags[kFLAGS.HAD_FIRST_HELSPAWN_TALK] == 0) {
            ...
            //{if Sluttymander:
            if (this.flags[kFLAGS.HELSPAWN_PERSONALITY] >= 50) this.outputText("  Even if she wants to be just like mom, she shouldn't be bringing monsters home.  Some of them may be dangerous, and she's not a warrior quite like you and Helia yet.");
            else this.outputText("  She should know better than to bring monsters back.  She's not her mother; lust doesn't have to dominate her life like that.");
            ...
            if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) this.outputText("Of course, as her father, you could always say it's your duty to keep her happy...");
        ...
        //{Kiha must be at camp}
            ...
            this.outputText("\n\n\"<i>C'mon, " + this.championRef() + ", try it!</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " adds happily.  \"<i>It's aunt Kiha's favorite recipe.</i>\"");
        ...
        //{Needs Rath at camp]
            ...
            this.outputText("\"<i>" + this.flags[kFLAGS.HELSPAWN_NAME] + "!</i>\" you hear a ragged old voice call, \"<i>Get back here this instant!  I'm not done with you yet!</i>\"");

            this.outputText("\n\n\"<i>Coming!</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " calls back as Rathazul shuffles into view, waving around what looks like a tiny hammer.  Whispering, " + this.flags[kFLAGS.HELSPAWN_NAME] + " says, \"<i>Could you come with me, " + this.championRef() + "? Rathazul weirds me out.</i>\"");

            this.outputText("\n\nChuckling, you nod and follow " + this.flags[kFLAGS.HELSPAWN_NAME] + " as she returns to Rath's makeshift laboratory and seats herself on one of his tables.  Grumbling about kids, the old rat proceeds to rap his little hammer on her knee - nearly getting kicked in the face for his trouble - before asking her to hold her breath as he listens to her heartbeat.  She's nearly blue in the face before he nods approvingly and tells her she can go. " + this.flags[kFLAGS.HELSPAWN_NAME] + " goes running, happy to get away as Rath turns to you, shaking his head.");
            ...
            if ((this.amilyScene.amilyFollower() && !this.amilyScene.amilyCorrupt())) this.outputText("Little Amily's solution was makeshift at best, and the children are beyond my reach to study.  Still.  ");
        ...
        //{Bath Slut w/ DD or HHH must be at camp}
            ...
            this.outputText("\"<i>Hey, have you seen " + this.flags[kFLAGS.MILK_NAME] + " around anywhere, " + this.championRef() + "?</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " asks, looking around the camp.  \"<i>Mom asked me to milk her, but I haven't seen her around.  She wouldn't have run off, would she?</i>\"");
            if (this.flags[kFLAGS.MILK_SIZE] == 1) this.outputText("\n\nYou tell her it'd be more of a wobble, but");
            else this.outputText("\n\nYou doubt she would have; " + this.flags[kFLAGS.MILK_NAME] + " seems to have a fancy for you, but");
        ...
        //{Isabella must be at camp}
            ...
            if (this.player.race() != "human") this.outputText(" and you yourself have suffered as well");
            this.outputText(".  You smile as Isabella seats herself beside " + this.flags[kFLAGS.HELSPAWN_NAME] + ", joining their voices together for the final chorus that leaves them both shaking as " + this.flags[kFLAGS.HELSPAWN_NAME] + " strums the last, desperate notes, nearly clawing through the strings in her passion.");

            this.outputText("\"<i>");
            if (!this.isabellaAccent()) this.outputText("Sehr gut");
            else this.outputText("Very good");
    ...
    private dontFuckAlex(): void {
        this.clearOutput();
        this.outputText("Stroking " + this.flags[kFLAGS.HELSPAWN_NAME] + "'s hair, you try to tell her that there's a better way, that she doesn't have to just fuck everything she comes across.  Her mother's that way, sure, but she could do so much better, so much more with herself than giving in to constant lust.");
        //{If Sluttymander:}
        if (this.flags[kFLAGS.HELSPAWN_PERSONALITY] >= 50) {
            this.outputText("\n\n" + this.flags[kFLAGS.HELSPAWN_NAME] + " scoffs, shaking her head.  \"<i>But I </i>like<i> mom's way.  I love jilling off, the feeling of my cunt getting stretched and my ass being torn apart by the big toys I buy in town.  That's who I am, " + this.championRef() + ".  I'm like mom, and I don't think... no, I know I don't want to change.  I love sex, even if you stopped me and Alex from it.  I want to feel a gang of minotaurs raping me, I want to jump on a drider's cock and ride him down.  I want to do the things I heard mom doing, and that's just what I'm going to do. I'm sorry.</i>\"");

            this.outputText("\n\nYou try to say something, but " + this.flags[kFLAGS.HELSPAWN_NAME] + " steps away, calling back, \"<i>I love you, " + this.championRef() + ", but I can't be what you want me to be.  I'm going to be a slut like mom, and there's nothing you can do to stop me!  It's who I was born and raised to be.</i>\"");

            this.outputText("\n\nYou may have made a horrible mistake.");
        }
        //{If Chastemander:}
        else {
            this.outputText("\n\n" + this.flags[kFLAGS.HELSPAWN_NAME] + " nods slowly, taking in your words.  \"<i>I... I know, " + this.championRef() + ".  I'm sorry I brought Alex home last night.  It won't happen again.  I don't... I love mother, but I don't want to be like her.  A whore.  She's a great woman, but the things she does... they disgust me.</i>\"");
    ...
    private ohGodEwKihaAndHelspawnSuckAtCooking(): void {
        this.clearOutput();
        this.outputText("You spew the stew onto the ground and grab a nearby waterskin, trying to flush the taste from your mouth.  Kiha and " + this.flags[kFLAGS.HELSPAWN_NAME] + " look on with horror as you wipe your mouth and begin to try and coherently explain just how god awful whatever that... STUFF... you just put in your mouth was.  Shock turns to anger before you’re halfway through admonishing the pair of scaly redheads.  Kiha scowls at you and snatches the ladle from you.");
        this.outputText("\n\n\"<i>Oh, what the fuck do you know anyway, you big ass!  It's perfectly fine, isn't it " + this.flags[kFLAGS.HELSPAWN_NAME] + "?</i>\" she growls, spooning up a mouthful of the stuff before you can stop her.  A heartbeat after she swallows, Kiha goes completely stiff; her eyes growing as wide as saucers as they water.  She collapses backwards, falling onto her back with a muted <i>THUD</i>.");

        this.outputText("\n\n\"<i>K-Kiha?</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " yelps, watching the dragoness collapse into a pile of limp scales and leather.  \"<i>Are you okay?  Is she okay, " + this.championRef() + "?</i>\"");
        ...
        this.outputText("\n\nYou and " + this.flags[kFLAGS.HELSPAWN_NAME] + " are soon working away, stirring up your own special blend of your rations and the spices she and Kiha gathered for the first attempt.  Once you've got the basics in, you step back and hand the ladle over, telling the young 'mander to go wild.  Grinning like a kid, she goes to work, mixing in a wild combination of ingredients.  The cauldron's full to bursting by the time she's done, and no sooner does she ladle out the first bowel of it than Kiha sits up, groaning.");

        this.outputText("\n\n\"<i>The fuck happened?</i>\" the dragoness hisses, rubbing her temple.  \"<i>Feel like I got hit with a hammer.</i>\"");

        this.outputText("\n\n\"<i>You, uh, fell off the edge, aunt Kiha.  And spilled all the stew.  Here, we remade it for you!</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " lies, presenting \"aunt Kiha\" with a bowl full of an aromatic brown goop, full of carrots and spices floating atop it like boats on a placid lake.  At least it smells better.");
    ...
    private umYum(): void {
        ...
        this.outputText("\n\nCringing, you set the offered bowl back down, squeaking out that it's delicious, thank you very much, but you have something to take care of right now; maybe you can have more later.  Kiha rolls her eyes and shoos you off before she and " + this.flags[kFLAGS.HELSPAWN_NAME] + " spoon out their lunch.");
    ...
    private helSpawnMilkHerNow(): void {
        ...
        this.outputText("\n\n" + this.flags[kFLAGS.HELSPAWN_NAME] + " gives a rebellious huff as she clambers down, slipping down to the bottom and giving " + this.flags[kFLAGS.MILK_NAME] + " a gentle nudge.  \"<i>C'mon, cutey, wake up.  Time to relieve some pressure.</i>\"");

        this.outputText("\n\nYawning powerfully, " + this.flags[kFLAGS.MILK_NAME] + " rises to her knees and rubs the sleep from her eyes");
        if (this.flags[kFLAGS.MILK_SIZE] == 2) this.outputText(", already working to unfasten her shift");
        this.outputText(".  \"<i>Suckle?</i>\" she asks, turning her full teats toward " + this.flags[kFLAGS.HELSPAWN_NAME] + ".");

        //If Sluttymander:
        if (this.flags[kFLAGS.HELSPAWN_PERSONALITY] >= 50) this.outputText("\n\n\"<i>Wouldn't have it any other way!</i>\" the salamander answers with an eager grin, easing herself into " + this.flags[kFLAGS.MILK_NAME] + "'s arms and wrapping her lips around one of her prominent teats. " + this.flags[kFLAGS.MILK_NAME] + " gives a pleasured little shudder as " + this.flags[kFLAGS.HELSPAWN_NAME] + " starts to suckle like a babe, drinking down the dusky maid's seemingly endless supply of milk.");
    ...
    public loseSparringToDaughter(): void {
        ...
        if (this.flags[kFLAGS.HELSPAWN_PERSONALITY] >= 50) {
            ...
            if (this.player.lust > 99) this.outputText("succumbing to your own lusts");
            else this.outputText("unable to withstand her unending hail of attacks");
            this.outputText(", " + this.flags[kFLAGS.HELSPAWN_NAME] + " quickly sweeps your [legs] out from under you, dropping you right on your ass.  You collapse with a grunt, ");
            if (this.player.weaponName != "fists") this.outputText("weapon tumbling out of hand");
            else this.outputText("unable to even clench your fists anymore");
            this.outputText(", and a moment later " + this.flags[kFLAGS.HELSPAWN_NAME] + "'s on you, straddling you with her powerful legs, pushing you down into the dirt.");
        ...
        //Else If Chastemander:
            ...
            this.outputText(", " + this.flags[kFLAGS.HELSPAWN_NAME] + " grabs your arm, catching you before you can fall.  \"<i>Easy there, " + this.championRef() + ", I got you,</i>\" she says, pulling you into a quick hug.");
            ...
            if (this.player.HP < 1) this.outputText("recover from that ass kicking");
            else this.outputText("dunk your head");
            this.outputText(".");
        }
    ...
    public beatUpYourDaughter(): void {
        ...
        if (this.flags[kFLAGS.HELSPAWN_PERSONALITY] >= 50 && this.monster.lust > 99) {
            ...
            if (this.flags[kFLAGS.HELSPAWN_INCEST] == 0) this.outputText("  You wash your hands of the defeated slut and head back to camp, leaving her to work through her tension herself.");
                ...
                if (!this.player.hasCock()) this.outputText("sex");
                else this.outputText("your cock");
                this.outputText("... and that if she's lucky, you might tend to her when you've cooled off.");
            }
        }
        else {
            this.outputText(this.flags[kFLAGS.HELSPAWN_NAME] + " stumbles back, dropping her weapon and waving her arms, \"<i>I give, I give.</i>\"");
            ...
            if (this.monster.HP < 1) this.outputText(", rubbing her many bruises");
            else this.outputText(", blushing brightly after your slutty display");
    ...
    public maiVisitsHerKids(): void {
        this.clearOutput();
        this.spriteSelect(68);
        this.outputText("As you head back into camp, you notice Hel and " + this.flags[kFLAGS.HELSPAWN_NAME] + " are sitting around the cook fire in the center of camp, with your foster daughter balanced precariously on the lap of her own father, Mai the fox-girl.");
        ...
        this.outputText("\n\n\"<i>Hi, [name],</i>\" Mai says with a demure smile, arms wrapped around " + this.flags[kFLAGS.HELSPAWN_NAME] + "'s belly, trying to hold the daughter that's near a foot taller than she is.  Finally, laughing, she lets the young salamander go.  " + this.flags[kFLAGS.HELSPAWN_NAME] + " drops down to sit beside her mother and father, and quickly has both a smooth and scaly hand stroking her hair.");
        //If Chastemander:
        if (this.flags[kFLAGS.HELSPAWN_PERSONALITY] < 50) this.outputText("\n\n" + this.flags[kFLAGS.HELSPAWN_NAME] + " happily leans her head against Mai's leg, accepting the affection with her tail swishing gaily behind her, foxy ears twitching as Mai brushes them.");
        else this.outputText("\n\n" + this.flags[kFLAGS.HELSPAWN_NAME] + " leans back, pulling out a flask and taking a deep swill before passing it to her father, who takes it with a knowing smile and knocks it back like a champ.");
        ...
        this.outputText("\n\n\"<i>Heeeeeyyyyyy,</i>\" " + this.flags[kFLAGS.HELSPAWN_NAME] + " whines, crossing her arms with a huff.");
        ...
        this.outputText("\n\n" + this.flags[kFLAGS.HELSPAWN_NAME] + " lights up as Mai slips the gold chain around her daughter's neck.  \"<i>Something to remember me by when I'm not around,</i>\" she says with a wink, letting her kid skip off to find a mirror.");
    ...
    public spiderBrosGift(): void {
        ...
        this.outputText("As you're getting ready to head out for the day, you notice " + this.flags[kFLAGS.HELSPAWN_NAME] + " prancing around camp with a long, brightly-colored scarf wrapped around her neck, standing painfully in contrast with her midnight-black scales and pale flesh.");
    ...
    public hakonAndKiriComeVisit(): void {
        this.clearOutput();
        this.spriteSelect(68);
        this.flags[kFLAGS.HAKON_AND_KIRI_VISIT] = 1;
        ...
        if (this.camp.companionsCount() > 1) this.outputText(" and everyone else's too, much to their chagrin");
        ...
        this.outputText("\n\nIt hardly matters, as a few minutes later, you hear a cry of unabashed glee from the edge of camp, and turn to see " + this.flags[kFLAGS.HELSPAWN_NAME] + " running up to meet a pair of oncoming figures.  You wave as Hel's father swoops his granddaughter up into a great bear hug, hefting the half-breed 'mander off her feet in his powerful arms.  " + this.flags[kFLAGS.HELSPAWN_NAME] + " squeals with delight as she's practically carried into camp, until Hakon sets her down and hugs her mother, pulling Hel into a tight hug before she can finish saying hello.");
        ...
        this.outputText("\n\nTurning to " + this.flags[kFLAGS.HELSPAWN_NAME] + ", Hakon smiles with pride.  \"<i>Gods and demons, girl, you've grown!  What have you two been feeding her, huh?</i>\"");

        this.outputText("\n\n\"<i>And you should see how strong she is!</i>\" Hel grins.  \"<i>Go on, " + this.flags[kFLAGS.HELSPAWN_NAME] + ", show the old scales what you've got.</i>\"");

        this.outputText("\n\n\"<i>Old scales!?  I'll show you old, you little brat.  Come on, hit me!</i>\" Hakon says, just before " + this.flags[kFLAGS.HELSPAWN_NAME] + " punches him right in the chest, knocking him right on his ass.  He tumbles back with a grunt, shaking his head with a wry laugh.  \"<i>Ha!  Maybe these scales are getting old after all.  This one's gonna be a hell of a fighter, though.  Should take her hunting with us one of these days.  What do you say, kid?</i>\"");

        this.outputText("\n\n\"<i>Sure!  ");
        if (this.flags[kFLAGS.HELSPAWN_WEAPON] == "scimitar and shield") this.outputText(this.championRef() + "'s been teaching me how to fight.");
        else this.outputText("Mom's been teaching me how to fight.");
        ...
        if (this.flags[kFLAGS.HELSPAWN_DADDY] == 0) this.outputText("your old man");
        else this.outputText("[name]");
