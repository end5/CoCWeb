...
export class ShouldraFollower extends NPCAwareContent {
    ...
    private shouldraSleeping(changes: number = 0, reset: boolean = false): boolean {
        ...
        if (this.flags[kFLAGS.SHOULDRA_SLEEP_TIMER] < 0) this.flags[kFLAGS.SHOULDRA_SLEEP_TIMER] = 0;
        if (this.flags[kFLAGS.SHOULDRA_SLEEP_TIMER] > 48) this.flags[kFLAGS.SHOULDRA_SLEEP_TIMER] = 48;
        //Is she sleeping?
    ...
    public initialShouldersRecruitment(): void {
        ...
        this.doYesNo(this, this.recruitShouldraForCampSexyTimes, this.noFollowerShoulders);
    }
    ...
    private shouldraAndValaGetBigger(): void {
        this.clearOutput();
        this.spriteSelect(67);
        if (this.flags[kFLAGS.SHOULDRA_MET_VALA] < 1) this.flags[kFLAGS.SHOULDRA_MET_VALA] = 1;
        ...
        this.menu();
        this.addButton(this, 0, "Next", this.pureMacroValaCont);
        this.addButton(this, 1, "Be A Toy", this.pureValaAndShouldraMacroMax);
    }
    ...
    private pureValaAndShouldraMacroMax(): void {
        ...
        if (this.player.gender > 0) {
            ...
            if (this.player.hasCock()) this.outputText(", becoming firmly erect.  You happily piston your hips to grind your " + this.multiCockDescriptLight() + " along the pink-glossed walls, oozing your fluids in delight.");
            else this.outputText(", growing wet, though in this slick fluid, your contribution is a mere drop in the ocean.");
        }
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.cumQ() >= 1000) this.outputText("  Somehow, you pour out almost as much spunk as Vala's orgasm, turning the inside of her honeypot into an off-white mess.");
        }
            ...
            if (this.player.wetness() >= 4) this.outputText(", squirting with joy");
            this.outputText(".");
        }
        ...
        if (this.player.gender == 0) this.outputText(" somehow");
    ...
    public shouldraMeetsCorruptVala(): void {
        ...
        if (this.player.hasCock()) this.outputText("  Your cock slips past the folds, pressed up against your stomach as the underside is massaged with the gentle tightness within.");
    ...
    public sandWitchGetsGhostly(): void {
        ...
        if (this.player.hasCock() && (!this.player.hasVagina() || ShouldraFollower.rand(2) == 0)) {
            ...
            if (!this.player.hasFuckableNipples()) this.outputText("\"<i>You know, maybe I should give you a couple of those chest-mounted treats.  They're pretty fun,</i>\" Shouldra teases, tweaking your [nipples].  ");
    ...
    private shouldraFappinTimes(): void {
        ...
        else if (this.player.gender == 0) this.genderlessShouldraMasturbation();
        else if (this.player.hasCock() && (!this.player.hasVagina() || ShouldraFollower.rand(2) == 0)) this.maleMasturbationProper();
        else {
            if (ShouldraFollower.rand(2) == 0) this.girlyMasturbationWithGhosts();
            else this.shouldraTongueLicksPCs();
        }
    }
    ...
    private genderlessShouldraMasturbation(): void {
        ...
        if (this.flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && this.flags[kFLAGS.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) {
        ...
        //if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 & event occurrence = 0
        else if (this.flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && this.flags[kFLAGS.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) {
        ...
        if (this.flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && this.flags[kFLAGS.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) this.outputText("Within lies a rather rustic yet well-built chest.");
        else if (this.flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && this.flags[kFLAGS.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0) this.outputText("The moment the light reveals what lies within, your [butthole] instinctively clenches shut.  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\" It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
        else this.outputText("Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
        ...
        if (this.flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0) this.outputText("  Just what else did she pack in there since last you peered at its contents back at the ruins?");
        ...
        if (this.flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || this.flags[kFLAGS.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) this.outputText("familiar, ");
        ...
        //if analLooseness >= 3
            ...
            if (this.player.cor < 50) this.outputText(", which only serves to confuse you");
            this.outputText(".");
        }
        ...
        if (this.flags[kFLAGS.ZETSUKO_MET] > 0) this.outputText("\"<i>Your run-in with a particular kitsune inspired me,</i>\" Shouldra teases");
        else this.outputText("Shouldra is too caught up in her machination to give you any hint as to what is going on, leaving you confused");
        ...
        if (this.flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || this.flags[kFLAGS.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) this.outputText("You recognize it instantly as she pulls it free from her chest: the nearly foot-long floppy horse cock she impaled you on before.");
        //{if SHOULDRA_GENDERLESS_FUCK_COUNT = 0}
        else if (this.flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0) this.outputText("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
    ...
    private nongenderlessAnalShouldraMasturbation(): void {
        ...
        //if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 OR event occurrence > 0
        if (this.flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || this.flags[kFLAGS.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) this.outputText("You recognize it instantly as she pulls it free from her chest: the nearly 2-foot floppy horse cock she impaled you on before.");
        //if SHOULDRA_GENDERLESS_FUCK_COUNT = 0
        else this.outputText("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
        this.outputText("  Shouldra smacks the dildo of doom onto a nearby boulder as she wearily rises you to your [feet].  The inferior, forgotten playthings slide out uselessly from the ghoul's gasping craw as she slowly clambers over top the wiggling horsey wang.");
        this.outputText("\n\nToo tired to object or plead, you meekly brace for impact as the wraith lines up its target.  Your immense cheeks part way.  Your rectum gapes wide.  Your anal tongue presses firmly against your taint.  \"<i>Down the hatch!</i>\"");
        this.outputText("\n\nThe forest shudders with your scream.  The phantom easily downs six inches on the first go.  She wraps her tongue around the shaft, forcing more and more into you.  You swear your pelvis is bulging.  Muffled moans blended with laughter emit from your belabored behind.  You dare not guess how deep Shoudra drills the cock into you before she climaxes - hard.  Your voluptuous ass quavers with ferocity alongside her gagged yelping, throwing you from your perch atop the rock back down to the earth.  Her fun over, you begin to feel the magic subsiding.  Having no desire to learn of the consequences of leaving a massive wang lodged in your shrinking anus, you muster what little strength you can to plumb it free.  With the experience settling down, you can't help but laugh - closer to a timid chuckle, as its the best your exhausted body can offer.  You think you hear Shouldra saying something to you, but you're too worn out to listen to her boasting.");
    ...
    private shouldraTongueLicksPCs(): void {
        ...
        //if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 || event occurrence > 0
        if (this.flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || this.flags[kFLAGS.SHOULDRA_TONGUE_LICKS_TIMES] == 0) this.outputText("  The consistency of your forgetfulness while traveling this path leads you to believe that your ethereal inhabitant would rather you not come back here on your own.");
        ...
        if (this.flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && this.flags[kFLAGS.SHOULDRA_TONGUE_LICKS_TIMES] == 0) this.outputText("  Within lies a rather rustic yet well-built chest.");
        else if (this.flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && this.flags[kFLAGS.SHOULDRA_TONGUE_LICKS_TIMES] == 0) this.outputText("  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\"  It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
        else this.outputText("  Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
        this.outputText("  She has you pull the container out from its hiding place, removing the clear tarp she used to protect it from the elements.  Despite the fact that you have yet to control your body once through this entire ordeal, an intrinsic urge to open the container has you bending over to pry into the contents within.");
        ...
        if (this.flags[kFLAGS.SHOULDRA_TONGUE_LICKS_TIMES] == 0) {
            ...
            if (this.player.tongueType > TONUGE_HUMAN) this.outputText("some of ");
        ...
        //if demon tongue
            ...
            if (this.flags[kFLAGS.SHOULDRA_TONGUE_LICKS_TIMES] == 0) this.outputText("  Almost shocked enough to wrest control away from your companion, you begin to babble dissent and try to reel your lengthening tongue back in to no avail.  Shouldra stops for a moment, quite frustrated by the interruption.  Sitting straight up, she consoles you, \"<i>Relax, Champ, it'll stop feeling weird in a minute and you know it's all temporary.</i>\"\n\nGranted a moment to adjust, you let yourself settle down and hand her the reins once more.  She happily goes back to ");
            else this.outputText("\n\n She happily continues ");
    ...
    public shouldraDream1(): void {
        ...
        this.flags[kFLAGS.SHOULDRA_PLOT_LEVEL]++;
        this.flags[kFLAGS.SHOULDRA_TALK_NEEDED] = 1;
        this.doNext(this, this.playerMenu);
    }
    ...
    private shouldraYappin(): void {
        ...
        if (this.flags[kFLAGS.SHOULDRA_TALK_NEEDED] == 1) {
            ...
            if (this.flags[kFLAGS.SHOULDRA_PLOT_LEVEL] == 1) {
                ...
                this.spriteSelect(67);
                if (this.flags[kFLAGS.SHOULDRA_PLOT_LEVEL] < 4) this.flags[kFLAGS.SHOULDRA_PLOT_COUNTDOWN] = 7;
                ...
                if (this.player.findPerk(PerkLib.Incorporeality) >= 0) this.outputText(" - and I mean <b>really</b> possess someone -");
            ...
            //Talk 2 - First Transformation Discussion
                ...
                else this.outputText("  A fleshy, penis-shaped bulge begins to swell out of your loins; looking up at the ghost, you realized she hasn't uttered a single word!");
                ...
                if (this.player.gender == 0) this.outputText("dick");
                ...
                if (this.player.gender == 0) this.outputText("burgeoning dickflesh");
                ...
                if (this.player.gender == 0) this.outputText("Your now erect dick is leaking out pre at an alarming rate, which your possessed hands greedly lap up and distribute all along your length.  ");
            ...
            //Talk 3 - "Recent" Goblin Possession Discussion
                ...
                if (this.player.hasCock()) this.outputText("</i>\"  Shouldra eyes your [cock] gingerly, acting as if she can stare right through your outfit at it.  \"<i>");
                else this.outputText("  ");
                ...
                if (this.player.biggestCockArea() >= 200) this.outputText("Ones like yours?  ");
    ...
    public shouldraFollowerScreen(): void {
        this.spriteSelect(67);
        this.clearOutput();
        if (!this.shouldraSleeping()) this.outputText("You pulse a desire for interaction with Shouldra, summoning her head to form from your shoulder. \"<i>What's on your mind, Champ? I could've found out on my own, but I was... preoccupied.</i>\"");
        ...
        this.simpleChoices(this, "Talk", this.shouldraTalkMenu, "Sex", sex, "", undefined, "Go Away", this.kickFollowerShouldraOut, "Back", this.camp.campFollowers);
    }
    ...
    private shouldraSexMenu(): void {
        ...
        var vala = undefined;
        if (this.flags[kFLAGS.FREED_VALA] != 0 && (this.model.time.hours >= 12 && this.model.time.hours <= 21)) {
        ...
        this.simpleChoices(this, "GhostMasturbate", this.shouldraFappinTimes, "Anal Masturb", anal, "Visit Vala", vala, "", undefined, "Back", this.shouldraFollowerScreen);
    }
    ...
    private shouldraTalkMenu(): void {
        ...
        if (this.flags[kFLAGS.SHOULDRA_PLOT_LEVEL] >= 2 && this.flags[kFLAGS.SHOULDRA_MAGIC_COOLDOWN] <= 0) {
        ...
        else if (this.flags[kFLAGS.SHOULDRA_PLOT_LEVEL] >= 2) this.outputText("\n\nShouldra needs some time before she can grow or shrink one of your body parts permanently.");
        this.simpleChoices(this, "Normal Talk", this.shouldraYappin, "Grow Something", blowShitUp, "Shrink Something", shrinkShit, "", undefined, "Back", this.shouldraFollowerScreen);
    }
    ...
    private shouldraGroPlus(): void {
        ...
        this.choices(this, "Balls", balls, "Breasts", breast, "Clit", clit, "Cock", cock, "Nipples", nipples, "Butt", butt, "", undefined, "", undefined, "", undefined, "Back", this.shouldraTalkMenu);
    }
    ...
    private groBallsBiggaGHOSTYSTYLE(): void {
        ...
        //if ballsize > 10
        if (this.player.ballSize > 10) this.outputText("  You second guess your decision when your swinging cum barrels seem to impede walking.  Shouldra reassures you that the enhanced libido and baby batter supply is worth the wider gait.");
    ...
    private shouldraGrowsYoTits(): void {
        ...
        if (this.player.breastRows.length == 1) this.player.growTits((1 + ShouldraFollower.rand(5)), 1, true, 1);
        else this.player.growTits(1 + ShouldraFollower.rand(2), this.player.breastRows.length, true, 1);
        this.dynStats("lus", 10);
        this.doNext(this, this.camp.returnToCampUseOneHour);
    }
    //Clit
    ...
    private shouldraReductoMenu(): void {
        ...
        if (this.player.nippleLength > .25) nipples = this.shrinkDemNipplzForYoGhost;
        ...
        if (this.player.balls > 0 && this.player.ballSize > 1) balls = this.shouldraReductosYourBallsUpInsideYa;
        if (this.player.hasCock() && this.player.longestCockLength() > 4) cock = this.shouldraMakesCocksDisappear;
        if (this.player.hasVagina() && this.player.clitLength > .25) clit = this.clittyVanishingActShouldra;
        this.choices(this, "Balls", balls, "Breasts", breasts, "Clit", clit, "Cock", cock, "Nipples", nipples, "Butt", butt, "", undefined, "", undefined, "", undefined, "Back", this.shouldraTalkMenu);
    }
    //Balls
    ...
    private shouldraMakesCocksDisappear(): void {
        ...
        if (this.player.cocks.length == 1) {
            ...
            else if (choice == 3) this.outputText("monster");
            else if (choice == 4) this.outputText("cretin");
            else if (choice == 5) this.outputText("fiend");
            else if (choice == 6) this.outputText("asshole");
            else if (choice == 7) this.outputText("villain");
            else this.outputText("dick");
    ...
    private shrinkDatBootyForYoGhost(): void {
        ...
        if (this.player.buttRating >= 15) {
            ...
            this.player.buttRating -= 1 + ShouldraFollower.rand(3);
            if (this.player.buttRating < 1) this.player.buttRating = 1;
    ...
    private kickFollowerShouldraOut(): void {
        ...
        if (this.flags[kFLAGS.SHOULDRA_PLOT_LEVEL] < 4 || this.flags[kFLAGS.SHOULDRA_TALK_NEEDED] > 0) {
        ...
        //She Fucking Explodes
            ...
            this.doYesNo(this, this.definitelyKickOutCrazyGhosts, this.shouldraTalkMenu);
        }
    }
    ...
    public shouldersWarnings(): boolean {
        ...
        //7 days (168 hours) since last Shouldra scene
            ...
            if (ShouldraFollower.rand(3) == 0) {
                ...
                if (this.player.gender == 0) this.outputText("libido seems to be going haywire");
    ...
    public nightTimeShouldraRapesThePC(): void {
        ...
        if (this.player.biggestTitSize() >= 1) this.outputText(", circling their entire lengths around your [fullChest]");
        ...
        if (this.player.hasCock()) this.outputText("magically enlarged member");
        else this.outputText("magically gaped snatch");
        ...
        this.doNext(this, this.edwardShouldraDickHandsPartII);
    }
    ...
    public shouldraWakesUpOrPokesPCsForShitsAndGigglesIdunnoHowLongCanIMakeThisFunctionNameQuestionMark(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.PureCampJojo) >= 0) choices[choices.length] = 15;
        //16 (has any of Marae's gift perks)
        if (this.player.findPerk(PerkLib.MaraesGiftStud) >= 0 || this.player.findPerk(PerkLib.MaraesGiftFertility) >= 0 || this.player.findPerk(PerkLib.MaraesGiftProfractory) >= 0 || this.player.findPerk(PerkLib.MaraesGiftButtslut) >= 0) choices[choices.length] = 16;
        //17 (has had children with Izma)
        if (this.flags[kFLAGS.IZMA_CHILDREN_SHARKGIRLS] + this.flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS] > 0) choices[choices.length] = 17;
        ...
        switch (select) {
            ...
            case 1:
                ...
                if (subSelect == 0) this.outputText("Your [cock] tingles, and your [armor] suddenly feels much tighter.  A little giggle from Shouldra reveals her intention.  Though adamant about making you as big and virile as possible, Shouldra does realize it <b>is</b> your body, and your meat shrinks at least three inches to its former glory.");
                else if (subSelect == 1) this.outputText("An uncomfortable stirring in your nether regions has you stop to undo your [armor] and figure out what's the matter.  Your [cock] is wagging enthusiastically.  Shouldra's face quickly sprouts from your shoulder, \"<i>Oh look, he's happy to see you!</i>\" You pay her an irritated glance which she returns with a content snicker.  With your cock under control, you redo your [armor] and continue on your way.");
                else if (subSelect == 2) this.outputText("You're distracted as you notice your [cock] snaking its way down your [leg], quickly growing larger.  You make your displeasure obvious, causing it to recoil back to its original size.  Seems as though your ghostly friend is getting restless.");
                else if (subSelect == 3) this.outputText("You come to a halt as you suddenly lose control of your arms, forcing you to watch your hands dart down your [armor] to fiercely grab at your [cock].  Though you let out a growl for her to stop, it takes Shouldra a minute to return your arms to you.  She must be getting restless.");
                else if (subSelect == 4 && this.player.horseCocks() == 0) this.outputText("There's a sudden twisting pain in your [cock], forcing you to undo your [armor] and see what's wrong.  To your dismay, a large equine dong is staring back at you, leaking profusely.  \"<i>Just testing out some new transformations, Champ,</i>\" Shouldra snickers.  You yell out again as your dick quickly turns back to normal.");
            ...
            case 6:
                ...
                if (subSelect == 0) this.outputText("While minding your business, you realize faint whispering is coming from your [cock].  Shouldra and Exgartuan are scheming something, probably something you wouldn't be a fan of.  Every so often your wang lets out a chuckle.  Maybe getting these two together wasn't in your best interest.");
                else if (subSelect == 1) this.outputText("Your [cock] is wiggling around inside your [armor], making you incredibly uncomfortable.  You go to undo your outfit when the demonic dong plows its way out anyway, scaring you half to death.  Shouldra and Exgartuan just laugh, too content with one another to pay you much mind.  You never expected them to get along THIS well.");
                else if (subSelect == 2) this.outputText("\"<i>So what other sorts of things can you concoct?</i>\" Exgartuan asks, shuffling around in your armor.  You're incredibly confused by the sudden question.\n\n\"<i>Honestly, that put me in the mood to make all of mother nature our bitch,</i>\" you respond... Shouldra responds.  The spirit is gabbing through your mouth to your possessed pecker.  The two prattle on like this for some time.");
                else if (subSelect == 3) this.outputText("Your hands slip inside your [armor], exploring your [cock].  Your fingers caress each little nub and nodule they come across.  \"<i>That's the ticket,</i>\" Exgartuan gleefully responds, getting rigid in the tight confines of your outfit.  Looks like your ghostly partner is out to make everyone happy.");
                else if (subSelect == 4) this.outputText("\"<i>Why don't you help out during battles, babe?</i>\" Exgartuan asks rather bluntly.\n\n\"<i>Hey, I've offered to help out with Champ's more seductive poses like you do...</i>\"\n\nIt isn't like you have much choice in the matter as far as Exgartuan is concerned!  The two continue their discussion, belittling your apparently meager combat performance.  Maybe your possessors could actually help deal some real damage for a change!");
                else this.outputText("Your [armor] has vanished!  You're butt naked!  It's pretty obvious who's at fau-\n\n\"<i>Really, [name], you don't even need it,</i>\" Exgartuan says, surprising you.\n\nShouldra laughs, \"<i>Right, Champ?  Two sexually-charged tyrants dwelling within you and you insist on sneaking around like common rabble?  Why not let it all hang out?</i>\"\n\nYour hands caress your [cock], exciting the demonic member into attention.  You remind the couple that your outfit isn't <b>just</b> for protection, citing many other-  Oh, your [armor] is back.  Guess they don't want to hear you prattle.");
                break;
            //General flavor text
            case 7:
                subSelect = ShouldraFollower.rand(6);
                if (subSelect == 0) this.outputText("\"<i>Yoooo, Champ, are we gonna fuck something, or am I just gonna stagnate here?</i>\" Shouldra moans, managing to steal control of an arm long enough to give your [butt] a honk.  You sigh.");
                else if (subSelect == 1) this.outputText("Out of nowhere, a colossal yawn bursts forth from your craw.  \"<i>Come on, Champ. I'm dying to try out some new stuff I've cooked up.</i>\" What is this sense of dread consuming your being?");
                else if (subSelect == 2) this.outputText("It's been pretty quiet recently.  No doubt Shouldra is up to something -- she always is.  Honestly, you've forgotten what it's like to not having your body passing for a bloated something or other.");
                else if (subSelect == 3) this.outputText("As you are going about your business, you swear you make out muttering emanating from somewhere on your person. It sounds like the ghost girl is reciting some magic, but you don't feel any different. This only worries you even more.");
                else if (subSelect == 4) this.outputText("\"<i>Do foot fetishes still apply if it's your own foot?</i>\" Shouldra asks.  For a moment you wonder how such a feat would even be possible.  But only for a brief, forgetful moment...");
                else if (subSelect == 5) this.outputText("A familiar voice works its way from the back of your mind, \"<i>Hey, Champ.  You think there's any way to make armpits sexy?  What about knees or noses?</i>\"  This line of questioning could lead to some very troubling consequences...");
                else this.outputText("\"<i>I like what you did with those racks,</i>\" Shouldra says unexpectedly, her voice losing its mischievous edge for a moment.  \"<i>I'm glad you kept them.</i>\"  You both sit in silence for several moments as you consider her mood shift to tenderness, but the moment passes.  Shouldra chuckles softly.  \"<i>Just thought I'd mention.</i>\"");
                break;
            ...
            case 17:
                this.outputText("\"<i>");
                if (this.flags[kFLAGS.IZMA_CHILDREN_SHARKGIRLS] + this.flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS] == 1) this.outputText("That shark daughter of yours is");
                else this.outputText("Those shark daughters of yours are");
    ...
    private shouldraAndWormsYoureGonnaHaveABadTime(): void {
        ...
        if (this.player.cocks[0].cockLength < 48) this.outputText(" and cuts right to the chase, ballooning your [cock] into a rigid, wide, four-foot tall slab of meat");
        ...
        if (this.model.time.hours <= 12) this.outputText("morning ");
        ...
        this.simpleChoices(this, "Keep Shouldra", this.kickOutWormiesForYourGhostPalPAL, "Keep Worms", this.kickOutShouldra4YoWormyBuddies, "", undefined, "", undefined, "", undefined);
    }
    //Keep Worms
    private kickOutShouldra4YoWormyBuddies(): void {
        ...
        if (this.player.cocks[0].cockLength < 48) this.outputText("  With that little ordeal dealt with, you now are left to decide how best to manage the time until your cock returns to its normal size.");
    ...
    public exgartuMonAndShouldraShowdown(): void {
        ...
        if (this.silly()) this.outputText("  It's a man's right to choose what's done with his body.  Not anyone else's!");
        ...
        this.simpleChoices(this, "Keep Exgartuan", this.keepExgartuanInsteadOfShouldra, "Keep Shouldra", this.keepShouldraAndKickOutExgartuan, "Keep Both", this.keepAllTheGhosts, "", undefined, "", undefined);
    }
    //Keep Exgartuan
    ...
    public keepShouldraPartIIExgartumonsUndeatH(): void {
        ...
        //[next page]
        this.doNext(this, this.keepShouldraPartIIIExgartumonsUndeatH);
    }
    ...
    private keepAllTheGhosts(): void {
        ...
        if (this.player.cumQ() >= 1000) this.outputText(", trails of your cum pouring out from them");
        this.outputText(".");
    ...
    public shouldraBakeryIntro(): void {
        ...
        this.simpleChoices(this, "Continue", this.feedShouldraACake, "Bail Out", this.bailOut, "", undefined, "", undefined, "", undefined);
    }
    ...
    private feedShouldraACake(): void {
        ...
        //Maddie
        this.simpleChoices(this, "Maddie", this.shouldraAndMaddieSittingInATree, "", undefined, "", undefined, "", undefined, "", undefined);
    }
    ...
    private shouldraAndMaddieSittingInATree(): void {
        ...
        if (this.player.tone >= 60) this.outputText("well-toned");
        ...
        this.simpleChoices(this, "Go Along", this.goAlongWIthShouldrasEatingSpree, "Resist", this.resistBeingAFatass, "", undefined, "", undefined, "", undefined);
    }
    //Go along with it
    private goAlongWIthShouldrasEatingSpree(): void {
        ...
        if (this.flags[kFLAGS.PC_FETISH] > 0) this.outputText(", though not completely unwelcome");
        ...
        if (this.flags[kFLAGS.PC_FETISH] > 0) {
            ...
            if (this.player.hasVagina()) this.outputText("trembling nether-lips");
            if (this.player.gender == 0) this.outputText("groin");
        }
        ...
        if (this.flags[kFLAGS.PC_FETISH] > 0) this.outputText(", even with that kink of yours");
        this.outputText(", Champ.  Here's something extra anyway, just in case.</i>\" One ghostly incantation later, and ");
        if (this.player.femininity <= 50) this.outputText("a buttoned shirt and pair of belted jeans");
        else this.outputText("a tight-fitting tank top and skirt");
        ...
        if (this.player.hasCock()) {
            ...
            if (this.silly()) this.outputText("  This is a <i>fucking good eclair</i>.");
            ...
            if (this.player.cocks[0].cockLength < 8 || this.player.cocks[0].cockLength > 25) this.outputText("\"<i>First though, let's change it to something more manageable.</i>\" Arcane sounds float up towards you, causing your shaft to shift and morph until your throbbing rod is around 2 feet long and dripping pre.  \"<i>Mmm, much better!</i>\"  ");
        ...
        this.flags[kFLAGS.SHOULDRA_BAKERY_TIMES]++;
        this.player.slimeFeed();
    }
