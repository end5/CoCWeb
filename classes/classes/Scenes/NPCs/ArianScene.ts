...
export class ArianScene extends NPCAwareContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (this.model.time.hours > 23) {
            if (this.arianScene.arianFollower() && this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.flags[kFLAGS.ARIAN_EGG_EVENT]++;
            this.flags[kFLAGS.ARIAN_LESSONS] = 0;
            this.flags[kFLAGS.ARIAN_TREATMENT] = 0;
        }
        return false;
    }
    ...
    private arianCockSize(): number {
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] < 0 || this.flags[kFLAGS.ARIAN_COCK_SIZE] > 3) return 0;
        else if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 1) return 9;
        else if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 2) return 16;
        else return 36;
    }

    public arianFollower(): boolean {
        return this.flags[kFLAGS.ARIAN_FOLLOWER] > 0;
    }
    public arianMF(boy: string, girl: string): string {
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) return girl;
            else return boy;
        }
        return girl;
    }
    public arianHealth(arg: number = 0): number {
        if (arg != 0) {
            ...
            else if (this.flags[kFLAGS.ARIAN_HEALTH] < 0) this.flags[kFLAGS.ARIAN_HEALTH] = 0;
        }
    ...
    public arianChestAdjective(): string {
        ...
        else if (this.flags[kFLAGS.ARIAN_BREASTS] == 2) {
    ...
    private helpArianWhenYouMeetHim(): void {
        ...
        this.flags[kFLAGS.ARIAN_PARK] = 1;
        this.flags[kFLAGS.ARIAN_COCK_SIZE] = 1;
        this.flags[kFLAGS.ARIAN_CAPACITY] = 50;

        this.outputText("You approach the hooded figure with caution, asking if they're all right; it feels a little silly to say that, but you can't think of much else to say.");
    ...
    public visitThePark(): void {
        this.clearOutput();
        this.outputText("As you enter the ragged remnants of the park, you spot the sickly lizan, Arian, sitting at his usual bench, and greet him.  \"<i>Oh, hello there [name].  Good to see you.</i>\"  He waves lazily.");

        //Visit 1
        if (this.flags[kFLAGS.ARIAN_PARK] == 1) {
            this.outputText("\n\nFeeling ");
            if (this.player.cor < 50) this.outputText("curious");
            else this.outputText("bored");
            this.outputText(", you decide to ask him what his story is.");
            ...
            if (this.player.hasSpells()) {
                this.outputText("  You thought spellcasting merely took fatigue and the proper mindset, not life force, and you express that sentiment to the lizan.");
            }
        ...
        //Visit 3
            ...
            if (this.player.cor > 66) this.outputText(", whilst privately thinking to yourself how useless that looks - no wonder they can't fight the demons if this is the best they're capable of");
            this.outputText(".");
            ...
            if (this.player.cor >= 66) this.outputText(", keeping your real reasons for bothering with him to yourself");
            this.outputText(".");
    ...
    public visitAriansHouse(): void {
        this.clearOutput();
        if (this.flags[kFLAGS.ARIAN_HEALTH] < 29 || this.flags[kFLAGS.ARIAN_VIRGIN] == 1) this.arianHealth(1);
        if (this.arianFollower()) {
            this.outputText("You approach the enchanted tent and slip easily inside the doors to the luxurious interior.  ");
            var temp: number = ArianScene.rand(10);
            if (temp == 0) {
                this.outputText("However, Arian isn't here right now, so you instead make yourself comfortable on the couch.  After a few minutes, Arian [Arian emself] walks in through the entrance.  \"<i>Oh, [name].  I wasn't aware you were here... have you been waiting for long?</i>\" [Arian ey] asks.  You tell [Arian em] not very long.  \"<i>That's good to hear.  So, what can I do for you?</i>\" [Arian ey] asks, with a smile.");
            }
            else if (temp == 1) {
                this.outputText("Inside, the lizan is sitting at a table, fastidiously drinking from a cup of something hot while pouring over an arcane-looking text.  You politely cough to draw [Arian eir] attention and [Arian ey] looks at you, smiling.  \"<i>Hello, [name].  I was just catching up on my studies.  Can I offer you a cup of tea, or maybe something else?</i>\" [Arian ey] asks.");
            }
            else if (temp <= 2) {
                this.outputText("The lizan is currently busy tinkering with some occult-looking paraphernalia when you find [Arian em].  You politely cough to attract [Arian eir] attention, then do so again when [Arian ey] fails to heed that.  It's only on the third attempt that [Arian ey] looks up apologetically from [Arian eir] work.  \"<i>Ah, [name]; I'm sorry, but I was preoccupied with something.</i>\" [Arian ey] states in an apologetic tone, indicating the mess on [Arian eir] desk.  \"<i>Was there something you wanted?</i>\" [Arian ey] asks.");
            }
            else if (temp <= 4) {
                this.outputText("The smell of fresh cooking fills the air and you can see Arian happily sitting down at his couch with a plate of something just cooked.  \"<i>Oh, [name]; I was just about to eat, can I offer you a dish?  Or if you'd rather do something else, this can wait,</i>\" the lizan tells you with a smile.");
            }
            else if (temp <= 6) {
                ...
                this.outputText("A strange smell hits your nose as you enter the tent; it takes you a few moments, but then you place it, your ears pricking as you hear Arian letting out some very familiar groans of release.  With a smirk, you sneak up and lean over the couch, looking right into the eyes of Arian, [Arian eir] fingers still glistening with ");
                if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                    if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText("mixed sexual fluids");
                    else this.outputText("jizz");
                }
                else this.outputText("femjizz");
                this.outputText(" and, indeed, still hovering over [Arian eir] ");
                if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                    if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText("twin sexual slits");
                    else {
                ...
                this.outputText(".  For a long moment, [Arian ey] just stares back at you; if lizans could blush, you're certain [Arian ey]'d be red as a beet.  \"<i>[name]! I was- I was just....  Oh, this is embarrassing,</i>\" [Arian ey] mutters, looking at [Arian eir] feet.");
            ...
            if (this.flags[kFLAGS.ARIAN_PARK] == 4) {
                ...
                if (this.player.cor < 33) this.outputText("curiously");
                else if (this.player.cor < 66) this.outputText("casually");
                else this.outputText("indifferently");
                this.outputText(" ask why he's thanking you.");
                ...
                if (!this.player.isTaur()) this.outputText("pull up a chair");
                else this.outputText("seat your tauric body on the floor");
                this.outputText(".  You rack your brains for polite conversation, and finally ask how he's been since you saw him last.");
                ...
                if (this.flags[kFLAGS.ARIAN_VAGINA] > 0 && this.flags[kFLAGS.ARIAN_HERM_CHAT] == 1) {
                    ...
                    this.outputText("\n\n\"<i>Coming!</i>\"  You hear Laika yell.  Shortly after the ferret girl opens the door and greets you.  \"<i>Hello [name].  Came to visit " + this.arianMF("master", "mistress") + " Arian?  Please come in!</i>\"  She motions for you to enter the house.  You thank her for the polite invitation and step through the doorway.  The sound of dishes being washed draws your attention to the nearby kitchen, where you see Boon diligently washing a pan.  He looks up and waves at you with a soapy hand.  You return the gesture.");
                    this.outputText("\n\n\"<i>Sorry to leave you unattended [name], but we're kinda busy; do you think you can make the way to " + this.arianMF("master", "mistress") + " Arian's room by yourself?</i>\"  Laika asks.");
                ...
                //20-29 health:
                    ...
                    if (this.flags[kFLAGS.ARIAN_VIRGIN] == 0 && this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 2) {
                        ...
                        this.menu();
                        this.addButton(this, 0, "Eavesdrop", this.eavesDropOnArian);
                        this.addButton(this, 1, "Peep", this.peepOnArian);
                        this.addButton(this, 2, "Barge In", this.bargeInOnArian);
                        this.addButton(this, 3, "Leave", this.leaveFappingArian);
                        return;
                    }
                    //(else)
                    else {
                ...
                //50-74 health
                    ...
                    if (!this.player.isTaur()) this.outputText("take the offered chair");
                    else this.outputText("the offered chair");
                    this.outputText(".");
                ...
                //75-100 health:
                    ...
                    if (!this.player.isTaur()) this.outputText(" and offers you the chair nearby");
                    this.outputText(".  Taking another for [Arian em]self.  \"<i>I love when you come visit, [name].  So, what are we going to do today?</i>\" [Arian ey] asks, expectantly.");
                }
                //(Display Options)
                this.arianHomeMenu();
            }
    ...
    private arianHomeMenu(): void {
        this.menu();
        if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 0 && this.arianHealth() >= 10) this.addButton(this, 0, "Next", this.arianStoryDialogue1);
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 1 && this.arianHealth() >= 20) this.addButton(this, 0, "Next", this.arianStoryDialogue2);
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 2 && this.arianHealth() >= 30) this.addButton(this, 0, "Next", this.arianDialogue3);
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 3 && this.arianHealth() >= 50) this.addButton(this, 0, "Next", this.arianImbue);
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 4 && this.arianHealth() >= 75) this.addButton(this, 0, "Next", this.arianPlot4);
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 5 && this.arianHealth() >= 100) this.addButton(this, 0, "Next", this.arianPlot5);
        //If no story dialogue
        else {
            this.addButton(this, 0, "Talk", this.talkToArianChoices);
            if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] >= 2) this.addButton(this, 1, "Sex", this.arianSexMenu);
            if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] >= 1) this.addButton(this, 3, "Give Item", this.giveArianAnItem);
            if (this.player.hasKeyItem("Arian's Talisman") >= 0 || this.player.hasKeyItem("Arian's Charged Talisman") >= 0)
                this.addButton(this, 2, "Talisman", this.imbueTalisman);
            if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] >= 5) this.addButton(this, 4, "Treat Corr.", this.treatCorruption);
            if (this.model.time.hours >= 17 && this.arianFollower()) this.addButton(this, 8, "Sleep With", this.sleepWithArian, true);
            if (this.flags[kFLAGS.SLEEP_WITH] == "Arian") this.addButton(this, 8, "NoSleepWith", this.dontSleepWithArian);
            if (!this.arianFollower()) this.addButton(this, 9, "Back", this.telAdre.telAdreMenu);
            else this.addButton(this, 9, "Back", this.camp.campLoversMenu);
        }
    }

    private dontSleepWithArian(): void {
        this.clearOutput();
        this.outputText("You decide not to sleep with Arian at night, for now.");
    ...
    private eavesDropOnArian(): void {
        this.clearOutput();
        this.outputText("You sidle up to the door, pressing your ear against the wood and start to listen intently.");
        ...
        this.dynStats("int", 1);
        this.menu();
        this.addButton(this, 0, "Barge In", this.bargeInOnArian);
        this.addButton(this, 4, "Leave", this.leaveFappingArian);
    }

    //[=Peep=]
    private peepOnArian(): void {
        this.clearOutput();
        this.outputText("Curious, you decide to take a little peek through the lock; you press yourself against it as best you can, looking through into the bedroom beyond.  True to what your ears heard, the sickly albino's health has improved enough for him to focus on more... carnal matters.  Naked from the waist down, he sits on the edge of his bed, groinal slit disgorging a single, average-sized phallus.  Maybe 6 inches long, it's a bright purple-red color, covered in strange lumps");
        if (this.player.lizardCocks() > 0) this.outputText(" just like yours");
        this.outputText(", though this isn't stopping him from enthusiastically stroking himself off.");
    ...
    private leaveFappingArian(): void {
        this.clearOutput();
        this.outputText("You decide to let Arian have some privacy and leave for the moment... after all, what the lizan mage does in his free time is not really your business....");
    ...
    private bargeInOnArian(): void {
        this.clearOutput();
        this.outputText("With a wry smirk you turn the knob and find that Arian's door is unlocked; without missing a beat, you open the door and step in right in time to see a sticky rope of pre paint Arian's slender belly as he scrambles to cover himself up.");
        ...
        if (this.player.hasCock() && !this.player.hasVagina()) {
            ...
            this.menu();
            this.addButton(this, 0, "Don't Mind", this.youDontMindBeingGayForArian);
            this.addButton(this, 1, "Like Girls", this.youLikeGirlsNotSickLizardDudes);
        }
        ...
        //(else if PC is female)
            ...
            this.menu();
            this.addButton(this, 0, "Next", this.getPenetratedByArianAndHisHitlerMustache);
        }
        //(else) //if PC is a herm
            ...
            this.menu();
            this.addButton(this, 0, "Like Male", this.hermsLikeMaleArian);
            this.addButton(this, 1, "Like Female", this.hermsLikeFemaleArian);
        }
    ...
    private hermsLikeMaleArian(): void {
        this.clearOutput();
        this.outputText("You tell him that's not the case for you; you don't have a problem with him being a guy.  In fact, you think he looks very cute, earning you a nervous smile.  Arian relaxes, letting you look over his body and decide what you want to do....");
        //(Should you penetrate him or mount him?)
        //Penetrate - (Proceed to appropriate scene)
        //Get Penetrated - (Proceed to appropriate scene)
        this.menu();
        if (this.player.hasCock() && this.player.cockThatFits(50) >= 0) this.addButton(this, 0, "Penetrate", this.giveArianAnal);
        this.addButton(this, 1, "Get Penetrated", this.getPenetratedByArianAndHisHitlerMustache);
    }
    //[=Prefer Female=]
    private hermsLikeFemaleArian(): void {
        this.clearOutput();
        this.outputText("You tell him that while you do like to play with guys once in a while, you prefer girls.");
        ...
        this.flags[kFLAGS.ARIAN_BREASTS] = 1;
        this.menu();
        this.addButton(this, 0, "Next", this.penetrateArian);
    }

    //[=Don't mind=]
    private youDontMindBeingGayForArian(): void {
        this.clearOutput();
        this.outputText("You tell him that you don't have a problem with males, as long as they're cute.  You smile at him.  \"<i>You... do you really think I'm cute?</i>\"");
        ...
        this.menu();
        if (this.player.cockThatFits(50) == -1) {
            this.addButton(this, 0, "Next", this.getBlownByArian);
        }
        else {
            this.addButton(this, 0, "Next", this.giveArianAnal);
        }
    ...
    private youLikeGirlsNotSickLizardDudes(): void {
        this.clearOutput();
        this.outputText("You tell him that you prefer females.... Arian looks at you expectantly.  \"<i>So... if I was a girl... then you wouldn't mind?</i>\"");
    ...
    private arianStoryDialogue2(): void {
        this.clearOutput();
        this.arianHealth(1);
        this.outputText("You look Arian over, remarking that he seems to be getting better after all.");
        ...
        //[Drop It] [Pry]
        this.menu();
        this.addButton(this, 0, "Drop It", this.arianStory2DropIt);
        this.addButton(this, 1, "Pry", this.arianStoryPry);
    }

    //=Drop It=
    private arianStory2DropIt(): void {
        this.clearOutput();
        this.outputText("Though you do feel a little curious, you decide to stop making him uncomfortable, and tell him that it's okay, you'll let him get some sleep now.");
    ...
    private arianStoryPry(): void {
        this.clearOutput();
        this.outputText("Oh, no, you're not letting him wriggle out of this that easily.  You playfully tap [Arian eir] nose and tell him he should come clean and confess");
        if (this.player.cor < 40) this.outputText("; he'll sleep better with the burden off [Arian eir] conscience");
        this.outputText(".");
    ...
    private arianDialogue3(): void {
        this.clearOutput();
        this.arianHealth(1);
        this.outputText("Before you can say anything, Arian asks you, \"<i>[name], I've been wondering....  Do you have any interest in magic?  You've done so much for me; I believe I should return the favor somehow.</i>\"");
        //ArianSDialogue++;
        this.flags[kFLAGS.ARIAN_S_DIALOGUE]++;
        //[Yes] [No]
        this.menu();
        this.addButton(this, 0, "Yes", this.yesArianShouldMagicTeach);
        this.addButton(this, 1, "No", this.noArianShouldntMagicTeach);
    }

    //=Yes=
    private yesArianShouldMagicTeach(): void {
        this.clearOutput();
        this.outputText("You tell [Arian em] that sounds fascinating.  You'd love to learn how to cast spells the way [Arian ey] can, and you're grateful [Arian ey] wants to take you on as an apprentice.  Especially when [Arian ey]'s already so busy with the ones [Arian ey] already has.  Arian rubs the back of [Arian eir] neck.  \"<i>Sorry, [name].  But I can't actually teach you how to cast spells the same way I do....  That would take years to teach, not to mention it's very dangerous; I mean, look at what it's done to me....</i>\"  [Arian Ey] smiles at you.  \"<i>But I could still teach you about magic in general - how to cast more spells, how to make them more powerful, the principles behind every spell....  Basically, theory that might help you in the pursuit of magical studies.  I spent my whole childhood buried in books, so I'm sure I could help you out somehow.</i>\"");
        ...
        this.menu();
        this.addButton(this, 0, "Next", this.arianMagicLessons);
    }
    ...
    private arianImbue(): void {
        this.clearOutput();
        this.arianHealth(1);
        this.outputText("Before you can say anything, Arian gasps, \"<i>Oh, [name].  I have a surprise for you.</i>\"  Arian says with a smile.");
        ...
        //[Yes] [No]
        this.menu();
        this.addButton(this, 0, "Yes", this.yesPlotSexArian);
        this.addButton(this, 1, "No", this.noPlotSexNauArian);
    }

    //=Yes=
    private yesPlotSexArian(): void {
        this.clearOutput();
        this.outputText("You approach the awkwardly amorous lizan and place your arms around [Arian eir] neck.  Leaning in close, you whisper into [Arian eir] ear that [Arian ey] only had to ask.");
        //(Display Sex Menu)
        this.arianSexMenu(false);
    }
    ...
    private noPlotSexNauArian(): void {
        this.clearOutput();
        this.outputText("You apologize to the lizan, telling [Arian em] that you aren't in the mood right now....");
    ...
    private arianPlot4(): void {
        this.clearOutput();
        this.arianHealth(1);
        this.outputText("Before you can say anything, Arian says, \"<i>Oh, I have good news, [name]!</i>\"");
    ...
    private arianPlot5(): void {
        this.clearOutput();
        this.arianHealth(1);
        this.outputText("Before you can say anything, Arian stops you.  \"<i>I've been meaning to ask you something, [name].  I've been feeling a lot better lately; in fact, I may be even better than I was before.</i>\"  Arian blushes.");
        ...
        if (this.camp.companionsCount() > 1) this.outputText("  There are other people living with you already, so what's one more?  Right?");
        this.outputText("</i>\"");
        ...
        //[Accept] [Deny]
        this.menu();
        this.addButton(this, 0, "Accept", this.acceptArianMovingIntoCamp);
        this.addButton(this, 1, "Deny", this.denyAriansMoveIn);
    }

    //[=Accept=]
    private acceptArianMovingIntoCamp(): void {
        this.clearOutput();
        this.outputText("You tell Arian you'd be delighted to have [Arian em] move in with you.  Arian's face lights up like a kid's who's been given a bucket of candy.  \"<i>Really!?  Great!  I'll pack my stuff and we can go right away!</i>\"");

        //(Skip to ‘Invite to Camp')
        this.menu();
        this.addButton(this, 0, "Next", this.inviteArianToCamp);
    }

    //[=Deny=]
    private denyAriansMoveIn(): void {
        this.clearOutput();
        this.outputText("You tell Arian you'd like some time to think about it.  Arian looks disappointed at first, but smiles at you all the same.  \"<i>I understand... no pressure....  So, what are we going to do today?</i>\"");
    ...
    private talkToArianChoices(): void {
        this.clearOutput();
        this.outputText("You tell Arian you'd like to talk to [Arian em].  Arian smiles at the prospect of chatting with you.  \"<i>I love talking with you; so what do you want to talk about?</i>\"");

        this.menu();
        if (this.flags[kFLAGS.ARIAN_VIRGIN] > 0) this.addButton(this, 0, "Sexy Talk", this.arianSexingTalk);
        if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] >= 3) this.addButton(this, 1, "Teach Magic", this.arianMagicLessons);
        if (!this.arianFollower() && this.flags[kFLAGS.ARIAN_S_DIALOGUE] >= 6) this.addButton(this, 4, "Invite2Camp", this.inviteArianToCamp);
        if (this.flags[kFLAGS.ARIAN_VIRGIN] == 0 && this.flags[kFLAGS.ARIAN_S_DIALOGUE] < 3) this.outputText("\n\n<b>Arian doesn't have much to talk about right now.  Maybe you ought to just visit him from time to time or find him an item that would help combat [Arian eir] sickness.</b>");
        this.addButton(this, 9, "Back", this.arianHomeMenu);
    }
    ...
    private arianMagicLessons(): void {
        ...
        //(else if PC int < 50 //Teach Charge Weapon if PC doesn't know.
            ...
            if (this.player.findStatusAffect(StatusAffects.KnowsCharge) < 0) {
                ...
                this.outputText(" away, you start pondering Arian's lesson of the day.  You wonder if you could perhaps apply the channeling principle to strengthen your [weapon] in combat.");
        ...
        this.flags[kFLAGS.ARIAN_LESSONS]++;
        this.dynStats("int", 1);
        if (this.player.inte < 75) this.dynStats("int", 1);
    ...
    private arianSexingTalk(): void {
        this.clearOutput();
        this.arianHealth(1);
        this.outputText("You smirk knowingly at [Arian em] and ask how [Arian ey] feels about sex now that [Arian ey]'s had [Arian eir] first time?");
        ...
        //(else if (ArianAnalXP <= 100))
            ...
            if (!this.player.hasCock()) {
                ...
                //[Yes][No]
                this.menu();
                this.addButton(this, 0, "Yes", this.yesYouButtslutIllFuckYou);
                this.addButton(this, 1, "No", this.goddamnitNoYouButtSlut);
                return;
            }
        }
        this.sexTalkFinish();
    ...
    private yesYouButtslutIllFuckYou(): void {
        this.clearOutput();
        this.outputText("How could you refuse such a request?  You tell [Arian em] to strip and get ready.");
    ...
    private goddamnitNoYouButtSlut(): void {
        this.clearOutput();
        this.outputText("You apologize, but you really can't do that right now.  Arian looks a bit disappointed, but smiles at you all the same.  \"<i>Oh... okay.  Next time then?</i>\"");
        this.outputText("\n\nYou nod.");
    ...
    private sexTalkFinish(newl: boolean = false): void {
        ...
        this.outputText("Satisfied with your little chat, you pat the lizan's head and excuse yourself, heading back to camp.");
        this.doNext(this, this.camp.returnToCampUseOneHour);
    }
    ...
    private inviteArianToCamp(): void {
        this.clearOutput();
        this.outputText("You ask the lizan if [Arian ey] still really wants to leave [Arian eir] comfortable home in the city and come out to live with you in your little camp in the wastelands?");
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            if (this.flags[kFLAGS.ARIAN_VAGINA] == 0) this.outputText("man");
        ...
        this.outputText("\n\n\"<i>I'm happy for you, " + this.arianMF("master", "mistress") + ", I really am,</i>\" Boon says.");
        this.outputText("\n\n\"<i>Make sure you're taking care of yourself, " + this.arianMF("master", "mistress") + ".  Don't forget to bathe regularly and eat well... and contact us once in awhile... and visit on the holidays,</i>\" Laika says, sniffing.");

        this.outputText("\n\nArian smiles at [Arian eir] pupils.  \"<i>I'm not your " + this.arianMF("master", "mistress") + " anymore.  From now on you're full-fledged mages.  Remember all that you learned, and I'm sure you'll be okay.</i>\"");
        this.outputText("\n\nBoon is the first to break the hug, followed shortly by Laika.  The twins then turn to you.");

        this.outputText("\n\n\"<i>Please, [name], take good care of the " + this.arianMF("mas", "mis") + "... err... Arian.  [Arian Ey] might be a handful sometimes, but [Arian ey]'s a really good person,</i>\" Boon says with a bow.");

        this.outputText("\n\n\"<i>If " + this.arianMF("master", "mistress") + "... I mean... if Arian gives you any trouble call us and we'll come running to give [Arian em] a good scolding,</i>\" Laika says with a bow.");
        ...
        this.menu();
        this.addButton(this, 0, "Next", this.takeYerLizardHomePartII);
    }

    private takeYerLizardHomePartII(): void {
        this.clearOutput();
        this.outputText("Upon arriving at the camp, the first thing Arian notices is the shimmering portal.  \"<i>Is this... where you came from?</i>\" Arian asks.");
        ...
        if (this.camp.followersCount() > 1) this.outputText(" and your other companions");
        this.outputText(".");
    ...
    private arianSexMenu(output: boolean = true): void {
        if (output) {
            this.clearOutput();
            this.outputText("You ask Arian if [Arian ey] feels strong enough to do a little lovemaking.");
            //(if ArianHealth < 30)
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.cockThatFits(50) >= 0) this.addButton(this, 0, "Anal - Pitch", this.giveArianAnal);
            //Get Blown:
            //PC must have a cock.
            this.addButton(this, 2, "Get Blown", this.getBlownByArian);
            //Penetrate:
            //Arian must be herm/female.
            //PC must have a cock that fits (cock area 50 or less)
            if (this.player.cockThatFits(this.flags[kFLAGS.ARIAN_CAPACITY]) >= 0 && this.flags[kFLAGS.ARIAN_VAGINA] > 0)
                this.addButton(this, 4, "Fuck Vagina", this.penetrateArian);
            //Double Pen Arian:
            ...
            if (this.player.cockThatFits(50) >= 0 && this.player.cockThatFits2(50) >= 0 && this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.addButton(this, 8, "Double Pen", this.doublePenetrateArian);
            //Docking
            //ArianCockSize needs to be below 3. (ArianDblCock does not affect this decision.) 
            //PC cock area must be <= 30.
            if (this.player.smallestCockArea() <= 30 && this.flags[kFLAGS.ARIAN_COCK_SIZE] < 3 && this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.addButton(this, 7, "Docking", this.arianDocking);
        }
        //Get Anal:
        //Arian must have a cock.
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.addButton(this, 1, "Anal - Catch", this.getButtWreckedByArian);
        //Blow:
        //Arian must have a cock.
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.addButton(this, 3, "Blow " + this.arianMF("Him", "Her"), this.suckAriansDick);

        //PC must have a vagina.
        if (this.player.hasVagina()) {
            //Get Penetrated:
            //Arian must have a cock.
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.addButton(this, 5, "Get Fucked", this.getPenetratedByArianAndHisHitlerMustache);
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0 && (this.player.hasKeyItem("Dildo") >= 0 || this.player.hasKeyItem("Deluxe Dildo") >= 0)) {
                this.addButton(this, 6, "Dildo Fun", this.arianDildoFun);
            }
        }
        this.addButton(this, 9, "Back", this.arianHomeMenu);
    }
    ...
    private giveArianAnal(): void {
        ...
        if (this.flags[kFLAGS.ARIAN_ANAL_XP] >= 100) this.flags[kFLAGS.ARIAN_ANAL_XP] = 100;
        this.flags[kFLAGS.ARIAN_VIRGIN] += 1;
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
            this.outputText(this.images.showImage("arianfemale-home-giveArianAnal"));
        else
        ...
        this.outputText("You tell Arian that, if [Arian ey]'s willing, you'd like to take [Arian em] from behind.");
        //AnalXP < 33
        ...
        if (this.flags[kFLAGS.ARIAN_ANAL_XP] < 33) {
            ...
            if (this.flags[kFLAGS.ARIAN_VAGINA] < 1) {
                ...
                if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 0) this.outputText("girl");
                else this.outputText("herm");
                this.outputText(" isn't going to make it any smaller either.</i>\"  Arian fidgets and shudders as you stroke [Arian eir] behind.");
            }
            ...
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) {
                this.outputText("the puffy lips of [Arian eir] drooling vagina");
                if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.outputText(" and ");
            }
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                this.outputText(this.arianMF("his", "her") + " erecting ");
                if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("pair of reptilian pricks");
                else this.outputText("reptilian prick");
            }
            this.outputText(".  Oh, someone's excited about this.  Why, you almost half expect [Arian eir] ass to start drooling like a pussy, [Arian ey]'s obviously that turned on by the idea of you fucking [Arian eir] ass.  You emphasize your point by gently worming one probing finger into the virgin-like tightness of [Arian eir] anus.  Wow, [Arian ey]'s so tight....");
            ...
            if (this.player.balls == 0) this.outputText("the base of your cock");
            else this.outputText("the bottom of your balls");
            this.outputText(" and you warn Arian that you're going to cum inside [Arian em] if you keep going.  Arian drools in pained pleasure, for a moment you wonder if [Arian ey] even heard you.  \"<i>Cum.  Cum inside me.  I want it... all of it! Ah!</i>\"");
            ...
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.outputText("spewing forth large gobs of cum of [Arian eir] own");
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0 && this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(" and ");
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText("wetting both your and her legs with a healthy serving of lizan pussy juice");
            this.outputText(".  You shudder and gasp until, utterly spent, you can't resist sinking down atop Arian and bearing [Arian em] into the comforting embrace of the bed.  You lay there, recovering your strength from your most delightful exertions, wriggling to give the lizan a proper snuggling while you lay there.");
        ...
        //Loose, sensitive enough to make Arian scream in pleasure. (It's like a tight vagina, feels like one too, minor pain)
            ...
            if (this.flags[kFLAGS.ARIAN_VAGINA] < 1) {
                ...
                if (this.flags[kFLAGS.ARIAN_BREASTS] == 1) this.outputText(" perky");
                else if (this.flags[kFLAGS.ARIAN_BREASTS] == 2) this.outputText(" rounded");
                else this.outputText(" pillowy");
                this.outputText(" mounds.");
            ...
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                this.outputText(" a");
                if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText(" pair of dripping cocks");
                else this.outputText(" dripping cock");
                if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(" and");
            }
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(" a sopping wet pussy");
            this.outputText(" greet");
            if ((this.flags[kFLAGS.ARIAN_VAGINA] == 0 && this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0) || (this.flags[kFLAGS.ARIAN_VAGINA] == 1 && this.flags[kFLAGS.ARIAN_COCK_SIZE] == 0)) this.outputText("s");
            this.outputText(" your touch.  What a little slut Arian is turning into; [Arian ey]'s already raring to go, and even knowing it's going to be [Arian eir] ass that's getting fucked, too!");
            ...
            if (this.flags[kFLAGS.ARIAN_VAGINA] < 1) this.outputText("boypussy");
            else this.outputText("nether-pussy");
            this.outputText(".  \"<i>Yesssss.... Oh, sometimes it hurts a bit, but the feeling, the pleasure, the pain.... It's heavenly.</i>\"");
            ...
            this.outputText("\n\nArian's ass tightens around your " + this.cockDescript(x) + " as [Arian eir] ");
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0) this.outputText("cock spews its load");
                else this.outputText("twin cocks spew their loads");
                this.outputText(" on the bedsheets");
                if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(" and [Arian eir] ");
            }
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(" contracting pussy paints your lower body with lizan femcum");
            this.outputText(".  Arian is only capable of moaning and shuddering as [Arian eir] powerful orgasm rocks the poor lizan to [Arian eir] core.  The extra tightness of [Arian eir] contracting butthole increases the friction on your " + this.cockDescript(x) + ", pushing you ever closer to the climax.");
            ...
            if (this.player.balls == 0) this.outputText("base of your spine");
            else this.outputText("depths of your balls");
            this.outputText(".  Arian, completely blissed out, lays limply on [Arian eir] bed, [Arian eir] butt held up by your gripping hands.  With a final deep thrust you finally go over the edge.");
            ...
            //(Medium Cum Amount)
            else if (this.player.cumQ() < 1000) this.outputText("\n\nArian's butt feels so good that you can't help but paint it in white.  Gob after gob of searing hot cum flows from your " + this.player.cockHead(x) + " and into the lizan's willing bowels.  You can't help but continue thrusting inside [Arian eir] deliciously slick insides, even as Arian's distended sphincter tries its best to hold you in place.  By the time you're done, Arian looks like [Arian ey]'s three months pregnant; after a few final thrusts, you're confident you've given [Arian em] all that you can muster for the moment and pull out of the lizan's hungry ass.");
            //(High Cum Amount)
            else {
                this.outputText("\n\nYou bury yourself as deep as you can into the lizan's behind and brace yourself, holding onto [Arian eir] hips as the first of many jets of cum finally escapes your throbbing " + this.cockDescript(x) + ".  You can feel the groaning lizan shudder with each blast that you pump into [Arian eir] inviting interior; each of your sticky ropes of cum filling [Arian em] up until [Arian eir] belly looks as big as a beach ball.  Arian's contracted sphincter tries its best to hold your prodigious load in, but it can't hope to contain it all; soon white jism explodes from around the seal of your cock.  Trails of your pleasure run down the lizan's legs to soak the bedsheets along with the lizan's own fluids.  One final jet pushes Arian off your shaft, and you gaze at the messy results of your recent activities.");
            }
            ...
            if (!this.arianFollower()) this.outputText("room");
        ...
        //Very Loose, sensitive enough to make Arian cum just from insertion. (Feels better than anything else. Yep, Arian really becomes a buttslut at this point)
            ...
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText("wet gash is positively leaking lizan juices");
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0 && this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.outputText(" and [Arian eir] ");
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.outputText("erect cock is already leaking pre like a sieve");
            this.outputText(".  You can hardly believe how far Arian has come; you ask [Arian em] if it really feels that good?  \"<i>More than you can... ah... imagine, now fill me up!</i>\" [Arian ey] demands with a sense of urgency.");
            ...
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText("her pussy clenches, spilling a flood of juices; ");
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0 && this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.outputText(" and ");
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.outputText("his cock throbs and shoots rope after rope of cum onto the bedsheets");
            this.outputText(". Overwhelmed by your sudden intrusion, Arian collapses forward, burying [Arian eir] face on [Arian eir] pillow and dragging you on top of [Arian em].  \"<i>Ah... [name], you feel so good.  It's amazing.  I never thought buttsex could ever feel this good.</i>\"");

            this.outputText("\n\nYou almost blink in surprise; you were just throwing the term buttslut around as a joke, you didn't think Arian was really like that.  Still, there are far more important matters - like digging your way into the depths of Arian's greedy little ass with your " + this.cockDescript(x) + "!  You squeeze the lizan's scaly butt and begin to rut [Arian em] like an animal, thrusting your way in and out of [Arian eir] back passage with all the eagerness you can muster.");
            ...
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("cocks are still rock hard, and surprisingly, still leaking ropes of cum");
                else this.outputText("cock is still rock hard, and surprisingly, still leaking ropes of cum");
                if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(", and her");
            }
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(" pussy squirting juices against your [legs] with each shuddering impact of your [hips]");
            this.outputText(".  \"<i>Ah!  Yes!  More, give me more!  Harder!  Faster!</i>\" Arian pleads, before biting on [Arian eir] pillow in pleasure.");
            ...
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) {
                this.outputText("  Her cunt grips the air and blasts one last jet of wetness at your crotch.");
            }
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.outputText("  [Arian Eir] throbbing cock flexes and shoots one last rope of lizan-cum against your belly.");
            this.outputText("  [Arian Eir] ass grips you forcefully, forming a vacuum and finally loosens.  \"<i>That... that wash da best,</i>\"  Arian slurs, before finally collapsing on [Arian eir] bed.");
            ...
            if (this.flags[kFLAGS.ARIAN_ASS_CHAT] == 0) {
                ...
                this.outputText("... besides, you think [Arian ey]'s kind of sexy when [Arian ey] gets like that.  At that Arian perks up.  \"<i>Really?</i>\"  Yes, really, you reply.  Arian smiles happily at you.  \"<i>So... do you want to go again?</i>\"");
                ...
                if (!this.arianFollower()) this.outputText("back to Tel'Adre's streets, and from there back to camp.");
                else this.outputText("out of [Arian eir] tent.");
                this.flags[kFLAGS.ARIAN_ASS_CHAT]++;
            }
            else {
    ...
    private getBlownByArian(): void {
        ...
        if (this.player.cumQ() < 250) this.outputText("small");
        else if (this.player.cumQ() < 1500) this.outputText("big");
        else this.outputText("huge");
        this.outputText(" pot belly.  [Arian Ey] lets go of your dick with a sigh of pleasure and lays back on the bed.  \"<i>Ah... I'm full.</i>\"");

        this.outputText("\n\nYou tell [Arian em] that [Arian ey] looks full");
        if (this.player.cumQ() >= 1500) this.outputText(", in fact, you're amazed [Arian ey] didn't pop with how much [Arian ey] drank");
        this.outputText("; is [Arian ey] sure [Arian ey]'s all right?");
        ...
        if (this.player.cumQ() >= 1000) this.outputText(" which, considering the fact [Arian ey] looks like a mother ready to birth a toddler from the size of [Arian eir] belly, isn't as easy as you'd think");
        this.outputText(".  Letting the lizan use your shoulder, you pull [Arian eir] sheets up and help [Arian em] back in the bed, where [Arian ey] flops down with a protest of springs");
        if (this.player.cumQ() >= 250) this.outputText(", [Arian eir] gut audibly sloshing as the cum inside is churned by the impact");
        this.outputText(".");
        ...
        if (!this.arianFollower()) this.outputText(" back to camp, stopping only to notify Laika and Boon that Arian is sleeping.");
        else this.outputText(" out of Arian's tent.");
        this.player.orgasm();
        this.flags[kFLAGS.ARIAN_HAS_BLOWN]++;
        this.doNext(this, this.camp.returnToCampUseOneHour);
    }
    ...
    private penetrateArian(): void {
        ...
        this.outputText(this.images.showImage("arianfemale-home-penetrate"));
        this.outputText("You admire the transgendered lizan's body, from her feminized features, down past her [arian chest], all the way to her shapely thighs.  You tell Arian that the change looks very good on her; few boys would really be so naturally pretty when turned into a ");
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.outputText("dick");
        ...
        this.outputText(".  \"<i>Err... readier?</i>\"  She asks, smiling nervously.");
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 3) {
            this.outputText(", her exposed cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
            this.outputText(" hardening at what she's about to do");
        }
        else if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            this.outputText(", her bulbous cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s peeking out of their");
            else this.outputText(" peeking out of its");
            this.outputText(" hiding place, already at full mast in anticipation");
        }
        this.outputText(".  Arian swallows audibly.  \"<i>What now?</i>\" she asks, already panting in lust.");
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 0) this.outputText("female");
        else this.outputText("herm");
        this.outputText(" lizan, until you have managed to hilt yourself inside of her.");
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            this.outputText("  Her ");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("twin shafts");
            else this.outputText("shaft");
            this.outputText(" bobbing just out of your field of vision, leaking pre all over your belly.");
        }
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            this.outputText("  Her cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s twitch as they spurt ");
            else this.outputText(" twitches as it spurts ");
            this.outputText("a small rope of pre over her belly, anticipating what's to come as she braces herself for the inevitable pounding you're about to give her.");
        }
        ...
        this.outputText("\n\n");
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            this.outputText("Arian's ");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("twin cocks throb");
            else this.outputText("cock throbs");
            this.outputText(" as she splashes her [arian chest] and face with her own futa-lizan seed.  ");
        }
        this.outputText("Her walls grip you tightly, almost painfully, as a flood of juices hit the " + this.player.cockHead(x) + " of your " + this.cockDescript(x) + " with the force of a tidal wave, only to spill around her spread nethers and run down your lower body.  Her sopping wet pussy works overtime, trying its best to pull you in as deep as possible, intent on sucking all the cum out of ");
        if (this.player.balls == 0) this.outputText("you");
        else this.outputText("your [balls]");
        this.outputText(".");

        this.outputText("\n\nFinally having kept your promise, you give yourself over to the inevitable. ");
        if (this.player.hasVagina()) this.outputText(" A wash of fluids spills down your body from your cunt as it spasms in release and y");
        ...
        if (this.player.cumQ() < 250) this.outputText("lean");
        else if (this.player.cumQ() < 1000) this.outputText("full");
        else this.outputText("overfilled");
        this.outputText(" belly.  \"<i>Stay with me, like this, a little longer?</i>\" Arian asks.");
    ...
    private getButtWreckedByArian(): void {
        ...
        this.arianHealth(3);
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
            this.outputText(this.images.showImage("arianfemale-home-getArianAnal"));
        else
            this.outputText(this.images.showImage("arianmale-home-getArianAnal"));
        this.outputText("With a lick of your lips with your tongue, you ask how Arian would like to be on the pitching end of anal?");
        //(if AnalXP < 33)
        if (this.flags[kFLAGS.ARIAN_ANAL_XP] < 33) this.outputText("\n\n\"<i>I don't know... won't it hurt, are you sure?</i>\"");
        //(if AnalXP < 66)
        else if (this.flags[kFLAGS.ARIAN_ANAL_XP] < 66) this.outputText("\n\n\"<i>I like it when you use my ass, but if you want me to use yours, I would gladly comply.  That is, if you're sure you want me to...?</i>\"");
        //(if AnalXP <= 100)
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] >= 3) {
            this.outputText("exposed cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s harden");
            else this.outputText(" hardens");
            this.outputText(" to full mast.");
        }
        else {
            this.outputText(this.arianMF("his", "her") + " cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s peek out of their");
            else this.outputText(" peeks out of its");
            this.outputText(" hiding place, rising to full mast.");
        }

        this.outputText("\n\nYou sashay yourself towards the prone lizan, straddling [Arian eir] legs and reaching out to grasp [Arian eir] ");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("primary ");
        this.outputText("cock.  Slowly and gently you begin to stroke its strange, knobbly surface, your fingers eliciting moans and gasps from Arian as [Arian ey] shudders under your touch.  Precum begins to ooze from the reptilian prick's head, and you purposefully rub it into [Arian eir] shaft as a kind of makeshift lubricant.");

        this.outputText("\n\n\"<i>Ohm... T-this feels great, [name], b-but if you keep this up, I won't be able to hold back!</i>\" Arian pants, [Arian eir] shaft");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" throbbing against your hands.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) {
            this.outputText("  [Arian Eir] other shaft twiches and sprays your ");
            if (this.player.tailType == TAIL_TYPE_NONE) this.outputText("back");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("stroking [Arian eir] other cock and sliding it between your ass cheeks, ");
        this.outputText("caressing the base of [Arian eir] tail.  With a devious grin, you begin to gently but insistently slide your finger into [Arian eir] ");
        if (this.flags[kFLAGS.ARIAN_ANAL_XP] > 66) this.outputText("eager ");
        this.outputText("ass, probing for [Arian eir] prostrate...");

        this.outputText("\n\nThe reaction is almost instantaneous; Arian moans into your kiss and you feel [Arian eir] ");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("pair of lizan-cocks practically jump");
        else this.outputText("lizan-cock practically jump");
        this.outputText(" back to full mast, even surprising you with Arian's eagerness.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("sliding [Arian eir] neglected prick through your ass-cheeks, ");
        this.outputText("clenching your inner walls to grip and feel the excitingly unusual lumpy, bulbous texture of [Arian eir] shaft inside you.  You moan and gasp, telling [Arian em] that [Arian ey] feels so unique, and yet [Arian ey] stimulates you so wonderfully.");
        ...
        if (this.player.hasVagina()) this.outputText("cunt dripping with arousal");
        if (this.player.hasCock() && this.player.hasVagina()) this.outputText(" and ");
        if (this.player.hasCock()) {
            this.outputText("cock");
            if (this.player.cockTotal() > 1) this.outputText("s");
            this.outputText(" drooling precum onto [Arian eir] belly");
        }
        this.outputText(".  You're so close now, you can feel it.  You tell Arian you're going to cum, and beg [Arian em] to cum with you.");

        this.outputText("\n\nAs if on cue, Arian gasps and [Arian eir] shaft");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s twitch");
        else this.outputText(" twitches");
        this.outputText(", signaling [Arian eir] oncoming orgasm.  [Arian Ey] grabs the sheets on [Arian eir] bed and curls [Arian eir] toes.  \"<i>...[name]!</i>\"  Jet after jet of lizan spunk paints your bowels white");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText(", as well as your butt");
        this.outputText(", as Arian climaxes");
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(", her own pussy clenching and drizzling juices on the matted sheets beneath the two of you.");
        else this.outputText(".");
        ...
        if (this.player.hasCock()) this.outputText("  [EachCock] spasms and shoots rope after rope of cum, painting Arian's [arian chest] white, as well as the groaning lizan's face.");
        if (this.player.hasVagina()) this.outputText("  Your [pussy], though unattended, joins in the perverted display, leaking copious amount of fluids on top of the prone lizan.");
        ...
        //(if AnalXP <= 100)
            ...
            if (this.player.hasCock()) {
                ...
                this.menu();
                if (this.player.cockThatFits(50) >= 0 && this.player.hasCock()) this.addButton(this, 0, "Yes", this.giveArianAnal);
                else this.outputText(".  You're too big to fit inside Arian's ass, though.");
                this.addButton(this, 1, "No", this.camp.returnToCampUseOneHour);
                return;
            }
            else {
    ...
    private suckAriansDick(): void {
        ...
        this.arianHealth(3);
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
            this.outputText(this.images.showImage("arianfemale-home-suckariandick"));
        else
            this.outputText(this.images.showImage("arianmale-home-suckariandick"));
        this.outputText("You make a show of lewdly licking your lips and ask Arian if [Arian ey]'d be willing to let you have a little taste of lizan essence...?");
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 3) {
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("exposed cocks hardening to full mast");
            else this.outputText("exposed cock hardening to full mast");
        }
        else {
            this.outputText("cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s emerging from their");
            else this.outputText(" emerging from its");
            this.outputText(" hiding place, already fully erect");
            this.outputText(".");
        }
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("uppermost ");
        this.outputText("cock, marveling at the strange textures of its bumpy, knobbly, bulbous surface.  It feels so strange to you, and you continue to stroke it eagerly.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("twin ");
        this.outputText("shaft");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s tremble");
        else this.outputText(" trembles");
        this.outputText(" and throb");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0) this.outputText("s");
        this.outputText(" against your hand; pre quickly forming on the tip");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" only to slowly slide down the bulbous surface of Arian's lizan prick");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(".  You continue to slide your hand along the increasingly slick surface, playfully asking if your hand really feels that good to [Arian em]?");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" a long, loving, slurpy lick.  The taste is surprising; a sort of sugary-tinted spice, not the usual salt of another creature.  It's actually quite pleasant, and you find yourself running your tongue up and down [Arian eir] prick as if it were some kind of candied stick, eagerly sucking and slobbering as you coax more of [Arian eir] yummy goo from its strangely-shaped tip.");
        ...
        this.outputText(" one more time and erupts into your waiting mouth");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText(" and chin");
        this.outputText(".  [Arian Eir] legs close around your back, effectively keeping you from moving too far away as [Arian ey] finishes giving you all of [Arian eir] load.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText(", oblivious to the second cock spewing cum all over your [chest] in your eagerness");
        this.outputText(".  After [Arian ey] finally finishes, you continue to lick and suckle for a few minutes longer, making sure you've lapped up every last drop.  Then and then alone do you lift your head, flirtatiously licking your lips to tease the now-spent lizan.");
        ...
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(" and tickle her cunt with a finger");
        this.outputText(" for emphasis, kiss [Arian em] gently on the tip of [Arian eir] nose, and then get your clothes together, planning on getting dressed and heading back out again.");
    ...
    private getPenetratedByArianAndHisHitlerMustache(): void {
        ...
        this.arianHealth(3);
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
            this.outputText(this.images.showImage("arianfemale-home-getpenetrated"));
        else
            this.outputText(this.images.showImage("arianmale-home-getpenetrated"));
        this.outputText("You look at Arian's ");
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] < 3) this.outputText("slit, and think of what it's hiding in there");
        else {
            this.outputText("exposed cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        }
        this.outputText("; then you turn to look at Arian and ask [Arian em] how would [Arian ey] feel about giving [Arian eir] ");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("twin cocks");
        else this.outputText("cock");
        this.outputText(" a bit of a workout?");
        ...
        if (this.player.isBiped()) this.outputText("spread your [legs] and ");
        this.outputText("expose your [vagina] to [Arian eir] viewing pleasure.  The reaction is almost instantaneous; Arian's ");
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] >= 3) this.outputText("exposed");
        else this.outputText("hidden");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText(" twin shafts immediately fill");
        else this.outputText(" shaft immediately fills");
        this.outputText(" with blood, growing fully erect in ");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("their");
        else this.outputText("its");
        this.outputText(" bulbous, throbbing glory.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" and get ready to thrust in; otherwise neither of you are going to be feeling good any time soon.");

        this.outputText("\n\nSnapping to [Arian eir] senses, Arian quickly ");
        this.outputText("kneels between your [legs]");
        this.outputText(", aligning the tip of ");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("each of ");
        this.outputText(this.arianMF("his", "her") + " ");
        this.outputText("shaft");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" with your [vagina]");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText(" and [asshole]");
        this.outputText(".  [Arian Ey] looks at you, waiting for you to confirm that [Arian ey] should indeed get going.  You smile and nod");
        if (this.player.hasLongTail()) this.outputText(", looping your tail around [Arian eir] waist");
        this.outputText(".");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) {
            ...
            if (this.player.hasLongTail()) this.outputText(", further encouraging [Arian em] to do so, by looping your tail around [Arian eir] waist");
            this.outputText(".  The lizan doesn't need much encouragement to comply, easily snuggling against you, breast to breast, and sighing softly.  \"<i>I'm sorry.  Sometimes I wonder why you bother doing something like this, with someone like me,</i>\" [Arian ey] says, a hint of sadness in [Arian eir] voice.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" sit inside you.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("twin bulbous shafts feel");
        else this.outputText("bulbous shaft feels");
        this.outputText(" exquisite, and ");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("their");
        else this.outputText("its");
        this.outputText(" bumps massage your entrance");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" unlike anything else.  You praise the lizan on being a quick study, letting go of [Arian eir] tail and grabbing onto [Arian eir] scaly shoulders instead, giving [Arian em] a few more directions so [Arian ey] can catch your most sensitive spots.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("and [ass] ");
        this.outputText("with [Arian eir] sticky wet seed.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" swelling inside you, the lizan grabbing your shoulders and starting to thrust [Arian em]self back into you once more.");
        ...
        if (this.flags[kFLAGS.ARIAN_VAGINA] == 0) this.outputText("[Arian ey] himself ");
        else this.outputText("she ");
        this.outputText("is being overwhelmed by, spraying your ");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0) this.outputText("cavity");
        else this.outputText("cavities");
        this.outputText(" with a second helping of lizan spunk, dredging up every last drop of jizz left in [Arian eir] internal balls before, with a groan, [Arian ey] sinks down atop you.  Arian's orgasm pushes you over the edge, and you find your pussy ");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("and ass ");
        this.outputText("contracting, trying ");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0) this.outputText("its");
        else this.outputText("their");
        this.outputText(" best to milk the poor lizan of all [Arian ey] is worth, until finally with one last spasm, you slump down and release the lizan shaft");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" deeply embedded inside you, some of your mixed juices running down to mat Arian's bed.");
    ...
    private doublePenetrateArian(): void {
        ...
        this.outputText("You look over your feminine lizard lover, and feel your " + this.multiCockDescriptLight() + " starting to stir in your [armor].  Since you have enough tools for the job, and Arian has enough holes, you ask if Arian would be willing to let you fuck her in both her ass and pussy at the same time?");
        ...
        if (this.flags[kFLAGS.ARIAN_ANAL_XP] <= 33) this.outputText("tight");
        else if (this.flags[kFLAGS.ARIAN_ANAL_XP] <= 66) this.outputText("loose");
        else this.outputText("throbbing");
        this.outputText(" ass and dripping wet vagina; a pair of clawed hands reach behind to spread her butt open for you.  \"<i>Okay, I'm ready.</i>\"  Arian says, looking behind at you.");
        ...
        this.outputText("\n\n\"<i>What do you think I'm trying to do!?  Now get in here and paint my womb white!</i>\" Arian snaps, bracing herself on her bed and allowing her ass and pussy both to suck you in with surprising force.  As soon as you're hilted within both holes, her ass clamps shut on your " + this.cockDescript(y) + ", while her pussy's contractions begin truly milking you for all you're worth.  Stuck as you are, you have no option but to sit back and enjoy her contractions as you feel yourself nearing the edge of an inevitable orgasm.");
        ...
        //(Medium Cum Amount)
            ...
            if (this.player.balls == 0) this.outputText("themselves");
            else this.outputText("your [balls]");
            this.outputText(" of their load; a load Arian is not only pleased to accept, but also eager to relieve you of every single stray drop off.  The tightness of her ass, pressing down on your " + this.cockDescript(y) + ", her pussy milking on your " + this.cockDescript(x) + ".  How could anyone refuse such an invitation?  You let yourself go, stuffing the eager lizan with more cum than you thought yourself capable of producing.");

            this.outputText("\n\n\"<i>M-more,</i>\" Arian pleads, even as her belly starts to distend.");
        }
        //(High Cum Amount)
        else {
            this.outputText("\n\nYou cum with such force, that if Arian's ass wasn't clamping down on your " + this.cockDescript(y) + " so tightly, you'd be sure she was going to get pushed off.  \"<i>So much cum!  Yesssss!</i>\" Arian moans, as you quickly give her usually lithe belly a very blatant bump.  Even though your prodigious amount of cum is enough to completely fill the eager lizan-girl, her ass and pussy work overtime to ensure you're completely spent; and you have no desire to resist.");

            this.outputText("\n\nBy the time you're finished, Arian's belly is positively bulging.  \"<i>Ahhh... eggs... cum... yes...</i>\" she states in a stupor.");
        }
        this.outputText("\n\nArian's ass goes slack around your " + this.cockDescript(y) + ", and Arian slowly slides off your shaft to plop on her bed; eyes closed in bliss, as she takes a short nap.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_PENETRATION_CHAT] == 0) {
            ...
            if (!this.arianFollower()) this.outputText("bedchambers");
            else this.outputText("tent");
            this.outputText(".");
        }
        else {
    ...
    private arianDocking(): void {
        ...
        this.arianHealth(3);
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
            this.outputText(this.images.showImage("arianfemale-home-docking"));
        else
            this.outputText(this.images.showImage("arianmale-home-docking"));
        this.outputText("You set your eyes on Arian's genital slit, and then smile at [Arian em].  You ask how [Arian ey]'d feel about ‘hiding' your cock, rather than [Arian eirs], inside [Arian eir] slit?");
        ...
        //(if ArianDblCock == 0)
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0) {
            this.outputText("didn't [Arian ey] always want to have two dicks?  Well, now it's [Arian eir] chance to find out what it'd be like.");
        }
        else {
            this.outputText("[Arian ey]'s already hiding two in there... an extra cock won't make such a huge difference, would it?");
        }
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("pair of ");
        this.outputText("pecker");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" from your prying eyes.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" of Arian's dick");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(".  You brush your fingers against the tip");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(", seeing what kind of reaction that will elicit from your scaly lover.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0) this.outputText(" brushing up against [Arian eir] own lizan pecker");
        else this.outputText("sandwiched between [Arian eir] two lizard dicks");
        this.outputText(", the friction is absolutely incredible, sending sparks of pleasure cascading along your shaft.  You moan in delicious lust and tell Arian that this is absolutely incredible.");
        ...
        this.outputText("slides across your intruding shaft as you go, and [Arian eir] sheathe-walls grip and squeeze as best they can, leaving you moaning and panting like a bitch in heat.  Arian's moans of pleasure join yours, [Arian eir] twitching bulbous shaft");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" massaging your [cock smallest] almost like a vagina would.  Shaky hands grab a hold of your shoulders, helping you down and up on Arian's male slit.  \"<i>Oh, Marae!  It feels like I'm getting an internal handjob... it feEels so... so good!  H - harder... almost cumming!</i>\" [Arian ey] pleads, as [Arian ey] breathes hotly on your face; panting like a bitch in heat.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(" out before cumming.");
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("pair of cummy masts emerge");
        else this.outputText("cummy mast emerges");
        this.outputText(" to feebly shoot a couple more ropes of jism, before returning limply to ");
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("their");
        else this.outputText("its");
        this.outputText(" hiding place.  \"<i>Ahhh,</i>\" is all Arian says, before slumping down.");
    ...
    private giveArianAnItem(): void {
        this.clearOutput();
        this.outputText("Thinking about the many items in your possession, you ask if Arian would be willing to take something for you?");
        ...
        else if (this.flags[kFLAGS.ARIAN_HEALTH] < 20) { //May only give Vitality T.
        ...
        this.menu();
        if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 0 && this.arianHealth() >= 10) this.arianStoryDialogue1();
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 1 && this.arianHealth() >= 20) this.arianStoryDialogue2();
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 2 && this.arianHealth() >= 30) this.arianDialogue3();
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 3 && this.arianHealth() >= 50) this.arianImbue();
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 4 && this.arianHealth() >= 75) this.arianPlot4();
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 5 && this.arianHealth() >= 100) this.arianPlot5();
        else {
            if (this.player.hasItem(this.consumables.VITAL_T)) this.addButton(this, 0, "Vital Tinct", this.arianVitalityTincture);
            if (this.flags[kFLAGS.ARIAN_HEALTH] >= 20) {
                if (this.player.hasItem(this.consumables.P_DRAFT)) this.addButton(this, 1, "P. Incubi D", this.giveIncubusDraftToArian);
                if (this.player.hasItem(this.consumables.P_S_MLK)) this.addButton(this, 2, "P. Suc.Milk", this.succubiMilkForArian);
                if (this.player.hasItem(this.consumables.LACTAID)) this.addButton(this, 3, "Lactaid", this.giveArianLactaid);
                if (this.player.hasItem(this.consumables.REDUCTO)) this.addButton(this, 4, "Reducto", this.giveArianReducto);
                if (this.player.hasItem(this.consumables.REPTLUM)) this.addButton(this, 5, "Reptilum", this.giveArianReptilum);
            }
            this.addButton(this, 9, "Back", this.arianHomeMenu);
        }
    }
    ...
    private arianVitalityTincture(): void {
        this.clearOutput();
        this.outputText("Fishing around amongst your pockets, you withdraw a vial of that strange potion Giacomo peddles and offer it to the sickly lizan, explaining it will bolster [Arian eir] constitution and fill [Arian em] with permanent vitality.");
        ...
        this.player.consumeItem(this.consumables.VITAL_T);
        this.arianHealth(10);
        this.menu();
        this.addButton(this, 0, "Next", this.giveArianAnItem);
    }
    ...
    private giveIncubusDraftToArian(): void {
        ...
        this.outputText("Fishing around in your pockets, your hand closes on the vial of purified incubus draft.  You offer this to Arian, asking ");
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.outputText("if [Arian ey] would like to make [Arian eir] cock bigger");
        else this.outputText("if she'd like to have a touch of her old malehood back");
        this.outputText(".");
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 0) {
            ...
            if (this.player.hasVagina()) this.outputText("[vagina] and ");
            ...
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0) this.outputText("shaft emerges from its hiding place.  It throbs and grows, settling in its");
            else this.outputText("pair of shafts emerge from their hiding place.  They throb and grow, settling in their");
            this.outputText(" new size as Arian moans and cums all over [Arian em]self.");
            ...
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s back into their");
            else this.outputText(" back into its");
            this.outputText(" slit.");
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 2) {
                this.flags[kFLAGS.ARIAN_COCK_SIZE]++;
                this.dynStats("lus", 10 + this.player.lib / 20);
                ...
                if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("them");
                else this.outputText("it");
                this.outputText(" until two thirds are inside, then it doesn't looks like [Arian ey] can take in anymore.");

                if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("\n\n\"<i>W-Wow, they're so big I can't even manage to hide them,</i>\" Arian pants as the tip of [Arian eir] cocks hang limply, exposed for all to see.");
                ...
                this.outputText("\n\nYou can't resist reaching down and gently tweaking the exposed tip.  Mmm... your little " + this.arianMF("boy-toy", "herm-toy") + " is going to be a very naughty thing from now on, you tease, unless [Arian ey] intends to start wearing a loincloth?");
                ...
                if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) {
                    ...
                    if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
                    this.outputText(" starts hardening, no longer able to hide Arian's excitement inside her protective slit.");
                }
                else {
        ...
        if (this.flags[kFLAGS.ARIAN_BREASTS] > 1 && (!this.flags[kFLAGS.HYPER_HAPPY])) {
        ...
        else if (this.flags[kFLAGS.ARIAN_BREASTS] == 1 && (!this.flags[kFLAGS.HYPER_HAPPY])) { //Now you are male again.
            ...
            this.flags[kFLAGS.ARIAN_BREASTS] = 0;
    ...
    private succubiMilkForArian(): void {
        ...
        this.outputText("Fishing out the bottle of purified demon's milk, you ask if Arian is willing to get ");
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText("further ");
        this.outputText("in touch with [Arian eir] feminine side.  For you?");
        ...
        if (this.flags[kFLAGS.ARIAN_VAGINA] == 0) {
        ...
        else if (this.flags[kFLAGS.ARIAN_BREASTS] < 3) {
            this.flags[kFLAGS.ARIAN_BREASTS]++;
            ...
            if (this.flags[kFLAGS.ARIAN_BREASTS] == 2) {
                ...
                if (this.flags[kFLAGS.ARIAN_COCK_SIZE] >= 3) this.outputText(" and her exposed shaft begins to grow hard");
                this.outputText(".");
            }
            else {
                ...
                if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 3) this.outputText(" and you see her exposed shaft slowly rising to point at you");
                this.outputText(".");
        ...
        //(else If ArianCockSize > 1) //Dick shrinkage.
            ...
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
            this.outputText(".  \"<i>Oh... that's good.  Right?</i>\" Arian looks at you nervously.  You think it over, and confess that while you are more attracted to women than men, you won't force Arian to become fully female if she doesn't want to.");
        ...
        else if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 1 && this.flags[kFLAGS.ARIAN_DOUBLE_COCK] >= 1 && (!this.flags[kFLAGS.HYPER_HAPPY])) {
    ...
    private giveArianLactaid(): void {
        ...
        this.outputText("Your hand closes around the vial of lactation-inducing potion that is Lactaid.  You almost reject it automatically, but then you stop and think.  There's odder things in this world, after all.  You remove the vial and ask Arian if [Arian ey] would be willing to let you see what lizan milk tastes like.");
        ...
        if (this.flags[kFLAGS.ARIAN_BREASTS] == 0) {
            ...
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 3) this.outputText(", her exposed cock growing hard as she flushes with arousal");
            this.outputText(".  Her hands grip the sheets and you watch her ");
            if (this.flags[kFLAGS.ARIAN_BREASTS] == 1) this.outputText("perky");
            else if (this.flags[kFLAGS.ARIAN_BREASTS] == 2) this.outputText("luscious");
            else this.outputText("pillowy");
            this.outputText(" breasts grow before your eyes.  \"<i>My breasts... d-do something, [name]!</i>\" Arian pleads.  Unsure of what you should be doing, you grab her breasts and begin kneading them as they grow in your hands; her erect nipples poking your palms.  \"<i>Ah... that feels good!  Don't stop!</i>\"");
    ...
    private giveArianReducto(): void {
        this.clearOutput();
        this.outputText("Eyeing Arian up and down, you fish your pouches for a tube of Reducto.  Once you've found it, you hand it over to Arian and tell [Arian em] you'd like [Arian em] to reduce something for you.");
        this.outputText("\n\n\"<i>Umm... sure, which part?</i>\"");
        //(Display Options)
        this.menu();
        if (this.flags[kFLAGS.ARIAN_BREASTS] > 0) this.addButton(this, 0, "Breasts", this.useReductoOnAriansBreasts);
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.addButton(this, 1, "Cock", this.useReductoOnArianCocks);
        this.addButton(this, 2, "Asshole", this.useReductoOnAriansAsshole);
    ...
    private useReductoOnAriansBreasts(): void {
        ...
        this.outputText("You point at Arian's ");
        if (this.flags[kFLAGS.ARIAN_BREASTS] == 1) this.outputText("small");
        else if (this.flags[kFLAGS.ARIAN_BREASTS] == 2) this.outputText("generous");
        else this.outputText("pillowy");
        this.outputText(" breasts.  \"<i>Okay... I suppose it would be good to lose a bit of weight up here and spare my back.</i>\"  Arian smiles, opening the tube of Reducto and her robes; then squeezing the contents of the paste her hands.");
        ...
        if (this.flags[kFLAGS.ARIAN_BREASTS] > 1) {
            ...
            this.outputText("The two of you watch Arian's breasts, but nothing seems to be changing.  You reach forward to feel them, and indeed they haven't shrunk down at all.");
    ...
    private useReductoOnArianCocks(): void {
        this.clearOutput();
        this.player.consumeItem(this.consumables.REDUCTO);
        this.outputText("You point at [Arian eir] crotch, mentioning that you'd like [Arian em] to be smaller.");
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 3) {
            ...
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("pair of exposed lizard cocks");
            else this.outputText("exposed lizard cock");
            this.outputText(" into full erection.  Finally [Arian ey] begins applying the paste.");

            this.outputText("\n\nThe changes are almost immediate; Arian groans and watches as [Arian eir] oversized dick");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
            this.outputText(" slowly shrink");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0) this.outputText("s");
            this.outputText(" to a more manageable size.  Once the transformation is complete Arian tries to fully retract [Arian eir] cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s back into their");
            else this.outputText(" back into its");
            this.outputText(" hiding place; it's a tight fit, but it fits.");
            ...
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s from their tight hiding place");
            else this.outputText(" from its tight hiding place");
            this.outputText(", and into full erection.  Finally [Arian ey] begins applying the Reducto.");

            this.outputText("\n\nIt takes mere moments for the changes to occur; Arian groans in slight discomfort as [Arian eir] cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s return to their");
            else this.outputText(" returns to its");
            this.outputText(" original size.  Once the transformation is complete, [Arian ey] retracts ");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("them");
            else this.outputText("it");
            this.outputText(" back into [Arian eir] genital slit.");
        ...
        else if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) {
            this.outputText("Then, [Arian ey] coaxes [Arian eir] twin reptilian-peckers out of their hiding place.  Finally [Arian ey] applies the paste to both shafts in turn.");
    ...
    private useReductoOnAriansAsshole(): void {
        this.clearOutput();
        this.player.consumeItem(this.consumables.REDUCTO);
        this.outputText("You ask Arian to hand the tube of reducto back over to you, telling [Arian em] that you want to make [Arian em] a little tighter when you do [Arian em] from behind.  The lizard-");
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            if (this.flags[kFLAGS.ARIAN_VAGINA] == 0) this.outputText("man");
            else this.outputText("herm");
        }
        else this.outputText("woman");
        this.outputText(" does as you ask, allowing you to smear your fingers generously with the shrinking cream, even as [Arian ey] anticipates your next request and removes [Arian eir] clothes before lying down, bum up in the air and tail slowly swishing to and fro, [Arian eir] anus exposed and waiting for your ministrations.");
        //(if AnalXP == 1)
        ...
        //(else if AnalXP < 33)
            ...
            this.flags[kFLAGS.ARIAN_ANAL_XP] = 1;
        }
        else if (this.flags[kFLAGS.ARIAN_ANAL_XP] < 66) {
            this.flags[kFLAGS.ARIAN_ANAL_XP] -= 33;
            ...
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                this.outputText(", [Arian eir] cock");
                if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s suddenly jolting from their sheathe");
                else this.outputText(" suddenly jolting from its sheathe");
                if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(" and ");
            }
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) {
                if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 0) this.outputText(", ");
                this.outputText("her pussy growing wet from the stimulation");
            }
            this.outputText(".  You playfully shake your head and slap [Arian em] on the butt, then use it for balance as you start to pump the blade of your fist inside and out, smearing the cream copiously around to restore some of [Arian eir] once-virginal anal tightness to him.  Eventually, you've used up all the cream and [Arian eir] ass definitely feels tighter, so you decide to pull out.");
    ...
    private giveArianReptilum(): void {
        ...
        this.outputText("Fingering the vial of reptilium, you smirk to yourself.  Quickly wiping it off your face, you instruct Arian to close [Arian eir] eyes and open [Arian eir] mouth, as you have a special surprise for [Arian em].");
        ...
        if (!this.player.hasVagina() && !this.player.hasCock()) this.outputText("genderless crotch");
        this.outputText(".  \"<i>Hmm, that looks much tastier than candy actually.</i>\"  [Arian Ey] licks your ");
        if (this.player.hasCock()) this.outputText("cock from base to tip");
        else if (this.player.hasVagina()) this.outputText("pussy all the way from the back to the tip of your [clit]");
        else this.outputText("your sweaty crotch");
        this.outputText(".  \"<i>Yummy!  I could use seconds, but roleplay time is over; let's... masturbate each other!</i>\"  [Arian Ey] begins stroking [Arian eir] ");

        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
            this.outputText(this.images.showImage("arianfemale-home-mutualmasturbation"));
        else
            this.outputText(this.images.showImage("arianmale-home-mutualmasturbation"));

        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 1) {
            this.outputText("quickly erecting lizan cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(" and her ");
        }
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText("slavering vagina");
        this.outputText(".");
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            ...
            if (this.flags[kFLAGS.ARIAN_BREASTS] == 0) this.outputText("chest");
            else if (this.flags[kFLAGS.ARIAN_BREASTS] == 1) this.outputText("perky little breasts");
            else if (this.flags[kFLAGS.ARIAN_BREASTS] == 2) this.outputText("soft breasts");
            else this.outputText("pillowy breasts");
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(" while ");
        }
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText("her vagina doing its best to soak her bed with lizan femcum");
        this.outputText(".");
        ...
        if (ArianScene.rand(2) == 0 && this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0 && this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
        ...
        if (tfed) {
            ...
            this.flags[kFLAGS.ARIAN_DOUBLE_COCK] = 1;
        }
        else {
    ...
    private treatCorruption(): void {
        this.clearOutput();
        this.outputText("You ask Arian if [Arian ey] thinks [Arian ey] can help you reduce some of the taint that has infected your soul.");

        if (this.flags[kFLAGS.ARIAN_TREATMENT] == 1) {
            ...
            this.dynStats("cor", -1);
            this.fatigue(20);
            if (this.player.cor == 0) this.outputText(" - in fact, you're quite sure that Arian has purified you entirely");
            this.outputText(".  You thank the lizan for [Arian eir] magical treatment.");
            ...
            this.flags[kFLAGS.ARIAN_TREATMENT]++;
            this.doNext(this, this.camp.returnToCampUseOneHour);
        }
    }
    ...
    private imbueTalisman(): void {
        this.clearOutput();
        this.outputText("You tell Arian that, if it's not too much trouble, you'd like [Arian em] to ");
        if (this.player.hasKeyItem("Arian's Talisman") >= 0) this.outputText("place a spell in the enchanted talisman [Arian ey] created for you");
        else this.outputText("change the spell in the talisman [Arian ey] created for you");
        ...
        */
        ...
        this.menu();
        if (this.player.hasItem(this.useables.B_CHITN, 2) && this.player.hasItem(this.useables.T_SSILK)) this.addButton(this, 0, "Shielding", this.arianSpellPlace, "Shielding Spell");
        if (this.player.hasItem(this.consumables.GOB_ALE, 2) && this.player.hasItem(this.consumables.S_GOSSR)) this.addButton(this, 1, "Immolation", this.arianSpellPlace, "Immolation Spell");
        this.addButton(this, 9, "Back", this.arianHomeMenu);
    }

    private arianSpellPlace(spell: string): void {
        this.clearOutput();
        this.outputText("You tell Arian that you want [Arian em] to place the " + spell + " spell in your talisman for you.");
    ...
    private clearCharges(): void {
        if (this.player.findStatusAffect(StatusAffects.ShieldingSpell) >= 0) this.player.removeStatusAffect(StatusAffects.ShieldingSpell);
        if (this.player.findStatusAffect(StatusAffects.ImmolationSpell) >= 0) this.player.removeStatusAffect(StatusAffects.ImmolationSpell);
    }
    public clearTalisman(): void {
        this.player.removeKeyItem("Arian's Charged Talisman");
        this.player.createKeyItem("Arian's Talisman", 0, 0, 0, 0);
    }
    ...
    */
    ...
    public sleepWithArian(newl: boolean = false): void {
        ...
        this.outputText("Tired after a whole day of adventuring, you decide to retire and catch some shut-eye.  While going through the day's events, you recall Arian had offered to let you stay in [Arian eir] tent and sleep with [Arian em] in [Arian eir] bed.  Your tired body could surely use a soft bed today, and maybe a certain lizan to keep you company too.  With that in mind, you head to [Arian eir] tent.");
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            this.outputText("You can clearly see [Arian eir] exposed cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
            this.outputText(" stiffening at the sight.  ");
        }
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText("You can tell that [Arian ey] is aroused by the little show, despite [Arian eir] body doing a good job of hiding it.  ");
        this.outputText("You just let [Arian em] squirm, pretending you don't see [Arian eir] reactions, and then, indicating the bed, you ask which of you should get in first.");
        ...
        if (this.flags[kFLAGS.ARIAN_ANAL_XP] < 33) {
            ...
            this.menu();
            this.addButton(this, 0, "Listen", this.listenToLowAnalXPArian);
            this.addButton(this, 1, "Sleep", this.dontListenToLowAnalXPArian);
        }
        //(else if AnalXP <66)
        else if (this.flags[kFLAGS.ARIAN_ANAL_XP] < 66) {
            ...
            //[Listen][Sleep]
            this.menu();
            this.addButton(this, 0, "Listen", this.listenToMediumAnalXPArian);
            this.addButton(this, 1, "Sleep", this.dontListenToMediumAnalXPArian);
        }
        else { //AnalXP <= 100
            ...
            //[Tease][Sleep]
            this.menu();
            this.addButton(this, 0, "Tease", this.TeaseHighAnalXPArian);
            this.addButton(this, 1, "Sleep", this.dontTeaseHighAnalXPArian);
        }
    }
    ...
    private dontListenToLowAnalXPArian(): void {
        this.clearOutput();
        this.outputText("You gently stroke the lizan's side and plant a soft, gentle kiss on [Arian eir] neck.  At your ministrations, Arian sighs and slowly falls still again.  You wait, but it seems that whatever dreams [Arian ey] was having have faded and [Arian ey]'s truly asleep now, allowing you to go back to sleep yourself.");
        this.menu();
        this.addButton(this, 0, "Next", this.camp.sleepWrapper);
    }
    ...
    private listenToLowAnalXPArian(): void {
        ...
        this.outputText("You turn you head to try and catch what Arian might be dreaming about.");
    ...
    private listenToMediumAnalXPArian(): void {
        ...
        if (this.player.hasCock()) {
            ...
            this.outputText(" still worming its way into [Arian eir] fuckable ass, but it really does look like [Arian ey]'s fast asleep.  Weird.  \"<i>So many muffins... all [name] flavored...</i>\"  [Arian ey] calms down and begins snoring lightly.");
            ...
            if (this.player.cor >= 66) this.outputText(", despite the silly lizan being deserving of it");
            this.outputText(".  But as payback, [Arian ey]'s going to sleep with you inside, not that it seems like it'll be much of a problem to [Arian em]... Too bad you can't hold an erection all the time while asleep, so you know eventually you'll slip out...");
        }
        else {
    ...
    private TeaseHighAnalXPArian(): void {
        ...
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
            ...
            this.outputText(this.images.showImage("arianmale-camp-dreamingArian"));
        this.flags[kFLAGS.ARIAN_MORNING] = 1;
        ...
        if (this.player.hasCock()) {
            ...
            this.outputText("\n\nIs that so, you ask?  Does [Arian ey] really want a big squishy belly, stuffed full of hot, salty " + this.player.mf("man", "herm") + "-goo?");
    ...
    public wakeUpAfterArianSleep(): void {
        ...
        if (this.player.hasCock()) {
            ...
            if (this.flags[kFLAGS.ARIAN_ANAL_XP] < 33) {
                ...
                this.outputText("Your eyes open, and you suddenly realize that whatever dreams you were having... are now gone.  You make note of the still sleeping lizan right next to you and recall last night's events.  You wonder if Arian will remember any of that...");
                ...
                this.outputText("\n\nYou promptly seize the lizan in a tight embrace, already guiding your [cock smallest] towards " + this.arianMF("his naughty boy-pussy", "her tail-hole") + " with a smile on your face.  You teasingly muse aloud about who was saying you were giving [Arian em] the choice to say no?");
                ...
                if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) {
                    this.outputText(this.arianMF("his", "her") + " pussy is squirting her juices all over your lower body");
                    if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                        this.outputText(" and ");
                    }
                    else this.outputText("; ");
                    ...
                    this.outputText(this.arianMF("his", "her") + " ");
                    if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("twin shafts are");
                    else this.outputText("shaft is");
                    this.outputText(" squirting white ropes of lizan cum all over the bedsheets;");
                }
                this.outputText(" what a naughty little lizan.");

                this.outputText("\n\nYou can't resist playfully teasing Arian that [Arian ey] is such a buttslut.  ");
                if (!this.player.hasVagina()) this.outputText("Did [Arian ey] always know [Arian ey] wanted a man to top [Arian em], or is this development new to [Arian em]?");
                else this.outputText("Does [Arian ey] really get off on having a girl with a dick all her own fuck [Arian em] like a girl, hmm?");
            ...
            if (this.flags[kFLAGS.ARIAN_ANAL_XP] < 33) {
                ...
                if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                    this.outputText(this.arianMF("his", "her") + " ");
                    if (this.flags[kFLAGS.ARIAN_COCK_SIZE] >= 3) this.outputText("exposed ");
                    this.outputText("bulbous shaft");
                    if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s begin");
                    else this.outputText(" begins");
                    this.outputText(" sporting an erection");
                    if (this.flags[kFLAGS.ARIAN_COCK_SIZE] < 3) {
                        this.outputText(", poking ");
                        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 1) this.outputText("its tip");
                        else this.outputText("their tips");
                        this.outputText(" out of Arian's genital slit");
                    }
                    this.outputText(".");
                }
                else if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText("and her moist pussy, begins forming a stream of juices that gently run its course through the inside of Arian's thighs, evaporating before hitting the floor due to the lizan's currently elevated body heat.");
                this.outputText("  [Arian Ey] pants, eyes glazed.  \"<i>Uhh... [name]...</i>\"");
        ...
        this.flags[kFLAGS.ARIAN_MORNING] = 0;
    ...
    public arianEggingEvent(): void {
        ...
        if (this.flags[kFLAGS.ARIAN_EGG_CHAT] == 0) {
            this.flags[kFLAGS.ARIAN_EGG_CHAT]++;
            this.outputText("As you are about to enter Arian's tent, you hear a moan emanate from within.  Those aren't moans of pleasure though.  They are moans of discomfort.  Wondering if the lizan is all right, you decide to enter her tent.");
            ...
            this.outputText("As you are about to enter Arian's tent, you hear a moan emanate from within.  Seems like it's that time of the month again...");
        ...
        this.menu();
        this.addButton(this, 0, "Brown", this.pickAnEggArian, "brown");
        this.addButton(this, 1, "Purple", this.pickAnEggArian, "purple");
        this.addButton(this, 2, "Blue", this.pickAnEggArian, "blue");
        this.addButton(this, 3, "Pink", this.pickAnEggArian, "pink");
        this.addButton(this, 4, "White", this.pickAnEggArian, "white");
        this.addButton(this, 5, "Rubber", this.pickAnEggArian, "rubbery black");
        this.addButton(this, 6, "No Thanks", this.leaveEggs);
    }
    ...
    private leaveEggs(): void {
        this.clearOutput();
        this.outputText("You tell her that you don't want any colored eggs from her this month.  The lizan nods, deciding it's not necessary for her to spell out that this means she'll just produce ordinary eggs and eat them for breakfast after she's laid them.  \"<i>So, do you want something?</i>\" she asks.");
        this.doNext(this, this.camp.returnToCampUseOneHour);
    }
    ...
    public arianLaysEggs(): void {
        ...
        this.outputText("As you approach the tent, you wonder if Arian's ready to lay those eggs...  Your contemplations are promptly interrupted as you hear a pained moan coming from inside.  Seems like Arian is in labor!  Without delay, you rush inside, to be greeted by the sight of naked lizan-");
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) this.outputText("herm");
        else this.outputText("girl");
        this.outputText(" squatting over a wooden basin, one hand between her legs, massaging her dripping treasure, while the other is rubbing her belly to try and coax the eggs out of her.");
    ...
    private arianDildoFun(): void {
        ...
        if (this.flags[kFLAGS.TIMES_ARIAN_DILDOED] == 0) {
            ...
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                this.outputText(", past her cock");
                if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
            }
            else this.outputText(",");
            this.outputText(" to her dripping wet pussy, clearly yearning for some action.  Reflecting that your own love-lips feel the same way, you decide to try something fun and present to Arian the dildo that you ");
            if (this.player.hasKeyItem("Dildo") >= 0) this.outputText("bought from Giacomo");
            else this.outputText("got from Tamani");
            this.outputText(", asking if she's willing to give this a try.");
            ...
            this.outputText("You present your dildo to Arian again, telling her that this time, you want to play with your toys - after all, she enjoyed it so much the first time.");

            this.outputText("\n\nArian fidgets in embarrassment.  \"<i>Okay.</i>\"");
        }
        this.flags[kFLAGS.TIMES_ARIAN_DILDOED]++;
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            this.outputText("Ignoring her cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
            this.outputText(", y");
        }
        else this.outputText("Y");
        this.outputText("ou slide first one finger into her slick pussy and then another, gently caressing her inner labia to see if you can find her sweet spots.  Arian's eyes open in surprise as the first pang of pleasure hits her, she moans into your kiss.  Her tongue darts to dance with your own in excitement.");
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            this.outputText("brushing it tantalizingly across the sensitive skin of her pre-oozing cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
            this.outputText(" before");
        }
        this.outputText(" brushing it against her outer lips, stroking it up and down each labia, rubbing it against her clitoris, gently pushing the very tip of it into her cunt only to then withdraw it before it can really start to stretch her cunt.");
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            this.outputText("Her cock");
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s throb");
            else this.outputText("throbs");
            this.outputText(" throb as a jet of whiteness escape them.  ");
        }
        this.outputText("You can feel it... the suddenness of your penetration brought the poor lizan mage to an instant orgasm.");
