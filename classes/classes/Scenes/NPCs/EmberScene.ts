...
export class EmberScene extends NPCAwareContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        //Ember fuck cooldown
        if (this.player.statusAffectv1(StatusAffects.EmberFuckCooldown) > 0) {
            this.player.addStatusValue(StatusAffects.EmberFuckCooldown, 1, -1);
            if (this.player.statusAffectv1(StatusAffects.EmberFuckCooldown) < 1) this.player.removeStatusAffect(StatusAffects.EmberFuckCooldown);
        }
        //Ember napping
        if (this.player.findStatusAffect(StatusAffects.EmberNapping) >= 0) {
            this.player.addStatusValue(StatusAffects.EmberNapping, 1, -1);
            if (this.player.statusAffectv1(StatusAffects.EmberNapping) <= 0) this.player.removeStatusAffect(StatusAffects.EmberNapping);
        }
        if (this.followerEmber() && this.player.findStatusAffect(StatusAffects.EmberNapping) < 0) {
            //Mino cum freakout - PC partly addicted!
            if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] == 1 && this.player.findPerk(PerkLib.MinotaurCumAddict) < 0 && this.flags[kFLAGS.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM] == 0) {
                this.minotaurJizzFreakout();
                needNext = true;
            }
            //Ember is freaking out about addiction, but PC no longer addicted!
            else if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] == 0 && this.flags[kFLAGS.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM] == 1) {
                this.emberGetOverFreakingOutAboutMinoJizz();
                needNext = true;
            }
            //At max lust, count up - if ten hours lusty, ember yells at ya!
            if (this.player.lust >= 100 && this.player.gender > 0) {
                this.flags[kFLAGS.EMBER_LUST_BITCHING_COUNTER]++;
                if (this.flags[kFLAGS.EMBER_LUST_BITCHING_COUNTER] >= 10) {
                    this.emberBitchesAtYouAboutLustiness();
                    needNext = true;
                }
            }
            //Reset lust counter if not max lust'ed
            else this.flags[kFLAGS.EMBER_LUST_BITCHING_COUNTER] = 0;
        }
        if (this.model.time.hours > 23) {
            if (!this.player.isPregnant()) this.flags[kFLAGS.EMBER_BITCHES_ABOUT_PREGNANT_PC] = 0;
            this.flags[kFLAGS.DRANK_EMBER_BLOOD_TODAY] = 0;
        }
        return needNext;
    }

    public timeChangeLarge(): boolean {
        if (this.player.findStatusAffect(StatusAffects.EmberNapping) < 0 && this.followerEmber() && this.player.findStatusAffect(StatusAffects.EmberFuckCooldown) < 0) {
            //Ember get's a whiff of fuckscent and knocks up PC!
            if (this.player.hasVagina() && this.player.inHeat && this.player.pregnancyIncubation == 0 && EmberScene.rand(10) == 0 && (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3)) {
                this.emberRapesYourHeatness();
                return true;
            }
            else if (this.player.hasCock() && this.player.inRut && !this.pregnancy.isPregnant && EmberScene.rand(10) == 0 && this.flags[kFLAGS.EMBER_GENDER] >= 2) {
                this.emberRapesYourHeatness();
                return true;
            }
        }
    ...
    private emberCorruption(changes: number = 0): number {
        ...
        return this.flags[kFLAGS.EMBER_COR];
    }

    public followerEmber(): boolean {
        return this.flags[kFLAGS.EMBER_HATCHED] > 0;

    }

    public emberMF(man: string, woman: string): string {
        if (this.flags[kFLAGS.EMBER_GENDER] == 1) return man;
        else return woman;
    }
    ...
    private emberHasCock(): boolean {
        return (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3);
    }

    public emberChildren(): number {
        return (this.flags[kFLAGS.EMBER_CHILDREN_MALES] + this.flags[kFLAGS.EMBER_CHILDREN_FEMALES] + this.flags[kFLAGS.EMBER_CHILDREN_HERMS]);
    }

    private emberInternalDick(): boolean {
        return (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] > 0);
    }

    //Approaching Ember (Z)
    public emberCampMenu(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("ember-visit-at-camp"));
        //Low Affection:
        if (this.emberAffection() <= 25) this.outputText("Ember sighs as you approach, and doesn't even look you in the eye before speaking.  \"<i>What do you want?</i>\"");
        //Moderate Affection:
        else if (this.emberAffection() <= 75) this.outputText("Ember fidgets as " + this.emberMF("his", "her") + " tail starts to sway from side to side, " + this.emberMF("he", "she") + " looks at you and asks, \"<i>What is it?</i>\"");
        else this.outputText("Ember's eyes light up as you close in on " + this.emberMF("him", "her") + ", and " + this.emberMF("he", "she") + " smiles nervously.  \"<i>Y-Yes?</i>\"");
        //OPTIONS HERE
        //[APPEARANCE]
        ...
        var egg = undefined;
        var milk = undefined;
        if (this.flags[kFLAGS.EMBER_OVIPOSITION] > 0 && this.flags[kFLAGS.EMBER_GENDER] >= 2 && !this.pregnancy.isPregnant) egg = this.emberIsAnEggFactory;
        if (this.flags[kFLAGS.EMBER_MILK] > 0) milk = this.getMilkFromEmber;
        this.choices(this, "Appearance", this.embersAppearance, "Talk", this.talkToEmber, "DrinkBlood", this.bloodForTheBloodGod, "Drink Milk", milk, "Get Egg", egg,
            "Sex", this.emberSexMenu, "Spar", this.decideToSparEmbra, "", undefined, "", undefined, "Back", this.camp.campFollowers);
    }
    ...
    private emberSexMenu(output: boolean = true): void {
        if (output) {
            this.clearOutput();
            this.outputText("You ogle Ember, checking out the nuances of " + this.emberMF("his", "her") + " body.");
            //(Low Affection)
            if (this.emberAffection() <= 25) this.outputText("\n\n\"<i>Why are you looking at me like that?</i>\" " + this.emberMF("he", "she") + " says, flatly.");
            //(Medium Affection)
            else if (this.emberAffection() < 75) this.outputText("\n\n\"<i>What is it?  Is something wrong with my body?</i>\" Ember asks, checking " + this.emberMF("him", "her") + "self.");
            //(High Affection)
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) {
                this.outputText(this.emberMF("His", "Her") + " cock ");
                if (this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 1 || this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText("is poking out of " + this.emberMF("his", "her") + " slit");
                else this.outputText("is starting to swell with blood");
                this.outputText(".  ");
            }
            if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("You can see tiny rivulets of moisture starting to run down Ember's inner thighs as they rub together, barely hiding her precious treasure from your hungry eyes.  ");
            this.outputText("Well, " + this.emberMF("he", "she") + " is a sexy beast; you ask what naturally comes to mind.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) {
            ...
            if (this.player.hasCock() && this.player.lust >= 33) penetrateHer = this.penetrateEmbrah;
        }
        if (this.player.hasVagina() && this.player.lust >= 33) getEatenOut = this.getEatenOutByEmbra;
        ...
        this.addButton(this, 0, "Catch Anal", catchAnal_);
        this.addButton(this, 1, "Pitch Anal", pitchAnal);
        this.addButton(this, 2, "Blow Ember", blowEmber);
        this.addButton(this, 3, "Get Blown", getBlown);
        this.addButton(this, 4, "Eat Her Out", eatOut);
        this.addButton(this, 5, "Get Eaten Out", getEatenOut);
        this.addButton(this, 6, "Penetrate Her", penetrateHer);
        this.addButton(this, 7, "Get Penetrated", getPenetrated);
        if (this.emberAffection() >= 95 && this.player.hasCock() && this.player.cockThatFits(this.emberVaginalCapacity()) >= 0 && (this.player.hasItem(this.consumables.L_DRAFT) || this.player.lib >= 50 || this.player.minLust() >= 40))
            this.addButton(this, 8, "LustyFuck", this.highAffectionEmberLustFuck);
        this.addButton(this, 9, "Leave", this.emberCampMenu);
    ...
    public findEmbersEgg(): void {
        this.clearOutput();
        if (this.flags[kFLAGS.TIMES_FOUND_EMBERS_EGG] == 0) {
            this.outputText("You spot a cave entrance partially hidden behind mossy vegetation and decide to investigate.");
            ...
            this.outputText("You spot a familiar cave partially hidden behind the mossy vegetation and decide to confirm your suspicion.");
        ...
        this.simpleChoices(this, "Take It", this.takeEmbersEggHomeInADoggieBag, "Destroy It", this.destroyBabyEmberYouMonster, "", undefined, "", undefined, "Leave", this.leaveEmbersAssOutToDry);
    }

    //[=Leave=] (Z)
    private leaveEmbersAssOutToDry(): void {
        this.clearOutput();
        this.outputText("You can't decide what to do right now, so you leave the egg where it is and return to your camp.");
    ...
    private destroyBabyEmberYouMonster(): void {
        this.clearOutput();
        this.outputText("Raising your [weapon], you rain down blow after blow upon the egg.  The shell is freakishly tough, taking a lot of punishment before it shatters apart to spill a wave of egg white onto your " + this.player.feet() + "; a great pulpy mass of weirdly bluish-red yolk remains in the broken shell.");
        ...
        //[Eat][Leave]
        this.simpleChoices(this, "Eat It", this.eatEmbersYolkLikeAnEvenBiggerDick, "", undefined, "", undefined, "", undefined, "Leave", this.camp.returnToCampUseOneHour);
    }

    //[=Eat=]
    private eatEmbersYolkLikeAnEvenBiggerDick(): void {
        this.clearOutput();
        this.outputText("Unsure of where the impulse comes from, but uncaring, you crouch over the ruined shell of your 'kill' and begin messily scooping handfuls of yolk into your mouth.");
        ...
        this.player.createKeyItem("Dragon Eggshell", 0, 0, 0, 0);
        //(+5-10 to strength, toughness, and speed.)
        //(+20 Corruption)
        //(also slimefeed!)
        this.dynStats("str", 5 + EmberScene.rand(5), "tou", 5 + EmberScene.rand(5), "int", 5 + EmberScene.rand(5), "cor", 20);
    ...
    public getSomeStuff(): void {
        this.clearOutput();
        this.outputText("Your mouth tightens in consternation, and you pull out the shell of the so-called 'dragon egg', passing it over and asking if she can use it.");
        ...
        this.player.removeKeyItem("Dragon Eggshell");
        this.inventory.takeItem(this.weapons.DRGNSHL, this.telAdre.armorShop);
    }
    ...
    private takeEmbersEggHomeInADoggieBag(): void {
        this.clearOutput();
        this.outputText("You decide to take the egg, figuring that perhaps this dragon could aid you in your quest.");
        //(If player is shorter than 7 feet)
        if (this.player.tallness < 84) this.outputText("  Lifting it isn't as much of a problem as you thought; it's surprisingly light.  It is, however, very big and very awkward to carry.");
        else this.outputText("  Between the egg's surprising lightness, and your own size and wide arms, you can easily carry the egg.");
        ...
        //set flags
        this.player.createKeyItem("Dragon Egg", 0, 0, 0, 0);
        this.flags[kFLAGS.TOOK_EMBER_EGG] = 1;
        this.flags[kFLAGS.EMBER_COR] = 50;
        this.doNext(this, this.camp.returnToCampUseOneHour);
    }
    ...
    public emberCampDesc(): void {
        //Iz Ember an egg?
        if (this.flags[kFLAGS.EMBER_HATCHED] == 0) this.outputText("\nThat mysterious egg that you brought back to the camp is sitting in the crude nest you made.\n");
        //NOT AN EGG! HAHA!
        else {
            var choice: number = EmberScene.rand(3);
            if (choice == 0) this.outputText("Ember is lying in front of " + this.emberMF("his", "her") + " excavated den, watching the camp and resting.  Every once in a while " + this.emberMF("his", "her") + " eyes dart in your direction, before " + this.emberMF("he", "she") + " quickly looks away.\n\n");
            else if (choice == 1) this.outputText("Ember doesn't seem to be around " + this.emberMF("his", "her") + " excavated den... it doesn't take long before Ember lands in front of it and then takes off again, apparently occupied with darting around in the sky for fun.\n\n");
            else this.outputText("Ember appears to be fighting to stay awake; sometimes " + this.emberMF("he", "she") + " falls into a doze, but quickly snaps back to consciousness.\n\n");
        }
    }

    //Followers Descriptor (Z)
    private emberFollowerDesc(): void {
        this.outputText("The mysterious egg you found in the cave sits in the grass nest you assembled for it; it is three feet tall and nearly two feet in circumference.  The nest itself isn't very pretty, but at least it's sturdy enough to keep the egg from rolling around.\n\n");
    }
    ...
    public emberEggInteraction(): void {
        this.clearOutput();
        this.outputText("You approach the egg you found in that illusion-concealed cave. Though the light continues to pulse with its heartbeat overtones, it still just sits there, doing nothing.");
        //(If the egg Corruption level is 0-25, aka \"<i>Pure</i>\")
        if (this.flags[kFLAGS.EMBER_COR] <= 25) {
            this.outputText("  As you observe the egg, it glows with soft, metered pulses and you're overcome with a sense of calm and peace.  Watching them, you feel serene, as if untouched by this world's taint.");
        }
        //(else If the egg Corruption level is 26-74, aka \"<i>Tainted</i>\")
        else if (this.flags[kFLAGS.EMBER_COR] <= 75) {
            this.outputText("  As you observe the egg, it glows with bright, gaudy pulses and you're overcome with a sense of arrogance and strength.  You feel somehow as if you could do anything, as if you were superior to everything, and no one could dare say otherwise.");
        }
        //(If the egg Corruption level is 75-100, aka \"<i>Corrupt</i>\")
        else {
            this.outputText("  As you observe the egg you realize what you had taken for its pulses are actually just its normal color; the egg is actually 'glowing' with a black light!  As you stare, mesmerized, you begin to consider the pleasing contrast that would result if you covered it in your ");
            if (this.player.gender == 0) this.outputText("cum, if you had any... ");
            else {
                if (this.player.hasCock()) this.outputText("white ");
                else this.outputText("glistening girl-");
                this.outputText("cum... ");
            }
            this.outputText(" You stop yourself and shake your head.  Where did that thought come from?");
            this.dynStats("lus", 10 + this.player.cor / 10);
        }
        //(If player has lust >= 33)
        if (this.player.lust >= 33 && (this.flags[kFLAGS.EMBER_EGG_FLUID_COUNT] < 5 || this.flags[kFLAGS.EMBER_JACKED_ON] == 0)) {
            ...
            if (this.flags[kFLAGS.EMBER_EGG_FLUID_COUNT] == 4) {
            ...
            //[Yes][No]
            //[= Yes =]
            this.doYesNo(this, this.masturbateOntoAnEgg, this.dontEggFap);
        ...
        var draft = undefined;
        if (this.player.hasItem(this.consumables.INCUBID)) draft = this.createCallBackFunction(this, this.useIncubusDraftOnEmber, false);
        var pDraft = undefined;
        if (this.player.hasItem(this.consumables.P_DRAFT)) pDraft = this.createCallBackFunction(this, this.useIncubusDraftOnEmber, true);
        var milk = undefined;
        if (this.player.hasItem(this.consumables.SUCMILK)) milk = this.createCallBackFunction(this, this.useSuccubiMilkOnEmber, false);
        var pMilk = undefined;
        if (this.player.hasItem(this.consumables.P_S_MLK)) pMilk = this.createCallBackFunction(this, this.useSuccubiMilkOnEmber, true);
        var hair = undefined;
        ...
        var hatch = undefined;
        if (this.flags[kFLAGS.EMBER_EGG_FLUID_COUNT] >= 5 && this.flags[kFLAGS.EMBER_JACKED_ON] > 0 && this.flags[kFLAGS.EMBER_GENDER] > 0) {
            hatch = this.hatchZeMuzzles;
            this.outputText("\n\n<b>The egg is ready to be hatched - if you're just as ready.</b>");
        }
        if (hatch != undefined) this.choices(this, "Hatch", hatch, "Blood", this.giveEmberBludSausages, "IncubiDraft", draft, "Pure Draft", pDraft, "Succubi Milk", milk, "Pure Milk", pMilk, "Hair Serum", hair, "Ovi Elixir", ovi, "Lactaid", lactaid, "Back", this.leaveWithoutUsingAnEmberItem);
        else if (fap != undefined) this.choices(this, "Masturbate", fap, "Blood", this.giveEmberBludSausages, "IncubiDraft", draft, "Pure Draft", pDraft, "Succubi Milk", milk, "Pure Milk", pMilk, "Hair Serum", hair, "Ovi Elixir", ovi, "Lactaid", lactaid, "Back", this.leaveWithoutUsingAnEmberItem);
        else this.choices(this, "Hatch", hatch, "Blood", this.giveEmberBludSausages, "IncubiDraft", draft, "Pure Draft", pDraft, "Succubi Milk", milk, "Pure Milk", pMilk, "Hair Serum", hair, "Ovi Elixir", ovi, "Lactaid", lactaid, "Back", this.leaveWithoutUsingAnEmberItem);
    }

    //[= No =]
    private dontEggFap(): void {
        this.clearOutput();
        this.outputText("Shaking your head, confused and startled by these strange impulses, you step away for a moment. Once away from the egg, its pattern of pulsations returns to normal and you feel the urges disappear.");
        ...
        if (this.player.hasItem(this.consumables.LACTAID)) lactaid = this.useLactaidOnEmber;
        var hatch = undefined;
        if (this.flags[kFLAGS.EMBER_EGG_FLUID_COUNT] >= 5 && this.flags[kFLAGS.EMBER_JACKED_ON] > 0 && this.flags[kFLAGS.EMBER_GENDER] > 0) hatch = this.hatchZeMuzzles;

        this.outputText("  The egg's rhythm suddenly changes; as if it were excited by something - something that you have brought near it.");
        ...
        if (hatch != undefined) this.outputText("\n\n<b>The egg is ready to be hatched - if you're just as ready.</b>");
    ...
    private leaveWithoutUsingAnEmberItem(): void {
        this.clearOutput();
        this.outputText("You shake your head; it would probably be best not to tamper with it. Returning the items to your pockets, you leave the egg alone.  As you put them away, the egg's glow slows down dramatically, almost as if it were feeling... disappointment?");
        this.doNext(this, this.inventory.inventoryMenu);
    }
    ...
    private useIncubusDraftOnEmber(purified: boolean = false): void {
        ...
        if (purified) {
            ...
            this.flags[kFLAGS.EMBER_COR] -= 10;
            if (this.flags[kFLAGS.EMBER_COR] < 0) this.flags[kFLAGS.EMBER_COR] = 0;
        }
        else {
        ...
        this.outputText("Uncorking the vial, you drizzle the slimy off-white fluid onto the pointed cone of the egg.  It oozes slowly across the surface, then seeps through the shell, leaving not a drop of moisture.");
        if (this.flags[kFLAGS.EMBER_GENDER] == 3 || this.flags[kFLAGS.EMBER_GENDER] == 0) {
            this.outputText("  The egg's shell slowly changes to a soft, pastel blue.");
            this.flags[kFLAGS.EMBER_GENDER] = 1;
        }
        else if (this.flags[kFLAGS.EMBER_GENDER] == 2) {
            this.outputText("   The egg's shell slowly changes to a lavender hue.");
            this.flags[kFLAGS.EMBER_GENDER] = 3;
        }
        this.flags[kFLAGS.EMBER_EGG_FLUID_COUNT]++;
    ...
    private useSuccubiMilkOnEmber(purified: boolean = false): void {
        ...
        this.outputText("Popping the cap off of the milk bottle, you pour the contents onto the egg - the porous shell soaks up the milk as fast as you dump it, spilling not a drop.");
        //(If Unsexed or Herm:
        if (this.flags[kFLAGS.EMBER_GENDER] == 3 || this.flags[kFLAGS.EMBER_GENDER] == 0) {
            this.outputText("  The egg's shell slowly changes to a muted pink color.");
            this.flags[kFLAGS.EMBER_GENDER] = 2;
        }
        //If Male:
        else if (this.flags[kFLAGS.EMBER_GENDER] == 1) {
            this.outputText("  The egg's shell slowly changes to a lavender hue.");
            this.flags[kFLAGS.EMBER_GENDER] = 3;
        }
        this.flags[kFLAGS.EMBER_EGG_FLUID_COUNT]++;
    ...
    private useLactaidOnEmber(): void {
        ...
        //max uses 1
        this.outputText("Feeling a little bemused, you pour the creamy fluid onto the egg.  It is absorbed through the shell, and a spiderwork of creamy yellow vein-like markings suddenly forms on the shell's surface.");
        this.flags[kFLAGS.EMBER_MILK] = 1;
        this.flags[kFLAGS.EMBER_EGG_FLUID_COUNT]++;
        this.doNext(this, this.playerMenu);
    }
    ...
    private hairExtensionSerum(): void {
        ...
        //max uses 2
        this.outputText("Wondering at your motivations, you pour the goblin gunk onto the egg.  Most rolls harmlessly off of the shell, leaving you annoyed at the waste... until you see ");
        if (this.flags[kFLAGS.EMBER_HAIR] == 0) {
            this.outputText("a narrow tiger-stripe pattern suddenly develop");
            this.flags[kFLAGS.EMBER_HAIR] = 1;
        }
        else {
            this.outputText("the tiger-stripes multiply");
            this.flags[kFLAGS.EMBER_HAIR] = 2;
        }
        this.outputText(" on the egg.");
        this.flags[kFLAGS.EMBER_EGG_FLUID_COUNT]++;
        this.doNext(this, this.playerMenu);
    }
    ...
    private masturbateOntoAnEgg(): void {
        ...
        if (this.player.gender == 0) {
            this.outputText("The light pulses decrease in speed as you disrobe and expose your bare crotch, leaving you disappointed after summoning your perversity to bring you this far.  You feel as if you've let it down somehow...  This is confusing!  You decide to go away and deal with this fickle egg another time.");
            this.doNext(this, this.camp.returnToCampUseOneHour);
            return;
        }
        ...
        if (this.player.gender == 1) {
            this.outputText("The light of the egg pulses rapidly, throwing strange illumination and shadow over your form as you hastily peel off your [armor], too far gone to recognize the absurdity.  Your heart is racing so fast with excitement, lust, anticipation... it actually matches the tempo of the pulses from the egg, when you care to notice.");
            this.outputText("\n\nGrabbing your [cock] in your hands, you stand in front of the egg, ");
            if (this.player.cockTotal() <= 2) this.outputText("pumping vigorously.");
            else this.outputText("wrangling all your shafts together into one awkward bouquet of male organs and furiously stroking and squeezing them as best you can manage");
            this.outputText(".  The egg's pulsations lure you on, coaxing you to squeeze, pull, thrust, and massage " + this.sMultiCockDesc() + " as best you can.  Harder and faster you go, feeling the churning ache from deep inside you.  Finally, with a cry of release, you unleash a ");
            if (this.player.cumQ() < 100) this.outputText("trickle");
            else if (this.player.cumQ() <= 500) this.outputText("stream");
            else if (this.player.cumQ() <= 1000) this.outputText("gout");
            else this.outputText("wave");
            this.outputText(" of cum onto the egg.");
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 0) {
        ...
        //Female Version (Z)
            ...
            if (this.player.wetness() <= 3) this.outputText("few drops");
            else if (this.player.wetness() < 5) this.outputText("squirt");
            else this.outputText("torrent");
            this.outputText(" of girlcum jumps from your pussy onto the egg.");
            ...
            if (this.player.cockTotal() == 1) this.outputText("your [cock]");
            else this.outputText("as many of your cocks as you can manage to grab without falling off");
            this.outputText(".  Finally, relentlessly, inexorably, you cum, spraying your semen into the air to spatter back onto the egg, mixing with the girlish juices from your netherlips to soak into the egg's surface, leaving it slick with your mixed sexual fluids.");
        ...
        if (this.flags[kFLAGS.EMBER_EGG_FLUID_COUNT] < 5) {
        ...
        this.flags[kFLAGS.EMBER_JACKED_ON] = 1;
    ...
    private hatchZeMuzzles(): void {
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) {
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1) {
                ...
                this.outputText(" the monster's crotch; there, a deceptively small slit in the flesh suddenly disgorges a 16-inch penis unlike anything you've ever seen before, bearing a rounded, elongated head and a series of ridges that give it an almost segmented look.  A pair of apple-sized balls drop into place under it.  He is most definitely male");
                if (this.flags[kFLAGS.EMBER_MILK] > 0) this.outputText(", drooling nipples notwithstanding");
                this.outputText(".");
            }
            //Female Anthro (Z)
            else if (this.flags[kFLAGS.EMBER_GENDER] == 2) {
                ...
                this.outputText(" the monster's crotch; there, you see that the fine scales actually separate to reveal a slick-looking pussy.  She's clearly a female, with no noteworthy 'additions' that you can see.");
            }
            //Herm Anthro (Z)
            else {
                this.outputText("\n\nIt's huge, standing 7 feet tall at the very least.  Its build is lean and slender, with powerful arms and legs that end in reptilian claws, complete with splay-toed feet capped by menacing talons.");
                ...
                this.outputText("the monster's crotch; there, you see the scales part in two places.  The lower opening is unmistakably a pussy; but from the slit just above it suddenly distends a 16-inch penis unlike anything you've ever seen before, bearing a rounded, elongated head and a series of ridges that give it an almost segmented look.  Beneath it, a pair of apple-sized balls fall into place heavily, leaving you no doubt that she is a hermaphrodite.");
            }
        }
        //Boring version
        else {
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1) {
                ...
                if (this.flags[kFLAGS.EMBER_MILK] > 0 || this.flags[kFLAGS.EMBER_HAIR] > 0) this.outputText(", complete with ");
                if (this.flags[kFLAGS.EMBER_MILK] > 0) {
                    this.outputText("huge breasts, easily F-cups");
                    if (this.flags[kFLAGS.EMBER_HAIR] > 0) this.outputText(" and ");
                }
                if (this.flags[kFLAGS.EMBER_HAIR] > 0) this.outputText("long, feminine locks of hair");
                this.outputText(", but his maleness is undeniable.  Especially when you spot ");
                if (this.flags[kFLAGS.EMBER_INTERNAL_DICK] > 0) this.outputText("the slit in his pelvis that disgorges a foot and a half-long inhuman penis");
                else this.outputText("a foot and a half long human penis that sways between his legs");
                this.outputText(", completed by apple-size nuts held inside a fleshy sack.");
            }
            ...
            //Herm Monstergirl (Z)
                ...
                if (this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 0) this.outputText("huge, human prick");
                else this.outputText("huge inhuman cock hanging from some kind of internal sheath");
                this.outputText(" and apple-size nuts held inside a fleshy sack slung under - the ensemble hangs nearly level with her knees.  She... or he? is obviously a hermaphrodite.");
            }
        }
        //Aftermath (Z)
        this.doNext(this, this.meetEmberAftermath);
    }

    //Aftermath (Z)
    private meetEmberAftermath(): void {
        this.clearOutput();
        this.outputText("You can only stand there and stare at this strange creature, supposedly a dragon, for what feels like hours.");
        ...
        this.flags[kFLAGS.EMBER_HATCHED] = 1;
        this.player.removeKeyItem("Dragon Egg");
        this.doNext(this, this.camp.returnToCampUseOneHour);
    }
    ...
    private embersAppearance(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("ember-examine-appearance"));
        //Anthro Ember's Appearance (Z)
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) {
            this.outputText("Ember is a 7' 3\" tall humanoid dragon, with supple, long limbs and a build that is toned and athletic, with powerful underlying muscles.  " + this.emberMF("He", "She") + " looks strong and imposing, but ");
            if (this.flags[kFLAGS.EMBER_GENDER] == 1) this.outputText(" not overly muscled.");
            else this.outputText(" feminine.");
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1) {
                ...
                if (this.flags[kFLAGS.EMBER_MILK] > 0) this.outputText(" dribbling breasts,");
                this.outputText(" snout, wings, and horns, gives him the silhouette of a prince from your village's stories: dashing and handsome.");
            }
            //(Female/Herm)
            else {
                this.outputText("\n\nEmber's body is a curvy thing, not rough like you'd expect from a reptilian creature, but rounded and almost soft-looking, with a taut belly and a perfect hourglass figure, giving " + this.emberMF("him", "her") + " the silhouette of an amazon from your village's histories: beautiful but powerful.  Excepting the wings and horns, of course.");
            }
            ...
            this.outputText("  The ");
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText("little ");
            this.outputText("exposed flesh of Ember's body is a light shade of pink; but flushes when " + this.emberMF("he", "she") + "'s aroused, drawing your eyes towards " + this.emberMF("his", "her") + " most sexual parts.");

            switch (this.pregnancy.event) {
                case 2: this.outputText("  At the moment there's a slight pudgyness to her golden belly.");
                    break;
                case 3: this.outputText("  Her golden belly is a little larger and firmer than usual.");
                    break;
                case 4:
                case 5: this.outputText("  Her golden belly has grown quite a bit.  Ember often rests her hand on it, especially when she sees you looking at it.");
                    break;
                case 6: this.outputText("  Her large pregnant belly has forced Ember to change her posture.  It looks like she has a big golden bullseye painted on her stomach.  Ember looks tired but happy.");
                    break;
                case 7: this.outputText("  Her swollen belly is as large as that of any pregnant woman you can remember from Ingnam and you " + (this.flags[kFLAGS.EMBER_OVIPOSITION] > 0 ? "can easily make out the egg's outline under her golden skin" : "occasionally see movement as the baby shifts in Ember's womb") + ".");
                    break;
                case 8:
                case 9: this.outputText("  Ember's golden belly is stretched taut by the large " + (this.flags[kFLAGS.EMBER_OVIPOSITION] > 0 ? "egg" : "baby") + " in her womb.  You're sure she'll give birth very soon, there just isn't room for the " + (this.flags[kFLAGS.EMBER_OVIPOSITION] > 0 ? "egg" : "baby") + " to grow any larger.");
                    break;
                default:
            }
            ...
            if (this.flags[kFLAGS.EMBER_MILK] > 0 || this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("F-cup soft, pillowy breasts");
            else this.outputText("flat manly pecs");
            this.outputText(" covered in fine scales excepting " + this.emberMF("his", "her") + " areolas; 0.5 inch nipples protrude from the center of the ");
            if (this.flags[kFLAGS.EMBER_MILK] > 0 || this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("generous mounds");
            else this.outputText("masculine pectorals");
            this.outputText(".");
        ...
        //Dragon-girl Appearance (By Radar) (Z)
            ...
            this.outputText("Ember is a 7' 3</i>\" tall ");
            if (this.flags[kFLAGS.EMBER_GENDER] == 1) this.outputText("male");
            else if (this.flags[kFLAGS.EMBER_GENDER] == 2) this.outputText("female");
            else this.outputText("hermaphrodite");
            this.outputText(" dragon-" + this.emberMF("boy", "girl") + ", with slender limbs and a thin frame; " + this.emberMF("he", "she") + " refuses to wear any kind of clothing and exposes " + this.emberMF("him", "her") + "self naturally to the world.  " + this.emberMF("He", "She") + " sports a rather human looking face, but with several key differences.  Where a normal set of human eyes would be located, instead a pair of orange, reptilian eyes stares back at you, filled with immeasurable pride and ferocity.");
            ...
            //Short:
            if (this.flags[kFLAGS.EMBER_HAIR] < 1) this.outputText("is quite short, giving " + this.emberMF("him", "her") + " that definitive " + this.emberMF("masculine", "tomboy") + " look.");
            else this.outputText("drops down to " + this.emberMF("his", "her") + " shoulders, giving " + this.emberMF("him", "her") + " the look of the " + this.emberMF("handsome", "beautiful") + " warriors from your village legends.");

            this.outputText("\n\n" + this.emberMF("His", "Her") + " chest is also human in appearance and houses a pair of ");
            if (this.flags[kFLAGS.EMBER_MILK] > 0 || this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("F-cup breasts that support 0.5 inch nipples and hang heavily; you idly wonder if " + this.emberMF("he", "she") + "'ll develop lower back problems as she spends more time in Mareth");
            else this.outputText("manly pectorals with 0.5 inch nipples");
            this.outputText(".  Just below " + this.emberMF("his", "her") + " collarbone, in the middle of " + this.emberMF("his", "her") + " chest, you see what looks like a small, golden, heart-shaped scale; adorning the chest like a birthmark of some sort.");
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) {
                if (this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 0) this.outputText("  " + this.emberMF("He", "She") + " sports a flaccid penis and a pair of apple-sized balls that sit dangerously exposed to the elements, let alone to naked blades or a heavy blow.  Yet " + this.emberMF("he", "she") + " doesn't seem concerned about that in the least, almost daring anyone to focus on them.  While " + this.emberMF("he", "she") + " isn't aroused right now, Ember's penis can reach a length of approximately 16 inches, and it looks to be about 2 inches thick.");

                else this.outputText("  " + this.emberMF("He", "She") + " sports what looks like a protective slit of some sort, protecting " + this.emberMF("his", "her") + " dick from the elements as well as stray blows.  You can't see it right now; but you remember it to be about 16 inches long and 2 inches thick.");
            }
            //Ember has a pussy:
            if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("  The inviting lips of a human-looking pussy purse between her legs; some moisture seems to have gathered on her labia, giving it a slick look that just begs for attention.");
        ...
        this.doNext(this, this.emberCampMenu);
    }
    ...
    private talkToEmber(): void {
        ...
        if (this.player.isPregnant()) { //Extra check might protect against inappropriate Ember complaints
            if (this.flags[kFLAGS.EMBER_OVI_BITCHED_YET] == 0 && this.player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS) {
                this.emberBitchesAboutPCBeingFullOfEggs();
                this.doNext(this, this.camp.returnToCampUseOneHour);
                return;
            }
            if (this.player.pregnancyIncubation < 200 && this.player.pregnancyType != PregnancyStore.PREGNANCY_EMBER && this.flags[kFLAGS.EMBER_BITCHES_ABOUT_PREGNANT_PC] == 0) {
                this.manEmberBitchesAboutPCPregnancy();
                this.doNext(this, this.camp.returnToCampUseOneHour);
                return;
            }
            if (this.player.pregnancyType == PregnancyStore.PREGNANCY_EMBER && this.player.pregnancyType < 300 && this.flags[kFLAGS.EMBER_TALKS_TO_PC_ABOUT_PC_MOTHERING_DRAGONS] == 0) {
                this.emberTalksToPCAboutPCDragoNPregnancy();
                this.doNext(this, this.camp.returnToCampUseOneHour);
                return;
            }
        }
        if (this.flags[kFLAGS.EMBER_PREGNANT_TALK] == 0 && this.pregnancy.event > 1) {
            this.emberIsPregnantFirstTimeTalkScene();
            this.doNext(this, this.camp.returnToCampUseOneHour);
        }
        this.clearOutput();
        this.outputText("What will you talk about?");
        //Else you can pick one of three topics:
        //Talk about Dragons
        //Talk about Exploring
        //Talk about Yourself
        this.simpleChoices(this, "Dragons", this.talkToEmberAboutDragonzzz, "Exploring", this.discussExplorationWithEmber, "Yourself", this.talkToEmberAboutYourself, "", undefined, "Back", this.emberCampMenu);
    }

    //Talk about Dragons (Z)
    private talkToEmberAboutDragonzzz(): void {
        this.clearOutput();
        this.outputText("You ask Ember to tell you more about " + this.emberMF("his", "her") + " species.");
        ...
        if (choice == 0) {
            ...
            //Low affection:
            if (this.emberAffection() <= 25) this.outputText("\n\nSnorting, Ember cracks an amused smile as " + this.emberMF("he", "she") + " chuckles. \"<i>What, does the 'Champion' think " + this.player.mf("him", "her") + "self worthy of courting me?  That's a good one!</i>\"  " + this.emberMF("He", "She") + " giggles openly to make " + this.emberMF("his", "her") + " lack of interest in you known... yet, it seems rather forced.");
            ...
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 1) this.outputText(", a blush creeping onto " + this.emberMF("his", "her") + " cheeks");
            this.outputText(".");
    ...
    private discussExplorationWithEmber(): void {
        ...
        this.outputText("You ask Ember for news of anything interesting " + this.emberMF("he", "she") + " has seen during " + this.emberMF("his", "her") + " aerial explorations of Mareth.");
        this.outputText("\n\nEmber nods " + this.emberMF("his", "her") + " head and scratches " + this.emberMF("his", "her") + " chin thoughtfully.  \"<i>Let me think...</i>\"");

        if (choice == 0) {
            subChoice = EmberScene.rand(6);
            ...
            if (subChoice == 0) this.outputText("imps. Only a few imps, and they seemed very nervous. I'm guessing that whatever naturally lives there is something that they really don't want to get involved with.");
            else if (subChoice == 1) this.outputText("two goblins in the foothills, arguing with each other. One was saying that the other shouldn't go up into the hills, as apparently the minotaurs that live there are too much for an untrained girl like her to take without being split open. The second goblin just laughed at her, called her a 'wimp' and told her that she's going to get herself 'some juicy bull-cock and tasty mino-spooge'. Ugh, disgusting little creatures.");
            else if (subChoice == 2) this.outputText("a pair of muscle-bound bull-men beating on each other with their bare fists. They spent over an hour smashing each other into a bloody pulp, and then the winner promptly started fucking the loser up the ass.  I had seen more than enough by that point, so I left.");
            else if (subChoice == 3) this.outputText("this... creature... that looked kind of like a human woman, but with a big dick where her clit should be.  She was walking around stark naked, 'cept for a bunch of piercings, and leading this bull-man along like a pet by a chain attached to a ring anchored into his cockhead.");
            else if (subChoice == 4) this.outputText("a couple of goblins sharpening scissors on some rocks outside of a cave with a door on it. Weird. Wonder what they could be doing in there?");
            else if (this.player.findStatusAffect(StatusAffects.WormsOff) < 0 && EmberScene.rand(2) == 0) this.outputText("a horrible swarm of slimy white worms, clumped together into a mockery of a human form and squelching along.  It managed to latch onto this two-headed dog-creature and... ugh!  The worms started forcing their way into both of its cocks!  I've never seen anything so disgusting!");
            else if (subChoice == 5) this.outputText("this two-headed dog-morph loping around; it spotted an imp, dropped to all fours, then gave chase. Managed to catch the ugly little demon, whereupon it ass-raped it, then ate it.");
        }
        else if (choice == 1) {
            subChoice = EmberScene.rand(5);
            ...
            if (subChoice == 0) this.outputText("a whole tribe of imps, just lounging around in a glade, jerking themselves off or squabbling over food.  Nasty little things, but easily dispatched.");
            else if (subChoice == 1) this.outputText("a  goblin with a huge pregnant belly, laughing to herself and swilling down that ale they brew, slopping it all over herself.  Little hedonists.");
            else if (subChoice == 2) this.outputText("this strange bee-woman creature... she made this, this music that started messing with my head.  I spat a tongue of flames at her and she flew away in fright, luckily.");
            //(If player has impregnated Tamani)
            else if (subChoice == 3 && this.flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] > 0) {
                this.outputText("that green-skinned baby-making whore, Tamani.  She was letting some of her daughters suckle from her and grinning ear to ear as she named the 'prize catch' she got to father them, exhorting them to hunt him down.");
                if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("  You should have more pride than to let some brainless cunt like that have her way with you!");
            }
            //(If player has not yet impregnated Tamani)
            else if (subChoice == 3 && !kGAMECLASS.forest.tamaniScene.pregnancy.isPregnant) this.outputText("one goblin being teased by a bunch of pregnant goblins for not being pregnant yet.  She just spat back that she wanted a 'better catch' to be her baby-maker than a mere imp and wandered off.");
            //(If Jojo isn't in the camp & not corrupt)
            else if (EmberScene.rand(2) == 0 && this.monk <= 1 && this.player.findStatusAffect(StatusAffects.PureCampJojo) < 0) this.outputText("this mouse-morph monk, sitting in a glade and meditating. A goblin tried to proposition him; he just gave her a lecture and sent her running away in tears.  When an imp tried to attack him, he crushed its skull with a staff he had.  Not bad moves for such a weedy little thing...");
            else this.outputText("one glade I touched down in to catch myself a nice brace of plump coneys, when all of a sudden this... this thing made out of flailing vines and fruit attacks me.  It went up in a puff of smoke once I torched it, of course.");
        }
        else if (choice == 2) {
            subChoice = EmberScene.rand(2);
            ...
            if (subChoice == 0) {
                this.outputText("a pair of shark-women - well, one was a woman, the other had breasts but also had a cock");
                if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" like me");
                this.outputText(". They were on the beach, fucking each other's brains out.");
            }
            else {
                //(If Whitney's Farm is not yet in the Places menu)
                if (EmberScene.rand(3) == 0 && this.player.statusAffectv1(StatusAffects.MetWhitney) <= 1) this.outputText(" a big farm on the shoreline.  I saw some sort of cow-woman tending to fields of peppers, and a dog-woman herding cattle. They looked pretty fat and tasty, too... What?  I meant the cattle, stupid!  And I wouldn't have taken them - it's no fun snatching up livestock.  I prefer to chase down a deer or a wild auroch or something like that.");
                //(if Fetish Cult is encounterable)
                else if (EmberScene.rand(3) == 0 && this.player.findStatusAffect(StatusAffects.FetishOn) >= 0) this.outputText("a pair of humans, arguing about sex.  They both wanted it, but the guy said he wanted to do the 'naughty schoolboy and female teacher' routine, while the girl wanted to do the 'noblewoman and love-servant' routine.  Weird; their clothes kept changing back and forth as they argued.");
                //(If Beautiful Sword has not been acquired)
                else if (EmberScene.rand(3) == 0 && this.player.findStatusAffect(StatusAffects.TookBlessedSword) < 0 && this.player.findStatusAffect(StatusAffects.BSwordBroken) < 0) this.outputText("a sword jammed into a tree.  Weird; what kind of idiot would stick a weapon there like that?  And what kind of weakling wouldn't be able to take it out?");
                else if (EmberScene.rand(3) == 0 && this.player.findStatusAffect(StatusAffects.CampRathazul) < 0) this.outputText("a smelly rat-man moping around while some weird equipment bubbled and boiled.  I think maybe he was an alchemist.");
                else this.outputText("a great blob of green goo, sliding along and minding its own business.  I could swear it looked up at me once, and grew a penis... that can't be right, though.");
            }
        }
        else if (choice == 3) {
            subChoice = EmberScene.rand(4);
            ...
            if (subChoice == 0) this.outputText("a woman with four big breasts, squeezing milk out of her tits and into the sand.  I didn't know breasts could hold that much milk!");
            else if (subChoice == 1) this.outputText("a whole tribe of demons, lounging around an oasis.  Would have been too much bother to kick the crap out of them, so I left them alone - well, alright, I did buzz them to make them scatter like scared sheep for fun.");
            //(if player hasn't solved Marcus & Lucia's argument)
            else if (EmberScene.rand(2) == 0 && this.player.findStatusAffect(StatusAffects.WandererDemon) < 0 && this.player.findStatusAffect(StatusAffects.WandererHuman) < 0) this.outputText("a human with balls so big he had to carry them in a wheelbarrow, trundling through the wasteland with a succubus.  They were arguing about whether or not he should become an incubus.");
            else {
                this.outputText("this strange creature, like a woman with a snake's tail for legs, slithering through the sand.  ");
                if (this.player.isNaga()) this.outputText("She looked a lot like you.");
                else this.outputText("I've never seen anything like her before.");
            }
        }
        this.outputText("</i>\"");
        this.doNext(this, this.camp.returnToCampUseOneHour);
    ...
    private talkToEmberAboutYourself(): void {
        ...
        //(If PC Str < 50)
        if (this.player.str < 50) this.outputText("  \"<i>You could use more training; you look pretty weak...</i>\" Ember says, critically.");
        //(If PC Str < 90)
        else if (this.player.str < 90) {
            this.outputText("  \"<i>You're well on your way to being as strong as a dragon, but there's still room for improvement.</i>\" Ember says, pleased.");
            //(+1 point)
            points++;
        }
        //(If PC Str >= 90)
        else {
            this.outputText("  \"<i>With muscles like these, you could easily lift anything you wanted... even me...</i>\" Ember trails off dreamily, almost purring.  \"<i>N-Not that I'd want you to carry me in your arms or anything...</i>\"");
            //(+2 points)
            points += 2;
        }
        this.outputText("\n\nNext, Ember pokes at your chest and your ribs.  ");
        //(If PC Tou < 50)
        if (this.player.tou < 50) this.outputText("\"<i>I don't know how you could hope to survive out there.  You look like the wind could blow you away.</i>\"");
        //(If PC Tou < 90)
        else if (this.player.tou < 90) {
            this.outputText("\"<i>Not bad; some more training and you could be as tough as me!</i>\" Ember says, pleased.");
            //(+1 point)
            points++;
        }
        //(If PC Tou >= 90)
        else {
            this.outputText("\"<i>Your body's as tough as rock, you should show off more frequently...  N-Not that I'd want you to go naked!  You should always wear something to protect yourself,</i>\" " + this.emberMF("he", "she") + " quickly adds.");
            //(+1 Affection)
        ...
        //(If PC Int < 50)
        if (this.player.inte < 50) this.outputText("\"<i>You're still very naive... anyone or anything could trick you at any time...</i>\"  Ember frowns.");
        //(If PC Int < 90)
        else if (this.player.inte < 90) {
            this.outputText("\"<i>I see wisdom in your eyes, but you could always use more lessons.</i>\"");
            //(+1 point)
            points++;
        }
        //(If PC Int >= 90)
        else {
            this.outputText("Ember's eyes widen.  \"<i>I see great wisdom in your eyes...</i>\"  Ember's face grows more absent by the minute as " + this.emberMF("he", "she") + " looks at you in deep thought, until the trance breaks.  " + this.emberMF("He", "She") + " blows at your face, brushing a claw on your forehead.  \"<i>Y-You had something in your face... I wasn't staring into your steely eyes.</i>\"");
            //(+2 points)
            points += 2;
        }
        ...
        //(If PC Spd < 50)
        if (this.player.spe < 50) this.outputText("You try to block the pebble, but you're not quick enough.  It hits your belly, not hard enough to hurt.  \"<i>You need to work on your reactions; anything could surprise you out there and you'd be helpless.</i>\"");
        //(If PC Spd < 90)
        else if (this.player.spe < 90) {
            this.outputText("You successfully deflect the pebble.  \"<i>Not bad!  Next time try to catch it,</i>\"  Ember says, pleased.");
            //(+1 point)
            points++;
        }
        //(If PC Spd >= 90)
        else {
            this.outputText("You easily catch the pebble and throw it back at Ember, surprising " + this.emberMF("him", "her") + ".  \"<i>With reflexes like these, you could even...</i>\" Ember mumbles.  \"<i>D-Don't even think about it!</i>\" " + this.emberMF("he", "she") + " snaps.");
            //(+2 points
            points += 2;
        }
        this.outputText("\n\nSatisfied, Ember turns to take another look at you.");
        //(If PC has high dragon or lizard score)
        if (this.player.nagaScore() >= 3 || this.player.lizardScore() >= 3 || this.player.dragonScore() >= 3) {
            this.outputText("  Ember's eyes linger on your form.  After a moment of awkward silence, you clear your throat.  " + this.emberMF("he", "she") + " blinks and says hurriedly.  \"<i>Sorry... I was just admiring you-</i>\" Realizing what " + this.emberMF("he", "she") + " was about to say,and quickly blurts out.  \"<i>I mean the weather! Yes, nice day today isn't it?</i>\"  You're not convinced, but let it slide. Ember recomposes and clears " + this.emberMF("his", "her") + " throat before saying.");
            //(+1 Affection)
            points++;
        }
        this.outputText("\n\n\"<i>All right, here's what I think about you.</i>\"  ");
        if (points < 2) {
            this.outputText("\"<i>This is not good at all; it's a miracle you even managed to survive in this world thus far.  With the things I've seen roaming the land...</i>\" Ember trails off, concerned.  \"<i>You should stay in the camp; I'll help you train.</i>\"  You don't much like the assessment.");
        }
        //(else if points < 6)
        else if (points < 6) {
            this.outputText("\"<i>You're doing fine, but make sure you don't slack off, and keep training.</i>\"  You thank Ember for sharing " + this.emberMF("his", "her") + " thoughts.");
        }
        //(else)
        else {
            this.outputText("\"<i>You're quite a catch... if you strolled down the street you'd have dragons fawning all over you...</i>\"  Realizing what " + this.emberMF("he", "she") + " just said, Ember coughs furiously.  \"<i>I-I mean... lesser dragons might fawn all over you.  You don't meet my standards!</i>\"");
            this.emberAffection(5);
        }
        this.doNext(this, this.camp.returnToCampUseOneHour);
    }
    ...
    private manEmberBitchesAboutPCPregnancy(): void {
        this.clearOutput();
        this.flags[kFLAGS.EMBER_BITCHES_ABOUT_PREGNANT_PC] = 1;
        //(Low Affection)
        if (this.emberAffection() <= 25) this.outputText("The two of you talk about nothing in particular.  It's light, airy and pointless.  When you finish up, though, you realize something odd; Ember was doing " + this.emberMF("his", "her") + " best all throughout the conversation to avoid looking at your pregnant belly - almost as if " + this.emberMF("he", "she") + " were upset by it?");
        //(Moderate Affection)
        else if (this.emberAffection() < 75) this.outputText("Ember stares coldly at your gravid midriff as icebreaker after icebreaker from you falls flat.  You ask what's wrong.  \"<i>Nothing.  Nothing is wrong,</i>\" Ember states, flatly.");
        //(High Affection)
        else this.outputText("The expression Ember gives you is very cold.  \"<i>What is the meaning of this!?</i>\" " + this.emberMF("he", "she") + " says, motioning towards your bloated belly.  \"<i>You're supposed to help <b>me</b> breed more dragons; not slut around with trash!</i>\"  Ember blows out an indignant puff of smoke and walks away.");
    }
    ...
    private emberTalksToPCAboutPCDragoNPregnancy(): void {
        ...
        this.outputText("You notice Ember's eyes are fixated on your swollen belly, and cautiously ask what " + this.emberMF("he", "she") + "'s looking at.");
    ...
    private emberIsPregnantFirstTimeTalkScene(): void {
        this.clearOutput();
        this.flags[kFLAGS.EMBER_PREGNANT_TALK] = 1;
        this.outputText("You can't help but stare at Ember's swollen belly; it's still so hard to take in that you have actually fathered a child with a creature of legend.  Especially given that there are times when it's hard to be entirely certain that Ember genuinely likes you.");
        this.outputText("\n\nEmber catches you staring");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 1) this.outputText(" and blushes");
        this.outputText(", one of her hands rubbing her belly lovingly and tenderly.  \"<i>W-what is it?  Why are you looking at me like that?  Something wrong?</i>\"");

        this.outputText("\n\nYou merely give her a faint smile and tell her that she's beautiful when pregnant.");
        if (this.player.cor >= 66) this.outputText("  You almost choke on those cheesy words; still, she'll never put out for you if you don't butter her up.");
        ...
        if (this.player.cor >= 66) this.outputText("  Is she seriously buying this nonsense?  Your smile would be miles wide if you let your lip curl even a bit, so you keep a straight face with due dilligence.");
        ...
        if (this.player.cor < 30) this.outputText("  Given her attitude and nature, you know how lucky you are to have someone like her want to be your mate.");
    ...
    private emberBitchesAboutPCBeingFullOfEggs(): void {
        this.clearOutput();
        this.outputText("As you try and think of a topic to talk about, you realize Ember is staring at your egg-swollen stomach - not with anger or disdain, but with interest.  With a smirk, you place one hand on your belly and ask if " + this.emberMF("he", "she") + " finds you interesting to look at like this.");
    ...
    public emberBitchesAtYouAboutLustiness(): void {
        ...
        if (this.player.hasCock()) {
            this.outputText("your " + EmberScene.num2Text(this.player.cockTotal()) + " stiff as iron");
            if (this.player.hasVagina()) this.outputText(" and ");
        }
        if (this.player.hasVagina()) this.outputText("your " + this.vaginaDescript() + " slick and wet with moisture, ready to fuck");
        this.outputText(".  You want sex so bad it almost hurts...");
    ...
    public minotaurJizzFreakout(): void {
        ...
        if (this.followerKiha()) this.outputText("\n\nKiha saunters up and smirks.  \"<i>I thought I had a temper.</i>\"");
        this.flags[kFLAGS.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM] = 1;
        this.outputText("\n");
    }
    ...
    public emberGetOverFreakingOutAboutMinoJizz(): void {
        ...
        //(High Affection)
            ...
            //(Male/HermEmber:
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(", especially for sucking cock,");
            this.outputText(" you should come to me instead; I'll help you.</i>\"  Then as if realising what " + this.emberMF("he", "she") + "'s just implied, Ember looks away; and hurriedly adds, \"<i>But not the way you're thinking...</i>\"");
    ...
    private bloodForTheBloodGod(): void {
        ...
        //(Medium Affection)
            ...
            if (this.player.cor < 50) this.outputText(", expressing your gratitude; gently, you kiss " + this.emberMF("his", "her") + " fingers, then the cut, letting its cool, iron-tinted tang roll across your lips and tongue.  You carefully lick, trying to avoid causing pain as you drink that which " + this.emberMF("he", "she") + " has so generously offered you.");
            else this.outputText(", quickly drawing it to your mouth and forming a seal around the cut with your lips, anxious to let not a drop escape.");
            ...
            //[Continue][Stop]
            this.simpleChoices(this, "Continue", this.drinkDeeplyOfDagronBlud, "Stop", this.noMoDagronBlud, "", undefined, "", undefined, "", undefined);
        }
        //(High Affection)
        else {
    ...
    private noMoDagronBlud(): void {
        ...
        if (this.emberAffection() < 75) {
            this.outputText("You decide to stop for now and pull away.  Ember licks " + this.emberMF("his", "her") + " own wound " + this.emberMF("him", "her") + "self and you thank " + this.emberMF("him", "her") + " for sharing.");
            this.outputText("\n\n\"<i>D-Don't mention it...</i>\"");
        }
        else {
            this.outputText("You decide to stop for now and pull away.  Ember licks " + this.emberMF("his", "her") + " lips, draws " + this.emberMF("his", "her") + " tongue back into " + this.emberMF("his", "her") + " mouth and purrs with delight.  When " + this.emberMF("he", "she") + " realizes what " + this.emberMF("he", "she") + "'s doing, though, " + this.emberMF("he", "she") + " sobers.  \"<i>D-Don't get any strange ideas...</i>\"");
    ...
    private drinkDeeplyOfDagronBlud(): void {
        this.clearOutput();
        if (this.emberAffection() < 75) {
            this.outputText("You decide to continue drinking Ember's blood; intent on acquiring all the power it can bring out from within you.");
    ...
    private emberTFs(): void {
        ...
        //Gain Dragon Dick
        if (changes < changeLimit && this.player.dragonCocks() < this.player.totalCocks() && EmberScene.rand(3) == 0) {
            this.temp = 0;
            var choices: any[] = [];
            var select: number;
            ...
            while (this.temp > 0) {
                this.temp--;
                if (this.player.cocks[this.temp].cockType != CockTypesEnum.DRAGON) choices[choices.length] = this.temp;
            }
            //Randomly choose one of those locations
            select = choices[EmberScene.rand(choices.length)];
            this.outputText("\n\nYour " + this.cockDescript(select) + " tingles as pins and needles sweep across it.  You pull open your [armor] to watch as it changes; the tip elongates and tapers, like a spear; a series of ridges form along the shaft, giving it an almost segmented look, and a prominent knot swells at its base.  You can't resist stroking it, until it begins dripping pre; ");
            if (this.player.sens >= 50) this.outputText("however, it's not until you press on your new, sensitive knot that you manage to blow your load and enjoy the last few spasms of pleasure as it finally finishes changing.");
            else this.outputText("but you sternly rein in your hands and tuck them into your armpits as the arousing changes run their course.");
            this.outputText("  <b>You now have a dragon penis.</b>");
            //lose lust if sens>=50, gain lust if else
            this.dynStats("sen", 10, "lus", 10);
            changes++;
            //Apply the TF
        ...
        //Gain Dragon Head
        if (changes < changeLimit && EmberScene.rand(3) == 0 && this.player.faceType != FACE_DRAGON && this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) {
            ...
            this.player.faceType = FACE_DRAGON;
            changes++;
        }
        //-Existing horns become draconic, max of 4, max length of 1'
        if (this.player.hornType != HORNS_DRACONIC_X4_12_INCH_LONG && changes < changeLimit && EmberScene.rand(5) == 0) {
            //No dragon horns yet.
            if (this.player.hornType != HORNS_DRACONIC_X2 && this.player.hornType != HORNS_DRACONIC_X4_12_INCH_LONG) {
                //Already have horns
            ...
            //ALREADY DRAGON
                ...
                if (this.player.hornType == HORNS_DRACONIC_X2) {
                    if (this.player.horns < 12) {
                        ...
                        if (this.player.horns >= 12) this.outputText("  <b>Your horns settle down quickly, as if they're reached their full size.</b>", false);
                        changes++;
                    }
                    //maxxed out, new row
                    else {
        ...
        //Gain Dragon Ears
        if (changes < changeLimit && EmberScene.rand(3) == 0 && this.player.earType != EARS_DRAGON) {
            this.player.earType = EARS_DRAGON;
        ...
        //Gain Dragon Tongue
        if (changes < changeLimit && EmberScene.rand(3) == 0 && this.player.tongueType != TONUGE_DRACONIC) {
        ...
        //Gain Dragon Scales
        if (this.player.skinType != SKIN_TYPE_SCALES && changes < changeLimit && EmberScene.rand(3) == 0) {
        ...
        //Gain Dragon Legs
        if (this.player.lowerBody != LOWER_BODY_TYPE_DRAGON && changes < changeLimit && EmberScene.rand(3) == 0) {
            //(if drider)
            if (this.player.lowerBody == LOWER_BODY_TYPE_DRIDER_LOWER_BODY)
         ...
         */
        //Grow Dragon Wings
        if (this.player.wingType != WING_TYPE_DRACONIC_LARGE && changes < changeLimit && EmberScene.rand(3) == 0) {
            if (this.player.wingType == WING_TYPE_NONE) {
                ...
                this.player.wingType = WING_TYPE_DRACONIC_SMALL;
                this.player.wingDesc = "small, draconic";
            }
            //(If Small Dragon Wings Present)
            else if (this.player.wingType == WING_TYPE_DRACONIC_SMALL) {
            ...
            else if (this.player.wingType == WING_TYPE_SHARK_FIN) {
        ...
        if (this.player.dragonScore() >= 4 && changes < changeLimit && this.player.findPerk(PerkLib.Dragonfire) < 0) {
            ...
            if (this.emberAffection() >= 75) this.outputText("\n\nEmber immediately dives back in to soothe your battered throat and mouth with another kiss.");
            changes++;
        }
        if (this.player.dragonScore() >= 4 && EmberScene.rand(3) == 0 && this.player.gender > 0) {
            ...
            this.outputText(", making you wish Ember hadn't run you off.  All you can think about now is fucking " + this.emberMF("his", "her") + "; ");
            if (this.player.hasCock() && this.flags[kFLAGS.EMBER_GENDER] >= 2) {
                this.outputText("filling her womb with your seed and fertilizing her eggs");
                if (this.player.hasVagina() && this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" even while ");
            }
            if (this.player.hasVagina() && (this.flags[kFLAGS.EMBER_GENDER] == 3 || this.flags[kFLAGS.EMBER_GENDER] == 1)) this.outputText("taking that hard, spurting cock inside your own " + this.vaginaDescript(0));
            this.outputText("... too late, you realize that <b>Ember's blood has sent your draconic body into ");
            if (this.player.hasCock() && this.flags[kFLAGS.EMBER_GENDER] >= 2) {
                this.outputText("rut");
    ...
    private emberIsAnEggFactory(): void {
        this.clearOutput();
        this.outputText("You ask Ember if she would be willing to lay an egg for you.");
        //(Low Affection)
        if (this.emberAffection() <= 25) {
            ...
            if (this.player.cor >= 40) this.outputText("or sell it for quick cash ");
            this.outputText("anyway, but you don't tell her that.");
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) {
                this.outputText(" and her ");
                if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] > 0) this.outputText("dragon-dick jutting from its slit,");
                else this.outputText("veiny, bulbous penis fully");
                this.outputText(" erect");
            }
            this.outputText(".  One of her hands gently teases her clit, as another traces her outer labia; she bites her lips, trying to stifle her moans of pleasure, but it's useless... every touch brings a sigh.");
            ...
            //[(if Ember has a dick)
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" and her throbbing cock is soft enough to hide");
            this.outputText(" before she presents the egg to you.");
            ...
            this.inventory.takeItem(this.consumables.DRGNEGG, this.camp.returnToCampUseOneHour);
        }
        //(Medium Affection)
        else if (this.emberAffection() < 75) {
            this.outputText("\n\nEmber gasps");
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 1) this.outputText(" and her cheeks grow red with embarrassment");
            this.outputText(".  \"<i>T-This...</i>\"  She can't quite bring herself to finish the sentence and settles for just looking at the ground.");
            ...
            //[Watch][Fob Off]
            this.simpleChoices(this, "Watch", this.watchMediumAffectionEmberEggLay, "Don't Watch", this.dontWatchEmberLayEgg, "", undefined, "", undefined, "", undefined);
        }
        //(High Affection)
        else {
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(", and her cock points you accusingly");
            this.outputText(".");
            ...
            if (this.player.tongueType > TONUGE_HUMAN) this.outputText(" and out... and out...");
            this.outputText(" and then lean forward to give her a great, wet, sloppy lick, straight up the center of her pussy");
            //(E.Herm:
            if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" not stopping until you have slurped your way to the very tip of her cock");
            this.outputText(", savoring the unmistakable taste of her intimate juices.");
            ...
            if (this.player.tongueType > TONUGE_HUMAN) this.outputText("which is quite far indeed, ");
            this.outputText("caressing and stroking and playing all the tricks you can possibly think of to orally pleasure your draconic lover.  From the amount of juices beginning to seep onto your lapping tongue");
            if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" and dribbling down her painfully stiff dick");
            this.outputText(", you think you're doing rather well.");
            ...
            if (this.player.tongueType > TONUGE_HUMAN) this.outputText("  Right at this time, you feel something round and smooth on the tip of your tongue, gently spreading Ember's walls.  Realizing that this can only be her egg, you start trying to worm your long, sinuous tongue between it and her innermost walls, hoping to coax it out of her.");
    ...
    private dontWatchEmberLayEgg(): void {
        this.clearOutput();
        this.outputText("You take her hand and tell her that you wouldn't dream of intruding on her privacy, but ask her to think of you if she needs the inspiration.  She looks away shyly, and the barest hint of a smile breaks on her face.  Seems like she's already following your instructions.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(".. it even drips with some kind of off-white fluid.");
        this.outputText("  \"<i>H-here's your egg.  Use it while it's fresh, okay?</i>\"  Her eyes glaze over a bit at the suggestion, and she giggles.  ");
    ...
    private watchMediumAffectionEmberEggLay(): void {
        this.clearOutput();
        this.outputText("Ember fails to hide her arousal when you accept.  \"<i>Okay, then follow me.</i>\"  The two of you move into a secluded spot.  Once she is certain nobody is around to spy, Ember turns to face you");
        //(if Ember has a cock:
        if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(", cock poking straight out");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] > 0) this.outputText(" of its hiding place");
        this.outputText(".  \"<i>Okay... but you're only allowed to watch!  If you do anything weird, I swear I'll smack you!</i>\"");
        this.outputText("\n\nYou promise her that you won't do anything");
        if (this.player.lib >= 50) this.outputText(" she doesn't ask you to, beaming a salacious grin");
        this.outputText(".");
        ...
        if (this.player.gender > 0) {
            this.outputText("; ");
            if (this.player.hasCock()) {
                this.outputText("the bulge inside your [armor]");
                if (this.player.hasVagina()) this.outputText(" and ");
            }
            if (this.player.hasVagina()) this.outputText("wetness gathering in your own pussy");
            this.outputText(" more than indicate");
            if (!this.player.hasCock() || !this.player.hasVagina()) this.outputText("s");
            this.outputText(" your desire to break your promise.");
        }
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" and caressing her throbbing cock");
        this.outputText(".");
        this.outputText("\n\nWith a deep moan, Ember shakes and orgasms.");
        if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("  Ropes of jism spew out of her cock, arching into the air to hit the ground nearby.");
        this.outputText("  The egg plugs her wet pleasure hole, preventing any liquid from escaping, until finally with a wet, squelching pop,it flies out of her pussy and into your waiting hands; releasing a flood of juices.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" and her cock hanging out");
        this.outputText(", it's hard to see her as the same angrily defensive ice queen from before.");
    ...
    private getMilkFromEmber(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("ember-drink-her-milk"));
        this.outputText("You think for a few moments, then find your gaze drawn to Ember's round, firm");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText(", scaly");
        this.outputText(" breasts, " + this.emberMF("his", "her") + " perky nipples bare as always and enticing you.  With a repressed smile, you ask if " + this.emberMF("he", "she") + "'ll let you suckle " + this.emberMF("his", "her") + " milk.");
        //Summary:
        ...
        if (this.emberAffection() <= 25) {
            ...
            //noEmberkids:
            if (this.flags[kFLAGS.EMBER_EGGS] == 0 && this.emberChildren() == 0) this.outputText(this.emberMF("he", "she") + " has no offspring to feed them with");
            //(1+ EmberEggs:
            else if (this.flags[kFLAGS.EMBER_EGGS] > 0) this.outputText(this.emberMF("his", "her") + " children haven't hatched yet");
            else this.outputText(this.emberMF("his", "her") + " children don't drink nearly enough to properly empty " + this.emberMF("him", "her") + "), so is " + this.emberMF("he", "she") + " just going to suffer with " + this.emberMF("his", "her") + " breasts so full and aching?");
            ...
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText("scales");
            else this.outputText("skin");
            this.outputText(".  " + this.emberMF("He", "She") + " lets out a cute gasp of pleasure, eyes darting furiously around to look at anything but you. You beam a smile and " + this.emberMF("he", "she") + " turns her head away in embarrassment.  \"<i>T-They're not getting any emptier like this, y'know!?</i>\"");
            ...
            if (this.player.findStatusAffect(StatusAffects.DragonBreathCooldown) >= 0) {
                ...
                this.outputText("  Your throat feels soothed as the scratching and soreness die down; you feel like you could shout to the mountaintops!");
            }
            //(no new PG, PC has dragon-morph status and is opposite Ember's sex:
            if (EmberScene.rand(2) == 0 && this.player.dragonScore() >= 4 && this.player.gender > 0 && (this.player.gender != this.flags[kFLAGS.EMBER_GENDER] || (this.player.gender == 3 && this.flags[kFLAGS.EMBER_GENDER] == 3))) {
                this.outputText("  Though, a sudden swell of lust races through your ");
                if (this.player.hasCock()) {
                    this.outputText(this.cockDescript(0));
                    if (this.player.hasVagina()) this.outputText(" and ");
                }
                ...
                if (this.player.hasVagina() && (this.flags[kFLAGS.EMBER_GENDER] == 3 || this.flags[kFLAGS.EMBER_GENDER] == 1)) this.outputText("taking that hard, spurting cock inside your own " + this.vaginaDescript(0));
                this.outputText("... too late, you realize that <b>Ember's milk has sent your draconic body into ");
                if (this.player.hasCock() && this.flags[kFLAGS.EMBER_GENDER] >= 2) {
                    this.outputText("rut");
        ...
        //(High Affection)
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) {
                this.outputText("the faintest hint of " + this.emberMF("his", "her") + " cock ");
                if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] > 0) this.outputText("emerging");
                else this.outputText("hardening");
            }
            else this.outputText("small beads of moisture gathering on her honeypot");
            this.outputText(".  \"<i>Ok... I suppose I could use some help draining myself, but no funny moves!</i>\"");
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) {
                this.outputText(this.emberMF("his", "her") + " erecting dragon-prick, gently stroking its");
                if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] > 0) this.outputText(" strangely ridged, almost scaly");
                this.outputText(" surface");
            }
            else this.outputText("her gently-dripping cunt, sliding in between the lips to stroke the wet interior");
            this.outputText(".");
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("stroke " + this.emberMF("his", "her") + " shaft");
            else this.outputText("probe gently into her slick depths");
            this.outputText(".  Ember squirms and moans, humming softly, while you suckle and tease, delighting as much in the pleasure you are giving as in the delicious milk cascading down your thirsty throat.");
            ...
            this.outputText("  You tell your dragon that you're finished.  You're up and turning to leave when a looping tail around your waist stops you.");
        ...
        if (this.emberAffection() < 75) this.dynStats("lus", 20);
        this.fatigue(-50);
        this.HPChange(this.player.maxHP() * .33, false);
    }
    ...
    private decideToSparEmbra(): void {
        this.clearOutput();
        this.outputText("You feel like you could use some practice; just to be ready for whatever you stumble upon when adventuring, and ask Ember how " + this.emberMF("he", "she") + "'d feel about sparring with you.");
        //(Low Affection)
        if (this.emberAffection() <= 25) {
    ...
    public beatEmberSpar(): void {
        this.clearOutput();
        if (this.emberAffection() <= 25) {
            this.outputText("Ember lies on " + this.emberMF("his", "her") + " back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on pulling " + this.emberMF("him", "her") + " up, but " + this.emberMF("he", "she") + " bats it away, flinching in shame at " + this.emberMF("his", "her") + " defeat.");
        ...
        //Medium affection
            ...
            this.outputText("Ember lies on " + this.emberMF("his", "her") + " back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on helping " + this.emberMF("him", "her") + " up; " + this.emberMF("he", "she") + " takes it.");
        ...
        //High affection
            ...
            this.outputText("Ember lies on " + this.emberMF("his", "her") + " back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on helping " + this.emberMF("him", "her") + " up; " + this.emberMF("he", "she") + " gladly accepts your help.");
    ...
    public loseToEmberSpar(): void {
        ...
        //Low affection
        if (this.emberAffection() <= 25) {
            this.outputText("Panting, you drop your fighting stance and sit on the floor, lifting a hand and saying you've had enough.");
    ...
    private catchAnal(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("ember-fucks-your-ass"));
        this.outputText("You contemplate Ember's somewhat dominant streak in bed and ask if " + this.emberMF("he", "she") + " feels in the mood to ride your ass.");
        //(Low affection)
        if (this.emberAffection() <= 25) {
        ...
        if (this.emberAffection() < 75) {
            ...
            if (this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 1 || this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText(" of its hiding place");
            this.outputText(", growing slowly but steadily.");
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] > 0) this.outputText("segmented");
        else this.outputText("smooth");
        this.outputText(" length of " + this.emberMF("his", "her") + " prick between your " + this.buttDescript() + " cheeks, letting you feel the refreshing coolness of " + this.emberMF("his", "her") + " pre dripping from the tip.  \"<i>This... isn't going to work,</i>\" " + this.emberMF("he", "she") + " growls.");
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0) this.outputText("mouth");
        else this.outputText("maw");
        this.outputText(" and sliding " + this.emberMF("his", "her") + " cock meaningfully against your pucker.  \"<i>You sure you want this?  Last chance to back out.</i>\"  As if you're going to turn away now, so close to filling that hunger for a nice, hot cock in your ass.  Rearing your rump up, you show " + this.emberMF("him", "her") + " that you're ready if " + this.emberMF("he", "she") + " is.");
        ...
        if (this.player.ass.analLooseness == 0 || this.player.analCapacity() < 25) this.outputText(", leaving both of you wincing as the sensitive head of " + this.emberMF("his", "her") + " cock struggles to make room for its insertion.  \"<i>I can't believe you really want me to put it here... you're so tight it almost hurts,</i>\" " + this.emberMF("he", "she") + " whimpers.");
        //(PC has a loose asshole:)
        if (this.player.analCapacity() < 50) {
            this.outputText(", causing you to gasp as " + this.emberMF("his", "her") + " ");
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 1) this.outputText("draconic ");
            this.outputText("dick slips inside of you; stretching you and making you wince a bit in pain as " + this.emberMF("his", "her") + " spit rubs off along the interior of your anus.  \"<i>It feels pretty snug... good thing I prepared for this,</i>\" " + this.emberMF("he", "she") + " remarks with a gasp of pleasure.");
        }
        //(PC has a gaping asshole:)
        else {
            this.outputText("; the looseness of your " + this.assholeDescript() + " gives " + this.emberMF("him", "her") + " no resistance whatsoever as " + this.emberMF("he", "she") + " glides in without trouble.  \"<i>Huh... that was easier than I expected,</i>\" " + this.emberMF("he", "she") + " remarks in surprise.  \"<i>What have you been getting yourself into?</i>\"");
        }
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 1) this.outputText("draconic ");
        this.outputText("prick in and out, reaching for your deepest recesses to rub on the right spots.");
        ...
        //(PC has prick:
        if (this.player.hasCock()) this.outputText("  You can feel the dragon's member rubbing against your prostate, making " + this.sMultiCockDesc() + " jump to painful erectness, bubble pre-cum, and drool it down the shaft to pool on the ground.");
        //(PC has cunt:
        if (this.player.hasVagina()) {
            this.outputText("  The stimulus is starting to make you wet as well, your [vagina] drooling feminine lubricant ");
            if (this.player.balls > 0) this.outputText("over your [balls]");
            this.outputText(" to puddle underneath you");
            if (this.player.hasCock()) this.outputText(", mingling with your pre to form an ever-growing pool of mixed sexual fluids.");
        }
        ...
        //(if PC has balls:
        if (this.player.balls > 0) this.outputText("  " + this.emberMF("His", "Her") + " own balls swing into yours, and every time they connect a shrill wave of pleasure flows through you.");
        ...
        if (this.pregnancy.event > 3) {
            if (this.flags[kFLAGS.EMBER_OVIPOSITION] == 0) this.outputText("swollen ");
            else this.outputText("egg-laden ");
        }
        this.outputText("belly, and start to rise and sink, pistoning your ass up and down. You have total control over the sex, and you intend to use that.");
        ...
        if (this.player.hasVagina()) this.outputText("your cunt gushing feminine lubricant to glaze Ember's belly");
        if (this.player.hasVagina() && this.player.hasCock()) this.outputText(" and ");
        if (this.player.hasCock()) {
            this.outputText("your cock ");
            if (this.player.cumQ() < 250) this.outputText("spurting gouts of cum onto Ember's upper torso");
            else if (this.player.cumQ() < 1000) this.outputText("raining spunk across the supine dragon");
            else this.outputText("cascading your man-cream across your blissed-out lover until " + this.emberMF("he", "she") + " is completely glazed in a sperm coating");
        }
        this.outputText(".");

        this.outputText("\n\nEmber hugs you close, enjoying the afterglow");
        if (this.player.gender > 0) this.outputText(" and rubbing the results of your own orgasm against you");
        this.outputText(".  You sink readily into the dragon's embrace, rubbing against " + this.emberMF("him", "her") + ", feeling sleepy and blissful after a good, hard fuck.  Sadly, this moment of respite doesn't last long; you feel a certain discomfort as Ember's body heats up.  Looking up, you see " + this.emberMF("his", "her") + " eyes are wide open.  Before you can ask " + this.emberMF("him", "her") + " what's wrong, " + this.emberMF("he", "she") + " knocks you off with a start.");
    ...
    private suckEmberCock(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("ember-give-her-a-blowjob"));
        this.outputText("You stare at Ember's crotch, imagining the taste of draconic cum on your tongue.  The thought makes you lick your lips in eagerness, and you ask if Ember would enjoy having you suck on " + this.emberMF("his", "her") + " cock.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1) {
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 1) this.outputText("his single genital slit");
            else this.outputText("his genitals");
        }
        else {
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 1) this.outputText("her twin genital slits");
            else this.outputText("her genitals");
        }
        this.outputText(", " + this.emberMF("his", "her") + " ");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 1) this.outputText("draconic ");
        this.outputText("prick already hard and throbbing.  The dragon taps a clawed finger idly on the stump, awaiting your attention.");
        ...
        if (this.player.hasCock()) this.outputText("  [EachCock] hardens, spearing aimlessly into the ground in your arousal.");
        if (this.player.hasVagina()) this.outputText("  Your [vagina] starts to seep with feminine juices, your [clit] hardening in anticipation as your excitement dribbles down onto the thirsty ground.");
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText("scaly");
        else this.outputText("soft-skinned");
        this.outputText(" lower belly");
        if (this.pregnancy.event > 3) this.outputText(", though the gravid orb resists you");
        this.outputText(", and discover a surge of pride in you that you got all sixteen inches of it down.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 3 && (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] > 0)) {
            this.outputText("  Struck by perverse inspiration, you manage to wriggle your tongue under the base of " + this.emberMF("his", "her") + " cock and thrust it into " + this.emberMF("his", "her") + " genital slit.  You guide it as deep into the strangely pussy-like orifice as you can, tickling and caressing.");
            //(DemonTongue:
            if (this.player.tongueType > TONUGE_HUMAN) this.outputText("  Your inhuman length slithers deeper and deeper inside, and you realize you can feel two rounded objects; " + this.emberMF("his", "her") + " balls! You're actually touching the testicles that are normally locked away inside " + this.emberMF("his", "her") + " body, except when " + this.emberMF("he", "she") + " reaches " + this.emberMF("his", "her") + " most aroused states...");
        }
        ...
        this.outputText("\n\nYou close your eyes and submit to the dragon's gentle ministrations, savoring the close contact as " + this.emberMF("his", "her") + " cool tongue glides across your " + this.player.skinFurScales() + ".  Ember takes " + this.emberMF("his", "her") + " time, making sure you're absolutely spotless.  Once you've been cleaned and licked to " + this.emberMF("his", "her") + " satisfaction, " + this.emberMF("he", "she") + " finally settles down; getting off the stump to lay on the floor and pull you atop " + this.emberMF("him", "her") + "; hugging you close.");
        ...
        if (this.player.cor < 80) this.outputText("amused");
        else this.outputText("increasingly leery of " + this.emberMF("his", "her") + " batty behavior");
        this.outputText(", then pick yourself up to head back to the camp.");
    ...
    private stickDickInKnifeDrawer(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("ember-gives-you-a-blowjob"));
        this.outputText("Trying to appear confident and a little aloof, you suggest that maybe Ember would be willing to put " + this.emberMF("his", "her") + " kinky tongue to work on your cock.");
        ...
        if (!this.player.isNaga()) this.outputText("seat yourself on a conveniently lichen-covered boulder.");
        else this.outputText("coil up on your serpentine tail) and start peeling off the clothes on your lower half, exposing [eachCock] to the air and awaiting Ember's response.");
        ...
        if (this.player.cockTotal() > 1) {
            this.outputText("  " + this.emberMF("He", "She") + " selects the biggest cock and gives it a gentle stroke,");
        }
        else this.outputText("  " + this.emberMF("He", "She") + " takes your [cock " + y + "] in hand and begins gently stroking it,");
        this.outputText(" sniffing your musk with obvious delight.");

        //(if Ember chose a dragon cock)
        if (this.player.cocks[x].cockType == CockTypesEnum.DRAGON) this.outputText("\n\n\"<i>Such a magnificent, beautiful cock... it feels powerful... and familiar.</i>\"");
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText(", the scales on " + this.emberMF("his", "her") + " palms producing an indescribable feeling as they slide across your sensitive flesh,");
        this.outputText(" and unthinkingly blurt out your understanding.");

        this.outputText("\n\nSatisfied at that, Ember extends " + this.emberMF("his", "her") + " tongue, slowly wrapping it around your " + this.cockDescript(x) + ", an act that makes you shiver as the inhumanly long, prehensile appendage twines around your member like a slick-skinned snake.  " + this.emberMF("He", "She") + " coils it around your shaft expertly, gripping you into a makeshift cock-sleeve.");

        this.outputText("\n\nLooking down at Ember, you see the dragon wreathed in happiness; " + this.emberMF("he", "she") + "'s so focused on " + this.emberMF("his", "her") + " task that " + this.emberMF("he", "she") + " doesn't even notice you staring.  Using " + this.emberMF("his", "her") + " coiled tongue " + this.emberMF("he", "she") + " begins jerking you off; the sensation is familiar and yet so alien... it only takes a few strokes before your " + this.cockDescript(x) + " is glistening with Ember's saliva, and in a particularly pleasurable tug, you moan.  A bead of pre escapes your " + this.player.cockHead(x) + " and slowly slides down your shaft, until it makes contact with Ember's tongue.");

        this.outputText("\n\nThe moment " + this.emberMF("he", "she") + " tastes you, " + this.emberMF("his", "her") + " eyes seem to glow.  In a whip-like motion, Ember pulls " + this.emberMF("his", "her") + " tongue back into " + this.emberMF("his", "her") + " awaiting mouth, engulfing your " + this.cockDescript(x) + ".  You gasp as the cool, slick flesh surrounds you, heedless of the sharp teeth whose hard surfaces you can occasionally feel grazing you.  You trust your dragon too much to believe " + this.emberMF("he", "she") + " would hurt you.");

        this.outputText("\n\nEmber sucks you hard, slurping around your shaft with " + this.emberMF("his", "her") + " prehensile tongue and cooing with pleasure.");
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("  " + this.emberMF("His", "Her") + " shaft is already hard as rock, and sometimes you can feel it poke your " + this.player.feet() + ", smearing small beads of Ember's barely contained pleasure on your " + this.player.skinFurScales() + ".");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("  Looking behind her, you see a small pool of her feminine juices has formed; pheromones fly through the air and hit you like a wave, spurring you on, making you feed Ember more of your delicious pre.");
        ...
        if (this.flags[kFLAGS.EMBER_HAIR] == 0 && this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText("scaly pate");
        else this.outputText("hair");
        this.outputText(", telling " + this.emberMF("him", "her") + " that " + this.emberMF("he", "she") + " certainly has your attention and you're looking forward to finding out what " + this.emberMF("he", "she") + " has in mind.");
        ...
        this.outputText(" and begins licking you from base to tip in slow, almost torturous, strokes.  \"<i>Mmm.  So tasty...</i>\"  Sparks of pleasure surge up your body from the dragon's ministrations, and you find yourself biting back words because you're not sure if you want " + this.emberMF("him", "her") + " to go faster or to keep at it like this.");
        ...
        if (this.player.cumQ() < 250) {
            ...
            if (!this.player.isTaur()) this.outputText("a finger");
            else this.outputText("the thumb of her wing");
            this.outputText(" into your ass to tickle your prostate, drawing a few last drops out of you.");
        }
        //(Medium Cum Amount)
        ...
        if (this.player.cor < 75) this.outputText(" and chuckle; you know " + this.emberMF("he", "she") + " loves you, really.");
        else this.outputText(", folding your arms; " + this.emberMF("his", "her") + " dementia is getting worse...");
    ...
    private stickItInEmbersButt(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("ember-fuck-her-in-her-buttz"));
        this.outputText("Your eyes are drawn to Ember's butt like iron filings to a magnet.  Haunted by the temptation to see " + this.emberMF("him", "her") + " bent over and offering " + this.emberMF("his", "her") + " ass to your hungry touch, you ask if Ember would be willing to be the catcher in a bout of anal sex.");
        ...
        //(High Affection)
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) {
                this.outputText(this.emberMF("his", "her") + " stiffening cock");
                if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" and ");
            }
            if (this.flags[kFLAGS.EMBER_GENDER] == 2) this.outputText("her ");
            if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("dripping pussy");
            this.outputText(" make it obvious that the idea is at least partially arousing.");
        }
        //(first time and corr < 40)
        if (this.flags[kFLAGS.TIMES_BUTTFUCKED_EMBER] == 0) this.outputText("\n\nYou ask if " + this.emberMF("he", "she") + "'s really so certain that it won't fit; you're eager to give it a try if " + this.emberMF("he", "she") + " is, but you won't push if " + this.emberMF("he", "she") + "'s really that scared of the idea...");
        ...
        if (this.player.hasVagina()) this.outputText("and [vagina] ");
        this.outputText("to the dragon.  Once you're fully undressed, you turn to look at Ember, to see " + this.emberMF("him", "her") + " openly masturbating; " + this.emberMF("he", "she") + " pants as " + this.emberMF("he", "she") + " strokes " + this.emberMF("his", "her") + " ");
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("cock ");
        if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("and ");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("pussy ");
        this.outputText("with a hand, while " + this.emberMF("he", "she") + " suckles on the fingers of the other.");

        var x: number = this.player.cockThatFits(this.emberAnalCapacity());
        if (x < 0) x = this.player.smallestCockIndex();

        this.outputText("\n\nOnce " + this.emberMF("he", "she") + " sees you're fully stripped, " + this.emberMF("he", "she") + " walks over to you and pushes you on the grassy ground with a growl of lust.  The slick digits of one hand find themselves wrapped around your erect " + this.cockDescript(x) + ", while the others press into Ember's tight pucker, slowly stretching it to accommodate you.");
        ...
        this.outputText("\n\nYou ");
        if (this.player.cor < 50) this.outputText("promise the dragon you understand, and insist you wouldn't dream of doing otherwise; you want this to be good for " + this.emberMF("him", "her") + ", not to cause " + this.emberMF("him", "her") + " pain.");
        else this.outputText("nod.");

        this.outputText("\n\n\"<i>Umm... okay then.</i>\"  Stepping over you, the dragon lifts " + this.emberMF("his", "her") + " tail and aligns you with the puckered hole.  Slowly, Ember rubs " + this.emberMF("his", "her") + " ass against you, stimulating you to produce a single drop of pre to ease your way in.  " + this.emberMF("He", "She") + " doesn't have to wait long, and with a deep breath, " + this.emberMF("he", "she") + " finally lets gravity run its course and your " + this.cockDescript(x) + " plunges into " + this.emberMF("his", "her") + " tight depths.");
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText("scaly ");
        this.outputText("cheeks and rubbing the base of " + this.emberMF("his", "her") + " long, flexible tail to try and convey just how wild " + this.emberMF("his", "her") + " ass is driving you.");

        this.outputText("\n\n\"<i>Ow!  Okay, okay!  Just stop and give me a moment to adjust at least,</i>\" Ember replies");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText(", panting with lust");
        else this.outputText(", a blush adorning " + this.emberMF("his", "her") + " face");
        this.outputText(".  You force yourself to remain still, giving the horny dragon a chance to recover from your " + this.cockDescript(x) + " impaling " + this.emberMF("his", "her") + " back passage, savoring the feeling of " + this.emberMF("his", "her") + " cool muscles squeezing and rippling around your shaft.");

        this.outputText("\n\nAfter a few minutes, Ember finally starts to move, stroking you with " + this.emberMF("his", "her") + " inner muscles.  ");
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("Droplets of pre slide along " + this.emberMF("his", "her") + " own shaft and down " + this.emberMF("his", "her") + " balls to gather at your belly.  ");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("Her slick pussy drips rhythmically, wetting your lower body with slick dragon juice.  ");
        this.outputText(this.emberMF("His", "Her") + " lusty moans each time " + this.emberMF("he", "she") + " comes down on your " + this.cockDescript(x) + " send a thrill up your spine, and the deep purring emanating from " + this.emberMF("his", "her") + " chest is audible enough to give you an idea of how much Ember is enjoying this, despite " + this.emberMF("his", "her") + " initial reluctance.");
        ...
        else this.outputText("the base of your spine");
        this.outputText(", and you groan to Ember that you're going to cum, soon.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(", jet upon jet of spunk arcing over " + this.emberMF("his", "her") + " head to paint the ground below");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("; a veritable gush of juices spews forth from her forgotten love-hole to splash against your lower body and over herself");
        this.outputText(".  The force of " + this.emberMF("his", "her") + " orgasm makes the dragon clench " + this.emberMF("his", "her") + " ass, sending waves of renewed pleasure through you.");
        ...
        if (this.player.str < 70) {
            this.outputText("desperately try to grab any of the flying pieces, but Ember keeps you pinned with " + this.emberMF("his", "her") + " tail");
            this.outputText(".");
        ...
        //(else str >=80)
            ...
            this.outputText("grab " + this.emberMF("his", "her") + " tail, pulling the dragon off " + this.emberMF("his", "her") + " feet and throwing " + this.emberMF("him", "her") + " to the ground as you roll over... and causing " + this.emberMF("him", "her") + " to drop them everywhere as " + this.emberMF("he", "she") + " falls.");
            ...
            //New
            this.outputText("\"<i>What do you think you're doing!?</i>\" Ember yells at you, obviously mad at your little stunt, one hand reaching behind to prevent any more cum from spilling.");
    ...
    private slurpDraggieCunnies(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("ember-eat-out-her-vagoo"));
        this.outputText("You contemplate the possibilities, and then offer to get down on your knees before the dragon and pleasure her orally.  You would love to have a taste of some dragon juice.");
        ...
        if (this.emberAffection() <= 25) {
            ...
            if (!this.silly()) this.outputText("with me");
            else this.outputText("on my affection meter");
            this.outputText(" just because you're offering to do this,</i>\" the dragon hastily adds.");
        }
        //(Medium Affection)
        else if (this.emberAffection() < 75) {
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" and slowly hardening shaft");
            this.outputText(".");
        }
        //(High Affection)
        else {
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" and shaft");
            this.outputText(".  \"<i>Ok... I suppose I would like that,</i>\" Ember replies, trying to calm herself down, lest she seem too excited by the idea.");
        }
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("  With Ember's cock jutting out above your head, you can't resist sliding your tongue along the underside of her shaft, just for an extra teasing motion.");
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0) this.outputText("blushing all the while and ");
        this.outputText("panting from the stimulation.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("  You think droplets of pre are starting to slide down her girl-cock and drip onto your head... but that's part and parcel of loving a herm, so you pay it no attention.");
        ...
        if (this.player.tongueType > TONUGE_HUMAN) this.outputText("inhuman ");
        this.outputText("flexibility you can muster, wriggling into the dragon's depths and trying to caress and stroke every last one of her most intimate places.  At one point in your exploration of Ember's quivering depths, you discover a special spot, and every time you make contact with that little spongy spot Ember rewards you with a buck and renewed gush of fluids.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(", heedless of the steady rain of pre dribbling into your " + this.hairDescript() + ".  You're confident she'll crack soon...");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("  Her throbbing shaft, not intending to be left out of the fun, records the event by marking the tree bark with white dragon spooge.");
        ...
        if (this.silly()) this.outputText("  My meter's not that full!");
        this.outputText("</i>\"");
        ...
        if (this.player.hasCock()) this.outputText("stiffened shaft");
        if (this.player.gender == 3) this.outputText(" and ");
        if (this.player.hasVagina()) this.outputText("moist cunt");
        if (this.player.gender == 0) this.outputText("empty crotch");
        this.outputText(" in sympathy, you head back to camp.");
    ...
    private getEatenOutByEmbra(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("ember-eats-your-vagoo-out"));
        this.outputText("You think about Ember's long tongue and the many advantages it must have, when you suddenly get an idea of what you'd like to do. You ask Ember if " + this.emberMF("he", "she") + "'d be willing to put that tongue of " + this.emberMF("his", "hers") + " to use and eat you out.");
        //(Low Affection)
        if (this.emberAffection() <= 25) {
        ...
        if (!this.player.isNaga()) this.outputText(" with your [legs] spread wide");
        this.outputText(", displaying your [vagina].  You tell Ember that " + this.emberMF("he", "she") + "'ll need to make the next move from here, smiling as you do so.");
        ...
        if (this.player.hasCock()) this.outputText(", as well as the musk emanating from your drooling cock");
        this.outputText(".  Ember's eyes close as " + this.emberMF("he", "she") + " savors your scent, committing it to memory and licking " + this.emberMF("his", "her") + " lips in preparation for the task ahead.");
        ...
        if (this.player.hasCock()) this.outputText(", and pre-cum beginning to bubble out of " + this.sMultiCockDesc());
        this.outputText(".  You wriggle in delight, praising Ember's skill with " + this.emberMF("his", "her") + " tongue and begging " + this.emberMF("him", "her") + " to keep going.");
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText("maw");
        else this.outputText("mouth");
        this.outputText(", kissing your contracting box as if it were a long-lost lover, pushing " + this.emberMF("his", "her") + " tongue ever deeper inside you, as if intent on reaching your womb.");
        ...
        if (this.player.wetness() >= 4) this.outputText("spattering");
        else this.outputText("drenching");
        this.outputText(" Ember's face with your feminine honey");
        if (this.player.hasCock()) this.outputText(" even as " + this.sMultiCockDesc() + " belches cum all over the preoccupied dragon");
        this.outputText(".");
        ...
        if (this.player.wetness() >= 4) this.outputText("  However, your squirting spatters make this an impossible task.  Ember doesn't seem to care, even as " + this.emberMF("his", "her") + " face is completely covered in femcum.");
    ...
    private penetrateEmbrah(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("ember-fuck-her-in-the-vagoo-with-your-penor"));
        ...
        this.outputText("Your eyes are drawn to the dragon's crotch, and you ask if she has any feminine itches she'd like scratched.");
        ...
        //(High Affection)
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 3) {
                if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 1) this.outputText(" and her cock peek out of its protective slit");
                else this.outputText(" and her cock begin hardening");
            }
            this.outputText(" in anticipation.  \"<i>Yeah... maybe... why do you ask?</i>\"");
        }
        //(First Time)
        if (this.flags[kFLAGS.EMBER_PUSSY_FUCK_COUNT] == 0) {
            ...
            if (this.player.cor > 75) this.outputText("  As if bitches like her could qualify as maidens in any but the most literal sense.");
        ...
        this.outputText("\n\nYou ");
        if (this.player.cor < 50) this.outputText("hasten to undress yourself for the impatient dragoness, quickly pulling");
        else this.outputText("languidly denude yourself, making sure to exaggerate every movement as you pull");
        ...
        if (this.player.cockThatFits(this.emberVaginalCapacity()) == -1) {
            ...
            if (this.player.cockTotal() == 1) this.outputText("It doesn't ");
            else this.outputText("None of them ");
            this.outputText("fit!</i>\"");
            ...
            if (this.player.cockTotal() == 1) this.outputText("that... that... monster inside me,");
            else this.outputText("any of those... those... monsters inside me,");
            this.outputText(" you'll tear me apart!</i>\"  Ember finishes by exhaling a puff of smoke in frustration... clearly she wants this as much as you.");

            this.outputText("\"<i>Find some way to shrink ");
            if (this.player.cockTotal() == 1) this.outputText("that");
            else this.outputText("those");
            this.outputText(" and then come back!</i>\"  Ember turns on her heels and walks away, moodier than usual.");
            this.doNext(this, this.camp.returnToCampUseOneHour);
            return;
        }
        ...
        x = this.player.cockThatFits(this.emberVaginalCapacity());
        if (x < 0) x = this.player.smallestCockIndex();
        if (this.player.cockThatFits(this.emberVaginalCapacity()) >= 0 && this.player.cockThatFits2(this.emberVaginalCapacity()) >= 0) {
            this.outputText("\n\n\"<i>You have quite a selection, but I only need one... this one!</i>\" Ember says, taking your " + this.cockDescript(x) + " in her hand and stroking it into a full erection.");
        }
        //(if PC has a dragon cock)
        if (this.player.cocks[x].cockType == CockTypesEnum.DRAGON) {
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("; her own erect cock spurts a small jet of pre");
        this.outputText(".");
        ...
        if (this.player.cumQ() < 250) this.outputText("dribble");
        else if (this.player.cumQ() < 1000) this.outputText("flow");
        else this.outputText("gush");
        this.outputText(" from her used cunt and you sit on the ground, while Ember remains limp on the ground, panting. Still suffering from the afterglow of your recent orgasm, you dimly realize that Ember herself doesn't seem to have cum yet; the only fluids dripping from her pussy are yours");
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(", and her cock, while engorged and swollen, hasn't poured forth yet");
        this.outputText(".");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("; her cock jumps and unloads on the ground below, marking this as the spot of your lovemaking");
        this.outputText(".  Her pussy grabs at you like a vice, constricting you with such pressure that you have trouble moving, even though her cunt is well-lubed from your mixed fluids.");
        ...
        //(High Cum Amount)
            ...
            if (this.player.balls == 0) this.outputText("prick is ");
            else this.outputText("[balls] are ");
            this.outputText("finished, Ember doesn't only look more than a few months pregnant; she looks absolutely huge, like her water could break at any moment!");
        }
        ...
        this.player.orgasm();
        this.dynStats("lib", .5, "sen", -2);
        this.doNext(this, this.penetrateEmbrahPartII);
    }

    //PART II!
    private penetrateEmbrahPartII(): void {
        this.clearOutput();
        this.outputText("You wake up to find Ember's face hovering over you with a smile; once she realizes you're awake, she quickly averts her gaze.");
        ...
        if (this.player.cumQ() < 1000) {
            ...
            this.player.cumMultiplier += 1 + EmberScene.rand(3);
        }
    ...
    private getPenetratedByEmberLastSexSceneWoooo(): void {
        this.clearOutput();
        this.outputText(this.images.showImage("ember-sticks-her-penor-in-your-vagoo"));
        this.outputText("Your eyes are drawn to the 16 inches of cool, throbbing ");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] > 0) this.outputText("dragon");
        else this.outputText("human");
        this.outputText(" dick between Ember's legs, and you lick your lips hungrily.  Your " + this.vaginaDescript() + " throbs eagerly as you tell Ember you have certain itches you need to scratch... is Ember enough of a " + this.emberMF("man", "herm") + " to scratch them for you?");
        ...
        //(High Affection)
            ...
            this.outputText("Ember's cock jumps ");
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0 && this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 0) this.outputText("to attention");
            else this.outputText("out of its protective slit");
            this.outputText(" at the mere mention of what you expect " + this.emberMF("him", "her") + " to do.  \"<i>I'm more than enough to handle two... no... three of you!  And if you want proof all you have to do is ask!</i>\" Ember boasts proudly.");
        }
        ...
        if (this.player.tongueType == TONUGE_HUMAN) this.outputText("yours ");
        else this.outputText("your own inhumanly sinuous muscle ");
        this.outputText("and slithering almost into your throat.  " + this.emberMF("He", "She") + " kisses you madly, even as " + this.emberMF("he", "she") + " sinks to " + this.emberMF("his", "her") + " knees and gently lays you out on the ground, clearly ready to start the sexing.");
        if (this.player.hasCock()) this.outputText("  As if " + this.emberMF("his", "her") + " hands caressing your cock weren't evidence of that.");
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] > 0) this.outputText("draconic ");
        else this.outputText("human-like ");
        this.outputText("cock begins making its way past your labia.");
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] > 0) {
            ...
            this.outputText("Finally you arrive at the base of " + this.emberMF("his", "her") + " cock where a bulbous knot sits; thankfully it hasn't inflated yet, but still, you can't help but groan as you are stretched even more by its entrance.");
        }
        //else
        else {
            ...
            this.outputText("  It's not as if it isn't still warm... but its lower temperature feels relieving and exciting all the same.");
        }
        ...
        this.outputText("\n\nEmber leans over you, panting, " + this.emberMF("his", "her") + " hot breath mere inches from your " + this.player.face() + ", gazing at you through half-lidded, lust-addled eyes.  \"<i>You feel so good... so hot... so sexy...</i>\" Ember says mid-pant.  You pant right back, reaching up and wrapping your arms around " + this.emberMF("his", "her") + " neck, staring back at " + this.emberMF("him", "her") + " with the same expression and telling " + this.emberMF("him", "her") + " you think " + this.emberMF("he", "she") + " feels just as good.");
        ...
        if (this.player.hasCock()) {
            this.outputText("  Your cock explodes, belching cum to ");
            if (this.player.cumQ() < 250) this.outputText("spatter");
            else if (this.player.cumQ() < 1000) this.outputText("drench");
            else if (this.player.cumQ() < 2000) this.outputText("completely cover");
            else this.outputText("utterly soak");
            this.outputText(" your draconic partner.");
        }
        ...
        if (this.player.findPerk(PerkLib.HistoryWhore) >= 0 || this.player.findPerk(PerkLib.HistorySlut) >= 0) this.outputText("  Using the skills you've honed, you make the muscles in your vagina ripple and wrinkle, teasing the cock caught inside you in a way few women can.");
        this.outputText("  Mmm... you can feel your own second orgasm coming in hot.  Maybe after another 4 or 5, you'll let the dragon go...");
    ...
    private getPenetratedByEmberLastSexSceneWooooPartII(): void {
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 1) this.outputText("protective slit");
        else this.outputText("limp cock and balls");
        this.outputText(".  \"<i>You used me like a fucktoy!</i>\"");
        ...
        if (this.emberAffection() < 75) this.outputText(this.emberMF("his", "her") + " cock was unlike anything else in this world?");
        else this.outputText(this.emberMF("he", "she") + " could handle two or even three of you at the same time?");
        this.outputText("  You expected more resistance if that was the case.");
    ...
    public emberRapesYourHeatness(): void {
        ...
        if (this.player.hasVagina() && this.player.inHeat && (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3))
            this.outputText("your [vagina] aching to be filled and your womb put to its proper purpose");
        else this.outputText("[eachCock] burning to be seeding ripe, ready wombs");
        this.outputText(".");

        this.outputText("\n\nEmber responds by ");
        if (this.player.hasVagina() && this.player.inHeat && (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3))
            this.outputText("humping against you, smearing your thigh with dragon pre.");
        else this.outputText("tightly pressing her drooling, puffy netherlips on your thighs.");
        this.outputText("  \"<i>I need you, [name].  I need you so badly... can you see how badly I need you?</i>\" Ember asks, panting in barely contained lust.  \"<i>I want to fuck you so badly... Let's make a baby now!</i>\"");
        this.dynStats("lus", 10 + this.player.lib / 10);
        this.outputText("\n\nWhat do you say?");
        //[Accept] [Deny]
        this.simpleChoices(this, "Accept", this.timeToPuffTheMagicDragon, "Deny", this.fuckOffEmberIWantANap, "", undefined, "", undefined, "", undefined);
    }
    ...
    private fuckOffEmberIWantANap(): void {
        this.clearOutput();
        this.dynStats("lus", 10 + this.player.lib / 10);
        this.outputText("Oh, your ");
        if (this.player.hasVagina()) this.outputText("[vagina]");
        if (this.player.gender == 3) this.outputText(" and ");
        if (this.player.hasCock()) this.outputText("[eachCock]");
        this.outputText(" so yearn");
        if (this.player.gender == 1 || this.player.gender == 2) this.outputText("s");
        this.outputText(" to take Ember up on " + this.emberMF("his", "her") + " offer! ...But you are better than this; you are not some mindless animal, a slave to your lusts.  You fight down the arousal gnawing at you and tell Ember you don't want to fuck " + this.emberMF("him", "her") + ".");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("protruding prick");
        if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" and ");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("creaming wet vagina");
        this.outputText(".  No doubt " + this.emberMF("he", "she") + "'s off to take care of " + this.emberMF("his", "her") + " needs by " + this.emberMF("him", "her") + "self.  You doubt " + this.emberMF("he", "she") + "'s going to be happy once " + this.emberMF("his", "her") + " head is clear... still, " + this.emberMF("he", "she") + "'ll get over it.");
        this.emberAffection(-10);
        //Set some cooldown so this doesn't proc all the goddamn time!
        this.player.createStatusAffect(StatusAffects.EmberFuckCooldown, 12, 0, 0, 0);
        this.doNext(this, this.playerMenu);
    }
    ...
    private timeToPuffTheMagicDragon(): void {
        this.clearOutput();
        this.outputText("Dazed and befuddled, you sniff Ember right back.  Mmm... " + this.emberMF("He", "She") + " smells delicious too, you tell " + this.emberMF("him", "her") + ".");
        ...
        if (this.player.pregnancyIncubation == 0 && !this.pregnancy.isPregnant && this.flags[kFLAGS.EMBER_GENDER] == 3 && this.player.gender == 3) {
            this.outputText("\n\n(Who should bear the kids?)");
            //[Ember] [You]
            this.simpleChoices(this, "Ember", this.breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding, "You", this.getKnockedUpByEmbrahBroBaby, "", undefined, "", undefined, "", undefined);
        }
        //[Play appropriate breeding scene.]
        //Female Breeding Scene:
        //PC not pregnant, Ember has dick, PC is in heat.
        else if (this.player.pregnancyIncubation == 0 && (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) && this.player.inHeat) {
    ...
    private getKnockedUpByEmbrahBroBaby(): void {
        this.clearOutput();
        this.outputText("Ember grabs you and rolls you around, pinning you under " + this.emberMF("his", "her") + " weight, whilst kissing you.  You allow the dragon to press you into the ground, rubbing your hands across ");
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 && this.flags[kFLAGS.EMBER_MILK] == 0) this.outputText("his flat, muscular chest");
        else this.outputText(this.emberMF("his", "her") + " soft, squeezable boobs");
        this.outputText(" and savoring the kiss.  Emboldened, you poke your tongue into " + this.emberMF("his", "her") + " mouth, to see how " + this.emberMF("he", "she") + " will react.");
        ...
        if (this.flags[kFLAGS.EMBER_OVIPOSITION] == 0) this.outputText("little dragons");
        else this.outputText("dragon eggs");
        this.outputText(", my beautiful.</i>\"  Ember resumes " + this.emberMF("his", "her") + " kissing; " + this.emberMF("his", "her") + " own hands roaming all over your body, as if mapping you out through touch, looking for the best places to touch and tease.");
        ...
        if (this.flags[kFLAGS.EMBER_OVIPOSITION] == 0) this.outputText("dragonlings");
        else this.outputText("dragon eggs");
        this.outputText(".  You want to breed!");
        ...
        this.outputText("  You groan throatily, your need burning through you, and start trying to grind yourself against Ember's shaft.  " + this.emberMF("He", "She") + " pins you down, though, making it impossible for you to actually do anything more than rub " + this.emberMF("his", "her") + " glans against your needy netherlips.");
        ...
        //(if Ember has a vagina)
        if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("\n\nYour bedroll is growing slick underneath you as lubricant drools from the herm dragon's neglected cunt, her tail thrashing around madly before sliding into the wet orifice to help goad her on in her goal to breed you.  ");
        else this.outputText("\n\n");
        this.outputText("You growl throatily, feeling that same need burning inside you. You clutch and claw against the dragon, fingers grasping deeply into " + this.emberMF("his", "her") + " scaly limbs as you drag, scrape, and thrust yourself against " + this.emberMF("him", "her"));
        if (this.player.hasCock()) this.outputText(", your neglected [cock] drooling pre-cum that is being smeared all across your belly and " + this.emberMF("his", "hers") + " with every move you make");
        this.outputText(".  Gods, you can feel " + this.emberMF("him", "her") + " filling you...");
        ...
        this.outputText("Your contracting walls milk the dragon's sensitive cock, trying to coax even more seed out of your panting dragon lover.  \"<i>I-it's time,</i>\" Ember mutters, crashing into your wet bedroll and thrusting deep inside you.");
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 1) this.outputText("blushing ");
        this.outputText("face.  \"<i>But if you must know, I do love you.</i>\"   With a final sigh Ember sets down on your bedroll for a quick nap.");
        ...
        if (this.flags[kFLAGS.EMBER_OVIPOSITION] == 0) this.outputText("baby dragon");
        else this.outputText("dragon egg");
        this.outputText(".  Yawning, you curl up to the dragon for a quick nap of your own.");
        this.player.orgasm();
        this.dynStats("sen", -2);
        ...
        this.doNext(this, this.createCallBackFunction(this, this.emberBreedingAfterMathWatchOutForRadioactiveFallout, false));
    }
    ...
    private breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding(): void {
        ...
        if (x < 0) x = this.player.smallestCockIndex();
        this.outputText("Ember catches you and rolls you around, pinning you to the ground under her.  She smiles at you seductively and reaches down to stroke your " + this.cockDescript(x) + ".");
        if (this.player.balls > 0) this.outputText("  She then reaches down lower to rub at your cum filled orbs.");
        if (this.player.hasVagina()) this.outputText("  The tip of her tail gently teases your slick [vagina] with tiny strokes.");
        this.outputText("  You shudder and instinctively thrust towards her, letting her feel your needy cock, full and aching to father young.");
        ...
        this.outputText("\n\nEmber jumps off you momentarily to inhale your musk, stroking you until a bead of pre forms on your " + this.player.cockHead(x) + ".  Ember's eyes grow wide, as if she had spotted a pearl; and she wastes no time in extending her tongue to lick the bead away.  \"<i>Hmm, delicious...  You're ready, and so am I!</i>\"");

        this.outputText("\n\nAiming your cock upwards; Ember straddles you and lowers herself on your pulsing " + this.cockDescript(x) + ", hissing in pleasure as your hot flesh finally makes contact with her drooling fuckhole");
        if (this.emberHasCock()) this.outputText(", her own cock springing up and throbbing with the electric contact.");
        else this.outputText(".");
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText("scale-covered ");
        this.outputText("orbs gently squish ");
        if (this.flags[kFLAGS.EMBER_MILK] > 0) this.outputText("and leak milk ");
        this.outputText("under your grasp.");
        ...
        if (this.flags[kFLAGS.EMBER_MILK] > 0) this.outputText("the slowly trickling rivulets of milk along with ");
        this.outputText("Ember's pleased moans are enough to make you push away such thoughts, you're glad she has those!");

        this.outputText("\n\nYou hungrily kiss her ");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText("draconic ");
        else this.outputText("oh-so-human ");
        this.outputText("face, nipping lightly at her lip, rolling your tongue over hers and savoring her inhuman tastes; Ember answers in kind with a kiss of her own, purring and moaning into your mouth.  Her vibrating breasts massage your hands as you knead, lift, and then roll them about.");

        //(if Ember has a dick)
        if (this.emberHasCock()) {
            this.outputText("\n\nYou feel something poke your [chest] and look down to see Ember's painfully erect ");
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 1) this.outputText("dragon ");
            this.outputText("prick.  With a mischievous curl of your lips, you pull Ember deeper into your kiss and grab her cock, stroking it vigorously.");
        }
        ...
        if (this.emberHasCock()) this.outputText("her cock spurts a few jets of cum onto your [chest]; ");
        this.outputText("her eyes roll to the back of her head.  With a muffled roar she finally caves, a veritable wave of dragon juices crashing into your lower body and pooling on your bedroll, filling the whole tent with the delicious smell of sex and pheromones.");

        //(If Ember has a dick)
        if (this.emberHasCock()) this.outputText("  Ember's cock wets the side of the tent with continuous jets of cum, only adding to the already overpowering smell of sex as she empties herself on top of you.");
        ...
        if (this.emberHasCock()) this.outputText("her spent cock throb; ");
        this.outputText("her eyes shut tight in pleasure as she reaches her second orgasm.  You muffle her groan of pleasure with one of your own.  A second wave of dragon fluids joins the first in wetting your lower body, along with your bedroll.");
        if (this.emberHasCock()) this.outputText("  Little gobs of cum spill from her cock to slide down her shaft and gather on your belly button.");
        this.outputText("  Her tail coils around you, helping her stay in position as she melts on top of you like putty.");
        ...
        if (this.player.hasVagina()) this.outputText("your neglected cunt splashing fluid down your [legs] and ");
        this.outputText("your " + this.cockDescript(x) + " gushing into the dragon's ready womb.");
        ...
        //(Low Cum Amount)
        if (this.player.cumQ() < 250) this.outputText("  Though you usually don't cum that much, Ember's contracting walls literally suck the cum out of you; forcing you to give up more than you usually would... something you're quite glad to do!");
        //(Medium Cum Amount)
        else if (this.player.cumQ() < 1000) this.outputText("  Your generous helping of cum works its way towards her awaiting womb.  In fact, due to the delicious massage your shaft is receiving from Ember's contracting, almost sucking vaginal walls, you find yourself pumping out even more cum that you would usually have, quickly filling her to the brim and beyond.  It's not until Ember's belly is slightly distended that you stop.");
        //(High Cum Amount)
        else this.outputText("  You dump a huge load of jism into Ember's awaiting belly, but her contracting walls seem to milk you for ever more!  Ember is filled to the brim, her belly already slightly distended, but you still continue to fill her with your seed.  Ember's tightly sealed pussy lips means the cum has nowhere to go, so instead it gathers in her womb, distending her belly until she looks at least a few months pregnant; and by the look of pleasure on Ember's panting face, you don't think she minds it.");

        this.outputText("\n\nWith a gasp and a sigh of utter relief, you slump into Ember's arms, collapsing onto her ");
        if (this.emberHasCock()) this.outputText("cum-slick ");
        this.outputText("belly and laying there to regain your strength, ");
        if (this.emberHasCock()) this.outputText("dismissive of the spent cock lying sandwiched between you as you start ");
        this.outputText("feeling the heat in her gut that marks the beginning of a child.");

        this.outputText("\n\nEmber purrs and pulls you into another kiss, holding you close with her tail, legs and arms; intent on keeping you right where you are.  Well, it's not like you could go anywhere else with her pussy effectively keeping your " + this.cockDescript(x) + " hostage inside its depths.");
        ...
        this.player.orgasm();
        this.dynStats("sen", -2);
        this.doNext(this, this.createCallBackFunction(this, this.emberBreedingAfterMathWatchOutForRadioactiveFallout, true));
    }
    ...
    private emberBreedingAfterMathWatchOutForRadioactiveFallout(emberPregged: boolean = true): void {
        ...
        this.outputText("\n\nEmber's cheeks ");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText(" are probably blushing under " + this.emberMF("his", "her") + " scales.");
        else this.outputText("blush.");
        this.outputText("  \"<i>You're welcome, but this is the only time I'll clean up after you!  I'm not your personal maid!</i>\"");
        ...
        if (!emberPregged) this.outputText("touch your belly");
        else this.outputText("point at Ember's belly");
        this.outputText(", and you ask the dragon if " + this.emberMF("he", "she") + " thinks it \"<i>took</i>\".");
        ...
        if (emberPregged) this.outputText("pumped into me...");
        else this.outputText("had me pump into you...");
        this.outputText("</i>\" Ember trails off, and you can see " + this.emberMF("his", "her") + " hand move to " + this.emberMF("his", "her") + " crotch to caress " + this.emberMF("his", "her") + " ");
        if (emberPregged) this.outputText("slit");
        else this.outputText("cock");
        this.outputText(", as if remembering the intense sex session you two just had.");
        ...
        this.outputText("\n\nEmber's ");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 1) this.outputText("blush");
        else this.outputText("discomfort");
        this.outputText(" deepens, and " + this.emberMF("he", "she") + " frowns at you.  \"<i>A-are you questioning my virility!?  Well, if you want, I can go again, right now!</i>\"");
    ...
    private emberPregUpdate(): boolean {
        ...
        switch (this.pregnancy.eventTriggered()) {
            case 1: //
                ...
                return true;
            case 2: if (this.flags[kFLAGS.EMBER_OVIPOSITION] > 0) {
                ...
                return true;
            case 4: this.outputText("\nEmber's belly has grown quite a bit.  Anyone can tell she's pregnant with a single glance.  ");
                if (this.flags[kFLAGS.EMBER_OVIPOSITION] > 0) {
                    this.outputText("Ember notices you looking.  \"<i>W-what? Never seen a pregnant woman before?</i>\" she asks indignantly, although she can't hide her smile as you continue to look.\n");
                }
                else {
                    this.outputText("Ember catches you looking" + (this.flags[kFLAGS.EMBER_ROUNDFACE] == 1 ? " and blushes" : "") + ".  \"<i>W-what is it?</i>\"  You reply merely that she carries a baby bump very well; she looks good pregnant.  \"<i>Oh, uh...  Thanks I guess?</i>\" she replies, looking away and flicking her tongue out nervously.\n");
                }
                return true;
            case 5: if (this.flags[kFLAGS.EMBER_OVIPOSITION] > 0) {
            ...
            case 6: if (this.flags[kFLAGS.EMBER_OVIPOSITION] > 0) {
                ...
                return true;
            case 7: if (this.flags[kFLAGS.EMBER_OVIPOSITION] > 0) {
    ...
    public emberGivesBirth(): void {
        ...
        if (this.flags[kFLAGS.EMBER_OVIPOSITION] == 0) {
            ...
            if (this.emberHasCock()) {
                if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 && this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 0) this.outputText("  Her human-like cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
                else this.outputText("  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect.");
            }
            ...
            this.outputText("\n\nYou cradle ");
            if (roll < 40) this.outputText("him");
            else this.outputText("her");
            this.outputText(" in your arms, uncaring of the stains as the fluid drenching its little body soaks into your [armor], gently soothing ");
            if (roll < 40) this.outputText("him");
            else this.outputText("her");
            this.outputText(".  Then, seeing Ember is recovering from her recent exertions, you grin proudly at her and gently hold your ");
            if (roll < 40) this.outputText("son");
            else this.outputText("daughter");
            this.outputText(" out for her to take.");
            ...
            if (this.emberChildren() > 1) this.outputText("  Your other draconic offspring quietly peek into the lair, having vacated the place when Ember went into labor.  They smile at the sight of their new sibling, gently slipping in to curl up beside the mother and new child.");
            this.outputText("  With a contented expression on your face, you leave Ember and your ");
            if (this.emberChildren() <= 1) this.outputText("new child");
            else this.outputText("newly expanded draconic brood");
            this.outputText(" to get some rest, leaving the den.\n");
            if (roll < 40) this.flags[kFLAGS.EMBER_CHILDREN_MALES]++;
            else if (roll < 80) this.flags[kFLAGS.EMBER_CHILDREN_FEMALES]++;
            else this.flags[kFLAGS.EMBER_CHILDREN_HERMS]++;
        }
        //Ember Lays Egg
        else {
            ...
            if (this.emberHasCock()) {
                if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0 && this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 0) this.outputText("  Her human cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
                else this.outputText("  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect.");
            }
            this.outputText("\n\nMind racing, you bend your head in close and stick out your tongue, sliding it back in and out of your mouth to get it nice and wet before delivering a long, sloppy lick to her inner walls.  You can taste her, and the strange salty-blood taste of amniotic fluid mingles with her natural lubricants.  It's an unusual taste but not unbearable, and you begin to lick with greater fervor and purpose.  Your intention is to try and drown the pain of her contractions with pleasurable stimulus.  It's a crazy idea, but it makes sense in a place like Mareth.");
            ...
            if (this.flags[kFLAGS.EMBER_EGGS] > 0) {
                this.outputText(", along with the ");
                if (this.flags[kFLAGS.EMBER_EGGS] == 1) this.outputText("other");
                else this.outputText(EmberScene.num2Text(this.flags[kFLAGS.EMBER_EGGS]) + " others");
            }
            this.outputText(", before telling her that she's beautiful too, and your daughters will certainly hatch and grow up to be just as gorgeous as she.");
            ...
            this.flags[kFLAGS.EMBER_EGGS]++;
        }
        this.player.createStatusAffect(StatusAffects.EmberNapping, 12, 0, 0, 0);
    }
    ...
    public giveBirthToEmberKids(): void {
        ...
        if (this.flags[kFLAGS.EMBER_OVIPOSITION] == 0) {
            ...
            if (this.player.balls > 0) this.outputText("slowly pushing your [balls]");
            else if (this.player.hasCock()) this.outputText("slowly pushing your " + this.multiCockDescriptLight());
            if (this.player.balls > 0 || this.player.hasCock()) this.outputText(" out of " + this.emberMF("his", "her") + " way, ");
            this.outputText("then " + this.emberMF("he", "she") + " blows softly on your contracting love-hole, slowly extending " + this.emberMF("his", "her") + " tongue to penetrate you.");
            ...
            if (this.player.findPerk(PerkLib.Masochist) >= 0) this.outputText("deliciously ");
            this.outputText("on the boundary between pleasure and pain.  You aren't sure how much more you can take.");
            ...
            this.outputText("\n\nEmber calmly smiles at you and points to your [chest].  You follow " + this.emberMF("his", "her") + " finger to see the little dragon nursing from your " + this.nippleDescript(0) + ". \"<i>Sorry.  ");

            if (roll < 40) this.outputText("He");
            else this.outputText("She");
            this.outputText(" was getting hungry and I didn't know what to do,</i>\" Ember explains.");
            if (this.flags[kFLAGS.EMBER_MILK] > 0) {
                this.outputText("  \"<i>I tried to feed ");
                if (roll < 40) this.outputText("him");
                else this.outputText("her");
                this.outputText(" myself, but ");
                if (roll < 40) this.outputText("he");
                else this.outputText("she");
                this.outputText(" wanted yours...</i>\"");
            }

            this.outputText("\n\nYou sigh softly, stroking your newborn's head even as it industriously sucks away at your " + this.nippleDescript(0) + ".  Speaking of which, you ask Ember what you've had - a boy?  A girl?  Both?");
            ...
            this.outputText(" face and starts to cry softly.  You gently help ");
            if (roll < 40) this.outputText("him");
            else this.outputText("her");
            this.outputText(" up onto your shoulder and gently pat ");
            if (roll < 40) this.outputText("him");
            else this.outputText("her");
            this.outputText(" on the back between ");
            if (roll < 40) this.outputText("his");
            else this.outputText("her");
            this.outputText(" little wings, eliciting a burp that clearly leaves ");
            if (roll < 40) this.outputText("him");
            else this.outputText("her");
            this.outputText(" feeling better.  ");
            if (roll < 40) this.outputText("He");
            else this.outputText("She");
            this.outputText(" coos, giggles and nuzzles into your neck, clearly happy to be here in the real world at last.");
        ...
        //PC Lays Egg
            ...
            this.outputText("\n\nYou're not certain how long you were sleeping for when you finally regain consciousness.  You wake, though, to the most wonderful sensations emanating from your " + this.nippleDescript(0) + ", and the feel of soft hands caressing and squeezing your [chest].  You open your eyes and find Ember leaning over you, greedily nursing " + this.emberMF("himself", "herself") + " from your milk.  You can't resist asking what " + this.emberMF("he", "she") + "'s doing.");

            this.outputText("\n\nEmber ");
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0) this.outputText("blushes and ");
            this.outputText("withdraws, licking " + this.emberMF("his", "her") + " lips of a stray droplet of milk.  \"<i>Sorry, it's just that you looked so full, and all that milk would've been wasted...  So, I thought I could help myself, not that I've been wanting to drink your milk or anything like that.</i>\"");
            ...
            if (this.flags[kFLAGS.EMBER_EGGS] > 0) this.outputText("s hatch");
            else this.outputText(" hatches");
            this.outputText(", anyway.  The dragon ");
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0) this.outputText("blushes and then ");
            this.outputText("scurries away, even as you pull yourself upright and get ready to go about your business.\n");
            this.flags[kFLAGS.EMBER_EGGS]++;
        }
        this.player.createStatusAffect(StatusAffects.EmberNapping, 5, 0, 0, 0);
    ...
    private highAffectionEmberLustFuck(): void {
        ...
        var x: number = this.player.cockThatFits(this.emberVaginalCapacity());
        if (x < 0) x = this.player.smallestCockIndex();
        this.outputText("You strip your [armor] and watch Ember as " + this.emberMF("he", "she") + " appraises your naked body.  You can see " + this.emberMF("his", "her"));
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) {
            this.outputText(" dragon cock ");
            if (this.emberInternalDick()) this.outputText("poking out of " + this.emberMF("his", "her") + " protective slit");
            else this.outputText("growing erect");
        }
        if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" and");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText(" her pussy beginning to drip in excitement");
        this.outputText(".");

        this.outputText("\n\n\"<i>Mmm... now that's a view and a half; just look at you... what a gorgeous " + this.player.mf("man", "girl") + ", with such ");
        if (this.player.cockTotal() == 1) this.outputText("a wonderful " + this.cockDescript(0));
        else this.outputText(" wonderful cocks");
        this.outputText("... and you're all mine, here and now,</i>\" " + this.emberMF("he", "she") + " croons appreciatively, giving you a lewd wink and flick of " + this.emberMF("his", "her") + " tongue.  \"<i>Still, while I'm enjoying the view, don't keep a " + this.emberMF("guy", "girl") + " in suspense; what do you have planned?</i>\" " + this.emberMF("he", "she") + " asks, tail flicking from side to side in an amused manner.");
        ...
        if (this.flags[kFLAGS.TIMES_EMBER_LUSTY_FUCKED] == 0) {
            ...
            this.outputText("\"<i>All of them,</i>\" you state.");
            ...
            this.outputText("\n\nYou nod");
            if (this.player.lib < 50 && this.player.minLust() < 40) {
                this.outputText(", grabbing a vial of Lust Draft from your pouches");
                this.player.consumeItem(this.consumables.L_DRAFT);
            }
            this.outputText(".");
        }
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1) {
            ...
            this.outputText("\n\nYou look into his eyes and smile, slowly pressing into his tight boy-pussy and spreading his cheeks with your girth.  Ember moans, arches his back and growls with delight, ass already clenching eagerly around your invading " + this.cockDescript(x) + ".  A few more humps and you feel yourself go as far inside his ass as possible.");
            ...
            this.outputText("\n\nYour ");
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText("she-dragon");
            else this.outputText("dragon-girl");
            this.outputText(" moans throatily and gives you a rapturous look.  \"<i>I love you, [name]... gods, I love you so much, and I'm so happy I can finally say that to your face,</i>\" she tells you.");
        ...
        //else if Ember has a pussy and the PC more than one cock:
            ...
            this.outputText(" of those?  I-I know it's not exactly part of the pose and all, but...</i>\"");
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(", not to mention the stiffness of her dick");
            this.outputText(", making it obvious she's ready.  \"<i>Oh, my mate... Please, fill me now!  Quit teasing me and just shove your cocks up your dragon's ready holes, jam them in as far as they can fit!</i>\" she pleads, but unable to meet your eyes in her embarrassment at her dirty talk.");

            this.outputText("\n\nYou have the urge to do just that, but you'd also like to savor it.  You align your shafts with her ready holes and begin pushing in.  It's a strange, but pleasant, feeling.  Her ass tries to reject the intruding advances of your " + this.cockDescript(x) + ", while her pussy seems intent on pulling your other " + this.cockDescript(x) + " in.  It only takes a couple insistent humps before you pierce the barrier formed by her sphincter and penetrate her ass.");
            this.outputText("\n\nEmber cries out, her whole body quivering in delight, both holes squeezing and clenching as they try to suck you inside");
            if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(", cock dribbling precum onto her belly");
            this.outputText(".");

            this.outputText("\n\nThe feeling of having ");
            if (this.player.cockTotal() == 2) this.outputText("both ");
            else this.outputText("two of ");
            this.outputText("your cocks enveloped in slick tightness and warmth is almost enough to make you fill her up with your seed right then and there, but somehow you manage to hold on.");

            this.outputText("\n\nYour ");
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText("she-dragon");
            else this.outputText("dragon-girl");
            this.outputText(" moans throatily and gives you a rapturous look.  \"<i>I love you, [name]... gods, I love you so much, and I'm so happy I can finally say that to your face,</i>\" she tells you.  Then she starts looking less in love and more in lust.  \"<i>But I also just love these cocks; these two wonderful breeding rods - come on, mate!  Breed your horny dragon!</i>\" she commands lustfully.");
        }
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2 && this.player.cockTotal() > 1) this.outputText(" and ass");
        this.outputText(" rippling and squeezing your intruding member");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2 && this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(".");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("vagina");
        else this.outputText("ass");
        this.outputText(" of " + this.emberMF("his", "hers") + " the pounding it deserves.");

        this.outputText("\n\n\"<i>C-come on... cum into me, please!  Fill me with your seed!</i>\" Ember gasps, pleading with you.");

        this.outputText("\n\nYou penetrate " + this.emberMF("him", "her") + " once, twice and finally hilt as much of your " + this.cockDescript(x) + " as you can inside " + this.emberMF("his", "her"));
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) {
            this.outputText(" slick pussy");
            if (this.player.cockTotal() > 1) this.outputText(" and tight ass");
        }
        else this.outputText(" tight ass");
        this.outputText(" and blow your load.");

        this.outputText("\n\nEmber howls exultantly as your steaming load gushes inside of " + this.emberMF("him", "her") + ".  " + this.emberMF("His", "Her") + " ");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) {
            this.outputText("cunt shudders, splashing femcum all over your intruding shaft");
            if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(", and her ");
            else this.outputText(", ");
        }
        if (this.flags[kFLAGS.EMBER_GENDER] == 3 || this.flags[kFLAGS.EMBER_GENDER] == 1) {
            this.outputText("cock erupts, spraying draconic cum up into the air to rain back down upon " + this.emberMF("his", "her") + " body, spattering " + this.emberMF("him", "her") + " in " + this.emberMF("his", "her") + " own juices and ");
        }
        this.outputText("leaving " + this.emberMF("him", "her") + " gasping for breath as the climax ebbs away.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2 && this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(", letting a satisfying stream of white mat the leaves below.  You smile at Ember, shaft");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" still flagging above " + this.emberMF("him", "her") + ".");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1) this.outputText("ass");
        else if (this.player.cockTotal() == 1) this.outputText("pussy");
        else this.outputText("holes");
        this.outputText(" with a squelch.  It feels so good... taking " + this.emberMF("him", "her") + " one time after the other.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 1 && this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(".  The dragon lets out an exultant cry as " + this.emberMF("his", "her") + " cum-slimed ");
        if (this.flags[kFLAGS.EMBER_GENDER] == 1) this.outputText("ass squeezes");
        else if (this.player.cockTotal() == 1) this.outputText("pussy squeezes");
        else this.outputText("holes squeeze");
        this.outputText(" you, milking your ");
        if (this.flags[kFLAGS.EMBER_GENDER] == 1) this.outputText("shaft as his cock spurts dragon-seed onto the leaves below him, filling the air with the scent of spunk and matting them into a steaming morass.");
        //{twin shafts} as {her cunt spasms wetly, drenching your lap with fresh femcum} {and/or} {[her] cock spurts dragon-seed onto the leaves below {her}, filling the air with the scent of spunk and matting them into a steaming morass}
            ...
            if (this.player.cockTotal() == 1) this.outputText("shaft as her cunt spasms wetly, drenching your lap with fresh femcum");
            else this.outputText("twin shafts as her cunt spasms wetly, drenching your lap with fresh femcum");
            if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" and her cock spurts dragon seed onto the leaves below her, filling the air with the scent of spunk and matting them into a steaming morass");
            this.outputText(".");
        }
        this.outputText("  Groaning and moaning like a whore, the dragon's wings beat, sending  cool air wafting over your overheating bodies before " + this.emberMF("he", "she") + " slumps onto the ground, barely able to hold " + this.emberMF("him", "her") + "self upright.  \"<i>...So good...</i>\"  Ember pants.  \"<i>I... uh... are you sure you want... round three?</i>\" " + this.emberMF("he", "she") + " asks, sounding a little tired as " + this.emberMF("he", "she") + " does, obviously not sure if you'll manage it.");
        ...
        //Else:
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("her cunt starts to drip with moisture");
            if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" and ");
            if (this.flags[kFLAGS.EMBER_GENDER] == 3 || this.flags[kFLAGS.EMBER_GENDER] == 1) this.outputText(this.emberMF("his", "her") + " cock begins to grow erect again");
            this.outputText(", amazement giving way to arousal.  " + this.emberMF("He", "She") + " licks " + this.emberMF("his", "her") + " lips with " + this.emberMF("his", "her") + " inhuman tongue, clearly eager to begin again.");
        }

        this.outputText("\n\nEmber crawls over towards you, gripping the base of your shaft");
        if (this.player.cockTotal() > 1 && this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("s");
        this.outputText(" tenderly yet firmly, stroking you slowly.  \"<i>I can't believe how hot I get when I see you sporting ");
        if (this.player.cockTotal() == 1) this.outputText("this");
        else this.outputText("these");
        this.outputText(".  It's just so... intoxicating... your scent, the way you look at me, everything really.</i>\"  You pat Ember's head");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 1 || this.flags[kFLAGS.EMBER_HAIR] > 0) {
            this.outputText(", ruffling " + this.emberMF("his ", "her "));
            if (this.flags[kFLAGS.EMBER_HAIR] >= 2) this.outputText("mane");
            else this.outputText("hair");
        }
        this.outputText(".  " + this.emberMF("He", "She") + " leans against your hand, hugging your midriff and letting your shaft");
        if (this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" brush against " + this.emberMF("his", "her") + " cheek.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1) this.outputText("depths");
        else this.outputText("nethers");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2 && this.player.cockTotal() > 1) this.outputText(" and depths");
        this.outputText(".  \"<i>Ahn.  D-deeper...</i>\"  You hook your arm under " + this.emberMF("his", "her") + " knee and pull " + this.emberMF("his", "her") + " leg up, nearly throwing the dragon off balance.  \"<i>Ah!  D-do you like it when I let you take charge?  Ugh- oh!  Well, I think maybe I kind of like letting you be in charge, too...</i>\"  You barely pay attention to " + this.emberMF("his", "her") + " teasing remarks, instead focusing on exploring " + this.emberMF("his", "her") + " cummy ");
        if (this.flags[kFLAGS.EMBER_GENDER] == 1) this.outputText("ass");
        else if (this.player.cockTotal() == 1) this.outputText("pussy");
        else this.outputText("love-holes");
        this.outputText(".  \"<i>Uhn... yes...  take me again.</i>\"  " + this.emberMF("He", "She") + " lets " + this.emberMF("his", "her") + " tongue loll out as " + this.emberMF("he", "she") + " pants in pleasure.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2 && this.player.cockTotal() > 1) this.outputText("both of your dicks");
        else this.outputText("your dick");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText(", moisture drenching the cock buried in her cunt as it slobbers greedily across your burning flesh.");
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("  " + this.emberMF("His", "Her") + " cock throbs and pulsates, ripples of arousal giving way to cumvein-bulging jets that shoot from " + this.emberMF("his", "her") + " prick to splatter wetly against the wall.");
        this.outputText("  Wobbling unsteadily, the dragon becomes increasingly dependent on you to hold " + this.emberMF("him", "her") + " upright - a climax is coming, and it looks like it's going to be big...");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) {
            this.outputText(this.emberMF("his", "her") + " cock fountaining seed across the wall, practically whitewashing it in steaming hot dragon-spunk");
        }
        if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText(" and ");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("her cunt gushing female ejaculate, almost soaking you with the cascade of feminine fluids and puddling wetly against your [feet]");
        this.outputText(".");
        ...
        this.outputText(", feeling the excess slide out around your cock");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2 && this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(".  \"<i>Ahh... more seed from my lover...</i>\" " + this.emberMF("he", "she") + " whispers, nearly passing out from the pleasure.  You feel yourself grow dizzy with pleasure and tumble onto the soft grass covering the den's floor, bringing Ember along.  Your dick");
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2 && this.player.cockTotal() > 1) this.outputText("s");
        this.outputText(" slide");
        if (this.player.cockTotal() == 1) this.outputText("s");
        this.outputText(" out of " + this.emberMF("his", "her") + " with a wet slurp, allowing your deposit to leak its way under your prone forms.");
        ...
        //[Yes] [No]
        this.menu();
        this.addButton(this, 0, "Yes", this.stayWithEmberAfterLustFuck);
        this.addButton(this, 1, "No", this.noStayingForCuddlesPostLustFuck);
    }
    ...
    private noStayingForCuddlesPostLustFuck(): void {
        this.clearOutput();
        this.outputText("You tell Ember that you can't stay, you have to get going now.  " + this.emberMF("He", "She") + " looks a bit disappointed, but forces " + this.emberMF("him", "her") + "self to smile all the same.  \"<i>I understand, you have other things to do... just know that I'll always be here for you, for better or worse.</i>\"  You ");
        //50 or less Corruption:
        if (this.player.cor < 50) this.outputText("thank her for being so understanding");
        else this.outputText("grunt an acknowledgement");
        this.outputText(" and then gather your things before heading off to wash yourself down.");
        this.doNext(this, this.camp.returnToCampUseOneHour);
    }
    ...
    private stayWithEmberAfterLustFuck(): void {
        ...
        var x: number = this.player.cockThatFits(this.emberVaginalCapacity());
        if (x < 0) x = this.player.smallestCockIndex();
        this.outputText("With a smile, you tell " + this.emberMF("him", "her") + " that you'd be happy to.  \"<i>Great, come here...</i>\" " + this.emberMF("he", "she") + " croons, scooting over towards you.  You open your arms and allow the dragon to snuggle up against you, folding your arms comfortably under " + this.emberMF("his", "her") + " wings.");
        ...
        //else if Ember is female:
            ...
            this.outputText("\n\nShe breaks the kiss and lightly strokes your shaft, smiling at you.  Then she aligns the tip of your " + this.cockDescript(x) + " with her pussy.");
            this.menu();
            this.addButton(this, 0, "Next", this.penetrateWithEmber);
        }
            ...
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0) this.outputText("-girl");
            this.outputText(" states, idly rubbing your cocks together.  One of your hands slips down between the two of you, sliding over the ridged surface of the herm dragon's cock before pressing itself against her well-used quim; sure enough, it's already drooling eagerly at the prospect of being filled again.");
            ...
            //[Frotting] [Penetrate]
            this.menu();
            this.addButton(this, 0, "Frotting", this.frottingWithFrottingEmberHerm);
            this.addButton(this, 1, "Penetrate", this.penetrateEmberHerm);
        }
    }
    ...
    private frottingWithFrottingEmberHerm(): void {
        this.clearOutput();
        this.outputText("You think the matter over, and then slowly rub your [cock biggest] against Ember's to answer her question.  The dragon-herm gasps, then smiles lewdly at you.");
    ...
    private frottingWithEmber(clear: boolean = true): void {
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0) this.outputText(this.emberMF("-boy", "-girl"));
        this.outputText("'s overly long tongue sticks out as pleasure overrides " + this.emberMF("his", "her") + " senses.  You groan as you slide your cock against Ember's, the thought occurring to you that this is going to be pretty messy when the inevitable comes, but you are too overwhelmed to actually say so.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("  Her moist dragon pussy is not helping the mess in the least.  You can smell her femcum pooling along with her pre.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2) this.outputText("  Her other hand frigs her pussy as she comes down from her orgasm, gushing female juices and leaking some of the cum you've pumped into her earlier.");

        this.outputText("\n\nThe dragon");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0) this.outputText(this.emberMF("-boy", "-girl"));
        this.outputText(" uses what strength " + this.emberMF("he", "she") + " still has to crawl over to you and slide back between your arms.  \"<i>Sorry, just had to get that last bit out of my system.</i>\"  " + this.emberMF("He", "She") + " yawns lowdly.  \"<i>[name]?  How about a quick nap?</i>\"  " + this.emberMF("He", "She") + " asks not even bothering to hear your reply before exhaustion gets " + this.emberMF("his", "her") + " and " + this.emberMF("he", "she") + " falls asleep, snoring lightly.  You smile at " + this.emberMF("him", "her") + " and stroke " + this.emberMF("his", "her"));
        if (this.flags[kFLAGS.EMBER_HAIR] == 0 && this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) this.outputText(" head");
        else if (this.flags[kFLAGS.EMBER_HAIR] == 2) this.outputText(" mane");
        else this.outputText(" hair");
        this.outputText(" before allowing yourself to also fall asleep.");
        this.player.orgasm();
        //doNext(14);
        this.menu();
        this.addButton(this, 0, "Next", this.emberJizzbangbangEnding);
    }
    ...
    private penetrateWithEmber(clear: boolean = true): void {
        ...
        var x: number = this.player.cockThatFits(this.emberVaginalCapacity());
        if (x < 0) x = this.player.smallestCockIndex();
        this.outputText("\"<i>Go on.</i>\"  She moves her arms around you and into a hug.  \"<i>Enter me.</i>\"  With no further prelude needed, you slide yourself into the damp interior of her cunt, the organ eagerly accepting you back for the fourth time.");
        ...
        if (this.flags[kFLAGS.EMBER_MILK] > 0) this.outputText(", milk seeping across your fingers,");
        this.outputText(" and promise you will, sliding slowly in until you have hilted yourself yet again.");

        this.outputText("\n\nThe dragon");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0) this.outputText("-girl");
        this.outputText(" moans at your ministrations");
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) {
            this.outputText(", her draconic prick pressing tightly against you as it leaks a steady stream of pre, matting ");
            if (this.player.biggestTitSize() > 1) this.outputText("your [chest], ");
            this.outputText("your belly");
            if (this.player.biggestTitSize() > 1) this.outputText(",");
            this.outputText(" and Ember's own.");
        }
        this.outputText("  \"<i>Do you like playing with my body?</i>\"  Ember asks, gazing at you through half-lidded eyes.  You give her a playful smirk and run your fingers expertly over her nipples, making her moan and mewl at the stimulation and saying more than mere words could on the matter.");
        ...
        if (this.player.tailType > TAIL_TYPE_NONE) this.outputText("the base of your tail, tugging lightly on it and stroking it for a moment, then her hands move on to ");
        this.outputText("your [butt], grabbing the ");
        this.outputText("cheeks.  You wriggle appreciatively under her grip, making it clear she's not half bad at this herself.  The she-dragon giggles at your compliment, coiling her tail around your [legs].  \"<i>I haven't even started playing with you properly yet, my mate, and you're already excited...</i>\"  She clicks her tongue in mock reproval.  \"<i>You're such a pervert aren't you, [name]?  Lucky for us, you are <b>my</b> pervert, and I enjoy being played with a lot... so go ahead and toy with my body as much as you want.  I'll make sure to return the favor,</i>\" she purrs lovingly, sliding her hands back up your back, ");
        if (this.player.wingType > WING_TYPE_NONE) this.outputText("stopping momentarily to stroke along your " + this.player.wingDesc + " wings before continuing up and ");
        this.outputText("stopping at the back of your head.");

        this.outputText("\n\nSlowly she guides you towards one of her erect nipples.  \"<i>");
        if (this.flags[kFLAGS.EMBER_MILK] > 0) this.outputText("Drink my milk, I've been saving it up specially for you,");
        else this.outputText("I may not have any milk in my breasts right now, but maybe if you keep suckling I'll be able to make you some,");
        this.outputText("</i>\" Ember teases you.  You smile at her and accept the nipple, rolling it between your lips and exerting gentle, teasing pressure with your teeth.");
        if (this.flags[kFLAGS.EMBER_MILK] > 0) this.outputText("  You savor the resultant gush of sweet dragon's milk as it squirts obediently down your throat.");
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("  A small dollop of pre escapes Ember's dragon-dick at the pleasure of the movement.");
        this.outputText("  \"<i>And don't forget about my other breast either.</i>\"  She takes your hand in her own and guide it to her other mound, helping you knead it.");
        ...
        if (this.flags[kFLAGS.EMBER_MILK] > 0) this.outputText("allowing milk to spill into your mouth and gulp down sweet load after load, ");
        this.outputText("listening to her moan softly in pleasure from your actions, feeling her netherlips rippling around your " + this.cockDescript(x) + ".  Your nursing is suddenly interrupted when you're pulled off Ember's breasts and into a deep kiss.  The dragon-girl shoves her tongue inside your mouth, exploring it in detail");
        if (this.flags[kFLAGS.EMBER_MILK] > 0) this.outputText(", not even caring that she's tasting her own milk as you finish gulping it down");
        this.outputText(".  You kiss her back as eagerly as you can until lack of air forces you to break the kiss and catch your breath.");
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0) this.outputText("-girl");
        this.outputText(" roaring towards the ceiling, then turning to look at you as her pussy constricts and milks you for all you're worth.  Already spent from your past three orgasms, you just sit back and enjoy yourself being milked by her strong vaginal muscles one last time.");
        ...
        this.outputText(".");
        if (this.flags[kFLAGS.EMBER_GENDER] == 3) this.outputText("  Her draconic cock throbs all the way through your orgasm, shooting blanks a few times before spurting a couple ropes of pre onto her belly.");
    ...
    private emberJizzbangbangEnding(): void {
        ...
        this.outputText(", letting you see the whiteness on " + this.emberMF("his", "her") + " tongue, before tipping " + this.emberMF("his", "her") + " head back and gulping it down, licking " + this.emberMF("his", "her") + " lips and moaning as if " + this.emberMF("he", "she") + " was tasting a fine wine.  \"<i>Thanks for the snack, my mate.</i>\" " + this.emberMF("He", "She") + " croons at you, a teasing expression on " + this.emberMF("his", "her") + " face.");
