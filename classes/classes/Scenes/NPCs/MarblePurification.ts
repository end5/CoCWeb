...
export class MarblePurification extends NPCAwareContent {
    ...
    public BLUHBLUH(): void {
        ...
        if (this.amilyScene.amilyFollower() && !this.amilyScene.amilyCorrupt() || this.followerKiha() || this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00238] == 1) {
            this.outputText("  ...Has she been fighting with ");
            if (this.amilyScene.amilyFollower() && !this.amilyScene.amilyCorrupt()) this.outputText("Amily");
        ...
        if (this.player.hasItem(this.consumables.PURHONY)) this.outputText("  You then recall that bottle of honey the bee-girl gave you, and ask if maybe it might work?");
        if (this.player.findStatusAffect(StatusAffects.PureCampJojo) >= 0) this.outputText("  You remember how Jojo helped you become pure, and suggest maybe his meditations could assist Marble?");
    ...
    public giveMarblePureQuestLabova(): void {
        ...
        if (this.flags[kFLAGS.TIMES_GIVEN_MARBLE_PURE_LABOVA] == 0) {
            ...
            if (this.player.statusAffectv2(StatusAffects.MarbleSpecials) == 4) this.outputText("eight");
    ...
    public startCombatWithMarbleSister(inCamp: boolean = true): void {
        ...
        if (inCamp && this.player.findStatusAffect(StatusAffects.ClaraFoughtInCamp) < 0) this.player.createStatusAffect(StatusAffects.ClaraFoughtInCamp, 0, 0, 0, 0);
    }
    //Chose submit
    ...
    public mediumIntelligenceDoesntDrinkCowCuntsTea(): void {
        ...
        if (kGAMECLASS.camp.companionsCount()) this.outputText("\n\nYou rush over to your fallen companion");
    ...
    public loseToClara(combat: boolean = true): void {
        ...
        if (combat) {
            ...
            //PC loses via lust
            else this.outputText("You just can’t fight it anymore, and desperately start to masturbate in front of Clara.  Laughing triumphantly, Clara walks up to you and forces a pill down your throat.  A moment later, you pass out.");
            this.outputText("\n\n");
        }
        //proceed to loss scene
    ...
    public defeatClaraCuntInAFight(cheated: boolean = false): void {
        if (cheated) this.clearOutput();
            ...
            if (this.player.statusAffectv1(StatusAffects.ClaraCombatRounds) >= 10 && this.player.findStatusAffect(StatusAffects.ClaraFoughtInCamp) >= 0) {
                this.clearOutput();
        ...
        //(if Marble is a futa)
        if (this.flags[kFLAGS.MARBLE_DICK_TYPE] > 0) this.outputText("egg Marble into violating Clara with her cock, ");
    ...
    public calmMurblesFatAssDown(): void {
        ...
        if ((this.player.tallness >= 80 && this.flags[kFLAGS.MARBLE_BOVA_LEVEL] < 2) || (this.player.tallness >= 86 && this.flags[kFLAGS.MARBLE_BOVA_LEVEL] >= 2)) {
    ...
    public marbleLeavesThePCOverCorruption(): void {
        ...
        else if (this.flags[kFLAGS.MARBLE_KIDS] == 1) this.outputText(", and holding her child's hand");
    ...
    public claraCampAddition(): void {
        ...
        if (this.flags[kFLAGS.CLARA_PURIFIED] > 0) {
            if (MarblePurification.rand(10) == 0) this.outputText("You catch Clara staring at you for a moment, then her gaze once again returns to some distant mountain top.  She has not taken her purification well.");
            else this.outputText("As always, Clara sits within, staring off into the distance.  She has not taken her purification well.");
        }
        else {
            //if (time of day <= 7 OR >= 20)
            if (kGAMECLASS.time.hours <= 7 || kGAMECLASS.time.hours >= 20) this.outputText("Clara is laying sprawled out on the ground of her cage, snoring loudly.");
            else {
