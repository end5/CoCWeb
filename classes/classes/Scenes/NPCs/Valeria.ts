...
export class Valeria extends NPCAwareContent implements TimeAwareInterface {
    ...
    public pcWinsValeriaSparDefeat(): void {
        ...
        if (this.player.HP < 1) this.outputText("in too much pain");
        else this.outputText("too turned on");
    ...
    private followersValeriaSex(display: boolean = true): void {
        ...
        if (display) {
            ...
            if (this.player.gender > 0) this.outputText("she's interested in a little fluid exchange");
            else this.outputText("she wouldn't mind helping you get some release, despite your genderlessness");
            this.outputText(".");
    ...
    private valeriaGetFucked(): void {
        ...
        if (this.player.hasVagina()) this.outputText("stroking the slick lips of your [vagina], ask Valeria to fuck you.");
        else this.outputText("posing seductively, ask Valeria to fuck you.");
        this.outputText("\n\nShe grins.  \"<i>You just lie back and let me take care of everything, partner.</i>\"  You do as she asks, flopping onto your back ");
        if (this.player.isBiped()) this.outputText("and spreading your legs ");
        this.outputText("as Valeria ");
        if (this.flags[kFLAGS.VELARIA_FUTA] == 0) this.outputText("forms a thick, gooey cock");
        else this.outputText("grabs her thick, gooey cock");
        ...
        if (this.player.hasCock()) this.outputText(", as she rhythmically flexes her ass cheeks, shifting her fluidity around your shaft within her with a feeling like lotion-covered hands gently stroking you");
        this.outputText(".");
        ...
        if (!this.player.hasCock() && this.player.hasVagina()) {
            ...
            if (this.player.hasFuckableNipples()) this.outputText("and [chest] ");
    ...
    private gooFlation(clearText: boolean = true): void {
        ...
        if (!this.player.hasVagina()) this.outputText("one lonely ");
    ...
    private valeriaSexDominated(): void {
        ...
        if (this.player.hasVagina() && (!this.player.hasCock() || Valeria.rand(2) == 0)) {
            ...
            if (this.player.vaginalCapacity() > 80 || this.player.analCapacity() > 80) this.outputText(" and your own receptibility");
            ...
            if (this.player.lactationQ() > 0) this.outputText("and milk ");
    ...
    private talkWithValeria(): void {
        ...
        if (this.player.race() != "human") this.outputText(" were");
    ...
    private flirtWithValeria(): void {
        ...
        if (this.player.hasCock()) this.outputText("brushing along the length of your rapidly-hardening cock");
        else this.outputText("brushing a wet, warm knuckle across your [clit]");
        this.outputText(".");
    ...
    private acceptValeriasNeeds(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.SlimeCraving) >= 0) this.outputText(" after all, you have the same ones");
    ...
    public valeriaAndGooThreeStuff(): void {
        ...
        */
        if (this.player.hasCock()) {
            ...
            if (this.player.cockTotal() == 1) this.outputText("it as it distends");
            else this.outputText("them as they distend");
        ...
        this.outputText("\n\nThe bluish");
        if (this.monster.skinTone != "blue") this.outputText(" " + (this.monster as GooGirl).gooColor6());
        ...
        if (this.player.hasCock() || this.player.hasVagina()) this.outputText("genitals");
        ...
        this.outputText("\n\nYou ");
        if (this.flags[kFLAGS.TIMES_VALERIA_GOO_THREESOMED] == 0) this.outputText("demand that she release you");
        else this.outputText("playfully comment that she really ought to release you");
    ...
    private valeriaGooRapeII(): void {
        ...
        if (this.player.biggestTitSize() >= 3) this.outputText("the valley of your cleavage");
        ...
        if (this.flags[kFLAGS.TIMES_VALERIA_GOO_THREESOMED] == 0) this.outputText(", unexpected");
        else this.outputText(" but expected");
        ...
        else this.outputText("You hold tight, keeping up your faux resistance until she yanks your cheeks open and twists her gooey reaming-tool into a sphincter-penetrating drill bit.");
        ...
        if (this.player.hasFuckableNipples() && this.player.biggestTitSize() >= 5) {
            ...
            if (this.player.hasVagina()) this.outputText(" as your snatch");
            else this.outputText(" as the randiest slut's snatch");
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.cockTotal() == 1) this.outputText("this little guy");
            else this.outputText("these little guys");
            this.outputText(", did you? ");
            if (this.player.biggestCockArea() >= 50) this.outputText("...Maybe big guy is a bit more appropriate. Whatever. ");
        ...
        if (this.flags[kFLAGS.TIMES_VALERIA_GOO_THREESOMED] > 0) this.outputText("You grin in eager anticipation, waiting to feel the flood of fluid that you know is coming. It won't take long.... You feel it in your butt first.");
        else this.outputText("At first, you don't realize what it is. You're floating in a bit of a sexual daze by now and barely cogent of what's going on, but it does feel quite nice, whatever it is. The spreading heat in your posterior is what finally clues you in to what's happening.");
        ...
        if (this.player.hasVagina()) {
            ...
            if (this.player.wetness() <= 2) this.outputText("leaking");
            else if (this.player.wetness() <= 3) this.outputText("dripping");
        ...
        if (this.player.hasFuckableNipples() && this.player.biggestTitSize() >= 5) {
            ...
            if (this.player.bRows() > 1) this.outputText("top row of breasts");
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.totalCocks() > 1) this.outputText("do not");
            else this.outputText("does not");
            this.outputText(" differ greatly from Valeria's other liquid attentions. ");
            if (this.player.totalCocks() > 1) this.outputText("They start");
            else this.outputText("It starts");
            ...
            if (this.player.balls == 0) this.outputText("something expanding as you cum, filling up with spunk or slime or some mixture of the two");
            else this.outputText("your [balls] expanding as you cum, filling up with spunk or slime or some mixture of the two, weighing down your sack with ever-increasing heaviness. Tender licks of tongue roll across the surface as you fill beyond completely");
            this.outputText(".");
        }
        //Butt fillinggasm
        ...
        if (this.player.biggestTitSize() >= 5 && this.player.hasFuckableNipples()) this.outputText(", pausing to handle your hefty, swelling tits");
        ...
        if (this.player.hasVagina()) this.outputText(", so gravid from both holes that a real mother would be shamed by your obscene \"pregnancy.\"");
        else this.outputText(".");
        ...
        if (this.player.hasCock()) {
            if (this.player.balls > 0) this.player.changeStatusValue(StatusAffects.GooStuffed, 2, 2);
            else this.player.changeStatusValue(StatusAffects.GooStuffed, 2, 1);
        }
        if (this.player.biggestTitSize() >= 5 && this.player.hasFuckableNipples()) this.player.changeStatusValue(StatusAffects.GooStuffed, 4, 1);
        this.cleanupAfterCombat();
    }
    ...
    public birthOutDatGooSlut(): void {
        ...
        if (this.player.statusAffectv4(StatusAffects.GooStuffed) > 0) this.outputText(" Your tits are even wobbling around wildly, shaking and jiggling obscenely inside your [armor] in a way that makes you your [nipples] more than a little leaky.");
        ...
        if (this.player.skinType == SKIN_TYPE_FUR) this.outputText("furry ");
        this.outputText("bitch in heat. Gods, there's so much!");
        if (this.player.statusAffectv4(StatusAffects.GooStuffed) > 0) {
            this.outputText("\n\n");
            if (!this.player.hasFuckableNipples()) this.outputText("Stiffening up");
            else this.outputText("Opening up");
            ...
            if (this.player.bRows() > 1) this.outputText(" and the next row down");
            if (this.player.bRows() > 2) this.outputText(", and so on");
        ...
        if (this.player.hasCock() && this.player.statusAffectv2(StatusAffects.GooStuffed) > 0) {
            ...
            if (this.player.balls > 0) this.outputText("ball-clenching");
            else this.outputText("body-clenching");
            ...
            if (this.player.statusAffectv2(StatusAffects.GooStuffed) >= 2) this.outputText("  Your balls keep their enhanced sized. Odd.");
        }
