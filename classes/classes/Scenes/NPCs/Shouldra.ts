...
export class Shouldra extends Monster {
    ...
    private shouldrattack(): void {
        ...
        //("Misdirection"
        if (this.player.findPerk(PerkLib.Misdirection) >= 0 && Shouldra.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
        ...
        if (damage <= 0) {
            ...
            if (Shouldra.rand(this.player.armorDef + this.player.tou) < this.player.armorDef) this.outputText("You absorb and deflect every " + this.weaponVerb + " with your " + this.player.armorName + ".", false);
        ...
        //everyone else
            ...
            //(regular attack 1)
            if (choice == 0) this.outputText("Ducking in close, the girl thunders a punch against your midsection, leaving a painful sting.", false);
            //(regular attack 2)
            else if (choice == 1) this.outputText("The girl feints a charge, leans back, and snaps a kick against your " + kGAMECLASS.hipDescript() + ". You stagger, correct your posture, and plunge back into combat.", false);
            //(regular attack 3)
            else if (choice == 2) this.outputText("You momentarily drop your guard as the girl appears to stumble. She rights herself as you step forward and lands a one-two combination against your torso.", false);
