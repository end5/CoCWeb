...
export class Clara extends Monster {
    ...
    protected claraDrugAttack(): void {
        ...
        //Dodge chance!
        if ((this.player.findPerk(PerkLib.Evade) >= 0 && Clara.rand(10) <= 3) || (Clara.rand(100) < this.player.spe / 5)) {
    ...
    protected performCombatAction(): void {
        if (this.player.findStatusAffect(StatusAffects.ClaraFoughtInCamp) >= 0 && this.player.statusAffectv1(StatusAffects.ClaraCombatRounds) >= 10) {
            this.HP = 0;
            this.combatRoundOver();
        }
        if (this.HP < 50 && Clara.rand(2) == 0) {
            ...
            var actions: any[] = [this.eAttack, this.claraDrugAttack, this.claraTeaseAttack, this.claraCastsBlind];
            var action: number = Clara.rand(actions.length);
            trace("ACTION SELECTED: " + action);
            actions[action]();
        }
        if (this.player.findStatusAffect(StatusAffects.ClaraCombatRounds) < 0) this.player.createStatusAffect(StatusAffects.ClaraCombatRounds, 1, 0, 0, 0);
        else this.player.addStatusValue(StatusAffects.ClaraCombatRounds, 1, 1);

        //Bonus damage if not in camp
        if (this.HP > 0 && this.lust < 100 && this.player.findStatusAffect(StatusAffects.ClaraFoughtInCamp) < 0) this.claraBonusBaseLustDamage();
    }
    public defeated(hpVictory: boolean): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.ClaraFoughtInCamp) >= 0 && this.player.statusAffectv1(StatusAffects.ClaraCombatRounds) >= 10) { }
            ...
            this.clearOutput();
            //PC wins via health
            if (this.HP <= 0) this.outputText("The pissed off cowgirl finally collapses to the ground.  She tries to stand up again, but finds that she can’t.  \"<i>Noo!</i>\" she cries out in frustration, \"<i>You were the perfect slave!  We were meant to be toogether!</i>\"\n\n");
            //PC wins via lust
            else this.outputText("The fury and anger finally give out to the overwhelming lust that you’ve help Clara feel.  She can’t fight anymore, and falls onto her backside.  She starts feeling herself up, and desperately asks you to fuck her.\n\n");
        }
