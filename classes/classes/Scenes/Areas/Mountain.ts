...
export class Mountain extends BaseContent {
    ...
    public exploreMountain(): void {
        ...
        if (this.flags[kFLAGS.PC_PROMISED_HEL_MONOGAMY_FUCKS] == 1 && this.flags[kFLAGS.HEL_RAPED_TODAY] == 0 && Mountain.rand(10) == 0 && this.player.gender > 0 && !kGAMECLASS.helScene.followerHel()) {
        ...
        if (this.isHolidays()) {
            //Gats xmas adventure!
            if (Mountain.rand(5) == 0 && this.player.gender > 0 && this.isHolidays() && this.flags[kFLAGS.GATS_ANGEL_DISABLED] == 0 && this.flags[kFLAGS.GATS_ANGEL_GOOD_ENDED] == 0 && (this.flags[kFLAGS.GATS_ANGEL_QUEST_BEGAN] > 0 && this.player.hasKeyItem("North Star Key") < 0)) {
                kGAMECLASS.gatsSpectacularRouter();
                return;
            }
            if (Mountain.rand(6) == 0 && this.flags[kFLAGS.JACK_FROST_YEAR] < this.date.fullYear && this.silly()) {
        ...
        if (Mountain.rand(100) <= 77) {
            if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00141] < 3) {
                ...
                if (this.player.faceType == FACE_DOG && (this.player.dogCocks() >= 2 || (this.player.hasVagina() && this.player.pregnancyType == PregnancyStore.PREGNANCY_HELL_HOUND)) && this.player.cor >= 60 && this.player.tailType == TAIL_TYPE_DOG && (this.player.lowerBody == LOWER_BODY_TYPE_DOG || this.player.hairColor == "midnight black")) {
                    ...
                    else if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00141] == 1 && this.player.hasKeyItem("Marae's Lethicite") >= 0 && this.player.keyItemv2("Marae's Lethicite") < 3) {
                        this.hellHoundScene.HellHoundMasterEncounter();
                        return;
                    }
                }
        ...
        if (!kGAMECLASS.ceraphFollowerScene.ceraphIsFollower() && this.player.level > 2 && (this.player.exploredMountain % 30 == 0) && this.flags[kFLAGS.PC_FETISH] > 0) {
            kGAMECLASS.ceraphScene.friendlyNeighborhoodSpiderManCeraph();
            return;
        }
        //15% chance of Ceraph
        if (!kGAMECLASS.ceraphFollowerScene.ceraphIsFollower() && this.player.level > 2 && (this.player.exploredMountain % 15 == 0) && this.flags[kFLAGS.PC_FETISH] != 1) {
            kGAMECLASS.ceraphScene.encounterCeraph();
            return;
        }
        //10% chance of hairdresser encounter if not found yet
        if (Mountain.rand(10) == 0 && this.player.findStatusAffect(StatusAffects.HairdresserMeeting) < 0) chooser = 4;
        if ((Mountain.rand(8) == 0 && this.player.findStatusAffect(StatusAffects.MetMarae) >= 0)
        ...
        if (this.player.exploredMountain % 16 == 0 && this.player.findPerk(PerkLib.MinotaurCumAddict) >= 0) {
        ...
        if (chooser == 1) {
            this.spriteSelect(44);
            if (this.player.findStatusAffect(StatusAffects.TF2) < 0 && this.player.level <= 1 && this.player.str <= 40) {
                if (this.silly()) {
            ...
            //Mino gangbang
            if (this.player.findStatusAffect(StatusAffects.MinoPlusCowgirl) < 0 || Mountain.rand(10) == 0) {
                if (this.flags[kFLAGS.HAS_SEEN_MINO_AND_COWGIRL] == 1 && this.player.horns > 0 && this.player.hornType == HORNS_COW_MINOTAUR && this.player.earType == EARS_COW && this.player.tailType == TAIL_TYPE_COW && this.player.lactationQ() >= 200 && this.player.biggestTitSize() >= 3 && this.player.minotaurAddicted()) {
                    ...
                    if (this.player.hasVagina()) this.outputText("dampens");
                    else this.outputText("twinges");
                    this.outputText(", aching to be filled");
                    if (this.player.hasCock()) this.outputText(", while [eachCock] rises to attention, straining at your [armor]");
                    this.outputText(".");
                ...
                if (this.player.findStatusAffect(StatusAffects.MinoPlusCowgirl) < 0) this.player.createStatusAffect(StatusAffects.MinoPlusCowgirl, 0, 0, 0, 0);
        ...
        if (chooser == 2) {
            ...
            if (this.player.findStatusAffect(StatusAffects.WormsOn) >= 0 && this.player.findStatusAffect(StatusAffects.Infested) < 0) {
                if (this.player.findStatusAffect(StatusAffects.WormsHalf) >= 0 && Mountain.rand(2) == 0) {
                ...
                if (this.player.findStatusAffect(StatusAffects.WormsOff) >= 0 || this.player.findStatusAffect(StatusAffects.Infested) >= 0 || (Mountain.rand(2) == 0 && this.player.findStatusAffect(StatusAffects.WormsHalf) >= 0)) {
                    if (this.player.cor < 90) {
                        this.outputText("Your hike in the mountains, while fruitless, reveals pleasant vistas and provides you with good exercise and relaxation.", true);
                        this.dynStats("tou", .25, "spe", .5, "lus", this.player.lib / 10 - 15);
                    }
    ...
    private joinBeingAMinoCumSlut(): void {
        ...
        if (this.player.tallness <= 96) this.outputText(" despite your immense height");
        ...
        if (this.player.hasVagina()) this.outputText("swirls around your clitoris");
        else this.outputText("presses against your perineum");
        this.outputText(".");
        ...
        if (this.player.hasCock()) this.outputText(", once again ignoring your manhood completely");
        this.outputText(", dives tongue first into your wet ");
        if (this.player.hasVagina()) this.outputText("quim");
        else this.outputText("back door");
        ...
        if (this.player.hasVagina()) this.outputText("sodden box");
        else this.outputText("moist butthole");
        ...
        if (this.player.hasVagina()) this.outputText("within your womb, knocked up by this manly beast");
        else this.outputText("within your bowels");
        ...
        if (this.player.hasVagina()) this.outputText("cow-pussy");
        ...
        if (!this.player.hasVagina()) this.outputText("new ");
        ...
        if (this.player.bRows() > 1) this.outputText("uppermost ");
