...
export class Salon extends BaseContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (this.model.time.hours > 23) {
            if (this.flags[kFLAGS.LYNNETTE_CARRYING_COUNT] == 0 || this.flags[kFLAGS.LYNNETTE_PREGNANCY_CYCLE] != 4) {
                this.flags[kFLAGS.LYNNETTE_PREGNANCY_CYCLE]++;
            }
            if (this.flags[kFLAGS.LYNNETTE_PREGNANCY_CYCLE] == 7) {
    ...
    public salonGreeting(): void {
        if (this.player.findStatusAffect(StatusAffects.HairdresserMeeting) >= 0) {
            ...
            this.player.createStatusAffect(StatusAffects.HairdresserMeeting, 0, 0, 0, 0);
    ...
    public salonPurchaseMenu(): void {
        ...
        if (this.player.hairLength >= 26) cutLong2 = this.cutLong;
        if (this.player.hairLength < this.player.tallness) lengthening = this.hairGrow;
        if (this.player.femininity < 100 && this.player.gender == 2) mudFacial2 = this.mudFacial;
        else if (this.player.femininity < 85 && (this.player.gender == 0 || this.player.gender == 3)) mudFacial2 = this.mudFacial;
        else if (this.player.femininity < 70 && this.player.gender == 1) mudFacial2 = this.mudFacial;
        else if (this.player.femininity < 100 && this.player.findPerk(PerkLib.Androgyny) >= 0) mudFacial2 = this.mudFacial;
        if (this.player.femininity > 0 && this.player.gender == 1) sandFacial2 = this.sandFacial;
        else if (this.player.femininity > 30 && this.player.gender == 2) sandFacial2 = this.sandFacial;
        else if (this.player.femininity > 20 && (this.player.gender == 0 || this.player.gender == 3)) sandFacial2 = this.sandFacial;
    ...
    private hairDresserRepeatGreeting(): void {
        ...
        if (this.flags[kFLAGS.LYNNETTE_PREGNANCY_CYCLE] == 4 && this.flags[kFLAGS.LYNNETTE_CARRYING_COUNT] > 0) {
            this.outputText("As soon as you enter the Salon, Lynnette is beaming ");
            if (this.player.tallness >= 48) this.outputText("up ");
            this.outputText("at you. \"<i>Hey there, honey. Come for some more service or to check up on the kids?</i>\" She pats her now-flat midsection. \"<i>I popped them out last night. We had " + Salon.num2Text(this.flags[kFLAGS.LYNNETTE_CARRYING_COUNT]) + " kids... all girls, of course.</i>\" She tucks a lock of beautifully styled hair behind an elfin ear. \"<i>Now you be sure and let me know when you want to contribute to my baby-bank again.</i>\"");
            if (this.flags[kFLAGS.LYNNETTE_BABY_COUNT] > 0) this.outputText("\n\nA few new, green faces can be seen looking your way and blushing as they work the gloryholes, probably from a previous litter. They grow up so fast.");
        ...
        else if (this.flags[kFLAGS.LYNNETTE_APPROVAL] >= 100 && this.flags[kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL] == 0) {
        ...
        //Favorite Greeting 
            ...
            //Nonpreggers
                ...
                if (this.player.tallness <= 48) this.outputText("side");
                else if (this.player.tallness <= 68) this.outputText("[hips]");
                else this.outputText("[leg]");
    ...
    private gloryholeMinotaur(): void {
        ...
        if (this.player.biggestTitSize() > 1) this.outputText("hold up ");
        else this.outputText("arch your back to expose ");
    ...
    private goblinHairDresserFacefuck(): void {
        ...
        if (this.player.cocks[0].cockThickness >= 4) this.outputText("around as much of you as she can", false);
        ...
        if (this.player.cumQ() >= 250) this.outputText(" She can't hold it all and suddenly starts swallowing as runnels of it escape her nose and a wave of it rolls down her lips.", false);
    ...
    private minotaurCumBukkakeInSalon(): void {
        ...
        if (this.player.hasVagina()) this.outputText(" and puffing up your twat with liquid arousal.\n\n", false);
        else if (this.player.hasCock()) this.outputText(" and turning " + this.sMultiCockDesc() + " into a turgid, pulsating mass.\n\n", false);
        ...
        if (this.player.hasVagina() || this.player.hasCock()) {
            ...
            if (this.player.hasCock()) {
                ...
                else if (this.player.cumQ() < 500) this.outputText("pre-cum a few inches wide ", false);
        ...
        if (this.player.gender > 0) this.outputText(" and increasing the size of the puddle you're leaving on the floor", false);
        ...
        if (this.player.biggestTitSize() >= 2) {
            ...
            if (this.player.biggestTitSize() < 6) this.outputText("Even so, you can't quite get your breasts the entire way around him, so you make up for it by pressing it harder into you with your busy hands.  ", false);
        ...
        if (this.player.biggestTitSize() >= 2) this.outputText("The sudden girth change sends an enticing ripple through your " + this.allBreastsDescript() + " that's pleasurable enough to make you moan into his steadily-widening urethra.  ", false);
        ...
        if (this.player.hasVagina()) this.outputText(" while the animal part of your brain hopes it makes you pregnant with an equally girthy son", false);
        this.outputText(".\n\n", false);
        //ADD PREG CHECK
        //Preggers chance!
        this.player.knockUp(PregnancyStore.PREGNANCY_MINOTAUR, PregnancyStore.INCUBATION_MINOTAUR, 70);
        ...
        else if (this.player.analCapacity() < 140) this.outputText("delighting in the feeling of perfect fullness.", false);
        else this.outputText("delighting in realizing that you could take far larger than even this virile specimen!", false);
        //(buttchange here: 90)
        ...
        if (!this.player.hasCock()) this.outputText("drags through your body, touching sensitive nerves you didn't even know you had until you cum, shuddering and shaking like a wanton whore.", false);
        else this.outputText("presses on your prostate as it squeezes by, making " + this.sMultiCockDesc() + " drip and spurt freely until you can bear it no longer and cum, shaking and shuddering like a wanton whore.  Jizz drips and pours from " + this.sMultiCockDesc() + " in a steady stream that pools on the floor, slowly rolling towards a drain that doubtless empties into a tank or greedy goblin cunt.", false);
        this.outputText("\n\n", false);
    ...
    private minotaurSalonFollowUp(): void {
        ...
        else this.outputText("You're woken up with a slap to the face, and still muzzy from your cum-induced glory-hole orgy, you stagger up to your " + this.player.feet() + ".  Lynnette shakes her stinging palm and apologizes, \"<i>Sorry I have to keep doing that, but you're damned hard to wake after you go on these binges!  Anyways, you've earned some hair treatment if you want it.  Of course you could always buy more of your favorite fluid...</i>\"\n\n", false);
        //Menu
        this.outputText("Lynnette offers and explains their options, \"<i>So what'll it be hun?  We could cut it down or give you a lengthening treatment. Or you can get a hair-dye to use on your own.  Just remember to come back in a few days for a touchup.</i>\"", false);
        this.salonPurchaseMenu();
    }
    ...
    */
    ...
    private fuckLynnette(): void {
        ...
        if (this.flags[kFLAGS.LYNNETTE_FUCK_COUNTER] == 0) {
            ...
            if (this.player.tallness >= 72) this.outputText("[leg]");
            else if (this.player.tallness >= 55) this.outputText("[hips]");
            else this.outputText("side");
            this.outputText(" so that she can slip a manicured hand inside your underclothes. She handles [eachCock] ");
            if (this.player.cockTotal() > 1) this.outputText("in turn ");
        ...
        //Repeat cum within 24 hours check
            ...
            if (this.player.lust <= 75) this.outputText("it fills to full size almost immediately");
            else this.outputText("it's fully-erect state makes itself known");
            this.outputText(". Lynnette strokes a few times to make sure you're suitably engorged and then shifts lower, squeezing ");
            if (this.player.balls == 0) this.outputText("around your [sheath] to get a feel for how pent-up you are");
            else this.outputText("gently at your [balls], weighing them in her palm to get a feel for just how pent-up you are");
            this.outputText(".");
            //NOT ENOUGH!
            if (this.player.hoursSinceCum < 24) {
        ...
        if (this.player.cockTotal() > 1) this.outputText("es have");
        ...
        if (this.player.cockArea(x) > 50) {
            ...
            if (this.player.cockArea(x) <= 80) this.outputText("her belly.");
            else this.outputText("into her ribcage, penetrating farther than any lesser race could ever hope to handle.");
        }
        ...
        if (this.player.cockArea(x) <= 90) {
            ...
            if (this.player.balls > 0) this.outputText(", [balls] resting on her soaked, juicy ass-cheeks.");
        }
        else {
            ...
            if (this.player.balls > 0) this.outputText("  Her feet wrap around your [sack] to squeeze your [balls] affectionately, compensating quite nicely for her own shortcomings.");
        }
        ...
        if (this.player.balls > 0) this.outputText(", making your sack feel tight and your testes bloated and tender.");
        else this.outputText(", making your gut clench and spasm with near-orgasmic contractions.");
        ...
        if (this.player.cockArea(x) > 24) this.outputText("up against her slightly-yielding cervix");
        else this.outputText("in as close to the cervix as you possibly can");
        ...
        if (this.player.cockArea(x) <= 90) this.outputText("completely inside once more");
        else this.outputText("as deeply inside as you can");
        ...
        if (this.player.cockTotal() > 1) {
            ...
            if (this.player.longestCockLength() <= 12) this.outputText("on the greedy green slut's belly");
            else this.outputText("in the milk-stained valley between her tits, absolutely buried in squishy-soft breast");
        ...
        if (this.player.cumQ() >= 3000) {
            ...
            if (this.player.cumQ() >= 10000) this.outputText("most");
            else this.outputText("some");
        ...
        if (this.player.hasKnot(x)) this.outputText(" with a wet-sounding 'pop'.  Your knot does a terrific job of gaping her lips during its exit, and you cannot help but marvel at how well you've broken this bitch in.  A shame she'll probably tighten up again in short order");
    ...
    public lynnetteApproval(arg: number = 0): number {
        ...
        if (this.flags[kFLAGS.LYNNETTE_APPROVAL] < -100) this.flags[kFLAGS.LYNNETTE_APPROVAL] = -100;
        else if (this.flags[kFLAGS.LYNNETTE_APPROVAL] > 100) this.flags[kFLAGS.LYNNETTE_APPROVAL] = 100;
        return this.flags[kFLAGS.LYNNETTE_APPROVAL];
    }
}
