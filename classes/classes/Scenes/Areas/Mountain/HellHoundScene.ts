...
export class HellHoundScene extends BaseContent {
    ...
    public hellhoundEncounter(): void {
        ...
        this.startCombat(new HellHound());
    ...
    public hellhoundRapesPlayer(): void {
        ...
        if (this.player.lust >= 100) this.outputText("Overcome by sexual desire, you submit to the hungry hellhound.\n\n", true);
        else this.outputText("Too weak to continue fighting, you fall to your knees.\n\n", true);
        ...
        if (this.player.lowerBody == LOWER_BODY_TYPE_CENTAUR) this.outputText("hocks and knees", false);
        ...
        if (this.player.vaginas.length > 0 && this.player.lowerBody != LOWER_BODY_TYPE_NAGA) this.outputText("slamming his twin dicks into your " + this.vaginaDescript(0) + " and your " + this.assholeDescript() + ".  ", false);
        else this.outputText("slamming his lower dick into your " + this.assholeDescript() + ", while rubbing the other between the cheeks of your " + this.buttDescript() + ".  ", false);
        if (this.player.vaginas.length > 0 && this.player.lowerBody != LOWER_BODY_TYPE_NAGA) {
            if (this.player.vaginalCapacity() < this.monster.cockArea(0)) this.outputText("You feel an intense mixture of sensations in your lower body as your " + this.assholeDescript() + " feels like it is being torn apart, while your " + this.vaginaDescript(0) + " is filled with an intense pleasure at being filled with the creature's large member.  ", false);
            else this.outputText("Your lower body explodes with pain as the hellhound forces himself in too quickly for either your " + this.assholeDescript() + " or your " + this.vaginaDescript(0) + " to handle.  ", false);
            if (this.player.vaginas[0].virgin) this.outputText("<b>You are no longer a virgin!  </b>", false);
            if (this.player.cuntChange(this.monster.cockArea(0), false)) this.outputText("The beast howls as your " + this.vaginaDescript(0) + " is stretched to accommodate the large shaft.  ", false);
            this.outputText("The hellhound pants for a few seconds before continuing.  ", false);
            if (this.player.vaginas[0].vaginalWetness < VAGINA_WETNESS_WET) this.outputText("The pause gives your " + this.vaginaDescript(0) + " time to moisten, ensuring the next thrust won't be quite as rough.  ", false);
            ...
            if (this.player.cor < 40) this.outputText("You moan while trying to break free to get the heat off your back... but the beast keeps a firm grip on you as more and more of its fiery seed is pumped into and onto your body. After what feels like an eternity, the beast pulls out of you. He gives your " + this.assholeDescript() + " a single extended lick with his long dog-like tongue before running off out of sight. You are finally able to roll around and get the heat off of your back, but you can do nothing about the tainted warmth inside you. The heat proves to be too much and you pass out. After some time passes, you wake up to find the fire inside you has thankfully faded away. You're able to stand up again, but the damage is done and the creature's seed has left you feeling rather weak.", false);
    ...
    public hellHoundPropahRape(): void {
        ...
        if (this.monster.HP < 1) this.outputText("spring to life, extending rapidly from the sheath. Tentatively you give one of them a gentle lick, being rewarded with a drop of pre-cum.\n\n", false);
        else this.outputText("still manage to leak plenty of hot, steamy pre-cum all over his belly. Tentatively you give one of them a gentle lick, being rewarded with a dollop of the stuff.\n\n", false);
        //--- IF CORRUPTION < 20 ---
        ...
        //--- ELSE (CORRUPTION >= 20) ---
            ...
            if (this.player.biggestTitSize() > 1) this.outputText("  Your " + this.chestDesc() + " jiggle in exquisite ways along with your bumping, grinding motions.", false);
            ...
            if (this.player.hasCock()) {
                ...
                //--- IF ONE PENIS ---
                if (this.player.totalCocks() == 1) this.outputText("  Soon your own " + this.cockDescript(0) + " reacts, quickly growing into a full throbbing erection. You leave your member alone, and it keeps flapping against the hellhound's belly while you ride him.", false);
                //--- ELSE ---
                else this.outputText("  Soon " + this.sMultiCockDesc() + " reacts, quickly growing into a throbbing erection.  You leave your members alone, and they flap against the hellhound's belly while you ride him.", false);
            }
            //--- IF PC HAS BALLS ---
            if (this.player.balls > 0) this.outputText("  Whenever you lower your hips to your mates belly, your " + this.ballsDescriptLight() + " touches his. The magical flames fill your scrotum with unnatural heat but no pain. You literally feel the cum in your balls boiling upwards, building up immense pressure.", false);
            this.outputText("\n\n", false);
            ...
            if (this.player.totalCocks() > 0) {
                // --- IF ONE PENIS ---
                if (this.player.cockTotal() == 1) this.outputText("Your " + this.cockDescript(0) + " twitches as it ejects its sticky load.", false);
                //--- ELSE ---
                else this.outputText("Your " + this.multiCockDescriptLight() + " twitch as they eject their sticky loads.", false);
