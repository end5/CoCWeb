   ...
   public minoVictoryRapeChoices(): void {
        ...
        if (this.player.hasCock()) {
            ...
            while (counter < this.player.cockTotal()) {
                if (this.player.cocks[counter].cockThickness <= 4 && this.player.cocks[counter].cockLength >= 14) urethralPen = this.minoUrethralPen;
                counter++;
            }
        }
        if (this.player.hasCock()) {
            if (this.player.cockThatFits(80) >= 0) bj = this.minotaurBlowjob;
        }
        if (this.player.hasCock() && x >= 0) dickRape = this.bumRapeaMinotaur;
        if (x >= 0 && this.player.cockTotal() > 1 && this.player.tentacleCocks() > 0) tentaRape = this.rapeMinotaurTentacles;
        if (this.player.hasVagina()) cuntRape = this.girlRapeAMinotaur;
        //Centaurs can't do the herm scene
        if (this.player.gender == 3 && x >= 0 && !this.player.isTaur()) hermRape = this.minotaurGetsRapedByHerms;
        ...
        if ((temp2 == undefined || MinotaurScene.rand(2) == 0) && this.player.hasVagina() && this.player.biggestTitSize() >= 4 && this.player.armor instanceof LustyMaidensArmor) {
            feedposit = "B.Titfuck";
        ...
        if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] >= 1 || this.player.findPerk(PerkLib.MinotaurCumAddict) >= 0) {
        ...
        //Not an addict
            ...
            if (this.monster.lust > 99) this.outputText("You smile in satisfaction as the " + this.monster.short + " drops down on all fours and begins masturbating feverishly.  Sadly you realize your own needs have not been met.  Of course you could always fuck the eager bull...\n\nWhat do you do?", true);
            else this.outputText("You smile in satisfaction as the " + this.monster.short + " collapses, unable to continue fighting.  Sadly you realize your own needs have not been met.  Of course you could always rape the poor thing...\n\nWhat do you do?", true);
        }
        //Not able to rape but a feeder
        else if (this.player.findStatusAffect(StatusAffects.Feeder) >= 0 && feedposit == "B. Feed") {
            if (this.monster.lust > 99) this.outputText("You smile in satisfaction as the " + this.monster.short + " collapses, unable to continue fighting.  Sadly you realize your own need to breastfeed has not been met.  You could always let the poor thing have a drink...\n\nDo you?", true);
            else this.outputText("You smile in satisfaction as the " + this.monster.short + " collapses, unable to continue fighting.  Sadly you realize your own need to breastfeed has not been met.  You could always let the poor thing have a drink...\n\nWhat do you do?", true);
        }
        //No rape, no feeder
        else {
            if (this.monster.lust > 99) this.outputText("You smile in satisfaction as the " + this.monster.short + " drops down on all fours and begins masturbating feverishly.", true);
            else this.outputText("You smile in satisfaction as the " + this.monster.short + " collapses, unable to continue fighting.", true);
    ...
    private rapeMinotaurTentacles(): void {
        ...
        if (this.monster.HP <= 0) this.outputText("The minotaur collapses backwards after your last blow. Defeated and tired, he can barely stand up as you approach.  ", false);
        else {
            this.outputText("The minotaur drops his arm", false);
            if (this.monster.weaponName == "axe") this.outputText(", letting his axe clatter to the floor", false);
        ...
        if (this.player.cor < 25) this.outputText("You cry, petting his hair, smoothing it across his horns, justifying to yourself that you are raping this beast because this is how this world works as you maneuver your " + this.multiCockDescript() + " towards his tailhole.  ", false);
        if (this.player.cor >= 25 && this.player.cor < 50) this.outputText("You smoothly guide your " + this.multiCockDescript() + " toward his tailhole.  ", false);
        if (this.player.cor >= 50 && this.player.cor < 80) this.outputText("You roughly grab him, slamming him close to your " + this.multiCockDescript() + " - just how this beast likes it.  ", false);
        if (this.player.cor >= 80) this.outputText("You grin and roughly pull his horns towards your " + this.multiCockDescript() + ", ready to roughly ride this beast.  ", false);
        //Mixed dix
        if (this.player.cockTotal() != this.player.tentacleCocks()) {
            ...
            while (this.temp > 0 && temp2 == -1) {
                this.temp--;
                if (this.player.cocks[this.temp].cockType != CockTypesEnum.TENTACLE) temp2 = this.temp;
            }
            //failsafe
            if (temp2 == -1) temp2 = 0;
            ...
            while (this.temp > 0) {
                ...
                if (this.player.cocks[this.temp].cockLength > this.player.cocks[temp2].cockLength && this.player.cocks[this.temp].cockType != CockTypesEnum.TENTACLE) {
                    temp2 = this.temp;
                }
            }
        ...
        //Multivaginas...wtf
        if (this.player.vaginas.length > 1 && !mismatched && this.player.cocks.length > 2) this.outputText("Your tentacle cocks writhe upon themselves, each turning to an unattended " + this.vaginaDescript(1) + " on your body, and quickly filling up the cavernous depths. You groan in ecstasy at the self-fucking you are receiving as all your pussies start leaking their wonderful fluids.  ", false);
        //more dicks than available holes
        if ((this.player.tentacleCocks() == 2 || this.player.tentacleCocks() == 3)) this.outputText("Your next tentacle dick, saddened by not having anything to writhe around, turns to the minotaurs protruding member. It darts out from your crotch, wrapping around his monstrous meat and constricting, like a snake might around its prey. Your tongue lolls out as you feel that cock twist and grab his dick, pulsing with both your blood and the beast's twitching member.  ", false);
        if (this.player.tentacleCocks() > 3) this.outputText("Your remaining tentacle dicks wave around the minotaur, tending to his balls, weaving around his limbs, and generally rubbing and throbbing all over him, spreading pre-cum around and through him, leaving both of you moaning in pleasure.  ", false);
        //Cum
    ...
    private rapeMinotaurTentacle(): void {
        ...
        if (this.player.vaginas.length > 0) {
            ...
            if (this.player.cumQ() < 50) this.outputText("Your cock made quite a mess of things, and when you pull him out, your " + this.vaginaDescript(0) + " makes a wet sucking sound.  ", false);
            if (this.player.cumQ() >= 50 && this.player.cumQ() < 400) this.outputText("Your cock made quite a mess of things inside your " + this.vaginaDescript(0) + ".  As you pull free, cum drips freely down your thighs.  The heavenly scent of minotaur spunk fills your nose, and begin scooping the mixed jizz up to sample.  ", false);
            if (this.player.cumQ() >= 400) this.outputText("Your cock made quite a mess of things inside your " + this.vaginaDescript(0) + ".  As you pull free, a river of cum pours from betwixt your thighs.  The heavenly scent of minotaur spunk fills your nose, and you begin scooping the mixed jizz up to sample.  ", false);
        }
        //Free ass!
        else {
            this.temp = 2;
    ...
    private AddictNagaOnMinotaur(): void {
        ...
        this.outputText("", true);
        //[if(monster.lust >= 99)
        if (this.monster.lust > 99) this.outputText("You slither towards the fallen monster, wishing to taste his addictive semen.  He's currently engaged in stroking his enormous shaft up and down with both of his hands, but offers little resistance when you push him over to a sitting position.  You pull his hands away and replace them with yours, running your fingers up and down along his thickness, working yourself up with anticipation.  He leans back and exhales loudly, clearly appreciative.\n\n", false);
        else this.outputText("You slither towards the fallen monster, wishing to taste his addictive semen.  You notice that, despite the beating you just gave him, his huge cock is fully erect.  Delighted, you gently run both of your hands up and down its long shaft.  He exhales loudly, signaling his appreciation.\n\n", false);
        ...
        if (this.player.hasCock()) this.outputText("furiously stroke your own " + this.multiCockDescriptLight() + ". Your muffled vocalizations of pleaure weakly join his own.\n\n", false);
        else this.outputText("part the folds of your " + this.vaginaDescript(0) + ", sliding it inside and masturbating with a furious vigor. Your muffled vocalizations of pleaure weakly join his own.\n\n", false);
    ...
    private minoRapeIntro(): void {
        ...
        //Monster lost to lust
            ...
            //Vagina or not flavor texts
            if (this.player.vaginas.length > 0) this.outputText("  Glancing back at you with hope in its eyes, the minotaur's " + this.monster.cockDescriptShort(0) + " twitches as he lays his gaze upon your " + this.vaginaDescript(0) + ".", false);
            else this.outputText("  Glancing back at you with eyes full of despair, the minotaur doubts you'll help relieve the predicament his " + this.monster.ballsDescriptLight() + " are in.", false);
        }
    }
    ...
    private bumRapeaMinotaur(): void {
        ...
        if (this.player.cumQ() < 25) this.outputText("dribbling a bit of slick pre-cum into his fur.  ", false);
        if (this.player.cumQ() >= 25 && this.player.cumQ() < 250) this.outputText("leaking thin ropes of pre-cum over his muscular ass.  ", false);
    ...
    private girlRapeAMinotaur(): void {
        ...
        if (this.player.lowerBody == LOWER_BODY_TYPE_CENTAUR) this.outputText("hindquarters, forcefully grinding your vagina along his length like a toy.  ", false);
        else this.outputText(this.hipDescript() + ", forcefully grinding up and down along his length like a cheap sex-toy.  ", false);
        //Size variants
        //Too small
        if (this.player.vaginalCapacity() < this.monster.cockArea(0)) this.outputText("Every ridge of his " + this.monster.cockDescriptShort(0) + " presses tightly against your walls, smearing more of his wondrous pre-seed into your body with every stroke.  ", false);
        //Just right
        if (this.player.vaginalCapacity() >= this.monster.cockArea(0) && this.player.vaginalCapacity() * 1.5 < this.monster.cockArea(0)) this.outputText("Every stroke of his " + this.monster.cockDescriptShort(0) + " deposits a bit more of his wondrous preseed into your body, spreading a relaxing warmth throughout your groin.  ", false);
        //Too big
        if (this.player.vaginalCapacity() * 1.5 >= this.monster.cockArea(0)) this.outputText("Every stroke of his " + this.monster.cockDescriptShort(0) + " slides easily into your " + this.vaginaDescript(0) + ", allowing him to slowly build up a puddle of his wondrous druggy pre-seed into your aching womb.  ", false);
        //Resume!
    ...
    private minotaurGetsRapedByHerms(): void {
        ...
        this.player.cuntChange(this.monster.cockArea(0), true);
        if (this.player.vaginas[0].vaginalLooseness < VAGINA_LOOSENESS_GAPING) this.outputText("A small suppressed part of your mind dimly registers that this should be fairly painful, but the lust and drug-like pre-cum wash that thought away with waves of pleasure.  ", false);
        if (this.player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS_GAPING) this.outputText("The intense feelings seem to grow more and more powerful as he stays inside you, his drug-like pre-cum making your " + this.vaginaDescript(0) + " tingle from the inside-out.", false);
    ...
    public minoPheromones(): void {
        ...
        if (this.player.findPerk(PerkLib.MinotaurCumAddict) >= 0 || this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] == 2) {
            if (MinotaurScene.rand(2) == 0) this.outputText("\n<b>You shiver with need, wanting nothing more than to bury your face under that loincloth and slurp out every drop of goopey goodness.</b>", false);
    ...
    public getRapedByMinotaur(autoRape: boolean = false): void {
        ...
        if (!autoRape) {
            ...
            //Oral rape chance
            if (MinotaurScene.rand(2) == 0 && this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) {
        ...
        if (this.player.vaginas.length > 0) {
            ...
            if (this.player.averageVaginalWetness() < 2) this.outputText("You aren't very wet, and fear the damage this beast will inflict on your " + this.vaginaDescript(0) + ".  ", false);
            else this.outputText("You're shamefully wet by this point, and your knees are ready to buckle.  ", false);
            this.minoCumAddiction(5);
        }
        //BUTTSECKS!
        else {
        ...
        //Vag stretch texts
        if (this.player.hasVagina()) this.player.cuntChange((24 * 3), true, false, true);
        ...
        if (this.player.biggestTitSize() > 0 && this.player.mostBreastsPerRow() > 1 && this.player.breastRows.length > 0) {
            ...
            if (this.player.biggestLactation() > 1.5) this.outputText("He then gives a grunt in what you take to be approval as your milk begins to squirt out. He licks a milk-coated finger in satisfaction.  ", false);
        }
        if (this.player.cockTotal() == 1) this.outputText("The bull-man bends forward a little, and grabs your " + this.cockDescript(0) + " in a crushing grip. He makes short jerking motions as he keeps thrusting into you.", false);
        if (this.player.cockTotal() > 1) this.outputText("The bull-man bends forward a little, and grabs one of your " + this.cockDescript(0) + "s in a crushing grip. He makes short jerking motions as he keeps thrusting into you.", false);
        if (this.player.vaginas.length > 0) {
        ...
        if (this.player.vaginas.length > 0) {
            if (this.player.averageVaginalWetness() >= 2) {
                if (this.player.averageVaginalWetness() < 4) this.outputText("You squirm and quiver, orgasming from the beast's rough usage.  ", false);
                if (this.player.averageVaginalWetness() == 4) this.outputText("You squirm and quiver, orgasming from the beast's rough usage, soaking him with your " + this.vaginaDescript(0) + ".  ", false);
                if (this.player.averageVaginalWetness() == 5) this.outputText("You orgasm on his massive rod, splattering the beast with girlcum.  ", false);
                this.player.orgasm();
        ...
        if (this.player.cockTotal() > 0 && (this.player.sens + MinotaurScene.rand(40) > 50)) {
            ...
            if (this.player.cumQ() < 25) this.outputText("spurting your seed helplessly from the brutal rape.  ", false);
            if (this.player.cumQ() >= 25 && this.player.cumQ() < 250) this.outputText("squirting thick ropes of cum over the cave.  ", false);
            if (this.player.cumQ() >= 250 && this.player.cumQ() < 500) this.outputText("erupting potent ropes of seed in thick bursts, splattering the cave walls and floors.  ", false);
            if (this.player.cumQ() >= 500) this.outputText("erupting a thick torrent of seed that seems to go on forever, spurred by the constant pressure of the huge minotaur cock.  You paint the cave wall with cum, the beast egging you on as it roughly jacks your " + this.cockDescript(0) + ".  You are vaguely aware of your orgasm dragging on and on, until eventually your orgasm stops, leaving a sizable puddle of cum on the floor.  ", false);
            this.player.orgasm();
            this.dynStats("lib", .5, "sen", 1, "cor", 1);
        }
    ...
    private getOralRapedByMinotaur(): void {
        ...
        if (this.player.hasBreasts()) this.outputText("  The bull-man grabs your " + this.biggestBreastSizeDescript() + ", squeezing and mauling them with his calloused hand.", false);
        ...
        if (this.player.hasBreasts()) this.outputText(", painfully grinding your " + this.nippleDescript(0) + " against his wiry fur", false);
        if (this.player.balls > 0) this.outputText(", and you can feel the minotaur's hot breath on your " + this.ballsDescriptLight(), false);
    ...
    private minoGetsTitFucked(): void {
        ...
        if (this.player.totalCocks() > 0) this.outputText("  Your " + this.cockDescript(0) + " grows harder and more aroused than ever before while your ", false);
        else this.outputText("  Your body aches with arousal while your ", false);
        if (this.player.hasVagina()) this.outputText(this.vaginaDescript(0) + " grows hotter and hotter, begging to be taken.", false);
        else this.outputText(this.assholeDescript() + " puckers slightly, as if begging to be taken by another monstrous beast.", false);
        this.outputText("\n\n", false);
    ...
    private takeMinoCumDirectly(): void {
        ...
        if (this.monster.HP < 1) this.outputText("The beast-man has fallen on his back, comatose and insensate, though his shaft has risen to full erectness, displacing his loincloth.", false);
        else this.outputText("The beast-man is on his back with his legs splayed, masturbating hopelessly.  His anguished moans tell you all you need to know – he'll never get off alone.", false);
        ...
        if (this.player.sens < 80) this.dynStats("sen", 1);
        if (this.player.sens < 60) this.dynStats("sen", 1);
        if (this.player.sens < 40) this.dynStats("sen", 1);
        if (this.player.sens < 20) this.dynStats("sen", 1);
        //+15 addiction
        this.minoCumAddiction(15);
        this.cleanupAfterCombat();
    }
    ...
    public minoCumUpdate(): boolean {
        ...
        if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] == 0 && this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_TRACKER] >= 50) {
        ...
        if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_TRACKER] < 50) {
            ...
            if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] > 0) {
                ...
                if (this.player.cor > 60) this.outputText("but miss the tingling apathy and fuzziness that filled your head every time one of them was squirting inside you.", false);
                else this.outputText("and resolve to give the beasts a wide berth from now on.", false);
                this.outputText("</b>\n", false);
        ...
        //If over 50, addicted, become addicted, withdrawl, etc
            ...
            if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] > 1 && this.flags[kFLAGS.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] < 24) {
                this.outputText("<b>\nYou sigh happily before you return to camp.  You got just what you needed.</b>\n", false);
                this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] = 1;
                output = true;
            }
            //Go into 'need' if its time.
            if (this.flags[kFLAGS.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] >= 24 && this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] == 1) {
                this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] = 2;
                output = true;
                this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00330] = 12;
            ...
            if (this.flags[kFLAGS.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] >= 48 && this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] == 2) {
                this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] = 3;
                output = true;
                this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00330] = 12;
            ...
            if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_TRACKER] >= 100) {
                if (this.player.findPerk(PerkLib.MinotaurCumAddict) >= 0) { }
                else {
                    this.player.createPerk(PerkLib.MinotaurCumAddict, 0, 0, 0, 0);
            ...
            if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] == 3 && this.flags[kFLAGS.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] >= 48) {
                this.dynStats("lus", 2);
            }
            if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] == 2 && this.flags[kFLAGS.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] >= 48) {
                this.dynStats("lus", 1);
            }
        }

        //Decrement addiction value as needed
        if (this.flags[kFLAGS.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] >= 48 && this.player.findPerk(PerkLib.MinotaurCumAddict) < 0) {
            if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_TRACKER] >= 0.15) this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_TRACKER] -= 0.15;
        }
        //Increment time since last imbibing cum...
        //used for withdrawal and reducing overall addiction.
        this.flags[kFLAGS.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM]++;
        //Minotaur cum item's status affect decrement
        if (this.flags[kFLAGS.MINOTAUR_CUM_REALLY_ADDICTED_STATE] > 0) {
            this.flags[kFLAGS.MINOTAUR_CUM_REALLY_ADDICTED_STATE]--;
            if (this.flags[kFLAGS.MINOTAUR_CUM_REALLY_ADDICTED_STATE] == 0) {
                this.outputText("\n<b>Pain and pleasure are no longer so indistinguishable to you.\n</b>", false);
                output = true;
            }
            if (this.flags[kFLAGS.MINOTAUR_CUM_REALLY_ADDICTED_STATE] < 0) this.flags[kFLAGS.MINOTAUR_CUM_REALLY_ADDICTED_STATE] = 0;
        }
        return output;
    }
    ...
    private minotaurBlowjob(): void {
        ...
        if (!this.player.isTaur()) this.outputText("peeling away your " + this.player.armorName + " and ", false);
        ...
        this.outputText(", you guide its mouth closer to your member as if on autopilot.  \"<i>Let's see just how good you are at milking,</i>\" you blurt; the minotaur moos in defiance but is soon silenced when the head of your " + Appearance.cockNoun(this.player.cocks[x].cockType) + " presses into its bovine mouth.\n\n", false);
        ...
        else if (this.player.findPerk(PerkLib.ArousingAura) >= 0) this.outputText("the power of your corruption", false);
        ...
        if (!this.player.isTaur()) this.outputText("finding your hands leaving his horns for the back of his head ", false);
    ...
    private layEggsInAMinotaurSpiderLike(): void {
        ...
        if (!this.silly()) this.outputText("his bovine rosebud.");
        else this.outputText("the entrance to his O.K. Corral.");
        ...
        if (this.player.gender > 0) {
            this.outputText("\n\n");
            if (this.player.hasCock()) this.outputText("Forcing [oneCock] between his bound thighs, you frot his flared bull-cock, your mixed pre soaking into the fur of his belly as you thrust against him.");
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.cumQ() < 100) this.outputText("spattering");
            else if (this.player.cumQ() < 500) this.outputText("drenching");
            else this.outputText("nearly drowning");
            this.outputText(" your helpless prey.  ");
        }
        if (this.player.hasVagina()) this.outputText("Your femcum sprays across his cock.  ");
        ...
        if (this.player.gender > 0) this.outputText("combination of the scent of your fluids and the ");
    ...
    private layBeeEggsInCowbutt(): void {
        ...
        if (this.monster.lust > 99) this.outputText(", stroking his flared cock desperately");
        ...
        if (!this.player.isTaur()) this.outputText("  Wrapping his tail around one hand, you lift his furred flanks up");
        else this.outputText("  Straddling him with all four legs, you slide forward");
        ...
        if (this.player.cor < 50) this.outputText("You hesitate, feeling a surge of pity for the once-mighty creature, but your new instincts drive your egg-layer further into his prone body.  ");
        else this.outputText("Grinning cruelly, you grasp him by the horns and haul back with both hands until the minotaur is forced to look you in the eye.  You lean in close to his snout and buzz mockingly, \"<i>Whoz the bitch now?  Iz it you?  Yezz, it izz!</i>\"  ");
    ...
    private getMinoHawtDawged(): void {
        ...
        if (this.player.hasCock()) this.outputText("[eachCock] twitching beneath you, painting the floor with dribbles of your pre.");
        if (this.player.gender == 3) this.outputText("  You also feel ");
        if (this.player.hasVagina()) this.outputText("your [vagina] clenching and unclenching as your entire hips try to push against the Minotaur's searching tongue, dripping with his spit - along with your own juices.");
