...
export class InfestedHellhoundScene extends BaseContent {
    ...
    public infestedHellhoundEncounter(): void {
        ...
        if (this.player.lowerBody == LOWER_BODY_TYPE_GOO) this.outputText("making your jello-like body jiggle", false);
    ...
    public infestedHellhoundLossRape(): void {
        this.outputText("", true);
        //[BOTH INFESTED]
        if (this.player.totalCocks() > 0 && this.player.findStatusAffect(StatusAffects.Infested) >= 0) {
            //(LUST)
            if (this.player.lust > 99) {
                ...
                else if (this.player.cor < 66) this.outputText("In spite of your better sense ", false);
        ...
        //[PLAYER'S COCKS ARE BIG ENOUGH TO BE INFECTED]
        else if (this.player.findStatusAffect(StatusAffects.Infested) < 0 && this.player.biggestCockArea() >= 40 && this.player.hasCock()) {
            //(LUST)
            if (this.player.lust > 99) {
                this.outputText("No amount of shame from the act of submitting to such a beast can overpower the furnace of lust raging in your loins.  ", false);
                if (this.player.cor < 33) this.outputText("In spite of your revulsion ", false);
            ...
            if (this.player.totalCocks() > 1) {
                ...
                if (this.player.balls > 0) this.outputText(this.ballsDescriptLight() + " shiver as hot doggie-cum and wiggling worms are pumped directly into them.  ", false);
                else this.outputText("body feels uncomfortably full as hot doggie-cum and wriggling worms are pumped directly into your prostate.  ", false);
        ...
        //[HAS PUSSY AND NO DICK BIG ENOUGH TO BE INFECTED]
            ...
            if (this.player.cor < 66) this.outputText("You beg, \"<i>Please, no, no!  I don't want them... in me!</i>\"\n\n", false);
            else this.outputText("You start to beg it not to put them inside you, but your protests drop off when you realize how good they could feel squirming inside you.\n\n", false);
            ...
            if (this.player.findStatusAffect(StatusAffects.WormPlugged) >= 0)
                ...
                this.player.createStatusAffect(StatusAffects.WormPlugged, 1 + InfestedHellhoundScene.rand(5), 0, 0, 0);
            this.player.knockUpForce(PregnancyStore.PREGNANCY_WORM_STUFFED, 100 + this.player.statusAffectv1(StatusAffects.WormPlugged)); //Will be cleared when the WormPlugged effect ends
            this.player.orgasm();
        ...
        //[GENDERLESS OR MALE WITH DICK TOO SMALL]
            ...
            this.outputText("The hell-hound rises up, looking nearly humanoid in shape for a moment before it drops down, planting its paws on your shoulders.  ", false);
            if (this.player.cor < 80) this.outputText("You beg, \"<i>Please, no, no!  I don't want it... in me!</i>\"\n\n", false);
            else this.outputText("You start to beg it not to put it inside you, but your protests drop off when you realize how good they could feel squirming inside you.\n\n", false);
