...
export class PhoukaScene extends BaseContent implements TimeAwareInterface {
    ...
    static phoukaForm: number = 0; //This keeps track of the form of the phouka across different scenes and through combat
    static PHOUKA_FORM_FAERIE: number = 0;
    static PHOUKA_FORM_BUNNY: number = 1;
    static PHOUKA_FORM_GOAT: number = 2;
    static PHOUKA_FORM_HORSE: number = 3;
    ...
    public timeChange(): boolean {
        if (this.player.statusAffectv1(StatusAffects.PhoukaWhiskeyAffect) > 0) {
            ...
            if (this.player.statusAffectv1(StatusAffects.PhoukaWhiskeyAffect) <= 0) {
                kGAMECLASS.consumables.P_WHSKY.phoukaWhiskeyExpires(this.player);
                return true;
            }
        }
        return false;
    ...
    public phoukaEncounter(): void { //General entry point for everything except halloween special encounter
        ...
        if (this.flags[kFLAGS.PHOUKA_ENCOUNTER_STATUS] == 0) { //Guarantee first Phouka encounter is with faerie fire
    ...
    protected phoukaFaerieFire(): void { //In old Irish legends the Phouka are responsible for Will-o’-the-wisps. In the deep woods it’s Kitsune, but in the bog it’s a Phouka
        ...
        this.menu();
        if (this.flags[kFLAGS.PHOUKA_ENCOUNTER_STATUS] != 0 || this.player.hasKeyItem("Traveler's Guide") >= 0) this.addButton(this, 4, "Leave", this.phoukaFaerieFireLeave);
        this.addButton(this, 0, "Follow", this.phoukaStuck);
    }
    ...
    protected phoukaFaerieFireLeave(): void {
        ...
        this.doNext(this, this.camp.returnToCampUseOneHour);  //Return to camp, 1 hour used
    }

    protected phoukaStuck(): void {
        this.clearOutput();
        var hasAlcohol: boolean = this.player.hasItem(this.consumables.P_WHSKY, 1);
        ...
        if (this.player.isNaga()) this.outputText("tail is");
        else this.outputText("legs are");
        ...
        if (hasAlcohol) this.outputText("\n\nYou suspect that if you offer up that whiskey in your pack the " + this.phoukaName() + " probably won’t try to rape you.");
        if (this.flags[kFLAGS.PHOUKA_ENCOUNTER_STATUS] == 0) this.flags[kFLAGS.PHOUKA_ENCOUNTER_STATUS] = 1;

        this.menu();
        if (hasAlcohol) this.addButton(this, 0, "Offer Drink", this.phoukaStuckOfferWhiskey);
        this.addButton(this, 1, "Talk", this.phoukaTalk);
        this.addButton(this, 2, "Fight", this.phoukaStartFight);
    }
    ...
    protected phoukaStuckOfferWhiskey(): void {
        ...
        if (this.flags[kFLAGS.PHOUKA_LORE] == 0) this.flags[kFLAGS.PHOUKA_LORE] = 1; //Now you know what to call them
        ...
        if ((this.player.pregnancyIncubation == 0) && (this.player.buttPregnancyIncubation == 0)) {
            this.menu();
            this.addButton(this, 1, "Drink", this.phoukaDrinkAccept);
        }
        else this.phoukaDrinkWhilePregnant(true);
    }
    ...
    protected phoukaTalk(): void {
        ...
        if (this.player.cor + PhoukaScene.rand(125) <= 110) { //Players with 0% corruption have a 12% chance that the Phouka is willing to talk, players with 100% corruption have a 92% chance
            ...
            else this.outputText("fuckable little " + this.phoukaName());
            ...
            switch (this.player.level < 10 ? PhoukaScene.rand(8) : PhoukaScene.rand(12)) { //Low level characters don't get the chance to be offered whiskey
                ...
                case 6: //Offers you a chance to submit, which leads to the various defeat sex scenes
                    ...
                    this.addButton(this, 4, "Fight", this.phoukaStartFight);
                    break;
                case 7:
                ...
                default: //(9, 10 or 11) Offers to share some whiskey
                    ...
                    if ((this.player.pregnancyIncubation == 0) && (this.player.buttPregnancyIncubation == 0)) {
                        ...
                        this.menu();
                        this.addButton(this, 0, "Refuse", this.phoukaDrinkRefuse);
                        this.addButton(this, 1, "Drink", this.phoukaDrinkAccept);
                    }
                    else this.phoukaDrinkWhilePregnant(false);
            } //End of Switch
        }
    }
    ...
    protected phoukaDrinkWhilePregnant(playerOfferedTheBooze: boolean): void {
        ...
        var acceptable: number = this.consumables.P_WHSKY.phoukaWhiskeyAcceptable(this.player);
        if (acceptable < 0) {
            this.outputText("\n\nYou think about the unborn child" + (acceptable <= -3 ? "ren" : "") + " you're carrying.");
            if (acceptable == -4) this.outputText("  One of them seems to crave the whiskey, but the other is a more normal child.");
        ...
        this.menu();
        this.addButton(this, 0, "Refuse", (playerOfferedTheBooze ? this.phoukaDrinkRefuseAfterOffer : this.phoukaDrinkRefuse));
        if (acceptable) this.addButton(this, 1, "Drink", this.phoukaDrinkAccept);
    }
    ...
    protected phoukaDrinkAccept(): void { //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
        ...
        this.flags[kFLAGS.PREGNANCY_CORRUPTION]++; //Faerie or phouka babies become more corrupted, no effect if the player is not pregnant or on other types of babies
    ...
    protected phoukaDrinkRefuse(): void { //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
        ...
        if ((this.player.pregnancyIncubation == 0) && (this.player.buttPregnancyIncubation == 0) && (this.player.pregnancyIncubation <= 100)) { //Pregnancy is obvious to the phouka
            ...
            this.menu();
            this.phoukaSexAddStandardMenuChoices();
            this.addButton(this, 0, "Leave", this.phoukaDrinkRefusePregnantLeave);
        }
            ...
            this.addButton(this, 0, "Leave", this.phoukaDrinkRefuseLeave);
        }
    }
    ...
    protected phoukaDrinkTalk(playerNotDrinking: boolean): void {
        if (this.flags[kFLAGS.PHOUKA_LORE] == 1) {
            this.flags[kFLAGS.PHOUKA_LORE]++;
        ...
        else if (this.flags[kFLAGS.PHOUKA_LORE] == 2) {
            this.flags[kFLAGS.PHOUKA_LORE]++;
        ...
        else if (this.flags[kFLAGS.PHOUKA_LORE] == 3) {
            this.flags[kFLAGS.PHOUKA_LORE] += 1;
        ...
        var playerIsInebriated: boolean = (playerNotDrinking == false) && (this.player.tou < 70);
        if (playerIsInebriated) {
            ...
            this.menu();
            this.addButton(this, 0, "Leave", this.phoukaDrinkLeave);
        }
        else {
            ...
            if (this.player.lust < 33)
                ...
                else this.outputText("and you feel your sphincter twitch as your brain thinks about what you could do with this phouka now."); //Genderless
            }
            this.menu();
            this.addButton(this, 0, "Leave", this.phoukaDrinkLeave);
            if (this.player.lust >= 33) {
                this.addButton(this, 1, "Ride Faerie", this.phoukaSexFaerieNonCombat); //Works for all
                if (this.player.hasCock()) this.addButton(this, 2, "Fuck Faerie", this.phoukaSexFaerieFemaleNonCombat); //Male or Herm Only
            }
        }
    }
    ...
    public phoukaHalloween(): void {
        this.flags[kFLAGS.TREACLE_MINE_YEAR_DONE] = this.date.fullYear;
        this.clearOutput();
    ...
    public phoukaPlayerWins(hpVictory: boolean): void {
        ...
        if (hpVictory) { //You win by physical damage, the phouka cheats and runs
            ...
            if (this.player.lust < 33) {
                ...
                else this.outputText(".  It would be so easy to rape this perverted faerie, how can you resist?");
                this.menu();
                this.addButton(this, 0, "Leave", this.phoukaLeaveOnLustWin);
                this.addButton(this, 1, "Anal Ride", this.phoukaSexFaeriePostCombat); //Works for all
                if (this.player.hasCock()) this.addButton(this, 2, "Fuck Faerie", this.phoukaSexFaerieFemalePostCombat); //Male or Herm Only
            }
        }
    }
    ...
    public phoukaPregBirth(): void {
        ...
        if (this.flags[kFLAGS.PREGNANCY_CORRUPTION] > 6) { //You’ve been drinking like a fish, haven’t you?
            ...
            if (this.player.fertility < 5) this.player.fertility = 5;
            this.fatigue(75);
            this.flags[kFLAGS.BIRTHS_PHOUKA]++;
            this.player.orgasm();
        }
        else if (this.flags[kFLAGS.PREGNANCY_CORRUPTION] > 0) {
            ...
            this.fatigue(5);
            this.flags[kFLAGS.BIRTHS_FAERIE]++;
        }
    }
    ...
    public phoukaPregUpdate(): boolean { //Belly size doesn't change, instead you get updates on what's going on
        ...
        if (this.player.pregnancyIncubation == 36) { //Stage 5:
            ...
            else this.outputText("Despite being pregnant you still don't feel restricted or slowed in any way.  It's as if the tiny life inside is living in harmony with you.\n");
            return true;
        }
        return false;
    }
    ...
    protected phoukaSexAddStandardMenuChoices(): void { //This happens several times so it's broken out here in case additional options get added later
        if (this.player.hasVagina()) {
            this.addButton(this, 1, "Bunny", this.phoukaSexBunnyChoice);
            this.addButton(this, 2, "Horse", this.phoukaSexHorseChoice);
        }
        this.addButton(this, 3, "Goat", this.phoukaSexGoatChoice);
    }
    ...
    public phoukaSexBunny(postCombat: boolean, lustLoss: boolean): void { //Player must have a vagina for bunny-morph sex
        ...
        if (postCombat) {
            ...
            else this.outputText("body too bruised to");
            ...
            if (this.player.cor <= 50) {
                ...
                if (PhoukaScene.phoukaForm != PhoukaScene.PHOUKA_FORM_BUNNY) this.outputText(", and morphs into a rabbit");
                ...
                if (PhoukaScene.phoukaForm != PhoukaScene.PHOUKA_FORM_BUNNY) this.outputText("  He goes through a short series of warm up exercises and then transforms himself into the rabbit you desire.");
            }
        }
        PhoukaScene.phoukaForm = PhoukaScene.PHOUKA_FORM_BUNNY;
    ...
    public phoukaSexGoat(postCombat: boolean, lustLoss: boolean): void { //Player does not have a vagina or has chosen anal
        ...
        if (postCombat) {
            ...
            if (PhoukaScene.phoukaForm != PhoukaScene.PHOUKA_FORM_GOAT) this.outputText("  As you lay there, the " + this.phoukaName() + "'s body stretches and morphs until it takes the form of a large, black furred goat-morph.");
        }
        else {
            ...
            else this.outputText("Then you put on a show, taking your clothes off piece by piece.  You strip off your final undergarment, bend over and wink your asshole at the goat.");
        }
        ...
        this.player.orgasm();
        if (postCombat) this.outputText("  While you're recovering the goat-morph reaches into your gem pouch and takes a handful.");
    ...
    protected phoukaSexFaerie(postCombat: boolean): void {
        ...
        if (this.player.hasVagina()) {
            ...
            if (this.player.hasCock()) this.outputText("  [EachCock] fires several blasts of cum into the bog [if (isTaur = true)beneath you][if (isTaur = false)and all over your faerie partner].");
            ...
            else this.doNext(this, this.camp.returnToCampUseOneHour); //Return to camp, 1 hour used
        }
    }
    ...
    protected phoukaSexFaerieFemale(postCombat: boolean): void {
        ...
        if ((this.flags[kFLAGS.FREED_VALA] == 1) || (this.flags[kFLAGS.TIMES_PC_DEFEATED_VALA] > 0) || (this.flags[kFLAGS.TIMES_FUCKED_VALA_IN_DUNGEON] > 0) || (this.flags[kFLAGS.INVESTIGATED_VALA_AFTER_ZETAZ_DEFEATED] > 0)) //If any of these is true you've had some contact with Vala
    ...
    protected phoukaSexPregnate(postCombat: boolean): void { //Whether by horse, bunny or (male) faerie sex it all ends up here if the PC has a vagina
        ...
        this.player.knockUp(PregnancyStore.PREGNANCY_FAERIE, PregnancyStore.INCUBATION_FAERIE);
        if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_FAERIE) {
        ...
        if ((!postCombat) || (this.player.cor > 50)) {
            if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_HORSE)
                ...
                else this.outputText("Perhaps he's being a considerate lover or perhaps he just wants to better his chances of knocking you up.");
            }
        }
        else {
            if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_HORSE)
        ...
        if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_FAERIE)
            ...
            this.outputText("\n\nHe");
        if (postCombat) this.outputText(" rummages around in your gem pouch and then");
        this.outputText(" takes to the air and hovers nearby as if waiting for something.\n\n");
        if (this.player.isPregnant()) {
            var isFirstTime: boolean = (this.flags[kFLAGS.BIRTHS_PHOUKA] == 0) && (this.flags[kFLAGS.BIRTHS_FAERIE] == 0);
            this.flags[kFLAGS.PREGNANCY_CORRUPTION] = 2; //Start off with a corrupt little phouka. It will take three drinks of pure honey to turn it into a faerie
            ...
            if (this.flags[kFLAGS.PHOUKA_LORE] == 0) this.flags[kFLAGS.PHOUKA_LORE] = 1; //Now you know what to call them
            if (!postCombat || (this.player.cor > 50)) this.outputText(", yer a real prize slut");
            ...
            else this.outputText("At least you know what to expect from this form of pregnancy now.");
        }
        else this.outputText("When you pull yourself up your belly begins to deflate and leak its gooey grey payload into the mud.  The " + this.phoukaName() + " says <i>“Ah well, there's always next time,”</i> and buzzes away to some other part of the bog.");
        this.phoukaSexPregnateEnd(postCombat);
    }
    ...
    protected phoukaSexPregnateEnd(postCombat: boolean): void { //Everything after the sex. Handles awards, gem loss and text for player leaving the bog
        ...
        if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_FAERIE) { //In this case postCombat means you need an award because you must have won to get faerie sex
            ...
            if (this.player.cor > 50) this.outputText("  But either way you plan to return and give all of them that lesson.");
            if (postCombat) {
                kGAMECLASS.clearStatuses(false);
                kGAMECLASS.awardPlayer();  //This will provide loot and return to camp, 1 hour used
            }
            ...
            if (postCombat && this.player.gems > 0) this.outputText("  While you're recovering the " + this.phoukaName() + " reaches into your gem pouch and takes a handful.");
            ...
            this.outputText("\n\nYou ");
            if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_HORSE && !this.player.isTaur()) this.outputText("wait for the throbbing pain in your pelvis to subside.  Then you ");
            if (postCombat) {
            ...
            else this.outputText("Not what you expected, but at least you got off.");
        }
    }
