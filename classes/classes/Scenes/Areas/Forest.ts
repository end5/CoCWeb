...
export class Forest extends BaseContent {
    ...
    public exploreDeepwoods(): void {
        ...
        if (this.player.statusAffectv1(StatusAffects.ExploredDeepwoods) % 10 == 0 && this.isHalloween()) {
            ...
            if (this.player.findPerk(PerkLib.FerasBoonBreedingBitch) < 0 && this.player.findPerk(PerkLib.FerasBoonAlpha) < 0) {
        ...
        if (this.flags[kFLAGS.PC_PROMISED_HEL_MONOGAMY_FUCKS] == 1 && this.flags[kFLAGS.HEL_RAPED_TODAY] == 0 && Forest.rand(10) == 0 && this.player.gender > 0 && !kGAMECLASS.helScene.followerHel()) {
        ...
        if (this.flags[kFLAGS.TAMANI_TIME_OUT] == 0 && Forest.rand(5) == 0 && this.player.gender > 0 && (this.player.totalCocks() > 0 || this.player.hasKeyItem("Deluxe Dildo") < 0)) {
        ...
        if (this.flags[kFLAGS.ERLKING_DISABLED] == 0 && this.flags[kFLAGS.ERLKING_ENCOUNTER_COUNTER] == 4) {
    ...
    public exploreForest(): void {
        ...
        if (this.player.findPerk(PerkLib.PiercedFurrite) >= 0 && Forest.rand(5) == 0 && (this.player.cor > 25 || kGAMECLASS.monk > 0)) {
            chooser = 1;
        }
        //If Jojo lives in camp, never encounter him
        if (this.player.findStatusAffect(StatusAffects.PureCampJojo) >= 0 || this.flags[kFLAGS.JOJO_DEAD_OR_GONE] == 1) {
        ...
        if ((Forest.rand(100) <= 1) && this.player.gender > 0 && (this.flags[kFLAGS.ESSY_MET_IN_DUNGEON] == 0 || this.flags[kFLAGS.TOLD_MOTHER_TO_RELEASE_ESSY] == 1)) {
        ...
        //Marble randomness
        if (this.player.exploredForest % 50 == 0 && this.player.exploredForest > 0 && this.player.findStatusAffect(StatusAffects.MarbleRapeAttempted) < 0 && this.player.findStatusAffect(StatusAffects.NoMoreMarble) < 0 && this.player.findStatusAffect(StatusAffects.Marble) >= 0 && this.flags[kFLAGS.MARBLE_WARNING] == 0) {
        ...
        if (chooser == 0) {
            ...
            //Encounter Gobbalin!
                ...
                if (Forest.rand(4) <= 0 && this.flags[kFLAGS.TAMANI_TIME_OUT] == 0 && this.player.gender > 0 && (this.player.totalCocks() > 0 || this.player.hasKeyItem("Deluxe Dildo") < 0)) {
                    if (this.player.totalCocks() > 0 && this.flags[kFLAGS.TAMANI_DAUGHTER_PREGGO_COUNTDOWN] == 0 && this.flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] >= 24) {
                        this.tamaniDaughtersScene.encounterTamanisDaughters();
                    }
                    else
        ...
        if (chooser == 1) {
            ...
            if (kGAMECLASS.monk >= 2) {
                ...
                if (kGAMECLASS.monk == 2) this.outputText("his mousey visage twisted into a ferocious snarl.  \"YOU!\" he screams, launching himself towards you, claws extended.", false);
                if (kGAMECLASS.monk == 3) this.outputText("unsteady on his feet, but looking for a fight!", false);
                if (kGAMECLASS.monk == 4) this.outputText("visibly tenting his robes, but intent on fighting you.", false);
                if (kGAMECLASS.monk == 5) this.outputText("panting and nude, his fur rustling in the breeze, a twitching behemoth of a cock pulsing between his legs.", false);
        ...
        if (chooser == 2) {
            ...
            if (this.temp == 1) {
                if (this.player.cor < 80) {
                    ...
                    if (this.player.gender == 1) this.outputText("stroking your half-erect " + this.multiCockDescriptLight() + " as you daydream about fucking all kinds of women, from weeping tight virgins to lustful succubi with gaping, drooling fuck-holes.", false);
                    if (this.player.gender == 2) this.outputText("idly toying with your " + this.vaginaDescript(0) + " as you daydream about getting fucked by all kinds of monstrous cocks, from minotaurs' thick, smelly dongs to demons' towering, bumpy pleasure-rods.", false);
                    if (this.player.gender == 3) this.outputText("stroking alternatively your " + this.multiCockDescriptLight() + " and your " + this.vaginaDescript(0) + " as you daydream about fucking all kinds of women, from weeping tight virgins to lustful succubi with gaping, drooling fuck-holes, before, or while, getting fucked by various monstrous cocks, from minotaurs' thick, smelly dongs to demons' towering, bumpy pleasure-rods.", false);
                    if (this.player.gender == 0) this.outputText("daydreaming about sex-demons with huge sexual attributes, and how you could please them.", false);
    ...
    public bigJunkForestScene(lake: boolean = false): void {
        ...
        if (this.player.cocks.length == 1) {
            if (lake) this.outputText("  As it drags through the lakeside mud, the sensation forces you to imagine the velvety folds of a monstrous pussy sliding along the head of your " + Appearance.cockNoun(this.player.cocks[x].cockType) + ", gently attempting to suck it off.", false);
            else this.outputText("  As it drags across the grass, twigs, and exposed tree roots, the sensation forces you to imagine the fingers of a giant hand sliding along the head of your " + Appearance.cockNoun(this.player.cocks[x].cockType) + ", gently jerking it off.", false);
        }
        else if (this.player.cocks.length >= 2) {
            if (lake) this.outputText("  With all of your " + this.multiCockDescriptLight() + " dragging through the mud, they begin feeling as if the lips of " + Forest.num2Text(this.player.cockTotal()) + " different cunts were slobbering over each one.", false);
            else this.outputText("  With all of your " + this.multiCockDescriptLight() + " dragging across the grass, twigs, and exposed tree roots, they begin feeling as if the rough fingers of " + Forest.num2Text(this.player.cockTotal()) + " different monstrous hands were sliding over each shaft, gently jerking them off.", false);
        }
        this.outputText("\n\n", false);
        ...
        if (!this.player.isTaur()) {
            ...
            if (this.player.biggestTitSize() >= 35) {
                ...
                else this.outputText("  Your " + this.chestDesc() + " hang lewdly off your torso to rest on the twings and dirt, covering up much of the ground to either side of you.  Their immense weight anchors your body, further preventing your torso from lifting itself up.  The rough texture of the bark on various tree roots teases your " + this.nippleDescript(0) + "s mercilessly.", false);
            }
            ...
            if (this.player.vaginas.length >= 1) {
                ...
                if (this.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_DROOLING) {
                    ...
                    if (lake) this.outputText("The drooling fem-spunk only makes the ground more muddy.", false);
                    else this.outputText("The sticky fem-spunk immediately soaks down into the rich soil.", false);
                }
            }
        }
        //FOR CENTAURS
            ...
            if (this.player.biggestTitSize() >= 35) {
                ...
                else this.outputText("  Your " + this.chestDesc() + " pull your human torso forward until it also is forced to face the ground, obscured as it is in boob-flesh.  Your tits rest on the dirt and twigs to either side of you.  Their immense weight anchors you, further preventing any part of your equine body from lifting itself up.  The rough texture of the bark on various tree roots teases your " + this.nippleDescript(0) + "s mercilessly.", false);
            }
            //IF CHARACTER HAS A BALLS ADD SENTENCE
            if (this.player.balls > 0) {
                this.outputText("  Your " + this.player.skinTone + this.sackDescript() + " rests beneath your raised " + this.buttDescript() + ".  Your " + this.ballsDescriptLight() + " pulse with the need to release their sperm through your " + this.multiCockDescriptLight() + " and ", false);
            ...
            if (this.player.vaginas.length >= 1) {
                ...
                //IF CHARACTER HAS A DROOLING PUSSY ADD SENTENCE
                if (this.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_DROOLING) {
                    if (lake) this.outputText("  A leaf falls from a tree and lands on the wet lips of your cunt, its light touch teasing your sensitive skin.  Like a mare or cow in heat, your juices stream from your womanhood and pool in the mud beneath you.  The sloppy fem-spunk only makes the ground more muddy.", false);
                    else this.outputText("  A leaf falls from a tree and lands on the wet lips of your cunt, its light touch teasing your sensitive skin.  Like a mare or cow in heat, your juices stream from your womanhood and pool in the dirt and twigs beneath you.", false);
                }
            }
        }
        this.outputText("\n\n", false);
        ...
        if (this.player.cor < 33) this.outputText("  Panic slips into your heart as you realize that if any dangerous predator were to find you in this state, you'd be completely defenseless.  You must find a way to regain your mobility immediately!", false);
        ...
        if (lake) {
            ...
            //SCENE END = IF CHARACTER HAS FULL WINGS ADD SENTENCE
            if (this.player.canFly()) this.outputText("  You extend your wings and flap as hard as you can, until at last, you manage to lighten the bulk of your body.  It helps just enough to let you drag your genitals out of the forest and back to camp.  The ordeal takes nearly an hour for you to return and deal with.", false);
    ...
    private ignoreSatyr(): void {
        ...
        if (this.player.cor < 50) this.outputText("disgusted by the strange thoughts this place seems to put into your mind", false);
    ...
    private rapeSatyr(): void {
        ...
        //(Low Corruption)
        if (this.player.cor < 33) this.outputText("For a moment you hesitate... taking someone from behind without their consent seems wrong... but then again you doubt a satyr would pass on the opportunity if you were in his position.", false);
        //(Medium Corruption)
        else if (this.player.cor < 66) this.outputText("You smirk; normally you would have given this some thought, but the idea of free booty is all you need to make a decision.", false);
        //High Corruption
        else this.outputText("You grin; this is not even a choice!  Passing on free anal is just not something a decent person does, is it?", false);
        ...
        if (this.player.cockArea(x) >= 100) this.outputText("stretching it all out of normal proportion and ruining it for whomever might happen to use it next.", false);
        else this.outputText("stretching it to fit your " + this.cockDescript(x) + " like a condom.", false);
        ...
        if (this.player.cumQ() >= 1000) this.outputText(", although his eyes have widened perceptibly along with the stretching of his stomach", false);
        this.outputText(".\n\n", false);
