...
export class SatyrScene extends BaseContent {
    ...
    */
    ...
    private consensualSatyrFuck(loc: number = 0): void {
        ...
        if (loc == 0) this.outputText("trampled-down grass");
        else this.outputText("relatively dry and solid ground");
    ...
    private loseToSatyr(): void {
        ...
        this.spriteSelect(98);
        //[Lust loss
        if (this.player.lust > 99) this.outputText("You pant as you begin masturbating furiously, too horny to care about anything the grinning satyr before you has in mind.");
        else this.outputText("You try to steady yourself, clutching your body in pain as the satyr grins at you.");
        ...
        if (this.player.isNaga()) this.outputText("pins down your serpentine body");
        else if (this.player.isBiped()) this.outputText("spreads your [legs]");
        else this.outputText("pins you down");
        this.outputText(" and reaches forward, probing for a vagina.  ");
        if (this.player.hasVagina()) this.outputText("Upon finding one, he grins lustfully, his already impressively erect cock rising to full flag.");
        else this.outputText("Finding you lacking that trait, he grunts disdainfully and probes roughly for an asshole, which he locates right between your buttcheeks where it belongs, then jerks himself with several strokes to help coax his masculinity to full erection.");
        ...
        if ((this.player.hasVagina() && this.player.vaginalCapacity() < this.monster.cockArea(0)) || (!this.player.hasVagina() && this.player.analCapacity() < this.monster.cockArea(0))) this.outputText(", not even bothered by the fact that his cock doesn't even fit inside you");
        this.outputText(".");
        //[cunt/buttchange]
        if (this.player.hasVagina()) this.player.cuntChange(this.monster.cockArea(0), true, true, false);
        ...
        if (this.player.gender == 0) this.outputText("aroused ");
        ...
        if (this.player.hasCock()) this.outputText("  [EachCock] throbs as it send strings of cum flying straight into the satyr's chest, painting it white.");
        if (this.player.hasVagina()) this.outputText("  A veritable flood of girlcum escapes the small space between your vaginal walls and the satyr's massive shaft, covering his balls and legs with your female fluids.");
        ...
        if (this.player.cor < 50) this.outputText("shamefully ");
        else this.outputText("shamelessly ");
    ...
    private defeatASatyr(): void {
        ...
        this.spriteSelect(98);
        //Lust Victory
        if (this.monster.lust > 99) this.outputText("The satyr collapses to its caprine knees, bleating in dismay as it paws frantically at its huge cock, oblivious to everything in its need to get off.  Already, pre-cum is fountaining from the goat-man's shaft, his jerking motions smearing the pungent sexual fluid across the crown.");
        //HP Victory
        else this.outputText("Beaten and dazed, the satyr collapses to its caprine knees, shaking his head in a futile attempt to recover himself from the brutal trouncing you've just given him.  The combination of the blows and his previous drunken state mean he's quite incapable of getting back, however.");
        var butt = undefined;
        var faces = undefined;
        if (this.player.lust >= 33 && this.player.gender > 0) {
            ...
            //[Male][Female][Leave]
            if (this.player.hasCock() && this.player.cockThatFits(this.monster.analCapacity()) >= 0) butt = this.malesTakeAdvantageOfSatyrs;
    ...
    private femaleTakesAdvantageOfSatyr(): void {
        ...
        if (this.player.hasCock()) {
            ...
            else this.outputText("the base of your " + this.multiCockDescriptLight());
        }
        ...
        if (this.player.hasCock()) this.outputText(", your own cock belching its usual load across his stomach and the surroundings");
        this.outputText(".");
    ...
    private malesTakeAdvantageOfSatyrs(): void {
        ...
        //Low Cum Amount
        if (this.player.cumQ() < 250) this.outputText("  He tries to milk you for far more than you can give him, and at one point you feel like his anal muscles are going to tear your dick off, until finally they go lax and lets you slip out.");
        //(Medium Cum Amount)
        else if (this.player.cumQ() < 1000) this.outputText("  You dump far more than you usually do; the satyr's clenching asshole makes sure to drain you of every little drop of cum you have until finally it goes lax and lets you slip out, leaving the goat-man with a pudgy belly.");
        //(High Cum Amount)
        else this.outputText("  You shoot jet after jet of cum, far faster than the satyr's ass can milk you for it, and still he doesn't seem to stop trying.  His rippling muscles draw your cum inside his bowels in big bubbles.  When his ass finally goes lax, you're propelled halfway out of him by the force of your own backflowing seed; the satyr's ass looks like a wreck, and his belly is overflowing with so much spunk you're surprised it didn't come out of his mouth.");
    ...
    private willinglyBoneSatyr(): void {
        ...
        if (this.player.hasCock()) this.outputText("hardening maleness, giving it a few strokes to bring it to full mast, then he moves on to your ");
        if (this.player.balls > 0) this.outputText("[balls], rolling each of your orbs around with reverence at the life-giving load they produce; finally he addresses your ");
        //((if PC has a pussy)
        if (this.player.hasVagina()) this.outputText("[vagina], spreading your labia wide to see the interior of your rapidly moistening walls.");
        else this.outputText("[ass], spreading your cheeks apart to gaze at your winking rosebud.");
        ...
        if (this.player.hasVagina()) this.outputText("[vagina], licking all over your netherlips and tasting your juices.");
        else this.outputText("[butt], licking all around your hole to prepare it for what's coming.");
        ...
        if (!this.player.hasVagina()) this.outputText("clenching butt");
        else this.outputText("spasming vagina");
        ...
        if (this.player.hasVagina()) this.outputText("pussy gushes fluids down his massive manhood");
        if (this.player.gender == 3) this.outputText(" and ");
        if (this.player.hasCock()) this.outputText("cock spurts cream across your intertwined forms");
        ...
        if (!this.player.hasVagina() && this.flags[kFLAGS.SATYR_KIDS] == 0) {
            ...
            if (this.player.hasCock()) this.outputText("males");
            else this.outputText("neuters");
    ...
    private satyrPreggo(): void {
        if (this.player.hasVagina()) {
            ...
            this.player.buttKnockUp(PregnancyStore.PREGNANCY_SATYR, PregnancyStore.INCUBATION_SATYR, 1, 1);
        }
    }
    ...
    public satyrBirth(vag: boolean): void {
        ...
        else if (vag) this.outputText("[vagina]");
