...
export class Kaiju extends AbstractLakeContent {
    ...
    private meetDatKaijuYo(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.FactoryOverload) >= 0) this.flags[kFLAGS.KAIJU_COCK] = 1;
    ...
    private insultTheKaijuFirstMeeting(): void {
        ...
        if (this.kaijuCock()) {
            ...
            if (this.player.findStatusAffect(StatusAffects.KnowsWhitefire) >= 0 && (this.player.findPerk(PerkLib.BloodMage) < 0 && this.player.fatigue + kGAMECLASS.spellCost(30) > 100)) {
    ...
    private letKaijuHaveWayWithYou(): void {
        ...
        if (this.player.hasCock()) this.outputText(", [eachCock] stiffening in your [armor] from the stimulation as your whole body is used in what seems more and more like a massive boob job");
        this.outputText(".");
        if (this.player.hasVagina()) this.outputText("  Your [vagina] moistens from your body's growing arousal.");
        ...
        if (this.player.gender == 3) this.outputText(this.multiCockDescriptLight() + " and [vagina]");
        else if (this.player.gender == 1) this.outputText(this.multiCockDescriptLight());
        ...
        if (this.player.hasCock()) {
            ...
            //[if PC has balls]
            if (this.player.balls > 0) this.outputText("  Your [balls] begin to ache as you slowly empty your load onto green flesh.");
            this.player.orgasm();
        }
    ...
    private flirtWithKaiju(): void {
        ...
        if (this.player.sens < 50) {
            ...
            //[If PC has cock]
            if (this.player.hasCock()) this.outputText("  [EachCock] spasms as you release your load onto her pussy walls, making them just a bit more slick with your contribution.");
            if (this.player.hasVagina()) this.outputText("  Your own pussy releases its own feminine juices, soaking your thighs even further in wetness.");
        ...
        if (this.kaijuCock()) {
            ...
            if (this.player.gender > 0) {
                ...
                if (this.player.biggestTitSize() >= 1) this.outputText("  You press your [chest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.");
                if (this.player.lactationQ() >= 200) this.outputText("  Milk spurts and flows from your [nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.");
            }
            ...
            if (this.player.gender > 0) {
                ...
                if (this.player.hasVagina()) this.outputText("feminine juices");
                if (this.player.gender == 3) this.outputText(" and ");
                if (this.player.hasCock()) this.outputText("thick cum");
    ...
    private fuckThisGiantYouDumbCunt(): void {
        ...
        if (this.player.sens >= 50) {
            ...
            if (this.kaijuCock()) {
                ...
                //[If PC has boobs]
                if (this.player.biggestTitSize() >= 1) this.outputText("You press your [fullChest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.");
                if (this.player.lactationQ() >= 200) this.outputText("  Milk spurts and flows from your [nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.");
    ...
    private urethraFuckDatGiantCock(): void {
        ...
        if (this.player.hasCock()) this.outputText(" and your " + this.multiCockDescriptLight());
        ...
        if (this.player.hasCock()) this.outputText("  So much jizz fills your ass it can’t help but to press in on your prostate, making [eachCock] stiffen with a seriously growing urge.");
        if (this.player.hasVagina()) this.outputText("  Your ass isn’t alone in the feeling of being flooded as your [vagina] and womb are packed with the hermaphrodite’s sperm.");
    ...
    private peekAtSomePhatAssKaijuButt(): void {
        ...
        else this.outputText(" while her thumb begins rubbing up and down her slowly stiffening cock");
    ...
    private mockDatTurtleGirl(): void {
        ...
        if (this.player.balls > 0 && this.player.ballSize >= 6) this.outputText("Your [balls] rest on the hermaphrodite's hand, bouncing loudly with each thunderous stroke from the speed and force she is employing to get herself off.  ");
        //[end balls] [If player has cock]
        if (this.player.hasCock()) this.outputText("You can feel your [armor] getting tighter as [eachCock] begins to stiffen under your clothing from all the constant pressure of the cock squeezing down on your member.  ");
        //[If player has vagina]
        if (this.player.hasVagina()) this.outputText("You begin to feel a wetness between your legs as your cunt begins leaking as your arousal spikes from the intense rubbing the herm's penis is giving your whole body.  ");
        //[If genderless]
        if (this.player.gender == 0) this.outputText("Your cheeks blush in arousal from all this heavy rubbing.  ");
        if (this.player.cor < 33) this.outputText("You can't believe that you're being aroused by this.  Aren't you suppose to be a champion?");
        else if (this.player.cor < 66) this.outputText("Wow, this is arousing you.  You wonder if you really are a champion.");
        else this.outputText("You just want to get off now.");
        ...
        if (this.player.hasCock()) this.outputText(" as you cum in your [armor]");
    ...
    private helpNewFutaKaijuGetOff(): void {
        ...
        //[If PC has cock]
        if (this.player.hasCock()) this.outputText("  [EachCock] quickly hardens now as it is rubbed up against the giant's massive shaft.");
        if (this.player.hasVagina()) this.outputText("  The warm heat that radiates off of the titanic member seems to seep into your nether region, making your [vagina] leak out moisture onto the massive prick.");
    ...
    private yesTurnKaijuFuta(): void {
        ...
        while (counter > 0) {
            ...
            else if (this.player.hasItem(this.consumables.INCUBID)) this.player.consumeItem(this.consumables.INCUBID);
        }
    ...
    private badEndPartTwo(): void {
        ...
        if (this.player.tongueType == TONUGE_SNAKE) this.outputText("  Your snaky forked tongue begins lashing about, flicking at the nipple tip in your mouth while rubbing at the underside of it, urging the tender spot to give up a drop of the heavenly white liquid.");
        ...
        if (this.kaijuCock()) {
            ...
            if (this.player.hasCock()) this.outputText("  You prostate is pressed down like a button, and there’s just no letting up!");
            if (this.player.hasVagina()) this.outputText("  Even if your cunt is empty, there is no free space left there, your cunt walls forced together like glue.");
        ...
        if (this.player.biggestTitSize() >= 1) this.outputText("  Another tendril snakes its way up your abdomen to nestle between your [chest], slicking your mounds with pre as it slides between your tits, giving itself a titty-fuck with your titties!");
        ...
        if (this.player.hasCock()) this.outputText("  More tentacles arrive and begin to wrap around your " + this.multiCockDescriptLight() + ", slowly jacking you off to the tempo set by the cocks in your ass and mouth, wracking you with further pleasure.");
