...
export class BasiliskScene extends BaseContent {
    ...
    public basiliskGreeting(): void {
        ...
        //Standard encounter:
            ...
            this.outputText("You notice a large boulder ahead.  There is something curiously shaped about it. A small, wet grey shape on it catches your eye...\n\n", false);

            this.outputText("You look away in the nick of time, and ready yourself to fight as the basilisk slides from its hiding place and advances upon you, its deadly eyes and sharp claws glinting coldly in the sunlight.\n\n", false);
            this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00276]++;
            this.startCombat(new Basilisk());
        }
        this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00276]++;
        this.spriteSelect(75);
    }
    ...
    public defeatBasilisk(): void {
        ...
        if (this.player.canOvipositBee() && this.player.gender > 0) eggs = this.layBeeEggsInABasilisk;
        if (this.player.cockThatFits(this.monster.analCapacity()) >= 0 && this.player.cor >= 66) evil = this.defeatBasiliskAndAnal;
        //Player HP victory: 
        if (this.monster.HP < 1) this.outputText("Unable to stand anymore, the basilisk shakily sinks down on one knee, drops its head and looks at the ground, evidently demonstrating submission.", false);
        //Player Lust victory: 
        else this.outputText("No longer able to control its raging erection, the basilisk closes its eyes and sinks to one knee.  It would probably be attempting to signal its submission to you if it weren't furiously masturbating its long, purple cock, which has emerged straining from the creature's genital slit.", false);
    ...
    private tongueBasiliskSmex(): void {
        this.spriteSelect(75);
        this.outputText("", true);
        this.outputText("You tap your jaw as you stare down at the defeated reptile, smiling at its oddly formal gesture of surrender.  You'd very much like to take some satisfaction from the basilisk, extract a bit of pleasurable payment for what it tried to do to you: but the more you get to look at it properly, the more difficult doing that seems. The tall, thin creature is all angles, tough scales and pointed edges. You don't like the thought of putting your genitals anywhere near its sickle claws or curved fangs.\n\n", false);

        this.outputText("It is as you are taking in this latter article that the basilisk swallows nervously, and you get a glimpse inside its mouth at a long, sticky, pale purple tongue, another gift of its reptilian ancestors. It's the only part you've seen of it so far which looks... soft.  Pliable.  Your smile widens and you step forward, slowly loosening your " + this.player.armorName + ".\n\n", false);
    ...
    public loseToBasilisk(): void {
        ...
        if (this.player.hasVagina() && (this.player.inHeat || this.player.findPerk(PerkLib.Oviposition) >= 0 || this.player.findPerk(PerkLib.BasiliskWomb) >= 0 || this.player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS))
            this.basiliskHasVagEggStuff();
        else this.defaultBasiliskRape();
    }
    //Loss, vag rape conditions not met:
    private defaultBasiliskRape(): void {
        this.outputText("Working briskly, the basilisk tears off your " + this.player.armorName + " until you are entirely naked.  It then rummages through your pockets; it carelessly discards everything it finds without apparent interest.  It grabs a handful of gems from your purse and then prowls back to you.\n\n", false);

        //Male/Herm: 
        if (this.player.hasCock()) {
            this.outputText("With surprising gentleness and deftness, the basilisk rubs your " + this.cockDescript(0) + " with one palm", false);
            if (this.player.hasVagina()) this.outputText(" and sticks some of the smaller fingers of its other hand in your " + this.vaginaDescript(0), false);
            this.outputText(", thankfully angling its sickle claw away.  You can't do anything against it, and some of its mental compulsion remains; a backwash of erotic images from your past fill your head, and you can't even grit your teeth as the gentle, insistent pressure brushing your prick makes you rock hard.  It stops when you are erect and then, with the very faintest of smiles playing over its cruel mouth, leaves. You're naked, your " + this.cockDescript(0) + " is begging for release; you're utterly helpless... you can only hope that the spell will wear off, and before anything else in the mountain finds you.\n\n", false);
        }
        //Female: 
        else if (this.player.hasVagina()) {
            this.outputText("With surprising gentleness and deftness, the basilisk slips the smaller fingers of one hand into your " + this.vaginaDescript(0) + ", and carefully flicks at your " + this.clitDescript() + " with the other, thankfully holding its sickle claws away from you.  You can't do anything against it, and some of its mental compulsion remains; a backwash of erotic images from your past fill your head, and you can't even grit your teeth as the gentle, insistent caresses make you wet.  It stops when you are beading moisture involuntarily onto its hand and then, with the very faintest of smiles playing over its cruel mouth, leaves. You're naked, your " + this.vaginaDescript(0) + " begs to be filled; you're utterly helpless. You can only hope that its spell will wear off, and before anything else in the mountain finds you...\n\n", false);
        }
        //Genderless: 
        else this.outputText("Staring into your eyes, the basilisk moves its smaller fingers onto your groin... and then stops.  It looks downwards, and then back into your face.  You aren't very good at reading lizard facial expressions, but the creature looks distinctly baffled.  Finally with a slight shake of its head, it slowly turns and leaves.  It has left you paralyzed and naked to the open air, your skin prickling from the exposure.  You can only hope that its spell will wear off, and before anything else in the mountain finds you...\n\n", false);
    ...
    private basiliskHasVagEggStuff(): void {
        ...
        if (this.player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS) {
            ...
            this.player.knockUpForce(PregnancyStore.PREGNANCY_BASILISK, PregnancyStore.INCUBATION_BASILISK - 150); //Convert Ovi Elixir eggs to Basilisk eggs
        }
    ...
    public basiliskBirth(): void {
        ...
        if (this.player.vaginas.length == 0) {
            this.outputText("You feel a terrible pressure in your groin... then an incredible pain accompanied by the rending of flesh.  <b>You look down and behold a new vagina</b>.\n\n", false);
        ...
        if (this.player.findPerk(PerkLib.BasiliskWomb) >= 0) {
            ...
            if (this.player.hasCock()) this.outputText(" and a downpour of hermcum");
            this.outputText(".");
            ...
            if (this.flags[kFLAGS.BENOIT_EGGS] > 0) this.outputText(" as well");
            this.outputText(".  You place the egg back down and gather them all up, moving them closer to the campfire to stay warm while you recover from your exertions.");
    ...
    private basiliskAdvantageHarpy(): void {
        this.spriteSelect(75);
        this.outputText("Time stretches by at an agonizingly slow pace as you stand there, a bizarre, motionless flesh statue.  You have no way of measuring how much time is passing; the sun is not in your direct line of vision.   You try to move any and every part of yourself in turn, but it is hopeless.  Your body is a cage, and you begin to hate the basilisk less because it paralyzed you and more because it left your mind entirely aware of it.  Every so often another unbidden backwash of erotic memories overwhelms your senses, keeping you helplessly aroused and reminded of who did this to you.  Coupled with the unscratchable itches and the aching in your limbs the experience is one of sensational hell.\n\n", false);
        this.outputText("You hear a shrill cry from above you, half eagle scream and half mocking, female laughter.  With a fluttering flap of feathers, a harpy lands at your side before proceeding to stalk around you, taking in your helpless, frozen form with stiff, jerky movements.  You reflect bitterly that if the big bottomed bird woman had turned up fifteen minutes ago she would probably have scared the basilisk off.  As it is, you are going to have to take whatever she can throw at you... with a stiff upper lip, as it were.\n\n", false);

        //Male/Herm: 
        if (this.player.hasCock()) {
            this.outputText("The harpy's eyes zero in on your erect cock greedily.  Stepping back from you she raises her head and lets out a screech which echoes around the mountains; as answering calls roll back to her she closes in, threads her arms around your neck and scalp and kisses you roughly. She pushes her golden lips against yours and squeezes her rough bird tongue into your mouth.  You feel your lips tingle and you raggedly moan against the savage frenching, her hot breath pushing down your throat as she circles your still tongue with her own, before exploring further down towards your tonsils.  By the time she has finished with you your whole body feels like it is glowing red from the effect of her lipstick, " + this.sMultiCockDesc() + " straining.\n\n", false);

            this.outputText("You wonder vaguely how she and her no-doubt-soon-to-arrive sisters are going to take advantage of you as you are; you find out a moment later when with no preamble whatsoever the harpy shoves you roughly in the chest.  ", false);
            if (this.player.isBiped()) this.outputText("You teeter horribly on your frozen heels for a moment and then fall onto your back like a collapsing statue.  ", false);
            else this.outputText("You feel yourself slowly and horribly lose your balance before flopping onto your side like a collapsing statue.  ", false);
            this.outputText("Unable to brace yourself, you bang your head painfully; as you are lying there dazed, you feel something build at the back of your mind.  Involuntary sensations prickle your skin and groin as, once again, the intermittent mental backwash that the basilisk's hypnosis has forced upon your mind hits you.  Memory after memory of sexually-charged encounters, daydreams or fantasies crowd your consciousness.  It is made worse, much worse by the pheromones the harpy has pushed into your mouth; your body rides the chemical glow at the same time as imaginings of soft skin, tight muscle and musk overload your senses.  You groan raggedly as " + this.sMultiCockDesc() + " bulges and leaks pre-cum, almost screaming for attention.  When you finally, woozily come to your senses, you find that all vision has been blotted out by a big, wobbly harpy bum, her moistening lips rubbing impatiently against your mouth.\n\n", false);

            this.outputText("The bang on the head you took, the harpy lipsticks trilling in your bloodstream and the involuntary tide of erotic memories which ebbs and flows over you mean the next couple of hours goes by for you in a haze of forceful sex.  You can't control your cock", false);
            if (this.player.cockTotal() > 1) this.outputText("s", false);
            this.outputText(", your sex drive or what you are thinking about; you are a prisoner of lust and you quickly subside under the sexual concussion, hoping eventually the harpies and your own body will stop fucking you.  It does sink in around your second involuntary orgasm, your aching " + this.cockDescript(0) + " spurting ribbon after ribbon of jizz into a harpy's clenching warmth, that the two arriving harpies are not happy with the first one; with your mouth and hands frozen you can't pleasure them with anything but your manhood, which leaves two of them fighting each other for time with your groin whilst a third grinds frustratedly at your face, mainly using your nose to rub at her inner walls and clit, forcing her juices down your nostrils and making you cough raggedly, struggling to breathe.\n\n", false);

            //Single cock: 
            if (this.player.cockTotal() == 1) this.outputText("Their inability to properly satisfy themselves on your frame raises their tempers and frustration to the point where the three of them are physically fighting each other, treading all over you as they scream, bite and tear, their feathers flying all over the place.  Eventually the biggest of the trio drives the other two off, flapping and screeching into the mountains, before throwing herself onto her scratched and battered prize, her eyes lit up and wild.  She fucks you with a gusto born of bloodlust, her petit breasts bouncing up and down as she slams her powerful thighs into your " + this.hipDescript() + ", picking up the pace.  She quickly forces you to another achingly pleasurable peak, your " + this.cockDescript(0) + " spurting more of your seed deep into her, and then just keeps on working you. Your cock seems incapable of going soft.  You feel the involuntary erotic backwash build in your skull again...\n\n", false);
            //Multicock: 
            else {
                this.outputText("The three of them do eventually work out a compromise however, once they discover you have more than one manpole to your name.  One pulls your " + this.cockDescript(0) + " forwards painfully until it is almost pointing towards your face before squatting over it, whilst another clutches at your " + this.cockDescript(1) + " as she spreads her legs and works her way inwards until she is spearing herself on you. You can't see them - you can't see anything except pink wobbly flesh and lavender feathers - but you can hear them shift impatiently around each other as they find a position which is comfortable to them both, their inner walls rubbing you from every direction as they move.  The third continues to thrust her needy sex into your face as best she can", false);
                if (this.player.cockTotal() >= 3) this.outputText(", crowded out by the bodies of the others despite her transparent desire for the unoccupied man meat flopping against them,", false);
        ...
        //Genderless: 
            ...
            this.outputText("You feel something build at the back of your mind.  Involuntary sensations prickle your skin as, once again, the intermittent mental backwash that the basilisk's hypnosis has forced upon your mind hits you.  Memory after memory of sexually-charged encounters, daydreams or fantasies crowd your consciousness.  It is made worse, much worse by the pheromones the harpy has pushed into your mouth; your body rides the chemical glow at the same time as imaginings of soft skin, tight muscle and musk overload your senses.  The denial the petrification has forced upon you is becoming unbearable.\n\n", false);
            ...
            this.outputText("The harpy manages to cum twice on your hand, gobbling with excitement as she spatters your arm with her juices.  During this time you are forced to ride the potent cocktail of hypnotic sexual compulsion and the harpy's golden lipstick again and again, until you feel you would have collapsed in a pool of steaming sex long ago if your knees would only allow it.  Once she is finished with you the harpy clambers down, taking care to wipe her leaking twat on your naked front as she does so, before flapping off with a winsome smirk, entirely ignoring your own plight.\n\n", false);
    ...
    private basiliskAdvantageGoblin(): void {
        ...
        //Unsexed: 
            ...
            this.outputText("\"<i>A " + this.player.race() + "!</i>\" she yelps.  Hesitantly at first, then with increasing confidence as how vulnerable you are sinks in, she does a round circuit of you, taking in every angle of your frozen, helpless flesh.\n\n", false);
            ...
            this.outputText("\"<i>Hey you!  Yeah you, shit-for-brains!  Wanna free assfuck?  Come and get it then, you ugly, dumb muscle-bound moron!</i>\"  A moment later you feel a slap on your calf as the goblin runs past you, giggling.  \"<i>Have fun, smartass!</i>\"  A huge, angry bellow vibrates the air around you and the earth shakes as something big approaches you from behind.  You strain with every sinew of your being to escape, but you are, as ever, glued in place.  You manage a raspy moan as the enraged minotaur grasps you roughly by both arms, his animal musk filling your nostrils. <i>Fucking goblins.</i>\n\n", false);

            this.outputText("The huge bull-man is not one to look a gift fuck in the mouth.  Without bothering to take you in, bar a long wet sniff of your " + this.hairDescript() + ", he sticks his cock between your ass cheeks.  He grunts as he forces his head past your sphincter, squirting pre-cum into your passage as he does so.  Your body is incapable of clenching instinctively against the invasion, but there is no escaping how huge the dong feels as he impatiently forces your ass open.", false);
            this.player.buttChange(60, true, true, false);
            if (this.player.analCapacity() < 60) this.outputText("  The giant cock stretches you out painfully, and everything else blots out as your body attempts to accommodate the beast.  As he begins to thrust more of his length up you, he grunts and beads more of his drugged pre-cum, lubricating your anus.  This thankfully makes his cock easier to take, but also increases the pace of his thrusting as your hole becomes more receptive to it.", false);
            //V loose/Buttslut: 
            else this.outputText("  Your well-worn ass is a perfect fit for the giant cock and accepts it eagerly, every bit as welcoming as a moist vagina.  As the minotaur rubs against your tender inner walls he grunts and beads more of his drugged pre-cum, turning your hole into a helplessly wet, clenching ass cunt.", false);
            this.outputText("  You begin to pant from exertion and the overpowering sensation of the fuck as you feel first one ring and then a second push past your sphincter, then out again, then in and out as the minotaur picks up the pace.  As rut takes hold of him, he picks you up by your arms and uses his strength to force you up and down his dick, using you as a cocksleeve to sate his animalistic desires.  Your ass is slick with his pre-cum by now however, and the drugged slime has bumped you upwards into a hazy high; only the height of his downward thrust when his dick is almost completely buried in your bowels brings you out of it, the painful intensity of it dragging you down of your cloud with a gasp.\n\n", false);

            this.outputText("Eventually the minotaur's balls swell against your " + this.buttDescript() + " and with a long, satisfied moo, he reaches his peak, holding you down so his cum jets forth deep inside you.  The sensual high it has already instilled in you amplifies by tenfold as the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your anus helplessly milking the creature for all it can get.\n\n", false);

            this.outputText("The minotaur holds onto you until he has finished spurting his last into you, before abruptly setting you back down on your frozen feet, pulling his slimy, receding member out of your abused anus and with a satisfied snort, takes his leave.  You feel his cum drooling out of you and down your " + this.hipDescript() + ", but in your hazy, druggy state the feeling is almost sensual.", false);
            //(Addict: 
            if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] > 0 || this.player.findPerk(PerkLib.MinotaurCumAddict) >= 0) this.outputText("  You're incredibly frustrated that you can't clench yourself and hold the magical substance deep inside you, so you can savor the wonderful, soft elation it blossoms inside of you for as long as you can.", false);
            this.outputText("\n\n", false);

            this.outputText("Eventually, after another thirty or so minutes of being forced to stand there and savor the cum trickling down your legs, you find with great relief you can begin to move your fingers again. With some effort you manage to work power into each corner of your body and finally shake free of the basilisk's curse; quickly, you shake the aching out of your " + this.player.legs() + " and re-dress before anything else finds you and woozily begin to make your way back down the mountain, trying to ignore the feeling of ooze dripping out of you.", false);
            this.player.orgasm();
            this.player.slimeFeed();
        }
        //Female: 
            ...
            this.outputText("\"<i>Get caught with our pants down by a basilisk, did we slut?</i>\" she purrs.  \"<i>Well, not to worry.  I'll take good care of you.</i>\"  You somehow doubt her good intentions, and your suspicions are confirmed when, after completing her sauntering circumference of you, she shoves your bottom half as hard as she can. You fall slowly like a wooden board; being unable to brace yourself, the sensation of dropping is horrible, and, once your skull connects with the ground, painful.  Before you can clear your swimming vision you feel the goblin's soft, dense weight on your chest, and then her eager lips upon yours, sucking and tonguing every inch of your frozen mouth that she can reach.  A tingling sensation spreads from your mouth downwards as her pheromone-laced lipstick takes effect.  By the time she has finished with your mouth and worked her way downwards, your paralyzed body feels warm and receptive, and your " + this.vaginaDescript(0) + " is wet with need.  You feel small fingers caressing your mons before slipping their way inside, circling your " + this.clitDescript() + " before testing your depths.  You wish you could cringe away or even thrust yourself forward to try and make the goblin better address your growing need, but that is impossible; you stare in front of you, petrified, as the fingers continue their lazy tour of your sex until your juices are running like a river in spring.\n\n", false);

            this.outputText("\"<i>You are well up for this, aren't you hun?</i>\" coos the goblin as she begins to flick your sopping clit, sounding delighted with the involuntary heat she has stoked within you.  \"<i>It's such a shame that I don't have the big cock you obviously want and need.  Don't worry though; I've got the next best thing!</i>\"  You hear her rummage around in her satchel, which is followed by the sounds of her wetly sucking on something with obvious enthusiasm.  You shrink inwardly as you make a strong guess as to what she's holding and what she intends to do with it.  Sure enough, a moment later you feel a dildo pushing unceremoniously into your " + this.vaginaDescript(0) + ".", false);
            this.player.cuntChange(30, true, true, false);
            //[(tight)
            if (this.player.vaginalCapacity() < 30) this.outputText("  You didn't get the wet end, and you groan as you feel the thing begin to puff up as it reacts with your eager juices until it is pushing almost painfully against your inner walls.", false);
            else this.outputText("  You didn't get the wet end, and you groan as you feel the thing begin to puff up as it reacts with your eager juices until it fills your well-used passage almost perfectly.", false);
            this.outputText("  You feel the goblin crawl on top of you, pulling the dildo upwards so it rubs against your " + this.clitDescript() + " and then with a squealing giggle impales herself on the other end.  Her lustful movements translate through the artificial cock buried in your dripping sex, sending tremors and waves of pleasure to your very core.  A gush of female fluid splatters over your own sex as the goblin works herself right down the shaft connecting you until she is squelching against you wetly.  The goblin twists, grinding and scissoring her thighs, the hard bud of her clit rubbing back and forth over your own.  By now your passage feels as stuffed as it ever has been, crammed totally full of the squishy expanded double-dong.  Every motion the tiny slut makes is amplified directly into the fuck-stick plugging your " + this.vaginaDescript(0) + ".  Judging by how wonderful it feels rubbing and twisting against your sensitive walls, the aphrodisiac it is leaking into you is definitely having an effect.  You whine at the goblin twists herself and her dildo around, rubbing and riding you so effectively, the involuntary tide of erotic memories building again...\n\n", false);

            this.outputText("After cumming over and over again in tandem with the dominant little slut she eventually takes mercy upon you, leaving you lying in a pool of mingled girl cum, dazed by the hypnotic backwash and the drugs which have left you so hazy and sensitive.  After another ten or twenty minutes of being forced to lie there and marinate in your own shameful memories, you find with great relief you can begin to move your fingers again.  Eventually with some effort you manage to work power into each corner of your body and finally shake free of the basilisk's curse; quickly, you pick yourself up and redress before anything else finds you and woozily begin to make your way back down the mountain.  The smell of horny goblin on you is a lingering reminder of what just happened to you.\n\n", false);
    ...
    private basiliskAdvantageMinotaur(): void {
        ...
        this.outputText("Eventually the minotaur's balls swell against your " + this.buttDescript() + " and with a long, satisfied moo, it reaches its peak, holding you down so its cum jets forth deep inside you.  ", false);
        if (this.player.gender == 1) this.outputText("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously ejaculate, ropes of your own cum spattering on the ground and your own nerveless " + this.player.feet() + ".", false);
        else if (this.player.gender == 2) this.outputText("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your juices spattering your thighs and calves.", false);
        else if (this.player.gender == 3) this.outputText("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, ropes of your own jizz spattering on the ground and girl cum dripping down your thighs.", false);
        else this.outputText("The sensual high it has already instilled in you amplifies by tenfold as the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your anus helplessly milking the creature for all it can get.", false);
        this.outputText("\n\n", false);
    ...
    private driderPCEggLaysBasilisk(): void {
        this.spriteSelect(75);
        this.clearOutput();
        this.outputText("Leaning back on your carapace-clad abdomen, you try and look over your defeated opponent.  Even slumped over in ");
        if (this.monster.HP < 1) this.outputText("exhaustion");
        else this.outputText("arousal");
        this.outputText(", the basilisk is still striving to meet your eyes with its own.  Clenching your teeth, you avert your eyes and stomp forward blindly, colliding roughly with the spined lizard, knocking him flat on his back on the rocky ground.");
        ...
        if (this.player.hasVagina()) this.outputText("already-dripping pussy");
        ...
        this.outputText(" up against his half-open mouth, smearing your free-running juices across his scaled face.  You wait a few seconds, just enjoying the feeling of his hot breath against your oversensitive sex, before you state your intentions in an imperious tone.  \"<i>Lick.</i>\"");
        ...
        if (this.player.hasCock()) this.outputText("wrap tightly around your waiting prick");
        else this.outputText("burrow deep into your needy slit");
        this.outputText(".  You coo in delight at the sensation, and begin to grind yourself against his face as you wait for your egg-laying to begin.");
        this.outputText("\n\nThe monster's stomach bloats as a final rush of goo fills his bowels in preparation for your ");
        if (this.player.fertilizedEggs() == 0) this.outputText("un");
        this.outputText("fertilized orbs, providing you with a moment's warning before the ecstasy-inducing ripple of an egg sliding deep into him rolls your eyes backwards in pleasure.  You twitch and squirm atop the bound basilisk, mashing your ");
        if (this.player.hasCock()) this.outputText("dick");
        else this.outputText("cunt");
        this.outputText(" against his face while your egg-laden tube pumps away, each pulse forcing another egg through his squeezing ring and sending trills of intoxicating euphoria up your spine.  You lose yourself in the act, your heartbeat racing in time with contractions of your ovipositor, spraying ");
        if (this.player.hasCock()) this.outputText("semen");
        else this.outputText("femcum");
        this.outputText(" across your partner's face and neck.");
        ...
        //(if corruption >= 50 
        if (this.player.cor >= 50) this.outputText(" before heading off towards your camp. The harpies will probably free him from your webs... eventually.");
        else this.outputText(".  You reach up and slice him free from the webbing, carefully placing him on the ground below.  The blindfold stays, though.  You're kind, not stupid.  With your good deed for the day complete, you gather yourself up and head back to camp.");
    ...
    private layBeeEggsInABasilisk(): void {
        ...
        if (this.player.isTaur() || this.player.gender == 0) this.outputText("\n\nYou drool messily on your hand and cup it to the basilisk's hole; he hisses in displeasure - though he must have had an inkling of what was coming, the visceral sensation of having his asshole lubed for entry is probably sending his imagination careening off in reckless, tormenting anticipation.");
            ...
            if (this.player.totalCocks()) this.outputText("s");
            this.outputText("; rubbing enthusiastically, you start at the base but end in a gentle stroke at the sensitive tip.  With you doing your best to milk as much pre as you can, the viscous ooze that dribbles out of your dong");
            if (this.player.totalCocks()) this.outputText("s");
            this.outputText(" lands with soft pitters on the overgrown lizard's back, the intense mountain sun baking the gunk onto the scales.  You sigh in languor, but remember you're doing this for a reason.");
        }
        //(else if pc has vag:
        ...
        if (this.player.isTaur()) this.outputText("above");
        else this.outputText("behind");
        this.outputText(" the desperately-squirming lizard, delighting in the fear-scented air around the beast as your ovipositor slaps his cheeks in search of his tight little black hole.  Aside from the difference in feel, you know once you hit the sweet spot by the rigid jolt from the basilisk as his worst fear is confirmed.  You openly laugh this time, and enjoy the feel of the lizard squirming under you as your bulbous appendage painstakingly, slowly works its way inside his anus.  Dry and tight, you really have to force your way, but the feeling is amazing, and it just keeps getting better.  It probably doesn't feel too great for the basilisk, but that's not exactly your problem.  The slit at the end of your ovipositor widens, leaking golden liquid, and the squirming basilisk halts its struggles and relaxes as the narcotics in the slime work their way into the bloodstream.  You begin humming in a fashion similar to the giant bees you now resemble as you enjoy the blissful sensation of the eggs progressing through the long tubular organ, stretching the reptile's pucker to accommodate your ");
        if (this.player.fertilizedEggs()) this.outputText("fertilized offspring");
        else this.outputText("burdensome load");
        this.outputText(".");
        ...
        if (this.player.isTaur() || this.player.gender == 0) {
            ...
            //[(U)
            if (this.player.gender == 0) this.outputText("no needy cock or cunt demanding a caress - except the basilisk's, and you care not a bit about that one.");
            else if (this.player.isTaur()) this.outputText("even if you wanted to reach your genitals, your arms resolutely refuse to bend that way, so you focus on the slippery, silky feelings travelling along your ovipositor.");
        }
        if (this.player.hasCock()) {
            this.outputText("\n\nYour seeping member");
            if (this.player.cockTotal() > 1) this.outputText("s");
            this.outputText(" demand");
            if (this.player.cockTotal() == 1) this.outputText("s");
            this.outputText(" attention, and you don't deny them, as you resume the pleasurable stroking you started earlier.  More white cream leaks out, landing on the earth and the basilisk's back.  The combined titillation of your ovipositor massaging the overgrown lizard's anus from the inside out and the stroking of " + this.sMultiCockDesc() + "  is more than your overly-sensitised body can handle for long.  Soon, the tell-tale feeling at the base of your spine ");
            if (this.player.balls > 0) this.outputText("and inside your balls ");
            this.outputText("is more than you can take, and ropes of thick semen coat the basilisk from the cheeks of its ass to the back of its head.  You give a guttural groan as your orgasm hurries the last of your eggs up the long black organ that is your ovipositor to be deposited into the tightly packed cavern that you've made the basilisk's intestines into.");
        }
        //(no-horse vag: 
        else if (this.player.hasVagina()) {
