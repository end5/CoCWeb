...
export class HighMountains extends BaseContent {
    ...
    public exploreHighMountain(): void {
        ...
        if (this.flags[kFLAGS.PC_PROMISED_HEL_MONOGAMY_FUCKS] == 1 && this.flags[kFLAGS.HEL_RAPED_TODAY] == 0 && HighMountains.rand(10) == 0 && this.player.gender > 0 && !kGAMECLASS.helScene.followerHel()) {
            kGAMECLASS.helScene.helSexualAmbush();
            return;
        }
        //Gats xmas adventure!
        if (HighMountains.rand(5) == 0 && this.player.gender > 0 && this.isHolidays() && this.flags[kFLAGS.GATS_ANGEL_DISABLED] == 0 && this.flags[kFLAGS.GATS_ANGEL_GOOD_ENDED] == 0 && (this.flags[kFLAGS.GATS_ANGEL_QUEST_BEGAN] == 0 || this.player.hasKeyItem("North Star Key") >= 0)) {
            kGAMECLASS.gatsSpectacularRouter();
            return;
        }
        //Minerva
        if (this.flags[kFLAGS.DISCOVERED_HIGH_MOUNTAIN] % 8 == 0 && this.flags[kFLAGS.MET_MINERVA] < 4) {
            this.minervaScene.encounterMinerva();
            return;
        }
        //25% minotaur sons!
        if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00326] >= 3 && HighMountains.rand(4) == 0 && this.player.hasVagina()) {
        ...
        //Sophie
        if (chooser == 2) {
            if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00282] > 0 || this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00283] > 0 || kGAMECLASS.sophieFollowerScene.sophieFollower()) {
                this.outputText("A harpy wings out of the sky and attacks!", true);
                this.startCombat(new Harpy());
                this.spriteSelect(26);
            }
