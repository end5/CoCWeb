...
export class NagaScene extends BaseContent {
    ...
    public nagaEncounter(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.Naga) < 0) this.player.createStatusAffect(StatusAffects.Naga, 0, 0, 0, 0);
        ...
        if (this.player.lowerBody == LOWER_BODY_TYPE_NAGA) {
            ...
            if (this.player.gender == 1 || (this.player.gender == 3 && NagaScene.rand(2) == 0)) {
                ...
                if (this.player.cockArea(0) > 35) {
                    ...
                    //[lots of jizz]
                    if (this.player.cumQ() > 250) this.outputText("Her cheeks bulge with the volume and she struggles to swallow as much of it as she can. ", false);
                    //[JIZZOFTHEGODS]
                    if (this.player.cumQ() > 1000) this.outputText("Her cheeks bulge out with the sheer volume of your cum and she is forced to release you from her mouth to avoid drowning in your seed. ", false);
                ...
                //[Cock isn't too big]
                    ...
                    if (this.player.cockTotal() == 1) this.outputText("it starts ", false);
                    else this.outputText("they start ", false);
                    ...
                    //[player has one dick]
                    if (this.player.totalCocks() == 1) this.outputText("Gently she starts to stroke the length of your " + this.cockDescript(0) + " with one hand while circling the tip with the other. ", false);
                    //[player has two dicks]
                    if (this.player.totalCocks() == 2) this.outputText("She takes one in each of her hands and gently strokes them up and down. Every few seconds she stops at the tip to rub her palms over them. ", false);
                    //[player has three or more dicks]
                    if (this.player.totalCocks() >= 3) this.outputText("She takes one in each hand, stroking them slowly and making sure to pay attention to the tip. Every so often she switches to a different dick to make sure that each and every one of your throbbing cocks has some love given to them. ", false);
                    ...
                    //[player has one dick] 
                    if (this.player.totalCocks() == 1) this.outputText("She carefully lines it up with your member and starts to tease the tip before gently inserting the first few inches. ", false);
                    //[player has two dicks]
                    if (this.player.totalCocks() == 2) this.outputText("She carefully lines it up with both of your members and starts to tease their tips before gently inserting the first few inches. ", false);
                    //[player has three or more dicks]
                    if (this.player.totalCocks() >= 3) this.outputText("She carefully lines it up with one of your members and starts to tease the tip before gently inserting the first few inches. ", false);
                    ...
                    //[lots of jizz]
                    if (this.player.cumQ() > 250) this.outputText("You quickly fill her with your seed to the point where she overflows, leaving her pussy dripping with semen afterwards.", false);
                    //[JIZZ, JIZZ EVERYWHERE] 
                    else if (this.player.cumQ() > 1000) this.outputText("Her stomach quickly swells from the sheer volume of seed pumped into her. The sperm that her womb is unable to hold starts to gush out from her stuffed cunt.", false);
            ...
            //chick secks
                ...
                //(if player has fangs)
                if (this.player.faceType == FACE_SNAKE_FANGS) this.outputText("carefully nibbling at it so as to not break the skin.  ", false);
                else this.outputText("nibbling gently at it.  ", false);
            ...
            //Genderleast
                ...
                //(If tongue is forked)
                //9999 - BLAH!
                if (false) this.outputText("You move your tongue faster and deeper into her pussy, your forked tongue reaching deeper than any human tongue ever could.  ", false);
                //(Otherwise)
                else this.outputText("You move your tongue faster and faster into her pussy, reaching as far in as you possibly can.  ", false);
    ...
    private gooNagaRape(): void {
        ...
        if (this.monster.HP < 0) this.outputText("weak ", false);
        else this.outputText("flushed ", false);
        ...
        //(if corrupt)
        if (this.player.cor > 50) this.outputText("but immediately after, you decide that this is going to be too much fun to not push on.", false);
        //(if not corrupt)
        else this.outputText("but a nagging voice at the back of your head urges you to push further.", false);
        ...
        if (this.player.totalCocks() > 0) {
            if (this.player.cockTotal() == 1) this.outputText("your " + this.cockDescript(0), false);
            else this.outputText("one of your " + this.multiCockDescriptLight(), false);
        ...
        //(if herm) 
            ...
            if (this.player.cockTotal() == 1) this.outputText("your " + this.cockDescript(0), false);
            else this.outputText("your " + this.multiCockDescriptLight(), false);
        ...
        if (this.player.gender == 1) {
            ...
            //(normal) 
            if (this.player.cumQ() < 250) this.outputText("you come all over the naga's stomach.", false);
            else if (this.player.cumQ() < 1000) this.outputText("you come all over the naga's face and stomach.", false);
            //(if THE LEVEYS HAVE BROKE RUN FOR THE HILLS) 
            else this.outputText("you come violently, covering the naga in a layer of your sperm.", false);
        }
        //(if female)
        else if (this.player.gender == 2) {
            this.outputText("You shriek in delight as wave after wave of orgasms rush over you", false);
            //(if squirter)
            if (this.player.vaginas[0].vaginalWetness == VAGINA_WETNESS_SLAVERING) this.outputText(", your girlcum gushing out and pooling on the stomach of the naga", false);
            this.outputText(".", false);
        }
        //(if herm)
        if (this.player.gender == 3) {
            this.outputText("You scream in ecstasy as you hit your peak, your girlcum ", false);
            if (this.player.vaginas[0].vaginalWetness < VAGINA_WETNESS_SLAVERING) this.outputText("leaking out", false);
            ...
            //(normal)
            if (this.player.cumQ() < 250) this.outputText(" spurting onto her stomach as well.", false);
            //(lots of jizz)
            else if (this.player.cumQ() < 1000) this.outputText(" spurting onto her stomach and face.", false);
    ...
    private nagaVictoryMale(): void {
        ...
        if (this.player.totalCocks() > 0) {
            ...
            if (this.player.lowerBody == LOWER_BODY_TYPE_CENTAUR) {
                ...
                //[extra cum]
                else if (this.player.cumQ() < 1000) this.outputText("you fill her insides with huge spurts of cum, ", false);
                //[SUPER CUM]
                else this.outputText("you explode with huge fountains of cum spurting from your " + this.cockDescript(0) + ", gushing out from where the two of you are joined to rain on the sands below, ", false);
            ...
            //Uncorrupted  
                ...
                if (this.player.cockArea(0) <= this.monster.vaginalCapacity()) {
                    ...
                    //[normal cum load]
                    if (this.player.cumQ() < 250) this.outputText("shooting thick ropes of sperm deep inside her as the both of you wail with excitement.", false);
                    //[lots of cum]
                    else if (this.player.cumQ() < 500) this.outputText("pumping large amounts of cum deep inside her as both your voices rise in screams of pleasure.", false);
                    //[Spermageddon]
                    else this.outputText("your " + this.cockDescript(0) + " erupting, blasting a violent torrent of semen deep inside her petite frame, filling her up completely as both your voices rise in screams of pleasure.", false);
                }
                //[If cock too large]
                else {
                    ...
                    //[normal cum load]
                    if (this.player.cumQ() < 250) this.outputText("filling her mouth with your warm seed.", false);
                    //[lots of cum]
                    else if (this.player.cumQ() < 500) this.outputText("pumping her mouth full of your sticky seed, some of which she is forced to swallow to avoid choking.", false);
                    //[Oh God, it just keeps coming out, help me!]
                    else this.outputText("easily flooding her mouth with your seed. Your seed gushes from her nose, with still more rushing down her throat. She gags and chokes a bit, a large amount of your seed leaking out onto her chest.", false);
                    this.outputText("  The naga removes your " + this.cockDescript(0) + " from her mouth", false);
                    //[lots of cum OR more]
                    if (this.player.cumQ() > 500) this.outputText(", the last sticky blasts of your semen splashing across her face,", false);
                    this.outputText(" and looks to you for instruction. You simply nod and she understands, ", false);
                    //[normal cum load]
                    if (this.player.cumQ() < 250) this.outputText(" gulping down the warm sperm in her mouth.", false);
                    //[lots of cum OR more]
                    else this.outputText(" doing the best she can to swallow her mouthful of cum.", false);
                }
            ...
            //Corrupted (event raises fatigue by 20)
                ...
                if (this.player.cumQ() > 250) this.outputText("  Your spooge explodes from the naga's abused cunt, flooding the desert sands as you black out.\n\n", false);
                ...
                else if (this.player.cumQ() > 2000) this.outputText(" that's filled with your thick seed", false);
                this.outputText(".\n\n", false);
        ...
        if (this.player.hasVagina()) {
            ...
            if (this.player.lowerBody == LOWER_BODY_TYPE_CENTAUR) {
                ...
                if (this.monster.lust > 99) {
                    ...
                    if (this.player.gender == 3) this.outputText("  As her tail gently squeezes your body, it draws your own " + this.multiCockDescriptLight() + " up against your body, her soft underbelly stimulating the sensitive skin.", false);
                    ...
                    //(If PC is Herm i.e. Has a cock(s)){
                    if (this.player.gender == 3) this.outputText(" and your " + this.multiCockDescriptLight() + " constantly stimulated by the tail wrapped around your body", false);
            ...
            //b) Corrupted
                ...
                if (this.player.findStatusAffect(StatusAffects.MeanToNaga) < 0) this.player.createStatusAffect(StatusAffects.MeanToNaga, 0, 0, 0, 0);
                ...
                if (this.player.str > 50) {
                    ...
                    //(If high corr)
                    if (this.player.cor > 80) this.outputText("  As a parting gift, you kick sand in her face, and walk away without a care in the world.", false);
                    //(If low corr )
                    else this.outputText("  You think to yourself, \"<i>Was all that really necessary?</i>\"  But, in the end you decide that yes, it was.", false);
                }
    ...
    private nagaFUCKSJOOOOOO(): void {
        ...
        if ((this.player.findPerk(PerkLib.BimboBrains) >= 0 || this.player.findPerk(PerkLib.FutaFaculties) >= 0) && this.player.hasVagina()) {
            ...
            //[If Lactating]
            if (this.player.lactationQ() >= 50 && this.player.biggestLactation() >= 1) this.outputText("Your warm milk is leaking from your " + this.nippleDescript(0) + "s the entire time, coating your skin and soaking her hands. At one point, she withdraws a hand and licks the milk from it. You can only assume that she liked it, as she begins to focus even more on your breasts.\n\n", false);
            //-------
            ...
            //----
            //[If Lactating]
            if (this.player.lactationQ() >= 50 && this.player.biggestLactation() >= 1) this.outputText("She shares a mouthful of your delicious milk with you.\n\n", false);
            //------
        ...
        //too)
            ...
            //(If defeated by lust)
            if (this.player.lust > 99) this.outputText("You fall to your knees, your body shaking with desire.\n\n", false);
            //(If defeated by KO)
            else this.outputText("You fall to your knees, your battered body unable to hold itself up any longer.\n\n", false);
            ...
            //(if defeated by lust)
            if (this.player.lust > 99) this.outputText("all your resistance seems to do is make your " + this.cockDescript(0) + " harder.", false);
            //if defeated by KO)
            ...
            if (this.player.cor > 75) this.outputText("  The thought somehow makes your " + this.cockDescript(0) + " throb even more.", false);
            this.outputText("\n\n", false);
            ...
            if (this.player.cockArea(0) <= this.monster.vaginalCapacity()) {
                ...
                else this.outputText("absolutely engorges her, with the excess spurting out in torrents from her pussy.", false);
            }
            //(Cock too big)
            else {
                ...
                //OH SHIT MOAR CUM
                //(Normal)
                if (this.player.cumQ() < 250) this.outputText("The naga manages to hold the warm cum within her mouth, only causing her cheeks to bulge a little.", false);
                //(Lots of cum)
                else if (this.player.cumQ() < 1000) this.outputText("The naga's cheeks balloon as her mouth is filled with your hot, gooey sperm, forced to swallow some of it as her cheeks reach their full capacity.", false);
                //(Cumception, we need to go deeper)
                else this.outputText("Hot semen blasts from the naga's nose as her mouth is completely filled in seconds. Much of your warm cum is forced down her throat and into her stomach, causing it to distend slightly within her thin frame.", false);
        ...
        //b) female //http://nekomimichan.org/mg/src/12844585127.jpg
            ...
            //(if uncorrupted)
            else this.outputText("and while you know you probably won't like it, your " + this.vaginaDescript(0) + " says otherwise.  ", false);
        ...
        //d) genderless
            ...
            if (this.player.cor < 50) this.outputText("You try to turn your head away, but she forcefully shoves it down into her soft folds.  ", false);
    ...
    public nagaPlayerConstrict(): void {
        ...
        if (NagaScene.rand(this.player.spe + 40) > this.monster.spe) {
            ...
            this.monster.createStatusAffect(StatusAffects.Constricted, 1 + NagaScene.rand(4), 0, 0, 0);
        }
        //Failure
        else {
            //Failure (-10 HPs) -
    ...
    private eggUpANagaSpiderLike(): void {
        ...
        if (this.player.gender > 0) {
            this.outputText(", ");
            if (this.player.hasCock()) this.outputText("jizz");
            if (this.player.gender == 3) this.outputText(" and ");
            if (this.player.hasVagina()) this.outputText("femcum");
    ...
    private beePositANagaPlease(): void {
        ...
        if (this.player.hasCock()) this.outputText("  [EachCock] erupts to splatter her with white");
        if (this.player.hasCock() && this.player.hasVagina()) this.outputText(" and y");
        else if (this.player.hasVagina()) this.outputText("  Y");
        if (this.player.hasVagina()) {
            this.outputText("our [vagina] ");
            if (this.player.wetness() < 3) this.outputText("drools");
            else if (this.player.wetness() < 5) this.outputText("gushes");
            else this.outputText("splatters");
            this.outputText(" clear femcum over the Naga's tight belly");
            if (this.player.hasCock()) this.outputText(" as well");
        }
        ...
        if (this.player.eggs() > 30) this.outputText(" from the outside at least.  You stuffed her so full that she looks positively pregnant, bulging to a picture of utter fertility");
        ...
        if (this.player.fertilizedEggs() == 0) this.outputText("egg-bearer");
        else this.outputText("mother");
