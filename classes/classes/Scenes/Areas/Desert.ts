...
export class Desert extends BaseContent {
    ...
    public exploreDesert(): void {
        ...
        if (this.player.level >= 4 && this.player.exploredDesert % 15 == 0 && this.flags[kFLAGS.DISCOVERED_WITCH_DUNGEON] == 0) {
        ...
        if (this.flags[kFLAGS.PC_PROMISED_HEL_MONOGAMY_FUCKS] == 1 && this.flags[kFLAGS.HEL_RAPED_TODAY] == 0 && Desert.rand(10) == 0 && this.player.gender > 0 && !kGAMECLASS.helScene.followerHel()) {
            kGAMECLASS.helScene.helSexualAmbush();
            return;
        }
        if ((this.player.exploredDesert == 20 && this.player.findStatusAffect(StatusAffects.TelAdre) < 0) || (Desert.rand(20) == 0 && this.player.statusAffectv1(StatusAffects.TelAdre) == 0)) {
            kGAMECLASS.telAdre.discoverTelAdre();
            return;
        }
        if (this.sandWitchScene.pregnancy.event == 2 && Desert.rand(4) == 0) {
            if (this.flags[kFLAGS.EGG_WITCH_TYPE] == PregnancyStore.PREGNANCY_DRIDER_EGGS) this.sandWitchScene.sammitchBirthsDriders();
            else this.sandWitchScene.witchBirfsSomeBees();
            return;
        }
        //Ant colony debug chances
        if (this.player.level >= 5 && this.flags[kFLAGS.ANT_WAIFU] == 0 && (this.player.exploredDesert % 8 == 0) && this.flags[kFLAGS.ANTS_PC_FAILED_PHYLLA] == 0 && this.flags[kFLAGS.ANT_COLONY_KEPT_HIDDEN] == 0) {
            this.antsScene.antColonyEncounter();
            return;
        }
        //int over 50?  Chance of alice encounter!
        if (Desert.rand(4) == 0 && this.player.inte > 50 && this.flags[kFLAGS.FOUND_WIZARD_STAFF] == 0) {
