...
export class D3 extends BaseContent {
    ...
    public discoverD3(): boolean {
        if (this.flags[kFLAGS.D3_DISCOVERED] == 0 && this.player.hasKeyItem("Zetaz's Map") >= 0 && this.player.level >= 10 && D3.rand(5) == 0) {
    ...
    private roomofmirrorsRoomFunc(): boolean {
        ...
        if (this.flags[kFLAGS.D3_MIRRORS_SHATTERED] == 1) this.outputText(" now shattered,");
    ...
    private magpiehallsRoomFunc(): boolean {
        if (this.flags[kFLAGS.D3_JEAN_CLAUDE_DEFEATED] == 0) {
            if (this.flags[kFLAGS.D3_ENTERED_MAGPIEHALL] == 0) {
                ...
                if (this.flags[kFLAGS.BENOIT_AFFECTION] == 100) this.outputText("  This can only be the hall that " + this.getGame().bazaar.benoit.benoitMF("Benoit", "Benoite") + " once worked in.");
                ...
                if (this.player.canFly()) this.outputText(", even if you tried to fly it");
                this.outputText(". Wouldn’t you? You can’t outrun the gaze of a thousand basilisks... could you?");
                if (this.player.hasKeyItem("Laybans") >= 0) this.outputText("  You take the Laybans out of your pouch and hold them up against the glass. It’s exactly as you hoped - they are made of the same material, and are almost certainly what the demons wear when they themselves interact with the basilisks. They would surely help you get across the hall, if you were crazy enough to try.");
            }
            else {
                ...
                if (this.player.hasKeyItem("Laybans") < 0) this.outputText("  Are you going to try this?");
                else this.outputText("  You take the Laybans out of your pocket, turning them around in your hands as you consider. Are you going to try this?");
            }
    ...
    private fallbackFromMagpieHallS(): void {
        ...
        if (this.player.hasKeyItem("Laybans") < 0 && this.player.inte >= 50) this.outputText("  Surely the demons themselves are not immune to the basilisks’ glares - the darkened screen is proof of that. How do they interact with the creatures, then? Maybe if you keep poking around, you might find an answer.");
    ...
    private westwalkRoomFunc(): boolean {
        ...
        if (this.flags[kFLAGS.D3_STATUE_DEFEATED] == 0) this.outputText(" see a massive statue with an immense hammer");
        else this.outputText(" a mound of rubble, the scattered remains of the animated statue that you slew");
    ...
    private eastwalkRoomFunc(): boolean {
        ...
        if (this.flags[kFLAGS.D3_STATUE_DEFEATED] == 0) this.outputText(" a massive statue with an immense hammer.");
        else this.outputText(" a mound of rubble, the scattered remains of the animated statue that you slew.");
        return false;
    }
