...
export class DriderIncubus extends AbstractSpiderMorph {
    ...
    public get goblinFree(): boolean { return this._goblinFree; }
    ...
    private performPhysicalAttack(): void {
        if (this._combatRound >= 3 && (this._combatRound % 6 == 0 || this._combatRound == 3)) this.stunningSpear();
        else {
            var opts: any[] = [this.bite, this.spiderMorphWebAttack, this.kick, this.kick, this.doubleStrike, this.doubleStrike];
            opts[DriderIncubus.rand(opts.length)]();
        }
    }

    private performLustAttack(): void {
        if (this._combatRound >= 3 && (this._combatRound % 6 == 0 || this._combatRound == 3)) this.constrictingThoughts();
        else {
            var opts: any[] = [this.arouseSpell, this.arouseSpell];
            if (this.player.findStatusAffect(StatusAffects.TaintedMind) < 0 && !this._seenResolute) opts.push(this.taintedMind);
    ...
    private goblinAI(): void {
        var opts: any[] = [this.goblinHandjob, this.goblinTongueGrapple, this.bootyTwerking, this.webNipplechain];
    ...
    private gainHpAndLust(): void {
        ...
        if (this._hpGains == 0) this.outputText("<i>“You won’t defeat me so easily!”</i>");
        else if (this._hpGains == 1) this.outputText("<i>“I can keep this up longer than you, mortal!”</i>");
        else if (this._hpGains == 2) this.outputText("<i>“So stubborn! Lethice take you!”</i>");
        else this.outputText("<i>“Why won’t you give in?!”</i>");
        ...
        if (this.lust > 65) {
            if (this._goblinFree) this.outputText(" His dick is rigid and bouncing, so hard it looks like it could go off at any moment.");
            else this.outputText(" His balls are tensing underneath the goblin, and he keeps moaning under his breath.");
    ...
    private spearStrike(): void {
        ...
        var damage: number = (this.str + this.weaponAttack) * 0.40;
    ...
    private bite(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.Stunned) >= 0 || (this.player.spe <= 1 && this.player.findStatusAffect(StatusAffects.Web) >= 2)) {
            ...
            if (this.player.str <= 25) this.outputText(" It’s getting harder just to remain upright.");
            else if (this.player.str <= 1) this.outputText(" You don’t think you can stand any longer, let alone resist.");

            this.outputText("\n\nWhen he pulls out, he’s smiling and a little flushed. <i>“");
            if (this.player.str > 25) this.outputText("That should do.");
            else this.outputText("Soon you won’t have the strength to resist.");
            this.outputText("”</i>");
        }
        else {
            ...
            else if (this.combatMiss() || this.combatFlexibility()) this.outputText(" You avoid his venomous strike!");
            else {
                //Hits
    ...
    private kick(): void {
        ...
        if (this.combatMisdirect()) {
            ...
            var damage: number = (this.str + this.weaponAttack + 25) - DriderIncubus.rand(this.player.tou);
    ...
    private doubleStrike(): void {
        ...
        var damage: number = this.str + this.weaponAttack + 10 - DriderIncubus.rand(this.player.tou);
    ...
    private arouseSpell(): void {
        ...
        if (this.player.cocks.length > 0 || this.player.vaginas.length > 0) {
            ...
            if (this.player.cocks.length > 0) this.outputText(" [cocks]");
        ...
        if (this.player.lust < 100) this.outputText(", but the effect is fleeting, thankfully. You try to ignore the residual tingles. You can’t afford to lose this close to your goal!");
    ...
    private taintedMind(): void {
        ...
        // 9999
        if (this.player.cor <= 33) this.outputText(" What did he mean about fighting like a demon?");
    ...
    private purpleHaze(): void {
        ...
        if (this.player.findPerk(PerkLib.Resolute) >= 0 || DriderIncubus.rand(3) >= 0) {
            ...
            this.player.createStatusAffect(StatusAffects.PurpleHaze, 2 + DriderIncubus.rand(2), 0, 0, 0);
            this.player.createStatusAffect(StatusAffects.Blind, this.player.statusAffectv1(StatusAffects.PurpleHaze), 0, 0, 0);
        }
    }
    ...
    public get goblinOiled(): boolean { return this._goblinOiled; }
