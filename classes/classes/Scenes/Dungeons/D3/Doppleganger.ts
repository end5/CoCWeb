...
export class Doppleganger extends Monster {
    ...
    public mirrorAttack(damage: number): void {
        ...
        if (damage > 0 && Doppleganger.rand(8) < 6) {
            ...
            if (this.player.weaponName == "fists") this.outputText(" [weapon]");
            else this.outputText(" [weapon]s");
    ...
    private addTalkShit(): void {
        ...
        switch (this._roundCount) {
            ...
            case 2:
                ...
                if (this.player.hasCock()) this.outputText(" strokes [his] [cock]");
                else if (this.player.hasVagina()) this.outputText(" slides two fingers into [his] [vagina] and gently frigs [himself]");
                else this.outputText(" slips a hand ");
    ...
    public constructor() {
        ...
        if (this.player.cocks.length > 0) {
            for (var i: number = 0; i < this.player.cocks.length; i++) {
                this.createCock((this.player.cocks[i] as Cock).cockLength, (this.player.cocks[i] as Cock).cockThickness, (this.player.cocks[i] as Cock).cockType);
            }
        }

        if (this.player.vaginas.length > 0) {
            this.createVagina();
            (this.vaginas[0] as VaginaClass).vaginalLooseness = this.player.vaginas[0].vaginalLooseness;
    ...
    public get long(): string {
        ...
        if (this.player.tailType != 0) str += " " + this.player.mf("His", "Her") + " tail flicks this way and that.";
        ...
        if (this.player.biggestTitSize() >= 2) str += "It’s difficult not to notice the way the mirror image of your " + this.player.breastDescript(this.player.biggestTitRow()) + " ebbs and heaves within it.";
