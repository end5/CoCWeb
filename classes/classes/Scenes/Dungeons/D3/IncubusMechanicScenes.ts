...
import { IncubusMechanic } from "../Factory/IncubusMechanic";
...
export class IncubusMechanicScenes extends BaseContent {
    ...
    private metIncubusMechanicInD1(): boolean {
        if (this.player.findStatusAffect(StatusAffects.FactoryIncubusDefeated) >= 0) return true;
        if (this.player.findStatusAffect(StatusAffects.IncubusBribed) >= 0) return true;
        return false;
    }
    ...
    public meetAtElevator(): void {
        if (this.flags[kFLAGS.D3_MECHANIC_LAST_GREET] == this.MECHNIC_NO_SELECTION) {
            ...
            this.outputText("\n\n");
            if (this.metIncubusMechanicInD1()) this.outputText("\"<i>Once more, you bumble");
            else this.outputText("\"<i>You've bumbled");
    ...
    private payDaToll(): void {
        this.flags[kFLAGS.D3_MECHANIC_LAST_GREET] = this.MECHANIC_PAID;
    ...
    public suckIncubusDick(): void {
        this.flags[kFLAGS.D3_MECHANIC_LAST_GREET] = this.MECHANIC_SUCKED;

        this.clearOutput();

        if (this.player.gems < 500) this.outputText("You spare a glance to your depleted pouches and admit, \"<i>I guess I'll suck your dick then.</i>\"");
        else this.outputText("You shrug and smile. \"<i>I guess now is as good a time as any to suck some dick, huh?</i>\"");
        ...
        if (this.player.hasVagina()) {
            ...
            if (this.player.wetness() <= 2) this.outputText(" even more soaked than normal");
            else if (this.player.wetness() <= 4) this.outputText(" wet with fresh lust");
            else this.outputText(" sticky with lust")
    ...
    public beatDaMechanic(hpVictory: boolean): void {
        this.flags[kFLAGS.D3_MECHANIC_LAST_GREET] = this.MECHANIC_FOUGHT;

        this.clearOutput();
        this.outputText("The mechanic collapses, his overalls");
        if (hpVictory) this.outputText(" falling off his prone from, shredded.");
        else this.outputText(" hanging open as he reaches down for his exposed, dangling cock to wildly masturbate.");
    ...
    private letMechanicGo(hpVictory: boolean): void {
        this.flags[kFLAGS.D3_MECHANIC_FIGHT_RESULT] = this.MECHANIC_RELEASED;
        this.clearOutput();
        ...
        this.outputText("\n\nThe incubus");
        if (hpVictory) this.outputText(" slowly struggles to his feet");
        else this.outputText(" reluctantly stops masturbating");
    ...
    private buttfuckTheMechanic(hpVictory: boolean): void {
        ...
        if (!hpVictory) this.outputText(" throbbing hard");
        else this.outputText(" gently bobbing");
        ...
        if (this.player.cocks.length > 1) this.outputText(" one");
        else this.outputText(" it");
        this.outputText(" in hand, pumping slowly as you eye your tainted prey. He whimpers");
        if (hpVictory) this.outputText(" in need, and");
        else this.outputText(", but");
        ...
        if (this.silly()) this.outputText(" tab C with slot buttfuck");
        else this.outputText(" your eager tool with the corrupted anus");
        ...
        //Huge cum
            ...
            if (this.player.cumQ() <= 5000) this.outputText(" pudgy");
            else if (this.player.cumQ() <= 10000) this.outputText(" swollen");
            else this.outputText(" absolutely gravid");
    ...
    private rideMechanicsCock(): void {
        ...
        if (this.player.tailType != 0) this.outputText(", tail draped across his face");
    ...
    private rideMechanicsCockII(cType: number): void {
        this.flags[kFLAGS.D3_MECHANIC_COCK_TYPE_SELECTION] = cType;
        ...
        if (this.player.wetness() <= 2) this.outputText(", letting your free-flowing pussyjuice wash over it, lubricating it with your glorious girlcum");
        else if (this.player.wetness() <= 3) this.outputText(", letting your oh-so-juicy vagina drip and dribble, soaking it with your copious girlcum");
        else this.outputText(", letting your lubricant drip over it");
        ...
        if (pCuntArea >= mCockArea * 1.5) {
            ...
            if (cType == this.MECHANIC_HORZ_GOG) this.outputText(", your belly bulging with his equine shape");
            else if (cType == this.MECHANIC_DOG_COCK) this.outputText(", your lips slurping noisily about his knot");
            this.outputText(".");
        }
        ...
        if (tightFit) this.outputText(" well-stretched");
        else this.outputText(" stuffed");
        ...
        if (this.player.findStatusAffect(StatusAffects.IncubusBribed) >= 0) this.outputText(" Nothing but hentai mags for you to drain your balls with....");
        this.outputText("</i>\"");
        ...
        if (this.player.clitLength >= 5) this.outputText(" You grab hold of it in your hand and start to stroke it to the tempo your rising and falling body is setting, pumping the oversensitive organ like a dick. Every stroke sends your walls into fits of fluttering ecstasy. It feels so good that you don't think you'll be able to stop until you're cumming and gushing all over your demonic lover.");
        else this.outputText(" You thumb at it, rubbing in slow circles around it. Sometimes you push on its hood. Other times you oh-so-gently squeeze at it. It feels so good that the touches drive you crazy with ecstasy, and your walls flutter excitedly around your demonic lover.");
        ...
        if (cType == this.MECHANIC_HORZ_GOG) this.outputText(", the head flaring to seal itself against your cervix");
        else if (cType == this.MECHANIC_DOG_COCK) this.outputText(", the knot popping in just in time to start bloating, locking you in place");
        ...
        if (!this.player.isPregnant()) {
            ...
            if (cType != this.MECHANIC_DOG_COCK) this.outputText(" and drip out over his balls, much to his chagrin.");
            else this.outputText(" and stretch you further.");
        }
        ...
        if (this.player.wetness() < 4) this.outputText(", even though you aren't normally a squirter");
        ...
        if (!this.player.isPregnant()) this.outputText(" insemination");
        else this.outputText(" tryst");
        this.outputText(".");
    ...
    private maleLossToMechanic(hpVictory: boolean): void {
        ...
        if (this.player.cocks.length > 1) {
            ...
            if (!allDemon) this.outputText(" The skin of your transforming length is darkening to a purplish hue, and in the wake of the magical cock-ring, nodules are slowly emerging, growing into an imitation of the cock you're still suckling.");
    ...
    private errybodyelseLossToMechanic(hpVictory: boolean): void {
        ...
        //Genderless HP Loss
            ...
            if (this.player.hasFuckableNipples()) this.outputText(" slipping inside a [nipple] to thrust just like your other hand. You work your upper and lower pussies like a woman possessed.");
            else {
                this.outputText(" pinching and tugging on");
                if (this.player.totalNipples() > this.player.totalBreasts()) this.outputText(" nipple after nipple, your myriad breastly endowments providing you with oh so many buttons to tend to");
                else this.outputText("your [nipples] one after another");
            }
        ...
        if (this.player.isNaga()) this.outputText("r coils don't get sore");
        else if (!this.player.isBiped()) this.outputText(" don't even need to support yourself anymore");
        else this.outputText(" knees don't get sore");
        ...
        if (this.player.clitLength > 5) this.outputText(", even if that makes it truly monstrous indeed");
