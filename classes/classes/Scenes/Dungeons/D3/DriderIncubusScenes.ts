...
export class DriderIncubusScenes extends BaseContent {
    ...
    public spooderbuttGetsANewCockSleeve(hpVictory: boolean, pcCameWorms: boolean): void {
        ...
        this.outputText("The floor of Lethice’s throne room isn’t as cold as you would have thought against your");
        if (hpVictory) this.outputText(" wounded");
        else this.outputText(" flushed");
        this.outputText(" cheek.");
        //HP Fork (no new PG):
        if (hpVictory) {
            this.outputText(" You don’t even remember dropping. One moment you were fighting an eight-legged demonic aberration, and the next you were face down on polished granite, your head throbbing.");
        ...
        //Lust Fork (no new PG):
            ...
            this.outputText(" You were too turned on to care when you started slinking down to the polished granite.");
            if (this.player.weapon != WeaponLib.FISTS) this.outputText(" Your [weapon] clattered dully next to you, forgotten next to your all-consuming need.");
        ...
        if (hpVictory) this.outputText(" breath through such a thick covering.");
        else this.outputText(" pant needily under such a thick covering.");
        this.outputText(" If only you could move!");
        this.outputText("\n\nKinarial easily passes you to his human hands, tucking you under a shoulder");
        if (!this.player.isTaur() && !this.player.isDrider()) this.outputText(" and across the top portion of his spidery back");
        if (m.goblinFree) this.outputText(" while his goblin slave remounts his dick");
        this.outputText(". <i>“I must confess, I wondered what Lethice would choose to do with such a prize. Would she try to turn you into a loyal demonic footsoldier or break you into a beast of burden, suitable only to serve our whims and salacious desires?”</i> He skitters through the throng of spectating demons. <i>“Time to find out.”</i>");
    ...
    public spooderbuttGetsANewCockSleeveII(): void {
        ...
        this.outputText("The way you’re being held, you can’t get a proper look at the Demon Queen. You can barely see anything past the dick-mounted goblin’s heaving tits and slow-grinding body, and what you can see is mostly floor, a little bit of throne, and petite pink feet with black-painted nails. Those toes wiggle happily. The owner clicks her heels together, revealing bone-spikes that keep her soles arched like high-heels.");
        this.outputText("\n\n<i>“I must say, I’m surprised, Kinariel. I did not think you’d manage to vanquish such a troublemaker all by your lonesome.”</i> Long-nailed fingers dig into your chin and twist your head around for a better view. <i>“And I’m disappointed. I had hoped you would prove a decent challenge. It has been so long since I’ve gotten to let loose in a fight.”</i> She tuts at you. <i>“Such a shame. Still, you have the aura of a breeder about you. You may yet serve me. Kinariel, put " + this.player.mf("him", "her") + " down.”</i>");
    ...
    public spooderbuttGetsANewCockSleeveIII(): void {
        ...
        this.outputText("Your bindings are cut from you by something short and sharp, the point pressing hard enough that you worry it will score your [skin]");
        if (this.player.skinType != 0) this.outputText(" through your [skinFurScales]");
        this.outputText(". It doesn’t. Lethice is too skilled to physically damage what she sees as her future property. To think, you believed you’d be able to defeat her all by yourself, and now you’re on the floor, unable to stop her from handling your supine form as she will.");
        ...
        if (this.player.humanScore() < 5) this.outputText(", no matter how mutated");
        this.outputText(". You’re drowning in those infinitely deep eyes, aware for once that even a champion cannot hold a candle to an immortal devourer of souls.");
        ...
        if (this.player.cor >= 50) this.outputText(", you’ve let yourself become corrupted");
        else this.outputText(", you’ve been sorely tempted by the rampant corruption of these lands");
        this.outputText(".");
        if (this.player.findStatusAffect(StatusAffects.FactoryOverload) >= 0) this.outputText(" You couldn’t even save one of the last pure spirits in this place, Marae.");
        this.outputText(" It’s only a matter of time until Tel’Adre falls,");
        if (this.flags[kFLAGS.FARM_CORRUPTION_STARTED] == 0) this.outputText(" Whitney’s farm is consumed");
        else this.outputText(" Lethice thanks you for preparing Whitney’s farm");
        this.outputText(", or worse. You go limp, defeated");
        if (this.player.lust >= 90) this.outputText(" and horny");
        this.outputText(", but unwilling to admit as much to your long-time foe.");
        this.outputText("\n\n<i>“Good " + this.player.mf("boy", "girl") + ".”</i> Lethice strokes your hair, ignoring the tears at the corners of your eyes. <i>“You deserve a reward I rarely give.”</i> She breaks your gaze, allowing you to watch her lift long black skirt, almost like something a nun would wear, if a nun had holes cut in the top to display her rigid, pierced nipples. Higher and higher, the fabric ascends up her perfectly smooth legs until a dewy, pink slit is revealed. <i>“You will be permitted to lick me as a free " + this.player.mf("man", "woman") + ", before you go in the submission tanks.”</i>");
        ...
        this.outputText("\n\n<i>“If you do a good job, I’ll instruct them to maintain as much of your personality as possible.”</i> Lethice’s voice rings with as much truth as it does authority. <i>“Displease me and you shall be an empty-headed husk, fit only for reproduction.”</i> She giggles, <i>“But there’s no way a </i>good " + this.player.mf("boy", "girl") + "<i> like you would fail to please, is there?”</i>");
    ...
    public spooderbuttGetsANewCockSleeveIV(): void {
        ...
        this.outputText("Lethice’s will presses on you like a physical thing.");
        if (this.player.cor < 50) this.outputText(" No matter how repugnant the idea of eating her out might have seemed a moment ago, you find your thoughts increasingly turning to her pussy.");
        else this.outputText(" No matter what objections you might have had to the idea, you’re too horny to make even a token effort at resistance.");
        this.outputText(" What will she taste like? She’s a demon; it could be anything. Her pussy could be flavored with the sweetest strawberries or contain flavors your mortal tongue could barely comprehend.");
        ...
        if (this.player.vaginas.length > 0) {
            this.outputText(" judging by the way your [armor] is clinging to your crotch, she’s not the only one.");
            if (this.silly()) this.outputText(" You could probably drown someone in your panties.");
        }
        else if (this.player.cocks.length > 0) {
            this.outputText(" judging by the way pre is pouring out of your [cocks], she’s not the only one.");
        }
        else if (this.player.lust >= 95) {
            this.outputText(" judging by how much you’re salivating, she’s not the only one.");
        }
        else {
            this.outputText(" judging by how hot your [skin] feels, she’s not the only one overcome with randiness.");
        }
        this.outputText(" With each droplet that spills from her feminine crevice, your curiosity and instinctive desire spikes. The splatter of a droplet on the floor is like a lightning bolt of erotic energy to your brain.");
        ...
        if (this.player.cocks.length > 0) {
            this.outputText(" Your [cocks] spurt");
            if (this.player.cocks.length > 1) this.outputText("s");
            this.outputText(" uselessly into your [armor].");
        }
        if (this.player.vaginas.length > 0) {
            this.outputText(" Your [vagina]");
            if (this.player.cocks.length == 0) this.outputText(" drenches your undergarments");
            else this.outputText(" mixes right in with all that waste cum");
            this.outputText(".");
        }
    ...
    public spooderbuttGetsANewCockSleeveVI(): void {
        ...
        this.outputText("True to " + this.player.mf("his", "her") + " words, Lethice conquered Tel’Adre that same year, and the rest of Mareth fell in short order. The Champion came to understand the truth of Lethice’s words");
        if (this.player.vaginas.length == 0) this.outputText(", and after being outfitted with suitable genitalia,");
        else this.outputText(" and");
        this.outputText(" served in the breeding pits, endlessly fornicating to maintain an army of expendable imps.");
        this.outputText("\n\nThe Dark Queen did not forget her favorite prize. At least once a week, time permitting, she would visit the former champion and allow " + this.player.mf("him", "her") + " to tend to her endless, corrupted needs. Hours would pass with little more than the friction of a tongue against a slick slit, and the former champion had never been happier.");
    ...
    public beatTheSpooderbutt(hpVictory: boolean): void {
        ...
        //HP
        if (hpVictory) {
            this.outputText("<i>“No!”</i> the corrupted drider cries before collapsing on the floor, too battered to stand any longer. <i>“It cannot be! A mortal cannot stand against Lethice and prevail!”</i>");
        }
        //Lust
        else {
            this.outputText("<i>“No!”</i> the corrupted drider cries before collapsing to the floor, writhing widly. He’s too aroused for any kind of proper coordination");
            if (!m.goblinFree) this.outputText(", but he pushes the goblin off his cock all the same");
            else this.outputText(", but he grabs hold of his cock all the same");
            this.outputText(", masturbating like wild. <i>“H-h-how could a mortal stand against Lethice and win? H-h-how!?”</i>");
        }
        ...
        this.outputText("\n\n");
        if (!m.goblinFree) this.outputText("The goblin slowly separates from her fallen master.");
        else this.outputText("The goblin drops to her knees.");
        this.outputText(" Her eyes are full and luminous. She laces her fingers together and begs, <i>“Please don’t kill Mitzi, Champion! She can be yours.”</i> She drops to her hands and knees, crawling toward you. A petite hand tugs at your [armor]. <i>“Mitzi will be a good slave.”</i>");
        this.outputText("\n\nMitzi");
        if (this.player.cocks.length > 0) {
            this.outputText(" wiggles her");
            if (m.goblinOiled) this.outputText(" oiled-up");
            this.outputText(" butt at you");
        }
        else {
            this.outputText(" unspools a three-foot long tongue from between her plump lips");
        }
        this.outputText(", winking. <i>“Be mitzi’s [master], and mitzi will help you!”</i>");
    ...
    public driderDefeatMenu(): void {
        ...
        this.menu();
        if (this.flags[kFLAGS.DRIDERINCUBUS_KILLED] != 1 && this.flags[kFLAGS.DRIDERINCUBUS_FUCKED] != 1) {
            this.addButton(0, "Kill Drider", this.killDrider);
            if (this.player.vaginas.length > 0) this.addButton(1, "Ride Drider", this.rideDrider);
            if (this.player.cocks.length > 0) this.addButton(2, "Buttfuck Drider", this.buttfuckDrider);
        }
        else {
            doneDriderbus = true;
        }
        ...
        if (this.flags[kFLAGS.MITZI_RECRUITED] != 1) {
            if (this.flags[kFLAGS.MITZI_FUCKED] != 1) {
                ...
                if (this.player.vaginas.length > 0) this.addButton(7, "Goblin Licks", this.mitziEatsPussy);
            }
    ...
    public killDrider(): void {
        this.clearOutput();
        this.outputText("You laugh at Lethice and the goblin. <i>“This is all the reward I need.”</i> You press your");
        if (this.player.lowerBody == 5) this.outputText(" heel");
        else this.outputText(" [foot]");
        this.outputText(" down on the corrupted drider’s neck until you hear a sickening ‘pop’. He doesn’t move after that.");
    ...
    public fuckMitzi(): void {
        ...
        this.outputText("You shrug and start stripping out of your [armor] while you have the chance. Demons aren’t exactly known for their trustworthiness, but she’s doing this as a show of strength. Resorting to trickery to beat a simple mortal might lower her subordinates’ opinions of her. If there’s one thing you’ve learned to count on with demons");
        if (this.silly()) this.outputText(" and smut writers");
        this.outputText(", it’s hubris.");
        ...
        if (this.player.balls > 0) this.outputText(" wraps her tail around your [sack], giving eager little tugs.");
        else this.outputText(" wraps her tail around your [leg], giving you eager little tugs.");
        this.outputText(" She doesn’t need to pull you in - you can handle that all on your own. Grabbing her petite waist one handed, you pull her down onto your [cock biggest] using your free hand to keep your aim true.");
        ...
        if (this.getGame().camp.companionsCount() > 0) {
            ...
            this.outputText(" wouldn’t mind having a well-practiced goblin cooze around, would they? You certainly wouldn’t.");
        }
        //No Followers
        else {
        ...
        this.outputText("\n\n");
        if (this.player.cor <= 33) this.outputText(" You could even try to help her cope with her excessive sexual desires, maybe see what she’s like when she doesn’t want to be plugged with dick.");
        this.outputText(" There’s no harm in waking up with a sexpot milking every drop from your [cocks]. You’d be way more resistant to the demons that way.");
        if (this.player.cumQ() >= 15000) this.outputText(" The thought that you produce cum far too fast for that to matter doesn’t even occur to you.");
        this.outputText(" Maybe she’d even give you some daughters. You could raise up an army of petite greenskins to rebuild the land.");
        ...
        if (this.flags[kFLAGS.PC_FETISH] >= 1) this.outputText(" Actually, that thought’s probably just going to make you blow that much harder. Damn you, Ceraph!");
        this.outputText("\n\nWith another victorious slap on Mitzi’s ass, you bottom out.");
        if (this.player.balls > 0) this.outputText(" Your [balls] quake, pulling your [sack] taut against your body as they disgorge their creamy relief.");
        this.outputText(" You hold yourself there, pumping incessantly against your chosen cum-dump. Her face radiates absolute bliss. Goblins must have some kind of sense of when cum is splattering against their cervix, because you’ve never seen Mitzi happier, not even when she was mounted on her drider lord’s dick.");
        this.outputText("\n\nPulling out");
        if (this.player.hasKnot()) this.outputText(" with a wet sounding pop after solid knotting");
        this.outputText(", satisfied, you grab your [armor] and consider your options. Mitzi is still laying there");
        if (this.player.cumQ() >= 5000) this.outputText(" with a cum-pregnant belly");
        if (this.player.cumQ() >= 1000) this.outputText(", spooge dripping down her thighs from her overfilled snatch");
        this.outputText(". She’s slowly rousing back to consciousness, but do you really need a goblin distracting you? You could tell her to wait for you outside, or forget about her and move on.");
    ...
    public recruitMitzi(): void {
        this.clearOutput();
        this.outputText("You give Mitzi a friendly pat once she’s ready to go and tell her to wait for you outside.");
    ...
    public mitziEatsPussy(): void {
        ...
        if (this.player.cocks.length > 0) this.outputText(" and while you do have one for her to lavish her attention upon");
        else this.outputText(" and while you may lack such bulgy organs");
        this.outputText(", that tongue you saw is just too much to pass up. You spread your legs");
        if (this.player.balls > 0) this.outputText(" and lift your balls before");
        else this.outputText(",");
        this.outputText(" stretching your cunny with a pair of fingers. <i>“Get over here and put that tongue of yours to work; if you do a good job, maybe I’ll take you away from here”</i>");
        ...
        if (this.player.wetness() >= 3) this.outputText(" copious");
        this.outputText(" lubrication down the crease in the middle of pussy-pleaser to collect in her mouth.");
    ...
    public mitziEatsPussyII(): void {
        this.clearOutput();
        this.outputText("When your vision clears, Mitzi is sitting on your lap and gently massaging your [chest]. Only a few seconds have passed, but the nearby demons seem closer than you remember. You push the slut off and rise up onto your [feet], well and truly sated.");
    ...
    public titfuckMitzi(): void {
        ...
        this.outputText("You grin at the goblin and push your [armor] out of the way to expose your [cocks], explaining that you’d like to see her please your [cock biggest] with her tits before you make any decisions about taking her as a slave of your own. After all, her and her master are responsible for your current, rigid state");
        if (this.player.balls > 0 && this.player.lust >= 75) this.outputText(" and too-full feeling balls");
        this.outputText(". It’s only fitting that she make recompense for the irredeemable demon’s actions.");
    ...
    public titfuckMitziSlow(): void {
        ...
        if (this.player.biggestCockArea() <= 20) this.outputText(", tiny");
        this.outputText(" tower of meat");
        if (this.player.biggestCockArea() <= 20) this.outputText(", nearly obscuring the whole thing");
        else if (this.player.biggestCockArea() >= 100) this.outputText(", barely able to support it");
        this.outputText(".");
        if (this.player.cocks.length > 1) {
            this.outputText(" She keeps the other");
            if (this.player.cocks.length > 2) this.outputText("s");
            this.outputText(" angled out of the way, though from time to time she favors");
            if (this.player.cocks.length == 2) this.outputText(" it");
            else this.outputText(" them");
            this.outputText(" with a gentle, encouraging stroke. Her attention remains fixated squarely on the largest.");
        }
        this.outputText(" <i>“Though she’s never made someone come with kisses before...”</i> Mitzi brushes purple-dyed hair from her eyes and bends down press her lips against your [cockHead biggest].");
        ...
        this.outputText("\n\nGiggling, the viridian vixen collects your pre-cum on her tongue and spreads it across her puffy, purple cocksuckers until they shine like amethyst, glossed over by your own leaking desire. <i>“‘Course Mitzi’d never disobey " + this.player.mf("master’s", "mistress’s") + " command.”</i> She kisses the bare flesh above your insistent loins. <i>“Only the best titfucks for [master].”</i>");
        this.outputText("\n\nArching her back, Mitzi drags her oversized bosom");
        if (this.player.balls > 0) this.outputText(" over your [balls] and");
        this.outputText(" up to either side of your [cock biggest], swaddling it in flesh that feels a little too cushy to be real. Her manicured fingertips press down from either side, tightening her vise until you feel her heartbeat hammering back against your boner, sending pleasant sensations thundering through the all-too-sensitive organ.");
        if (this.player.balls > 0 && this.player.ballSize > 5) this.outputText(" You dribble yet more pre-cum onto the sweat-shined swells. Big as they are, they can’t quite ensconce your entire length, letting you watch it happen in lurid detail.");
        else this.outputText(" You dribble yet more pre-cum into her swampy cleavage. You can feel it getting slicker and slicker but can’t quite see it happen. You’ve nothing to go on but the feel against your [cockHead biggest].");
        ...
        if (this.player.biggestCockArea() >= 100) this.outputText(" Sometimes she even plants a few more kisses on your [cockHead biggest] or just below. Other times she licks you like a lollipop - at least the parts not buried in bust.");
        ...
        if (this.flags[kFLAGS.TAMANI_MET] > 0) this.outputText(" since meeting Tamani");
        this.outputText(". Even the breezes feel good on your dick.");
        ...
        if (this.player.cocks.length > 1) {
            ...
            if (this.player.cocks.length == 2) this.outputText(" both");
            else this.outputText(" all");
            this.outputText(" are, even the one");
            if (this.player.cocks.length > 2) this.outputText("s");
            this.outputText(" not getting much of her attention");
        }
        else {
            this.outputText(", flinging ropes of pre against your belly and Mitzi’s face");
        }
        this.outputText(". You’re so close; you can feel it, can feel that hot load bubbling up your urethra");
        if (this.player.cocks.length > 1) this.outputText("s");
        this.outputText(", on the cusp of firing everywhere. Mitzi’s licking her lips and tugging on her ardor-distended nipples, her jaw open so that you can admire the ridiculously plushness of her lower lip and just how good it would look coated in a layer of virile love.");
    ...
    public titfuckMitziQuick(): void {
        ...
        this.outputText("<i>“Do it quick, if you really can,”</i> you answer the petite green slut.");
        ...
        if (this.player.balls > 0) this.outputText(", her nipples dragging over the skin of your [sack]");
        this.outputText(". Matter of factly, she grabs her own tits and pulls them apart, revealing an expanse of green flesh made slippery by her own sweat. [EachCock] sinks right into the welcoming valley a second before she brings her girls back to close around you, enveloping your length");
        if (this.player.cocks.length > 1) this.outputText("s");
        this.outputText(" in slick, soft boobflesh. Her chest feels almost molded to your [cocks], designed to press evenly on every sensitive place. You can’t help but give a little throb of excitement.");
        ...
        this.outputText(" exposed to her purple-painted lips and drooling tongue. She gobbles [oneCock] up in a hurry, easily wrapping her spit-soaked tongue around its circumference while her cheeks hollow from her near-constant sucking.");
        ...
        //Multidick, no new PG
        if (this.player.cocks.length > 1) {
            this.outputText(" She only has one mouth, so the rest of your load spills across her bouncing tits, building to a milky froth between the quivering, dick-milking mounds. One of her hands starts smearing it around before you’ve even finished.");
        }

        //Merge
        this.outputText("\n\nThe cock-hungry goblin never completely stops either, attacking your oversensitive member");
        if (this.player.cocks.length > 1) this.outputText("s");
        this.outputText(" with the kind of reckless abandon that could only come from a goblin fuckslave. Your [legs] wobble and [eachCock] bulges obscenely, unsure if it should wilt or start shooting a whole ‘nother load.");
        this.outputText("\n\nLucky for you, you manage to stumble back before she has you ready to go again. Mitzi bats her");
        if (this.player.cocks.length > 1 || this.player.cumQ() > 500) this.outputText(" cummy");
        this.outputText(" eyelashes at you and pushes a stray droplet of spooge past her puffy cocksuckers, swallowing noisily.");
    ...
    public rideDriderToGoblin(): void {
        ...
        this.outputText("You grin outlandishly at the demon’s quiet begging and answer, <i>“Sure thing, but you owe me once I take down your boss. Got it?”</i>");
    ...
    public actuallyRideDriderDick(): void {
        ...
        this.outputText("<i>“Fat chance,”</i> You hiss in the drider demon’s ear. <i>“");
        if (this.player.cor <= 33) this.outputText("You deserve worse than a little humiliation, but I’m so turned on that this’ll have to do.");
        else this.outputText("I’m going to work your dick better than that goblin hussy ever could. You’re going to beg like a bitch to the pathetic </i>mortal<i> in front of all your cohorts. They’ll never respect you again.");
        this.outputText("”</i>");
        ...
        if (this.player.wetness() >= 3) this.outputText(" soaked");
        else this.outputText(" aroused");
        this.outputText(" lips spreading against him, the way your petals push open to reveal the delicate smoothness of your inner flower.");
        ...
        if (this.player.clitLength > 3) {
            ...
            if (this.player.pregnancyType != 0) this.outputText(" cunt");
            else this.outputText(" womb");
            this.outputText("-stuffing satisfaction that you can only get from real sex.");
        }
        else if (this.player.cocks.length > 0) {
            this.outputText("\n\nThe best part is the way your [cocks]");
            if (this.player.cocks.length == 1) this.outputText(" is");
            else this.outputText(" are");
            this.outputText(" smashed up against his, rubbing thick, veiny shaft against rigid length in the most sinfully delicious way. You can barely keep yourself from humping away at his cock until you cum, but isn’t that what you wanted? To use this demon until your base urges were wholly satisfied?");
            this.outputText("\n\nThe mounting hunger in your channel for some kind of penetration jars you from stirring");
            if (this.player.cocks.length == 1) this.outputText(" your");
            else this.outputText(" a");
            this.outputText(" dick against his. You want him inside you. He can always manage [oneCock] with a hand while you’re riding him again and again, trading a little corruption for the kind of");
            if (this.player.pregnancyType != 0) this.outputText(" cunt");
            else this.outputText(" womb");
            this.outputText("-stuffing satisfaction that you can only get from real sex.");
        }
        ...
        if (this.player.clitLength > 3 || this.player.cocks.length > 0) this.outputText(" one of");
        this.outputText(" his nipples");
        if (this.player.clitLength > 3 || this.player.cocks.length > 0) {
            this.outputText(" while still trying to");
            if (this.player.clitLength > 3) this.outputText(" pump your [clit]");
            else this.outputText(" give you a handjob");
        }
        this.outputText(". He floods you with his tainted seed, stuffing your passage until every nook and cranny is chock full of demonspunk, bathed in his reproductive corruption.");
        ...
        if (this.flags[kFLAGS.PC_FETISH] < 1 || this.player.hasVirginVagina() || this.player.cor <= 33) this.outputText("n unexpectedly");
        this.outputText(" wonderful feeling, getting to cum in front of an audience like this.");
        ...
        if (this.player.findPerk(PerkLib.HistorySlut) < 0 && this.player.findPerk(PerkLib.HistoryWhore) < 0) this.outputText(" chaste");
        else this.outputText(" determined");
        this.outputText(" champion that entered Mareth so long ago, but it feels too good to stop, like your pussy is aflame and the only way to put it out is to hose it down with even more drider goo.");
    ...
    public buttfuckDrider(): void {
        ...
        this.outputText("You look the defeated drider over, contemplating what <i>“reward”</i> you intend to take from him. Your eyes trace over his semi-human form, a twisted cavalcade of humanity and pitch-black demonic corruption. Your hands trace over his curling, razor-edged horns, making the defeated spider-man recoil in... not fear, but something else. Anticipation, maybe?");
        if ((this.monster as DriderIncubus).goblinFree) this.outputText(" You can see his turgid rod hanging between his legs jump at your touch, spurting a thick, viscous trickle of corrupted spunk across the marble floor.");
        else this.outputText(" The goblin slung under his insectile abdomen yelps as the cock buried balls-deep in her gash twitches, blowing a load of corrupted pre into her womb.");
        if (this.player.cor > 75) this.outputText(" What a magnificent creature. You look forward to seeing what this... Kinarial can do.");
        ...
        if (this.player.biggestCockArea() >= 1200) this.outputText(" You feel like your mammoth member is going to tear him apart at this rate! The drider’s mouth twists into a silent ring of shock, unable to give voice to his reaction as you bury yourself inside him.");
        else if (this.player.biggestCockArea() >= 500) this.outputText(" Your animalistically-proportioned prick stretches the virginal passage to its limit, making the drider roar in a mix of surprise and unexpected pleasure.");
        else this.outputText(" Your prick sinks deep into the drider’s ass, gliding through his virginal hole.");
        this.outputText(" It’s surprisingly moist as you plunge in, naturally lubed - of course a demon would be. It’s hard to be surprised by these sorts of things anymore, after all you’ve been through. You");
        if (this.player.isDrider()) this.outputText(" reach down from your bestial position overtop him and");
        this.outputText(" slap the drider’s ass, digging your fingers into his charcoal skin. A cheer goes up through you demonic audience as you hilt yourself inside the drider’s ass, the wet sounds of flesh slapping against flesh echoing sickly through the corrupted throne room.");
        ...
        if (this.player.isTaur()) this.outputText(", which has the added bonus of carrying through from your bestial backside to his, and down to the cock-goblin mounted on Kinarial’s dick.");
        this.outputText(" The green slut screams in ecstatic, mindless pleasure. Her voice carries through the throne room, drowning out the moans and cries of the demon court with a shriek of ecstasy.");
        ...
        if (this.player.cumQ() <= 1000) this.outputText(" You squirt your load deep into the drider’s asshole, packing his virginal hole with sticky white spooge.");
        else if (this.player.cumQ() <= 5000) this.outputText(" You pump a thick, creamy load into the demon-spider’s behind, filling his virginal hole with hot spunk until it’s drooling out around your hammering member.");
        else this.outputText(" You flood the drider’s ass with a torrential wave of seed, bloating the demon’s gut with your spunk. Cum spurts out of his ass around your thrusting member, pouring out with every motion.");
        ...
        if (this.player.cor >= 75) this.outputText(" You can't wait to see how the Demon Queen feels when she's skewered on your rod!");
