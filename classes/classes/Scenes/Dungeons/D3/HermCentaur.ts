...
export class HermCentaur extends Monster {
    ...
    public constructor() {
        ...
        this.initStrTouSpeInte(100, 100, 65, 65);
        this.initLibSensCor(85, 40, 100);
        ...
        this.gems = 75 + HermCentaur.rand(50);
        this.level = 22;
    ...
    public defeated(hpVictory: boolean): void {
        this.game.d3.hermCentaur.beatThePony(hpVictory);
    }

    public won(hpVictory: boolean, pcCameWorms: boolean): void {
        this.game.d3.hermCentaur.inSovietCoCPonyRidesYou(hpVictory, pcCameWorms);
    }
    ...
    protected performCombatAction(): void {
        ...
        // Priority use
        if (this._chargingArouse) this.arouseSpellCast();
        else if (this.lust >= 60 && !this._usedGottaCum) this.gottaCum();
        else if (this.HPRatio() <= 0.5 && !this._usedHeal) this.healUp();
        else if (HermCentaur.rand(this.lust) >= 40) this.hypnoCock();
        else {
            // Selections
            var opts: any[] = [this.feminineMusk, this.aphrodisiacSquirt];
            if (!this._chargingArouse && this._arouseCooldown <= 0) opts.push(this.arouseSpellCharge);

            opts[HermCentaur.rand(opts.length)]();
        }
    ...
    private aphrodisiacSquirt(): void {
        ...
        if (this.player.lust < 30) this.outputText("\n\nYou close your mouth tight and endure the shimmering shower, trying your damnedest to resist the effects of this insidious liquid.  Wherever it strikes you, it vanishes soon after, absorbed directly into your body.");
            ...
            if (this.player.hasCock()) this.outputText(" [eachCock] awakens, filling itself towards full erection.");
            else if (this.player.hasVagina()) this.outputText(" a wet heat answers in your loins, growing more insistent from moment to moment.");
            ...
            if (this.player.hasCock()) this.outputText(" ignoring the stiffening pressure of [eachCock]");
            else this.outputText(" ignoring the increasing wetness of your inner [armor]");
            this.outputText(". It's getting harder to think straight with all this desire welling up inside you.  It isn't for a few moments that you realize you tongue is hanging out and drooling all over youself.  Gods, you want to fuck!");
        }
            ...
            if (this.player.hasCock()) this.outputText("  Pre begins to dribble from your hot-and-ready cum-slit, eager to pour out in a deluge of sperm.");
            else if (this.player.hasVagina()) this.outputText("  A flood of sopping moisture dribbles down your [legs], the proof of your unholy desire to breed.");
        ...
        else this.outputText(" Sinking to your knees, you start trying to tear your way out of your [armor].  You don't care about winning anymore - fucking this woman... this beast... that's all that matters.  You don't care how she takes you, as long as you get to cum all over her!");

    }

    private arouseSpellCharge(): void {
        // one turn charge-up, can be interrupted by tease
        this.outputText("The centauress closes her eyes for a moment, then opens them.  Her eyes have gone completely, solidly black.  She's chanting low, though you see her dick bobbing beneath her in time with the mysterious utterances, leaking pre-cum.  You've got to stop her!");
        if (this.player.inte > 80) this.outputText("  A tease attack would likely be the most effective method of attack.");
    ...
    private hypnoCock(): void {
        ...
        if (this.player.inte * (2 / this._hypnoCockUses) > HermCentaur.rand((this.player.lib / 3) + (this.player.sens / 3) + (this.player.cor / 3))) {
            this.game.dynStats("lus+", 2 + HermCentaur.rand((this.player.lib / 20) + (this.player.sens / 20)));
            ...
            this.game.dynStats("lus+", 20 + 2 * this._hypnoCockUses + 2 + HermCentaur.rand((this.player.lib / 10) + (this.player.sens / 10)));

            this.outputText("Down it bobs, slowly hanging lower and lower... SMACK!  Up it goes, taking your bedazzled eyes along for the ride.  \"<i>That's a good " + this.player.mf("boy", "girl") + ",</i>\" the dick's director whispers, \"<i>Just follow the tempo and let it fill your mind, oozing inside you with each thump.</i>\"");
