...
export class MinotaurLord extends Monster {
    ...
    protected performCombatAction(): void {
        if (this.HP < 300 && this.statusAffectv1(StatusAffects.MinoMilk) < 4) this.minotaurDrankMalk();
        else if (MinotaurLord.rand(4) == 0 && this.player.weaponName != "fists") this.minotaurDisarm();
        else if (this.findStatusAffect(StatusAffects.Timer) < 0) this.minotaurLordEntangle();
        else if (this.findStatusAffect(StatusAffects.MinotaurEntangled) >= 0) this.minotaurCumPress();
        else {
    ...
    private minotaurCumPress(): void {
        ...
        if (this.player.lust > 99) this.outputText("you rise up, horny and hungry for more.");
        else {
            ...
            if (this.player.lust >= 80) this.outputText(" and drippy");
    ...
    private minotaurPrecumTease(): void {
        ...
        if (MinotaurLord.rand(2) == 0) {
            ...
            if (this.player.lust >= 70) this.outputText("swallowing it into your mouth without thinking.  You greedily guzzle the potent, narcotic aphrodisiac down, even going so far as to lick it from each of your fingers in turn, sucking every drop into your waiting gullet.");
        ...
        //(1)
        if (this.player.lust <= 75) this.outputText("  You shiver with need, wanting nothing more than to bury your face under that loincloth and slurp out every drop of goopey goodness.");
