...
export class Cotton extends TelAdreAbstractContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (this.flags[kFLAGS.COTTON_KID_COUNT] > 0 && this.model.time.hours == 23) this.flags[kFLAGS.COTTON_OLDEST_KID_AGE]++;
        trace("\nCotton time change: Time is " + this.model.time.hours + ", incubation: " + this.pregnancy.incubation + ", event: " + this.pregnancy.event);
        return false;
    }

    public timeChangeLarge(): boolean {
        if (this.model.time.hours == 6 && this.flags[kFLAGS.COTTON_BREAKFAST_CLUB] == 1 && this.player.biggestLactation() >= 2) {
    ...
    public cottonGreeting(): void {
        ...
        if (this.flags[kFLAGS.COTTON_MET_FUCKED] == 0) {
            ...
            if (this.player.tallness > 78) this.outputText("Despite her impressive stature, you still look down on her.", false);
            //(If PC height is in between:
            else if (this.player.tallness >= 76) this.outputText("She's roughly your height, which is rather impressive in itself.", false);
    ...
    private acceptYoga(): void {
        ...
        //First time
            ...
            if (this.player.gender == 3) this.outputText("  She spies your crotch and smiles, \"<i>Oh, best of both worlds, are we? Well you're in good company then.</i>\"", false);
            //(If PC is male:
            else if (this.player.hasCock()) this.outputText("  She cradles your " + this.cockDescript(0) + " and smiles at you, \"<i>Well, we might find a use for that later.</i>\"", false);
            //(If PC is female:
            else if (this.player.hasVagina()) this.outputText("  She pats your groin and says, \"<i>Y'know, I know some good exercises that really work the vaginal muscles... perhaps we'll talk about that later.</i>\"", false);
            this.outputText("\n\n", false);
    ...
    private cottonFucksYou(): void {
        ...
        if (this.player.gender == 1) {
            ...
            if (this.player.cumQ() >= 1000) this.outputText("  You nearly clog the drain with all the spooge leaking from your flaccid shaft.", false);
            this.outputText("\n\n", false);
        ...
        */
        ...
        if (this.player.isTaur()) {
            ...
            if (this.player.cockArea(x) >= 100) {
                if (this.player.cockTotal() == 1) this.outputText("You feel your " + this.cockDescript(x) + " stirring beneath your large body, while Cotton's remains curiously limp.  While her cock dangles, yours strains for attention.", false);
                //(If PC has multiple cocks)
                else if (this.player.cockTotal() > 1) this.outputText("You feel your " + this.multiCockDescriptLight() + " stirring beneath your large body, while Cotton's remains curiously limp.  While her cock dangles, your group of cocks strains for attention.", false);
            }
            else {
                if (this.player.cockTotal() == 1) this.outputText("You feel your " + this.cockDescript(x) + " stirring beneath your large body, while you watch Cotton's do the same. Her cock rubs against your stomach while yours strains for attention.", false);
                //(If PC has multiple cocks)
                else if (this.player.cockTotal() > 1) this.outputText("You feel your " + this.multiCockDescriptLight() + " stirring beneath your large body, while you watch Cotton's do the same. Her cock rubs against your stomach while your group of cocks strains for attention.", false);
            }
            //(If PC has a pussy, add the following)
            if (this.player.hasVagina()) this.outputText("  Meanwhile, your " + this.vaginaDescript(0) + " behind you moistens both from the steam and from arousal, and your " + this.clitDescript() + " aches, craving attention.", false);
            this.outputText("\n\n", false);
            ...
            if (this.player.biggestTitSize() >= 2) {
                ...
                else this.outputText("  Cotton soon switches to the other breast, teasing your " + this.nippleDescript(0) + " with her talented tongue.", false);
                this.outputText("\n\n", false);
            }
            ...
            if (this.player.cockTotal() == 2) this.outputText("  With a slight grin, you begin poking your extra dick at Cotton's other hole. You hear an indignant grunt, followed by a sigh, and feel her hands on your member, guiding it all the way into her hole.", false);
            //(If the player has more cocks, add)
            else if (this.player.cockTotal() > 2) this.outputText("  The rest of your cocks jiggle and bob, rubbing up against her thighs and butt wildly.", false);
            ...
            else this.outputText("  You piston back and forth, and though Cotton's cunt is sizable, it's able to grip you tightly. Your equine lover moans and grasps at your chest, eventually seizing your leg for support, though she leans backwards.", false);
            this.outputText("\n\n", false);
            ...
            else this.outputText("Oblivious to you, her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and the bench as an orgasm rocks her body.", false);
            //(If player is too big, add)
            ...
            //(If PC has a pussy)
            if (this.player.hasVagina()) this.outputText("  Your " + this.cockDescript(x) + " erupts inside Cotton's warm cunt which clenches you tightly, while your own femme sex twinges and clenches on air, drooling juices down your legs.", false);
            //(else)
            else this.outputText("   Your " + this.cockDescript(x) + " erupts inside Cotton's warm cunt which clenches you tightly.", false);
            //(regardless or above, add)
            this.outputText("  You give a couple more token thrusts and pull out. As you do, ", false);
            if (this.player.cumQ() < 200) this.outputText("cum begins to seep from her folds", false);
            else if (this.player.cumQ() < 700) this.outputText("a gush of cum escapes from Cotton's folds", false);
            else if (this.player.cumQ() < 1200) this.outputText("a small torrent of cum escapes from Cotton's folds", false);
            else if (this.player.cumQ() < 2000) this.outputText("cum surges from her abused hole", false);
        ...
        //(Repeat fuck her, for nagas)
            ...
            if (this.player.cockArea(x) >= 100) {
                //(If PC has one cock)
                if (this.player.cockTotal() == 1) this.outputText("  You feel your " + this.cockDescript(x) + " stirring beneath you, slowly coming to attention, though Cotton's remains curiously limp.  Your " + this.player.cockHead(x) + " rubs against her belly, sending ripples of pleasure up your spine.", false);
                //(If PC has multiple cocks)
                else if (this.player.cockTotal() > 1) this.outputText("  You feel your " + this.multiCockDescriptLight() + " stirring beneath you, slowly coming to attention, though Cotton's remains curiously limp.  The group of cocks rubs together as you make out, sending ripples of pleasure up your spine.", false);
            }
            else {
                //(If PC has one cock)
                if (this.player.cockTotal() == 1) this.outputText("You feel your " + this.cockDescript(x) + " stirring beneath you, slowly coming to attention alongside Cotton's equine member.  The two rub together as you make out, sending ripples of pleasure up your spine.", false);
                //(If PC has multiple cocks)
                else if (this.player.cockTotal() > 1) this.outputText("You feel your " + this.multiCockDescriptLight() + " stirring beneath you, slowly coming to attention alongside Cotton's equine member. The group of cocks rubs together as you make out, sending ripples of pleasure up your spine.", false);
            }
            //(If PC has a pussy, add the following)
            if (this.player.hasVagina()) this.outputText("Your " + this.vaginaDescript(0) + " meanwhile moistens both from the water and from arousal, and your " + this.clitDescript() + " aches, craving attention.", false);
            this.outputText("\n\n", false);
            ...
            //(If player has two cocks, add)
            if (this.player.cockTotal() == 2) this.outputText("  With a slight grin, you position your extra dick at Cotton's other hole. She quirks her eyebrow and a look of panic momentarily crosses her face, but you push forward anyway. Cotton's eyes roll up briefly and her lip quivers.", false);
            //(If the player has more cocks, add)
            else if (this.player.cockTotal() > 2) this.outputText("  The rest of your cocks strain, aching for holes to fill. With none available, they throb as they rub against Cotton's smooth skin.", false);
            //(If player is too big, add)
            ...
            //(otherwise add)
            else this.outputText("  You piston back and forth, and though Cotton's cunt is sizeable, it's able to grip you tightly. Your equine lover moans and grasps at your chest, eventually seizing your shoulder for support, though she leans backwards.", false);
            this.outputText("\n\n", false);
            ...
            else this.outputText("Her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and face as an orgasm rocks her body.", false);
            //(If player is too big, add)
        ...
        //(Repeat Fuck Her, for humanoid bodies)
            ...
            if (this.player.hasVagina()) this.outputText("  Your " + this.vaginaDescript(0) + " moistens both from the water and from arousal, and your " + this.clitDescript() + " aches, craving attention.", false);
            this.outputText("\n\n", false);
            ...
            //(If player has two cocks, add)
            if (this.player.cockTotal() == 2) this.outputText("  With a slight grin, you position your other dick at Cotton's other hole. She quirks her eyebrow and a look of panic momentarily crosses her face, but you push forward anyway. Cotton's eyes roll up briefly and her lip quivers.", false);
            //(If the player has more cocks, add)
            if (this.player.cockTotal() > 2) this.outputText("  The rest of your cocks strain, aching for holes to fill. With none available, they throb as they rub against Cotton's smooth skin.", false);
            //(If player is too big, add)
            if (this.player.cockArea(x) > 70) this.outputText("  Though you are far too big for her, you make sure to stuff her as much as you can. Her cunt, big as it is, squeezes tightly on your " + this.cockDescript(x) + ". She shivers and quakes, and leans against the shower wall, gripping the shower-head for support. You thrust your overly large tool inside her over and over, stretching her already not-insignificant cunt even wider. Her tongue actually rolls out of her snout, lolling to the side while her eyes roll up into her head, lost in the pleasure. Her body rocks with an orgasm while you piston away under the spray of water.", false);
            //(if player is too small (under 4</i>\") add)
            else if (this.player.cocks[x].cockLength < 4) {
    ...
    private cottonFucksYouInShowerRepeat(): void {
        ...
        if (this.player.isTaur()) {
            ...
            if (this.player.gender == 1) this.outputText("  Your " + this.cockDescript(0) + " tingles betwixt your legs, but remains limp in the presence of Cotton's impressive member.", false);
            //(If PC has a penis and vagina)
            else if (this.player.gender == 3) this.outputText("  Your " + this.cockDescript(0) + " tingles betwixt your legs, but remains limp in the presence of Cotton's impressive member, while your " + this.vaginaDescript(0) + " moistens almost immediately from the steam and arousal.", false);
            //(If PC has a vagina and no penis)
            else if (this.player.gender == 2) this.outputText("  Your " + this.vaginaDescript(0) + " moistens almost immediately as you make out, both from the steam and your increasing arousal.", false);
            //(If PC is genderless)
            else this.outputText("  A deep aching burns within you, a need your body is ill-equipped to process, but still your nipples harden and you find your " + this.assholeDescript() + " puckering in anticipation.", false);
            this.outputText("\n\n", false);
            ...
            if (this.player.hasCock()) {
                if (this.player.cockArea(0) > 70) this.outputText("  \"<i>Oh my, what's this?</i>\" She puts a hand under your enormous, yet embarrassingly limp cock and lifts it slightly. \"<i>My little pet has such a big dick... Just how I like it. Perhaps next time I'll get to try it out... but not today, hm? This is all about you right now.</i>\"", false);
                ...
                else this.outputText("Cotton puts a hand under your embarrassingly limp cock and smiles, \"<i>You know how to show a girl you like her... There's only room for one cock right now.</i>\"", false);
            }
            //(If PC has a vagina)
            if (this.player.hasVagina()) this.outputText("  Spying your " + this.vaginaDescript(0) + ", Cotton smiles and flicks your " + this.clitDescript() + " teasingly before slipping two fingers inside your folds and bringing them to her mouth, licking them clean. \"<i>Mmm... I love the taste of your juices, pet...</i>\"", false);
            //(if PC is genderless)
            if (this.player.gender == 0) this.outputText("  She places a hand on your bare crotch and quirks an eyebrow. \"<i>Well this is new... Certainly not bad though.</i>\" She runs a hand along your bare mound, which somehow manages to send ripples of pleasure up your spine.", false);
            this.outputText("\n\n", false);
        ...
        //(Repeat get fucked for humanoids and nagas)
            ...
            this.outputText("You decide to take her up on her offer, and she pulls you towards the showers, quickly disrobing you forcibly and then herself. She turns only one shower-head on and pulls you into an embrace underneath the rapidly heating water. She kisses up your neck and playfully bites you with a grin.", false);
            //(If PC has a penis, no vagina)
            if (this.player.gender == 1) this.outputText("  Your " + this.cockDescript(0) + " tingles beneath you, but remains limp in the presence of Cotton's impressive member.", false);
            //(If PC has a penis and vagina)
            else if (this.player.gender == 3) this.outputText("  Your " + this.cockDescript(0) + " tingles beneath you, but remains limp in the presence of Cotton's impressive member while your " + this.vaginaDescript(0) + " moistens almost immediately from the steam and arousal.", false);
            //(If PC has a vagina and no penis)
            else if (this.player.gender == 2) this.outputText("  Your " + this.vaginaDescript(0) + " moistens almost immediately as you make out, both from the steam and your increasing arousal.", false);
            //(If PC is genderless)
            else this.outputText("  A deep aching burns within you, a need your body is ill-equipped to process, but still your nipples harden and you find your " + this.assholeDescript() + " puckering in anticipation.", false);
            ...
            if (this.player.isNaga()) {
                ...
                else this.outputText("Moving quickly, you wrap your arms around Cotton's neck and coil your body around her waist and down one leg.", false);
            }
            //(If Humanoid body)
            else {
                ...
                else this.outputText("Moving quickly, you wrap your arms and legs around Cotton so you don't fall over.", false);
            }
    ...
    public cottonTantricSex(): void {
        ...
        //(for Naga Body, add)
        if (this.player.isNaga()) this.outputText("  You impale yourself fully, settling down and sitting on her lap, stretching your snake-like body out behind and to one side of her, with your arms propping you up as you lean back.", false);
        //(for Humanoid Body, add)
        else this.outputText("  You impale yourself fully, settling down and sitting on her lap, mimicking her pose with your legs stretched out behind her and your arms propping you up as you lean back.", false);
        this.outputText("\n\n", false);
        ...
        if (this.player.hasCock()) this.outputText("  Meanwhile, your own " + this.cockDescript(0) + " hangs limply in front of you, tingling with arousal, but unable to harden. Cotton gives it a pat and says, \"<i>Worry not, pet, there's only room for one cock right now, but I'll want it some other time.</i>\"", false);
        this.outputText("\n\n", false);

        this.outputText("\"<i>Now let's move on to the Lotus,</i>\" Cotton says, and sits up.", false);
        //(Naga body, add)
        if (this.player.isNaga()) this.outputText("  You do the same, curling your arms around your partner while your lower body coils around her midsection. Cotton pulls her legs in as well, and it almost looks like you're both sitting normally, if it weren't for you being on her lap with her dick up inside you.", false);
        //(Humanoid body, add)
        else this.outputText("  You do the same and curl your arms and legs around your partner. Cotton pulls her legs in as well, and it almost looks like you're both sitting cross legged if it weren't for you being on her lap with her dick up inside you.", false);
        ...
        if (this.player.biggestTitSize() >= 2) {
            ...
            else this.outputText("  She quickly switches to the other nipple, her tongue expertly teasing and tantalizing it before pulling away once more.", false);
            this.outputText("\n\n", false);
        }
        ...
        //(If PC has a penis)
        if (this.player.hasCock()) this.outputText("  Your " + this.cockDescript(0) + " twitches and tingles, releasing its pent up seed in a dribbling torrent down your belly and chest, right into your face and pooling around your head.", false);
        //(If PC has a vagina)
        if (this.player.hasVagina()) this.outputText("  Your " + this.vaginaDescript(0) + " shivers and clamps down hard on the invading cock, pulsing and milking it as it unloads directly into your womb.", false);
        //(If PC doesn't have a vagina)
        else this.outputText("  Your " + this.assholeDescript() + " shivers and clamps down hard on the invading cock, pulsing and milking it as it unloads directly into your intestines.", false);
