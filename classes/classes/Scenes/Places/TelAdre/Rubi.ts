...
export class Rubi extends TelAdreAbstractContent {
    ...
    public rubiCock(lust: number = 50): string {
        ...
        return Appearance.cockDescription(this.rubiGetCockType(), this.flags[kFLAGS.RUBI_COCK_SIZE], this.flags[kFLAGS.RUBI_COCK_SIZE] / 6, lust, cumQ);
    }
    ...
    public rubiIntros() {
        ...
        if (this.flags[kFLAGS.RUBI_DISABLED] > 0 || this.flags[kFLAGS.RUBI_PROSTITUTION] == -2) return undefined;
        this.outputText("\n");
        //(Haven't met yet)
        if (this.flags[kFLAGS.RUBI_INTRODUCED] == 0) this.outputText("A cute human girl moves from table to table, bussing the empty ones and serving drinks to the occupied.  She's pretty flat-chested, and wears a pink and white frilly waitress uniform with an apron.  When she turns her back to you, you can tell she's not <i>entirely</i> human, what with the long red spaded tail coming out from under her dress.");
        //(Have met, relationship 0-19)
        else if (this.rubiAffection() < 30) this.outputText("Rubi is here, moving from table to table efficiently clearing the empty ones and serving new customers.  Her long red tail swishes back and forth, sometimes lifting her dress enough to get a peek at her panties.");
        //(Have met, relationship 40, Bimbo Rubi; cheating scene)
        else if (this.rubiBimbo() && this.flags[kFLAGS.RUBI_PROSTITUTION] == 0 && Rubi.rand(9) == 0) {
        ...
        else if (this.rubiBimbo() && this.flags[kFLAGS.RUBI_DEBIMBO] == 0) this.outputText("Rubi is here, moving from table to table, not quite as efficiently as [rubi ey] used to, but with a lot more exuberance.  [rubi Eir] long red tail swishes around, seemingly drawn towards the crotch of patrons and co-workers alike.");
        else this.outputText("Rubi is here, moving from table to table efficiently clearing the empty ones and serving new customers.  [rubi Eir] long red tail swishes back and forth excitedly.");
        this.outputText("\n");
    ...
    public approachRubiScenes(): void {
        ...
        //(Approach repeat, relationship 20+.  At this stage, randomly select an outfit from what the PC has given him, plus his waitress outfit.  Variants included for Bimbo /Incubus Rubi.)
            ...
            if (select == 1) {
                ...
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] >= 4) this.outputText("  Rubi's breasts strain against the tight blouse, threatening to pop a button at any moment.");
                if (this.flags[kFLAGS.RUBI_COCK_SIZE] >= 9) this.outputText("  You notice Rubi holds [rubi emself] a little awkwardly, and it's only when [rubi ey] sits down that you spy the slight bulge in [rubi eir] skirt.");
            }
            //(Suit Clothes (Normal))
            else if (select == 2) {
                //(Suitclothes)
                ...
                if (this.flags[kFLAGS.RUBI_HORNTYPE] > 0) this.outputText("just behind the horns");
                this.outputText(".  In a sexy sort of way, [rubi ey] looks like a girl dressed in her boyfriend's clothes");
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] >= 4) this.outputText(", though with [rubi eir] large bosom, [rubi ey] can't quite button up all the way, giving the appearance of a sexy secretary");
                this.outputText(".");
            }
            else if (select == 3) {
                ...
                if (this.flags[kFLAGS.RUBI_NIPPLE_TYPE] > 0) this.outputText("black ");
                ...
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] >= 4) this.outputText(" impressive bosom, and sort of makes [rubi em] look like the kind of rubber doll one might buy for those lonely nights.");
                else this.outputText(" relatively flat chest, though with the rest of [rubi eir] figure, it's hard to imagine [rubi em] as anything but a woman.");
            }
            else if (select == 5) {
                //(Tube Top)
                ...
                //(D breasts or higher)
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] >= 4) this.outputText("  [rubi Eir] breasts strain the fabric of the tube top, any wrong move threatening to have them burst forth.");
                //(dick 9" or higher)
                if (this.flags[kFLAGS.RUBI_COCK_SIZE] >= 4) this.outputText("  [rubi Eir] rather large bulge can't be concealed, however, and you sometimes catch Rubi holding [rubi eir] notepad down across [rubi eir] crotch when [rubi ey] moves from table to table.");
                else this.outputText("[rubi Eir] bulge is well concealed at least.  You're sure it would take a skilled eye, or someone who knew what to look for, to notice Rubi's little package.");
            }
            else if (select == 6) {
                //(Bodysuit)
                ...
                if (this.flags[kFLAGS.RUBI_COCK_SIZE] >= 9) this.outputText("  However, with an outfit this tight, it's almost impossible for [rubi em] to hide [rubi eir] impressive package.  Rubi's cock pokes straight upwards, creating a bulge from [rubi eir] crotch up towards [rubi eir] belly button.");
                else if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("  However, with an outfit this tight you can see a slight outline where [rubi eir] small package is tucked away.  You wonder how many other patrons have noticed the same thing.");
            }
            else if (select == 7) {
                //(Long Dress (Bimbo))
                ...
                if (this.rubiBimbo()) this.outputText("  [rubi Ey]'s even modified the dress with false gems and a slit up the side to show off [rubi eir] leg.");
                ...
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] <= 1) this.outputText("  The dress must have come with a padded bra, because you're surprised to see Rubi with  breasts.  Though small, they help to accentuate [rubi eir] feminine figure.");
            }
            else if (select == 8) {
                ...
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] >= 4) this.outputText("  With [rubi eir] large rack, [rubi ey] looks much like a busty piratess.  From the way [rubi eir] breasts strain at the top, you're certain that one wrong breath could send a button flying across the room.");
                else this.outputText("  With [rubi eir] relatively flat chest, however, you wonder if perhaps it makes [rubi em] look a bit too much like a man.  An effeminate man, but a man nonetheless.");
                //Cock variants:
                //(9" or higher)
                if (this.flags[kFLAGS.RUBI_COCK_SIZE] >= 9) this.outputText("  [rubi Eir] generous cock is on display, much to [rubi eir] chagrin, as a bulge running down the length of one pant leg.");
                else this.outputText("Luckily, given [rubi eir] relatively small package, [rubi eir] bulge is kept nice and secret even with the tightness of the clothing.");
            }
            else if (select == 9) {
                //(Nurse Clothes (Bimbo))
                ...
                //(D breasts or higher)
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] >= 4) this.outputText("  Rubi's large breasts strain the fabric, drawing the dress further up [rubi eir] body and showing off more of [rubi eir] legs and ass.");
            }
            //(Slutty Swimwear (Normal))
            else if (select == 10) {
                //(Slutty Swimwear)
                ...
                //(D breasts or higher)
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] >= 4) this.outputText("  Rubi's tremendous cleavage threatens to spill out of the skimpy bikini top with every movement, and from the red faces around the bakery, you wonder if that's already happened.");
                else if (this.flags[kFLAGS.RUBI_BREAST_SIZE] <= 1) this.outputText("  The top only emphasizes Rubi's relatively flat chest, though with the rest of [rubi eir] figure, [rubi ey] still looks extremely feminine.");
            }
            //(Minidress Outfit (Bimbo))
                ...
                if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("  [rubi Eir] skirt rides so high you catch glimpses of [rubi eir] unsurprisingly unpantied crotch, and the enticing package held within.  You wonder how many people have gotten a surprise with their drinks today!");
            }
            //(Bondage Straps)
            else if (select == 12) {
            ...
            //(Risque Waitress Uniform)
                ...
                if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText(this.rubiCock(33) + ", clad in a cute little frilly pink cocksock");
                if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0 && this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText(" and ");
                if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0) {
                    this.outputText("feminine mound trapped beneath a pair of pink panties");
                    //(if Bimbo Brain)
                    if (this.rubiBimbo() && this.flags[kFLAGS.RUBI_DEBIMBO] == 0) this.outputText(" trimmed feminine mons, free and open to the air");
                }
                this.outputText(".");
                //(D breasts or higher)
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] >= 4) this.outputText("  Rubi's breasts, large as they are, show off a decent amount of sideboob and threaten to spill out at any moment.  Judging by the flushed faces of some customers, this may have already happened.");
            ...
            if (this.rubiBimbo() && this.flags[kFLAGS.RUBI_DEBIMBO] == 0) this.outputText("Like, h");
            ...
            if (select == 4) {
                ...
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] >= 4) this.outputText("  In an odd sort of way, [rubi ey] looks like an adventurer who met the wrong end of a witch's curse.");
            }
            ...
            if (select == 1) {
                ...
                if (this.rubiBimbo() && this.flags[kFLAGS.RUBI_DEBIMBO] == 0) {
                    ...
                    if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0 && this.flags[kFLAGS.RUBI_COCK_SIZE] == 0) this.outputText(" and wet cunt");
                    else if (this.flags[kFLAGS.RUBI_NO_CUNT] > 0 && this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText(" and limp cock");
                    else this.outputText(", wet cunt, and limp cock");
                    this.outputText(" to anyone behind [rubi em]");
                }
                else this.outputText("excited to see Rubi's choice of frilly pink panties clinging to [rubi eir] supple ass");
                this.outputText(".");
            }
            //(Dashing Outfit)
            else if (select == 8) this.outputText("\n\nAs [rubi ey] bends over, you get a good look at [rubi eir] ass, emphasized by the tight leather pants.");
            //(Adventurer's Outfit)
            else if (select == 15) {
                ...
                //If Bimbo Body:
                if (this.rubiBimbo()) this.outputText("  You even spy a wet patch in the crotch, with rivulets of moisture running down the legs, which [rubi ey] must have trouble hiding... if [rubi ey] even bothers to try.");
            }
            //(Long Dress)
            else if (select == 7) {
                this.outputText("\n\nAs [rubi ey] moves to another table, Rubi sways back and forth, as if dancing with an invisible partner.  ");
                if (this.rubiBimbo() && this.flags[kFLAGS.RUBI_DEBIMBO] == 0) this.outputText("And then [rubi ey] begins to shake [rubi eir] ass, making spanking motions and effectively ruining the air of sophistication.");
            }
            //(Nurse Uniform)
            else if (select == 9) {
                this.outputText("\n\nAs [rubi ey] bends over, you get a good look up [rubi eir] skirt, ");
                ...
                else this.outputText("excited to see Rubi's choice of frilly white panties clinging to [rubi eir] supple ass");
                this.outputText(".");
            }
            //(Rubber Fetish Outfit)
                ...
                else this.outputText("excited to see a pair of translucent rubber panties clinging to [rubi eir] supple ass");
                this.outputText(".");
            }
            //(Suitclothes)
            else if (select == 2) {
            ...
            //(Bimbo Minidress)
                ...
                else this.outputText("excited to see a cute, little thong wedged into [rubi eir] supple ass");
                this.outputText(".");
            }
            //(Bodysuit)
            else if (select == 6) {
                ...
                //If Bimbo Body:
                if (this.rubiBimbo()) this.outputText("  You even spy a moist patch in the crotch, which must be impossible to hide... if [rubi ey] even bothers to try.");
            }
            //(Bondage Straps)
            else if (select == 12) {
            ...
            //(Risque Waitress Uniform)
                ...
                else this.outputText("excited to see a pair of pink frilly panties clinging to [rubi eir] supple ass");
                this.outputText(".");
            }
        }
    ...
    public rubisFuckingHouseYouPervert(): void {
        ...
        //Bimbo Shopping!
        else if (this.rubiBimbo() && (this.player.findPerk(PerkLib.BimboBrains) >= 0 || this.player.findPerk(PerkLib.FutaFaculties) >= 0) && Rubi.rand(7) == 0) {
    ...
    public fuckRubi(): void {
        ...
        if (size == 0) {
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] <= 5) this.outputText("I guess we're in the same boat.</i>\"");
            else if (this.rubiBimbo() && this.flags[kFLAGS.RUBI_DEBIMBO] == 0) this.outputText("That's too bad, I was soooo looking forward to a real fucking.</i>\"");
            else this.outputText("That's pretty sad, getting shown up by a dainty little thing like me...</i>\"");
        }

        else if (size == 1) this.outputText("\n\n\"<i>Oh wow, you're quite the stud,</i>\" Rubi says, eyes sparkling with anticipation.");
        ...
        if (this.player.hasCock()) this.outputText("swollen member");
        else this.outputText("impressive love button");
        ...
        if (this.player.hasCock()) this.outputText("your cockhead");
        else this.outputText("the tip of your clit");
        this.outputText(", sending shivers of pleasure up your spine.");
        //(If Cock)
        if (this.player.hasCock()) this.outputText("  Amazingly, with the expert ministrations of your lover's mouth, you can't hold yourself in much longer, and begin to tense up, but at the last minute you pull yourself away from Rubi's grip and lips, halting yourself before you can cum.");
        ...
        if (size < 2) {
            ...
            if (this.flags[kFLAGS.RUBI_NO_CUNT] > 0) this.outputText("tailhole");
            ...
            if (this.rubiBimbo()) this.outputText(", not that [rubi ey] needs the extra lubrication, as [rubi ey]'s practically dripping like a faucet");
            this.outputText(".");
        ...
        //(These two paragraphs for cocks over 24 area)
            ...
            if (this.flags[kFLAGS.RUBI_NO_CUNT] > 0) this.outputText("balls");
            ...
            //(if bimbo)
            if (this.rubiBimbo()) this.outputText(", not that [rubi ey] needs the extra lubrication, as she's practically dripping like a faucet, which dribbles down [rubi eir] legs suitably enough");
            this.outputText(".");
            this.outputText("\n\nYou press forward, your enormous ");
            if (this.player.hasCock()) this.outputText(this.cockDescript(x));
            else this.outputText("[clit]");
            this.outputText(" slipping between Rubi's thighs, which grip you tightly.  You feel [rubi eir] ");
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("hard cock");
            else this.outputText("stiff clit");
            this.outputText(" pressing against the top of ");
            if (this.player.hasCock()) this.outputText("your dick");
            else this.outputText("your clit");
        ...
        if (this.flags[kFLAGS.RUBI_ORGASM_DENIAL] > 0 && !this.rubiBimbo()) this.outputText("  You give Rubi's ass a hard smack and remind [rubi em] that no matter what happens, [rubi ey] still can't cum yet.  [rubi Eir] eyes flutter open and [rubi ey] actually whimpers a little, but can't form the words to object.");
        //(If Orgasm Denial is NOT activated; male/herm Rubi)
        else if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("  Rubi’s body shivers and trembles, and almost collapses.  [rubi Ey] grasps at the blankets with wild abandon as ropes of pearly white cum erupt from [rubi eir] cock, splattering against [rubi eir] chest and the blankets.");
        //(Female Rubi)
        else this.outputText("  Rubi's body shivers and trembles, and [rubi ey] almost collapses.  [rubi Ey] grasps at the blankets with wild abandon and gives and orgasmic scream as [rubi eir] cunt clenches and unclenches fiercely.  A spray of femjuices erupts outwards from [rubi em], coating your crotch and dripping down both yours and [rubi eir] legs.");
        ...
        if (this.player.hasCock() && this.player.cockArea(x) <= this.rubiCapacity()) {
            ...
            if (this.player.cumQ() < 250) this.outputText("Cum dribbles out from [rubi eir] hole as you give a couple extra thrusts.");
            else if (this.player.cumQ() < 1000) this.outputText("A generous amount of cum trickles from [rubi eir] depths as you give a couple extra thrusts.");
            else if (this.player.cumQ() < 2000) {
                this.outputText("Cum practically floods from  [rubi eir] abused ");
                if (this.flags[kFLAGS.RUBI_NO_CUNT] > 0) this.outputText("ass");
                else this.outputText("cunt");
            ...
            if (this.player.cumQ() < 250) this.outputText("with a fair amount of your seed.");
            else if (this.player.cumQ() < 1000) this.outputText("with a generous amount of your spunk.");
            else this.outputText("with a veritable torrent of potent pearlescent spooge.");
        }
        //(Player has clit?)
        else this.outputText("  Your " + this.clitDescript() + " throbs and engorges briefly as your " + this.vaginaDescript(0) + " reflexively contracts and a warm feeling rocks through your entire body.  Femjuices run down your [legs] and relief overtakes you like a powerful wave upon a beach.");
        ...
        if (this.rubiBimbo() && this.flags[kFLAGS.RUBI_DEBIMBO] == 0) this.outputText("like, totally ");
        else this.outputText("absolutely ");
    ...
    public dildoFuckRubi(): void {
        ...
        this.rubiSprite();
        //(If PC has no cock)
        if (!this.player.hasCock()) this.outputText("You don't have the equipment to give Rubi a good fucking, but there is one thing you do have.");
        //(If PC has cock)
        else this.outputText("While you do have the requisite equipment to give Rubi a good fucking, you'd much rather use something else.");
        //(Add)
        ...
        //(If PC has a cunt)
        if (this.player.hasVagina()) this.outputText("You press the tip into your cunt quickly, giving it a taste of what is to come.  The brief contact is enough to make you shudder slightly with pleasure.  ");
        //(If PC has fuckable nipples)
        if (this.player.hasFuckableNipples()) this.outputText("You let the dildo meander up your body until it reaches your breasts, and with a bit of a grin you insert the tip right into one nipple, enjoying the feeling of your breasts swell up with arousal.  You do the same with the other nipple, and take a minute going back and forth.  ");
        ...
        //(Normal/Incubus Rubi)
        if (!this.rubiBimbo()) this.outputText("They part shakily and [rubi eir] tongue reaches out to taste the dildo.  ");
        //(Bimbo Rubi)
        else this.outputText("They easily part, and [rubi eir] tongue takes a long slurp, just to taste the dildo.  ");
        ...
        if (this.flags[kFLAGS.RUBI_NO_CUNT] == 1) this.outputText("against [rubi eir] tight ass");
        else this.outputText("against [rubi eir] tight pussy");
        ...
        //(If player has pussy)
        if (this.player.hasVagina()) this.outputText("  Your " + this.clitDescript() + " stiffens, aching for attention.  It peeks from your folds like a shy little lewd faerie.");
        //(If player has a cock)
        if (this.player.hasCock()) this.outputText("  Meanwhile, [eachCock] stiffens, throbbing as the drug overtakes it.");
        if (this.player.lactationQ() > 0) this.outputText("  As the warmth spreads up your body, not even your breasts are left alone.  Your nipples harden and begin leaking milk, which pours down the sides of your body and is soaked up into the cushions beneath you.");
        ...
        //(Normal/Incubus Rubi, Orgasm Denial off)
            ...
            if (!this.rubiIncubus()) this.outputText("little cock");
            else this.outputText("demonic cock");
        ...
        //(PC has cock?)
        if (this.player.hasCock()) this.outputText("[eachCock], stroking it fervently, in desperate need of release.");
        //(PC doesn't have cock, but has pussy?)
        else if (this.player.hasVagina()) this.outputText("your " + this.clitDescript() + ", rubbing it frantically, in desperate need of release.");
        //(PC doesn't have either, but breasts above A-cup?)
        else this.outputText("your [chest], rubbing your nipples vigorously, in desperate need of release.");
        //(Bonus, fuckable nipples?)
        if (this.player.hasFuckableNipples()) this.outputText("  Your fingers easily slip inside each nipple, pumping in and out like little cocks.");
        //(No cock, no pussy, no breasts?)
        if (this.player.gender == 0 && this.player.biggestTitSize() == 0) this.outputText("  Your hands roam all along your body, from your bare crotch to your nipples, even through your hair, all in search of something to give you the release you so desperately crave.");
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.cumQ() < 250) this.outputText("splattering messily");
            else if (this.player.cumQ() < 500) this.outputText("coating you lightly");
            else if (this.player.cumQ() < 1000) this.outputText("covering you in a copious layer of cum");
            else this.outputText("covering you in a thick layer of your potent seed");
            this.outputText(".");
        }
        //(PC has pussy?)
        if (this.player.hasVagina()) this.outputText("  Your " + this.vaginaDescript(0) + " clenches down on the enormous invader, which at this point feels like an enormous steel rod lodged firmly inside you, and a spray of fem-juices erupts from your nethers, coating the already slick dildo and Rubi.");
        //(PC lactating?)
        if (this.player.lactationQ() > 0) this.outputText("  Your breasts shudder and quake as the orgasmic tsunami washes over them, spraying your sweet smelling milk all over yourself, Rubi and the couch.");
        ...
        this.outputText("\n\nRubi rubs [rubi eir] ");
        if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0) this.outputText("pussy");
    ...
    public teaseRubi(): void {
        ...
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) {
            ...
            if (this.flags[kFLAGS.RUBI_SKIN] == 0) this.outputText("pale");
            else if (this.flags[kFLAGS.RUBI_SKIN] == 2) this.outputText("fur-covered");
            else this.outputText("red");
        ...
        //Girl Rubi!
            ...
            if (this.flags[kFLAGS.RUBI_SKIN] == 0) this.outputText("pale");
            else if (this.flags[kFLAGS.RUBI_SKIN] == 1) this.outputText("crimson");
            else if (this.flags[kFLAGS.RUBI_SKIN] == 2) this.outputText("furred");
            else this.outputText("striped");
            ...
            if (this.rubiBimbo() && this.flags[kFLAGS.RUBI_DEBIMBO] == 0) this.outputText("like, ");
    ...
    public releaseRubi(): void {
        ...
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("[rubi eir] [rubi cock] straining at the seams, trying to break free.");
        else this.outputText("clinging tight to [rubi eir] flat little mound.");
        ...
        if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 3) {
            ...
            if (this.flags[kFLAGS.RUBI_BALLS_TYPE] == 1) this.outputText(", trapped in their little sissy sack");
            this.outputText(", which you think have swelled somewhat, although you haven't been teasing [rubi em] long.");
        }
        //(if Blueballs 4-9)
        else if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 9) this.outputText("  Not to mention [rubi eir] sizeable balls, which seem to have swelled up to twice their normal size, thanks to your relentless teasing.  They even have a slight bluish tinge to them, although it's not immediately visible.");
        else this.outputText("  Not to mention [rubi eir] enormous balls, which, thanks to your endless teasing, are seemingly the size of baseballs.  They even have a noticeable blue tinge to them, obviously aching for relief.");
        ...
        if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 3) this.outputText("swollen");
        else if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 9) this.outputText("slightly blue");
        else this.outputText("incredibly swollen blue");
        ...
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("P-spot");
        else this.outputText("G-spot");
        ...
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("rectum");
        else this.outputText("folds");
        ...
        //(Blueballs 1-3)
        if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 3) this.outputText("  It's quickly followed by another, less forceful jet of pearly white cum, which lands on [rubi eir] face, and several even smaller jets which splatter the area around [rubi em].  [rubi Eir] cock twitches and spasms, then slowly softens.");
        //(Blueballs 4-5)
        else if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 5) this.outputText("  It's quickly followed by another, less forceful jet of pearly white cum, then another, and another, each landing closer to [rubi eir] face.  Finally, it gives a few more token jerks, spurting out tiny globules of cum onto the surrounding area.  [rubi Eir] cock twitches and spasms, squirting nothing but air, as if [rubi eir] body is still craving release.");
        //(Blueballs 6-7)
        else if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 7) this.outputText("  It's quickly followed by another, more forceful jet of pearly white cum which splatters against the ceiling, dripping down onto the both of you.  Another spurt of cum bursts forth, then another, and another, each one liberally covering Rubi's stomach, chest and face.  [rubi Eir] cock finally gives a few last twitches and spasms, squirting nothing but air, as if [rubi eir] body is still craving release.");
        //(Blueballs 8-9)
        else if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 9) this.outputText("  It's quickly followed by another, more forceful jet of pearly white cum, which splatters against the ceiling, dripping down onto the both of you, and then another, and another.  It isn't long before the ceiling has a huge white cum-covered blotch right above you, drizzling down on you like dew dripping from trees.  Rubi's cock doesn't stop there, however, it continues twitching and spurting [rubi eir] seed until [rubi ey] is completely covered in the stuff.  Finally, mercifully, it ends, though [rubi eir] [rubi cock] continues to spasm, squirting nothing but air, as if [rubi eir] body is still craving release.");
        //(Blueballs 10+)
        else this.outputText("  It's quickly followed by another, more powerful jet of pearly white cum, which splatters against the ceiling, dripping down onto the both of you, and then another, and another.  It isn't long before the ceiling has a huge white cum-covered blotch right above you, drizzling down on you like dew dripping from the trees.  Rubi's cock doesn't stop there, however, it continues twitching and spurting [rubi eir] seed, liberally covering [rubi em]self, you and the surrounding area until everything appears to be in a white haze.");
        ...
        //(Blueballs 1-5)
        if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 5) this.outputText("You clean yourself up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.");
        //(Blueballs 6-9)
        else if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 9) this.outputText("You head into the bathroom and wash the cum off you, taking a short shower.  Afterwards, you dry up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.");
        //(Blueballs 10+)
        else this.outputText("You head into the bathroom and wash the cum off you, no easy feat to be sure.  Afterwards, you dry up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.  You certainly don't want to be there when [rubi ey] wakes up, lest you have to clean the entire house with [rubi em].");
    ...
    private interruptTheNTRsYouCrazyFool(): void {
        ...
        if (this.silly()) this.outputText("waifu");
    ...
    private waitAndGetNTRedLikeTheBoyBitchYouAre(): void {
        ...
        if (!this.player.isTaur()) {
            ...
            else if (this.player.hasCock()) this.outputText("[eachCock]");
            this.outputText(".  ");
        }
        else this.outputText("\n\n");
        ...
        if (this.silly()) this.outputText("They're even doing positions that aren't available in the game!  ");
    ...
    private chocoRubiMassage(): void {
        ...
        if (this.player.cor >= 66) this.outputText("start sensually undressing, turning the simple act into a seductive show that causes Rubi's petite boner to jerk unpredictably beneath [rubi eir] clothes.  A small, damp stain appears at the tip by the time you finish your salacious tease.");
        else if (this.player.cor >= 33) this.outputText("get undressed without much pause or fanfare.  Rubi makes a show of turning [rubi eir] head, but you can see [rubi em] peeping glimpses out of the corners of [rubi eir] eyes, and [rubi eir] boner seems to be bigger and harder than it was moments before.");
        else {
            ...
            if (this.player.biggestCockArea() >= 40 || this.player.biggestTitSize() >= 8) this.outputText(", not that it does much good");
            this.outputText(".");
        }
        ...
        if (this.flags[kFLAGS.RUBI_BLU_BALLS] < 3) this.outputText("small");
        else if (this.flags[kFLAGS.RUBI_BLU_BALLS] < 8) this.outputText("big");
        else this.outputText("huge, stretched");
    ...
    private goInRubisClosetSoThatYouCanComeOutOfTheCloset(): void {
        ...
        if (this.rubiBimbo() && this.flags[kFLAGS.RUBI_DEBIMBO] == 0) this.outputText("Oh my god!  ");
    ...
    private rubiAppearance(): void {
        ...
        if (this.flags[kFLAGS.RUBI_SKIN] == 0) this.outputText("pale, almost like porcelain");
        else if (this.flags[kFLAGS.RUBI_SKIN] == 1) this.outputText("an exotic cherry red");
        else if (this.flags[kFLAGS.RUBI_SKIN] == 2) this.outputText("covered with a fine layer of silky-smooth pink fur");
        else this.outputText("an exotic cherry red which contrasts well with a grey belly and stripes that run up and down [rubi eir] legs and arms");

        if (this.flags[kFLAGS.RUBI_SKIN] != 2) this.outputText(" and completely hairless, except for [rubi eir]");
        else this.outputText(", and [rubi ey] sports");

        if (this.flags[kFLAGS.RUBI_HAIR_LENGTH] == 0) this.outputText(" short");
        else if (this.flags[kFLAGS.RUBI_HAIR_LENGTH] == 1) this.outputText(" shoulder-length");
        else this.outputText(" long");
        if (this.flags[kFLAGS.RUBI_HAIR] == 0) this.outputText(" black");
        else this.outputText(" blonde");
        this.outputText(" hair, which [rubi ey] keeps cut in a cute, feminine style.  Rubi's face is soft and rounded");
        if (this.flags[kFLAGS.RUBI_WHISKERS] > 0) this.outputText(", with a cute set of whiskers sprouting from [rubi eir] cheeks");
        this.outputText(", [rubi eir] lips painted a ");
        //(normal/furry skin)
        if (this.flags[kFLAGS.RUBI_SKIN] == 0 || this.flags[kFLAGS.RUBI_SKIN] == 2) this.outputText("crimson red");
        //(red/siren skin)
        else this.outputText("dusky black");
        this.outputText(", [rubi eir] ");
        //if(flags[kFLAGS.RUBI_EYE_TYPE] == 1) outputText("slitted ");
        if (this.flags[kFLAGS.RUBI_EYE_TYPE] == 0 || this.rubiBimbo()) this.outputText("green");
        ...
        if (this.flags[kFLAGS.RUBI_HORNTYPE] > 0) this.outputText("Two tiny horns extend upwards from [rubi eir] hairline, solid black in color.  ");
        this.outputText("A pair of ");
        if (this.flags[kFLAGS.RUBI_EAR_TYPE] == 0) this.outputText("cute, normal ears protrude from the sides of [rubi eir] head, each one pierced with a small dangling earring");
        else if (this.flags[kFLAGS.RUBI_EAR_TYPE] == 1) this.outputText("cute, cat-like ears pop up from the top of [rubi eir] head, each one seemingly able to move independently");
        else if (this.flags[kFLAGS.RUBI_EAR_TYPE] == 2) this.outputText("feathered ears protrude from the sides of [rubi eir] head, each one fluttering cutely from time to time, and each pierced with a small dangling earring");
        this.outputText(".");

        this.outputText("\n\n[rubi Ey] has a ");
        if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 1) this.outputText("flat-chest ");
        else this.outputText("[rubi breasts]");
        this.outputText(" supporting two perky ");
        if (this.flags[kFLAGS.RUBI_NIPPLE_TYPE] == 1) this.outputText("black ");
        this.outputText("nipples.");

        this.outputText("\n\nBetween [rubi eir] legs is a ");
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) {
            this.outputText(this.rubiCock() + ", which starts to perk up under your scrutiny. From experience, you know it can reach a impressive " + this.flags[kFLAGS.RUBI_COCK_SIZE] + " inches when erect.  ");
            this.outputText("Under [rubi eir] cock swings a set of ");
            if (this.flags[kFLAGS.RUBI_BLU_BALLS] >= 4 && this.flags[kFLAGS.RUBI_BLU_BALLS] < 9) this.outputText("cum-swollen ");
            else if (this.flags[kFLAGS.RUBI_BLU_BALLS] >= 9) this.outputText("engorged, blue-tinged ");
            this.outputText("testicles");
            if (this.flags[kFLAGS.RUBI_BALLS_TYPE] == 1) this.outputText(", which cling tight to [rubi eir] body in a cute spherical pouch");
        }
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0 && this.flags[kFLAGS.RUBI_NO_CUNT] == 0) this.outputText(" and a ");
        if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0) {
            this.outputText("cute ");
            if (this.flags[kFLAGS.RUBI_CUNTTYPE] > 0) this.outputText("black ");
        ...
        if (this.flags[kFLAGS.RUBI_FEET] == 1) this.outputText("Finally, Rubi stands on a long black spike that makes [rubi em] walk as though [rubi ey] were wearing high heels.");
    ...
    private dressUpRouter(arg: string): void {
        ...
        else if (arg == "A Green Adventurer's Outfit") this.goOnAnAnalAdventureRubiNotReallyJustAnAdventurersOutfit();
        ...
        else if (arg == "A Dashing Outfit With Tight Leather Pants") this.putOnADashingOutfitYouWhore();
        else if (arg == "Nurse's Clothes") this.rubiPutsOnNursesClothesSoSheCanCheckYourTemperature();
        ...
        else if (arg == "An Adventurer's Outfit") this.goOnAnAnalAdventureRubiNotReallyJustAnAdventurersOutfit();
    ...
    private putRubiInAWaitressUniform(): void {
        ...
        if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 1) this.outputText(", though it's obvious [rubi ey] doesn't need it with [rubi eir] flat-chest");
        ...
        if (this.flags[kFLAGS.RUBI_SKIN] != 2) this.outputText("hairless");
        else this.outputText("furred");
        ...
        if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 1) this.outputText(", showing off some of [rubi eir] flat chest");
        else this.outputText(", showing off a generous portion of cleavage");
        this.outputText(", and tucks the hem of the blouse into the skirt.  Finally, Rubi selects a pair of ");
        if (this.flags[kFLAGS.RUBI_FEET] == 1) this.outputText("what appears to be flats, with a hole in the heel for [rubi eir] demonic heels");
        else this.outputText("short heels");
        ...
        if (!this.rubiBimbo() || this.flags[kFLAGS.RUBI_DEBIMBO] > 0) this.outputText("girly panties");
        else this.outputText("naked nethers");
        this.outputText(".");
    ...
    private putOnADashingOutfitYouWhore(): void {
        ...
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("tucking [rubi eir] cock firmly into the triangular strip of cloth covering [rubi eir] crotch, while ");
        ...
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText(", not to mention the sizeable package in [rubi eir] crotch");
        this.outputText(".");
        ...
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("  Combined with the sizeable bulge in the crotch, Rubi looks like quite the dashing, if slutty, piratess.");
    ...
    private goOnAnAnalAdventureRubiNotReallyJustAnAdventurersOutfit(): void {
        ...
        if (this.silly()) this.outputText("collector of waifus!");
        else this.outputText("layer of ladies");
        ...
        if (this.flags[kFLAGS.RUBI_EAR_TYPE] == 1) this.outputText(", covering [rubi eir] feline ears");
        this.outputText(".  The boots that go with the outfit are mid-calf length and made of supple, fine leather");
        if (this.flags[kFLAGS.RUBI_FEET] == 1) this.outputText(" with a hole cut out in the heel, apparently for Rubi's demonic heels to fit through");
    ...
    private giveRubiALongDressToHideHerHideousBody(): void {
        ...
        if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 1) this.outputText(", despite the fact that [rubi ey] clearly doesn't need one");
        ...
        if (this.flags[kFLAGS.RUBI_SKIN] == 1 || this.flags[kFLAGS.RUBI_SKIN] == 3) this.outputText(" (or darkens at least)");
        ...
        if (this.flags[kFLAGS.RUBI_FEET] == 1) this.outputText(" with the heels cut off and a hole for Rubi's demonic heel to fit through");
    ...
    private rubiPutsOnNursesClothesSoSheCanCheckYourTemperature(): void {
        ...
        if (this.flags[kFLAGS.RUBI_SKIN] == 2) this.outputText("furred ");
        else this.outputText("hairless ");
        ...
        if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 1) this.outputText("heart");
        ...
        if (this.flags[kFLAGS.RUBI_FEET] > 0) this.outputText(" with the heels removed and a hole for [rubi eir] demonic heels");
        this.outputText(".");
    ...
    private rubiHasARubberFetish(): void {
        ...
        if (this.flags[kFLAGS.RUBI_SKIN] != 2) this.outputText("skin");
        else this.outputText("furred body");
        ...
        //(If Not Bimbo)
        if (!this.rubiBimbo() || this.flags[kFLAGS.RUBI_DEBIMBO] > 0) this.outputText("[rubi Ey] first takes a pair of transparent rubber panties and slides them up [rubi eir] legs until they lovingly cling around [rubi eir] ass.  ");
        ...
        if (this.flags[kFLAGS.RUBI_FEET] == 1) this.outputText(", the actual heel cut off to make room for [rubi eir] demonic heel,");
    ...
    private putOnASuitYouSlut(): void {
        ...
        if (this.flags[kFLAGS.RUBI_FEET] == 1) this.outputText(", with the actual heel cut off to make room for [rubi eir] demonic heel,");
    ...
    private putOnATubeTopYouWhore(): void {
        ...
        if (this.flags[kFLAGS.RUBI_BREAST_SIZE] > 2) this.outputText(", which strain the flimsy piece of material");
        this.outputText(".  Completing the look is a simple pair of sandals");
        if (this.flags[kFLAGS.RUBI_FEET] == 1) this.outputText(", with a hole cut in the sole to make room for Rubi's demonic heels");
        this.outputText(".");
        this.outputText("\n\nNow that Rubi is dressed up, what do you want to do?");
    ...
    private putOnSluttySwimwearYouSkank(): void {
        ...
        if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 3) this.outputText(" sit loosely over Rubi's small chest");
        else this.outputText("strain thanks to Rubi's ample busom");
        this.outputText(".");

        this.outputText("\n\nTopping it off, your demonic lover slips on a pair of sandals");
        if (this.flags[kFLAGS.RUBI_FEET] > 0) this.outputText(", with a hole in the sole to make room for [rubi eir] demonic heels");
        this.outputText(".");
    ...
    private putOnADressYouBimbo(): void {
        ...
        if (this.flags[kFLAGS.RUBI_FEET] > 0) this.outputText(", with the heels removed to make room for [rubi eir] own demonic heels");
        this.outputText(".");
    ...
    private putOnMyBodysuitYouWhore(): void {
        ...
        if (this.flags[kFLAGS.RUBI_SKIN] != 2) this.outputText("smooth");
        else this.outputText("furred");
        ...
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("[rubi eir] package is neatly tucked away");
        else this.outputText("a red rose is neatly covering up [rubi eir] pubic mound");
    ...
    private putOnBondageStrapsYouBondageTrap(): void {
        ...
        if (this.flags[kFLAGS.RUBI_SKIN] > 0) this.outputText(", or so you imagine");
    ...
    private whyTheFuckIsRubiWearingAnInquisitorsCorset(): void {
        ...
        if (this.flags[kFLAGS.RUBI_FEET] == 1) this.outputText(", with the heels removed to make room for [rubi eir] own demonic heels");
        this.outputText(".");
        this.outputText("\n\nNow that Rubi is dressed up, what do you want to do?");
        //[Date (go to bar date)] [Back]
        this.menu();
    ...
    private putOnAWaitressUniformYouWhore(): void {
        ...
        if (!this.rubiBimbo() || this.flags[kFLAGS.RUBI_DEBIMBO] > 0) {
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("Oddly enough, these panties only seem to cover Rubi's balls, leaving [rubi eir] cock right out in the open.  ");
        }
        ...
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("  To top it all off, Rubi takes a pink lacy cocksock and slips it down [rubi eir] [rubi cock], fastening it into place with a pink ribbon that [rubi ey] ties into a bow around the base of [rubi eir] cock.");
    ...
    private iceCreamDate(): void {
        ...
        if (this.flags[kFLAGS.RUBI_AFFECTION] >= 100 && this.flags[kFLAGS.RUBI_ICECREAM_CONFESSION] == 0) {
    ...
    private fancyDate(): void {
        ...
        if (this.player.gems >= 10) {
            ...
            if (this.flags[kFLAGS.RUBI_AFFECTION] >= 100 && this.flags[kFLAGS.RUBI_FANCY_CONFESSION] == 0) {
            ...
            //(if PC has met Callu)
            if (this.flags[kFLAGS.MET_OTTERGIRL] == 1) this.outputText(" though not as good as Callu's, the ottergirl fisherman you met at the lake.");
            else this.outputText(", which makes you wonder who Rubi is thinking about.  Perhaps one of [rubi eir] parents?");
    ...
    private exhibitionistDate(): void {
        ...
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText(this.rubiCock());
        else this.outputText("glistening, wet pussy");
        this.outputText(".");
    ...
    private barDate(): void {
        ...
        //(If Relationship 100, happens once)
        if (this.flags[kFLAGS.RUBI_AFFECTION] >= 100 && this.flags[kFLAGS.RUBI_BAR_CONFESSION] == 0) {
    ...
    private anallyTrainYourBitchBoySlutHowToBeAnalForDCL(): void {
        ...
        if (this.flags[kFLAGS.RUBI_TIMES_ANALLY_TRAINED] == 1) {
            ...
            if (this.rubiBimbo() || this.flags[kFLAGS.RUBI_DEBIMBO] == 0) this.outputText("totally ");
            ...
            if (this.flags[kFLAGS.RUBI_SKIN] == 1 || this.flags[kFLAGS.RUBI_SKIN] == 3) this.outputText(", or redder at least");
            else this.outputText(", what little of it you can see");
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText(", [rubi eir] dick wobbling enticingly");
            this.outputText(".");
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("pre-cum");
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] == 0) this.outputText("slick vagina");
            else this.outputText("hungry asshole");
            this.outputText(".");
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] == 0) this.outputText("cunt");
            else this.outputText("asshole");
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText(this.rubiCock() + " is standing at full attention, bobbing up and down as Rubi fucks [rubi emself] on the dildo");
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0 && this.flags[kFLAGS.RUBI_NO_CUNT] == 0) this.outputText(", and [rubi eir] ");
            if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0) this.outputText("cunt lips are glistening red with arousal, just begging for attention");
            ...
            if (this.flags[kFLAGS.RUBI_NO_CUNT] == 1) this.outputText("backside");
            else this.outputText("fuck hole");
            ...
            if (this.flags[kFLAGS.RUBI_SKIN] != 2) this.outputText("bare");
            else this.outputText("furred");
            ...
            if (this.flags[kFLAGS.RUBI_NO_CUNT] == 1) this.outputText("asshole");
            else this.outputText("cunt");
            ...
            if (this.flags[kFLAGS.RUBI_NO_CUNT] == 1) this.outputText("ass");
            else this.outputText("pussy");
            this.outputText(".");
            ...
            if (this.flags[kFLAGS.RUBI_EAR_TYPE] == 0) this.outputText("ear");
            else if (this.flags[kFLAGS.RUBI_EAR_TYPE] == 1) this.outputText("feline ear");
            else this.outputText("feathered ear");
        ...
        //Rubi's capacity won't go larger than 100.
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText(this.rubiCock());
            else this.outputText("sweet pussy");
            ...
            if (this.flags[kFLAGS.RUBI_SKIN] == 0) this.outputText("go bright red");
            else if (this.flags[kFLAGS.RUBI_SKIN] == 1 || this.flags[kFLAGS.RUBI_SKIN] == 3) this.outputText("go a vivid purple");
            else this.outputText("would be blushing bright red now, were it not for the fine layer of fur");
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("ass");
            else this.outputText("pussy");
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("asshole");
            else this.outputText("cunt");
            ...
            if (this.flags[kFLAGS.RUBI_NO_CUNT] == 1) this.outputText("anus");
            else this.outputText("cunt");
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("You can feel your lover's cock straining against your leg, aching for some release.  You idly press your foot against it, feeling a slick layer of precum coating it from top to bottom already.  ");
            ...
            if (this.flags[kFLAGS.RUBI_EYE_TYPE] == 0 || this.rubiBimbo()) this.outputText("green");
            else if (this.flags[kFLAGS.RUBI_EYE_TYPE] == 1) this.outputText("purple");
            else this.outputText("shiny black");
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("  Even, no, <i>especially</i> [rubi eir] cock twitches and spasms jerking upwards and splattering globules of sticky white goo against the inside of Rubi's babydoll before dripping down onto the floor between [rubi eir] legs, creating a rapidly growing puddle.");
    ...
    private getFuckedByRubi(): void {
        ...
        //(normal/furry skin)
        if (this.flags[kFLAGS.RUBI_SKIN] == 0 || this.flags[kFLAGS.RUBI_SKIN] == 2) this.outputText("red");
        else this.outputText("black");
        ...
        if (this.flags[kFLAGS.RUBI_EYE_TYPE] > 0) this.outputText(" slitted");
        ...
        if (!this.player.hasVagina()) this.outputText("asshole");
        else this.outputText("cunt");
        this.outputText(" of yours all ready for me.</i>\"  Rubi's head descends between your legs");
        if (this.player.hasCock()) this.outputText(", ignoring your " + this.multiCockDescriptLight() + " completely,");
        ...
        if (!this.player.hasVagina()) this.outputText("ass");
        else this.outputText("moistening pussy");
        this.outputText(".  You let out a small moan and lean backwards, taking your eyes off the ");
        if (this.flags[kFLAGS.RUBI_HAIR] == 0) this.outputText("black-haired");
        else this.outputText("blonde");
        ...
        if (this.rubiGetCockType() == CockTypesEnum.HUMAN) this.outputText("pink, mushroom-like tip");
        else if (this.rubiGetCockType() == CockTypesEnum.DEMON) this.outputText("purple, nodule-laden tip");
        else if (this.rubiGetCockType() == CockTypesEnum.HORSE) this.outputText("blunted, musky tip");
        else if (this.rubiGetCockType() == CockTypesEnum.TENTACLE) this.outputText("tentacle-ringed crown");
        else this.outputText("<b>ERROR: Rubi Cock Type set invalid. Currently: " + this.rubiGetCockType() + "</b>  ")
        ...
        if (!this.player.hasVagina()) this.player.buttChange(this.flags[kFLAGS.RUBI_COCK_SIZE] * this.flags[kFLAGS.RUBI_COCK_SIZE] / 6, true, true, false);
        else this.player.cuntChange(this.flags[kFLAGS.RUBI_COCK_SIZE] * this.flags[kFLAGS.RUBI_COCK_SIZE] / 6, true, true, false);
        ...
        if (!this.player.hasVagina()) this.outputText("prostate");
        else this.outputText("g-spot");
        ...
        if (!this.player.hasVagina()) this.outputText("bowels");
        else this.outputText("womb");
        this.outputText(".  ");
        ...
        //(Blueballs 10)
        else this.outputText("Your insides rumble dangerously and your stomach actually swells up, gaining a noticeable paunch as Rubi's lavish load coats your insides, filling every available nook and cranny and even creating new ones.  What doesn't fit slips out of your abused hole in a flood, coating the bed and floor beneath you in a slick, white glaze.  ");
    ...
    private rubiHotdogging(): void {
        ...
        if (this.flags[kFLAGS.RUBI_SKIN] == 0) this.outputText("pale skinned");
        else if (this.flags[kFLAGS.RUBI_SKIN] == 1 || this.flags[kFLAGS.RUBI_SKIN] == 3) this.outputText("red skinned");
        else this.outputText("fur-covered");
        this.outputText(" " + this.rubiMF("boy", "girl") + " lays naked on your lap.  Rubi giggles and dips a hand between [rubi eir] thighs ");
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("taking hold of [rubi eir] cock");
        else this.outputText("rubbing a finger along [rubi eir] clit");
        ...
        if (this.flags[kFLAGS.RUBI_SKIN] != 2) this.outputText("tender skin");
        else this.outputText("silky fur");
        ...
        if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0) this.outputText("cunt moistening");
        if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0 && this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText(" and ");
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("cock hardening");
        ...
        if (this.flags[kFLAGS.RUBI_SKIN] == 0) this.outputText("pale skin");
        else if (this.flags[kFLAGS.RUBI_SKIN] == 1 || this.flags[kFLAGS.RUBI_SKIN] == 3) this.outputText("crimson skin");
        else this.outputText("fur-covered flesh");
        ...
        if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0) this.outputText("clit");
        else this.outputText("perineum");
        this.outputText(".");
    ...
    private teaseButtjobs(): void {
        ...
        if (this.flags[kFLAGS.RUBI_NO_CUNT] == 1) this.outputText("prostate");
        else this.outputText("g-spot");
        ...
        if (this.player.cumQ() <= 50) this.outputText("in one go, with a small, proud look on [rubi eir] face");
        else if (this.player.cumQ() <= 150) this.outputText("with one, throat-straining swallow, a small, proud smile taking [rubi eir] face when [rubi ey] stops panting for breath");
        else this.outputText("as it comes, mouthful after mouthful of cum practically injected straight into [rubi eir] stomach. There's so much of it, [rubi ey] can't help but smile mid-way through, proud to take such a huge load");
        if (this.player.cumQ() >= 1000) this.outputText(".  After guzzling down so much, you're certain Rubi's actually sporting a bit of a belly now, almost as if [rubi ey] were several months pregnant");
        this.outputText(".");
    ...
    private popButtjobs(): void {
        ...
        this.outputText("\n\nRubi's ");
        if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0) this.outputText("cunt clamps down on your thumb and gushes, coating your hand in [rubi eir] slick juices");
        else this.outputText("asshole quivers and attempts to suck in more of your thumb before clamping down, leaking slick fluids all over your hand");
        ...
        //(If Rubi has Penis)
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) this.outputText("  Beneath [rubi em], [rubi eir] cock jerks and twitches, loosing a spray of pearly white cum which splatters against the couch below");
        if (this.flags[kFLAGS.RUBI_BLU_BALLS] < 4) { }
        //((blueballs 4-5)
        else if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 5) this.outputText(" which is quickly followed by another, more powerful stream of cum, which splashes against [rubi eir] stomach, coating [rubi eir] [rubi breasts] in semen");
        else if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 7) this.outputText(" which is quickly followed by another, more powerful stream of cum, and a third, and a fourth.  Before you know it, Rubi's entire torso, including [rubi eir] [rubi breasts] are practically glazed with cum");
        else if (this.flags[kFLAGS.RUBI_BLU_BALLS] <= 9) this.outputText(" which is followed up by several more, each seemingly more poweful than the one previous.  Rubi's body twitches and shudders with every jet, until [rubi eir] entire torso, and the couch, is covered in the musky jizz");
        else this.outputText(" which is followed by a dozen more, each one thicker and more powerful than the last.  Rubi's cock twitches uncontrollably, just like [rubi eir] body, as it coats the entire couch, [rubi eir] entire upper body, and even some of the wall five feet away in the musky stuff");
        this.outputText(".");
        ...
        if (this.player.balls > 1) this.outputText("churn");
        else this.outputText("churns");
    ...
    private giveRubiATFItem(itype: ItemType): void {
        ...
        if (itype == this.consumables.SUCMILK || itype == this.consumables.P_S_MLK) {
            ...
            if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 7 || (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 30 && this.flags[kFLAGS.HYPER_HAPPY])) {
            ...
            if ((this.flags[kFLAGS.RUBI_COCK_SIZE] > 4) && (!this.flags[kFLAGS.HYPER_HAPPY])) {
                ...
                this.outputText(Math.round(this.flags[kFLAGS.RUBI_COCK_SIZE] * 10) / 10 + "-inch penis.");
            }
            //(If Penis is removed)
            else if (this.flags[kFLAGS.RUBI_COCK_SIZE] <= 4 && this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) {
            ...
            if (this.flags[kFLAGS.RUBI_FEET] == 0 && Rubi.rand(3) == 0 && itype == this.consumables.SUCMILK) {
            ...
            //(If Rubi gets horns)
            if (Rubi.rand(3) == 0 && this.flags[kFLAGS.RUBI_HORNTYPE] == 0 && itype == this.consumables.SUCMILK) {
            ...
            if (Rubi.rand(3) == 0 && this.flags[kFLAGS.RUBI_SKIN] != 1 && itype == this.consumables.SUCMILK) {
                this.outputText("\n\nRubi clutches [rubi eir] stomach suddenly as you notice red spots appearing on [rubi eir] ");
                if (this.flags[kFLAGS.RUBI_SKIN] != 2) this.outputText("skin");
                else this.outputText("coat of fur");
                this.outputText(".  The blotches soon spread, rapidly overtaking [rubi eir] body.  ");
                if (this.flags[kFLAGS.RUBI_SKIN] == 2) this.outputText("Meanwhile the silky soft fur that covered [rubi eir] body seems to be falling out as this red menace takes over.  ");
            ...
            if (itype == this.consumables.INCUBID) this.outputText("You hand over the bottle of milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the label before pulling out the cork and downing the whole bottle in one gulp.");
            ...
            else this.outputText("You hand over the bottle of purified milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the pink label before pulling out the cork and downing the whole bottle in one gulp.");
            ...
            if ((this.flags[kFLAGS.RUBI_BREAST_SIZE] > 0) && (!this.flags[kFLAGS.HYPER_HAPPY])) {
                ...
                this.flags[kFLAGS.RUBI_BREAST_SIZE]--;
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 0) this.flags[kFLAGS.RUBI_BREAST_SIZE] = 0;
            }
            //(If Penis size increases)
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0 && ((this.flags[kFLAGS.RUBI_COCK_SIZE] < 12 || (this.rubiGetCockType() == CockTypesEnum.HORSE && this.flags[kFLAGS.RUBI_COCK_SIZE] < 20)) ||
            ...
            //(If Rubi has no penis; instead)
                ...
                if (this.flags[kFLAGS.RUBI_SKIN] == 1 || this.flags[kFLAGS.RUBI_SKIN] == 3) this.outputText("red");
            ...
            //(If Rubi gets red skin)
            if (Rubi.rand(3) == 0 && this.flags[kFLAGS.RUBI_SKIN] != 1 && itype == this.consumables.INCUBID) {
                this.outputText("\n\nRubi clutches [rubi eir] stomach suddenly as you notice red spots appearing on [rubi eir] ");
                if (this.flags[kFLAGS.RUBI_SKIN] != 2) this.outputText("skin");
                else this.outputText("coat of fur");
                this.outputText(".  The blotches soon spread, rapidly overtaking [rubi eir] body.  ");
            ...
            //(If Rubi gets horns)
            if (Rubi.rand(3) == 0 && this.flags[kFLAGS.RUBI_HORNTYPE] == 0 && itype == this.consumables.INCUBID) {
                this.outputText("\n\nRubi's hands suddenly dart up, clutching [rubi eir] head as though [rubi ey] has a massive headache.  [rubi Ey] feels around blindly, trying to locate the source of the pain, and then you see it.  Two small bulges appear on [rubi eir] forehead, just at the hairline.  They grow larger and larger until the skin splits cleanly open revealing shiny black bone.  Horns.  They grow up and backwards, forming light ridges.  They don't quite grow completely past [rubi eir] head, however, before the growth stops.  Rubi calms and stands.  [rubi Eir] hands roam over every inch of [rubi eir] new addition as glances in the mirror, marveling at [rubi eir] demonic appearance.");
                this.flags[kFLAGS.RUBI_HORNTYPE] = 1;
            }
        }
        ...
        //Bimbo Liqueur (Modified for variant genders)
            ...
            if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) {
                ...
                if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0) this.outputText("[rubi eir] vagina");
                else this.outputText("a pristine new vagina");
            ...
            if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 7) {
                ...
                //(if flat-chested)
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] == 0) this.outputText("  Within seconds, Rubi has the chest of a pubescent girl, within a minute they're at least a B-cup.");
            ...
            if (this.player.biggestTitSize() < 1) this.outputText("yours");
        ...
        //Grows cock. (As Incubus Draft above)
            ...
            if (this.rubiGetCockType() != CockTypesEnum.HORSE) {
                ...
                if (this.flags[kFLAGS.RUBI_BLU_BALLS] >= 4 && this.flags[kFLAGS.RUBI_BLU_BALLS] < 9) this.outputText("swollen ");
                else if (this.flags[kFLAGS.RUBI_BLU_BALLS] >= 9) this.outputText("engorged ");
                if (this.flags[kFLAGS.RUBI_BALLS_TYPE] == 1) this.outputText("tight, trappy ");
        ...
        //Trap Oil
            ...
            else this.outputText("  Rubi's cunt glistens, soaked through entirely thanks to your ministrations");
            if (this.flags[kFLAGS.RUBI_BREAST_SIZE] > 0) this.outputText(", while [rubi eir] [rubi breasts] gently rock up and down with the motions of [rubi eir] breathing");
            this.outputText(".");
            ...
            if ((this.flags[kFLAGS.RUBI_BREAST_SIZE] > 1) && (!this.flags[kFLAGS.HYPER_HAPPY])) {
                this.outputText("\n\nRubi gasps a little and [rubi eir] hands immediately dart to [rubi eir] chest.  Before your eyes, Rubi's breasts begin to dwindle, shrinking down an entire cup size.  Rubi cups [rubi eir] breasts experimentally, getting used to their new, reduced, weight.");
                this.flags[kFLAGS.RUBI_BREAST_SIZE]--;
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 1) this.flags[kFLAGS.RUBI_BREAST_SIZE] = 1;
            }
            //(If Bust size increases)
        ...
        //Progression of furriness: cat ears, then whiskers and paws, then cat penis, then furry body.
            ...
            //(Gaining Cat Penis)
            else if (this.rubiGetCockType() != CockTypesEnum.CAT && this.flags[kFLAGS.RUBI_COCK_SIZE] > 0) {
        ...
        if (itype == this.consumables.PURPEAC) {
            ...
            if (this.flags[kFLAGS.RUBI_SKIN] == 0) this.outputText("skin");
            else if (this.flags[kFLAGS.RUBI_SKIN] == 1) this.outputText("red skin");
            else if (this.flags[kFLAGS.RUBI_SKIN] == 2) this.outputText("furred skin");
            else this.outputText("striped skin");
            ...
            //(Any changes? Yes)
            if (this.flags[kFLAGS.RUBI_EAR_TYPE] != 2 || this.flags[kFLAGS.RUBI_SKIN] != 3 || (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0 && this.rubiGetCockType() != CockTypesEnum.TENTACLE)) {
                ...
                if (this.flags[kFLAGS.RUBI_EAR_TYPE] != 2) {
                    ...
                    if (this.flags[kFLAGS.RUBI_EAR_TYPE] == 0) this.outputText("normal");
                    else this.outputText("feline");
                ...
                //(Shark Skin)
                    ...
                    if (this.flags[kFLAGS.RUBI_SKIN] == 0) this.outputText("its normal paleness replaced with a vibrant cherry red");
                    else if (this.flags[kFLAGS.RUBI_SKIN] == 1) this.outputText("which is barely noticeable on [rubi eir] already cherry red flesh");
                    else if (this.flags[kFLAGS.RUBI_SKIN] == 2) this.outputText("the fur rapidly retreating and replaced with bright red flesh underneath");
                ...
                //(Anemone Penis, must have a penis)
                else if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 0 && this.rubiGetCockType() != CockTypesEnum.ANEMONE) {
                    ...
                    if (this.rubiGetCockType() != CockTypesEnum.HUMAN) this.outputText(", first shifting towards a more human-like appearance");
    ...
    private rubiGrowPlusBreasts(): void {
        ...
        if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 7 || (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 30 && this.flags[kFLAGS.HYPER_HAPPY])) {
            ...
            if (this.flags[kFLAGS.HYPER_HAPPY]) {
                ...
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] > 30) this.flags[kFLAGS.RUBI_BREAST_SIZE] = 30;
            }
            else {
                // trace("Normal grow");
                if (this.flags[kFLAGS.RUBI_BREAST_SIZE] > 7) this.flags[kFLAGS.RUBI_BREAST_SIZE] = 7;
            }
        }
    ...
    private rubiPenisGroPlus(): void {
        ...
        //(If Penis increase)
        if (this.flags[kFLAGS.RUBI_COCK_SIZE] < 12 || (this.rubiGetCockType() == CockTypesEnum.HORSE && this.flags[kFLAGS.RUBI_COCK_SIZE] < 20) || (this.flags[kFLAGS.RUBI_COCK_SIZE] < 30 && this.flags[kFLAGS.HYPER_HAPPY])) {
            ...
            if (this.flags[kFLAGS.HYPER_HAPPY]) {
                ...
                if (this.rubiGetCockType() != CockTypesEnum.HORSE) {
                    if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 12) this.flags[kFLAGS.RUBI_COCK_SIZE] = 12;
                }
                else {
                    if (this.flags[kFLAGS.RUBI_COCK_SIZE] > 20) this.flags[kFLAGS.RUBI_COCK_SIZE] = 20;
                }
            }
        }
        else {
            this.outputText("\n\nSadly, apart from making [rubi eir] cock hard, it doesn't seem to have much of an effect on Rubi's ");
            if (this.rubiGetCockType() == CockTypesEnum.HORSE) this.outputText("sheath-stuffing horse-dick");
            else this.outputText("giant dick.");
        }
        this.player.consumeItem(this.consumables.GROPLUS);
        this.menu();
        this.addButton(0, "Next", this.pickAnItemToFeedRubi);
    ...
    private pickAnItemToFeedRubi(): void {
        ...
        if (gifts.length > 0) {
            ...
            if (this.silly()) this.outputText("  (Achievement Unlocked: Dress-tacular)");
            this.outputText("\n\n");
        }
    ...
    public hypnoBimboficationForRubiSloots(): void {
        ...
        if (this.flags[kFLAGS.RUBI_BREAST_SIZE] < 1) this.outputText("femmy boy");
        else if (this.flags[kFLAGS.RUBI_NO_CUNT] > 0) this.outputText("dick-girl");
        else this.outputText("little slut");
        ...
        if (kGAMECLASS.monk >= 5) this.outputText("familiar ");
        else this.outputText("strange ");
        ...
        if (this.flags[kFLAGS.RUBI_NO_CUNT] > 0) this.outputText("femboy");
        ...
        if (this.player.tallness >= 72) this.outputText("bend down");
        else this.outputText("lean forward");
        ...
        this.outputText("\n\nBeyond words, the ");
        if (this.flags[kFLAGS.RUBI_NO_CUNT] == 0) this.outputText("dick-");
        this.outputText("girl");
        if (this.flags[kFLAGS.RUBI_NO_CUNT] > 0) this.outputText("y-boy");
        ...
        this.outputText("\n\n<b>Rubi is now a ");
        if (this.flags[kFLAGS.RUBI_NO_CUNT] > 0) this.outputText("shemale ");
