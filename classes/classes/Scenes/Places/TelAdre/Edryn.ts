...
export class Edryn extends TelAdreAbstractContent implements TimeAwareInterface {
    ...
    public edrynBarTalk(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.Edryn) < 0) this.player.createStatusAffect(StatusAffects.Edryn, 0, 0, 0, 0);
        ...
        if (this.flags[kFLAGS.EDRYN_NEEDS_TO_TALK_ABOUT_KID] == 1) {
            ...
            if (this.flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] == 1) {
                ...
                this.outputText("I can't believe we've had " + Edryn.num2Text(this.flags[kFLAGS.EDRYN_NUMBER_OF_KIDS]) + " kids together!  You better come see me later on - ", false);
                if (this.flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] < 3) this.outputText("I miss the way you fill me", false);
        ...
        this.outputText("Edryn smiles pleasantly as you approach, ", false);
        if (this.player.lowerBody == LOWER_BODY_TYPE_CENTAUR) this.outputText("offering you a spot at the table across from her.  You realize your companion isn't on any kind of seat at all, and is instead 'sitting' on her lower half.  You do the same, settling down across from her, thankful this bar was made to accommodate centaurs.", false);
        else this.outputText("gesturing to a nearby stool.  You grab the seat and realize that your centaur companion isn't sitting still at all, but is instead 'sitting' on her lower half, which currently lies on the floor.", false);
        ...
        if (this.player.lowerBody == LOWER_BODY_TYPE_CENTAUR && this.player.totalCocks() > 0) {
            ...
            if (this.player.cocks[0].cockType == CockTypesEnum.HORSE) this.outputText("pours out of its sheath", false);
            ...
            //Non horsedick
                ...
                if (this.player.cor > 50 || this.player.statusAffectv1(StatusAffects.Edryn) > 0 || this.flags[kFLAGS.LOW_STANDARDS_FOR_ALL]) {
                    ...
                    if (this.player.cocks[0].cockType == CockTypesEnum.TENTACLE) this.outputText("  She smiles coyly when your " + this.cockDescript(0) + " wriggles, wrapping around her arm.", false);
                    this.outputText("\n\n", false);
    ...
    private fuckEdrynTaur(): void {
        ...
        if (this.player.cor < 75) this.outputText("You recoil for a second, scolding yourself for the strange thoughts, but you don't slow down the fucking in the slightest.", false);
        else this.outputText("You smile, realizing you hadn't meant to think of her as a mare and you as her stud, but that's exactly what you are right now.  Your thrusts speed up as you renew your determination to fill this cunt full of fillies.", false);
    ...
    private fuckEdrynNonTaur(): void {
        ...
        if (this.player.tallness > 72) this.outputText(" and drop down to your knees to better level yourself with her", false);
        ...
        if (this.player.tallness < 60) this.outputText("You grab a stool so you'll be tall enough to fuck her properly and climb onto it.  ", false);
        ...
        if (this.player.cocks[x].cockType == CockTypesEnum.HORSE) this.outputText("feeling your ring of prepuce slipping between her lips.  ", false);
        ...
        if (this.player.hasKnot(x)) this.outputText("tries to rise, lifting up enough for you to get a few good breaths in, but your knot keeps her locked to you, and the two of you are tied together for another minute or two while it deflates, emptying the very last dregs of your spunk into her.  You pop free, ", false);
        ...
        if (this.player.statusAffectv1(StatusAffects.Edryn) <= 1) this.outputText("She sighs happily and looks down at you, \"<i>Ohhhh that was nice.  Most of my customers can't get me off like that.  I hope you're ok!  Don't worry about me getting pregnant either, I take some herbs to prevent that sort of thing until I'm ready for it.</i>\"\n\n", false);
        else if (this.player.statusAffectv1(StatusAffects.Edryn) <= 3) this.outputText("She sighs happily and looks down at you, \"<i>Ooh " + this.player.short + ", you're the best.  We should do this again soon.  Oh goddess, I can feel my juices starting just thinking about it.</i>\"\n\n", false);
        else if (this.player.statusAffectv1(StatusAffects.Edryn) == 4) this.outputText("She sighs happily and says, \"<i>Oh baby, you know what?  I'm never charging you again.  Just keep getting me off like that and we can do it anytime.</i>\"\n\n", false);
    ...
    public edrynBar(): boolean {
        if (this.flags[kFLAGS.EDRYN_NEVER_SEE_AGAIN] == 0 && this.model.time.hours >= 14 && this.model.time.hours <= 19 && (this.model.time.hours < 17 || this.flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] == 0))
            return true;
        return false;
    }
    ...
    */
    ...
    public edrynHeliaThreesomePossible(): boolean {
        ...
        if (this.player.gender == 0 || this.model.time.hours < 14 || this.model.time.hours >= 20 || Edryn.rand(2) == 0 || (this.flags[kFLAGS.HEL_FUCKBUDDY] == 0 && !kGAMECLASS.helFollower.followerHel())
    ...
    public approachHelAtZeBitch(): void {
        ...
        var edryn = undefined;
        if (this.edrynBar() && this.player.cockThatFits(300) >= 0 && this.player.statusAffectv1(StatusAffects.Edryn) >= 4)
    ...
    private threesomeEdrynAndHel(): void {
        ...
        if (this.player.balls > 0) this.outputText(", your sack slapping wetly against the bottom of her cunny", false);
        this.outputText(".\n\n", false);
    ...
    private pregdrynOffer(cs: boolean = true): void {
        ...
        if (this.pregnancy.incubation < 250) {
            ...
            if (this.player.cockTotal() > 1) this.outputText("All of your " + this.multiCockDescriptLight() + " fill in seconds, growing rock hard and actually aching with their need.  ", false);
            else if (this.player.cockTotal() == 1) this.outputText("Your " + this.cockDescript(0) + " fills in seconds, growing rock hard and actually aching with need.  ", false);
            this.outputText("You're totally dazed by the massive spike in arousal, and ", false);
            if (this.player.cor + this.player.lib < 100 || this.player.cockTotal() < 1) this.outputText("you struggle not to reach into your " + this.player.armorName + " to touch yourself.", false);
            else {
        ...
        if (this.player.cockArea(x) >= 24 && this.player.cockArea(x) < 300) {
            ...
            if (this.flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] == 0) this.outputText("Does my scent have an even stronger effect on you now", false);
    ...
    public fuckPregEdryn(): void {
        ...
        if (this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) {
            ...
            if (this.player.tallness < 60) this.outputText("bouncing against the top of your head", false);
        ...
        //TAURZILLA
            ...
            if (this.player.cumQ() < 300) { }
            else if (this.player.cumQ() < 1000) this.outputText("Your prodigious jism output soaks her sloppy tunnel and drips from the entrance with the centauress' girlcum.", false);
            else {
                ...
                if (this.player.cumQ() >= 4000) this.outputText("  By the time you calm down you've soaked all the pillows and spooge is puddling underneath.", false);
            }
            this.outputText("\n\n", false);
    ...
    private jizzFromEatingPregdrynOut(): void {
        ...
        if (this.player.cockArea(x) < 24) this.outputText("small dick begins to twitch and leak.", false);
        ...
        else if (this.player.cumQ() < 500) this.outputText("leaks pre-cum in a steady stream, quickly soaking a few pillows.", false);
        else this.outputText("pours out enough pre-cum to soak a pillow, and in no time your overenthusiastic dick is doing its damnedest to dribble over everything.", false);
        ...
        if (this.player.lowerBody == LOWER_BODY_TYPE_CENTAUR) {
            this.outputText("  Your hooves twitch weekly on the floor, forgetten about as you focus entirely on your hands, mouth, and pulsating " + Appearance.cockNoun(CockTypesEnum.HUMAN), false);
        ...
        if (this.player.hasVagina()) {
            ...
            if (this.player.vaginas[0].vaginalWetness == VAGINA_WETNESS_SLAVERING) this.outputText("squirting", false);
        ...
        if (this.player.cumQ() < 1000) {
            ...
            if (this.player.cumQ() < 3000) this.outputText("out the last of your liquid pleasure.", false);
            else this.outputText("ever greater amounts of liquid peasure.  Jism drips through the floorboards as you create a puddle a few inches deep.", false);
        }
        this.outputText("\n\n", false);
    ...
    private edrynFucktroduction(): void {
        ...
        if (!this.player.isTaur()) {
            ...
            if (this.player.cocks[0].cockType == CockTypesEnum.HORSE) this.outputText("making your " + this.cockDescript(x) + " twitch and ache.  It floods your body with animalistic desires, overpowering any sense of propriety you might have once had.", false);
            else this.outputText("overpowering you with strange lust.  As turned on as you are, it's difficult not to enjoy this.", false);
        }
        //TAUR
        else {
    ...
    public eatEdrynPussyLikeABawss(): void {
        ...
        if (this.player.tongueType > TONUGE_HUMAN) this.outputText("She's not even begun to taste the fruits of your talents, and you let your long, long tongue spool out inside her, pressing hard on her walls and tenderly flicking across each sensitive fold inside her.  ");
        ...
        if (y >= 0) this.outputText(" and your " + this.cockDescript(y) + " into her asshole");
        this.outputText(".  Her ");
