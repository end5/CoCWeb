...
export class Niamh extends TelAdreAbstractContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (this.flags[kFLAGS.NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER] > 1) this.flags[kFLAGS.NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER]--;
        if (this.flags[kFLAGS.NIAMH_MOVED_OUT_COUNTER] > 1) this.flags[kFLAGS.NIAMH_MOVED_OUT_COUNTER]--;
        if (this.player.findStatusAffect(StatusAffects.BimboChampagne) >= 0) {
            this.player.addStatusValue(StatusAffects.BimboChampagne, 1, -1);
            if (this.player.statusAffectv1(StatusAffects.BimboChampagne) <= 0) {
                this.removeBimboChampagne();
                needNext = true;
            }
        }
        if (this.player.statusAffectv1(StatusAffects.BlackCatBeer) > 0) {
            this.player.addStatusValue(StatusAffects.BlackCatBeer, 1, -1);
            if (this.player.statusAffectv1(StatusAffects.BlackCatBeer) <= 0) {
    ...
    public telAdreNiamh(): void {
        if (this.flags[kFLAGS.MET_NIAMH] == 0) {
            ...
            if (this.model.time.hours <= 8) this.outputText("massive N-cup breasts, spilling over the table");
            else if (this.model.time.hours <= 9) this.outputText("huge M-cup breasts, taking up much of the table");
            else if (this.model.time.hours <= 10) this.outputText("great L-cup breasts, advancing far onto the table");
            else if (this.model.time.hours <= 11) this.outputText("heavy K-cup breasts, lying on the table for support");
            else if (this.model.time.hours <= 12) this.outputText("hefty J-cup breasts, resting comfortably on the table");
            else if (this.model.time.hours <= 13) this.outputText("weighty I-cup breasts, dangling to lie on the table");
            else if (this.model.time.hours <= 14) this.outputText("squeezable H-cup breasts, settling burdensomely on her chest");
            else this.outputText("moderately manageable G-cups, trembling with every breath she takes");
        ...
        //This scene only plays if the PC has either Talked to Niamh or has gotten at least 1 mug of Black Cat Beer from her.
            ...
            if (this.model.time.hours <= 8) this.outputText("her titanic N-cup 'kegs' resting against the thankfully sturdy table in front of her.  Her teats are engorged with the beer she's cursed to carry, and the stuff dribbles out whenever she shifts.");
            else if (this.model.time.hours <= 14) {
                this.outputText("her ");
                if (this.model.time.hours <= 9) this.outputText("M-cup");
                else if (this.model.time.hours <= 10) this.outputText("L-cup");
                else if (this.model.time.hours <= 11) this.outputText("K-cup");
                else if (this.model.time.hours <= 12) this.outputText("J-cup");
                else if (this.model.time.hours <= 13) this.outputText("I-cup");
                else this.outputText("H-cup");
    ...
    public getANiamhBeer(): void {
        ...
        //Feeder
        if (this.player.findPerk(PerkLib.Feeder) >= 0) this.outputText("  You understand exactly what she's going through; the weight of a huge pair of breasts, the sensitivity as they engorge with fluids, the near-maddening sensation of the contents sloshing around underneath your straining skin...");
    ...
    private drinkNiamhsBeerInTelAdre(): void {
        this.clearOutput();
        if (this.player.lactationQ() >= 300 && this.player.biggestTitSize() >= 5 && Niamh.rand(2) == 0 && this.flags[kFLAGS.MET_NIAMH] > 0) {
            ...
            if (this.isHolidays() && this.date.date >= 31) {
                ...
                if (this.player.hasVagina()) this.outputText("pussy to wetten");
                if (this.player.gender == 3) this.outputText(" and ");
                if (this.player.hasCock()) this.outputText("[eachCock] to stiffen");
    ...
    private talkToNiamh(): void {
        ...
        if (this.flags[kFLAGS.TALKED_NIAMH] == 0) {
            ...
            if (this.model.time.hours < 15) this.outputText("who usually sits over yonder ");
    ...
    public blackCatBeerExpires(): void {
        this.dynStats("spe", 4.5, "int", 4.5, "lib", (-1 * this.player.statusAffectv2(StatusAffects.BlackCatBeer)));
        this.player.removeStatusAffect(StatusAffects.BlackCatBeer);
    ...
    public taintNiamh(bimbo: boolean = false): void {
        ...
        //[if bimbo liquor]
            ...
            this.flags[kFLAGS.NIAMH_STATUS] = 2;
        }
    ...
    public bimboChampagne(player: Player, clearScreen: boolean, intro: boolean): void {
        ...
        if (intro) {
            ...
            else if (player.findStatusAffect(StatusAffects.BimboChampagne) < 0) this.outputText("You uncork the bottle and breathe in the fizzy, spicy aroma of the sparkling liquor.  Breathing deeply, you open your mouth and begin pouring the ever-effervescent fluid inside.  It's sweet and slightly gooey, and the feel of it sliding down your throat is intensely... awesome?  Like, totally!");
            else this.outputText("You find yourself falling even further into the dense bimbo mindset.  You do feel, like, super-good and all, though!\n\nMoaning lewdly, you begin to sway your hips from side to side, putting on a show for anyone who might manage to see you.   You just feel so... sexy.  Too sexy to hide it.  Your body aches to show itself and feel the gaze of someone, anyone upon it.  Mmmm, it makes you so wet!  You sink your fingers into your sloppy cunt with a groan of satisfaction.  Somehow, you feel like you could fuck anyone right now!");
        }
        if (player.findStatusAffect(StatusAffects.BimboChampagne) >= 0) {
            ...
            if (!player.hasVagina()) {
                ...
                player.genderCheck();
                this.outputText("\n\nYou can feel ");
                if (player.hasCock()) this.outputText("the flesh under your cock[if (hasBalls = true)  and behind your [balls]]");
                else this.outputText("the blank expanse of flesh that is your crotch");
    ...
    public removeBimboChampagne(): void {
        ...
        if (this.player.statusAffectv2(StatusAffects.BimboChampagne) > 0) {
            this.player.breastRows[0].breastRating -= this.player.statusAffectv2(StatusAffects.BimboChampagne);
        ...
        if (this.player.statusAffectv3(StatusAffects.BimboChampagne) > 0) {
        ...
        if (this.player.statusAffectv4(StatusAffects.BimboChampagne) > 0) {
            this.player.buttRating -= this.player.statusAffectv4(StatusAffects.BimboChampagne);
            this.outputText("  Of course, the added junk in your trunk fades too, leaving you back to having a [butt].");
        }
        this.player.removeStatusAffect(StatusAffects.BimboChampagne);
    ...
    private bazaarSex(): void {
        ...
        else if (this.flags[kFLAGS.NIAMH_STATUS] == 1)	// Succubi Milk
        {
    ...
    private sellYourBooze(): void {
        ...
        if (this.temp > 40) this.temp = 40 + Niamh.rand(3);
        this.player.gems += this.temp;
        this.player.gems = Math.round(this.player.gems);
        this.statScreenRefresh();
        //If player has only two breasts.
        if (this.player.bRows() == 1) this.outputText("With a chipper giggle Niamh pulls you closer to the bar.  With both hands she helps you heft your tits up and on to the counter.");
        //If player has four or six breasts.
        else this.outputText("Niamh giggles as you try to move your breasts closer to the bar to be milked by the patrons, but it's obvious only the top pair of your tits can easily rest on the counter top.  \"<i>Lassie, it might be unconventional, but givin' the circumstance mayhaps it'd be best if ye laid down on the bar.</i>\"");
        ...
        if (this.player.hasVagina()) this.outputText("  You feel your excitement dripping down the inside of your thighs as your pussy wettens.");
        this.dynStats("lus", 10 + this.player.sens / 5, "resisted", false);
        //[If player has cocks]
        if (this.player.hasCock()) this.outputText("  You start to feel sympathetic sensations in [eachCock] as the hands and paws of the bar goers milk your nipples.  With their fingers sliding over your sensitive nubs you can easily imagine them sliding up and down your cock");
        ...
        //If lust is high
            ...
            if (this.player.hasVagina()) this.outputText("You can feel your labia dripping with need.  ");
            if (this.player.hasCock()) {
    ...
    private barBeerOrgyTits(): void {
        this.clearOutput();
        //If [player has pussy]
        if (this.player.hasVagina()) this.outputText("The drooling of your [vagina] gets worse as the constant \"tapping of your kegs\" drives your sense of self-restraint to its limits.  The hands of various drunks start moving more aggressively over your breasts, eventually winding their way down between your thighs.");
        ...
        if (this.player.buttRating >= 20) this.outputText("  She slaps your ass to emphasize her meaning, and the action sends jiggling waves through each immense cheek.");
        ...
        if (this.player.biggestTitSize() >= 20) this.outputText("Your giant boobs are large enough to temporarily hang off the sides of your body as you're carried.");
        ...
        if ((this.player.pregnancyIncubation == 0 || this.player.pregnancyIncubation > 100) && this.player.bRows() > 1) this.outputText("  You are essentially a pile of " + this.boozeBoobsType() + " filled tits.");
        // [IF player is hugely pregnant with four or more tits]
        else if (this.player.bRows() > 1) this.outputText("  You are essentially a giant pregnant belly surrounded by your [chest] on the table, helpless to the sexual advances of the bar goers and you can't get enough of it.");

        //===============
        //Third Paragraph
        //===============
        //IF player has pussy and Urta is in the bar and sex with her is unlocked.
        if (this.model.time.hours < 15 && this.flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 0 && this.player.hasVagina()) {
            ...
            if (this.player.pregnancyIncubation == 0) this.outputText("  Some folks give yelps and cheers when they see the bulge that repeatedly forms in your abdomen each time Urta thrusts herself inside you.");
        ...
        //ELSE IF player has pussy and Urta is not in the bar.
            ...
            if (this.player.vaginalCapacity() < 60) this.outputText("  One is sporting a cock that is obviously too massive for your pussy, and eventually backs down so that the other one, despite still being on the large size, can take the honor of fucking your cunt silly.");
            //ELSE IF [player has a gaping pussy or bigger]
            else this.outputText("  They're both sporting large canine cocks, but one is particularly massive while the other would likely be a loose fit for a pussy as naturally stretched out as yours.  The largest one wins out and positions himself to give your overly wide fuck hole a much needed stretching.");
        ...
        if (this.player.pregnancyIncubation > 0 && this.player.pregnancyIncubation < 100) this.outputText("  \"<i>Oi!  Careful folks, this babe could blow at any time!</i>\"  Niamh pats your belly with chiding affection.");
        ...
        if (this.player.hasFuckableNipples()) this.outputText("  Eventually some of the randier drunks start whipping out their cocks, and begin pressing the heads against the openings of your nipples.  Within moments you feel your booze bloated breasts being fucked silly by " + Niamh.num2Text(this.player.totalNipples()) + " hard cocks.  Each thrust causes " + this.boozeBoobsType() + " to spill out around their shafts.");
        // ELSE[Character doesn't have fuckable nipples]
        else this.outputText("  Some of the hornier customers start shoving cocks of all shapes and sizes into the moist wet cleavages formed by yours and Niamh's breasts.");
        ...
        if (this.player.cockTotal() > 0) {
            ...
            if (this.player.cockTotal() > 1) this.outputText("s have");
            ...
            if (this.player.pregnancyIncubation > 0 && this.player.pregnancyIncubation < 200) this.outputText("  Globs of cum splatter across your gravid belly.");
        }
        ...
        if (this.player.statusAffectv1(StatusAffects.Edryn) >= 5 && this.player.gender == 3 && this.model.time.hours == 14 && this.flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 0) {
            ...
            // IF [player has a single cock]
            if (this.player.cockTotal() == 1) this.outputText("  Edryn moans out loud in ecstasy as your [cock] fills her up.");
            // If [player has multiple cocks]
            else if (this.player.cockTotal() > 1) this.outputText("  The heads of your cocks press in between Edryn's labia causing her to cry out, \"<i>Oh gawd!  How many dicks are y-you f-f-fucking meeee with???</i>\"  Edryn's orgasm cuts her voice off and every muscle in her cunt contracts around your " + this.multiCockDescriptLight() + ".");
            ...
            if (this.player.pregnancyIncubation > 0 && this.player.pregnancyIncubation < 200) this.outputText("  Your pregnant belly presses up against Edryn's ass as both Urta's cock and Edryn's pussy keeps you pinned in place between them.");
            // IF[player is not pregnant]
            else this.outputText("  Urta's cock and Edryn's pussy pin you in place between them, and as Urta fucks you deeper her cock head forces your belly to bulge outwards until it presses against Edryn's ass.");
            ...
            if (this.player.bRows() == 2) this.outputText("  Your second row of tits get thrust out to either side every time Urta's torso presses your own against Edryn.");
            //IF [player has six giant breasts]
            else if (this.player.bRows() == 3) this.outputText("  Your lowest row of tits slap against all three of your thighs as the love making becomes more brutal.");
            //IF [player has large balls]
            if (this.player.balls > 0 && this.player.ballSize > 14) this.outputText("  Your hanging testicles bounce around between everyone's legs.");
            //IF [player has giant ass]
            if (this.player.buttRating >= 22) this.outputText("  The fat of your ass cheeks jiggles and quakes even as Urta practically stuffs her hips into your crack.  \"<i>Damn, you've got a bigger ass than Edryn,</i>\" she chides as she spanks you.  \"<i>Hey!</i>\"  Edryn calls out with a false tone of hurt pride.");
        ...
        // IF [Urta but not Edryn is present in the bar and sex with her is unlocked and character has pussy]
        else if (this.player.hasVagina() && this.model.time.hours < 15 && this.flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 0) {
            ...
            if (this.player.bRows() > 1) this.outputText("  Your rows of breasts shift around as your posture changes, making squelching sounds as they slide around.");
            //If [player not pregnant]
            if (this.player.pregnancyIncubation == 0) this.outputText("  Urta's small ocean of sperm streams out from your pussy like a river down your legs as you try to stand.");
        ...
        //Generic ending if the first two don't trigger
            ...
            // If [player has pussy]
            if (this.player.hasVagina()) this.outputText("  You gag on it at the same time a new cock fills your cunt, ready to renew the thrusting that's been causing your tits to bounce all over the place.");

            //IF [player has single cock small-med cock]
            if (this.player.cockTotal() == 1 && this.player.cockArea(0) < 75) this.outputText("\n\nA cat girl with six C-cup breasts jumps up onto the table and mounts you.  She grabs your cock and proceeds to shove it up between the folds of her tight pussy.");
            ...
            if (this.player.cockTotal() == 1) this.outputText("  The six titted girl smiles at you, patting her belly while your cum runs down the inside of her thighs.  You only now wonder about how fertile she might be.");
            // IF [player has multiple cocks]
            else if (this.player.cockTotal() > 1) this.outputText("  Various girls around the room are patting their bellies and giving you sultry looks while your cum continues to drip from their pussies.  You passively wonder if any of them might indeed be hauling around some larger pregnant bellies in a few days, but you try to push those thoughts from your head.  At the moment they seem happy.");
            // IF [player has pussy]
            if (this.player.hasVagina()) {
                ...
                //IF [player is not pregnant]
                if (this.player.pregnancyType == 0) this.outputText("  You can't help but wonder how virile those dog morphs might have been as their cum and the cum of other customers sloshes around inside your uterus.");
                ...
                switch (this.temp) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        this.player.knockUp(PregnancyStore.PREGNANCY_MOUSE, PregnancyStore.INCUBATION_MOUSE); //I'm betting this was meant to be dog morph chance
                        break;
                    default:
                        this.player.knockUp(PregnancyStore.PREGNANCY_CENTAUR, PregnancyStore.INCUBATION_CENTAUR);
                        break;
                }
            }
            this.player.growTits(2, this.player.bRows(), false, 2);
    ...
    private boozeBoobsType(): string {
        // New Years
        if (this.isHolidays() && this.date.date >= 31) return "champagne";
        // Saint Patrick's
        else if (this.date.date == 17 && this.date.month == 2) return "Saint Patrick's Day lagar";
        // Christmas
        else if (this.isHolidays()) return "egg nog";
        // Valentine's
        else if (this.date.date == 14 && this.date.month == 1) return "wine";
        // Non-Holiday, Generic
        else return "beer";
    }
}
