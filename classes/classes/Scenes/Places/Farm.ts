...
export class Farm extends BaseContent {
    ...
    public farmExploreEncounter(): void {
        ...
        if (this.player.statusAffectv1(StatusAffects.MetWhitney) < 2) {
            ...
            if (this.player.findStatusAffect(StatusAffects.MetWhitney) < 0) {
                ...
                if (this.player.statusAffectv1(StatusAffects.MetWhitney) == 2) this.outputText("<b>You've been to the farm enough to easily find it.  You can return by selecting it from the places menu (and will no longer encounter it during random lake exploration)</b>.\n\n", false);
            }
    ...
    private talkWhitney(): void {
        ...
        if (this.flags[kFLAGS.MURBLE_FARM_TALK_LEVELS] > 0 && this.player.findStatusAffect(StatusAffects.Kelt) >= 0 && this.flags[kFLAGS.WHITNEY_TALK_MURBLE_AND_KELT] == 0) {
        ...
        if (this.player.findStatusAffect(StatusAffects.TelAdre) >= 0 && this.player.statusAffectv1(StatusAffects.TelAdre) >= 1 && this.flags[kFLAGS.WHITNEY_TALK_TELADRE] == 0 && this.flags[kFLAGS.WHITNEY_TALK_MURBLE_AND_KELT] == 1) {
        ...
        if (this.flags[kFLAGS.DISCOVERED_HIGH_MOUNTAIN] > 0 && this.flags[kFLAGS.WHITNEY_TALK_HIGH_MOUNTAIN] == 0 && this.flags[kFLAGS.WHITNEY_TALK_TELADRE] == 1) {
            ...
            if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00331] > 0 && this.flags[kFLAGS.ISABELLA_MILKED_YET] != -1) {
                // Jojo only
                this.outputText(" At the far end there is a small alabaster cluster labelled ‘Jojo’.");
            }
            else if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00331] == 0 && this.flags[kFLAGS.ISABELLA_MILKED_YET] == -1) {
                // Isabella Only
        ...
        if (this.player.findStatusAffect(StatusAffects.DungeonShutDown) >= 0 && this.flags[kFLAGS.WHITNEY_TALK_DUNGEON] == 0 && this.flags[kFLAGS.WHITNEY_TALK_HIGH_MOUNTAIN] == 1) {
        ...
        if (Farm.rand(4) == 0 && this.player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") < 0) {
            if (this.player.hasKeyItem("Breast Milker") >= 0) {
                ...
                else if (this.player.cor <= 66) this.outputText("You flush with a touch of exhibitionism", false);
                else this.outputText("You flush hotly and arch your back, openly displaying your milk to the dog-girl", false);
        ...
        if (this.player.cor > 50) this.outputText("or at least the parts you think she would want to hear ", false);
        ...
        if (this.player.findStatusAffect(StatusAffects.FactoryOverload) < 0) this.outputText("how well the farm has been going", false);
        else this.outputText("how poorly the farm has been going since the lake became tainted.  She has to work three times as hard to keep her livestock and crops from succumbing to the taint, and the demons and monsters of the forest are many times more bold", false);
    ...
    public workFarm(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.NoMoreMarble) < 0 && this.player.findStatusAffect(StatusAffects.MarbleWithdrawl) >= 0) {
            if (this.player.statusAffectv3(StatusAffects.Marble) == 1) this.marbleScene.addictedEncounterHappy();
            else this.marbleScene.encounterMarbleAshamedAddiction();
            return;
        }
        //1/3 chance of marblez
        if (Farm.rand(3) == 0 && this.player.findStatusAffect(StatusAffects.NoMoreMarble) < 0 && this.player.findStatusAffect(StatusAffects.Marble) > 0) {
            //Rapez Override normal
            if (this.player.findStatusAffect(StatusAffects.MarbleRapeAttempted) >= 0 || this.flags[kFLAGS.MARBLE_WARNING] == 3) {
    ...
    public exploreFarm(): void {
        ...
        //FIND CARROT!
        if (kGAMECLASS.nieveHoliday() && this.flags[kFLAGS.NIEVE_STAGE] == 3 && this.player.hasKeyItem("Carrot") < 0) {
            kGAMECLASS.findACarrot();
            return;
        }
        //Free Isabella Milkings!
        if (this.player.hasCock() && this.flags[kFLAGS.FOUND_ISABELLA_AT_FARM_TODAY] == 0 && this.flags[kFLAGS.ISABELLA_MILKED_YET] < 0 && kGAMECLASS.isabellaFollowerScene.isabellaFollower() && this.flags[kFLAGS.ISABELLA_MILK_COOLDOWN] == 0 && Farm.rand(2) == 0) {
    ...
    public getMilked(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.BreastsMilked) < 0) {
            if (this.player.cor < 50) this.outputText("You shiver nervously when you step inside.", false);
        ...
        //Repeat
            ...
            if (this.player.findStatusAffect(StatusAffects.LactationReduction) >= 0 && this.player.totalCocks() == 0) this.outputText("  Your " + this.nippleDescript(0) + "s are engorged and ready to be taken care of.", false);
            //If cocks
            ...
            //If both
            if (this.player.findStatusAffect(StatusAffects.LactationReduction) >= 0 && this.player.cockTotal() > 0) {
        ...
        if (this.player.findStatusAffect(StatusAffects.BreastsMilked) < 0 || application == 0) {
            if (this.player.findStatusAffect(StatusAffects.BreastsMilked) < 0) this.player.createStatusAffect(StatusAffects.BreastsMilked, 0, 0, 0, 0);
            ...
            if (this.player.nippleLength <= 1.5) this.outputText("pulling each of your nipples entirely into the suction-tubes.  ", false);
            else this.outputText("struggling to fit around each of your nipples as they slide into the suction-tubes.  ", false);
        ...
        //Apply repeat alternate
            ...
            if (this.player.nippleLength >= 3) this.outputText("They nearly burst the tubes designed to milk them by virtue of their sheer size.  ", false);
        ...
        if (payout > 0) {
            ...
            if (this.flags[kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK] >= cap) this.outputText("  <b>The machinery warns you that Whitney can't afford any more this week!</b>", false);
        ...
        if (liters > 2) {
            ...
            else if (this.player.inte < 25) this.outputText("  You quietly moo, then giggle to yourself at how strange you're acting.", false);
        }
        ...
        //Horny
            ...
            if (this.farmCorruption.whitneyCorruption() < 90) this.outputText(" It isn't until you get back that you remember the disapproving look Whitney gave you, but if anything, it only makes you hornier.", false);
    ...
    public cockPumping(): void {
        ...
        //REPEAT
            ...
            if (this.player.cockTotal() == 1) this.outputText("hose wriggles out from the wall, curving up towards your crotch. ", false);
            else this.outputText("hoses wriggle out from the wall, curving up towards your crotch. ", false);
        ...
        //BAD END!?
        if (this.player.cumQ() >= 50 && this.player.fatigue >= 100 && this.flags[kFLAGS.USED_MILKER_TODAY] > 0) {
            ...
            if (cumQ < 1000) {
                ...
                if (cumQ > 100) this.outputText("and a clatter of gems drop into the plate ", false);
        ...
        //Medium Scene – output less than 250 mLs
            ...
            if (this.player.cockTotal() == 1) {
                ...
                if (this.player.balls > 0) this.outputText("bloated " + this.ballsDescriptLight() + " are", false);
                ...
                this.outputText("An orgasm hits you like a an ocean wave, shutting down any remaining conscious thoughts and replacing them with one overriding emotion: relief.   You spurt helplessly into the tube, wracked with pleasure as your ", false);
                if (this.player.balls > 0) this.outputText("bloated " + this.ballsDescriptLight() + " are", false);
                else this.outputText("sensitive " + this.multiCockDescriptLight() + " are", false);
        ...
        if (payout > 0) {
            ...
            if (payout == 1) this.outputText(Farm.Num2Text(payout) + " gem rolls ", false);
            else this.outputText(Farm.Num2Text(payout) + " gems roll", false);
    ...
    private milkerBadEnd1(): void {
        ...
        if (this.player.hasVagina()) {
            ...
            //[(bigclit)
            if (this.player.clitLength > 3) this.outputText("A smaller suction tube fastens over your " + this.clitDescript() + " as it rouses from the vaginal stimulation.", false);
        }
        ...
        //[(balls)
        if (this.player.balls > 0) this.outputText(" and your " + this.sackDescript() + " stretches as your " + this.ballsDescriptLight() + " nearly double in size", false);
        ...
        //[(balls)
        if (this.player.balls > 0) this.outputText(" and the new volume of your monstrous testicles", false);
        //[(big skeet)
        if (cumQ > 1000) this.outputText(", spraying out of the overflow valve and coating your entire lower body", false);
        ...
        if (this.player.biggestTitSize() > 1) {
            ...
            //[(nipplecunts)
            if (this.player.hasFuckableNipples()) this.outputText(" and rings raise from the bases of the shafts inside your nipples, forming a seal,", false);
        ...
        //[(lactation)
        if (this.player.biggestLactation() > 1) this.outputText(", the drug now tasting of the milk mixed in", false);
        ...
        //[(corruption > 90, high demonscore)
            ...
            //[(onecox)
            if (this.player.cockTotal() == 1) this.outputText("a gigantic, erect penis bobbing in front of you and throwing off your balance", false);
            //(multi)
            else this.outputText("multiple gigantic, erect penises bobbing in front of you and dragging between your legs, head pinned against the floor", false);
    ...
    private centaurToysHoooooo(): void {
        ...
        if (this.player.cor < 50) this.outputText("You shift awkwardly and ask her what's wrong.", false);
        else this.outputText("You strut a bit, showing yourself off in a subtly lewd manner.  When you're finished, you ask the dog-girl if she likes what she saw.", false);
        this.outputText("\n\n", false);
