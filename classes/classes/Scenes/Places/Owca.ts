...
export class Owca extends BaseContent {
    ...
    private gangbangVillageFirstGoRound(): void {
        ...
        if (this.flags[kFLAGS.DECLINED_TO_VISIT_REBECCS_VILLAGE] == 0 && this.flags[kFLAGS.TIMES_REFUSED_REBECCS_OFFER] == 0 && this.flags[kFLAGS.TIMES_IN_DEMON_PIT] == 0) {
        ...
        //Second (and subsequent) encounter; only happens once PC accepted being sacrificed once (Z)
            ...
            //[if corr >= 50]
            if (this.player.cor > 50) this.outputText("  You can't help but smile internally.  Let her think that, if she wants.");
    ...
    private agreeToFollowRebecFirstTime(): void {
        ...
        if (this.flags[kFLAGS.TIMES_REFUSED_REBECCS_OFFER] == 0) {
            ...
            if (this.player.gender == 0) this.outputText("regretfully bare ");
            ...
            //[if silly mode on]
            if (this.silly()) this.outputText("I-I don't think there's a single grown man or woman in our village that still has their anal virginity.");
            //[if silly mode off]
            else this.outputText("Th-they abused us.  They mistreated us.  They would gather in circles and... use us in every possible way.");
            this.outputText("</i>\"");
            //[if Corr < 20]
            if (this.player.cor < 20) {
                ...
                //[[if silly mode on]
                if (this.silly()) this.outputText("she'll never have to worry about her asshole again.");
                //[if silly mode off]
            ...
            if (this.player.cor < 50 && this.player.inte >= 50) {
                ...
                if (this.player.lib > 50) this.outputText("... and considering what Rebecc meant by 'gratitude'");
    ...
    private intoTheDemonPit(sacrifice: boolean = true): void {
        ...
        //[if first time]
        if (this.flags[kFLAGS.TIMES_IN_DEMON_PIT] == 0) this.outputText("\"<i>Awww, look at the poor little thing!  So this is what they gave us this time... Look at [him], everyone!  Do you know you'd be a cute pet?  People here call me Vapula, and I don't think you'll forget that name any time soon.</i>\"");
        //[else if Vapula Submissiveness >50]
        else if (this.flags[kFLAGS.VAPULA_SUBMISSIVENESS] > 50) this.outputText("\"<i>You again?  By Lethice, you must really enjoy it!  You love being a bitch for Vapula, don't you?  Yes you do!</i>\"");
        //[else]
        else this.outputText("\"<i>Fuck, you again?  Tell me it's so!  I've been looking forward to putting you in your place as my newest slut!</i>\"");
        //[(if sacrificed)
        if (sacrifice) {
            //[if str >= 80]
    ...
    private fightZeDemons(sacrifice: boolean = true): void {
        ...
        if (sacrifice) {
            ...
            this.monster.createStatusAffect(StatusAffects.BowDisabled, 0, 0, 0, 0);
            if (this.player.str < 80 && this.player.spe < 80) {
                this.monster.createStatusAffect(StatusAffects.AttackDisabled, 0, 0, 0, 0);
                this.monster.createStatusAffect(StatusAffects.RunDisabled, 0, 0, 0, 0);
                this.monster.createStatusAffect(StatusAffects.PhysicalDisabled, 0, 0, 0, 0);
            }
            else {
                if (!this.player.canFly()) this.monster.createStatusAffect(StatusAffects.RunDisabled, 0, 0, 0, 0);
            }
        }
    ...
    public loseOrSubmitToVapula(): void {
        ...
        if (this.player.hasVagina()) this.outputText(", followed by your " + this.vaginaDescript(0));
        if (this.player.hasCock()) this.outputText(", before finally wrapping around your " + this.cockDescript(0) + " and stroking it; the friction uncontrollably arouses you, and you find yourself reaching full erectness");
        ...
        //[(no Buttslut)
        if (this.player.findPerk(PerkLib.Buttslut) < 0) this.outputText("No!  You aren't supposed to enjoy it...  ");
        ...
        if (this.player.hasCock()) this.outputText("  It wraps around [eachCock] for a while, jerking it for a bit and rubbing its tip against yours.");
        //[if antennae]
        if (this.player.antennae > ANTENNAE_NONE) this.outputText("  Your antennae are being harshly pulled and twisted; some imps, in a crazy show of libertinism, start inserting your sensitive peduncles down their bloated urethrae. The intimate friction and the sudden jolts when the internal walls slather your appendages in slick, hot pre-cum are driving you mad with irregular shots of unbearable pleasure.");
        //[if horns]
        if (this.player.horns > 0) this.outputText("  You feel your horns being used as leverage to slap your head with even more rock-hard dick.  The impacts stun you until your whole forehead feels numb and coated with a mixture of ballsweat and pre-cum.");
        //[if vagina
        if (this.player.hasVagina()) {
            ...
            if (this.player.findStatusAffect(StatusAffects.BonusVCapacity) < 0) this.player.createStatusAffect(StatusAffects.BonusVCapacity, 0, 0, 0, 0);
            if (this.player.statusAffectv1(StatusAffects.BonusVCapacity) < 200) this.player.addStatusValue(StatusAffects.BonusVCapacity, 1, 15);
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.cockTotal() > 1) this.outputText("  She is soon joined by other horny girls, too eager to wait for a piece of your crotch to play with.  They all comfortably position themselves, positioning their pussy or anus atop each of your cocks before riding you like there's no tomorrow.");
            //[if 4 to 7 cocks]
            if (this.player.cockTotal() >= 4) this.outputText("  Nevertheless, they aren't satisfied with a single insertion and most grab hold of one of your extra cocks before forcefully stuffing it in their other hole, howling and thrashing all the more as they are penetrated a second time.");
            //[if 7 cocks or more]
            if (this.player.cockTotal() >= 7) {
                ...
                if (this.player.cockTotal() > 8) this.outputText(" as well as your remaining pricks");
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.cockTotal() > 7) this.outputText(" and mouths");
            ...
            if (this.player.cumQ() > 1500) this.outputText("  The effort appears to be vain, as the absurdly high volume you've ejected is enough to completely pack every hole; the remaining jism spills on the tainted ground, soaking it further.");
        }
        ...
        if (this.player.hasVagina()) this.outputText(" and filling your womb to its maximum capacity");
    ...
    public defeetVapulasHorde(): void {
        this.clearOutput();
        this.flags[kFLAGS.REBECCS_LAST_PLEA] = 0;
        if (this.flags[kFLAGS.VAPULA_SUBMISSIVENESS] <= 0) {
            this.subdueVapula();
            return;
        }
        //Sacrificed and bound
        if (this.monster.findStatusAffect(StatusAffects.AttackDisabled) >= 0) {
            if (this.monster.HP < 1) this.outputText("You grin wickedly as the last demons fall, defeated.  Some of the errant blows have broken and mangled the links of your chains, and you find you can get free with a bit of additional effort.");
            //[if won by Lust]
            else this.outputText("You grin wickedly as the demons give up the fight, too turned on to care about you.  One even begins unfastening your bindings, hopeful desperation glinting in her eyes as she attempts to entice you with her long, thick nipples and enormous, dripping gash.");
        }
        //Not
        else {
            if (this.monster.HP < 1) this.outputText("You grin wickedly as the last demons fall, defeated.");
            //[if won by Lust]
            else this.outputText("You grin wickedly as the demons give up the fight, too turned on to care about you.  One even has hopeful desperation glinting in her eyes as she attempts to entice you with her long, thick nipples and enormous, dripping gash.");
        }
    ...
    private rapeZeVapula(): void {
        ...
        if (this.player.hasCock()) {
            ...
            //[if vagina]
            if (this.player.hasVagina()) this.outputText("You stick a finger in your own " + this.vaginaDescript() + " in order to lubricate it.  ");
            ...
            if (this.monster.HP < 1) this.outputText("the many bruises she's received");
            else this.outputText("the constant flushing due to intense arousal");
            ...
            if (this.player.hasVagina()) this.outputText("bitch in heat");
            else this.outputText("centaur in rut, abusing and filling her teasing cunt and ass with every protuberance you have");
            ...
            if (this.player.cockTotal() > 1) this.outputText("  Seeing that she has extra room left, you grab another of your dongs and put it at the entrance of her dripping cunt before forcing it in.");
            ...
            if (this.player.cumQ() > 1000) this.outputText("  Some of it spurts by little jets outside of her, even as she instinctively contracts her muscles to keep as much spooge as possible and her belly accomodates by bloating out absurdly.");
            this.outputText("  Needless to say, watching this rough ");
            if (this.player.cockTotal() == 1) this.outputText("anal");
            else this.outputText("double-penetration");
            ...
            if (this.player.cumQ() > 1000) this.outputText(", expandable");
            ...
            else this.outputText(this.player.skin());
            ...
            if (this.player.balls > 0) this.outputText("fondling your " + this.ballsDescriptLight() + ", ");
            //[if multicocks]
            if (this.player.cockTotal() > 1) this.outputText("alternatively deepthroating or jerking every cock of yours, ");
            ...
            if (this.player.cockTotal() > 1) this.outputText("s harden and thicken ");
            else this.outputText(" hardens and thickens ");
            ...
            if (this.player.cockTotal() > 1) this.outputText("In an unholy sex-frenzy, you want more; you start grabbing creatures by groups and alternatively pump in and out of their asses with your " + this.cockDescript(0) + ", always hungry for more anal.");
            ...
            if (this.silly()) this.outputText("  As you look back, you snicker, \"<i>Ass for ass, assholes.</i>\"");
        ...
        //Victory rape using vagina/anus [Take Dicks in All Holes](Z)
            ...
            if (this.player.vaginalCapacity() > 80) this.outputText("His cock is lost in your vast cunt and you work him as hard as you can to feel him against your walls, his body convulsing and dazed eyes rolling against the wet, savage rape.  ");
            ...
            //[if squirter]
            if (this.player.wetness() >= 5) this.outputText("The sound of spattering fluid fills the air as your cunt spurts its juices around the incubus's cock, coating him in your fragrant slime.  ");
            ...
            if (this.player.biggestTitSize() >= 2) this.outputText(" and your " + this.breastDescript(0) + " bounce gently");
            ...
            //[cunt nipples]
            if (this.player.hasFuckableNipples()) this.outputText("  Your freakish nipples moisten with excitement to the smell of man juice, and you feel your libido ratcheting up even higher.  Bending forward, you grab two other imps with smaller dicks, quickly guiding their lengths into your nipplecunts; they are small enough that you can mash their bodies against your soft flesh, doing all the work for them as your sensitive nubs are penetrated.");
            ...
            if (this.player.hasFuckableNipples()) this.outputText("s");
            ...
            if (this.player.hasFuckableNipples()) this.outputText(", teats, ");
            else this.outputText(" ");
    ...
    private owcaMainScreenOn(): void {
        ...
        if (this.flags[kFLAGS.REBECCS_LAST_PLEA] == 1 && !kGAMECLASS.vapula.vapulaSlave()) {
            this.rebeccsLastPlea();
            return;
        }
        if (this.flags[kFLAGS.OWCAS_ATTITUDE] <= 5 && this.flags[kFLAGS.OWCA_ANGER_DISABLED] == 0 && this.flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0) {
            this.desperateVillages();
            return;
        }
        //Desperate plea
        if (Owca.rand(10) <= 2 && this.flags[kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS] >= 7 && this.flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0) {
        ...
        //[if attitude > 80]
        if (this.flags[kFLAGS.OWCAS_ATTITUDE] > 80) this.outputText("Villagers greet you as you arrive, praising your courage and your heroism.  People shake your hands or ask you to kiss them, as if they hoped your nobility were somehow contagious.  Once they're done complimenting and flattering you in every way possible, they leave you to your business.");
        //[if 60<=attitude<80]
        else if (this.flags[kFLAGS.OWCAS_ATTITUDE] > 60) this.outputText("Villagers greet you with a friendly smile as you arrive.  Some approach you and thank you for what you've done, before leaving awkwardly.  They're obviously not used to strangers.");
        //[if 40<=attitude<60]
        else if (this.flags[kFLAGS.OWCAS_ATTITUDE] > 40) this.outputText("Villagers seem to accept your presence without comment now; though none go out of their way to greet you, neither do they avoid doing so when you pass by.");
        //[if 20<attitude <= 40]
        else if (this.flags[kFLAGS.OWCAS_ATTITUDE] > 20) this.outputText("Villagers keep looking at you uncomfortably; no one seems to be willing to talk to you apart from Rebecc, your friend.");
        //[if attitude <= 20]
        else this.outputText("Villagers here appear to be openly hostile.  You keep hearing muffled whispers as you pass by, and people look at you rancorously.  They don't bother you, though.");
        ...
        if (this.flags[kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS] == 7 && this.flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0) this.outputText("  A week");
        else if (this.flags[kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS] > 7 && this.flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0) this.outputText("  More than a week");
        if (this.flags[kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS] >= 7 && this.flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0) this.outputText(" has passed since the last offering to the demons; guarding the pit would certainly help improve your relations with the little town's denizens.");
        //Option:
        var pit = undefined;
        var herd = undefined;
    ...
    private owcaBuySetup(item: ItemType): void {
        if (item == this.consumables.SHEEPMK) this.buyOwcaShit(item, (180 - this.flags[kFLAGS.OWCAS_ATTITUDE]));
        else if (item == this.consumables.GOB_ALE) this.buyOwcaShit(item, (60 - Math.round(this.flags[kFLAGS.OWCAS_ATTITUDE] / 2)));
        else if (item == this.consumables.BROBREW) this.buyOwcaShit(item, 2000);
    ...
    private rebeccBathScene(): void {
        ...
        if (this.player.gender == 0) this.outputText(".");
            ...
            //[if cock]
            if (this.player.hasCock()) this.outputText("; your " + this.cockDescript(0) + " grows to full erect size");
            if (this.player.hasVagina() && this.player.hasCock()) this.outputText(" and ");
            else if (this.player.hasVagina()) this.outputText("; ");
            if (this.player.hasVagina()) this.outputText("your " + this.vaginaDescript() + " leaks a little trickle of girl-juice");
    ...
    private rapeRebecc(outside: boolean = false): void {
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.cocks[0].cockLength > 12) this.outputText("  You keep burying more of your " + this.cockDescript(0) + " deep inside her rectum until you reach her limit and can't fit more cock in.");
            else this.outputText("  Eventually the entirety of your prick is lodged inside her lush asshole.");
            ...
            //[if balls]
            if (this.player.balls > 0) this.outputText(", and your " + this.ballsDescriptLight() + " almost feel sore from colliding with her dribbling cunt");
            ...
            if (outside) this.outputText("Some of the villagers are apathetically watching you, too stunned and bruised to intervene.  ");
            ...
            if (outside) this.outputText(" to these miserable people, and even moreso");
        ...
        //Rebecc Female Rape Scene (Z)
            ...
            if (this.player.tone > 75) this.outputText("  The sheep girl's plush softness against your own toughened, toned physique manages to both arouse and anger you.  \"<i>This is what you were bred to be, isn't it bitch?</i>\" you hiss, your hands roaming over her inviting curves.  \"<i>Your fat ass hasn't had to work a day in its life.  The town whore, the cock-tease they wheel out when your people need a dupe, that's what you are.  How does it feel to be in the hands of a real woman?</i>\"");
            else this.outputText("  Gods, this girl has a fine ass; you give it a few slaps here and there as it jiggles in the most enticing way.");
    ...
    public loseToOwca(): void {
        ...
        if (this.player.weaponName != "fists") this.outputText("your " + this.player.weaponName + " is taken away and ");
    ...
    private forgiveOwca(): void {
        ...
        if (this.player.cor >= 50) this.outputText(", lying through your teeth,");
    ...
    private subdueVapula(): void {
        ...
        if (this.monster.HP < 1) this.outputText("overwhelmed by your might");
        else this.outputText("rendered insensate by his own lust");
    ...
    private enslaveVapulaAsACuntWielder(): void {
        ...
        this.flags[kFLAGS.VAPULA_FOLLOWER] = .5;
