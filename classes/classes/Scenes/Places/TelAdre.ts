...
export class TelAdre extends BaseContent {
    ...
    private telAdreCrystal(): void {
        if (this.player.findStatusAffect(StatusAffects.TelAdre) < 0) this.player.createStatusAffect(StatusAffects.TelAdre, 0, 0, 0, 0);
        ...
        else if (this.player.cor >= 25 || this.player.findStatusAffect(StatusAffects.FactoryOverload) >= 0) {
    ...
    public telAdreMenu(): void {
        ...
        if (!kGAMECLASS.urtaQuest.urtaBusy() && this.flags[kFLAGS.PC_SEEN_URTA_BADASS_FIGHT] == 0 && TelAdre.rand(15) == 0 && this.model.time.hours > 15) {
            this.urtaIsABadass();
            return;
        }
        if (!kGAMECLASS.urtaQuest.urtaBusy() && kGAMECLASS.urta.pregnancy.event > 5 && TelAdre.rand(30) == 0) {
        ...
        switch (this.flags[kFLAGS.KATHERINE_UNLOCKED]) {
            ...
            case 0: //Still potentially recruitable
                if (this.flags[kFLAGS.KATHERINE_RANDOM_RECRUITMENT_DISABLED] == 0 && this.player.gems > 34 && TelAdre.rand(25) == 0) {
        ...
        if (this.flags[kFLAGS.ARIAN_PARK] == 0 && this.player.level >= 4 && TelAdre.rand(10) == 0 && this.flags[kFLAGS.NOT_HELPED_ARIAN_TODAY] == 0) {
        ...
        if (kGAMECLASS.urta.pregnancy.event > 2 && TelAdre.rand(4) == 0 && this.flags[kFLAGS.URTA_PREGNANT_DELIVERY_SCENE] == 0 && this.player.hasKeyItem("Spare Key to Urta's House") >= 0) {
    ...
    private normalPierceAssemble(): void {
        ...
        switch (this.piercingMat) {
            ...
            case 9:
                shortP += "lethite ";
                if (this.player.findPerk(PerkLib.PiercedLethite) < 0) this.player.createPerk(PerkLib.PiercedLethite, 0, 0, 0, 0);
                longP += "Lethite ";
                break;
            case 10:
                shortP += "fertite ";
                if (this.player.findPerk(PerkLib.PiercedFertite) < 0) this.player.createPerk(PerkLib.PiercedFertite, 5, 0, 0, 0);
                else this.player.addPerkValue(PerkLib.PiercedFertite, 1, 5);
                longP += "Fertite ";
                break;
            case 11:
                shortP += "furrite ";
                if (this.player.findPerk(PerkLib.PiercedFurrite) < 0) this.player.createPerk(PerkLib.PiercedFurrite, 0, 0, 0, 0);
                longP += "Furrite ";
                break;
            case 12:
                shortP += "crimstone ";
                if (this.player.findPerk(PerkLib.PiercedCrimstone) < 0) this.player.createPerk(PerkLib.PiercedCrimstone, 5, 0, 0, 0);
        ...
        //Dudes
        else if (this.piercingLoc == 1 && (this.player.cockThatFits(36) >= 0 || this.flags[kFLAGS.HYPER_HAPPY])) {
            this.yaraSex(false);
            return;
        }
    ...
    public oswaldPawn(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.Oswald) < 0) {
            ...
            if (this.player.cor < 75) this.outputText("Who would've thought that seeing someone NOT aroused would ever shock you?", false);
            else this.outputText("What a shame, but maybe you can give him a reason to stand up straight?", false);
    ...
    public barTelAdre(): void {
        ...
        var button = 0;
        this.clearOutput();
        if (this.flags[kFLAGS.LOPPE_DISABLED] == 0 && this.flags[kFLAGS.LOPPE_MET] == 0 && TelAdre.rand(10) == 0) {
        ...
        //DOMINIKA
        if (this.model.time.hours > 17 && this.model.time.hours < 20 && this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00150] != -1) {
        ...
        if (this.auntNancy.auntNancy(false)) {
            ...
            if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00263] > 0) button = this.anotherButton(button, "Nancy", this.auntNancy.interactWithAuntNancy);
    ...
    */
    ...
    public tailorShoppe(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.Victoria) < 0) {
            ...
            if (this.player.tallness > 60) this.outputText("looking for the source, eventually looking down and at a short but busty Corgi dog-girl.  ", false);
    ...
    private buyClothes(itype: ItemType): void {
        ...
        if (this.player.biggestCockArea() > 30 || this.player.totalCocks() > 1) this.outputText("fondle your bulging package", false);
    ...
    private watchUrtaBeABadass(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.Infested) >= 0) this.outputText("aroused", false);
    ...
    public gymDesc(): void {
        //PREGGO ALERT!
        if (this.flags[kFLAGS.PC_IS_A_GOOD_COTTON_DAD] + this.flags[kFLAGS.PC_IS_A_DEADBEAT_COTTON_DAD] == 0 && this.cotton.pregnancy.isPregnant) {
        ...
        else if (this.flags[kFLAGS.PC_FETISH] > 0 || this.player.cor > 80) this.outputText("unfortunately ", false);
        ...
        if (this.flags[kFLAGS.LIFETIME_GYM_MEMBER] == 0) this.outputText("10 gems an hour to use the facilities here, or 500 for a life-time membership.</i>\"  She has her hands on her hips, and it looks you'll have to pay ten gems to actually get to use any of this stuff.", false);
        else this.outputText("Oh, welcome back " + this.player.short + ".  Have a nice workout!</i>\"", false);
    ...
    private weightLifting(): void {
        ...
        //(< 25 str)
        if (this.player.str < 25) this.outputText("You have to start out on the smaller weights to the left side of the rack due to your strength, but even so, you manage to work up a good burn and a modest sweat.", false);
        //(< 40 str)
        else if (this.player.str < 40) this.outputText("You heft a few of the weights and select some of the ones just to the left of the middle.  It doesn't take you long to work up a sweat, but you push on through a variety of exercises that leave your body feeling sore and exhausted.", false);
        //(< 60 str)
        else if (this.player.str < 60) this.outputText("You smile when you grip a few of the heavier weights on the rack and start to do some lifts.  With a start, you realize you're probably stronger now than Ingnam's master blacksmith, Ben.  Wow!  This realization fuels you to push yourself even harder, and you spend nearly an hour doing various strength-building exercises with the weights.", false);
        //(<80 str)
        else if (this.player.str < 80) this.outputText("You confidently grab the heaviest dumbbells in the place and heft them.  It doesn't take long for you to work up a lather of sweat and feel the burn thrumming through your slowly tiring form.  The workout takes about an hour, but you feel you made some good progress today.", false);
        //(<90)
        else if (this.player.str < 90) this.outputText("You grab the heaviest weights they have and launch into an exercise routine that leaves you panting from exertion.  Setting the weights aside, you flex and marvel at yourself – you could probably arm wrestle a minotaur or two and come out victorious!", false);
        //(else)
        else this.outputText("This place barely has anything left to challenge you, but you take the heaviest weights you can get your mitts on and get to it.  By the time an hour has passed, you've worked up a good sweat, but without heavier weights you probably won't get any stronger.", false);
        //Stat changes HERE!
        if (this.player.str < 90) this.dynStats("str", .5);
        if (this.player.tou < 40) this.dynStats("tou", .3);
        //Body changes here
        //Muscleness boost!
    ...
    private goJogging(): void {
        ...
        //(<25 tou)
        if (this.player.tou < 25) this.outputText("but you get so winded you have to stop after a few minutes.  Determined to improve, you force yourself to stay at a fast walk until you can run again.", false);
        //(<40 tou)
        else if (this.player.tou < 40) this.outputText("but your performance isn't that great.  You nearly stop jogging a few times but manage to push through until you're completely exhausted.", false);
        //(<60 tou)
        else if (this.player.tou < 60) this.outputText("and you do quite well.  You jog around for nearly an hour, working up a healthy lather of sweat.  Even your " + this.player.legs() + " tingle and burn with exhaustion.", false);
        //(<80 tou)
        else if (this.player.tou < 80) this.outputText("and it doesn't faze you in the slightest.  You run lap after lap at a decent clip, working yourself until you're soaked with sweat and fairly tired.", false);
        //(<90 tou)
        else if (this.player.tou < 90) this.outputText("and you have a terrific time.  You can keep yourself just below your sprinting speed for the entire time, though you work up a huge amount of sweat in the process.", false);
        //else)
        else this.outputText("and it barely challenges you.  You run at a sprint half the time and still don't feel like you're improving in the slightest.  Still, you do manage to burn a lot of calories.", false);
        //Stat changes HERE!
        if (this.player.spe < 40) this.dynStats("spe", .3);
        if (this.player.tou < 90) this.dynStats("tou", .5);
    ...
    private letsDoYaraSex(girl: Boolean = true): void {
        ...
        if (!girl && this.player.hasCock()) this.outputText(" as your " + this.cockDescript(x) + " rises to attention, her eyes fixed upon the stiffened tool.  You resist the urge to grab her thin-yet-girlish hips and power into her right then and there, curious enough to allow her teasing.", false);
        else this.outputText(" as a growing puddle of love stains the cushioned chair.  It takes most of your power to not drag her down and force her face into your box.", false);
        this.outputText("\n\n", false);
        ...
        if (this.player.balls > 0) this.outputText("  Her dexterous fingertips brush against your " + this.ballsDescriptLight() + ", light and fluttery strokes that send shivers coursing through you.  The near-lack of contact is at least as titillating as the less-subtle Marethians you've come across.", false);
        ...
        //female
            ...
            if (this.player.wetness() >= 3) this.outputText("She may as well be licking a melting popsicle with how wet your snatch is.", false);
    ...
    private yvonneFlirt(): void {
        ...
        if (this.flags[kFLAGS.YVONNE_FUCK_COUNTER] == 0) this.outputText("Well, I could use a quick fuck.  If you meant what you said, go change the sign to say 'out' please.");
    ...
    private fuckYvonneInZeBlacksmith(): void {
        ...
        if (this.player.cockArea(x) < 20) this.outputText("runt");
        else if (this.player.cockArea(x) < 40) this.outputText(this.player.mf("boy", "girl"));
        else if (this.player.cockArea(x) < 60) this.outputText("big " + this.player.mf("boy", "girl"));
        else this.outputText(this.player.mf("stud", "breeder"));
        this.outputText(".</i>\"");
    ...
    private flirtWithVictoria(itype: ItemType): void {
        ...
        else this.outputText("  She presses her arms inward to increase the pressure on your cock even further, and gives you another wide smile.  <i>\"Ready for this, love?\"</i>");
