...
export class Bazaar extends BaseContent {
    ...
    public findBazaar(): void {
        ...
        if (this.flags[kFLAGS.BAZAAR_ENCOUNTERED] == 0) {
            this.flags[kFLAGS.BAZAAR_ENCOUNTERED]++;
    ...
    public enterTheBazaarAndMenu(demons: boolean = true): void {
        ...
        if (this.model.time.hours >= 15 && this.model.time.hours <= 20 && this.flags[kFLAGS.CINNABAR_NUMBER_ENCOUNTERS] > 0) rat = "Cinnabar";
        ...
        if (this.model.time.hours >= 9 && this.model.time.hours <= 17) {
            if ((this.flags[kFLAGS.FEMOIT_NEXTDAY_EVENT_DONE] == 1 && this.getGame().model.time.days >= this.flags[kFLAGS.FEMOIT_NEXTDAY_EVENT]) || this.flags[kFLAGS.FEMOIT_NEXTDAY_EVENT_DONE] != 1) {
        ...
        if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00292] == 0 && Bazaar.rand(4) == 0 && demons) {
            this.overHearDemonsAboutSyrena();
            return;
        }
        if ((this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00292] == 1 || this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00292] == 2) && demons && Bazaar.rand(10) == 0) {
            //[Repeat Variant]
    ...
    private joeyAndrogyny(): void {
        ...
        else if (this.player.gender == 2) this.outputText("you'll be able to be as masculine as you want!", false);
        else this.outputText("you'll be able to be as cute as me or as masculine as you want!", false);
    ...
    private joeysMassageWifNoExtraJizz(): void {
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.totalCocks() > 1) {
                ...
                if (this.player.totalCocks() > 2) {
                    ...
                    if (this.player.totalCocks() > 3) this.outputText(" as necessary until " + this.sMultiCockDesc() + " is coated with dick-drenching bunny spit.", false);
                    else this.outputText(", coating your " + this.cockDescript(2) + " with dick-drenching bunny spit.", false);
                }
            }
        ...
        if (!this.player.hasCock()) {
            ...
            //(Female orgasm)
                ...
                if (this.player.clitLength >= 4) this.outputText("like a practiced whore fellating a john.", false);
        ...
        //(Dickgasms
            ...
            if (this.player.cumQ() >= 700) {
                ...
                if (this.player.cumQ() >= 2000) this.outputText("massive, jiggling belly you've given him.", false);
        ...
        this.player.orgasm();
        if (this.player.lib > 20) this.dynStats("lib", -.5);
    ...
    private joeysMassageWithEXTRASpooge(): void {
        ...
        if (this.player.balls > 0) this.outputText("Your balls pull and relax and pull tight, bouncing below you over and over, all while working hard to produce more juice for you to gush.  ", false);
        ...
        if (this.player.cumQ() >= 1500) this.outputText(".  The sound of your rivers of spooge splattering against the floor barely registers, but a part of you is proud to be so incredibly fertile", false);
        this.outputText(".\n\n", false);
        ...
        if (this.player.cumQ() >= 1500) this.outputText("  Joey's jaw drops from the unexpectedly large volume you're producing, and he watches in awe as each huge globule of cum rolls off you towards the floor.", false);
        this.outputText("\n\n", false);
        ...
        if (this.player.skinType != SKIN_TYPE_FUR) this.outputText("over your " + this.player.skinDesc, false);
        ...
        if (!this.player.hasFuckableNipples()) this.outputText("squeezing and tugging on them", false);
        ...
        if (this.player.cumQ() >= 1500) this.outputText("  You really flooded the room though, so you'd best walk on your tip-toes until we can get a succubi or a goblin in here to clean up.", false);
    ...
    private joeyBigBalls(): void {
        ...
        if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00348] == 0) {
            ...
            if (this.player.cor > 60) this.outputText(", with no more than a bit of wishful thinking,", false);
    ...
    private suckOffJoeysGardenHose(): void {
        ...
        if (this.player.minotaurAddicted()) this.outputText(" and find yourself wishing you had one of these potions to use on a minotaur.  It would be divine!", false);
        else this.outputText(" and keep at it.  The knowledge that you're helping a friend is almost as filling as his delicious cum.", false);
        ...
        else if (this.player.cor < 66) this.outputText("also kind of fun, in a naughty way.", false);
        else this.outputText("exactly the kind of nasty stuff that gets your blood pumping.", false);
    ...
    private overHearDemonsAboutSyrena(): void {
        this.outputText("", true);
        if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00292] == 0) {
    ...
    private woolCockSock(): void {
        ...
        this.flags[kFLAGS.SOCK_HOLDING] = "wool";
    ...
    private cockringCockSock(): void {
        ...
        this.flags[kFLAGS.SOCK_HOLDING] = "cockring";
        this.cockSelectionMenu();
    }
    ...
    private viridianCockSock(): void {
        ...
        this.flags[kFLAGS.SOCK_HOLDING] = "viridian";
        this.cockSelectionMenu();
    }
    ...
    private scarletCockSocK(): void {
        ...
        this.flags[kFLAGS.SOCK_HOLDING] = "scarlet";
        this.cockSelectionMenu();
    }
    ...
    private gildedCockSock(): void {
        ...
        this.flags[kFLAGS.SOCK_HOLDING] = "gilded";
        this.cockSelectionMenu();
    }
    ...
    private yesPutDatSockOnMe(target: number): void {
        ...
        if (this.flags[kFLAGS.SOCK_HOLDING] == "gilded") this.player.gems -= 3000;
        ...
        if (this.flags[kFLAGS.SOCK_HOLDING] == "viridian") {
        ...
        if (this.flags[kFLAGS.SOCK_HOLDING] == "cockring") {
        ...
        if (this.flags[kFLAGS.SOCK_HOLDING] == "alabaster") this.player.gems -= 25;
    ...
    private removeTargettedSock(index: number): void {
        ...
        if (extra && storage == "cockring") {
            if (this.player.perkv1(PerkLib.PentUp) >= 10) this.player.addPerkValue(PerkLib.PentUp, 1, -5);
            else this.player.setPerkValue(PerkLib.PentUp, 1, 10);
        }
        else {
            if (storage == "gilded") {
    ...
    */
    ...
    private abuseHisAss(): void {
        ...
        //[(If multicock)
        if (this.player.cockTotal() > 1) this.outputText("  Your hand strokes along your " + this.multiCockDescriptLight() + " and press them to the warm cheeks exposed before you.");
        else this.outputText("  You pump your " + this.multiCockDescriptLight() + " and milk a pearl of pre-cum out to drip onto the enticing ass before you.");
        ...
        if (this.player.smallestCockLength() >= 16) {
            ...
            if (this.player.totalCocks() == 1) this.outputText("  You squeeze your " + this.multiCockDescriptLight() + " between his warm ass cheeks, stroking yourself with his exposed bottom.  The tiger whimpers meekly under you.");
            else this.outputText("  Your fattest cock grinds between the tiger man's rear, dripping spunk over his back and clothing while your other cocks make a sloppy mess of his ass.");
        ...
        //[[(If largest cock is smaller than 16)
            ...
            //[(If multicock)
            if (this.player.cockTotal() > 1) this.outputText("  One of your cocks slips into his hot, constricting confines, the remaining lengths pressed fast to the tiger man's rear and leaving glistening trails of eager fluids like markings on a bitch's ass.");
            else this.outputText("  Your " + this.multiCockDescriptLight() + " pushes in, your pre-cum-slicked length sliding in with a wet noise.");
            ...
            if (this.player.balls > 0) this.outputText(" swatting your " + this.ballsDescriptLight() + " against him,");
            this.outputText(" making him shout out in discomfort.  Your " + this.multiCockDescriptLight() + " stretching him out");
            if (this.player.cockTotal() > 1) this.outputText(" while beating his rear");
    ...
    private collectSomeButtSmex(): void {
        ...
        if (this.player.hasVagina()) this.outputText("  Your " + this.vaginaDescript(0) + " grows slick in your excitement.");
        if (this.player.gender == 0) this.outputText("  Your skin heats and tingles, sweat wetting its surface.");
        ...
        if (this.player.hasCock()) this.outputText(" wrap around your " + this.multiCockDescriptLight() + ".");
        else this.outputText(" rub over your " + this.assholeDescript() + ".");
        ...
        if (this.player.hasCock()) this.outputText(" and hard shaft");
        ...
        if (this.player.cockTotal() > 1) this.outputText(" and your " + this.multiCockDescriptLight() + " wetly slaps your stomach with need");
        ...
        if (this.player.lib < 50) this.outputText("fear");
        else this.outputText("excitement");
        this.outputText(".");
        ...
        if (this.player.isBiped()) this.outputText("  The bear shifts where he stands, grabbing your ankles and spreading your limbs almost painfully wide.  You wince from the sting of the strain on your leg muscles.");
        ...
        if (this.player.lib < 50) this.outputText("horrified");
        ...
        if (this.player.hasCock()) this.outputText(" while [eachCock] twitches and leaks generously over your " + this.player.skinFurScales());
        this.outputText(".");
    ...
    private finalGayFinallee(road: number = 0): void {
        ...
        if (road == 0) {
            ...
            if (this.player.hasCock()) this.outputText("His hand dips down to play with your " + this.multiCockDescriptLight() + ", stroking you off as he gives a few more brutal pounds of his hips.  ");
            else if (this.player.hasVagina()) this.outputText("Fingers ram into your " + this.vaginaDescript(0) + ", thrusting into you as he fucks your other hole.  ");
            ...
            if (this.player.gender > 0) this.outputText("  Your lips part as you moan, feeling your climax building before coating the tiger's hand and your lower body in your sex.  The tiger man just brings his hand up to his lips to taste your juices before hilting into you one last time.");
        ...
        //((Ending 2 , suck bear, //crazy steel jaw))
            ...
            if (this.player.gender > 0) this.outputText("  Your lips part as you moan, feeling your climax building before coating the bear's hand and your lower body in your sex.  The bear just brings his hand up to his lips to taste your honey before hilting into you one last time.");
        ...
        //((Ending 3 , suck none, //No furry plx))
            ...
            if (this.player.hasCock()) this.outputText("  His hand drips down to clumsily play with your " + this.multiCockDescriptLight() + ", stroking you off as he gives you a few more brutal pounds of his hips.");
            else if (this.player.hasVagina()) this.outputText("  Fingers dip into your " + this.vaginaDescript(0) + ", ramming into you as he fucks your other hole.");
            if (this.player.gender > 0) this.outputText("  Your lips part as you feel your climax building, and soon you coat the orc's hand and your lower body in your lust.  The orc just brings his hand up to taste your juices before hilting into you one last time.");
