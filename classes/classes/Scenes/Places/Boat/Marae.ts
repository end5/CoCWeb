...
export class Marae extends AbstractBoatContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        if (this.model.time.hours > 23) {
            if (this.flags[kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] > 0) { //Marae met 2nd time?
                if (this.flags[kFLAGS.FUCK_FLOWER_KILLED] == 0) { //If flower hasn't been burned down yet
                    if (this.flags[kFLAGS.FUCK_FLOWER_LEVEL] < 4 && this.flags[kFLAGS.FUCK_FLOWER_GROWTH_COUNTER] < 1000) { //Grow flower if it isn't fully grown.
                        this.flags[kFLAGS.FUCK_FLOWER_GROWTH_COUNTER]++;
                    }
                }
            }
            if (this.flags[kFLAGS.HOLLI_FUCKED_TODAY] == 1) this.flags[kFLAGS.HOLLI_FUCKED_TODAY] = 0; //Holli Fuck Tracking
        }
        return false;
    }
    ...
    public encounterMarae(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.DungeonShutDown) < 0) {
            ...
            if (this.player.findStatusAffect(StatusAffects.MetMarae) < 0) {
                ...
                if (this.player.cor > 66) this.outputText("The woman bellows, \"<i>Begone demon.  You tread on the precipice of damnation.</i>\"  The tree's eyes flash, and you find yourself rowing back to camp.  The compulsion wears off in time, making you wonder just what that tree-woman was!", false);
                //Explain the dungeon scenario
                    ...
                    if (this.player.lib + this.player.cor > 80) this.outputText("Your eyes can't help but take note of the tree-woman's shapely breasts, and wonder if they feel like tits or wood.  ", false);
    ...
    private maraeBadEnd(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.MetCorruptMarae) < 0) this.outputText("The goddess flows out of the tree, stepping away from it as a living woman, curvy and nude.\n\n", false);
        ...
        if (this.player.gender == 0) this.outputText(" where it pauses in momentary confusion", false);
        ...
        if (this.player.statusAffectv3(StatusAffects.Marble) > 0) this.outputText("Any thought or need to drink Marble's milk vanishes from your mind and body.  ", false);
    ...
    public level2MaraeEncounter(): void {
        ...
        if (this.player.cor < 33) this.outputText("Knowing all too well what kinds of horrors await", false);
        else if (this.player.cor < 66) this.outputText("Knowing all too well what kinds of sexual escapades lurk ahead", false);
        ...
        else this.outputText("nearly dropping you flat on your ass before you catch yourself and sit down", false);
        ...
        if (this.player.weaponName != "fists") this.outputText(", adjust your " + this.player.weaponName + ",", false);
        ...
        //(FAIL-STOLE)
        else if (this.player.findPerk(PerkLib.MaraesGiftStud) >= 0 || this.player.findPerk(PerkLib.MaraesGiftFertility) >= 0) {
    ...
    private MaraeIIStageII(): void {
        ...
        //Marae uses a tree-tentacle to 'milk' male PC's.  Oral or Vajayjay? Not sure.
            ...
            if (this.player.cockTotal() == 2) this.outputText(" while your other dangles against her thigh", false);
            else if (this.player.cockTotal() > 2) this.outputText(" while your others dangle against her thighs", false);
            ...
            if (this.player.cockTotal() == 2) this.outputText("  Another vine that may as well be the first's twin snakes from between the goddess' legs and jumps onto your " + this.cockDescript(1) + ".", false);
            else if (this.player.cockTotal() > 2) this.outputText("  More 'open' vines shimmy forth from between Marae's legs and jump up onto your " + Appearance.cockNoun(CockTypesEnum.HUMAN) + "s.", false);
        ...
        //[Hermz]  Marae grows vinecawks for DP under her flowercunt and sexes.
            ...
            if (this.player.hasFuckableNipples()) this.outputText(", slipping inside your cunt-like nipples with ease.  You moan into her ear at the unexpected penetration and shiver from the tingling shocks of pleasure in your " + this.breastDescript(0) + ".", false);
            ...
            if (this.player.cockTotal() == 2) this.outputText(" while your other cock rubs over her surface", false);
            else if (this.player.cockTotal() > 2) this.outputText(" while your other cocks rubs over her surface", false);
            this.outputText(".\n\n", false);
            ...
            if (this.player.cocks[0].cockThickness > 5 || this.player.cockArea(0) > 100) this.outputText("With how big you are, there's no way it should be able to fit, but her body isn't even distorted by your girth.  Perhaps she changed you to fit her?  You pull back and your thickness seems unchanged.  You shake your head to clear the unwelcome thoughts and ram yourself back into her.  Fucking is what's important.  ", false);
            else this.outputText("She feels perfect.  A velvet vice of hot, slippery wetness clutches tightly around your " + this.cockDescript(0) + ".  It almost feels like it's actually gripping you, cradling your cock in her ambrosia-slicked box.", false);
            if (this.player.cockTotal() > 1) {
                if (this.player.cockTotal() > 2) this.outputText("  Another ", false);
                else this.outputText("Your other ", false);
                this.outputText(Appearance.cockNoun(CockTypesEnum.HUMAN) + " prods at her tight pucker, and with a slight adjustment, you're able to line it up.  It's wet!  Inch after inch slides in with incredible ease, violating her slippery butthole until you've completely double-penetrated her.", false);
            }
            this.outputText("\n\n", false);

            if (this.player.vaginas[0].vaginalWetness < VAGINA_WETNESS_WET) this.outputText("Sticky wetness glistens between your thighs", false);
            else if (this.player.vaginas[0].vaginalWetness < VAGINA_WETNESS_DROOLING) this.outputText("Drops of feminine arousal run down your thighs", false);
            else this.outputText("Trails of viscous feminine fluid leak from your " + this.vaginaDescript(0), false);
            ...
            if (this.player.cockTotal() > 1) this.outputText("Her asshole is squelching and dripping from your " + this.cockDescript(1) + ", actually squirting more lubricant than her pussy from the violent fucking!  ", false);
            ...
            if (this.player.cockTotal() > 1) this.outputText("The " + this.cockDescript(1) + " in her ass spasms and explodes with its brother, glazing her slippery colon with a coating of syrupy spunk.  ", false);
            if (this.player.cockTotal() > 2) {
                ...
                if (this.player.cockTotal() > 3) this.outputText(this.multiCockDescriptLight(), false);
            ...
            if (this.player.cumQ() < 500) this.outputText("The orgasm is so much more massive than normal, and you wonder if her magic enhanced it.  ", false);
            if (this.player.cumQ() > 1000) this.outputText("She's actually surprised when you keep fountaining more seed into her.  Her nipples start to squirt out the excess seed, but you keep cumming until she's squirted enough to soak herself with your jism.  ", false);
    ...
    private MaraePt2RoundIIIPrizes(): void {
        ...
        if (this.player.gender == 1) {
            ...
            //(STUD)
            if (this.player.findPerk(PerkLib.MaraesGiftStud) >= 0) this.outputText("Well, I see my gift is working out quite well for you, isn't it?  That's excellent.  It was an incomplete gift given by an incomplete goddess, but now that I've gotten my hands on you again, I was able to fix it.  You'll build up cum three times as fast as before, no more waiting for days just to build up a huge load for all the horny girls out there!</i>\"  ", false);
            //(NON STUD)
            else this.outputText("You might be a little sore.  I did some work to make sure you'll be a perfect breeding stud for me.  No tiny cum-shots for you!  You'll squirt out enough to knock up anyone, and I even touched up your seed so it'll get through most contraceptives.  Aren't I the nicest?</i>\"  ", false);
            //(CONTINUED)
        ...
        //[HERMS]
            ...
            //(HAZ NEITHER)
            if (this.player.findPerk(PerkLib.MaraesGiftFertility) < 0 && this.player.findPerk(PerkLib.MaraesGiftStud) < 0) {
                //(RANDOM 1)
                if (Marae.rand(2) == 0) {
                    this.outputText("You might feel a little sore.  I gave your little womb a makeover to make sure you'll be nice and fertile for all the boys out there.  You're going to serve me so well.  So many died fighting the demons, and you'll be popping out kids from every dick that gets anywhere near your little birth-hole.</i>\"  ", false);
                    this.player.createPerk(PerkLib.MaraesGiftFertility, 0, 0, 0, 0);
                ...
                if (this.player.findPerk(PerkLib.MaraesGiftFertility) >= 0) this.outputText("<b>(New Perk Gained: Marae's Gift - Fertility)</b>", false);
