...
export class Kelly extends AbstractFarmContent implements TimeAwareInterface {
    ...
    public constructor() {
        super();
        this.pregnancy = new PregnancyStore(kFLAGS.KELLY_PREGNANCY_TYPE, kFLAGS.KELLY_INCUBATION, 0, 0);
    ...
    public timeChange(): boolean {
        ...
        if (this.model.time.hours > 23) {
            if (this.flags[kFLAGS.KELLY_REWARD_COOLDOWN] > 0 && this.model.time.days % 3 == 0) this.flags[kFLAGS.KELLY_REWARD_COOLDOWN] = 0;
            if (this.flags[kFLAGS.KELT_BREAK_LEVEL] >= 4) this.flags[kFLAGS.KELLY_DISOBEYING_COUNTER]++;
        }
    ...
    public breakingKeltOptions(): void {
        ...
        if ((!this.player.hasCock() && this.flags[kFLAGS.KELT_BREAK_LEVEL] == 0) || this.flags[kFLAGS.NEVER_RESIST_KELT] == 1 || this.player.statusAffectv2(StatusAffects.Kelt) >= 40 || this.player.findStatusAffect(StatusAffects.Kelt) < 0) {
    ...
    private breakKeltGo(): void {
        ...
        if (!this.player.isTaur()) this.outputText("pulling [eachCock] out of your [armor] and stroking yourself with your free hand");
        else {
        ...
        if (!this.player.isTaur()) this.outputText("fully whip out your [cock biggest] and shove it between her busty tits and slowly start grinding");
        else this.outputText("step over her, shoving your [cock biggest] between her busty tits, grinding it through that smooth valley");
        this.outputText(".");
        ...
        if (this.player.isTaur()) this.outputText("step over her, moving your hips back and forth to thrust through her more-than-ample cleavage.");
        else this.outputText("grab her nipples and begin to thrust your pecker against her boob valley.");
    ...
    public defeatKellyNDBREAKHIM(): void {
        ...
        //(Silly:)
        if (this.silly() && kGAMECLASS.amilyScene.amilyFollower() && kGAMECLASS.amilyScene.amilyCorrupt()) this.outputText("\n\nSomewhere, Amily perks up, \"<i>[Master]?</i>\"  How many cumbuckets do you need anyway?");
    ...
    public breakingKeltNumeroThree(): void {
        ...
        if (!this.player.isTaur()) this.outputText("Soon one breast isn't enough to contain your lust-driven energy and you reach for the other one with your free hand.  Your constant nipple-teasing has an effect on");
        else this.outputText("Soon, you cannot contain your lust, and you begin to buck against her hands, whinnying with delight.  The rigid, pulsing dick-flesh in her mouth has an effect on");
        ...
        if (!this.player.isTaur()) this.outputText("You squeeze Kelly's boobs so harshly she screams in pain, and y");
        else this.outputText("Y");
    ...
    private finalKeltBreaking(): void {
        ...
        if (this.player.balls > 0) this.outputText("Her free hands moves down on your groin and caresses your [balls] with expert care and precision; obviously her previous experience in groping has had an effect on her current skills.  ");
        ...
        if (!this.player.isTaur()) this.outputText("You grope her bountiful tits and pull them closer to you as you fuck them.  ");
    ...
    private kellyAppearance(): void {
        ...
        if (this.flags[kFLAGS.KELLY_HEAT_TIME] > 0 && !this.pregnancy.isPregnant) this.outputText("<b>  There is so much leaking from her that you think she might be in heat, rendering her more receptive to impregnation.</b>");
        ...
        if (this.flags[kFLAGS.KELLY_KIDS] > 0) {
            if (this.flags[kFLAGS.KELLY_KIDS] == 1) {
                ...
                if (this.flags[kFLAGS.KELLY_FIRST_KID_GENDER] == 1) this.outputText("him playing in the distance, lost in his own little world.");
                else this.outputText("her playing in the distance, lost in her own little world.");
            }
            else if (this.flags[kFLAGS.KELLY_KIDS] == 2) this.outputText("\n\nThe " + Kelly.num2Text(this.flags[kFLAGS.KELLY_KIDS]) + " children you have had with Kelly are off in a separate field; the sound of their play drifts over the grasslands to your ears.  Kelly evidently prefers to keep her brood away from what you do with her, a remarkably sensible attitude coming from a centauress cumslut.  Maybe motherhood genuinely suits her?");
            else this.outputText("\n\nThe " + Kelly.num2Text(this.flags[kFLAGS.KELLY_KIDS]) + " children you have had with Kelly are as ever off in a separate field, the distance sounds of their acting out the wars and drama of childhood drifting out over the grasslands to your ears.");
            this.outputText("\n");
        }
        this.menu();
    ...
    private approachKelly(): void {
        ...
        if (this.flags[kFLAGS.FARM_CORRUPTION_STARTED] == 0) this.outputText("You aren't welcome on the farm proper, but you can go visit Kelly's field.");

        //09:00-11:00, 2 or more children:
        if (this.model.time.hours >= 9 && this.model.time.hours <= 11 && this.flags[kFLAGS.KELLY_KIDS] >= 2) {
    ...
    private fuckKellysCunt(): void {
        ...
        if (this.player.cumQ() >= 500) this.outputText("  Her pussy seems like a soggy, cum-soaked sex-toy before you're even halfway done.");
        if (this.player.cumQ() >= 1200) {
            ...
            if (this.player.cumQ() >= 4000) this.outputText(", until she's so pregnant with spooge that she can barely stand");
            this.outputText(".  ");
        }
        ...
        if (!this.pregnancy.isPregnant) this.outputText(" so the cum can turn you into a proper broodmare");
        else this.outputText(", maybe knit some for the kids you're going to be pumping out for me non-stop");
        this.outputText(".</i>\"");
    ...
    private taurOnTaurSexKelly(): void {
        ...
        if (this.player.balls > 0) this.outputText("  Your [balls] tingle pleasantly from the attention, and you groan when she sucks one into her mouth, worshipping it with her lips.");
        ...
        if (this.player.tallness < 60) this.outputText(" before sitting back down (she's too tall to take standing up) and lifting her tail");
        ...
        if (this.player.cumQ() >= 1000) {
            ...
            if (this.player.cumQ() >= 4000) this.outputText(", and when she grows too full to hold any more, waves of alabaster spooge roll out of her abused nether-lips, adding to the messy sex-puddle below");
            this.outputText(".  ");
        }
        ...
        if (this.player.cor < 50) this.outputText("  You feel a little bad for her and help her up - she did just milk your dick in the way that only a centaur can.");
    ...
    private tentaFuckKelly(): void {
        ...
        if (this.player.cor < 33) this.outputText("but also a bit disgusted to watch such shameless, wanton behavior, ");
    ...
    private getARimjobFromKelly(): void {
        ...
        if (this.player.isNaga()) this.outputText("  You clench her almost bone crushingly tight in your coils as rut overtakes you.");
        ...
        if (this.player.hasVagina()) this.outputText("; she slides as many fingers as she can into your [vagina] and frigs you as best she can");
        ...
        //[Pussy:]
        if (this.player.hasVagina()) this.outputText("  Your female sex orgasms in tandem, your wet flesh seizing up around Kelly's fingers and spattering her arm and chest with girl juices.");
        ...
        if (this.flags[kFLAGS.TIMES_RIM_JOBBED_BY_KELLY] == 1) {
            ...
            if (this.player.cumQ() >= 700) this.outputText("You, as ever, have produced a veritable oozing lake of the stuff.  ");
    ...
    private rideKellyForPunishment(): void {
        ...
        if (this.flags[kFLAGS.TIMES_RIDDEN_KELLY_FOR_PUNISHMENT] == 1) {
            ...
            if (this.player.tallness < 78 && !this.player.isTaur()) {
                ...
                //[Naga:]
                if (this.player.isNaga()) this.outputText("  Your reptilian half is hardly ideal for riding a horse but eventually you reach a happy compromise; you perch upon her with your long tail wrapped around her barrel-like lower body.");
                //[Drider:]
                else if (this.player.isDrider()) this.outputText("  Your spider half is hardly ideal for riding a horse but eventually you reach a happy compromise; you perch upon her with your long, chitinous legs wrapped carefully around her barrel-like lower body.  You must look utterly bizarre but feel reasonably comfortable.");
            ...
            //[PC is 6'6</i>\" or more/centaur:
                ...
                if (this.flags[kFLAGS.KELLY_LACTATING] > 0) {
                    if (this.flags[kFLAGS.KELLY_BONUS_TIT_ROWS] > 0) this.outputText("All four of h");
        ...
        //First repeat: 
            ...
            if (this.flags[kFLAGS.KELLY_BONUS_TIT_ROWS] > 0) this.outputText("  Just as well there were four of them, eh?");
    ...
    private takeKellysVirginity(): void {
        ...
        if (this.flags[kFLAGS.KELLY_CUNT_TYPE] == 1) this.outputText("lust-swollen, black-lipped fuck-hole");
        ...
        if (this.flags[kFLAGS.KELLY_CUNT_TYPE] == 1) this.outputText(", taking time to squeeze her plush, onyx vulva teasingly.  Kelly whinnies in response, overtaken by her potent equine instincts.");
        else this.outputText(", taking time to fondle her slick labia.  Kelly moans in response, overtaken by the sensations you're forcing through her.");
        ...
        if (!this.player.isTaur()) this.outputText("get down behind her and line your " + this.cockDescript(x) + " with her dribbling lips.");
        else this.outputText("lie down behind her, sliding your hindlegs behind hers in order to properly align your " + this.cockDescript(x) + " with her dribbling lips.");
        ...
        this.outputText("\n\nYou squeeze Kelly's ");
        if (this.player.isTaur()) this.outputText("breast as you snuggle up behind her, centaur to centaur,");
        ...
        else this.outputText(", pressing your " + this.player.cockHead(x) + " up against the barrier her cervix makes");
        ...
        if (this.player.cumQ() >= 500) {
            ...
            if (this.player.cumQ() >= 1500) this.outputText("  In no time flat, her belly has begun to round slightly, puffed up with spunk.");
            if (this.player.cumQ() >= 3500) this.outputText("  That doesn't stop you from seeding it even further, pumping her poor horse-gut until it jiggles and jostles with your spermy cargo.  There's no way she'll forget this.");
        }

        this.outputText("\n\nYou pull out with a self-satisfied sigh, admiring ");
        if (this.flags[kFLAGS.KELLY_CUNT_TYPE] == 0) this.outputText("the redness of her well-used cunt.");
        else this.outputText("the shining juices that drip from her oh-so-equine entrance, gilding the smooth onyx lips with a slimey sheen.");
    ...
    private kellyTitJob(): void {
        ...
        if (this.flags[kFLAGS.KELLY_BONUS_BOOB_ROWS] == 0) x = this.player.cockThatFits(18, "length");
        ...
        //[Lactation:
        if (this.pregnancy.isPregnant) this.outputText(", glittering beads of milk flicking here and there as she advances");
    ...
    private kellyPreggers(): void {
        ...
        if (Kelly.rand(100) + 1 <= 80) {
            this.pregnancy.knockUpForce(PregnancyStore.PREGNANCY_PLAYER, PregnancyStore.INCUBATION_CENTAUR - 84);
    ...
    private kellyPregSex(): void {
        ...
        if (this.player.cocks[x].cockType != CockTypesEnum.HORSE) this.outputText("\n\nToo bad she gets yours instead.");
    ...
    private talkNHandToKelly(): void {
        ...
        if (this.flags[kFLAGS.KELLY_TALK_N_HAND_TIMES] == 0) {
            ...
            if (!this.player.isTaur()) {
                ...
                else this.outputText("first your [cock], then your [cock 2], gently tugging until they are both hard from the pleasing friction she coils around them.");
            }
            //Centaur:
            else {
                this.outputText("You leisurely clop yourself around so your back end is facing her and sigh as ");
                if (this.player.cockTotal() == 1) this.outputText("a warm hands wraps itself around your [cock] and then begins to gently tug, quickly hardening it with pleasing coils of friction.");
        ...
        if (temp == 0) {
            ...
            //Futa:
            else this.outputText("  Now I am a woman, I work for another woman, and I spend most days polishing her [cock biggest]!");
            ...
            //1< cock:
            if (this.player.cockTotal() > 1) this.outputText("imagining thrusting every single one of your cocks into a wet, willing hole, ");
            //vagina:
            else if (this.player.hasVagina()) this.outputText("imagining one of them tongue-fucking your [vagina] as you buck into another's tight cunt, ");
            ...
            if (this.player.cor < 70) this.outputText("  Plus, the thought of making Kelt's victims pay the price for his own crimes makes you feel uncomfortable.");
        ...
        //4.
            ...
            if (this.player.balls == 0) this.outputText("suckles and laps at the very base of your [cock] for a time");
            ...
            if (this.flags[kFLAGS.KELLY_KIDS] > 0) this.outputText(" and bearing and raising your children");
    ...
    private giveKellyAppleSauce(): void {
        ...
        if (this.flags[kFLAGS.KELLY_TIMES_APPLESAUCED] == 0) {
            ...
            if (!this.farm.farmCorruption.whitneyCorrupt()) this.outputText(" \"<i>I'd like to go and check on it, but... I don't think Whitney likes me much anymore.  I'd hate to cause more trouble than I already have.</i>\"");
            else this.outputText("“<i>I'd like to go and check on it, but... I’d hate to get into Mistress Whitney’s bad books.  I’ve caused enough trouble already.</i>”");
            ...
            if (!this.player.isTaur()) {
                ...
                //[High cum:]
                if (this.player.cumQ() >= 750) this.outputText("  The bowl quickly begins to brim as your [cock] torrents out cum, and eventually you have to turn away, grunting out the last jets of filth into the undergrowth.");
            }
            //[Centaur:]
            else {
                ...
                if (this.player.hasVagina()) this.outputText("\n\nYour [vagina] quivers and clenches, and gasping with the dual orgasm which is clutching your body you stand over the bowl and let some of your femcum drip into it.  Taste my summer flavor Kelly, you think wildly, and laugh with a slightly manic edge.");
                //[High cum:
                if (this.player.cumQ() >= 750) this.outputText("  Your [cock] torrents out cum, and eventually you have to clop away, grunting out the last jets of filth onto the ground to avoid overflowing the bowl.");
            }
        ...
        //Repeat:
            ...
            if (this.flags[kFLAGS.KELLY_KIDS] > 1) this.outputText("  Somebody has even cleared an oblong patch of earth alongside the trees- a piece of card stuck into it is scrawled with a childish hand which declares it to be “CAROTS”.  You marvel at it all.  Who would have imagined your lust for horse pussy would have turned a pointless asshole into a loving mother who gardens with her children?  Really, you deserve a medal for your efforts.");
            ...
            if (!this.player.isTaur()) {
                ...
                if (this.player.hasVagina()) this.outputText("  Your [vagina] quivers and clenches, and once again, gasping with the dual orgasm which is clutching your body, you stand over the bowl and let some of your femcum drip into it.");
                //[High cum:
                if (this.player.cumQ() >= 750) this.outputText("  The bowl quickly begins to brim as your [cock] torrents out cum, and eventually you have to turn away, grunting out the last jets of filth into the undergrowth.");
            }
            //[Centaur:
        ...
        if (!this.player.isTaur()) {
            ...
            if (this.player.cocks[x].cockThickness >= 4) this.outputText("  Your girth spreads her fuck tunnel wide and you grunt at the delicious tightness of it as you pack her full of cock.");
            ...
            this.outputText("\n\nWith her head thrown back it's easy for you to grip her " + this.flags[kFLAGS.KELLY_HAIR_COLOR] + " braid and use it for leverage as you fuck into her with everything you've got, bucking into her tight, hot flesh ");
            if (this.player.balls > 0) this.outputText(", your [balls] slapping into her thighs demandingly ");
        ...
        //Centaur: 
            ...
            this.outputText("\n\nYou grip her " + this.flags[kFLAGS.KELLY_HAIR_COLOR] + " braid and force her head to arch back before pushing the bowl against her lips, tipping the last of the green sweet down her gullet whilst you begin to fuck into her with everything you've got, bucking into her tight, hot flesh");
            if (this.player.balls > 0) this.outputText(", your [balls] slapping into her thighs demandingly");
            ...
            if (y >= 0) this.outputText(" as her anus clenches around your " + this.cockDescript(y));
    ...
    private kellyBJsAhoy(): void {
        ...
        if (!this.player.isTaur()) {
            ...
            //Balls:
                ...
                if (this.player.ballSize <= 8) {
                    ...
                    if (this.player.balls >= 4) this.outputText(", then the third, then the fourth");
            ...
            if (this.player.biggestCockLength() > 15) this.outputText("It's a long way up and the effect is like a lift slowly rising, turning on a glow of lights on each level as it goes.  ");
            ...
            if (this.flags[kFLAGS.KELLY_BONUS_BOOB_ROWS] > 0) this.outputText(" onto all four of her tits");
        ...
        //Centaur:
            ...
            if (this.player.cockTotal() > 1 || this.player.hasVagina()) this.outputText("  You're in no hurry, and with a slight roll of your [hips] you direct her to start at the front and work her way down.");
            ...
            //[Vagina:
            if (this.player.hasVagina()) this.outputText("  Your female sex clenches and orgasms in sympathy, and you feel fluid dribble down your [hips] as you continue to paint Kelly.  Your only dim regret is that there's no way of soaking her with that, too.");
            this.outputText("\n\nFinally you are spent.  You clop around to admire your handiwork.");
            //Low cum:
            if (this.player.cumQ() < 750) this.outputText("  Kelly has done a sterling job of catching your load; you've frosted her boobs with long strokes quite convincingly.");
            else {
        ...
        if (!this.player.isTaur()) this.outputText(", still clutching your pleasantly aching prick");
        ...
        //Libido >50:
            ...
            if (this.player.hasVagina()) this.outputText(", letting her sensually lick you out again until your [vagina] is gleaming with arousal");
            if (this.player.balls > 0) this.outputText(", letting her needily suckle on your [balls] again until they are inflamed with renewed pressure");
