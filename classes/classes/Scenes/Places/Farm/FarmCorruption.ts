...
export class FarmCorruption extends AbstractFarmContent {
    ...
    public whitneyCorruption(mod: number = 0): number {
        if (mod != 0) {
            ...
            if (this.flags[kFLAGS.WHITNEY_CORRUPTION] > this.flags[kFLAGS.WHITNEY_CORRUPTION_HIGHEST]) this.flags[kFLAGS.WHITNEY_CORRUPTION_HIGHEST] = this.flags[kFLAGS.WHITNEY_CORRUPTION];
        }

        // Cap the values into the appropriate range
        if (this.flags[kFLAGS.FARM_CORRUPTION_APPROACHED_WHITNEY] == 0 && this.flags[kFLAGS.WHITNEY_CORRUPTION] > 30) {
            this.flags[kFLAGS.WHITNEY_CORRUPTION] = 30;
            trace("Whitney at 30 corruption clamp.");
        }
        else if (this.flags[kFLAGS.WHITNEY_LEAVE_0_60] == 0 && this.flags[kFLAGS.WHITNEY_CORRUPTION] > 60) {
            this.flags[kFLAGS.WHITNEY_CORRUPTION] = 60;
            trace("Whitney at 60 corruption clamp.");
        }
        else if (this.flags[kFLAGS.WHITNEY_LEAVE_61_90] == 0 && this.flags[kFLAGS.WHITNEY_CORRUPTION] > 90) {
            this.flags[kFLAGS.WHITNEY_CORRUPTION] = 90;
            trace("Whitney at 90 corruption clamp.");
        }
        else if (this.flags[kFLAGS.WHITNEY_MENU_91_119] == 0 && this.flags[kFLAGS.WHITNEY_CORRUPTION] > 119) {
            this.flags[kFLAGS.WHITNEY_CORRUPTION] = 119;
            trace("Whitney at 119 corruption clamp");
        }

        // Clamp values to valid min/max
        if (this.flags[kFLAGS.WHITNEY_CORRUPTION] < 0) this.flags[kFLAGS.WHITNEY_CORRUPTION] = 0;
        if (this.flags[kFLAGS.WHITNEY_CORRUPTION] >= 120) {
            this.flags[kFLAGS.WHITNEY_CORRUPTION] = 120;
        }

        trace("Whitney corruption changed by " + String(mod));
        trace("Whitney corruption now at " + String(this.flags[kFLAGS.WHITNEY_CORRUPTION]));

        return this.flags[kFLAGS.WHITNEY_CORRUPTION];
    }

    public whitneyCorrupt(): boolean {
        if (this.flags[kFLAGS.WHITNEY_CORRUPTION_COMPLETE] > 0) return true;
        return false;
    }

    public whitneyDom(): boolean {
        if (this.flags[kFLAGS.WHITNEY_DOM] == 1) return true;
        return false;
    }

    public whitneyDefurred(): boolean {
        if (this.flags[kFLAGS.WHITNEY_DEFURRED] == 1) return true;
        return false;
    }
    ...
    public whitneyMaxedOralTraining(): boolean {
        if (this.flags[kFLAGS.WHITNEY_ORAL_TRAINING] == 100) return true;
        return false;
    }
    ...
    public updateFarmCorruption(): number {
        ...
        if (this.flags[kFLAGS.FARM_CORRUPTION_STARTED] <= 0) return this.flags[kFLAGS.WHITNEY_CORRUPTION];

        // If Whitney was disabled the previous day, set the flag for a follow up
        if (this.flags[kFLAGS.WHITNEY_DISABLED_FOR_DAY] == 1) {
            this.flags[kFLAGS.WHITNEY_DISABLED_FOR_DAY] = 2;
        }

        // Process queued farm upgrades

        // Breastmilkers
        if (this.flags[kFLAGS.QUEUE_BREASTMILKER_UPGRADE] != 0 && this.player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") < 0) {
            this.flags[kFLAGS.QUEUE_BREASTMILKER_UPGRADE] = 0;
            this.player.createKeyItem("Breast Milker - Installed At Whitney's Farm", 0, 0, 0, 0);
        }

        // Cockmilkers
        if (this.flags[kFLAGS.QUEUE_COCKMILKER_UPGRADE] != 0 && this.player.hasKeyItem("Cock Milker - Installed At Whitney's Farm") < 0) {
            this.flags[kFLAGS.QUEUE_COCKMILKER_UPGRADE] = 0;
            this.player.createKeyItem("Cock Milker - Installed At Whitney's Farm", 0, 0, 0, 0);
        }

        // Contraceptives
        if (this.flags[kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE] != 0) {
            this.flags[kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE]++;

            if (this.flags[kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE] > 7) {
                this.flags[kFLAGS.FARM_UPGRADES_CONTRACEPTIVE] = 1;
                this.flags[kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE] = 0;
            }
        }

        // Refinery
        if (this.flags[kFLAGS.QUEUE_REFINERY_UPGRADE] != 0) {
            this.flags[kFLAGS.QUEUE_REFINERY_UPGRADE]++;

            if (this.flags[kFLAGS.QUEUE_REFINERY_UPGRADE] > 3) {
                this.flags[kFLAGS.FARM_UPGRADES_REFINERY] = 1;
                this.flags[kFLAGS.QUEUE_REFINERY_UPGRADE] = 0
            }
        }

        // Milktank
        if (this.flags[kFLAGS.QUEUE_MILKTANK_UPGRADE] != 0) {
            this.flags[kFLAGS.QUEUE_MILKTANK_UPGRADE]++;

            if (this.flags[kFLAGS.QUEUE_MILKTANK_UPGRADE] > 3) {
                this.flags[kFLAGS.QUEUE_MILKTANK_UPGRADE] = 0;
                this.flags[kFLAGS.FARM_UPGRADES_MILKTANK] = 1;
        ...
        // Branding
        if (this.flags[kFLAGS.QUEUE_BRANDING_UPGRADE] != 0) {
            this.flags[kFLAGS.QUEUE_BRANDING_UPGRADE]++

            if (this.flags[kFLAGS.QUEUE_BRANDING_UPGRADE] > 2) {
                this.flags[kFLAGS.QUEUE_BRANDING_UPGRADE] = 0;
                this.flags[kFLAGS.FARM_CORRUPTION_BRANDING_MENU_UNLOCKED] = 1;
                this.flags[kFLAGS.QUEUE_BRANDING_AVAILABLE_TALK] = 1;
            }
        }

        // Orgyroom
        if (this.flags[kFLAGS.QUEUE_ORGYROOM_UPGRADE] != 0) {
            this.flags[kFLAGS.QUEUE_ORGYROOM_UPGRADE]++;

            if (this.flags[kFLAGS.QUEUE_ORGYROOM_UPGRADE] > 7) {
                this.flags[kFLAGS.QUEUE_ORGYROOM_UPGRADE] = 0;
                this.flags[kFLAGS.FARM_UPGRADES_ORGYROOM] = 2;
            }
        }
        ...
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 1 && kGAMECLASS.sophieBimbo.bimboSophie()) modValue += 1;
        ...
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_KELLY] == 1) modValue += 2;

        this.whitneyCorruption(modValue);

        // Now figure out how many gems we're gonna get for the day
        this.flags[kFLAGS.FARM_CORRUPTION_GEMS_WAITING] += this.farmValue();

        // If Amily is producing, stack up some milks to gib
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_AMILY] == 1 && this.flags[kFLAGS.FOLLOWER_PRODUCTION_AMILY] == 1 && this.flags[kFLAGS.FARM_UPGRADES_REFINERY] == 1) {
            if (this.flags[kFLAGS.FOLLOWER_AT_FARM_AMILY_GIBS_MILK] == 0) this.flags[kFLAGS.FOLLOWER_AT_FARM_AMILY_GIBS_MILK] = 1;
            this.flags[kFLAGS.FARM_SUCCUMILK_STORED]++;
        }

        // If jojo is producing, stack up some drafts to gib
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_JOJO] == 1 && this.flags[kFLAGS.FOLLOWER_PRODUCTION_JOJO] == 1 && this.flags[kFLAGS.FARM_UPGRADES_REFINERY] == 1) {
            if (this.flags[kFLAGS.FOLLOWER_AT_FARM_JOJO_GIBS_DRAFT] == 0) this.flags[kFLAGS.FOLLOWER_AT_FARM_JOJO_GIBS_DRAFT] = 1;
            this.flags[kFLAGS.FARM_INCUDRAFT_STORED]++;
            if (kGAMECLASS.jojoScene.tentacleJojo()) this.flags[kFLAGS.FARM_INCUDRAFT_STORED]++;
        }

        // If Sophie is producing, count up time since last egg and gib a new one when ready
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] > 0 && this.flags[kFLAGS.FOLLOWER_PRODUCTION_SOPHIE] == 1) {
            if (this.flags[kFLAGS.FARM_EGG_STORED] == 0) {
                this.flags[kFLAGS.FARM_EGG_COUNTDOWN]--;

                if (this.flags[kFLAGS.FARM_EGG_COUNTDOWN] == 0) {
                    this.flags[kFLAGS.FARM_EGG_STORED] = 1;
        ...
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 1 && this.flags[kFLAGS.FOLLOWER_PRODUCTION_VAPULA] == 1) {
            this.flags[kFLAGS.FARM_SUCCUMILK_STORED]++;
            if (this.flags[kFLAGS.FARM_UPGRADES_REFINERY] == 1) this.flags[kFLAGS.FARM_SUCCUMILK_STORED]++;

            if (this.flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA_GIBS_MILK] == 0) this.flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA_GIBS_MILK] = 1;
        }

        // Item caps
        if (this.flags[kFLAGS.FARM_SUCCUMILK_STORED] > 5) {
            this.flags[kFLAGS.FARM_CORRUPTION_GEMS_WAITING] += (this.flags[kFLAGS.FARM_SUCCUMILK_STORED] - 5) * 3; // 3 extra gems per succumilk
            this.flags[kFLAGS.FARM_SUCCUMILK_STORED] = 5;
        }
        if (this.flags[kFLAGS.FARM_INCUDRAFT_STORED] > 5) {
            this.flags[kFLAGS.FARM_CORRUPTION_GEMS_WAITING] += (this.flags[kFLAGS.FARM_INCUDRAFT_STORED] - 5) * 5; // 5 extra gems per incudraft (less of them produced etc)
            this.flags[kFLAGS.FARM_INCUDRAFT_STORED] = 5;
        }
        ...
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_CERAPH] > 0) {
            this.flags[kFLAGS.FOLLOWER_AT_FARM_CERAPH]++;

            if (this.flags[kFLAGS.FOLLOWER_AT_FARM_CERAPH] > 7) this.flags[kFLAGS.FOLLOWER_AT_FARM_CERAPH] = -1;
        }

        // If Holli is doing her thing
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_HOLLI] > 0) {
            this.flags[kFLAGS.FOLLOWER_AT_FARM_HOLLI]++;

            if (this.flags[kFLAGS.FOLLOWER_AT_FARM_HOLLI] > 20) this.flags[kFLAGS.FOLLOWER_AT_FARM_HOLLI] = -1;
        }

        // Contraceptives
        if (this.flags[kFLAGS.FARM_UPGRADES_CONTRACEPTIVE] == 1) {
            if (this.flags[kFLAGS.FARM_CONTRACEPTIVE_STORED] == 0) this.flags[kFLAGS.FARM_CONTRACEPTIVE_STORED] = 1;
        }

        // Increment days since last paid out
        this.flags[kFLAGS.FARM_CORRUPTION_DAYS_SINCE_LAST_PAYOUT] += 1;
    ...
    public collectTheGoodies(): void {
        ...
        // Get gems
        if (this.flags[kFLAGS.FARM_CORRUPTION_DAYS_SINCE_LAST_PAYOUT] >= 7) {
            this.outputText("You stroll over to the big rock at the edge of the pepper patch, smiling as you see a small burlap sack shoved underneath a fold in the stone. You scoop it up, open it and count out " + this.flags[kFLAGS.FARM_CORRUPTION_GEMS_WAITING] + " gems.\n\n");

            if (this.farmValue() < 25) this.outputText(" You frown; it seems like a feeble amount for such a large operation. Perhaps you could talk to Whitney about that.\n\n");

            this.player.gems += this.flags[kFLAGS.FARM_CORRUPTION_GEMS_WAITING];
            this.flags[kFLAGS.FARM_CORRUPTION_GEMS_WAITING] = 0;
            this.flags[kFLAGS.FARM_CORRUPTION_DAYS_SINCE_LAST_PAYOUT] = 0;
            kGAMECLASS.showStats();
        }

        if (this.flags[kFLAGS.FARM_SUCCUMILK_STORED] > 0 || this.flags[kFLAGS.FARM_INCUDRAFT_STORED] > 0 || this.flags[kFLAGS.FARM_EGG_STORED] > 0 || this.flags[kFLAGS.FARM_CONTRACEPTIVE_STORED] > 0) {
        ...
        if (this.flags[kFLAGS.FARM_SUCCUMILK_STORED] > 0) {
            this.outputText("<b>" + this.flags[kFLAGS.FARM_SUCCUMILK_STORED] + "x ");
        ...
        if (this.flags[kFLAGS.FARM_EGG_STORED] > 0) {
            ...
            this.outputText("large " + this.flags[kFLAGS.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE] + " egg");
    ...
    public farmProtection(): number {
        ...
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_JOJO] == 1) protection += 2;

        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 1) {
        ...
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 1) protection += 2;

        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_KELLY] == 1) {
            if (this.flags[kFLAGS.KELT_BREAK_LEVEL] >= 4) protection += 1;
            else protection += 2;
        }

        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_MARBLE] == 1) protection += 2;

        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL] == 1) {
            if (this.flags[kFLAGS.MILK_SIZE] > 0) protection += 1;
        }

        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_CERAPH] > 0) protection += 7;
    ...
    public farmValue(): number {
        ...
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 1) fValue += 1;

        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 1) fValue += 3;

        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 1) fValue += 1;

        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_LATEXY] == 1) fValue += 2;

        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_KELLY] == 1) {
            if (this.flags[kFLAGS.KELT_BREAK_LEVEL] >= 4) fValue += 1;
        }

        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_MARBLE] == 1) fValue += 2;

        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL] == 1) {
            if (this.flags[kFLAGS.MILK_SIZE] > 0) fValue += 3;
            else fValue += 7;
        }

        if (this.flags[kFLAGS.WHITNEY_CORRUPTION_COMPLETE] == 1) fValue *= 1.25;
        return fValue;
    }

    public takeoverPrompt(): boolean {
        if (this.flags[kFLAGS.FARM_CORRUPTION_DISABLED] == 1) return false;
        if (this.flags[kFLAGS.FARM_CORRUPTION_STARTED] == 1) {
            this.farmMenu();
            return true; // Hook the corrupt menu here
        }

        if (this.player.cor >= 70 && this.corruptFollowers() >= 2 && this.player.level >= 12) {
            if (this.flags[kFLAGS.FARM_CORRUPT_PROMPT_DISPLAY] == 0) {
                this.clearOutput();
                if (this.flags[kFLAGS.KELT_BREAK_LEVEL] >= 4) {
                    this.takeoverPromptKelly();
                }
                else if (this.player.findStatusAffect(StatusAffects.MarbleRapeAttempted) >= 0) {
            ...
            else if (this.flags[kFLAGS.FARM_CORRUPT_PROMPT_DISPLAY] == 3 || this.flags[kFLAGS.FARM_DISABLED] == 1) {
                this.clearOutput();
                this.takeoverPromptMerge();
                return true;
            }
            else {
                this.flags[kFLAGS.FARM_CORRUPT_PROMPT_DISPLAY]++;
                return false;
            }
        }
    ...
    public takeoverPromptNow(): void {
        ...
        if (this.player.weaponName != "fists") this.outputText(", and to demonstrate your peacefulness, you theatrically hold up your [weapon] and then discard it with a careless swing of your arm.");
        else this.outputText(".");

        // if PC has Kelly or PC raped Marble
        if (this.flags[kFLAGS.KELT_BREAK_LEVEL] >= 4 || this.player.findStatusAffect(StatusAffects.MarbleRapeAttempted) >= 0) {
        ...
        if (this.flags[kFLAGS.KELT_BREAK_LEVEL] >= 4 || this.player.findStatusAffect(StatusAffects.MarbleRapeAttempted) >= 0) {
            ...
            if (this.flags[kFLAGS.KELT_BREAK_LEVEL] >= 4) this.outputText(" Kelt");
            else this.outputText(" Marble");
            ...
            if (this.player.findStatusAffect(StatusAffects.MarbleRapeAttempted) < 0 && this.player.findStatusAffect(StatusAffects.NoMoreMarble) < 0) {
    ...
    protected takeoverPromptNever(): void {
        this.clearOutput();
        this.flags[kFLAGS.FARM_CORRUPTION_DISABLED] = 1;
    ...
    public rootScene(): void {
        ...
        if (this.flags[kFLAGS.WHITNEY_DISABLED_FOR_DAY] == 2) {
            this.flags[kFLAGS.WHITNEY_DISABLED_FOR_DAY] = 3;
        ...
        if (!this.whitneyCorrupt()) {
            ...
            if (this.flags[kFLAGS.FARM_UPGRADES_REFINERY] == 1) this.outputText(" A large machine, bulky and rotund with a conical top, has been built into the milking barn. Fat pipes crawl up onto the roof from it like metal ivy, and white smoke billows busily out of its whistle chimney.");

            if (this.flags[kFLAGS.FARM_UPGRADES_CONTRACEPTIVE] == 1) this.outputText(" Next to the pepper patch another crop has been planted, a field of verdant green shrubs. Their thin stems bob idly in the breeze.");

            if (this.flags[kFLAGS.QUEUE_ORGYROOM_UPGRADE] > 1) {
    ...
    public collectionAvailable(): boolean {
        if (this.flags[kFLAGS.FARM_CORRUPTION_DAYS_SINCE_LAST_PAYOUT] >= 7) return true;
        if (this.flags[kFLAGS.FARM_SUCCUMILK_STORED] > 0) return true;
        if (this.flags[kFLAGS.FARM_INCUDRAFT_STORED] > 0) return true;
        if (this.flags[kFLAGS.FARM_EGG_STORED] > 0) return true;
        if (this.flags[kFLAGS.FARM_CONTRACEPTIVE_STORED] > 0) return true;
        return false;
    }
    ...
    private keltAChangeInManagement(): void {
        ...
        if (this.player.balls == 0) this.outputText(" So to speak, anyway.");
    ...
    private numSlavesAtFarm(): number {
        ...
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 1 && kGAMECLASS.sophieBimbo.bimboSophie()) count++;
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 1) count++;
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_LATEXY] == 1) count++;
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL] == 1) count++;

        return count;
    }

    private slavesAtFarm(): boolean {
        if (this.numSlavesAtFarm() > 0) return true;
        return false;
    }
    ...
    private followersAtFarm(): boolean {
        if (this.numFollowersAtFarm() > 0) return true;
        return false;
    }
    ...
    private loversAtFarm(): boolean {
        if (this.numLoversAtFarm() > 0) return true;
        return false;
    }
    ...
    private dogeNotCorruptYet(): void {
        ...
        this.flags[kFLAGS.FARM_CORRUPTION_APPROACHED_WHITNEY] = 1;

        // Farm Corruption drops below 30 after being higher: 
        if (this.whitneyCorruption() < 30 && this.flags[kFLAGS.WHITNEY_CORRUPTION_HIGHEST] > 30 && this.flags[kFLAGS.WHITNEY_CORRUPTION_0_30_DROP_MESSAGE] == 0) {
        ...
        if (this.whitneyCorruption() >= 31 && this.whitneyCorruption() <= 60) {
            if (this.flags[kFLAGS.WHITNEY_MENU_31_60] == 0) {
                this.flags[kFLAGS.WHITNEY_MENU_31_60] = 1;
        ...
        if (this.whitneyCorruption() >= 61 && this.whitneyCorruption() <= 90) {
            if (this.flags[kFLAGS.WHITNEY_MENU_61_90] == 0) {
                this.flags[kFLAGS.WHITNEY_MENU_61_90] = 1;
        ...
        if (this.whitneyCorruption() >= 91 && this.whitneyCorruption() <= 119) {
            ...
            if (this.flags[kFLAGS.WHITNEY_MENU_91_119] == 0) {
                this.flags[kFLAGS.WHITNEY_MENU_91_119] = 1;
        ...
        if (this.whitneyCorruption() == 120) {
            if (this.flags[kFLAGS.WHITNEY_CORRUPTION_COMPLETE] == 0) {
                this.flags[kFLAGS.WHITNEY_CORRUPTION_COMPLETE] = 1;
    ...
    private dogeNotCorruptLeaveFirstTime(): void {
        ...
        this.flags[kFLAGS.WHITNEY_LEAVE_0_60] = 1;
    ...
    private dogeNotCorruptLeave6190(): void {
        ...
        this.flags[kFLAGS.WHITNEY_LEAVE_61_90] = 1;
    ...
    public availableInvestments(): boolean {
        if (this.player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") < 0 && this.flags[kFLAGS.QUEUE_BREASTMILKER_UPGRADE] == 0) return true;
        if (this.player.hasKeyItem("Cock Milker - Installed At Whitney's Farm") < 0 && this.flags[kFLAGS.QUEUE_COCKMILKER_UPGRADE] == 0) return true;
        if (this.flags[kFLAGS.FARM_UPGRADES_REFINERY] == 0 && this.flags[kFLAGS.QUEUE_REFINERY_UPGRADE] == 0) return true;
        if (this.flags[kFLAGS.FARM_UPGRADES_CONTRACEPTIVE] == 0 && this.flags[kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE] == 0) return true;
        if (this.flags[kFLAGS.FARM_UPGRADES_MILKTANK] == 0 && kGAMECLASS.milkWaifu.milkSlave() && this.flags[kFLAGS.QUEUE_MILKTANK_UPGRADE] == 0) return true;
        return false;
    }
    ...
    private whitneyAppearanceCorrupt(): void {
        ...
        if (this.whitneyDefurred()) this.outputText("-girl. Her human transformation has rendered her pretty in a delicate, diffident kind of way.");
        else this.outputText("-morph. Her muzzle is suggestive of a golden retriever but really she could be any breed.");
        if (this.whitneyMaxedOralTraining()) this.outputText(" In sharp contrast to her otherwise fairly thin features, her lips are plump and a depthless black, shining like wet tar. She constantly moves her tongue over them unconsciously.");
        ...
        this.outputText("\n");
        if (!this.whitneyDefurred()) this.outputText("Her fur is sandy, dusking to black at her extremities");
        else this.outputText("Her skin is a sandy color, and she wears black nail varnish");
    ...
    private investmentMilktank(): void {
        ...
        if (!this.whitneyCorrupt()) {
            // If Bathgirl normal:
            if (this.flags[kFLAGS.MILK_SIZE] > 0 && this.flags[kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL] == 1) {
    ...
    private doMilktankInvestment(): void {
        ...
        if (!this.whitneyCorrupt()) this.outputText("You press the gems into your taskmaster’s hand, turn and leave without another word. Your thoughts turn to huge, delightfully plush brown boobs and luxurious milky baths; it will be well worth it. ");
        else this.outputText("You press the gems into your taskmaster’s hand, turn and leave without another word. Your thoughts turn to huge, delightfully plush, brown boobs, of luxurious milky baths; it will be well worth the cost. ");
    ...
    private dontDeFurDoge(): void {
        ...
        if (this.player.hasCock()) this.outputText(" crotch into");
        else this.outputText(" hips against");
    ...
    private dogeCorruptedMissionComplete(output: boolean = true): void {
        ...
        if (this.flags[kFLAGS.FARM_UPGRADES_ORGYROOM] == 2 && output) {
            this.orgyRoomTalk();
            this.flags[kFLAGS.FARM_UPGRADES_ORGYROOM] = 1;
            return;
        }
    ...
    private cockOralTraining() {
        if (this.flags[kFLAGS.WHITNEY_ORAL_TRAINING] == 0) {
            return this.firstCockOralTraining;
        }
        else if (this.flags[kFLAGS.WHITNEY_ORAL_TRAINING] <= 3) {
            return this.cockOralTrainingStageOne;
        }
        else if (this.flags[kFLAGS.WHITNEY_ORAL_TRAINING] == 4) {
            return this.firstCockOralTrainingStageTwo;
        }
        else if (this.flags[kFLAGS.WHITNEY_ORAL_TRAINING] <= 9) {
            return this.cockOralTrainingStageTwo;
        }
        else if (this.flags[kFLAGS.WHITNEY_ORAL_TRAINING] == 10) {
            return this.firstCockOralTrainingStageThree;
        }
        else if (this.flags[kFLAGS.WHITNEY_ORAL_TRAINING] == 11) {
    ...
    private firstOralTraining(): void {
        ...
        if (this.silly()) this.outputText(" “<i>Do. Or do not. There is no try.</i>”");
        else this.outputText(" “<i>You will.</i>”");
    ...
    private cockOralTrainingStageOne(): void {
        ...
        if (this.player.biggestCockLength() > 12) this.outputText(" She still struggles a bit with her gag reflex but it only takes her a few tries to swallow your thick prick, allowing you to pump into the delightful tightness of her throat without issue.");
    ...
    private firstCockOralTrainingStageTwo(): void {
        ...
        if (this.player.hasVagina() && this.player.balls > 0) {
            ...
            if (this.player.cumQ() >= 1500) this.outputText(" which is no mean feat, given it feels like you’ve fucked what feels like a gallon of it down her gullet. Once you’re almost done, as ever you spurt the last few creamy ropes across her face and clothes");
            this.outputText(".");
        ...
        //Male w/ balls:
            ...
            if (this.player.cumQ() >= 1500) this.outputText(" which is no mean feat, given it feels like you’ve fucked what feels like a gallon of it down her gullet. Once you’re almost done, as ever you spurt the last few creamy ropes across her face and clothes");
            this.outputText(".");
            if (this.player.cocks.length > 1) this.outputText(" The second cock trapped in her grasp flexes out cum just as eagerly as the first, sweat standing out on your forehead as you are clenched by your twin high; it paints her skirt with its generous discharge.");
        }
        //Male without balls:
        else if (!this.player.hasVagina() && this.player.balls == 0) {
            //multicock
        ...
        if (!this.player.hasVagina()) {
            ...
            if (this.player.cocks.length > 1) this.outputText(" and [cock biggest2]");
    ...
    private cockOralTrainingStageTwo(): void {
        ...
        if (!this.player.hasVagina()) {
            ...
            if (this.player.cocks.length > 1) this.outputText("She wraps her hand around your [cock biggest2] when she’s ready, giving it an experimental pump before beginning to softly and slowly work it, remembering exactly how to grip and bend its girth as she prepares for the main course.");
            ...
            if (this.player.cocks.length > 1) this.outputText("she occasionally forgets the length bulging in her hand so eager is she to push her lips further down your [cock biggest] and work at her own clit, and you need to murmur to get her to concentrate.");
            else this.outputText("she still occasionally forgets the length buried in her mouth, so eager is she to work keenly at her own clit, and you need to murmur to get her to concentrate.");
            ...
            if (this.player.cocks.length == 1) this.outputText("Whitney cleans your [cock biggest] with her flat tongue, humming as she laps your oozing head clean, deliciously soothing your aching cock.");
            else this.outputText("Whitney cleans first your [cock biggest2] and then your [cock biggest] with her flat tongue, humming as she laps each of your oozing heads clean, deliciously soothing your aching cock.");
    ...
    private firstCockOralTrainingStageThree(): void {
        ...
        if (!this.player.hasVagina()) {
            ...
            if (this.player.cocks.length > 2) {
                ...
                if (this.player.cocks.length > 1) this.outputText(" jerking both your cocks with her smooth, exacting grasp in tandem");
                this.outputText(".");
            }
            ...
            if (this.player.cocks.length > 1) {
                ...
                if (this.player.cocks.length > 2) this.outputText(" and [cock biggest3]");
                this.outputText(" bulge and vibrate with pleasure in her pumping hand");
                if (this.player.cocks.length > 2) this.outputText("s");
            }
            ...
            if (this.player.cocks.length > 1 || this.player.cumQ() >= 1500) this.outputText("\n\n");
            if (this.player.cocks.length > 1) {
                this.outputText("She doesn’t forget the cock");
                if (this.player.cocks.length > 2) this.outputText("s");
                this.outputText(" trapped in her grasp for a moment; she milks");
                if (this.player.cocks.length > 2) this.outputText(" them");
                else this.outputText(" it");
                this.outputText(" as hard as you’re fucking her mouth, and");
                if (this.player.cocks.length > 2) this.outputText(" they flex");
                else this.outputText(" it flexes");
                this.outputText(" out cum just as eagerly as the first, delirium seizing you as you are clenched by your");
                if (this.player.cocks.length == 2) this.outputText(" twin");
                else if (this.player.cocks.length == 3) this.outputText(" triple");
                else this.outputText(" transcendent");
                this.outputText(" high;")
                if (this.player.cocks.length > 2) this.outputText(" they paint");
                else this.outputText(" it paints");
                this.outputText(" her skirt with")
                if (this.player.cocks.length > 2) this.outputText(" their");
        ...
        //Herm
            ...
            if (this.player.biggestCockLength() < 8) this.outputText("the back of her mouth");
            else this.outputText("her gullet");
            ...
            if (this.player.wetness() > 2) this.outputText("soaking her arm as it gutters your juices, ");
            this.outputText("heightening the ecstasy you feel to mind-blowing heights. ");
            if (this.player.cocks.length > 1) this.outputText(" She doesn’t forget the cock trapped in her grasp for a moment either; she milks it as hard as you’re fucking her mouth, and it flexes out cum just as eagerly as the first, delirium seizing you as you are clenched by your triple high as it paints her skirt with its generous discharge.");
            if (this.player.cumQ() >= 1500) this.outputText(" Once you’re almost done, as ever you pull out and spurt the last few creamy ropes across her face and clothes. Red with her own huge high, the dog woman does her level best to catch them in her mouth.");

            this.outputText("\n\nOnce you have tensed your last you sigh beatifically and float high and formless on your post-blowjob haze. You close your eyes as, without any bidding, ");
            if (this.player.cocks.length == 1) this.outputText("Whitney first cleans your [vagina] and then your [cock biggest] with her flat tongue, polishing your dripping pussy and thighs to a shine before lapping your oozing head clean, deliciously soothing your aching cock.");
            else this.outputText("Whitney uses her flat tongue to clean first your [vagina], polishing your dripping pussy and thighs to a shine, and then each of your pricks, lapping the oozing heads clean, deliciously soothing your aching cocks.");
    ...
    private cockOralTrainingStageThree(): void {
        ...
        if (this.player.cocks.length >= 2) this.outputText(", allowing your other cock");
        if (this.player.cocks.length > 2) this.outputText("s");
        if (this.player.cocks.length >= 2) this.outputText(" to unfurl and bob behind it");
        if (this.player.hasVagina() && this.player.cocks.length >= 2) this.outputText(" and");
        if (this.player.hasVagina()) this.outputText(", leisurely exposing your moist [vagina]");
        if (this.player.cocks.length >= 2 && this.player.hasVagina()) this.outputText(" at the same time");
        ...
        if (!this.player.hasVagina()) {
            ...
            if (this.player.cocks.length > 2) this.outputText("Finally, she snakes her other hand around your [cock biggest3], rolling her wrist as she gently wrings it with the same sliding thoroughness.} You open your mouth to the shifting delectation inundating your overgrowth of cocks.");
            ...
            if (this.player.balls > 0) this.outputText(", your saliva-glossed [balls] beating against her chin");
            ...
            if (this.player.cocks.length > 1) {
                ...
                if (this.player.cocks.length > 2) this.outputText("s");
                this.outputText(" trapped in her grasp for a moment; she milks");
                if (this.player.cocks.length == 2) this.outputText(" it");
                else this.outputText(" them");
                this.outputText(" as hard as you’re fucking her mouth, and");
                if (this.player.cocks.length == 2) this.outputText(" it flexes");
                else this.outputText(" they flex");
                this.outputText(" out cum just as eagerly as the first, delirium seizing you as you are clenched by your");
                if (this.player.cocks.length == 2) this.outputText(" twin");
                else if (this.player.cocks.length == 3) this.outputText(" triple");
                else this.outputText(" transcendent");
                this.outputText(" high;");
                if (this.player.cocks.length == 2) this.outputText(" it paints");
                else this.outputText(" they paint");
                this.outputText(" her skirt with ");
                if (this.player.cocks.length == 2) this.outputText(" its");
            ...
            if (this.player.cocks.length > 1) this.outputText(" all of");
        ...
        //Herm 
            ...
            if (this.player.cocks.length > 1) this.outputText("As she bathes your [cock biggest] in delicious sensation she wraps her hand around your [cock biggest2], softening and then tightening her grip, sliding up and down its stem until it is every bit as rock-hard as the one she is lapping at. ");
            ...
            if (this.player.biggestCockLength() < 8) this.outputText("mouth");
            ...
            if (this.player.cocks.length > 1) {
                ...
                if (this.player.cocks.length == 2) this.outputText("triple");
                else this.outputText("transcendent");
    ...
    private cockOralTrainingMaxed(): void {
        ...
        if (!this.player.hasVagina()) {
            ...
            if (this.player.balls > 0) this.outputText("You sigh blissfully as her plump lips push into your [balls] before the flat, warm flannel-like tongue spreads itself across them, covering every inch, each sensitive orb first sensuously kissed and then taken into your slave’s mouth to be suckled gently until they are thrumming and bulging with packed heat. The loving attention laved on them is made all the better by the soft, lustful moans turning into outright panting sobs of pleasure drifting up from between your legs as she obsequiously polishes them.");

            this.outputText("\n\nShe rises until her head is level with your semi-erect prick and with long, careful, exquisite licks makes it fiercely erect, running her tongue along its full length and then swirling it around your head, pressing into the sensitive spots she knows will make you dense and tight with need. She grips it steadily at the base as she does so, every so often giving it a tight squeeze to contrast with a particularly soft lap at your bulging head.");
            if (this.player.cocks.length > 1) this.outputText(" As she bathes your [cock biggest] in delicious sensation she wraps her hand around your [cock biggest2], softening and then tightening her grip, sliding up and down its stem until it is every bit as rock-hard as the one she is lapping at.");
            if (this.player.cocks.length > 2) this.outputText(" Finally, she snakes her other hand around your [cock biggest3], rolling her wrist as she gently wrings it with the same sliding thoroughness.");
            ...
            if (this.player.cocks.length > 1) this.outputText("In the long, delicious daze that follows you move her around");
            if (this.player.cocks.length == 2) this.outputText(", switching her attention to your [cock biggest2] for a while");
            else if (this.player.cocks.length >= 3) this.outputText(" switching her attention first to one cock and then the other");
            if (this.player.cocks.length > 1) this.outputText(" so that each of your fiercely erect pricks has time to enjoy the tight, sucking embrace of her mouth.");
            ...
            if (this.player.balls > 0) this.outputText(" and her tongue once again sliding across your [balls], ");
            ...
            if (this.player.cocks.length > 1) this.outputText(", her hands wrapping back around your other cock");
            if (this.player.cocks.length > 2) this.outputText("s");
            if (this.player.cocks.length > 1) this.outputText(" as if they never left them");
        ...
        //Herm 
            ...
            if (this.player.cocks.length > 1) this.outputText(" In the long, delicious daze that follows you move her around");
            if (this.player.cocks.length == 2) this.outputText(", switching her attention to your [cock biggest2] for a while");
            if (this.player.cocks.length >= 3) this.outputText(", switching her first to one cock then another");
            if (this.player.cocks.length > 1) this.outputText(" so that each of your fiercely erect pricks has time to enjoy the tight, sucking embrace of her mouth.");
            ...
            if (this.player.cocks.length > 1) this.outputText(" and her other hand wrapping back around your [cock biggest2]");
            ...
            if (this.player.wetness() > 2) this.outputText("soaking her arm as it gutters your juices, ");
            this.outputText("heightening the ecstasy you feel to mind-blowing heights.");
            if (this.player.cocks.length > 1) this.outputText(" She doesn’t forget the [cock biggest2] trapped in her grasp for a moment; she milks it as hard as you’re fucking her mouth, and it flexes out cum just as eagerly as the first, delirium seizing you as you are clenched by your triple high; it paints her skirt with its generous discharge.");
    ...
    private firstVaginaOralTraining(): void {
        ...
        if (this.player.clitLength > 3) this.outputText(" despite the fact it must be practically poking her in the eye");
    ...
    private vaginaOralTrainingStageOne(): void {
        ...
        else this.outputText(" The dog woman coughs and splutters around the lubricant you dribble into her, some of it dripping out of her nose even as you continue to push yourself into her. This doesn’t stop her from furiously moving her hand in her knickers, flicking her clit for all she is worth as she attempts to cope with the pressing demand of your snatch. On the edge already it doesn’t take her long, and her eyes roll as she tenses and shudders.");
    ...
    private firstVaginaOralTrainingStageTwo(): void {
        ...
        if (this.player.lactationQ() > 0) this.outputText(" dripping slightly");
        else this.outputText(" semi-erect");
        ...
        if (this.player.hasFuckableNipples()) this.outputText(" obscene folds");
        else this.outputText(" sensitive points");
        ...
        if (this.player.hasFuckableNipples()) this.outputText(" sends her tongue questing into its sensitive inside.");
        else this.outputText("bites it ever so gently.");
        ...
        if (this.player.wetness() <= 2) this.outputText(" wetting both your slave’s hand and mouth");
        else this.outputText(" absolutely soaking your slave’s face and arm");
        ...
        if (this.player.wetness() >= 5) this.outputText(" which is no mean feat, given it feels like you gutter what feels like a gallon down her gullet. Once you’re almost done, as ever you get up and spurt the last of it on her hair and clothes");
        this.outputText(".");
    ...
    private vaginaOralTrainingStageTwo(): void {
        ...
        if (this.player.lactationQ() > 0) this.outputText(" and it quickly makes the liquid warmth in your breasts rise to a point, beading milk freely from your teats. Whitney laps and sucks it down eagerly, but gratifyingly she remembers your order and stops reluctantly after only taking a single pull");
        this.outputText(", leaving your nipples");
        if (!this.player.hasFuckableNipples()) this.outputText(" rock-hard and");
        ...
        if (this.player.wetness() >= 5) this.outputText(", fountaining fem juice down her throat");
        ...
        if (this.player.wetness() >= 5) this.outputText(", soaking her again and again with warm, female approval");
        this.outputText(".");
    ...
    private firstVaginaOralTrainingStageThree(): void {
        ...
        if (!this.player.hasFuckableNipples()) this.outputText(" she takes each of your [nipples] into her mouth to bathe them in close, sucking attention");
        else this.outputText(" she sends her tongue lapping into each of your cunt nipples, curling at their sensitive pink until they are wet with need");
        ...
        if (this.player.lactationQ() > 0) this.outputText(" It quickly makes the liquid warmth in your breasts rise to a point, beading milk freely from your teats. Whitney laps and sucks it down eagerly, but gratifyingly she remembers your order and stops reluctantly after only taking a single pull.");
        ...
        if (this.player.clitLength >= 4) this.outputText(" She uses her new anchorage to take your bulging fem-dick entirely into her mouth, sucking on it avidly; she hums as she does it, sending ecstatic pleasure thrumming through you.");
        ...
        if (this.player.wetness() >= 5) this.outputText(", soaking her mouth and face again and again with warm, female approval");
        ...
        if (this.player.wetness() >= 5) this.outputText(" It is in a state of ecstatic momentum that you rise away from her mouth and spurt the last of it onto her hair and clothes. Red with her own huge high, the dog woman does her level best to catch your spotting fluids in her mouth.");
    ...
    private vaginaOralTrainingStageThree(): void {
        ...
        if (this.player.lactationQ() > 0) this.outputText(" It quickly makes the liquid warmth in your breasts rise to a point, beading milk freely from your teats. Whitney laps and sucks it down eagerly, whimpering as she withdraws after taking a single pull. You smile softly, knowing that the small, teasing taste of your sweetness only serves to make her thirstier.");
        this.outputText(" She sinks downwards leaving your nipples");
        if (this.player.hasFuckableNipples()) this.outputText(" rock-hard and");
        this.outputText(" shining, your [vagina] beading eagerly.");
    ...
    private vaginaOralTrainingMaxed(): void {
        ...
        if (!this.player.hasFuckableNipples()) this.outputText(" she envelopes each of your [nipples] with her lips, kissing them with their sumptuous sponginess before bathing them in close, sucking attention");
        else this.outputText(" she pushes her lips against your cunt nipples before sending her tongue gently into each of them, curling at their sensitive pink until they are oozing wetness in delight");
        ...
        if (this.player.buttRating < 4) this.outputText("firm butt");
        if (this.player.buttRating < 8) this.outputText("supple rump");
        else this.outputText("soft ass");
    ...
    private whitneyDomPleasure(): void {
        ...
        if (this.flags[kFLAGS.WHITNEY_DOM_FIRST_PLEASURE] == 0) {
            this.firstWhitneyDomPleasure();
            this.flags[kFLAGS.WHITNEY_DOM_FIRST_PLEASURE] = 1;
        }
        else {
    ...
    private whitneyDomBondageOral(): void {
        ...
        if (this.player.hasCock() && !this.player.hasVagina()) {
            ...
            if (this.player.totalCocks() > 1) this.outputText(" “<i>So many things to play with tucked away in here!</i>” comes her merry voice again, as you feel her grasp your [cock smallest] at the same time; you groan into the confines of her flesh as she gives it a soft, teasing pump. It’s obvious your main member has really caught her attention though, and both her hands soon return to it.");
        ...
        if (this.player.hasCock()) this.outputText("[cock biggest]");
        if (this.player.hasCock() && this.player.hasVagina()) this.outputText(" and ");
        if (this.player.hasVagina()) this.outputText("[vagina]");
    ...
    private whitneyDomStraponDoggy(): void {
        ...
        if (this.player.isBiped() || this.player.isTaur()) this.outputText(" Next, she pushes your ankles apart, threads a leather strap around each and then pulls them tight with a sharp yank; you carefully test them and find with a cold thrill that there is a metal bar attached between them, forcing you to stay in this position.");
        else if (this.player.isDrider()) this.outputText(" You don’t struggle as she then forces your spindly legs beneath the bed, looping a leather strap attached to the frame around them all, trapping them there.");
        else if (this.player.isNaga()) this.outputText(" She then ties your reptilian coils around the bedposts, securing them with leather straps fitted for the purpose.");
        ...
        if (this.player.hasVagina() && !this.player.hasCock() && kGAMECLASS.vapula.vapulaSlave()) {
        ...
        // wet and/or loose
        var capacity: number = (this.player.hasVagina()) ? this.player.vaginalCapacity() : this.player.analCapacity();
        if (capacity >= 36) {
            this.outputText("\n\nThe dog woman huffs in surprise as she slides her prosthetic length in with ease, your ");
            if (this.player.hasVagina()) this.outputText("deep, eager vagina");
            else this.outputText("oily, well-used passage");
            ...
            if (this.player.hasVagina()) this.outputText("sit and spread ‘em for anything going, don’t you?");
            else this.outputText("I bet you get on your knees for anything with a nice, juicy cock, right?");
            ...
            if (this.player.hasVagina()) this.outputText("take of your tight cunt");
            else this.outputText("give of your tight ass");
            ...
            if (this.player.hasVagina()) this.outputText("servants, you tight, bossy little bitch");
            else this.outputText("cattle, you tight, bossy little boy-bitch");
            this.outputText(".</i>” ");
    ...
    public whitneyDomRide(): void {
        ...
        if (this.player.isBiped()) this.outputText(" She does the same with your ankles, leaving you helplessly spread-eagled across the bed.");
        else if (this.player.isDrider()) this.outputText(" She has long, leather straps attached to the underside of the bed; these are wrapped around the joining of your spindly legs to your heavy abdomen, leaving your chitinous feet pointing helplessly at the ceiling.");
        else if (this.player.isGoo()) this.outputText(" “<i>I can’t secure you properly,</i>” she breathes, squeezing your gooey form, sinking her fingers into it. “<i>Owing to this nonsense you call a body. So I will jus’ say it again. Lie. Still.</i>”");
        else if (this.player.isNaga()) this.outputText(" Your long, heavy tail is wrapped around the bedsteads at the foot of the bed and secured with more leather straps, leaving you helplessly spread-eagled across the bed.");
        ...
        this.outputText("\n\n“<i>Stop fussing now, sweetheart,</i>” Whitney croons, adjusting the black, elastic blindfold so it is firmly secured over your eyes." + ((this.player.eyeType == EYES_FOUR_SPIDER_EYES) ? " Once she’s finished with that she places another blindfold over your second pair of eyes. You can only groan with laughter at the sheer level of preparation the dog woman is capable of." : "") + " “<i>Wouldn’t want you to see what’s coming, would we? That’d ruin half the fun.</i>” ");
    ...
    private brandingMenu(): void {
        ...
        if (this.flags[kFLAGS.FARM_CORRUPTION_BRANDING_MENU_UNLOCKED] == 0) {
    ...
    private dontGetBrandingStuff(): void {
        ...
        if (this.whitneyDom()) this.outputText(" “<i>You’ll pay for that later, I swear!</i>”");
    ...
    private brandingAvailableTalk(): void {
        ...
        if (this.whitneyDefurred()) this.outputText(" fingernail");
        else this.outputText(" claw");
    ...
    private tribalTattoo(slot: number): void {
        ...
        if (slot == 0) {
            ...
            tText += "collarbone.";
            this.flags[kFLAGS.WHITNEY_TATTOO_COLLARBONE] = tText;
        }
        else if (slot == 1) {
            this.shouldersIntro();
            tText += "shoulders.";
            this.flags[kFLAGS.WHITNEY_TATTOO_SHOULDERS] = tText;
        }
        else if (slot == 2) {
            this.lowerbackIntro();
            tText += "lower back.";
            this.flags[kFLAGS.WHITNEY_TATTOO_LOWERBACK] = tText;
        }
        else if (slot == 3) {
            this.buttIntro();
            tText += "butt.";
            this.flags[kFLAGS.WHITNEY_TATTOO_BUTT] = tText;
        }
    ...
    private amilyTribalTattoo(slot: number): void {
        ...
        if (slot == 0) {
            ...
            tText += "collarbone.";
            this.flags[kFLAGS.AMILY_TATTOO_COLLARBONE] = tText;
        }
        else if (slot == 1) {
            this.amilyShouldersIntro();
            tText += "shoulders.";
            this.flags[kFLAGS.AMILY_TATTOO_SHOULDERS] = tText;
        }
        else if (slot == 2) {
            this.amilyLowerBackIntro();
            tText += "lower back.";
            this.flags[kFLAGS.AMILY_TATTOO_LOWERBACK] = tText;
        }
        else if (slot == 3) {
            this.amilyButtIntro();
            tText += "butt.";
    ...
    private jojoTribalTattoo(slot: number): void {
        ...
        if (slot == 0) {
            ...
            tText += "collarbone.";
            this.flags[kFLAGS.JOJO_TATTOO_COLLARBONE] = tText;
        }
        else if (slot == 1) {
            this.jojoShouldersIntro();
            tText += "shoulders.";
            this.flags[kFLAGS.JOJO_TATTOO_SHOULDERS] = tText;
        }
        else if (slot == 2) {
            this.jojoLowerbackIntro();
            tText += "lower back.";
            this.flags[kFLAGS.JOJO_TATTOO_LOWERBACK] = tText;
        }
        else if (slot == 3) {
            this.jojoButtIntro();
            tText += "butt.";
    ...
    private bimboSophieTribalTattoo(slot: number): void {
        ...
        if (slot == 0) {
            ...
            tText += "collarbone.";
            this.flags[kFLAGS.SOPHIE_TATTOO_COLLARBONE] = tText;
        }
        else if (slot == 1) {
            this.bimboSophieShouldersIntro();
            tText += "shoulders.";
            this.flags[kFLAGS.SOPHIE_TATTOO_SHOULDERS] = tText;
        }
        else if (slot == 2) {
    ...
    private vapulaTribalTattoo(slot: number): void {
        ...
        if (slot == 0) {
            ...
            tText += "collarbone.";
            this.flags[kFLAGS.VAPULA_TATTOO_COLLARBONE] = tText;
        }
        else if (slot == 1) {
            this.vapulaShouldersIntro();
            tText += "shoulders.";
            this.flags[kFLAGS.VAPULA_TATTOO_SHOULDERS] = tText;
        }
        else if (slot == 2) {
    ...
    private kellyTribalTattoo(slot: number): void {
        ...
        if (slot == 0) {
            ...
            tText += "collarbone.";
            this.flags[kFLAGS.KELLY_TATTOO_COLLARBONE] = tText;
        }
        else if (slot == 1) {
            ...
            tText += "lower back.";
            this.flags[kFLAGS.KELLY_TATTOO_LOWERBACK] = tText;
        }
        else if (slot == 3) {
            this.kellyButtIntro();
            tText += "butt.";
    ...
    private smallMilkyTribalTattoo(slot: number): void {
        ...
        if (slot == 0) {
            ...
            tText += "shoulders.";
            this.flags[kFLAGS.MILKY_TATTOO_SHOULDERS] = tText;
        }
        else if (slot == 2) {
            ...
            this.outputText("\n\n“<i>Yes, but what is it? [name]? Hello?</i>”");
            tText += "lower back.";
            this.flags[kFLAGS.MILKY_TATTOO_LOWERBACK] = tText;
        }
        else if (slot == 3) {
    ...
    private numWhitneyTribalTats(): number {
        ...
        {
            if (this.flags[kFLAGS.WHITNEY_TATTOO_COLLARBONE].indexOf("A tribal tattoo") >= 0) count++;
        }
        if (typeof this.flags[kFLAGS.WHITNEY_TATTOO_SHOULDERS] == 'string')
        {
            if (this.flags[kFLAGS.WHITNEY_TATTOO_SHOULDERS].indexOf("A tribal tattoo") >= 0) count++;
        }
        if (typeof this.flags[kFLAGS.WHITNEY_TATTOO_LOWERBACK] == 'string')
        {
            if (this.flags[kFLAGS.WHITNEY_TATTOO_LOWERBACK].indexOf("A tribal tattoo") >= 0) count++;
        }
    ...
    public whitneyFullTribalTats(): boolean {
        if (this.numWhitneyTribalTats() == 4) return true;
        return false;
    }

    public amilyFullTribalTats(): boolean {
        if (this.numAmilyTribalTats() == 4) return true;
        return false;
    }
    ...
    public jojoFullTribalTats(): boolean {
        if (this.numJojoTribalTats() == 4) return true;
        return false;
    }
    ...
    private numSophieTribalTats(): number {
        ...
        {
            if (this.flags[kFLAGS.SOPHIE_TATTOO_COLLARBONE].indexOf("A tribal tattoo") >= 0) count++;
        }
    ...
    public sophieFullTribalTats(): boolean {
        if (this.numSophieTribalTats() == 4) return true;
        return false;
    }
    ...
    private numVapulaTribalTats(): number {
        ...
        {
            if (this.flags[kFLAGS.VAPULA_TATTOO_COLLARBONE].indexOf("A tribal tattoo") >= 0) count++;
        }
    ...
    public vapulaFullTribalTats(): boolean {
        if (this.numVapulaTribalTats() == 4) return true;
        return false;
    }
    ...
    public kellyFullTribalTats(): boolean {
        if (this.numKellyTribalTats() == 4) return true;
        return false;
    }
    ...
    public milkyFullTribalTats(): boolean {
        if (this.numMilkyTribalTats() == 4) return true;
        return false;
    }
    ...
    private orgyRoomRouter(): void {
        ...
        if (this.flags[kFLAGS.FARM_UPGRADES_ORGYROOM] == 0 && this.flags[kFLAGS.QUEUE_ORGYROOM_UPGRADE] == 0 && !this.whitneyDom()) {
            doFunctor = this.wantOrgyRoom;
        }

        if (this.flags[kFLAGS.FARM_UPGRADES_ORGYROOM] == 1) {
            if (this.whitneyDom()) {
                // Nothing... yet. Hopefully.
            }
            else {
                if (!this.player.isGoo() && !this.player.isTaur() && (this.player.hasCock() || this.player.hasVagina())) doFunctor = this.orgyRoomSubMassage;
            }
        }
    ...
    private orgyRoomTalk(): void {
        ...
        if (this.whitneyDom()) this.outputText("The sight sends a shuddering thrill down your spine. “<i>You didn’t think I’d forget those, did you [master]? We both know how important they are for properly relaxing you,</i>” Whitney murmurs, pushing her hot suppleness into your side. ");
        else this.outputText("You raise an eyebrow at these and Whitney giggles and blushes. “<i>Just in case you’re feeling adventurous, [master]. Or, um, want help.</i>”");
        ...
        if (!this.whitneyDom()) this.outputText("“<i>It’s exactly what a harem owner needs, [master] - with this you could take care of me and your other sluts all night, every night!</i>”");
        else this.outputText("“<i>I could do so many things to you [master], if you were on this stuff. And for so long. Just imagine it!</i>”");
    ...
    private orgyRoomSubMassage(): void {
        ...
        if (this.player.isBiped()) this.outputText(" you clamber onto the heated slab, placing your chin on your hands on the cushion at the end");
        else if (this.player.isDrider()) this.outputText(" you tick-tack over and sink your spider underbelly onto the heated marble, before resting your human half down as well. Whitney was foresighted enough to make the slab quite long, and you complete the action in comfort");
        else if (this.player.isNaga()) this.outputText(" you slither over and slide onto the heated slab, placing your chin on your hands on the cushion at the end. You allow your long, heavy tail to pool onto the floor below");
        this.outputText(".");
        ...
        if (this.player.buttRating >= 6) this.outputText("soft");
        else this.outputText("brawny");
        ...
        if (this.player.hasCock()) this.outputText(" grips your [cock]");
        else if (this.player.hasVagina()) this.outputText(" traces the entrance of your [vagina]");
        ...
        if (this.player.isBiped()) {
            ...
            if (!(this.player.lowerBody == LOWER_BODY_TYPE_HOOFED || this.player.lowerBody == LOWER_BODY_TYPE_NAGA || this.player.lowerBody == LOWER_BODY_TYPE_GOO || this.player.lowerBody == LOWER_BODY_TYPE_PONY)) {
                ...
                if (this.player.hasCock()) this.outputText(" cock thicken");
                if (this.player.hasCock() && this.player.hasVagina()) this.outputText(" and your");
                if (this.player.hasVagina()) this.outputText(" pussy moisten");
                this.outputText(".");
                ...
                if (this.player.lowerBody == LOWER_BODY_TYPE_DEMONIC_CLAWS || this.player.lowerBody == LOWER_BODY_TYPE_LIZARD || this.player.lowerBody == LOWER_BODY_TYPE_HARPY || this.player.lowerBody == LOWER_BODY_TYPE_DRAGON) this.outputText(" claws");
                else this.outputText(" toes")
        ...
        // Naga: 
            ...
            if (this.player.tongueType == TONUGE_SNAKE) this.outputText(" You grin, flicking your forked tongue out to smell her arousal.");
        ...
        if (this.player.lust >= 33) this.outputText(" The hot tub bubbles quietly behind her and it is not just oil which is making your skin glow. This would be the moment to pursue a happy ending if you so wished.");
        else this.outputText(" You play with the idea of ‘rewarding’ her but... damn. She’s actually managed to relax you <i>too</i> much. You thank her regally, redress, and head out the door, enjoying the lightness of step and self-control which thrums through you.");
    ...
    private subHappyEndingMale(): void {
        ...
        if (this.player.cockArea(cockThatFits - 1) != this.player.biggestCockArea()) hasBiggerCock = true;
        ...
        if (hasBiggerCock) this.outputText(" [cock biggest]");
        else if (this.player.isBiped()) this.outputText(" against your [hips]");
        else this.outputText(" around your back");
        ...
        if (this.player.cumQ() >= 1500) this.outputText(" Her small capacity is almost immediately filled and jizz spurts out with every rich thrust you make, polluting the water around you with your massive load.");
        if (hasBiggerCock) this.outputText(" Whitney has stroked and tantalized your [cock biggest] with her hot hands and smooth belly intermittently this whole time, and as your orgasm builds and overwhelms you she gets her just reward for it; you jet cum from [eachCock], pasting first her face and then her tight breasts with your thick cream as you ride her hard.");
    ...
    private subHappyEndingFemale(): void {
        ...
        if (this.player.wetness() >= 5) this.outputText(" Your pussy spurts excitement gleefully with each thrust, utterly drowning her own in your juices, polluting the water around you with your musk.");
        if (this.player.hasCock()) this.outputText(" Whitney has stroked and tantalized your [cock biggest] with her hot hands and smooth belly intermittently this whole time, and as your orgasm builds and overwhelms you she gets her just reward for it; you jet cum from [eachCock], pasting first her face and then her tight breasts with your thick cream as you ride her hard.");
