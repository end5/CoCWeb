...
export class Boat extends AbstractLakeContent {
    ...
    public discoverBoat(): void {
        ...
        if (this.player.cor > 60) this.outputText(" or fuck", false);
    ...
    public boatExplore(): void {
        ...
        if (this.flags[kFLAGS.PC_PROMISED_HEL_MONOGAMY_FUCKS] == 1 && this.flags[kFLAGS.HEL_RAPED_TODAY] == 0 && Boat.rand(10) == 0 && this.player.gender > 0 && !kGAMECLASS.helScene.followerHel()) {
        ...
        if (this.player.findStatusAffect(StatusAffects.FactoryOverload) >= 0) {
            ...
            if (this.player.inte > 40) this.outputText("You realize what it smells like – sex.  ", false);
        }
        //3% chance of finding lost daughters
        if (Boat.rand(100) <= 3 && this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00412] > 0 && kGAMECLASS.izmaScene.izmaFollower()) {
        ...
        if (Boat.rand(10) <= 2 && this.player.findStatusAffect(StatusAffects.MaraeComplete) < 0 && this.player.findStatusAffect(StatusAffects.MetCorruptMarae) < 0) {
            this.marae.encounterMarae();
            return;
        }
        //10% chance of corrupt Marae followups
        if ((this.debug || Boat.rand(10) == 0) && this.flags[kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] == 0 && this.player.findStatusAffect(StatusAffects.MetCorruptMarae) >= 0 && this.player.gender > 0) {
            this.marae.level2MaraeEncounter();
            return;
        }
        //BUILD LIST OF CHOICES
        var choice: any[] = [0, 1, 2, 3];
        if (this.player.findStatusAffect(StatusAffects.DungeonShutDown) >= 0 && this.player.level > 2)
