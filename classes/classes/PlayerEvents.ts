...
export class PlayerEvents extends BaseContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (this.player.findPerk(PerkLib.StrongBack) >= 0 && !this.player.itemSlot4.unlocked) this.player.itemSlot4.unlocked = true;
        if (this.player.findPerk(PerkLib.StrongBack2) >= 0 && !this.player.itemSlot5.unlocked) this.player.itemSlot5.unlocked = true;
        ...
        if (this.player.findPerk(PerkLib.MaraesGiftProfractory) >= 0) this.player.hoursSinceCum += 2;
        ...
        if (this.player.findPerk(PerkLib.WellAdjusted) < 0) {
            ...
            if (this.player.findPerk(PerkLib.Lusty) >= 0) this.getGame().dynStats("lus", this.player.lib * 0.02, "resisted", false); //Double lust rise if lusty.
        }
        else { //Well adjusted perk
            this.getGame().dynStats("lus", this.player.lib * 0.02); //Raise lust
            if (this.player.findPerk(PerkLib.Lusty) >= 0) this.getGame().dynStats("lus", this.player.lib * 0.01, "resisted", false); //Double lust rise if lusty.
        }
        if (this.player.tailType == TAIL_TYPE_BEE_ABDOMEN || this.player.tailType == TAIL_TYPE_SPIDER_ADBOMEN) { //Spider and Bee Sting Recharge
        ...
        if (this.player.tailType == TAIL_TYPE_CAT && this.player.lowerBody == LOWER_BODY_TYPE_CAT && this.player.earType == EARS_CAT) { //Check for gain of cat agility - requires legs, tail, and ears
        ...
        else if (this.player.findPerk(PerkLib.Flexibility) >= 0) { //Remove flexibility perk if not meeting requirements
        ...
        if (this.flags[kFLAGS.FOX_BAD_END_WARNING] == 1) {
            if (this.player.faceType != FACE_FOX || this.player.tailType != TAIL_TYPE_FOX || this.player.earType != EARS_FOX || this.player.lowerBody != LOWER_BODY_TYPE_FOX || this.player.skinType != SKIN_TYPE_FUR) {
                this.flags[kFLAGS.FOX_BAD_END_WARNING] = 0;
            }
        }
        if (this.player.findPerk(PerkLib.EnlightenedNinetails) >= 0 || this.player.findPerk(PerkLib.CorruptedNinetails) >= 0) { //Check ninetails perks!
        ...
        if (this.player.lowerBody == LOWER_BODY_TYPE_HARPY && this.player.tailType == TAIL_TYPE_HARPY && this.player.findPerk(PerkLib.HarpyWomb) >= 0) { //Make eggs big if harpied!
            if (this.player.findStatusAffect(StatusAffects.Eggs) >= 0 && this.player.statusAffectv2(StatusAffects.Eggs) == 0) {
        ...
        if (this.player.hasCock() && this.player.cocks[0].cockType == CockTypesEnum.BEE) { //All the hourly bee cock checks except the 'seek out the bee girl' check. That's in timeChangeLarge
            this.outputText("\n");
            ...
            if (this.player.cocks[0].cockLength < 25 || this.player.cocks[0].cockThickness < 4) {
                ...
                else this.outputText(this.player.cocks[0].cockLength < 25 ? "longer again" : "wider again");
        ...
        if (!this.player.hasVagina() && this.player.findPerk(PerkLib.Diapause) >= 0) { //Lose diapause
        ...
        if (this.player.findPerk(PerkLib.MaraesGiftButtslut) >= 0 && this.player.ass.analWetness < 2) { //Prevent Buttsluts from getting dry backdoors
        ...
        if (this.player.pregnancyIncubation <= 0 && this.player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS) { //Fixing Egg Preg Preglocked Glitch
            this.player.knockUpForce(); //Clear Pregnancy
        }
        if (this.player.findStatusAffect(StatusAffects.Uniball) >= 0 && this.player.ballSize > 1 && this.player.balls > 0) { //Testicles Normalise:
        ...
        if (this.player.findPerk(PerkLib.Androgyny) < 0) { //Fix femininity ratings if out of whack!
        ...
        if (this.player.findStatusAffect(StatusAffects.LustStickApplied) >= 0) { //Lust stick!
        ...
        if (this.player.findStatusAffect(StatusAffects.Luststick) >= 0) { //Luststic countdown
        ...
        if (this.player.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00285] >= 50 && this.player.findPerk(PerkLib.LuststickAdapted) < 0) { //Luststick resistance unlock
            this.getGame().sophieBimbo.unlockResistance();
            if (this.player.findStatusAffect(StatusAffects.Luststick) >= 0) this.player.removeStatusAffect(StatusAffects.Luststick);
            needNext = true;
        }
        ...
        if (this.player.findStatusAffect(StatusAffects.Eggchest) >= 0) { //Eggs in tits!
            ...
            if (this.player.statusAffectv1(StatusAffects.Eggchest) <= 0) {
                ...
                this.player.growTits(this.player.statusAffectv2(StatusAffects.Eggchest), this.player.bRows(), true, 2);
                this.outputText("\n");
                this.player.removeStatusAffect(StatusAffects.Eggchest);
                needNext = true;
            }
        }
        if (this.player.findPerk(PerkLib.SpiderOvipositor) >= 0 || this.player.findPerk(PerkLib.BeeOvipositor) >= 0) { //Spider and Bee ovipositor updates
            if (this.player.findPerk(PerkLib.SpiderOvipositor) >= 0 && (!this.player.isDrider() || this.player.tailType != TAIL_TYPE_SPIDER_ADBOMEN)) { //Remove dat shit!
            ...
            else if (this.player.findPerk(PerkLib.BeeOvipositor) >= 0 && this.player.tailType != TAIL_TYPE_BEE_ABDOMEN) { //Remove dat shit!
                ...
                if (prevEggs < 10 && this.player.eggs() >= 10) { //Stage 1 egg message
                    if (this.player.findPerk(PerkLib.SpiderOvipositor) >= 0) {
                        ...
                        if (!this.player.hasVagina()) this.outputText("  Wait, how would you even go about that?");
                    }
                    ...
                    if (this.player.findPerk(PerkLib.SpiderOvipositor) >= 0) {
                        ...
                        if (!this.player.hasVagina()) this.outputText("  Wait, that's not right...");
                    }
                    else {
        ...
        if (this.player.findPerk(PerkLib.Oviposition) >= 0 || this.player.findPerk(PerkLib.BunnyEggs) >= 0) { //Oviposition perk for lizard and bunny folks
            if ((this.player.nagaScore() + this.player.lizardScore() < 3) && this.player.findPerk(PerkLib.Oviposition) >= 0 && this.player.findPerk(PerkLib.BasiliskWomb) < 0) { //--Lose Oviposition perk if lizard score gets below 3.
            ...
            else if (this.player.bunnyScore() < 3 && this.player.findPerk(PerkLib.BunnyEggs) >= 0) { //--Lose Oviposition perk if bunny score gets below 3.
                ...
                if ((this.player.totalFertility() > 50 && this.getGame().model.time.days % 15 == 0) || this.getGame().model.time.days % 30 == 0) { //every 15 days if high fertility get egg preg
                    ...
                    this.player.knockUp(PregnancyStore.PREGNANCY_OVIELIXIR_EGGS, PregnancyStore.INCUBATION_OVIELIXIR_EGGS, 1, 1);
                    this.player.createStatusAffect(StatusAffects.Eggs, PlayerEvents.rand(6), PlayerEvents.rand(2), (5 + PlayerEvents.rand(3)), 0); //v1 is type, v2 is size (1 == large) and v3 is quantity
        ...
        if (this.player.inHeat) { //Heats v1 is bonus fertility, v2 is bonus libido, v3 is hours till it's gone
            if (this.player.statusAffectv3(StatusAffects.Heat) <= 1 || this.player.vaginas.length == 0) { //Remove bonus libido from heat
                this.getGame().dynStats("lib", -this.player.statusAffect(this.player.findStatusAffect(StatusAffects.Heat)).value2, "resisted", false, "noBimbo", true);
        ...
        if (this.player.inRut) { //Rut v1 is bonus cum, v2 is bonus libido, v3 is hours till it's gone
            ...
            if (this.player.statusAffectv3(StatusAffects.Rut) <= 1 || this.player.totalCocks() == 0) { //Remove bonus libido from rut
                this.getGame().dynStats("lib", -this.player.statusAffectv2(StatusAffects.Rut), "resisted", false, "noBimbo", true);
        ...
        if (this.player.findStatusAffect(StatusAffects.LustyTongue) >= 0) { //Lusty Tongue Check!
            if (PlayerEvents.rand(5) == 0) {
                ...
                if (this.player.lust >= 100) {
                    ...
                    this.player.changeStatusValue(StatusAffects.LustyTongue, 1, this.player.statusAffectv1(StatusAffects.LustyTongue) + 10); //Tongue orgasming makes it last longer.

                }
                this.outputText("\n");
                needNext = true;
            }
            this.player.changeStatusValue(StatusAffects.LustyTongue, 1, this.player.statusAffectv1(StatusAffects.LustyTongue) - 1); //Decrement
        ...
        if (this.player.statusAffectv2(StatusAffects.Kelt) > 0) this.player.addStatusValue(StatusAffects.Kelt, 2, -0.15); //Reduce kelt submissiveness by 1 every 5 hours
        //Mino cum update.
        if (this.getGame().mountain.minotaurScene.minoCumUpdate()) {
            needNext = true;
        }
        else if (this.flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] >= 2 && this.getGame().model.time.hours % 13 == 0 && this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00330] == 0) { //Repeated warnings!
        ...
        else if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00330] > 0) this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00330]--;
        if (this.player.findPerk(PerkLib.FutaForm) >= 0) { //Futa checks
        ...
        if (this.player.findPerk(PerkLib.BimboBody) >= 0 || this.player.findStatusAffect(StatusAffects.BimboChampagne) >= 0) { //Bimbo checks
            if (this.player.breastRows[0].breastRating < 5) { //Tits!
                this.player.breastRows[0].breastRating = 5;
                if (this.player.findPerk(PerkLib.BimboBrains) >= 0 || this.player.findStatusAffect(StatusAffects.BimboChampagne) >= 0)
        ...
        if (this.player.findPerk(PerkLib.BroBody) >= 0) { //Bro checks
            ...
            if (this.player.cocks[0].cockLength < 10) { //(Dick rebiggening)
                ...
                if (this.player.findPerk(PerkLib.BroBrains) >= 0) this.outputText("Goddamn, that thing is almost as tough as you!  ");
        ...
        if (this.player.findStatusAffect(StatusAffects.Feeder) >= 0) { //Feeder checks
            if (this.player.cor <= 20) { //Go away if pure
                ...
                if (this.player.statusAffectv2(StatusAffects.Feeder) >= 72 && this.getGame().model.time.hours == 14) {
                    ...
                    this.getGame().dynStats("sen", 2 + (((this.player.statusAffectv2(StatusAffects.Feeder)) - 70) / 20));
                    needNext = true;
                }
            }
        }
        if (this.player.findStatusAffect(StatusAffects.WormPlugged) >= 0 && this.flags[kFLAGS.PLAYER_PREGGO_WITH_WORMS] == 0) { //Update worm drippy-cooch
            if (this.player.hasVagina()) {
                if (PlayerEvents.rand(5) == 0) {
                    ...
                    if (this.player.fertility > 5) this.player.fertility -= (1 + Math.round(this.player.fertility / 4));
                    this.player.addStatusValue(StatusAffects.WormPlugged, 1, -1); //Lower chances
                    if (this.player.statusAffectv1(StatusAffects.WormPlugged) <= 0) { //Remove if too low
        ...
        if (this.player.findStatusAffect(StatusAffects.Milked) >= 0) { //"Milked"
        ...
        if (this.player.findStatusAffect(StatusAffects.LactationReduction) < 0) { //Lactation reduction
            if (this.player.biggestLactation() > 0) this.player.createStatusAffect(StatusAffects.LactationReduction, 0, 0, 0, 0);
        }
        else if (this.player.biggestLactation() > 0 && this.player.findStatusAffect(StatusAffects.Feeder) < 0 && this.player.pregnancyIncubation == 0) {
            ...
            if (this.player.statusAffectv1(StatusAffects.LactationReduction) >= 48) {
                if (this.player.findStatusAffect(StatusAffects.LactationReduc0) < 0) {
                    ...
                    if (this.player.biggestLactation() <= 2) this.player.createStatusAffect(StatusAffects.LactationReduc1, 0, 0, 0, 0);
                    if (this.player.biggestLactation() <= 1) this.player.createStatusAffect(StatusAffects.LactationReduc2, 0, 0, 0, 0);
                    needNext = true;
                }
                this.player.boostLactation(-0.5 * this.player.breastRows.length / 24);
                if (this.player.biggestLactation() <= 2.5 && this.player.findStatusAffect(StatusAffects.LactationReduc1) < 0) {
                ...
                else if (this.player.biggestLactation() <= 1.5 && this.player.findStatusAffect(StatusAffects.LactationReduc2) < 0) {
                ...
                if (this.player.biggestLactation() < 1 && this.player.findStatusAffect(StatusAffects.LactationReduc3) < 0) {
        ...
        if (this.player.findStatusAffect(StatusAffects.CuntStretched) >= 0) { //Cunt stretching stuff
            this.player.addStatusValue(StatusAffects.CuntStretched, 1, 1);
            if (this.player.vaginas.length > 0) {
                if (this.player.findPerk(PerkLib.FerasBoonWideOpen) < 0) {
                    if (this.player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS_LOOSE && this.player.statusAffectv1(StatusAffects.CuntStretched) >= 200) {
                    ...
                    if (this.player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS_GAPING && this.player.statusAffectv1(StatusAffects.CuntStretched) >= 100) {
                        this.outputText("\nYour " + this.getGame().vaginaDescript(0) + " recovers from your ordeals, tightening up a bit.\n");
                        this.player.vaginas[0].vaginalLooseness--;
                        this.player.changeStatusValue(StatusAffects.CuntStretched, 1, 0);
                        needNext = true;
                    }
                    if (this.player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS_GAPING_WIDE && this.player.statusAffectv1(StatusAffects.CuntStretched) >= 70) {
                ...
                if (this.player.vaginas[0].vaginalLooseness == VAGINA_LOOSENESS_LEVEL_CLOWN_CAR && this.player.statusAffectv1(StatusAffects.CuntStretched) >= 50) {
        ...
        if (this.player.findStatusAffect(StatusAffects.ButtStretched) >= 0) { //Butt stretching stuff
            this.player.addStatusValue(StatusAffects.ButtStretched, 1, 1);
            if (this.player.ass.analLooseness == 2 && this.player.statusAffectv1(StatusAffects.ButtStretched) >= 72) {
            ...
            if (this.player.ass.analLooseness == 3 && this.player.statusAffectv1(StatusAffects.ButtStretched) >= 48) {
                this.outputText("\n<b>Your " + this.getGame().assholeDescript() + " recovers from your ordeals, tightening up a bit.</b>\n");
                this.player.ass.analLooseness--;
                this.player.changeStatusValue(StatusAffects.ButtStretched, 1, 0);
                needNext = true;
            }
            if (this.player.ass.analLooseness == 4 && this.player.statusAffectv1(StatusAffects.ButtStretched) >= 24) {
                this.outputText("\n<b>Your " + this.getGame().assholeDescript() + " recovers from your ordeals and becomes tighter.</b>\n");
                this.player.ass.analLooseness--;
                this.player.changeStatusValue(StatusAffects.ButtStretched, 1, 0);
                needNext = true;
            }
            if (this.player.ass.analLooseness == 5 && this.player.statusAffectv1(StatusAffects.ButtStretched) >= 12) {
        ...
        if (this.player.findPerk(PerkLib.SlimeCore) >= 0) { //Lose slime core perk
            if (this.player.vaginalCapacity() < 9000 || this.player.skinAdj != "slimy" || this.player.skinDesc != "skin" || this.player.lowerBody != LOWER_BODY_TYPE_GOO) {
        ...
        if (this.player.hasKeyItem("Ruby Heart") >= 0) { //Regain slime core
            if (this.player.findStatusAffect(StatusAffects.SlimeCraving) >= 0 && this.player.findPerk(PerkLib.SlimeCore) < 0 && this.player.isGoo() && this.player.gooScore() >= 4 && this.player.vaginalCapacity() >= 9000 && this.player.skinAdj == "slimy" && this.player.skinDesc == "skin" && this.player.lowerBody == LOWER_BODY_TYPE_GOO) {
        ...
        if (this.player.findStatusAffect(StatusAffects.SlimeCraving) >= 0) { //Slime craving stuff
            if (this.player.vaginalCapacity() < 9000 || this.player.skinAdj != "slimy" || this.player.skinDesc != "skin" || this.player.lowerBody != LOWER_BODY_TYPE_GOO) {
                ...
                if (this.player.statusAffectv1(StatusAffects.SlimeCraving) >= 18) {
                    if (this.player.findStatusAffect(StatusAffects.SlimeCravingOutput) < 0) { //Protects against this warning appearing multiple times in the output
                        this.player.createStatusAffect(StatusAffects.SlimeCravingOutput, 0, 0, 0, 0);
                    ...
                    if (this.player.spe > 1) this.player.addStatusValue(StatusAffects.SlimeCraving, 3, 0.1); //Keep track of how much has been taken from speed
        ...
        if (this.player.findStatusAffect(StatusAffects.SlimeCravingFeed) >= 0) { //Slime feeding stuff
            this.outputText("\n<b>You feel revitalized from your recent intake, but soon you'll need more...</b>\n");
            this.getGame().dynStats("str", this.player.statusAffectv2(StatusAffects.SlimeCraving) * 0.5, "spe", this.player.statusAffectv3(StatusAffects.SlimeCraving)); //Boost speed and restore half the player's lost strength
        ...
        if (this.flags[kFLAGS.BIKINI_ARMOR_BONUS] > 0) {
            if (this.player.armorName == "lusty maiden's armor") {
                ...
                if (this.flags[kFLAGS.BIKINI_ARMOR_BONUS] < 0) this.flags[kFLAGS.BIKINI_ARMOR_BONUS] = 0; //Keep in bounds.
                if (this.flags[kFLAGS.BIKINI_ARMOR_BONUS] > 8) this.flags[kFLAGS.BIKINI_ARMOR_BONUS] = 8;
            }
        ...
        if (this.flags[kFLAGS.TIME_SINCE_VALA_ATTEMPTED_RAPE_PC] > 0) this.flags[kFLAGS.TIME_SINCE_VALA_ATTEMPTED_RAPE_PC]--; //Vala post-rape countdown
        if (this.flags[kFLAGS.GATS_ANGEL_TIME_TO_FIND_KEY] > 0 && this.flags[kFLAGS.GATS_ANGEL_TIME_TO_FIND_KEY] < 500) this.flags[kFLAGS.GATS_ANGEL_TIME_TO_FIND_KEY]++;

        if (this.getGame().model.time.hours > 23) { //Once per day
            this.flags[kFLAGS.BROOKE_MET_TODAY] = 0;
            if (this.getGame().model.time.days % 2 == 0 && this.flags[kFLAGS.KAIJU_BAD_END_COUNTER] > 0) {
                this.flags[kFLAGS.KAIJU_BAD_END_COUNTER]--;
                if (this.flags[kFLAGS.KAIJU_BAD_END_COUNTER] < 0) this.flags[kFLAGS.KAIJU_BAD_END_COUNTER] = 0;
            }
            ...
            if (this.flags[kFLAGS.NOT_HELPED_ARIAN_TODAY] != 0) this.flags[kFLAGS.NOT_HELPED_ARIAN_TODAY] = 0;
            if (this.flags[kFLAGS.RUBI_PROSTITUTION] > 0) this.flags[kFLAGS.RUBI_PROFIT] += 2 + PlayerEvents.rand(4);
            ...
            if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00285] > 0) this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00285]--; //Reduce lust-stick resistance building
            if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00155] > 0) { //Dominika fellatrix countdown
                this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00155]--;
                if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00155] < 0) this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00155] = 0;
            }
            if (this.flags[kFLAGS.LOPPE_DENIAL_COUNTER] > 0) { //Loppe denial counter
                this.flags[kFLAGS.LOPPE_DENIAL_COUNTER]--;
                if (this.flags[kFLAGS.LOPPE_DENIAL_COUNTER] < 0) this.flags[kFLAGS.LOPPE_DENIAL_COUNTER] = 0;
            }
            if (this.flags[kFLAGS.WEEKLY_FAIRY_ORGY_COUNTDOWN] > 0) { //Countdown to next faerie orgy
                this.flags[kFLAGS.WEEKLY_FAIRY_ORGY_COUNTDOWN]--;
                if (this.flags[kFLAGS.WEEKLY_FAIRY_ORGY_COUNTDOWN] < 0) this.flags[kFLAGS.WEEKLY_FAIRY_ORGY_COUNTDOWN] = 0;
            }
            if (this.getGame().model.time.days % 7 == 0) this.flags[kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK] = 0; //Clear Whitney's Weekly limit
            ...
            if (this.getGame().latexGirl.latexGooFollower()) { //Latex goo follower daily updates
                ...
                if (this.getGame().latexGirl.gooFluid() < 50) this.getGame().latexGirl.gooHappiness(-1, false);
                if (this.getGame().latexGirl.gooFluid() < 25) this.getGame().latexGirl.gooHappiness(-1, false);
                if (this.getGame().latexGirl.gooHappiness() < 75) this.getGame().latexGirl.gooObedience(-1, false);
                if (this.getGame().latexGirl.gooHappiness() >= 90) this.getGame().latexGirl.gooObedience(1, false);
            }
            this.getGame().farm.farmCorruption.updateFarmCorruption(); //Farm Corruption updating
            if (this.player.findStatusAffect(StatusAffects.Contraceptives) >= 0) { // Herbal contraceptives countdown
                if (this.player.statusAffectv1(StatusAffects.Contraceptives) == 1) {
                    this.player.addStatusValue(StatusAffects.Contraceptives, 2, -1);
                    if (this.player.statusAffectv1(StatusAffects.Contraceptives) < 0) this.player.removeStatusAffect(StatusAffects.Contraceptives);
                }
            }
            if (this.player.statusAffectv1(StatusAffects.SharkGirl) > 0) this.player.addStatusValue(StatusAffects.SharkGirl, 1, -1); //Lower shark girl counter
            ...
            if (this.player.findStatusAffect(StatusAffects.DragonBreathCooldown) >= 0) this.player.removeStatusAffect(StatusAffects.DragonBreathCooldown);
        }
        return needNext;
    }

    public timeChangeLarge(): boolean {
        if (PlayerEvents.rand(4) == 0 && this.getGame().isHolidays() && this.player.gender > 0 && this.getGame().model.time.hours == 6 && this.flags[kFLAGS.XMAS_CHICKEN_YEAR] < this.getGame().date.fullYear) {
            this.getGame().getAChristmasChicken();
            return true;
        }
        if (this.getGame().model.time.hours == 1 && this.getGame().isHolidays() && this.getGame().date.fullYear > this.flags[kFLAGS.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE]) { //XMAS ELF
            this.getGame().xmasBitchEncounter(); //Set it to remember the last year encountered
            return true;
        }
        if (this.checkedTurkey++ == 0 && (PlayerEvents.rand(5) == 0 && (this.getGame().model.time.hours == 18 || this.getGame().model.time.hours == 19)) && (this.getGame().date.fullYear > this.flags[kFLAGS.TURKEY_FUCK_YEAR_DONE] || this.flags[kFLAGS.MORE_TURKEY] > 0) && this.getGame().isThanksgiving() && this.player.gender > 0) {
        ...
        if (this.checkedDream++ == 0 && this.getGame().model.time.hours == 3) { //You can only have one dream each night
            ...
            if (this.player.hasCock() && this.player.findPerk(PerkLib.BeeOvipositor) >= 0 && (this.player.eggs() >= 20 && PlayerEvents.rand(6) == 0)) { //Bee dreams proc
                ...
                if (this.player.cumQ() >= 1000) this.outputText("  It's completely soaked your bedroll, too... you won't be sleeping on this again until you wash it out.  Grumbling, you roll the soggy, white-stained fabric up and stow it.");
            ...
            if (this.player.hasCock() && this.player.findPerk(PerkLib.SpiderOvipositor) >= 0 && (this.player.eggs() >= 20 && PlayerEvents.rand(6) == 0)) { //Drider dreams proc
                ...
                if (this.player.hasVagina()) this.outputText(this.getGame().vaginaDescript(0) + " and ");
                ...
                if (this.player.hasVagina()) this.outputText(" while you bend and force your own into her cunt");
                this.outputText(".  It's not long until you feel ");
                if (this.player.hasVagina()) this.outputText("her pussy clenching around you as you orgasm explosively inside, followed by ");
                ...
                if (this.player.cumQ() > 1000) this.outputText("It's all over your bedroll, too...");
            ...
            switch (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00218] + this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00219] + this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00220]) {
            ...
            if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00157] > 0 && this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00157] < 4) { //Dominika Dream
                this.outputText("\n<b>Your rest is somewhat troubled with odd dreams...</b>\n");
                this.getGame().telAdre.dominika.fellatrixDream();
                return true;
            }
            if (this.getGame().anemoneScene.kidAXP() >= 40 && this.flags[kFLAGS.HAD_KID_A_DREAM] == 0 && this.player.gender > 0) {
        ...
        if (this.player.statusAffectv1(StatusAffects.SlimeCraving) >= 18 && this.player.str <= 1) { //Bad end!
            this.getGame().lake.gooGirlScene.slimeBadEnd();
            return true;
        }
        if (this.player.hasCock() && this.player.cocks[0].cockType == CockTypesEnum.BEE && this.player.lust >= 100) {
