...
import { describeEachOfYourCocks, describeCock, describeCockShort, describeCocksLight, nounCock } from 'Content/Descriptors/CockDescriptor';
import { lustyMaidenPaizuri } from 'Content/Items/Armors/LustyMaidensArmor';
import { LegType } from 'Engine/Body/Legs';
import { describeBreastRow, describeAllBreasts, describeNipple, describeChest, describeBiggestBreastRow } from 'Content/Descriptors/BreastDescriptor';
...
import { growSmallestBreastRow, boostLactation } from 'Content/Modifiers/BreastModifier';
import { Settings } from 'Content/Settings';
import { ArmorName } from 'Content/Items/ArmorName';
import { ImpPregEvent } from 'Content/Scenes/Pregnancy/Imp';
import { ImpGang } from 'Content/Scenes/Areas/Camp/ImpGang';
...
function rapeImpWithDick(player: Character, imp: Character): NextScreenChoices {
    ...
    if (player.body.cocks.length === 1) {
        CView.clear();
        CView.text("With a demonic smile you grab the insensible imp and lift him from the ground by his neck.  The reduced airflow doesn't seem to slow his feverish masturbation at all, and only serves to make him harder.");
...
function rapeImpWithPussy(player: Character, imp: Character): NextScreenChoices {
    ...
    // Big enough!
        ...
        // Preggers chance!
        attemptKnockUpNoEggs(player, PregnancyType.IMP, IncubationTime.IMP, ImpPregEvent);
...
export function impGangabangaEXPLOSIONS(player: Character): NextScreenChoices {
    ...
    if (player.body.legs.type === LegType.CENTAUR) {
        if (randInt(2) === 0 && (player.body.cocks.length === 0 || player.gender === 3)) {
            ...
            attemptKnockUpNoEggs(player, PregnancyType.IMP, IncubationTime.IMP - 14, ImpPregEvent); // Bigger imp means faster pregnancy
        }
    ...
    if (player.stats.lust >= 100) {
        ...
        if (player.gender === 0 || player.gender === 1) {
            ...
            if (player.gender === 1 && randInt(2) === 0) {
                ...
                CView.clear();
                CView.text("Your eyes glaze over with lust as the imp's dark magic destroys your will to continue fighting. You sink to your ");
...
function noogaisNippleRape(player: Character): NextScreenChoices {
    ...
    if (Settings.sillyMode) CView.text("  The imp is sickened, but curious.");
    CView.text("\n\n");
