...
import { describeCock, describeOneOfYourCocks, describeEachOfYourCocks } from 'Content/Descriptors/CockDescriptor';
import { describeHips } from 'Content/Descriptors/HipDescriptor';
import { describeLeg, describeFoot, describeLegs } from 'Content/Descriptors/LegDescriptor';
import { EffectType } from 'Content/Effects/EffectType';
import { describeChest, describeNipple } from 'Content/Descriptors/BreastDescriptor';
...
import { displayStretchVagina, attemptKnockUpNoEggs } from 'Content/Modifiers/VaginaModifier';
...
export function akbalDefeated(player: Character, akbal: Character, hpVictory: boolean): NextScreenChoices {
    AkbalFlags.AKBAL_SUBMISSION_STATE = 1;
    // [General Victory]
    if (hpVictory) {
        // [Common chance of dropping 'Incubus Draft', 'Smart Tea' or 'Pipe']
        CView.clear().text("Akbal falls to the ground in a beaten bloody heap.");
    }
    // [Victory via Lust]
    else {
        CView.clear().text("Akbal falls to the ground, unable to go on. Yet a growl still rumbles in his chest, and you quickly recognize the submissive gesture when he bows his head, his cat belly hugging the ground.  His body begins shifting, and soon he has a vaguely humanoid form. You assume this is the form he uses for sex, as his lust is out of control.\n\n");
        if (player.stats.lust >= 33 && player.gender > 0) {
            CView.text("You walk around Akbal's beaten and lust crazed form with a smile on your face. The demon's growl continues as he awaits your judgment.");
            let vagoo: ClickFunction | undefined;
            let vagooLick: ClickFunction | undefined;
            let buttFuck: ClickFunction | undefined;
            let bikiniTits: ClickFunction | undefined;
            if (player.body.vaginas.length > 0) {
                vagoo = choiceWrap(girlsRapeAkbal, akbal);
                vagooLick = rapeAkbalForcedFemaleOral;
            }
            if (player.body.vaginas.length > 0 && player.body.chest.sort(BreastRow.Largest).get(0)!.rating >= 4 && player.inventory.armor.name === ArmorName.LustyMaidensArmor)
                bikiniTits = choiceWrap(lustyMaidenPaizuri, akbal);
            if (player.body.cocks.length > 0)
                buttFuck = rapeAkbal;
            CView.text("\n\nDo you rape him?");
            // Rape / Don't Rape
            return { choices: [["Butt-fuck", buttFuck], ["Take Vaginally", vagoo], ["Force Lick", vagooLick], ["B.Titfuck", bikiniTits], ["Leave", passTime(1)]] };
        }
    }
    return { next: passTime(1) };
}

export function akbalWon(player: Character, akbal: Character, hpVictory: boolean, pcCameWorms: boolean): NextScreenChoices {
    AkbalFlags.AKBAL_SUBMISSION_STATE = -1;
    AkbalFlags.AKBAL_BITCH_Q = 0;

    if (pcCameWorms) {
        CView.text("\n\nYour foe doesn't seem disgusted enough to leave...");
        return { next: choiceWrap(loseToAckballllllz, akbal) };
    } else if (hpVictory) {
        CView.text("The intense pain causes you to black out. The last thing you see is Akbal standing over you on his two hind legs, his massive cock ominously swinging between them as he watches you lose consciousness.\n\n");
        return { next: passTime(1) };
    } else {
        return loseToAckballllllz(player, akbal);
    }
}

// Victory/Defeat Scenes
function rapeAkbalForcedFemaleOral(player: Character): NextScreenChoices {
    AkbalFlags.AKBAL_BITCH_Q++;

    // Naga RAPPUUUUUU
    if (player.body.legs.isNaga()) {
        CView.image("akbal-deepwoods-naga-forcedfemaleoral");
        CView.text("You slither around the demon cat's form, wrapping him up until a scared whimper rises from his chest.  You continue to tighten your coils around Akbal until he's gasping for breath.  You ask him if he's going to be a good little demon for you.  He nods.\n\n");
        CView.text("As you uncoil you can't help but bump into the slimy leaking faucet Akbal calls his dick.  At your command he raises his hind quarters, allowing you a perfect view of his low hanging balls and sheath.  You reach around his fuzzy orbs and pull his swollen cock back until the jaguar demon's 15 inch pecker is visible.\n\n");

        CView.text("Your eyes widen as you see several wicked looking barbs around his pre-pumping slit.  Instead of trying to deal with this massive phallus that looks like it was made to punish sinners you slither around to the front of the creature.\n\n");

        CView.text("Your scent causes the growl ripping out of Akbal's chest to quiver.  You lay before him, reaching up to smash his furry lips into your " + describeVagina(player, player.body.vaginas.get(0)) + ".  The demon, unable to escape his debilitating arousal, begins to lap at your " + describeVagina(player, player.body.vaginas.get(0)) + ".  He shoves his face against your " + describeVagina(player, player.body.vaginas.get(0)) + " twisting his lips and drilling his tongue into you, mercilessly attacking your " + describeClit(player) + " as you scream, howl and cringe in ecstasy.\n\n");

        CView.text("He begins to lift up, probably to get into position above you and sake his lust, but you force his face back down into your " + describeVagina(player, player.body.vaginas.get(0)) + ".  After dropping a line about how he has to make your cum or get his head ripped off Akbal whines, obviously distressed at not being able to slip his aching member into your " + describeVagina(player, player.body.vaginas.get(0)) + ".\n\n");

        CView.text("With a sadistic laugh you ride out your orgasm until you're reduced to a shuddering heap on the floor.  After you've recovered you gather your " + player.inventory.armor.displayName + ", leaving Akbal in a groaning mess behind you.  He howls as he claws the ground, his barbed cock still rock hard beneath him.  Just as you begin to leave you notice a group of imps watching you and the jaguar demon, their cocks out and leaking as their jagged teeth spread into feral grins.  You even spy a few goblins mixed in the crowd, each twirling a bottle of liquid and playing with her snatch.\n\n");

        CView.text("As you leave Akbal snarls as the creatures that once feared him use his overtly aroused state to get revenge on the \"god\" of the terrestrial fire.  Even after you've reached the edge of the forest the wails of the jaguar demon can still be heard but just barely over the high pitched laughter of the demon imps and goblin females.");
    }
    // Centaur RAPPUUUUU
    else if (player.body.legs.isTaur()) {
        CView.image("akbal-deepwoods-taur-forcedfemaleoral");
        CView.text("You roughly grab the scruff of the demon's neck, aiming a gut crushing blow to his stomach and causing him to call out in pain.\n\n");

        CView.text("\"<i>Who's gonna submit now... bitch?</i>\"\n\n");
        CView.text("Akbal snarls as you shove him back first onto the forest ground.  You look down at him, fully prepared to have your way with his demon dick.  What you see causes your " + describeVagina(player, player.body.vaginas.get(0)) + " to flinch at the thought of what would have happened if you had lost and the demon had used that on you.\n\n");

        CView.text("The head of the massive dick sitting between Akbal's thighs is covered in a dozen tiny barbs that look like they were created to punish sinners.\n\n");

        CView.text("You walk around the creature, irritated at the fact that you can't squat on that crazily large demon cat dick.  Once you're on the other side of him you sit your hind quarters on his face.\n\n");

        CView.text("Akbal gives a muffled scream at first but soon he gets the message. His tongue slithers into your " + describeVagina(player, player.body.vaginas.get(0)) + ".  You lift yourself a little; well... you lean forward a bit to let the demon actually breath.  This proves to be the right choice as Akbal is ravenous for your " + describeVagina(player, player.body.vaginas.get(0)) + ".\n\n");

        CView.text("He drills his tonuge into you, mercilessly attacking your " + describeClit(player) + " as you scream, howl and cringe in ecstasy.  He begins to lift up, probably to get the weight of your body off of the rest of his face, but you grab his tender furry balls in your hand and he stops before you're forced to do something drastic.\n\n");

        CView.text("After dropping a line about how he has to make you cum or get his head ripped off Akbal whines, obviously distressed at not being able to slip his aching member into your " + describeVagina(player, player.body.vaginas.get(0)) + ".\n\n");

        CView.text("With a sadistic laugh you ride out your orgasm until you're reduced to a shuddering heap.  After you've recovered you stand and gather your " + player.inventory.armor.displayName + ", leaving Akbal in a groaning mess behind you.  He howls as he claws the ground, his barbed cock still rock hard beneath him.  Just as you begin to leave you notice a group of imps watching you and the jaguar demon, their cocks out and leaking as their jagged teeth spread into feral grins.  You even spy a few goblins mixed in the crowd, each twirling a bottle of liquid and playing with her snatch.\n\n");

        CView.text("As you leave Akbal snarls as the creatures that once feared him use his overtly aroused state to get revenge on the \"god\" of the terrestrial fire.  Even after you've reached the edge of the forest the wails of the jaguar demon can still be heard but just barely over the high pitched laughter of the demon imps and goblin females.");
    }
    // Everybody else
    else {
        CView.image("akbal-deepwoods-forcedfemaleoral");
        CView.text("You roughly grab the scruff of the demon's neck and give a gut-crushing blow to his stomach, causing him to call out in pain.");
        CView.text("\n\n\"<i>Who's gonna submit now, bitch?</i>\"\n\n");
        CView.text("Akbal grunts as you smash his face into the ground.  At your command he raises his hind quarters, allowing you a perfect view of his low-hanging balls and sheath.  You reach around his fuzzy orbs and pull his swollen cock back until the jaguar demon's 15-inch pecker is visible.\n\n");
        CView.text("Your eyes widen when you see several wicked-looking barbs around his pre-pumping cockhead.  Instead of trying to deal with this massive phallus that looks like it was made to punish sinners, you walk around to the front of the creature.\n\n");
        CView.text("Your scent causes Akbal's growling to quiver.  You lay down before him, grabbing his head and smashing his furry lips into your " + describeVagina(player, player.body.vaginas.get(0)) + ".  The demon, unable to escape his debilitating arousal, begins to lap at your " + describeVagina(player, player.body.vaginas.get(0)) + ".  He twists his lips and drills his tongue into you, mercilessly attacking your " + describeClit(player) + ", and you scream, howl and cringe in ecstasy.  He begins to lift up, probably to get into position above you, but you force his face back down into your " + describeVagina(player, player.body.vaginas.get(0)) + ".  After dropping a line about how he has to make your cum or get his head ripped off, Akbal whines, obviously distressed at not being able to slip his aching member into your " + describeVagina(player, player.body.vaginas.get(0)) + ".\n\n");

        CView.text("With a sadistic laugh you ride out your orgasm until you're reduced to a shuddering heap on the floor.  After you've recovered, you stand and gather your " + player.inventory.armor.displayName + ", leaving Akbal in a groaning mess behind you.  He howls as he claws the ground, his barbed cock still rock hard beneath him.  As you walk away, you notice a group of imps watching you and the jaguar demon with their cocks out and leaking, their jagged teeth spread into feral grins.  You even spy a few goblins mixed in the crowd, and each is twirling a bottle of liquid while playing with her snatches.\n\n");
        CView.text("Akbal snarls as you leave, the creatures that once feared him using his aroused state to get revenge on the 'god' of the terrestrial fire.  Even after you've reached the edge of the forest, the jaguar demon's pained howls can still be heard – though, just barely over the high-pitched laughter of the demon imps and the cackling of the goblin females.");
    }
    // END CENTAUR STUFF
    player.orgasm();
    player.stats.cor += 1;
...
function rapeAkbal(player: Character): NextScreenChoices {
    AkbalFlags.AKBAL_BITCH_Q++;
    ...
    // Naga RAPPUUUUUU
    if (player.body.legs.type === LegType.NAGA) {
        CView.text("You slither around the demon cat's form, wrapping him up until a scared whimper rises from his chest.  You continue to tighten your coils around Akbal until he's gasping for breath.  You ask him if he's going to be a good little demon for you.  He nods.\n\n");

        // (If player has a dick)
        if (player.body.cocks.length > 0) {
            CView.image("akbal-deepwoods-male-naga-rapeakbal");
            // (Sm Penis: 7 inches or less)
            if (cockThatFits.length <= 7) {
                CView.text("As you uncoil you can't help but bump into the slimy leaking faucet Akbal calls his dick.  At your command he raises his hind quarters, giving you a perfect view of his tight pucker.  From the looks of it nothing has ever even touched the tightly sealed rim.  First you poke it with your finger, causing Akbal to flinch at the sensation.  Taking your " + describeCock(player, cockThatFits) + " in hand you shove in without hesitation or mercy.  The virginally-tight hole clamps shut and Akbal hisses in pain as you force him open.  In no time at all you're sawing your " + describeCock(player, cockThatFits) + " in and out of the demon's virginally-tight hole, relishing in the way it quivers and squirms around your embedded " + describeCock(player, cockThatFits) + ".\n\n");
            }
            // (Md Penis: 8-12 inches)
            else if (cockThatFits.length <= 12) {
                CView.text("As you uncoil you can't help but bump into the slimy leaking faucet Akbal calls his dick.  At your command he raises his hind quarters, giving you a perfect view of his tight pucker.  From the looks of it nothing has ever even touched the tightly sealed rim.  A light tap of your finger causes the tiny hole to constrict in fear as Akbal's entire body flinches away from you.  You grab your " + describeCock(player, cockThatFits) + " with a cruel smile.\n\n");

                CView.text("As you shove your " + describeCock(player, cockThatFits) + " into his tight pucker you are not surprised to find your " + describeCock(player, cockThatFits) + " barely able to breach the tightly sealed walls.  Grunting with effort you slowly inch forward, Akbal howling and squirming beneath you as he is taken without regard for his own pleasure.\n\n");

                CView.text("After a dozen achingly slow thrusts Akbal's asshole begins to loosen and you begin to saw your " + describeCock(player, cockThatFits) + " in and out of his pucker with force as the demon cat's howls range from pained to pleased.\n\n");
            }
            // (Lg Penis: 13 inches and up)
            else {
                CView.text("As you uncoil you can't help but bump into the slimy leaking faucet Akbal calls his dick.  At your command he raises his hind quarters, giving you a perfect view of his tight pucker.  A short glance tells you your " + describeCock(player, cockThatFits) + " won't fit... but since when has that stopped you from trying?\n\n");

                CView.text("When you slide a single finger against his virginally-tight hole Akbal flinches, the already tight hole clamping shut in fear.\n\n");

                CView.text("You twist Akbal's tail into your fist and laugh as a scared whimper rises out of the jaguar demon's throat.  You begin to push your huge cock in with one hand as your other pulls Akbal's tail.  The creature wines and howls as you invade its clenching sphincter, forcibly stretching its tight pink hole to a dangerous degree with your " + describeCock(player, cockThatFits) + ".  All too soon you are halted, barely able to fit more than a foot of your huge cock into Akbal's virginally-tight hole.  Deciding that's good enough you pull out and push forward, meeting the same resistance as before.\n\n");

                CView.text("After hours of resistance and howling, Akbal's body shudders as his asshole relaxes due to complete exhaustion.  Battling with your tremendous cock seems to have made him almost pass out and he no longer has the energy to resist you.  The jaguar demon's body flinches with your every thrust as you begin to pound him raw, without lube and without mercy.\n\n");
            }
            // (transition)
            CView.text("You rape the jaguar demon's tight hole with steadily mounting force, your " + describeHips(player) + " smashing into his body with freight train force and causing him to cry out.  Despite this his 15 inch swollen sex organ pumps pre beneath him, letting you know that his pain is mixed with plenty of unwilling pleasure.\n\n");

            // (With Fertility/Lots of Jizz Perk)
            if (player.cumQ() > 1000) {
                CView.text("The crushing tightness of Akbal's quivering hole pushes you over the edge and with a titanic howl you begin hosing down his insides.  The jaguar demon erupts as well, his body convulsing in time with your still thrusting " + describeCock(player, player.body.cocks.get(0)) + ".\n\n");

                CView.text("As your cock continues to pump massive tons of liquid into the jaguar demon you grind your still swelling sex organ inside him and beneath the two of you his belly begins to bulge as he is filled.  As your massive orgasm subsides you pull out, releasing a gargantuan deluge of your thick spunk that rolls down his legs and creates a large puddle in the forest floor.  Akbal heaves a relieved sighs, obviously happy you are done raping him.\n\n");
            }
            // (without perk)
            else
                CView.text("The crushing tightness of Akbal's quivering hole pushes you over the edge and with a titanic howl you begin hosing down his insides. Beneath you Akbal's own orgasm erupts and the jaguar demon goes limp as you continue to pound him through his orgasm.  When you pull out you aim a slap at Akbal's now very tender ass, making him yelp as your unexpected blow connects.\n\n");

            // (Ending)
            CView.text("As you stand you gather your " + player.inventory.armor.displayName + " and turn to leave the weakened demon behind you.  Just as you begin to walk away you notice a group of imps watching you and the jaguar demon, their cocks out and leaking.  Mixed in with the crowd are several goblins, each with a vial of liquid and a malicious grin.\n\n");
        ...
        // (If the player has a vagina)
            ...
            CView.image("akbal-deepwoods-female-naga-rapeakbal");
            CView.text("As you uncoil you can't help but bump into the slimy leaking faucet Akbal calls his dick.  At your command he raises his hind quarters, allowing you a perfect view of his low hanging balls and sheath.  You reach around his fuzzy orbs and pull his swollen cock back until the jaguar demon's 15 inch pecker is visible.\n\n");
    ...
    // Centaur RAPPUUUUU
        ...
        // (If player has a dick)
        if (player.body.cocks.length > 0) {
            CView.image("akbal-deepwoods-male-taur-rapeakbal");
            // (Sm Penis: 7 inches or less)
            if (cockThatFits.length <= 7) {
                CView.text("Akbal snarls pitifully as you shove him stomach down against a log.  Your " + describeCock(player, cockThatFits) + " leaks at the thought of bringing this \"god\" down a notch .  At your command he arches his back and lifts his tail, giving you a perfect view of his tight pucker.  From the looks of it nothing has ever even touched the tightly sealed rim.  As you lightly tap it with your finger it flinches, becoming even tighter as Akbal's body jerks away in fear.\n\n");

                CView.text("You rear up, standing on two legs for a moment before your hooves fall to the ground, Akbal's head between your front legs with his hair tickling your belly.  You give a few experimental thrusts.  The first one causes your " + describeCock(player, cockThatFits) + " to slide up Akbal's upturned cheeks, making him cringe.  The second one, however, catches.\n\n");

                CView.text("The virginally-tight hole clamps shut and Akbal hisses in pain as you force him open.  In no time at all you're sawing your " + describeCock(player, cockThatFits) + " in and out of the demon's virginally-tight hole, relishing in the way it quivers and squirms around your embedded " + describeCock(player, cockThatFits) + ".  The sounds coming from Akbal's throat are varied but include many unwillingly pleased groans.  He's obviously liking this more  than he's willing to admit.\n\n");
            }
            // (Md Penis: 8-12 inches)
            else if (cockThatFits.length <= 12) {
                CView.text("Akbal snarls pitifully as you shove him stomach down against a log.  Your " + describeCock(player, cockThatFits) + " leaks at the thought of bringing this \"god\" down a notch.  At your command he arches his back and lifts his tail, giving you a perfect view of his tight pucker.  From the looks of it nothing has ever even touched the tightly sealed rim.  As you lightly tap it with your finger it flinches, becoming even tighter as Akbal's body jerks away in fear.\n\n");

                CView.text("You rear up, standing on two legs for a moment before your hooves fall to the ground, Akbal's head between your legs tickling your belly.  You give a few experimental thrusts.  The first one causes your " + describeCock(player, cockThatFits) + " to slide up Akbal's upturned cheeks, making him cringe.  The second one, however, catches.\n\n");

                CView.text("As you shove your " + describeCock(player, cockThatFits) + " forward into his virginally-tight pucker you are not surprised to find your " + describeCock(player, cockThatFits) + " barely able to breach the tightly sealed walls.  Grunting with effort you slowly inch forward, Akbal howling and squirming beneath you as he is taken without regard for his own pleasure.\n\n");

                CView.text("After a dozen achingly slow thrusts Akbal's asshole begins to loosen and you begin to saw your " + describeCock(player, cockThatFits) + " in and out of his pucker.  The demon cat's howls range from pained to pleased, obviously enjoying his butt rape more than he is willing to admit.\n\n");
            }
            // (Lg Penis: 13 inches and up)
            else {
                CView.text("Akbal snarls pitifully as you shove him stomach down against a log. Your " + describeCock(player, cockThatFits) + " leaks at the thought of bringing this \"god\" down a notch .  At your command he arches his back and lifts his tail, giving you a perfect view of his tight pucker.  From the looks of it nothing has ever even touched the tightly sealed rim.  As you lightly tap it with your finger it flinches, becoming even tighter as Akbal's body jerks away in fear.  There's no way in hell your " + describeCock(player, cockThatFits) + " can fit inside something so incredibly tight but... since when has that stopped you?\n\n");

                CView.text("You rear up, standing on two legs for a moment before your hooves fall to the ground, Akbal's head between your legs, his hair tickling your belly.  You give a few experimental thrusts.  The first one causes your " + describeCock(player, cockThatFits) + " to slide up Akbal's upturned cheeks, making him cringe.  The second one, however, catches.\n\n");

                CView.text("The demon wines and wraps his arms around your front legs, pulling himself away from the burning pressure of your " + describeCock(player, cockThatFits) + ".  His voice breaks as you invade his clenching sphincter, forcibly stretching Akbal's tight pink hole to a dangerous degree.  All too soon you are halted, barely able to fit more than a foot of your huge cock into Akbal's virginally-tight hole.  Deciding that's good enough you pull out and push forward, meeting the same resistance as before.\n\n");

                CView.text("You grin as you realize that this may take a while...\n\n");

                CView.text("After hours of resistance and howling, Akbal's body shudders as his asshole relaxes due to complete exhaustion.  Battling with your tremendous cock seems to have made him almost pass out and he no longer has the energy to resist you.  The jaguar demon's body flinches with your every thrust as you begin to pound him raw, without lube and without mercy.\n\n");
            }
            // (transition)
            CView.text("You rape the jaguar demon's tight hole with steadily mounting force, your trunk smashing into his cringing body with freight train force and causing him to cry out in times with your grunts.  Despite this his 15 inch swollen sex organ pumps pre beneath him, letting you know that his pain is mixed with plenty of unwilling pleasure.\n\n");

            // (With Fertility/Lots of Jizz Perk)
            if (player.cumQ() > 1000) {
                CView.text("The crushing tightness of Akbal's quivering hole pushes you over the edge and with a titanic howl you begin hosing down his insides.  Akbal's suddenly clenching sphincter lets you know he has reached his orgasm as well.  ");
                CView.text("You continue to slide your still swollen sex organ inside his quivering hole as you pump massive tons of liquid into the false god's stomach and bowels.  Beneath the two of you his belly begins to bulge as he is filled to a dangerous degree.  ");
                CView.text("Once your massive orgasm subsides you pull out, releasing a gargantuan deluge of your thick spunk that rolls down his legs and creates a large puddle in the forest floor.  Akbal heaves a relieved sigh, obviously happy you are finally done raping him.\n\n");
            }
            // (without perk)
            else {
                CView.text("The crushing tightness of Akbal's quivering hole pushes you over the edge and with a titanic howl you begin hosing down his insides.  Akbal's suddenly clenching sphincter lets you know he has reached his orgasm as well.  The jaguar demon's body goes limp as you continue to pound him through his orgasm.  When you pull out you aim a slap at Akbal's now very tender ass, making him yelp as your unexpected blow connects.\n\n");
            }
            // (Ending)
            CView.text("As you stand you gather your " + player.inventory.armor.displayName + " and turn to leave the weakened demon behind you.  Just as you begin to walk away you notice a group of imps watching you and the jaguar demon, their cocks out and leaking.  Mixed in with the crowd are several goblins, each with a vial of liquid and a malicious grin.\n\n");
            CView.text("As you leave Akbal snarls as the creatures that once feared him use his weakened state to exact revenge on the \"god\" of the terrestrial fire.  Even After you've reached the edge of the forest the jaguar demon's pained snarls can still be heard but just barely over the high pitched laughter of the demon imps and the cackling of the goblin females.\n\n");
        ...
        // (If the player has a vagina)
            ...
            CView.image("akbal-deepwoods-female-taur-rapeakbal");
            CView.text("Akbal snarls as you shove him back first onto the forest ground.  You look down at him, fully prepared to have your way with his demon dick.  What you see causes your " + describeVagina(player, player.body.vaginas.get(0)) + " to flinch at the thought of what would have happened if you had lost and the demon had used that on you.\n\n");
    ...
    if (player.body.cocks.length > 0) {
        ...
        CView.image("akbal-deepwoods-male-rapeakbal");
        CView.text("Akbal grunts as you smash his face into the ground.  At your command he raises his hind quarters, allowing you a perfect view of his tight pucker.  From the looks of it, his tightly sealed rim would look at home on a virgin.\n\n");
        // [Small penis (7 inches or less)]
        if (firstCock.area < 13) {
            CView.text("You first poke it with your finger, causing Akbal to flinch at the sensation.  Taking your " + describeCock(player, firstCock) + " in hand, you shove it in without hesitation or mercy.  The virgin-like hole clamps shut and Akbal hisses in pain as you force him open.  In no time at all you're sawing your " + describeCock(player, firstCock) + " in and out of the demon's tight hole, relishing in the way it quivers and squirms around your embedded " + describeCock(player, firstCock) + ".\n\n");
        }
        // [Medium penis (8-12 inches)]
        else if (firstCock.area < 25) {
            CView.text("A light tap of your finger causes the tiny hole to constrict and Akbal's entire body flinches in fear.  You grab your " + describeCock(player, firstCock) + " with a cruel smile.  As you shove yourself into his tight pucker, you aren't surprised to find that your " + describeCock(player, firstCock) + " is barely able to breach the tightly sealed walls.  Grunting with effort you slowly inch forward, Akbal howling and squirming beneath you as he is taken without regard for his own pleasure.\n\n");
            CView.text("After a dozen achingly slow thrusts, Akbal's asshole begins to loosen and you start sawing your " + describeCock(player, firstCock) + " in and out of his pucker with force. The demon cat's howls fluctuate between yelps of pain and moans of pleasure.\n\n");
        }
        // [Large penis (13 inches and up)]
        else {
            CView.text("A single look tells you your " + describeCock(player, firstCock) + " won't fit... but since when has that stopped you from trying?  You slide a single finger against the virginally-tight hole and Akbal flinches. His already tight hole clamps shut in fear, as if doing so will somehow stop the inevitable intrusion.\n\n");
            CView.text("You twist Akbal's tail into your fist and laugh as a scared whimper escapes from the jaguar demon's throat.  You begin to push your " + describeCock(player, firstCock) + " in with one hand as you pull Akbal's tail with the other.  The demon whines and howls as you invade his clenching sphincter, forcibly stretching the tight pink hole to a dangerous degree.  You are halted all too soon, barely able to fit more than a foot of your " + describeCock(player, firstCock) + " into Akbal's virginally-tight hole.  Deciding that's good enough, you pull out before pushing forward again, meeting the same resistance as before.\n\n");
            CView.text("After what seems like hours of resistance and howling, Akbal's body shudders and his asshole relaxes from complete exhaustion.  Battling with your " + describeCock(player, firstCock) + " seems to have nearly made him pass out, and he no longer has the energy to resist you.  The jaguar demon's body flinches with your every thrust as you pound him raw – without lube, and without mercy.\n\n");
        }
        CView.text("You rape the jaguar demon's tight hole with steadily mounting force, your " + describeHips(player) + " smashing into his body with freight-train force, and causing him to cry out.  Despite this, his 15-inch swollen sex organ is pumping out pre beneath him, letting you know that his pain is mixed with plenty of unwilling pleasure.\n\n");
        CView.text("The crushing tightness of Akbal's quivering hole pushes you over the edge, and with a titanic howl you unleash your load inside him.  Akbal's suddenly clenching sphincter lets you know that he has reached his orgasm as well.\n\n");

        // [With Fertility/Lots of Jizz Perk]
        if (player.cumQ() > 1000) {
            CView.text("You continue to slide your still-swollen " + describeCock(player, firstCock) + " inside his quivering hole as you pump massive tons of liquid into the false god's stomach and bowels.  Beneath the two of you, his belly begins to bulge as he is filled to a dangerous degree.\n\n");
            CView.text("Once your massive orgasm subsides you pull out, releasing a gargantuan deluge of your thick spunk that rolls down his legs, creating a large puddle in the forest floor.  Akbal heaves a relieved sigh, obviously glad that you are finally done raping him.\n\n");
        }
        // [Without Fertility/Lots of Jizz Perk]
        else {
            CView.text("The jaguar demon's body goes limp as you continue to pound him through his orgasm.  When you finally pull out, you aim a slap at Akbal's now very tender ass and make him yelp when your unexpected blow connects.\n\n");
        }
        CView.text("Standing up, you gather your " + player.inventory.armor.displayName + " and turn to leave the weakened demon behind you.  As you walk away, you notice a group of imps watching you and the jaguar demon with their cocks out and leaking.  Mixed in with the crowd are several goblins, each with a vial of liquid and a malicious grin.\n\n");
    ...
    // [Player has a vagina]
        ...
        CView.image("akbal-deepwoods-female-rapeakbal");
        CView.text("Akbal grunts as you smash his face into the ground.  At your command he raises his hind quarters, allowing you a perfect view of his low-hanging balls and sheath.  You reach around his fuzzy orbs and pull his swollen cock back until the jaguar demon's 15-inch pecker is visible.\n\n");
        CView.text("Your eyes widen when you see several wicked-looking barbs around his pre-pumping cockhead.  Instead of trying to deal with this massive phallus that looks like it was made to punish sinners, you walk around to the front of the creature.\n\n");
        CView.text("Your scent causes Akbal's growling to quiver.  You lay down before him, grabbing his head and smashing his furry lips into your " + describeVagina(player, player.body.vaginas.get(0)) + ".  The demon, unable to escape his debilitating arousal, begins to lap at your " + describeVagina(player, player.body.vaginas.get(0)) + ".  He twists his lips and drills his tongue into you, mercilessly attacking your " + describeClit(player) + ", and you scream, howl and cringe in ecstasy.  He begins to lift up, probably to get into position above you, but you force his face back down into your " + describeVagina(player, player.body.vaginas.get(0)) + ".  After dropping a line about how he has to make your cum or get his head ripped off, Akbal whines, obviously distressed at not being able to slip his aching member into your " + describeVagina(player, player.body.vaginas.get(0)) + ".\n\n");

        CView.text("With a sadistic laugh you ride out your orgasm until you're reduced to a shuddering heap on the floor.  After you've recovered, you stand and gather your " + player.inventory.armor.displayName + ", leaving Akbal in a groaning mess behind you.  He howls as he claws the ground, his barbed cock still rock hard beneath him.  As you walk away, you notice a group of imps watching you and the jaguar demon with their cocks out and leaking, their jagged teeth spread into feral grins.  You even spy a few goblins mixed in the crowd, and each is twirling a bottle of liquid while playing with their snatches.\n\n");
        CView.text("Akbal snarls as you leave, the creatures that once feared him using his aroused state to get revenge on the 'god' of the terrestrial fire.  Even after you've reached the edge of the forest, the jaguar demon's pained howls can still be heard – though, just barely over the high-pitched laughter of the demon imps and the cackling of the goblin females.");
    }
    player.orgasm();
    player.stats.cor += 1;
...
function girlsRapeAkbal(player: Character, akbal: Character): NextScreenChoices {
    AkbalFlags.AKBAL_BITCH_Q++;

    CView.text("You smirk to yourself quietly as the so called \"God of Terrestrial Fire\" lays in a twitching heap on the ground, his flesh squirming as he shifts into his more humanoid form. Removing your " + player.inventory.armor.displayName + ", your hand lowers to your feminine slit, pondering how to make use of him.\n\n");
    // {If Centaur}
    CView.image("akbal-deepwoods-female-bindakbal");
    if (player.body.legs.isTaur()) {
        CView.text("Already you can see the trouble of trying to accommodate someone with your body type, but as they said in your village, \"<i>where there's a will, there's a way</i>\". Grabbing some of the vines from the nearby trees, you approach the nearly comatose Akbal and sling him over your back, grinning as you work through the details in your mind.\n\n");

        CView.text("Before the feline demon can recover and protest, you bind his arms to the end of the longest vines, throwing them over some of the stronger branches to make a makeshift pulley system. Hauling him up against the bark of the tree, he stirs slightly, but is still unable to work up the strength to fight back. Making the most of the time you figure you have left before he fully awakens, you bind his feet near the base of the tree, effectively turning him into a mounted toy for you to impale your " + describeVagina(player, player.body.vaginas.get(0)) + " upon.\n\n");

        CView.text("With his arms and legs bound, you take the time to examine your prize, reaching forward to stroke his full, baseball sized sack and swinging barbed shaft. It looks that despite the fact that you beat him into submission, the demon isn't totally opposed to the idea as he stirs to life in your hand, throbbing slightly. The jaguar starts to open his emerald eyes and glares at you, but nonetheless tries to push his hips forward, giving a snarl of annoyance when he can't work get proper leverage on the tree thanks to your binding.\n\n");

        CView.text("Keeping the vines holding his arms up in hand, you slowly turn around and lift your tail, allowing your sopping mare cunt to wink at Akbal");
        if (player.body.cocks.length > 0)
            CView.text(" " + describeOneOfYourCocks(player) + " hanging beneath, twitching in anticipation");
        CView.text(". With a smirk at the helpless Gods expense, you take a step back, lining up before pushing yourself against him roughly, shoving his barbed feline shaft deeply into you.");
    }
    // {If Pussy is <loose}
    else if (player.vaginalCapacity() < akbal.body.cocks.get(0)!.area) {
        CView.text("Rolling him over onto his back with your foot, you tsk in annoyance, knowing that you won't be able to fit his large kitty-cock between your legs without some considerable pain, and you don't really trust the demon to let his mouth, with all those oh-so-sharp teeth, near your most sensitive areas. Thankfully, another option is available, currently twitching around your feet.\n\n");

        CView.text("Reaching down, you grab his flicking tail, ignoring his feline yowl of discomfort, stroking the spotted, silky smooth fur with a few fingers, the sensation creating shivers of lust up your spine. Already images bloom in your mind, creating all kinds of acts you could use this appendage for and plucking the most prominent one from your mind, you grab the demon cat by the scruff of his neck, ");
        // ({If strength >60}
        if (player.stats.str > 60)
            CView.text("hauling him towards a tree like a newborn kitten");
        else
            CView.text("dragging him towards the nearest tree");
        CView.text(" and forcing him to lean back against it.\n\n");

        CView.text("Gripping his tail once more, you grab him by the fur of his head and yank him down to meet the tip, pressing insistently against his lips. Even in his weakened state Akbal refuses until you happen to glance down between his legs. It seems like the demon is more turned on by you taking charge than he would like to admit.  ");
        // ({if Naga}
        if (player.body.legs.isNaga())
            CView.text("You curl the tip of your tail around his length, catching the barbs lightly on your scales and squeezing, coaxing a low yowl of pleasure which you hurriedly make use of, ");
        // {If Goo}
        else if (player.body.legs.isGoo())
            CView.text("You focus, extending a small part of your liquid-like body, enveloping his feline meat in yourself, careful to avoid the tip. The sensation of damp warmth around his feline member is too much, making him open his mouth in a silent moan which you hastily use, ");
        else
            CView.text("Lifting up your " + describeLeg(player) + ", you firmly place the sole of your foot against his barbed flesh, pressing it against his stomach, forcing the jaguar to let out a low moan which you quickly take advantage of, ");

        CView.text("stuffing his own tail into his maw.  As he tries to push his tail back out, you remove your " + describeFoot(player) + " pointedly, staring down at the demon. Lifting his hips back up weakly towards you, he meekly begins to suckle on his own appendage, closing his vibrant emerald eyes as you return your attention to his shaft.\n\n");
    }
    // {If Pussy >= Loose}
    else {
        ...
        CView.text("In his dazed state, Akbal offers little resistance when you position him against the tree. That changes however, when you grab the hair of his head and push him down towards his own straining cock. Angry whispers flit at the edge of your mind, easily brushed aside as you reach down and squeeze the demon's cheeks, forcing his mouth to open. Quickly, you plug his maw with his own shaft, holding his head down, forcing him to coat his meat with his own saliva.  ");
        // {If Corruption <30}
        if (player.stats.cor < 33)
            CView.text("In a brief moment of pity you reach down, stroking Akbal's strained balls as he suckles, feeling them quiver as he noisily slurps and drools.");
        else
            CView.text("You smirk, grabbing his full sack almost roughly, shaking them as he stares at you with his emerald eyes, although with rage or lust you can't quite tell.");
        CView.text("\n\n");

        CView.text("Finally, you judge that he's done enough, allowing him to lift his head back up with a splutter, although considering the flowing pre around his tip and over his lips, the experience was hardly a painful one. He glares at you with his burning green eyes as you ");
        // ({if Player is naga}
        if (player.body.legs.isNaga())
            CView.text("rear up, supporting your weight on your own tail as your hands explore down below, finding your puffed up slit and slowly parting them with a pair of fingers, allowing your scent and dampness to coat the jaguar's snout.");
        else if (player.body.legs.isGoo())
            CView.text("reach out, your liquid-like body coating his legs and holding them open, your hands reaching down to play with your damp, open fuck hole, chuckling as he tries to feign disinterest despite the obvious throbbing of his sack.");
        else
            CView.text(" begin to stand over him, spreading your legs to reveal your damp, open pussy, his expression turning comically from rage to confusion to one of sheer lust, the feline licking his lips despite his own flavor on them.");
        CView.text("\n\n");

        CView.text("Judging by the shudder of longing that runs through his body, it's clear that he's more turned on by your actions than he would have liked. Using your lower body to pin his legs down, you grab his arms as you sink down, moaning more for his benefit as you brush the tip of his slick member against your entrance.");
    }
    player.stats.lust += 50;

    // -Page Turn-
    return { next: choiceWrap(girlsRapeAkbalPart2, akbal) };
}

function girlsRapeAkbalPart2(player: Character, akbal: Character): NextScreenChoices {

    // Centaur
    CView.image("akbal-deepwoods-female-taur-bindakbal");
    if (player.body.legs.isTaur()) {
        CView.text("You moan deeply as the thick shaft spreads your lips wide, throbbing against your clit as the barbs shudder against your inner walls, pushing your rump firmly against his lower abdomen as you squirm, leaking over his waist");
        if (player.body.cocks.length > 0)
            CView.text(", " + describeOneOfYourCocks(player) + " swinging back and forth, occasionally bumping into his bound legs");
        CView.text(". The demon groans, clenching his eyes shut as he refuses to like the treatment you're forcing on him, but his body betrays him as a liar as his felinehood thickens and lengthens, more than eager to stuff your box full. It's with no small shiver of delight when you drink in his moans of longing as you pull back to the very tip of his cock.\n\n");

        // ({If Corruption > 30}
        if (player.stats.cor > 33)
            CView.text("If you weren't so eager on using him to get off, you might have considered teasing him like that then leaving him bound to the tree.  ");
        CView.text("Quickly, you settle into a rhythm of rocking back and forth, the tree creaking as you throw your weight against it with every plunge down, the cat behind you reduced to a mewling, begging kitten, trying in vain to thrust into your sopping cunt.\n\n");

        CView.text("You continue like this for the better part of an hour, his barbs raking your insides and sending constant shivers through your body, but you just can't seem to get off on your bounces alone. Without really thinking it through, you release the vines holding Akbal's arms up to grope your own " + describeChest(player) + ", fondling yourself roughly.\n\n");

        CView.text("Thankfully, Akbal is just as desperate to get off as you as he makes use of his new found freedom, one hand clutching the base of your tail, the other grabbing your flank with a clawed paw. Immediately he snarls, shoving his feline dong deep into you, a much better sensation then when you were merely rocking on him, your mare cunt squirting and clenching around him, trying to milk his seed");
        if (player.body.cocks.length > 0)
            CView.text(", while your cock slaps at your stomach, pre flying from the tip to coat your lower body and the earth below");
        CView.text(". He buries himself deep over and over, his fat, swollen balls slapping against you, feeling oddly natural");
        if (player.body.balls.count > 0) {
            CView.text(", your own swaying back to meet him");
            // ({If >=Grapefruit}
            if (player.body.balls.size >= 12)
                CView.text(" dwarfing him despite his pent up lusts");
        }
        CView.text(", coaxing moans and groans of sheer pleasure to mingle with his snarls and purrs of enjoyment.\n\n");

        CView.text("In the end the jaguar finishes first, roaring his pleasure to the trees as he squirts his kitten-cream into your cunt, filling you up. If you were in any state to guess, you could probably imagine the cat filling up your womb as well judging by the swelling of your lower body, causing a flash of concern, wondering if the feline could possibly impregnate your womb. He's not finished however, as he continues to thrust through his release, pulling you higher and higher into orgasmic bliss as you finally release in tandem with his fifth orgasm. Your " + describeVagina(player, player.body.vaginas.get(0)) + " clenches tightly over him, coating his waist in fem juices as you milk him");
        if (player.body.cocks.length > 0)
            CView.text(" as " + describeOneOfYourCocks(player) + " twitch and let loose, spraying the ground with your seed");
        CView.text(".\n\n");
        ...
        // Imp pregnancy
        // Preggers chance!
        attemptKnockUpNoEggs(player, PregnancyType.IMP, IncubationTime.IMP, ImpPregEvent, 101);
    ...
    // TOIGHT
        ...
        // -Page Turn-
        CView.text("Judging the silky fur to be slick enough, you yank out the tail from his mouth, pulling it towards your " + describeVagina(player, player.body.vaginas.get(0)) + " lips, teasing your folds with the tip before guiding him in. Taking the hint, the feline squirms his tail, pushing into you as he lifts his hips up once more, purring lightly as he presses himself against your " + describeFoot(player) + ".  His cock is already leaking pre from the tip.  You gasp loudly as his fur bristles inside you, his saliva making your womanhood quiver in need as you feel him slip deeper, coiling and twisting. Making sure not to neglect the feline, you");
        // ({If Naga}
        if (player.body.legs.isNaga())
            CView.text(" squeeze his meat tighter, moving your coils up and down his feline prick while using the very tip of your tail to tease and poke his urethra, coating it in his pre");
        // ({If Goo}
        else if (player.body.legs.isGoo())
            CView.text(" squirm and writhe over the base of his feline shaft, vibrating your liquid-like appendage like a living sex toy, causing his balls to jiggle slightly");
        // ({If Bipedal}
        else
            CView.text(" press his shaft firmer against his stomach, slowly slipping the sole of your foot up and down his length, coaxing out small yowls and purrs of pleasure");
        CView.text(" as silent payment for the pleasure he's flooding your pussy with.\n\n");

        CView.text("Time seems to stretch as you stand there, pinning the god, once so proud and mighty, now just a mewling kitten devoted to your pleasure");
        // ({If Herm}
        if (player.body.cocks.length > 0)
            CView.text(" while you stroke " + describeEachOfYourCocks(player) + " in time with his thrusts");
        CView.text(". His flexible tail pokes and strokes your deepest recesses as you keep him permanently on the edge with your teasing treatment while his sharp claws dig furrows into the earth as he strains up against you, too caught up in his pleasure to mentally voice his desires. Instead, he opens his maw to signal his lust as a savage beast would, full of yowls, snarls and purrs, creating an oddly pleasing cacophony to your ears, making you feel like the " + mf(player, "Ruler of Beasts", "Queen of Beasts") + ", adding your own savage calls to his.\n\n");

        CView.text("The squirming of his tail becomes too much however, as a familiar pressure builds up down below. Increasing the pace upon which you please his shaft, you lower yourself, trying to push more of that skilled tail into you, ");
        // ({If Herm}
        if (player.gender === 3)
            CView.text("stroking " + describeEachOfYourCocks(player) + " roughly, ");
        CView.text("moaning loudly as your pussy lips begin to clench and tighten, slick juices trickling down his fur. Sensing your closeness, Akbal redoubles his efforts, his writhing tail bristling, dragging his fur along your insides. Finally it becomes too much as you release, your thick fem juices pouring out of your stuffed pussy, falling down onto his straining shaft");
        if (player.body.cocks.length > 0)
            CView.text(" as " + describeOneOfYourCocks(player) + " offers up its bounty, spasming and adding to the mess");
        CView.text(". The added heat and wetness of your orgasm sets him over the edge as he gives a roar loud enough to shake the trees, his thick, barbed shaft squirting hard, arcing his back as his seed splats onto the leaves of the tree above, falling down as a perverse rain over the pair of you.\n\n");

        CView.text("Your " + describeChest(player) + " chest heaves as you struggle to gulp air, " + describeLegs(player) + " quivering from the sheer power of the orgasm the cat's tail gave you.  ");
        // ({if cum volume > normal}
        if (player.cumQ() > 500)
            CView.text("His entire waist is coated in your juices, the once proud cat sitting in a pool of your leavings, with a contented grin on his face, like the cat that caught the canary");
        // ({If cum volume low/normal}
        else
            CView.text("He sits, dazed as your cum covers his groin, his meat still shiny from the torrent you dropped down upon it. Nevertheless, his face is twisted into a purr");
        CView.text(" as he sags against the tree trunk, overwhelmed by the pleasure. With a satisfied grin of your own, you pick up your " + player.inventory.armor.displayName + " and head out. Perhaps you should look out for the \"<i>God of Terrestrial fire</i>\" again sometime...");
    ...
    // Loose
        ...
        CView.text("You groan loudly as Akbal's impressive shaft stretches your pussy wide, instantly thankful that you had the idea to lube him up beforehand. Your less-than-willing partner adds his own groan to yours, the twitching of his meat signalling that he's not quite to opposed to the idea as he makes out. Maybe he's got a thing about dominance, from either angle. Nevertheless, you continue to push down until your hips reach his with a light bump. The sensation of his immense cock filling you up causes you to shudder, before leaning into the demon, pressing your " + describeChest(player) + " against his, revealing in his silky fur stroking multiple parts of your body all at once. The yowling male takes off, ducking his head to lick and nip at your " + describeNipple(player, player.body.chest.get(0)) + "s");
        // ({If Lactating}
        if (player.body.chest.sort(BreastRow.LactationMost).get(0)!.lactationMultiplier >= 1)
            CView.text(", adding a purr of pleasant surprise as he locks his lips around one nipple, drawing out mouthfuls of your sweet milk before gulping it down.  The sheer taboo of feeding a demon your milk sends shivers down your spine");
        CView.text(".\n\n");

        CView.text("As you lift yourself off, your head begins to swim as his previously unnoticed barbs rake along your inner walls, tugging, catching and massaging from the inside as they start to vibrate, causing you to ");
        // ({if Naga}
        if (player.body.legs.isNaga())
            CView.text("wrap your coils around him and the tree, adding leverage to your plunges down");
        // ({If Goo}
        else if (player.body.legs.isGoo())
            CView.text("encase yourself around his hips and groin, your entire lower body writhing and squirming around his shaft");
        // ({If Bipedal}
        else
            CView.text("lock your legs around his torso, bouncing upon him with greater force");
        CView.text(".  As your mouth hangs open, the demon lunges forward, pressing his own snout against your " + describeFaceShort(player) + ". You can still taste the traces of his own pre on his lips and tongue, furthering your lust as you use his groin roughly, impaling yourself hard enough to leave bruises on the pair of you, while he uses his tail to ");
        // ({if herm}
        if (player.body.cocks.length === 1)
            CView.text("wrap around your own straining shaft, the fur enhancing the effects of his pumps");
        // ({If more than 1 cock}
        else if (player.body.cocks.length > 1)
            CView.text("wrap around each of your throbbing malehoods with difficulty, teasing them with his bristled fur");
        // ({If female}
        else
            CView.text("stroke and tickle your ass and lower back");
        CView.text(".\n\n");

        CView.text("You're not sure how long you sit there, bouncing roughly on the feline demon's cock, his eyes clenched tightly shut as he revels in the feeling of your warm damp pussy and your " + describeButt(player) + " grinding against his swollen sack. Eventually, his combined efforts of mouth, shaft and tail force you over the edge, your hungry pussy lips clenching tightly over him, rhythmically squeezing as you attempt to milk his shaft");
        // ({if Herm}
        if (player.body.cocks.length > 0)
            CView.text(" as " + describeOneOfYourCocks(player) + "  twitches and strains, ready to blow");
        CView.text(", and he certainly doesn't disappoint. With a roar loud enough to shake the trees, he erupts violently within your passage, his hot, steaming, fertile seed pouring into your depths");
        // ({if Herm}
        if (player.body.cocks.length === 1)
            CView.text(", setting your own malehood off in sympathy, squirting thick cream over his chest");
        else if (player.body.cocks.length > 1)
            CView.text(" setting each of your malenesses surging, splattering the feline with your seed");
        CView.text(", his face twisted into a snarl of pleasure and satisfaction.\n\n");

        CView.text("You slowly come down from your orgasmic high, struggling to remove yourself from the demon's lap and heading unsteadily towards your " + player.inventory.armor.displayName + " as fresh feline seed pours down your body, wincing at the slight bruising to your womanhood. Rubbing a hand over your stomach, you start to wonder if perhaps it was a touch risky to allow a demon to shoot his seed into your womb. However, despite the mild throbbing, you feel refreshed and oddly strengthened by Akbal's potent seed, glancing over your shoulder to see the once proud god reveling in his own release. Perhaps it wouldn't be a bad idea to seek him out some other time...");
        player.orgasm();
        player.stats.cor += 1;
...
function loseToAckballllllz(player: Character, akbal: Character): NextScreenChoices {
    // [Defeat via Lust]
    if (player.body.legs.type !== LegType.CENTAUR) {
        CView.image("akbal-deepwoods-losslust-analed");
        CView.text("You fall to your knees and begin to feverishly masturbate.  Akbal rises onto his two hind legs, his body shifting into a more humanoid form as he stands.  His long cock swings ominously between his legs as he walks towards you.  The first thing he does is pull his massive 15-inch cock to your lips, slapping the shaft against your chin.\n\n");
        CView.text("Like a whore in heat, you open your mouth and lewdly lick the jaguar demon's cock head, feeling odd barbs rub against your tongue.  Your mouth opens wide, but can't even get past the head before the sheer girth of Akbal's massive sex organ halts its advance.  Akbal is content to let you orally fumble with the head for only a few moments before he pushes down onto your back. His claws tickle your thighs as he forces your " + describeLegs(player) + " up over your head, bringing your " + describeButthole(player.body.butt) + " into plain view.\n\n");
        CView.text("\"<i>Defiance repaid,</i>\" is all you hear from the chorus of voices in your head as Akbal displays his massive length to you.  Your eyes widen in horror, counting a dozen wicked-looking barbs on the head of his overtly thick and over-sized cock.\n\n");
    }
    // Centaurs R SPECIALZ
    else if (player.body.legs.type === LegType.CENTAUR) {
        CView.image("akbal-deepwoods-losslust-taur-analed");
        CView.text("You stumble like a drunken pony as your lust goes into the red zone and you know in the pit of your stomach that you are at this wicked demon's mercy.\n\n");

        CView.text("Akbal rises onto his two hind legs, his body becoming more humanoid as he does. His long, semi-erect cock swings ominously his legs as he walks towards you.\n\n");

        CView.text("\"<i>Defiance repaid,</i>\" is all you hear from the chorus of voices in your head as Akbal displays his massive length to you. Your eyes widen in horror as you count a dozen wicked looking barbs on the head of his overtly thick, gargantuan cock.\n\n");

        // (Small/Virgin Pucker)
        if (player.body.butt.looseness < 3) {
            CView.text("Akbal begins to push into you, the barbs on his massive head causing you to howl as your " + describeButthole(player.body.butt) + " is forcibly stretched. His jaguar claws grab your sides as he uses your body as leverage to force his demonic erection into you.\n\n");

            CView.text("When Akbal shoves forward the strain makes you feel as though you are going to pass out; the pain from his spiked sex organ is just enough to leave you conscious. He begins to withdraw and you realize he's not even forcing half the length of that swollen sex organ into your " + describeButthole(player.body.butt) + ".\n\n");

            CView.text("After hours of Akbal's long cat dick being slowly forced into your " + describeButthole(player.body.butt) + " your body gives out and you become too exhausted from the strain to even lift your arms. With a triumphant growl Akbal thrusts forward, his cock head spikes burying themselves into you but, without your resistance, they seem to vibrate inside you like twelve little beads massaging your innards. The sudden change makes you croon as you paw the ground with your hooves, suddenly desperate for more.");
        }
        // (Medium Pucker)
        else if (player.body.butt.looseness < 5) {
            CView.text("Akbal begins to push into you, the barbs on his massive cock head causing you to wince as you are forcibly stretched. Without warning he forces the entirety of his massive length into you with a snarl. The initial incursion makes you grind your teeth as that spiked rod invades your " + describeButthole(player.body.butt) + ".  You widen your stance in an attempt to lessen the sudden slicing pressure created by the barbed cock head. The moment you do the barbs start to vibrate, beginning to feel more like humming sex beads than the wicked looking battering ram you know is inside you. You can't suppress the sudden sounds coming from your throat and exclaiming your ecstasy to your rapist.");
        }
        // (Gapping Pucker)
        // [hp heals 50% after this if that's ok with
        // you Fen]
        else {
            CView.text("Akbal begins to push into you, the barbs on his massive cock head causing you to wince. He bottoms out instantly and you hear a pleased purr behind you. As he begins to pump his blood engorged sex organ in and out of your " + describeButthole(player.body.butt) + " with steadily mounting force you can't help but wonder why the barbs aren't causing you pain. You release a groan as those very barbs start to vibrate and begin feeling more like humming sex beads than punishing spikes.\n\n");

            CView.text("Akbal snarls as he slams his hips into you, obviously happy that you're able to take his massive length. The demon appears to forget he's raping you and begins licking the back of your horse-like bottom half, sending shivers throughout your entire body as he roughly fucks you while painting your back with his saliva.");
        }
        displayStretchButt(player, akbal.body.cocks.get(0)!.area, true);
        CView.text("\n\n");

        // (Ending)
        CView.text("The entire length of Akbal's embedded cock begins to hum inside you, causing you to cry out as he picks up the pace. His every thrust is a hammer-like thump against your hungry cheeks. Without warning his thrusts become sloppy and you feel his giant tool swelling inside you, stretching you out even more.\n\n");

        CView.text("Suddenly Akbal roars as he reaches his climax. You feel his giant cock hosing down your insides, filling you with his corrupted demon seed as he rides out his orgasm. His hips never stop. You feel your own orgasm rising to the surface only to suddenly fizzle out and you realize the corrupted seed inside you is actually stopping you from reaching climax. Akbal, however, sprays his spunk into your " + describeButthole(player.body.butt) + " again and again and never slows for a moment. Soon your stomach is obscenely swollen and you even taste cat jizz in your throat. Yet Akbal just keeps going, brutally fucking your helpless body and denying you release.\n\n");

        CView.text("After hours of being his toy you pass out, never having reached your own orgasm.");
        player.stats.lust += 10 + player.stats.lib / 10;
        player.stats.cor += 5 + randInt(10);
    ...
    // [Small/virgin pucker]
    if (player.body.butt.looseness < 3) {
        CView.text("Akbal begins to push into you, the barbs on his massive cockhead causing you to howl as your " + describeButthole(player.body.butt) + " is forcibly stretched.  His jaguar claws grab your shoulders, and he uses your body as leverage to force his demonic erection into you.\n\n");
        CView.text("The strain from Akbal's shoving makes you feel like you're going to pass out, yet the pain from his spiked sex organ keeps you conscious. After a while, you realize he's not even fucking you with his entire length; his swollen member is slowly forcing less than half its massive length into your " + describeButthole(player.body.butt) + ".\n\n");
        CView.text("After what seems like hours of Akbal's long cat dick being slowly forced into your " + describeButthole(player.body.butt) + ", your body finally gives out. You've become so exhausted from the strain that you can barely even lift your arms.  With a triumphant growl Akbal thrusts forward, his cockhead spikes burying themselves into you.  Without your resistance, they seem to vibrate inside you like twelve little beads, massaging your innards.  The sudden change makes you croon as you widen your " + describeLegs(player) + ", suddenly desperate for more.");
    } // [Medium pucker]
    else if (player.body.butt.looseness < 5) {
        CView.text("Akbal begins to push into you, the barbs on his massive cockhead causing you to wince as are forcibly stretched.  Without warning, he forces the entirety of his massive length into you in with a snarl.  You almost pass out just from the initial incursion, but as he saws his length in and out, your body reacts of its own accord. You widen your " + describeLegs(player) + " in an attempt to lessen the sudden slicing pressure created by the barbed cock head.  The moment you stop resisting, the barbs start to vibrate; they start to feel more like humming sex beads than the wicked-looking barbs you know are inside you.  You can't suppress the shuddering and groaning coming from your body as you are hit with a tidal wave of ecstasy.");
    }
    // [Gaping pucker]
    else {
        CView.text("Akbal begins to push into you, the barbs on his massive cockhead causing you to wince.  He bottoms out near-instantly, and he looks up at you with a feral grin.  Akbal begins to pump his engorged sex organ in and out of your " + describeButthole(player.body.butt) + " with a steadily mounting force.  You release a groan as the barbs covering Akbal's long cat-dick start to vibrate and begin to feel more like humming sex beads than anything else.  You widen your " + describeLegs(player) + " in response and Akbal snarls as he slams his hips into you, obviously happy that you're able to take his massive length.  The demon seems to forget that he's raping you; he starts giving your neck licks that send shivers throughout your entire body as he roughly fucks you into the forest floor.");
    }
    displayStretchButt(player, akbal.body.cocks.get(0)!.area, true);
    CView.text("\n\n");

    CView.text("Akbal's embedded cock begins to hum inside you, causing you to cry out as he picks up the pace.  His every thrust is a hammer-like thump against your spread " + describeLegs(player) + ".  Suddenly his thrusts become sloppy, and you feel his giant tool swelling inside you, stretching you out even more.  Akbal roars as he reaches his climax, and you feel his giant cock hosing down your insides, filling you with his corrupted demon seed as he rides out his orgasm.  His hips never stop.  You feel your own orgasm rising to the surface, only to suddenly fizzle out.  It is with horror that you realize that the corrupted seed inside you is actually stopping you from reaching climax.  Akbal, however, sprays his spunk into your " + describeButthole(player.body.butt) + " again and again, not slowing for even a moment.  Soon your stomach is obscenely swollen, and you even taste cat jizz in your throat.  Yet Akbal just keeps going, brutally fucking you into the ground.\n\n");

    CView.text("After hours of being his toy you finally pass out, never having reached your own orgasm.");
    player.stats.lust += 10 + player.stats.lib / 10;
    player.stats.cor += 5 + randInt(10);
    return { next: passTime(1) };
}
...
export function supahAkabalEdition(player: Character): NextScreenChoices {
    ...
    if (AkbalFlags.AKBAL_SUBMISSION_STATE === 2) {
        return repeatAkbalPostSubmission(player, akbal);
    }
    if (AkbalFlags.AKBAL_SUBMISSION_STATE === 1) {
        return ackbalRepeatAfterWin(player);
    }
    if (AkbalFlags.AKBAL_SUBMISSION_STATE === -1) {
        return ackbalRepeatAfterLoss(player, akbal);
    }

    // (Player should be level 4 or greater before encounter
    // chance is activated)
    CView.text("As you are walking through the forest, a twig suddenly snaps overhead.  You quickly look up towards the source of the noise, only to be met by a pair of glowing emerald eyes hidden in the shadowed canopy of a tree.  A demonic force sweeps over you, keeping you frozen in place as a single jaguar paw emerges from the darkness.  The graceful killer stalks across the branch of its tree and soon is fully exposed to you.  Bathed in the sunlight filtering through the trees overhead, the creature holds you within its gaze – a gaze far too intelligent to belong to a mere wild animal.  A chorus of whispering voices tickles your ear, but too quietly for you to make out what is being said.\n\n");

    CView.text("The jaguar blinks, releasing you from your trance, and the creature finally jumps down to the ground.  It widens its stance and unleashes a roar so loud that it seems to be coming from every direction at once, drowning out everything but the sound of your own heartbeat hammering away in the confines of your chest.\n\n");

    CView.text("The creature circles you once before you hear a deep male voice rise amongst the chorus of whispers.\n\n");

    CView.text("\"<i>I am Akbal, God of the Terrestrial Fire. You are trespassing on sacred ground... Submit, or die.</i>\"\n\n");

    CView.text("The aura pouring forth from this 'Akbal' is anything but god-like; you recognize the demon for what it truly is.  Yet its ivory teeth and sharp claws prove to you that it can make good on its threat.  What do you do?");
    // Talk / Fight / Run
    return { choices: [["Talk", choiceWrap(superAkbalioTalk, akbal)], ["Fight", startuAkabalFightomon], ["", undefined], ["", undefined], ["Leave", passTime(1)]] };
}

// [Talk]
function superAkbalioTalk(player: Character, akbal: Character): NextScreenChoices {
    CView.sprite(SpriteName.Akbal); // 2;

    CView.text("After a few moments of silence you ask, \"<i>What do you mean, 'submit'?</i>\" Akbal grins, revealing a row of wicked ivory teeth as he opens his mouth. You suddenly feel the demon's powerful body pinning you down, a wide tongue licking your neck and claws tickling your back in a way that is both horrifying and sensual. Yet after a moment of taking it in, you realize that he is still there in front of you, unmoved and grinning. You can guess what the image means: he wants you to become his mate for a day to make up for invading his territory.  What do you do?\n\n");

    // Submit / Fight
    return { choices: [["Fight", startuAkabalFightomon], ["Submit", choiceWrap(akbalSubmit, akbal)]] };
}

// [Encounter if previously submitted]
function repeatAkbalPostSubmission(player: Character, akbal: Character): NextScreenChoices {
    CView.sprite(SpriteName.Akbal); // 2;

    CView.text("As you walk through the forest, you hear a purring coming from behind you.  Turning around reveals that Akbal has come to find you.  He uses his head to push you in the direction of his territory, obviously wanting to dominate you again.\n\n");
    CView.text("What do you do?");
    // Submit / Deny / Fight
    return { choices: [["Submit", choiceWrap(akbalSubmit, akbal)], ["Deny", akbalDeny], ["Fight", startuAkabalFightomon]] };
}

// [Deny]
function akbalDeny(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Akbal); // 2;

    CView.text("You shake your head and rub the lust-filled jaguar behind the ear as you tell him you're busy.  The demon's eyes roll, and he licks your " + describeLeg(player) + " before his eyes find an imp in the trees above the two of you.\n\n");
    CView.text("Knowing he's found a new toy, Akbal allows you to leave unmolested.");
    return { next: passTime(1) };
}

// [Encounter if previously fought and won/raped him]
function ackbalRepeatAfterWin(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Akbal); // 2;

    CView.text("As you walk through the forest, you hear a snarl and look up just in time to dodge a surprise attack by the jaguar demon, Akbal.  Your ");
    if (player.body.legs.type === LegType.CENTAUR)
        CView.text("equine leap places you a good distance away from him.  Do you fight or flee?\n\n");
    else
        CView.text("dodging roll places you a good distance away from him.  Do you fight or flee?\n\n");
    // Fight / Flee
    return { choices: [["Fight", startuAkabalFightomon], ["", undefined], ["", undefined], ["", undefined], ["Leave", passTime(1)]] };
}

// [Encounter if previously fought and lost]
function ackbalRepeatAfterLoss(player: Character, akbal: Character): NextScreenChoices {
    CView.sprite(SpriteName.Akbal); // 2;

    CView.text("A chorus of laughter sounds inside your mind as the jaguar demon, Akbal, drops to the ground in front of you.  His masculine voice says, \"<i>Well, if it isn't the defiant welp who, in all their great idiocy, has wandered into my territory again.  Will you submit, or do I have to teach you another harsh lesson?</i>\"\n\n");

    // Submit / Fight / Run
    return { choices: [["Submit", choiceWrap(akbalSubmit, akbal)], ["Fight", startuAkabalFightomon], ["", undefined], ["", undefined], ["Leave", passTime(1)]] };
}

// [Fight]
function startuAkabalFightomon(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Akbal); // 2;

    CView.text("You ready your " + player.inventory.weapon.displayName + " and prepare to battle the demon jaguar.");
    // [battle ensues]
    AkbalFlags.PLAYER_RESISTED_AKBAL++;
    return CombatManager.beginBattle(new Encounter(player, new Akbal()));
}

// [Submit]
function akbalSubmit(player: Character, akbal: Character): NextScreenChoices {
    CView.sprite(SpriteName.Akbal); // 2;
    player.slimeFeed();
    AkbalFlags.AKBAL_SUBMISSION_COUNTER++;
    AkbalFlags.AKBAL_SUBMISSION_STATE = 2;
    AkbalFlags.AKBAL_BITCH_Q = -1;
    // Big booty special
    if (AkbalFlags.AKBAL_SUBMISSION_COUNTER > 5 && AkbalFlags.PLAYER_RESISTED_AKBAL < 2 && player.body.butt.rating >= 13 && player.body.tone < 80) {
        return akbalBigButtSubmit(player);
    }

    // Naga variant goez here
    if (player.body.legs.type === LegType.NAGA) {
        CView.image("akbal-deepwoods-naga-sumbitanal");
        CView.text("After a few moments of thinking you nod to Akbal and the masculine voice in your head commands you to disrobe. You take off your " + player.inventory.armor.displayName + ", setting it aside moments before the demon is upon you.\n\n");

        CView.text("Akbal pushes you face first into the ground and places his forward paws on your back, pinning your chest against the ground. He removes the paw and you attempt to reposition yourself only to have your body pushed back down, a silent command for you to stay in this position.  You can't help but squirm a little as your bottom half is bunched up under your abdomen, putting your " + describeButt(player) + " in the air.\n\n");

        CView.text("His paws slide to your waist, melting into hands along the way. You look from beneath your arm and watch as Akbal's body changes, becoming more humanoid.  Once his transformation is complete he gets on his knees behind you and roughly pulls your upturned bottom half back.  When he shoves his face into your " + describeButt(player) + " you flinch at the sudden and inexplicably erotic feeling.\n\n");

        CView.text("He works his slippery wet tongue into your " + describeButthole(player.body.butt) + ", greedily lapping at your exposed backside as if it were a quickly-melting ice cream cone. The sensation causes you to groan as you push back against the tongue, suddenly lost in ecstasy. A part of your long tail wraps around his waist, holding him there as you revel in the sensation of being rimmed by a pro.  You even arch your back, allowing his long, thick jaguar tongue to penetrate deeper into your " + describeButthole(player.body.butt) + ". You feel his saliva, thick and warm like melted candy, sliding inside of you and coating your insides.\n\n");

        // (transition)
        CView.text("A sudden warmth heats your innards, making you shiver in ecstasy.  Akbal takes a moment to uncoil your bottom half from around his chest before he rises to mount you. A single paw shoves your lifted chest and face back into the dirt, causing cold earth to cling to your body as Akbal gets into position above you.\n\n");

        // (Small/Virgin Pucker)
        if (player.body.butt.looseness < 3) {
            CView.text("You feel him poking around your " + describeButthole(player.body.butt) + " and quickly realize his member is not only insanely large but its head is covered in a dozen tiny barbs. You grit your teeth, expecting pain and yet, thanks to the weird saliva he slathered your innards with, there is none as his gargantuan member forcibly widens your " + describeButthole(player.body.butt) + ".\n\n");

            CView.text("The feeling of being stretched by Akbal's long, slimy member makes you shudder, the weird spit even heats up, creating a steamy warmth inside you as Akbal's equally hot member stretches you out and makes your body spasm slightly. After a few slow, shallow strokes you begin to feel the barbs vibrate. This vibrating drives you insane, and the wicked looking barbs feel more like humming sex beads than punishing spikes. When Akbal picks up the pace you grit your teeth as you are stretched beyond your natural limits.");
        }
        // (Medium Pucker)
        else if (player.body.butt.looseness < 5) {
            CView.text("You feel him poking around your " + describeButthole(player.body.butt) + " and quickly realize his member is not only quite large but covered in almost a dozen tiny barbs. Yet, thanks to the weird spit he slathered your innards with, there is none as his gargantuan member forcibly widens your " + describeButthole(player.body.butt) + ".\n\n");

            CView.text("Akbal's titanic member stretches your " + describeButthole(player.body.butt) + " and makes you groan beneath him, reveling in the slick heat and fullness of your bowels. His saliva heats up, creating a steamy,pleasurable warmth inside your body. As he begins to pump his huge sex organ in and out of you the barbs covering his head begin to vibrate and hit your body with tidal waves of unbearable pleasure, feeling more like vibrating sex beads than punishing spikes. Your body begins to act of its own accord, your " + describeButt(player) + " grinding against his thrusts as his large sex pump slides in with his trunk slamming into your " + describeButt(player) + " in rhythmic, echoing claps.");
        }
        // (Gapping Pucker - Akbal's dick = 15 inches)
        else {
            CView.text("You feel him poking around your " + describeButthole(player.body.butt) + " and quickly realize his member is not only quite large but covered in almost a dozen tiny barbs. When Akbal begins to penetrate you he is surprised when his trunk suddenly slams into your " + describeButt(player) + ", the sudden invasion causing you to croon.\n\n");

            CView.text("The weird spit he slathered your insides with begins to heat up instantly and the barbs covering his cock head start vibrating. The sensation of the vibrating barbs is like a dozen small slimy sex beads spinning and shaking as they are pushed inside you. Akbal wastes no time and begins forcibly fucking your " + describeButthole(player.body.butt) + " with reckless abandon, his every brutal thrust causing your body to slide forward through the dirt. You try to meet his deep thrusts but the jaguar fucks you with speed and force befitting a cheetah and the constantly vibrating barbs make your body shiver with each hammer blow to your insides.");
        }
        displayStretchButt(player, akbal.body.cocks.get(0)!.area, true);
        CView.text("\n\n");

        // (ending)
        CView.text("Akbal works his hips fast, piston-pumping his long demon-cat dick in and out of your " + describeButthole(player.body.butt) + ". Your voice rises and falls with his frantic thrusts. Your body is racked by orgasm after orgasm and soon you're lying on your chest and knees in a pool of your own love juices.\n\n");

        CView.text("Akbal releases a harsh growl and you feel the large feline member twitching and swelling inside you. A growl sounds in your own chest as the hot, corrupted seed of the demon cat shoots into you. Despite having reached his climax the jaguar's piston-pumping doesn't slow until he's erupted no less than six times, masterfully working your hole the entire time.\n\n");

        CView.text("After his last massive eruption you feel the jaguar demon pull out, releasing even more of his copious loads from your happy hole in an oddly satisfying cascade of thick white cream that rushes like a waterfall down your body.\n\n");

        CView.text("The jaguar demon no longer seems to mind your presence in his territory as he drapes his tired body over yours and the two of you fall into a sex-induced coma.");
        // [+ 4-12 Corruption]
        player.stats.cor += 4 + randInt(8);

        // [+ 1-2 Speed]
        player.stats.spe += 1 + randInt(2);

        player.orgasm();
        // [Chance of butt growth]
        if (player.body.butt.rating < 8) {
    ...
    if (player.body.legs.type === LegType.CENTAUR) {
        CView.image("akbal-deepwoods-taur-sumbitanal");
        CView.text("After a few moments of thinking you nod to Akbal.  The deep voice in your head commands you to disrobe.  You take off your " + player.inventory.armor.displayName + ", setting it aside while mentally preparing yourself for whatever this demon has in mind.\n\n");

        CView.text("You watch with fascination as Akbal rises onto his hind legs, his body melting into a more humanoid form.  His long, demonic cat-dick is already rock-hard and jutting out of his sheath.  He walks past your top half, moving around your body while sliding his hand across your haunch.  His hands are oddly warm.");

        // (if centaur player has a tail)
        if (player.body.tails.length > 0)
            CView.text("  As he makes his way to your hind quarters he grabs you by the tail, pulling it up and out the way with a feral, jaguar grin.  His free hand slides a finger across your " + describeButthole(player.body.butt) + " making you paw the ground with your hooves.");

        // (No tail)
        else
            CView.text("  As he makes his way to your hind quarters he grabs your " + describeButt(player) + ", groping the mass of flesh with one hand and sliding a finger across your exposed " + describeButthole(player.body.butt) + ", making you paw the ground with your hooves.");
        CView.text("\n\n");
        // (Transition)

        CView.text("The jaguar demon begins pushing you forward with his powerful hips, his demon-cat-dick sliding up the crevice of your " + describeButt(player) + ".  He steers you towards a tree, forcing you chest-first against the rough bark.\n\n");

        CView.text("Looking back you watch Akbal bend low, losing sight of the jaguar demon as he disappears behind you.  For a moment you wonder what he's doing but soon you feel him shove his face into your " + describeButt(player) + " with a snarl.  He begins working his slippery wet tongue into your " + describeButthole(player.body.butt) + ", greedily lapping at your exposed backside as if it were a quickly-melting ice cream cone.  The sensation causes you to groan and grind against the tongue, suddenly lost in ecstasy.  You are surprised at the strength in the demon's neck as he smashes your body against the tree with nothing but his mouth.  You feel his saliva, thick and warm like melted candy, sliding inside of you and coating your insides.");
        // (If Player has Vagina)
        if (player.body.vaginas.length > 0)
            CView.text("  Akbal slurps his way down to your " + describeVagina(player, player.body.vaginas.get(0)) + " twisting his face and drilling his tongue into you, mercilessly attacking your " + describeClit(player) + " as you scream, howl, and cringe in ecstasy.  He then uses his lips to gently suck your " + describeClit(player) + " into his mouth and twirl his tongue on it, making your grind your swollen sex against his jaguar lips.");
        // (If Player has balls)
        if (player.body.balls.count > 0)
            CView.text("  Akbal slurps his way down to your " + describeSack(player) + " where he slathers his thick, heated saliva over your orbs, making you groan as your sensitive " + describeBalls(true, true, player) + " are teased and gently juggled by Akbal's masterful tongue.  He sucks both orbs into his mouth.  The sensation sends your eyes to the back of your skull and makes your entire body shiver.");
        // (transition)
        CView.text("  Once his oral machinations are finished a sudden warmth heats your innards, making you shiver in ecstasy as the demon rises to mount you.\n\n");
        ...
        // (Medium Pucker)
            ...
            CView.text("You feel him poking around your " + describeButthole(player.body.butt) + " and quickly realize his member is not only quite large but covered in almost a dozen tiny barbs.  Yet,  thanks to the weird spit he slathered your innards with, there is none as his gargantuan member forcibly widens your " + describeButthole(player.body.butt) + ".\n\n");
            CView.text("Akbal's titanic member stretches your " + describeButthole(player.body.butt) + " and makes you groan and claw at the tree he has you pressed against, reveling in the slick heat and fullness of your bowels.  His saliva heats up, creating a steamy yet pleasurable warmth inside your body.  As he begins to pump his huge sex organ in and out of you the barbs covering his head begin to vibrate and hit your body with tidal waves of unbearable pleasure, feeling more like vibrating sex beads than punishing spikes.  You lean back into his thrusts as his trunk begins slamming into your " + describeButt(player) + " in rhythmic claps that echo throughout the forest.");
        }
        // (Gapping Pucker - Remember Akbal's dick is 15 inches)
        else {
            CView.text("You feel him poking around your " + describeButthole(player.body.butt) + " and quickly realize his member is not only quite large but covered in almost a dozen tiny barbs.  When Akbal begins to penetrate you he groans in surprise as his large dick sinks into your " + describeButt(player) + " easily, the sudden invasion causing you to croon.\n\n");
            CView.text("The weird spit he slathered your insides with begins to heat up instantly and the barbs covering his cock head start vibrating.  The sensation is like a dozen small slimy sex beads spinning and shaking as they are pushed inside you.  Akbal wastes no time and begins forcibly fucking your " + describeButthole(player.body.butt) + " with reckless abandon, his every brutal thrust causing the tree you're shoved up against to shake back and forth.  You try to meet his deep thrusts but the jaguar fucks you with speed and force befitting a cheetah and the constantly vibrating barbs make your body shiver with each hammer blow to your insides.");
        }
        displayStretchButt(player, akbal.body.cocks.get(0)!.area, true);
        CView.text("\n\n");

        // (ending)
        CView.text("Akbal works his hips fast, piston-pumping his long demon-cat dick in and out of your " + describeButthole(player.body.butt) + ".  Your body is racked by orgasm after orgasm and soon the ground beneath your pawing hooves is covered in a thick layer of your own love juices.  Then Akbal releases a harsh growl and you feel the large feline member twitching and swelling inside you.  A growl sounds in your own chest as the hot, corrupted seed of the demon cat shoots into you.  The feeling of the hot, wet warmth being fucked deeper into you is so close to supreme bliss you can scarcely tell the difference.\n\n");

        CView.text("After a few moments you realize Akbal isn't slowing down.  His piston pumping hips drive right through his orgasm and never stop slamming into your " + describeButt(player) + ".  He continues until he's erupted no less than eight times, masterfully working your hole the entire time.\n\n");

        CView.text("After his last massive eruption you feel the jaguar demon pull out, releasing even more of his copious load from your happy hole in an oddly satisfying cascade of thick white cream that rushes like a waterfall down your legs");
        if (player.body.cocks.length > 0)
            CView.text(" to join your own");
        CView.text(".\n\n");

        CView.text("You close your eyes, willingly falling into a sex induced sleep.");
        // [+ 4-12 Corruption]
        player.stats.cor += 4 + randInt(8);
    ...
    CView.image("akbal-deepwoods-sumbitanal");
    CView.text("After thinking for a minute, you nod to Akbal.  The deep voice in your head commands you to disrobe.  You obediently take off your " + player.inventory.armor.displayName + " and set it aside just before the demon is upon you.\n\n");
    ...
    CView.text("His paws slide to your waist, changing into hands along the way.  You look back and watch as his body starts morphing into a more humanoid shape, his muscles shifting themselves beneath his skin as his body changes to something better suited to dominate you.\n\n");

    CView.text("He suddenly yanks your upturned bottom half toward himself and shoves his face into your " + describeButt(player) + ".  His slippery wet tongue begins to work its way into your " + describeButthole(player.body.butt) + ", greedily lapping at your exposed backside as though it were a quickly-melting ice cream cone.  The sensation causes you to groan and grind against the tongue, quickly losing yourself into ecstasy.  You spread your " + describeLegs(player) + " and arch your back, allowing his long and thick jaguar tongue to drill deeper into your " + describeButthole(player.body.butt) + ".  You feel his thick and warm saliva sliding into you and coating your insides.\n\n");

    // [Player has a vagina]
    if (player.body.vaginas.length > 0) {
        CView.text("Akbal slurps his way down to your " + describeVagina(player, player.body.vaginas.get(0)) + ", twisting his face and drilling his tongue into you, mercilessly attacking your " + describeClit(player) + " as you scream, howl and cringe from the stimulation.  He then gently sucks your " + describeClit(player) + " into his mouth and twirls his tongue around it, making you grind your swollen sex against his jaguar lips.  ");
    }
    // [Player has balls]
    else if (player.body.balls.count > 0) {
        CView.text("Akbal slurps his way down to your " + describeSack(player) + ", slathering his heated saliva over your orbs and making you groan as your sensitive " + describeBalls(true, true, player) + " are teased and gently juggled by Akbal's masterful tongue.  Your body continually twitches with pleasure from the sensations.  ");
    }

    CView.text("His oral ministrations end when a sudden warmth heats your innards, and you shiver in ecstasy as the demon rises to mount you.  A single paw-like hand shoves your lifted chest and face back into the dirt, causing cold earth to cling to your face as Akbal gets into position above you.\n\n");

    CView.text("You feel him poking around your " + describeButthole(player.body.butt) + ", learning quickly that not only is his member insanely large, but its head is covered in dozens of tiny barbs.  ");
    // [Small/virgin pucker]
    if (player.body.butt.looseness < 3) {
        CView.text("You grit your teeth, expecting pain. However, thanks to the weird saliva he slathered your innards with, you feel none as his gargantuan member forcibly widens your " + describeButthole(player.body.butt) + ".");
        displayStretchButt(player, akbal.body.cocks.get(0)!.area, true);
        CView.text("\n\n");

        CView.text("Being stretched by Akbal's long and slick member makes you shudder. The weird spit even begins to heat up, creating a steamy warmth inside you as Akbal's equally hot member stretches you out, your body spasming slightly in response.  After a few slow and shallow strokes, you can feel the barbs begin to vibrate.  The sudden motion sends your body into convulsions, the wicked-looking barbs acting more like humming sex beads than barbs.  When Akbal picks up the pace, you can only grit your teeth harder as you're stretched more and more beyond your natural limits.\n\n");
    }
    // [Medium Pucker]
    else if (player.body.butt.looseness < 5) {
        CView.text("Thanks to the weird saliva he slathered your innards with, you feel no pain as his gargantuan member forcibly widens your " + describeButthole(player.body.butt) + ".");
        displayStretchButt(player, akbal.body.cocks.get(0)!.area, true);
        CView.text("\n\n");

        CView.text("Akbal's titanic member stretching your " + describeButthole(player.body.butt) + " makes you groan beneath him, reveling in the slick heat and the fullness of your bowels.  His saliva heats up, creating a steamy and pleasurable warmth inside your body.  As he begins to pump his huge member in and out of you, the barbs covering his head begin to vibrate.  Your body is hit with waves of unbearable pleasure, the wicked-looking barbs acting more like humming sex beads than barbs.  Your body begins to act of its own accord; your " + describeButt(player) + " grinds against his thrusts as his large sex slides in, his trunk slamming into your " + describeButt(player) + " with rhythmic claps that echo throughout the forest.\n\n");
    }
    // [Gapping Pucker - Remember Akbal's dick is 15 inches]
    else {
        CView.text("As Akbal begins to penetrate you, he groans in surprise at how easily his large dick sinks into your " + describeButt(player) + ", and the sudden invasion causes you to croon.\n\n");

        CView.text("The weird spit he slathered your insides with instantly heats up, and the barbs covering his cock head suddenly start vibrating.  The vibrating barbs feel like slimy sex beads, spinning and shaking as they are pushed inside you.  Akbal wastes no time and begins forcibly fucking your " + describeButthole(player.body.butt) + " with reckless abandon, his every brutal thrust causing your body to slide forward through the dirt.  You try to meet his deep thrusts, but the jaguar is fucking you with speed and force befitting a cheetah. The constantly vibrating barbs make your body shiver with each hammering blow to your insides.\n\n");
    }

    CView.text("Akbal works his hips fast, piston-pumping his long demon cat-dick in and out of your " + describeButthole(player.body.butt) + ".  The rampant babbling coming from your mouth breaks with his every thrust, and your body is racked by orgasm after orgasm. You're soon on your chest and knees ");
    if (player.body.vaginas.length > 0 || player.body.cocks.length > 0)
        CView.text("in a pool of your own love juices");
    else
        CView.text("in sexual bliss");
    CView.text(".\n\n");

    CView.text("Akbal releases a harsh growl and you feel his large feline member twitch and swell inside you.  You let out a growl of your own as the hot, corrupted seed of the demon cat shoots into you.  Feeling the hot, wet warmth being fucked deeper into you is so close to supreme bliss that you can scarcely tell the difference.\n\n");

    CView.text("After the sensation begins to settle, you realize Akbal isn't slowing down.  His hips carry on with their pistoning right through his orgasm and continually slam into your " + describeButt(player) + ".  He persists until he's erupted no less than eight times, masterfully working your hole the entire time.\n\n");

    CView.text("After his final massive eruption, the jaguar demon pulls out. His copious load is released from your happy hole in an oddly satisfying cascade of thick white cream, rushing like a waterfall down your " + describeLegs(player) + ".\n\n");

    CView.text("The jaguar demon no longer seems to mind your presence in his territory as he drapes his body over yours, and the two of you fall into a sex-induced sleep.");
...
export function akbalSubmissionFollowup(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Akbal); // 2;

    if (AkbalFlags.AKBAL_SUBMISSION_COUNTER < 4) {
    ...
    // (fighting Akbal disables this scene, but you retain the ability if you rape him after)
        ...
        if (player.stats.cor < 80 || player.effects.has(EffectType.FireLord)) {
            ...
            CView.text("You open your eyes and almost yell in surprise when you see Akbal's emerald eyes looking into yours.  You are still in the forest and his lithe jaguar body is still over you; you quickly realize he hasn't moved you, as you're still resting in a puddle of mixed sex juices.\n\n");
            CView.text("\"<i>You are a loyal pet,</i>\" Akbal says as he stands. The compliment makes you smile, but it quickly fades into a look of fear when he suddenly releases a bone-chilling roar right in your face.  Green flames begin to pour from his open maw, and you scream as you flail your hands in a pointless attempt to block the fire.\n\n");
            CView.text("After a moment of horror, you realize you aren't burning.  You can feel the emerald flames inside your lungs, glowing with a palpable warmth.  Akbal snaps his teeth together, a feral grin on his face as he halts the torrent of flame.\n\n");
            CView.text("You can feel Akbal's demonic presence inside your lungs, slowly building up until it finally explodes out of your open mouth in a titanic roar, accompanied with a jet of emerald flame.\n\n");
...
function akbalBigButtSubmit(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("akbal-deepwoods-bigbuttanaled");
    CView.text("Smiling in anticipation of servicing the jaguar-lord once more, you remove your [armor] and drop down to all fours, slowly lowering your face down to the ground.  You give your [butt] a slow back-and-forth wiggle as your cheek comes to rest on the dirt, degrading yourself for your demon-god's enjoyment.");
    ...
    if (player.body.balls.count > 0)
        CView.text(" and [balls]");
    else if (player.body.vaginas.length > 0)
        CView.text(" and [vagina]");
    CView.text(".  You cannot help but squirm from the attention, feeling the slippery-hot length of his muscle probing your sensitive ring.  In its wake, the panther's flexible tongue leaves behind copious amounts of his slick saliva that makes your skin tingle pleasantly.  You don't have long to wonder at this as he broadens his roving attentions to slather each of your cheeks with love, licking and lapping over your " + skinFurScales(player) + " in what could only be described as worship.  A tremor of gleeful pride works through your mind - your master approves of his servant's body!");
    ...
    if (player.body.cocks.length > 0)
        CView.text("  Bobbing beneath you, [eachCock] makes no secret of how much you're enjoying this, with thin drops of pre-cum dangling precariously below.");
    else if (player.body.vaginas.length > 0)
        CView.text("  Your vulva have long since grown bigger and puffier, visibly aroused and glistening with the moisture of your lust.");
    ...
    displayStretchButt(player, 16, true, true, false);
    ...
    if (player.body.cocks.length > 0)
        CView.text("  [EachCock] is drooling freely by this point, and though Akbal doesn't bother to touch, the feeling of his warm pole inside you seems to reach all the way to your [cockHead].");
    else if (player.body.vaginas.length > 0)
        CView.text("  Your [vagina] is dripping freely by this point, and though Akbal doesn't bother with it, the feelings radiating from his warm pole seem to reach all the way through your pulsing pussy to your [clit], rigid and soaked as it is.");
    CView.text("  Suddenly, that blessed tool departs from your over-sensitive innards, only to return a moment later, plunged back in with a firm, measured stroke.   You squeal as another precision stroke of the demonic jaguar shaft bottoms out into your [asshole] once more, the thrust teasing an orgasm from your pleasure wracked body");
    if (player.body.cocks.length > 0) {
        CView.text(", your [cock] freely unleashing its load into the mud beneath you and tainting the ground with your wasted cum");
        if (player.cumQ() >= 500) {
            CView.text(" as it puddles around you");
            if (player.cumQ() >= 1000) {
                if (player.cumQ() < 3000)
                    CView.text(" into a small lake");
                else
                    CView.text(" into a deep, slimy morass");
            }
        }
    }
    else if (player.body.vaginas.length > 0) {
        CView.text(", ");
        if (player.body.vaginas.get(0)!.wetness >= 4)
            CView.text("squirting ");
        else
            CView.text("leaking ");
        CView.text("into the dirt until it stinks of your pussy");
    }
    CView.text(".");
    ...
    if (player.body.cocks.length > 0)
        CView.text(", barely noticing when a fresh wave of your own spunk slops into your mouth");
    CView.text(".  You cum and cum, caught up in the rush of taking in Akbal's seed.  It seems to fill your body and soul, flooding anus and mind with hot, corrupted bliss.  Twice... thrice... four times... he just keeps firing more thick seed into you.  The fifth blast has you feeling full of it.  The sixth makes your gut distend slightly.  The seventh bloats you further and squirts from your [asshole] to soak butt and crotch-fur alike.  The eight is the largest and last, flooding you completely with your lord's essence.  It inflates your belly into a rounded sperm tank.  When he pulls back, a river of it washes out, and you cum again.");
    CView.text("\n\nSated, you're lifted up and moved to a ");
    if (player.body.cocks.length > 0 && player.cumQ() >= 1000)
        CView.text("dry, ");
    CView.text("comfortable place.  Akbal wraps his arms around you, though one steals a quick squeeze of your [butt].  Invigorated by this, he docks his cat-cock back inside your spunk-oozing asshole to plug it and snuggles up to you.  You fall asleep like that, dreaming of him taking you again and again.");
    if (player.body.butt.rating < 20) {
        CView.text("  Tingling in your sleep, your [butt] bulges slightly as it grows bigger, changed by Akbal's saliva to serve him even more capably.");
        player.body.butt.rating++;
    }
    else if (player.body.tone > 30) {
        CView.text("  Tingling in your sleep, your [butt] jiggles slightly as it softens along with the rest of your body, changed by Akbal's saliva to be a softer, more pleasant fuck.");
        displayModTone(player, 30, 5);
    }
    player.orgasm();
    player.stats.cor += 5;
...
function akbitchEncounter(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("As you explore the deep woods you begin to hear a soft slurping sound. In this world you know that any strange sound, especially the wet ones, most likely means something dangerous is up ahead... or something dangerous is fucking something a little less dangerous.  As you cautiously advance you spy the pelt of the jaguar demon, Akbal.  The demon jaguar sits in the middle of the clearing with one leg extended as he repeatedly swipes his wide tongue against his hole, probably cleaning up imp spunk thanks to you.  He is so utterly focused on the task that he doesn’t notice your approach.");
    AkbalFlags.AKBAL_BITCH_Q = 1;
    // {corruption < 40/choose no}
    if (player.stats.cor < 40 || player.stats.lust < 33)
        return akbitchNoThnx(player, false);
    // {corruption > 40}
    else {
        CView.text("\n\nDo you take advantage of him again?");

        return { choices: [["Yes", takeAdvantageOfAkbitch], ["No", choiceWrap(akbitchNoThnx)]] };
    }
}

function akbitchNoThnx(player: Character, clear: boolean = true): NextScreenChoices {
    if (clear)
        CView.clear();
    else
        CView.text("\n\n");
    CView.text("You turn back, allowing the demon to finish cleaning himself and thankful he didn’t ambush you this time.");
    if (player.stats.lust < 33)
        CView.text("  Besides, you aren't aroused right now, anyway.");
    return { next: passTime(1) };
}

// {Choose Rape}
export function takeAdvantageOfAkbitch(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You creep behind the many woods trees surrounding Akbal’s clearing until your eyes chance upon a vine.  It’s spongy, long, and hard to rip apart - in other words: perfect.");
    ...
    if (player.stats.str < 50) {
        ...
        akbal.combat.action.subActions.find((action) => action.name === "Special")!.use(akbal, player);
        return CombatManager.beginBattle(new Encounter(player, akbal));
    }
    // {strength > 50}
    ...
    if (player.stats.int < 60) {
        ...
        const akbal: Akbal = new Akbal();
        akbal.combat.action.subActions.find((action) => action.name === "Attack")!.use(akbal, player);
        return CombatManager.beginBattle(new Encounter(player, akbal));
    }
    // {intelligence > 60}
    ...
    AkbalFlags.AKBAL_TIMES_BITCHED++;

    const choices: ScreenChoice[] = [];
    choices[0] = ["Normal", basicAkbitchScene];
    // AMB Strength Scene
    // 70+
    if (player.stats.str >= 70)
        choices[1] = ["Strong", akbitchHighStrengthVariant];
    // AMB Speed Scene
    // 70
    if (player.stats.spe >= 70)
        choices[2] = ["Fast", akbalBitchSpeed];
    // AMB Toughness Scene
    // 70
    if (player.stats.tou >= 70)
        choices[3] = ["Toughness", akbitchToughness];

    return { choices };
}

// Basic AMB Scene (no +70 stats)
function basicAkbitchScene(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("With a grin, you tug on Akbal’s collar, and he lets out a barely suppressed purr.  ");
    if (AkbalFlags.AKBAL_TIMES_BITCHED === 1)
        CView.text("The smile on your [face] spreads even wider as the unexpected sound tells you you’ve turned this demonic sexual predator into your own personal slut.  As if to confirm this, h");
    else
        CView.text("H");
    CView.text("e lifts his tail, giving you a perfect view of his entire package, from his self-lubing sphincter to his full balls, and rock-hard, demon-cat dick.  Through lust, his will has been broken, and now he is yours.");
    CView.text("\n\nWhat will you do with him?");

    const choices: ScreenChoice[] = [];
    if (player.body.cocks.length > 0)
        choices[0] = ["Fuck Him", buttFuckbuttFuckbuttFuckAkbal];
    choices[1] = ["Ride Him", topAkbitchFromDaBottom];

    return { choices };
}

// Butt Fuck - Vaginal - Anal
function buttFuckbuttFuckbuttFuckAkbal(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("akbal-deepwoods-male-buttfuck");
    // [if (hasCock = true)]
    if (player.body.cocks.length > 0)
        CView.text("You widen your stance as you sink into the demon’s moist depths with a grin.");
    if (player.body.cocks.length > 1)
        CView.text("  You widen your stance and look down, choosing the biggest among [eachCock].");
    CView.text("  You lift your [cock biggest] into position, and with a grin, you begin to push into the demon’s moist depths.");

    const largestCock = player.body.cocks.sort(Cock.Largest).get(0)!;

    // {if penis < 7 inches}
    if (largestCock.length < 7)
    ...
    return { next: fuckAkbitchsButt };
}

// - page turn -
function fuckAkbitchsButt(player: Character): NextScreenChoices {
    CView.clear();
    const largestCock = player.body.cocks.sort(Cock.Largest).get(0)!;
    CView.image("akbal-deepwoods-male-buttfuck2");
    CView.text("Without warning, Akbal’s insides become vacuum tight.  Convulsions rocket through his body and you realize he has reached his climax without any attention to his barbed dick.  ");
    if (player.stats.cor < 90)
        CView.text("You hold the demon’s make-shift collar, letting him ride out his orgasm as he cries out in pure ecstasy, reveling in the satisfaction of knowing you have made him your bitch.");
    else
        CView.text("You pound your bitch’s quivering rectum, reveling in the pulsating pressure as Akbal roars beneath you, a sound of mixed pain and ecstasy as you decimate his clenching hole with brutal thrusts.");
    ...
    else if (largestCock.type === CockType.HORSE)
    ...
    else if (largestCock.type === CockType.TENTACLE)
    ...
    else if (largestCock.type === CockType.CAT)
...
function topAkbitchFromDaBottom(player: Character): NextScreenChoices {
    ...
    return { next: topAkbitchFromBottomDuex };
}

function topAkbitchFromBottomDuex(player: Character): NextScreenChoices {
    CView.clear();
    CView.image("akbal-deepwoods-male-akbalonback2");
    // -page turn-
    // {tight/virgin vag/ass}
    if ((player.body.vaginas.length <= 0 && player.body.butt.looseness < 2) || (player.body.vaginas.length > 0 && player.body.vaginas.get(0)!.looseness <= 2))
        CView.text("As you begin to impale yourself on the demonic pole, you gasp as your [vagOrAss] is spread wide by Akbal’s rather large demon-cat dick.  The remnants of his spit on your [vagOrAss] reacts to the mystic lube and any pain you would have felt is gone, replaced by mind numbing bliss.  The demon begins to thrust up into your [vagOrAss], which you cannot help but allow.  The sensation is beyond ecstasy - it is pure euphoria, exploding like a firecracker and blasting through you like a wave.  All too soon the pleasure peaks.");
    // {medium vag/ass}
    else if ((player.body.vaginas.length <= 0 && player.body.butt.looseness < 4) || (player.body.vaginas.length > 0 && player.body.vaginas.get(0)!.looseness < 4))
        CView.text("As you impale yourself on the demon’s quivering dick, a moan finds itself coming out of your mouth.  The moment the remnants of saliva in your [vagOrAss] touch the lube coating his barbed cock, you see stars.  Lost in the moment, you cannot believe anything can feel this good.  You ride up and down the entire length of the demon’s damned sex organ.  It’s almost as if you’re bouncing up and down on a rod of pure bliss, like your entire being is being impaled by pure joy.  All too soon the pleasure peaks.");
    // {gapped pussy}
    else
        CView.text("As you slide down the demon’s quivering dick your eyes roll back.  As the demonic saliva coating your [vagOrAss] comes into contact with the lube on his barbed, demon-cat dick, you cannot believe anything can feel this good.  Suddenly, you are bouncing as hard as you can while the demon thrusts up into you.  Your bodies slam together causing a thunderous applause to ring out across the clearing as you ride him as hard as you can.  The combination of the size and thickness of his ribbed, textured dick combined with the chemical stimulants makes your entire being cry out.  You feel as if you are drowning in ecstasy, as if an unending paradise is blooming inside you from your [vagOrAss].  All too soon the pleasure peaks.");
    if (player.body.vaginas.length > 0)
        displayStretchVagina(player, 16, true, true, false);
    else
        displayStretchButt(player, 16, true, true, false);
    ...
    if (player.body.cocks.length > 0) {
        ...
        if (player.body.vaginas.length > 0)
            CView.text("both ");
        CView.text("your sex organ");
        if (player.body.vaginas.length > 0)
            CView.text("s climax at once");
        else
            CView.text(" climaxes");
        CView.text(".  Your [vagOrAss] spasms around the demon’s embedded cock as [eachCock] paints his chest and face with a generous coating of baby batter.  Your orgasm rages on, covering the demon cat in your sex fluids, matting his fur and darkening it with more seed than you ever thought possible.");
    }
    // [if (hasCock = false)
    else
...
// AMB Strength Scene
// 70+
function akbitchHighStrengthVariant(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("With a wicked grin, you rip off your [armor] and grab the bound demon by the scruff on his neck.  He does this sexy little wiggle as you hoist him until he reaches eye level, easily manipulating his light weight as you inspect his slim, toned body.  His chest heaves, his nipples stand at attention, and his erect demon-cat dick drools a heavy river of thick cream, darkening the fur on his sack and inner thighs.  This is going to be fun.");
    if (player.body.cocks.length > 0) {
        CView.image("akbal-deepwoods-male-highstrength");
        // if (hasCock = true)
        ...
        if (largestCock.length > 12)
            CView.text("another");
        else
            CView.text("a");
        CView.text(" titanic orgasm onto your [fullChest].");
        ...
        else if (largestCock.type === CockType.DRAGON)
        ...
        else if (largestCock.type === CockType.LIZARD)
        ...
        CView.image("akbal-deepwoods-female-highstrength");
        ...
        if (player.body.vaginas.length > 0)
            CView.text("nether lips");
        else
            CView.text("entrance");
        CView.text(" with his hot saliva you flinch.  A jolt, like lightning, shoots through your body and causes you to smash yourself into his face.  His long, thick tongue is hot bliss, teasing and pleasing you like a pro.  He uses his black lips to tease your ");
        if (player.body.vaginas.length > 0)
            CView.text("clit");
        else
            CView.text("prostate");
        CView.text(" and drills his long tongue into you, carrying that euphoria inducing spit into your [vagOrAss] over and over.  Your body grinds against the sensation as the good little bitch’s saliva drenches your [vagOrAss] until it’s dripping with a combination of your sex and his spit.  Here you keep him until the teasing pleasure is so intense you’re fit to burst.");
        ...
        // {medium vagorAss}
        if ((player.body.vaginas.length <= 0 && player.body.butt.looseness < 4) || (player.body.vaginas.length > 0 && player.body.vaginas.get(0)!.looseness < 4))
        ...
        // [if (hasCock = true) \"<i>
        if (player.body.cocks.length > 0)
            CView.text("  [EachCock] swells and explodes, shooting cream all over Akbal’s thighs, chest and face as you unload thick white rope after rope.");
...
// AMB Speed Scene
// 70
function akbalBitchSpeed(player: Character): NextScreenChoices {
    CView.clear();
    ...
    // [if (hasCock = true)]
    if (player.body.cocks.length > 0) {
        CView.image("akbal-deepwoods-male-highspeed");
        ...
        if (largestCock.length < 7) {
            ...
            // [if (hasBalls = false)
            if (player.body.balls.count === 0)
                CView.text("  Your [cock biggest] drips gobs of his slick lube down the lower half of your body.");
            else
                CView.text("  Your [cock biggest] drips gobs of his slick lube down your [sack] and the insides of your legs.");
            CView.text("  With clenched teeth you spur yourself to fuck the demon even faster, reaching your top speed.");
        }
    ...
    // [if (hasVagina = true)
        ...
        CView.image("akbal-deepwoods-female-highspeed");
        ...
        if (player.body.vaginas.length > 0)
            ...
            CView.text("prostate");
        CView.text(" with his long tongue, you begin to shiver.  Then you remember what it is you wanted to do.  Before he makes you cum, you shove his face away from you, pushing him onto his back with his cock once again standing tall.  Unable to contain yourself and draw this out any longer, you straddle the more than willing Jaguar demon and lower yourself onto his twitching erection without a second thought.");
...
function akbitchToughness(player: Character): NextScreenChoices {
    ...
    // [if (hasCock = true)]
    if (player.body.cocks.length > 0) {
        CView.image("akbal-deepwoods-male-hightoughness");
        ...
        if (largestCock.length > 12)
            ...
            CView.text("a");
        CView.text(" blast of corrupted seed.");
    ...
    // {female/genderless}
        ...
        if (player.body.vaginas.length > 0)
            ...
            CView.text("prostate");
        CView.text(" with his long tongue.  Your body convulses slightly as you maintain a tight grip on the collar, keeping his face buried under the weight of your [vagOrAss] as that wonderful tongue sends you to paradise.");
        ...
        if (player.body.thickness <= 33)
            CView.text("slim");
        else if (player.body.thickness <= 66)
            CView.text("muscular");
        else
            CView.text("voluptuous");
        CView.text(" body until he reaches your [fullChest].  After a second of feeling around he finds your [nipples].");
        ...
        // [if (hasCock = True)
        if (player.body.cocks.length > 0)
            CView.text("  [EachCock] erupts at the same time, painting Akbal’s chest with white goo.  As each orgasm overtakes you, you are reduced to a quivering mess.  Say what you want about the little bitch, but his dick is like no other.");
        CView.text("  As your orgasm subsides you allow the demon to grind his still erect dick around inside you for a moment. Without needing to threaten him again you rise, sending a cascade of spooge down your legs and onto Akbal’s lap.");
