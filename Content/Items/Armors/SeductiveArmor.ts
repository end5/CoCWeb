...
export class SeductiveArmor extends Armor {
    ...
    public useText(character: Character): void {
        if (!ceraphIsFollower()) {
            ...
            for (const nipple of character.inventory.piercings.nipples)
                nipple.equip(new Piercing(PiercingName.Stud, "seamless black nipple-studs", "Seamless black nipple-studs"));
    ...
    public onEquip(character: Character): Armor {
        const underClothes: Armor = ItemDict.getByName(ArmorName.ComfortUndercloth);
        if (underClothes) {
