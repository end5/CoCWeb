...
import { describeCockShort, describeCocksLight } from 'Content/Descriptors/CockDescriptor';
...
export class Arouse extends BlackMagic {
    ...
    public isPossible(character: Character): boolean {
        return character.effects.has(EffectType.KnowsArouse);
    }
    ...
    public calcDamage(character: Character, monster: Character): void | IActionDamage {
        return { lust: monster.stats.lustVuln * (character.stats.int / 5 * character.combat.spellMod() + randInt(monster.stats.lib - monster.stats.int * 2 + monster.stats.cor) / 5) };
    }
