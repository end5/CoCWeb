...
export class UrtaQuest extends NPCAwareContent {
    ...
    private comfortUrtaAfterFamFamTalk(): void {
        ...
        if (this.urta.urtaDrunk()) this.outx("boozy ");
        this.outx(
    ...
    private whoCaresUrtaAfterFamFamTalk(): void {
        this.clearOutput();
        this.outx('"<i>Who cares?</i>" you ask, without a hint of compassion.');
        this.outx(
    ...
    public infertilityQuestions(): void {
        ...
        if (this.player.cor < 33)
            ...
            this.outx("she's definitely a handful in the sack, you've got to give her that.");
        else
            this.outx(
    ...
    public startUrtaQuest(): void {
        ...
        this.outx("<b>You are Urta, a hermaphroditic fox-morph.</b>");
        this.outx(
    ...
    private fuckHelAndKitsunesAsUrta(): void {
        ...
        if (this.player2.hasCock()) {
            this.outx("  \"<i>I've got a better one ");
    ...
    private putUrtasWangInPCsCunt(truth: boolean): void {
        ...
        else if (this.player2.tone >= 65) this.outx("her toned abdomen");
        else if (this.player2.tone < 30) this.outx("her soft, forgiving middle");
        else this.outx("her belly");
        this.outx(
        ...
        if (this.player2.isTaur()) {
            ...
            else this.outx("  You stroke her swollen sex as you admire it");
            this.outx(
        ...
        // {goo}
            ...
            if (this.player2.clitLength < 0.5) this.outx("tiny ");
            else if (this.player2.clitLength >= 2.5) this.outx("massive ");
            this.outx(
        ...
        // {biped}
            ...
            if (this.player2.wetness() < 3) this.outx("every drop of nectar that she gives up");
                ...
                this.outx("the copious drops of nectar she gives up");
            else if (this.player2.wetness() < 5)
                this.outx("the drizzling moisture that oozes from her twat");
            else this.outx("the rivers of girl-lube that run unimpeded from her sopping twat");
            this.outx(
        ...
        if (this.player2.pregnancyIncubation > 0 && this.player2.pregnancyIncubation < 150)
            this.outx(" pregnant");
        this.outx(" dome, until at last you finally spend the last of your spunk.");
    ...
    private snuggleWithUrta(truth: boolean): void {
        ...
        if (this.player2.cor < 33) this.outx("playful pat");
    ...
    public winFuckAGoblinBroodmotherAsUrta(): void {
        this.clearOutput();
        this.outx("You greedily eye the ");
    ...
    private loseToGoblinsPartIIAsUrta(): void {
        this.clearOutput();
        this.outx("<b>Hours, and many orgasms later...</b>");
        this.outx(
    ...
    private loseToGoblinsPartIIIAsUrta(): void {
        this.clearOutput();
        this.outx("<b>Several days later...</b>");
    ...
    private urtaGameOver(): void {
        this.clearOutput();
        this.outx("<b>Urta has been lost to her fate...  Meanwhile, back at camp...</b>");
    ...
    public useSuccubiMilkOnGnollPrincesses(): void {
        ...
        if (this.flags[kFLAGS.URTA_FUCKED_HEL] == 1) this.outx("that salamander slut's cunt");
        else this.outx("a mino-cum addicted whore's sodden cunt");
        this.outx(
    ...
    private loseToGnollAsUrtaEpilogue(): void {
        ...
        if (this.player.HP < 1) this.outx("I hope I wasn't too rough on you");
        else this.outx("I hope you enjoyed the journey here");
        this.outx('.</i>"');
        ...
        this.outx('"<i>Good girl,</i>" she says encouragingly.  ');
        if (this.player.HP >= 1)
            this.outx(
    ...
    public winRapeAMinoLordAsUrta(): void {
        this.clearOutput();
        this.outx("Exulting in your victory, you quickly undress, barely paying heed to the ");
        if (this.monster.lust > 99) this.outx("lusty");
        else this.outx("pained");
    ...
    public urtaLosesToMinotaurRoughVersion(): void {
        this.clearOutput();
        this.outx("You collapse to your knees, too ");
        if (this.player.HP < 1) this.outx("battered and bruised");
        else this.outx("weak from lust");
        this.outx(" to continue fighting.  The hulking bull-man snorts as he approaches you, ");
        if (this.monster.lust < 70)
            this.outx("barely deigning to give your far smaller cock a contemptuous look, ");
        this.outx(
    ...
    public milkyUrtaTic(): void {
        this.outx("Your body heats up with the effects of the succubus-cow milk...  ");
        if (this.player.lust < 50) this.outx("Thankfully it hasn't become too distracting yet.");
    ...
    public urtaLosesToCowCubi(): void {
        this.clearOutput();
        this.outx("Sinking to your knees, ");
        if (this.player.HP < 1)
            this.outx(
        ...
        if (this.player.HP < this.player.maxHP()) this.outx(" and healing your wounds");
        this.outx(
    ...
    private urtaArrivesAtCampForFukks(): void {
        this.clearOutput();
        this.outx("<b><u>Meanwhile, back at camp...</u></b>");
        this.outx(
        ...
        else this.outx("if you'd rather not");
    ...
    private knockUpUrtaWithGodChild(): void {
        ...
        this.outx(".  Her nipples dig into yours");
        if (this.player.hasFuckableNipples()) {
            this.outx(", kissing into the ");
            if (this.player.lactationQ() > 50) this.outx("milky, ");
            this.outx(
        ...
        if (this.player.cumQ() > 1000) this.outx(", belly rounded with wobbly, womb-packing spunk");
        this.outx(".");
        this.outx(
    ...
    private postFuckUrtaUp(): void {
        ...
        if (this.camp.followersCount() >= 2)
            this.outx("leave instructions to your fellow camp-dwellers to look after the place, ");
        this.outx(
    ...
    private preggedUrtaWithGodChildEpilogue(): void {
        ...
        if (this.player.cor > 66) this.outx(", what do you care, anyway?");
        else this.outx(".");
        this.outx(
    ...
    private getKnockedUpWithGodChild(): void {
        ...
        if (this.player.balls > 0) this.outx(" and lifting your balls");
        this.outx(", exposing your puffy lips, and spreading them wide to expose your ");
        if (this.player.vaginaType() != 5) this.outx("pink, ");
        else this.outx("dusky, ");
        this.outx(
        ...
        if (this.player.biggestTitSize() > 1) this.outx("soft curve");
        else this.outx("flat muscles");
        this.outx(" until she hits your [nipple].  ");
        if (this.player.hasFuckableNipples())
            this.outx("Her tongue plunges inside, savoring the feminine lubricants");
        else
            this.outx(
        ...
        if (this.player.vaginalCapacity() < 60) this.outx("a little painfully");
        else this.outx("deliciously");
        this.outx(
        ...
        if (this.player.hasCock()) {
            this.outx(", your own forgotten length");
            if (this.player.cockTotal() > 1) this.outx("s");
            this.outx(" laying on your stomach, bouncing fitfully but ultimately ignored.");
        } else
            this.outx(
        ...
        if (this.player.balls > 0) this.outx("both sets of");
        else this.outx("her");
        ...
        // {If Jojo/Amily pet}
            ...
            if (this.jojoScene.campCorruptJojo()) this.outx("Jojo");
            else this.outx("Amily");
            this.outx(
        ...
        this.outx("  The hungry mouth between her legs wastes no time in cleaning them.");
        this.outx(
    ...
    private getKnockedUpUrtaEpilogue(): void {
        this.clearOutput();
        this.outx("When you wake, you're ");
        if (this.player.pregnancyIncubation > 0 && this.player.pregnancyIncubation < 150)
            this.outx("much more ");
        this.outx("pregnant ");
        if (this.player.pregnancyIncubation > 0 && this.player.pregnancyIncubation < 150)
            this.outx("than before");
        else this.outx("- very pregnant");
        this.outx(
    ...
    private urtaAndEdrynGodChildEpilogueII(): void {
        this.clearOutput();
        this.outx("As the double doors open, you hear a painful moan from the ");
        // {pre-pregnant }

        this.outx("horse-girl.");
        ...
        this.outx("  Edryn's eyes immediately close, sinking into a deep, restful sleep.");
        this.outx(
