import { ANAL_LOOSENESS_STRETCHED, ANAL_WETNESS_NORMAL, BUTT_RATING_TIGHT, HIP_RATING_BOYISH, LOWER_BODY_TYPE_HOOFED, WING_TYPE_IMP } from "../../../includes/appearanceDefs";
...
export class ImpLord extends Imp {
    ...
    protected impLordHeavyEncounter(): void {
        var damage: number = Math.floor((this.str + this.weaponAttack + 20) - ImpLord.rand(this.player.tou) - this.player.armorDef);
        this.outputText("The demonic creature slashes a clawed hand towards your stomach,");
        if (this.combatMiss() || this.combatEvade() || this.combatFlexibility() || this.combatMisdirect()) this.outputText(" but you handily avoid it.");
    ...
    protected performCombatAction(): void {
        var choices: any[] = [this.impFire, this.impLordLustAttack2, this.impLordLustAttack, this.impLordHeavyEncounter, this.eAttack];
