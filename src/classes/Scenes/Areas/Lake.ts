...
export class Lake extends BaseContent {
    ...
    public exploreLake(): void {
        ...
        if (this.flags[kFLAGS.PC_PROMISED_HEL_MONOGAMY_FUCKS] == 1 && this.flags[kFLAGS.HEL_RAPED_TODAY] == 0 && Lake.rand(10) == 0 && this.player.gender > 0 && !kGAMECLASS.helScene.followerHel()) {
        ...
        //Egg chooser
        if (Lake.rand(100) < 25 && this.player.pregnancyIncubation > 1 && this.player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS) {
            this.clearOutput();
        ...
        if (this.player.level >= 2 && this.player.findStatusAffect(StatusAffects.DungeonShutDown) >= 0)
            choice[choice.length] = 3;
        //Slimes/Ooze = level >= 2
        if (this.player.level >= 2)
            choice[choice.length] = 4;
        //Izma
        if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00230] > 0 && (this.player.exploredLake >= 10) && (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00233] == 0 || this.player.findStatusAffect(StatusAffects.Infested) < 0) && this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00238] <= 0)
        ...
        //UNCOMMON EVENTS
        //Goo finding!
        if (Lake.rand(30) == 0 && this.flags[kFLAGS.GOO_TFED_MEAN] + this.flags[kFLAGS.GOO_TFED_NICE] > 0 && this.flags[kFLAGS.GOO_SLAVE_RECRUITED] == 0) {
        ...
        //Sword Discovery
        if (this.player.findStatusAffect(StatusAffects.TookBlessedSword) < 0 && this.player.findStatusAffect(StatusAffects.BSwordBroken) < 0)
        ...
        //Chance of dick-dragging! 10% + 10% per two foot up to 30%
            ...
            if (this.player.findStatusAffect(StatusAffects.FactoryOverload) >= 0 && Lake.rand(10) == 0) {
            ...
            if (this.player.findStatusAffect(StatusAffects.DungeonShutDown) >= 0 && this.player.findStatusAffect(StatusAffects.FactoryOverload) < 0)
                girlOdds += 25;
            ...
            //OOZE!
                ...
                if (this.player.inte >= 25) {
                    ...
                    if (this.player.findStatusAffect(StatusAffects.DungeonShutDown) >= 0 && this.player.findStatusAffect(StatusAffects.FactoryOverload) < 0) this.outputText("\n\n<b>You are amazed to encounter a slime creature with the factory shut down - most of them have disappeared.</b>", false);
                    return;
                }
            ...
            //No boat, no kaiju
            if (this.player.level >= 5 && this.flags[kFLAGS.KAIJU_DISABLED] == 0 && this.player.findStatusAffect(StatusAffects.BoatDiscovery) >= 0) {
            ...
            if (this.player.cor > 30 || this.player.lust > 60 || this.player.lib > 40) this.outputText("Your imaginings increasingly seem to turn ", false);
    ...
    private eggChoose(eggType: number): void {
        ...
        switch (eggType) {
            case 0: this.outputText("brown"); break;
            case 1: this.outputText("purple"); break;
            case 2: this.outputText("blue"); break;
            case 3: this.outputText("pink"); break;
            case 4: this.outputText("white"); break;
            default: this.outputText("black"); break;
        }
    ...
    private ottahGirl(): void {
        ...
        if (this.flags[kFLAGS.MET_OTTERGIRL] == 1) {
            ...
            if (this.player.findStatusAffect(StatusAffects.FactoryOverload) < 0) this.outputText("crystal clear, giving you a stunning view of the lakebed");
            else this.outputText("only slightly clouded, giving you an obscured view of the lakebed");
            ...
            //Silly Mode:
            if (this.silly()) this.outputText("  No tragic backstory to go through? No annoying combat encounter? Just meet and fuck?  My god, what has this world come to?");
    ...
    private ottergirlLikesDongs(): void {
        ...
        while (this.temp < this.player.cockTotal()) {
            if (this.player.cocks[this.temp].cockLength < 48) {
                if (y < 0) y = this.temp;
                else if (this.player.cocks[this.temp].cockLength > this.player.cocks[y].cockLength) y = this.temp;
            }
            this.temp++;
        }
        ...
        //[equine]
        if (this.player.cocks[x].cockType == CockTypesEnum.HORSE) this.outputText("to your flare, sucking, teasing and ");
        //[canine]
        else if (this.player.hasKnot(x)) this.outputText("to the base of your cock, planting sloppy kisses on your knot, ");
        //[demonic]
        else if (this.player.cocks[x].cockType == CockTypesEnum.DEMON) this.outputText("to the demonic nodules ringing your cock, ");
        //[anemone]
        else if (this.player.cocks[x].cockType == CockTypesEnum.ANEMONE) this.outputText("to the little wriggling tentacles ringing the head and base of your cock, ");
        else this.outputText("to the sensitive little spot on the underside of the head, ");
        ...
        if (this.player.balls > 0 && this.player.hasVagina()) this.outputText("balls and cunt");
        ...
        if (this.player.cumQ() >= 250) {
            ...
            if (this.player.cumQ() < 500) this.outputText("some of it begins to dribble down your cock, forming a puddle right under your ass cheeks");
            else if (this.player.cumQ() < 1000) this.outputText("it begins to spray out of the edges of your cock, like water coming out of a blocked faucet");
            else if (this.player.cumQ() < 2000) this.outputText("Callu's stomach begins to visibly inflate, even from your point of view");
            else this.outputText("Callu's stomach inflates to a huge degree. She suddenly looks to be about eight months pregnant, though she doesn't seem bothered by this in the least");
            this.outputText(".");
        }
        ...
        if (this.player.cumQ() >= 2000) this.outputText(", despite the huge belly she now sports");
        ...
        if (this.player.hasKnot(x)) this.outputText("fails, held fast by the knot tying the two of you together. She looks at you in surprise, but eventually smiles and leans back down. The two of you cuddle for half an hour, until your knot deflates enough to let her *pop* off of it. She stands and ");
    ...
    private ottersForGals(): void {
        ...
        else this.outputText("unnatural bare crotch");
        ...
        if (this.player.hasVagina()) this.outputText("quivering quim");
        else this.outputText("rump");
        ...
        if (this.player.hasFuckableNipples()) this.outputText("You even slip a finger or two inside, stretching your nipple-cunts out with deliciously pleasurable results.  ");
        this.outputText("Combined with Callu's tender tongue ");
        if (this.player.hasVagina()) this.outputText("paying lip service to your wet cunt");
        else this.outputText("doing a cave dive in your rear");
        ...
        if (this.player.hasVagina()) this.outputText("Your snatch clenches and clamps down on thin air, flooding Callu's face with your feminine juices in a tasty, refreshing spray.");
        else this.outputText("Your asshole clenches and spasms randomly, aching to be filled by something, anything in your quest for release.");
