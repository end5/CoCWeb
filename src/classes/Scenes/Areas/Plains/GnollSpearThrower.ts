...
import { VAGINA_WETNESS_DROOLING, VAGINA_LOOSENESS_LOOSE, ANAL_LOOSENESS_STRETCHED, ANAL_WETNESS_DRY, HIP_RATING_AMPLE, BUTT_RATING_TIGHT, SKIN_TYPE_FUR } from "../../../../includes/appearanceDefs";
...
export class GnollSpearThrower extends Monster {
    private hyenaPhysicalAttack(): void {
        ...
        if (this.findStatusAffect(StatusAffects.Blind) >= 0 && GnollSpearThrower.rand(3) < 2) {
            ...
            if (this.statusAffectv1(StatusAffects.Attacks) == 0) this.removeStatusAffect(StatusAffects.Attacks);
        ...
        //("Misdirection"
        if (this.player.findPerk(PerkLib.Misdirection) >= 0 && GnollSpearThrower.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
        ...
        if (damage <= 0) {
            ...
            if (GnollSpearThrower.rand(this.player.armorDef + this.player.tou) < this.player.armorDef) this.outputText("The gnoll before you suddenly charges, almost too fast to see.  Twin fists slam into your " + this.player.armorName + " with enough force to stagger you, but the force is absorbed without doing any real damage.  As jaws powerful enough to crush bone flash at your neck, you are able to twist to the side, letting the furious hyena slip by you.", false);
            else this.outputText("You deflect and block every " + this.weaponVerb + " " + this.a + this.short + " throws at you.", false);
        }
        else {
            if (damage < 10) this.outputText("The gnoll runs forward, fury in her dark eyes as twin fists glance off your chest.  The glancing blow sends her off balance and the flashing ivory jaws barely miss your throat.  You push back, stumbling away from the furious hyena. (" + damage + ")", false);
            else this.outputText("The gnoll rushes forward, almost too fast to detect before twin fists slam into your torso.  Before you can recover, ivory jaws flash before your eyes and you feel the sharp teeth start to clamp onto the " + this.player.skinDesc + " of your neck.  Blinding pain causes you to fling yourself backwards, away from the teeth and drawing angry scrapes as you escape the jaws.  You roll away before picking yourself up, the hyena moving confidently towards you as you try to shake off the pain from the blow. (" + damage + ")", false);
        }
        if (damage > 0) {
    ...
    private hyenaJavelinAttack(): void {
        ...
        //("Misdirection"
        else if (this.player.findPerk(PerkLib.Misdirection) >= 0 && GnollSpearThrower.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
            this.outputText("Using Raphael's teachings, you anticipate and sidestep " + this.a + this.short + "' thrown spear.\n", false);
        }
        //Determine if cat'ed
        else if (this.player.findPerk(PerkLib.Flexibility) >= 0 && GnollSpearThrower.rand(100) < 6) {
        ...
        //<Hyena Attack 2 – Javelin – Successful – Player Entangled>
            ...
            if (this.player.findStatusAffect(StatusAffects.GnollSpear) < 0) this.player.createStatusAffect(StatusAffects.GnollSpear, 0, 0, 0, 0);
            slow = 15;
    ...
    private hyenaSnapKicku(): void {
        ...
        //Determine damage - str modified by enemy toughness!
            ...
            //<Hyena Attack 3 – Snap Kick – Successful>
                ...
                if (this.player.biggestTitSize() >= 1) this.outputText(", sending a wave of pain through the sensitive flesh", false);
    ...
    public constructor() {
        ...
        this.drop = new ChainedDrop().add(this.consumables.GROPLUS, 1 / 5).add(this.consumables.INCUBID, 1 / 2).elseDrop(this.consumables.BROWN_D);
