...
export class CorruptedDrider extends AbstractSpiderMorph {
    ...
    public driderKiss(): void {
        ...
        //Dodge
            ...
            if (temp == 0) this.outputText("I just wanted to give my delicious morsel a kiss...</i>\"\n", false);
            else if (temp == 1) this.outputText("Why won't you let me kiss you?</i>\"\n", false);
            else if (temp == 2) this.outputText("Mmm, do you have to squirm so much, prey?</i>\"\n", false);
            else this.outputText("Just look at my glossy, dripping lips.  Imagine how great it would feel to have them locked against you.  Why resist?</i>\"\n", false);
        }
        //Determine if evaded
        ...
        //("Misdirection"
        else if (this.player.findPerk(PerkLib.Misdirection) >= 0 && CorruptedDrider.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
        ...
        //Get hit 2nd time)
            ...
            if (this.player.statusAffectv1(StatusAffects.DriderKiss) == 1) {
                ...
                if (this.player.hasCock()) this.outputText("  " + this.SMultiCockDesc() + " spews a rope of pre-cum into your " + this.player.armorName + ", desperate to get out and fuck.", false);
                if (this.player.hasVagina()) this.outputText("  Fem-cum dribbles down your " + this.player.legs() + " while your " + this.player.clitDescript() + " gets so hard you think it'll explode.", false);
                this.outputText("  This time, the drider is the one to break the kiss.  She asks, \"<i>Are you ready, my horny little morsel?</i>\"\n", false);
                if (this.player.lust <= 99) this.outputText("You shake your head 'no' and stand your ground!\n", false);
            }
            //(Get hit 3rd+ time)
            else {
                ...
                if (this.player.hasCock() || this.player.hasVagina()) {
                    ...
                    if (this.player.wetness() < 3 && this.player.cumQ() < 200) this.outputText("small", false);
                    else if (this.player.wetness() < 5 && this.player.cumQ() < 500) this.outputText("large", false);
                    ...
                    if (this.player.lust <= 99) this.outputText("  Amazingly, you resist her and pull back, panting for breath.", false);
                }
            }
        }
    ...
    protected performCombatAction(): void {
        ...
        else if (this.game.hasSpells() && this.player.findStatusAffect(StatusAffects.WebSilence) < 0 && CorruptedDrider.rand(4) == 0) {
            this.spiderSilence();
        }
        //1/4 chance of disarm
        else if (this.player.findStatusAffect(StatusAffects.Disarmed) < 0 && this.player.weaponName != "fists" && CorruptedDrider.rand(4) == 0) {
            this.spiderDisarm();
        }
        //Always web unless already webbed
        else if (this.player.spe >= 2 && (this.player.findStatusAffect(StatusAffects.Web) < 0 || CorruptedDrider.rand(2) == 0)) {
    ...
    public constructor() {
        ...
        this.drop = new WeightedDrop().add(this.consumables.B_GOSSR, 5)
