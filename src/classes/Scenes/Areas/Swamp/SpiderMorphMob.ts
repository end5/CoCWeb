import { ANAL_LOOSENESS_STRETCHED, ANAL_WETNESS_SLIME_DROOLING, BUTT_RATING_LARGE, HIP_RATING_AMPLE, TAIL_TYPE_SPIDER_ADBOMEN, VAGINA_LOOSENESS_LOOSE, VAGINA_WETNESS_SLICK } from "../../../../includes/appearanceDefs";
...
export class SpiderMorphMob extends Monster {
    ...
    private spiderStandardAttack(): void {
        ...
        if (this.findStatusAffect(StatusAffects.MissFirstRound) >= 0 || this.combatMiss() || this.combatEvade() || this.combatFlexibility() || this.combatMisdirect()) {
            this.removeStatusAffect(StatusAffects.MissFirstRound);
            this.outputText("A number of spiders rush at you, trying to claw and bite you.  You manage to beat them all back, though, with some literal covering fire from Kiha.", false);
        }
        //SPIDER HORDE ATTACK - Hit
        else {
            this.outputText("A number of spiders rush at you, trying to claw and bite you.  You manage to knock most of them away, but a few nasty hits manage to punch through your [armorName].  ", false);
            //Determine damage - str modified by enemy toughness!
            var damage: number = Math.floor((this.str + this.weaponAttack) - SpiderMorphMob.rand(this.player.tou) - this.player.armorDef) + 20;
            if (damage > 0) damage = this.player.takeDamage(damage);
            if (damage <= 0) {
                damage = 0;
                if (SpiderMorphMob.rand(this.player.armorDef + this.player.tou) < this.player.armorDef) this.outputText("You absorb and deflect every " + this.weaponVerb + " with your " + this.player.armorName + ".", false);
            ...
            else if (damage < 11) this.outputText(this.capitalA + this.short + " wounds you! (" + damage + ")", false);
            else if (damage < 21) this.outputText(this.capitalA + this.short + " staggers you with the force of " + this.pronoun3 + " " + this.weaponVerb + "! (" + damage + ")", false);
            else if (damage > 20) {
    ...
    private kihaSPOIDAHAI(): void {
        ...
        this.outputText("While they're tangled up with you, however, Kiha takes the opportunity to get in a few shallow swings with her axe, to the accompaniment of crunching chitin.", false);
        //horde loses HP
        this.HP -= 50;
        this.combatRoundOver();
    }
    ...
    protected performCombatAction(): void {
        this.game.spriteSelect(72);
        if (SpiderMorphMob.rand(2) == 0 || this.player.findStatusAffect(StatusAffects.UBERWEB) >= 0) this.spiderStandardAttack();
