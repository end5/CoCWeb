...
export class Essrayle extends BaseContent {
    ...
    public essrayleMeetingI(): void {
        ...
        if (this.flags[kFLAGS.MET_ESSY] == 0) {
            ...
            if (this.player.cor < 33) this.outx("Despite however chivalric you think yourself");
            else if (this.player.cor < 66) this.outx("Despite your better sense");
            else this.outx("Perhaps best of all");
            this.outx(
                ", your attention falls to her chest.  This emerald maiden sports a pair of breasts that'd make even a demon blush, gorgeous green globes that look like small watermelons resting upon her upper belly, capped with purple areolae.  Four, thick, teat-like nipples dangle a good three or four inches from each."
            );
            ...
            if (this.player.cor < 33) this.outx("hastily looking away from her nude body");
            else if (this.player.cor < 66) this.outx("looking away out of courtesy");
            else
                this.outx(
                    "letting your eyes roam over her as you search for even more depraved features on her nubile body"
                );
            this.outx(".");
            ...
            if (this.player.biggestTitSize() >= 10) this.outx("own");
            else this.outx("[chest]");
            this.outx(
                '.  "<i>You must be one of those adventurers I\'ve been hearing about, am I right?</i>"'
            );
    ...
    private plantsForMe(): void {
        ...
        if (this.player.biggestTitSize() > 1)
            this.outx("Your nipples stiffen, breasts puffing out with a heated breath");
        else
            this.outx(
                "You arch your back gently, unconsciously pressing into her slender fingers a bit further"
            );
        this.outx(
            ' as her hand begins to circle your chest.  "<i>Though you, that\'s another story.</i>"  She grins seductively as the air around you seems to thicken with a gentle pink mist.  Out of instinct, you look around, trying to discern the source - could she be one of the demons?'
        );
        this.outx(
        ...
        if (this.player.breastRows[0].breastRating >= 1) {
            ...
            if (this.player.bRows() > 1) this.outx(" of your uppermost pair");
            this.outx(
                ".  You moan, and this only seems to encourage her, as she takes your nipples between thumb and forefinger, twisting and turning them this way and that.  She giggles at how your moans seem to fluctuate by coincidence, continuing to tease them"
            );
            if (this.player.bRows() > 1)
                this.outx(", steadily working over the multitude of fleshy mounds on your body");
            this.outx(".");
            if (this.player.hasFuckableNipples())
                this.outx(
                    "  Her fingers play with your nipples and press gently to tease at them.  She seems quite surprised as they sink in with minimal effort.  A grin spreads over her face as she begins to steadily thrust those digits in and out of your nipples, fingers swirling about the interiors all the while."
                );
            this.outx(
    ...
    private plantsForMe2(BE = 0): void {
        ...
        // If No=
        else if (BE == 2) this.outx('"<i>Heh, suit yourself.</i>"\n\n');
        ...
        if (this.player.hasCock() && this.player.hasVagina())
            this.outx(
                "find the dual sexes there.  Another hand joins the first, one gripping and tugging on your hardening cock as the other cups and squeezes over your pussy."
            );
        else if (this.player.hasCock()) {
            this.outx("circle about ");
            if (this.player.cockTotal() > 1) this.outx("one of ");
            this.outx("your shaft");
            if (this.player.cockTotal() > 1) this.outx("s");
            this.outx(
                ", beginning to squeeze and slide about its length until your nursing is interrupted by moans of your own delight"
            );
        } else
            this.outx(
                "stroke about the puffy, now-inflamed lips of your own flower, taking the lips between two fingers and stroking up and down across their length fully before switching to your moist core."
            );
        ...
        this.outx(
            " and pulls you close, forcing your [legs] against her bulb.  They sink in slightly as you go down, reluctantly relinquishing her nipple from your mouth.  As it springs free, a thick glob of sap splatters over your lips and nose.  Essy giggles softly as she wipes them off with a hand."
        );
        ...
        this.outx("and [legs], ");
        if (!this.player.isTaur()) this.outx("lifting you slowly into the air");
        else this.outx("forcing you to rear up");
        this.outx(
        ...
        if (this.player.biggestTitSize() >= 1) {
            ...
            if (this.player.lactationQ() >= 400) this.outx("your milk and ");
            this.outx(
                "your pleasure.  In time, their pumping increases in force and tempo just as her lips and tongue do."
            );
        }
        ...
        if (this.player.cumQ() >= 500 && this.player.cumQ() < 2000)
            this.outx(
                "  It takes some time for her to completely drain your virile reserves, humming happily as splurt after splurt bulges her cheeks and forces her throat to work.  By the time you start to go dry, you can swear the bulb that is her base looks a little bit bigger, and perhaps her breasts as well."
            );
        else if (this.player.cumQ() >= 2000)
            this.outx(
                "  This is a bit hard, as your virile cum-spout renders her trim belly bloated, and the bulb that forms her base rapidly swells with growth thanks to your fecund deposits.  The seed even trickles out around the corners of her mouth as she greedily attempts to devour it all, sighing around each cheek-stretching ejaculation."
            );
        ...
        if (BE == 1) {
            ...
            if (this.player.totalNipples() > 2) this.outx("s have");
            else this.outx(" has");
            this.outx(" been swallowed up as well.");
            ...
            if (this.player.bRows() > 1)
                this.outx(
                    "  She eventually dislodges with a wet pop, setting your new breasts to wobbling about as she moves down your body, repeating the gestures upon each set of breasts in return."
                );
    ...
    private feedTrappedEssy(): void {
        this.clearOutput();
        this.outx(
            "Well, it's not like she's going anywhere right now, and she probably would love a good fuck from a friend anyway."
        );
        this.outx(
        ...
        if (this.player.gender == 3) {
            this.outx("  How best to make use of this floral beauty?");
            this.menu();
    ...
    private hasCockFeedEssy(newPage = true): void {
        ...
        if (this.player.biggestCockLength() < 20) this.outx("her lower pair of wobbling udders");
        else this.outx("both sets of milk-bloated breasts");
        this.outx(
            " faster and faster, the hot slapping of her bountiful bosom against your [cock]"
        );
        if (this.player.balls > 0)
            this.outx(" and your swollen, heavy balls against her fecund belly");
        this.outx(" setting an inspiring drum beat to your feverish pace.");
    ...
    private acceptEssyPrizes(): void {
        ...
        if (this.player.biggestTitSize() <= 1) {
            this.outx("four bulging tits erupt from your chest");
        ...
        // if two breasts:
            ...
            this.outx("two more bulging tits erupt from your chest");
            if (this.player.breastRows[0].breastRating < 4) {
                this.outx(" while your top row expands");
                this.player.breastRows[0].breastRating = 4;
            }
            if (this.player.bRows() == 1) this.player.createBreastRow();
        ...
        this.player.boostLactation(this.player.bRows());
        this.outx(
            ", filling with warm, fluid weight.  They jiggle and wobble against each other and your belly as they adjust to their brimming plumpness.  You now proudly sport [fullChest]."
        );
    ...
    private noGimmeGiantNipplesEssy(): void {
        this.clearOutput();
        this.outx(
            'She shrugs her shoulders, giving one bundle of milk-drooling nipples a soft squeeze.  "<i>Eh, to each their own, I suppose.</i>"'
        );
        this.essyRewardEpilogueOUTTIES();
    }
    ...
    public askMotherToReleaseEssy(): void {
        ...
        if (this.flags[kFLAGS.SAND_WITCHES_COWED] == 0) {
            this.outx(
                "You point out that the witches have a friend of yours trapped here with magic and you'd like her released."
            );
            this.outx(
            ...
            this.outx(
                'You command the Sand Mother to release your friend, Essrayle from her magical bondage.  The sorcerous queen looks about to protest, but after a brief pause, she admits, "<i>Fine, I\'ll see her released.  You can go pick her up.</i>"  She practically spits the last sentence.'
            );
            this.outx("\n\nThat'll do.");
        }
