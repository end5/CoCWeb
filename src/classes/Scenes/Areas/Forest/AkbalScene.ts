import { LOWER_BODY_TYPE_CENTAUR, LOWER_BODY_TYPE_NAGA, TAIL_TYPE_NONE } from "../../../../includes/appearanceDefs";
...
export class AkbalScene extends BaseContent {
    ...
    public akbalDefeated(hpVictory: boolean): void {
        ...
        } else //[Victory via Lust]
        {
            ...
            if (this.player.lust >= 33 && this.player.gender > 0) {
                ...
                if (this.player.hasVagina() && this.player.biggestTitSize() >= 4 && this.player.armor instanceof LustyMaidensArmor)
    ...
    public akbalWon(hpVictory: boolean, pcCameWorms: boolean): void {
        this.flags[kFLAGS.AKBAL_SUBMISSION_STATE] = -1;
        this.flags[kFLAGS.AKBAL_BITCH_Q] = 0;
    ...
    private loseToAckballllllz(): void {
        ...
        //Centaurs R SPECIALZ
            ...
            this.player.buttChange(new Akbal().cockArea(0), true);
            this.outputText("\n\n", false);
    ...
    public supahAkabalEdition(): void {
        ...
        this.monster.cocks[0].cockType = CockTypesEnum.HUMAN;
        if (this.flags[kFLAGS.AKBAL_BITCH_Q] >= 2) {
            this.akbitchEncounter();
            return;
        }
        if (this.flags[kFLAGS.AKBAL_SUBMISSION_STATE] == 2) {
            this.repeatAkbalPostSubmission();
            return;
        }
        if (this.flags[kFLAGS.AKBAL_SUBMISSION_STATE] == 1) {
    ...
    private akbalSubmit(): void {
        ...
        this.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER]++;
        this.flags[kFLAGS.AKBAL_SUBMISSION_STATE] = 2;
        this.flags[kFLAGS.AKBAL_BITCH_Q] = -1;
        //Big booty special
        if (this.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] > 5 && this.flags[kFLAGS.PLAYER_RESISTED_AKBAL] < 2 && this.player.buttRating >= 13 && this.player.tone < 80) {
    ...
    public akbalSubmissionFollowup(): void {
        this.spriteSelect(2);
        this.outputText("", true);
        if (this.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] < 4) {
        ...
        else if (this.flags[kFLAGS.PLAYER_RESISTED_AKBAL] == 0 && this.flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] >= 8 && this.player.cor > 80) {
            if (this.player.cor < 80 || this.player.findPerk(PerkLib.FireLord) >= 0) {
                this.outputText("You awake in your camp feeling dangerous, powerful and fiercely satisfied.", false);
            }
            else {
        ...
        else if (this.player.findPerk(PerkLib.Whispered) < 0 && this.player.cor >= 40) {
    ...
    private basicAkbitchScene(): void {
        ...
        if (this.flags[kFLAGS.AKBAL_TIMES_BITCHED] == 1)
    ...
    private akbitchHighStrengthVariant(): void {
        ...
        if (this.player.hasCock()) {
            ...
            else if (this.player.cocks[x].cockType == CockTypesEnum.DRAGON)
            ...
            else if (this.player.cocks[x].cockType == CockTypesEnum.LIZARD)
