...
export class Kitsune extends Monster {
    ...
    public kitsuneStruggle(): void {
        ...
        //Success
        if (Kitsune.rand(20) + this.player.str / 20 + this.statusAffectv1(StatusAffects.PCTailTangle) >= 12) {
    ...
    private illusionKitsuneAttack(): void {
        ...
        if (this.player.findPerk(PerkLib.HistoryReligious) >= 0 && this.player.cor < 20) resist += 20 - this.player.cor;
    ...
    protected performCombatAction(): void {
        var moves: any[] = [this.foxFireAttack, this.foxFireAttack, this.kitSuneTeases, this.kitSuneTeases];
        if (this.player.findStatusAffect(StatusAffects.Sealed) < 0) moves.push(this.kitsuneSealAttack);
        if (this.player.findStatusAffect(StatusAffects.Sealed) < 0) moves.push(this.kitsuneSealAttack);
        if (this.findStatusAffect(StatusAffects.PCTailTangle) < 0) moves.push(this.kitsuneEntwine);
        if (this.findStatusAffect(StatusAffects.Illusion) < 0) moves.push(this.illusionKitsuneAttack);
