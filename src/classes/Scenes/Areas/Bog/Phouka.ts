...
export class Phouka extends Monster {
    ...
    protected performCombatAction(): void {
        ...
        if ((!blinded) && this.player.findStatusAffect(StatusAffects.WebSilence) < 0 && Phouka.rand(4) == 0) {
            ...
            if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_FAERIE) {
                if (blinded) transformChance = Phouka.rand(3); //100% chance of change from blind phouka if not doing silence attack
            ...
            else if ((PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_BUNNY) && (Phouka.rand(4) != 0) && (!blinded))
                this.phoukaFightLustAttack(); //Bunny has a 75% chance of teasing attack, no teasing while blind
            else if ((PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_HORSE) && (Phouka.rand(4) == 0) && (!blinded))
