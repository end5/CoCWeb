import { ANAL_LOOSENESS_NORMAL, ANAL_WETNESS_DRY, BUTT_RATING_LARGE, HIP_RATING_AMPLE, SKIN_TYPE_PLAIN, VAGINA_LOOSENESS_LOOSE, VAGINA_WETNESS_SLAVERING } from "../../../../includes/appearanceDefs";
...
export class ChameleonGirl extends Monster {
    ...
    public chameleonClaws(): void {
        ...
        //Evade:
        else if (this.game.combatMiss() || this.game.combatEvade() || this.game.combatFlexibility() || this.game.combatMisdirect()) this.outputText("The chameleon girl's claws slash towards you, but you lean away from them and they fly by in a harmless blur.");
        //Get hit
        else {
            var damage: number = Math.floor((this.str + this.weaponAttack) - ChameleonGirl.rand(this.player.tou));
            if (damage > 0) {
    ...
    public rollKickClawWhatTheFuckComboIsThisShit(): void {
        ...
        //Get hit
            ...
            var damage: number = Math.floor((this.str + this.weaponAttack) - ChameleonGirl.rand(this.player.tou) - this.player.armorDef) + 25;
            if (damage > 0) {
                damage = this.player.takeDamage(damage);
