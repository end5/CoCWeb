...
export class Salon extends BaseContent implements TimeAwareInterface {
    ...
    private buyMinoCum(): void {
        if (this.player.gems < 60) {
            this.outx("You can't afford any minotaur cum right now!", true);
            if (this.flags[kFLAGS.SALON_PAID] == 0) this.doNext(this.salonGreeting);
            else this.salonPurchaseMenu();
        } else {
            this.player.gems -= 60;
            this.outx(
                "You happily give Lynnette 60 gems and pick up the bottle full of glistening, heavenly cum.  ",
                true
            );
    ...
    public salonPurchaseMenu(): void {
        ...
        if (this.player.femininity < 100 && this.player.gender == 2) mudFacial2 = this.mudFacial;
            ...
            (this.player.gender == 0 || this.player.gender == 3)
        )
            mudFacial2 = this.mudFacial;
        else if (this.player.femininity < 70 && this.player.gender == 1)
            mudFacial2 = this.mudFacial;
        else if (this.player.femininity < 100 && this.player.findPerk(PerkLib.Androgyny) >= 0)
            mudFacial2 = this.mudFacial;
        if (this.player.femininity > 0 && this.player.gender == 1) sandFacial2 = this.sandFacial;
            ...
            this.player.femininity > 20 &&
            (this.player.gender == 0 || this.player.gender == 3)
        )
            sandFacial2 = this.sandFacial;
        else if (this.player.femininity > 0 && this.player.findPerk(PerkLib.Androgyny) >= 0)
            sandFacial2 = this.sandFacial;
    ...
    private hairDresserGreeting(): void {
        ...
        this.outx(
            "   \"<i>I apologize for my daughters,</i>\" she says as she presses herself against you.  \"<i>They're a bunch of brainless hussies for the most part.  My name is Lynnette, and welcome to my salon!  You want your hair cut or lengthened?  We've got you covered, and we don't ask for much - just a shot of cum.",
            false
        );
        if (this.player.cockTotal() == 0) {
            this.outx(
                "  You look like you don't got any of your own, but we've got glory holes in the back if you need to get some.  Just don't swallow too much, ok?</i>\"\n\nShe shows you to the back of the cave, which is boarded-up.  There are about 20 holes in boards, and most are empty.  While you watch, a few new dicks slide in, and just as quickly the goblin's daughters commence sucking and fucking them.  There are only a few you could take a crack at - do you blow one (and if so which one)?",
                false
            );
        } else {
            this.outx(
                "  I'll even do you the favor of letting you blow it in my mouth, I've already got a bun in the oven.  So what do you say?  Want a spooge and a haircut?  Or would you rather go get your payment from one of the gloryholes in the back, you " +
                    this.player.mf("kinky boy", "naughty girl") +
                    '?</i>"\n\n',
                false
            );
            this.outx(
                "In the very back of the salon you can see a boarded-up wall with holes cut in it, some of which are currently plugged by various monstrous penises.  Do you let the goblin blow you, or do you go suck your payment from one of them?",
                false
            );
        }
    ...
    private hairDresserRepeatGreeting(): void {
        ...
        if (
            ...
            this.outx("As soon as you enter the Salon, Lynnette is beaming ");
            if (this.player.tallness >= 48) this.outx("up ");
            this.outx(
                'at you. "<i>Hey there, honey. Come for some more service or to check up on the kids?</i>" She pats her now-flat midsection. "<i>I popped them out last night. We had ' +
                    Salon.num2Text(this.flags[kFLAGS.LYNNETTE_CARRYING_COUNT]) +
                    ' kids... all girls, of course.</i>" She tucks a lock of beautifully styled hair behind an elfin ear. "<i>Now you be sure and let me know when you want to contribute to my baby-bank again.</i>"'
            );
            if (this.flags[kFLAGS.LYNNETTE_BABY_COUNT] > 0)
                this.outx(
        ...
        // Non-Pregnant Greetings:
            ...
            if (this.flags[kFLAGS.LYNNETTE_MET_UNPREGNANT] == 0) {
                ...
                this.outx(
                    "Lynnette's familiar face greets you at the door to her salon once more, and she waves you in with a beatific smile plastered upon her face. Clinging to her like a second skin, the businessgoblin's dress highlights her surprisingly flat midsection; she's not pregnant!"
                );
                this.outx(
        ...
        // Standard repeats
            ...
            if (this.player.cockTotal() == 0) {
                this.outx(
                    "I see you still can't make payment for our services on your own.  No worries, the glory holes in the back are pretty popular with the demons and monsters, just go back there and catch some spooge and we'll see about helping with your hair, ok?</i>\"\n\n(There are a number of dicks in the glory hole, which do you want to deal with (if you're willing at all)?",
                    false
                );
            } else {
                this.outx(
                    'Will you be getting our normal services?  Just one BJ per hair treatment!  Or would you rather get it from the gloryholes in back?</i>"',
                    false
                );
            }
        }
    ...
    private gloryholeImp(): void {
        ...
        this.outx("", true);
        this.outx(
            "You walk over to the hole in the wall, looking at the erect demon-member you'll have to service.  Judging by the height and constant bobbing up and down it does, the imp must be hovering on the other side, trying pretty damn hard to stay in the hole.\n\n",
            true
        );
    ...
    private gloryholeIncubus(): void {
        ...
        this.outx("", true);
        this.outx(
            "You kneel down in front of the throbbing demonic dick, ready to earn your pay.\n\n",
            false
        );
        if (this.player.cor < 33) {
            this.outx(
    ...
    private gloryholeMinotaur(): void {
        this.player.slimeFeed();
        this.outx(
            "Your eyes are drawn to the huge minotaur cock, and you instinctively kneel down in front of it.\n\n",
            true
        );
        this.outx(
        ...
        this.outx(
            "Driven on by the minotaur's reactions, you take more and more of the minotaur cock into your mouth. As you work your mouth around the first ring, you open up the top section of your " +
                ...
                ". With each pass, you're able to take more and more of the bull dick down your throat. You caress your " +
                this.biggestBreastSizeDescript() +
                " while moaning around the dick. When you get your mouth around the second ring, the minotaur's cock throbs sharply, and his cum explodes in your mouth. Your eyes bulge in shock, and you pull off the cock as quickly as you can. You swallow the first several shots of thick, rich minotaur cum, while the rest shoots all over your face and tits. You "
        );
        ...
        this.outx(
            "Your head spins from the minotaur's musk, and you idly mop up and swallow the cum on your " +
                this.player.face() +
                ". A goblin aide comes in with a bowl, and gently scrapes the cum off your tits with a smooth, flat rock. Once you're cleaned up and you're dressed, the aide leads you back to Lynnette.\n\n",
            false
        );
    ...
    private goblinHairDresserFacefuck(): void {
        ...
        this.outx(
            "Lynnette licks her lips and practically tears her way into your " +
                this.player.armorName +
                ", having your crotch exposed in seconds.  Your " +
                this.cockDescript(0) +
        ...
        else this.outx("around your first few inches");
        this.outx(
        ...
        this.outx(
            "It's impossible to hold back while receiving such expertly given stimulation, and you blow your load, bulging her cheeks out like a squirrel's.",
            false
        );
        if (this.player.cumQ() >= 250)
            this.outx(
                " She can't hold it all and suddenly starts swallowing as runnels of it escape her nose and a wave of it rolls down her lips.",
                false
            );
        this.outx(
            "  In time it ends, and she pops back, spitting most of the cum into a funnel.  It washes down a pipe and you have to wonder if there's some horny goblin girl at the other end with her cunt spread wide.",
            false
        );
        this.outx(
    ...
    private cutShort(): void {
        ...
        this.outx(
            "Lynnette and her daughters crowd around you with razor-sharp scissors, effortlessly paring down your " +
                this.hairDescript() +
                ".  When they've finished, you're left with ",
            true
        );
        this.player.hairLength = 1;
        this.outx(this.hairDescript() + ".", false);
    ...
    private hairGrow(): void {
        ...
        this.outx(
            "Lynnette grabs a bottle and squirts a white fluid into your hair.  You really hope it isn't your payment.  But it must not be, as within short order you feel the added weight of ",
            true
        );
        ...
        this.outx(
            Salon.num2Text(this.temp) + " more inches of " + this.player.hairColor + " hair.",
            false
        );
        this.doNext(this.camp.returnToCampUseOneHour);
    }
    ...
    private dyeMenu(): void {
        ...
        this.outx("", true);
        this.outx(
            "Lynnette pulls open a cabinet in the corner, displaying a wide array of exotic hair-dyes.  Which kind do you want?",
            false
        );
        this.menu();
    ...
    private minotaurCumBukkakeInSalon(): void {
        ...
        this.outx(
            "You hear a deep, strangled sigh as more and more delicious pre floods your mouth, lighting your senses up with a fireworks show of pleasure",
            false
        );
        if (this.player.gender > 0)
            this.outx(" and increasing the size of the puddle you're leaving on the floor", false);
        this.outx(
        ...
        this.outx(
            "Tied down by the cum-spurting flare locked in your gullet, you pump on the other two dicks with feverish speed and sway on your " +
                this.player.feet() +
                " as the narcotic spooge intoxicates your already-addled mind.  You can feel each muscular contraction pulsing through the bestial shaft while it finishes depositing the heavy, sticky load, and your eyes cross from the viscous inebriation that's pooling in your belly.  Drizzles of pre-cum soak into your arms and palms, drawn out from the frenzied pumping of your fists.  They won't come from just a hand though.  They need something... tighter.\n\n",
            false
        );
        ...
        if (this.player.biggestTitSize() >= 2) {
            this.outx(
                "  You wrap the pillowy flesh of your " +
                    this.allBreastsDescript() +
                    " around the new member, pleasantly surprised by its girth and wide, already-flared tip.  Maybe you could have gotten him off with your hands after all?  ",
                false
            );
            if (this.player.biggestTitSize() < 6)
                this.outx(
                    "Even so, you can't quite get your breasts the entire way around him, so you make up for it by pressing it harder into you with your busy hands.  ",
                    false
                );
            this.outx(
                "Runnels of pre cover the shaft and squish wetly between your tits, turning your body into a slip-n-slide that reeks of hot, sticky minotaur sex.",
                false
            );
        } else {
            this.outx(
        ...
        this.outx(
            "You lower and raise yourself, bouncing up and down on your " +
                this.player.legs() +
                " to enhance the ",
            false
        );
        ...
        if (this.player.biggestTitSize() >= 2)
            this.outx(
                "The sudden girth change sends an enticing ripple through your " +
                    this.allBreastsDescript() +
                    " that's pleasurable enough to make you moan into his steadily-widening urethra.  ",
                false
            );
        this.outx(
            "You pull off and bounce faster, lost to your lust and the haze of sex-musk permeating the air, intent on seeing just how much this huge stud can spray onto you.\n\n",
            false
        );

        this.outx(
            "The minotaur does not disappoint.  His hole dilates from the size of the approaching cum-blast, and you sink down his shaft slowly until it's aimed directly at your face.  You close your eyes and feel the first explosion splatter over your " +
                this.hairDescript() +
                " and forehead.  The next takes you full in the face, making it difficult to breathe through the mask of drug-like goo, but a few quick licks gives you a fix and makes it easy to breathe again.  On and on, the minotaur pumps fat ropes of spooge over your body until you're a syrupy, sticky mess that reeks of minotaur pheromones so strongly that dizziness overwhelms you and you fall free of the still-orgasming mino-cock, taking a few final blasts of seed on your " +
                this.chestDesc() +
                " and crotch.  Your hands instinctively shovel a few loads into your " +
                this.assholeOrPussy(),
            false
        );
        if (this.player.hasVagina())
            this.outx(
                " while the animal part of your brain hopes it makes you pregnant with an equally girthy son",
                false
            );
        this.outx(".\n\n", false);
        // ADD PREG CHECK
        ...
        this.outx(
            "Giggling, you stagger over to the next cock in line and turn around, possessed with the idea of taking its spooge in the most direct way possible – anally.   You pull your butt-cheeks apart and lean back, surprising one of the horny beasts with the warmth of your " +
                this.assholeDescript() +
                " as you slowly relax, spreading over his flare.  He actually squirts ropes of something inside of you, but you've been around minotaurs enough to know that it can't be cum, at least not yet.  The slippery gouts of preseed make it nice and easy to rock back and spear yourself on the first few inches, ",
            false
        );
        if (this.player.analCapacity() < 80) {
            this.outx(
                "delighting in the drug-numbed pain of stretching yourself beyond your normal capacity.",
                false
            );
        } else if (this.player.analCapacity() < 140)
            this.outx("delighting in the feeling of perfect fullness.");
        else
            this.outx(
        ...
        this.outx(
            "\n\nYou slide down the twitching bull-shaft until your " +
                this.buttDescript() +
        ...
        if (!this.player.hasCock())
            this.outx(
                "drags through your body, touching sensitive nerves you didn't even know you had until you cum, shuddering and shaking like a wanton whore.",
                false
            );
        else
            this.outx(
                "presses on your prostate as it squeezes by, making " +
                    this.sMultiCockDesc() +
                    " drip and spurt freely until you can bear it no longer and cum, shaking and shuddering like a wanton whore.  Jizz drips and pours from " +
                    this.sMultiCockDesc() +
                    " in a steady stream that pools on the floor, slowly rolling towards a drain that doubtless empties into a tank or greedy goblin cunt.",
                false
            );
        this.outx("\n\n", false);

        this.outx(
            "The strength goes completely out of your " +
                this.player.legs() +
    ...
    private minotaurSalonFollowUp(): void {
        ...
        if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00142] == 0) {
            ...
            this.outx(
                'You nod, a little shocked by your behavior, but already licking your lips in anticipation.  Lynnette smiles and says, "<i>You can buy some for the usual price, plus 60 gems, but for right now, how about some hair service?  It\'s on the house!</i>"',
                false
            );
        } else
            this.outx(
                "You're woken up with a slap to the face, and still muzzy from your cum-induced glory-hole orgy, you stagger up to your " +
                    this.player.feet() +
    ...
    private mudFacial(): void {
        ...
        this.outx("", true);
        this.outx(
            "You sit back in a comfortable chair and pull on a lever to recline it.  The goblins buzz around you, gathering up 'special mud'.  You close your eyes, letting them plaster your " +
                this.player.face() +
                " with the stuff in hopes that it will improve your complexion as much as you've been promised.  A pair of cucumber slices are laid out on your eyes, obscuring your view.\n\n",
            false
        );

        this.outx(
            "With that finished, the crowd of busty, green-skinned women disperses to leave you in peace.  Time drags on, but eventually the mud hardens and cracks.  As if on cue, tiny hands emerge with wet rags to scrub your face clean.  Once they've finished, you feel like a whole new you! (+10 femininity)",
            false
        );
    ...
    private sandFacial(): void {
        ...
        this.outx("", true);
        this.outx(
            "You sit back in a comfortable chair and pull on a lever to recline it.  The goblins buzz around you, gathering up 'special sand'.  You close your eyes, letting them splatter your " +
                this.player.face() +
                " with the rough, textured goop.  It doesn't feel very good, but that won't matter if it makes you as handsome as it's supposed to.\n\n",
            false
        );

        this.outx(
            "After a while the goblin girls come back and clean the stuff from your face. (+10 masculinity)",
            false
        );
    ...
    */
    ...
    private fuckLynnette(): void {
        ...
        if (this.flags[kFLAGS.LYNNETTE_FUCK_COUNTER] == 0) {
            ...
            if (this.player.tallness >= 72) this.outx("[leg]");
            else if (this.player.tallness >= 55) this.outx("[hips]");
            else this.outx("side");
            this.outx(
                " so that she can slip a manicured hand inside your underclothes. She handles [eachCock] "
            );
            if (this.player.cockTotal() > 1) this.outx("in turn ");
            this.outx("with the skill of a practiced snake-charmer before ");
            if (this.player.balls == 0) this.outx("pressing down near the [sheath]");
            else this.outx("curling her petite fingers around your [sack] to heft it");
            this.outx(".");
            // IF FAIL!
        ...
        // Repeat cum within 24 hours check
            ...
            this.outx(
                "Lynnette licks her lips at your decision and sashays forward, her immense hips rocking with enough sensuality that her plush ass is practically having a sexquake.  \"<i>You'll understand if we do a quick check then, right, [name]?</i>\" She doesn't wait for an answer, pressing herself against you so that you can get a good feel of her soft, sopping-wet tits conform to the shape of your body, letting the moist heat that seems to radiate from every pore of Lynnette's petite shape soak into your exposed " +
                    ...
                    ".  Her hand dives right into your underwear without asking, seizing tight around [oneCock] to heft it."
            );
            this.outx(
                "\n\nYou give up an appreciative groan, [eachCock] twitching in lewd enjoyment as "
            );
            if (this.player.lust <= 75) this.outx("it fills to full size almost immediately");
            else this.outx("it's fully-erect state makes itself known");
            this.outx(
                ". Lynnette strokes a few times to make sure you're suitably engorged and then shifts lower, squeezing "
            );
            if (this.player.balls == 0)
                this.outx("around your [sheath] to get a feel for how pent-up you are");
            else
                this.outx(
        ...
        this.outx(" with hunger in her eyes.");
        ...
        if (this.player.cockTotal() > 1) this.outx("es have");
        else this.outx(" has");
        this.outx(
            " begun to bead clear droplets of precum. They run down the underside of your distended, eager cumvein"
        );
        if (this.player.cockTotal() > 1) this.outx("s");
        this.outx(" and race down towards your ");
        if (this.player.balls > 0) this.outx("[balls]");
        else this.outx("[sheath]");
        this.outx(
            ", leaving a glossy, sexual shine in their wake, and they're only coming on faster and faster the longer you gaze upon Lynnette's emerald twat and inhale her prick-stiffening scent."
        );
        ...
        if (this.player.cockTotal() > 1) this.outx("s");
        this.outx(" won't take no for an answer.");
        ...
        this.outx(
            "\n\nGrunting as you aggressively move yourself into place, you lock eyes with the shameless slut, letting her know with a single look just how hard you are going to fuck her, how perfectly pounded her pussy is going to be.  That one expression leaves her unequivocally sure of her impending bowleggedness, and the little tart just grins.  You thrust forward with all the subtleness of a rampaging bull, battering your " +
                ...
                " right through her slobbering delta, feeling the lips cling to your girth as they're stretched out into an o-ring of cock-squeezing pleasure, somehow incredibly tight around you but with more give than any woman ought to have."
        );
        if (this.player.cockArea(x) > 50) {
            this.outx(
                "  Her tummy bulges with the outline of your swollen mass, clearly displaying the shape of your bitch-breaking dong as it travels up "
            );
            if (this.player.cockArea(x) <= 80) this.outx("her belly.");
            else
                this.outx(
                    "into her ribcage, penetrating farther than any lesser race could ever hope to handle."
                );
        }
        this.outx(
            " The breeder grabs hold of your [hips] and tugs as hard as her meager strength will allow.  "
        );
        if (this.player.cockArea(x) <= 90) {
            this.outx(
                "Slapping against her hard enough to release a splash of girlish cum, you hilt yourself completely"
            );
            if (this.player.balls > 0)
                this.outx(", [balls] resting on her soaked, juicy ass-cheeks.");
        } else {
            this.outx(
                "Eventually stopping, you look down to realize that she hasn't taken all of you. Going any farther would likely hurt her and ruin the fun, so you'll have to make to with partial strokes."
            );
            if (this.player.balls > 0)
                this.outx(
                    "  Her feet wrap around your [sack] to squeeze your [balls] affectionately, compensating quite nicely for her own shortcomings."
                );
        }

        this.outx(
            "\n\nLynette is so goddamned wet! Her twat is a sopping-wet furnace around your " +
                this.cockDescript(x) +
                ", clenching down tightly to hold you still while she adjusts to the shape and size of hard-throbbing inseminator.  A happy, brainless smile spreads across her face in reaction to the mounting, though her hands remain stubbornly on your hips, helping to keep you from pounding away until she's ready.  While you're immobilized, you decide to avail yourself of the goblin's other features, gripping as much of one titanic tit as you possibility can.  Your hand barely covers a quarter of the swollen bosom, and your fingers sink deeply into the forgiving green flesh, eliciting a gasp of pleasure from the hairdresser when you shift your grope to place her nipple within your reach."
        );
        this.outx(
            "\n\nLosing control of her abdominal muscles from the forcible, nipple-bound bliss, Lynnette's cock-arresting grip collapses, and you're free to saw away at her gushing nethers, dragging your " +
                this.cockDescript(x) +
                " out until only the head remains embedded within her purple-tinged interior and then, slamming it back in just as hard as your initial penetration.  Her hands slip off your [hips] and down to the sheets, where they gather up fistfuls of the increasingly sex-stained fabric and clench.  The honey hole around you flutters uncontrollably, clenching wildly as you thrust powerfully through it, mixing your pre-cum with its own copious leavings until there's a whitish slurry leaking from the increasingly puffy entrance."
        );
        this.outx(
        ...
        if (this.player.balls > 0)
            this.outx(", making your sack feel tight and your testes bloated and tender.");
        else this.outx(", making your gut clench and spasm with near-orgasmic contractions.");
        this.outx(
            "  There's a tsunami of sperm building up inside you to the point where holding it in is actually starting to hurt you, but at the same time, your body refuses to give in and climax yet either."
        );
        this.outx(
        ...
        this.outx(
            "\n\nYour thrusts transform from powerful lunges into a frenzied jackhammering so powerful that the impact ripples along her considerable thighs and ass.  Even her squirting tits shake and jostle with the pussy-plowing impacts as you mount the goblin like a rutting beast, battering your " +
                this.player.cockHead(x) +
                " "
        );
        if (this.player.cockArea(x) > 24) this.outx("up against her slightly-yielding cervix");
        else this.outx("in as close to the cervix as you possibly can");
        this.outx(
            ", grunting and growling as you lose yourself in the feel of her soaked, velvet tunnel.  Your focus collapses entirely down to the feel of that slick tunnel around you and how far you can push into it, how strongly you can make it climax and squeeze around you."
        );
        this.outx(
        ...
        this.outx(
            "\n\nYour body has been ready to go off for some time now, and the words act like an irresistible trigger for your climax, causing you to lurch your " +
                this.cockDescript(x) +
                " "
        );
        if (this.player.cockArea(x) <= 90) this.outx("completely inside once more");
        else this.outx("as deeply inside as you can");
        this.outx(" and hold it there.");
        if (this.player.cockTotal() > 1) {
            this.outx("  Your other member");
            if (this.player.cockTotal() > 2) this.outx("s are");
            else this.outx(" is");
            this.outx(" spasming ");
            if (this.player.longestCockLength() <= 12)
                this.outx("on the greedy green slut's belly");
            else
                this.outx(
                    "in the milk-stained valley between her tits, absolutely buried in squishy-soft breast"
                );
            this.outx(
                ", jizzing in wild spurts.  The cum mixes with all the goblin-cream into a sticky-slick morass that paints Lynnette white."
            );
        }
        this.outx(
        ...
        this.outx(
            "\n\nTiny hands latch onto your shoulders as you pour your distilled lusts into the lucky woman's womb, digging deeply into your " +
                this.player.skinFurScales() +
        ...
        // 10 liter ludicrous cumflate!
            ...
            this.outx(
                "\n\nOh gods!  Every torrential outflow of cum is so substantial that it's literally stretching the poor goblin out as it rushes into her womb.  The first two blasts are enough to turn her once-taut middle into a sperm-stuffed dome, and you just keep shooting after that, each time making her gravid tummy jiggle and swell, rounding fuller and fuller until her bulging midsection seems almost as big as her.  Her belly button long ago popped out into an outtie, and the smooth, emerald skin shines with its overstretched tightness.  Lynnette whimpers softly, and a sudden torrent of spunk rolls out around your " +
                    this.cockDescript(x) +
                    ".  It seems she just couldn't hold it all in, and you aren't even done cumming yet!  You resume sliding in and out as you jizz, washing your soon-to-be-pregnant fucktoy's tunnel with generous waves of spooge until you finally exhaust yourself."
            );
        this.outx(
        ...
        if (this.player.cumQ() >= 3000) {
            this.outx(", even if ");
            if (this.player.cumQ() >= 10000) this.outx("most");
            else this.outx("some");
            this.outx(" of it is on the bed at this point");
        }
        this.outx(".  Only then does she release you, allowing you to slide out");
        if (this.player.hasKnot(x))
            this.outx(
                " with a wet-sounding 'pop'.  Your knot does a terrific job of gaping her lips during its exit, and you cannot help but marvel at how well you've broken this bitch in.  A shame she'll probably tighten up again in short order"
            );
        else this.outx(" with a gush of girlspunk to chase you");
        this.outx(".  She groans, ");
        // BELOW ONE LITER!
        if (this.player.cumQ() < 1000) {
            this.outx(
        ...
        if (this.player.cumQ() >= 1000)
            this.flags[kFLAGS.LYNNETTE_CARRYING_COUNT] += 1 + Salon.rand(3);
        if (this.player.cumQ() >= 2000)
            this.flags[kFLAGS.LYNNETTE_CARRYING_COUNT] += 1 + Salon.rand(3);
        if (this.player.cumQ() >= 3000)
            this.flags[kFLAGS.LYNNETTE_CARRYING_COUNT] += 2 + Salon.rand(3);
    ...
    public lynnetteApproval(arg = 0): number {
        ...
        if (this.flags[kFLAGS.LYNNETTE_APPROVAL] < -100)
            this.flags[kFLAGS.LYNNETTE_APPROVAL] = -100;
        else if (this.flags[kFLAGS.LYNNETTE_APPROVAL] > 100)
            this.flags[kFLAGS.LYNNETTE_APPROVAL] = 100;
