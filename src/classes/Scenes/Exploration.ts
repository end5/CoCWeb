...
export class Exploration extends BaseContent {
    ...
    public doExplore(): void {
        if (this.player.explored == 0) {
            ...
            return;
        } else if (this.player.explored > 1) this.outputText("You can continue to search for new locations, or explore your previously discovered locations.", true);
    ...
    public tryDiscover(): void {
        ...
        if (this.flags[kFLAGS.PC_PROMISED_HEL_MONOGAMY_FUCKS] == 1 && this.flags[kFLAGS.HEL_RAPED_TODAY] == 0 && Exploration.rand(10) == 0 && this.player.gender > 0 && !kGAMECLASS.helFollower.followerHel()) {
        ...
        if (this.player.explored > 1) {
            ...
            if (this.player.exploredLake >= 1 && Exploration.rand(3) == 0 && this.player.exploredDesert == 0) {
                ...
                if (this.player.lowerBody == LOWER_BODY_TYPE_HUMAN) this.outputText("inside your footwear, between your toes", false);
                if (this.player.lowerBody == LOWER_BODY_TYPE_HOOFED) this.outputText("in your hooves", false);
                if (this.player.lowerBody == LOWER_BODY_TYPE_DOG) this.outputText("in your paws", false);
                if (this.player.lowerBody == LOWER_BODY_TYPE_NAGA) this.outputText("in your scales", false);
            ...
            //EXPLOOOOOOORE
            if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00272] == 0 && this.flags[kFLAGS.TIMES_EXPLORED_PLAINS] > 0 && Exploration.rand(3) == 0) {
    ...
    public bigJunkDesertScene(): void {
        ...
        if (this.player.cocks.length == 1) this.outputText("  As it drags along the dunes, the sensation forces you to imagine the rough textured tongue of a monstrous animal sliding along the head of your " + Appearance.cockNoun(this.player.cocks[x].cockType) + ".", false);
        else if (this.player.cocks.length >= 2) this.outputText("  With all of your " + this.multiCockDescriptLight() + " dragging through the sands they begin feeling as if the rough textured tongues of " + Exploration.num2Text(this.player.cockTotal()) + " different monstrous animals were slobbering over each one.", false);
        this.outputText("\n\n", false);
        ...
        if (!this.player.isTaur()) {
            ...
            if (this.player.biggestTitSize() >= 35) this.outputText("  Your " + kGAMECLASS.allBreastsDescript() + " hang lewdly off your torso to rest on the desert sands, seeming to bury the dunes on either side of you.  Their immense weight anchors your body, further preventing your torso from lifting itself up.  The burning heat of the desert teases your " + this.nippleDescript(0) + "s mercilessly as they grind in the sand.", false);
            //IF CHARACTER HAS A BALLS ADD SENTENCE
            if (this.player.balls > 0) this.outputText("  Your " + this.player.skinTone + this.sackDescript() + " rests beneath your raised " + this.buttDescript() + ".  The fiery warmth of the desert caresses it, causing your " + this.ballsDescriptLight() + " to pulse with the need to release their sperm through your " + this.multiCockDescriptLight() + ".", false);
            ...
            if (this.player.vaginas.length >= 1) {
                ...
                if (this.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_DROOLING) this.outputText("  Juices stream from your womanhood and begin pooling on the hot sand beneath you.  Wisps of steam rise up into the air only to tease your genitals further.  ", false);
            }
        }
        //FOR CENTAURS
        else {
            ...
            //IF CHARACTER HAS GIANT BREASTS ADD SENTENCE
            if (this.player.biggestTitSize() >= 35) this.outputText("  Your " + kGAMECLASS.allBreastsDescript() + " pull your human torso forward until it also is forced to rest facedown, just like your horse half.  Your tits rest, pinned on the desert sand to either side of you.  Their immense weight anchors you, further preventing any part of your equine body from lifting itself up.  The burning heat of the desert teases your " + this.nippleDescript(0) + "s incessantly.", false);
            //IF CHARACTER HAS A BALLS ADD SENTENCE
            if (this.player.balls > 0) this.outputText("  Your " + this.player.skinTone + this.sackDescript() + " rests beneath your raised " + this.buttDescript() + ".  The airy warmth of the desert teases it, causing your " + this.ballsDescriptLight() + " pulse with the need to release their sperm through your " + this.multiCockDescriptLight() + ".", false);
            //IF CHARACTER HAS A VAGINA ADD SENTENCE
            if (this.player.vaginas.length >= 1) {
                this.outputText("  Your " + this.vaginaDescript() + " and " + this.clitDescript() + " are thoroughly squashed between the bulky flesh where your male genitals protrude from between your hips and the " + this.buttDescript() + " above.", false);
                //IF CHARACTER HAS A DROOLING PUSSY ADD SENTENCE
                if (this.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_DROOLING) this.outputText("  The desert sun beats down on your body, its fiery heat inflaming the senses of your vaginal lips.  Juices stream from your womanhood and begin pooling on the hot sand beneath you.", false);
            }
        }
        this.outputText("\n\n", false);
        //PARAGRAPH 3
        ...
        if (this.player.canFly()) this.outputText("  You extend your wings and flap as hard as you can, until at last you manage to lighten the bulk of your body somewhat - enough to allow yourself to drag your genitals across the hot sands and back to camp.  The ordeal takes nearly an hour.", false);
