...
export class Niamh extends TelAdreAbstractContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (this.flags[kFLAGS.NIAMH_MOVED_OUT_COUNTER] > 1)
            this.flags[kFLAGS.NIAMH_MOVED_OUT_COUNTER]--;
    ...
    public telAdreNiamh(): void {
        if (this.flags[kFLAGS.MET_NIAMH] == 0) {
            ...
            if (this.model.time.hours <= 8)
                this.outx("massive N-cup breasts, spilling over the table");
            else if (this.model.time.hours <= 9)
                this.outx("huge M-cup breasts, taking up much of the table");
            else if (this.model.time.hours <= 10)
                this.outx("great L-cup breasts, advancing far onto the table");
            else if (this.model.time.hours <= 11)
                this.outx("heavy K-cup breasts, lying on the table for support");
            else if (this.model.time.hours <= 12)
                this.outx("hefty J-cup breasts, resting comfortably on the table");
            else if (this.model.time.hours <= 13)
                this.outx("weighty I-cup breasts, dangling to lie on the table");
            else if (this.model.time.hours <= 14)
                this.outx("squeezable H-cup breasts, settling burdensomely on her chest");
            else this.outx("moderately manageable G-cups, trembling with every breath she takes");
            this.outx(
        ...
        // This scene only plays if the PC has either Talked to Niamh or has gotten at least 1 mug of Black Cat Beer from her.
            ...
            if (this.model.time.hours <= 8)
                this.outx(
                    "her titanic N-cup 'kegs' resting against the thankfully sturdy table in front of her.  Her teats are engorged with the beer she's cursed to carry, and the stuff dribbles out whenever she shifts."
                );
            else if (this.model.time.hours <= 14) {
                this.outx("her ");
                if (this.model.time.hours <= 9) this.outx("M-cup");
                else if (this.model.time.hours <= 10) this.outx("L-cup");
                else if (this.model.time.hours <= 11) this.outx("K-cup");
                else if (this.model.time.hours <= 12) this.outx("J-cup");
                else if (this.model.time.hours <= 13) this.outx("I-cup");
                else this.outx("H-cup");
                this.outx(
                    " brew-burdened breasts have clearly gotten smaller as the day dragged on, although they're still uncomfortably sizeable and swollen."
                );
            } else
                this.outx(
                    "her G-cup beer-filled boobs far smaller than their morning volume, though still impressive in stature.  You can follow the progress she's made throughout the day by the beer stains running along the wooden surface, trailing from all the way off the table and ending where her boobs now lie."
                );
        }
    }
    ...
    public approachNiamh(): void {
        ...
        this.outx(this.images.showImage("niamh-approach-in-bar"));
        if (
            Niamh.rand(5) == 0 &&
            this.flags[kFLAGS.MET_NIAMH] > 0 &&
            (this.player.hasItem(this.consumables.BIMBOLQ) ||
    ...
    public getANiamhBeer(): void {
        this.clearOutput();
        this.outx(this.images.showImage("niamh-get-beer"));
        this.outx(
        ...
        // (Player has G-cups or bigger;
            ...
            if (this.player.lactationQ() == 0)
                this.outx(
                    "  Not terribly surprising; you know your tits are sensitive, and you don't even make milk in them."
                );
            else
                this.outx(
                    "  You can easily imagine just how much trouble she must have, especially given the bulk of her tits are fluid."
                );
        ...
        if (this.flags[kFLAGS.MET_NIAMH] > 0) this.outx("Niamh ");
        else this.outx("the cat-girl ");
        this.outx("sighs in relief, biting her lower lip slightly.");
        // General Ending
        this.outx(
    ...
    private drinkNiamhsBeerInTelAdre(): void {
        this.clearOutput();
        if (
            this.player.lactationQ() >= 300 &&
            this.player.biggestTitSize() >= 5 &&
            Niamh.rand(2) == 0 &&
            this.flags[kFLAGS.MET_NIAMH] > 0
        ) {
            this.outx(
            ...
            if (this.isHolidays() && this.date.date >= 31) {
                ...
                if (this.player.hasVagina()) this.outx("pussy to wetten");
                if (this.player.gender == 3) this.outx(" and ");
                if (this.player.hasCock()) this.outx("[eachCock] to stiffen");
                this.outx(
    ...
    private blackCatBeerToGo(): void {
        this.clearOutput();
        this.outx(
            '"<i>That\'ll be two more gems, then,</i>" the girl says.  "<i>Though, if your hands get any more skillful I might have to pay you to take it,</i>" she flirts.  You pass over two more gems, and she gratefully bags them and beams you a smile.'
        );
        this.player.gems -= 2;
        this.statScreenRefresh();
        // both output
    ...
    private talkToNiamh(): void {
        ...
        if (this.flags[kFLAGS.TALKED_NIAMH] == 0) {
            this.outx("You ask the cat-girl to tell you about her past.");
            this.outx(
            ...
            if (this.model.time.hours < 15) this.outx("who usually sits over yonder ");
            this.outx(
        ...
        // (repeated)
            ...
            if (this.player.isTaur()) this.outx("You clop up and stand awkwardly");
            else this.outx("You pull up a chair and sit");
            if (Niamh.rand(2) == 0)
                this.outx(
    ...
    public blackCatBeerEffects(player: Player, clearScreen = true, newLine = false): void {
        ...
        if (newLine) this.outx("\n\n");
        this.outx(
            "Uncapping the mug, you swill the stuff down in a single swig, gasping as it burns a fiery trail into your belly.  It's rich and sweet, and damn, but it's strong stuff!"
        );
        this.outx(
    ...
    public blackCatBeerExpires(): void {
        this.dynStats(
            ...
            4.5,
            "lib",
            -1 * this.player.statusAffectv2(StatusAffects.BlackCatBeer)
        );
    ...
    private corruptOrBimboNiamhIntro(): void {
        this.clearOutput();
        this.outx(
            '"<i>Heya, favored customer,</i>" the busty kitty greets as you approach.  "<i>I have a favor to ask!  My drink\'s empty, and I get bored of me own brew,</i>" Niamh explains.  "<i>Would ya mind fetchin\' me somethin\', or are ye gonna force me to sample me own goods once more?</i>"'
        );
        ...
        if (this.player.hasItem(this.consumables.BIMBOLQ))
            this.outx(
                "'bubbly' for her, in the form of the Bimbo Liqueur you got from the imp lord, or "
            );
        this.outx(
    ...
    public niamhPostTelAdreMoveOut(): void {
        ...
        this.clearOutput();
        this.outx(
            "Upon one of your routine searches, you're given pause as a half-coherent mess of a song comes tumbling across the grasses.  You move toward the cacophony of sound, fairly confident in what you will find.  Indeed, stumbling and shambling towards you comes a "
        );
        if (this.flags[kFLAGS.NIAMH_STATUS] == 1) this.outx("demonic-looking");
        else this.outx("hourglass-shaped");
        ...
        this.outx(
            " she slurs, struggling to stay upright between her impaired motor skills and her absolutely overfilled chest.  \"<i>Aaah've been drinkin' me 'shtash',</i>\" she explains, noticing your gaze.  \"<i>Canno' leddit go'tuh waste!</i>\" You nod your agreement, sneaking a glance behind her to observe the twin trails of "
        );
        if (this.flags[kFLAGS.NIAMH_STATUS] == 1) this.outx("dark");
        else this.outx("light");
        this.outx(" fluid she's leaving behind.");
        this.outx(
    ...
    public bimboChampagne(player: Player, clearScreen: boolean, intro: boolean): void {
        ...
        if (intro) {
            if (
                (player.findPerk(PerkLib.FutaFaculties) >= 0 &&
                    player.findPerk(PerkLib.FutaForm) >= 0) ||
                (player.findPerk(PerkLib.BimboBody) >= 0 &&
                    player.findPerk(PerkLib.BimboBrains) >= 0)
            ) {
                this.outx(
                    "You could've swore the stuff worked when you saw Niamh do it to others, but for some reason, it had, like, no effect on you. How weird!"
                );
            } else if (player.findStatusAffect(StatusAffects.BimboChampagne) < 0)
                this.outx(
        ...
        if (player.findStatusAffect(StatusAffects.BimboChampagne) >= 0) {
            ...
            if (player.breastRows[0].breastRating < 5) {
                ...
                player.changeStatusValue(
                    StatusAffects.BimboChampagne,
                    2,
                    5 - player.biggestTitSize()
                );
            ...
            if (!player.hasVagina()) {
                ...
                if (player.hasCock())
                    this.outx(
                        "the flesh under your cock[if (hasBalls = true)  and behind your [balls]]"
                    );
                else this.outx("the blank expanse of flesh that is your crotch");
                this.outx(
                    " start to tingle and squirm... mmm... that feels nice!  There's a sensation you, like, can't describe, and then your crotch feels all wet... but in a good, sticky sorta way.  Oh, wow!  <b>You've, like, just grown a new virgin pussy!</b>  Awesome!"
                );
    ...
    public removeBimboChampagne(): void {
        ...
        if (this.player.statusAffectv2(StatusAffects.BimboChampagne) > 0) {
            this.player.breastRows[0].breastRating -= this.player.statusAffectv2(
                StatusAffects.BimboChampagne
            );
            this.outx(
                "  As the trecherous brew fades, your [chest] loses some of its... bimboliciousness.  Your back feels so much lighter without the extra weight dragging down on it."
            );
        }
        if (this.player.statusAffectv3(StatusAffects.BimboChampagne) > 0) {
            this.outx(
                "  At the same time, your [vagina] slowly seals itself up, disappearing as quickly as it came.  Goodbye womanhood."
            );
        ...
        if (this.player.statusAffectv4(StatusAffects.BimboChampagne) > 0) {
            ...
            this.outx(
                "  Of course, the added junk in your trunk fades too, leaving you back to having a [butt]."
            );
        }
    ...
    public bazaarNiamh(): void {
        ...
        if (this.flags[kFLAGS.NIAMH_STATUS] == 2) {
            if (this.flags[kFLAGS.TIMES_NIAMH_BAZAAR_MET] == 0) {
                this.outx(
                    'As you enter the bazaar, you hear a familiar voice calls out to you. "<i>Hey!  [name]!  Over here!  Like, it\'s me!</i>"'
                );
                this.outx(
                    "You follow the sound and find Niamh seated in a corner of the bazaar.  She is totally naked save for a small strip of cloth around her loins; her huge breasts lie exposed for all the world to admire.  She's surrounded by a small entourage of assorted beings; some are either hoping to earn a drink or else are simply here to ogle the massively buxom catgirl, others have evidently already drunk their fill, given their swollen bellies, feminine forms and platinum blonde coloration.  She grins as you approach."
                );
                this.outx(
    ...
    private bazaardNiamhDrink(): void {
        ...
        // Corrupt
            ...
            this.outx("Succubus milk Niamh Not implemented yet");
            this.doNext(this.camp.returnToCampUseOneHour);
        }
    }
    ...
    private drinkFromZeTap(): void {
        this.clearOutput();
        this.outx(
            "Your eyes lock onto her large, ever-dribbling nipples, the froth of spilt champagne caked on the table supporting her mammoth breasts, and you know you just have to have it from the source.  You push your way forward and seat yourself on a plush cushion that, from its many tears and suspicious-smelling stains, has seen better days, then grasp the huge boob in front of you, making the ditzy catgirl yowl in anticipation of pleasure."
        );
        this.outx(
    ...
    private getBimboChampFromNiamh(): void {
        this.clearOutput();
        this.outx(
            "You explain your need for a bottle of her champagne; the ditzy kitty takes several moments to register your words, then nods idly.  \"<i>Works fer me!</i>\" she responds, reaching behind her and rummaging in a large burlap bag.  She produces a flask from the sack's depths, unscrewing the cap and attempting to line it up with a nipple.  Try as she might, she just can't quite reach all the way, and with an airy giggle, she passes it to you instead."
        );
        this.outx(
    ...
    private bazaarSex(): void {
        ...
        if (this.flags[kFLAGS.NIAMH_STATUS] == 2) {
            ...
            this.player.orgasm();
            this.dynStats("int", -0.5, "sen", -2);
        } else if (this.flags[kFLAGS.NIAMH_STATUS] == 1) {
            // Succubi Milk
    ...
    public seanBimboBrewing(): void {
        this.clearOutput();
        this.outx(
            'The uncharacteristically gentlemanly incubus raises his eyebrow as you produce an armful of filled flasks and drop them onto his mostly-empty table.  "<i>I assume you\'d like me to do something special with these,</i>" he quips, somewhat dryly.'
        );
        this.outx(
    ...
    public getBimboozeFromSean(): void {
        this.clearOutput();
        this.outx(
            'Sean turns as you enter, managing a thin smile as he regards you.  "<i>Excellent timing, I have just finished this!</i>"'
        );
        this.outx(
        ...
        this.outx("You drop the vial into a pouch on your person.\n\n");
    ...
    private barBeerOrgyTits(): void {
        ...
        if (this.player.hasCock()) {
            this.outx("[EachCock] grows to its full length.  You try to keep ");
            if (this.player.cockTotal() == 1) this.outx("it");
            else this.outx("them");
            this.outx(" discreet but it's impossible.  You try to push ");
            if (this.player.cockTotal() == 1) this.outx("it");
            else this.outx("them");
            this.outx(" down or aside, but other pairs of hands begin stroking you.");
        }
        this.outx(
        ...
        if (this.player.biggestTitSize() >= 20)
            this.outx(
                "Your giant boobs are large enough to temporarily hang off the sides of your body as you're carried."
            );
        this.outx(
            "  Your [butt] hits the table first as the crowd puts you down.  Some patrons begin sucking on your nipples directly."
        );
        ...
        if (
            ...
            if (this.player.pregnancyIncubation == 0)
                this.outx(
                    "  Some folks give yelps and cheers when they see the bulge that repeatedly forms in your abdomen each time Urta thrusts herself inside you."
                );
 ...
 ================*/
        ...
        if (this.player.pregnancyIncubation > 0 && this.player.pregnancyIncubation < 100)
            this.outx(
                '  "<i>Oi!  Careful folks, this babe could blow at any time!</i>"  Niamh pats your belly with chiding affection.'
            );
        ...
        if (this.player.cockTotal() > 0) {
            ...
            if (this.player.cockTotal() > 1) this.outx("s have");
            else this.outx(" has");
            this.outx(" begun spurting ");
            ...
            if (this.player.pregnancyIncubation > 0 && this.player.pregnancyIncubation < 200)
                this.outx("  Globs of cum splatter across your gravid belly.");
        }
        ...
        if (
            ...
            // IF [player has a single cock]
            if (this.player.cockTotal() == 1)
                this.outx("  Edryn moans out loud in ecstasy as your [cock] fills her up.");
            ...
            if (this.player.pregnancyIncubation > 0 && this.player.pregnancyIncubation < 200)
                this.outx(
                    "  Your pregnant belly presses up against Edryn's ass as both Urta's cock and Edryn's pussy keeps you pinned in place between them."
                );
            // IF[player is not pregnant]
            else
                this.outx(
                    "  Urta's cock and Edryn's pussy pin you in place between them, and as Urta fucks you deeper her cock head forces your belly to bulge outwards until it presses against Edryn's ass."
                );
            ...
            if (this.player.balls > 0 && this.player.ballSize > 14)
                this.outx("  Your hanging testicles bounce around between everyone's legs.");
        ...
        // Generic ending if the first two don't trigger
            ...
            if (this.player.hasVagina())
                this.outx(
                    "  The giant knot of the dog morph finally breaks its way inside causing your pussy to become overstretched.  The pleasurable torture only gets worse as he begins unloading a river of his cum inside you.  It begins squirting out around his cock and soaks both of your thighs in the process."
                );
            this.player.cuntChange(60, true, true, false);

            this.outx(
                "  Niamh finally has her fill and dismounts you, but no sooner has her pussy left your face than a determined cock belonging to some sort of cat morph fills your mouth.  Your eyes bug out as it thrusts down your throat."
            );
            // If [player has pussy]
            if (this.player.hasVagina())
                this.outx(
            ...
            if (this.player.cockTotal() == 1)
                this.outx(
                    "  The six titted girl smiles at you, patting her belly while your cum runs down the inside of her thighs.  You only now wonder about how fertile she might be."
                );
            // IF [player has multiple cocks]
            else if (this.player.cockTotal() > 1)
                this.outx(
                    "  Various girls around the room are patting their bellies and giving you sultry looks while your cum continues to drip from their pussies.  You passively wonder if any of them might indeed be hauling around some larger pregnant bellies in a few days, but you try to push those thoughts from your head.  At the moment they seem happy."
                );
            // IF [player has pussy]
            if (this.player.hasVagina()) {
                this.outx(
