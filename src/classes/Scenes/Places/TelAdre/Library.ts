...
export class Library extends TelAdreAbstractContent {
    ...
    public visitZeMagesTower(): void {
        ...
        if (this.flags[kFLAGS.TIMES_BEEN_TO_LIBRARY] == 0 || this.model.time.hours <= 17) {
            ...
            if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00175] > 0)
    ...
    private firstTowerVisit(): void {
        this.clearOutput();
        this.outx(
            "You make your way to the largest fixture of the city, the impressive tower in the center.  The large spire could easily hold everyone you know ten times over and still have room to spare.  It is far too large for a city with Tel'adre's population – but then, you reflect, so is Tel'adre itself."
        );
        ...
        if (this.model.time.hours <= 17) {
            ...
            this.outx("  A single man carefully turns through the pages of one book");
    ...
    private commonQuinnTroduction(): void {
        this.outx(
            ", though he does not seem to be reading it.  Stacks of books sit next to him.  As you close the door, he glances up at you."
        );
    ...
    private studyInTA(): void {
        ...
        // [Study, 6:00-17:00]
        if (this.model.time.hours <= 17) {
            this.outx("You ask Quinn if you can use the library to study and learn.");
            this.outx(
        ...
        // [Study, 18:00-20:00]
            ...
            if (this.flags[kFLAGS.TIMES_BEEN_TO_LIBRARY] == -1) {
                this.outx("Looking around you decide to spend some time reading");
            } else {
                this.outx(
                    "Without Quinn to hassle you and request your absence from the presences, you have some time to read"
                );
            }
            this.outx(" through some of the literature collected by the Covenant.");
            if (Library.rand(3) == 0) {
                // magic)
                this.outx(
            ...
            // OR (player is bimbo/bimbro/whatever)
                ...
                (this.player.lib > 75 ||
                    ...
                    this.player.findPerk(PerkLib.BroBrains) >= 0) &&
                Library.rand(2) == 0
            )
                this.outx(
    ...
    private youOkayBuddy(): void {
        this.clearOutput();
        this.outx(
            "A bit perturbed by Quinn's countenance and apparent exhaustion you can't help but inquire as to his well-being."
        );
    ...
    private talkToMali(): void {
        ...
        // [[Mali], player has spellblade]
            ...
            this.flags[kFLAGS.MALI_TAKEN_BLADE] == 0
        ) {
            this.outx(
                "You tell Quinn you're here to see Mali.  He seems intrigued by the wrapped blade you're carrying, but doesn't ask any questions.  Unlocking the second floor as usual, he escorts you to Mali's quarters."
            );
            this.outx(
