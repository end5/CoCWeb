...
export class Brooke extends TelAdreAbstractContent {
    ...
    public brookeAffection(arg = 0): number {
        ...
        if (
            (this.player.isTaur() || this.flags[kFLAGS.BROOKE_SHOWERED_WITH] == 0) &&
            this.flags[kFLAGS.BROOKE_AFFECTION] > 20
        )
            this.flags[kFLAGS.BROOKE_AFFECTION] = 20;
    ...
    public meetBrookeFirstTime(): void {
        this.clearOutput();
        this.outx(this.images.showImage("brooke-first-meeting"));
        this.outx(
        ...
        if (this.player.isNaga()) this.outx("coil your tail nervously under yourself");
        else this.outx("shuffle your [feet]");
        this.outx(
            ', and say that you\'re just about to shower yourself.  She steps aside, dropping the towel off her head and wrapping it around her body (almost teasingly), allowing you to pass.  "<i>Plenty of stalls to pick from,</i>" she laughs, and you thank her, walking on and avoiding eye contact.'
        );
    ...
    public repeatChooseShower(): void {
        ...
        // Affection >=40, after first-time sex
            ...
            this.brookeAffection() >= 40 &&
            this.flags[kFLAGS.BROOKE_MEDIUM_SCENE] == 0 &&
            this.player.tone >= 50 &&
            (this.player.hasVagina() || this.player.cockThatFits(this.brookeCapacity()) >= 0)
        ) {
    ...
    public brookeThirdEncounter(): void {
        this.clearOutput();
        this.outx(
            "You head your way back to the showers after stopping at the lockers to store your [armor].  You wonder to yourself if you'll see Brooke there again.  She seems like the fond, friendly type, and you both have at least one mutual interest, after all.  If nothing else, she's certainly not hard on the eyes."
        );
        this.outx(
    ...
    public lowAffectionBrookeMeeting(): void {
        this.clearOutput();
        this.outx(
            "You head your way back to the showers, feeling the sweet ache of your muscles as you easily find your way there.  Brooke is there, already rinsing the day's work out of her muscles.  She hears you coming in, and turns to give you a warm greeting, which you happily return."
        );
    ...
    public mediumLowBrookeAffectionOneTime(): void {
        this.clearOutput();
        this.outx(
            "Another good workout and another fine sheen of sweat to wash off.  The day hasn't been so bad so far, and you start to whistle to yourself as you head to the showers, dropping your [armor] off in a locker beforehand.  Glancing out one of the windows, you guess that Brooke ought to be there around this time of day; and as you approach, you hear the sounds of rushing water.  Sure, it could actually be someone else, but you have a feeling it's the Shepherd girl."
        );
    ...
    public helpBrookeOut(): void {
        ...
        if (this.player.hasCock())
            this.outx(
                " and being so close to such a woman, you find it difficult to keep [eachCock] from getting too excited"
            );
        this.outx(".");
        ...
        if (this.player.gender == 1)
            this.outx("not caring about the stirring in [eachCock] in the privacy of the stall");
        // [if {PC is female}]
        if (this.player.gender == 2)
            this.outx("feeling a fire stoke in your [vagina] even under the constant water");
        if (this.player.gender == 3)
            this.outx("your loins beginning to stir, each and every part of you enticed");
        this.outx(
            " as you replay the memory of the Shepherd girl's taut, firm ass swaying, just for you."
        );
    ...
    public mediumBrookeAffectionMeetingAfterSex(): void {
        ...
        this.addButton(2, "Her Bod", this.brookesRockinBod);
        if (this.brookeAffection() >= 40 && this.flags[kFLAGS.BROOKE_MEDIUM_SCENE] > 0)
            this.addButton(3, "Grope", this.gropeDatBrooke);
    }
    ...
    public talkWithBrookeAboutWorkingOut(): void {
        this.clearOutput();
        this.outx(
            "You ask Brooke how she manages to work out, given her condition.  You can understand the legs and the washboard abs, but she's still packing more heat in her arms and shoulders than most people do in their bodies."
        );
        ...
        if (this.player.gender == 1) this.outx("[eachCock]");
        else if (this.player.gender == 2) this.outx("your [vagina]");
        else this.outx("[eachCock]");
        this.outx(".");
    ...
    public talkToBrookeAboutGymFolks(): void {
        this.clearOutput();
        this.outx(
            "You ask about what she thinks of the other gym members.  Surely she's seen them as often as you have, given how frequently she visits the place."
        );
        this.outx(
    ...
    public brookesRockinBod(): void {
        this.clearOutput();
        this.outx(
            "You say that you recall her talking about wanting to get buff since a young age.  She even got a dumbbell for one of her birthdays, you recall.  What pushed her to want to become such a pinnacle of modern-day fitness?"
        );

        this.outx(
            `\n\n<i>"Ooh, keep talking sweet things, sweet thing,"</i> she moans, her eyes drifting dreamily closed.  <i>"I love it when a ${this.player.mf(
                "guy",
                "girl"
            )} notices all my hard work."</i>  She stops, expecting you to keep admiring her body, when you remind her that you asked her a question first.  <i>"You did?  Oh, right!  Why I... yeah, why I work out.`
        );
        this.outx(
    ...
    public gropeDatBrooke(): void {
        this.clearOutput();
        this.outx(this.images.showImage("brooke-grope-her"));
        this.outx(
        ...
        this.outx("  The tension is beginning to get a little unbearable and your resolve wavers.");
        ...
        if (this.player.hasCock()) this.outx("[eachCock]");
            ...
            this.outx("your pelvis");
            if (this.player.hasVagina())
                this.outx(
                    " and dragging her wet tail enticingly through the vulva of your [vagina]"
                );
        }
        this.outx(
            ".  <i>\"I'm game.  Hope you're ready, though; you're gonna need more than a few laps around the track to keep up with me.\"</i>"
        );
        ...
        if (this.player.hasCock() && this.player.cockThatFits(this.brookeCapacity()) >= 0)
            this.addButton(4, "Get Laid", this.getLaidByBrooke);
    ...
    public penetrateBrooke(): void {
        ...
        this.outx(
            `.  She moans appreciatively with you some more, loving the feel of your ${this.cockDescript(
                x
            )} in her.`
        );
        ...
        if (this.player.balls > 0) this.outx(" across your [sack] and");
        this.outx(
            " down your thighs that she's getting wet enough for the both of you all the while."
        );

        this.outx(
            `\n\nHer body jerks with everything you do to it: out, into your hands while you play with her tits, and onto your ${this.cockDescript(
                x
            )} with every thrust into her tight tunnel you make.  Her whines and sighs quickly begin to rise and quicken, and with a few louder grunts, she cums onto you, clamping her cunt on you like a vice and doing her very best to coax everything out of you`
        );
        if (this.player.balls > 0) this.outx("r [balls]");
        this.outx(
            ".  You're not too far behind her, and with just a few more pumps, you're shooting your load deep into her, pumping and fucking the whole time, making sure to get it in deep."
        );
        if (y >= 0)
            this.outx(
                `  Your ${this.cockDescript(
        ...
        // [if (cocks = 2)
        if (this.player.cockTotal() == 2 && y >= 0) this.outx(".");
        if (this.player.gender == 3)
            this.outx(
                "  Your [vagina] cums along with the rest of you, spurting your femcum down your legs, washing the inside of your thighs in your juices.  The combined pleasure between your genitals makes your knees buckle – if only you had something, or someone, fucking you too!"
            );

        this.outx(
            `\n\nYou two stay there, standing under the cold shower nozzle, basking in the glow of each other post-coitus.  You begin to slide out when she clenches hard onto your ${this.cockDescript(
                x
            )}.  <i>"Just a minute longer,"</i> she says, and you don't fight her.  You simply enjoy each other for a while, your stiff tool resting in her, and her just relaxing in your embrace.  Eventually, you soften and slip out, and she relents.  <i>"Good as ever, sweet thing,"</i> she says, and you return the compliment, giving her a loving, lingering kiss on her neck.`
        );
    ...
    public brookeAnal(): void {
        ...
        if (this.player.cockArea(x) >= 61) {
            this.outx(
                `Bluntly, you ask Brooke what her opinion is on anal.  <i>"What, with you?"</i> she asks, leaning her ass back and trapping your ${this.cockDescript(
                    x
                )} between her asscheeks expertly.  You moan with pleasure, but before you get the wrong idea, she hums disapprovingly.  She was actually using her ass to measure your ${this.cockDescript(
                    x
        ...
        this.outx(
            `, leaning her ass back and trapping your ${this.cockDescript(
                x
            )} between her asscheeks expertly.  You moan with pleasure while she glides her globes up and down, feeling your manhood rise and swell with each bob.  Your hands are already on her butt, clenching the skin hard together while you thrust in the space between her buttocks.  <i>"Well... all right.  But only because I like you so much."</i>`
        );

        this.outx(
            `\n\nYou grip onto her hips, dragging your ${this.cockDescript(
                x
        ...
        // [if {first time}]
        if (this.flags[kFLAGS.TIMES_IN_BROOKES_BUTT] == 0)
            this.outx("  You wonder – is this the first time she's ever had anal before?");
        else
            this.outx(
                "  She's tight as a vice, but you know she can take you.  She's just getting herself ready."
            );

        this.outx(
            `\n\nThe ${this.player.cockHead(x)} of your ${this.cockDescript(
                x
            )} rubs against her rosebud in anticipation`
        );
        if (this.player.cockTotal() > 1)
            this.outx(
                " while your other member entertains itself between her thighs, bumping and sliding pleasurably across her vulva"
            );
        this.outx(
            '.  She instinctively clenches her ass, and you give her a moment to relax before you stroke her there again.  She loosens, and, almost teasingly now, you prod gently at her hole, implying but not acting just yet.  She takes deep breaths, working to steady herself.  <i>"Okay,"</i> she says, looking over her shoulder to you with a smile.'
        );

        this.outx(this.images.showImage("brooke-gym-male-analbrooke"));

        this.outx(
            `\n\nYou plan to take it easy on her.  You realign yourself, pointing your ${this.player.cockHead(
                x
            )} right at her hole, and slowly begin to push yourself in.  On reflex, she inhales sharply, pressing her upper body into the wall some more.  You reassure her and push in a little deeper, sliding a quarter of your ${this.cockDescript(
                x
            )} into her anus.  She gasps, but doesn't object.  Another push, sliding in to halfway.  A grunt, and she shuffles her feet slightly, getting more stability.  Another push, nearly to the base.  She gives out a whorish moan, and you wonder if she's at her comfort limit.  You push forward anyway, to your [sheath], and gasp in pleasure and sigh with relief when she doesn't object.`
        );

        this.outx(
            `\n\nYou stay still for a moment, basking in the heat of her ass as she completely takes your ${this.cockDescript(
                x
            )}.  The ring of her sphincter pulls and tightens around you, dragging the skin of your cock with each movement, and her inner muscles clench and massage every fiber of your hard meat.  You shuffle a bit, when Brooke interrupts you.  <i>"Just a sec,"</i> she says, rocking her hips some more, this way and that, trying to get a good feel for your dick in her ass.  You let her take her time – it's her ass you're fucking.  She eventually shuffles into a comfortable standing position, and with a nod, gives you the go ahead.`
        );
        ...
        this.outx(
            `\n\n<i>"Little harder,"</i> she asks, when you're already going about as hard as you can.  You sacrifice a bit of speed for power as you plunge harder into her, spearing her anus apart with your ${this.cockDescript(
                x
            )}, driving yourself harder into her.  She relents more space, practically crawling herself up the wall, and you continue to go up to your `
        );
        if (this.player.balls > 0) this.outx("[balls]");
        else this.outx("[sheath]");
        this.outx(
            ", burying yourself into her voracious ass.  With her body climbs her voice: a few pumps more and her moans become wails, loud enough that you're sure someone else must have heard."
        );
        if (this.player.cor >= 45)
            this.outx(
                "  With any luck, that someone will come to investigate – you're sure it wouldn't take much convincing to start a party."
            );
        ...
        this.outx("; your cock stiffens");
        if (this.player.gender == 3) this.outx("; your lonely pussy clenches at nothing");
        this.outx("; and you cum into her, blasting rope after rope of your jism inside her.");
        if (this.player.cockTotal() > 1)
            this.outx(
                `  Your second dick flails and erupts with your ${this.cockDescript(
                    x
        ...
        if (this.player.cockTotal() > 2)
            this.outx(
                `  [EachCock] blasts in time with your ${this.cockDescript(
                    x
                )}, each of your dicks pointing in a different direction.  Your cum gets on everything: Brooke, yourself, the stall – some even reaches as high as the shower nozzle.`
            );
        ...
        if (this.flags[kFLAGS.TIMES_IN_BROOKES_BUTT] == 0)
            ...
            this.outx(
                "<i>\"Thanks for putting up with me being such a bossy bitch, [name].  But it's your fault, you know.  You're the one who awakened the anal-hungry monster within me.\"</i>  You're aware of the beast you've created, you tell her, but truth be told, since your first session, it's been a bit easier to keep up with her.  She's just that good of a workout partner."
            );

        this.outx(
            `\n\nWith some reluctance, she eventually lets go of your spent, flaccid cock, conceding momentary defeat.  The water's long-since turned cold, and you spend the remainder of your time together washing and drying each other off.  She does all she can to entice you some more – she fondles your ${this.cockDescript(
                x
    ...
    public tribadism(): void {
        this.clearOutput();
        this.outx(this.images.showImage("brooke-gym-female-tribadism"));
        this.outx(
        ...
        if (this.player.gender == 3)
            this.outx(" and all the way to the base of your now-straining, pre-leaking cock");
        this.outx(".");
        ...
        if (this.player.tone >= 50)
            this.outx(
                ", and she coos sexily, gripping and pulling at each of your toned, chiseled asscheeks, even giving one as hard a slap as her hand can manage"
            );
        this.outx(
            `.  You start to fuck against her in earnest, pressing and pumping, sliding and bucking against her, loving and living for the sounds and expressions she makes as you love her the way only another ${this.player.mf(
                "woman",
                "'woman'"
            )} can.`
        );
        ...
        if (!this.player.hasCock() && this.player.hasFuckableNipples())
            this.outx(
                ".  Feeling the texture of your [nipples] under her hands snaps her out of her reverie enough to think to stimulate you further by pressing into each with a thumb, fucking into and against you with each of your excited thrusts"
            );
        this.outx(".");
        if (this.player.hasCock())
            this.outx(
                "  Brooke moves one hand to your [chest] and wraps her other around your stiff [cock], pumping it in time with your thrusts against her."
            );
        ...
        if (this.player.gender == 3)
            this.outx(
                ".  Your [cock] explodes along with its sister, spurting your cum high between your pressed bodies, coating the underside of your breasts and your stomachs, but mostly onto Brooke, who keeps your flow pointed towards her as she pumps you"
            );
        else
            this.outx(
                ".  Brooke continues to drive against you as she mauls your breasts, using whatever strength is in her fingers to grope what she can, eager to reach her own peak not far off"
            );
        this.outx(
            ".  Brooke's moans rise and climb in her own ecstasy, until she climaxes along with you, returning the favor and spraying your crotch with her fluids, coating your [vagina] with her own gift."
        );
        ...
        // [if (isHerm = true)]
        if (this.player.gender == 3) this.outx(" and [eachCock] goes limp");
        this.outx(
        ...
        if (this.player.gender == 3)
            this.outx(
                '.  <i>"Lucky for me, we won\'t always have to choose."</i>  She gently strokes your waning erection some more, releasing you when the [cockHead] touches down on her skin limply'
            );
        this.outx(".");
    ...
    public goDownOnBrooke(): void {
        ...
        if (this.player.hasCock())
            this.outx(
                "and [eachCock] pulls forward, pointing towards her like a compass to magnetic north, leaking your pre at her scent and ready to fuck her, or something, anything."
            );
        else
            this.outx(
                "and you find yourself salivating – such a viable, powerful specimen like herself has you wishing you had a dick to fuck her with."
            );
        ...
        if (this.player.hasCock())
            ...
            this.outx(
                "  Your own [vagina] stands out, puffy and needy against the cold air, and it's certainly not gone unnoticed."
            );
        this.outx(
            "  Brooke offers to return the favor, more than once, but you decline each time, knowing that if she did, it'd be a cycle between you two that would last for hours.  She understands, but promises you that she owes you one."
        );
    ...
    public getLaidByBrooke(): void {
        ...
        this.outx(
            ".  You kiss and nip at her neck, knowing just what she likes, and she moans, knowing that you like hearing her."
        );
        ...
        if (this.player.hasCock())
            this.outx(
                `your ${this.cockDescript(
                    x
                )} continues to thrust and spear between her luscious ass, demanding attention be paid to it.  Brooke notices, and rocks her hips against you, squeezing your meat teasingly with each downstroke her ass makes on your ${this.cockDescript(
                    x
                )}`
            );
        else
        ...
        this.outx(
            '.  <i>"Excited?"</i> she asks, grinning back at you.  You let your actions speak for themselves, and you hump against her butt some more, dragging out your pleasure.'
        );
        ...
        if (this.player.hasCock()) this.outx("horndog");
        else this.outx("whore");
        this.outx(
            ' sometimes, [name],"</i> she says, turning herself around in your arms.  She reaches down between you, '
        );
        if (this.player.hasCock())
            this.outx(
                "stroking along [eachCock], hissing inwardly at what she finds, loving the way her fingers, hurt as they are, just manage to glide across your skin"
            );
        else
            this.outx(
                "stroking along your [vagina], grinning at how your lower lips just manage to wrap around her fingers like they were made for each other.  Her thumb drags across your [clit] intentionally, several times, and she smiles wider at every feminine grunt you make"
            );
        this.outx(
            '.  <i>"Feeling a little pent up, sweet thing?"</i> she asks innocently, her fingers working harder on your genitals.  <i>"It\'s okay.  Brooke\'s in a giving mood."</i>'
        );
        ...
        if (this.player.hasCock())
            this.outx(" [eachCock], now fully erect and at attention, just for her");
        else
            this.outx(
                " your [vagina], loving how you can hardly resist fucking your hips at nothing in anticipation"
            );
        this.outx(", and, licking her drooling lips, leans in.");

        // {male/herm scene}[if (cocks = 1)
        if (this.player.hasCock()) {
            this.outx(this.images.showImage("brooke-gym-male-getLaid"));
            this.outx(
                `\n\nBrooke opens her mouth and breaths hotly on your ${this.cockDescript(
                    x
                )}, hesitating just a moment to feel you jump in her hands, before taking you in.  She only wraps her lips around the crown, suckling gently on you, lapping at any precum that flows out.  Her tongue, though, runs out of her mouth unabated, lathering the underside of your ${this.cockDescript(
                    x
                )} with her saliva, preparing it for the journey it's about to take.`
            );
            if (this.player.cockTotal() == 2)
                this.outx(
                    "  Brooke grips onto [eachCock] gently, keeping them sturdy and pointing them right at her lips.  She pumps them alternately, watching how the skin glides with her fingers, while she ponders how precisely to pleasure you.  Throwing caution to the wind, she opens her mouth as wide as she can, and shoves both of [eachCock] into her mouth at once, suckling and laving at your rods gently, preparing both themselves and herself for the journey they're about to take."
                );
            if (this.player.cockTotal() >= 3)
                this.outx(
                    `  Brooke isn't quite sure where to begin, staring at [eachCock].  She grabs ahold of your ${this.cockDescript(
                        x
                    )} with one hand, before grasping at your other phallus with another, and seeing your last cock hanging loose and alone, she bends down and leans in, using just her face to lift it and shuck it into her mouth – only by the tip, and very gently, but nonetheless eagerly, spurred on by your jungle of a crotch for her to explore and conquer.`
                );
            if (this.player.cockTotal() >= 4)
                this.outx(
                    "  Still, she can't quite tear her eyes from the rest of your cold, wet, and alone penises, and she sighs, almost out of frustration, completely unsure how she's going to accomplish this."
                );
            if (this.player.hasVagina())
                this.outx(
                    `\n\nDespite her task, she's fully aware of your feminine side, and gently leads one of her hands down the shaft of your ${this.cockDescript(
                        x
                    )}, reaching back and between your legs for your other half.  You gasp and buck as she finds your [clit] first, and then thrusts her fingers in, up to the first set of knuckles.  She pumps them in time with the dick in her mouth, trying to build up a rhythm with you.`
                );
            ...
            if (this.player.cockTotal() >= 3) this.outx(" and all over the rest of her");
            this.outx(".");

            this.outx(
                `\n\nShe moans and pushes herself deeper, swallowing more of your ${this.cockDescript(
                    x
                )} into her tight, slurping maw: her head bobs and her tongue tastes, sucking more of your meat into her, eager to please.  You pump your hips with her sucking, your fingers twirling and playing with her hair affectionately, `
            );
            if (this.player.cockTotal() <= 2)
                this.outx(
                    "enjoying the lips of your canine lover as they slide over [eachCock] and how she seems to enjoy herself so thoroughly"
                );
            else if (this.player.cockTotal() >= 3)
                this.outx(
                    "enjoying how she tries her damndest to make sure every little bit of you has enough attention and is pleased to the best of her ability, no matter how much effort she requires"
                );
            this.outx(".");
            if (this.player.gender != 3) this.outx("\n\n");
                ...
                if (this.player.balls > 0)
                    this.outx(
                        "burying her muzzle underneath your [sack] to push your [balls] out of her way, "
                    );
                this.outx(
                    `and gets her tongue to work on your [vagina].  You gasp, unintentionally squatting just a bit to get more of her.  Ever the multi-tasker, she keeps her left hand busy, stroking your ${this.cockDescript(
                        x
                    )}, making sure no part of you is ever left alone for long.\n\nShe pushes her nose up, rubbing the bridge of her long dog-nose against your [clit], knowing how much that'd get to you.  From her position, she can't reach very deep into your tunnel with her tongue, but she certainly does what she can: she licks and digs at your vulva, tasting every drop of juice you drip, and allows you to fuck against the fine fur on her face as she services you.  With the work she does in your [vagina] and with the way her expert hand continues to masturbate your ${this.cockDescript(
                        x
                    )}, your body is horribly confused in which way to thrust against, but the sensations lighting up your mind nonetheless make it worthwhile.\n\nEventually, Brooke pulls back, releasing your cock from her hand and stepping out from between your legs.  She rolls her head on her neck a bit – staying in such a way for so long must have been a strain – and gets right back to work, sucking your ${this.cockDescript(
                        x
                    )} back into her mouth, deeper and faster this time.  `
                );
            }
            this.outx(
                `Brooke moans out loud, pressing herself farther in, loving the feel of your dick in her mouth as much as you do.  Your ${this.cockDescript(
                    x
                )} pulses and bounces with each of your heartbeats, and she can feel the jump in her mouth and across her tongue.  She does everything she can to enhance your pleasure, from pressing your ${this.cockDescript(
                    x
                )} against the roof of her mouth, to using the base of her tongue on your ${this.player.cockHead(
                    x
                )} while she toys with your shaft with the tip, to leaning in and taking all of you in her mouth at once and staying there for a moment.  You let her know that her efforts are paying off with a wordless grunt and jerking your hips: you're close, and she can tell.`
            );
            ...
            if (this.player.cumQ() <= 349)
                this.outx(
                    `\n\nYou blow your load into her sucking mouth, and she licks and laps at every drop.  Your ${this.cockDescript(
                        x
        ...
        // {female scene}
            ...
            this.outx(this.images.showImage("brooke-gym-female-getLaid"));
            this.outx(
            ...
            if (this.player.cor <= 49)
                this.outx("  That would almost be embarrassing enough to turn you off.");
            else
                this.outx(
                    "  You want this sexy bitch all for yourself, and you'll be damned if you're sharing.  At least not until you're done."
                );
            this.outx(
                "  Brooke, though, giggles through her mouthful of quim and thrusts again, enjoying the strained efforts you're making to not cry out.  Her hand jills hard and fast against her own neglected pussy, and from her actions, she's maybe as close as you are."
            );
        ...
        if (this.player.cockTotal() >= 3)
            this.outx(", and letting the excess wash from her fur half-heartedly");
        this.outx(
            ".  Moments pass in a completely undecipherable blur to you, and when you can finally make sense of your surroundings, Brooke's standing up and has her tongue in your mouth."
        );
        ...
        if (this.player.hasCock()) this.outx(", snowballing what she hasn't swallowed quite yet");
        this.outx(
            '.  She cradles you in her strong arms until she\'s done, and you pull away from the wall, still a bit wobbly.  <i>"Was it good for you too, sweet thing?"</i> she asks.  You barely nod.'
        );
    ...
    public doubleDicked(): void {
        ...
        this.outx(this.images.showImage("brooke-gym-male-doubledicked"));
        this.outx(
            `\n\nYou don't bother saying much and choose to just rub yourself against the wet, matted fur of her ass for a bit.  [EachCock] begins to rise and stiffen, thawed by the heat of her body and the friction between you two: your ${this.cockDescript(
                x
            )} drives itself sensually between her thighs and across her sex, while your ${this.cockDescript(
                y
            )} enjoys the warmth and coziness of her ass, rubbing across her taint and teasing her anal ring.  You don't make any attempts at either hole – that comes later.`
        );

        this.outx(
            `\n\nBrooke giggles to herself, letting you jerk yourself off with her lower body.  The ${this.cockDescript(
                x
            )} between her legs caresses and tickles against her pussy while another, ${this.cockDescript(
                y
        ...
        this.outx(
            '"</i> she mentions, almost casually, while you continue to masturbate yourself on her.  <i>"'
        );
        if (this.player.isNaga())
            this.outx(
                "Twice the dick means twice the fun... but twice the work, too.  I'm up to the challenge, though, if you are."
            );
        else
            this.outx(
                "Haven't actually seen anyone that's not half-snake sporting more than one.  Mind if I put 'em to the test?"
            );
        this.outx(
            `"</i>  She rocks her hips a little higher, brushing the ring of her anus across your ${this.cockDescript(
                y
            )} while driving your ${this.cockDescript(x)} further across her labia.`
        );
        ...
        if (this.flags[kFLAGS.TIMES_IN_BROOKES_BUTT] > 0)
            ...
            this.outx(
                '  Brooke hesitates just a bit as she feels the heat of your dick against her asshole.  <i>"Well, first time for everything,"</i> she says to herself.'
            );

        this.outx(
            `\n\nShe teases your dual penises, rubbing her wet holes against your ${this.cockDescript(
                x
            )} and your ${this.cockDescript(
                y
            )}, intent on bringing you to full mast before taking any sort of plunge.  Your hands travel down to her toned, trim hips, and you grasp onto them sharply, helping her rock against you.  She 'mmm's at the feel of your rough hands on her muscles, and the way you continue to drive [eachCock] against her.`
        );
        if (this.player.cockTotal() >= 3)
            this.outx(
                "  Your other dicks rise as well, though not as quickly as the first two, jealous that they were chosen instead."
            );
        ...
        this.outx(
            `\n\nYour tongues work together, arousing you both, preparing [eachCock] for the plunge and preparing herself, sexually and mentally, for the strain she's volunteering to take for your pleasure.  You were both well along before you had started, though: you're not going to get much harder and she's not going to get much wetter (all things considered).  She disengages from you so she can turn herself around, straddling you once more, face-to-face.  Her hips rise along with your ${this.cockDescript(
                x
            )} and your ${this.cockDescript(
                y
            )}, until one head is pushing against her impatient cunt while the other rests against her butthole.`
        );
        ...
        this.outx(
            `\n\nThe ${this.cockDescript(
                x
            )} in her quim slides in tightly, but easily.  The ${this.cockDescript(
                y
            )} pressing against her rim, though, takes a bit more convincing.  She grits her teeth and closes her eyes in concentration, grinding her hips just slightly, back and forth, working the meat into her butt.  Once the ${this.cockDescript(
                y
        ...
        // [if {first time anal}
        if (this.flags[kFLAGS.TIMES_IN_BROOKES_BUTT] == 0)
            this.outx("it's certainly not a workout she's accustomed to,");
        else
            this.outx(
                "no matter how much she loves getting fucked in the butt, it's still not a usual thing for her,"
            );
        this.outx(
            " and for every inch her ass takes, another simultaneously slides into her pussy, stuffing her fuller than she's ever felt."
        );

        this.outx(
            `\n\nYou can feel her glutes become rigid in your palms whenever she moves, the muscles practically turning to stone.  The pressure on your ${this.cockDescript(
                y
            )} is both stimulating and a bit painful; you whisper encouraging nothings to her while your hands work on her cheeks, trying to get them to ease whenever she slides down a bit more.  It takes some patience, but eventually, she gets you all into her, in both ends.`
        );
        ...
        if (this.player.cockTotal() >= 3)
            this.outx(
                "The rest of you lamentably stands between you, leaking their pre uselessly between your bodies, pleading for some attention themselves.  "
            );
        if (this.player.gender == 3)
            this.outx(
                "Your [vagina] clenches in time with its neighbor – you can easily imagine what it might feel like to have two dicks in you at once.  You almost wish for it while you're waiting for Brooke's go-ahead.  "
            );
        this.outx(
            'Brooke\'s face contorts, not in pain but in concentration, while she fidgets with her ass and lower body, adjusting to both of her holes being so totally full.  After minutes of her positioning and repositioning herself, her eyes finally meet yours.  <i>"Okay,"</i> she whispers, <i>"but... a bit gentler than what we\'re used to, okay?"</i>'
        );

        this.outx(
            `\n\nYou accede; it's the least you can do in return.  After all the effort she went through to right herself on you, you pull out from her, ruining all her hard work, and then thrust back in, slowly.  Her breath catches in her throat, but she doesn't object – you repeat yourself, pulling out just as far, and she shucks in some air once you thrust in.  Her pussy squelches and squeezes, but not anything like her anal ring: it's like she's both trying to keep you in and trying to keep you from entering her body, all at once.  Her muscles move and form around your ${this.cockDescript(
                x
            )} and your ${this.cockDescript(
                y
            )} with each movement you make, doing their best to shape themselves around both of your intruders.`
        );

        this.outx("\n\nYour fucking keeps steady and slow.  ");
        if (this.flags[kFLAGS.TIMES_IN_BROOKES_BUTT] == 0)
            this.outx(
                "Brooke's breathing slowly starts to even out, and she starts to moan enjoyably.  Maybe she's starting to like it?"
            );
        else
            this.outx(
                "Brooke rocks her hips onto both of your cocks, alternating from taking one deeper than the other.  As much as she just loves sex in general, it's the alien feeling of being so full that keeps her from going any harder."
            );
        this.outx("  She gasps and jerks with your every thrust, and you grunt, shoving yourself");
        if (this.player.balls > 0) this.outx(" down to your [balls]");
        this.outx(
            " deep into her.  You can practically feel your two dicks rubbing against each other inside her with each thrust."
        );
        ...
        if (this.player.cockTotal() >= 3)
            this.outx(
                "  Your extra equipment, painfully erect, squashes between your bodies, pointing right at the underside of her boobs and your own [chest]."
            );
        this.outx(
            "  You fuck her a bit more forcefully, but not any faster, for both your sakes.  She moans into you: her body's adjusted to the feel, and she's more receptive of your actions."
        );
        ...
        if (this.player.cumQ() <= 349)
            this.outx(
                `Your jizz blasts into her, filling into her womb nice and comfortably and shooting deep into her bowels.  Your both shake: her from the feeling of getting two doses of your cum at once and you from having your ${this.cockDescript(
                    x
                )} and your ${this.cockDescript(
                    y
        ...
        // [if (cumQuantity <= 999)
            ...
            this.outx(
                `Once your orgasm finally, powerfully overtakes you, a glimmer of a thought questions that maybe finishing inside her wasn't the best idea.  Your cum rockets into both her pockets and then some: the ${this.cockDescript(
                    x
                )} in her vagina fills it to the brim in just a few spurts, and the ${this.cockDescript(
                    y
                )} in her ass shoots and shoots into the rest of her, distending even her tight stomach and ruining her form.  She grunts hard, gripping onto you while you fill her out, but the tightness from both your cocks makes keeping herself composed a harder task than she anticipated.  Your orgasm goes for what seems like minutes, and when you're finally done, she looks primed to pop like a balloon.`
            );
        if (this.player.cockTotal() >= 3)
            this.outx(
                "  Your [eachCock] sprays from between you, launching its own jizz up and uselessly between you, forming a warm, sticky blanket between your stomachs.  Some of it even crawls up between her cleavage, sticking them together from the inside, and with her pressed against you like this, it can't wash off right away."
            );
        if (this.player.gender == 3)
            this.outx(
        ...
        this.outx(
            ".  Wearily, she pulls herself away, just enough to look you in the eye, and, without a moment, leans in to kiss you, tongue and all."
        );
    ...
    public mediumAffectionOneTimeEvent(): void {
        this.clearOutput();
        this.outx(
            "You wander your way back into the showers, stopping once again at the lockers to deposit your [armor].  Right on time, you hear the tell-tale sound of rushing water just up ahead, and as usual, you see Brooke washing at her fur lightly in the first shower stall."
        );
    ...
    public brookeSpecialMediumSceneContinued(): void {
        ...
        if (x >= 0 && !this.player.hasVagina()) {
            ...
            this.outx(
                ", and you can feel the heat, the need, pour from Brooke's pussy as she continues to grind on you.  You can hear her pant as she continues to kiss your neck, whimpering at your denial to penetrate her and moaning with each brush across her vulva."
            );

            this.outx(
                `\n\nShe needs it as badly as you do, and you're not about to deny it.  With your grip on her thighs, you bend your [legs] just slightly, lining your ${this.player.cockHead(
                    x
                )} up with her target.  She stops breathing, tensing slightly, waiting for your thrust, and you oblige.  Straightening your legs and pushing forward, you enter her, pushing in only slightly so you can adjust.`
            );
            ...
            if (this.player.balls > 0) this.outx("[sack] resting gently against her crotch");
            else this.outx("crotch resting gently against hers");
            this.outx(", her fur lightly tingling the skin.");
            ...
            this.outx(
                `\n\nA part of you was wishing she'd say that.  You reach down, around her thighs once more, and you lift her off the ground.  She gasps in surprise, as, impaled on your ${this.cockDescript(
                    x
                )} and lifted off the ground, you walk forward and slam her into the other wall of the stall, rattling the whole setup.  With your new leverage, you pull your hips back and slam them back home, rocking the whole shower room.`
            );

            this.outx(
                `\n\nBrooke nearly yelps out, but quickly bites her lip, trying to keep her composure as you pull back and thrust in once more.  You put everything you have into fucking the Shepherd girl: gripping her tightly, you pound into her wet, warm, inviting snatch, feeling every worked muscle lave and caress your ${this.cockDescript(
                    x
                )}.`
            );
            if (this.player.balls > 0)
                this.outx(
                    "  Your [balls] swing away with you, slapping onto her pussy with every inward thrust.  You feel no pain, focused only on fucking this girl raw, knowing she's strong enough to take you."
                );
            if (this.player.cockTotal() == 2)
                this.outx(
                    "  Your other dick swings freely, cold and alone in the shower between you, occasionally spurting pre into the air and onto Brooke's fur.  It's quickly washed away with the water."
                );
            if (this.player.cockTotal() >= 3)
                this.outx(
                    "  Your other dicks swing freely, cold and alone in the shower between you, occasionally spurting pre into the air and onto Brooke's fur.  They're quickly washed away with the water."
                );
            ...
            if (this.player.balls > 0) this.outx("  Your [balls] begin to churn.");
            this.outx(
                `  Your heart races, and your ${this.cockDescript(
                    x
            ...
            if (this.player.cumQ() <= 250)
                ...
                this.outx(
                    `\n\nYou blast her hot tunnel with an orgasm more powerful than any you can recall at the moment.  Seed erupts from your ${this.cockDescript(
                        x
                    )}, driving deeper and deeper into her with each spurt.  She coos and moans with each wave she can feel; she stops her humping and instead takes to grinding, sensuously sliding her crotch with yours, trying to get out every drop you have.`
                );
            else if (this.player.cumQ() <= 1500)
                this.outx(
                    `\n\nYou cum as hard as you ever have yet, punching your seed into her waiting tunnel.  Rope after white rope flies from your ${this.cockDescript(
                        x
                    )}, slowly filling Brooke up with each successive spasm.  She moans, rocking her hips along with yours, feeling each little drop drip from you and into her.  She shudders a bit, feeling another, milder orgasm rip through her with your own.`
                );
                ...
                this.outx(
                    `\n\nYou burst inside of her, immediately filling her to capacity with your intense orgasm.  Your cum can't blast through your ${this.cockDescript(
                        x
                    )} into her fast enough, thickening your dick with each pulse as another glob passes through and into her hungry cunt.  Brooke easily gets what she wants, and then some: she grinds her hips greedily on yours, her pussy voracious for your seed, but despite her best efforts, a small trickle of semen escapes her tight pussy.  She nearly yells out again, feeling your seed sink deeper into her body with every pulse, cumming herself once more – and then once more.`
                );
            if (this.player.cockTotal() == 2)
                this.outx(
                    "  Your other prick tenses along with the first, pointing up and along the tight abs of the Shepherd girl, jizz erupting from the tip and blasting her on the underside of her perky tits.  If she even notices, she certainly doesn't seem to care."
                );
            if (this.player.cockTotal() >= 3)
                this.outx(
                    "  [EachCock] spurts and flails along with the first, painting the stall white with your warm sperm.  It lands on the walls and the floor mostly, but more than a good deal lands on Brooke and into her fur.  She's so out of it from her own orgasm that she barely even notices."
                );
        ...
        // {Female only}
            ...
            this.outx(this.images.showImage("brooke-gym-female-specialscene"));
            this.outx(
                "You can barely keep up with her sheer, wanton desire as her hands grope your body and her tongue conquers your mouth.  You can tell from her squeezing that she's trying to lift your left leg up and across her hips, but, unable to grip or lift with her hands, all she can do is grunt in frustration as she instead takes to humping into you.  You do her a favor and wrap your leg around her anyway."
            );
        ...
        // {Herms only}
            ...
            this.outx(
                `  She finally drops her hands down, one wrapping and caressing carefully, diligently, around your ${this.cockDescript(
                    x
                )} while the other trails the cleft of your [butt], a single claw running between them and towards your taint.  She doesn't waste a moment when she finds your [vagina] instead of plain skin, apparently not unfamiliar with multitasking.`
            );

            this.outx(
                `\n\nYou grind your hips on Brooke's hands as one glides easily up and down your lubricated ${this.cockDescript(
                    x
            ...
            if (this.player.cockTotal() == 1)
                this.outx(
                    `\n\nShe can hardly contain herself as she sets her tongue to work once again.  She opens wide and sucks you into her, clamping her mouth gently shut around your meat, minding her teeth.  She takes it slowly, in contrast to her apparent eagerness: she eases her head down, slowly taking it more and more, until her nose presses against your pelvis and she slowly begins to rise back up.  However, all the while, her tongue is going a mile a minute, pressing and playing, teasing and tasting, working every nerve you have in your sensitive ${this.cockDescript(
                        x
            ...
            if (this.player.cockTotal() == 2)
                this.outx(
                    `\n\nShe looks almost ecstatic, seeing [eachCock] up close.  She keeps her left hand busy with your [vagina], and puts her right back to work stroking your ${this.cockDescript(
                        x
                    )} while she eagerly swallows your other penis.  Her left hand thrusts into you, pressing and searching your pussy, gently trying to find your G-spot.  Her right hand pumps your ${this.cockDescript(
                        x
            ...
            this.outx(
                `\n\nHer request not-forgotten, you put everything your body has into fucking the Shepherd girl.  You're determined to fuck her until she regrets doubting you.  Your hands travel down to her ass, gripping hard for leverage, as you pump your ${this.cockDescript(
                    x
                )} in and out with everything you have.  Brooke's grin fades underneath her swishing light-brown hair and beneath the sheen of the water still clinging to her face while you ravage her.  Her tongue lolls and spittle sometimes convulsively flings from her slack mouth.  Her hands move from the stall wall to your neck, and she looks down at you – her expression is difficult to read, as focused as she is on the pounding you're giving her.  Nonetheless, her brown eyes never leave yours.`
            );

            this.outx(
                `\n\nHer breathing comes out ragged, rising in pitch the longer and harder you go without slowing the littlest bit.  Her breasts bounce with the rest of her, and eventually, she lifts her other leg, forcing you to carry all of her weight in your hands.  You grunt with her, fucking her, feeling her vice down on your ${this.cockDescript(
                    x
                )}, her voice ascending the closer she gets.  <i>"Fuck!"</i> she says occasionally, not having the energy to say it much louder than her senseless moaning.`
            );
            ...
            if (this.player.balls > 0) {
                if (this.player.findStatusAffect(StatusAffects.Uniball) < 0) {
                    this.outx(
                        "  Your [sack] swings with every pump, slapping onto the firm skin of Brooke's vulva every time you bottom out.  You feel no pain, and in fact the smoothness of her fur around her genitals tickles the cum out of your [balls] all the faster."
                    );
                } else {
                    this.outx(
                        "  Your [sack] is to tight and smooth to swing, but it still slaps against the firm skin of Brooke's vulva every time you bottom out.  The smoothness of her fur around her genitals tickles the cum out of your [balls] all the faster."
                    );
                }
            }
            ...
            if (this.player.cockTotal() == 1)
                this.outx(
                    "  You drill into that canine pussy, giving Brooke everything you have.  Your body burns and your cum churns the longer you go.  Her voice ascends from pitiful puppy whining to the jubilant cries of a woman well fucked, and the rise of her yells bring with them the rise of your cum."
                );
            if (this.player.cockTotal() == 2)
                this.outx(
                    `  Your second penis swings, cold and alone, between your body and Brooke's as you continue to saw into her with your ${this.cockDescript(
                        x
                    )}.  The tip points up to the cleft of her swinging, pounding breasts, ready to erupt and coat the underside of her tits with your offering.  She knows of it, and she does what she can to pleasure it, but even her well-defined six-pack can only do so much.`
                );
            if (this.player.cockTotal() >= 3)
                this.outx(
                    "  Unfortunately, as much as both you and Brooke want to pleasure as much of you as possible, she can only do so much.  [EachCock] thrusts into nothing between you, each of them stiff and pointed right up at her face – if they can't cum inside her, then they'll jealously paint her white on the outside."
                );

            this.outx(
                `\n\nShe howls to the ceiling as her body convulses, unable to contain herself anymore.  She orgasms hard, squeezing your ${this.cockDescript(
                    x
            ...
            if (this.player.cumQ() <= 500)
                this.outx(
                    `  Your fluid pumps into her hungry cooch with abandon: each of your heartbeats begets a pulse through your ${this.cockDescript(
                        x
            ...
            // [if (cumVeryHigh = true)]
                ...
                this.outx(
                    `  You orgasm powerfully, the cum ripping through your ${this.cockDescript(
                        x
            ...
            this.outx(
                `\n\nFinally turning the frigidly cold water off, you each make a show of drying yourselves off.  Despite how bow-legged Brooke is moving and despite the wear your ${this.cockDescript(
                    x
                )} is feeling after such a workout... well, to put it one way, you can't wait until you meet her again after this.  Miraculously, you both manage to keep your hands to yourselves, and the rest of your drying ends without incident.  She blows you a kiss before she leaves the locker room, and, after a vigorous dry, you put your [armor] back on and leave the gym.`
            );
            // Lust reduced to 0, sensitivity decreased by 2, fatigue increased by 10
            this.player.orgasm();
            this.dynStats("sen", -2);
    ...
    public specialHeckelAndBrookeEncounter(): void {
        this.clearOutput();
        this.outx(
            "You stroll into the gym, looking for Heckel, but she's not at her usual routine, running laps around the gym.  The gym itself is a little bare; there aren't a lot of people using the machines dotted around the room.  Perfectly good and ready machines, barbells, bench-presses, et al sit ready and waiting for someone to test their mettle on them."
        );
        this.outx(
    ...
    public nopeOutofBroke(): void {
        this.clearOutput();
        this.outx("You shake your head at Heckel, not feeling like playing second fiddle today.");
        this.outx(
    ...
    public submitHeckelXBrookeThreesome(): void {
        this.clearOutput();
        this.outx(this.images.showImage("brooke-gym-heckelbrooke3some"));
        this.outx(
            "Once you snap out of it, you comply, hastily removing your [armor] and leaving you just as naked as her.  Although you never really agreed to something like this in the first place, you <b>did</b> come here to see Heckel for a reason, and seeing the two of them like they are <b>is</b> really hot..."
        );
        ...
        if (this.player.hasCock()) this.outx("hard cock");
        if (this.player.cockTotal() > 1) this.outx("s");
        if (this.player.gender == 2) this.outx("wet, waiting vagina");
        this.outx(
            ', too out of it from the sudden penetration to do as Heckel said.  In response, Heckel thrusts forward much harder, roughly shoving a good third of her dick into Brooke\'s cunt.  <i>"<b>Now!</b>"</i> she demands, pushing her skull forward into your crotch, and finally, Brooke gets the sense to obey.'
        );
        ...
        if (this.player.hasCock())
            this.outx(
                "  You might have taken offense to the idea you're not as good a fuck if you weren't too focused on the amazing blowjob Brooke's giving you."
            );
        ...
        if (this.player.hasCock())
            this.outx(
                "you erupt, spraying your cum directly into Brooke's thirsty throat.  Despite her own ecstasy, she knows to swallow to avoid choking, taking hurried breaths through her nostrils when she can while you coat her mouth white several times over.  Her mouth and her tongue are mostly still, barely conscious enough to move much less pleasure you farther.  You don't mind, and you cum and cum into her mouth regardless."
            );
        if (this.player.gender == 2)
            this.outx(
                "you cum into her mouth, spraying your feminine juices onto her tongue and wherever else Brooke didn't have the sense to cover.  It gets into her mouth; onto her muzzle; and down her face.  You coat her from your end as your pleased cunny quivers and cums again and again.  Brooke, for her part, swallows whatever happens to land into her maw, but she barely even seems to notice whenever your happen to get her anywhere else."
            );
    ...
    public brookeVHeckelBrookeWins2(): void {
        ...
        if (x >= 0) {
            ...
            if (
                ...
                if (bj && anal)
                    this.outx(
                        "  Every time you met Heckel, you either went down on her, or had to take her dick up your ass – now, with the rare opportunity to return the favor, y"
                    );
                else if (bj && !anal)
                    this.outx(
                        "  Every time you met Heckel, you went down on her - now, with the rare opportunity to return the favour, y"
                    );
                else
                    this.outx(
                        "  Every time you met Heckel, you had to take her dick up your ass - now, with the rare opportunity to return the favour, y"
                    );
            } else this.outx("Y");
            this.outx(
                `ou savor every thrust into her tight velvet tunnel.  She's tight, far tighter than you had expected, but she's definitely no virgin either.  You draw your hands down to her hips, gripping and pulling delightedly at her tight muscles as you work her cunt, whetting your ${this.cockDescript(
                    x
            ...
            this.outx(
                `\n\nYour arms and your back begin to strain a bit from having to balance both of the horny bitches as they bounce on you and on each other, but the rising pleasure in your ${this.cockDescript(
                    x
                )} overrides them both: you're not too far off, and from the rising grunts from Brooke, neither is she.  She finally lets go of Heckel's face, smiling playfully down on the hyena, not caring to wipe the sweat from her brow as she continues her ride.`
            );
            ...
            this.outx(
                `\n\nHeckel cries out shortly after Brooke and begins flooding the eager Shepherd girl's cunt with her cum, blasting her womb with her potent spunk again and again, her whole body shaking and shivering uselessly, a collective slave to her cock taking all the energy out of her.  Her pussy spasms and clamps down on your ${this.cockDescript(
                    x
                )} simultaneously, milking you for every drop you have, and with a few barbaric thrusts and a wordless cry, you oblige, shooting your own load into her hungry vagina.`
            );
            if (this.player.hasKnot(x))
                this.outx(
                    `  Likewise with Brooke, you pound your knot against Heckel's quim again and again, refusing to be denied entry, and with your orgasm, her pussy relents, taking in every inch of your ${this.cockDescript(
                        x
                    )} with you.`
                );
            this.outx(
            ...
            if (this.player.gender == 3)
                this.outx(
                    `.  Your [vagina] pulses in time with your ${this.cockDescript(
                        x
                    )}, spraying your feminine juices down the inside of your tired legs uselessly, lonely and longing for a cock of its own to take`
                );
            if (this.player.cockTotal() == 2)
                this.outx(
                    ".  Your second dick flails and pulses, ejaculating in time with its lucky brother, shooting up and coating Heckel's back with your jism"
                );
            if (this.player.cockTotal() >= 3)
                this.outx(
                    " while [eachCock] pumps yet more of your virile jism every which way, staining the tile floor and tagging the lockers surrounding you with your cum"
                );
            this.outx(".");
        }
        // {Females only}
