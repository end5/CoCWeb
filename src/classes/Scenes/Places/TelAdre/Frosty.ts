...
export class Frosty extends TelAdreAbstractContent {
    ...
    public approachFrosty(): void {
        ...
        if (this.flags[kFLAGS.MET_FROSTY] == 0) {
            ...
            this.addButton(
                0,
                "Yes",
                this.yesFrostyLetsHaveSexForCupcakesWhoThoughtThisWasAGoodIdeaIMeanSeriously
            );
            ...
            this.outx("1. Cupcake x 1 = 15gems\n");
            this.outx("2. Cupcake x 5 = 60gems\n");
            this.outx("3. Cupcakes x 10 = 110gems\n");
            this.outx("4. Cupcakes x 20 = 200gems\n");
            this.outx(
    ...
    public frostyMainMenu(): void {
        ...
        if (this.flags[kFLAGS.SIGNED_FROSTYS_CONTRACT] == 0)
    ...
    public frostySpecialsMenu(free = false): void {
        ...
        else if (!free && this.flags[kFLAGS.FROSTY_POINTS] < 15)
        ...
        if (free || this.flags[kFLAGS.FROSTY_POINTS] >= 15)
            this.addButton(1, "Mouth", this.useFrostysMouth);
        if (free || this.flags[kFLAGS.FROSTY_POINTS] >= 1)
    ...
    public noIDontWantAFreebieDiseaseYouSlut(): void {
        this.clearOutput();
        this.outx(
            "You’re too busy right now, so you deny the free service. “<i>Okeydokey, see you around, [name]!</i>” Frosty does a twirl on one foot and runs back to her stand. Such a nice girl."
        );
        this.menu();
    ...
    public noContractForMeSloot(): void {
        this.clearOutput();
        this.outx(
            "Something about signing a contract doesn’t seem right to you, especially one for cupcakes. You shake your head no; she looks a bit sad about being rejected but quickly perks back up."
        );
        this.outx(
    ...
    public frostysHandsAreColdHolyShitWhyDontYouWarmTHoseMitsUp(): void {
        ...
        if (this.player.hasCock() && !this.player.hasVagina()) {
            ...
            this.outx("\n\n“<i>Ooooh, your ");
            if (this.player.biggestCockLength() < 7) this.outx("little");
            else this.outx("big");
            this.outx(" friend is just waiting for me to milk his cream out. Tee-hee.</i>”");
    ...
    */
    ...
    function analWithACupcakeSloot(): void
    {
        ...
        if(player.hasCock())
        {
            outx("“<i>So you wanna be the pitcher or the catcher?</i>” she ask with devilish smile. What the hell is that suppose to mean?");
            // [pitcher?]  [catcher?]
        }
        else
        {
