...
export class Heckel extends TelAdreAbstractContent {
    ...
    */
    ...
    public dominateHeckel(): void {
        this.clearOutput();
        this.outx("Smirking, you fold your arms and tell her to bend over - ");
        if (this.flags[kFLAGS.TIMES_DOMMED_HECKEL] == 0)
            this.outx("there's a new top dog just waiting to claim the town bitch");
        else this.outx("you're back for a taste of your favorite bitch");
        this.outx('.  Heckel snorts, "<i>Nice try, fuck-meat, but ');
        if (this.flags[kFLAGS.TIMES_DOMMED_HECKEL] > 0) this.outx("last time was a fluke");
        else this.outx("I'm always on top");
        this.outx(
        ...
        if (this.flags[kFLAGS.TIMES_LOST_HECKEL_DOM_CHALLENGE] > 0) this.outx(" again");
        this.outx('?</i>"  Her tough finger-tip slams into your solar plexus for emphasis');
        if (this.player.tou < 60) this.outx(", pushing you back a bit");
        this.outx(".");
        this.outx(
        ...
        // {Win}
            ...
            if (this.flags[kFLAGS.TIMES_DOMMED_HECKEL] > 0) this.outx(" AGAIN");
            this.outx('...</i>"');
    ...
    private dominateHeckelConclusion(): void {
        ...
        if (this.player.cockTotal() > 2)
            this.outx(", fanning the others across her neck and forehead, burying her in penis.");
        else if (this.player.cockTotal() == 2)
            this.outx(
                ", dropping your [cock smallest] alongside it over her eyes, burying her in penis."
            );
        else this.outx(", draping her in hard penis.");
        this.outx(
        ...
        if (this.player.cockTotal() > 1) this.outx("s throb and thicken");
        else this.outx(" throbs and thickens");
        this.outx(
        ...
        if (this.player.cumQ() < 250) this.outx("single drop");
        else if (this.player.cumQ() < 1000) this.outx("steady drip");
        else this.outx("never-ending flow");
        this.outx(
        ...
        if (this.player.biggestCockArea() > 40) this.outx("as much of it as possible");
        else this.outx("it");
        this.outx(
        ...
        if (this.player.cumQ() > 500) {
            ...
            if (this.player.cumQ() >= 3000) {
                this.outx("  That puddle then drips to the floor to make an alabaster lake");
                if (this.player.cumQ() >= 4000) {
                    this.outx(" at least ");
                    if (this.player.cumQ() < 10000) this.outx("a foot deep");
                    else if (this.player.cumQ() < 15000)
                        this.outx("knee high by most measurements");
                    else if (this.player.cumQ() < 20000) this.outx("waist high");
                    else this.outx("as high as the table");
                }
                this.outx(".  Thank the gods for drains.");
            }
        }
        if (this.player.cockTotal() > 1) {
            this.outx("  Your other member");
            if (this.player.cockTotal() > 2) this.outx("s contribute");
            else this.outx(" contributes");
            this.outx(
        ...
        if (this.silly()) {
            this.outx("  You grab a bacon shake from the shake-cart on your way out");
            if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00281] > 0)
                this.outx(", then give Lottie a high five");
            this.outx(".  You really love bacon.");
        }
        this.player.orgasm();
        this.dynStats("sen", -2);
        this.doNext(this.camp.returnToCampUseOneHour);
