...
export class AuntNancy extends TelAdreAbstractContent {
    ...
    public auntNancy(display: boolean = true): boolean {
        if (this.model.time.hours >= 6 && this.model.time.hours <= 14) {
            if (display) {
                ...
                //[If MetNancy = 0]
                if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00263] == 0) this.outputText("  She", false);
                else this.outputText("  Aunt Nancy", false);
                this.outputText(" glances up at you and smiles as you enter", false);
                //[(if unmet)
                if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00263] == 0) this.outputText(", and you see that she has additional red eyes spread across her forehead", false);
    ...
    public interactWithAuntNancy(): void {
        ...
        if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00264] >= 30 && this.model.time.hours == 14 && this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00266] == 0) {
    ...
    private lightStuff(): void {
        ...
        //[If Intelligence >= 20]
        if (this.player.inte >= 20) this.outputText("  You realize that what look like almost-shoulder-length gloves over her hands must actually be shell-like portions of this exoskeleton.", false);
        this.outputText("\n\n", false);
    ...
    private timeForAuntNancySpiderCooch(): void {
        ...
        if (this.player.biggestTitSize() >= 1) this.outputText(" and taking the opportunity to grope your " + this.player.allBreastsDescript() + " right through your " + this.player.armorName, false);
        this.outputText(".\n\n", false);
    ...
    private auntNancyPoonPartII(): void {
        ...
        //[Intelligence > 40]
        if (this.player.inte > 40) this.outputText("realize that the spit must contain some kind of chemical originally meant to still prey", false);
        else this.outputText("question whether or not the spider-lady's kiss is responsible", false);
        this.outputText(".\n\n", false);
        ...
        if (this.player.tailType > TAIL_TYPE_NONE) this.outputText(" to the top of your tail", false);
        this.outputText(".\n\n", false);

        //[If a naga]
        if (this.player.isNaga()) this.outputText("Gently, she peels away the fold of skin that disguises your genitals, now moist-and-dripping with anticipation.  ", false);
        ...
        if (this.player.balls > 0) {
            ...
            if (this.player.ballSize < 15) {
                if (this.player.ballSize > 5 && this.player.ballSize < 15) this.outputText("taking one", false);
                else this.outputText("both of them", false);
                this.outputText(" into her mouth to soak in her saliva as she wraps and caresses ", false);
                if (this.player.ballSize <= 5) this.outputText("them", false);
        ...
        if (this.player.hasVagina()) {
            ...
            if (this.player.balls > 0 && this.player.hasCock()) this.outputText("lifts away your other \"<i>equipment</i>\" and ", false);
        ...
        if (this.player.hasCock()) this.outputText("Aunt Nancy rises up above you, still smiling with gentle sensuality, and reaches out.  ", false);
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.cockTotal() >= 2) {
                if (this.player.cocks[y].cockLength >= 22) {
                    ...
                    if (this.player.totalCocks() > 2) this.outputText("another one of ", false);
                    this.outputText("your ", false);
                    if (this.player.cockTotal() == 2) this.outputText("other cock ", false);
                    else this.outputText("cocks ", false);
                    ...
                    if (this.player.cocks[y].cockLength >= 30) this.outputText("letting its head slip into her mouth, where she sucks and licks at it hungrily, ", false);
                    else this.outputText("letting her long, dexterous tongue dangle down from her mouth to wrap around the head of her dick, occasionally licking at her own impressive bosom as well, ", false);
                ...
                if (this.player.cockTotal() > 2) {
                    if (this.player.cockTotal() == 3) this.outputText("Another of her hands wraps itself around your other cock", false);
                    else this.outputText("Her other hands wrap themselves around more of your cocks", false);
                    this.outputText(", milking and stroking the ", false);
                    this.outputText("pulsating prick", false);
                    if (this.player.cockTotal() > 3) this.outputText("s", false);
                    this.outputText(" for all ", false);
                    if (this.player.cockTotal() == 3) this.outputText("it's", false);
            ...
            if (!titFucked || (titFucked && this.player.cockTotal() <= 2)) {
                ...
                if (this.player.balls > 0) this.outputText("gently kneading and caressing your " + this.ballsDescriptLight() + " as the cum inside churns and boils and ", false);
                //[If the player has a vagina]
                else if (this.player.hasVagina()) this.outputText("exploring the folds and creases of your " + this.vaginaDescript(0) + " and ", false);
        ...
        //[If player has vagina but no dicks: (Nancy prefers dicks and will always choose them over lesbian sex)]
            ...
            if (this.player.tailType > TAIL_TYPE_NONE) this.outputText("tail before rising to your ", false);
    ...
    private auntNancyPoonPartIII(): void {
        ...
        if (this.player.hasCock()) {
            ...
            //[If PC has balls]
            if (this.player.balls > 0) this.outputText(this.ballsDescriptLight() + " tense, churning and tightening.", false);
            //Else:]
            ...
            if (this.player.cumQ() >= 750) this.outputText("  Sooner or later though, even her marvelous body can simply contain no more of your jizz, and she slips her swollen cunt up off of your organ, letting it continue squirting your thick, plentiful cum all over her as it swings free.", false);
            this.outputText("\n\n", false);
            ...
            if (this.player.cocks[y].cockLength >= 22 && this.player.cockTotal() > 1) {
                ...
                //[If PC has cum multiplier of at least 2]
                if (this.player.cumQ() >= 750) this.outputText(", some even reaching to the ceiling to fall into her short, pretty white hair and splattering over her trailing abdomen", false);
                this.outputText(".\n\n", false);
            }
            //[If there are still dicks left over:]
            if (this.player.cockTotal() > 2) {
    ...
    private auntNancyPoonPartIV(): void {
        ...
        if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00266] == 0) {
            ...
            if (this.player.gender > 0) this.outputText("genitals", false);
            else this.outputText("jaw", false);
