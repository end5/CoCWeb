...
export class Benoit extends BazaarAbstractContent {
    ...
    public benoitMF(stringM: string, stringF: string): string {
        if (this.flags[kFLAGS.BENOIT_STATUS] == 1 || this.flags[kFLAGS.BENOIT_STATUS] == 2)
            return stringF;
        return stringM;
    }
    ...
    public benoitAffection(changes = 0): number {
        if (changes != 0) {
            ...
            if (this.flags[kFLAGS.BENOIT_AFFECTION] > 100)
                this.flags[kFLAGS.BENOIT_AFFECTION] = 100;
            else if (this.flags[kFLAGS.BENOIT_AFFECTION] < 0)
                this.flags[kFLAGS.BENOIT_AFFECTION] = 0;
        }
    ...
    private benoitKnocksUpPCCheck(): void {
        ...
        if (
            this.player.pregnancyType == PregnancyStore.PREGNANCY_BASILISK &&
            this.player.findPerk(PerkLib.BasiliskWomb) >= 0
        )
            this.player.knockUpForce(
                PregnancyStore.PREGNANCY_BENOIT,
        ...
        if (
            ...
            if (
                ...
                if (
                    ...
                    this.player.knockUp(
                        PregnancyStore.PREGNANCY_BENOIT,
                        PregnancyStore.INCUBATION_BASILISK
                    );
                }
    ...
    public benoitVeryHeavyPreggers(): boolean {
        if (this.flags[kFLAGS.BENOIT_STATUS] == 0) return false;
        if (this.flags[kFLAGS.FEMOIT_EGGS] >= 9 && this.flags[kFLAGS.FEMOIT_EGGS] <= 12)
            return true;
        return false;
    }
    ...
    */
    ...
    public clearBenoitPreggers(): void {
        if (this.flags[kFLAGS.FEMOIT_EGGS] != 0) {
            ...
            this.flags[kFLAGS.FEMOIT_NEXTDAY_EVENT] = this.getGame().model.time.days; // Cycle "resets" based off birth day.
        }
    }
    ...
    public benoitIntro(): void {
        ...
        if (this.flags[kFLAGS.TIMES_IN_BENOITS] == 0) {
            ...
            this.flags[kFLAGS.BENOIT_SUGGEST_UNLOCKED] == 0 &&
            ...
            (this.player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS ||
                this.player.pregnancyIncubation == 0) &&
            this.flags[kFLAGS.BENOIT_STATUS] == 0
        ) {
            if (this.flags[kFLAGS.BENOIT_SUGGEST_UNLOCKED] == 0)
                this.benoitAndFemPCTalkAboutEggings();
            suggest = this.eggySuggest;
            suggestText = "Suggest";
        } else if (
            this.flags[kFLAGS.FEMOIT_NEXTDAY_EVENT_DONE] == 1 &&
            this.flags[kFLAGS.FEMOIT_NEXTDAY_EVENT] <= this.getGame().model.time.days &&
            this.flags[kFLAGS.BENOIT_STATUS] == 0
        ) {
            this.femoitNextDayEvent();
        } else if (
            this.benoitInClutch() &&
            this.flags[kFLAGS.FEMOIT_READY_FOR_EGGS] == 0 &&
            (this.flags[kFLAGS.BENOIT_STATUS] == 1 || this.flags[kFLAGS.BENOIT_STATUS] == 2)
        ) {
            this.flags[kFLAGS.FEMOIT_READY_FOR_EGGS]++;

            this.outx(
                'As you enter the stall Benoite looks up; though her blind, milky eyes make her harder to read, she looks excited. "<i>Ah! [name]; it iz good you are here... I had hope zat you would come for a vizit today.</i>"'
            );
            ...
            !this.benoitInClutch() &&
            !this.benoitPreggers() &&
            this.getGame().model.time.days - this.flags[kFLAGS.FEMOIT_NEXTDAY_EVENT] >= 30 &&
            this.flags[kFLAGS.BENOIT_STATUS] > 0
        ) {
            if (this.flags[kFLAGS.FEMOIT_FIRST_CLUTCH_MISSED] == 0) {
                this.flags[kFLAGS.FEMOIT_FIRST_CLUTCH_MISSED]++;
                this.outx(
                    "When you enter the stall, you are greeted by the smell of something cooking. Investigating further brings you to the blind basilisk's small kitchen, where she is busy frying something. Her nose preoccupied with her meal, she doesn't realize you're approaching until you touch her shoulder, yelping in shock."
                );
            ...
            if (this.benoitAffection() <= 10)
                this.outx(
                    'Once again, you carefully enter the gloom of Benoit\'s salvage shop.  The proprietor sniffs the air as you enter, and then looks at you sourly.  "<i>Well?</i>" he rasps.'
                );
            // Benoit reacts after fucking the PC.
            else if (
                this.flags[kFLAGS.BENOIT_TIMES_SEXED_FEMPCS] == 1 &&
                this.flags[kFLAGS.BENOIT_POST_FIRSTFUCK_TALK] == 0
            )
        ...
        if (
            ...
            this.player.findPerk(PerkLib.BasiliskWomb) < 0 &&
            this.flags[kFLAGS.BENOIT_TESTED_BASILISK_WOMB] == 0 &&
            this.flags[kFLAGS.BENOIT_STATUS] == 0
        )
    ...
    public benoitsBuyMenu(): void {
        ...
        if (this.flags[kFLAGS.BENOIT_STATUS] == 1) {
            buyMod = 1.66;
            this.outx(
                '"<i>Some may call zis junk,</i>" says Benoite, indicating her latest wares.  "<i>Me... I call it garbage.</i>"'
            );
        } else {
            this.outx(
    ...
    private benoitTransactBuy(slot = 1): void {
        ...
        if (this.player.gems < Math.floor(buyMod * itype.value)) {
            this.outx(
                "You consider making a purchase, but you lack the gems to go through with it."
            );
    ...
    private benoitSellTransact(slot: number, sellMod: number): void {
        ...
        if (this.benoitLover())
            ...
            this.outx(
                "Following a painstaking examination of what you've given him with his hands and nose, Benoit grudgingly accepts it and carefully counts out your reward."
            );
    ...
    public updateBenoitInventory(): void {
        ...
        if (this.temp == 0) this.flags[kFLAGS.BENOIT_1] = this.consumables.INCUBID.id;
        else if (this.temp == 1) this.flags[kFLAGS.BENOIT_1] = this.consumables.MINOBLO.id;
        else if (this.temp == 2) this.flags[kFLAGS.BENOIT_1] = this.consumables.MINOCUM.id;
        else if (this.temp == 3) this.flags[kFLAGS.BENOIT_1] = this.consumables.EQUINUM.id;
        else if (this.temp == 4) this.flags[kFLAGS.BENOIT_1] = this.consumables.BLACKPP.id;
        else if (this.temp == 5) this.flags[kFLAGS.BENOIT_1] = this.consumables.SMART_T.id;
        else if (this.temp == 6) this.flags[kFLAGS.BENOIT_1] = this.consumables.VITAL_T.id;
        else this.flags[kFLAGS.BENOIT_1] = this.consumables.DBLPEPP.id;
        ...
        if (this.temp == 0) this.flags[kFLAGS.BENOIT_2] = this.consumables.SUCMILK.id;
        else if (this.temp == 1) this.flags[kFLAGS.BENOIT_2] = this.consumables.W_FRUIT.id;
        else if (this.temp == 2) this.flags[kFLAGS.BENOIT_2] = this.consumables.WETCLTH.id;
        else if (this.temp == 3) this.flags[kFLAGS.BENOIT_2] = this.consumables.GLDSEED.id;
        else if (this.temp == 4) this.flags[kFLAGS.BENOIT_2] = this.consumables.LABOVA_.id;
        else if (this.temp == 5) this.flags[kFLAGS.BENOIT_2] = this.consumables.SNAKOIL.id;
        else if (this.temp == 6) this.flags[kFLAGS.BENOIT_2] = this.consumables.S_GOSSR.id;
        else this.flags[kFLAGS.BENOIT_2] = this.consumables.B_GOSSR.id;
        ...
        if (Benoit.rand(100) < 4) {
            ...
            if (this.temp == 0) this.flags[kFLAGS.BENOIT_2] = this.consumables.BIMBOLQ.id;
            else if (this.temp == 1) this.flags[kFLAGS.BENOIT_2] = this.consumables.L_PNKEG.id;
            else if (this.temp == 2) this.flags[kFLAGS.BENOIT_2] = this.consumables.L_BLUEG.id;
            else if (this.temp == 3) this.flags[kFLAGS.BENOIT_2] = this.consumables.BROBREW.id;
            else this.flags[kFLAGS.BENOIT_2] = this.consumables.TSTOOTH.id;
        }

        // Slot 3 Any one of the following: Maid's Clothes, Wizard Robes, Tough Silk, Slutty Swimwear, Goo Chunk, Chitin Plate
        this.temp = Benoit.rand(5);
        if (this.temp == 0) this.flags[kFLAGS.BENOIT_3] = this.armors.W_ROBES.id;
        else if (this.temp == 1) this.flags[kFLAGS.BENOIT_3] = this.useables.T_SSILK.id;
        else if (this.temp == 2) this.flags[kFLAGS.BENOIT_3] = this.armors.S_SWMWR.id;
        else if (this.temp == 3) this.flags[kFLAGS.BENOIT_3] = this.useables.GREENGL.id;
        else this.flags[kFLAGS.BENOIT_3] = this.useables.B_CHITN.id;
        ...
        if (Benoit.rand(10) == 0) {
            ...
            if (this.temp == 0) this.flags[kFLAGS.BENOIT_3] = this.armors.BONSTRP.id;
            else this.flags[kFLAGS.BENOIT_3] = this.armors.NURSECL.id;
        }
    ...
    private talkToBenoit(): void {
        ...
        if (
            ((this.flags[kFLAGS.BENOIT_TIMES_SEXED_FEMPCS] > 2 &&
                this.player.inte >= 60 &&
                this.flags[kFLAGS.BENOIT_WOMB_TALK_UNLOCKED] == 0) ||
                this.flags[kFLAGS.BENOIT_TIMES_SEXED_FEMPCS] == 2) &&
            this.player.hasVagina()
        ) {
            this.outx(
        ...
        // First time Femoit talk
            ...
            this.flags[kFLAGS.FEMOIT_TALKED_TO] == 0 &&
            this.flags[kFLAGS.BENOIT_STATUS] != 0
        ) {
            this.flags[kFLAGS.FEMOIT_TALKED_TO]++;
            this.outx(
                "You ask Benoite if she intends to go back to the mountains now.  She laughs long and hard at this.  One thing the transformation has certainly gifted her is an extraordinarily filthy laugh."
            );
            ...
            this.flags[kFLAGS.BENOIT_TALKED_TO_PROPERLY] != 0 &&
            this.benoitAffection() >= 40 &&
            this.flags[kFLAGS.BENOIT_TIMES_SEXED_FEMPCS] == 0 &&
            this.flags[kFLAGS.FEMOIT_UNLOCKED] == 0
        ) {
        ...
        // Subsequent Talk
            ...
            if (choice == 0) {
                ...
                if (this.benoitMF("he", "she") == "he")
                    this.outx(
                        "  There's a hint of longing jealousy in the basilisk's voice and when he lapses into silence you decide not to push the subject."
                    );
            }
            ...
            // (Randomly generated.  Added to normal talk options after PC has had sex with Benoit two times or more regardless of womb quest.)
                ...
                this.outx(
                    "You ask Benoit there is anything useful he can tell you about the demon strongholds."
                );
                ...
                this.outx(
                    "You ask Benoit if he can suggest anything to help you fight his brethren in the high mountains."
                );
                this.outx(
                ...
                this.outx(
                    "You ask Benoit if he really, <b>really</b> can tell who you are just by smell."
                );
                ...
                this.outx("You ask Benoite if she really can tell who you are just by smell.");
    ...
    private benoitAndFemPCTalkAboutEggings(): void {
        ...
        this.outx(
            "Benoit seems very on edge today.  He paces up and down, returns your greeting with a stiff nod, and fiddles endlessly with the stock in reach of his counter as you search for a topic.  You ask if there's something wrong."
        );
    ...
    private eggySuggest(): void {
        this.clearOutput();
        if (this.flags[kFLAGS.BENOIT_TESTED_BASILISK_WOMB] == 0.5) {
        ...
        if (this.flags[kFLAGS.BENOIT_TIMES_SEXED_FEMPCS] > 0) {
            if (this.player.isTaur()) {
                this.outx(
                    "You silently reach across the counter to squeeze his hands again.  Benoit grins with deep affection at you and, hand in hand, the two of you quietly debunk to the store room again."
                );
                this.outx(
                ...
                this.outx(
                    "You silently reach across the counter to squeeze his hands again.  Benoit beams at you and, hand in hand and without a word, the two of you depart to the store room again."
                );
        ...
        if (this.player.isTaur()) {
            // Suggest:
            this.outx(
                "You reach your fingers across the counter and lightly touch Benoit's hands, saying you don't mind working out a few natural urges, if he's in the mood."
            );
            this.outx(
            ...
            if (this.player.gender == 3) {
                ...
                if (this.player.longestCockLength() <= 10)
                    this.outx(
                        "  Although this is evidently an uncanny experience for him, he does manage a cocky smile as his hand moves around your male sex. “Mine is bigger,” he teases.  Unable to reward his cheek from where he is, you simply snort and shuffle your hooves, impatient now with need."
                    );
                // [cock  >10 inches:
                else
                    this.outx(
                        "  This is evidently an uncanny experience for him, the alien nature of it deepening as his hands moves around your male sex. “Oly Gods [name], you are a monster,” he says thickly.  You smile and stamp your hooves, impatient now with need."
                    );
            }
            this.outx(
            ...
            if (this.player.inHeat)
                this.outx(
                    "  By now your vagina is practically gushing, your bodies' own deep seated pheromone need stoked to blazing heights by the basilisk's gentle, painstaking exploration of your body.  You cannot stop thrusting yourself against his soaked hand, announcing how badly you want this with heavy moans."
                );
            ...
            if (this.player.lactationQ() >= 50)
                this.outx(
                    "  The slow, sensual sex is enough for you to begin to bead milk from your sensitive [nipples]; you moan as the flow intensifies until you are instinctively kneading your [chest], spurting your sweet, warm fluids onto the floor.  Benoit starts in genuine amazement at the sound.  “Mammals are so damn weird,” he mutters, making you giggle."
                );
            ...
            if (this.player.gender == 3)
                this.outx(
                    "  [EachCock] thickens and spurts in sympathy to your female high, spattering the floor with white paint."
                );
        } else {
            this.outx(
                "You reach your fingers across the counter and lightly touch Benoit's hands, saying you don't mind working out a few natural urges, if he's in the mood."
            ...
            // [Reptile/Naga:
                ...
                (this.player.skinType == SKIN_TYPE_SCALES &&
                    (this.player.lizardScore() >= 3 || this.player.nagaScore() >= 3)) ||
            ...
            if (this.player.inHeat)
                this.outx(
                    " By now your vagina is practically gushing, your body's own deep-seated pheromone need stoked to blazing heights by the basilisk's gentle, painstaking exploration.  You cannot stop yourself thrusting against his soaked hand, announcing how badly you want this with heavy moans."
                );
            ...
            if (this.player.lactationQ() > 100) {
                this.outx(
                    "  You moan as your breasts reward his attention with spurts of milk; he starts in genuine amazement when they "
                );
                if (!this.player.isTaur()) this.outx("hit him in the face");
                else this.outx("trickle over his fingers");
                this.outx('.  "<i>Mammals are so damn weird,</i>" he mutters, making you giggle.');
            }
            this.outx(
        ...
        if (
            ...
            this.outx(
                "  I cannot give you babies unless you 'ave eggs.  I guess I should think a bit more before I go digging for things...</i>\""
            );
    ...
    private takeBenoitsContraceptives(): void {
        this.clearOutput();
        this.outx(
            "You gladly accept the herbal contraceptive and push it into your mouth, enjoying the pleasantly sharp, citrus flavour."
        );
    ...
    private dontTakeEggtraceptives(): void {
        this.clearOutput();
        this.outx(
            "You smile and say you don't mind carrying and laying a few basilisk eggs. \"<i>You... you don't?</i>\" he says hesitantly.  He faces you and for a moment looks like he's going to say something else; but then he shakes his head and puts the bag back into the drawer."
        );
    ...
    private firstTimeAfterBoningEncounterBenoit(): void {
        ...
        this.outx(
            "A strange, faint noise is emanating from the salvage shop.  It's only when you duck cautiously into the stall proper that you realize it's the sound of a basilisk humming.  Benoit stops to sniff the air when you come in, immediately puts down the mug he is polishing, and beckons you inside."
        );
    ...
    private repeatSexWithBenoitLetHim(): void {
        ...
        if (this.player.isTaur()) {
            ...
            if (this.player.biggestTitSize() >= 1) this.outx("and [fullChest] ");
            this.outx(
            ...
            if (this.player.isBiped()) this.outx("knees");
            else this.outx("lower body");
            this.outx(
                ".  His hands descend upon your [butt]; grinning, you poke it outwards invitingly, sighing with affected frustration as the blind basilisk painstakingly lines up his next shot with your drooling, sticky sex."
            );
    ...
    private repeatBenoitFuckTakeCharge(): void {
        ...
        if (this.player.isTaur()) {
            ...
            // Large capacity:
                ...
                this.outx(
                    "You slide back down onto him, cooing this time as your groin meets his muscular thighs with a wet kiss, your sex swallowing his cock whole.  You begin to ride him hard and slow, bending his cock upwards to push at your sensitive walls, waves of pleasure starting to flow through you."
                );
    ...
    private tryToConvertToBassyWomb(): void {
        ...
        if (
            !(
                this.player.hasItem(this.consumables.OVIELIX, 2) &&
                this.player.hasItem(this.consumables.REPTLUM) &&
        ...
        // Ingredients in inventory:
            ...
            this.outx(
                "You ferret out the ingredients you have collected and begin to bang them onto the counter in front of Benoit, telling him that you've got what he needs.  Pierre barks excitedly at the noise."
            );
            ...
            if (this.player.findPerk(PerkLib.Oviposition) < 0)
                this.outx(
                    "  You feel slightly bloated, but otherwise fine; you sense that you can lay eggs of your own volition now."
                );
            else
                this.outx(
                    "You feel slightly bloated, but otherwise fine; you sense that were you to get impregnated by basilisk seed, the eggs you produce would be pure basilisk."
                );
            ...
            this.flags[kFLAGS.BENOIT_TESTED_BASILISK_WOMB] = 0.5;

            // [Lust 30 or more:
            if (this.player.lust >= 33) {
                this.outx(
                    "  You grin and say you're not sure it worked, but you suppose there's only one real way of finding out..."
                );
                if (this.player.tallness <= 78 && !this.player.isTaur())
                    this.outx(
                        "  The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and hoists you over his shoulder.  You squeal in mock terror as he hauls you as fast as he can into the back room, knocking over half his stock as he does."
                    );
                else
                    this.outx(
                ...
                this.outx(
                    "  You grin and say you're not even sure it worked... but you'll be back at some point to try it out, and he'd better be ready for when you do.  You gently pry yourself out of his grip and leave as deliberately as you can, aware of the beguiling, invisible scent you are leaving for the stunned, silent basilisk to simmer in."
                );
            this.doNext(this.camp.returnToCampUseOneHour);
        }
    }
    ...
    private suggestSexAfterBasiWombed(later = true): void {
        ...
        if (later) {
            this.outx(
                "You smile coquettishly and ask if he's interested in taking your new body for a... test drive.  "
            );
            // (6'6 or less:
            if (this.player.tallness <= 78 && !this.player.isTaur())
                this.outx(
        ...
        if (!this.player.isTaur()) {
            ...
            if (this.player.tallness > 78)
                ...
                this.outx(
                    "Benoit staggers into the back room, bumping into several things as he sets you down, thankfully onto a clear space on the ground.  He leans into you, his weight pushing you onto the ground, hands flying over your [armor], fumbling off clasps and belts when he finds them.  Now that your gut has settled you feel imbued with an odd, ripe sensation; your belly bulges ever-so-slightly with unfertilized eggs and you feel red, soft and ready.  You work with him, wriggling out of your clothes and ripping his own trousers off, spreading your [hips] eagerly as his long, smooth prick springs to attention."
                );
            ...
            if (this.player.hasCock())
                this.outx(
                    "  He deliberately rubs himself up and down your body, and the [cock] trapped between your warm bodies quickly hardens against the warm, smooth friction."
                );
            this.outx(
            ...
            if (this.player.hasCock())
                this.outx(
                    "  He deliberately moves his hand over [oneCock] before trapping it in his warm grasp.   It quickly hardens against his warm, smooth friction."
                );
            this.outx(
        ...
        if (this.player.pregnancyType == PregnancyStore.PREGNANCY_BASILISK)
            this.player.knockUpForce(
                PregnancyStore.PREGNANCY_BENOIT,
                this.player.pregnancyIncubation
            );
    ...
    public popOutBenoitEggs(): void {
        ...
        if (this.player.hasCock()) this.outx(" and a downpour of semen");
        this.outx(".");
        ...
        if (this.flags[kFLAGS.BENOIT_EGGS] > 0) this.outx(" once more");
        this.outx(".  Well, he will be when this ");
        if (this.flags[kFLAGS.BENOIT_EGGS] > 0) this.outx("latest ");
        this.outx("clutch hatches, anyway.");
    ...
    public benoitFeminise(): void {
        ...
        // Ingredients not in inventory
        if (
            !this.player.hasItem(this.consumables.P_S_MLK, 2) ||
            !this.player.hasItem(this.consumables.L_PNKEG) ||
            !this.player.hasItem(this.consumables.OVIELIX) ||
            !this.player.hasItem(this.consumables.REPTLUM)
        ) {
            this.outx("You don't have the necessary ingredients to attempt this yet.");
            this.outx(
            ...
            this.flags[kFLAGS.FEMOIT_NEXTDAY_EVENT] = this.getGame().model.time.days + 1;
            this.flags[kFLAGS.FEMOIT_NEXTDAY_EVENT_DONE] = 1;
    ...
    public femoitNextDayEvent(): void {
        ...
        this.outx(
            "A strange, faint sound is emanating from the salvage shop.  It's only when you duck cautiously into the stall proper that you realize it's the sound of a basilisk humming.  Benoit stops to sniff the air when you come in, immediately puts down the mug she is polishing, and beckons you inside."
        );
    ...
    public femoitFirstTimeNo(): void {
        this.clearOutput();
        this.outx("You let her down as kindly as you can.");
        this.outx(
    ...
    public femoitFirstTimeYes(): void {
        ...
        this.clearOutput();
        this.outx(
            "Smiling, you reach across the counter and squeeze Benoite's hands until her nervous babble dies out and she smiles back.  Still holding her hand, you move behind the crates and then gently lead her behind the stall's canopy."
        );
        ...
        if (this.player.isTaur())
            ...
            (this.player.bunnyScore() >= 4 &&
                ...
                this.player.tailType == 10) ||
            (this.player.catScore() >= 4 && this.player.earType == 5 && this.player.tailType == 8)
        )
            this.outx(
            ...
            this.player.wingType != 0 &&
            this.player.armType == 1
        )
            this.outx(
            ...
            this.player.beeScore() >= 4 &&
            this.player.wingType != 0 &&
            this.player.lowerBody == 7
        )
            this.outx(
        ...
        if (this.player.biggestCockLength() <= 10)
            this.outx(
                '  Although this is evidently an uncanny experience for her, she does manage a cocky smile as her hand moves around your sex.  "<i>Mine was bigger,</i>" she teases.  You reward her cheek by doing some feeling yourself, grasping her large, supple behind, making her squeak as you move into her.'
            );
        else
            this.outx(
                "  This is evidently an uncanny experience for her, the alien nature of it deepening as her hands moves around your sex. \"<i>'Oly Gods, [name]; you are a monster,</i>\" she says thickly.  You smile and decide it's time to do some feeling yourself; you grasp her large, supple behind, making her squeak as you move into her."
            );
        ...
        if (this.player.biggestCockLength() < 15 && this.player.balls == 0)
            this.outx(
                "  Eventually you manage to hilt yourself entirely in her depths, your stomach pressing against her own tight belly."
            );
        else if (this.player.biggestCockLength() < 15 && this.player.balls > 0)
            this.outx(
                "  Eventually you manage to hilt yourself in her depths, your [balls] pressing into her sex as your stomach bumps into her own tight belly."
            );
        else if (this.player.biggestCockLength() >= 15 && this.player.balls == 0)
            this.outx(
                "  Eventually you manage to bottom out, your dick pressed against her cervix."
            );
        else if (this.player.biggestCockLength() >= 15 && this.player.balls > 0)
            this.outx(
                "  Eventually you manage to bottom out, your dick pressed against her cervix, your [balls] swinging heavily below your shaft."
            );
        this.outx(
        ...
        if (this.player.isLactating())
            this.outx(
                " The pressure and arousal makes your [nipples] dribble milk, spattering fluids across both of your chests."
            );
        this.outx(
            "  Soon the two of you are thrashing into each other, Benoite moaning huskily as you batter her against the wagon wall, both seeking your peak together, each pushing the other a bit further upwards.  Your world is lost to everything but the warm tightness around your [cock] and the pressure against your chest."
        );
    ...
    public femoitSexIntro(): void {
        ...
        if (!this.benoitPreggers()) {
            ...
            if (this.benoitInClutch())
                this.outx(
                    "  Her stomach bulges with clutch and the air is thick with female musk; the sight and smell of overwhelming ripeness speaks to your body in a way which bypasses your higher cognition entirely, sending you growling on top her, barely even hearing her giggle."
                );
            ...
            this.outx(
                "  You quickly pick up the pace as you enter rut, thrusting into your basilisk lover with red-flecked abandon, her powerful thighs working with yours to make each return plunge into her warm depths more gratifying than the last."
            );

            this.outx("\n\n");
            if (!this.player.isTaur()) {
                this.outx(
                    "Eventually, sweat dripping off you, you grab her thighs and heave them upwards so that you can really go to town, drawing yourself almost all the way out of her before smacking back into her,"
                );
                if (!this.benoitInClutch())
                    this.outx(
                        " your stomach beating out a slapping rhythm against her own flat abdomen.  "
                    );
                else
                    this.outx(
                        " your stomach beating out a slapping rhythm against her bulging, gravid abdomen.  "
                    );
            }
            this.outx(
                "Benoite moans, squeals and eventually screams to your exertions, her fluids spurting and spattering against your groin"
            );
            if (this.player.balls > 0)
                this.outx(
                    " and [balls].  You tumble over your peak as her cunt suddenly tightens around yours, sending surge after surge of cum into her fertile depths, your body seized in a rictus of pleasure."
                );
            if (this.player.cumQ() >= 2500)
                this.outx(
                    "  The quantity of it is such that it quickly dribbles back out around your cock and pools on the floor."
                );
            ...
            if (this.player.cumQ() >= 2500)
                this.outx(
                    "  Because her womb is already so jam-packed with eggs, the bulk of your deposit simply oozes messily back out of her, leaving her well and truly creampied."
                );
            ...
            if (this.player.isTaur()) {
                this.outx(
                    '"<i>No.  Non!  Absolutely non!</i>" Benoite insists.  "<i>You are my amazing stallion, [name], but... it is simply too much.  Wis all zis weight on me, zere is simply no way we can do it.</i>"'
                );
                ...
                if (this.flags[kFLAGS.FEMOIT_SPOONED] == 0) {
                    ...
                    this.outx(
                        'Benoite seems to peer at you, then places her clawed hand gently on her belly. "<i>And I sought my eyes were the ones that didn\'t work... You can surely see how big I am, yes?  Do you really sink you could carry me for the sex?  Because there is no way I can let you sit on my lap with all zese eggs in me.</i>"'
                    );
                    ...
                    this.outx(
                        'Benoite smirks at you.  "<i>Well, I guess ze last time was enjoyable enough.  Come, then; I am horny and you arrived in time to scratch my itch.</i>"'
                    );
                }
                ...
                if (this.player.balls == 0)
                    this.outx(" with a smile, knowing damn well you don't, literally");
                this.outx(".");
    ...
    public femoitBirths(): void {
        ...
        if (this.flags[kFLAGS.FEMOIT_HELPED_LAY] == 0) {
            ...
            if (this.benoitVeryHeavyPreggers() || this.benoitExtremePreggers())
                this.outx("... which is good, because you can tell there're a lot of them coming");
            this.outx(
                ".  That done, you squat down behind Benoite, keeping your head down to avoid having it slapped by her swishing tail, and reach up between her legs. She promptly lets out a shriek."
            );

            if (this.flags[kFLAGS.FEMOIT_HELPED_LAY] == 1) {
                this.outx('"<i>What\'s the matter?</i>" you blurt, alarmed.');

                this.outx(
                    "\"<i>You... you 'ave cold 'ands!</i>\" she squeals, shuddering in displeasure."
                );
            } else {
                this.outx(
                    '"<i>What have I told you about warming zose hands?</i>" she snaps angrily.'
                );
            }
        }
        ...
        if (this.silly())
            this.outx(".  It's shaped like a complex rhomboidal polygon with 15 sides.");
        else this.outx(", the smoothly curved peak of a jade egg beginning to crest.");
        this.outx(
            "  With a strangled cry of orgasm tinged with pained relief, Benoite pushes it from her passage into your hands. Slick with juices, the egg makes for quite a handful and you find yourself struggling to safely cradle it within your grasp. You quickly place it into the makeshift next at her feet."
        );
        ...
        if (this.flags[kFLAGS.FEMOIT_EGGS] > 1) this.outx("s");
        this.outx(
            " she just laid.  You nod from behind her, and cautiously move around to give her a hug.  She is a very, very brave woman, and an even braver man for making the decision to do this in the first place."
        );
        ...
        if (this.benoitRegularPreggers())
            this.outx(
                ' She sighs softly.  "<i>A small clutch, this is... normal for my people, yes, but not very good for my mission, is it?  Still, a small step to freedom is still a step.</i>"  She manages to smile.  "<i>My children will be free, and that is something to celebrate.</i>"'
            );
        else if (this.benoitHeavyPreggers())
            this.outx(
                ' "<i>My, a nice big clutch we had together, didn\'t we?</i>" she smiles, proudly.  "<i>Yes, these will be strong children, I zink.</i>"'
            );
        else if (this.benoitVeryHeavyPreggers())
            this.outx(
                ' "<i>...Wow, you\'re quite ze stud, aren\'t you?</i>" Benoite says, giving a throaty growl of lust.  "<i>I chose well when I decided to let you fertilize my eggs... so many eggs, too.</i>"'
            );
        else if (this.benoitExtremePreggers())
            this.outx(
                ' The basilisk stops after her counting, visibly stunned.  She recounts her eggs again, and then again, shaking her head in amazement.  "<i>Incredible... simply, incredible.  I didn\'t think it was possible for one woman to lay zo many eggs!  Zut alors!</i>"  A sudden look of horrified realization washes over her features.  "<i>I\'m going to be run off my feet with all zese little monsters, aren\'t I?</i>" she murmurs to herself.'
            );
        ...
        if (this.flags[kFLAGS.FEMOIT_EGGS] > 1) this.outx("s");
        this.outx(
            " away safely, though, and the blind reptilian clearly appreciates the help.  Leaving her to admire her new clutch you head back to camp."
        );
