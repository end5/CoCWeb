import { ANAL_LOOSENESS_NORMAL, ANAL_WETNESS_DRY, BUTT_RATING_AVERAGE, HIP_RATING_AVERAGE, LOWER_BODY_TYPE_CENTAUR, TAIL_TYPE_HORSE } from "../../../../includes/appearanceDefs";
...
export class Kelt extends Monster {
    ...
    private keltTramplesJoo(): void {
        ...
        var damage: number = Math.round((this.str + this.weaponAttack) - Kelt.rand(this.player.tou) - this.player.armorDef);
    ...
    private KellyuraAttack(): void {
        ...
        //(3)
            ...
            if (this.player.lust >= 80) this.outputText("Your hand moves towards your groin seemingly of its own volition.");
    ...
    private dayDreamKelly(): void {
        if (Kelt.rand(2) == 0) this.outputText("Kelt pauses mid-draw, looking you up and down.  He licks his lips for a few moments before shaking his head to rouse himself from his lusty stupor.  He must miss the taste of your sperm.");
    ...
    protected performCombatAction(): void {
        if (this.statusAffectv1(StatusAffects.BowCooldown) > 0) {
            ...
            if (this.statusAffectv1(StatusAffects.BowCooldown) <= 0) this.removeStatusAffect(StatusAffects.BowCooldown);
        }
        else {
