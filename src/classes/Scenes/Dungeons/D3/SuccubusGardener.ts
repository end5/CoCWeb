...
export class SuccubusGardener extends Monster {
    ...
    private cleanupEffects(): void {
        if (this.player.findStatusAffect(StatusAffects.Tentagrappled))
            this.player.removeStatusAffect(StatusAffects.Tentagrappled);
        if (this.player.findStatusAffect(StatusAffects.ShowerDotEffect))
    ...
    protected performCombatAction(): void {
        ...
        if (this.HPRatio() <= 0.6) {
            ...
            this.player.findStatusAffect(StatusAffects.GardenerSapSpeed) < 0 &&
            this.findStatusAffect(StatusAffects.VineHealUsed) >= 0
        ) {
            this.sapSpeed();
        } else {
    ...
    private vineHeal(): void {
        ...
        if (this.findStatusAffect(StatusAffects.Stunned) < 0) {
            this.outx(
                "Tipping herself backward, the succubus gardener lets herself pitch into the writhing tendrils behind her, her mouth opened welcomingly. The tentacles gently catch her, and rather than ravishing her vulnerable form, they merely gather above her parted lips, dribbling thick flows of pink slime. Her throat bobs as she swallows, her injuries vanishing in seconds. The vines push her back up onto her feet. She's smiling a little dopily."
            );

            if (this.lustVuln <= 0.15) {
                this.outx(
                    " <b>You aren't sure, but she seems to be leering at you a little more openly.</b>"
                );
            } else if (this.lustVuln <= 0.45) {
                this.outx(" <b>She's definitely leering at you now.</b>");
            } else {
                this.outx(
                    " <b>There's no disguising the lecherous look on her face while she eye-humps you.</b>"
                );
            }

            if (this.lustVuln <= 0.3)
                this.outx(
                    " Whatever is in that healing nectar must be weakening her self-control."
                );
        } else {
            this.outx(
                "Acting on some unseen signal, the forest of tentacles lurches into action, surrounding its insensate mistress in a veil of wiggling green. A bundle of slime-dripping vines can be spotted through gaps in the verdant veil, hanging over their demonic caretaker with remarkable care, leaking gooey strands of pink into her mouth. Her throat works to swallow it, and when she emerges a second later, her injuries have vanished and her eyes"
            );

            if (this.lustVuln <= 0.15) {
                this.outx(" are slightly glassy.");
            } else if (this.lustVuln <= 0.45) {
                this.outx(" are even glassier.");
            } else {
                this.outx(" are dilated and a little slow.");
            }
    ...
    public grappleStruggle(): void {
        ...
        if (SuccubusGardener.rand(this.player.str) > this.str / (1 + numRounds / 2)) {
            this.outx(
                "You scrabble desperately against the tentacles enveloping your body, pulling against the cast-iron grip around your limbs. You tug against them again and again, and with one final mighty heave, you slip free of their grasp!"
            );
    ...
    private sicem(): void {
        ...
        for (let i = 0; i < 10; i++) {
            if (
                ...
                damage +=
                    2 +
                    SuccubusGardener.rand(1 + this.player.lib / 20) +
                    SuccubusGardener.rand(1 + this.player.sens / 20);
            }
        }
        ...
        if (damage >= 0) {
            ...
            this.outx(
                " The sinuous plant-based tentacles strike at you like a dozen tiny whips, but, somehow, you manage to avoid every single one of their lashes!"
            );
        }
    }
    ...
    private showerDotEffect(): void {
        ...
        if (this.player.lust < 50)
            ...
            this.outx(
                "You try to wipe some of the fragrant seed from your palm, but all you succeed in doing is smearing it into your [hips]."
            );
            if (this.player.cor < 50) this.outx(" You're ashamed to admit");
            else this.outx(" You're a little irritated to admit");
            this.outx(" that it's starting to feel really good.\n\n");
        } else if (this.player.lust < 70) {
            this.outx(
                "You groan at the warm slipperiness enveloping your [skinFurScales] as the tainted tentacles' fluids go to work on you. There's nothing you can do but try to endure it. If only it didn't feel so... hot to be drenched in. If you wind up losing, you hope she'll do this again....\n\n"
            );
            ...
            this.outx(
                "You shudder in place, stumbling dazedly as your ardor rises to a fever pitch. Soon, you're going to wind up too turned-on to resist, and when that happens, those tentacles are going to take you. The worst part? It's starting to sound really, really... really good to you. Not struggling, no tension... just giving in to what your body craves and loving it.\n\n"
            );
        } else if (this.player.lust < 100) {
            this.outx(
                "Ohhhh, you're close now. You can feel the need hammering inside of you, soaking in through your [skinFurScales] to stoke the fires between your [legs] into a blazing inferno, one you couldn't resist even if you wanted to. Then... then you'll be free to cum. You shake your head. Gotta hold it together"
            );
            ...
            this.outx(
                "Ohhh fuck, there's no holding it back now. You're gonna do it, and there's nothing you could do it stop it even if you wanted to. You're going to drop to your knees and take off your [armor]. You're going to give this beautiful demoness what she wants. You're going to let her fuck you and use you, just so long as she allows you to cum. You'll be fine once you cum, even if it means throwing away a chance to defeat Lethice.\n\n"
            );
        ...
        if (this.player.statusAffectv1(StatusAffects.ShowerDotEffect) < 0) {
            this.player.removeStatusAffect(StatusAffects.ShowerDotEffect);
            this.outx(
                "<b>The lust-inducing effects of the plant-spunk feel like they've dissipated...</b>\n\n"
            );
        }
    }
    ...
    private taunt(): void {
        this.outx(
            "<i>“How do you expect to defeat me, [name],”</i> the green-thumbed temptress asks with a quirk of her head. <i>“You are but one, and we are many. You have the frailties and weaknesses of a soul. I have power and experience beyond your comprehension. What is there for you to do but willingly submit?”</i> She purses her puffy lips, thinking. <i>“If you submit willingly, I'll allow you to lay your head between my breasts while my plants feed on you. It'll be quite the experience.”</i>"
        );
    }

    private motorboat(): void {
        this.outx(
            "<i>“Oh fuck it,”</i> the demoness growls, stalking forward. <i>“We both need this, don't we, pet?”</i> She slips inside your guard, pressing her pendulous melons against your face"
        );

        if (this.player.tallness <= this.tallness - 6)
            this.outx(", somehow short enough in spite of the height differences.");
        else if (this.player.tallness >= this.tallness + 6)
            this.outx(", somehow tall enough in spite of the height differences.");

        this.outx(
            " They're so soft and pillowy that you can't help but enjoy the feel of them on your skin, and you take a deep, contented breath before remembering where you are and struggling out of the creamy valley."
        );
    ...
    private tasteTheEcstasy(): void {
        ...
        if (SuccubusGardener.rand(this.player.str - 30) + 30 > this.str) {
            ...
            this.game.dynStats(
                "lus",
                8 + Math.floor(this.player.lib / 20 + this.player.cor / 25),
                "cor+",
                5
            );
        } else {
