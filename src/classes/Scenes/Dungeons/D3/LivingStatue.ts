...
export class LivingStatue extends Monster {
    ...
    private backhand(): void {
        ...
        var damage: number = Math.floor((this.str + this.weaponAttack) - LivingStatue.rand(this.player.tou) - this.player.armorDef);
        //Dodge
        if (damage <= 0 || (this.combatMiss() || this.combatEvade() || this.combatFlexibility() || this.combatMisdirect())) this.outputText(" You slide underneath the surprise swing!");
        else {
            //Get hit
    ...
    private overhandSmash(): void {
        ...
        var damage: number = 175 + Math.floor((this.str + this.weaponAttack) - LivingStatue.rand(this.player.tou) - this.player.armorDef);
        if (damage <= 0 || LivingStatue.rand(100) < 25 || this.combatMiss() || this.combatEvade() || this.combatFlexibility() || this.combatMisdirect()) this.outputText(" You're able to sidestep it just in time.");
        else {
            //Hit
    ...
    private disarm(): void {
        ...
        //Avoid
        if ((this.combatMiss() && this.combatMiss()) || this.combatEvade() || this.combatFlexibility() || this.combatMisdirect()) this.outputText(" You manage to hold onto your equipment, for now.");
        //Oh noes!
        else {
    ...
    private cycloneStrike(): void {
        ...
        var damage: number = (175 + Math.floor((this.str + this.weaponAttack) - LivingStatue.rand(this.player.tou) - this.player.armorDef)) / (LivingStatue.rand(3) + 2);
        //Avoid
        if (damage <= 0 || this.combatMiss() || this.combatEvade() || this.combatFlexibility() || this.combatMisdirect()) this.outputText(" By the grace of the gods, you somehow avoid the spinning hammer.");
        else {
            //Hit
    ...
    protected performCombatAction(): void {
        ...
        else if (this.HPRatio() < 0.4 && this.findStatusAffect(StatusAffects.Disarmed) < 0 && this.player.weaponName != "fists") {
            this.disarm();
        }
        else {
            var opts: any[] = [];

            if (this.player.findStatusAffect(StatusAffects.Blind) < 0 && this.player.findStatusAffect(StatusAffects.Stunned) < 0) opts.push(this.dirtKick);
            if (this.player.findStatusAffect(StatusAffects.Blind) < 0 && this.player.findStatusAffect(StatusAffects.Stunned) < 0) opts.push(this.concussiveBlow);
