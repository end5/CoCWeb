...
export class LethiceScenes extends BaseContent {
    public static GAME_END_REDEMPTION = 1;
    public static GAME_END_CONSORT = 2;
    public static GAME_END_PURGE_0_50 = 3;
    public static GAME_END_PURGE_50_100 = 4;
    public static GAME_END_PURGE_100 = 5;
    public static GAME_END_CONQUER_LOW = 6;
    public static GAME_END_CONQUER_MED = 7;
    public static GAME_END_CONQUER_HIGH = 8;
    ...
    private exportSaveData(): void {
        ...
        this.menu();
        this.addButton(0, "Game Over", function (): void { });
    }
    ...
    private goFight(): void {
        ...
        if (!this.player.canFly()) this.outputText(" Good thing she’s not the only one that can fly!");
        this.outputText("\n\n<b>It’s a fight!</b>");
        ...
        if (this.flags[kFLAGS.MINOTAURKINGS_TOUCH] == 1) {
    ...
    public defeated(hpVictory: boolean): void {
        ...
        this.menu();
        if (this.player.hasCock()) this.addButton(0, "Plow Her", this.plowHer);
        if (this.player.hasVagina()) this.addButton(1, "Queen Her", this.queenHer);
        this.addButton(2, "Boob Play", this.boobPlay, hpVictory);

        var hasLethicite: boolean = (this.player.hasKeyItem("Sheila's Lethicite") > 0) || (this.player.hasKeyItem("Stone Statue Lethicite") > 0);
        if (hasLethicite && this.player.findStatusAffect(StatusAffects.MetMarae) >= 0 && this.player.findStatusAffect(StatusAffects.FactoryOverload) < 0) {
    ...
    private plowHer(): void {
        ...
        this.outputText("\n\nYou");
        if (!this.player.isNaga()) this.outputText(" step");
        ...
        if (this.player.cocks.length == 1) this.outputText(" [cock]");
    ...
    private plowHerPussy(): void {
        ...
        if (this.player.cor >= 66) this.outputText(" gleeful");
        else this.outputText(" shocked");
    ...
    private plowHerDouble(): void {
        ...
        if (this.player.cocks.length == 2) this.outputText(" other");
        ...
        if (this.player.isBiped()) this.outputText(" and prop your feet up on the Demon Queen’s prostrate back, smearing her face down in your seed");
        this.outputText(".");
    ...
    private queenHer(): void {
        ...
        this.outputText("\n\n");
        if (this.player.cor < 66) this.outputText("Scowling at the haughty bitch’s implications");
        else this.outputText("Laughing right in the bitch’s face");
        ...
        if (this.player.biggestTitSize() >= 2) this.outputText(" cupping a tit");
        else this.outputText(" pinching one of your nipples");
        ...
        if (this.player.hasCock()) this.outputText(" and dragging your [cock] across her face, smearing pre all through her hair");
        ...
        if (this.player.isBiped()) this.outputText(" to your feet");
        else this.outputText(" off");
    ...
    private boobPlay(hpVictory: boolean): void {
        ...
        if (!hpVictory) this.outputText(" by your superior sexual acumen");
        else this.outputText(" by the might of your [weapon]");
        ...
        if (this.player.cor >= 66) this.outputText(" throw her roughly onto her back");
        else this.outputText(" push her onto her back");
        ...
        if (this.player.hasCock()) this.outputText(" They’re easily big enough to fuck now!");
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.cor >= 66) this.outputText(" any other corrupted soul of her court");
            else this.outputText(" like you’re some kind of demon yourself");
            ...
            if (this.player.hasVagina()) {
                ...
                if (!this.player.isNaga()) this.outputText(" a leg");
                else this.outputText(" your coils");
            ...
            if (this.player.cor >= 66) this.outputText(" You’re finally where you belong!");
            else this.outputText(" Even the demons recognize what you’ve done! Now the only question is what to do with them afterwards...");
            ...
            if (this.player.hasVagina()) this.outputText(" holes");
    ...
    private redemption(hpVictory: boolean): void {
        ...
        if (hpVictory) this.outputText(" battered and beaten demon, once so full of haughty pride and self-assured in her power, fills you with a sense of pity.");
        else this.outputText(" horny, desperate demon before you fills you with a sense of pity.");
    ...
    private redemptionII(): void {
        ...
        if (this.player.isBiped()) this.outputText(" kneeling");
        else this.outputText(" leaning");
        ...
        this.outputText("\n\nWarily, you");
        if (this.player.weapon == WeaponLib.FISTS) this.outputText(" flip a knife out of your pack");
        else this.outputText(" take your [weapon]");
    ...
    private redemptionIIIGoName(input: HTMLInputElement): void {
        ...
        this.flags[kFLAGS.LETHICE_NAME] = input.value;

        this.clearOutput();

        if (this.flags[kFLAGS.LETHICE_NAME] == "Lethice") {
    ...
    private redemptionIV(): void {
        ...
        this.outputText("You spend some time there on the beach, comforting " + this.flags[kFLAGS.LETHICE_NAME] + ". Eventually, as the hazy sky begins to darken, you take her hand and board your small boat, pushing off towards land. A new day will soon dawn on Mareth, and with it, you introduce a new hope: a way to fight back against the demons.");

        this.outputText("\n\nIt soon comes to light just why " + this.flags[kFLAGS.LETHICE_NAME] + "’s rebirth was so costly, so tiring, to your patron goddess. Not a week after your victory at the mountain fortress, you and your new companion are ambushed by demons - assassins, bent on exacting vengeance for their queen’s humiliation. Instead, they end up on their knees before you, beaten as all who stood before you always are. This time it’s different, though. This time, rather than the choice of carnal punishment or death, you find yourself given a third option.");

        this.outputText("\n\nThe demons carried a few shards of Lethicite, which " + this.flags[kFLAGS.LETHICE_NAME] + " quickly finds, drawn to the power of it like a moth to flame. Before you have time to realize what she intends, " + this.flags[kFLAGS.LETHICE_NAME] + " grabs the brilliant pink stones and raises one of them aloft. Her arms and hands begin to glow, radiating that same awesome viridian power that Marae had shown you before, and in a flash of light one of the demons crumples to the ground, orgasming and writhing as the seething corruption in his veins is torn from his body in one last demonic ejaculation. The pool of cum he leaves catches flame, burning away to smoke and cinders a moment later. Corruption eradicated, utterly and completely.");
        ...
        this.outputText("\n\nWhen she’s finished, " + this.flags[kFLAGS.LETHICE_NAME] + " collapses to her knees, gasping, chest heaving against her comfortable clothes - she has long since abandoned the twisted habit of her demonic reign. You put a hand on her shoulder, expecting her to be almost ready to collapse as Marae was... but instead, she turns to you with a huge smile. " + this.flags[kFLAGS.LETHICE_NAME] + " throws her arms around you, laughing like a giddy child.");
        ...
        this.flags[kFLAGS.GAME_END] = LethiceScenes.GAME_END_REDEMPTION;
        this.saveExport();
    }
    ...
    private joinHer(): void {
        ...
        if (this.player.weapon == WeaponLib.FISTS) this.outputText(" raise your empty hands");
        else this.outputText(" set aside your [weapon]");
        ...
        if (this.flags[kFLAGS.DRIDERINCUBUS_KILLED] == 0 || this.flags[kFLAGS.MINOTAURKING_KILLED] == 0) this.outputText(" humiliated");
        else this.outputText(" slain");
        ...
        if (this.flags[kFLAGS.DRIDERINCUBUS_KILLED] == 1 || this.flags[kFLAGS.MINOTAURKING_KILLED] == 1) this.outputText(" Carved a bloody swathe");
        else this.outputText(" Fucked your way");
    ...
    private joinHerII(): void {
        ...
        else this.outputText(" newwgrown cock. Just another side benefit of demonhood: molding your body on the fly to suit whatever takes your fancy moment to moment.");
        ...
        this.flags[kFLAGS.GAME_END] = LethiceScenes.GAME_END_CONSORT;
        this.saveExport();
    }
    ...
    public won(hpVictory: boolean, pcCameWorms: boolean): void {
        ...
        if (!hpVictory) this.outputText(" by your own rampant sexual desires");
        else this.outputText(" beneath the demoness’s merciless assault. She’s more powerful than you could have ever imagined, far more so than a wisp of a girl like her has any right to be");
        this.outputText(". Gasping, you slump");
        if (!this.player.isBiped()) this.outputText(" down");
        else this.outputText(" to your knees");
        this.outputText(",");
        if (this.player.weapon != WeaponLib.FISTS) this.outputText(" your [weapon] clattering away across the stone");
        else this.outputText(" too weak to even ball your fists");
        ...
        if (this.monster.lust < 50 && this.monster.HP > (this.monster.eMaxHP() * 0.5)) this.outputText(" Almost.");
        ...
        if (hpVictory) this.outputText(" to hide just how turned on she’s left you... how close you are to orgasm, just in need of her lurid touch");
        else this.outputText(" not to let her see how weak you are, how easy it really would be for her to kill you... or worse");
    ...
    private woofwoof(): void {
        ...
        if (this.player.dogCocks() == 0) this.outputText(" newly-grown doggy cock");
        else this.outputText(" thick, throbbing dog-member");
        ...
        if (this.player.findStatusAffect(StatusAffects.DungeonShutDown) >= 0) this.outputText("Your predecessor destroyed our welcoming facility. Otherwise I’m sure a certain imp would have revealed the truth to you long ago, before joining you with our breeding machines.");
        else {
            ...
            if (this.flags[kFLAGS.ZETAZ_DEFEATED_AND_KILLED] == 0) this.outputText(" Zetaz");
            else this.outputText(" the new Warden");
    ...
    private killHer(): void {
        ...
        this.flags[kFLAGS.LETHICE_KILLED] = 1;
    ...
    private leaveHer(): void {
        ...
        this.flags[kFLAGS.LETHICE_LEFT_ALIVE] = 1;
    ...
    private goHomeVeng(): void {
        ...
        if (this.player.cor <= 33) this.outputText(" Despite the fear and terror of " + this.player.mf("his", "her") + " arrival, " + this.player.mf("he", "she") + " proved a fair and wise monarch. The village soon became a town, and that town a city. Prosperity came to those who lived in the valley, guided by the strength of its unshakable ruler.");
        else if (this.player.cor <= 66) this.outputText(" Despite the fear and terror of " + this.player.mf("his", "her") + " arrival, " + this.player.mf("he", "she") + " proved a competent, if harsh monarch. The village’s prosperity was marred only by its limited liberties. Still, it was soon a town, and later a city. Most folk were willing to sacrifice their freedoms for the safety " + this.player.mf("he", "she") + " provided.");
        else this.outputText(" Despite the fear and terror of " + this.player.mf("his", "her") + " arrival, " + this.player.mf("he", "she") + " proved a capable monarch in spite of " + this.player.mf("his", "her") + " rapacious tastes. The villagers dared not protest when their once-Champion treated their sons and daughters as little more than slaves. They feared to question where the fruits " + this.player.mf("he", "she") + " provided came from, and why they felt so strange after eating them. Of course, these concerns did not stop them from breeding. Soon, nothing could. As the population of the valley swelled, the Champion amassed an army, one large enough to rival the nearby countries that had ignored it for so long.");
    ...
    private goHomeJustice(): void {
        ...
        // 9999 check dis
        if (this.player.cor >= 50) this.outputText(" put an end to their rank existence right there.");
        else this.outputText(" sent them through a glowing portal of " + this.player.mf("his", "her") + " own creation, never to be seen again.");
        ...
        if (this.player.cor < 50) this.outputText(" In " + this.player.mf("his", "her") + " wisdom, " + this.player.mf("he", "she") + " stopped the townsfolk from lynching their traitorous leaders right there. Instead, they were banished, forever expelled from the lands of Ingnam on pain of death.");
        else this.outputText(" The townsfolk lynched their traitorous leaders right there and then under the watchful eyes of the Champion. " + this.player.mf("He", "She") + " nodded approvingly when they gave their last twitches, seemingly happy to let small-town justice run its course.");
        ...
        if (this.getGame().camp.companionsCount() > 0) this.outputText(" to say nothing of " + this.player.mf("his", "her") + " bizarre friends");
    ...
    private HowFuckedIsThePlayerOMeter(): number {
        ...
        //Marae corrupt +50
        if (this.player.findStatusAffect(StatusAffects.FactoryOverload) >= 0) levelOfFuckedness += 50;
        ...
        if (this.flags[kFLAGS.KID_A_XP] >= 100 && this.flags[kFLAGS.ANEMONE_WEAPON_ID] != '') levelOfFuckedness -= 10;
    ...
    private hasChildren(): boolean {
        if (this.player.statusAffectv1(StatusAffects.Birthed) > 0) return true;
        if (this.flags[kFLAGS.AMILY_BIRTH_TOTAL] > 0 || this.flags[kFLAGS.PC_TIMES_BIRTHED_AMILYKIDS] > 0) return true;
        if (this.flags[kFLAGS.BENOIT_EGGS] > 0) return true;
        if (this.flags[kFLAGS.COTTON_KID_COUNT] > 0) return true;
        if (this.flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] > 0) return true;
        if (this.flags[kFLAGS.EMBER_CHILDREN_MALES] > 0) return true;
        if (this.flags[kFLAGS.EMBER_CHILDREN_FEMALES] > 0) return true;
        if (this.flags[kFLAGS.EMBER_CHILDREN_HERMS] > 0) return true;
        if (this.flags[kFLAGS.EMBER_EGGS] > 0) return true;
        if (this.flags[kFLAGS.IZMA_CHILDREN_SHARKGIRLS] > 0) return true;
        if (this.flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS] > 0) return true;
        if (this.flags[kFLAGS.KELLY_KIDS_MALE] > 0 || this.flags[kFLAGS.KELLY_KIDS] > 0) return true;
        if (this.flags[kFLAGS.MARBLE_KIDS] > 0) return true;
        if (this.flags[kFLAGS.ANT_KIDS] > 0) return true;
        if (this.flags[kFLAGS.PHYLLA_DRIDER_BABIES_COUNT] > 0) return true;
        if (this.flags[kFLAGS.SHEILA_JOEYS] > 0) return true;
        if (this.flags[kFLAGS.SHEILA_IMPS] > 0) return true;
        if (this.flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] > 0 || this.flags[kFLAGS.SOPHIE_DAUGHTER_MATURITY_COUNTER] > 0) return true;
        if (this.flags[kFLAGS.SOPHIE_EGGS_LAID] > 0) return true;
        if (this.flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] > 0) return true;
        if (this.getGame().urtaPregs.urtaKids() > 0) return true;
        return false;
    }
    ...
    private purgeShit(): void {
        ...
        if (this.hasChildren()) this.outputText(", offspring");
        ...
        if (levelOfFuckedness <= 50) {
            ...
            if (this.player.findStatusAffect(StatusAffects.FactoryOverload) < 0) this.outputText(" Marae reclaimed the full network of her roots and single-handedly purged the last holdouts of corruption from the lands, choking it out of her world like weeds from a garden.");
            else this.outputText(" Marae fought for a time, but even corrupted goddesses cannot hold back your tide of purity. Her fall was the biggest blight on your campaign, a sad footnote on an otherwise successful reclamation.");
            this.outputText("\n\n<b>You ended the demon threat forever and saved Mareth.</b>");
            this.flags[kFLAGS.GAME_END] = LethiceScenes.GAME_END_PURGE_0_50;
        }
            ...
            this.outputText("\n\n");
            if (this.player.findStatusAffect(StatusAffects.FactoryOverload) < 0) this.outputText("Marae is a wonderful aid in your quest, always able to manifest in her newly purified roots and offer words of wisdom or encouragement. Her magical might is a rock you can lean upon when times are tough. You aren’t sure how long you could keep this up without her.");
            else this.outputText("Marae is always there, always lending her power to the demons, even when you dig her roots out of the ground with shovels and axes, burning the greasy lumber into nothingness. This task would be so much easier if the goddess was on your side, but still, you persevere.");
            ...
            if (this.player.findStatusAffect(StatusAffects.FactoryOverload) < 0) this.outputText(" Those few pockets that escaped your armies could not escape Marae’s all-encompassing roots. Mareth is purified once more.");
            else this.outputText(" A few places are tainted still, but it’s impossible to cleanse every cave. Instead, you make instructions to maintain the army. Should evidence of corruption arise anywhere, your forces will swiftly put it to flame.");
            ...
            this.flags[kFLAGS.GAME_END] = LethiceScenes.GAME_END_PURGE_50_100;
        }
        else {
            ...
            this.flags[kFLAGS.GAME_END] = LethiceScenes.GAME_END_PURGE_100;
        }
    ...
    private conquer(): void {
        ...
        if (this.player.cor <= 33) {
            ...
            if (this.getGame().camp.companionsCount() > 0) {
                ...
                if (this.getGame().camp.companionsCount() > 1) this.outputText("s");
            ...
            if (this.flags[kFLAGS.LETHICE_KILLED] == 0) this.outputText(" Lethice herself is even brought back, though this time securely leashed and empty-eyed.");
            ...
            this.flags[kFLAGS.GAME_END] = LethiceScenes.GAME_END_CONQUER_LOW;
        }
            ...
            if (this.getGame().camp.companionsCount() > 0) this.outputText("your followers from");
            ...
            this.flags[kFLAGS.GAME_END] = LethiceScenes.GAME_END_CONQUER_MED;
        }
        else {
            ...
            this.flags[kFLAGS.GAME_END] = LethiceScenes.GAME_END_CONQUER_HIGH;
        }
        this.saveExport();
    }
}
