...
export class Lethice extends Monster {
    ...
    public get long(): string {
        ...
        if (this.player.findStatusAffect(StatusAffects.LethicesRapeTentacles) >= 0) {
            ...
            if (this.player.statusAffectv3(StatusAffects.LethicesRapeTentacles) != 0) this.outputText(" Unfortunately, they’ve grabbed you. You need to break free to do anything!");
    ...
    public get fightPhase(): number { return this._fightPhase; }
    ...
    private rapetacles(): void {
        ...
        if (this.player.canFly()) {
            ...
            this.player.createStatusAffect(StatusAffects.LethicesRapeTentacles, 4 + Lethice.rand(2), 0, 0, 0);
        }
            ...
            this.player.createStatusAffect(StatusAffects.LethicesRapeTentacles, 4 + Lethice.rand(2), 0, 1, 0);
        }
    }
    ...
    private phase1Ends(hpVictory: boolean): void {
        ...
        if (this.findStatusAffect(StatusAffects.PhysicalDisabled) >= 0) this.removeStatusAffect(StatusAffects.PhysicalDisabled);
        if (this.findStatusAffect(StatusAffects.AttackDisabled) >= 0) this.removeStatusAffect(StatusAffects.AttackDisabled);

        this.combatRoundOver();
    }

    private phase2(): void {
        var atks: any[] = [this.demonLustMagic, this.dirtyDancing, this.hornyPoke, this.crushingBodies];
        if (Lethice.rand(10) == 0 && this.player.findStatusAffect(StatusAffects.Blind) < 0) atks.push(this.bukkakeTime);
    ...
    private phase2Ends(hpVictory: boolean): void {
        ...
        this.game.menu();
        if (this.player.hasCock() || this.player.hasVagina()) this.game.addButton(0, "DemonFuck", this.p2DemonFuck, hpVictory);
        if (this.player.findStatusAffect(StatusAffects.KnowsHeal) >= 0) this.game.addButton(1, "Heal", this.p2Heal);
    ...
    private p2DemonFuck(hpVictory: boolean): void {
        ...
        if (hpVictory) this.outputText(" lying stunned and helpless");
        else this.outputText(" who’ve fallen into a frenzied orgy");
        ...
        if (this.player.hasCock()) this.outputText(" wrap around your [cock]");
        else this.outputText(" lap across the lips of your [cunt]");
        ...
        if (!this.player.hasCock()) this.outputText(" munching rug.");
        else this.outputText(" sucking cock.");
    ...
    private fuckDemon(): void {
        ...
        if (this.player.hasFuckableNipples()) this.outputText(" until you give her a wicked grin and guide the tip inside your [nipple]");
        else this.outputText(" and smearing your [skinFurScales] with pre");
        ...
        if (this.player.balls > 0) this.outputText(" and around your balls");
        else this.outputText(" down to the base of your cock");
        ...
        if (!this.player.hasFuckableNipples()) this.outputText(" that splatters all over your chest");
        else this.outputText(" right into your tit");
    ...
    private rideCock(): void {
        ...
        if (this.player.hasVagina()) this.outputText(" licking at your [cunt]");
        else this.outputText(" reaching around to give your unused [cocks] a tug}. Another demoness snuggles up behind you, pressing a hefty pair of jugs into your back and murmuring about how she wants to see you get bred by that hermaphroditic stud under you.");
    ...
    private p2Heal(): void {
        ...
        var temp: number = Math.floor((this.player.inte / (2 + Lethice.rand(3)) * this.game.spellMod()) * (this.player.maxHP() / 150));
    ...
    private p2Next(): void {
        ...
        if (this._wingsDestroyed) this.outputText(" tattered");
    ...
    private phase3(): void {
        ...
        if (this.flags[kFLAGS.LAST_ATTACK_TYPE] == 1 || this.flags[kFLAGS.LAST_ATTACK_TYPE] == 2 || this.flags[kFLAGS.LAST_ATTACK_TYPE] == 4) this._defMode = 1;
        ...
        if (this._roundCount == 5) this.gropehands()
            ...
            if (this.player.findStatusAffect(StatusAffects.WhipSilence) < 0) atks.push(this.whipchoke);
