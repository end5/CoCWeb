...
export class HermCentaur extends Monster {
    ...
    private hypnoCock(): void {
        ...
        if (
            this.player.inte * (2 / this._hypnoCockUses) >
            HermCentaur.rand(this.player.lib / 3 + this.player.sens / 3 + this.player.cor / 3)
        ) {
            this.game.dynStats(
                "lus+",
                2 + HermCentaur.rand(this.player.lib / 20 + this.player.sens / 20)
            );
