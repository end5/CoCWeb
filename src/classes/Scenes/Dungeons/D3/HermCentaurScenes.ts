...
export class HermCentaurScenes extends BaseContent {
    ...
    public beatThePony(hpVictory: boolean): void {
        ...
        if (hpVictory) this.outx(" as she tries to rise");
        else this.outx(" as she tries to stretch for her pulsating, cum-dripping member");
        this.outx(
            ", but it's a futile gesture.  She doesn't have the power to face you on her own terms anymore, and she watches you with a"
        );
        if (hpVictory) this.outx(" resigned glare");
        else this.outx(" lustful glare");
        this.outx(" once she realizes she's utterly spent.");
    ...
    private maleFuckHer(): void {
        ...
        if (this.player.cumQ() >= 15000)
            this.outx(", practically dragging her gut on the stones as she runs");
        this.outx(
    ...
    private femFuckHerII(cockType: number): void {
        ...
        if (cockType == this.TENTACOCK) {
            ...
            if (this.player.wetness() < 4) this.outx(" practically");
    ...
    private maleLoss(hpVictory: boolean): void {
        ...
        if (this.player.balls > 0) this.outx(" [balls] feel fit to burst!");
        else this.outx(" manhood feels swollen to bursting!");
        this.outx("  Realizing the only way out will be with your tongue, you open wide");
        if (this.player.tongueType != 0) this.outx(" and uncoil your inhuman length to spear");
        else this.outx(" and begin to lap");
        this.outx(
        ...
        if (this.flags[kFLAGS.D1_OMNIBUS_KILLED] != 1) this.outx(", and a familiar one at that");
        this.outx(
        ...
        if (this.player.hornType != 0) this.outx(" horns");
        else this.outx(" head");
        this.outx(
