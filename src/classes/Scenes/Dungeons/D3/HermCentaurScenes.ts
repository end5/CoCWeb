...
export class HermCentaurScenes extends BaseContent {
    public encounterThePony(): void {
        ...
        if (this.player.vaginalCapacity() < 100 || this.player.analCapacity() < 100)
            this.outputText(
    ...
    public beatThePony(hpVictory: boolean): void {
        this.clearOutput();
        this.outputText(
            "With a heavy 'thud', the demonic red-head slumps onto her side in the grass.  Her tail flails fitfully"
        );
        ...
        this.outputText(
            ", but it's a futile gesture.  She doesn't have the power to face you on her own terms anymore, and she watches you with a"
        );
    ...
    private killHer(): void {
        this.clearOutput();
        this.outputText(
            "Sighing, you advance on the downed demon, trying to ignore the horrified look in her eyes as you end her."
        );
    ...
    private letHerGo(): void {
        this.clearOutput();
        this.outputText(
            "You dust off your [armor] and wave nonchalantly at the equine demoness.  She slowly staggers up, watching you warily.  Dismissively, you tell her to leave before she earns your ire - her queen will be dealt with soon enough.  She clops off in a huff towards the exit."
        );
    ...
    private maleFuckHer(): void {
        ...
        if (this.player.balls > 0)
            this.outputText(
                " Slapping wetly against hers, your [balls] ring in an increasing torrent of fleshy slaps."
            );
        this.outputText(
        ...
        if (this.player.cumQ() >= 1000)
            this.outputText(
                " You quake at the sensation of dumping so much of your creamy batter into her stomach, nearly closing your eyes in bliss.  Luckily, you hold onto your wits."
            );
        if (this.player.cumQ() >= 5000)
            this.outputText(
                " It doesn't make much difference to the demon beneath you, as her belly slowly rounds from the obscene deposit you're plugging her with."
            );
        if (this.player.cumQ() >= 15000)
            this.outputText(
                " Bigger and bigger, she bloats until she looks fat or pregnant, jiggling with your thick spooge."
            );
        ...
        if (this.player.cumQ() >= 15000)
            this.outputText(", practically dragging her gut on the stones as she runs");
        this.outputText(
            ".  Well, she might have gotten away, but she won't be able to stop you from getting to Lethice.  Indeed, it seems she was running for the exit."
        );
    ...
    private femFuckHerII(cockType: number): void {
        ...
        if (cockType == this.TENTACOCK) {
            ...
            if (this.player.clitLength >= 6)
                this.outputText(
                    " A coil of cock loops around your gigantic buzzer, effectively frotting it like another cock and chance at protest from your lips."
                );
            else
                this.outputText(
                    " A bend in the cock grinds against your buzzer with particular intensity, effectively robbing any chance at protest from your lips."
                );
            ...
            if (this.player.clitLength >= 6)
                this.outputText(
                    " - you swear it spends as much time caressing your [clit] as squeezing it off"
                );
            this.outputText(
                ".  You buck your hips, dragging the butt-piercing wang along for the ride as you start to fuck in earnest."
            );
            ...
            this.outputText(
                'You give her throbbing maleness an affection pat and proclaim, "<i>Make it a dog-cock, one that fit to knot properly.  Then we can fuck.</i>"'
            );
            ...
            if (this.player.clitLength >= 6)
                this.outputText(
                    " The hard, feminine pole twitches nicely at your touch, a far more sensitive rod than any cock."
                );
            this.outputText(
                " Your fingers sink into your gash with ease and set to work with ease.  Of course, they're soaked in a second."
            );
    ...
    private femLoss(hpVictory: boolean): void {
        ...
        if (hpVictory)
            this.outputText(
                " try to pull yourself up with on your palms, utterly defeated in spite of the centaur's complete lack of physical attacks."
            );
        else {
        ...
        this.outputText(
            " She smirks as she trots up, commenting, \"<i>That's more like it, slut.  I don't know how you got this far, but your quest ends here.</i>\"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits"
        );
        if (this.player.tallness >= 7 * 12)
            this.outputText(", even though your [feet] still touch the ground");
        this.outputText(
            '.  Brushing through your hair, a hand gently caresses your back as its owner coos, "<i>You\'ll make a fine mare for my stable, but lets get you in the right mindset first...</i>"'
        );
        ...
        this.outputText(
            "Your [vagina] is so puffy and inflamed with passion that it feels ready to burst out of your [armor], even as the drooling fem-cum makes it squish wetly with your every motion."
        );
        if (this.player.hasCock())
            this.outputText(
                " [EachCock] fares little better, a turgid mess of pre-cum leaking lust that seems to grow harder and more sensitive by the moment."
            );
        this.outputText(
            " That wonderful warmth excites you sexually, a deepening puddle of lust in your loins that makes you so hot and wet that you swear it must be what's dripping out onto the cobble below."
        );
    ...
    private maleLoss(hpVictory: boolean): void {
        ...
        if (hpVictory) {
            ...
            this.outputText(
                " try to pull [oneCock] out of your [armor] in an attempt to masturbate.  You're beaten, you just want to fuck or be fucked until you're both covered in a sheen of lady-spunk or your own thick cum."
            );
        }
        this.outputText(
        ...
        if (this.player.tallness >= 7 * 12)
            this.outputText(", even though your [feet] still touch the ground");
        this.outputText(
            '.  Brushing through your hair, a hand gently caresses your back as its owner coos, "<i>I\'m not sure you have what it takes to be one of my stallions...</i>"'
        );
        ...
        if (this.player.hasVagina())
            this.outputText(
                " Your [vagina] fares no better, puffing up and drooling like its big brother while your [clit] grows taut and sensitive."
            );
        this.outputText(
            " That wonderful warmth excites you sexually, moulding your loins into a simmering blaze of desire just looking for a something to cool its ache."
        );
        ...
        if (this.player.tongueType != 0)
            this.outputText(" and uncoil your inhuman length to spear");
        else this.outputText(" and begin to lap");
        this.outputText(
            " at her inhuman entrance.  She whinnies in delight, grinding her clydesdale-sized bulk back and forth on your face as you start to get into her eating her out."
        );
        ...
        if (this.flags[kFLAGS.D1_OMNIBUS_KILLED] != 1)
            this.outputText(", and a familiar one at that");
        this.outputText(
            ".  She wears a black reflective bra, though you swear you can see pink protrusions under the shiny fabric, wriggling around constantly.  As expected with such dress, her nipples are hard as bullets.  A bulbous, ten-inch penis bobs between her thighs, thickening nicely as it fills to its full size, kissing your lips with a moist bead of pre as it fully erects.  The omnibus"
        );
        if (this.flags[kFLAGS.D1_OMNIBUS_KILLED] != 1)
            this.outputText(
                ' overseer asks, "<i>Remember me?  I thought it only proper that I thank you for letting me live.  Now, be a good boy and suck my dick - show all everyone just what a horny, helplessly fuckable toy you are.</i>"'
            );
        else
            this.outputText(
                " asks, \"<i>I hope you don't mind slurping down a little cock, because you're going to anyway.</i>\""
            );
        ...
        this.outputText(
            " and sidles forward, ramming her glans into your mouth and down the back of your throat.  You can feel her glans pulsing deep inside you, and as soon as the herm starts moaning, the centaur starts trotting forward again.  Amazingly, having her holding you up makes it easy to maintain your position and keep your dick bouncing in that hot twat. Fuck it! You're too horny to care anymore."
        );
