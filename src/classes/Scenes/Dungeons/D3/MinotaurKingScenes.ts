...
export class MinotaurKingScenes extends BaseContent {
    ...
    public encounterMinotaurKing(): void {
        ...
        if (this.flags[kFLAGS.ISABELLA_FOLLOWER_ACCEPTED] != 0) this.outputText(" Even Isabella would struggle to wield such a weapon.");
        else if (this.player.findStatusAffect(StatusAffects.CampMarble) >= 0) this.outputText(" Even Marble would struggle to wield such a weapon");
        else if (this.getGame().helFollower.followerHel()) this.outputText(" Even Helia would struggle to wield such a weapon.");
    ...
    public theKingIsDeadLongLiveTheKing(hpVictory: boolean): void {
        ...
        if (this.player.cor <= 33) this.outputText(" The poor things are so corrupted.");
    ...
    private dockucocku(cockIdx: number): void {
        ...
        if (this.player.horseCocks() > 0) this.outputText(" a horse-dick like yours");
        else this.outputText(" the average equine prick");
    ...
    private dockucockuknotu(cockIdx: number): void {
        ...
        if (this.player.balls == 0) this.outputText(", fat with bull-cream.");
        else this.outputText(". Your [sack] swells larger and larger, complete with the realization that your [balls] are being transformed into spooge-filled balloons.");
    ...
    private buttufucku(): void {
        ...
        else this.outputText(" until you’ve buried the entirety of your [cockBiggest] inside his snugly squeezing intestines, repurposing his digestive system into your own personal, phallus playground.");
        ...
        if (this.player.hasKnot()) this.outputText(" Your knot balloons.");
        else if (this.player.horseCocks() > 0) this.outputText(" Your flare expands right on cue, matching the one pressing back against your tongue.");
        else this.outputText(" His flare blooms with all the force of a mushroom cloud, nearly pushing your tongue back into your mouth.");
        ...
        if (this.player.cumQ() > 2500) {
            ...
            if (this.player.biggestCockLength() >= 18) this.outputText(" even bigger");
            if (this.player.cumQ() > 5000) {
                ...
                else this.outputText(" your crotch");
    ...
    private mechanicalbullhue(): void {
        ...
        if (this.player.averageVaginalLooseness() <= 3) this.outputText(", probably");
        this.outputText(".");
        if (this.player.averageVaginalLooseness() == 4) this.outputText(" One of the perks of having experience with well-endowed lovers is not having to second-guess yourself when you come upon a real treat like this one.");
        else if (this.player.averageVaginalLooseness() < 4) this.outputText(" You’re not the most experienced at handling such well-endowed studs, but even if you’re wrong, nothing beats trying.");
        else this.outputText(" A big thing like that would stretch you so wonderfully. You barely manage to swallow before you start to drool.");
        ...
        if (this.player.wetness() >= 4) this.outputText(" gushes, drenching your thighs");
        else if (this.player.wetness() >= 3) this.outputText(" drools rivulets of desire down your thighs");
        else this.outputText(" drenches itself anew");
        this.outputText(". Part of you is tempted to drop to");
        if (!this.player.isNaga() && !this.player.isGoo()) this.outputText(" your knees");
        else this.outputText(" the ground");
        ...
        if (this.player.averageVaginalLooseness() >= 5) {
            ...
            if (!this.player.hasVirginVagina()) this.outputText("-looking");
        ...
        if (this.player.hasCock()) this.outputText(", stimulating the base of your jutting she-dick");
        ...
        if (this.player.lust >= 80) this.outputText(" - not that you needed much help -");
        this.outputText(" or his unbelievably potent fluids addling your mind, you find it hard to care. The heat burning");
        if (this.player.isBiped() || this.player.isTaur()) this.outputText(" between your legs");
        else this.outputText(" in your loins");
        ...
        if (!this.player.isTaur()) this.outputText(" in its entirety");
        ...
        if (this.player.tallness <= 50 && this.player.thickness <= 4) this.outputText(", stretching your skin until it shines");
        this.outputText(".");
        ...
        if (this.player.isLactating()) this.outputText(", tricking your full nipples into spraying their milk everywhere");
        else this.outputText(", denying you even the chance to climb off");
        ...
        if (this.player.pregnancyIncubation != 0) this.outputText(" force its way past your cervix");
        else this.outputText(" tear open your womb");
        this.outputText(". Your insides stretch like they’re made of rubber");
        if (this.player.averageVaginalLooseness() >= 5) this.outputText(", which isn’t far from the truth,");
        ...
        if (this.player.isBiped()) this.outputText(", [legs] akimbo");
    ...
    private titfuckCowslut(): void {
        ...
        if (this.player.cumQ() <= 500) this.outputText(" Most fall short, but one daring squirt manages to coat Excellia’s tongue.");
        else if (this.player.cumQ() <= 1500) this.outputText(" Most manage to fall on Excellia’s tongue, painting it a nearly white save for a few off-color splotches. The rest puddles in the nape of her neck, giving you time to enjoy the cow-print tongue you’ve created.");
        else {
            ...
            if (this.player.cumQ() >= 5000) this.outputText(" And her breasts? They’re basically white save for their peaks.");
        }
    ...
    private sloppySeconds(): void {
        ...
        if (this.player.findPerk(PerkLib.MinotaurCumAddict) >= 0) this.outputText(" wonderful");
        else this.outputText(" sort of... nice... sexy even");
        ...
        else this.outputText(" and a load of backed-up cum");
        ...
        if (this.player.balls > 0) this.outputText(" ball");
        ...
        if (this.player.biggestCockLength() < 18) this.outputText(" You don’t think she could possibly confuse your less-impressive erection for her minotaur master’s, but she doesn’t give any sign of caring. Perhaps all the minotaur jism stuffed in her twat has left her beyond caring, or maybe she’s just happy to serve a dick - any dick.");
        else if (this.player.biggestCockLength() <= 30) this.outputText(" You didn’t think she’d confuse your prick for her master’s, but she’s happily fucking it all the same. It probably stretches her cunt just about as much.");
        ...
        if (this.player.cocks.length == 2) this.outputText(" Your other penis is enjoying sliding between her perspiring thighs almost as much.");
        else if (this.player.cocks.length > 2) this.outputText(" Your other penises are enjoying sliding between her perspiring thighs nearly as much.");
        ...
        if (this.player.hasKnot()) this.outputText(" There’s not much point in it; your knot does that much and more, trapping your load inside as effectively as the remainder of the Minotaur King’s.");
        this.outputText(" Your");
        if (this.player.isBiped() || this.player.isTaur()) this.outputText(" [legs] wobble");
        else this.outputText(" [leg] wobbles");
    ...
    public hailToTheKingBaby(hpVictory: boolean, pcCameWorms: boolean): void {
        ...
        if (!hpVictory) {
            ...
            if (mk.orgasms > 0) this.outputText("... so wet with the juices of his slut");
        ...
        if (this.player.findPerk(PerkLib.BimboBrains) >= 0 || this.player.cor >= 66) this.outputText(" You’re long past the point of having a gag reflex and glad for it.");
        else this.outputText(" Miraculously, your gag reflex is completely absent. Maybe it’s too drenched in his heavenly pre to protest?");
        ...
        if (this.player.biggestTitSize() >= 5) this.outputText(", jiggling your tits wildly");
        ...
        else this.outputText(", shaking your whole body with the force of his passion");
    ...
    private leavethem(): void {
        ...
        if (this.player.weapon != WeaponLib.FISTS) this.outputText(", [weapon] in hand");
        else this.outputText(", balled fists raised");
        this.outputText(" and reject her offer out of hand. <i>“");
        if (this.player.findPerk(PerkLib.BimboBrains) >= 0) this.outputText("Ummm, like, I’m not </i>that<i> horny. Besides, I can fuck all I want once I take you out, cutie!");
        else if (this.player.cor >= 66) this.outputText("Oh come on. If you’re going to give me something to fuck, at least make it something worth my time. How about your ass, after I beat you?");
        else if (this.player.cor <= 20) this.outputText("Why would I debase myself in front of you like that? The pure need no such indulgence.");
        else this.outputText("Not all of us are as enslaved by our desires as you.");
        this.outputText("”</i>");
