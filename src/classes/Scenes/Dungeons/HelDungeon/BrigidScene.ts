...
export class BrigidScene extends BaseContent {
    ...
    public pcDefeatsBrigid(): void {
        this.clearOutput();
        this.outx("The harpy jailer collapses, ");
        if (this.monster.lust > 99) this.outx("too turned on");
        else this.outx("too badly beaten");
        this.outx(
    ...
    public pcDefeatedByBrigid(): void {
        ...
        if (this.flags[kFLAGS.HEL_HARPY_QUEEN_DEFEATED] == 0) {
            ...
            this.outx(
                '"<i>That\'s it?!</i>" Brigid screams, as you collapse in front of her.  "<i>A weak little piece of trash like you took out our queen?</i>"'
            );
    ...
    private brigitPostQueenDeathBadEndII(): void {
        ...
        if (this.player.gender <= 1 || this.player.pregnancyIncubation > 0) {
            ...
            if (!this.player.hasCock()) {
                ...
                if (this.player.pregnancyIncubation > 0)
                    this.outx(
                        "  Once this disgusting thing you're carrying falls out of you, we'll use you more traditionally, but...</i>\"  She pauses to press uncomfortably on your pregnant belly, then resumes speaking.  \"<i>I have no intention of letting you sit idle for however long that takes."
                    );
                this.outx(
                    "  For now, I've given you the most obvious modification - I can think of something else later.</i>\""
                );
                this.outx(
                    "\n\nShe squeezes Kiri's ass firmly with one hand, and lifts the girl partway out of your lap.  A smooth, hot sensation separates from your crotch and glides along nerves you hadn't noticed until now as Kiri's pussy slides upward, partially revealing the erect, " +
                        this.player.skinTone +
                        " flesh of a brand new cock!  Brigid gloats at your confusion, laughing her delight to the roof of the chamber, then shoves Kiri's ass downward again, spearing her pussy on your new flesh and making the girl wince and grit her teeth."
                );
            ...
            if (this.player.cockTotal() > 1 || this.player.cumQ() > 1000)
                this.outx(
                    "  " +
                        this.SMultiCockDesc() +
                        " holds forth for a while, until Kiri's ass, pussy, and tailfeathers are soaked with oozing, white cum."
                );
            this.outx(
        ...
        // (else if not visibly pregnant F or H)
            ...
            if (this.player.hasCock())
                this.outx(this.sMultiCockDesc() + " bobbing up and down, drooling pre-cum, as ");
            this.outx(
            ...
            if (this.player.hasCock())
                this.outx(" as the phoenix's cunt slides onto your [cock smallest]");
            this.outx(
