import { ANAL_LOOSENESS_VIRGIN, ANAL_WETNESS_DRY, BUTT_RATING_LARGE, HIP_RATING_CURVY, TAIL_TYPE_LIZARD, VAGINA_LOOSENESS_NORMAL, VAGINA_WETNESS_NORMAL } from "../../../includes/appearanceDefs";
...
export class Helspawn extends Monster {
    ...
    public doAI(): void {
        ...
        //Zerker ability
        if (this.weaponAttack < 50 || this.flags[kFLAGS.HELSPAWN_WEAPON] == "scimitar") choices[choices.length] = this.helSpawnBerserk;	//Shield Bash (Shieldmander Only)
        if (this.flags[kFLAGS.HELSPAWN_WEAPON] == "scimitar and shield") choices[choices.length] = this.helSpawnShieldBash;
        //Tease (Sluttymander Only)
        if (this.flags[kFLAGS.HELSPAWN_PERSONALITY] >= 50) choices[choices.length] = this.sluttyMander;
        //Focus (Chastemander Only)
        //Self-healing & lust restoration
        if (this.flags[kFLAGS.HELSPAWN_PERSONALITY] < 50) choices[choices.length] = this.helSpawnFocus;
    ...
    private helspawnTwinStrikes(): void {
        //if Bowmander
        if (this.flags[kFLAGS.HELSPAWN_WEAPON] == "bow") this.outputText(this.flags[kFLAGS.HELSPAWN_NAME] + " leaps back out of your reach and nocks a pair of blunted arrows, drawing them back together and loosing them at once!\n");
        else this.outputText(this.flags[kFLAGS.HELSPAWN_NAME] + " lunges at you, scimitar cleaving through the air toward your throat!\n");
    ...
    private calledShot(): void {
        this.outputText(this.flags[kFLAGS.HELSPAWN_NAME] + " draws back her bowstring, spending an extra second aiming before letting fly!");
        var damage: number = Math.floor((this.str + this.weaponAttack) - Helspawn.rand(this.player.tou) - this.player.armorDef);
        //standard dodge/miss text
        if (damage <= 0 || (Helspawn.rand(2) == 0 && (this.combatMiss() || this.combatEvade() || this.combatFlexibility() || this.combatMisdirect()))) this.outputText("\nYou avoid the hit!");
        else {
    ...
    private helSpawnBerserk(): void {
        this.outputText(this.flags[kFLAGS.HELSPAWN_NAME] + " lets out a savage warcry, throwing her head back in primal exaltation before charging back into the fray with utter bloodlust in her wild eyes!");
    ...
    private helSpawnShieldBash(): void {
        this.clearOutput();
        var damage: number = Math.floor((this.str) - Helspawn.rand(this.player.tou) - this.player.armorDef);
        // Stuns a bitch
        this.outputText(this.flags[kFLAGS.HELSPAWN_NAME] + " lashes out with her shield, trying to knock you back!");
        //standard dodge/miss text
        if (damage <= 0 || this.combatMiss() || this.combatEvade() || this.combatFlexibility() || this.combatMisdirect()) this.outputText("\nYou evade the strike.");
        else {
    ...
    private tailWhipShitYo(): void {
        ...
        this.outputText("\n" + this.flags[kFLAGS.HELSPAWN_NAME] + " whips at you with her tail, trying to sear you with her brilliant flames!");
        //standard dodge/miss text
        if (damage <= 0 || this.combatMiss() || this.combatEvade() || this.combatFlexibility() || this.combatMisdirect()) this.outputText("\nYou evade the strike.");
        else {
            this.outputText("\n" + this.flags[kFLAGS.HELSPAWN_NAME] + "'s tail catches you as you try to dodge.  Your [armor] sizzles, and you leap back with a yelp as she gives you a light burning.");
            damage = this.player.takeDamage(damage);
            this.outputText(" (" + damage + ")");
        }
    }
    ...
    private sluttyMander(): void {
        // Medium Lust damage
        this.outputText(this.flags[kFLAGS.HELSPAWN_NAME] + " jumps just out of reach before spinning around, planting her weapon in the ground as she turns her backside to you and gives her sizable ass a rhythmic shake, swaying her full hips hypnotically.");
        //if no effect:
        if (Helspawn.rand(2) == 0) this.outputText("\nWhat the fuck is she trying to do?  You walk over and give her a sharp kick in the kiester, \"<i>Keep your head in the game, kiddo.  Pick up your weapon!</i>\"");
        else {
    ...
    private helSpawnFocus(): void {
        this.outputText("Seeing a momentary lull in the melee, " + this.flags[kFLAGS.HELSPAWN_NAME] + " slips out of reach, stumbling back and clutching at the bruises forming all over her body.  \"<i>Come on, " + this.flags[kFLAGS.HELSPAWN_NAME] + ", you can do this. Focus, focus,</i>\" she mutters, trying to catch her breath.  A moment later and she seems to have taken a second wind as she readies her weapon with a renewed vigor.");
        this.lust -= 30;
    ...
    public constructor() {
        ...
        this.long = this.game.flags[kFLAGS.HELSPAWN_NAME] + " is a young salamander, appearing in her later teens.  Clad in " +
