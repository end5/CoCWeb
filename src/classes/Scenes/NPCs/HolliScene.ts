...
export class HolliScene extends NPCAwareContent {
    ...
    public getASprout(): void {
        ...
        if (this.flags[kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] == 2)
            this.outx("  She even promised to plant a tree in your camp...");
        this.outx(
    ...
    public fuckPlantGrowsToLevel2(): void {
        ...
        if (this.player.gender == 0)
            this.outx("  Sucks to be it!  You don't waste time with stupid shit, like genitals.");
        this.outx("  Is this Marae's idea of a gift?");
        if (this.player.gender > 0)
            this.outx(
                "  Well, you could give it a whirl... just being around it seems to slowly turn you on."
            );
        this.outx("  Of course, destroying it would be the safest option.");
    ...
    private fuckFuckingFuckFlowerP2(): void {
        ...
        if (this.player.cor < 33)
            this.outx(" and you timidly look around, hoping you haven't attracted an audience.");
    ...
    private rideDatFuckingFukkFlowerP2(): void {
        ...
        this.outx(", caressing the");
        if (this.player.biggestTitSize() > 1) this.outx("ir");
        this.outx(
        ...
        if (this.player.lactationQ() > 50) {
            this.outx("  One after the other, they begin to ");
            if (this.player.lactationQ() < 150) this.outx("trickle");
            else if (this.player.lactationQ() < 500) this.outx("leak");
            else this.outx("spray torrents of");
            this.outx(" milk.");
        }
        this.outx(
        ...
        if (this.player.cumQ() < 50) this.outx("drizzle");
        else if (this.player.cumQ() < 250) this.outx("stream");
        else if (this.player.cumQ() < 750) this.outx("spray");
    ...
    public flowerGrowsToP3(): void {
        ...
        if (this.flags[kFLAGS.TIMES_FUCKED_FLOWER] + this.flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0)
            this.outx("  The fucking probably only helped it to grow faster.");
        this.outx(
    ...
    private fuckTheFlower(): void {
        ...
        if (this.player.cockTotal() > 1) {
            ...
            if (this.player.cockTotal() > 2) this.outx("es rub");
            else this.outx(" rubs");
            this.outx(
                " sensually across the moist outer blossom, drooling their clear pre-cum over the petals."
            );
        }
        this.outx("  Schliiick.  You slide out.  Slorp-pop.  You slide in, this time to the ");
        ...
        if (this.player.hasVagina()) {
            this.outx("  Your hungry cunt, long denied in this exchange, trembles as it ");
            if (this.player.wetness() < 4) this.outx("leaks");
            else this.outx("sprays");
            this.outx(" your own female fluids in sympathy.");
        }
        this.outx(
    ...
    private drinkThePlantGirlsSap(): void {
        ...
        if (this.silly()) this.outx(", tree hugger that you are");
        this.outx(
    ...
    private torchP3Tree(): void {
        ...
        if (this.player.findStatusAffect(StatusAffects.KnowsWhitefire) >= 0)
            this.outx("magical will");
        else this.outx("breath");
        this.outx(".");
    ...
    public JojoTransformAndRollOut(): void {
        ...
        this.outx('"<i>[name], come here please.</i>"');
        this.outx(
        ...
        this.outx("  The monk's eyes widen in surprise and surmise.");
        ...
        this.outx('</i>"  So saying, he presses a small talisman into your hand.');

        this.outx(
            '"<i>Until then, you will find me in the forest if you need my assistance.</i>"  Jojo makes a polite bow and departs.'
        );
        this.outx("(<b>Jojo is no longer a companion!</b>)");
    ...
    public amilyHatesTreeFucking(): void {
        this.clearOutput();
        this.outx("Your mousey cohabitant tramps up to you, irritation plain on her expression.");
        this.outx(
    ...
    private slapAmilysWhoreFace(): void {
        this.clearOutput();
        this.outx("You slap Amily across the ");
        if (this.flags[kFLAGS.AMILY_NOT_FURRY] == 0) this.outx("nose");
        else this.outx("face");
        this.outx(
        ...
        if (this.flags[kFLAGS.AMILY_NOT_FURRY] == 0) this.outx("mouse-");
        this.outx('girl says, eyes wet with tears, "<i>but I don\'t.  Goodbye.</i>"');
    ...
    private fuckHolliInZeFlowerPuss(): void {
        ...
        if (this.player.cockTotal() > 1) {
            if (this.player.cockTotal() == 2) this.outx("  Your other penis splays");
            else this.outx("The rest of your penises splay");
            this.outx(
                " to the side, smearing furrows of the stuff around as they slide over the butter-smooth skin."
            );
        }
        this.outx(" The demon dryad's hand reaches below to ");
        if (this.player.balls > 0) this.outx("cradle your [balls], gently massaging your sack");
        else {
            this.outx("gently caress your ");
            if (this.player.hasSheath()) this.outx("sheath");
        ...
        if (this.player.tallness < 60) this.outx("tits");
        else this.outx("arms");
        this.outx(
        ...
        if (this.player.cockTotal() > 1)
            this.outx("s, massaging the bundle of cocks with uncommon ferver");
        else this.outx(", massing your tool with uncommon fervor");
        this.outx(
        ...
        if (this.player.biggestTitSize() >= 2) this.outx("your own");
    ...
    private level4RideHollisTentacruels(): void {
        ...
        if (!this.player.isBiped())
            this.outx("you'd be walking bowlegged for a week, if you had legs.");
        else this.outx("you'll be walking bowlegged for a week!");
    ...
    private eatHolliFruit(): void {
        this.clearOutput();
        this.outx("You reach up into Holli's branches and pluck ");
        if (this.flags[kFLAGS.HOLLI_FRUIT] == 1) this.outx("the fruit");
        else this.outx("one of the fruits");
        this.outx(".  ");
        if (this.flags[kFLAGS.HOLLI_FRUIT_EXPLAINED] == 0) {
            this.outx(
    ...
    private askBrokenHolliToGuard(): void {
        ...
        if (this.flags[kFLAGS.HOLLI_DEFENSE_ON] == 1) this.outx("keeping watch");
        else this.outx("sleeping, instead");
        this.outx('.</i>"');
    ...
    private holliGetsDickDommed(): void {
        ...
        if (this.player.biggestCockArea() > 80) this.outx(", jaw stretching obscenely");
        this.outx(".");
        this.outx(
        ...
        if (this.player.biggestCockArea() <= 80) {
            this.outx(", swallowed to the ");
            if (this.player.balls > 0) this.outx("[balls]");
            else this.outx("[sheath]");
            this.outx(".");
        } else this.outx(", swallowed as deep as Holli's distended neck can handle.");
        this.outx(
        ...
        if (this.player.balls > 0) this.outx("[sack] feel taut and ready to burst");
        else this.outx("body feel strung tight like an over-tuned guitar");
        this.outx(
        ...
        if (this.player.cumQ() > 2000)
            this.outx("  A moment later, that trickle turns into a river.");
        if (this.player.cumQ() > 3000)
            this.outx("  You swear that her trunk must be absolutely stuffed with spunk.");
    ...
    private fullOnTentacleTasticGangBangForHolli(): void {
        ...
        if (this.silly()) this.outx(", inflatable-arm, flailing tube-cocks");
        this.outx(
        ...
        if (this.player.cumQ() > 500) this.outx(", and the next");
        if (this.player.cumQ() > 1400) this.outx("... and the next");
        this.outx(
    ...
    private vaginalDomHollisTentacruels(): void {
        ...
        if (!this.player.hasFuckableNipples() && this.player.lactationQ() < 50)
            ...
            this.outx(" and sigh at the ");
            if (this.player.lactationQ() < 150) this.outx("squirts");
            else if (this.player.lactationQ() < 800) this.outx("sprays");
            else if (this.player.lactationQ() < 2000) this.outx("outpouring");
            else this.outx("thick fountains");
            this.outx(
        ...
        if (this.player.clitLength >= 3.5)
            this.outx(" for a moment before wrapping around and jerking on it");
        this.outx(".");
        ...
        if (this.player.wetness() < 4) this.outx("onto the ground");
        else this.outx("into the leavings of your squirting orgasm");
        this.outx(
    ...
    public defeatHolli(): void {
        ...
        if (this.monster.findStatusAffect(StatusAffects.JojoIsAssisting) < 0) {
            ...
            if (this.player.weaponName != "large axe") this.outx("fetch an axe and ");
            this.outx(
    ...
    private dontRecruitJojoToCamp(): void {
        this.clearOutput();
        this.outx("You hold your tongue, allowing Jojo to make his way back to the forest.");
        this.outx("\n\n(Key Item Gained: Holli's Ashes)");
        this.player.createKeyItem("Holli's Ashes", 0, 0, 0, 0);
        this.cleanupAfterCombat();
    }
    ...
    public enjoyYourBadEndBIYAAAATCH(): void {
        ...
        if (this.player.gender == 1 || (HolliScene.rand(2) == 0 && this.player.hasCock())) {
            ...
            if (this.player.balls > 0)
                this.outx(", and your [balls] draw close to your body to unload");
            this.outx(
        ...
        // branch of vagina-having
            ...
            if (this.player.isTaur()) this.outx(" and turning you around");
            this.outx(
            ...
            if (!this.player.isGoo() && !this.player.isNaga())
                this.outx("slips between your thighs and ");
            this.outx(
            ...
            if (this.player.hasCock()) this.outx(" and a glob of precum from [eachCock]");
            this.outx(".");
            ...
            if (this.player.biggestTitSize() > 1)
                this.outx("your breasts cupped and pressed together by vines, ");
            if (!this.player.isNaga() && !this.player.isGoo())
                this.outx("your thighs spread wide open, ");
            this.outx("and your nectar-drooling flower-sex exposed with your [clit]");
            if (this.player.hasCock()) this.outx(" and [eachCock]");
            this.outx(" bobbing in a state of permanent semi-arousal.");
            ...
            this.outx("The sap oozing from her member coats your passage, ");
            if (this.player.ass.analWetness > 0)
                this.outx(
            ...
            if (this.player.ass.analWetness > 0 || this.player.analCapacity() > 30)
                this.outx("spilling a wave of her deposited fluids down your [legs].  ");
            else
                this.outx(
            ...
            this.outx(" and your [legs] concealed below your buttcheeks by a thick tree trunk.");
    ...
    private girlsGetANiceManToBadEnd(): void {
        ...
        // [(if tits)
        if (this.player.biggestTitSize() > 1)
            this.outx("[chest] cupped by vines and held out toward the young man, ");
        ...
        if (this.player.ballSize > 10 && this.player.balls > 0)
            this.outx("lifting the hefty testicles to reveal a wet, eager pussy.");
        else this.outx("spreading the labia and exposing the eager, clenching hole inside.");
        ...
        if (this.player.biggestTitSize() >= 1) {
            ...
            if (this.player.gender == 2) this.outx("woman");
            else this.outx("hermaphrodite");
            this.outx("'s mouth open and close in bliss.  ");
        } else
            this.outx(
        ...
        if (this.player.hasCock())
            ...
            this.outx('"<i>Does that feel good?</i>" he asks, upping the pace of his thrusts.  ');
        }
        this.outx("As if in reply, the ");
        if (this.player.gender == 2) this.outx("woman");
        else this.outx("hermaphrodite");
        this.outx(" in the tree arches her back and her [vagina] clamps down on his tool");
        if (this.player.hasCock()) {
            this.outx("; ");
            if (this.player.balls > 0) this.outx("her balls draw closer to her body and ");
            this.outx("her trembling dick");
            if (this.player.cockTotal() == 1) this.outx(" throbs");
            else this.outx("s throb");
            this.outx(" as she nears climax");
        }
        this.outx(".");
        ...
        // (vagina and no cock)
            ...
            this.outx("\n\nA ");
            if (this.player.wetness() < 3) this.outx("gush");
            else if (this.player.wetness() < 4) this.outx("rush");
            else this.outx("torrent");
            this.outx(
        ...
        if (this.player.cockTotal() == 1) this.outx("cock");
        else if (this.player.cockTotal() > 1) this.outx("cocks");
        else this.outx("chest");
        this.outx(", he goes limp and slack-jawed as his dick squirts, filling her vagina.");
