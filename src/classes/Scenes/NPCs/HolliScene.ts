...
export class HolliScene extends NPCAwareContent {
    ...
    public treeMenu(output = true): void {
        ...
        if (this.flags[kFLAGS.FOUGHT_HOLLI] > 0) {
            ...
            this.outputText(
                "The demon tree is still here, and the face peeking from it still stares daggers at you even from a distance.  It looks like forgiveness won't be forthcoming."
            );
            ...
            if (output)
                this.outputText(
                    "Before you is a large, waist-high plant of questionable origin.  A huge, violet bloom sits atop a strong, central stalk, and the obscene flower looks positively female in every sense, right down to a clit-shaped bump and moist, gently parted lips.  Inside that central passage, you can see a bevy of wiggling staman surrounding a larger, flared stalk.  The scent that wafts out of that opening is sweet and floral, but undoubtedly sexual.  It looks like an organic sex toy and is clearly corrupt.  It wouldn't be hard to get rid of right now, assuming you don't let it grow larger."
                );
            if (
                this.player.hasCock() &&
                this.player.cockThatFits(100) >= 0 &&
                this.player.lust >= 33
            )
            ...
            if (output) {
                ...
                if (this.flags[kFLAGS.HOLLI_SUBMISSIVE] == 1) {
                    ...
                    if (this.flags[kFLAGS.TIMES_FUCKED_FLOWER] > 0 && HolliScene.rand(3) == 0)
                        this.outputText(
                            "Did you want to fertilize me again?  Gardening is such exhausting work, but so rewarding..."
                        );
                    else if (this.flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0 && HolliScene.rand(2) == 0)
                        this.outputText(
                            "Did you come back to play again?  My tentacles are so restless."
                        );
                    else if (HolliScene.rand(2) == 0)
                        this.outputText(
                            "Did you come for a drink?  I've built up lots of nectar for you.  Surely a hungry adventurer like yourself needs a sip, right?"
                        );
                    else
                        this.outputText(
                            "Do you want to play?  Come on, taste Mother Marae's finest creation."
                        );
                    this.outputText('</i>"');
                }
                // Extra line on description:
                if (this.flags[kFLAGS.HOLLI_FRUIT] > 0) {
                    if (this.flags[kFLAGS.HOLLI_FRUIT] == 1)
                        this.outputText(
                    ...
                    if (this.flags[kFLAGS.HOLLI_FRUIT_EXPLAINED] > 0)
                        this.outputText(
                            "  A very small, strange part of you actually feels proud to have made something like that with the tree-woman."
                        );
                }
            }
            ...
            if (
                this.flags[kFLAGS.FOLLOWER_AT_FARM_HOLLI] == 0 &&
    ...
    private helpWithFarm(): void {
        ...
        this.outputText(
            "You ask if a plant goddess might not be able to help the farm you recently acquired."
        );
    ...
    private noPlzDontFuckWithFarm(): void {
        ...
        this.outputText(
            "“<i>Such a fickle mortal,</i>” she sighs. “<i>But I am a kind goddess. Ask me for it anytime, assuming you’ve... performed the rites.</i>”"
        );
    ...
    public getASprout(): void {
        ...
        if (this.flags[kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] == 2)
            this.outputText("  She even promised to plant a tree in your camp...");
        this.outputText(
            "  You look closer and see purple veins on the undersides of the leaves, throbbing with what can only be the tainted liquids that suffuse much of this planet.  Fully grown, it might spread its corruption to your camp.\n\n<b>Do you destroy it?</b>"
        );
    ...
    private letZeFuckingSproutLive(): void {
        this.clearOutput();
        this.outputText(
            "Looking down at the sapling, you stay your wrath.  It may be corrupt, but it hasn't done anything to harm you just yet.  You give it a little pat on the uppermost leaves and leave it be.  It's not like it's going anywhere."
        );
        this.outputText(
    ...
    public fuckPlantGrowsToLevel2(): void {
        ...
        if (this.player.gender == 0)
            this.outputText(
                "  Sucks to be it!  You don't waste time with stupid shit, like genitals."
            );
        this.outputText("  Is this Marae's idea of a gift?");
        if (this.player.gender > 0)
            this.outputText(
                "  Well, you could give it a whirl... just being around it seems to slowly turn you on."
            );
    ...
    private fuckFuckingFuckFlowerP2(): void {
        ...
        this.outputText(
            " and form a sticky puddle.  Now moaning, you grab the stem and start to pump the sheath up and down, using the plant like the cock-toy it was meant to be.  Lewd, wet squishes ring out through the air,"
        );
        if (this.player.cor < 33)
            this.outputText(
                " and you timidly look around, hoping you haven't attracted an audience."
            );
        else if (this.player.cor < 66)
            this.outputText(
                " and you worriedly glance around, hoping yet terrified that someone will walk in on it."
            );
        else
            this.outputText(
        ...
        if (this.player.balls > 0)
            this.outputText(
                "Your [balls] gurgle, a surprisingly happy sound, and cling tight to your loins.  "
            );
        this.outputText(
        ...
        if (this.player.cumQ() > 500)
            this.outputText(
                "  You pour forth your prodigious essence without pause or fail, and the plant immediately begins to drink it down, the stem bulging with rounded balls of sperm that it carries underground."
            );
        if (this.player.cumQ() > 1500)
            this.outputText(
                "  Soon, the meager stem cannot swallow it all.  Spunk froths and squirts out around the base of your tool for a moment, and then a veritable tide of the stuff gushes out to soak  you."
            );
        this.outputText(
        ...
        if (this.flags[kFLAGS.FUCK_FLOWER_GROWTH_COUNTER] < 1000)
            this.flags[kFLAGS.FUCK_FLOWER_GROWTH_COUNTER] += 4;
        this.fatigue(5);
        this.doNext(this.camp.returnToCampUseOneHour);
    }
    ...
    private rideDatFuckingFukkFlowerP2(): void {
        ...
        if (this.player.clitLength > 4)
            this.outputText(
                "  Your heavy buzzer is so big that the flexible plant-cock has plenty of room to curl around it, and if you could see it, your clit would look almost like a candy-cane (if one set of stripes was jerking off the cane beneath)."
            );
        ...
        if (this.player.bRows() > 1)
            this.outputText(
                "lowest row of tits, cupping them before groping to your top row of breasts"
            );
        else this.outputText("[chest]");
        this.outputText(", caressing the");
        if (this.player.biggestTitSize() > 1) this.outputText("ir");
        this.outputText(
            " swell as you search for the lust-tightened skin of your [nipples].  They seem almost as sensitive as little clits, and as soon as you find them, you immediately set to circling them slowly, occasionally tugging and twisting when a particularly powerful sensation works through your pleasure-wracked body."
        );
        ...
        this.outputText(
            "  You moan in unabashed delight, your mouth hanging open in a mindless, blissful 'o' in between vocalizations.  If your friends back home could see you now, writhing in the dirt, face open in simple, animal pleasure, and perhaps worst of all, fucking a plant, you'd die of "
        );
        if (this.flags[kFLAGS.PC_FETISH] == 0 && this.player.cor + this.player.lib < 150)
        ...
        if (this.player.hasCock())
            this.outputText(
                "  [EachCock] explodes in release, pumping heavy flows of seed into the petals, even as y"
            );
        else this.outputText("Y");
        this.outputText(
            "our quivering pussy, already sopping wet, releases the last of your juices in a soaked "
        );
        ...
        this.outputText(
            ".  The tentacles inside you curl in on themselves and slowly pull back from the well-creamed quim, scraping out as much fluid as possible from your oversensitive honey-pot.  The binding on your [clit] releases, which of course sets off a whole new wave of rippling pleasure-contractions in your cunt."
        );
    ...
    public flowerGrowsToP3(): void {
        this.clearOutput();
        this.outputText(
            "Again, you hear odd noises coming from a corner of your camp - the corner where you let that tainted shrubbery grow."
        );
        if (this.flags[kFLAGS.TIMES_FUCKED_FLOWER] + this.flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0)
        ...
        if (
            ...
            this.outputText(
                ", though you suppose you could burn it down with magical fire if you set your mind to it"
            );
            burnIt = this.torchP3Tree;
        }
    ...
    private fuckTheFlower(): void {
        ...
        if (this.player.hasVagina())
            this.outputText(
                "  Wetness builds in your [vagina] as well, but you're focusing on your maleness just now.  Your neglected twat will have to wait its turn."
            );
        this.outputText(
            "  A feverish sweat breaks out on your brow, though whether from your own arousal, the plant's heat, or some chemical in its juices, you cannot tell."
        );
        ...
        if (this.player.cockArea(x) >= 150)
            this.outputText(
                ", sliding down the trunk as easily if the entire inside was a giant pussy just waiting to get fucked"
            );
        this.outputText(".");
        if (this.player.cockTotal() > 1) {
            ...
            this.outputText(
                " sensually across the moist outer blossom, drooling their clear pre-cum over the petals."
            );
        }
        this.outputText(
            "  Schliiick.  You slide out.  Slorp-pop.  You slide in, this time to the "
        );
        ...
        if (this.player.cockTotal() > 1)
            this.outputText(
                ", as your multiple endowments allow you to drizzle the purple bloom's exterior with alabaster cream"
            );
        this.outputText(".");
        if (this.player.cumQ() > 500)
            this.outputText(
                "  Pearly ropes big enough to fill a flask lurch out from your cum-slit, propelled by your quivering orgasmic muscles deep into the trunk, and you swear you can hear the pitch of your sexual pumping rising in tone, changed by the flooded vessel."
            );
        if (this.player.cumQ() > 1500)
            this.outputText(
                "  A wave of your thick gunk rolls out from under your pumping cock - you've filled the tree beyond even its capacity, and the thick puddle forming beneath your [feet] is all the proof you need of your virility."
            );
        if (this.player.hasVagina()) {
        ...
        this.outputText(
            "  After what feels like forever, the spurting seed no longer flows from your long-abused dick, and as your member softens, the tentacles swing you away, setting you down softly."
        );
        ...
        this.fatigue(5);
        if (this.flags[kFLAGS.FUCK_FLOWER_GROWTH_COUNTER] < 1000)
            this.flags[kFLAGS.FUCK_FLOWER_GROWTH_COUNTER] += 5;
    ...
    private drinkThePlantGirlsSap(): void {
        ...
        this.dynStats("lib", 0.5, "sen", 1, "lus", 15, "cor", 1);
        this.fatigue(-50);
        this.outputText(
        ...
        this.outputText(
            ".  The thick 'milk' quickly fills your body with energy, though it runs out nearly as soon as it started."
        );
        this.outputText(
    ...
    private torchP3Tree(): void {
        ...
        this.outputText(
            "This has gone on long enough!  This thing cannot continue to grow in your camp any longer, and you have just the means to remove it: fire.  "
        );
        // [(nerdfire)
        if (this.player.findStatusAffect(StatusAffects.KnowsWhitefire) >= 0)
            this.outputText(
                "Closing your eyes to focus, you gather your energies, and unleash your white, supernatural flames upon the thing."
            );
        // (bromancefire)
        else if (this.player.findPerk(PerkLib.FireLord) >= 0)
            this.outputText(
                "Sucking in your breath, you inflate your chest and stir the embers of the jaguar demon's gift.  Dredging up an enormous pine-green fireball, you exhale and launch it at the tree."
            );
        else
            this.outputText(
                "Sucking in your breath, you inflate your chest and stir the embers of your fiery gift.  Dredging up an enormous fireball, you exhale and launch it at the tree."
            );
        this.outputText(
            "  It goes up by the torch, though the fire starts to fade sooner than you would expect.  You huff, but repeat the action, layering a new blaze atop the previous one; you put all your concentration into this new conflagration, stoking the inferno with every ounce of your "
        );
        if (this.player.findStatusAffect(StatusAffects.KnowsWhitefire) >= 0)
    ...
    public treePhaseFourGo(): void {
        ...
        if (this.flags[kFLAGS.TIMES_FUCKED_FLOWER] + this.flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0)
            this.outputText(
                "Did you enjoy feeding me?  You should have seen the expression on your face when you were cumming in me.  Your eyes were rolling back and your mouth was just hanging open.  Do you know what it made me want to do?"
            );
        else
            this.outputText(
                "Did you enjoy watching me grow?  You should have fed me.  It was so much harder to grow without proper nutrients.  Do you know what I thought about during all the time I spent waiting?  Do you know what it made me want to do?"
            );
        this.outputText('</i>"');
    ...
    public JojoTransformAndRollOut(): void {
        ...
        if (this.player.cor < 25)
            this.outputText(
                "Not willing to lie to your friend, you also admit that she seems to have changed after your actions in the factory - that, though you were trying to render the tanks permanently inoperable, the spillage you created pushed the cthonic spirit over the edge."
            );
        else
            this.outputText(
                "Though, you do leave out the precise, needless details of what caused the change."
            );
        ...
        if (this.flags[kFLAGS.TIMES_FUCKED_FLOWER] + this.flags[kFLAGS.TIMES_RIDDEN_FLOWER] == 0)
            this.outputText(
                "To think that I must now consider Marae herself my enemy.  Forgive me, but this place is not safe for us anymore if it has attracted the attention of such a creature.  For now, I will return to the forest.  I know that you believe your mission keeps you here, but I urge you to stay away from that thing and watch the portal from an adequate distance."
            );
        // (totally deflowered that flower)
        else
            this.outputText(
                "And even knowing that this could be the case, you still chose to fornicate with it?  [name]... I cannot say that I am not disappointed.  This place is not safe for me anymore.  For now, I will return to the forest."
            );
        this.outputText('</i>"');

        this.outputText(
            'Jojo looks at you sadly; any argument you try to make is met with a resigned shake of his head.  "<i>If the creature truly is the demonic offspring of a corrupted earth spirit, doubtless it will be powerful and the wrath of its parent severe.'
        );
        if (this.player.level < 20)
            this.outputText(
                "  Nonetheless, if you find yourself in a position where you must confront this demon or perish, call upon me.  I will render what aid I can, though I prefer it not be in vain."
            );
        else
            this.outputText(
                "  Though, a mortal of your stature and resources may be able to destroy her and escape the reprisal.  I would counsel against direct confrontation until you are sure of victory, but if you must fight, then call upon me and you will have what aid I can give."
            );
        this.outputText('</i>"  So saying, he presses a small talisman into your hand.');

        this.outputText(
            '"<i>Until then, you will find me in the forest if you need my assistance.</i>"  Jojo makes a polite bow and departs.'
        );
    ...
    public amilyHatesTreeFucking(): void {
        this.clearOutput();
        this.outputText(
            "Your mousey cohabitant tramps up to you, irritation plain on her expression."
        );
        this.outputText(
    ...
    private stayQuietWhileAmilyBitchesAboutFuckingArborday(): void {
        this.clearOutput();
        this.outputText(
            "Amily berates you for some while longer, then eyeballs you.  Just as you begin to speak, she cuts you off again."
        );
        this.outputText(
    ...
    private fuckHolliInZeFlowerPuss(): void {
        ...
        if (this.player.cockTotal() > 1) {
            ...
            this.outputText(
                " to the side, smearing furrows of the stuff around as they slide over the butter-smooth skin."
            );
        }
        ...
        this.outputText(
            " encouragingly.  You respond by edging your [hips] upward, more effectively presenting your shaft to be pleasured."
        );
        ...
        if (this.player.cockArea(x) >= 100)
            this.outputText(
                "the air, where it hangs for a moment before splattering down atop the plant-girl's leaf-colored hair and face, forming a gossamer shroud."
            );
        else
            this.outputText(
                "her mouth, which works noisily to swallow the hot load even as you inject it into her.  After a few swallows, she leans back and lets you shoot it up into the air, so that it can fall down over her face and hair, into a gossamer shroud."
            );
        // highish cum:
        if (this.player.cumQ() >= 500)
            this.outputText(
                "  You keep blasting heavy ropes of seed until her body and trunk are painted with goo, and then you cum some more, smattering spunk until she looks more like a gooey waterfall than a fey creature."
            );
        if (this.player.cumQ() >= 1500)
            this.outputText(
                "  Before you know it, there's a lake around the two of you and a small river running downhill away from camp."
            );
        this.outputText(
            "  A mischievous grin breaks out under the goopy facial, followed by an unnaturally long tongue that pulls gobs of the stuff into the dryad's hungry mouth.  Once she can see again, the girl gives her tits a squeeze and affectionately kisses your cock, practically worshipping it with sloppy-sweet licks."
        );
        ...
        this.outputText(
            ".  [EachCock], instead of slamming into the bark painfully, slots into something warm and wet, pulsating and pliant.  A clingy, tight hold that ripples gently around your member"
        );
        if (this.player.cockTotal() > 1)
        ...
        this.outputText(
            ".  Your grunt of irritation vanishes as soon as your tender sex is forced into the heavenly hole, and with the soft breasts pressing into "
        );
        if (this.player.biggestTitSize() >= 2) this.outputText("your own");
        else this.outputText("your [chest]");
        this.outputText(
            ", you give up and kiss the dryad full on the lips, biting the lower one roughly in irritation."
        );
    ...
    private haveAMapleSyrupSnack(): void {
        ...
        if (this.flags[kFLAGS.HOLLI_SUBMISSIVE] == 0)
            this.outputText(
                'knowingly, Holli asks, "<i>Come for a little pick-me-up?  Well, go on; drink deeply of my nectar, </i>champion."  The last word is inundated with malicious, mocking tones.'
            );
        else
            this.outputText(
                "eagerly, Holli asks, \"<i>Care for a little pick-me-up?  They're getting very engorged, and I'm sure it would be a refreshing snack for you.</i>\""
            );
        this.outputText(
    ...
    private level4RideHollisTentacruels(): void {
        ...
        if (this.silly())
            this.outputText(
                "  She proceeds to sing about Jack Sparrow's awesome adventures for a solid ten minutes, then goes on as if nothing had happened."
            );
        ...
        if (this.player.lactationQ() == 0)
            this.outputText(
                "droplets of milk dripping from your nipples.  When did you start lactating?"
            );
        else
            this.outputText(
                "steady streams of milk erupting from your nipples.  When did you start lactating this hard?"
            );
        ...
        this.player.orgasm();
        this.dynStats("lib", 0.5, "sen", 2, "cor", 1);
    ...
    private eatHolliFruit(): void {
        ...
        if (this.flags[kFLAGS.HOLLI_FRUIT_EXPLAINED] == 0) {
            this.outputText(
                'Holli smiles over at you and asks, "<i>Enjoying the fruits of our union?</i>"'
            );
            this.outputText(
        ...
        this.outputText(
            "Biting into it, sweet juices seem to explode from the flesh, dribbling down your chin.  It tastes like a dessert and you chow down, happily munching away.  In no time flat, you're down to just a core.  You toss it and wipe your [face] clean, then burp.  Damn, that was good!"
        );
        // TF CHANCES
    ...
    private askHolliToWatch(): void {
        ...
        if (this.flags[kFLAGS.HOLLI_DEFENSE_ON] == 0) {
            this.outputText(
                'You ask Holli if she would mind using her tentacles to help guard the camp at night.  She rolls her eyes and taunts, "<i>I don\'t know; maybe if you get down on your knees and beg me.  ...And I mean </i>really<i> beg, with plenty of whimpering.  Lick my roots too.  After all, I should get something out of this.</i>"'
            );
            this.outputText(
        ...
        // Stop Guarding (edited)(C)
            ...
            this.outputText(
                "You tell Holli you'd like her to stop watching at night.  She laughs, \"<i>You just want a chance to come back and beg some more, don't you?  I thought you were supposed to be tough shit, not a simpering little submissive.</i>\""
            );
    ...
    private begHolli4Watches(): void {
        this.clearOutput();
        this.outputText(
            'You swallow your pride and drop down prostrate on the ground.  Sighing, you grudgingly say, "<i>Please, Holli, guard my camp at night.  I don\'t want to be raped by monsters.</i>"'
        );
        this.outputText(
    ...
    private domUpSomeHolli(): void {
        ...
        this.flags[kFLAGS.HOLLI_FUCKED_TODAY] = 1;
        this.outputText(
            'You swagger up to the tree and put your hand on the bark beside Holli.  Glancing over, she quips, "<i>Is that supposed to be intimidating?</i>"  The dryad smirks ingratiatingly.  "<i>I\'m the offshoot of a demon goddess.  I don\'t think you thought this through.</i>"'
        );
        this.outputText(
    ...
    private askBrokenHolliToGuard(): void {
        this.clearOutput();
        this.outputText(
            "Holli bows her head when you mention her guarding camp.  She asks, \"<i>Shall I keep watch for foes to drain or let them into camp for you to have the pleasure of dealing with, [name]?  <b>I've been "
        );
        ...
        this.menu();
        if (this.flags[kFLAGS.HOLLI_DEFENSE_ON] == 1)
    ...
    private toggleBrokenHolliGuard(): void {
        ...
        if (this.flags[kFLAGS.HOLLI_DEFENSE_ON] == 0) {
            this.outputText(
                '"<i>All right, I\'ll get to have some fun then,</i>" Holli quips while rubbing her palms together in anticipation.'
            );
            this.outputText(
        ...
        // Guard Off
            ...
            this.outputText(
                '"<i>All right, they\'re yours to smite or play with,</i>" Holli says with a wink.  You give her a glare, to which she responds with a shrug.  "<i>What?  I know how it is!  You mortals can\'t fight worth a damn when you\'re sleeping.</i>"'
            );
            this.outputText(
    ...
    private holliGetsDickDommed(): void {
        ...
        this.outputText(
            ".  Holli pulls back, inciting a snarl of displeasure to vibrate from you, but as soon as she's withdrawn, her hands clasp onto the slick shaft and begin to pump it with vigorous strokes.  Her mouth settles back into place around your [cockHead biggest], circling it with her tongue while she bobs up and down on it, snaring your most sensitive place with overwhelming sensations."
        );
        this.outputText(
        ...
        if (this.player.biggestCockArea() > 80)
            this.outputText(
                "  It bends slightly as the immense girth is forced inside her, aiming down, deep inside the trunk."
            );
        this.outputText(
        ...
        if (this.player.cockTotal() > 1) {
            this.outputText(
                "  Excess jism spurts onto Holli's jiggling udders from below, spotting the viridian mounds with white."
            );
            if (this.player.cumQ() >= 500)
                this.outputText(
                    "  The constant sprays turn them white in little time, dripping from the peaks of her nipples along with her sticky sap."
                );
        }
        if (this.player.cumQ() > 1000)
            this.outputText(
                "  A trickle of sperm washes out of the demoness's entrance, somehow escaping the heavy suction."
            );
    ...
    private fullOnTentacleTasticGangBangForHolli(): void {
        ...
        this.flags[kFLAGS.HOLLI_FUCKED_TODAY] = 1;
        this.outputText(
            'You remove your [armor] and expose your writhing mass of wriggling tendrils to the tree-tart.  She places a hand over her mouth and exhales, "<i>Oh my, Mother Marae sure knows how to pick them, doesn\'t she?</i>"'
        );
        this.outputText(
        ...
        if (this.player.balls > 0)
            this.outputText(
                "  Your [balls] draw up tight to your groin and squeeze, rhythmically discharging their heavy loads."
            );
        this.outputText(
        ...
        this.player.orgasm();
        this.dynStats("lib", 0.4, "sen", -3, "cor", 1);
        this.flags[kFLAGS.TIMES_FUCKED_FLOWER]++;
        this.flags[kFLAGS.HOLLI_FUCKED_TODAY] = 1;
        this.fertilizeHolli();
        this.fertilizeHolli();
    ...
    private vaginalDomHollisTentacruels(): void {
        ...
        if (!this.player.hasFuckableNipples() && this.player.lactationQ() < 50)
            this.outputText(
                " and pay special attention to your [nipples], tugging them in time with every sluggish thrust in your slit."
            );
            ...
            this.outputText(
                " of milk that escape with every tug, timed to match the sluggish thrusts at your slit."
            );
        } else
            this.outputText(
                " and sink your fingers into your [nipples], fucking them to the sluggish tempo of the thrusting tentacle."
            );
        this.outputText(
            "  A loud groan escapes your mouth, and you rock your [hips] against the undulating tendril.  Your body's eagerness for more pleasure is unparalleled, but you control yourself as efficiently as you control the dryad."
        );
        this.outputText(
    ...
    private threatenHolli(): void {
        this.clearOutput();
        this.outputText(
            "You snarl and threaten to burn the barked bitch to the ground if she won't make herself useful after you ask politely."
        );
        this.outputText(
        ...
        if (this.player.level < 30)
            this.outputText(
                "Mighty as you are, Marae is likely still beyond your power.  Perhaps you should just ignore this demon for the time being."
            );
        else
            this.outputText(
                "Marae is likely beyond your power yet, but... you feel like she might not pressure you too hard if you can make a suitably credible threat."
            );
        this.outputText(
    ...
    public defeatHolli(): void {
        ...
        if (this.monster.findStatusAffect(StatusAffects.JojoIsAssisting) < 0) {
            this.outputText(
                "Weary, the succubus godling folds into her tree, unwilling to allow you the sight of her defeat.  With her energy suppressed, you're free to "
            );
            if (this.player.weaponName != "large axe") this.outputText("fetch an axe and ");
            this.outputText(
                "hack at her trunk, laboriously peeling bark and making wedge-shaped cuts in it as you try to fell the abomination.  It takes nearly eight hours, but eventually the gnarled tree topples.  No sooner does it fall than it shrivels and turns to ash."
            );
            this.temp = 7;
            while (this.temp > 0) {
                this.temp--;
        ...
        // Win with Jojo -Z
            ...
            this.outputText(
                'The fire-blackened mouse cheers at you as the succubus groans and retreats completely into her bark.  "<i>Well done, [name]!  Help me carry tinder and we can burn this abomination to the ground!</i>"'
            );
            this.outputText(
    ...
    private recruitJojoToCamp(): void {
        this.clearOutput();
        this.outputText(
            'Jojo nods eagerly at you when you invite him to share your camp.  "<i>Excellent idea.  If you\'ll excuse me, I wish to attempt to purify the spot where the creature grew, and then I will fetch my things.</i>"'
        );
    ...
    public enjoyYourBadEndBIYAAAATCH(): void {
        ...
        if (this.player.gender == 1 || (HolliScene.rand(2) == 0 && this.player.hasCock())) {
            ...
            this.outputText(
                " engulfs you.  Holli's tongue plays over your slit and she looks up at you for just a moment to be sure you're watching."
            );
            ...
            if (this.player.biggestCockLength() > 24)
                ...
                this.outputText(
                    "she sinks her head down to the base of your prick in one quick thrust, bringing the stem with it.  "
                );
            this.outputText(
            ...
            this.outputText(
                ".  Your eyes roll as you flex and flex, but nothing comes out.  As you conclude your dry orgasm, you begin to feel a pressure on your prostate."
            );
            ...
            if (this.player.balls > 0)
                this.outputText(
                    "migrating the other direction into your [balls], entangling and infiltrating them, filling them with the same warmth, then pressing out against your sack.  "
                );
            else
                this.outputText(
        ...
        // branch of vagina-having
            ...
            this.outputText(
                ", the demon buries her face in your [vagina], penetrating it with her wet, flexible tongue."
            );
            ...
            if (this.player.hasCock())
                this.outputText(
                    "  [OneCock] drools precum down its length and onto the flower, triggering another spasm and another gush from your prick.  Holli laughs as you continue to self-stimulate with your uncontrollable fluids."
                );
            ...
            if (this.player.ass.analWetness > 0)
                this.outputText(
                    'Betraying your depravity, the slick crack opens wide and your greedy, wet asshole practically welcomes the demon\'s entire hand, sucking it up and making you shiver with repressed delight.  "<i>Oh, mother!  It seems as though this is not a new lesson to you, is it?  My little whore, will you beg and moan as I fill you?</i>"'
                );
            else if (this.player.analCapacity() < 30)
                this.outputText(
                    'With some difficulty, she forces it into your pucker.  "<i>I think we\'ll just use this as our inroads, instead.  It may be slow going, but the journey is half the fun.</i>"'
                );
            else
                this.outputText(
                    "Despite your clenching, your loose hole proves no impediment to the demon's exploration.  \"<i>Ah, how roomy you are!  I think I'll move right in.</i>\""
                );
            ...
            if (this.player.ass.analWetness > 0)
                this.outputText(
                    "Try as you might, you can't see far enough down your back to determine what the demon has done.  The question answers itself, though, when she presses a shaft almost as thick as her leg into your [butt], nosing it into your wet, receptive asshole.  Despite yourself, you shiver in anticipation at being filled so completely.  "
                );
            else if (this.player.analCapacity() < 30)
                this.outputText(
                    "Your jaw hits your collarbone as Holli angles the florid head of a long, narrow penis into your field of view.  Judging by how far it comes up her chest, it's almost a foot and half long, and it's drooling sticky sap all over your back.  With a twisted smile, the demon pushes it along your spine until it reaches your [butt], then shoves it toward the hole.  "
                );
            else
                this.outputText(
                    "The demon presses up to your back, brushing her nipples against your skin and shoving her new growth between your thighs.  Looking down, you can see a fat, green shaft sticking from your crotch, glistening and dripping with sticky slime.  With another moan, Holli pulls it back from between your legs, then pushes apart your asscheeks and plunges it into your [asshole].  "
                );
            this.outputText("The sap oozing from her member coats your passage, ");
            if (this.player.ass.analWetness > 0)
                this.outputText(
                    "mingling with your own lubrication and ]easing her thrusts.  Holli's tempo picks up quickly, and soon you can feel her twitch inside you as she nears her climax."
                );
            ...
            if (this.player.ass.analWetness > 0 || this.player.analCapacity() > 30)
                ...
                this.outputText(
                    "and your once-tight pucker reflexively squeezes shut as best it can, trapping all but a few trickles of Holli's deposit inside you.  "
                );
            this.outputText(
                "The freshly-grown shaft between Holli's legs begins to shrivel, withering and browning like a cut flower.  With an expression of mild disdain, she pulls it from her body and casts it aside, where it crumbles to pieces."
            );
            ...
            this.outputText(
                " and your [legs] concealed below your buttcheeks by a thick tree trunk."
            );
    ...
    private girlsGetANiceManToBadEnd(): void {
        ...
        if (this.player.biggestTitSize() >= 1) {
            this.outputText(
                "reaches up and grabs two handfuls of breast, passing his thumb over a nipple and watching the "
            );
            ...
            this.outputText(
                "leans down and kisses the [woman/hermaphrodite]'s smooth chest, causing her to arch her back in pleasure.  "
            );
        if (this.player.hasCock())
            this.outputText(
                "With a nervous glance downward, he slides one hand to her [cock smallest].  \"<i>It's not gay if it's a chick's,</i>\" he mumbles to himself, and then begins to stroke haltingly, playing his finger along the urethra and drawing a squirt of precum from his shivering partner.  "
            );
        else {
            this.outputText(
                "His hand slides to the [clit] above her wet fuck-hole; collecting some of the frothing lube from their wet sex, "
            );
            if (this.player.clitLength < 6)
                this.outputText(
                    "he smears it over the little button and then begins to trace up and down the underside with a finger.  "
                );
            else
                this.outputText(
                    "he coats the swollen organ with it and begins to jerk it off valiantly, like a cock.  "
                );
            this.outputText(
                '"<i>Does that feel good?</i>" he asks, upping the pace of his thrusts.  '
            );
        }
        ...
        if (this.player.hasCock()) {
            ...
            if (this.player.biggestCockArea() < 20) {
                this.outputText(
                    "semen spurts and dribbles down her [chest], pooling in the combined laps and mingling with the sticky lube ringing her vagina.  "
                );
                if (this.player.cumQ() >= 500)
                    this.outputText(
                        "So much so that by the time she finishes her climax, both her partner's legs and her trunk are covered in a sheen of milky white seed.  "
                    );
            } else if (this.player.biggestCockArea() < 80) {
                this.outputText(
                    "her mouth is now full of cockhead, and she gulps as eagerly as she spurts, drooling lines of white from the corners of her lips.  "
                );
                // [(big skeet)]
                if (this.player.cumQ() >= 500)
                    this.outputText(
                        "The waves of seed don't stop coming, pouring into her and swelling her belly out until its round bulge pushes the cock away from her mouth and the remaining squirts fall on her [chest].  "
                    );
            }
            // else cock biggest > 80)
            else {
                this.outputText(
                    "her body is easily dwarfed by her male sex's massive size, and globs of sperm shoot into her canopy to splatter among her leaves and branches.  "
                );
                if (this.player.cumQ() >= 1000)
                    this.outputText(
                        "The overproductive hermaphrodite's body pushes out so much seed that the entire canopy soon turns white and drips constantly as though it were in a tropical rainforest.  "
                    );
            }
        }
        // (vagina and no cock)
            ...
            this.outputText(
                " of female orgasm pours from her pussy, and her mouth opens wide in pleasure.  The tree branches surrounding the young man seem to close in, pushing him toward her.  Getting the message, he leans in and kisses his lover; her tongue probes his mouth with abandon as her body shakes out its fit and her [vagina] wrings his prick.  "
            );
        }
        this.outputText(
            "Pushed past his limit by her lewd display, the young man groans, shudders, and begins to unload his pent-up cargo.  Falling on top of his partner's slick "
        );
        ...
        this.outputText(
            '\'s stomach.  "<i>In fact, she seems to have enjoyed it so much that she would have me bestow a blessing on you.</i>"'
        );
