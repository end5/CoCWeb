...
export class HelFollower extends NPCAwareContent {
    ...
    public helAffection(diff = 0): number {
        ...
        if (
            ...
            (this.flags[kFLAGS.HEL_BONUS_POINTS] == 0 && diff < 0)
        ) {
            this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] += diff;
            if (this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] >= 100)
                this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] = 100;
            if (this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] < 0)
                this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] = 0;
        } else if (this.followerHel()) {
            this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] = 100;
            this.flags[kFLAGS.HEL_BONUS_POINTS] += diff * 3;
            if (diff > 0)
                if (this.flags[kFLAGS.HEL_BONUS_POINTS] > 150)
                    this.flags[kFLAGS.HEL_BONUS_POINTS] = 150;
                else if (diff < 0)
                    if (this.flags[kFLAGS.HEL_BONUS_POINTS] < 0)
    ...
    public heliaFollowerIntro(): void {
        ...
        this.outputText(
            " the Harpy Queen... and getting my sister out of that shithole.  For giving me a chance to meet my dad.  For everything.</i>\"  You start to tell her that you're happy to help, but Hel cuts you off, speaking quickly: <i>\"Kiri, Dad, and I have been trying to make it work, living out on the plains - we really have - but Dad's not in any shape to fight and Kiri's no good at it; the plains are just too dangerous for them and Dad's been having nightmares about mom, and...\"</i> she pauses to take a gasping breath, unaccustomed to talking so quickly."
        );
    ...
    private iCantLetFireButtsRapeMyCampsButt(): void {
        ...
        if (
            this.flags[kFLAGS.HEL_FOXY_FOURSOME_WARNED] > 0 ||
            this.flags[kFLAGS.HEL_EDRYN_OFFER] > 0
        )
            this.outputText("like usual");
        this.outputText(
            ', and maybe you and I can spend some quality time together.  Alright?"</i>'
        );
    ...
    private justFriendsWithAnalTailWaifu(): void {
        this.clearOutput();
        this.outputText(
            "Awkwardly, you spend the next few minutes saying that, while you appreciate the offer and her affections, you're more interested in just being friends with Hel, as you have been for some time."
        );
        ...
        this.outputText(
            ' to give you a little peck on the cheek.  <i>"Well, that\'s that, then.  I guess.  Well, see you around, [name]."</i>'
        );

        this.outputText('\n\n<i>"Sure will."</i>');

        this.outputText("\n\nYou wave as Hel retreats back toward her own home.");
        this.flags[kFLAGS.HELIA_FOLLOWER_DISABLED] = 0.5;
    ...
    public helFollowersIntro(): void {
        ...
        if (this.followerKiha() && this.flags[kFLAGS.HEL_INTROS_LEVEL] < 1) {
            ...
            this.outputText(
                "You pace around camp, awaiting the return of your new companion.  After several minutes, you notice Kiha sitting behind you, eyeing you with an eyebrow cocked.  Suddenly self-conscious, you stop and face the dragon-girl."
            );
            this.outputText(
        ...
        // If Isabella
            ...
            if (this.flags[kFLAGS.HEL_ISABELLA_THREESOME_ENABLED] > 0) {
                this.outputText(
                    "Showing Hel around, you eventually come to the part of your camp inhabited by the towering cowgirl Isabella.  When you arrive, you find Isabella reclining in her armchair, humming a sweet melody as she cleans her tower shield."
                );
            ...
            // If Isabella is at Camp (And she and Hel are NOT cool)
                ...
                if (this.isabellaAccent())
                    this.outputText(
                        '<i>"You are... velcome,"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.'
                    );
                else
                    this.outputText(
                        '<i>"You\'re... welcome,"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.'
                    );
        ...
        // If Bimbo Sophie is at Camp:
            ...
            this.flags[kFLAGS.HEL_INTROS_LEVEL] = 4;
            this.outputText(
                "As you help Hel string up her hammock between a few of the rocks inside your perimeter, you hear the tell-tale flapping of useless wings and a clattering of claws on the hard-packed dirt.  You brace for impact as your bimbo harpy prances up to you, planting a big, wet kiss on your cheek"
            );
    ...
    private bimboSophieGetsBooted4Firebutt(): void {
        this.clearOutput();
        this.outputText(
            'You sigh and pick Sophie up, slinging the harpy over your shoulders.  <i>"Yaaaay~"</i> she cheers.  </i>"I\'m going for a ride!"</i>'
        );
    ...
    private workItOutWithSophieAndFireTits(): void {
        this.clearOutput();
        this.outputText(
            "You cross your arms and tell Hel to deal with it, remarking that you won't just dump Sophie out in the wilderness - she can't take care of herself - but you still want Hel around."
        );
    ...
    public angryHelAndIzzyCampHelHereFirst(): void {
        this.clearOutput();
        this.outputText(
            "Showing Isabella around, you eventually come to the chaotic, cluttered part of camp inhabited by Hel the salamander, who's currently sitting on her hammock sharpening her scimitar."
        );
        this.outputText(
            "\n\nYou only have a moment to remember Hel's disdain for the cowgirl before..."
        );
    ...
    public heliaFollowerMenu(display = true): void {
        ...
        if (this.flags[kFLAGS.HEL_FOLLOWER_LEVEL] == 2) {
            if (
                this.flags[kFLAGS.HELIA_ANAL_TRAINING_OFFERED] == 0 &&
                display &&
                this.player.biggestCockArea() > this.heliaAnalCapacity()
            ) {
            ...
            if (display)
                this.outputText(
                    'You call your salamander lover over, and in a few moments Hel walks your way, hips and tail swaying gaily as she moves.  She wraps an arm around your shoulders, pressing her soft, warm body against yours, and grins.  <i>"Heyya, lover mine.  You need anything?"</i>'
                );
            this.menu();
            ...
            if (this.flags[kFLAGS.HEL_LOVE] == 1 || this.flags[kFLAGS.HEL_LOVE] == -1) {
                if (
                    this.player.hasCock() &&
                    this.player.cockThatFits(this.heliaCapacity()) >= 0 &&
                    this.player.lust >= 33 &&
                    !this.helPregnant() &&
                    this.flags[kFLAGS.HELSPAWN_AGE] == 0
                )
            ...
            if (this.flags[kFLAGS.HEL_HARPY_QUEEN_DEFEATED] == 1) {
                if (display)
                    this.outputText(
                        "(You've reached the culmination of Helia's current storyline.  Stay tuned for more!)"
                    );
            } else {
                if (display)
                    this.outputText(
                        "You approach Hel as she's pacing around camp.  She's clad in her normal field attire: a simple scale bikini top and leather thong which supports her scimitar's scabbard.  Her cloak is loosely thrown over her shoulders, giving her a slight measure of protection from the mountain's harsh environs."
                    );
    ...
    private heliaOptions(): void {
        ...
        if (
            ...
            this.player.biggestCockArea() > this.heliaAnalCapacity() &&
            this.player.hasItem(this.consumables.GOB_ALE, 1)
        )
            this.addButton(3, "Anal Train", this.heliaGapeSceneChoices);
        this.addButton(5, "Bathe", this.takeABath);
        if (this.flags[kFLAGS.HELSPAWN_AGE] == 1)
            this.addButton(7, this.flags[kFLAGS.HELSPAWN_NAME], this.helSpawnScene.playWithYourKid);
    ...
    private sparWithHeliaFirebuttsAreHot(): void {
        this.clearOutput();
        this.outputText(
            "Giving Hel a playful punch on the shoulder, you ask the salamander-girl if she'd be up for a little battle practice."
        );
        this.outputText(
    ...
    public loseToSparringHeliaLikeAButtRapedChump(): void {
        ...
        if (this.player.HP < 1)
            this.outputText(
                "You collapse on the ground, overwhelmed by pain and exhaustion caused by the berserker's onslaught."
            );
        else
            this.outputText(
                "Your arousal is too great, and your mind can no longer focus on anything but a desperate need for release.  Lust robs you of your will, and with buckling knees you collapse."
            );
    ...
    public PCBeatsUpSalamanderSparring(): void {
        ...
        if (this.monster.HP < 1)
            ...
            this.outputText(
                "Panting heavily, knees snaking, she collapses, heavily leaning upon her sword."
            );
        ...
        this.outputText(
            "  You chuckle and offer her a hand up. The blushing Salamander takes it as you pull her into a tight embrace.  The cool wetness between her thighs a potent reminder of how much your lover seems to get off on violence.  Since she's so turned on, you could easily turn this into some rough loving."
        );
    ...
    private talkToHel(): void {
        ...
        if (this.flags[kFLAGS.FOLLOWER_HEL_TALKS] == 0) {
            ...
            if (this.player.tou < 50)
                this.outputText(
                    "You cough and gag, eyes misting as the pure-grain booze burns your throat worse than hot coals.  Hel laughs riotously, slapping you on the back hard enough to cause booze to snort out through your nose."
                );
            else
                this.outputText(
                    "You knock back the stein easily, your super-human endurance keeping you from choking on the powerful brew.  Impressed, Hel gives you a high-five and throws her head back in a laugh."
                );
            this.outputText(
        ...
        // Hel Talk 2
            ...
            this.outputText(
                "Sitting Hel down, you ask if she's got any stories she'd like to share.  After all, she's been adventuring for years - surely she's got some fun tales to tell."
            );
        ...
        // Hel Talk 5(C)
            ...
            this.outputText(
                '<i>"Hey, [name],"</i> Hel says, slipping out of your grasp with a come-hither wag of her finger, <i>"wanna have a drink with me?"</i>'
            );
            ...
            this.outputText(
                'and, swinging onto your lap, clinks her glass against yours.  <i>"To loving friends and an awesome new home,"</i> she says, raising her tankard high before knocking it back.  Before you can finish yours, Hel belches loudly and violently shakes her head'
            );
            if (!this.helSpawnScene.helPregnant())
                this.outputText(
                    ", her eyes crossing a little.  Looks like she was dipping in before you got here..."
                );
            else this.outputText(".");
            ...
            if (!this.helSpawnScene.helPregnant()) {
                ...
                this.outputText(
                    " and launches into a series of debauched jokes. You answer her, lewd jest for jest."
                );
            }
            ...
            this.outputText(
                "into a hug, mashing your face into her warm, soft bosom.  You struggle for air, finally bursting out of her tight cleavage.  Hel chuckles, running her hand through your [hair] as her fiery tail wags gaily behind her."
            );
        ...
        // Hel Talk 6 (Needs Isabella and Kiha at camp; at least 1 gem)(C)
            ...
            this.flags[kFLAGS.FOLLOWER_HEL_TALKS] == 5 &&
            this.player.gems >= 1 &&
    ...
    private shutUpHelTalks(): void {
        this.clearOutput();
        this.outputText(
            'Quickly you interject before Helia can continue with the story; you\'ve heard quite enough.  <i>"Whaaaat?  We were just getting to the good part..."</i>  Hel whines, rolling her eyes.  <i>"Hey, you\'re the one who asked!"</i>'
        );
        this.outputText(
    ...
    private hugASmokeyTail(): void {
        ...
        if (this.model.time.hours >= 21) {
            ...
            this.outputText(
                ' intertwined.  <i>"Oh, this is nice,"</i> Hel laughs, running her long, scaled fingers through your hair.  <i>"So, you wanna stay with me tonight, lover mine?  I\'d appreciate the company...."</i>'
            );
    ...
    private heliaRoughSex(output = true): void {
        ...
        if (this.player.hasCock() && this.player.lust >= 33) {
            ...
            if (this.player.cockThatFits(this.heliaCapacity()) >= 0 && buttons < 9) {
                this.addButton(
                    buttons,
                    "FuckVag",
                    this.helScene.beatUpHelAndStealHerWalletFromHerVagina
                );
                buttons++;
            }
            // 85 ass capacity
            ...
            if (
                this.player.cockThatFits(this.heliaCapacity()) >= 0 &&
                this.player.cockThatFits2(this.heliaCapacity()) >= 0 &&
                buttons < 9
            ) {
            ...
            if (buttons < 9) {
                this.addButton(
                    buttons,
                    "Tail Wank",
                    this.helScene.helTailWanksYourDickBecauseSheLovesYouDesuDesuHoraHora
                );
                buttons++;
            }
        }
        ...
        if (
            this.player.lust >= 33 &&
            this.player.findPerk(PerkLib.Incorporeality) >= 0 &&
            this.izmaFollower() &&
        ...
        if (this.getGame().inCombat) this.addButton(9, "Leave", this.cleanupAfterCombat);
    ...
    private heliaCampPossession(): void {
        this.clearOutput();
        this.outputText(
            "With a coy grin, you let your ghostly attributes go transparent, and ask Hel if she'd mind taking an extra passenger for a little bit."
        );
    ...
    private inCampHelNagaLuv(): void {
        ...
        this.outputText(
            " well into her womb.  Your tail spasms as you cum, running wild inside Hel's rectum until she cums too, yelping into your open mouth as she climaxes a moment after you, girlcum squirting out of her twat until your [hips] are thoroughly soaked."
        );
    ...
    private centaurMountsCampHel(): void {
        ...
        if (this.player.biggestTitSize() < 3)
            this.outputText(
                " but your body is too big, and you must content yourself to endure Hel's ministrations"
            );
        else
            this.outputText(
                " but all you can do is take hold of your [chest] and tease and tweak your [nipples], getting what pleasure you can from your sensitive teats"
            );
        this.outputText(".");
        ...
        this.outputText(
            ".  Finally, though, you manage to settle down, your breath coming ragged as Hel's vaginal muscles clamp and release, gently massaging your shaft with expert movements, always keeping you on edge until..."
        );
    ...
    private heliaFollowerTentafuck(): void {
        this.clearOutput();
        this.outputText(
            "With a flourish you pull your [armor] off, exposing your writhing mass of tentacle-cocks for Hel`s enjoyment.  \"<i>Wow, think you got enough of th--urf!</i>\" the salamander starts to say before you shut her up with one of your tentacle pricks, lashing forward and plugging her mouth.  Hel staggers back, flailing around before grabbing the cock you've shoved down her throat and giving it a rough tug.  Just as she nearly succeeds in pulling it out, your other wriggling members wrap around her thick thighs; you pull her in toward you, grabbing her shoulders as your pricks peel off her bikini bottom to reveal the smooth face of Helia's crotch."
        );
        ...
        this.outputText(
            " feel painfully empty and white slush pours from all her holes, painting the ground white beneath her dangling feet."
        );
    ...
    private muddyLizardFeet(): void {
        ...
        if (this.player.balls > 0) this.outputText("her heel coming to caress your taut sack");
            ...
            this.outputText(
                "her heel going so low as to brush your taint, making your [asshole] clench in surprise before she moves back up your length"
            );
        this.outputText(
            ".  You groan lustily as her toes and soft sole jerk you off, gently urging the tight knot of pleasure in your gut to fruition.  Cupping one of her hefty tits, Hel licks her lips as the first thick smears of pre leak from your cock's [cockHead biggest], smearing onto her long-clawed toes."
        );
        ...
        if (this.player.biggestCockArea() > this.heliaCapacity())
            this.outputText(
                "you feel your [cockHead biggest] against the lips of her cervix, as fully into her as you can go"
            );
        else this.outputText("your groin meets Helia's, your [cock] wholly enveloped within her");
        this.outputText(
            '.  Before you can think to withdraw, Hel tightens her hold on you, whispering huskily, "<i>Just leave it... just for a second, [name].  Oh, God,</i>" she moans, her eyes closing as she savors the feeling of you buried so deep inside her, your slick prick twitching as her walls massage it, milking it of the precum that freely flows from your tip.'
        );
    ...
    private takeABath(): void {
        ...
        if (this.flags[kFLAGS.HEL_CAN_SWIM] == 0) {
            this.clearOutput();
            this.outputText(
                "Noticing Hel smells particularly strongly of beer and sex - even for her - you ask your salamander lover if she'd like to join you for a dip in the stream.  Rather than with her usual energy, though, Hel's reply is to look quickly away and say, \"<i>Uh, not today, [name]. I'm, uh, not feeling in the swimming mood.</i>\""
            );
        ...
        // {Repeat Intro (If you taught Hel to swin; else, play 1st Time)}
            ...
            this.clearOutput();
            this.outputText(
                "You ruffle Hel's hair and ask if the fiery salamander's up for a dip."
            );
            this.outputText(
    ...
    public heliaThreesomes(): void {
        this.clearOutput();
        this.outputText(
            "You slip an arm around Hel's waist and ask if she's up for some... group activities?"
        );
        this.outputText(
        ...
        if (
            (this.player.armorName == "goo armor" || this.flags[kFLAGS.VALARIA_AT_CAMP] == 1) &&
            this.player.lust >= 33 &&
            this.player.hasVagina()
        ) {
    ...
    private heliaAndVapula(): void {
        ...
        if (this.player.cumQ() >= 750)
            this.outputText(
                " until her belly swells and starts spurting ropes of jism back out of her love-tunnel"
            );
        this.outputText(
            ".  Your junk palpitates and bulges as globs push their way out of your shaft before being absorbed by her needy pussy, and her skin and scales seem to literally boil with consumed lust.  At the peak of your orgasm, you stare at your girls' faces: Hel's is nothing but complete, insensate bliss, while Vapula is literally crying in ecstasy, tears of joy streaming down her cheeks."
        );
        ...
        if (!this.followerHel()) {
            this.outputText(
                "  You kiss her and ask if she'll be able to find her way back to her home."
            );
    ...
    private girlsThreesomeHelAndKiha(): void {
        ...
        if (this.player.lactationQ() >= 50)
            this.outputText(
                "  A shudder rocks you as the first trickles of milk seep from your teats, a small stream soon flowing out of you into the twin lizards' mouths.  Their cheeks bulge between swallows, but both coo happily as they suckle, their forked tongues lapping up every drop you have to offer."
            );
        // {If PC has a dick:}
        if (this.player.hasCock())
            this.outputText(
                "  Even as the girls worship your [chest], you feel a sudden pressure on [oneCock].  Hel's tail deviates from your cunt for a moment, instead wrapping lovingly around your prick.  A groan escapes your lips as her hot leathery tail coils around your manhood before looping back to tease your clitty, ensuring that your dick won't be left out of the fun.  You smile and stroke her hair, eager to get to the main course."
            );
    ...
    private dudeHeliaAndKihaThreeSome(): void {
        ...
        if (this.player.cockTotal() == 1)
            ...
            this.outputText(
                "your second erection, taking it in both hands and giving it long, loving strokes before slurping up the [cockHead 1]"
            );
        this.outputText(".");
    ...
    private helXValeriaFemalePC(): void {
        this.clearOutput();
        this.outputText(
            "In the blink of an eye, Valeria slithers around Hel's body, almost completely encasing her in thick blue goo, leaving only her head and her resplendent reptilian rack bare.  Hel grins at you, putting a hand to her crotch and taking a handful of goo between her fingers.  With a bit of effort, she shapes a pair of thick blue pricks out of her groin, each shimmering in the sunlight as Hel strokes Valeria's new appendages, coaxing out a drop of slimey pre from both tips.  Valeria's face reappears on Hel's flat belly a moment after she's done, looking down to her groin with glee as she further shapes the erect pillars of slime, morphing them into a thick knotty doggy-dick on top, and a huge-flared horsecock on bottom.  You lick your lips as the dog-dick's knot inflates enticingly."
        );
    ...
    private helGuardToggle(): void {
        ...
        if (this.flags[kFLAGS.HEL_GUARDING] == 0) {
            this.outputText(
                "You ask Hel if she wouldn't mind watching the perimeter for a few hours during the night."
            );
            this.outputText(
    ...
    private heliaAnalTrainingPrompt(): void {
        ...
        this.outputText(
            "“<i>Hey there, lover mine. Got a minute?</i>” Helia asks with dangerous glint in her eyes."
        );
        this.outputText(
    ...
    private giveHeliaAnalTraining(): void {
        this.clearOutput();
        this.outputText(
            "You hand Helia a bottle of goblin ale and question if she’s sure this won’t make her loose."
        );
        this.outputText(
        ...
        if (this.player.balls > 0)
            this.outputText(
                "She tilts forward to press her chin into your [sack] and begins to lather your [balls] with frothy, warm, salamander spit. One hand hefts a weighty nut as she works, caressing it as if it would somehow coax the cum out of it. "
            );
        this.outputText(
            "Helia slides her palms along either side of your urethra, letting her fingers handle the central, sensitive bit on the underside, following in the path of slick saliva she left for them to follow. She rubs her nose into the sensitive area just below your [cockHead biggest]. You grunt as pre-cum begins to pour in response, your bloated boner flexing atop Helia’s head as you feel a dangerously powerful orgasm welling up within you."
        );
        ...
        this.outputText(
            ", and you erupt, cumming a steamy hot load of spunk straight onto Helia’s face, head, and back."
        );
        if (this.player.cumQ() < 250)
            this.outputText(
                " You run out before long, but it’s still enough to make her a white painted mess. She licks the excess from her upper lip and shudders happily."
            );
        else if (this.player.cumQ() < 2000)
            this.outputText(
                " You spurt rope after rope onto her, actually drenching her hair with alabaster moisture. Her back and face look equally smothered with spooge when you finish. She’s happy to wipe the goop out of her eyes and shovel it into her mouth, swallowing it down with a satisfied smile."
            );
        else {
            this.outputText(
                " You pour ivory waves of spooge across her body, soaking her from head to toe with the fruit of your loins, liberally basting her with baby-batter. She groans as the excess washes out around her knees"
            );
            if (this.player.cumQ() >= 3000)
                this.outputText(
                    ", but you keep cumming, cascading fresh waves onto her until there’s not a single part of her that isn’t stained and dripping with your salty outflow"
                );
            this.outputText(
                ". Helia licks her hands clean, wipes her eyes and devours that as well, and then gives you a dripping-wet, sensuous smile."
            );
        }
        this.outputText(
    ...
    private heliaAnalTrainingPartTwo(): void {
        ...
        this.clearOutput();
        this.outputText(
            "Pulling out another bottle of ale you dangle it before the slutty salamander, saying, “<i>A present for my favorite buttslut.</i>”"
        );
        this.outputText(
    ...
    private noGapeSlutForHelia(): void {
        this.clearOutput();
        this.outputText(
            "You step up and kiss Helia, telling her that she doesn’t have to this for you, at least not right now."
        );
        this.outputText(
    ...
    private yesMakeHeliaAGapeSlut(): void {
        ...
        if (this.player.cumQ() < 1000)
            this.outputText(
                ", ignoring the way your sheer volume of spunk leaks from the corners of her mouth and nose, her belly gradually rounding with cum in addition to thick dildo"
            );
        this.outputText(". The slutty salamander eventually pulls it off and ");
        if (this.player.cumQ() < 2000)
            this.outputText(
                "squeezes a last few dollops onto her face, letting them stain her with your scent."
            );
        else
            this.outputText(
                "allows you to hose her down with your ongoing waves of salty jism, soaking her in your sexual scent."
            );
        this.outputText(
            " Helia burps. “<i>Fuck me that was awesome! Whoah, don’t mind me, but I, uh... I think I’m gonna stay here for a little while, maybe just rock up and down till I come down off this buzz. She blushes and begins to finger her snatch again. Just let me... just let me get used to.</i>” She shoos you away and goes back to toying with herself, getting used to her new “flexibility”."
        );
        this.outputText(
    ...
    private stayHomeFromHeliaParty(): void {
        this.clearOutput();
        this.outputText(
            '"<i>Aww, lame,</i>" Hel groans, sighing.  "<i>Fiiiiiine.  I guess I\'ll just go have awesome fun and party hard without you. Don\'t stick too hard into that mud, lover,</i>" she chuckles, giving you a reassuringly affectionate kiss on the cheek before trucking off.'
        );
        this.doNext(this.playerMenu);
    }
    ...
    private helPartyMenu(): void {
        this.menu();
        // heliaDrinks
        if (this.flags[kFLAGS.HELIA_BDAY_DRINKS] == 0)
            this.addButton(0, "Drinks", this.heliaDrinks);
        // heliaHakonAndKiri
        if (this.flags[kFLAGS.HELIA_BDAY_HAKON_AND_KIRI] == 0)
            this.addButton(1, "Hakon 'n Kiri", this.heliaHakonAndKiri);
        // heliaPhoenixes
        if (this.flags[kFLAGS.HELIA_BDAY_PHOENIXES] == 0)
            this.addButton(2, "Phoenixes", this.heliaPhoenixes);
        // heliaAndFoxTwins
        if (this.flags[kFLAGS.HELIA_BDAY_FOX_TWINS] == 0)
    ...
    private goWithHelia(): void {
        this.clearOutput();
        this.outputText(
            '"<i>Sounds like fun.  Let\'s go,</i>" you say, offering your arm to Hel.  She grins and takes it, holding herself close as she takes the lead, steady paces quickly drawing you out into the vast deserts of Mareth beside your lover.'
        );
        ...
        if (
            ...
            this.outputText(
                "  Looking over your shoulder as you depart, though, you notice that camp looks particularly... empty. Perhaps your other camp-mate"
            );
            if (this.camp.companionsCount() > 1) this.outputText("s are");
            else this.outputText(" is");
            this.outputText(
                " out, too? You shrug. You and Hel aren't the only ones with lives, you suppose."
            );
        }
        ...
        this.outputText(
            " as you approach the great gates.  The only one on gate duty today is Urta, the grey-coated fox curled up under a small awning beside the gate with a huge canteen resting beside you.  Her centauress partner is noticeably missing, but Urta still gives you a wave and a grin you can't quite read."
        );
        ...
        this.outputText(
            '.  "<i>Hey, thanks for coming, [name],</i>" she says quietly, squeezing your arm, "<i>Kiri\'s done nothing but talk about you since the tower.  I think someone has a crush...</i>"'
        );
        ...
        */
        ...
        if (this.flags[kFLAGS.HARPY_QUEEN_EXECUTED] > 0)
            ...
            this.outputText(
                "setting the queen right, but leaving her alive for when it's finally time to bring the fight to Lethice"
            );
        this.outputText(
            ".  You nod at his thanks, and his hand is soon replaced by Helia's as she grabs you on her way toward the gigantic cake.  The towering white confectionary seems more than big enough to feed everyone in the group, a huge extravagance on the part of her family.  You hear a quiet whisper about "
        );
        if (this.flags[kFLAGS.FREED_VALA] == 1)
        ...
        this.outputText(
            " as Hel is handed an oversized knife and urged to blow out the candles atop it, arranged into a perfect 23."
        );
        ...
        if (this.flags[kFLAGS.HEL_FOXY_FOURSOME_WARNED] == 1)
            this.outputText("a pair of lovely fox-eared herms");
        else this.outputText("Miko and Mai");
        this.outputText(
            ' burst out, their big breasts covered in nothing but tiny pink pasties and an all-too-small thong sheathing their red pricks.  Arm in arm with tail wagging furiously, they cry out "<i>Happy birthday, Hel!</i>" before hopping out and slipping into the salamander\'s arms.  "<i>Ladies,</i>" Hel says, grinning like an idiot, a moment before she\'s pushed into a waiting chair behind her.  One of the girl\'s busts is quickly shoved into her face as the rest of the group laughs and starts passing out another round of drinks.'
        );
    ...
    private heliaPhoenixes(): void {
        this.clearOutput();
        this.outputText(
            "You approach the small squad of phoenixes milling around the cake, mostly talking amongst themselves.  They nod as you approach, a bit wary of the person who recently kicked their asses inside out.  Still, they nod respectfully as you approach."
        );
        ...
        this.outputText(
            ", he thought it would be good to try and at least... know each other.  I'm not complaining.  Things like this party... never would have seen anything like this under Queen Calais.  It's nice to see new things, meet new people.</i>\""
        );

        this.outputText("\n\nYou see a few glances going toward Edryn and the fox twins");
        if (this.followerKiha() && this.isabellaFollower())
            this.outputText(
                ", then to the fiery dragoness and the ultra-busty cowgirl talking in the corner"
            );
    ...
    private heliaAndFoxTwins(): void {
        this.clearOutput();
        this.outputText(
            "You approach the foxy strippers, still well into giving Helia a dual lap-dance, their red puppy peckers poking well out of their tiny thongs.  They grin as you approach, and Hel pulls you into her lap, and you quickly find yourself with a pair of big, soft breasts to rest your head on, courtesy of "
        );
        if (this.flags[kFLAGS.HEL_FOXY_FOURSOME_WARNED] == 1) this.outputText("Mai");
        else this.outputText("one of the foxes");
        this.outputText(
            ".  Hel plants a deep, serpent-tongue filled kiss on your lips, nuzzling her cheek on your [chest]."
        );
    ...
    private leaveWithoutSex(): void {
        this.clearOutput();
        this.outputText(
            "You decide you've had about enough of the party for now. You slip over to Hel and tell her you're ready to head out when she is."
        );
    ...
    private helAndSluttyHarpyMale(): void {
        ...
        this.outputText(
            "You take Helia’s hand and lead her through the camp in an attempt to find someone to include in your threesome. After some searching you see Sophie sitting on a rock and get a wonderful idea. You ask Hel if she would be ok with a threesome with the busty harpy. Her smile turns to a serious frown for a moment."
        );
        ...
        this.outputText(
            " her sodden cunt, and hear Hel gasp for air. Before the birdslut can comment on the matter, you spin her around and push her down onto Helia, leaving them face to face."
        );
        ...
        if (this.player.cocks.length == 1) {
            ...
            this.outputText(
                " to boil over. White jissom blasts from your [cock], coating Helia and Sophie’s stomachs, as well as the underside of the pair’s tits."
            );
            if (this.player.vaginas.length > 0)
                this.outputText(
                    " Your unused [vagina] cums with your cock in a show of sympathetic orgasm, coating your thighs with femcum."
                );
        }
        // 2-3 cocks
            ...
            if (this.player.vaginas.length > 0)
                this.outputText(
                    " Your unused [vagina] cums with your cocks in a show of sympathetic orgasm, coating your thighs with femcum."
                );
        }
        // > 4 cocks
        else {
            ...
            this.outputText(
                " asshole. She gasps at the sudden double penetration, and you reciprocate because of the furnace-like heat of both her holes. Your [cock biggest3] lines up with Sophie’s slutty pussy and drives home while your [cock smallest] does the same to her ass."
            ); // Haha look at me being a lazy shit.
            ...
            this.outputText(
                ". The two girls seem to share your problem, their teeth clenched and clinging tightly to each other trying to resist. Seeing their clearly shared pleasure in the moment puts you over the edge, and you let loose the torrent of liquid lust from [eachCock]. Sophie cums as the tsunami of jizz floods her levies, the copious amount spilling back out of her holes and pooling around your knees."
            );
            if (this.player.cocks.length > 4)
                this.outputText(" Your extra cocks spray pearly cum all over the nearby ground.");
            if (this.player.vaginas.length > 0)
                this.outputText(
                    " Your unused [vagina] cums with your cocks in sympathetic orgasm, coating your thighs with femcum."
                );
        }
        // Merge
    ...
    private helAndSluttyHarpyFemale(): void {
        ...
        this.outputText(
            "You grasp Helia’s hand and pull her to another part of your camp. On the way you try to think of someone to include in your threesome. You pause for a moment of serious thought and decide you want a threesome with her and Sophie.  You Inform her of your thoughts, and her smile flattens to a serious look."
        );
        ...
        if (this.player.cocks.length >= 1) {
            this.outputText(
                "The blasts spatter against the underside of Sophie’s gigantic melons and slide off Hel’s sides like an overfull cream pastry."
            );
        }
    ...
    private helAndSluttyHarpyGenderless(): void {
        ...
        this.outputText(
            "You walk up to Helia and ask if she would be up for fucking you with that wondrous tail of hers. She smiles slyly and wraps her arms around you. You take her response as an unspoken yes, hugging her back and pulling her off to find a better place for your loving."
        );
