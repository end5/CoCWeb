...
export class HelFollower extends NPCAwareContent {
    ...
    public helAffection(diff: number = 0): number {
        if (this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] > 70 && this.flags[kFLAGS.HEL_HARPY_QUEEN_DEFEATED] == 0) this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] = 70;
        if (this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] < 100 || (this.flags[kFLAGS.HEL_BONUS_POINTS] == 0 && diff < 0)) {
            this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] += diff;
            if (this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] >= 100) this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] = 100;
            if (this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] < 0) this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] = 0;
        }
        else if (this.followerHel()) {
            this.flags[kFLAGS.HEL_AFFECTION_FOLLOWER] = 100;
            this.flags[kFLAGS.HEL_BONUS_POINTS] += diff * 3;
            if (diff > 0) if (this.flags[kFLAGS.HEL_BONUS_POINTS] > 150) this.flags[kFLAGS.HEL_BONUS_POINTS] = 150;
            else if (diff < 0) if (this.flags[kFLAGS.HEL_BONUS_POINTS] < 0) this.flags[kFLAGS.HEL_BONUS_POINTS] = 0;
        }
    ...
    public heliaFollowerIntro(): void {
        ...
        if (this.player.tallness > 84) this.outputText("waist");
        else this.outputText("shoulders");
        ...
        if (this.flags[kFLAGS.HARPY_QUEEN_EXECUTED] > 0) this.outputText("kill");
        else this.outputText("bring down");
    ...
    private iCantLetFireButtsRapeMyCampsButt(): void {
        ...
        if (this.flags[kFLAGS.HEL_FOXY_FOURSOME_WARNED] > 0 || this.flags[kFLAGS.HEL_EDRYN_OFFER] > 0) this.outputText("like usual");
    ...
    public helFollowersIntro(): void {
        ...
        if (this.followerKiha() && this.flags[kFLAGS.HEL_INTROS_LEVEL] < 1) {
            this.flags[kFLAGS.HEL_INTROS_LEVEL] = 1;
        ...
        //If Isabella
        else if (this.isabellaFollower() && this.flags[kFLAGS.HEL_INTROS_LEVEL] < 2) {
            this.flags[kFLAGS.HEL_INTROS_LEVEL] = 2;
            ...
            //If Isabella is at Camp (And she and Hel are NOT cool)
                ...
                //(corruption = pussy)
                if (this.player.cor < 50) this.outputText("fall silent under their considering gazes.");
                //(corruption = high enough to call bitches out when they're hypocrites)
                else this.outputText("glower balefully at them, as if to challenge either to invite your opinion of their own sexual 'résumé' with an ill-considered comment.");
        ...
        //If Rath is in Camp
        else if (this.flags[kFLAGS.HEL_INTROS_LEVEL] < 3 && this.player.findStatusAffect(StatusAffects.CampRathazul) >= 0) {
            this.flags[kFLAGS.HEL_INTROS_LEVEL] = 3;
        ...
        //If Bimbo Sophie is at Camp:
        else if (this.flags[kFLAGS.HEL_INTROS_LEVEL] < 4 && this.bimboSophie() && this.flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 0) {
            this.flags[kFLAGS.HEL_INTROS_LEVEL] = 4;
            ...
            this.flags[kFLAGS.HEL_INTROS_LEVEL] = 9001;
            this.playerMenu();
            return;
        }
        this.menu();
    ...
    private bimboSophieGetsBooted4Firebutt(): void {
        ...
        if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00326] >= 3) {
            ...
            this.flags[kFLAGS.MINO_SONS_HAVE_SOPHIE] = 1;
        }
    ...
    public heliaFollowerMenu(display: boolean = true): void {
        ...
        if (this.flags[kFLAGS.HEL_FOLLOWER_LEVEL] == 2) {
            if (this.flags[kFLAGS.HELIA_ANAL_TRAINING_OFFERED] == 0 && display && this.player.biggestCockArea() > this.heliaAnalCapacity()) {
                this.heliaAnalTrainingPrompt();
                return;
            }
            if (display) this.outputText("You call your salamander lover over, and in a few moments Hel walks your way, hips and tail swaying gaily as she moves.  She wraps an arm around your shoulders, pressing her soft, warm body against yours, and grins.  <i>\"Heyya, lover mine.  You need anything?\"</i>");
            this.menu();
            ...
            if (this.flags[kFLAGS.HEL_HARPY_QUEEN_DEFEATED] == 1) {
                if (display) this.outputText("(You've reached the culmination of Helia's current storyline.  Stay tuned for more!)");
            }
            else {
    ...
    private heliaOptions(): void {
        if (kGAMECLASS.helScene.pregnancy.event >= 3 && this.flags[kFLAGS.HELIA_TALK_SEVEN] == 0) {
            this.helSpawnScene.heliaTalkSeven();
            return;
        }
        if (this.flags[kFLAGS.HELSPAWN_AGE] == 1 && this.flags[kFLAGS.HEL_TALK_EIGHT] == 0) {
    ...
    private sparWithHeliaFirebuttsAreHot(): void {
        ...
        this.monster.createStatusAffect(StatusAffects.Sparring, 0, 0, 0, 0);
        //No gems.
        this.monster.XP = 1;
        this.monster.gems = 0;
    }
    ...
    public loseToSparringHeliaLikeAButtRapedChump(): void {
        this.clearOutput();
        //If HP loss)
        if (this.player.HP < 1) this.outputText("You collapse on the ground, overwhelmed by pain and exhaustion caused by the berserker's onslaught.");
        else this.outputText("Your arousal is too great, and your mind can no longer focus on anything but a desperate need for release.  Lust robs you of your will, and with buckling knees you collapse.");
    ...
    public PCBeatsUpSalamanderSparring(): void {
        this.clearOutput();
        //(If HP loss)
        if (this.monster.HP < 1) this.outputText("Unable to withstand your onslaught, the salamander collapses to a knee, barely supporting her weight on her sword.");
        else this.outputText("Panting heavily, knees snaking, she collapses, heavily leaning upon her sword.");

        this.outputText("\n\n<i>\"Come on, [name],\"</i> Hel groans, swaying unsteadily.  ");
        if (this.monster.HP < 1) this.outputText("<i>\"Kick my ass a little harder, why don't you?\"</i>");
        else this.outputText("<i>\"Turn me on till I can't think straight, why don't you?\"</i>");
    ...
    private talkToHel(): void {
        ...
        if (this.flags[kFLAGS.FOLLOWER_HEL_TALKS] == 0) {
            ...
            //(If Toughness <50:
            if (this.player.tou < 50) this.outputText("You cough and gag, eyes misting as the pure-grain booze burns your throat worse than hot coals.  Hel laughs riotously, slapping you on the back hard enough to cause booze to snort out through your nose.");
            else this.outputText("You knock back the stein easily, your super-human endurance keeping you from choking on the powerful brew.  Impressed, Hel gives you a high-five and throws her head back in a laugh.");
        ...
        //Hel Talk 2
        else if (this.flags[kFLAGS.FOLLOWER_HEL_TALKS] == 1) {
        ...
        //Hel Talk 3(C)
        else if (this.flags[kFLAGS.FOLLOWER_HEL_TALKS] == 2) {
        ...
        //Hel Talk 4(C)
        else if (this.flags[kFLAGS.FOLLOWER_HEL_TALKS] == 3) {
            //Hel Talk 4(C)
        ...
        //Hel Talk 5(C)
        else if (this.flags[kFLAGS.FOLLOWER_HEL_TALKS] == 4) {
            ...
            if (this.helSpawnScene.helPregnant()) this.outputText("beer ");
            else this.outputText("fizzy, non-alcoholic beer ");
            ...
            if (!this.helSpawnScene.helPregnant()) this.outputText(", her eyes crossing a little.  Looks like she was dipping in before you got here...");
            else this.outputText(".");

            this.outputText("\n\nHel laughs ");
            if (!this.helSpawnScene.helPregnant()) this.outputText("drunkenly");
            else this.outputText("happily");
            if (!this.helSpawnScene.helPregnant()) {
                ...
                //if Kiha is in camp:
                if (this.kihaFollower.followerKiha()) this.outputText("  From across camp, you hear a certain dusky dragoness shout <i>\"HEY! That's MY trick, firebutt!\"</i>");
                ...
                if (this.kihaFollower.followerKiha()) this.outputText("further pissing off Kiha, who throws a rock at Hel, and ");
        ...
        //Hel Talk 6 (Needs Isabella and Kiha at camp; at least 1 gem)(C)
        else if (this.flags[kFLAGS.FOLLOWER_HEL_TALKS] == 5 && this.player.gems >= 1 && this.isabellaFollower() && this.kihaFollower.followerKiha()) {
            var gems: number = 0;
            ...
            if (!this.isabellaAccent()) this.outputText(", obviously only somewhat interested");
            this.outputText(".");
    ...
    private hugASmokeyTail(): void {
        ...
        if (this.model.time.hours >= 21) {
            ...
            if (this.player.tailType > TAIL_TYPE_NONE) this.outputText("s");
    ...
    private centaurMountsCampHel(): void {
        ...
        //if no boobs:
        if (this.player.biggestTitSize() < 3) this.outputText(" but your body is too big, and you must content yourself to endure Hel's ministrations");
        else this.outputText(" but all you can do is take hold of your [chest] and tease and tweak your [nipples], getting what pleasure you can from your sensitive teats");
        this.outputText(".");
        ...
        if (this.player.cockArea(x) > this.heliaCapacity()) this.outputText("she can cram in, the rest of your length twitching between her legs");
        else this.outputText("you have to offer her");
    ...
    private heliaFollowerTentafuck(): void {
        ...
        if (this.player.balls > 0) this.outputText("[balls]");
        else this.outputText("wriggling tentacles");
    ...
    private muddyLizardFeet(): void {
        ...
        if (this.player.balls > 0) this.outputText("her heel coming to caress your taut sack");
        else if (this.player.hasVagina()) this.outputText("her heel brushing against the nub of your [clit]");
        else this.outputText("her heel going so low as to brush your taint, making your [asshole] clench in surprise before she moves back up your length");
        ...
        if (this.player.biggestCockArea() > this.heliaCapacity()) this.outputText("you feel your [cockHead biggest] against the lips of her cervix, as fully into her as you can go");
        else this.outputText("your groin meets Helia's, your [cock] wholly enveloped within her");
    ...
    private takeABath(): void {
        //{First Time intro}
        if (this.flags[kFLAGS.HEL_CAN_SWIM] == 0) {
            this.clearOutput();
    ...
    private heliaAndVapula(): void {
        ...
        if (this.player.cumQ() >= 750) this.outputText(" until her belly swells and starts spurting ropes of jism back out of her love-tunnel");
    ...
    private dudeHeliaAndKihaThreeSome(): void {
        ...
        //if 1 cock:
        if (this.player.cockTotal() == 1) this.outputText("the rest of your shaft, her own tongue coiling around your tool");
        else this.outputText("your second erection, taking it in both hands and giving it long, loving strokes before slurping up the [cockHead 1]");
        this.outputText(".");
    ...
    private helGuardToggle(): void {
        ...
        if (this.flags[kFLAGS.HEL_GUARDING] == 0) {
            ...
            this.flags[kFLAGS.HEL_GUARDING] = 1;
        }
    ...
    private heliaAnalTrainingPrompt(): void {
        this.clearOutput();
        this.flags[kFLAGS.HELIA_ANAL_TRAINING_OFFERED] = 1;
        ...
        if (this.player.tallness < 60) this.outputText("her bosom towering over you");
        else if (this.player.tallness < 72) this.outputText("towering over you");
        else if (this.player.tallness < 90) this.outputText("meeting you eye to eye");
        else this.outputText("looking up at you confidently");
        this.outputText(" as she boldly reaches into your [armor] to squeeze your ");
        if (this.player.cockTotal() > 1) this.outputText("biggest ");
    ...
    private giveHeliaAnalTraining(): void {
        ...
        if (this.player.cockTotal() > 1) this.outputText("selecting the biggest and heaviest to stroke as you near her");
        else this.outputText("grabbing it and stroking it as you near her");
        ...
        if (this.player.balls > 0) this.outputText("She tilts forward to press her chin into your [sack] and begins to lather your [balls] with frothy, warm, salamander spit. One hand hefts a weighty nut as she works, caressing it as if it would somehow coax the cum out of it. ");
        ...
        if (this.player.balls > 0) this.outputText(", hard enough to lift your nuts in their [sack]");
        this.outputText(", and you erupt, cumming a steamy hot load of spunk straight onto Helia’s face, head, and back.");
        if (this.player.cumQ() < 250) this.outputText(" You run out before long, but it’s still enough to make her a white painted mess. She licks the excess from her upper lip and shudders happily.");
        else if (this.player.cumQ() < 2000) this.outputText(" You spurt rope after rope onto her, actually drenching her hair with alabaster moisture. Her back and face look equally smothered with spooge when you finish. She’s happy to wipe the goop out of her eyes and shovel it into her mouth, swallowing it down with a satisfied smile.");
        else {
            ...
            if (this.player.cumQ() >= 3000) this.outputText(", but you keep cumming, cascading fresh waves onto her until there’s not a single part of her that isn’t stained and dripping with your salty outflow");
    ...
    private heliaGapeSceneChoices(): void {
        if (this.flags[kFLAGS.HELIA_ANAL_TRAINING] == 0) this.giveHeliaAnalTraining();
        else if (this.flags[kFLAGS.HELIA_ANAL_TRAINING] == 1) this.heliaAnalTrainingPartTwo();
    }
    ...
    private yesMakeHeliaAGapeSlut(): void {
        this.clearOutput();
        this.flags[kFLAGS.HELIA_ANAL_TRAINING] = 2;
        ...
        if (this.player.str < 50) this.outputText("strain to ");
        ...
        if (this.player.cumQ() < 1000) this.outputText(", ignoring the way your sheer volume of spunk leaks from the corners of her mouth and nose, her belly gradually rounding with cum in addition to thick dildo");
        this.outputText(". The slutty salamander eventually pulls it off and ");
        if (this.player.cumQ() < 2000) this.outputText("squeezes a last few dollops onto her face, letting them stain her with your scent.");
        else this.outputText("allows you to hose her down with your ongoing waves of salty jism, soaking her in your sexual scent.");
    ...
    private goWithHelia(): void {
        ...
        if (this.isabellaFollower() || this.followerKiha() || this.flags[kFLAGS.VALARIA_AT_CAMP] == 1) {
            ...
            if (this.camp.companionsCount() > 1) this.outputText("s are");
        ...
        //if less than 7' PC:
        if (this.player.tallness < 84) this.outputText("sweeping you off your feet");
        else this.outputText("hanging off you");
        ...
        //If PC did UrtaQuest:
        this.outputText("\n\n");
        if (this.flags[kFLAGS.URTA_MET_HEL] == 1) this.outputText("From the quick look she and the salamander exchange, you get the feeling they've met before. Weird. Still, ");
        ...
        if (this.player.tallness <= 72) this.outputText(", her head resting on your shoulder");
        ...
        */
        ...
        if (this.flags[kFLAGS.HARPY_QUEEN_EXECUTED] > 0) this.outputText("who forgave us for the queen's death");
        else this.outputText("who wanted to be friends still, even after the Tower");
        ...
        if (this.flags[kFLAGS.HARPY_QUEEN_EXECUTED] > 0) this.outputText("finally killing that bitch of a queen");
        else this.outputText("setting the queen right, but leaving her alive for when it's finally time to bring the fight to Lethice");
        ...
        if (this.flags[kFLAGS.FREED_VALA] == 1) this.outputText("a certain waitress fairy having... supplied the icing herself");
        else this.outputText("a certain minotaur chef having supplied the icing himself");
        ...
        if (this.flags[kFLAGS.HEL_FOXY_FOURSOME_WARNED] == 1) this.outputText("a pair of lovely fox-eared herms");
        else this.outputText("Miko and Mai");
    ...
    private heliaDrinks(): void {
        ...
        //if PC is male:
        if (this.player.hasCock() && this.player.hasVagina()) this.outputText(" just as you do");
        ...
        this.flags[kFLAGS.HELIA_BDAY_DRINKS] = 1;
        this.helPartyMenu();
    }
    ...
    private heliaHakonAndKiri(): void {
        ...
        this.flags[kFLAGS.HELIA_BDAY_HAKON_AND_KIRI] = 1;
        this.helPartyMenu();
    }
    ...
    private heliaPhoenixes(): void {
        ...
        if (this.flags[kFLAGS.HARPY_QUEEN_EXECUTED] > 0) this.outputText("dead");
        else this.outputText("deposed");
        ...
        if (this.followerKiha() && this.isabellaFollower()) this.outputText(", then to the fiery dragoness and the ultra-busty cowgirl talking in the corner");
        this.outputText(", and you figure you know what they mean.");
        this.flags[kFLAGS.HELIA_BDAY_PHOENIXES] = 1;
        this.helPartyMenu();
    }
    ...
    private heliaAndFoxTwins(): void {
        ...
        if (this.flags[kFLAGS.HEL_FOXY_FOURSOME_WARNED] == 1) this.outputText("Mai");
        else this.outputText("one of the foxes");
        ...
        this.flags[kFLAGS.HELIA_BDAY_FOX_TWINS] = 1;
        this.helPartyMenu();
    }
    ...
    private helAndSluttyHarpyMale(): void {
        ...
        if (this.player.biggestCockLength() >= 8) this.outputText(" no longer be hilted inside");
        else this.outputText(" slip out of");
        ...
        if (this.player.cocks.length == 1) {
            ...
            if (this.player.vaginas.length > 0) this.outputText(" Your unused [vagina] cums with your cock in a show of sympathetic orgasm, coating your thighs with femcum.");
        }
        // 2-3 cocks
            ...
            if (this.player.cumQ() < 500) this.outputText(" with your cum");
            else {
                this.outputText(" full of cum");
                if (this.player.cumQ() <= 1000) {
                    this.outputText(" until it overflows");
                    if (this.player.cumQ() >= 5000) this.outputText(", pooling around your knees");
                }
            }
            this.outputText(".");
            if (this.player.cocks.length == 3) this.outputText(" Your unused cock splatters the two with hot, white jizz.");
            if (this.player.vaginas.length > 0) this.outputText(" Your unused [vagina] cums with your cocks in a show of sympathetic orgasm, coating your thighs with femcum.");
        }
        // > 4 cocks
        else {
            ...
            if (this.flags[kFLAGS.HELIA_ANAL_TRAINING] == 0) this.outputText(" tight");
            else if (this.flags[kFLAGS.HELIA_ANAL_TRAINING] == 1) this.outputText(" well-trained");
            else if (this.flags[kFLAGS.HELIA_ANAL_TRAINING] >= 2) this.outputText(" gaped");
            ...
            if (this.player.cocks.length > 4) this.outputText(" Your extra cocks spray pearly cum all over the nearby ground.");
            if (this.player.vaginas.length > 0) this.outputText(" Your unused [vagina] cums with your cocks in sympathetic orgasm, coating your thighs with femcum.");
        }
        // Merge
