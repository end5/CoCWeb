...
export class AnemoneScene extends BaseContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (this.flags[kFLAGS.ANEMONE_KID] > 0) {
            if (this.flags[kFLAGS.KID_ITEM_FIND_HOURS] < 20) this.flags[kFLAGS.KID_ITEM_FIND_HOURS]++;
            if (this.flags[kFLAGS.KID_SITTER] == 0 && this.flags[kFLAGS.MARBLE_KIDS] >= 5 && this.model.time.hours > 10 && this.model.time.hours < 18 && AnemoneScene.rand(4) == 0) {
                this.kidABabysitsCows();
                needNext = true;
            }
            if (this.flags[kFLAGS.KID_SITTER] == 1 && this.model.time.hours > 10 && this.model.time.hours < 18 && AnemoneScene.rand(4) == 0) {
    ...
    private anemonePreg(): void {
        this.player.knockUp(PregnancyStore.PREGNANCY_ANEMONE, PregnancyStore.INCUBATION_ANEMONE, 101);
    }
    ...
    public mortalAnemoneeeeee(): void {
        ...
        if (this.flags[kFLAGS.TIMES_MET_ANEMONE] == 0 || this.player.hasItem(this.consumables.MINOCUM)) {
    ...
    public defeatAnemone(): void {
        this.outputText("", true);
        //Win by HP:
        if (this.monster.HP < 1) this.outputText("The anemone's knees buckle and she collapses, planting her hands behind her with a splash.  You stand over her, victorious.\n\n", false);
        //win by lust:
        else this.outputText("The anemone slumps down and begins masturbating, stroking her cock furiously.  You think you can detect something like desperation in her opaque eyes.  It doesn't look like she'll trouble you anymore.\n\n", false);
        if (this.player.lust >= 33) {
            ...
            if (this.player.hasVagina() && this.player.clitLength >= 4) anal = this.anemoneButtPlugginz;
            else if (this.player.hasCock() && this.player.cockThatFits(48) >= 0) anal = this.anemoneButtPlugginz;
    ...
    private victoryButtholeRape(): void {
        ...
        if (this.player.cockTotal() > 0) {
            ...
            if (this.player.cumQ() < 50) this.outputText("squirt", false);
    ...
    private rapeAnemoneWithDick(): void {
        ...
        if (this.player.cockThatFits(36) >= 0) {
            ...
            //[(lust victory)
            if (this.monster.lust > 99) this.outputText(", which she's currently diddling with the hand she's not using to stroke her cock", false);
            ...
            if (this.player.cockTotal() > 1) {
                ...
                if (this.player.cockTotal() > 2) this.outputText(", and them,", false);
            ...
            if (this.player.cumQ() > 500) {
                ...
                //[(super skeet)
                if (this.player.cumQ() > 2000) this.outputText(" to enormous dimensions.  <i>\"Ohh...\"</i> she moans, as her waist distends to over four times its former slender diameter", false);
                this.outputText(".  She looks ", false);
                if (this.player.cumQ() < 2000) this.outputText("thoroughly", false);
                else this.outputText("monstrously", false);
        ...
        //Too-big male: -requires cockarea > 36
            ...
            this.outputText("Rubbing yourself through your " + this.player.armorName + ", you look over the anemone; your attention wanders down her torso to the blue slit between her legs", false);
            //[(lust victory)
            if (this.player.lust > 99) this.outputText(", which she's currently diddling with the hand she's not using to stroke her cock", false);
            ...
            if (this.player.tallness > 48) this.outputText("over", false);
            else this.outputText("before", false);
            ...
            //[(dick > 60")
            if (this.player.cocks[0].cockLength > 60) this.outputText(", despite being unable to look anywhere except over her head thanks to the giant ridge running up the front of her body,", false);
            ...
            //[(mega skeet)
            if (this.player.cumQ() > 1500) this.outputText("  Eventually she can't hold in the sheer volume of your ejaculate, and it erupts from her mouth in a white spray.  Spurt after spurt goes into and then out of her, dribbling out of her slack mouth and down her face until her hair is covered with seed.", false);
            ...
            //[(normal/big skeet)
            if (this.player.cumQ() < 500) this.outputText("her hands are still dazedly playing with the space between her breasts where your cock used to rest.", false);
            //[(mega skeet)
            else this.outputText("she continues to sputter and cough up bubbles of your spunk.", false);
            //(pass 1 hour, reset lust to min or min+10 if big or greater skeet)
            this.player.orgasm();
            this.cleanupAfterCombat();
        }
    ...
    private rapeAnemoneWithPussy(): void {
        ...
        //[(breastrow0>C-cup)
        if (this.player.biggestTitSize() > 2) this.outputText("  You give special attention to the presentation of your " + this.breastDescript(0) + ", removing your top with tantalizing slowness, letting each breast slip out and hang between you like fruit ripe to be taken, then making sure to rub them seductively to arouse the both of you further.", false);
        //(hipsize=girly or better)
        if (this.player.hipRating > 6) this.outputText("  You make good use of your " + this.hipDescript() + ", too, giving a little shimmy to show off your pronounced curves.", false);
        ...
        //[(breast0>C)
        if (this.player.biggestTitSize() > 2) this.outputText("making sure to let your " + this.player.allBreastsDescript() + " rub up against hers, ", false);
        this.outputText("then pull apart from her and ", false);
        //[(goddamn centaur)
        if (this.player.lowerBody == LOWER_BODY_TYPE_CENTAUR) this.outputText("turn away, kneeling down to display your animalistic, musky pussy readily.", false);
        else this.outputText("recline back on your " + this.player.legs() + ". Spreading your thighs, you reach down with two fingers and pull apart your " + this.vaginaDescript(0) + " welcomingly; it's the last act in your sexual performance.", false);
        this.outputText("\n\n", false);
        ...
        //[(no centaur)
        if (this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) this.outputText("You take her by the upper arms and pull her on top of you as you lie back in the sun-warmed shallows.  ", false);
        ...
        if (this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) {
            ...
            //[(breasts>manly)
            if (this.player.biggestTitSize() >= 1) this.outputText(this.breastDescript(0) + " and ", false);
        ...
        //(squirter)
        if (this.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_DROOLING) this.outputText("soaks her crotch with juice and ", false);
        ...
        if (this.player.lowerBody == LOWER_BODY_TYPE_CENTAUR) this.outputText(" on your back", false);
        //[(notits)
    ...
    public loseToAnemone(): void {
        ...
        if (this.player.gender == 0) {
            ...
            //[(lactating)
            if (this.player.biggestLactation() > 1) this.outputText("  For a moment the examination stops at the dribble of milk leaking from your " + this.nippleDescript(0) + "s.  With one finger she collects a bit and tastes it, only to grimace and stick her tongue out.", false);
            ...
            //[(low cor)
            if (this.player.cor < 33) this.outputText("You reach out instinctively to comfort her, but ", false);
            //(high cor)
        ...
        if (this.player.cockTotal() > 0) {
            ...
            if (this.player.cockThatFits(36) >= 0) {
                ...
                if (this.player.cumQ() < 50) this.outputText("A droplet", false);
                else if (this.player.cumQ() < 250) this.outputText("A squirt", false);
                else this.outputText("A steady flow", false);
                ...
                //[(normal)
                if (this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) this.outputText(" into your lap,", false);
                //(shitty taur)
                else this.outputText(" downwards, onto your hams,", false);
                this.outputText(" delivering lances of venom into your lower body.  The tingle warms your groin and more pre-cum leaks out of " + this.sMultiCockDesc() + ".  Her eyes lock onto a glistening dollop and she drops down quickly, enfolding the head of your " + this.cockDescript(x) + " in her cool mouth.  Her tongue dances around the crown of your " + Appearance.cockNoun(CockTypesEnum.HUMAN) + ", relieving it of the sticky pre.  Looking ", false);
                if (this.player.tallness > 48) this.outputText("up", false);
                else this.outputText("down", false);
                this.outputText(" at you, you can see a smile in the lines around her eyes even though her mouth is locked around your " + Appearance.cockNoun(CockTypesEnum.HUMAN) + ".  ", false);
                //[(if dual/multi-cox)
                if (this.player.cockTotal() > 1) {
                    //[(cock2 doesn't exist)
                    if (this.player.cockTotal() == 2) this.outputText("Your other dick rubs", false);
                    //(cock2 exists)
                    else this.outputText("The rest of your " + this.multiCockDescriptLight() + " rub", false);
                ...
                if (this.player.hasVagina()) {
                    ...
                    if (this.player.vaginalCapacity() < 15) this.outputText(" a few fingers", false);
                    else if (this.player.vaginalCapacity() < 30) this.outputText(" partway", false);
                    else this.outputText(" all the way", false);
                    ...
                    //[(if clit > 5")
                    if (this.player.clitLength > 5) this.outputText("  Having achieved this, she pulls her hand out and wraps another bundle of tentacles around your " + this.clitDescript() + ", then begins jerking it off in time to her efforts on your " + Appearance.cockNoun(CockTypesEnum.HUMAN) + ".  Your eyes roll back in your head and your mouth gapes involuntarily at the rough stimulation of your swollen chick-stick.", false);
                }
                this.outputText("\n\n", false);

                this.outputText("The heat rubbing on your cock", false);
                if (this.player.hasVagina()) this.outputText(" and clit", false);
                ...
                //[(cray-cray skeet)
                if (this.player.cumQ() > 2000) this.outputText(" and squirting out her nose in thick ribbons", false);
                //[(multi-dix)
                if (this.player.totalCocks() > 1) {
                    this.outputText(" as ", false);
                    //[(dick2 = no)
                    if (this.player.totalCocks() == 2) this.outputText("your cocks shoot", false);
                    else this.outputText("the rest of your " + this.multiCockDescriptLight() + " shoot", false);
                    this.outputText(" all over her face and hair, ", false);
                    //[(small skeet)
                    if (this.player.cumQ() < 500) this.outputText("drawing a pattern like a musical score on her blue skin", false);
                    //(big skeet)
                    else if (this.player.cumQ() < 2000) this.outputText("painting her skin white as she flinches and shuts her eyes tightly", false);
                    //(cray-cray skeet)
                    else this.outputText("whitewashing her entire upper body and running off until a fan of milky color spreads through the water around you", false);
                }
                this.outputText(".  The anemone swallows greedily as she pumps each stroke into her mouth", false);
                //[(big or > skeet)
                if (this.player.cumQ() >= 500) this.outputText(", her taut blue belly distending as it fills", false);
                this.outputText(".\n\n", false);

                this.outputText("After a grateful moment of rest as the anemone swallows your issue, her hands begin pumping once again.  Oh god!  Your " + Appearance.cockNoun(CockTypesEnum.HUMAN) + " quickly returns to erectness under the renewed siege of aphrodisiac", false);
                //[(multi)
                if (this.player.cockTotal() > 1) {
                    if (this.player.cockTotal() == 2) this.outputText(" and your other " + this.cockDescript(1) + " follows suit", false);
                    else this.outputText(" and your other pricks follow suit", false);
                }
            ...
            //loss, too-big male (cock > 5" width or > 48" length):
                ...
                this.outputText("Almost reverently, she caresses the shaft of your " + this.cockDescript(0) + ", stroking lightly up its enormous length.  She pulls it down to her eye level, inspecting the head from several angles.  Tentatively, she opens her mouth and pulls your " + Appearance.cockNoun(CockTypesEnum.HUMAN) + " into it, trying to fit your expansive dickflesh into a hole that even to your lust-crazed mind looks much too small.  Despite her best efforts, she can't get more than the crown past her lips, though the reflexive motions of her tongue poking around and inside the opening make you shiver and push out more pre-cum.  The anemone eventually pops your " + this.cockDescript(0) + " out of her mouth and frowns in frustration.  After a few seconds, she seems to reach a decision.  Moving your shaft out of the way, she walks around behind you.  She places one hand on your ", false);
                if (this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) this.outputText("waist", false);
                else this.outputText("flank", false);
                //[(not centaur)
                if (this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) this.outputText(" and pushes your shoulders down with the other", false);
                this.outputText(".  As she draws you backwards, you're forced to put your own ", false);
                if (this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) this.outputText("hands ", false);
                else this.outputText("forelegs knee-", false);
                this.outputText("down in front of you to keep from falling face-first.  ", false);
                //[(if non-centaur)
                if (this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) this.outputText("The head of your " + this.cockDescript(0) + " dips into the lukewarm lakewater, sending a tingle down the shaft.  ", false);
                ...
                //[(if balls)
                if (this.player.balls > 0) this.outputText(" and your " + this.sackDescript() + " tightens up", false);
                ...
                //[(if balls)
                if (this.player.balls > 0) this.outputText(this.ballsDescriptLight() + " and ", false);
                ...
                //[(man)
                if (this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) this.outputText("clawing at your tormentor's tentacles, trying to pry them from " + this.sMultiCockDesc() + " by main force.", false);
                //(horse)
                else this.outputText("bucking and stamping the ground, wanting to shake the tentacles loose but unable to reach them with your hands.", false);
                ...
                if (this.player.tallness < 48 && this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) this.outputText("fly over your head, and turning behind you, you see the anemone trying to catch them with an open mouth and her tongue out.", false);
                else if (this.player.tallness < 84 && this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) this.outputText("catch the air and rain down on both your faces, splashing quietly where they hit water.", false);
                else {
                ...
                if (this.player.tallness < 48 && this.player.lowerBody != LOWER_BODY_TYPE_CENTAUR) this.outputText("She looks you over, then bends down and drinks up as much of the semen floating in the water as she can find nearby.", false);
                else this.outputText("She leans over you and begins licking the semen off your body, not stopping until you're clean (if slightly sticky).", false);
        ...
        //loss rape, vaginal (only full females):
            ...
            //[(lactation)
            if (this.player.biggestLactation() > 1) this.outputText("  For a moment it brightens again when she notices the dribble of milk leaking from your " + this.nippleDescript(0) + "s. With one finger she collects a bit and tastes it, only to grimace and stick her tongue out.", false);
            ...
            if (this.player.tallness < 48) this.outputText(", pulling you upright", false);
            this.outputText(".\n\n", false);
    ...
    private giveMino(): void {
        ...
        if (this.player.lust < 30) {
            ...
            if (this.player.cockTotal() > 0) this.outputText(this.cockDescript(0) + " becomes a little erect", false);
            else if (this.player.hasVagina()) this.outputText(this.vaginaDescript(0) + " aches a bit with need", false);
            else this.outputText(this.assholeDescript() + " begins to tingle with want", false);
    ...
    private anemoneButtPlugginz(): void {
        ...
        if (this.monster.lust > 99) this.outputText(" as she plays with herself");
        ...
        if (!this.player.isTaur()) {
            ...
            else this.outputText("your " + this.vaginaDescript(0));
        ...
        if (this.player.hasCock() && this.player.cockThatFits(48) >= 0) {
            if (!this.player.isTaur()) this.outputText("you fish your [cockFit 48] out of your garments and rub it");
            else this.outputText("you rub your [cockFit 48]");
            this.outputText(" between the smooth blue curves");
        }
        else {
            if (!this.player.isTaur()) this.outputText("you finger your pussy");
            else this.outputText("you imagine the tightness of her hole");
        ...
        if (this.player.hasCock() && this.player.cockThatFits(48) >= 0) this.outputText(" as you smear a dribble of pre-cum into the crack of her ass");
        ...
        if (this.player.hasCock() && this.player.cockThatFits(48) >= 0) this.outputText("cock");
    ...
    private anemoneQuoteUnquoteAnal(): void {
        ...
        if (!this.player.isTaur()) {
            ...
            if (dick) this.outputText(this.cockDescript(x));
        ...
        if (!this.player.isTaur()) this.outputText("seize her head");
        else {
            this.outputText("mount her head");
            if (!dick) this.outputText(", angling your clit forward along your stomach by pinning it against the anemone's forehead");
        }
        ...
        if (this.player.balls > 0) {
            ...
            if (!this.player.hasCock()) this.outputText(".  As your [balls] swell with blood and seed, you can't help but groan; there'll be nowhere for the largess to go, and it will be with you until your body reabsorbs it - or you make an outlet");
        }
        ...
        if (dick) {
            ...
            if (this.player.cumQ() >= 1000) this.outputText("  You push so much cum into her that her belly actually rounds from the volume, transforming from a sleek, flat midriff into a barely-contained ball of sloshing liquid");
            if (this.player.cumQ() >= 2000) this.outputText("; even when her stomach skin can stretch no further, your body won't stop filling her, and the bulge expands up her esophagus, pushing out her chest and throat until she's full to the brim and rather reminiscent of a big, blue pear");
            ...
            if (this.player.cumQ() >= 1000) this.outputText("bobbing roundly in the water as she tries to make her way from the shallows, ");
            ...
            if (this.player.wetness() < 4) this.outputText("drools its lube along your length.  ");
            else this.outputText("squirts obscenely, coating your length and her face with your female orgasm.  ");
            this.outputText("Your body shivers as near-painful sensations wrack you, engulfed in the anemone's questing passage");
            if (this.player.balls > 0) this.outputText(".  Your lover, hopeful, watches and waits for your testicles to rise into your groin and unload their cargo");
            if (this.player.cockTotal() == 0) this.outputText("; she even tries to push them up herself with her hands, as if that would make your clit produce the semen inside them");
            this.outputText(".");
            if (this.player.hasCock()) this.outputText("  Your clit remains unproductive; your male orgasm squirts out uselessly around her head, mingling with the lake water.  The girl's expression upon seeing this is pained, and she tries but fails to get free to follow the arcing white ropes, sending further spasms of pleasure through your sensitive distaff staff.");
            ...
            if (this.player.hasCock()) this.outputText(", pointing to the squiggles of your seed floating lazily in the lakewater,");
    ...
    private hotdogTheAnemone(): void {
        ...
        if (dick) this.outputText("[cockFit 48]");
        else this.outputText("[clit]");
        ...
        if (dick) this.outputText("pre-cum");
        else this.outputText("your juice");
        ...
        this.outputText("\n\nThe anemone, ");
        if (dick) this.outputText("excited by the feel and sight of your pre-cum");
        else this.outputText("looking almost bored");
        ...
        if (dick) this.outputText("your [cockHead " + (x + 1) + "]");
        else this.outputText("the nerve-packed end of your [clit]");
        ...
        if (dick) this.outputText("now-throbbing prick");
        else this.outputText("swollen, red clit");
        ...
        if (dick) {
            ...
            if (this.player.cumQ() >= 500) this.outputText("  When you've finished, not a single place on the anemone's backside is still blue; the giddy girl is shoving your semen into her mouth with her tentacles and hands, swallowing almost as much lakewater as jizz.");
        }
        else {
            this.outputText("your pussy clamps down, trying and failing to find something to squeeze and ");
            if (this.player.wetness() < 4) this.outputText("drooling its lube down your thighs.");
            else this.outputText("squirting cum behind you to rain onto your partner's calves and the surface of the lake.");
        ...
        if (dick) this.outputText("dares not move from her knees, balancing your freshly-shot load on her back as she tries to push it toward her face with her tentacles.");
    ...
    private anemoneGetsLayedByBeePositor(): void {
        ...
        if (this.monster.HP < 1) this.outputText("reclining");
        else this.outputText("vigorously masturbating");
        ...
        if (this.player.antennae > ANTENNAE_NONE) this.outputText(" antennae");
        else this.outputText("nose");
        ...
        if (this.silly()) this.outputText("duck ");
        ...
        if (this.player.sens >= 50 || this.player.isTaur()) {
            ...
            if (this.player.hasCock()) {
                ...
                else this.outputText("male parts erupt");
            ...
            if (this.player.eggs() > 40) {
                ...
                if (this.player.hasCock() && this.player.cumQ() > 1000) this.outputText("and so much cum ");
            ...
            if (this.player.eggs() < 20) this.outputText("It doesn't take long before you finish, pushing your cargo down her passageway in a neat, orderly line.");
            else {
                this.outputText("So many come that you can see them under her skin, a myriad of tiny bumps");
                if (this.player.eggs() >= 40) this.outputText("; these same bumps multiply upward as your long-suffering abdomen pushes out line upon line of eggs, and soon you can feel them pressing back against the tip of your ovipositor.  You squeeze the girl's head in your hands, holding her against the base, and concentrate; slowly, the deposited eggs give way to their siblings, stretching her elastic stomach and chest wide");
                this.outputText(".");
            }
            ...
            if (this.silly() && this.player.fertilizedEggs() > 1) this.outputText("  You briefly amuse yourself imagining her carrying your eggs to term and having them hatch in her mouth, so that when she talks, she shoots bees.  Nicholas Cage would be proud.");
    ...
    private spiderOvipositAnAnemone(): void {
        ...
        if (this.player.HP < 1) this.outputText("eyes unfocused with fatigue");
        else this.outputText("two fingers plunging eagerly into her pussy");
        ...
        //[(cock)
        if (this.player.hasCock()) this.outputText(", and your [armor] tightens as [eachCock] swells");
        else if (this.player.hasVagina()) this.outputText(", and the inside of your [armor] dampens with the lube your [vagina] leaves as it clenches around empty air");
        ...
        if (this.player.cor < 60) this.outputText(", even though her adorable and slightly surreal puppy dog eyes implore you otherwise");
        ...
        //[(sexed)
        if (this.player.gender > 0) this.outputText("while unfastening your [armor] with one hand, ");
        ...
        //(egg level 3)
        if (this.player.eggs() >= 40) this.outputText("; so many eggs pump into her that she gives a little start when her distended belly touches the lukewarm water below");
    ...
    public anemoneKidBirthPtII(): void {
        ...
        if (this.player.tallness > 48) this.outputText(" even as the face angles up to maintain eye contact");
    ...
    public anemoneBarrelDescription(): void {
        ...
        else this.outputText("Kid A is here, seated demurely on the rim of her barrel and looking somewhat more purple under the red moon.  She glances slyly at you from time to time.\n\n");
    }
    ...
    public approachAnemoneBarrel(): void {
        ...
        //(if babysitting)
        if (this.flags[kFLAGS.KID_SITTER] > 1) this.outputText("Kid A is away right now, helping season some of Marble's children.  If you wanted to leave or take an item, she would discover it when she got back.  ");
        else if (this.kidAXP() < 25) this.outputText("Kid A sinks below the rim, peering nervously at you.  ");
        else if (this.kidAXP() < 75) this.outputText("Kid A pokes her head out and smiles at you.  ");
        else this.outputText("Kid A leans her head and shoulders out of the barrel and puts her elbows on the rim, staring unabashedly at you as she rests her chin on her hands.  ");
        this.outputText("What do you need from her?");
        //[(if N.Watch is toggled on)
        if (this.flags[kFLAGS.ANEMONE_WATCH] > 0) this.outputText("\n\n<b>Kid A is currently watching for enemies at night.</b>");
    ...
    private tutorAnemoneKid(): void {
        ...
        if (this.flags[kFLAGS.HAD_KID_A_DREAM] > 0 && this.kidAXP() >= 40 && this.player.lust > 99) {
            ...
            //if KidXP >= 40 and lust > 99 after tutor and PC has only huge dicks of area >= 60 or hasn't got shit
                ...
                if (this.player.hasCock()) this.outputText("something too enormous");
                else this.outputText("nothing useful");
    ...
    private kidASex(cont: boolean = true): boolean {
        ...
        if (this.flags[kFLAGS.ANEMONE_KID] == 2 && this.player.hasCock() && this.player.cockThatFits(36) >= 0) {
            ...
            //[(big skeet)]
            if (this.player.cumQ() >= 1000) this.outputText("  As you pour into her, the sensation of being stuffed with semen sets off a second orgasm in the quivering girl, and her own prick drools weakly onto your stomach");
            if (this.player.cumQ() >= 2000) this.outputText("; even a second orgasm from your daughter isn't enough to match one of yours in length.  Your magnificently productive " + Appearance.cockNoun(this.player.cocks[x].cockType) + " fills her until her belly is inflated so far that she can't contain more, and it begins to breach the seal her pussy makes around you, even as tight as it is, to squirt out in lazy globs as you deposit the last of your spunk");
            if (this.player.cumQ() >= 1000) this.outputText(".");
            //[(multi)]
            if (this.player.cockTotal() > 1) {
        ...
        //if KidXP >= 40 and lust > 99 after tutor and PC has wang of area <60, set that cock to x variable, also set biggest cock not yet used to y variable if multicocked
            ...
            if (y >= 0 || this.player.balls > 0 || this.player.hasVagina()) {
                ...
                //[(if cock or sack)]
                if (y >= 0 || this.player.balls > 0) this.outputText(", pulling it free from your armor as well");
                ...
                if (y >= 0) {
                    ...
                    if (this.player.cocks[y].cockLength >= 36) this.outputText("towers over her head");
                    else this.outputText("quivers and twitches into her venomous locks");
                ...
                //[(sac)
                else if (this.player.balls > 0) this.outputText("  One hand cups below your " + this.ballsDescript() + " and lifts them up to her chin, where the other, formerly tracing gentle circles along your shaft, grabs a tentacle from her head and begins to tickle.");
                //[(vag)
                else if (this.player.hasVagina()) {
                    this.outputText("  Her fingers part your labia and reveal your " + this.clitDescript() + ", ");
                    if (this.player.clitLength >= 6) this.outputText("working it free as well and allowing her to tease and stroke it, cock-like, with her hands and hair.");
                    else this.outputText("in order to give the sensitive button their attention.");
            ...
            if (this.player.balls > 0) this.outputText("your balls tighten up");
            else this.outputText("your body tenses");
            ...
            if (this.player.balls > 0 || y >= 0) {
                ...
                if (y >= 0) this.outputText(this.cockDescript(y));
                else this.outputText(this.sackDescript());
            ...
            if (y >= 0) this.outputText(" and onto her hair");
            ...
            if (this.player.cumQ() < 1000) this.outputText("then gets to her feet and returns to her barrel to digest the meal.");
            else this.outputText("but can barely move from the spot thanks to the incredible distension of her belly.  She manages to drag herself over to the barrel with a great shuffling of arms and legs but cannot by any means get into it, and settles for slumping beside it to occasionally sprinkle water on herself while she deflates.");
        ...
        //if KidXP >= 40 and lust > 99 after tutor and PC has vag and no cox
            ...
            //[(if pc is loose)
            if (this.player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS_GAPING) this.outputText("  You can barely feel her little shaft in your stretched cunt, but the chemical stimulation from the tentacles stroking your insides goes a long way toward making up for that.");
            ...
            //[(clitsize<6)]
            if (this.player.clitLength < 6) this.outputText("  The little button gets rubbed by her probing hand even as she resumes thrusting, accelerating your imminent peaking.");
            else if (this.player.clitLength < 36) this.outputText("  The monstrous chick-stick bobs in the air as she pounds you harder; the breeze alone would be enough to arouse you further, but the anemone grabs it and begins jacking the nerve-laden stalk off like a dick, which sends your back into spasms.");
            else this.outputText("  The incredible length of your clitoris is such that the pace of the fuck slows to a crawl as the anemone puzzles out what to do with it; finally falling back on her racial instinct, she pops it into her mouth and begins to caress the tip gently with her teeth and tongue as her short hair reaches around to tickle the shaft.  Your body flops weakly, too wracked by the sensation from your clit to allow conscious control.");
    ...
    public kidADreams(): void {
        ...
        if (this.player.hasCock() && (!this.player.hasVagina() || this.player.femininity < 50)) {
            ...
            if (this.player.isTaur()) this.outputText("thigh");
            else this.outputText("cheek");
            ...
            if (!this.player.isTaur()) this.outputText("soft hand strokes your face and a ");
        ...
        //if female:
            ...
            if (this.player.cor >= 66) this.outputText(" and, with a sigh, you grab her head and force it down to the mess, compelling her to lick it up.");
            else this.outputText(" and neither of you says a word as she backs away slowly on her knees.");
    ...
    public kidAWatchesSharks(): void {
        ...
        //(else KidXP < 75 and Kidweapon = White Book or Black Book)
            ...
            if (this.flags[kFLAGS.ANEMONE_WEAPON_ID] == this.consumables.W__BOOK.id) this.outputText("blinds them with a sudden flash of white light");
            else this.outputText("turns deep blue as the various shark-girls clap their thighs together and look away from one another in embarrassment");
            this.outputText(".  Before any of them can react, she dives into the water and fills the waterskins, then hightails it away.");
        }
        //(else KidXP < 75)
        else if (this.kidAXP() < 75) {
            this.outputText("\n\nThe sharks look up from their play, baring teeth at the anemone.  She steels herself, then holds her " + ItemType.lookupItem(this.flags[kFLAGS.ANEMONE_WEAPON_ID]).longName + " menacingly in front of her as best she can while she steps into the stream and fills her waterskins.  Occasionally one of the shark-girls will feint at the anemone, who accordingly turns the weapon toward the aggressor until she retreats.  Once the skins are filled, Kid A hangs them over her shoulders and backs away, still facing the sharks with her weapon out.");
        }
        //(else)
    ...
    public goblinNightAnemone(): void {
        ...
        if (this.player.tallness > 48) this.outputText("tall");
        else this.outputText("short");
        ...
        this.outputText("\n\nThough you can't see Kid A's expression from behind, she's probably steeling her face into as stern a mask as she's capable of, judging by the way she assumes an aggressive stance - albeit one that's still trying to protect her body as much as possible with " + ItemType.lookupItem(this.flags[kFLAGS.ANEMONE_WEAPON_ID]).longName + ".  As the anemone takes a step forward, the goblins all take a step back, except for the leader.  The two are now staring at each other; one looking up, the other down.");
