...
export class EmberScene extends NPCAwareContent implements TimeAwareInterface {
    ...
    public constructor() {
        ...
        this.pregnancy = new PregnancyStore(
            kFLAGS.EMBER_PREGNANCY_TYPE,
            kFLAGS.EMBER_INCUBATION,
            0,
            0
        );
    ...
    public timeChange(): boolean {
        ...
        if (this.player.statusAffectv1(StatusAffects.EmberFuckCooldown) > 0) {
            ...
            if (this.player.statusAffectv1(StatusAffects.EmberFuckCooldown) < 1)
                this.player.removeStatusAffect(StatusAffects.EmberFuckCooldown);
        }
        //Ember napping
        if (this.player.findStatusAffect(StatusAffects.EmberNapping) >= 0) {
            this.player.addStatusValue(StatusAffects.EmberNapping, 1, -1);
            if (this.player.statusAffectv1(StatusAffects.EmberNapping) <= 0)
                this.player.removeStatusAffect(StatusAffects.EmberNapping);
        }
    ...
    public timeChangeLarge(): boolean {
        if (
            this.player.findStatusAffect(StatusAffects.EmberNapping) < 0 &&
            this.followerEmber() &&
            this.player.findStatusAffect(StatusAffects.EmberFuckCooldown) < 0
        ) {
            ...
            if (
                ...
                EmberScene.rand(10) == 0 &&
                (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3)
            ) {
    ...
    public emberChildren(): number {
        return (
            this.flags[kFLAGS.EMBER_CHILDREN_MALES] +
            this.flags[kFLAGS.EMBER_CHILDREN_FEMALES] +
            this.flags[kFLAGS.EMBER_CHILDREN_HERMS]
        );
    }
    ...
    public emberCampMenu(): void {
        ...
        if (this.emberAffection() <= 25)
            this.outputText(
                'Ember sighs as you approach, and doesn\'t even look you in the eye before speaking.  "<i>What do you want?</i>"'
            );
        //Moderate Affection:
    ...
    private emberSexMenu(output: boolean = true): void {
        ...
        if (
            this.emberAffection() >= 95 &&
            this.player.hasCock() &&
            this.player.cockThatFits(this.emberVaginalCapacity()) >= 0 &&
            (this.player.hasItem(this.consumables.L_DRAFT) ||
                this.player.lib >= 50 ||
                this.player.minLust() >= 40)
        )
    ...
    public findEmbersEgg(): void {
        ...
        if (this.flags[kFLAGS.TIMES_FOUND_EMBERS_EGG] == 0) {
            this.outputText(
                "You spot a cave entrance partially hidden behind mossy vegetation and decide to investigate."
            );
            this.outputText(
            ...
            this.outputText(
                "You spot a familiar cave partially hidden behind the mossy vegetation and decide to confirm your suspicion."
            );
            this.outputText(
    ...
    private leaveEmbersAssOutToDry(): void {
        this.clearOutput();
        this.outputText(
            "You can't decide what to do right now, so you leave the egg where it is and return to your camp."
        );
    ...
    private eatEmbersYolkLikeAnEvenBiggerDick(): void {
        this.clearOutput();
        this.outputText(
            "Unsure of where the impulse comes from, but uncaring, you crouch over the ruined shell of your 'kill' and begin messily scooping handfuls of yolk into your mouth."
        );
        this.outputText(
    ...
    public getSomeStuff(): void {
        this.clearOutput();
        this.outputText(
            "Your mouth tightens in consternation, and you pull out the shell of the so-called 'dragon egg', passing it over and asking if she can use it."
        );
        this.outputText(
    ...
    private takeEmbersEggHomeInADoggieBag(): void {
        this.clearOutput();
        this.outputText(
            "You decide to take the egg, figuring that perhaps this dragon could aid you in your quest."
        );
        //(If player is shorter than 7 feet)
        if (this.player.tallness < 84)
            this.outputText(
                "  Lifting it isn't as much of a problem as you thought; it's surprisingly light.  It is, however, very big and very awkward to carry."
            );
        else
            this.outputText(
                "  Between the egg's surprising lightness, and your own size and wide arms, you can easily carry the egg."
            );
        this.outputText(
    ...
    public emberCampDesc(): void {
        //Iz Ember an egg?
        if (this.flags[kFLAGS.EMBER_HATCHED] == 0)
            this.outputText(
    ...
    private emberFollowerDesc(): void {
        this.outputText(
            "The mysterious egg you found in the cave sits in the grass nest you assembled for it; it is three feet tall and nearly two feet in circumference.  The nest itself isn't very pretty, but at least it's sturdy enough to keep the egg from rolling around.\n\n"
        );
    }
    ...
    public emberEggInteraction(): void {
        this.clearOutput();
        this.outputText(
            "You approach the egg you found in that illusion-concealed cave. Though the light continues to pulse with its heartbeat overtones, it still just sits there, doing nothing."
        );
        //(If the egg Corruption level is 0-25, aka \"<i>Pure</i>\")
        if (this.flags[kFLAGS.EMBER_COR] <= 25) {
            this.outputText(
                "  As you observe the egg, it glows with soft, metered pulses and you're overcome with a sense of calm and peace.  Watching them, you feel serene, as if untouched by this world's taint."
            );
        }
        //(else If the egg Corruption level is 26-74, aka \"<i>Tainted</i>\")
        else if (this.flags[kFLAGS.EMBER_COR] <= 75) {
            this.outputText(
                "  As you observe the egg, it glows with bright, gaudy pulses and you're overcome with a sense of arrogance and strength.  You feel somehow as if you could do anything, as if you were superior to everything, and no one could dare say otherwise."
            );
        }
        //(If the egg Corruption level is 75-100, aka \"<i>Corrupt</i>\")
        else {
            this.outputText(
                "  As you observe the egg you realize what you had taken for its pulses are actually just its normal color; the egg is actually 'glowing' with a black light!  As you stare, mesmerized, you begin to consider the pleasing contrast that would result if you covered it in your "
            );
            ...
            this.outputText(
                " You stop yourself and shake your head.  Where did that thought come from?"
            );
        ...
        if (
            this.flags[kFLAGS.EMBER_EGG_FLUID_COUNT] >= 5 &&
            this.flags[kFLAGS.EMBER_JACKED_ON] > 0 &&
            this.flags[kFLAGS.EMBER_GENDER] > 0
        ) {
    ...
    private dontEggFap(): void {
        this.clearOutput();
        this.outputText(
            "Shaking your head, confused and startled by these strange impulses, you step away for a moment. Once away from the egg, its pattern of pulsations returns to normal and you feel the urges disappear."
        );
        ...
        this.outputText(
            "  The egg's rhythm suddenly changes; as if it were excited by something - something that you have brought near it."
        );
        this.outputText(
    ...
    private leaveWithoutUsingAnEmberItem(): void {
        this.clearOutput();
        this.outputText(
            "You shake your head; it would probably be best not to tamper with it. Returning the items to your pockets, you leave the egg alone.  As you put them away, the egg's glow slows down dramatically, almost as if it were feeling... disappointment?"
        );
    ...
    private useIncubusDraftOnEmber(purified: boolean = false): void {
        ...
        this.outputText(
            "Uncorking the vial, you drizzle the slimy off-white fluid onto the pointed cone of the egg.  It oozes slowly across the surface, then seeps through the shell, leaving not a drop of moisture."
        );
    ...
    private useSuccubiMilkOnEmber(purified: boolean = false): void {
        ...
        this.outputText(
            "Popping the cap off of the milk bottle, you pour the contents onto the egg - the porous shell soaks up the milk as fast as you dump it, spilling not a drop."
        );
        //(If Unsexed or Herm:
        if (this.flags[kFLAGS.EMBER_GENDER] == 3 || this.flags[kFLAGS.EMBER_GENDER] == 0) {
    ...
    private useLactaidOnEmber(): void {
        ...
        //max uses 1
        this.outputText(
            "Feeling a little bemused, you pour the creamy fluid onto the egg.  It is absorbed through the shell, and a spiderwork of creamy yellow vein-like markings suddenly forms on the shell's surface."
        );
    ...
    private hairExtensionSerum(): void {
        ...
        //max uses 2
        this.outputText(
            "Wondering at your motivations, you pour the goblin gunk onto the egg.  Most rolls harmlessly off of the shell, leaving you annoyed at the waste... until you see "
        );
    ...
    private masturbateOntoAnEgg(): void {
        ...
        if (this.player.gender == 0) {
            this.outputText(
                "The light pulses decrease in speed as you disrobe and expose your bare crotch, leaving you disappointed after summoning your perversity to bring you this far.  You feel as if you've let it down somehow...  This is confusing!  You decide to go away and deal with this fickle egg another time."
            );
            this.doNext(this.camp.returnToCampUseOneHour);
            return;
        }
        ...
        //Female Version (Z)
            ...
            if (this.player.cockTotal() == 1) this.outputText("your [cock]");
                ...
                this.outputText(
                    "as many of your cocks as you can manage to grab without falling off"
                );
            this.outputText(
                ".  Finally, relentlessly, inexorably, you cum, spraying your semen into the air to spatter back onto the egg, mixing with the girlish juices from your netherlips to soak into the egg's surface, leaving it slick with your mixed sexual fluids."
            );
            this.outputText(
    ...
    private hatchZeMuzzles(): void {
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) {
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1) {
                ...
                this.outputText(
                    " the monster's crotch; there, a deceptively small slit in the flesh suddenly disgorges a 16-inch penis unlike anything you've ever seen before, bearing a rounded, elongated head and a series of ridges that give it an almost segmented look.  A pair of apple-sized balls drop into place under it.  He is most definitely male"
                );
                if (this.flags[kFLAGS.EMBER_MILK] > 0)
            ...
            //Female Anthro (Z)
                ...
                this.outputText(
                    " the monster's crotch; there, you see that the fine scales actually separate to reveal a slick-looking pussy.  She's clearly a female, with no noteworthy 'additions' that you can see."
                );
            }
            //Herm Anthro (Z)
            else {
                ...
                this.outputText(
                    "the monster's crotch; there, you see the scales part in two places.  The lower opening is unmistakably a pussy; but from the slit just above it suddenly distends a 16-inch penis unlike anything you've ever seen before, bearing a rounded, elongated head and a series of ridges that give it an almost segmented look.  Beneath it, a pair of apple-sized balls fall into place heavily, leaving you no doubt that she is a hermaphrodite."
                );
            }
        }
        //Boring version
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1) {
                ...
                if (this.flags[kFLAGS.EMBER_MILK] > 0 || this.flags[kFLAGS.EMBER_HAIR] > 0)
                ...
                if (this.flags[kFLAGS.EMBER_INTERNAL_DICK] > 0)
                    this.outputText(
                        "the slit in his pelvis that disgorges a foot and a half-long inhuman penis"
                    );
                else
                    this.outputText(
                        "a foot and a half long human penis that sways between his legs"
                    );
            ...
            //Herm Monstergirl (Z)
                ...
                this.outputText(
                    " and apple-size nuts held inside a fleshy sack slung under - the ensemble hangs nearly level with her knees.  She... or he? is obviously a hermaphrodite."
                );
            }
        }
        //Aftermath (Z)
    ...
    private meetEmberAftermath(): void {
        this.clearOutput();
        this.outputText(
            "You can only stand there and stare at this strange creature, supposedly a dragon, for what feels like hours."
        );
        this.outputText(
    ...
    private embersAppearance(): void {
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0) {
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1) {
                ...
                this.outputText(
                    " snout, wings, and horns, gives him the silhouette of a prince from your village's stories: dashing and handsome."
                );
            }
            //(Female/Herm)
            else {
            ...
            switch (this.pregnancy.event) {
                case 2:
                    this.outputText(
                        "  At the moment there's a slight pudgyness to her golden belly."
                    );
                    break;
                case 3:
                ...
                case 5:
                    this.outputText(
                        "  Her golden belly has grown quite a bit.  Ember often rests her hand on it, especially when she sees you looking at it."
                    );
                    break;
                case 6:
                    this.outputText(
                        "  Her large pregnant belly has forced Ember to change her posture.  It looks like she has a big golden bullseye painted on her stomach.  Ember looks tired but happy."
                    );
                    break;
                case 7:
                    this.outputText(
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 2 || this.flags[kFLAGS.EMBER_GENDER] == 3)
                this.outputText(
        ...
        //Dragon-girl Appearance (By Radar) (Z)
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] >= 2)
                this.outputText(
                    "  The inviting lips of a human-looking pussy purse between her legs; some moisture seems to have gathered on her labia, giving it a slick look that just begs for attention."
                );
    ...
    private talkToEmber(): void {
        ...
        if (this.player.isPregnant()) {
            //Extra check might protect against inappropriate Ember complaints
            if (
                this.flags[kFLAGS.EMBER_OVI_BITCHED_YET] == 0 &&
            ...
            if (
                this.player.pregnancyIncubation < 200 &&
                this.player.pregnancyType != PregnancyStore.PREGNANCY_EMBER &&
                this.flags[kFLAGS.EMBER_BITCHES_ABOUT_PREGNANT_PC] == 0
            ) {
            ...
            if (
                this.player.pregnancyType == PregnancyStore.PREGNANCY_EMBER &&
                this.player.pregnancyType < 300 &&
                this.flags[kFLAGS.EMBER_TALKS_TO_PC_ABOUT_PC_MOTHERING_DRAGONS] == 0
            ) {
    ...
    private talkToEmberAboutDragonzzz(): void {
        ...
        if (choice == 0) {
            ...
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 1)
                this.outputText(
                    ", a blush creeping onto " + this.emberMF("his", "her") + " cheeks"
                );
            this.outputText(".");
            this.outputText(
                "\n\nYou ask if " +
    ...
    private discussExplorationWithEmber(): void {
        ...
        if (choice == 0) {
            ...
            if (subChoice == 0)
                this.outputText(
                    "imps. Only a few imps, and they seemed very nervous. I'm guessing that whatever naturally lives there is something that they really don't want to get involved with."
                );
            else if (subChoice == 1)
                this.outputText(
                    "two goblins in the foothills, arguing with each other. One was saying that the other shouldn't go up into the hills, as apparently the minotaurs that live there are too much for an untrained girl like her to take without being split open. The second goblin just laughed at her, called her a 'wimp' and told her that she's going to get herself 'some juicy bull-cock and tasty mino-spooge'. Ugh, disgusting little creatures."
                );
            else if (subChoice == 2)
                this.outputText(
                    "a pair of muscle-bound bull-men beating on each other with their bare fists. They spent over an hour smashing each other into a bloody pulp, and then the winner promptly started fucking the loser up the ass.  I had seen more than enough by that point, so I left."
                );
            else if (subChoice == 3)
                this.outputText(
                    "this... creature... that looked kind of like a human woman, but with a big dick where her clit should be.  She was walking around stark naked, 'cept for a bunch of piercings, and leading this bull-man along like a pet by a chain attached to a ring anchored into his cockhead."
                );
            else if (subChoice == 4)
                this.outputText(
                    "a couple of goblins sharpening scissors on some rocks outside of a cave with a door on it. Weird. Wonder what they could be doing in there?"
                );
            else if (
                ...
                this.outputText(
                    "a horrible swarm of slimy white worms, clumped together into a mockery of a human form and squelching along.  It managed to latch onto this two-headed dog-creature and... ugh!  The worms started forcing their way into both of its cocks!  I've never seen anything so disgusting!"
                );
            else if (subChoice == 5)
                this.outputText(
                    "this two-headed dog-morph loping around; it spotted an imp, dropped to all fours, then gave chase. Managed to catch the ugly little demon, whereupon it ass-raped it, then ate it."
                );
        } else if (choice == 1) {
            ...
            if (subChoice == 0)
                this.outputText(
                    "a whole tribe of imps, just lounging around in a glade, jerking themselves off or squabbling over food.  Nasty little things, but easily dispatched."
                );
            else if (subChoice == 1)
                this.outputText(
                    "a  goblin with a huge pregnant belly, laughing to herself and swilling down that ale they brew, slopping it all over herself.  Little hedonists."
                );
            else if (subChoice == 2)
                this.outputText(
                    "this strange bee-woman creature... she made this, this music that started messing with my head.  I spat a tongue of flames at her and she flew away in fright, luckily."
                );
            //(If player has impregnated Tamani)
            else if (subChoice == 3 && this.flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] > 0) {
                this.outputText(
                    "that green-skinned baby-making whore, Tamani.  She was letting some of her daughters suckle from her and grinning ear to ear as she named the 'prize catch' she got to father them, exhorting them to hunt him down."
                );
                if (this.flags[kFLAGS.EMBER_GENDER] >= 2)
                    this.outputText(
                        "  You should have more pride than to let some brainless cunt like that have her way with you!"
                    );
            }
            //(If player has not yet impregnated Tamani)
            else if (subChoice == 3 && !kGAMECLASS.forest.tamaniScene.pregnancy.isPregnant)
                this.outputText(
                    "one goblin being teased by a bunch of pregnant goblins for not being pregnant yet.  She just spat back that she wanted a 'better catch' to be her baby-maker than a mere imp and wandered off."
                );
            //(If Jojo isn't in the camp & not corrupt)
                ...
                this.outputText(
                    "this mouse-morph monk, sitting in a glade and meditating. A goblin tried to proposition him; he just gave her a lecture and sent her running away in tears.  When an imp tried to attack him, he crushed its skull with a staff he had.  Not bad moves for such a weedy little thing..."
                );
            else
                this.outputText(
                    "one glade I touched down in to catch myself a nice brace of plump coneys, when all of a sudden this... this thing made out of flailing vines and fruit attacks me.  It went up in a puff of smoke once I torched it, of course."
                );
        } else if (choice == 2) {
            ...
            if (subChoice == 0) {
                this.outputText(
                    "a pair of shark-women - well, one was a woman, the other had breasts but also had a cock"
                );
                ...
                if (
                    ...
                    this.outputText(
                        " a big farm on the shoreline.  I saw some sort of cow-woman tending to fields of peppers, and a dog-woman herding cattle. They looked pretty fat and tasty, too... What?  I meant the cattle, stupid!  And I wouldn't have taken them - it's no fun snatching up livestock.  I prefer to chase down a deer or a wild auroch or something like that."
                    );
                //(if Fetish Cult is encounterable)
                else if (
                    EmberScene.rand(3) == 0 &&
                    this.player.findStatusAffect(StatusAffects.FetishOn) >= 0
                )
                    this.outputText(
                        "a pair of humans, arguing about sex.  They both wanted it, but the guy said he wanted to do the 'naughty schoolboy and female teacher' routine, while the girl wanted to do the 'noblewoman and love-servant' routine.  Weird; their clothes kept changing back and forth as they argued."
                    );
                //(If Beautiful Sword has not been acquired)
                    ...
                    this.outputText(
                        "a sword jammed into a tree.  Weird; what kind of idiot would stick a weapon there like that?  And what kind of weakling wouldn't be able to take it out?"
                    );
                else if (
                    EmberScene.rand(3) == 0 &&
                    ...
                    this.outputText(
                        "a smelly rat-man moping around while some weird equipment bubbled and boiled.  I think maybe he was an alchemist."
                    );
                else
                    this.outputText(
                        "a great blob of green goo, sliding along and minding its own business.  I could swear it looked up at me once, and grew a penis... that can't be right, though."
                    );
            }
        } else if (choice == 3) {
            ...
            if (subChoice == 0)
                this.outputText(
                    "a woman with four big breasts, squeezing milk out of her tits and into the sand.  I didn't know breasts could hold that much milk!"
                );
            else if (subChoice == 1)
                this.outputText(
                    "a whole tribe of demons, lounging around an oasis.  Would have been too much bother to kick the crap out of them, so I left them alone - well, alright, I did buzz them to make them scatter like scared sheep for fun."
                );
            //(if player hasn't solved Marcus & Lucia's argument)
            else if (
                EmberScene.rand(2) == 0 &&
                this.player.findStatusAffect(StatusAffects.WandererDemon) < 0 &&
                this.player.findStatusAffect(StatusAffects.WandererHuman) < 0
            )
                this.outputText(
                    "a human with balls so big he had to carry them in a wheelbarrow, trundling through the wasteland with a succubus.  They were arguing about whether or not he should become an incubus."
                );
            else {
                this.outputText(
                    "this strange creature, like a woman with a snake's tail for legs, slithering through the sand.  "
                );
    ...
    private talkToEmberAboutYourself(): void {
        ...
        if (this.player.str < 50)
            this.outputText(
                '  "<i>You could use more training; you look pretty weak...</i>" Ember says, critically.'
            );
        //(If PC Str < 90)
        else if (this.player.str < 90) {
            this.outputText(
                "  \"<i>You're well on your way to being as strong as a dragon, but there's still room for improvement.</i>\" Ember says, pleased."
            );
            //(+1 point)
            points++;
        }
        //(If PC Str >= 90)
            ...
            this.outputText(
                '  "<i>With muscles like these, you could easily lift anything you wanted... even me...</i>" Ember trails off dreamily, almost purring.  "<i>N-Not that I\'d want you to carry me in your arms or anything...</i>"'
            );
            //(+2 points)
            points += 2;
        }
        ...
        if (this.player.tou < 50)
            this.outputText(
                '"<i>I don\'t know how you could hope to survive out there.  You look like the wind could blow you away.</i>"'
            );
        //(If PC Tou < 90)
        else if (this.player.tou < 90) {
            this.outputText(
                '"<i>Not bad; some more training and you could be as tough as me!</i>" Ember says, pleased.'
            );
            //(+1 point)
            points++;
        }
        ...
        if (this.player.inte < 50)
            this.outputText(
                '"<i>You\'re still very naive... anyone or anything could trick you at any time...</i>"  Ember frowns.'
            );
        //(If PC Int < 90)
        else if (this.player.inte < 90) {
            this.outputText(
                '"<i>I see wisdom in your eyes, but you could always use more lessons.</i>"'
            );
            //(+1 point)
            points++;
        }
        ...
        if (this.player.spe < 50)
            this.outputText(
                "You try to block the pebble, but you're not quick enough.  It hits your belly, not hard enough to hurt.  \"<i>You need to work on your reactions; anything could surprise you out there and you'd be helpless.</i>\""
            );
        //(If PC Spd < 90)
        else if (this.player.spe < 90) {
            this.outputText(
                'You successfully deflect the pebble.  "<i>Not bad!  Next time try to catch it,</i>"  Ember says, pleased.'
            );
            //(+1 point)
            points++;
        }
        ...
        if (
            this.player.nagaScore() >= 3 ||
            this.player.lizardScore() >= 3 ||
        ...
        if (points < 2) {
            this.outputText(
                "\"<i>This is not good at all; it's a miracle you even managed to survive in this world thus far.  With the things I've seen roaming the land...</i>\" Ember trails off, concerned.  \"<i>You should stay in the camp; I'll help you train.</i>\"  You don't much like the assessment."
            );
        }
        //(else if points < 6)
        else if (points < 6) {
    ...
    private manEmberBitchesAboutPCPregnancy(): void {
        ...
        //(Moderate Affection)
            ...
            this.outputText(
                'Ember stares coldly at your gravid midriff as icebreaker after icebreaker from you falls flat.  You ask what\'s wrong.  "<i>Nothing.  Nothing is wrong,</i>" Ember states, flatly.'
            );
        //(High Affection)
        else
            this.outputText(
    ...
    private emberIsPregnantFirstTimeTalkScene(): void {
        ...
        this.outputText(
            "You can't help but stare at Ember's swollen belly; it's still so hard to take in that you have actually fathered a child with a creature of legend.  Especially given that there are times when it's hard to be entirely certain that Ember genuinely likes you."
        );
        this.outputText("\n\nEmber catches you staring");
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 1) this.outputText(" and blushes");
        this.outputText(
            ', one of her hands rubbing her belly lovingly and tenderly.  "<i>W-what is it?  Why are you looking at me like that?  Something wrong?</i>"'
        );
        ...
        if (this.player.cor >= 66)
            this.outputText(
                "  You almost choke on those cheesy words; still, she'll never put out for you if you don't butter her up."
            );
        this.outputText(
        ...
        if (this.player.cor >= 66)
            this.outputText(
                "  Is she seriously buying this nonsense?  Your smile would be miles wide if you let your lip curl even a bit, so you keep a straight face with due dilligence."
            );
        this.outputText(
        ...
        if (this.player.cor < 30)
            this.outputText(
                "  Given her attitude and nature, you know how lucky you are to have someone like her want to be your mate."
            );
    ...
    public emberBitchesAtYouAboutLustiness(): void {
        ...
        if (this.player.hasCock()) {
            this.outputText(
                "your " + EmberScene.num2Text(this.player.cockTotal()) + " stiff as iron"
            );
    ...
    private emberTFs(): void {
        ...
        if (
            changes < changeLimit &&
            this.player.dragonCocks() < this.player.totalCocks() &&
            EmberScene.rand(3) == 0
        ) {
            this.temp = 0;
            var choices: any[] = [];
            ...
            while (this.temp > 0) {
                this.temp--;
                if (this.player.cocks[this.temp].cockType != CockTypesEnum.DRAGON)
            ...
            if (this.player.sens >= 50)
                this.outputText(
                    "however, it's not until you press on your new, sensitive knot that you manage to blow your load and enjoy the last few spasms of pleasure as it finally finishes changing."
                );
            else
                this.outputText(
                    "but you sternly rein in your hands and tuck them into your armpits as the arousing changes run their course."
                );
        ...
        if (this.player.dragonScore() >= 4 && EmberScene.rand(3) == 0 && this.player.gender > 0) {
            ...
            if (this.player.hasCock() && this.flags[kFLAGS.EMBER_GENDER] >= 2) {
                ...
                if (this.player.hasVagina() && this.flags[kFLAGS.EMBER_GENDER] == 3)
    ...
    private emberIsAnEggFactory(): void {
        ...
        if (this.emberAffection() <= 25) {
            ...
            this.outputText(
                ".  One of her hands gently teases her clit, as another traces her outer labia; she bites her lips, trying to stifle her moans of pleasure, but it's useless... every touch brings a sigh."
            );
            this.outputText(
        ...
        //(Medium Affection)
            ...
            this.outputText(
                '.  "<i>T-This...</i>"  She can\'t quite bring herself to finish the sentence and settles for just looking at the ground.'
            );
            this.outputText(
        ...
        //(High Affection)
            ...
            this.outputText(
                " and then lean forward to give her a great, wet, sloppy lick, straight up the center of her pussy"
            );
            //(E.Herm:
            if (this.flags[kFLAGS.EMBER_GENDER] == 3)
                this.outputText(
                    " not stopping until you have slurped your way to the very tip of her cock"
                );
            ...
            this.outputText(
                "caressing and stroking and playing all the tricks you can possibly think of to orally pleasure your draconic lover.  From the amount of juices beginning to seep onto your lapping tongue"
            );
            if (this.flags[kFLAGS.EMBER_GENDER] == 3)
            ...
            if (this.player.tongueType > TONUGE_HUMAN)
                this.outputText(
                    "  Right at this time, you feel something round and smooth on the tip of your tongue, gently spreading Ember's walls.  Realizing that this can only be her egg, you start trying to worm your long, sinuous tongue between it and her innermost walls, hoping to coax it out of her."
                );
            this.outputText(
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 3)
                this.outputText(
                    "  Her cock throbs and pumps out long streams of cum to paint the ground around you two; marking it as your special place."
                );
            if (this.player.tongueType > TONUGE_HUMAN)
                this.outputText(
    ...
    private dontWatchEmberLayEgg(): void {
        this.clearOutput();
        this.outputText(
            "You take her hand and tell her that you wouldn't dream of intruding on her privacy, but ask her to think of you if she needs the inspiration.  She looks away shyly, and the barest hint of a smile breaks on her face.  Seems like she's already following your instructions."
        );
        this.outputText(
        ...
        this.outputText(
            "  \"<i>H-here's your egg.  Use it while it's fresh, okay?</i>\"  Her eyes glaze over a bit at the suggestion, and she giggles.  "
        );
    ...
    private watchMediumAffectionEmberEggLay(): void {
        this.clearOutput();
        this.outputText(
            'Ember fails to hide her arousal when you accept.  "<i>Okay, then follow me.</i>"  The two of you move into a secluded spot.  Once she is certain nobody is around to spy, Ember turns to face you'
        );
        //(if Ember has a cock:
        ...
        this.outputText(
            ".  \"<i>Okay... but you're only allowed to watch!  If you do anything weird, I swear I'll smack you!</i>\""
        );
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 3)
            this.outputText(
                "  Ropes of jism spew out of her cock, arching into the air to hit the ground nearby."
            );
        this.outputText(
            "  The egg plugs her wet pleasure hole, preventing any liquid from escaping, until finally with a wet, squelching pop,it flies out of her pussy and into your waiting hands; releasing a flood of juices."
        );
        this.outputText(
        ...
        this.outputText(
            ", it's hard to see her as the same angrily defensive ice queen from before."
        );
        this.outputText(
    ...
    private getMilkFromEmber(): void {
        ...
        this.outputText(
            "You think for a few moments, then find your gaze drawn to Ember's round, firm"
        );
        ...
        if (this.emberAffection() <= 25) {
            ...
            if (this.player.findStatusAffect(StatusAffects.DragonBreathCooldown) >= 0) {
                ...
                this.outputText(
                    "  Your throat feels soothed as the scratching and soreness die down; you feel like you could shout to the mountaintops!"
                );
            }
            ...
            if (
                ...
                (this.player.gender != this.flags[kFLAGS.EMBER_GENDER] ||
                    (this.player.gender == 3 && this.flags[kFLAGS.EMBER_GENDER] == 3))
            ) {
        ...
        //(High Affection)
            ...
            if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) {
                ...
                this.outputText(
                    "her gently-dripping cunt, sliding in between the lips to stroke the wet interior"
                );
            this.outputText(".");
            ...
            this.outputText(
                "  You tell your dragon that you're finished.  You're up and turning to leave when a looping tail around your waist stops you."
            );
    ...
    private catchAnal(): void {
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 1)
            this.outputText("draconic ");
        this.outputText(
            "prick in and out, reaching for your deepest recesses to rub on the right spots."
        );
        ...
        if (this.player.hasVagina()) {
            this.outputText(
                "  The stimulus is starting to make you wet as well, your [vagina] drooling feminine lubricant "
            );
            if (this.player.balls > 0) this.outputText("over your [balls]");
            this.outputText(" to puddle underneath you");
            if (this.player.hasCock())
                this.outputText(
                    ", mingling with your pre to form an ever-growing pool of mixed sexual fluids."
                );
        }
        ...
        this.outputText(
            "belly, and start to rise and sink, pistoning your ass up and down. You have total control over the sex, and you intend to use that."
        );
    ...
    private suckEmberCock(): void {
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 || this.flags[kFLAGS.EMBER_INTERNAL_DICK] == 1)
            this.outputText("draconic ");
        this.outputText(
            "prick already hard and throbbing.  The dragon taps a clawed finger idly on the stump, awaiting your attention."
        );
        ...
        if (this.player.hasCock())
            this.outputText(
                "  [EachCock] hardens, spearing aimlessly into the ground in your arousal."
            );
        if (this.player.hasVagina())
            this.outputText(
                "  Your [vagina] starts to seep with feminine juices, your [clit] hardening in anticipation as your excitement dribbles down onto the thirsty ground."
            );
        ...
        if (
            ...
            this.outputText(
                " that is even now springing forth from " + this.emberMF("his", "her") + " "
            );
        ...
        this.outputText(
            ", and discover a surge of pride in you that you got all sixteen inches of it down."
        );
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 3)
            this.outputText(
                ", your tongue wriggling around in " + this.emberMF("his", "her") + " slit"
            );
        this.outputText(
        ...
        if (this.player.cor < 80) this.outputText("amused");
            ...
            this.outputText(
                "increasingly leery of " + this.emberMF("his", "her") + " batty behavior"
            );
    ...
    private stickDickInKnifeDrawer(): void {
        ...
        if (!this.player.isNaga())
            ...
            this.outputText(
                "coil up on your serpentine tail) and start peeling off the clothes on your lower half, exposing [eachCock] to the air and awaiting Ember's response."
            );
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2)
            this.outputText(
                "  Looking behind her, you see a small pool of her feminine juices has formed; pheromones fly through the air and hit you like a wave, spurring you on, making you feed Ember more of your delicious pre."
            );
        ...
        if (this.player.cumQ() < 250) {
            ...
            this.outputText(
                " into your ass to tickle your prostate, drawing a few last drops out of you."
            );
        }
        //(Medium Cum Amount)
        else if (this.player.cumQ() < 1000) {
    ...
    private slurpDraggieCunnies(): void {
        ...
        this.outputText(
            "You contemplate the possibilities, and then offer to get down on your knees before the dragon and pleasure her orally.  You would love to have a taste of some dragon juice."
        );
        ...
        if (this.emberAffection() <= 25) {
            ...
            this.outputText(
                " just because you're offering to do this,</i>\" the dragon hastily adds."
            );
        }
        //(Medium Affection)
        else if (this.emberAffection() < 75) {
        ...
        //(High Affection)
            ...
            this.outputText(
                '.  "<i>Ok... I suppose I would like that,</i>" Ember replies, trying to calm herself down, lest she seem too excited by the idea.'
            );
        }
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3)
            this.outputText(
                "  With Ember's cock jutting out above your head, you can't resist sliding your tongue along the underside of her shaft, just for an extra teasing motion."
            );
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3)
            this.outputText(
                "  You think droplets of pre are starting to slide down her girl-cock and drip onto your head... but that's part and parcel of loving a herm, so you pay it no attention."
            );
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3)
            this.outputText(
                "  Her throbbing shaft, not intending to be left out of the fun, records the event by marking the tree bark with white dragon spooge."
            );
    ...
    private penetrateEmbrah(): void {
        ...
        this.outputText(
            "Your eyes are drawn to the dragon's crotch, and you ask if she has any feminine itches she'd like scratched."
        );
        ...
        if (this.flags[kFLAGS.EMBER_PUSSY_FUCK_COUNT] == 0) {
            ...
            if (this.player.cor > 75)
                this.outputText(
                    "  As if bitches like her could qualify as maidens in any but the most literal sense."
                );
            this.outputText(
        ...
        if (this.player.cor < 50)
            this.outputText(
                "hasten to undress yourself for the impatient dragoness, quickly pulling"
            );
        else
            this.outputText(
                "languidly denude yourself, making sure to exaggerate every movement as you pull"
            );
        this.outputText(
        ...
        if (this.player.cockThatFits(this.emberVaginalCapacity()) == -1) {
            ...
            this.outputText(
                " you'll tear me apart!</i>\"  Ember finishes by exhaling a puff of smoke in frustration... clearly she wants this as much as you."
            );
            ...
            this.outputText(
                ' and then come back!</i>"  Ember turns on her heels and walks away, moodier than usual.'
            );
            this.doNext(this.camp.returnToCampUseOneHour);
            return;
        }
        ...
        this.outputText(
            "gently lay yourself down, spreading your limbs and making it clear you're ready for whatever she has in mind."
        );
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) {
            ...
            this.outputText(
                " prick slaps wetly against your belly with each thrust, smearing pre on you as the engorged tool bounces back and forth"
            );
        }
        this.outputText(".");
        ...
        this.outputText(
            "  With a surge of lust-inflamed strength, you thrust yourself forward, knocking the unprepared Ember over.  You roll her over so that you are on top and spread her legs, draping them casually around your hips and then resume thrusting with ever-increasing force.  Like some sex-starved animal you thrust and rut, pounding the now-helpless dragon raw in pursuit of the tantalizing imminent release that haunts you."
        );
        ...
        this.outputText(
            " from her used cunt and you sit on the ground, while Ember remains limp on the ground, panting. Still suffering from the afterglow of your recent orgasm, you dimly realize that Ember herself doesn't seem to have cum yet; the only fluids dripping from her pussy are yours"
        );
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3)
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 3)
            this.outputText(
                "; her cock jumps and unloads on the ground below, marking this as the spot of your lovemaking"
            );
        this.outputText(
            ".  Her pussy grabs at you like a vice, constricting you with such pressure that you have trouble moving, even though her cunt is well-lubed from your mixed fluids."
        );
        ...
        //(High Cum Amount)
            ...
            this.outputText(
                "finished, Ember doesn't only look more than a few months pregnant; she looks absolutely huge, like her water could break at any moment!"
            );
        }
        this.outputText(
    ...
    private penetrateEmbrahPartII(): void {
        this.clearOutput();
        this.outputText(
            "You wake up to find Ember's face hovering over you with a smile; once she realizes you're awake, she quickly averts her gaze."
        );
        this.outputText(
    ...
    private getPenetratedByEmberLastSexSceneWoooo(): void {
        ...
        //else
            ...
            this.outputText(
                "  It's not as if it isn't still warm... but its lower temperature feels relieving and exciting all the same."
            );
        }
        this.outputText(
        ...
        if (
            ...
            this.outputText(
                "  Using the skills you've honed, you make the muscles in your vagina ripple and wrinkle, teasing the cock caught inside you in a way few women can."
            );
        this.outputText(
            "  Mmm... you can feel your own second orgasm coming in hot.  Maybe after another 4 or 5, you'll let the dragon go..."
        );
    ...
    private getPenetratedByEmberLastSexSceneWooooPartII(): void {
        ...
        if (this.emberAffection() < 75)
            this.outputText(
                this.emberMF("his", "her") + " cock was unlike anything else in this world?"
            );
        else
            this.outputText(
                this.emberMF("he", "she") +
    ...
    public emberRapesYourHeatness(): void {
        ...
        if (
            ...
            this.outputText(
                "your [vagina] aching to be filled and your womb put to its proper purpose"
            );
        ...
        this.outputText(
            '  "<i>I need you, [name].  I need you so badly... can you see how badly I need you?</i>" Ember asks, panting in barely contained lust.  "<i>I want to fuck you so badly... Let\'s make a baby now!</i>"'
        );
        this.dynStats("lus", 10 + this.player.lib / 10);
        this.outputText("\n\nWhat do you say?");
        //[Accept] [Deny]
    ...
    private getKnockedUpByEmbrahBroBaby(): void {
        ...
        this.outputText(
            "Your contracting walls milk the dragon's sensitive cock, trying to coax even more seed out of your panting dragon lover.  \"<i>I-it's time,</i>\" Ember mutters, crashing into your wet bedroll and thrusting deep inside you."
        );
        ...
        this.outputText(
            'face.  "<i>But if you must know, I do love you.</i>"   With a final sigh Ember sets down on your bedroll for a quick nap.'
        );
    ...
    private breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding(): void {
        ...
        if (this.player.hasVagina())
            this.outputText(
                "  The tip of her tail gently teases your slick [vagina] with tiny strokes."
            );
        this.outputText(
            "  You shudder and instinctively thrust towards her, letting her feel your needy cock, full and aching to father young."
        );
        ...
        this.outputText(
            "Ember's pleased moans are enough to make you push away such thoughts, you're glad she has those!"
        );
        ...
        this.outputText(
            "face, nipping lightly at her lip, rolling your tongue over hers and savoring her inhuman tastes; Ember answers in kind with a kiss of her own, purring and moaning into your mouth.  Her vibrating breasts massage your hands as you knead, lift, and then roll them about."
        );
        ...
        if (this.emberHasCock()) {
            ...
            if (
                ...
                this.outputText("dragon ");
            this.outputText(
                "prick.  With a mischievous curl of your lips, you pull Ember deeper into your kiss and grab her cock, stroking it vigorously."
            );
        }
        ...
        this.outputText(
            "her eyes roll to the back of her head.  With a muffled roar she finally caves, a veritable wave of dragon juices crashing into your lower body and pooling on your bedroll, filling the whole tent with the delicious smell of sex and pheromones."
        );

        //(If Ember has a dick)
        if (this.emberHasCock())
            this.outputText(
                "  Ember's cock wets the side of the tent with continuous jets of cum, only adding to the already overpowering smell of sex as she empties herself on top of you."
            );
        ...
        this.outputText(
            "her eyes shut tight in pleasure as she reaches her second orgasm.  You muffle her groan of pleasure with one of your own.  A second wave of dragon fluids joins the first in wetting your lower body, along with your bedroll."
        );
        if (this.emberHasCock())
            this.outputText(
                "  Little gobs of cum spill from her cock to slide down her shaft and gather on your belly button."
            );
        this.outputText(
            "  Her tail coils around you, helping her stay in position as she melts on top of you like putty."
        );
        ...
        if (this.player.cumQ() < 250)
            this.outputText(
                "  Though you usually don't cum that much, Ember's contracting walls literally suck the cum out of you; forcing you to give up more than you usually would... something you're quite glad to do!"
            );
        //(Medium Cum Amount)
        else if (this.player.cumQ() < 1000)
            this.outputText(
        ...
        //(High Cum Amount)
            ...
            this.outputText(
                "  You dump a huge load of jism into Ember's awaiting belly, but her contracting walls seem to milk you for ever more!  Ember is filled to the brim, her belly already slightly distended, but you still continue to fill her with your seed.  Ember's tightly sealed pussy lips means the cum has nowhere to go, so instead it gathers in her womb, distending her belly until she looks at least a few months pregnant; and by the look of pleasure on Ember's panting face, you don't think she minds it."
            );
        ...
        if (this.emberHasCock())
            this.outputText(
                "dismissive of the spent cock lying sandwiched between you as you start "
            );
    ...
    private emberBreedingAfterMathWatchOutForRadioactiveFallout(
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] == 0)
            this.outputText(
                " are probably blushing under " + this.emberMF("his", "her") + " scales."
            );
        ...
        if (!emberPregged)
            this.outputText(
                "  You touch your belly with a smirk.  Very certain they took indeed..."
            );
        this.doNext(this.playerMenu);
    }
    ...
    private emberPregUpdate(): boolean {
        ...
        switch (this.pregnancy.eventTriggered()) {
            ...
            case 4:
                ...
                if (this.flags[kFLAGS.EMBER_OVIPOSITION] > 0) {
                    this.outputText(
                        'Ember notices you looking.  "<i>W-what? Never seen a pregnant woman before?</i>" she asks indignantly, although she can\'t hide her smile as you continue to look.\n'
                    );
                } else {
                    this.outputText(
            ...
            case 8:
                if (this.flags[kFLAGS.EMBER_OVIPOSITION] > 0) {
                    ...
                    if (this.flags[kFLAGS.EMBER_MILK] > 0) {
                        ...
                        this.outputText(
                            "Soon, you've exhausted one of ther breasts, then you move to the other intent on doing the same; however all too soon she's drained and you're left wanting more.\n\n\"<i>Ahhh, that feels much better.  Good job,</i>\" she comments.  You smile back, then return to your duties.\n"
                        );
                    }
                } else {
                    this.outputText(
    ...
    public emberGivesBirth(): void {
        ...
        if (this.flags[kFLAGS.EMBER_OVIPOSITION] == 0) {
            ...
            this.outputText(
                " as her womb works to expel its long-time occupant.  You place a hand on either side of the distended orb and begin to massage it, trying to help soothe the strained, aching muscles.  Your eyes go from her stomach to her crotch; her vagina is opened wide as the infant inside her begins making its way into the world."
            );
            if (this.emberHasCock()) {
                if (
                    this.flags[kFLAGS.EMBER_ROUNDFACE] == 0 &&
                    ...
                    this.outputText(
                        "  Her human-like cock dangles heavily in front of her pussy, partially erect from the muscular spasms."
                    );
                else
                    this.outputText(
                        "  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect."
                    );
            }
            ...
            this.outputText(
                " in your arms, uncaring of the stains as the fluid drenching its little body soaks into your [armor], gently soothing "
            );
            if (roll < 40) this.outputText("him");
            else this.outputText("her");
            this.outputText(
                ".  Then, seeing Ember is recovering from her recent exertions, you grin proudly at her and gently hold your "
            );
            ...
            if (this.emberChildren() > 1)
                this.outputText(
                    "  Your other draconic offspring quietly peek into the lair, having vacated the place when Ember went into labor.  They smile at the sight of their new sibling, gently slipping in to curl up beside the mother and new child."
                );
            this.outputText(
                "  With a contented expression on your face, you leave Ember and your "
            );
        ...
        //Ember Lays Egg
            ...
            this.outputText(
                " as her womb works to expel its long-time occupant.  You place a hand on either side of the distended orb and begin to massage it, trying to help soothe the strained, aching muscles.  Your eyes go from her stomach to her crotch; her vagina is opened wide as the globular egg inside her begins dropping its way into the world."
            );
            if (this.emberHasCock()) {
                if (
                    this.flags[kFLAGS.EMBER_ROUNDFACE] > 0 &&
                    ...
                    this.outputText(
                        "  Her human cock dangles heavily in front of her pussy, partially erect from the muscular spasms."
                    );
                else
                    this.outputText(
                        "  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect."
            ...
            if (this.flags[kFLAGS.EMBER_EGGS] > 0) {
                ...
                if (this.flags[kFLAGS.EMBER_EGGS] == 1) this.outputText("other");
                    ...
                    this.outputText(EmberScene.num2Text(this.flags[kFLAGS.EMBER_EGGS]) + " others");
            }
            this.outputText(
    ...
    public giveBirthToEmberKids(): void {
        ...
        if (this.flags[kFLAGS.EMBER_OVIPOSITION] == 0) {
            ...
            if (!this.player.hasVagina()) {
                this.outputText(
                    "A terribly painful ripping feeling comes from your crotch. Reaching down to touch the tender spot you feel a spike of pleasure and moistness.  <b>You've grown a vagina!</b>\n\n"
                );
            ...
            this.outputText(
                "on the boundary between pleasure and pain.  You aren't sure how much more you can take."
            );
            ...
            this.outputText(
                " was getting hungry and I didn't know what to do,</i>\" Ember explains."
            );
            if (this.flags[kFLAGS.EMBER_MILK] > 0) {
            ...
            this.outputText(
                " coos, giggles and nuzzles into your neck, clearly happy to be here in the real world at last."
            );
        ...
        //PC Lays Egg
            ...
            this.outputText(
                "scurries away, even as you pull yourself upright and get ready to go about your business.\n"
            );
            this.flags[kFLAGS.EMBER_EGGS]++;
        }
    ...
    private highAffectionEmberLustFuck(): void {
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1) {
            ...
            this.outputText(
                ' moans throatily and gives you a rapturous look.  "<i>I love you, [name]... gods, I love you so much, and I\'m so happy I can finally say that to your face,</i>" she tells you.'
            );
        ...
        //else if Ember has a pussy and the PC more than one cock:
            ...
            this.outputText(
                " of those?  I-I know it's not exactly part of the pose and all, but...</i>\""
            );
            ...
            this.outputText(
                ", making it obvious she's ready.  \"<i>Oh, my mate... Please, fill me now!  Quit teasing me and just shove your cocks up your dragon's ready holes, jam them in as far as they can fit!</i>\" she pleads, but unable to meet your eyes in her embarrassment at her dirty talk."
            );
            ...
            this.outputText(
                "your cocks enveloped in slick tightness and warmth is almost enough to make you fill her up with your seed right then and there, but somehow you manage to hold on."
            );
            ...
            this.outputText(
                ' moans throatily and gives you a rapturous look.  "<i>I love you, [name]... gods, I love you so much, and I\'m so happy I can finally say that to your face,</i>" she tells you.  Then she starts looking less in love and more in lust.  "<i>But I also just love these cocks; these two wonderful breeding rods - come on, mate!  Breed your horny dragon!</i>" she commands lustfully.'
            );
        }
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 1 && this.player.cockTotal() > 1)
            this.outputText("s");
        this.outputText(
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1)
            this.outputText(
                "shaft as his cock spurts dragon-seed onto the leaves below him, filling the air with the scent of spunk and matting them into a steaming morass."
            );
        //{twin shafts} as {her cunt spasms wetly, drenching your lap with fresh femcum} {and/or} {[her] cock spurts dragon-seed onto the leaves below {her}, filling the air with the scent of spunk and matting them into a steaming morass}
            ...
            if (this.player.cockTotal() == 1)
                this.outputText(
                    "shaft as her cunt spasms wetly, drenching your lap with fresh femcum"
                );
            else
                this.outputText(
                    "twin shafts as her cunt spasms wetly, drenching your lap with fresh femcum"
                );
            if (this.flags[kFLAGS.EMBER_GENDER] == 3)
                this.outputText(
                    " and her cock spurts dragon seed onto the leaves below her, filling the air with the scent of spunk and matting them into a steaming morass"
                );
            this.outputText(".");
        }
        this.outputText(
        ...
        if (this.player.cockTotal() > 1 && this.flags[kFLAGS.EMBER_GENDER] >= 2)
            this.outputText("s");
        this.outputText(
            " tenderly yet firmly, stroking you slowly.  \"<i>I can't believe how hot I get when I see you sporting "
        );
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2)
            this.outputText(
                ", moisture drenching the cock buried in her cunt as it slobbers greedily across your burning flesh."
            );
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3)
            this.outputText(
                "  " +
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2)
            this.outputText(
                "her cunt gushing female ejaculate, almost soaking you with the cascade of feminine fluids and puddling wetly against your [feet]"
            );
        this.outputText(".");
    ...
    private stayWithEmberAfterLustFuck(): void {
        ...
        //else if Ember is female:
            ...
            this.outputText(
                ' takes your hand and presses it against her wet quim.  Your fingers instinctively move, sliding themselves over and slightly into the damp netherlips.  "<i>Ooh... that feels nice... but know what would feel even better?</i>" she asks teasingly as she strokes your side.  Playfully, you ask her what that might be.'
            );
            ...
            if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0) this.outputText("-girl");
            this.outputText(
                " states, idly rubbing your cocks together.  One of your hands slips down between the two of you, sliding over the ridged surface of the herm dragon's cock before pressing itself against her well-used quim; sure enough, it's already drooling eagerly at the prospect of being filled again."
            );
    ...
    private frottingWithFrottingEmberHerm(): void {
        this.clearOutput();
        this.outputText(
            "You think the matter over, and then slowly rub your [cock biggest] against Ember's to answer her question.  The dragon-herm gasps, then smiles lewdly at you."
        );
    ...
    private penetrateEmberHerm(): void {
        this.clearOutput();
        this.outputText(
            'You decide you\'d rather use her once more, so you finger her pussy once more.  "<i>Ooh... go ahead, I belong to you, my mate,</i>" she says, opening her legs slightly to give you better access.  You slide yourself around to properly position yourself at her entrance, and then hold yourself there, ready to begin.'
        );
    ...
    private frottingWithEmber(clear: boolean = true): void {
        ...
        if (this.player.gender == 1 && this.flags[kFLAGS.EMBER_GENDER] == 1)
            this.outputText(
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2)
            this.outputText(
                "  Her moist dragon pussy is not helping the mess in the least.  You can smell her femcum pooling along with her pre."
            );
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] >= 2)
            this.outputText(
                "  Her other hand frigs her pussy as she comes down from her orgasm, gushing female juices and leaking some of the cum you've pumped into her earlier."
            );
    ...
    private penetrateWithEmber(clear: boolean = true): void {
        ...
        this.outputText(
            " and promise you will, sliding slowly in until you have hilted yourself yet again."
        );
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3) {
            this.outputText(
                ", her draconic prick pressing tightly against you as it leaks a steady stream of pre, matting "
            );
        ...
        this.outputText(
            '  "<i>Do you like playing with my body?</i>"  Ember asks, gazing at you through half-lidded eyes.  You give her a playful smirk and run your fingers expertly over her nipples, making her moan and mewl at the stimulation and saying more than mere words could on the matter.'
        );
        ...
        if (this.flags[kFLAGS.EMBER_MILK] > 0)
            ...
            this.outputText(
                "I may not have any milk in my breasts right now, but maybe if you keep suckling I'll be able to make you some,"
            );
        this.outputText(
            '</i>" Ember teases you.  You smile at her and accept the nipple, rolling it between your lips and exerting gentle, teasing pressure with your teeth.'
        );
        if (this.flags[kFLAGS.EMBER_MILK] > 0)
            this.outputText(
                "  You savor the resultant gush of sweet dragon's milk as it squirts obediently down your throat."
            );
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 1 || this.flags[kFLAGS.EMBER_GENDER] == 3)
            this.outputText(
                "  A small dollop of pre escapes Ember's dragon-dick at the pleasure of the movement."
            );
        this.outputText(
            '  "<i>And don\'t forget about my other breast either.</i>"  She takes your hand in her own and guide it to her other mound, helping you knead it.'
        );
        ...
        if (this.flags[kFLAGS.EMBER_MILK] > 0)
            this.outputText(
                ", not even caring that she's tasting her own milk as you finish gulping it down"
            );
        this.outputText(
            ".  You kiss her back as eagerly as you can until lack of air forces you to break the kiss and catch your breath."
        );
        ...
        if (this.flags[kFLAGS.EMBER_ROUNDFACE] > 0) this.outputText("-girl");
        this.outputText(
            " roaring towards the ceiling, then turning to look at you as her pussy constricts and milks you for all you're worth.  Already spent from your past three orgasms, you just sit back and enjoy yourself being milked by her strong vaginal muscles one last time."
        );
        if (this.player.cumQ() < 250)
            this.outputText(
        ...
        if (this.flags[kFLAGS.EMBER_GENDER] == 3)
            this.outputText(
                "  Her draconic cock throbs all the way through your orgasm, shooting blanks a few times before spurting a couple ropes of pre onto her belly."
            );
