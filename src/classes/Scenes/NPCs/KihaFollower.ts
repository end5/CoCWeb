...
export class KihaFollower extends NPCAwareContent {
    ...
    public kihaFriendlyGreeting(output: boolean = true): void {
        ...
        if (output && this.flags[kFLAGS.KIHA_TALK_STAGE] == 6 && this.player.cor <= 50) {
            ...
            this.outputText(
                "You walk over to the ring of trees and call out for the dragoness.",
                false
            );
    ...
    private lieThere(): void {
        ...
        this.outputText(
            '[pg]"<i>FUCK YOU ANYWAY!</i>"  Hel yells after her, fist clenched.',
            false
        );
        this.outputText(
    ...
    private lovingHugDickings(): void {
        ...
        if (this.player.cocks[this.player.biggestCockIndex()].cockLength >= 36)
            this.outputText(
                ", even though you bump her repeatedly in the nose with your moist tip"
            );
        this.outputText(
        ...
        if (
            ...
            this.outputText("Growling");
        else this.outputText("Grunting");
        this.outputText(
            ", you clench for a moment as climax works through your body, expelling ",
            false
        );
    ...
    private lovingHugsForRetards(): void {
        ...
        if (this.player.ass.analLooseness < 4)
            this.outputText("you're sure you'll gape for a little after this");
        ...
        // Hang Out 3
            ...
            if (!this.followerKiha()) {
                ...
                if (this.model.time.hours < 21)
                    ...
                    this.outputText(
                        "stars are out, a thousand little pin-pricks in the heavens",
                        false
                    );
                this.outputText(".");
            } else {
                this.outputText(
                    "Looking up from your dragoness lover, you note the particularly clear "
                );
                if (this.model.time.hours < 21)
                    this.outputText(
    ...
    private blechKihaYourCooking(): void {
        ...
        if (this.player.canFly())
            this.outputText(
                "fly off.  Scowling, Kiha takes to the air too, landing behind you outside"
            );
        else
            this.outputText(
    ...
    private boneTheShitOutofKihaHolesWithHorsecock(): void {
        ...
        this.outputText(
            "You remove your [armor] as you close in with Kiha, happy to let the ",
            false
        );
        ...
        if (this.player.cocks[x].cockType == CockTypesEnum.HORSE)
            this.outputText("bestial equine dong");
        ...
        this.outputText(
            " with one hand, using slow, even strokes that tickle every sensitive area.  "
        );
        if (this.player.cocks[x].cockType == CockTypesEnum.HORSE)
            this.outputText(
        ...
        if (this.player.inte < 20) {
            ...
            if (this.player.cocks[x].cockType == CockTypesEnum.HORSE)
                this.outputText("bestial whinny ");
        ...
        this.outputText(
            ".  Between your inner embarrassment and the pleasure shooting through your "
        );
        ...
        if (this.player.cocks[x].cockType == CockTypesEnum.HORSE)
            ...
            if (this.player.balls > 0)
                ...
                this.outputText(
                    "her free hand eagerly caressing your [balls] with inhuman warmth."
                );
        }
        this.outputText(
        ...
        if (this.player.cocks[x].cockType == CockTypesEnum.HORSE)
            this.outputText("big, pony-like prick.");
        else this.outputText("[cock " + y + "].");
        if (this.player.balls > 0)
            this.outputText(
        ...
        if (c <= 250)
            this.outputText("She manages to guzzle it all down without too much difficulty.");
        else if (c <= 1000)
            this.outputText(
        ...
        if (this.player.cocks[x].cockType.Index > 0) {
            if (this.player.isTaur())
                this.outputText("throwing your hooves onto the rock, looming over Kiha");
        ...
        if (this.player.cocks[x].cockType == CockTypesEnum.HORSE)
            this.outputText("wide head of your stallion-shaft");
        ...
        if (this.player.cocks[x].cockType == CockTypesEnum.HORSE)
            this.outputText("well-lubed horse-dick");
        else this.outputText(this.cockDescript(x));
        this.outputText(
        ...
        if (this.player.cocks[x].cockType == CockTypesEnum.HORSE)
            this.outputText("flat-tipped horsecock");
        else this.outputText(this.cockDescript(x));
        this.outputText(
        ...
        if (this.player.isTaur())
            this.outputText(
                "you push her onto her hands and knees atop the rock and re-mount her, and "
            );
        this.outputText(
        ...
        if (this.player.isTaur())
            this.outputText("[pg]You reach down and grab Kiha's tail, providing ");
        ...
        if (!this.player.isTaur())
            this.outputText(
                "You run a hand over her rump, stopping only to give it familiar slaps as y"
            );
        ...
        if (this.flags[kFLAGS.KIHA_HORSECOCK_FUCKED] == 0) {
            ...
            if (this.player.lib < 80 && this.player.minLust() < 50)
                this.outputText("Was it the lust draft?  Or");
    ...
    private kihaPlaysWithBigassCocksFemDomAhoy(): void {
        ...
        if (this.player.cocks[x].cockType == CockTypesEnum.HUMAN)
            this.outputText(
                ", particularly your medial ring and the sensitive edge of your flare."
            );
        else if (this.player.hasKnot(x))
            this.outputText(", particularly your massive, bulging canid knot.");
        else if (this.player.cocks[x].cockType == CockTypesEnum.DEMON)
            this.outputText(", particularly the hundreds of demonic nodules that ring your shaft.");
        else if (this.player.cocks[x].cockType == CockTypesEnum.ANEMONE)
            this.outputText(
                ", particularly the groups of aphrodisiac laced tentacles under your crown."
            );
        else if (this.player.cocks[x].cockType == CockTypesEnum.TENTACLE)
            this.outputText(", particularly the underside of your tentacle's mushroom-like tip.");
        ...
        if (this.player.balls > 0)
            this.outputText(
                "Your balls clench and relax in time with the waves of pleasure, and y"
            );
        else this.outputText("Y");
        this.outputText(
        ...
        if (this.player.cumQ() >= 1000)
            this.outputText(", effectively bukkaking her from the sheer volume of your ejaculate");
        this.outputText(
        ...
        if (this.flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS] > 0) {
            if (this.player.cor < 50)
                this.outputText(
                    "  You hope she doesn't get jumped upby your tiger-shark children."
                );
            else
                this.outputText(
        ...
        if (
            ...
            if (!this.jojoScene.tentacleJojo())
                this.outputText(
                    ", ignoring the sounds of Jojo feverishly masturbating in the woods"
                );
            else
                this.outputText(
    ...
    private kihaGirlGirlSex(): void {
        ...
        if (this.player.clitLength >= 10)
            this.outputText(", gasping when it grows to a ridiculous size.");
        else if (this.player.clitLength >= 3)
            this.outputText(", humming appreciably when it grows to cock-like length.");
    ...
    private savinTheAnalForKiha(): void {
        ...
        if (this.player.biggestTitSize() >= 3)
            this.outputText(" as she palms your " + this.biggestBreastSizeDescript(), false);
        this.outputText(
    ...
    private fuckKihasVagInCamp(): void {
        ...
        if (this.player.skinType != SKIN_TYPE_SCALES)
            this.outputText("leave long, red scratches on your back.");
        ...
        if (this.player.cockTotal() > 1)
            this.outputText("[pg]Kiha's belly is quickly glazed white with goo, and her ");
        ...
        if (this.player.cumQ() >= 1000)
            this.outputText("  The drippings form a nice big puddle under Kiha.");
        this.outputText(
    ...
    private fuckKihaWithATentacle(): void {
        ...
        if (y != -1) {
            ...
            if (this.player.cockArea(y - 1) < 24)
                this.outputText(" and your rod buried to the hilt within her");
            this.outputText(
    ...
    public pcWinsDomFight(): void {
        ...
        if (this.player.hasCock())
            this.outputText("pull your quickly hardening " + this.multiCockDescriptLight());
        else if (this.player.hasVagina())
            this.outputText("release your lathered up " + this.vaginaDescript(0));
        this.outputText(
        ...
        // Random event two of three: Footjobs and missionary, oh my!
            ...
            if (this.player.cockTotal() > 1) this.outputText("s");
            this.outputText(
                ".  Gazing downward, you come to realize that the dragoness has your prick"
            );
            if (this.player.cockTotal() > 1) this.outputText("s");
            this.outputText(
