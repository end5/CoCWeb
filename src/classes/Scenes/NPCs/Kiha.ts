...
export class Kiha extends Monster {
    ...
    private sillyModeKihaAttack(): void {
        ...
        if (this.player.spe - this.spe > 0 && Math.floor(Math.random() * (((this.player.spe - this.spe) / 4) + 80)) > 80) {
            ...
            if (Kiha.rand(2) == 0) this.outputText("You only owned an XJasun back home, so you don't really understand the reference.", false);
            else this.outputText("You stifle a laugh as your memories turn to many an evening spent with your friends in front of your SharkCube console, contesting each other in games of ridiculous, stylized combat.", false);
        }
        else {
            //Determine damage - str modified by enemy toughness!
            var damage: number = Math.floor((this.str + this.weaponAttack) - Kiha.rand(this.player.tou) - this.player.armorDef);
            damage += 5;
            ...
            if (!this.game.kihaFollower.followerKiha()) this.outputText("the swamp");
            else this.outputText("the fight");
            this.outputText(". (" + damage + ")\n", false);
            if (this.player.HP >= 1) this.outputText("You follow the shrill cry of \"<i>B-BAKA!</i>\" in the distance until you reach the exact location you were in a few seconds earlier, prepared to fight again.", false);
        }
        this.combatRoundOver();
    }
    ...
    private kihaFirePunch(): void {
        ...
        //("Misdirection"
        else if (this.player.findPerk(PerkLib.Misdirection) >= 0 && Kiha.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
