...
export class JojoScene extends NPCAwareContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (this.flags[kFLAGS.JOJO_COCK_MILKING_COOLDOWN] > 0) this.flags[kFLAGS.JOJO_COCK_MILKING_COOLDOWN]--;
        if (this.player.findStatusAffect(StatusAffects.NoJojo) >= 0) this.player.removeStatusAffect(StatusAffects.NoJojo);
    ...
    private assholeOrDP(): string {
        if (this.player.hasVagina()) return (this.vaginaDescript(0) + " and " + this.assholeDescript());
    ...
    private vaginaOrDicksOrCrotch(): string {
        ...
        if (this.player.gender == 3) return (this.multiCockDescriptLight() + " and " + this.vaginaDescript(0));
        return "FUKK: ERROR";
    }
    ...
    public campCorruptJojo(): boolean {
        return this.monk >= 5 && this.player.findStatusAffect(StatusAffects.NoJojo) < 0 && this.flags[kFLAGS.JOJO_DEAD_OR_GONE] == 0;
    }
    ...
    public corruptCampJojo(): void {
        ...
        if (this.flags[kFLAGS.FOLLOWER_AT_FARM_JOJO] == 0) {
            ...
            if (this.flags[kFLAGS.AMILY_FOLLOWER] == 2 && this.amilyScene.amilyFollower() && this.campCorruptJojo() && this.flags[kFLAGS.AMILY_X_JOJO_COOLDOWN] <= 0 && JojoScene.rand(5) == 0 && this.flags[kFLAGS.FOLLOWER_AT_FARM_AMILY] == 0) {
            ...
            if (this.amilyScene.amilyFollower && this.flags[kFLAGS.AMILY_DISCOVERED_TENTATLE_JOJO] == 0 && JojoScene.rand(10) <= 1 && this.flags[kFLAGS.AMILY_FOLLOWER] == 1 && this.player.findStatusAffect(StatusAffects.TentacleJojo) >= 0) {
                this.finter.amilyDiscoversJojoWithTentaclesAndShitOhBoy();
                return;
            }
            //Oh shit goes down! (No tentacles)
            else if (this.flags[kFLAGS.AMILY_PISSED_PC_CORRUPED_JOJO] == 0 && JojoScene.rand(10) <= 1 && this.flags[kFLAGS.AMILY_FOLLOWER] == 1 && this.amilyScene.amilyFollower() && this.player.findStatusAffect(StatusAffects.TentacleJojo) < 0) {
                this.finter.amilyIsPissedAtYouForRuiningJojo();
                return;
            }
            //Offer lethicite jojo tf if the player is ready
            if (this.player.findStatusAffect(StatusAffects.JojoTFOffer) < 0 && this.player.hasKeyItem("Marae's Lethicite") >= 0 && this.player.keyItemv2("Marae's Lethicite") < 3 && this.player.cor >= 75) {
            ...
            if (this.tentacleJojo()) this.outputText(" A number of tentacles vaugley resembling cocks have sprouted from his back and groin. They sway restlessly around him, oozing thick, fragrant pre from their tips.");
    ...
    private sendToFarm(): void {
        ...
        if (this.tentacleJojo()) this.outputText("-come-tentacle-beast");
    ...
    private jojoMutationOfferYes(): void {
        ...
        //[Female]
            ...
            if (this.player.biggestLactation() < 2) this.outputText(" while your tits cover his head, nearly suffocating him", false);
            else this.outputText(" while he suckles one of your breasts hungrily, sucking down your milk", false);
            ...
            if (this.player.vaginas[0].virgin) this.outputText("(Yet somehow your virginity remains intact).  ", false);
    ...
    public useTentacleJojo(): void {
        ...
        if (this.player.hasVagina()) this.outputText("You feel it split in half, dividing in order to take you from both ends.  ", false);
        ...
        if (this.player.totalCocks() > 0) {
            ...
            if (this.player.cockTotal() == 1) {
                ...
                if (this.player.balls > 0) {
                    ...
                    if (this.player.cockArea(0) > 60) this.outputText("laboriously stretching around", false);
                ...
                //(1 dick no balls =
                    ...
                    if (!this.player.hasVagina()) this.outputText("rubbing back and forth along your taint and smearing you with their slime.", false);
                    else {
                        ...
                        //(If big clit –
                        if (this.player.clitLength >= 4) this.outputText("  You nearly cum on the spot from the clitoral stimulation; it's like you have a super-sensitive cock being licked by a thousand tongues.", false);
                    }
                    this.outputText("\n\n", false);
                }
            }
            ...
            if (this.player.cockTotal() == 2) {
                ...
                //(2 dicks no balls)
                    ...
                    else this.outputText("to rub itself along your taint, massaging the sensitive skin with its slimy lubricants.", false);
                    this.outputText("\n\n", false);
                }
            }
            //(3 dicks)
            if (this.player.cockTotal() >= 3) {
                ...
                if (this.player.cockArea(0) > 60) this.outputText("laboriously working to swallow", false);
                else if (this.player.cockArea(0) >= 20) this.outputText("engulfing", false);
                else this.outputText("easily swallowing", false);
                ...
                //(if extra:
                if (this.player.cockTotal() > 3) this.outputText("  The remaining " + Appearance.cockNoun(CockTypesEnum.HUMAN) + "'s pulse and throb with jealous desire, until one of the tentacles slips off and captures it.  They take turns, slipping off and capturing each of your offerings until you wish they'd just keep sucking forever.", false);
                this.outputText("\n\n", false);
            }
        }
        //(0 dicks but clit)
        if (this.player.totalCocks() == 0 && this.player.hasVagina()) {
            ...
            //(If big clit –
            if (this.player.clitLength >= 4) this.outputText("  You nearly cum on the spot from the clitoral stimulation; it's like you have a super-sensitive cock being licked by a thousand tongues.", false);
            else this.outputText("  The sensation was unreal, and you find yourself wishing your clit was bigger so you could feel even more of the tendrils pleasure-hole.", false);
            this.outputText("\n\n", false);
        }
        ...
        if (this.player.hasVirginVagina() || this.player.ass.analLooseness == 0) {
            ...
            if (this.player.hasVirginVagina() && this.player.ass.analLooseness == 0) this.outputText("s", false);
        }
        //not virgin taking
        else {
            //With cunt
            if (this.player.hasVagina()) {
                if (this.player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS_GAPING_WIDE) this.outputText(" practically falling into your oversized cum-receptacles", false);
                else {
                    if (this.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_SLICK) this.outputText(" sliding inside easily thanks to your copious lubrication", false);
                    else this.outputText("forcing their way inside your " + this.vaginaDescript(0) + " and " + this.assholeDescript(), false);
                }
            }
            //Just butt
            else {
                if (this.player.ass.analLooseness >= 4) this.outputText(" practically falling into your oversized cum-receptacle", false);
                else {
                    if (this.player.ass.analWetness >= 3) this.outputText(" sliding inside easily thanks to your copious lubrication", false);
                    else this.outputText("forcing its way inside your " + this.assholeDescript(), false);
                }
            }
        }
        ...
        if (this.player.totalCocks() > 0) {
            ...
            if (this.player.cockTotal() == 1) this.outputText("  The tentacle on your " + this.multiCockDescriptLight() + " seems appreciative, and begins sucking to swallow down all your pre.", false);
            if (this.player.cockTotal() > 1) this.outputText("  The tentacles on your " + this.multiCockDescriptLight() + " seem appreciative, and begin sucking to swallow down all your pre.", false);
        }
        if (this.player.hasVagina()) this.outputText("  Your " + this.vaginaDescript(0) + " spasms around the invader, clenching uncontrollably as it's plowed by the thick vine-prick.", false);
        this.outputText("\n\n", false);
        ...
        if (this.player.biggestLactation() >= 2) {
            ...
            if (this.player.cowScore() >= 5) this.outputText("You find yourself mooing in contentment as your fuck-slave works to milk you.  ", false);
        ...
        //(NEITHER - JUST NIPS)
            ...
            if (this.player.biggestLactation() >= 1) this.outputText("A squirt of milk escapes and is quickly sucked down by the hungry tentacles.  ", false);
        ...
        if (this.player.hasVagina()) {
            ...
            if (this.player.vaginas[0].vaginalWetness == VAGINA_WETNESS_SLAVERING) this.outputText(", and soaks him with a wave of puss juice.  ", false);
            else if (this.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_SLICK) this.outputText(", as your drooling cunt forms a slick puddle.  ", false);
                ...
                if (this.player.cumQ() >= 100) {
                    ...
                    if (this.player.cumQ() >= 300) {
                        ...
                        if (this.player.cumQ() >= 600) {
                            ...
                            if (this.player.cumQ() >= 800) this.outputText(" but you don't stop cumming until it lies in a deep puddle of jizz", false);
                            this.outputText(".", false);
                            cumPuddle = true;
                        }
                    }
                ...
                if (this.player.cumQ() >= 100) {
                    ...
                    if (this.player.cumQ() >= 300) {
                        ...
                        if (this.player.cumQ() >= 600) {
                            ...
                            if (this.player.cumQ() >= 800) this.outputText(" but you don't stop cumming until they lie in a deep puddle of jizz", false);
                            this.outputText(".", false);
                            cumPuddle = true;
                        }
                    }
        ...
        if (this.player.biggestLactation() >= 2 && nippleSucking) {
            ...
            else this.outputText("  Somehow you know they won't get any bigger from his rough treatment.", false);
        ...
        if (titFucking) {
            ...
            if (this.player.cor >= 80 && this.player.lib >= 70) this.outputText("You lick your lips and swallow as much of it as possible, wallowing in the mouse-cream.  ", false);
            if (this.player.hairLength > 0) this.outputText("He keeps unloading it until you feel it in your " + this.hairDescript(), false);
        ...
        if (mouthFucking) {
            ...
            if (this.player.pregnancyIncubation == 0 && this.player.buttPregnancyIncubation == 0) this.outputText("an eight month pregnant woman", false);
    ...
    private jojoMilkPay(tentacle: boolean = false): void {
        ...
        if (payout > 0) {
            ...
            if (payout == 1) this.outputText(JojoScene.Num2Text(payout) + " gem rolls out into a collection plate.  Whitney really put a lot of work into this!  You pocket the gem and g", false);
            else this.outputText(JojoScene.Num2Text(payout) + " gems roll out into a collection plate.  Whitney really put a lot of work into this!  You pocket the gems and g", false);
        }
    ...
    private jojoCumQ(): number {
        var cumQ: number = 0;
        cumQ = 400;
        if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00331] < 4) cumQ += this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00331] * 200;
    ...
    private milkJojoFirst(): void {
        ...
        if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00333] > 0) this.outputText(", even though it reeks of the spunk you've spilled into the milker's mechanical embrace", false);
        ...
        if (this.player.hasCock()) this.outputText("It starts to turn your way before it stops and rises, seeming to sniff at the rodent directly above.", false);
        else this.outputText("It slowly starts to rise, seeming to sniff at the rodent above as it orients itself.", false);
        ...
        if (this.player.hasCock()) {
            ...
            if (x < 0) {
                ...
                if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00333] > 0) this.outputText("You know from experience that machine usually takes an hour before it will allow release", false);
                else this.outputText("You figure the machine will probably pump him for quite a while before it allows release", false);
        ...
        //(Vagoozle!)
            ...
            if (this.player.clitLength > 3) this.outputText("Your other hand grabs your penis-sized 'button' and begins to jack it, the action sending tremors of sensation through your " + this.hipDescript() + ".  ", false);
        ...
        if (this.player.hasCock()) this.outputText(", squeezing your " + this.cockDescript(x) + " with agonizingly pleasurable pressure", false);
        ...
        if (!this.player.hasCock() && this.player.hasVagina()) this.outputText("  His balls tremble and shrink in your mouth, pumping his prodigious semen out, wasting it in exchange for a pittance of gems.", false);
        this.outputText("\n\n", false);

        if (this.player.hasCock()) {
            //(Too big dick:
            if (this.player.cockThatFits(40) < 0) this.outputText("You release your cum all over Jojo's back, matting his white fur with cum.  Each successive splatter of spunk makes the mouse look even more like a semen-drenched whore.  It's a good look for him.", false);
            else this.outputText("You pull out and release your cum all over Jojo's back, matting his white fur with cum.  Each successive splatter of spunk makes the mouse look even more like a semen-drenched whore.  It's a good look for him.", false);
        }
        else if (this.player.hasVagina()) this.outputText("You climax and clamp down on your fingers, squeezing your hand as your own cries of pleasure mix with your cock-toy's milked-out moans.", false);
        else this.outputText("You sigh wistfully as Jojo ejaculates his load, wishing you could feel such exquisite sensations.  Maybe you can find something to give you some proper sexual organs soon?", false);
    ...
    private repeatMilkJojo(tentacle: boolean = false): void {
        ...
        if (!tentacle || !this.tentacleJojo()) {
            ...
            if (this.jojoCumQ() < 1000) this.outputText("has already dripped everywhere, making a mess of the floor.", false);
            else if (this.jojoCumQ() < 4000) this.outputText("is drizzling a steady flow of man-slime, puddling on the floor.", false);
            else this.outputText("is already pouring out a river of pre-cum, turning the floor into a slippery morass.", false);
            ...
            if (!this.player.hasCock() && !this.player.hasVagina()) this.outputText(", even though to do so you're forced to stimulate your " + this.assholeDescript(), false);
        ...
        //TENTACLE JOJO MILKING (edited)
            ...
            if (this.flags[kFLAGS.PC_FETISH] > 0) this.outputText("An exhibitionist thrill shivers through you at the thought of being watched while engaged in such a carnal act.  ", false);
    ...
    private jojoPaysForPerms(): void {
        ...
        else this.outputText("and rolls them back and forth in her fingers, expertly pleasuring Jojo.", false);
    ...
    private amilyTeachingJojoBJ(): void {
        ...
        if (this.player.hasCock()) this.outputText(" Do you surprise them both by allowing Amily to demonstrate on you?");
    ...
    private fillAmilysMouth(): void {
        ...
        if (this.player.balls > 0) this.outputText(" leans down to desperately suckle at your [sack], subserviently worshipping your bloated testes and their liquid bliss one after another; tonguing, sucking, and moaning into the spunk - bloated mass of your ambrosial scrotum.");
        else this.outputText(" leans down to slurp needily at the bottom of your cock, licking it with languid tongue-strokes each time Amily gives him an opening.");
    ...
    private amilyAndJojoFuck(): void {
        ...
        if (this.player.hasVagina()) this.outputText(" It isn't until you notice how wet your crotch is that you come to the realization that you've masturbated yourself through a few orgasms as well.");
        else this.outputText(" It isn't until you notice how clearheaded you are that you come to the relaization that you've masturbated while watching the pair.");
    ...
    private jojosSecondRape(): void {
        ...
        if (this.player.gender == 1) {
            ...
            if (this.player.averageCockThickness() >= 3) this.outputText("It makes little difference to your " + this.cockDescript(0) + ", he still feels tight as a virgin's cunt to you.  ");
            ...
            if (this.player.cockTotal() > 1) this.outputText("  Your remaining equipment showers him with jizz, more than you ever thought you could produce.");
        ...
        if (this.player.gender == 2 || this.player.gender == 3) {
            ...
            if (this.player.biggestLactation() >= 1 && this.player.biggestLactation() < 2) this.outputText("Milk squirts from  your nipples, spraying him down with small droplets of your creamy tit-treat.  ");
            if (this.player.biggestLactation() >= 2 && this.player.biggestLactation() < 3) this.outputText("Streams of milk spray from your nipples in time with your pussy contractions, hosing the poor mouse down and puddling on his fur.  ");
            if (this.player.biggestLactation() >= 3) this.outputText("Gouts of milk erupt from your nipples, spraying continually as you cum.  The poor mouse is soaked from head to toe, your cream utterly drenching the monk.  ");
            if (this.player.averageVaginalWetness() == 5) this.outputText("Your " + this.vaginaDescript(0) + " drenches him with your squirting girl-cum, mixed with his own seed.");
            if (this.player.cockTotal() == 1) this.outputText("Jizz sprays onto his chest from your " + this.cockDescript(0) + ".  ");
            if (this.player.cockTotal() == 2) this.outputText("A hail of jizz splatters over Jojo from your " + this.multiCockDescriptLight() + ".  ");
            if (this.player.cockTotal() == 3) this.outputText("A multitude of thick cum-streams splatter over Jojo from head to waist as your " + this.multiCockDescriptLight() + " hose him down.  ");
    ...
    private jojosThirdRape(): void {
        ...
        if (this.player.gender == 1) {
            ...
            else this.outputText("presenting your " + this.multiCockDescriptLight() + " to him.  ");
            ...
            if (this.player.cockTotal() == 1) this.outputText("to ram your " + this.cockDescript(0) + " deep into his maw.  You roughly grab his ears, facefucking him hard, his tongue working hard to please.  ");
            if (this.player.cockTotal() > 1) this.outputText("to cram two cocks deep into his maw, making his jaw stretch obscenely.  You roughly grab his ears, facefucking him hard, his tongue working hard to please you.  ");
            ...
            if (this.player.cockTotal() == 1) this.outputText("Your " + this.cockDescript(0) + " clenches tightly, erupting ");
            if (this.player.cockTotal() > 1) this.outputText("Your twin dicks clench tightly, erupting ");
        ...
        if (this.player.gender == 2) {
            if (this.player.isBiped()) this.outputText("You spread your legs and crook your finger");
            else this.outputText("You crook your finger");
            ...
            if (this.player.biggestLactation() >= 1.5 && this.player.biggestTitSize() > 2 && this.player.mostBreastsPerRow() >= 2 && this.player.breastRows.length >= 1) this.outputText("Every sensual peak within you is mirrored with small spurts of milk from your nipples.  It eventually trickles down to Jojo's tongue, spurring his efforts on.  ");
            ...
            if (this.player.lowerBody == LOWER_BODY_TYPE_NAGA) this.outputText("your coils ");
            else if (this.player.lowerBody == LOWER_BODY_TYPE_GOO) this.outputText("your jiggling goo ");
            else this.outputText("your thighs ");
            this.outputText("around his head and quiver with passion, ");
            if (this.player.averageVaginalWetness() <= 1) this.outputText("squeezing him tightly into your " + this.vaginaDescript(0) + ".");
            if (this.player.averageVaginalWetness() > 1 && this.player.averageVaginalWetness() <= 3) this.outputText("creaming his tongue liberally with girlcum.");
            if (this.player.averageVaginalWetness() == 4) this.outputText("creaming all over him with your slobbering pussy.");
            if (this.player.averageVaginalWetness() == 5) this.outputText("splattering him with girlcum from your " + this.vaginaDescript(0) + ".");
            this.outputText("\n\nYou pull the cunt-dazed mouse away reluctantly, and bend over,");
            if (this.player.isBiped()) this.outputText(" spreading your legs and");
            this.outputText(" shaking your rump.  ");
            if (this.player.hasLongTail()) this.outputText("Your tail swishes back and forth teasingly.  ");
            ...
            if (this.player.isBiped()) this.outputText(" between your thighs");
            else this.outputText(" your cunt");
        ...
        if (this.player.gender == 3) {
            ...
            if (this.player.cockTotal() == 1) this.outputText("You sigh as your " + this.cockDescript(0) + " droops over his head.  ");
            if (this.player.cockTotal() > 1) this.outputText("You sigh as your " + this.multiCockDescriptLight() + " pile atop his head.  ");
            if (this.player.biggestTitSize() >= 2) {
            ...
            if (this.player.averageVaginalWetness() <= 1) this.outputText("squeezing him tightly into your " + this.vaginaDescript(0) + ".");
            if (this.player.averageVaginalWetness() > 1 && this.player.averageVaginalWetness() < 4) this.outputText("creaming his tongue liberally with girlcum.");
            if (this.player.averageVaginalWetness() == 4) this.outputText("creaming all over him with your slobbering pussy.");
            if (this.player.averageVaginalWetness() >= 5) this.outputText("splattering him with you girlcum from your " + this.vaginaDescript(0) + ".");
            if (this.player.cockTotal() > 0) this.outputText("  Thick runners of your pre dribble down his neck, sticking to his fur.  ");
            this.outputText("\n\nYou pull the cunt-dazed mouse away reluctantly, and bend over,");
            if (this.player.isBiped()) this.outputText(" spreading your legs and");
            this.outputText(" shaking your rump.  ");
            if (this.player.hasLongTail()) this.outputText("Your tail swishes back and forth teasingly.  ");
            this.outputText("Your tantalizing backside is too much for him.  He pounces your ass in a second, penetrating your " + this.vaginaDescript(0) + " forcefully.  The knowledge that your demonic influence is responsible for his state nearly makes you cream all over his cock.  You fantasize about your cunt making his cock grow longer and fatter as it corrupts him further, transforming his morals and values into cock and cum.  Jojo thrashes against you, brutally pounding your " + this.vaginaDescript(0) + " as he orgasms.  His " + this.monster.cockDescriptShort(0) + " erupts inside you, the feeling of being filled with mouse-spunk drives you over the edge with him.  More and more of his cum sprays inside you, unnatural amounts for a creature his size.  Thick streams of the stuff escape down your thighs as your belly distends.  ");
            this.player.cuntChange(2, true);
            if (this.player.cockTotal() == 1) this.outputText("Your " + this.cockDescript(0) + " trembles in orgasm, squirting your load into the thick forest loam.  ");
            if (this.player.cockTotal() > 1) this.outputText("Your " + this.multiCockDescriptLight() + " tremble in orgasm, squirting their hot loads all over the thick forest loam.  ");
            this.outputText("\n\n");
            this.outputText("You let the drained mouse pop free, reveling in the sensation of cum pouring from ");
            if (this.player.isBiped()) this.outputText("between your thighs");
            else this.outputText("your cunt");
    ...
    private jojosFourthRape(): void {
        ...
        if (this.player.gender == 1) {
            ...
            if (this.player.cockTotal() == 1) this.outputText("You touch his head softly and guide him to your " + this.cockDescript(0) + ", smiling as he licks his lips with anticipation.  You gasp at the feeling of his soft lips and wet tongue as he pleasures you, your knees going weak from his efforts.  Jojo cups your balls and slips a fuzzy finger into your ass.  He labors to take as much of you as possible into his mouth, panting warmly and wetly around your " + this.cockDescript(0) + ".  You twitch in pleasure as he alternates pressure on your prostate with smooth strokes of his slick tongue.  ");
            else this.outputText("You touch his head softly and guide him to your " + this.multiCockDescriptLight() + ", smiling as he licks his lips in anticipation.  You gasp at the feelings of his soft lips and wet tongue on your heads as he pleasures you, your knees going weak from his efforts.  Jojo cups your balls and slips a fuzzy finger into your ass.  He labors to take in two shafts, struggling to swallow them as deep as possible, panting wetly around you.  You twitch with pleasure as he alternates pressure on your prostate with smooth strokes of his slick tongue.  ");
            //Too thick for him
            if (this.player.averageCockThickness() > 3 || (this.player.dogCocks() > 0 && this.player.averageCockThickness() > 3)) this.outputText("Jojo is forced to back off of your " + this.cockDescript(0) + " from time to time to come up for air, barely opening his jaw wide enough to take your girth back inside his mouth.  ");
            //Too long for him
            if (this.player.averageCockLength() > 10 || (this.player.horseCocks() > 0 && this.player.averageCockLength() > 10)) this.outputText("He struggles not to gag on your " + this.cockDescript(0) + "'s length, opening his throat as far as he can.  ");
            this.outputText("You rock back and forth on his face as he expertly keeps you on the edge of orgasm.  ");
            if (this.player.cumQ() > 25) this.outputText("Your constant stream of heavy pre practically chokes the poor mouse as he edges you mercilessly, his own equipment drizzling in sympathetic lust.  ");
            ...
            if (this.player.cumQ() < 25) this.outputText("You buck against him as you orgasm, your small squirts of cum eagerly devoured by the slutty mouse.");
            if (this.player.cumQ() >= 25 && this.player.cumQ() < 250) this.outputText("You buck against him as you orgasm, the slutty mouse's throat squeezing against you as he works to swallow your seed.");
            if (this.player.cumQ() >= 250 && this.player.cumQ() < 500) this.outputText("You spasm against him as you orgasm, the pleasure erupting into the slut-mouse's throat as he tries to swallow it all.  Excess cum dribbles from the corners of his mouth as you fully spend yourself.");
            if (this.player.cumQ() >= 500) this.outputText("You buck against him as you orgasm, the slutty mouse's cheeks bulging in surprise as your cum explodes into his mouth.  Thick streams escape from the corners of his muzzle, your seed pouring into him faster than he can swallow.  Eventually you finish, and see the mouse dripping with your spunk nearly to the waist, a small bulge in his belly.");
        ...
        if (this.player.gender == 2 || this.player.gender == 3) {
            ...
            if (this.player.isBiped()) this.outputText(" brushes against your calf");
            else this.outputText(" brushes against your [leg]");
            this.outputText(".  You get a devilish idea, ");
            if (this.player.lowerBody == LOWER_BODY_TYPE_GOO) this.outputText("and push his " + this.monster.cockDescriptShort(0) + " in between folds of slime, sandwiching it in a mass of slippery, gooey tightness.  Holding his shoulder for balance, you slowly squeeze him, gently milking out small dribbles of pre.  He redoubles his efforts, burying his nose into your " + this.vaginaDescript(0) + ", tongue swirling over your folds and around your " + this.clitDescript() + ".  For every effort on his part you step up your own, squeezing and stroking him with your goo, doing your best to impart a fetish for being masturbated with someone's lower body on his slowly warping mind.  You feel a hot wetness in your slime, and it grows slicker and wetter every second. Jojo's cum drips out of you, glazing your jello-like form white.  ");
            else this.outputText("and push his " + this.monster.cockDescriptShort(0) + " with your " + this.player.foot() + ", sandwiching it under one " + this.player.foot() + " and on top of the other.  Holding his shoulder for balance, you slowly squeeze him, gently milking out small dribbles of pre.  He redoubles his efforts, burying his nose into your " + this.vaginaDescript(0) + ", tongue swirling over your folds and around your " + this.clitDescript() + ".  For every effort on his part you step up your own, squeezing and stroking him with your " + this.player.feet() + ", doing your best to impart a " + this.player.foot() + " fetish on his slowly warping mind.  You feel a hot wetness on your " + this.player.feet() + ", and they grow slicker and wetter every second. Jojo's cum drips out from between them, glazing them white.  ");
            if (this.player.averageVaginalWetness() <= 1) this.outputText("You clamp down on his muzzle as you writhe in orgasm.  ");
            if (this.player.averageVaginalWetness() > 1 && this.player.averageVaginalWetness() <= 3) this.outputText("Your legs trap his muzzle in your " + this.vaginaDescript(0) + " as orgasm wracks your body.  ");
            if (this.player.averageVaginalWetness() == 4) this.outputText("Your legs trap his muzzle in your " + this.vaginaDescript(0) + ", slicking his muzzle with girlcum as you spasm with bliss.  ");
            if (this.player.averageVaginalWetness() == 5) this.outputText("Your legs squeeze him against your " + this.vaginaDescript(0) + ", girlcum erupting over his face and soaking him as you bliss out with orgasm  ");
            if (this.player.cockTotal() == 1) this.outputText("Your " + this.cockDescript(0) + " drizzles cum in his hair, some dripping off by his ear.  ");
            if (this.player.cockTotal() > 1) this.outputText("Your " + this.cockDescript(0) + "s drizzle him with cum, covering his head with thick white streams of your jizz.  ");
            ...
            if (this.player.averageVaginalLooseness() == 0) this.outputText("He is painfully large, so much so that you feel more pain than pleasure.  ");
            if (this.player.averageVaginalLooseness() == 1) this.outputText("He stretches you around him like a latex glove, pulling your " + this.vaginaDescript(0) + " taught with his " + this.monster.cockDescriptShort(0) + ", the sensation riding a razor's edge between pleasure and pain.  ");
            if (this.player.averageVaginalLooseness() == 2) this.outputText("His " + this.monster.cockDescriptShort(0) + " stuffs you completely, filling your " + this.vaginaDescript(0) + " to capacity.  ");
            if (this.player.averageVaginalLooseness() == 3) this.outputText("His " + this.monster.cockDescriptShort(0) + " fits you perfectly, burying deep inside your folds.  ");
            if (this.player.averageVaginalLooseness() == 4) this.outputText("You easily accomadate his member into your " + this.vaginaDescript(0) + ".  ");
            if (this.player.averageVaginalLooseness() == 5) this.outputText("His " + this.monster.cockDescriptShort(0) + " slips inside your " + this.vaginaDescript(0) + " with little resistance, easily sinking in to the hilt.  You muse to yourself, \"<i>If only he were thicker...</i>\"  ");
            ...
            if (this.player.cockTotal() > 0) {
                ...
                if (this.player.cockTotal() == 1) {
                    //Horsefun!
                    if (this.player.horseCocks() == 1) this.outputText("Your " + this.cockDescript(0) + " feels a building pressure, the whole thing pulsating wildly with each of your heartbeats, most noticably the tip, which flares out wildly.  Powerful contractions wrack your sheath and " + this.cockDescript(0) + " as pre practically fountains from it.  ");
                    //DogFun!
                    if (this.player.dogCocks() == 1) this.outputText("Your " + this.cockDescript(0) + " feels an intense pressure, and begins bulging out obscenely above your sheath.  The knot thickens gratuitiously, filling as it pulses with need.  Cum drips from your pointed tip as it continues to bulge wider, filling you with unbearable pressure.  ");
                    //Else
                    if (this.player.normalCocks() == 1) this.outputText("Your " + this.cockDescript(0) + " twitches, muscle contractions slowly working their way up from the base.  ");
                    //CUMSPLOISION
                    if (this.player.cumQ() < 25) this.outputText("Your body tenses and cums, spraying spurts of jizz over the mouse.  ");
                    if (this.player.cumQ() >= 25 && this.player.cumQ() < 250) {
                        if (this.player.horseCocks() == 1) this.outputText(" Your " + this.cockDescript(0) + " flares wildly as musky horse-cum erupts from it, splattering over Jojo.  ");
                        if (this.player.dogCocks() == 1) this.outputText("The terrible pressure in your " + this.cockDescript(0) + " finally relents, in the form of a fountain of doggie-cum, spraying out from your " + this.cockDescript(0) + " in a steady stream that seems to last and last.  ");
                        if (this.player.cocks[0].cockType == CockTypesEnum.HUMAN || this.player.cocks[0].cockType.Index > 2) this.outputText("Your body tenses and cums a thick eruption far beyond what a normal human could produce.  Jojo is splattered with the stuff.  ");
                    }
                    if (this.player.cumQ() >= 250) {
                        if (this.player.horseCocks() == 1) this.outputText("Your " + this.cockDescript(0) + " ripples and bulges with animalistic vigor, horse-cum splattering everywhere with each contraction.  The musky animal-jizz never seems to stop pouring from your equine organ, soaking the mouse from the waist up.  ");
                        if (this.player.dogCocks() == 1) this.outputText("Your " + this.cockDescript(0) + " suddenly releases the pressure, a constant stream of doggie-cum spouting from your " + this.cockDescript(0) + " like some kind of cum-hose.  It seems to go on endlessly, covering the mouse from the waist up with thick ribbons of doggie-spooge as your knot slowly shrinks to normal.  ");
                        if (this.player.cocks[0].cockType == CockTypesEnum.HUMAN || this.player.cocks[0].cockType.Index > 2) this.outputText("Your body tenses and cums a thick eruption far beyond what a normal human could produce.  Jojo is splattered with the stuff.  ");
                    }
                }
            ...
            if (this.player.biggestLactation() >= 1 && this.player.biggestTitSize() > 3) {
                if (this.player.biggestTitSize() * this.player.biggestLactation() * this.player.breastRows.length < 8) this.outputText("Milk sprays from your " + this.breastDescript(0) + " in tiny streams, triggered by your orgasms.");
                if (this.player.biggestTitSize() * this.player.biggestLactation() * this.player.breastRows.length < 20 &&
                    this.player.biggestTitSize() * this.player.biggestLactation() * this.player.breastRows.length >= 8) this.outputText("Milk erupts from your " + this.breastDescript(0) + ", spraying out over the mouse, squirting out the contractions of each shuddering orgasm.");
                if (this.player.biggestTitSize() * this.player.biggestLactation() * this.player.breastRows.length < 35 &&
                    this.player.biggestTitSize() * this.player.biggestLactation() * this.player.breastRows.length >= 20) this.outputText("Milk erupts from your " + this.breastDescript(0) + ", spraying in pulsing blasts, soaking the mouse.  Each nerve-wracking orgasm seems to wring more and more milk from you, until it puddles around.");
                if (this.player.biggestTitSize() * this.player.biggestLactation() * this.player.breastRows.length >= 35) this.outputText("Milk fountains from your " + this.breastDescript(0) + ", soaking the mouse with a continuous river of cream.  For every blob of cum you feel pushing into your over-filled uterus, another torrent of milk sprays out.  As your mind-bending orgasms drag on, a small lake of milk forms around you.");
            }
        ...
        if (this.monk == 2 || this.monk == 3) {
            ...
            if (this.player.lust >= 100) {
                if (this.player.gender == 1) {
                    ...
                    if (this.player.cockTotal() == 1) this.outputText("Pre and cum drip from your " + this.player.cockDescript(0) + ", forced out of your prostate by the rough beating it's taking.  You feel a flash of warm wetness inside you, and realize Jojo is cumming.  A sense of relief washes over you as the last burst of cum squirts out from your cheeks, only to be replaced with a dawning sense of horror as he continues fucking you harder than ever.\n\nYou black out after a few dozen of his orgasms and one or two of your own, your gut painfully distended with semen.");
                    if (this.player.cockTotal() > 1) this.outputText("Pre and cum drip from your " + this.player.cockDescript(0) + "s, forced out of your prostate by the rough beating it's taking.  You feel a flash of warm wetness inside you, and realize Jojo is cumming.  A sense of relief washes over you as the last burst of cum squirts out from your cheeks, only to be replaced with a dawning sense of horror as he continues fucking you harder than ever.\n\nYou black out after a few dozen of his orgasms and one or two of your own, your gut painfully distended with semen.");
                ...
                if (this.player.gender >= 2) {
                    ...
                    if (this.player.cockTotal() > 1) this.outputText("Your " + this.player.cockDescript(0) + " splatters the ground with cum repeatedly, until both your genders are raw and sore.  ");
                    else this.outputText("Your " + this.vaginaDescript(0) + " cums on him many more times it until it is sore and tender, dripping with spunk.  ");
                    ...
                    //Preggers chance!
                    this.player.knockUp(PregnancyStore.PREGNANCY_MOUSE, PregnancyStore.INCUBATION_MOUSE + 82, 101); //Jojo's kids take longer for some reason
                }
    ...
    private corruptJojoCunnilingus(): void {
        ...
        switch (JojoScene.rand(3)) {
            case 0:
                ...
                break;
            case 1: this.outputText("You take the tip of his member into your mouth and slowly start to bob your head, one hand squeezing at his balls tenderly as your other hand strokes the length of his cock that your lips don't reach. You let your pace quicken over time, mimicking a vigorous fucking.  ");
                break;
            default:
    ...
    private corruptJojoVaginalSmother(): void {
        ...
        else this.outputText("You settle the full of your weight against his face and laugh as you feel him struggling to pleasure you, his nose and mouth trapped tight against your slit so that every attempt to breathe is halted, making him tremble breathlessly beneath you.  ");
    ...
    private corruptJojoAnalCruel(): void {
        ...
        else this.outputText("You grin as your mouse slut cries out with your " + this.player.cockDescript(x) + " spearing into his bowels. You can feel the weight of the tree against your " + this.player.cockDescript(x) + " as you force his belly to bulge out vulgarly to accommodate the enormous girth.  ");
    ...
    */
    ...
    private beeEggsInCorruptJojo(): void {
        ...
        if (!this.player.hasCock()) {
            if (this.flags[kFLAGS.TIMES_EGGED_JOJO] == 0) this.outputText("Jojo looks confused at this; he knows you don't have a cock!  ");
            else this.outputText("Jojo looks confused at first, and then nods in understanding, remembering the last time you laid in him.  ");
        }
        ...
        this.outputText("\n\nMoving forward, you ");
        if (this.player.isNaga()) this.outputText("slither around him");
        else if (this.player.isGoo()) this.outputText("ooze over him");
        else if (this.player.isTaur()) this.outputText("straddle him");
        else this.outputText("climb atop him");
        ...
        if (this.player.gender > 0) {
            ...
            if (this.player.gender == 3) this.outputText(" and [eachCock] ");
            if (this.player.gender == 1) this.outputText(this.multiCockDescriptLight() + " ");
            if (this.player.gender == 3 || this.player.cockTotal() > 1) this.outputText("are");
        ...
        if (this.player.eggs() > 30) this.outputText("  His belly is slightly rounded by the time you finish, with small, egg-shaped bumps visible through his fur.");
    ...
    public lowCorruptionIntro(): void {
        ...
        if (this.player.cor <= 5) this.outputText("don't ");
        else this.outputText("barely ");
    ...
    public meditateInForest(): void {
        ...
        if (this.player.statusAffectv1(StatusAffects.JojoMeditationCount) >= 5) {
            ...
            if (this.flags[kFLAGS.FUCK_FLOWER_LEVEL] >= 4 && this.flags[kFLAGS.FUCK_FLOWER_KILLED] == 0 && this.player.statusAffectv1(StatusAffects.JojoMeditationCount) % 5 == 0) {
    ...
    public acceptJojoIntoYourCamp(): void {
        this.jojoSprite();
        if (this.player.findStatusAffect(StatusAffects.EverRapedJojo) >= 0 || this.flags[kFLAGS.JOJO_MOVE_IN_DISABLED] == 1) {
    ...
    public jojoCamp(): void {
        this.clearOutput();
        this.jojoSprite();
        if (this.flags[kFLAGS.AMILY_MET_PURE_JOJO] == 0 && this.flags[kFLAGS.AMILY_FOLLOWER] == 1 && this.amilyScene.amilyFollower()) {
        ...
        if (this.player.findStatusAffect(StatusAffects.Infested) >= 0) { // Worms overrides everything else
    ...
    public jojoTalkYourOrigin(): void // Prob tack on some interaction count or something to unlock this
    {
        ...
        if (this.player.findPerk(PerkLib.HistoryAlchemist) >= 0) {
            ...
            if (kGAMECLASS.silly()) this.outputText("Savin");
            else this.outputText("Sellet");
    ...
    public jojoTalkSandCave(): void {
        ...
        else if (this.flags[kFLAGS.SAND_WITCHES_FRIENDLY] == 1 && this.flags[kFLAGS.SAND_MOTHER_DEFEATED] == 1) {
        ...
        else if (this.flags[kFLAGS.SAND_WITCHES_FRIENDLY] == 1 && this.flags[kFLAGS.SAND_MOTHER_DEFEATED] == 0) {
        ...
        //[if {PC met Cum Witch}
        if (this.flags[kFLAGS.CUM_WITCH_DEFEATED] == 1 || this.flags[kFLAGS.BEEN_BLESSED_BY_CUM_WITCH] == 1) {
    ...
    public apparantlyJojoDOESlift(): void {
        ...
        if (this.player.findPerk(PerkLib.ControlledBreath) < 0 && this.player.findPerk(PerkLib.CleansingPalm) < 0 && this.player.findPerk(PerkLib.Enlightened) < 0) {
            ...
            if (this.flags[kFLAGS.TIMES_TRAINED_WITH_JOJO] == 5) {
                ...
                if (this.player.biggestTitSize() == 0) this.outputText(" rises and falls");
                else this.outputText(" rise and fall");
        ...
        else if (this.player.findPerk(PerkLib.ControlledBreath) >= 0 && this.player.findPerk(PerkLib.CleansingPalm) < 0 && this.player.findPerk(PerkLib.Enlightened) < 0) {
        ...
        //{after PC has gained the Enlightened Perk}
            ...
            if (this.player.findStatusAffect(StatusAffects.CampMarble) >= 0) enlightenedBlurbs.push("You can hear Marble humming a song to herself you can’t place.");
            if (this.flags[kFLAGS.AMILY_FOLLOWER] > 0) enlightenedBlurbs.push("You can hear Amily changing the bedding to her nest.");
            if (kGAMECLASS.emberScene.followerEmber()) enlightenedBlurbs.push("You can hear Ember cleaning" + this.emberScene.emberMF("his", "her") + "scales.");
            if (this.player.findStatusAffect(StatusAffects.CampRathazul) >= 0) enlightenedBlurbs.push("You can hear Rathazul experimenting with surprisingly nimble fingers.");
            if (this.sophieFollower()) enlightenedBlurbs.push("You can hear Sophie breathing as she sleeps.");
            if (this.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00238] > 0) enlightenedBlurbs.push("You can hear Izma flipping through the pages of a book."); // TODO: (if Izmael gets put in) you can hear Izmael doing push ups to stay fit.
    ...
    public wormRemoval(): void {
        ...
        if (this.player.HP > Math.floor(this.player.maxHP() * .5)) this.player.HP = Math.floor(this.player.maxHP() * .5);
        this.player.sens = 11;
