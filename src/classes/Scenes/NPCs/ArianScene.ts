...
export class ArianScene extends NPCAwareContent implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (this.model.time.hours > 23) {
            if (this.arianScene.arianFollower() && this.flags[kFLAGS.ARIAN_VAGINA] > 0)
                this.flags[kFLAGS.ARIAN_EGG_EVENT]++;
    ...
    private helpArianWhenYouMeetHim(): void {
        ...
        this.outputText(
            "You approach the hooded figure with caution, asking if they're all right; it feels a little silly to say that, but you can't think of much else to say."
        );
    ...
    public visitThePark(): void {
        ...
        // Visit 3
            ...
            if (this.player.cor > 66)
                this.outputText(
                    ", whilst privately thinking to yourself how useless that looks - no wonder they can't fight the demons if this is the best they're capable of"
                );
            this.outputText(".");
    ...
    public visitAriansHouse(): void {
        ...
        if (this.arianFollower()) {
            ...
            if (temp == 0) {
                ...
                this.outputText(
                    'Inside, the lizan is sitting at a table, fastidiously drinking from a cup of something hot while pouring over an arcane-looking text.  You politely cough to draw [Arian eir] attention and [Arian ey] looks at you, smiling.  "<i>Hello, [name].  I was just catching up on my studies.  Can I offer you a cup of tea, or maybe something else?</i>" [Arian ey] asks.'
                );
            } else if (temp <= 2) {
                this.outputText(
                    'The lizan is currently busy tinkering with some occult-looking paraphernalia when you find [Arian em].  You politely cough to attract [Arian eir] attention, then do so again when [Arian ey] fails to heed that.  It\'s only on the third attempt that [Arian ey] looks up apologetically from [Arian eir] work.  "<i>Ah, [name]; I\'m sorry, but I was preoccupied with something.</i>" [Arian ey] states in an apologetic tone, indicating the mess on [Arian eir] desk.  "<i>Was there something you wanted?</i>" [Arian ey] asks.'
                );
            } else if (temp <= 4) {
                this.outputText(
                    'The smell of fresh cooking fills the air and you can see Arian happily sitting down at his couch with a plate of something just cooked.  "<i>Oh, [name]; I was just about to eat, can I offer you a dish?  Or if you\'d rather do something else, this can wait,</i>" the lizan tells you with a smile.'
                );
            } else if (temp <= 6) {
                this.outputText(
                ...
                this.outputText(
                    "A strange smell hits your nose as you enter the tent; it takes you a few moments, but then you place it, your ears pricking as you hear Arian letting out some very familiar groans of release.  With a smirk, you sneak up and lean over the couch, looking right into the eyes of Arian, [Arian eir] fingers still glistening with "
                );
                if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                ...
                this.outputText(
                    ".  For a long moment, [Arian ey] just stares back at you; if lizans could blush, you're certain [Arian ey]'d be red as a beet.  \"<i>[name]! I was- I was just....  Oh, this is embarrassing,</i>\" [Arian ey] mutters, looking at [Arian eir] feet."
                );
            ...
            if (this.flags[kFLAGS.ARIAN_PARK] == 4) {
                ...
                this.outputText(
                    ".  You rack your brains for polite conversation, and finally ask how he's been since you saw him last."
                );
                ...
                if (
                    ...
                    this.outputText(
                        "Figuring that Arian would enjoy your company, you make your way with confidence through the streets leading to the lizan's home.  Soon enough, you find yourself standing before the stately home in which he and his ferret associates dwell.  You pound heartily on the knocker to announce your presence."
                    );
                    this.outputText(
                        '\n\n"<i>Coming!</i>"  You hear Laika yell.  Shortly after the ferret girl opens the door and greets you.  "<i>Hello [name].  Came to visit ' +
                            this.arianMF("master", "mistress") +
                            ' Arian?  Please come in!</i>"  She motions for you to enter the house.  You thank her for the polite invitation and step through the doorway.  The sound of dishes being washed draws your attention to the nearby kitchen, where you see Boon diligently washing a pan.  He looks up and waves at you with a soapy hand.  You return the gesture.'
                    );
                    this.outputText(
                        "\n\n\"<i>Sorry to leave you unattended [name], but we're kinda busy; do you think you can make the way to " +
                            this.arianMF("master", "mistress") +
                            " Arian's room by yourself?</i>\"  Laika asks."
                    );
                    this.outputText(
    ...
    private arianHomeMenu(): void {
        this.menu();
        if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 0 && this.arianHealth() >= 10)
            this.addButton(0, "Next", this.arianStoryDialogue1);
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 1 && this.arianHealth() >= 20)
            this.addButton(0, "Next", this.arianStoryDialogue2);
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 2 && this.arianHealth() >= 30)
            this.addButton(0, "Next", this.arianDialogue3);
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 3 && this.arianHealth() >= 50)
            this.addButton(0, "Next", this.arianImbue);
        else if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] == 4 && this.arianHealth() >= 75)
        ...
        // If no story dialogue
            ...
            if (
                this.player.hasKeyItem("Arian's Talisman") >= 0 ||
                this.player.hasKeyItem("Arian's Charged Talisman") >= 0
            )
                this.addButton(2, "Talisman", this.imbueTalisman);
            if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] >= 5)
            ...
            if (this.flags[kFLAGS.SLEEP_WITH] == "Arian")
    ...
    private eavesDropOnArian(): void {
        this.clearOutput();
        this.outputText(
            "You sidle up to the door, pressing your ear against the wood and start to listen intently."
        );
        this.outputText(
    ...
    private peepOnArian(): void {
        ...
        this.outputText(
            ", though this isn't stopping him from enthusiastically stroking himself off."
        );
    ...
    private leaveFappingArian(): void {
        this.clearOutput();
        this.outputText(
            "You decide to let Arian have some privacy and leave for the moment... after all, what the lizan mage does in his free time is not really your business...."
        );
        this.outputText(
    ...
    private bargeInOnArian(): void {
        this.clearOutput();
        this.outputText(
            "With a wry smirk you turn the knob and find that Arian's door is unlocked; without missing a beat, you open the door and step in right in time to see a sticky rope of pre paint Arian's slender belly as he scrambles to cover himself up."
        );
        this.outputText(
        ...
        // (else) //if PC is a herm
            ...
            this.outputText(
                "\n\nOnce you toss your [armor] on the floor, however, Arian's eyes widen as he realizes you're not entirely female; he eyes your " +
                    this.multiCockDescriptLight() +
                    " and the moistening pussy between your legs with equal parts wonder and arousal."
            );
            this.outputText(
    ...
    private hermsLikeFemaleArian(): void {
        this.clearOutput();
        this.outputText(
            "You tell him that while you do like to play with guys once in a while, you prefer girls."
        );
    ...
    private youDontMindBeingGayForArian(): void {
        this.clearOutput();
        this.outputText(
            "You tell him that you don't have a problem with males, as long as they're cute.  You smile at him.  \"<i>You... do you really think I'm cute?</i>\""
        );
        this.outputText(
    ...
    private youLikeGirlsNotSickLizardDudes(): void {
        this.clearOutput();
        this.outputText(
            'You tell him that you prefer females.... Arian looks at you expectantly.  "<i>So... if I was a girl... then you wouldn\'t mind?</i>"'
        );
        this.outputText(
    ...
    private arianStoryDialogue1(): void {
        ...
        this.clearOutput();
        this.outputText(
            "You feel like you'd like to know a bit more about Arian, so you ask if he would mind sharing some of [Arian eir] history with you.  After all, as a survivor from at least the early days of the demon war, and a wizard to boot, he's got to have some stories up [Arian eir] voluminous sleeves."
        );
        this.outputText(
        ...
        if (this.player.findStatusAffect(StatusAffects.MetMarae) >= 0)
            ...
            this.outputText(
                "you'd guess a long while ago given the general mess they seem to have made of the world."
            );
    ...
    private arianStoryDialogue2(): void {
        ...
        this.arianHealth(1);
        this.outputText(
            "You look Arian over, remarking that he seems to be getting better after all."
        );
        this.outputText(
    ...
    private arianStory2DropIt(): void {
        this.clearOutput();
        this.outputText(
            "Though you do feel a little curious, you decide to stop making him uncomfortable, and tell him that it's okay, you'll let him get some sleep now."
        );
        this.outputText(
    ...
    private arianStoryPry(): void {
        this.clearOutput();
        this.outputText(
            "Oh, no, you're not letting him wriggle out of this that easily.  You playfully tap [Arian eir] nose and tell him he should come clean and confess"
        );
    ...
    private noArianShouldntMagicTeach(): void {
        this.clearOutput();
        this.outputText(
            "You think it over for a moment, and then tell Arian that while you are flattered by the offer and willing to consider it, you can't say that you want to study magic right this moment.  You'd like to discuss it at some other time, please."
        );
        this.outputText(
    ...
    private arianImbue(): void {
        ...
        this.arianHealth(1);
        this.outputText(
            'Before you can say anything, Arian gasps, "<i>Oh, [name].  I have a surprise for you.</i>"  Arian says with a smile.'
        );
    ...
    private noPlotSexNauArian(): void {
        this.clearOutput();
        this.outputText(
            "You apologize to the lizan, telling [Arian em] that you aren't in the mood right now...."
        );
        this.outputText(
    ...
    private arianPlot4(): void {
        ...
        this.arianHealth(1);
        this.outputText(
            'Before you can say anything, Arian says, "<i>Oh, I have good news, [name]!</i>"'
        );
    ...
    private arianPlot5(): void {
        ...
        this.arianHealth(1);
        this.outputText(
            "Before you can say anything, Arian stops you.  \"<i>I've been meaning to ask you something, [name].  I've been feeling a lot better lately; in fact, I may be even better than I was before.</i>\"  Arian blushes."
        );
        ...
        if (this.camp.companionsCount() > 1)
            this.outputText(
                "  There are other people living with you already, so what's one more?  Right?"
            );
        this.outputText('</i>"');
    ...
    private talkToArianChoices(): void {
        this.clearOutput();
        this.outputText(
            'You tell Arian you\'d like to talk to [Arian em].  Arian smiles at the prospect of chatting with you.  "<i>I love talking with you; so what do you want to talk about?</i>"'
        );

        this.menu();
        if (this.flags[kFLAGS.ARIAN_VIRGIN] > 0)
            this.addButton(0, "Sexy Talk", this.arianSexingTalk);
        if (this.flags[kFLAGS.ARIAN_S_DIALOGUE] >= 3)
            this.addButton(1, "Teach Magic", this.arianMagicLessons);
        if (!this.arianFollower() && this.flags[kFLAGS.ARIAN_S_DIALOGUE] >= 6)
            this.addButton(4, "Invite2Camp", this.inviteArianToCamp);
        if (this.flags[kFLAGS.ARIAN_VIRGIN] == 0 && this.flags[kFLAGS.ARIAN_S_DIALOGUE] < 3)
            this.outputText(
    ...
    private arianMagicLessons(): void {
        ...
        // (else if PC int < 50 //Teach Charge Weapon if PC doesn't know.
            ...
            if (this.player.findStatusAffect(StatusAffects.KnowsCharge) < 0) {
                ...
                this.outputText(
                    " away, you start pondering Arian's lesson of the day.  You wonder if you could perhaps apply the channeling principle to strengthen your [weapon] in combat."
                );
    ...
    private arianSexingTalk(): void {
        ...
        this.arianHealth(1);
        this.outputText(
            "You smirk knowingly at [Arian em] and ask how [Arian ey] feels about sex now that [Arian ey]'s had [Arian eir] first time?"
        );
    ...
    private yesYouButtslutIllFuckYou(): void {
        this.clearOutput();
        this.outputText(
            "How could you refuse such a request?  You tell [Arian em] to strip and get ready."
        );
        this.outputText(
    ...
    private goddamnitNoYouButtSlut(): void {
        this.clearOutput();
        this.outputText(
            'You apologize, but you really can\'t do that right now.  Arian looks a bit disappointed, but smiles at you all the same.  "<i>Oh... okay.  Next time then?</i>"'
        );
        this.outputText("\n\nYou nod.");
    ...
    private sexTalkFinish(newl = false): void {
        ...
        this.outputText(
            "Satisfied with your little chat, you pat the lizan's head and excuse yourself, heading back to camp."
        );
        this.doNext(this.camp.returnToCampUseOneHour);
    }
    ...
    private inviteArianToCamp(): void {
        this.clearOutput();
        this.outputText(
            "You ask the lizan if [Arian ey] still really wants to leave [Arian eir] comfortable home in the city and come out to live with you in your little camp in the wastelands?"
        );
        ...
        this.outputText(
            "\n\n\"<i>I'm happy for you, " +
                this.arianMF("master", "mistress") +
                ', I really am,</i>" Boon says.'
        );
        this.outputText(
            "\n\n\"<i>Make sure you're taking care of yourself, " +
                this.arianMF("master", "mistress") +
                ".  Don't forget to bathe regularly and eat well... and contact us once in awhile... and visit on the holidays,</i>\" Laika says, sniffing."
        );

        this.outputText(
            "\n\nArian smiles at [Arian eir] pupils.  \"<i>I'm not your " +
                this.arianMF("master", "mistress") +
                " anymore.  From now on you're full-fledged mages.  Remember all that you learned, and I'm sure you'll be okay.</i>\""
        );
        this.outputText(
        ...
        this.outputText(
            '\n\n"<i>Please, [name], take good care of the ' +
                this.arianMF("mas", "mis") +
                "... err... Arian.  [Arian Ey] might be a handful sometimes, but [Arian ey]'s a really good person,</i>\" Boon says with a bow."
        );

        this.outputText(
            '\n\n"<i>If ' +
                this.arianMF("master", "mistress") +
                "... I mean... if Arian gives you any trouble call us and we'll come running to give [Arian em] a good scolding,</i>\" Laika says with a bow."
        );
    ...
    private takeYerLizardHomePartII(): void {
        this.clearOutput();
        this.outputText(
            'Upon arriving at the camp, the first thing Arian notices is the shimmering portal.  "<i>Is this... where you came from?</i>" Arian asks.'
        );
        this.outputText(
    ...
    private arianSexMenu(output = true): void {
        ...
        if (this.player.hasVagina()) {
            ...
            if (
                this.flags[kFLAGS.ARIAN_VAGINA] > 0 &&
                (this.player.hasKeyItem("Dildo") >= 0 ||
    ...
    private giveArianAnal(): void {
        ...
        else this.outputText(this.images.showImage("arianmale-home-giveArianAnal"));
        ...
        if (this.flags[kFLAGS.ARIAN_ANAL_XP] < 33) {
            ...
            if (this.flags[kFLAGS.ARIAN_VAGINA] < 1) {
                ...
                this.outputText(
                    " isn't going to make it any smaller either.</i>\"  Arian fidgets and shudders as you stroke [Arian eir] behind."
                );
            }
            this.outputText(
            ...
            this.outputText(
                "\n\nYou begin jerking yourself off, using slow, careful strokes to bring precum dribbling from your cock, smearing it across your shaft in an effort to lube it for your lizan lover.  Finally deciding you've made yourself slick enough, unable to resist making your move, you take hold of [Arian eir] butt for support and begin pushing your " +
                    this.cockDescript(x) +
                    " against [Arian eir] virginal back door.  Arian lets out a girly yelp and [Arian eir] sphincter suddenly clenches up, halting your entry.  You ask what's wrong."
            );
            this.outputText(
            ...
            this.outputText(
                ' and you warn Arian that you\'re going to cum inside [Arian em] if you keep going.  Arian drools in pained pleasure, for a moment you wonder if [Arian ey] even heard you.  "<i>Cum.  Cum inside me.  I want it... all of it! Ah!</i>"'
            );
            ...
            // (High Cum Amount)
                ...
                this.outputText(
                    "\n\nFor a moment you hesitate....  Arian is clearly inexperienced and you know your load is nothing if not enormous; you worry if the inexperienced lizan will be able to handle you, but your worries are soon forgotten; it's much too late to worry about it now.  Your spunk jets inside the moaning lizan like a fire hose, dowsing both your and [Arian eir] flames of lust with its immense output of white, creamy goodness.  Arian's belly protrudes like a gravid woman's belly - no, actually, it's even wider, and yet the lizan's lewd little hole never tires as it milks you, even though by now Arian is beyond full - copious amounts of cum leak from around your " +
                        this.cockDescript(x) +
                        " as [Arian eir] ass fails to contain it all."
                );
            }
            this.outputText(
            ...
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0 && this.flags[kFLAGS.ARIAN_VAGINA] > 0)
                this.outputText(" and ");
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
                this.outputText(
                    "wetting both your and her legs with a healthy serving of lizan pussy juice"
                );
            this.outputText(
                ".  You shudder and gasp until, utterly spent, you can't resist sinking down atop Arian and bearing [Arian em] into the comforting embrace of the bed.  You lay there, recovering your strength from your most delightful exertions, wriggling to give the lizan a proper snuggling while you lay there."
            );
        ...
        // Loose, sensitive enough to make Arian scream in pleasure. (It's like a tight vagina, feels like one too, minor pain)
            ...
            if (
                ...
                (this.flags[kFLAGS.ARIAN_VAGINA] == 1 && this.flags[kFLAGS.ARIAN_COCK_SIZE] == 0)
            )
                this.outputText("s");
            this.outputText(
                " your touch.  What a little slut Arian is turning into; [Arian ey]'s already raring to go, and even knowing it's going to be [Arian eir] ass that's getting fucked, too!"
            );
            ...
            this.outputText(
                '.  "<i>Yesssss.... Oh, sometimes it hurts a bit, but the feeling, the pleasure, the pain.... It\'s heavenly.</i>"'
            );
            this.outputText(
            ...
            this.outputText(
                ".  Arian is only capable of moaning and shuddering as [Arian eir] powerful orgasm rocks the poor lizan to [Arian eir] core.  The extra tightness of [Arian eir] contracting butthole increases the friction on your " +
                    this.cockDescript(x) +
                    ", pushing you ever closer to the climax."
            );
        ...
        // Very Loose, sensitive enough to make Arian cum just from insertion. (Feels better than anything else. Yep, Arian really becomes a buttslut at this point)
            ...
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0 && this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
                this.outputText(" and [Arian eir] ");
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
                this.outputText("erect cock is already leaking pre like a sieve");
            this.outputText(
                '.  You can hardly believe how far Arian has come; you ask [Arian em] if it really feels that good?  "<i>More than you can... ah... imagine, now fill me up!</i>" [Arian ey] demands with a sense of urgency.'
            );
            ...
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
                this.outputText(
                    "his cock throbs and shoots rope after rope of cum onto the bedsheets"
                );
            this.outputText(
                '. Overwhelmed by your sudden intrusion, Arian collapses forward, burying [Arian eir] face on [Arian eir] pillow and dragging you on top of [Arian em].  "<i>Ah... [name], you feel so good.  It\'s amazing.  I never thought buttsex could ever feel this good.</i>"'
            );

            this.outputText(
                "\n\nYou almost blink in surprise; you were just throwing the term buttslut around as a joke, you didn't think Arian was really like that.  Still, there are far more important matters - like digging your way into the depths of Arian's greedy little ass with your " +
                    this.cockDescript(x) +
                    "!  You squeeze the lizan's scaly butt and begin to rut [Arian em] like an animal, thrusting your way in and out of [Arian eir] back passage with all the eagerness you can muster."
            );
            ...
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
                    this.outputText(
                        "cocks are still rock hard, and surprisingly, still leaking ropes of cum"
                    );
                else
                    this.outputText(
                        "cock is still rock hard, and surprisingly, still leaking ropes of cum"
                    );
                if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) this.outputText(", and her");
            }
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
                this.outputText(
                    " pussy squirting juices against your [legs] with each shuddering impact of your [hips]"
                );
            this.outputText(
                '.  "<i>Ah!  Yes!  More, give me more!  Harder!  Faster!</i>" Arian pleads, before biting on [Arian eir] pillow in pleasure.'
            );
            ...
            this.outputText(
                "\n\nYou can feel the pressure on [Arian eir] sphincter increasing and guess Arian must be close to another orgasm; you're not very far yourself, and if [Arian ey] keeps squeezing and massaging your " +
                    this.cockDescript(x) +
            ...
            this.outputText(
                '\n\n"<i>No! Ah, yes!  Cum inside me, [name]!  I need your seed inside my naughty ass.  I need to feel you filling me up, using me like the buttslut I am!  I want to cum with you!</i>"  You lift a brow, of all the people you know, Arian is the last one you\'d expect to hear this from... what would [Arian eir] apprentices say if they heard their ' +
                    this.arianMF("master", "mistress") +
                    " begging to be used like that?  \"<i>Ah... I don't care, just fill me up with your hot, slimy spunk!</i>\"  It would seem the lizan mage is too far gone to give you a straight answer.  You'll have to talk to [Arian em] after this."
            );
            ...
            // (High Cum amount)
                ...
                this.outputText(
                    '\n\nYou have no doubt Arian will appreciate your prodigious load, so you make sure to press as deep as you can into [Arian eir] ass to ensure [Arian ey] will keep it all inside, then finally let your cum flow out of you and into the cock and cum hungry ass, eagerly massaging your shaft.  "<i>I can feel it!  It\'s going to be a big one!  Shoot it!</i>" [Arian ey] screams in ecstasy.  You groan and begin dumping obscene amounts of seed into your lizan lover.  The first few jets inflate [Arian eir] belly slightly, and you dart a hand to feel it expand with every subsequent blast of sticky jism.  "<i>So much cum!  So good... more!</i>"  Arian demands, blissed out by your pumping appendage.  [Arian Eir] ass never stops its assault on your ' +
                        this.cockDescript(x) +
                        ' intent on massaging you as you orgasm, even as you fill Arian beyond full and some cum begins backflowing out of [Arian eir] ass.  "<i>Hmm, yesss... so full....</i>"  You pound into [Arian em] a few more times, ensuring you spill every last drop of cum.'
                );
            }
            this.outputText(
            ...
            if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) {
                this.outputText(
                    "  Her cunt grips the air and blasts one last jet of wetness at your crotch."
                );
            }
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
                this.outputText(
                    "  [Arian Eir] throbbing cock flexes and shoots one last rope of lizan-cum against your belly."
                );
            this.outputText(
                '  [Arian Eir] ass grips you forcefully, forming a vacuum and finally loosens.  "<i>That... that wash da best,</i>"  Arian slurs, before finally collapsing on [Arian eir] bed.'
            );
    ...
    private getBlownByArian(): void {
        ...
        else this.outputText(this.images.showImage("arianmale-home-getbj"));
        this.outputText(
        ...
        if (this.flags[kFLAGS.ARIAN_HAS_BLOWN] == 0) {
            ...
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 3)
                this.outputText(
                    "  You notice Arian's exposed shaft slowly hardening at your invitation."
                );
        ...
        this.outputText(
            ' pot belly.  [Arian Ey] lets go of your dick with a sigh of pleasure and lays back on the bed.  "<i>Ah... I\'m full.</i>"'
        );

        this.outputText("\n\nYou tell [Arian em] that [Arian ey] looks full");
        if (this.player.cumQ() >= 1500)
            this.outputText(
                ", in fact, you're amazed [Arian ey] didn't pop with how much [Arian ey] drank"
            );
        ...
        if (this.player.cumQ() >= 1000)
            this.outputText(
                " which, considering the fact [Arian ey] looks like a mother ready to birth a toddler from the size of [Arian eir] belly, isn't as easy as you'd think"
            );
        this.outputText(
            ".  Letting the lizan use your shoulder, you pull [Arian eir] sheets up and help [Arian em] back in the bed, where [Arian ey] flops down with a protest of springs"
        );
        if (this.player.cumQ() >= 250)
            this.outputText(
                ", [Arian eir] gut audibly sloshing as the cum inside is churned by the impact"
            );
        this.outputText(".");
        ...
        if (!this.arianFollower())
            this.outputText(
                " back to camp, stopping only to notify Laika and Boon that Arian is sleeping."
            );
    ...
    private penetrateArian(): void {
        ...
        this.outputText(
            "You admire the transgendered lizan's body, from her feminized features, down past her [arian chest], all the way to her shapely thighs.  You tell Arian that the change looks very good on her; few boys would really be so naturally pretty when turned into a "
        );
        ...
        this.outputText(
            '.  Arian swallows audibly.  "<i>What now?</i>" she asks, already panting in lust.'
        );
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            ...
            this.outputText(
                " bobbing just out of your field of vision, leaking pre all over your belly."
            );
        }
        this.outputText(
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            ...
            this.outputText(
                "a small rope of pre over her belly, anticipating what's to come as she braces herself for the inevitable pounding you're about to give her."
            );
        }
        this.outputText(
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            ...
            this.outputText(
                " as she splashes her [arian chest] and face with her own futa-lizan seed.  "
            );
        }
        this.outputText(
            "Her walls grip you tightly, almost painfully, as a flood of juices hit the " +
                this.player.cockHead(x) +
                " of your " +
                this.cockDescript(x) +
                " with the force of a tidal wave, only to spill around her spread nethers and run down your lower body.  Her sopping wet pussy works overtime, trying its best to pull you in as deep as possible, intent on sucking all the cum out of "
        );
        ...
        if (this.player.hasVagina())
            this.outputText(
                " A wash of fluids spills down your body from your cunt as it spasms in release and y"
            );
    ...
    private getButtWreckedByArian(): void {
        ...
        else this.outputText(this.images.showImage("arianmale-home-getArianAnal"));
        this.outputText(
        ...
        this.outputText(
            "cock.  Slowly and gently you begin to stroke its strange, knobbly surface, your fingers eliciting moans and gasps from Arian as [Arian ey] shudders under your touch.  Precum begins to ooze from the reptilian prick's head, and you purposefully rub it into [Arian eir] shaft as a kind of makeshift lubricant."
        );
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            this.outputText(
                "stroking [Arian eir] other cock and sliding it between your ass cheeks, "
            );
        this.outputText(
            "caressing the base of [Arian eir] tail.  With a devious grin, you begin to gently but insistently slide your finger into [Arian eir] "
        );
        ...
        this.outputText(
            "clenching your inner walls to grip and feel the excitingly unusual lumpy, bulbous texture of [Arian eir] shaft inside you.  You moan and gasp, telling [Arian em] that [Arian ey] feels so unique, and yet [Arian ey] stimulates you so wonderfully."
        );
        ...
        this.outputText(
            ".  You're so close now, you can feel it.  You tell Arian you're going to cum, and beg [Arian em] to cum with you."
        );
        ...
        this.outputText(
            ', signaling [Arian eir] oncoming orgasm.  [Arian Ey] grabs the sheets on [Arian eir] bed and curls [Arian eir] toes.  "<i>...[name]!</i>"  Jet after jet of lizan spunk paints your bowels white'
        );
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText(", as well as your butt");
        this.outputText(", as Arian climaxes");
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
            this.outputText(
                ", her own pussy clenching and drizzling juices on the matted sheets beneath the two of you."
            );
        else this.outputText(".");
        ...
        if (this.player.hasCock())
            this.outputText(
                "  [EachCock] spasms and shoots rope after rope of cum, painting Arian's [arian chest] white, as well as the groaning lizan's face."
            );
        if (this.player.hasVagina())
            this.outputText(
    ...
    private suckAriansDick(): void {
        ...
        else this.outputText(this.images.showImage("arianmale-home-suckariandick"));
        this.outputText(
            "You make a show of lewdly licking your lips and ask Arian if [Arian ey]'d be willing to let you have a little taste of lizan essence...?"
        );
        this.outputText(
        ...
        this.outputText(
            "cock, marveling at the strange textures of its bumpy, knobbly, bulbous surface.  It feels so strange to you, and you continue to stroke it eagerly."
        );
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(
            ".  You continue to slide your hand along the increasingly slick surface, playfully asking if your hand really feels that good to [Arian em]?"
        );
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(
            " a long, loving, slurpy lick.  The taste is surprising; a sort of sugary-tinted spice, not the usual salt of another creature.  It's actually quite pleasant, and you find yourself running your tongue up and down [Arian eir] prick as if it were some kind of candied stick, eagerly sucking and slobbering as you coax more of [Arian eir] yummy goo from its strangely-shaped tip."
        );
        ...
        this.outputText(
            ".  [Arian Eir] legs close around your back, effectively keeping you from moving too far away as [Arian ey] finishes giving you all of [Arian eir] load."
        );
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            this.outputText(
                ", oblivious to the second cock spewing cum all over your [chest] in your eagerness"
            );
        this.outputText(
            ".  After [Arian ey] finally finishes, you continue to lick and suckle for a few minutes longer, making sure you've lapped up every last drop.  Then and then alone do you lift your head, flirtatiously licking your lips to tease the now-spent lizan."
        );
        ...
        this.outputText(
            " rolling your neck in pleasure as [Arian ey] rubs it with [Arian eir] reptilian tail."
        );
        ...
        this.outputText(
            " for emphasis, kiss [Arian em] gently on the tip of [Arian eir] nose, and then get your clothes together, planning on getting dressed and heading back out again."
        );
    ...
    private getPenetratedByArianAndHisHitlerMustache(): void {
        ...
        else this.outputText(this.images.showImage("arianmale-home-getpenetrated"));
        ...
        this.outputText(
            "; then you turn to look at Arian and ask [Arian em] how would [Arian ey] feel about giving [Arian eir] "
        );
        ...
        this.outputText(
            "expose your [vagina] to [Arian eir] viewing pleasure.  The reaction is almost instantaneous; Arian's "
        );
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(
            " and get ready to thrust in; otherwise neither of you are going to be feeling good any time soon."
        );
        ...
        this.outputText(
            ".  [Arian Ey] looks at you, waiting for you to confirm that [Arian ey] should indeed get going.  You smile and nod"
        );
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) {
            ...
            if (this.player.hasLongTail())
                this.outputText(
                    ", further encouraging [Arian em] to do so, by looping your tail around [Arian eir] waist"
                );
            this.outputText(
                ".  The lizan doesn't need much encouragement to comply, easily snuggling against you, breast to breast, and sighing softly.  \"<i>I'm sorry.  Sometimes I wonder why you bother doing something like this, with someone like me,</i>\" [Arian ey] says, a hint of sadness in [Arian eir] voice."
            );
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(
            " unlike anything else.  You praise the lizan on being a quick study, letting go of [Arian eir] tail and grabbing onto [Arian eir] scaly shoulders instead, giving [Arian em] a few more directions so [Arian ey] can catch your most sensitive spots."
        );
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(
            " swelling inside you, the lizan grabbing your shoulders and starting to thrust [Arian em]self back into you once more."
        );
        ...
        this.outputText(
            " with a second helping of lizan spunk, dredging up every last drop of jizz left in [Arian eir] internal balls before, with a groan, [Arian ey] sinks down atop you.  Arian's orgasm pushes you over the edge, and you find your pussy "
        );
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("and ass ");
        ...
        this.outputText(
            " best to milk the poor lizan of all [Arian ey] is worth, until finally with one last spasm, you slump down and release the lizan shaft"
        );
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
        this.outputText(
            " deeply embedded inside you, some of your mixed juices running down to mat Arian's bed."
        );
    ...
    private doublePenetrateArian(): void {
        ...
        this.outputText(
            "You look over your feminine lizard lover, and feel your " +
                this.multiCockDescriptLight() +
                " starting to stir in your [armor].  Since you have enough tools for the job, and Arian has enough holes, you ask if Arian would be willing to let you fuck her in both her ass and pussy at the same time?"
        );
        ...
        this.outputText(
            ' ass and dripping wet vagina; a pair of clawed hands reach behind to spread her butt open for you.  "<i>Okay, I\'m ready.</i>"  Arian says, looking behind at you.'
        );
        ...
        this.outputText(
            "\n\nThat's better, you say.  With that, you start to thrust yourself into her two holes.  Penetrating both of Arian's holes is a unique feeling; her ass hugs your " +
                this.cockDescript(y) +
                " tightly, trying to prevent you from moving as it does its best to keep you hilted deeply within; while her pussy, so slick and moist, massages your shaft expertly. For a moment you wonder if you even have to move."
        );
        ...
        this.outputText(
            "\n\n\"<i>What do you think I'm trying to do!?  Now get in here and paint my womb white!</i>\" Arian snaps, bracing herself on her bed and allowing her ass and pussy both to suck you in with surprising force.  As soon as you're hilted within both holes, her ass clamps shut on your " +
                this.cockDescript(y) +
                ", while her pussy's contractions begin truly milking you for all you're worth.  Stuck as you are, you have no option but to sit back and enjoy her contractions as you feel yourself nearing the edge of an inevitable orgasm."
        );
        ...
        if (this.player.cumQ() <= 250) {
            this.outputText(
                "\n\nYour " +
                    this.multiCockDescriptLight() +
                    " explode inside Arian's eager holes, giving them the liquid warmth they so crave.  The massage that your two cocks are receiving only enhance the intense feeling, and you find yourself cumming more than usual.  They don't stop massaging you for more, even as you stop unloading."
            );
        ...
        // (Medium Cum Amount)
            ...
            this.outputText(
                "\n\nLiquid lust floods Arian's insides, as your " +
                    this.multiCockDescriptLight() +
                    " do their best to relieve "
            );
            if (this.player.balls == 0) this.outputText("themselves");
            else this.outputText("your [balls]");
            this.outputText(
                " of their load; a load Arian is not only pleased to accept, but also eager to relieve you of every single stray drop off.  The tightness of her ass, pressing down on your " +
                    this.cockDescript(y) +
                    ", her pussy milking on your " +
                    this.cockDescript(x) +
                    ".  How could anyone refuse such an invitation?  You let yourself go, stuffing the eager lizan with more cum than you thought yourself capable of producing."
            );
        ...
        this.outputText(
            "\n\nArian's ass goes slack around your " +
                this.cockDescript(y) +
    ...
    private arianDocking(): void {
        ...
        else this.outputText(this.images.showImage("arianmale-home-docking"));
        this.outputText(
            "You set your eyes on Arian's genital slit, and then smile at [Arian em].  You ask how [Arian ey]'d feel about ‘hiding' your cock, rather than [Arian eirs], inside [Arian eir] slit?"
        );
        ...
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0) {
            ...
            this.outputText(
                "[Arian ey]'s already hiding two in there... an extra cock won't make such a huge difference, would it?"
            );
        }
        ...
        this.outputText(
            ", the friction is absolutely incredible, sending sparks of pleasure cascading along your shaft.  You moan in delicious lust and tell Arian that this is absolutely incredible."
        );
        ...
        this.outputText(
            " to feebly shoot a couple more ropes of jism, before returning limply to "
        );
        if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("their");
        else this.outputText("its");
    ...
    private arianVitalityTincture(): void {
        this.clearOutput();
        this.outputText(
            "Fishing around amongst your pockets, you withdraw a vial of that strange potion Giacomo peddles and offer it to the sickly lizan, explaining it will bolster [Arian eir] constitution and fill [Arian em] with permanent vitality."
        );
    ...
    private giveIncubusDraftToArian(): void {
        ...
        this.outputText(
            "Fishing around in your pockets, your hand closes on the vial of purified incubus draft.  You offer this to Arian, asking "
        );
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 0) {
            ...
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0)
                this.outputText(
                    "shaft emerges from its hiding place.  It throbs and grows, settling in its"
                );
            else
                this.outputText(
                    "pair of shafts emerge from their hiding place.  They throb and grow, settling in their"
                );
            ...
            if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 2) {
                ...
                this.outputText(
                    " until two thirds are inside, then it doesn't looks like [Arian ey] can take in anymore."
                );
                ...
                this.outputText(
                    "\n\nYou can't resist reaching down and gently tweaking the exposed tip.  Mmm... your little " +
                        this.arianMF("boy-toy", "herm-toy") +
                        " is going to be a very naughty thing from now on, you tease, unless [Arian ey] intends to start wearing a loincloth?"
                );
                ...
                if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) {
                    ...
                    if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
                    this.outputText(
                        " starts hardening, no longer able to hide Arian's excitement inside her protective slit."
                    );
                } else {
                    this.outputText(
    ...
    private succubiMilkForArian(): void {
        ...
        this.outputText(
            "Fishing out the bottle of purified demon's milk, you ask if Arian is willing to get "
        );
        ...
        // (else If ArianCockSize > 1) //Dick shrinkage.
            ...
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
            this.outputText(
                ".  \"<i>Oh... that's good.  Right?</i>\" Arian looks at you nervously.  You think it over, and confess that while you are more attracted to women than men, you won't force Arian to become fully female if she doesn't want to."
            );
            this.outputText(
    ...
    private giveArianLactaid(): void {
        ...
        this.outputText(
            "Your hand closes around the vial of lactation-inducing potion that is Lactaid.  You almost reject it automatically, but then you stop and think.  There's odder things in this world, after all.  You remove the vial and ask Arian if [Arian ey] would be willing to let you see what lizan milk tastes like."
        );
        ...
        if (this.flags[kFLAGS.ARIAN_BREASTS] == 0) {
            ...
            this.outputText(
                ' breasts grow before your eyes.  "<i>My breasts... d-do something, [name]!</i>" Arian pleads.  Unsure of what you should be doing, you grab her breasts and begin kneading them as they grow in your hands; her erect nipples poking your palms.  "<i>Ah... that feels good!  Don\'t stop!</i>"'
            );
    ...
    private useReductoOnAriansBreasts(): void {
        ...
        this.outputText(
            ' breasts.  "<i>Okay... I suppose it would be good to lose a bit of weight up here and spare my back.</i>"  Arian smiles, opening the tube of Reducto and her robes; then squeezing the contents of the paste her hands.'
        );
        ...
        if (this.flags[kFLAGS.ARIAN_BREASTS] > 1) {
            ...
            this.outputText(
                "The two of you watch Arian's breasts, but nothing seems to be changing.  You reach forward to feel them, and indeed they haven't shrunk down at all."
            );
            this.outputText(
    ...
    private useReductoOnArianCocks(): void {
        ...
        this.player.consumeItem(this.consumables.REDUCTO);
        this.outputText(
            "You point at [Arian eir] crotch, mentioning that you'd like [Arian em] to be smaller."
        );
        this.outputText(
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] == 3) {
            ...
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0) this.outputText("s");
            this.outputText(
                " to a more manageable size.  Once the transformation is complete Arian tries to fully retract [Arian eir] cock"
            );
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s back into their");
            else this.outputText(" back into its");
            ...
            this.outputText(
                ", and into full erection.  Finally [Arian ey] begins applying the Reducto."
            );
            ...
            this.outputText(
                " original size.  Once the transformation is complete, [Arian ey] retracts "
            );
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("them");
            else this.outputText("it");
            ...
            this.outputText(
                "Then, [Arian ey] coaxes [Arian eir] twin reptilian-peckers out of their hiding place.  Finally [Arian ey] applies the paste to both shafts in turn."
            );
    ...
    private useReductoOnAriansAsshole(): void {
        ...
        this.player.consumeItem(this.consumables.REDUCTO);
        this.outputText(
            "You ask Arian to hand the tube of reducto back over to you, telling [Arian em] that you want to make [Arian em] a little tighter when you do [Arian em] from behind.  The lizard-"
        );
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            if (this.flags[kFLAGS.ARIAN_VAGINA] == 0) this.outputText("man");
            else this.outputText("herm");
    ...
    private giveArianReptilum(): void {
        ...
        this.outputText(
            "Fingering the vial of reptilium, you smirk to yourself.  Quickly wiping it off your face, you instruct Arian to close [Arian eir] eyes and open [Arian eir] mouth, as you have a special surprise for [Arian em]."
        );
        ...
        this.outputText(
            "\n\nYou shake your head and tell [Arian em] you'd rather [Arian ey] didn't lick your butt.  Why don't you go and get [Arian em] some candy instead?  \"<i>Candy!?  I love candy!  You can smear chocolate on yourself and I could lick it clean! Then we'd get chocolate flavored cloud ice-cream with " +
                this.player.race() +
                ' musk!  What a great idea!  Get your undies off so I can get started!</i>"  [Arian Ey] pounces on you, effectively removing your underpants and exposing your '
        );
        if (this.player.hasCock()) {
        ...
        this.outputText(
            '.  "<i>Hmm, that looks much tastier than candy actually.</i>"  [Arian Ey] licks your '
        );
        ...
        this.outputText(
            '.  "<i>Yummy!  I could use seconds, but roleplay time is over; let\'s... masturbate each other!</i>"  [Arian Ey] begins stroking [Arian eir] '
        );

        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
            this.outputText(this.images.showImage("arianfemale-home-mutualmasturbation"));
        else this.outputText(this.images.showImage("arianmale-home-mutualmasturbation"));
    ...
    private treatCorruption(): void {
        this.clearOutput();
        this.outputText(
            "You ask Arian if [Arian ey] thinks [Arian ey] can help you reduce some of the taint that has infected your soul."
        );
        ...
        if (this.flags[kFLAGS.ARIAN_TREATMENT] == 1) {
            ...
            if (this.player.cor == 0)
                this.outputText(
                    " - in fact, you're quite sure that Arian has purified you entirely"
                );
    ...
    private imbueTalisman(): void {
        this.clearOutput();
        this.outputText(
            "You tell Arian that, if it's not too much trouble, you'd like [Arian em] to "
        );
        ...
        */
        ...
        this.menu();
        if (
            this.player.hasItem(this.useables.B_CHITN, 2) &&
            this.player.hasItem(this.useables.T_SSILK)
        )
            this.addButton(0, "Shielding", this.arianSpellPlace, "Shielding Spell");
        if (
            this.player.hasItem(this.consumables.GOB_ALE, 2) &&
            this.player.hasItem(this.consumables.S_GOSSR)
        )
    ...
    private arianSpellPlace(spell: string): void {
        this.clearOutput();
        this.outputText(
            "You tell Arian that you want [Arian em] to place the " +
                spell +
                " spell in your talisman for you."
        );
        ...
        this.outputText(
            "\n\n(<b>Your talisman has been imbued with the " +
                spell +
                ". You can use it from the M. Specials menu in combat.</b>)\n\n"
        );
        this.clearCharges();
    ...
    private clearCharges(): void {
        if (this.player.findStatusAffect(StatusAffects.ShieldingSpell) >= 0)
            this.player.removeStatusAffect(StatusAffects.ShieldingSpell);
        if (this.player.findStatusAffect(StatusAffects.ImmolationSpell) >= 0)
    ...
    */
    ...
    public sleepWithArian(newl = false): void {
        ...
        this.outputText(
            "\n\nInside, you find the " +
                this.arianMF("male", "girly") +
                " lizan standing naked before [Arian eir] bed, stretching [Arian eir] arms and tail as widely as they can and groaning with satisfaction as [Arian eir] joints pop.  Apparently not having noticed you arrive, [Arian ey] hums to [Arian emself] as [Arian ey] busies [Arian emself] with the covers of [Arian eir] bed, bending over and letting you get a good look at [Arian eir] sweet ass.  You appreciate the view for a few moments, and then gently cough to announce your presence."
        );
        ...
        if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
            this.outputText(
                "You can tell that [Arian ey] is aroused by the little show, despite [Arian eir] body doing a good job of hiding it.  "
            );
        this.outputText(
            "You just let [Arian em] squirm, pretending you don't see [Arian eir] reactions, and then, indicating the bed, you ask which of you should get in first."
        );
        ...
        this.outputText(
            '\n\nArian sighs in happiness at your close contact.  "<i>It feels so good to have your ' +
    ...
    private dontListenToLowAnalXPArian(): void {
        this.clearOutput();
        this.outputText(
            "You gently stroke the lizan's side and plant a soft, gentle kiss on [Arian eir] neck.  At your ministrations, Arian sighs and slowly falls still again.  You wait, but it seems that whatever dreams [Arian ey] was having have faded and [Arian ey]'s truly asleep now, allowing you to go back to sleep yourself."
        );
        this.menu();
    ...
    private listenToMediumAnalXPArian(): void {
        ...
        if (this.player.hasCock()) {
            ...
            this.outputText(
                ' still worming its way into [Arian eir] fuckable ass, but it really does look like [Arian ey]\'s fast asleep.  Weird.  "<i>So many muffins... all [name] flavored...</i>"  [Arian ey] calms down and begins snoring lightly.'
            );
            this.outputText(
            ...
            this.outputText(
                ".  But as payback, [Arian ey]'s going to sleep with you inside, not that it seems like it'll be much of a problem to [Arian em]... Too bad you can't hold an erection all the time while asleep, so you know eventually you'll slip out..."
            );
        } else {
            this.outputText(
    ...
    private TeaseHighAnalXPArian(): void {
        ...
        if (this.player.hasCock()) {
            ...
            this.outputText(
                "\n\nIs that so, you ask?  Does [Arian ey] really want a big squishy belly, stuffed full of hot, salty " +
    ...
    public wakeUpAfterArianSleep(): void {
        ...
        if (this.player.hasCock()) {
            ...
            if (this.flags[kFLAGS.ARIAN_ANAL_XP] < 33) {
                ...
                this.outputText(
                    "\n\nArian promptly nuzzles your " +
                        this.multiCockDescriptLight() +
                        ", rubbing [Arian eir] face all over "
                );
                ...
                this.outputText(
                    "\n\nYou promptly seize the lizan in a tight embrace, already guiding your [cock smallest] towards " +
                        this.arianMF("his naughty boy-pussy", "her tail-hole") +
                        " with a smile on your face.  You teasingly muse aloud about who was saying you were giving [Arian em] the choice to say no?"
                );
                this.outputText(
                ...
                if (this.flags[kFLAGS.ARIAN_VAGINA] > 0) {
                    this.outputText(
                        this.arianMF("his", "her") +
                            " pussy is squirting her juices all over your lower body"
                    );
                    if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                        this.outputText(" and ");
                    } else this.outputText("; ");
                ...
                if (!this.player.hasVagina())
                    this.outputText(
                        "Did [Arian ey] always know [Arian ey] wanted a man to top [Arian em], or is this development new to [Arian em]?"
                    );
                else
                    this.outputText(
                        "Does [Arian ey] really get off on having a girl with a dick all her own fuck [Arian em] like a girl, hmm?"
                    );
            ...
            if (this.flags[kFLAGS.ARIAN_ANAL_XP] < 33) {
                ...
                if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
                    ...
                    this.outputText(".");
                } else if (this.flags[kFLAGS.ARIAN_VAGINA] > 0)
                    this.outputText(
                        "and her moist pussy, begins forming a stream of juices that gently run its course through the inside of Arian's thighs, evaporating before hitting the floor due to the lizan's currently elevated body heat."
                    );
    ...
    public arianEggingEvent(): void {
        ...
        if (this.flags[kFLAGS.ARIAN_EGG_CHAT] == 0) {
            ...
            this.outputText(
                "As you are about to enter Arian's tent, you hear a moan emanate from within.  Those aren't moans of pleasure though.  They are moans of discomfort.  Wondering if the lizan is all right, you decide to enter her tent."
            );
            ...
            this.outputText(
                "As you are about to enter Arian's tent, you hear a moan emanate from within.  Seems like it's that time of the month again..."
            );
            this.outputText(
    ...
    private pickAnEggArian(color = "pink"): void {
        ...
        this.outputText(
            "\n\nYou watch in wonder as a " +
                color +
                " light seems to gather within Arian's belly, slowly fading away.  The lizan mage smiles and opens her eyes, tail waving lazily behind her in happiness.  \"<i>It's done.  I tried to get all my eggs imbued, but I won't know if I was successful or not until they're laid.</i>\""
        );
    ...
    private leaveEggs(): void {
        this.clearOutput();
        this.outputText(
            "You tell her that you don't want any colored eggs from her this month.  The lizan nods, deciding it's not necessary for her to spell out that this means she'll just produce ordinary eggs and eat them for breakfast after she's laid them.  \"<i>So, do you want something?</i>\" she asks."
        );
        this.doNext(this.camp.returnToCampUseOneHour);
    }
    ...
    public arianLaysEggs(): void {
        ...
        this.outputText(
            " squatting over a wooden basin, one hand between her legs, massaging her dripping treasure, while the other is rubbing her belly to try and coax the eggs out of her."
        );
        ...
        this.outputText(
            "\n\nYou nod your head, and begin sorting through the eggs.  Most of them look like common eggs, not that much different from large chicken eggs.  Eventually, you find one that seems to be appropriate.  Separating this egg from the others, you check one more time and find out that Arian only managed to lay one " +
                color +
                ' egg.  Still, that\'s good enough for you.  You take the egg and turn to place it gently on the nearby counter.  When you turn to check up on Arian though you spot the lizan, casually lifting one of the "<i>normal</i>" eggs, sizing it up.'
        );
        ...
        this.outputText(
            "\n\nYou politely refuse, telling her that she probably needs it more than you do, since she just went through labor.  Anyways you should probably be going.  You thank the lizan for the " +
                color +
                " egg and tell her to enjoy her... meal... and point out that, while you personally appreciate the view, she should probably put on some clothes later."
        );
        this.outputText(
        ...
        else if (this.flags[kFLAGS.ARIAN_EGG_COLOR] == "rubbery black")
    ...
    private arianDildoFun(): void {
        ...
        if (this.flags[kFLAGS.TIMES_ARIAN_DILDOED] == 0) {
            ...
            this.outputText(
                " to her dripping wet pussy, clearly yearning for some action.  Reflecting that your own love-lips feel the same way, you decide to try something fun and present to Arian the dildo that you "
            );
            ...
            this.outputText(
                "You present your dildo to Arian again, telling her that this time, you want to play with your toys - after all, she enjoyed it so much the first time."
            );
        ...
        this.outputText(
            "ou slide first one finger into her slick pussy and then another, gently caressing her inner labia to see if you can find her sweet spots.  Arian's eyes open in surprise as the first pang of pleasure hits her, she moans into your kiss.  Her tongue darts to dance with your own in excitement."
        );
        ...
        if (this.flags[kFLAGS.ARIAN_COCK_SIZE] > 0) {
            this.outputText(
                "brushing it tantalizingly across the sensitive skin of her pre-oozing cock"
            );
            if (this.flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0) this.outputText("s");
            this.outputText(" before");
        }
        this.outputText(
            " brushing it against her outer lips, stroking it up and down each labia, rubbing it against her clitoris, gently pushing the very tip of it into her cunt only to then withdraw it before it can really start to stretch her cunt."
        );
        ...
        this.outputText(
            "You can feel it... the suddenness of your penetration brought the poor lizan mage to an instant orgasm."
        );
