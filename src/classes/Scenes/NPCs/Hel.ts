...
export class Hel extends Monster {
    ...
    private helAttack(): void {
        ...
        //("Misdirection"
        else if (this.player.findPerk(PerkLib.Misdirection) >= 0 && Hel.rand(100) < 10 && this.player.armorName == "red, high-society bodysuit") {
        ...
        //Determine damage - str modified by enemy toughness!
            ...
            if (damage <= 0) {
                ...
                if (Hel.rand(this.player.armorDef + this.player.tou) < this.player.armorDef) this.outputText("You absorb and deflect every " + this.weaponVerb + " with your " + this.player.armorName + ".", false);
                else this.outputText("You deflect and block every " + this.weaponVerb + " " + this.a + this.short + " throws at you.", false);
            }
            //Take Damage
            else this.outputText("The salamander lunges at you, sword swinging in a high, savage arc.  You attempt to duck her attack, but she suddenly spins about mid-swing, bringing the sword around on a completely different path.  It bites deep into your flesh, sending you stumbling back. (" + damage + ")", false);
            if (damage > 0) {
    ...
    private helAttack2(): void {
        ...
        //("Misdirection"
        if (this.player.findPerk(PerkLib.Misdirection) >= 0 && Hel.rand(100) < 5 && this.player.armorName == "red, high-society bodysuit") {
        ...
        if (damage <= 0) {
            damage = 0;
            //Due to toughness or amor...
            if (Hel.rand(this.player.armorDef + this.player.tou) < this.player.armorDef) this.outputText("The salamander's tail-swipe harmlessly deflects off your armor!", false);
            else this.outputText("The salamander's tail-swipe hits you but fails to move or damage you.", false);
        }
        //Take Damage
        else this.outputText("The salamander rushes at you, knocking aside your defensive feint and sliding in past your guard.  She lashes out at your feet with her tail, and you can feel the heated wake of the fiery appendage on your ensuing fall toward the now-smouldering grass. (" + damage + ")", false);
        if (damage > 0) {
            if (this.lustVuln > 0 && this.player.armorName == "barely-decent bondage straps") {
                this.outputText("\n" + this.capitalA + this.short + " brushes against your exposed skin and jerks back in surprise, coloring slightly from seeing so much of you revealed.", false);
                this.lust += 5 * this.lustVuln;
    ...
    private helCleavage(): void {
        //FAIL
        if ((this.player.findPerk(PerkLib.Flexibility) >= 0 && Hel.rand(100) < 6) || (this.player.findPerk(PerkLib.Evade) >= 0 && Hel.rand(100) < 10) || (this.player.spe - this.spe > 0 && Math.floor(Math.random() * (((this.player.spe - this.spe) / 4) + 80)) > 80)) {
        ...
        //Attack 3 – Lust – Cleavage (Failure)
            ...
            var lust: number = 20 + Hel.rand(10) + this.player.sens / 10 + Hel.rand(this.player.lib / 20);
    ...
    public defeated(hpVictory: boolean): void {
        if (this.findStatusAffect(StatusAffects.Sparring) >= 0) this.game.helFollower.PCBeatsUpSalamanderSparring();
