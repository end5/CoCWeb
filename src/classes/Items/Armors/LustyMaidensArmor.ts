...
export class LustyMaidensArmor extends Armor {
    public constructor() {
        super(
            ...
            "a bikini-like set of armor that could only belong to a lusty maiden",
            6,
            400,
    ...
    public get def(): number {
        if (this.game.player.hasVirginVagina())
            return 9 + this.game.flags[kFLAGS.BIKINI_ARMOR_BONUS];
    ...
    public playerEquip(): Armor {
        while (this.game.player.findPerk(PerkLib.SluttySeduction) >= 0)
            this.game.player.removePerk(PerkLib.SluttySeduction);
        if (this.game.player.hasVirginVagina()) {
            this.game.player.createPerk(
                PerkLib.SluttySeduction,
                10 + this.game.flags[kFLAGS.BIKINI_ARMOR_BONUS],
                0,
                0,
                0
            );
            ...
            this.game.player.createPerk(
                PerkLib.SluttySeduction,
                6 + this.game.flags[kFLAGS.BIKINI_ARMOR_BONUS],
                0,
                0,
                0
            );
    ...
    public lustyMaidenPaizuri(player?: Player, monster?: Monster): void {
        ...
        this.clearOutput();
        this.outx(
            `You make sure ${monster.a}${monster.short} is comfortably lying down, ${
                monster.pronoun3
            } ${monster.cockDescriptShort(0)} exposed to the air`
        );
        if (monster.lust < 50)
            this.outx(
                `, soft and not yet ready.  You purr throatily as you touch the burgeoning boner, tracing your thumb across the sensitive urethral bulge.  It pulses slowly at your touch, and the base begins to fill with blood, thickening against your palm.  You splay your remaining fingers just under the ${monster.cockHead()}, tickling around the glans until that too is flooding with blood, expanding under your caresses until it slowly lifts away from ${
                    monster.pronoun3
                } abdomen.`
            );
        else if (monster.lust < 100)
            this.outx(
                `, nicely turgid but quite ready to feel the sensuous pleasure of your girls' tight squeeze.  You lean over the defeated foe and kiss the rod just under the ${monster.cockHead()}, smiling when it expands under your slow kisses.  Your fingers move up to play with the sensitive, urethral bulge that runs along the underside, and in no time, ${
                    monster.a
                }${monster.short} is hard as a rock, so ready that ${
                    monster.pronoun3
                } member is lifting up on its own.`
            );
        else
            this.outx(
                `, bouncing with each beat of ${monster.pronoun3} heart, thick beads of pre dribbling from ${monster.pronoun3} tip as you bat ${monster.pronoun3} hands away before ${monster.pronoun1} can waste the load ${monster.pronoun1}'s saved up for you.`
            );
        this.outx(
        ...
        this.outx(
            `\n\n"<i>Such lascivious behavior!  I'll have to make sure you're thoroughly purified,</i>" you state matter-of-factly with a feigned serious look on your blushing [face].  To put proof to your taunt, you grab the throbbing shaft by the base and aim it straight up, dropping your [chest] down on either side.  The slippery, self-lubricating leather that joins the cups of your sexy, chainmail bra together loops over the top of the ${monster.cockDescriptShort(
                0
            )} to properly restrain it, pinned in the slick, sweaty valley you call your cleavage.  It thrums happily against your ${player.skin()} when you compress the jiggly flesh around it, leaning down to let it feel pleasure that rivals any pussy, no matter how wet or skilled.`
        );
        ...
        if (monster.balls > 0)
            this.outx(
                ` and fondle ${monster.pronoun3} balls one-handed, squeezing the virile orbs to try and coax more of ${monster.pronoun3} dirty, perverted thoughts to distill into salty seed`
            );
        else if (monster.short == "anemone")
            this.outx(
                ` and stroke ${monster.pronoun3} taint, even brushing over the featureless spot where an asshole would be, if she had one, to try and coax more of ${monster.pronoun3} dirty, perverted thoughts to distill into salty seed`
            );
        else
            this.outx(
                ` and stroke ${monster.pronoun3} taint, even brushing close to ${monster.pronoun3} asshole to try and coax more of ${monster.pronoun3} dirty, perverted thoughts to distill into salty seed`
            );
        this.outx(
            `.  A startled moan slips out of ${
                ...
                monster.pronoun3
            } ${monster.cockDescriptShort(
                0
            )}, taking the ${monster.cockHead()} straight into your mouth with a smooth gulp.`
        );
        ...
        this.outx(
            `  The strong, pulsing cock feels so good inside your mouth, like it belongs there, and you can't help but think that you're doing a good deed by helping ${monster.a}${monster.short} empty every last perverse desire onto your purifying breasts.`
        );
        ...
        this.outx(
            ` and tease, "<i>Oh, you're going to cum already, aren't you?  Well, go ahead then.</i>"  You pump your [chest] faster against the twitching rod and smile when a thick bead of pre sloughs off into your squishy boobs, smearing across your ${player.skin()}.  You kiss it, licking the dollop that slips out of the dilating cum-slit before commanding, "<i>Cum for me, ${monster.mf(
                "boy",
                "girl"
            )}.  Let it allll out.</i>"`
        );
        ...
        this.outx(
            `\n\nYou seize ${monster.a}${monster.short} by ${monster.pronoun3} base and jerk ${
                monster.pronoun2
            } off with quick, sharp little strokes, commanding, "<i>All of it!  Give me all of your lusts and cruel desires!</i>".  ${monster.mf(
                "His",
                "Her"
            )} back arches as ${
                monster.pronoun3
            } orgasm redoubles, and fresh ropes begin to spout out again, ensuring your face and breasts are soaked with the sloppy spooge.  It runs in moist, warm rivulets into your concealing top, and what doesn't drip down, you compulsively rub into your skin, feeling a positively healthy glow from the feeling.  You don't free the ${monster.cockDescriptShort(
                0
            )} from your chesty embrace until every single drop is splattered all over you, and when you do, you leave a thoroughly wiped-out ${
                monster.short
            } behind you.`
        );
        ...
        if (this.game.flags[kFLAGS.BIKINI_ARMOR_BONUS] > 8)
            this.game.flags[kFLAGS.BIKINI_ARMOR_BONUS] = 8;
        player.orgasm();
    ...
    /*
            public  equipEffect(wearer:Player, output: boolean): void
            {
